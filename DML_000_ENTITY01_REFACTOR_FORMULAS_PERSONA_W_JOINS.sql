...
SELECT * FROM PER_PERSONAS_FORMULAS_TEST1;

DROP TABLE PER_PERSONAS_FORMULAS_TEST1;

create table  PER_PERSONAS_FORMULAS_TEST1 AS 
WITH MOVIMIENTOS AS (
  SELECT * FROM (
    SELECT CNT_ID, MOV_FECHA_POS_VENCIDA, ROW_NUMBER() OVER (PARTITION BY CNT_ID ORDER BY MOV_FECHA_EXTRACCION DESC) N
    FROM MOV_MOVIMIENTOS
  ) WHERE N = 1
)
, PER_MOV AS (
  SELECT CPE.PER_ID, MIN(MOV.MOV_FECHA_POS_VENCIDA) MIN_MOV_FECHA_POS_VENCIDA
  FROM CPE_CONTRATOS_PERSONAS CPE
    JOIN DD_TIN_TIPO_INTERVENCION TIN ON CPE.DD_TIN_ID  =TIN.DD_TIN_ID AND TIN.DD_TIN_TITULAR = 1
    JOIN MOVIMIENTOS MOV ON CPE.CNT_ID = MOV.CNT_ID
  GROUP BY CPE.PER_ID    
)
select 
    PER_MAIN.PER_ID
    , est.dd_ecv_descripcion  AS EST_CICLO_VIDA
    , FLOOR(SYSDATE-PER_MOV.MIN_MOV_FECHA_POS_VENCIDA) AS DIAS_VENC_RIESGO_DIRECTO
...
/*          
    ...
    */
    from PER_PERSONAS PER_MAIN
      LEFT JOIN DD_ECV_ESTADO_CICLO_VIDA EST ON trim(PER_MAIN.per_ecv) = trim(est.dd_ecv_codigo)
      LEFT JOIN PER_MOV ON PER_MAIN.PER_ID = PER_MOV.PER_ID
    WHERE PER_MAIN.BORRADO = 0;
