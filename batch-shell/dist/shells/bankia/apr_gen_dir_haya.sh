	...
	if [ -f $MAINSH ]; then
	    ...
	    java -Xms512M -Xmx1536M -Dconfig.dir=$DIR_CONFIG -Dconfig.file.mask=$CFG_FILE  -Duser.country=ES -Duser.language=es -cp $CLASS2 $CLASEINICIO --context=Default "$@"
           ...
           if [ $? = 0 ] ; then
                 ...
                 > $DIR_LOCAL/apr_env_dir_haya.txt

                 rm -f $DIR_LOCAL/backup/DIRECCIONES*
                 ...
                 ftp -vn $SERVER <<END_OF_SESSION
                 	...
                 	lcd $DIR_LOCAL
                         ...
                         put DIRECCIONES*.zip
                         put DIRECCIONES*.sem
                         put apr_env_dir_haya.txt
                 
                 	bye
END_OF_SESSION

                 mv -f $DIR_LOCAL/DIRECCIONES*  $DIR_LOCAL/backup/
                 rm -f $DIR_LOCAL/apr_env_dir_haya.txt
