...
DIR_BASE_ETL=/etl/HRE/programas/etl
	...
	if [ -f $MAINSH ]; then
	    ...
	    /usr/java/jdk1.6.0_27/bin/java -Xms512M -Xmx1536M -Dconfig.dir=$DIR_CONFIG -Dconfig.file.mask=$CFG_FILE  -Duser.country=ES -Duser.language=es -cp $CLASS2 $CLASEINICIO --context=Default "$@"
	    exit $?
	else
