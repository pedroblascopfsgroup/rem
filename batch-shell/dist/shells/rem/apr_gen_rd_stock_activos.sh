...
echo "La ruta de los directorios es $1"
...
if [ -f $MAINSH ]; then
    ...
    java -Xms512M -Xmx1536M -Dconfig.dir=$DIR_CONFIG -Dconfig.file.mask=$CFG_FILE -Duser.country=ES -Duser.language=es -Dconfig.rutaSalida=$1 -cp $CLASS2 $CLASEINICIO --context=Default "$@" 
    exit $?
else
