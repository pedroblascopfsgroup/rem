...
if [ -f $MAINSH ]; then
    ...
    java -Xms512M -Xmx1536M -Dfile.encoding="UTF-8" -Dsun.jnu.encoding="UTF-8" -Dconfig.dir=$DIR_CONFIG -Dconfig.file.mask=$CFG_FILE -Duser.country=ES -Duser.language=es -cp $CLASS2 $CLASEINICIO --context=Default "$@"
    exit $?
else
