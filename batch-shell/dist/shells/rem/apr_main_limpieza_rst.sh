...
if [ -f $MAINSH ]; then
    ...
    java -Xms512M -Xmx1536M -Dconfig.dir=$DIR_CONFIG -Dconfig.file.mask=$CFG_FILE -Duser.country=ES -Duser.language=es -Dconfig.param_hist=$1 -Dconfig.param_backup=$2 -Dconfig.path_backup=$3 -cp $CLASS2 $CLASEINICIO --context=Default "$@"
    exit $?
else
