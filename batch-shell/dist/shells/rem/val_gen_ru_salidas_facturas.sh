...
if test ! -f ${OUTPUT_PATH}/RUFACTUSP_??????????????.txt ; then
   echo "$(basename $0) Error ${PREVALIDADO_PATH}/RUFACTUSP_??????????????.txt inexistente"
   exit 1
fi

if test ! -f ${OUTPUT_PATH}/RUFACTUCP_??????????????.txt ; then
   echo "$(basename $0) Error ${PREVALIDADO_PATH}/RUFACTUCP_??????????????.txt inexistente"
   exit 1
fi

if test ! -f ${OUTPUT_PATH}/RUFACTUSP_??????????????.sem ; then
   echo "$(basename $0) Error ${PREVALIDADO_PATH}/RUFACTUSP_??????????????.sem inexistente"
   exit 1
fi

if test ! -f ${OUTPUT_PATH}/RUFACTUCP_??????????????.sem ; then
   echo "$(basename $0) Error ${PREVALIDADO_PATH}/RUFACTUCP_??????????????.sem inexistente"
   exit 1
fi
