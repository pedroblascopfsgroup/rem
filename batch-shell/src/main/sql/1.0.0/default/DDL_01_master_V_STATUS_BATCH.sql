...
select execu.job_instance_id as ID,
    ...
    , '+' ||lpad(extract(hour from (NVL(execu.END_TIME,systimestamp) - execu.START_TIME)),2,0)||':'||lpad(extract(minute from (NVL(execu.END_TIME,SYSDATE) - execu.START_TIME)),2,0)||':00' TIEMPO
