    ...
    */
    public void afterPropertiesSet() throws Exception {
        ...
        try {
                ...
                if (processDefinitions != null || processDefinitionsResources != null) {
                    if (processDefinitions != null) {
                        ...
                        for (int i = 0; i < processDefinitions.length; i++) {
                            ProcessDefinition pd = context.getGraphSession().findLatestProcessDefinition(processDefinitions[i].getName());
                            if (pd == null || pd.getVersion() < processDefinitions[i].getVersion() || processDefinitions[i].getVersion() == -1) {
                                processDefinitions[i].setVersion(-1);
            ...
            //Verificamos si se desea iniciar el JobExecutor
            if (startJobExecutor) {
