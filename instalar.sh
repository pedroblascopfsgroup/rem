...
PLUGIN=recovery-busquedaProcedimientos-plugin

ENTORNO=$1

if [ x$1 == x ]; then 
	echo "Por favor, indica el entorno en el que quieres instalar, GAAN"
	exit 0
fi
...
if [ "x$jars" == "x" ] || [ $c -eq 0 ]; then
	echo "No se ha encontrado el plugin, ¿Has empaquetado? GAAN"
	exit 0
fi

if [ $c -gt 1 ]; then
	echo "Hay demasiados SNAPSHOTS. Haz una limpieza GAAN"
	exit 0
fi
