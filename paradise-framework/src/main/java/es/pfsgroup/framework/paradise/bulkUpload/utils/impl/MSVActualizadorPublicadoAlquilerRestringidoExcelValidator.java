...
@Component
public class MSVActualizadorPublicadoAlquilerRestringidoExcelValidator extends MSVExcelValidatorAbstract {
	...
	private static final String DESTINO_COMERCIAL_NO_ALQUILER = "Existen activos que no incluyen destino comercial de alquiler en la agrupación";
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws Exception {
		...
		if (!dtoValidacionContenido.getFicheroTieneErrores()) {
			...
			if (!mapaErrores.get(ACTIVO_NOT_EXISTS).isEmpty() || !mapaErrores.get(ACTIVO_SIN_INFORME_NI_PRECIO).isEmpty()
					...
					|| !mapaErrores.get(ACTIVO_NO_AGRUPACION_RESTRINGIDA).isEmpty() || !mapaErrores.get(ACTIVO_NO_PRINCIPAL_AGRUPACION_RESTRINGIDA).isEmpty()){
	...
	private List<Integer> activosSinInformeAprobadoNiPrecioOSinPublicarSinPrecioRows(MSVHojaExcel exc){
		...
		try{
			for(i=1; i<this.numFilasHoja;i++){
				String idAgrupacion = particularValidator.idAgrupacionDelActivoPrincipal(exc.dameCelda(i, 0));
				if(particularValidator.isActivoSinInformeAprobadoAgrupacion(idAgrupacion) && particularValidator.isActivoSinPrecioAlquilerWebAgrupacion(idAgrupacion)) {
					listaFilas.add(i);
				}
			}
		} catch (Exception e) {
