...
public class MSVActualizadorPublicadoVentaExcelValidator extends MSVExcelValidatorAbstract {
	...
	private static final String ACTIVO_SIN_PRECIO_VENTA = "Este activo no tiene asignado un precio de venta web";
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws Exception {
		...
		if (!dtoValidacionContenido.getFicheroTieneErrores()) {
			...
			mapaErrores.put(ACTIVO_SIN_PRECIO_VENTA, activosSinPrecioVentaOSinPublicarSinPrecioRows(exc));
			...
			if (!mapaErrores.get(CAMPO_OCULTAR_PRECIO_FORMATO_NO_VALIDO).isEmpty() || !mapaErrores.get(CAMPO_PUBLICAR_SIN_PRECIO_FORMATO_NO_VALIDO).isEmpty()
					...
					|| !mapaErrores.get(ACTIVO_SIN_PRECIO_VENTA).isEmpty()){
	...
	private List<Integer> activosSinPrecioVentaOSinPublicarSinPrecioRows(MSVHojaExcel exc){
		List<Integer> listaFilas = new ArrayList<Integer>();

		int i = 0;
		try{
			for(i=1; i<this.numFilasHoja;i++){
				if(!this.obtenerBooleanExcel(exc.dameCelda(i, 2)) && particularValidator.isActivoSinPrecioVentaWeb(exc.dameCelda(i, 0)))
					listaFilas.add(i);
			}
		} catch (Exception e) {
			if (i != 0) listaFilas.add(i);
