...
public class MSVActualizarPropuestaPreciosActivo extends MSVExcelValidatorAbstract {
	...
	// public static final String ACTIVE_NOT_ACTUALIZABLE = "El estado del
	// activo no puede actualizarse al indicado.";
	// public static final String ACTIVE_PRECIOS_BLOQUEO = "El activo tiene
	// habilitado el bloqueo de precios. No se pueden actualizar precios";
	// public static final String ACTIVE_OFERTA_APROBADA = "El activo tiene
	// ofertas aprobadas. No se pueden actualizar precios";
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws RowsExceededException, IllegalArgumentException, WriteException, IOException {
		...
		if (!dtoValidacionContenido.getFicheroTieneErrores()) {
			...
			// Si la propuesta ya ha sido cargada, no se realizan el resto de
			// comprobaciones
			if (mapaErrores.get(PROPUESTA_YA_CARGADA).isEmpty()) {
				...
				mapaErrores.put(ACTIVE_PRIZES_DESCUENTOS_LIMIT_EXCEEDED,
						getLimitePreciosDescAprobDescWebIncorrectoRows(exc));
				mapaErrores.put(messageServices.getMessage(ACTIVE_PRIZES_VENTA_MINIMO_LIMIT_EXCEEDED),
						getLimitePreciosAprobadoMinimoIncorrectoRows(exc));
				// mapaErrores.put(ACTIVE_PRIZES_VENTA_DESCUENTOWEB_LIMIT_EXCEEDED,
				// getLimitePreciosAprobadoDescWebIncorrectoRows(exc));
				...
				mapaErrores.put(messageServices.getMessage(ACTIVE_PMA_DATE_INIT_EXCEEDED),
						getFechaInicioMinimoAuthIncorrectaRows(exc));
	...
	private List<Integer> getNANPrecioIncorrectoRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					precioVNC = !Checks.esNulo(exc.dameCelda(i, COL_VAL_VNC))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_VNC)) : null;
					precioVentaAprobado = !Checks.esNulo(exc.dameCelda(i, COL_VAL_PAV))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_PAV)) : null;
					precioRentaAprobado = !Checks.esNulo(exc.dameCelda(i, COL_VAL_PAR))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_PAR)) : null;
					precioMinimoAuth = !Checks.esNulo(exc.dameCelda(i, COL_VAL_MIN))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_MIN)) : null;
					precioDescuentoAprobado = !Checks.esNulo(exc.dameCelda(i, COL_VAL_PDA))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_PDA)) : null;
					precioDescuentoPublicado = !Checks.esNulo(exc.dameCelda(i, COL_VAL_PDP))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_PDP)) : null;
					valorEstimadoVenta = !Checks.esNulo(exc.dameCelda(i, COL_VAL_VEV))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_VEV)) : null;
					valorReferencia = !Checks.esNulo(exc.dameCelda(i, COL_VAL_VRF))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_VRF)) : null;
					valorTransferencia = !Checks.esNulo(exc.dameCelda(i, COL_VAL_VTF))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_VTF)) : null;
					costeAdquisicion = !Checks.esNulo(exc.dameCelda(i, COL_VAL_CAD))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_CAD)) : null;
					valorFSVVenta = !Checks.esNulo(exc.dameCelda(i, COL_VAL_FSV))
							? Double.parseDouble(exc.dameCelda(i, COL_VAL_FSV)) : null;
	...
	private List<Integer> getLimitePreciosDescAprobDescWebIncorrectoRows(MSVHojaExcel exc) {
		...
		// Validacion que evalua si los precios estan dentro de los límites,
		// comparandolos entre si
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					precioDescuentoAprobado = !Checks.esNulo(exc.dameCelda(i, COL_VAL_PDA))
					...
					if (!Checks.esNulo(precioDescuentoAprobado) && !Checks.esNulo(precioDescuentoPublicado)
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					...
					if (!Checks.esNulo(precioMinimoAuth) && !Checks.esNulo(precioVentaAprobado)
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					...
					// Condiciones Limites: dto<=dto web<=aprobado y
					// aprobado>=minimo

					// Limite: Precio Aprobado Venta >= Precio Descuento Web
					if (!Checks.esNulo(precioVentaAprobado) && !Checks.esNulo(precioDescuentoPublicado)
	...
	private List<Integer> getFechaInicioAprobadoVentaIncorrectaRows(MSVHojaExcel exc) {
		...
		// Validacion que evalua si las fechas de precios estan dentro de los
		// límites
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaInicioPAV = !Checks.esNulo(exc.dameCelda(i, COL_FINI_PAV))
							? ft.parse(exc.dameCelda(i, COL_FINI_PAV)) : null;
					fechaFinPAV = !Checks.esNulo(exc.dameCelda(i, COL_FFIN_PAV))
							? ft.parse(exc.dameCelda(i, COL_FFIN_PAV)) : null;

					// Fecha Inicio <= Fecha Fin
					if (!Checks.esNulo(fechaInicioPAV) && !Checks.esNulo(fechaFinPAV)
	...
	private List<Integer> getFechaInicioAprobadoRentaIncorrectaRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaInicioPAR = !Checks.esNulo(exc.dameCelda(i, COL_FINI_PAR))
							? ft.parse(exc.dameCelda(i, COL_FINI_PAR)) : null;
					fechaFinPAR = !Checks.esNulo(exc.dameCelda(i, COL_FFIN_PAR))
							? ft.parse(exc.dameCelda(i, COL_FFIN_PAR)) : null;

					// Fecha Inicio <= Fecha Fin
					if (!Checks.esNulo(fechaInicioPAR) && !Checks.esNulo(fechaFinPAR)
	...
	private List<Integer> getFechaInicioMinimoAuthIncorrectaRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaInicioPMA = !Checks.esNulo(exc.dameCelda(i, COL_FINI_MIN))
							? ft.parse(exc.dameCelda(i, COL_FINI_MIN)) : null;
					fechaFinPMA = !Checks.esNulo(exc.dameCelda(i, COL_FFIN_MIN))
							? ft.parse(exc.dameCelda(i, COL_FFIN_MIN)) : null;

					// Fecha Inicio <= Fecha Fin
					if (!Checks.esNulo(fechaInicioPMA) && !Checks.esNulo(fechaFinPMA)
	...
	private List<Integer> getFechaInicioDescuentoAprobIncorrectaRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaInicioPDA = !Checks.esNulo(exc.dameCelda(i, COL_FINI_PDA))
							? ft.parse(exc.dameCelda(i, COL_FINI_PDA)) : null;
					fechaFinPDA = !Checks.esNulo(exc.dameCelda(i, COL_FFIN_PDA))
							? ft.parse(exc.dameCelda(i, COL_FFIN_PDA)) : null;

					// Fecha Inicio <= Fecha Fin
					if (!Checks.esNulo(fechaInicioPDA) && !Checks.esNulo(fechaFinPDA)
	...
	private List<Integer> getFechaInicioDescuentoPubIncorrectaRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaInicioPDP = !Checks.esNulo(exc.dameCelda(i, COL_FINI_PDP))
							? ft.parse(exc.dameCelda(i, COL_FINI_PDP)) : null;
					fechaFinPDP = !Checks.esNulo(exc.dameCelda(i, COL_FFIN_PDP))
							? ft.parse(exc.dameCelda(i, COL_FFIN_PDP)) : null;

					// Fecha Inicio <= Fecha Fin
					if (!Checks.esNulo(fechaInicioPDP) && !Checks.esNulo(fechaFinPDP)
	...
	private List<Integer> getFechaVEVIncorrectaRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaVEV = !Checks.esNulo(exc.dameCelda(i, COL_FECHA_VEV))
							? ft.parse(exc.dameCelda(i, COL_FECHA_VEV)) : null;
				} catch (ParseException e) {
					listaFilas.add(i);
					logger.error(e.getMessage());
				}
	...
	private List<Integer> getFechaVRFIncorrectaRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaVRF = !Checks.esNulo(exc.dameCelda(i, COL_FECHA_VRF))
							? ft.parse(exc.dameCelda(i, COL_FECHA_VRF)) : null;
				} catch (ParseException e) {
					listaFilas.add(i);
					logger.error(e.getMessage());
				}
	...
	private List<Integer> getFechaFSVIncorrectaRows(MSVHojaExcel exc) {
		...
		try {
			for (int i = EXCEL_FILA_INICIAL; i < this.numFilasHoja; i++) {
				try {
					fechaFSV = !Checks.esNulo(exc.dameCelda(i, COL_FECHA_FSV))
							? ft.parse(exc.dameCelda(i, COL_FECHA_FSV)) : null;
				} catch (ParseException e) {
					listaFilas.add(i);
					logger.error(e.getMessage());
				}
