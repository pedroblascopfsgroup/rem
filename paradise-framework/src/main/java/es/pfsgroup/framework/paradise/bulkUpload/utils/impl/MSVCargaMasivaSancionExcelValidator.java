...
@Component
public class MSVCargaMasivaSancionExcelValidator extends MSVExcelValidatorAbstract {
	
	private static final String ACTIVO_NO_EXISTE = "msg.error.masivo.agrupar.activos.asistida.activo.noExiste";
	private static final String SANCION_INCORRECTA = "msg.error.masivo.sancion.ejerce";
	private static final String ACTIVO_SIN_COMUNICACION_ACTIVA = "msg.error.masivo.activo.sin.comunicacion.viva";
	private static final String NIF_ERRONEO = "msg.error.masivo.nif.erroneo";
	...
	private static final String SANCION_VALOR_VALIDO = "msg.error.masivo.sancion.valor.diccionario";
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws Exception {
		...
		if (!dtoValidacionContenido.getFicheroTieneErrores()) {
			...
			mapaErrores.put(messageServices.getMessage(SANCION_VALOR_VALIDO), validarValorSancion(exc));
			mapaErrores.put(messageServices.getMessage(SANCION_INCORRECTA), validarResultadoSancion(exc));
			mapaErrores.put(messageServices.getMessage(ACTIVO_SIN_COMUNICACION_ACTIVA), validarActivoComunicacionViva(exc));
			mapaErrores.put(messageServices.getMessage(NIF_ERRONEO), validarNIF(exc));
			...
			if (!mapaErrores.get(messageServices.getMessage(ACTIVO_NO_EXISTE)).isEmpty()
					|| !mapaErrores.get(messageServices.getMessage(SANCION_VALOR_VALIDO)).isEmpty()
					|| !mapaErrores.get(messageServices.getMessage(SANCION_INCORRECTA)).isEmpty()
					|| !mapaErrores.get(messageServices.getMessage(ACTIVO_SIN_COMUNICACION_ACTIVA)).isEmpty()
					|| !mapaErrores.get(messageServices.getMessage(NIF_ERRONEO)).isEmpty()
	...
	// Validaciones para activos de la lista excel	
		...
		private List<Integer> validarResultadoSancion(MSVHojaExcel exc) {
			List<Integer> listaFilas = new ArrayList<Integer>();
			...
			int i = 0;
				for(i=1; i<this.numFilasHoja;i++){
					try {
						...
						if (COD_EJERCE.equals(exc.dameCelda(i, POSICION_COLUMNA_RESULTADO_SANCION)) 
								&&  (
									exc.dameCelda(i, POSICION_COLUMNA_NIF).trim().isEmpty()
									|| exc.dameCelda(i, POSICION_COLUMNA_NOMBRE).trim().isEmpty()
									)
								) {
							listaFilas.add(i);
						} else if (COD_NO_EJERCE.equals(exc.dameCelda(i, POSICION_COLUMNA_RESULTADO_SANCION)) 
								&&  (
									!exc.dameCelda(i, POSICION_COLUMNA_NIF).trim().isEmpty()
									|| !exc.dameCelda(i, POSICION_COLUMNA_NOMBRE).trim().isEmpty()
									)
		...
		private List<Integer> validarValorSancion(MSVHojaExcel exc) {
			List<Integer> listaFilas = new ArrayList<Integer>();
			...
			int i = 0;
				for(i=1; i<this.numFilasHoja;i++){
					try {
						if (!COD_EJERCE.equals(exc.dameCelda(i, POSICION_COLUMNA_RESULTADO_SANCION)) 
								&& !COD_NO_EJERCE.equals(exc.dameCelda(i, POSICION_COLUMNA_RESULTADO_SANCION))
								) {
							listaFilas.add(i);
						}
					} catch (Exception e) {
		...
		private List<Integer> validarActivoComunicacionViva(MSVHojaExcel exc) {
			...
			try{
				for(i=1; i<this.numFilasHoja;i++){
					if(particularValidator.existeActivo(exc.dameCelda(i, POSICION_COLUMNA_NUMERO_ACTIVO)) 
							&& !particularValidator.esActivoConComunicacionViva(Long.valueOf(exc.dameCelda(i, POSICION_COLUMNA_NUMERO_ACTIVO)))) {
						listaFilas.add(i);
					}
		...
		private List<Integer> esActivoConComunicacionComunicada(MSVHojaExcel exc) {
			...
			try{
				for(i=1; i<this.numFilasHoja;i++){
					if(particularValidator.existeActivo(exc.dameCelda(i, POSICION_COLUMNA_NUMERO_ACTIVO))
							&& !particularValidator.esActivoConComunicacionComunicada(Long.valueOf(exc.dameCelda(i, POSICION_COLUMNA_NUMERO_ACTIVO)))) {
						listaFilas.add(i);
					}
		...
		private List<Integer> validarNIF(MSVHojaExcel exc) {
			List<Integer> listaFilas = new ArrayList<Integer>();
			...
			try{
				for(i=1; i<this.numFilasHoja;i++){
					if(COD_EJERCE.equals(exc.dameCelda(i, POSICION_COLUMNA_RESULTADO_SANCION)) 
							&& !particularValidator.esNIFValido(exc.dameCelda(i, POSICION_COLUMNA_NIF))) {
						listaFilas.add(i);
					}
