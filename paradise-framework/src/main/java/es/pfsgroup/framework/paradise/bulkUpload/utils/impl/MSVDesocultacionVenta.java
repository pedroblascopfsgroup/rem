...
@Component
public class MSVDesocultacionVenta extends MSVExcelValidatorAbstract{
	...
	private static final String ACTIVO_PUBLICADO = "El activo está publicado en venta";
	private static final String ACTIVO_NO_PUBLICADO = "El activo no está publicado en venta";
	private static final String ACTIVO_NO_OCULTO = "El activo no está oculto venta";
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws Exception {
		...
		if (!dtoValidacionContenido.getFicheroTieneErrores()) {
			...
			mapaErrores.put(ACTIVO_NO_PUBLICADO, activoNoPublicadoVentaByRows(exc));
			...
			mapaErrores.put(ACTIVO_PUBLICADO, activoPublicadoVentaByRows(exc));
			mapaErrores.put(ACTIVO_NO_OCULTO, activoNoOcultoVento(exc));
	...
	private List<Integer> activoNoPublicadoVentaByRows(MSVHojaExcel exc){
		List<Integer> listaFilas = new ArrayList<Integer>();

		int i = 0;
		try{
			for(i=1; i<this.numFilasHoja;i++){
				if(particularValidator.isActivoNoPublicadoVenta(exc.dameCelda(i, COL_NUM.ID_ACTIVO_HAYA))) listaFilas.add(i);
			}
		}catch (Exception e){
			if(i!=0) listaFilas.add(i);
			logger.error(e.getMessage());
	...
	private List<Integer> destinoFinalNoVentaByRows(MSVHojaExcel exc){
		...
		try{
			for(i=1; i<this.numFilasHoja;i++){
				if(particularValidator.destinoFinalNoVenta(exc.dameCelda(i, COL_NUM.ID_ACTIVO_HAYA))) listaFilas.add(i);
			}
		}catch (Exception e){
			if (i!=0) listaFilas.add(i);
			logger.error(e.getMessage());
	...
	private List<Integer> activoPublicadoVentaByRows(MSVHojaExcel exc){
		List<Integer> listaFilas = new ArrayList<Integer>();
		
		int i = 0;
		try{
			for(i=1; i<this.numFilasHoja;i++){
				if(particularValidator.isActivoPublicadoVenta(exc.dameCelda(i, COL_NUM.ID_ACTIVO_HAYA))) listaFilas.add(i);
			}
		}catch (Exception e){
			if (i!=0) listaFilas.add(i);
			logger.error(e.getMessage());
	...
	private List<Integer> activoNoOcultoVento(MSVHojaExcel exc){
		List<Integer> listaFilas = new ArrayList<Integer>();
		
		int i = 0;
		try{
			for(i=1; i<this.numFilasHoja;i++){
				if(!particularValidator.activoOcultoVenta(exc.dameCelda(i, COL_NUM.ID_ACTIVO_HAYA))) listaFilas.add(i);
			}
		}catch (Exception e){
			if (i!=0) listaFilas.add(i);
			logger.error(e.getMessage());
	...
	private List<Integer> activoOcultoPorMotivosAutomaticosByRows(MSVHojaExcel exc){
		...
		try{
			for(i=1; i<this.numFilasHoja;i++){
				if(!particularValidator.isActivoOcultoVentaPorMotivosManuales(exc.dameCelda(i, COL_NUM.ID_ACTIVO_HAYA))) listaFilas.add(i);
			}
		}catch (Exception e){
			if (i!=0) listaFilas.add(i);
			logger.error(e.getMessage());
