...
public class MSVGestionPeticionesDePreciosExcelValidator extends MSVExcelValidatorAbstract {
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws Exception {
		...
		if (Boolean.FALSE.equals(dtoValidacionContenido.getFicheroTieneErrores())) {
			...
			mapaErrores.put(CODIGO_PETICION_NO_EXISTE, isCodigoPeticionExistente(exc));
	...
	private List<Integer> isCodigoPeticionExistente(MSVHojaExcel exc) {
		List<Integer> listaFilas = new ArrayList<Integer>();
		
		int i = 0;
		try {
			for(i=1; i<this.numFilasHoja;i++){
				if(Boolean.FALSE.equals(particularValidator.existeCodigoPeticion(exc.dameCelda(i, COL_NUM.COD_PETICION))))
					listaFilas.add(i);
			}
		} catch (Exception e) {
