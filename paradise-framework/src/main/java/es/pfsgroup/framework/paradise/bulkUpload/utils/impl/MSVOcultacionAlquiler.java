...
public class MSVOcultacionAlquiler extends MSVExcelValidatorAbstract{
	...
	private static final String AGRUPACION_PROMOCION_CONJUNTA_ALQUILER= "19";
		...
		try{
			for(i=1; i<this.numFilasHoja;i++){
				if(!particularValidator.esActivoEnAgrupacionPorTipo(Long.parseLong(exc.dameCelda(i, COL_NUM.NUM_ACTIVO_HAYA)), AGRUPACION_RESTRINGIDA)
						...
						|| !particularValidator.esActivoEnAgrupacionPorTipo(Long.parseLong(exc.dameCelda(i, COL_NUM.NUM_ACTIVO_HAYA)), AGRUPACION_PROMOCION_CONJUNTA_ALQUILER)){
