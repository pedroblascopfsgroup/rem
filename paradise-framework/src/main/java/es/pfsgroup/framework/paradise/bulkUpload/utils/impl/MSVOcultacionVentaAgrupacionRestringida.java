...
@Component
public class MSVOcultacionVentaAgrupacionRestringida extends MSVExcelValidatorAbstract{

	private static final String ACTIVE_NOT_EXISTS = "El activo no existe";
	private static final String ACTIVO_VENDIDO = "Hay activos que está vendido";
	private static final String ACTIVO_NO_PUBLICABLE = "Hay activos no publicables";
	private static final String ACTIVO_NO_COMERCIALIZABLE = "Hay activos no comerciables";
	private static final String DESTINO_FINAL_NO_VENTA = "Hay activos cuyo destino comercial no es venta";
	private static final String ACTIVO_NO_PUBLICADO = "Hay activos no publicados";
	private static final String ACTIVO_OCULTO = "Hay activos que están ocultos";
	...
	private static final String ACTIVO_NO_PERTENECE_AGRUPACION_RESTRINGIDA = "El activo no pertenece a una agrupación restringida";
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws Exception {
		...
		if (!dtoValidacionContenido.getFicheroTieneErrores()) {
			...
			mapaErrores.put(ACTIVO_NO_PERTENECE_AGRUPACION_RESTRINGIDA, activoNoPerteneceAgrupacionRestringida(exc));
			...
			if (!mapaErrores.get(ACTIVE_NOT_EXISTS).isEmpty()
				...
				||!mapaErrores.get(ACTIVO_NO_PERTENECE_AGRUPACION_RESTRINGIDA).isEmpty()
	...
	private List<Integer> activosVendidosRows(MSVHojaExcel exc) {
		...
		try {
			for(i=1; i<this.numFilasHoja;i++){
				...
				if(particularValidator.esActivoVendidoAgrupacion(numAgrupacion))
					listaFilas.add(i);
			}
		} catch (Exception e) {
			if (i != 0) listaFilas.add(i);
	...
	private List<Integer> activoNoPublicadoByRows(MSVHojaExcel exc){
		...
		try{
			for(i=1; i<this.numFilasHoja;i++){
				String numAgrupacion = particularValidator.idAgrupacionDelActivoPrincipal(exc.dameCelda(i, COL_NUM.NUM_ACTIVO_HAYA));
				if(!particularValidator.isActivoPublicadoAgrupacion(numAgrupacion)) listaFilas.add(i);
			}
		}catch (Exception e){
			if(i!=0) listaFilas.add(i);
			logger.error(e.getMessage());
	...
	private List<Integer> activoNoPerteneceAgrupacionRestringida(MSVHojaExcel exc){
		List <Integer> listaFilas = new ArrayList<Integer>();
