...
@Component
public class MSVValidatorIndicadorActivoVenta extends MSVExcelValidatorAbstract {
	...
	public static final String ACTIVO_DESTINO_COMERCIAL_VENTA = "msg.error.masivo.activo.no.tiene.destino.comercial.venta";
	...
	public MSVDtoValidacion validarContenidoFichero(MSVExcelFileItemDto dtoFile) throws Exception {
		...
		if (!dtoValidacionContenido.getFicheroTieneErrores()) {
			...
			mapaErrores.put(messageServices.getMessage(ACTIVO_DESTINO_COMERCIAL_VENTA), isActivoDestinoComercialVenta(exc));
	...
	private List<Integer> isActivoDestinoComercialVenta(MSVHojaExcel exc){
		...
		try {
			for (i = 1 ; i < this.numFilasHoja ; i++){
				if(particularValidator.isActivoDestinoComercialNoVenta(exc.dameCelda(i, POSICION_COLUMNA_ID_ACTIVO_HAYA)))
					listaFilas.add(i);
			}
		} 
		catch (Exception e) {
