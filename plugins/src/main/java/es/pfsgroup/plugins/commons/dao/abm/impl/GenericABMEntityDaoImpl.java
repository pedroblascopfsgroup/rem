...
public class GenericABMEntityDaoImpl implements GenericABMDao {
	 ...
	 */
	private abstract static class OverriddedAbstractEntityDao<T extends Serializable> extends AbstractEntityDao<T, Long>{
	 ...
	 * ExtendedDao
	 */
	private static class ExtendedDao<T extends Serializable> extends OverriddedAbstractEntityDao<T> implements RWOperations<T>{
	...
	@Override
	public <T extends Serializable> void deleteById(Class<T> clazz,
			Long id) {
		ExtendedDao<T> dao = (ExtendedDao<T>) createExtendedDao(clazz);
		dao.deleteById(id);
	}
