...
var cb_suspension = items[3 + muestraBotonGuardar];


cb_resultado.on('select', function(){
	//Borramos todos los combos dependientes ante cualquier cambio
	cb_suspension.setValue('');
	
	if(cb_resultado.getValue() == 'SUS') {// Suspender subasta
		cb_suspension.allowBlank = false;
	}
	else { // Continuar o rechazar
		cb_suspension.allowBlank = true;
	}
});
