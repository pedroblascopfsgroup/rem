...
import org.cajamar.ws.S_C_RECOVERY_DAT_TJ.INPUT;
import org.cajamar.ws.S_C_RECOVERY_DAT_TJ.OUTPUT;
import org.cajamar.ws.S_C_RECOVERY_DAT_TJ.ObjectFactory;
import org.cajamar.ws.S_C_RECOVERY_DAT_TJ.SCRECOVERYDATTJ;
import org.cajamar.ws.S_C_RECOVERY_DAT_TJ.SCRECOVERYDATTJType;
...
@Component(value="wsCSCuentaTJ")
public class WSCSTarjetas extends BaseWS implements WSConsultaSaldos {

	private static final String WEB_SERVICE_NAME = "S_C_RECOVERY_DAT_TJ";
	...
	private ResultadoConsultaSaldo ejecutaServicio(INPUT input) throws MalformedURLException {
		...
		SCRECOVERYDATTJ service = new SCRECOVERYDATTJ(wsdlLocation, qName);
		SCRECOVERYDATTJType servicePort = service.getSCRECOVERYDATTJPort();
		OUTPUT output = servicePort.sCRECOVERYDATTJ(input);
		...
		logger.info(String.format("INTERESESRECIBOS: %s", output.getINTERESESRECIBOS()));
		resultado.setInteresesRecibos(output.getINTERESESRECIBOS());
