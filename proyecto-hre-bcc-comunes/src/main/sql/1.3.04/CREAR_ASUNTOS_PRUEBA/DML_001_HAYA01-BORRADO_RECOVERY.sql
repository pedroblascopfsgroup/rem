...
DECLARE
        ...
        V_ESQUEMA_BM VARCHAR2(25 CHAR):= 'HAYAMASTER';
        USUARIO      VARCHAR2(50 CHAR):= 'MIGRABNKF2';
...
--##########################################
     ...
     EXECUTE IMMEDIATE 'CREATE INDEX idx_borrado_asu ON '||V_ESQUEMA||'.TABLA_TMP_ASU (ASU_ID) nologging';
     DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TABLA_TMP_ASU... Indice creado');
...
--    **************************/
     ...
     EXECUTE IMMEDIATE 'CREATE INDEX idx_borrado_prc ON '||V_ESQUEMA||'.TABLA_TMP_PRC (PRC_ID) nologging';
     EXECUTE IMMEDIATE 'CREATE INDEX idx_borrado_prc_2 ON '||V_ESQUEMA||'.TABLA_TMP_PRC (PRC_PROCESS_BPM) nologging';
...
--    **************************/
     ...
     EXECUTE IMMEDIATE 'CREATE INDEX idx_borrado_exp ON '||V_ESQUEMA||'.TABLA_TMP_EXP (EXP_ID) nologging';
     DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TABLA_TMP_EXP... Indice creado');
...
--    **************************/
     ...
     EXECUTE IMMEDIATE 'CREATE INDEX idx_borrado_bie ON '||V_ESQUEMA||'.TABLA_TMP_BIE (BIE_ID) nologging';
     DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TABLA_TMP_BIE... Indice creado');
...
--    **************************/
     ...
     FROM  '||V_ESQUEMA||'.TABLA_TMP_PRC PRC
       LEFT JOIN '||V_ESQUEMA_BM||'.JBPM_PROCESSINSTANCE INS ON PRC.PRC_PROCESS_BPM = INS.ID_
       LEFT JOIN '||V_ESQUEMA_BM||'.JBPM_TOKEN TOK ON INS.ID_ = TOK.PROCESSINSTANCE_
       LEFT JOIN '||V_ESQUEMA_BM||'.JBPM_MODULEINSTANCE MODU ON INS.ID_ = MODU.PROCESSINSTANCE_
       LEFT JOIN '||V_ESQUEMA_BM||'.JBPM_TOKENVARIABLEMAP TKVM ON TOK.ID_ = TKVM.TOKEN_
       LEFT JOIN '||V_ESQUEMA_BM||'.JBPM_VARIABLEINSTANCE VINS ON INS.ID_ = VINS.PROCESSINSTANCE_
	...
	EXECUTE IMMEDIATE('ANALYZE TABLE '||V_ESQUEMA||'.TABLA_BPM COMPUTE STATISTICS');
      ...
      IF EXISTE > 0 THEN
             ...
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_JOB J where NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.TABLA_TMP_PRC where PRC_PROCESS_BPM = J.PROCESSINSTANCE_)';
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_JOB where PROCESSINSTANCE_ IN (SELECT DISTINCT PROCESSINSTANCES FROM '||V_ESQUEMA||'.TABLA_BPM)';
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_JOB where TOKEN_ IN (SELECT DISTINCT TOKENS FROM '||V_ESQUEMA||'.TABLA_BPM)';
             
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_VARIABLEINSTANCE where PROCESSINSTANCE_ NOT IN (SELECT DISTINCT PRC_PROCESS_BPM FROM '||V_ESQUEMA||'.TABLA_TMP_PRC)';
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_VARIABLEINSTANCE  WHERE TOKEN_ NOT IN (
                  SELECT DISTINCT ROOTTOKEN_ FROM '||V_ESQUEMA_BM||'.JBPM_PROCESSINSTANCE I JOIN TABLA_TMP_PRC P ON I.ID_ = P.PRC_PROCESS_BPM
                )';
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_VARIABLEINSTANCE where ID_ IN (SELECT DISTINCT VARIABLEINSTANCES FROM '||V_ESQUEMA||'.TABLA_BPM)';
             
             EXECUTE IMMEDIATE 'DELETE '||V_ESQUEMA_BM||' .JBPM_TOKENVARIABLEMAP WHERE TOKEN_ NOT IN (
                  SELECT DISTINCT ROOTTOKEN_ FROM '||V_ESQUEMA_BM||'.JBPM_PROCESSINSTANCE I JOIN TABLA_TMP_PRC P ON I.ID_ = P.PRC_PROCESS_BPM
                )';
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_TOKENVARIABLEMAP where ID_ IN (SELECT DISTINCT TOKENVARIABLEMAPS FROM '||V_ESQUEMA||'.TABLA_BPM)';
             
             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_MODULEINSTANCE M where NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.TABLA_TMP_PRC where PRC_PROCESS_BPM = M.PROCESSINSTANCE_)';

             EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_MODULEINSTANCE where ID_ IN (SELECT DISTINCT MODULEINSTANCES FROM '||V_ESQUEMA||'.TABLA_BPM)';
             
             EXECUTE IMMEDIATE 'update '||V_ESQUEMA_BM||'.JBPM_PROCESSINSTANCE I set roottoken_ = null where NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.TABLA_TMP_PRC where PRC_PROCESS_BPM = I.ID_)';

             EXECUTE IMMEDIATE 'update '||V_ESQUEMA_BM||'.JBPM_PROCESSINSTANCE set roottoken_ = null where roottoken_ in (SELECT TOKENS FROM '||V_ESQUEMA||'.TABLA_BPM)';
	    ...
	    IF (existe=0) THEN
	       EXECUTE IMMEDIATE('CREATE INDEX IDX_TAR_TAR_ID_ASU_ID ON '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES(TAR_ID, ASU_ID)');
	    END IF;

	     EXECUTE IMMEDIATE 'update TEX_TAREA_EXTERNA SET TEX_TOKEN_ID_BPM = NULL WHERE TEX_ID IN 
                       ( SELECT TEX_ID FROM TEX_TAREA_EXTERNA TEX
                         INNER JOIN TAR_TAREAS_NOTIFICACIONES TAR ON TEX.TAR_ID = TAR.TAR_ID
                         INNER JOIN TABLA_TMP_ASU ASU ON ASU.ASU_ID = TAR.ASU_ID)';
             
	    EXECUTE IMMEDIATE 'update TEX_TAREA_EXTERNA SET TEX_TOKEN_ID_BPM = NULL WHERE TEX_TOKEN_ID_BPM  in (SELECT DISTINCT TOKENS FROM '||V_ESQUEMA||'.TABLA_BPM)';
             
            EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_TOKEN T where NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.TABLA_TMP_PRC where PRC_PROCESS_BPM = T.PROCESSINSTANCE_)';
             
	    EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_TOKEN where ID_ IN (SELECT DISTINCT TOKENS FROM '||V_ESQUEMA||'.TABLA_BPM)';
             
            EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_PROCESSINSTANCE I where NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.TABLA_TMP_PRC where PRC_PROCESS_BPM = I.ID_)';
            
	    EXECUTE IMMEDIATE 'delete  '||V_ESQUEMA_BM||'.JBPM_PROCESSINSTANCE where ID_ IN (SELECT DISTINCT PROCESSINSTANCES FROM '||V_ESQUEMA||'.TABLA_BPM)';
...
--    **************************/
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.HAC_HISTORICO_ACCESOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.GAH_GESTOR_ADICIONAL_HISTORICO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.GAA_GESTOR_ADICIONAL_ASUNTO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
...
--    ********************/
      ...
      -- Recursos
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.RCR_RECURSOS_PROCEDIMIENTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.LOB_LOTE_BIEN... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.LOB_LOTE_BIEN(2)... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
...
--    ********************/
       ...
       DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.LOS_LOTE_SUBASTA... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
...
--    ********************/
      --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.EMP_NMBEMBARGOS_PROCEDIMIENTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.SUB_SUBASTA... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRC_PER... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      -- CRE_PRC_CEX
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.CRE_PRC_CEX... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      --PRC_CEX
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRC_CEX... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRC_CEX(2)... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TEV_TAREA_EXTERNA_VALOR... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      IF (EXISTE>0) THEN
	   ...
	   EXECUTE IMMEDIATE('ANALYZE TABLE '||V_ESQUEMA||'.TEX_TAREA_EXTERNA COMPUTE STATISTICS FOR ALL INDEXES');
           ...
           DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TEV_TAREA_EXTERNA_VALOR .. Se han eliminado '||
EXISTE||' registros');
           COMMIT;
           PRO_KEYS_STATUS(V_ESQUEMA, 'TEV_TAREA_EXTERNA_VALOR', 'ENABLE');
      END IF;
      --
       DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TER_TAREA_EXTERNA_RECUPERACION... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TEX_TAREA_EXTERNA... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.TEX_TAREA_EXTERNA WHERE TAR_ID IN ( SELECT TAR_ID FROM TAR_TAREAS_NOTIFICACIONES TAR, TABLA_TMP_ASU ASU WHERE ASU.ASU_ID = TAR.ASU_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'TEX_TAREA_EXTERNA', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.TEX_TAREA_EXTERNA WHERE TAR_ID IN ( SELECT TAR_ID FROM TAR_TAREAS_NOTIFICACIONES TAR, TABLA_TMP_ASU ASU WHERE ASU.ASU_ID = TAR.ASU_ID)';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.SPR_SOLICITUD_PRORROGA... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.SPR_SOLICITUD_PRORROGA WHERE TAR_ID IN ( SELECT TAR_ID FROM TAR_TAREAS_NOTIFICACIONES TAR, TABLA_TMP_ASU ASU WHERE ASU.ASU_ID = TAR.ASU_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'SPR_SOLICITUD_PRORROGA', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.SPR_SOLICITUD_PRORROGA WHERE TAR_ID IN ( SELECT TAR_ID FROM TAR_TAREAS_NOTIFICACIONES TAR, TABLA_TMP_ASU ASU WHERE ASU.ASU_ID = TAR.ASU_ID)';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.CRT_CICLO_RECOBRO_TAREA_NOTI... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.CRT_CICLO_RECOBRO_TAREA_NOTI WHERE TAR_ID IN (
                                                  ...
                                                  FROM '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES TAR
                                                    JOIN TABLA_TMP_ASU TMP ON TAR.ASU_ID = TMP.ASU_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRD_PROCEDIMIENTOS_DERIVADOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.DPR_DECISIONES_PROCEDIMIENTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.ADA_ADJUNTOS_ASUNTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';                                                                                                                                      
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'TAR_TAREAS_NOTIFICACIONES', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.TAR_TAREAS_NOTIFICACIONES WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRB_PRC_BIE... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PRB_PRC_BIE WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE ) AND PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'PRB_PRC_BIE', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.PRB_PRC_BIE WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE ) AND PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      ...
      -- 
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_DATOS_REGISTRALES', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_DATOS_REGISTRALES WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
    ...
    --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_LOCALIZACION... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_LOCALIZACION WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_LOCALIZACION', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_LOCALIZACION WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
    ...
    --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_VALORACIONES... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_VALORACIONES WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_VALORACIONES', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_VALORACIONES WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      ...
      --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_ADICIONAL... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_ADICIONAL WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_ADICIONAL', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_ADICIONAL WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      ...
      --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_ADJ_ADJUDICACION', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_ADJ_ADJUDICACION WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      ...
      END IF; 
        ...
        --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_PER... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_PER WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_PER', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_PER WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      ...
      END IF; 
        --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_CNT... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_CNT WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_CNT', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_CNT WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      ...
      END IF; 
        --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_CAR_CARGAS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_CAR_CARGAS WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_CAR_CARGAS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_CAR_CARGAS WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      ...
      END IF; 
        --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TEA_TERMINOS_ACUERDO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.TEA_TERMINOS_ACUERDO WHERE TEA_ID IN 
                 ( SELECT TEA_ID FROM BIE_TEA TEA
                   INNER JOIN TABLA_TMP_BIE BIE ON BIE.BIE_ID = TEA.BIE_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
      ...
      END IF; 
        --
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_TEA... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_TEA WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_TEA', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_TEA WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
           DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_CAR_CARGAS .. Se han eliminado '||EXISTE||' registros');
           COMMIT;
...
--    ********************/
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_BIEN... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_BIEN';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_BIEN', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_BIEN WHERE BIE_ID IN ( SELECT BIE_ID FROM TABLA_TMP_BIE )';
      ...
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PRD_PROCEDIMIENTOS_DERIVADOS WHERE DPR_ID IN (
                                                    SELECT DPR_ID
                                                    FROM '||V_ESQUEMA||'.DPR_DECISIONES_PROCEDIMIENTOS DPR
                                                        JOIN TABLA_TMP_PRC PRC ON DPR.PRC_ID = PRC.PRC_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'PRD_PROCEDIMIENTOS_DERIVADOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.PRD_PROCEDIMIENTOS_DERIVADOS WHERE DPR_ID IN (
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.DPR_DECISIONES_PROCEDIMIENTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.DPR_DECISIONES_PROCEDIMIENTOS WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'DPR_DECISIONES_PROCEDIMIENTOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.DPR_DECISIONES_PROCEDIMIENTOS  WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
           DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.DPR_DECISIONES_PROCEDIMIENTOS .. Se han eliminado '||EXISTE||' registros');
           COMMIT;
           EXECUTE IMMEDIATE 'Alter table '||V_ESQUEMA||'.PRC_PROCEDIMIENTOS enable constraint FK_PRC_PROC_REFERENCE_PRC_PROC';
           PRO_KEYS_STATUS(V_ESQUEMA, 'DPR_DECISIONES_PROCEDIMIENTOS', 'ENABLE');
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.COV_CONVENIOS_CREDITOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.COV_CONVENIOS_CREDITOS WHERE COV_ID IN (
                                                  SELECT COV_ID 
                                                  FROM '||V_ESQUEMA||'.COV_CONVENIOS COV
                                                    JOIN TABLA_TMP_PRC TMP ON COV.PRC_ID = TMP.PRC_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
      ...
      --COV_CONVENIOS
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.COV_CONVENIOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.COV_CONVENIOS WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'COV_CONVENIOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.COV_CONVENIOS  WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_SUI_SUBASTA_INSTRUCCIONES... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_SUI_SUBASTA_INSTRUCCIONES WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_SUI_SUBASTA_INSTRUCCIONES', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_SUI_SUBASTA_INSTRUCCIONES WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.IAG_IMPULSO_AUTO_GENERADO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.IAG_IMPULSO_AUTO_GENERADO WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'IAG_IMPULSO_AUTO_GENERADO', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.IAG_IMPULSO_AUTO_GENERADO WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRB_PRC_BIE... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PRB_PRC_BIE WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'PRB_PRC_BIE', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.PRB_PRC_BIE WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
           DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.IAG_IMPULSO_AUTO_GENERADO... Se han eliminado '||EXISTE||' registros');
           COMMIT;
           PRO_KEYS_STATUS(V_ESQUEMA, 'PRB_PRC_BIE', 'ENABLE');
      END IF;      
      ...
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.RCR_RECURSOS_PROCEDIMIENTOS WHERE TAR_ID IN (
                                                SELECT TAR_ID 
                                                FROM '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES TAR
                                                  JOIN TABLA_TMP_PRC TMP ON TAR.PRC_ID = TMP.PRC_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'RCR_RECURSOS_PROCEDIMIENTOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.RCR_RECURSOS_PROCEDIMIENTOS WHERE TAR_ID IN (
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'TAR_TAREAS_NOTIFICACIONES', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.TAR_TAREAS_NOTIFICACIONES WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
           DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES... Se han eliminado '||EXISTE||' registros');
           COMMIT;
           PRO_KEYS_STATUS(V_ESQUEMA, 'TAR_TAREAS_NOTIFICACIONES', 'ENABLE');
      END IF;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.SUB_SUBASTA... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.SUB_SUBASTA WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'SUB_SUBASTA', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.SUB_SUBASTA WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
           DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.SUB_SUBASTA... Se han eliminado '||EXISTE||' registros');
           COMMIT;
           PRO_KEYS_STATUS(V_ESQUEMA, 'SUB_SUBASTA', 'ENABLE');
      END IF;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.RES_RESOLUCIONES_MASIVO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.RES_RESOLUCIONES_MASIVO WHERE RES_PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'RES_RESOLUCIONES_MASIVO', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.RES_RESOLUCIONES_MASIVO WHERE RES_PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRD_PROCEDIMIENTOS_DERIVADOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PRD_PROCEDIMIENTOS_DERIVADOS WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'PRD_PROCEDIMIENTOS_DERIVADOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.PRD_PROCEDIMIENTOS_DERIVADOS WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
           DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRD_PROCEDIMIENTOS_DERIVADOS... Se han eliminado '||EXISTE||' registros');
           COMMIT;
           PRO_KEYS_STATUS(V_ESQUEMA, 'PRD_PROCEDIMIENTOS_DERIVADOS', 'ENABLE');
      END IF;      
      ...
      -- PRC_PROCEDIMIENTOS
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PRC_PROCEDIMIENTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PRC_PROCEDIMIENTOS WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           ...
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.PRC_PROCEDIMIENTOS  WHERE PRC_ID IN ( SELECT PRC_ID FROM TABLA_TMP_PRC )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.ANA_ANALIS_ACUERDO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ANA_ANALIS_ACUERDO WHERE ACU_ID IN ( SELECT ACU_ID FROM ACU_ACUERDO_PROCEDIMIENTOS ACU
                                                                                         INNER JOIN TABLA_TMP_ASU TMP ON ACU.ASU_ID = TMP.ASU_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'ANA_ANALIS_ACUERDO', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.ANA_ANALIS_ACUERDO WHERE ACU_ID IN ( SELECT ACU_ID FROM ACU_ACUERDO_PROCEDIMIENTOS ACU
                                                                                         INNER JOIN TABLA_TMP_ASU TMP ON ACU.ASU_ID = TMP.ASU_ID)';
      ...
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_TEA WHERE TEA_ID IN (
                                                    ...
                                                    FROM '||V_ESQUEMA||'.TEA_TERMINOS_ACUERDO TEA
                                                      ...
                                                      JOIN TABLA_TMP_ASU TMP ON ACU.ASU_ID = TMP.ASU_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_TEA', 'DISABLE');
      ...
      -- TEA_CNT
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TEA_CNT... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.TEA_TERMINOS_ACUERDO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.TEA_TERMINOS_ACUERDO WHERE ACU_ID IN ( SELECT ACU_ID FROM ACU_ACUERDO_PROCEDIMIENTOS ACU
                                                                                         INNER JOIN TABLA_TMP_ASU TMP ON ACU.ASU_ID = TMP.ASU_ID)';

      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'TEA_TERMINOS_ACUERDO', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.TEA_TERMINOS_ACUERDO WHERE ACU_ID IN ( SELECT ACU_ID FROM ACU_ACUERDO_PROCEDIMIENTOS ACU
                                                                                         INNER JOIN TABLA_TMP_ASU TMP ON ACU.ASU_ID = TMP.ASU_ID)';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.AAR_ACTUACIONES_REALIZADAS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.AEA_ACTUACIO_EXPLOR_ACUERDO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.ACU_ACUERDO_PROCEDIMIENTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ACU_ACUERDO_PROCEDIMIENTOS WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'ACU_ACUERDO_PROCEDIMIENTOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.ACU_ACUERDO_PROCEDIMIENTOS WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.AAS_ANALISIS_ASUNTO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.AAS_ANALISIS_ASUNTO WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'AAS_ANALISIS_ASUNTO', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.AAS_ANALISIS_ASUNTO WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.BIE_ANC_ANALISIS_CONTRATOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.BIE_ANC_ANALISIS_CONTRATOS WHERE ANC_ID IN ( SELECT ANC_ID FROM ANC_ANALISIS_CONTRATOS ANC
                                                                                INNER JOIN TABLA_TMP_ASU TMP ON ANC.ASU_ID = TMP.ASU_ID )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'BIE_ANC_ANALISIS_CONTRATOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.BIE_ANC_ANALISIS_CONTRATOS WHERE ANC_ID IN ( SELECT ANC_ID FROM ANC_ANALISIS_CONTRATOS ANC
                                                                                INNER JOIN TABLA_TMP_ASU TMP ON ANC.ASU_ID = TMP.ASU_ID )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.ANC_ANALISIS_CONTRATOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      IF (EXISTE>0) THEN
           ...
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.ANC_ANALISIS_CONTRATOS WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.HCA_HISTORICO_CAMBIOS_ASUNTO... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.HCA_HISTORICO_CAMBIOS_ASUNTO WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'HCA_HISTORICO_CAMBIOS_ASUNTO', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.HCA_HISTORICO_CAMBIOS_ASUNTO WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      ...
      -- ADJ_ADJUNTOS
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.ADJ_ADJUNTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ADJ_ADJUNTOS WHERE ADJ_ID IN (
                                                  SELECT ADJ_ID 
                                                  FROM '||V_ESQUEMA||'.ADA_ADJUNTOS_ASUNTOS ADA
                                                    JOIN TABLA_TMP_ASU TMP ON ADA.ASU_ID = TMP.ASU_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
      ...
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ADA_ADJUNTOS_ASUNTOS WHERE ASU_ID IN (
                                                    SELECT ASU_ID FROM TABLA_TMP_ASU)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'ADA_ADJUNTOS_ASUNTOS', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.ADA_ADJUNTOS_ASUNTOS WHERE ASU_ID IN (
...
--    *ORDEN DE BORRADO 4 *
--    ********************/      
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.ASU_ASUNTOS... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ASU_ASUNTOS WHERE ASU_ID IN ( SELECT ASU_ID FROM TABLA_TMP_ASU )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
...
--    ********************/
      ...
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.CRE_PRC_CEX WHERE CRE_PRC_CEX_CEXID IN (
                                                SELECT CEX_ID 
                                                FROM '||V_ESQUEMA||'.CEX_CONTRATOS_EXPEDIENTE CEX
                                                  JOIN TABLA_TMP_EXP TMP ON CEX.EXP_ID = TMP.EXP_ID)';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'CRE_PRC_CEX', 'DISABLE');
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.CEX_CONTRATOS_EXPEDIENTE... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.CEX_CONTRATOS_EXPEDIENTE WHERE EXP_ID IN ( SELECT EXP_ID FROM TABLA_TMP_EXP )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'CEX_CONTRATOS_EXPEDIENTE', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.CEX_CONTRATOS_EXPEDIENTE  WHERE EXP_ID IN ( SELECT EXP_ID FROM TABLA_TMP_EXP )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.PEX_PERSONAS_EXPEDIENTE... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PEX_PERSONAS_EXPEDIENTE WHERE EXP_ID IN ( SELECT EXP_ID FROM TABLA_TMP_EXP )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'PEX_PERSONAS_EXPEDIENTE', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.PEX_PERSONAS_EXPEDIENTE 
           WHERE EXP_ID IN ( SELECT EXP_ID FROM TABLA_TMP_EXP )';
...
--    ********************/  
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.EXP_EXPEDIENTES... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      V_SQL:= 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.EXP_EXPEDIENTES  WHERE EXP_ID IN ( SELECT EXP_ID FROM TABLA_TMP_EXP )';
      EXECUTE IMMEDIATE V_SQL INTO EXISTE;
      IF (EXISTE>0) THEN
           PRO_KEYS_STATUS(V_ESQUEMA, 'EXP_EXPEDIENTES', 'DISABLE');
           EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA ||'.EXP_EXPEDIENTES WHERE EXP_ID IN ( SELECT EXP_ID FROM TABLA_TMP_EXP )';
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.CCL_CONTRATOS_CLIENTE... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' '||V_ESQUEMA||'.CLI_CLIENTES... Comprobando si existen registros para el usuario MIGRABNKF2');
      EXISTE := 0;
