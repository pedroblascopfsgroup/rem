...
LEFT JOIN CNV_TMP_CNT_ID CNT ON C.NUMERO_CONTRATO=CNT.NUMERO_CONTRATO 
   AND C.NUMERO_ESPEC=CNT.NUMERO_ESPEC AND C.TIPO_PRODUCTO=CNT.TIPO_PRODUCTO  
...
where TMP.CNT_ID not in ( SELECT CEX.CNT_ID FROM CEX_CONTRATOS_EXPEDIENTE CEX) AND TMP.CNT_ID IS NOT NULL AND EXP.EXP_ID IS NOT NULL;
