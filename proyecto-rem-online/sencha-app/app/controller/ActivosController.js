/**
 * Controlador global de aplicaci贸n que gestiona la funcionalidad de la entidad Activo
 */
Ext.define('HreRem.controller.ActivosController', {
    ...
    control: {
    	...
    	/**
		 * De esta manera conseguimos que todas aquellas pesta帽as marcadas para refrescar, llamen a su funcion de refresco correspondiente.
		 * En este punto la funci贸n funcionRecargar existir谩 porque unicamente se marcan aquellas pesta帽as que la tengan.
		 */
    ...
    abrirDetalleActivoById: function(id, titulo, refLinks) {
    	...
    	HreRem.model.Activo.load(id, {
		    ...
		    success: function(activo) {
		    	 ...
		    	 * para evitar el uso de Notify() si no hay activacion de pesta馻s (refLinks != null)
		    	 */
		    	if (refLinks != null){
    ...
    seleccionarTabByXtype: function (tab, refLinks) {
		 ...
		 *   tantas veces como xtype de pesta馻s contenga refLinks, separados por puntos
		 * - Cada llamada activa una pesta馻 por xtype y pasa a la siguiente llamada el resto
    	 * - La 1a llamada debe ir precedida por un Notify(), que es necesario para evitar un error
    	 *   por falta de alg鷑 dato. El error bloquea la aplicacion 
    ...
    abrirDetalleAgrupacion: function(record) {
    	var me = this,
    	titulo = "Agrupaci贸n " + record.get("numAgrupacionRem"),
    	id = record.get("id");		
    ...
    //FIXME: Unir con el m茅todo abrirDetalleAgrupacion cuando se modifique para que no traiga objetos a la vista
    
    abrirDetalleAgrupacionActivo: function(record) {

    	var me = this,
    	titulo = "Agrupaci贸n " + record.get("numAgrupRem"),
    ...
    /**
     * Crea una pesta帽a en funci贸n de los parametros recibidos.
     * @param {} tabpanel donde crear el tab
     * @param {} prefix para a帽adir al id del tab
     * @param {} xtype clase a instanciar
     * @param {} rec modelo que servir谩 para pintar la informaci贸n el tab
     * @param {} cfg configuraci贸n del tab
     */
    ...
    abrirDetalleTramite: function(grid,record) {
    	...
    	var me = this;
    	//debugger;
    	//Sacar que valor le paso para referenciar el tr谩mite
