Ext
		.define(
				'HreRem.view.activos.detalle.ActivoDetalleController',
				{
					...
					cargarTabData : function(form) {
						...
						if (!form.saveMultiple) {
							...
							if (Ext.isDefined(model.getProxy().getApi().read)) {
								...
								// Si la API no contiene metodo de lectura
								// (read).
							...
							models[index].load({
								success : function(record) {
									if (!Ext.isEmpty(me.getViewModel())) {
										me.getViewModel().set(
												nameModels[index], record);
										index++;

										if (index < models.length) {
											me.cargarTabDataMultiple(form,
													index, models, nameModels);
										} else {
											form.unmask();
										}
									}
					...
					cargarTabDataPresupuestoGrafico : function(form) {
						var me = this, model = form.getModelInstance(), id = me
								.getViewModel().get("activo.id");

						form.down('cartesian').store.proxy.setExtraParam(
								'idActivo', form.lookupController()
										.getViewModel().get("activo.id"));
						...
						if (form.isFormValid() || form.disableValidation) {
							...
							if (!Ext.isEmpty(btn)) {
								...
								if (Ext.isDefined(btn.name)
										&& btn.name === 'firstLevel') {
									me.getViewModel().set("editingFirstLevel",
											false);
								} else {
							...
							if (tabData.models != null) {
								if (tabData.models[0].name == "datosregistrales") {
									...
									var fechaInscripcionReg = record
											.get("fechaInscripcionReg");
									if ((typeof fechaInscripcionReg) == 'string') {
										var from = fechaInscripcionReg
												.split("/");
										fechaInscripcionReg = new Date(from[2],
												from[1] - 1, from[0])
									}
									if (fechaInscripcionReg != null) {
										// tabData.models[0].data.fechaInscripcionReg
										// = new Date(fechaInscripcionReg);
									}
					...
					onSaveFormularioCompletoDistribuciones : function(btn, form) {
						...
						if (form.isFormValid() || form.disableValidation) {
							...
							// Obtener jsondata para guardar activo
							var numPlanta = me.lookupReference(
									'comboNumeroPlantas').getValue();
							var tipoHabitaculoCodigo = me.lookupReference(
									'comboTipoHabitaculo').getValue();
							var superficie = me.lookupReference('superficie')
									.getValue();
							var cantidad = me.lookupReference('cantidad')
									.getValue();
							...
							var activosPropagables = me.getViewModel().get(
									"activo.activosPropagables")
									|| [];
							...
							if (activosPropagables.length > 0) {
								...
								tabPropagableData = me
										.createFormPropagableData(form, tabData);
								if (!Ext.isEmpty(tabPropagableData)) {
									...
									me.getView().add(
											ventanaOpcionesPropagacionCambios);
									me.getView().unmask();
									return false;
								}
							}
					...
					saveMultipleRecords : function(contador, records, form) {
						...
						if (records[contador] != null
							...
							// Si la API no contiene metodo de escritura (create
							// or update).
							contador++;
						...
						Ext.Ajax
								.request({
									...
									success : function(response, opts) {
										me.getViewModel().data.storeTramites
												.load();
										if (Ext.decode(response.responseText).errorCreacion)
											me
													.fireEvent(
															"errorToast",
															Ext
																	.decode(response.responseText).errorCreacion);
										else
											me.fireEvent("infoToast", HreRem
													.i18n("msg.operacion.ok"));
									},
					...
					onTramitePublicacionClick : function(btn) {
						...
						var idActivo = me.getViewModel().get("activo.id");
						var url = $AC
								.getRemoteUrl('activo/crearTramitePublicacion');
					...
					onClickCrearTrabajo : function(btn) {
						...
						var codCartera = me.getViewModel().get(
								"activo.entidadPropietariaCodigo");
					...
					onEliminarPropietarioClick : function(btn) {
						...
						var me = this;
						var grid = btn.up('fieldsettable').down(
								'#listadoPropietarios');
					...
					onDivisionHorizontalSelect : function(combo, value) {
						...
						me.lookupReference('estadoDivHorizontal').setDisabled(
								disabled);
					...
					onChangeBloqueo : function(field, newValue, oldValue) {
						...
						if (newValue > 0) {
							me.lookupReference('bloqueoPrecioFechaIni')
									.setValue($AC.getCurrentDate());
						} else {
							me.lookupReference('bloqueoPrecioFechaIni')
									.setValue("");
					...
					onDivisionHorizontalAdmisionSelect : function(combo, value) {
						...
						me.lookupReference('estadoDivHorizontalAdmision')
								.setDisabled(disabled);
						me.lookupReference(
								'estadoDivHorizontalNoInscritaAdmision')
								.setDisabled(disabled);
						me.lookupReference('estadoDivHorizontalAdmision').allowBlank = disabled;
						me
								.lookupReference('estadoDivHorizontalNoInscritaAdmision').allowBlank = disabled;
					...
					onEstadoDivHorizontalSelect : function(combo, value) {
						...
						var me = this, disabled = (value == 1 || Ext
								.isEmpty(value));
					...
					onEstadoDivHorizontalAdmisionSelect : function(combo, value) {
						...
						if (disabled) {
							me.lookupReference(
									...
									.setValue("");
							me
									.lookupReference('estadoDivHorizontalNoInscritaAdmision').allowBlank = true;
						}
					...
					onChangeProvincia : function(combo, value, oldValue, eOpts) {
						var me = this;
						me.getViewModel().get('activo').set(
								'asignaGestPorCambioDeProv', false);
						if (value != oldValue) {
							var me = this;
							me.getViewModel().get('activo').set(
									'asignaGestPorCambioDeProv', true);
						}
					...
					onChangeTipoGestor : function(cmb) {
						...
						me.lookupReference('usuarioGestor').getStore()
					...
					onChangeTipoTitulo : function(btn, value) {
						...
						me.lookupReference('judicial')
								.setVisible(value == '01');
						me.lookupReference('judicial').setDisabled(
								value != '01');
						me.lookupReference('noJudicial').setVisible(
								value == '02');
						me.lookupReference('noJudicial').setDisabled(
								value != '02');
					...
					onChangeTipoTituloAdmision : function(btn, value) {
						...
						me.lookupReference('judicialAdmision').setVisible(
								value == '01');
						me.lookupReference('judicialAdmision').setDisabled(
								value != '01');
						me.lookupReference('noJudicialAdmision').setVisible(
								value == '02');
						me.lookupReference('noJudicialAdmision').setDisabled(
								value != '02');
						me.lookupReference('pdvAdmision').setVisible(
								value == '03');
						me.lookupReference('pdvAdmision').setDisabled(
								value != '03');
					...
					onChangeSujetoAExpediente : function(btn, value) {
						...
						if (value == '0') {
							me.lookupReference('expropiacionForzosa')
									.setVisible(false);
						} else {
							me.lookupReference('expropiacionForzosa')
									.setVisible(true);
					...
					onAgregarGestoresClick : function(btn) {
						...
						var url = $AC
								.getRemoteUrl('activo/insertarGestorAdicional');
						var parametros = btn.up("combogestores").getValues();
						parametros.idActivo = me.getViewModel()
								.get("activo.id");
						...
						Ext.Ajax
								.request({
									...
									success : function(response, opts) {
										...
										btn.up("gestoresactivo").down(
												"[reference=listadoGestores]")
												.getStore().load();
										btn.up("gestoresactivo").down("form")
												.reset();
					...
					onChkbxMuestraHistorico : function(chkbx, checked) {
						...
						var grid = chkbx.up('gestoresactivo').down(
								"gestoreslist");
						var store = me.getViewModel().get(
					...
					onClickBotonEditar : function(btn) {
						...
						Ext.Array.each(btn.up('tabpanel').getActiveTab().query(
								'component[isReadOnlyEdit]'), function(field,
								index) {
							field.fireEvent('edit');
						});

						btn.up('tabpanel').getActiveTab().query(
								'component[isReadOnlyEdit]')[0].focus();
						if (Ext.isDefined(btn.name)
								&& btn.name === 'firstLevel') {
					...
					onSaveFormularioCompletoTabComercial : function(btn, form) {
						...
						if (me.lookupReference('dtFechaVenta').value != null
								&& (me.getViewModel().get(
										'comercial.situacionComercialCodigo') != CONST.SITUACION_COMERCIAL['VENDIDO'])) {
							Ext.Msg
									.show({
										title : HreRem
					...
					onClickBotonCancelarPropietario : function(btn) {
						...
						var tab = padre
								.down('tituloinformacionregistralactivo');
						tab.funcionRecargar();
					...
					onClickBotonGuardarPropietario : function(btn) {
						var me = this;
						var url = $AC
								.getRemoteUrl('activo/updateActivoPropietarioTab');
						...
						params['porcPropiedad'] = form.findField(
								"porcPropiedad").getValue();
						params['tipoGradoPropiedadCodigo'] = form.findField(
								"tipoGradoPropiedad").getValue();
						params['tipoPersonaCodigo'] = form.findField(
								"tipoPersona").getValue();
						params['nombre'] = form.findField("nombre").getValue();
						params['tipoDocIdentificativoCodigo'] = form.findField(
								"tipoDoc").getValue();
						params['docIdentificativo'] = form.findField("numDoc")
								.getValue();
						params['direccion'] = form.findField("direccion")
								.getValue();
						params['provinciaCodigo'] = form.findField("provincia")
								.getValue();
						params['localidadCodigo'] = form.findField("localidad")
								.getValue();
						params['codigoPostal'] = form.findField("codigoPostal")
								.getValue();
						params['telefono'] = form.findField("telefono")
								.getValue();
						params['email'] = form.findField("email").getValue();
						params['tipoPropietario'] = form.findField(
								"tipoPropietario").getValue();
						params['nombreContacto'] = propietario
								.get('nombreContacto');
						params['telefono1Contacto'] = propietario
								.get('telefono1Contacto');
						params['telefono2Contacto'] = propietario
								.get('telefono2Contacto');
						params['emailContacto'] = propietario
								.get('emailContacto');
						...
						params['direccionContacto'] = propietario
								.get('direccionContacto');
						params['observaciones'] = propietario
								.get('observaciones');
					...
					onClickBotonAnyadirPropietario : function(btn) {
						var me = this;
						var url = $AC
								.getRemoteUrl('activo/createActivoPropietarioTab');
						form = btn.up('window').down('formBase').getForm();
						formulario = btn.up('window').down('formBase');
						var window = btn.up('window');
						var padre = window.floatParent;
					...
					onTramitesListDobleClick : function(grid, record) {
						var me = this;
						// HREOS-846 Si el activo esta fuera del perimetro Haya,
						// no debe poder abrir tramites desde el activo
						if (me.getViewModel().get('activo').get(
								'incluidoEnPerimetro') == "false") {
							me
									.fireEvent(
											"errorToast",
											HreRem
							...
							me.getView().fireEvent('abrirDetalleTramite', grid,
									record);
						}
					},
					...
					/**
					 * FunciÃ³n que refresca la pestaña activa, y marca el resto
					 * de componentes para referescar. Para que un componente
					 * sea marcado para refrescar, es necesario que implemente
					 * la funciÃ³n funciÃ³nRecargar con el cÃ³digo necesario
					...
					refrescarActivo : function(refrescarPestanyaActiva) {
						var me = this, refrescarPestanyaActiva = Ext
								.isEmpty(refrescarPestanyaActiva) ? false
								: refrescarPestanyaActiva, activeTab = me
								.getView().down("tabpanel").getActiveTab();
						...
						// Actualizamos la pestaña actual si tiene funciÃ³n de
						// recargar
						if (refrescarPestanyaActiva
					...
					abrirFormularioAdjuntarDocumentos : function(grid) {
						...
						Ext
								.create(
										"HreRem.view.common.adjuntos.AdjuntarDocumento",
										{
											entidad : 'activo',
											idEntidad : idActivo,
											parent : grid
					...
					updateOrdenFotosInterno : function(data, record, store) {
						...
						for (i = 0; i < record.store.getData().items.length; i++) {
							...
							if (store.getData().items[i].data.orden != orden) {
								...
								// me.getView().mask(HreRem.i18n("msg.mask.loading"));
								var url = $AC
										.getRemoteUrl('activo/updateFotosById');
								Ext.Ajax
										.request({
											...
											success : function(a, operation,
												...
												if (me.ordenGuardado >= me.storeGuardado
														.getData().items.length
														&& me.refrescarGuardado) {
													me.storeGuardado.load();
													me.refrescarGuardado = false;
													/*
													 * //FIXME: Â¿Poner
													 * mÃ¡scara? Ext.toast({
													 * html: 'LA OPERACIÃN SE
													 * HA REALIZADO
													 * CORRECTAMENTE', width:
													 * 360, height: 100, align:
													 * 't' });
													 */
													// me.getView().unmask();
												}
					...
					onDeleteFotoClick : function(btn) {
						...
						Ext.Msg
								.show({
									title : HreRem
											.i18n('title.confirmar.eliminacion'),
									msg : HreRem.i18n('msg.desea.eliminar'),
									buttons : Ext.MessageBox.YESNO,
									fn : function(buttonId) {
										if (buttonId == 'yes') {

											var nodos = btn.up('form').down(
													'dataview')
													.getSelectedNodes();
											var storeTemp = btn.up('form')
													.down('dataview')
													.getStore();
											var idSeleccionados = [];
											for (var i = 0; i < nodos.length; i++) {

												idSeleccionados[i] = storeTemp
														.getAt(
																nodos[i]
																		.getAttribute('data-recordindex'))
														.getId();

											}

											var url = $AC
													.getRemoteUrl('activo/deleteFotosActivoById');
											Ext.Ajax
													.request({
					...
					onDownloadFotoClick : function(btn) {
						...
						if (nodos.length != 0) {
							...
							var storeTemp = btn.up('form').down('dataview')
									.getStore();

							config.url = $AC.getWebPath()
									+ "activo/getFotoActivoById."
									+ $AC.getUrlPattern();
							config.params = {};

							config.params.idFoto = storeTemp.getAt(
									nodos[0].getAttribute('data-recordindex'))
									.getId();
					...
					// Se deja planteado para fase 2, pero para fase 1 se
					// elimina el botÃ³n imprimir.
					...
					cargarTabFotos : function(form) {
						...
						me.getViewModel().data.storeFotos.getProxy()
								.setExtraParams({
									'id' : idActivo,
									tipoFoto : '01'
								});
						me.getViewModel().data.storeFotosTecnicas.getProxy()
								.setExtraParams({
									'id' : idActivo,
									tipoFoto : '02'
								});

						me.getViewModel().data.storeFotos.on('load',
								function() {
									me.getViewModel().data.storeFotosTecnicas
											.load();
								});

						me.getViewModel().data.storeFotosTecnicas.on('load',
								function() {
									me.getView().unmask();
								});
					...
					onHistoricoPeticionesActivoDobleClick : function(grid,
							record) {
						var me = this;
					...
					onHistoricoPresupuestosActivoListDobleClick : function(
							grid, record) {
						grid.up('form').down(
					...
					// FIXME: Funciones para el grÃ¡fico de presupuestos. Llevar
					// a otro controlador aparte.
					onAxisLabelRender : function(axis, label, layoutContext) {
						// Custom renderer overrides the native axis label
						// renderer.
						// Since we don't want to do anything fancy with the
						// value
						// ourselves except appending a '%' sign, but at the
						// same time
					...
					onSeriesTooltipRender : function(tooltip, record, item) {
						var fieldIndex = Ext.Array.indexOf(item.series
								.getYField(), item.field), browser = item.series
								.getTitle()[fieldIndex];
					...
					onClickVerificarDireccion : function(btn) {
						...
						latLng.latitud = me.getViewModel().get(
								"informeComercial.latitud");
						latLng.longitud = me.getViewModel().get(
								"informeComercial.longitud");
						geoCodeAddr = me.getViewModel().get("geoCodeAddr");

						Ext
								.create(
										"HreRem.ux.window.geolocalizacion.ValidarGeoLocalizacion",
										{
					...
					// Este mÃ©todo comprueba si el municipio es 'Barcelona,
					// Madrid, Valencia o Alicante/Alacant'.
					checkDistrito : function(combobox) {
						var me = this;
						var view = me.getView();
						var distrito = combobox.getRawValue();

						// Comprobar distrito y mostrar u ocultar el textfield
						// de distrito.
					...
					onSearchPropuestasActivoClick : function(btn) {
						...
						var me = this;
						this.lookupReference('propuestasActivoList').getStore()
								.loadPage(1);
					...
					beforeLoadPropuestas : function(store, operation, opts) {
						...
						if (searchForm.isValid()) {
							...
							store.getProxy().extraParams = me
									.getFormCriteria(searchForm);
							store.getProxy().extraParams.idActivo = me
									.getViewModel().get("activo.id");
					...
					// Funcion que se ejecuta al hacer click en el botÃ³n
					// limpiar
					onCleanFiltersClick : function(btn) {
						btn.up('form').getForm().reset();
					},

					// Esta funciÃ³n es llamada cuando cambia el estado de
					// publicaciÃ³n del activo.
					onChangeEstadoPublicacion : function(field) {
						...
						switch (codigo) {
						case "01": // Publicado.
							view.lookupReference('seccionPublicacionForzada')
									.hide();
							view.lookupReference('seccionOcultacionForzada')
									.show();
							view.lookupReference('seccionOcultacionPrecio')
									.show();
							view
									.lookupReference(
											'seccionDespublicacionForzada')
									.show();
							break;
					...
					// Esta funciÃ³n es llamada cuando cambia el estado del
					// combo 'otro' en los
					// condicionantes de la publicaciÃ³n del activo. Muestra u
					// oculta el Ã¡rea de
					...
					onChangeComboOtro : function(combo) {
						...
						if (combo.getValue() === '0') {
							...
							view.lookupReference('fieldtextCondicionanteOtro')
									.setValue('');
							view.lookupReference('fieldtextCondicionanteOtro')
									.hide();
						} else {
					...
					onClickAbrirExpedienteComercial : function(grid, rowIndex,
							colIndex) {

						var me = this, record = grid.getStore().getAt(rowIndex);
						me.getView()
								.fireEvent('abrirDetalleExpediente', record);
					...
					onClickBotonCancelarWizard : function(btn) {
						...
						Ext.Msg.show({
							...
							fn : function(buttonId) {
								if (buttonId == 'yes') {
									var url = $AC.getRemoteUrl('activooferta/deleteTmpClienteByDocumento');
									Ext.Ajax.request({
				    	    		     url: url,
				    	    			 method : 'POST',
				    	    		     params: {docCliente: docCliente},
					...
					onClickBotonGuardarOferta : function(btn) { 
						...
						ventanaDetalle = window.down('anyadirnuevaofertadetalle');
					...
					onChkbxOfertasAnuladas : function(chkbox, checked) {
						var me = this;
						var grid = chkbox.up('ofertascomercialactivo').down(
								"ofertascomercialactivolist");
					...
					// Este mÃ©todo copia los valores de los campos de 'Datos
					// Mediador' a los campos de 'Datos admisiÃ³n'.
					onClickCopiarDatosDelMediador : function(btn) {
						var me = this;
						var view = me.getView();

						view.lookupReference('tipoActivoAdmisionInforme')
								.setValue(
										view.lookupReference(
												'tipoActivoMediadorInforme')
												.getValue());
						view.lookupReference(
								'subtipoActivoComboAdmisionInforme').setValue(
								view.lookupReference(
										'subtipoActivoComboMediadorInforme')
										.getValue());
						view.lookupReference('tipoViaAdmisionInforme')
								.setValue(
										view.lookupReference(
												'tipoViaMediadorInforme')
												.getValue());
						view.lookupReference('nombreViaAdmisionInforme')
								.setValue(
										view.lookupReference(
												'nombreViaMediadorInforme')
												.getValue());
						view.lookupReference('numeroAdmisionInforme').setValue(
								view.lookupReference('numeroMediadorInforme')
										.getValue());
						view.lookupReference('escaleraAdmisionInforme')
								.setValue(
										view.lookupReference(
												'escaleraMediadorInforme')
												.getValue());
						view.lookupReference('plantaAdmisionInforme').setValue(
								view.lookupReference('plantaMediadorInforme')
										.getValue());
						view.lookupReference('puertaAdmisionInforme').setValue(
								view.lookupReference('puertaMediadorInforme')
										.getValue());
						view.lookupReference('codPostalAdmisionInforme')
								.setValue(
										view.lookupReference(
												'codPostalMediadorInforme')
												.getValue());
						view
								.lookupReference(
										'municipioComboAdmisionInforme')
								.setValue(
										view
												.lookupReference(
														'municipioComboMediadorInforme')
												.getValue());
						view.lookupReference('poblacionalAdmisionInforme')
								.setValue(
										view.lookupReference(
												'poblacionalMediadorInforme')
												.getValue());
						view
								.lookupReference(
										'provinciaComboAdmisionInforme')
								.setValue(
										view
												.lookupReference(
														'provinciaComboMediadorInforme')
												.getValue());
						view.lookupReference('latitudAdmisionInforme')
								.setValue(
										view.lookupReference('latitudmediador')
												.getValue());
						view.lookupReference('longitudAdmisionInforme')
								.setValue(
										view
												.lookupReference(
														'longitudmediador')
					...
					onSaveFormularioCompletoOferta : function(form, window) { 
						...
						record = form.getBindRecord(),
						ofertaForm = null,
						wizardAltaOferta = form.up('wizardaltaoferta');
					...
					onActivosVinculadosClick : function(tableView, indiceFila,
							indiceColumna) {
						var me = this;
						...
						me.getView().fireEvent('abrirDetalleActivo',
								record.get('idActivo'),
								"Activo " + record.get("numActivo"));
					},

					// Este mÃ©todo comprueba si el campo fechaHasta o
					// UsuarioBaja tiene datos, lo que supone que el registro
					// ya se encuentra dado de baja y no se permite volver a dar
					// de baja.
					onGridCondicionesEspecificasRowClick : function(grid,
							record, tr, rowIndex) {
					...
					// MÃ©todo para borrar/anular un precio vigente sin guardar
					// en el Historico
					onDeletePrecioVigenteClick : function(tableView,
					...
					// MÃ©todo que es llamado cuando se solicita la tasaciÃ³n
					// del activo desde Bankia.
					...
					onVisitasListDobleClick : function(grid, record, tr,
							rowIndex) {
						var me = this, record = grid.getStore().getAt(rowIndex);
					...
					onProveedoresListClick : function(gridView, record) {
						...
						//		
						// var fieldset =
						// me.lookupReference('estadoPbcCompradoRef');
						// fieldset.mask(HreRem.i18n("msg.mask.loading"));
						//	
						...
						gridView.up('form')
								.down('[reference=listadogastosref]')
								.getStore().getProxy().setExtraParams({
									'idActivo' : idActivo,
					...
					abrirPestanyaProveedor : function(tableView, indiceFila,
							indiceColumna) {
						var me = this;
						var grid = tableView.up('grid');
						var record = grid.store.getAt(indiceFila);
						...
						if (!Ext.isEmpty(record.get('idProveedor'))) {
							...
							me.getView().fireEvent('abrirDetalleProveedor',
									record);
						} else if (!Ext.isEmpty(record.get('id'))) {
							var codigoProveedor = record
									.get('codigoProveedorRem');
					...
					onClickAbrirGastoProveedorIcono : function(tableView,
							indiceFila, indiceColumna) {
						var me = this;
					...
					// Este mÃ©todo obtiene los valores de las fechas fin e
					// inicio de la fila que se estÃ¡ editando y comprueba las
					...
					validateFechas : function(datefield, value) {
						...
						// Obtener columnas automÃ¡ticamente por 'dataindex =
						// fechaFin' y 'dataindex = fechaInicio'.
						var fechaFinActualRow = grid.columns[Ext.Array.indexOf(
								Ext.Array.pluck(grid.columns, 'dataIndex'),
								'fechaFin')];
						var fechaInicioActualRow = grid.columns[Ext.Array
								.indexOf(Ext.Array.pluck(grid.columns,
										'dataIndex'), 'fechaInicio')];

						if (!Ext.isEmpty(selected)) {
							// Almacenar la fila selecciona para cuando estÃ©
							// siendo editada.
						...
						// Recogemos los valores actuales del grid y los mismos
						// almacenados en el store segÃºn casos.
						var fechaInicioMinimo = fechaInicioActualRow
								.getEditor().value;
						var fechaInicioExistenteMinimo = grid.getStore()
								.findRecord('codigoTipoPrecio',
										tipoMinimoAutorizado).getData().fechaInicio;
						var fechaFinMinimo = grid.getStore().findRecord(
								'codigoTipoPrecio', tipoMinimoAutorizado)
								.getData().fechaFin;
						var fechaFinAprobadoVentaWeb = fechaFinActualRow
								.getEditor().value;
						var fechaFinExistenteAprobadoVentaWeb = grid.getStore()
								.findRecord('codigoTipoPrecio',
										tipoAprobadoVentaWeb).getData().fechaFin;
						var fechaInicioAprobadoVentaWeb = fechaInicioActualRow
								.getEditor().value;
						var fechaInicioExistenteAprobadoVentaWeb = grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoAprobadoVentaWeb).getData().fechaInicio;
						var fechaInicioDescuentoAprobado = fechaInicioActualRow
								.getEditor().value;
						var fechaInicioExistenteDescuentoAprobado = grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoDescuentoAprobado).getData().fechaInicio;
						var fechaFinDescuentoAprobado = fechaFinActualRow
								.getEditor().value;
						var fechaFinExistenteDescuentoAprobado = grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoDescuentoAprobado).getData().fechaFin;
						var fechaInicioDescuentoPublicadoWeb = fechaInicioActualRow
								.getEditor().value;
						var fechaFinDescuentoPublicadoWeb = fechaFinActualRow
								.getEditor().value;
						var fechaInicioExistenteDescuentoPublicadoWeb = grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoDescuentoPublicadoWeb).getData().fechaInicio;
						var fechaFinExistenteDescuentoPublicadoWeb = grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoDescuentoPublicadoWeb).getData().fechaFin;
						...
						case tipoMinimoAutorizado:
							if (datefield.dataIndex === 'fechaInicio') {
								...
								if (Ext.isEmpty(fechaInicioMinimo)) {
									...
									return HreRem
											.i18n('info.fecha.precios.msg.validacion');
								} else {
							...
							return true;
						case tipoAprobadoVentaWeb: // La fecha de fin de
							// aprobado venta(web) debe
							// ser menor o igual a la
							// fecha fin mÃ­nimo.
							if (datefield.dataIndex === 'fechaFin') {
								// La fecha de fin
								if (Ext.isEmpty(fechaFinMinimo)
										|| Ext
												.isEmpty(fechaFinAprobadoVentaWeb)) {
									// Si la fecha contra la que compara o la
									// misma no estÃ¡n definidas, se valida
									// positivo.
									return true;
								}
								if (fechaFinAprobadoVentaWeb > fechaFinMinimo) {
									// Ha de ser menor o igual a la fecha fin
									// mÃ­nimo.
									return HreRem
								...
								if (!Ext.isEmpty(fechaInicioExistenteMinimo)) {
									...
									if (!Ext
											.isEmpty(fechaInicioAprobadoVentaWeb)
											&& fechaInicioAprobadoVentaWeb < fechaInicioExistenteMinimo) {
										// Si la propia fecha estÃ¡ definida, ha
										// de ser mayor o igual que la fecha
										// inicio mÃ­nimo
										return HreRem
						...
						case tipoDescuentoAprobado:
							if (datefield.dataIndex === 'fechaFin') {
								...
								if (!Ext
										.isEmpty(fechaFinExistenteDescuentoPublicadoWeb)
										&& fechaFinExistenteDescuentoPublicadoWeb > fechaFinDescuentoAprobado) {
									// Ha de ser menor o igual que la fecha fin
									// aprobado venta web
									return HreRem
								...
								if (!Ext
										.isEmpty(fechaInicioExistenteDescuentoPublicadoWeb)
										&& fechaInicioExistenteDescuentoPublicadoWeb < fechaInicioDescuentoAprobado) {
									// Ha de ser mayor o igual que la fecha
									// inicio aprobado venta web
									return HreRem
						...
						case tipoDescuentoPublicadoWeb:
							if (datefield.dataIndex === 'fechaFin') {
								...
								if (!Ext
										.isEmpty(fechaInicioExistenteDescuentoAprobado)
										&& fechaInicioDescuentoPublicadoWeb < fechaInicioExistenteDescuentoAprobado) {
									// Ha de ser mayor o igual que la fecha fin
									// descuento aprobado, si existe
									return HreRem
											.i18n('info.datefield.begin.date.pdw.msg.validacion');
								}
								if (!Ext
										.isEmpty(fechaFinExistenteDescuentoAprobado)
										&& fechaFinExistenteDescuentoAprobado < fechaFinDescuentoPublicadoWeb) {
									// Ha de ser mayor o igual que la fecha fin
									// aprobado venta web, si existe
									return HreRem
								...
								// La fecha de inicio
								if (Ext
										.isEmpty(fechaInicioDescuentoPublicadoWeb)) {
									// No puede estar vacÃ­a
									return HreRem
											.i18n('info.fecha.precios.msg.validacion');
								}
								if (!Ext
										.isEmpty(fechaFinExistenteDescuentoAprobado)
										&& fechaInicioDescuentoPublicadoWeb < fechaInicioExistenteDescuentoAprobado) {
									// Ha de ser menor o igual que la fecha
									// inicio descuento aprobado, si existe
									return HreRem
											.i18n('info.datefield.begin.date.pdw.msg.validacion');
								}
								...
								// // Ha de ser menor o igual que la fecha
								// inicio aprobado venta web, si existe
								// return
					...
					// Este mÃ©todo obtiene el valor del campo importe que se
					// estÃ¡ editando y comprueba las validaciones oportunas.
					...
					validatePreciosVigentes : function(value) {
						...
						var selected = grid.getSelectionModel().getSelection();
						// Obtener columna automÃ¡ticamente por 'dataindex =
						// importe'.
						var importeActualColumn = grid.columns[Ext.Array
								.indexOf(Ext.Array.pluck(grid.columns,
										'dataIndex'), 'importe')];
						...
						var importeMinimo = parseFloat(grid.getStore()
								.findRecord('codigoTipoPrecio',
										tipoMinimoAutorizado).getData().importe);
						var importeDescuentoAprobado = parseFloat(grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoDescuentoAprobado).getData().importe);
						var importeDecuentoPublicadoWeb = parseFloat(grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoDescuentoPublicadoWeb).getData().importe);
						var importeAprobadoVentaWeb = parseFloat(grid
								.getStore().findRecord('codigoTipoPrecio',
										tipoAprobadoVentaWeb).getData().importe);

						var codTipoPrecio = grid.codTipoPrecio;

						switch (codTipoPrecio) {
						case tipoMinimoAutorizado: // MÃ­nimo <= Aprobado venta
							// web.

							var importeActualMinimo = importeActualColumn
									.getEditor().value;
						...
						case tipoDescuentoAprobado: // Descuento aprobado <=
							...
							if (!Ext.isEmpty(importeActualDescuentoAprobado)
									&& !Ext
											.isEmpty(importeDecuentoPublicadoWeb)
							...
							// if(!Ext.isEmpty(importeActualDescuentoAprobado)
							// && !Ext.isEmpty(importeAprobadoVentaWeb) &&
							// (importeActualDescuentoAprobado >
							// importeAprobadoVentaWeb)){
							// return
						...
						case tipoDescuentoPublicadoWeb: // Descuento aprobado <=
							...
							var importeActualDescuentoWeb = importeActualColumn
									.getEditor().value;
						...
						case tipoAprobadoVentaWeb: // MÃ­nimo <= Aprobado venta
							// web.
					...
					// Este mÃ©todo desmarca el checkbox de formalizar cuando el
					// checkbox de comercializar se desmarca.
					onChkbxPerimetroChange : function(chkbx) {
						var me = this;
						var ref = chkbx.getReference();

						// Si se quita comercializar, hay que quitar tambien los
						// datos de formalizacion en perimetro
						...
						var chkbxFormalizar = me
								.lookupReference('chkbxPerimetroFormalizar');
						...
						case 'chkbxPerimetroComercializar':
							if (!Ext.isEmpty(chkbxPerimetroComercializar
									.getValue())
					...
					buscarProveedor : function(field, e) {
						...
						var data;
						var nifEmisorField = field.up('formBase').down(
								'[name=nifProveedor]');
						nombreProveedorField = field.up('formBase').down(
								'[name=nombreProveedor]'),
								subtipoProveedorField = field.up('formBase')
										.down('[name=subtipoProveedorField]');
									...
									.request({
										...
										success : function(response, opts) {
											...
											if (!Utils.isEmptyJSON(data.data)) {
												...
												if (!Ext
														.isEmpty(nifEmisorField)) {
													nifEmisorField
															.setValue(nifProveedor);
												}
												if (!Ext
														.isEmpty(nombreProveedorField)) {
													nombreProveedorField
															.setValue(nombreProveedor);
												}
												if (!Ext
														.isEmpty(subtipoProveedorField)) {
					...
					onClickBotonGuardarEntidad : function(btn) {
						...
						// me.fireEvent("infoToast",
						// HreRem.i18n("msg.operacion.ok"));
						// me.getView().unmask();
					...
					onSaveFormularioCompletoActivoIntegrado : function(btn,
							form) {
						var me = this;
							...
							if (!form.saveMultiple) {
								...
								if (Ext.isDefined(form.getBindRecord()
									...
									form.getBindRecord().getProxy().extraParams.idActivo = form
											.down('field[name=idActivo]')
											.getValue();
								}

								if (form.getBindRecord() != null
										&& (Ext.isDefined(form.getBindRecord()
												.getProxy().getApi().create) || Ext
												.isDefined(form.getBindRecord()
					...
					onRetenerPagosChange : function(combo, a, b) {
						...
						var motivoRetencionField = form
								.down('[name=motivoRetencion]');
					...
					cargarDatosActivoIntegrado : function(window) {
						...
						if (!Ext.isEmpty(id)) {
							...
							form.mask(HreRem.i18n("msg.mask.loading"));
							if (!form.saveMultiple) {
								model = form.getModelInstance(), model
										.setId(id);
								if (Ext
										.isDefined(model.getProxy().getApi().read)) {
									// Si la API tiene metodo de lectura (read).
									model
											.load({
												success : function(record) {
						...
						var form = window.down('formBase');
						form.setBindRecord(Ext
								.create('HreRem.model.ActivoIntegrado'));
						window.down('field[name=idActivo]').setValue(
								window.idActivo);

						Ext.Array.each(window.down('form').query(
					...
					beforeLoadLlaves : function(store, operation, opts) {
						var me = this, idActivo = this.getViewModel().get(
								'activo').id;
					...
					onLlavesListClick : function(grid, record) {
						...
						me.lookupReference('fieldsetmovimientosllavelist')
								.expand();
						me.lookupReference('movimientosllavelistref')
								.getStore().loadPage(1);
					...
					beforeLoadMovimientosLlave : function(store, operation,
							opts) {

						var me = this;
						if (!Ext
								.isEmpty(me.getViewModel().get('llaveslistref').selection)) {
							var idLlave = me.getViewModel()
									.get('llaveslistref').selection.id;

							if (idLlave != null
									&& Ext.isNumber(parseInt(idLlave))) {
								store.getProxy().extraParams = {
									idLlave : idLlave
								};
					...
					onClickEditRowMovimientosLlaveList : function(editor,
							context, eOpts) {
						var me = this;
					...
					// Llamar desde cualquier GridEditableRow, y asÃ­ se
					// desactivaran las ediciones.
					quitarEdicionEnGridEditablePorFueraPerimetro : function(
							grid, record) {
						var me = this;
					...
					// Este mÃ©todo filtra los anyos de construcciÃ³n y
					// rehabilitaciÃ³n de una vivienda
					// de modo que si el value es '0' lo quita. Es una medida de
					// protecciÃ³n al v-type
					// por que en la DB estÃ¡n establecidos a 0 todos los
					// activos.
					...
					valdacionesEdicionLlavesList : function(editor, grid) {
						var me = this, textMotivo = me
								.lookupReference('motivoIncompletoRef'), comboCompleto = me
						...
						var mostrarObligatoriedad = comboCompleto.getValue() == "0"
								&& (textMotivo.getValue() == null || textMotivo
										.getValue() == "");
						me.vaciarCampoMostrarRojoObligatoriedad(textMotivo,
								mostrarObligatoriedad,
								comboCompleto.getValue() == "1")
					...
					// Este mÃ©todo se usa para marcar en rojo el campo en
					// primera instancia, o vaciar su contenido
					...
					// Este mÃ©todo es llamado cuando se pulsa el botÃ³n 'ver'
					// del ID de visita en el detalle de una oferta
					...
					onClickMostrarVisita : function(btn) {
						...
						Ext.Ajax
								.request({
									url : $AC
											.getRemoteUrl('visitas/getVisitaById'),
									params : {
					...
					// Este mÃ©todo es llamado cuando se selecciona una oferta
					// del listado de ofertas del activo.
					// Obtiene el ID de la oferta y carga sus detalles en la
					// secciÃ³n 'Detalle ofertas'.
					onOfertaListClick : function(grid, record) {
						...
						var fieldset = me
								.lookupReference('detalleOfertaFieldsetref');
						fieldset.mask(HreRem.i18n("msg.mask.loading"));
						...
						storeOfertantes.load({
							success : function(record) {
								me.lookupReference(
										'ofertanteslistdetalleofertaref')
										.refresh();
							}
						});
						...
						storeHonorarios.load({
							success : function(record) {
								me.lookupReference(
										'honorarioslistdetalleofertaref')
										.refresh();
							}
						});
						...
						model.load({
							success : function(record) {
								me.getViewModel().set("detalleOfertaModel",
										record);
								fieldset.unmask();
							}
						});
					...
					// Este mÃ©todo abre el activo o agrupaciÃ³n asociado a la
					// oferta en el grid de ofertas del activo.
					onClickAbrirActivoAgrupacion : function(tableView,
							indiceFila, indiceColumna) {
						var me = this;
						var grid = tableView.up('grid');
						var record = grid.store.getAt(indiceFila);
						...
						if (Ext.isEmpty(record.get('idAgrupacion'))) {
							...
							me.redirectTo('activos', true);
							me.getView().fireEvent('abrirDetalleActivoOfertas',
									record);
						} else {
					...
					onClickBotonGuardarInfoFoto : function(btn) {
						...
						var fotosActuales = btn.up('tabpanel').getActiveTab()
								.down('dataview').getStore().data.items;
						for (i = 0; i < fotosActuales.length; i++) {
							if (form.getValues().id != fotosActuales[i].data.id
									&& form.getValues().principal) {
								console.log(i + " id"
										+ fotosActuales[i].data.id)
								...
								if (fotosActuales[i].data.principal == 'true'
										&& form.getValues().interiorExterior
												.toString() == fotosActuales[i].data.interiorExterior) {
									tienePrincipal = true;
									break;
								}
							}
						...
						if (!tienePrincipal) {
							...
							if (form.findField("nombre") != null) {
								params['nombre'] = form.findField("nombre")
										.getValue();
							}
							if (form.findField("principal") != null) {
								params['principal'] = form.findField(
										"principal").getValue();
							}
							if (form.findField("interiorExterior") != null) {
								params['interiorExterior'] = form.findField(
										"interiorExterior").getValue();
							}
							if (form.findField("orden") != null) {
								params['orden'] = form.findField("orden")
										.getValue();
							}
							if (form.findField("descripcion") != null) {
								params['descripcion'] = form.findField(
										"descripcion").getValue();
							}
							if (form.findField("fechaDocumento") != null) {
								params['fechaDocumento'] = form.findField(
										"fechaDocumento").getValue();
							}
									...
									.request({
										...
										success : function(a, operation,
											...
											btn
													.up('tabbar')
													.down(
															'button[itemId=botonguardar]')
													.hide();
											btn
													.up('tabbar')
													.down(
															'button[itemId=botoneditar]')
													.show();
											Ext.Array
													.each(
															btn
							...
							me.fireEvent("errorToast",
									"Ya dispone de una foto principal");
							btn.up('tabpanel').unmask();
						}
					},
					...
					onClickMostrarPrescriptorVisita : function(btn) {
						var me = this;
						var record = btn.up('visitascomercialdetalle')
								.getViewModel().get('detallevisita');
						...
						me.getView().fireEvent('abrirDetalleProveedorDirectly',
								idProveedor, titulo);
					},
					...
					onRenderCargasList : function(grid) {
						var me = this,
						// isCarteraCajamar =
						// me.getViewModel().get("activo.isCarteraCajamar"),
					...
					abrirFormularioAnyadirCarga : function(grid) {
						var me = this, record = Ext
								.create("HreRem.model.ActivoCargas");
						record.set("idActivo", me.getViewModel().get(
								"activo.id"));
					...
					onClickPropagation : function(btn) {
						var me = this;
						var idActivo = btn.up('tabpanel').getActiveTab()
								.getBindRecord().activo.id, url = $AC
									...
									success : function(response, opts) {
										...
										if (me.getViewModel() != null) {
											if (me.getViewModel().get('activo') != null) {
												if (me.getViewModel().get(
														'activo').data != null) {
													me.getViewModel().get(
															'activo').data.activosPropagables = activosPropagables;
												}
											}
										}
					...
					onClickBotonGuardarCarga : function(btn) {
						...
						var me = this, form = me
								.lookupReference("formDetalleCarga"), window = form
					...
					buscarPrescriptor : function(field, e) {
						...
						var me = this;
						var url = $AC
								.getRemoteUrl('proveedores/searchProveedorCodigo');
						var codPrescriptor = field.getValue();
						var data;
						var re = new RegExp(
								"^((04$))|^((18$))|^((28$))|^((29$))|^((31$))|^((37$))|^((30$))|^((35$))|^((23$))|^((38$)).*$");
						...
						Ext.Ajax
								.request({
									...
									success : function(response, opts) {
										data = Ext
												.decode(response.responseText);
										var buscadorPrescriptor = field.up(
												'formBase').down(
												'[name=buscadorPrescriptores]'), nombrePrescriptorField = field
												.up('formBase')
												.down(
										...
										if (!Utils.isEmptyJSON(data.data)) {
											...
											if (re.test(tipoProveedorCodigo)) {
												if (!Ext
														.isEmpty(buscadorPrescriptor)) {
													buscadorPrescriptor
															.setValue(codPrescriptor);
												}
												if (!Ext
														.isEmpty(nombrePrescriptorField)) {
													nombrePrescriptorField
															.setValue(nombrePrescriptor);
					...
					buscarSucursal : function(field, e) {
						...
						var carteraBankia = me.view.up().lookupController()
								.getViewModel().get('activo.isCarteraBankia');
						var carteraCajamar = me.view.up().lookupController()
								.getViewModel().get('activo.isCarteraCajamar');
						var codSucursal = '';
						var nombreSucursal = '';
						if (carteraBankia) {
					...
					onChangeFechasMinimaMovimientosLlaveList : function() {
						var me = this;
						var dateDevolucion = me
								.lookupReference('datefieldDevolucion');
						var dateEntrega = me
								.lookupReference('datefieldEntrega');

						me.setFechaMinimaDevolucionMovimientoLlave(dateEntrega
								.getValue(), dateDevolucion);
					},

					comprobarFechasMinimasMovimientosLlaveList : function(
						...
						me.setFechaMinimaDevolucionMovimientoLlave(
								fila.fechaEntrega, dateDevolucion);
					},

					// Establece fecha mÃ­nima en DevoluciÃ³n en funciÃ³n de la
					// fecha de Entrega
					setFechaMinimaDevolucionMovimientoLlave : function(
							valorFecha, dateDevolucion) {
					...
					onChkbxRevisionDeptoCalidadChange : function(btn) {
						var me = this;
						var nomGestorCalidad = me
								.lookupReference('nomGestorCalidad');
						var fechaRevisionCalidad = me
								.lookupReference('fechaRevisionCalidad');
					...
					onChangeEstadoCargaCombo : function(combo) {
						...
						if (CONST.SITUACION_CARGA['CANCELADA'] == combo
								.getSelection().get('codigo')) {
							fechaCancelacionRegistral.allowBlank = false;
						} else {
							fechaCancelacionRegistral.allowBlank = true;
							if (!Ext.isEmpty(fechaCancelacionRegistral
									.getValue())) {
					...
					onChangeEstadoEconomicoCombo : function(combo) {
						...
						if (CONST.SITUACION_CARGA['CANCELADA'] == combo
							...
							if (!Ext.isEmpty(fechaCancelacionEconomica
									.getValue())) {
					...
					saveActivo : function(jsonData, successFn) {
						var me = this, url = $AC
								.getRemoteUrl('activo/saveActivo');
						...
						if (Ext.isEmpty(jsonData)) {
							me
									.fireEvent("log",
											"Obligatorio jsonData para guardar el activo");
						} else {
					...
					onClickGuardarPropagarCambios : function(btn) {
						...
						if (Ext.isEmpty(targetGrid)) {
							if (!Ext.isEmpty(formActivo)) {
								var successFn = function(record, operation) {
									if (activosSeleccionados.length > 0) {
										...
										me.getView().unmask();
										me
												.refrescarActivo(formActivo.refreshAfterSave);
										...
										me
												.actualizarGridHistoricoDestinoComercial(formActivo);
							...
							if (targetGrid == 'mediadoractivo') {
								...
								me.saveActivo(me
										.createTabDataCondicionesEspecificas(
												activosSeleccionados,
												window.tabData), successFn);
							}
						}
					...
					/**
					 * Replica una operación ya realizada sobre un activo,
					 * utilizando el array de activos que recibe y llamandose
					 * recursivamente hasta que no quedan activos .
					 * 
					 * @param {}
					 *            config - url y parametros comunes para
					 *            guardar(datos modificados y tab)
					 * @param {}
					 *            activos
					 * @return {Boolean}
					 */
					propagarCambios : function(window, activos, jsonResponse) {
						...
						var me = this, grid = window.down("grid"), propagableData = window.propagableData, numTotalActivos = grid
								.getSelectionModel().getSelection().length + 1, targetGrid = window.targetGrid, numActivoActual = numTotalActivos;
						...
						if (activos.length > 0) {
							...
							if (Ext.isEmpty(targetGrid)) {
								...
								if (targetGrid == 'mediadoractivo') {
									...
									// Los lanzamos todos de golpe sin necesidad
									// de iterar
									activos = [];
								}
							}
							...
							var successFn = function(response, opts) {
								// Lanzamos el evento de refrescar el activo por
								// si está abierto.
								me.getView().fireEvent(
										"refreshEntityOnActivate",
										CONST.ENTITY_TYPES['ACTIVO'],
										activo.get("activoId"));
								me.manageToastJsonResponse(me,
							...
							window.mask("Guardando activos " + numActivoActual
									+ " de " + numTotalActivos);
							me.saveActivo(propagableData, successFn);

						} else {
							Ext.ComponentQuery
									.query('opcionespropagacioncambios')[0]
									.destroy();
							me.getView().unmask();
							return false;
						}
					...
					createTabData : function(form) {
						...
						if (form.saveMultiple) {
							...
							Ext.Array.each(form.getBindRecords(), function(
									record, index) {
								var model = me.createModelToSave(record,
										types[index]);
								if (!Ext.isEmpty(model)) {
							...
							var model = me.createModelToSave(form
									.getBindRecord(), type);
							if (!Ext.isEmpty(model)) {
								tabData.models.push(model);
							}
					...
					createTabDataCondicionesEspecificas : function(
							listadoActivos, data) {
						var me = this, tabData = {};
						tabData.id = me.getViewModel().get("activo.id");
						tabData.models = [];
					...
					createModelToSave : function(record, type) {
						...
						if (Ext.isDefined(record.getProxy().getApi().update)) {
							...
							model.type = type;
							model.data = record.getProxy().getWriter()
					...
					createFormPropagableData : function(form, tabData) {
						...
						Ext.Array.each(records, function(record, index) {
							...
							camposPropagables[name] = record
									.get("camposPropagables");
						});

						Ext.Array.each(tabData.models, function(model, index) {
							var data = {}, modelHasData = false;
							Ext.Array.each(camposPropagables[model.name],
					...
					onClickBotonGuardarMotivoRechazo : function(btn) {
						...
						if (grid.isValidRecord(record)) {
							...
							// TODO: Encontrar la manera de realizar esto que me
							// ha obligado a
							// duplicar este save del record y en este punto
							// "editor" es indefinido
							// editor.isNew = false;
						} else {
							grid.getStore().load();
						}
					...
					onClickBotonGuardarDistribucion : function(btn) {
						...
						window.gridDistribuciones.up('informecomercialactivo')
								.funcionRecargar();
						window.close();
					},
					...
					onChangeComboMotivoOcultacionVenta : function() {
						var me = this;
						var combo = me
								.lookupReference('comboMotivoOcultacionVenta');
						var textArea = me
								.lookupReference(combo.textareaRefChained);

						if (combo
								&& combo.value === CONST.MOTIVO_OCULTACION['OTROS']) {
					...
					onChangeComboMotivoOcultacionAlquiler : function() {
						var me = this;
						var combo = me
								.lookupReference('comboMotivoOcultacionAlquiler');
						var textArea = me
								.lookupReference(combo.textareaRefChained);
					...
					onChangeCheckboxOcultar : function(checkbox, isDirty) {
						var me = this;
						var combobox = me
								.lookupReference(checkbox.comboRefChained);
						var textarea = me
								.lookupReference(combobox.textareaRefChained);
					...
					onActivateTabDatosPublicacion : function(tab, eOpts) {
						...
						me.getViewModel().get(
								'filtrarComboMotivosOcultacionVenta');
						me.getViewModel().get(
								'filtrarComboMotivosOcultacionAlquiler');
					},

					onChangeCheckboxPublicarVenta : function(checkbox, isDirty) {

						var me = this;
						var estadoPubVentaPublicado = me.getViewModel().get(
								'activo').getData().estadoVentaCodigo === CONST.ESTADO_PUBLICACION_VENTA['PUBLICADO']
								|| me.getViewModel().get('activo').getData().estadoVentaCodigo === CONST.ESTADO_PUBLICACION_VENTA['OCULTO'];
						var textarea = me
								.lookupReference(checkbox.textareaRefChained);

						if (!isDirty && estadoPubVentaPublicado) {
							var readOnly = Ext
									.isEmpty(me.getViewModel().get(
											'datospublicacionactivo').getData().precioWebVenta)
									&& !checkbox.getValue();
					...
					onChangeCheckboxPublicarAlquiler : function(checkbox,
							isDirty) {
						var me = this;
						if (checkbox.getValue()
								&& me
										.getViewModel()
										.get(
												'debePreguntarPorTipoPublicacionAlquiler')) {
							Ext
									.create(
											'HreRem.view.activos.detalle.VentanaEleccionTipoPublicacion')
									.show();
						...
						var estadoPubAlquilerPublicado = me.getViewModel().get(
								'activo').getData().estadoAlquilerCodigo === CONST.ESTADO_PUBLICACION_ALQUILER['PUBLICADO']
					...
					onChangeCheckboxPublicarSinPrecioVenta : function(checkbox,
							isDirty) {
						var me = this;
						var estadoCheckPublicarFicha = me.getViewModel().get(
								'activo.aplicaPublicar');
						var checkboxPublicarVenta = checkbox.up(
								'activosdetallemain').lookupReference(
								'chkbxpublicarventa');
						var estadoPubVentaPublicado = me.getViewModel().get(
								'activo').getData().estadoVentaCodigo === CONST.ESTADO_PUBLICACION_VENTA['PUBLICADO']
					...
					onChangeCheckboxPublicarSinPrecioAlquiler : function(
							checkbox, isDirty) {
						var me = this;
						var estadoCheckPublicarFicha = me.getViewModel().get(
								'activo.aplicaPublicar');
						var checkboxPublicarAlquiler = checkbox.up(
								'activosdetallemain').lookupReference(
								'chkbxpublicaralquiler');
						var estadoPubAlquilerPublicado = me.getViewModel().get(
								'activo').getData().estadoAlquilerCodigo === CONST.ESTADO_PUBLICACION_ALQUILER['PUBLICADO']
					...
					establecerTipoPublicacionAlquiler : function(btn) {
						var me = this;
						var list = Ext.ComponentQuery
								.query('activosdetallemain');
					...
					onGridImpuestosActivoRowClick : function(grid, record, tr,
							rowIndex) {
						grid.up().disableRemoveButton(false);
					},

					onImpuestosActivoDobleClick : function(grid, record, tr,
							rowIndex) {
						var me = this, record = grid.getStore().getAt(rowIndex);
					},
						...
						Ext.Ajax
								.request({
									...
									success : function(response, opts) {
										...
										if (activosPropagables.length > 0
												&& tabData.models[0].name != "datospublicacion") {
											tabPropagableData = me
													.createFormPropagableData(
															form, tabData);
											if (!Ext.isEmpty(tabPropagableData)) {
												// sacamos el activo actual del
												// listado
												var activo = activosPropagables
														.splice(
																activosPropagables
	...
	existeCliente: function(btn){
		...
		if(form.isValid()){
			...
			if(!Ext.isEmpty(btn.up('wizardaltaoferta'))){
				...
				if(!Ext.isEmpty(btn.up('wizardaltaoferta').oferta.data.idAgrupacion)){
			...
			Ext.Ajax.request({
	    		url: url,
				method : 'POST',
	    		params: {idActivo: idActivo,idAgrupacion: idAgrupacion,idExpediente: idExpediente,dniComprador:dniComprador,codtipoDoc:codtipoDoc},
	    		success: function(response, opts){
	    			...
	    			if(!Ext.isEmpty(btn.up('wizardaltaoferta'))){
	        			...
	        			if(!Ext.isEmpty(comprador)){
	            			...
	            			if(!Ext.isEmpty(comprador.tipoPersonaCodigo)){
	            				ventanaAnyadirOferta.getForm().findField('comboTipoPersona').setValue(comprador.tipoPersonaCodigo);
	            				ventanaAnyadirOferta.getForm().findField('comboTipoPersona').setDisabled('disabled');
	            			}
	            			if(!Ext.isEmpty(comprador.estadoCivilCodigo)){
	            				ventanaAnyadirOferta.getForm().findField('comboEstadoCivil').setValue(comprador.estadoCivilCodigo);
	            			...
	            			if(!Ext.isEmpty(comprador.regimenMatrimonialCodigo)){
	            				ventanaAnyadirOferta.getForm().findField('comboRegimenMatrimonial').setValue(comprador.regimenMatrimonialCodigo);
	        			...
	        			ventanaWizard.height = Ext.Element.getViewportHeight() > 600 ? 600 : Ext.Element.getViewportHeight() -100;
	        			ventanaWizard.setY( Ext.Element.getViewportHeight()/2 - ((Ext.Element.getViewportHeight() > 600 ? 600 : Ext.Element.getViewportHeight() -100)/2));
	    				...
	    				if(!Ext.isEmpty(btn.up('wizardaltacomprador'))){ 
	    				   ...
	    				   if(!Ext.isEmpty(pedirDoc)){
	    					   ventanaAltaWizard.getForm().findField('pedirDoc').setValue(pedirDoc);
	           				   if(pedirDoc == "true"){
	           					  ventanaAltaWizard.down('button[itemId=btnCrear]').setText("Crear");
	           				   }else{
