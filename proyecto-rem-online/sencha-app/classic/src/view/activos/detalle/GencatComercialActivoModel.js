    ...
    formulas: {
    	...
    	esSoloLecturaCheckAnularGencat: function(get){
    		...
    		if (!comunicadoAnulacionGencat) {
				// Si estadoComunicacion es COMUNICADO / RECHAZADO / ANULADO o
				// bien SANCIONADO + estadoSancion NO EJERCE, y además el
				// usuario tiene perfil HAYAGESTFORMADM / HAYASUPER
				if ((estadoComunicacion === CONST.ESTADO_COMUNICACION_GENCAT['COMUNICADO'])
						&& ($AU.userIsRol(CONST.PERFILES['HAYAGESTFORMADM']) || $AU
								.userIsRol(CONST.PERFILES['HAYASUPER']))) {
					soloLectura = false
				}
			}
    		return soloLectura;
