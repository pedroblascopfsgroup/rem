Ext.define('HreRem.view.activos.detalle.Publicacion', {
		...
		var editionEnabled = function(tab) {
			...
			if(tab.xtype=='informecomercialactivo'){
				if(me.lookupController().getViewModel().get('activo').get('claseActivoCodigo')=='01'){
					visible = (($AU.userIsRol(CONST.PERFILES['GESTOPDV']) || $AU.userIsRol(CONST.PERFILES['HAYASUPER']) || $AU.userIsRol(CONST.PERFILES['HAYACAL'])) 
    ...
    initComponent: function () {
		...
		if(me.lookupController().getViewModel().get('activo').get('claseActivoCodigo')=='01'){
			ocultarInformecomercialactivo = !(($AU.userIsRol(CONST.PERFILES['GESTOPDV']) || $AU.userIsRol(CONST.PERFILES['HAYASUPER']) || $AU.userIsRol(CONST.PERFILES['HAYACAL'])) 
				 && $AU.userHasFunction('EDITAR_TAB_INFO_COMERCIAL_PUBLICACION'));
		}else{
		...
		if(me.lookupController().getViewModel().get('activo').get('claseActivoCodigo')=='01'){
			ocultarDatospublicacionactivo = !(($AU.userIsRol(CONST.PERFILES['GESTOPDV']) || $AU.userIsRol(CONST.PERFILES['HAYASUPER']) || $AU.userIsRol(CONST.PERFILES['HAYACAL'])) 
				 && $AU.userHasFunction('EDITAR_TAB_DATOS_PUBLICACION'));
		}else{
