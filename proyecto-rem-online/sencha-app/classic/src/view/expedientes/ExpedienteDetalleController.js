Ext
		.define(
				'HreRem.view.expedientes.ExpedienteDetalleController',
				{
					...
					requires : [
							'HreRem.view.expedientes.NotarioSeleccionado',
							'HreRem.view.expedientes.DatosComprador',
					...
					onRowClickListadoactivos : function(gridView, record) {
						...
						var expedienteDeAlquiler = me.getViewModel().get(
								'expediente.tipoExpedienteDescripcion');

						viewModel.set("activoExpedienteSeleccionado", record);
						viewModel.notify();

						if (me.lookupReference('activoExpedienteMain').down(
								'bloqueosformalizacionlist')) {
							me.lookupReference('activoExpedienteMain').down(
									'bloqueosformalizacionlist').getStore()
									.load();
						}

						var bloqueado = me.getViewModel().get(
								'expediente.bloqueado');
						if (me.lookupReference('activoExpedienteMain') != undefined) {
							me
									.lookupReference('activoExpedienteMain')
									.bloquearExpediente(
											me
													.lookupReference('activoExpedienteMain'),
											bloqueado);
						...
						HreRem.model.ActivoExpedienteCondicionesModel.load(
								idExpediente, {
									params : {
										idActivo : idActivo,
									...
									success : function(condiciones) {
										me.getViewModel().set("condiciones",
												condiciones);
										tabPanel.unmask();
									},
						...
						HreRem.model.ExpedienteInformeJuridico.load(
								idExpediente, {
									params : {
										idActivo : idActivo
									},
						...
						var panelTanteo = tabPanel
								.down('activoexpedientetanteo');
						...
						var panelJuridico = tabPanel
								.down('activoexpedientejuridico');
						if (panelJuridico != undefined) {
							me.cargarTabDataInformeJuridico(panelJuridico,
									false);
						}
					},
					...
					cargarTabData : function(form) {
						var me = this, model = null, models = null, nameModels = null, id = me
								.getViewModel().get("expediente.id");
								...
								model
										.load({
											success : function(record) {
												var itemsExpedienteForm = form
														.getForm().getFields().items;
												var itemReserva = itemsExpedienteForm
														.find(function(item) {
															return item.fieldLabel === "Fecha de reserva"
														});
												if (itemReserva != null) {
													me
															.tareaDefinicionDeOferta(itemReserva);
												}
					...
					cargarTabDataInformeJuridico : function(form,
							refreshActivoExpediente) {
						var me = this, model = null, models = null, nameModels = null, id = me
								.getViewModel().get("expediente.id");
						var tabPanel = me
								.lookupReference('activoExpedienteMain');
						// Si la API tiene metodo de lectura (read).
						HreRem.model.ExpedienteInformeJuridico
								.load(
										id,
										{
											params : {
												idActivo : me
														.getViewModel()
														.get(
																"activoExpedienteSeleccionado.idActivo")
											},
											scope : this,
											success : function(informeJuridico) {
												me.getViewModel().set(
					...
					cargarTabDataMultiple : function(form, index, models,
						...
						var me = this, id = me.getViewModel().get(
								"expediente.id");
											...
											.save(
													{
														success : function(a,
															...
															me.getView()
																	.unmask();
															me
																	.refrescarExpediente(form.refreshAfterSave);
														},
					...
					onSaveFormularioCompletoActivoExpediente : function(btn,
							form) {
						var me = this;
						// disableValidation: Atributo para indicar si el
						// guardado del formulario debe aplicar o no, las
						...
						if (form.isFormValid() && form.disableValidation) {
							...
							if (!form.saveMultiple) {
											...
											.save(
													{
														...
														success : function(a,
															...
															me.getView()
																	.unmask();
															me
																	.refrescarActivoExpediente(form.refreshAfterSave);
														},
								...
								var records = form.getBindRecords();
								var contador = 0;
								me.saveMultipleRecordsActivoExpediente(
										contador, records);
							}
						} else {
					...
					saveMultipleRecordsActivoExpediente : function(contador,
							records) {
						var me = this;
									...
									.save({
										...
										success : function(a, operation, c) {
											...
											if (contador < records.length) {
												...
												me.getView().unmask();
												me
														.refrescarActivoExpediente(false);
											}
										},
										failure : function(a, operation) {
											me.fireEvent("errorToast", HreRem
					...
					onClickBotonGuardarActivoExpediente : function(btn) {
						...
						if (btn.up('tabpanel').getActiveTab().getReference() == "activoexpedientetanteo") {
							me.onSaveFormularioActivoExpedienteTanteo(btn, btn
									.up('tabpanel').getActiveTab());
						} else {
							me.onSaveFormularioCompletoActivoExpediente(btn,
									btn.up('tabpanel').getActiveTab());
						}
					...
					onSaveFormularioActivoExpedienteTanteo : function(btn, form) {
						var me = this;
						if (form.isFormValid()) {
							var formulario = btn.up('tabpanel').getActiveTab()
									.getForm();
							me.getView().mask(HreRem.i18n("msg.mask.loading"));
							var url = $AC
									.getRemoteUrl('expedientecomercial/saveTanteo');
							var params = {};
							if (formulario.findField("condicionesTransmision") != null) {
								params['condicionesTransmision'] = formulario
										.findField("condicionesTransmision")
										.getValue();
							}
							if (formulario.findField("fechaComunicacion") != null) {
								params['fechaComunicacion'] = formulario
										.findField("fechaComunicacion")
										.getValue();
							}
							if (formulario.findField("fechaRespuesta") != null) {
								params['fechaRespuesta'] = formulario
										.findField("fechaRespuesta").getValue();
							}
							if (formulario.findField("fechaSolicitudVisita") != null) {
								params['fechaSolicitudVisita'] = formulario
										.findField("fechaSolicitudVisita")
										.getValue();
							}
							if (formulario.findField("fechaVisita") != null) {
								params['fechaVisita'] = formulario.findField(
										"fechaVisita").getValue();
							}
							if (formulario.findField("fechaFinTanteo") != null) {
								params['fechaFinTanteo'] = formulario
										.findField("fechaFinTanteo").getValue();
							}
							if (formulario.findField("fechaResolucion") != null) {
								params['fechaResolucion'] = formulario
										.findField("fechaResolucion")
										.getValue();
							}
							if (formulario.findField("fechaVencimiento") != null) {
								params['fechaVencimiento'] = formulario
										.findField("fechaVencimiento")
										.getValue();
							}
							if (formulario.findField("codigoTipoResolucion") != null) {
								params['codigoTipoResolucion'] = formulario
										.findField("codigoTipoResolucion")
										.getValue();
							}
							if (formulario.findField("numeroExpediente") != null) {
								params['numeroExpediente'] = formulario
										.findField("numeroExpediente")
										.getValue();
							}
							if (formulario.findField("solicitaVisitaCodigo") != null) {
								params['solicitaVisitaCodigo'] = formulario
										.findField("solicitaVisitaCodigo")
										.getValue();
							}
					...
					/**
					 * Función que refresca la pestaña activa, y marca el resto
					 * de componentes para referescar. Para que un componente
					 * sea marcado para refrescar, es necesario que implemente
					 * la función funciónRecargar con el código necesario para
					 * refrescar los datos.
					 */
					...
					refrescarActivoExpediente : function(refrescarTabActiva) {
						...
						activeTab.up("expedientedetallemain").fireEvent(
								"refrescarExpediente",
								activeTab.up("expedientedetallemain"));
						var activoExpedienteMain = activeTab
								.up('activosexpediente');
						var grid = activoExpedienteMain
								.down('gridBaseEditableRow');
						if (grid) {
							var store = grid.getStore();
							// grid.expand();
							store.loadPage(1)
					...
					onEnlaceActivosClick : function(tableView, indiceFila,
						...
						me.getView().fireEvent('abrirDetalleActivoPrincipal',
								record.get('idActivo'));
					},
					...
					downloadDocumentoAdjunto : function(grid, record) {
						...
						config.url = $AC.getWebPath()
								+ "expedientecomercial/bajarAdjuntoExpediente."
								+ $AC.getUrlPattern();
						config.params = {};
					...
					onCompradoresListDobleClick : function(gridView, record) {
						var me = this, codigoEstado = me.getViewModel().get(
								"expediente.codigoEstado"), fechaPosicionamiento = me
								.getViewModel().get(
										"expediente.fechaPosicionamiento"), tipoExpedienteAlquiler = CONST.TIPOS_EXPEDIENTE_COMERCIAL["ALQUILER"], tipoExpedienteVenta = CONST.TIPOS_EXPEDIENTE_COMERCIAL["VENTA"];
						me.getView().fireEvent("refrescarExpediente",
								me.getView());
						if ((codigoEstado != CONST.ESTADOS_EXPEDIENTE['VENDIDO'] && me
								.getViewModel().get(
										'expediente.tipoExpedienteCodigo') === tipoExpedienteVenta)
								|| (me.getViewModel().get(
										'expediente.tipoExpedienteCodigo') === tipoExpedienteAlquiler && Ext
										.isEmpty(fechaPosicionamiento))) {
							var idCliente = record.get("id"), expediente = me
									.getViewModel().get("expediente");
							var storeGrid = gridView.store;
							var edicion = me.getViewModel().get(
									"puedeModificarCompradores");
							var deshabilitarCamposDoc = false;
							if (me.getViewModel().get(
									'expediente.tipoExpedienteCodigo') === tipoExpedienteAlquiler) {
							...
							Ext
									.create(
											"HreRem.view.expedientes.DatosComprador",
											{
												...
												deshabilitarCamposDoc : deshabilitarCamposDoc
											}).show();
						}
						if (me.getViewModel().get(
								'expediente.tipoExpedienteCodigo') === tipoExpedienteAlquiler
								&& !Ext.isEmpty(fechaPosicionamiento)) {
							me
									.fireEvent("errorToast",
											"Se ha avanzado la tarea Posicionamiento, no se puede editar los inquilinos");
						}
					},
					esEditableCompradores : function(field) {
						...
						if (viewModel.get('esCarteraBankia')) {
							if ((viewModel.get('expediente.codigoEstado') != CONST.ESTADOS_EXPEDIENTE['FIRMADO'] && viewModel
									.get('expediente.codigoEstado') != CONST.ESTADOS_EXPEDIENTE['VENDIDO'])
									&& $AU
											.userHasFunction([ 'EDITAR_TAB_COMPRADORES_EXPEDIENTES' ])) {
								field.topBar = true;
					...
					onHaCambiadoSolicitaFinanciacion : function(combo, value) {
						var me = this, disabled = value == 0, entidadFinanciacion = me
								.lookupReference('entidadFinanciacion');
						numExpedienteRiesgo = me
								.lookupReference('numExpedienteRiesgo');
						comboTipoFinanciacion = me
								.lookupReference('comboTipoFinanciacion');
					...
					onHaCambiadoTipoCalculo : function(combo, value) {
						var me = this;
						var valorCombo = combo.getValue(), porcentajeReserva = me
								.lookupReference('porcentajeReserva'), importeReserva = me
								.lookupReference('importeReserva'), plazoParaFirmar = me
								.lookupReference('plazoFirmaReserva');

						if (CONST.TIPOS_CALCULO['PORCENTAJE'] == valorCombo) {
							if (Ext.isEmpty(porcentajeReserva.getValue())) {
								me.getViewModel().get('condiciones').set(
										'importeReserva', null);
							}
					...
					onHaCambiadoPorcentajeReserva : function(combo, value) {
						var me = this, importeOferta = parseFloat(
								me.getViewModel().get('expediente.importe'))
								.toFixed(2), importeReserva = null, importeReservaField = me
								.lookupReference('importeReserva'), tipoCalculo = me
								.lookupReference('tipoCalculo').getValue();
					...
					onHaCambiadoPlusvalia : function(combo, value) {
						...
						porCuentaDe = me
								.lookupReference('plusvaliaPorCuentaDe');
					...
					onHaCambiadoNotaria : function(combo, value) {
						...
						notariaPorCuentaDe = me
								.lookupReference('notariaPorCuentaDe');
					...
					onHaCambiadoCompraVentaOtros : function(combo, value) {
						...
						compraventaOtrosPorCuentaDe = me
								.lookupReference('compraventaOtrosPorCuentaDe');
					...
					onHaCambiadoProcedeDescalificacion : function(combo, value) {
						...
						procedeDescalificacionPorCuentaDe = me
								.lookupReference('procedeDescalificacionPorCuentaDe');

						if (value == 1) {
							procedeDescalificacionPorCuentaDe
									.setDisabled(false);
					...
					onHaCambiadoLicencia : function(combo, value) {
						...
						licenciaPorCuentaDe = me
								.lookupReference('licenciaPorCuentaDe');
					...
					onHaCambiadoCargasPendientesOtros : function(combo, value) {
						...
						cargasPendientesOtrosPorCuentaDe = me
								.lookupReference('cargasPendientesOtrosPorCuentaDe');
					...
					// var criteria = Ext.apply(initialData, searchForm ?
					...
					onHaCambiadoComunidad : function(combo, value) {
						...
						porCuentaDe = me
								.lookupReference('comunidadPorCuentaDe');
					...
					onHaCambiadoAlquilerSuministros : function(combo, value) {
						...
						porCuentaDe = me
								.lookupReference('suministrosPorCuentaDe');
					...
					cargarDatosComprador : function(window) {
						var me = this, model = null, models = null, nameModels = null, id = window.idComprador, idExpediente = window.expediente
								.get("id");
						if (!Ext.isEmpty(id)) {
							...
							form.setBindRecord(Ext
									.create('HreRem.model.FichaComprador'));
						}
					},
					...
					onClickBotonModificarComprador : function(btn) {
						...
						me.onSaveFormularioCompletoComprador(form, success,
								failure);
					},

					onSaveFormularioCompletoComprador : function(form, success,
							failure) {
						var me = this, datoscom = form.up(), storeGrid = datoscom.storeGrid, record = form
								.getBindRecord();
						success = success
								|| function() {
									me.fireEvent("infoToast", HreRem
						...
						if (form.isFormValid()) {
							var idExpedienteComercial = record
									.get("idExpedienteComercial");
							var numeroClienteUrsus = record
									.get("numeroClienteUrsus");
							var nombreRazonSocial = record
							...
							var documentoConyuge = record
									.get("documentoConyuge");
							var relacionHre = record.get("relacionHre");

							var numDocumentoRte = record.get("numDocumentoRte");
							var nombreRazonSocialRte = record
									.get("nombreRazonSocialRte");
							...
							record
									.save({
										params : {
											idExpedienteComercial : idExpedienteComercial,
					...
					onCreateFormularioComprador : function(idExpediente, form,
							success, failure) {
						var me = this;
						...
						if (form.isFormValid()) {
							...
							if (Ext.isDefined(form.getModelInstance()
									.getProxy().getApi().create)) {
					...
					onHaCambiadoDestinoActivo : function(combo, value) {
						var me = this, otrosDetallePbc = me
								.lookupReference('otrosDetallePbc');
					...
					tareaDefinicionDeOferta : function(rec) {
						var me = this;
						var url = $AC
								.getRemoteUrl('expedientecomercial/getTareaDefinicionDeOferta');
						Ext.Ajax
								.request({

									url : url,
									params : {
										idExpediente : me.getViewModel().get(
												"expediente.id")
									}

									,
									success : function(response, a, operation,
											context) {
										var data = {};
										try {
											data = Ext
										...
										if (label != "venta" && label != "null") {
											...
											rec
													.setFieldLabel(HreRem
															.i18n('fieldlabel.fecha.reserva'));
										}
									},
					...
					onMarcarPrincipalClick : function(grid, rowIndex, colIndex,
							item, e, rec) {
						var me = this;
						me.gridOrigen = grid;
						...
						if (rec.data.titularContratacion != 1) {
							Ext.Msg
									.show({
										title : HreRem
												.i18n('title.confirmar.comprador.principal'),
										msg : HreRem
												.i18n('msg.confirmar.comprador.principal'),
										buttons : Ext.MessageBox.YESNO,
										fn : function(buttonId) {
											if (buttonId == 'yes') {
												me.getView().mask();
												var url = $AC
														.getRemoteUrl('expedientecomercial/marcarCompradorPrincipal');
												Ext.Ajax
														.request({
					...
					onClickConsultaFormalizacionBankia : function(btn) {
						var me = this;
						var tipoRiesgoCodigo = me.lookupReference(
								'comboTipoFinanciacion').getValue();
						var numExpedienteRiesgo = me.lookupReference(
								'numExpedienteRiesgo').getValue();
						var url = $AC
								.getRemoteUrl('expedientecomercial/obtencionDatosPrestamo');
					...
					onHaCambiadoFechaInicioFinanciacion : function(field,
							value, oldValue) {
						var me = this;
						var fechaInicioFinanciacion = value;
						var fechaFinFinanciacion = me
								.lookupReference('fechaFinFinanciacion').value;
						if (!Ext.isEmpty(fechaFinFinanciacion)
								&& !Ext.isEmpty(fechaInicioFinanciacion)
								&& fechaInicioFinanciacion > fechaFinFinanciacion) {
							me.fireEvent("errorToast", HreRem
									.i18n("msg.fechaFin.mayor.Fecha.Inicio"));
							field.setValue('');
						}
						var fieldFechaFinFinanciacion = me
								.lookupReference('fechaFinFinanciacion');
						fieldFechaFinFinanciacion
								.setMinValue(fechaInicioFinanciacion);
					},

					onHaCambiadoFechaInicioFinanciacionBankia : function(field,
							value, oldValue) {
						var me = this;
						var fechaInicioFinanciacionBankia = value;
						var fechaFinFinanciacionBankia = me
								.lookupReference('fechaFinFinanciacionBankia').value;
						if (!Ext.isEmpty(fechaFinFinanciacionBankia)
								&& !Ext.isEmpty(fechaInicioFinanciacionBankia)
								&& fechaInicioFinanciacionBankia > fechaFinFinanciacionBankia) {
							me.fireEvent("errorToast", HreRem
									.i18n("msg.fechaFin.mayor.Fecha.Inicio"));
							field.setValue('');
						}
						var fieldFechaFinFinanciacionBankia = me
								.lookupReference('fechaFinFinanciacionBankia');
						fieldFechaFinFinanciacionBankia
								.setMinValue(fechaInicioFinanciacionBankia);
					},

					onHaCambiadoFechaFinFinanciacion : function(field, value,
							oldValue) {
						var me = this;
						var fechaFinFinanciacion = value;
						var fechaInicioFinanciacion = me
								.lookupReference('fechaInicioFinanciacion').value;
						if (!Ext.isEmpty(fechaInicioFinanciacion)
								&& !Ext.isEmpty(fechaFinFinanciacion)
								&& fechaInicioFinanciacion > fechaFinFinanciacion) {
							me.fireEvent("errorToast", HreRem
									.i18n("msg.fechaFin.mayor.Fecha.Inicio"));
							field.setValue('');
					...
					onHaCambiadoFechaFinFinanciacionBankia : function(field,
							value, oldValue) {
						var me = this;
						var fechaFinFinanciacionBankia = value;
						var fechaInicioFinanciacionBankia = me
								.lookupReference('fechaInicioFinanciacionBankia').value;
						if (!Ext.isEmpty(fechaInicioFinanciacionBankia)
								&& !Ext.isEmpty(fechaFinFinanciacionBankia)
								&& fechaInicioFinanciacionBankia > fechaFinFinanciacionBankia) {
							me.fireEvent("errorToast", HreRem
									.i18n("msg.fechaFin.mayor.Fecha.Inicio"));
							field.setValue('');
					...
					onTipoComisionSelect : function(combo, recordSelected) {
						var me = this;
						var comboTipoProveedor = me
								.lookupReference('tipoProveedorRef');
							...
							storeTipoProveedor
									.filter([ {
										filterFn : function(rec) {
											if (rec.get('codigo') == CONST.TIPO_PROVEEDOR_HONORARIO['MEDIADOR']
													|| rec.get('codigo') == CONST.TIPO_PROVEEDOR_HONORARIO['FVD']) {
												return true;
											}
											return false;
										}
					...
					onSelectComboActivoHonorarios : function(combo,
							recordSelected) {

						var me = this;
						var grid = me.lookupReference('listadohoronarios');
						var record = Ext.isDefined(grid.rowEditing.context) ? grid.rowEditing.context.record
								: null;
						var importeParticipacionActivo = recordSelected
								.get("importeParticipacion");
						...
						if (Ext.isEmpty(importeParticipacionActivo)) {
							...
							me
									.fireEvent(
											"errorToast",
											HreRem
													.i18n("msg.necesaria.participacion.calculo.honorario"));
						} else {
							record.set("participacionActivo",
									importeParticipacionActivo);
						}

					},

					onHaCambiadoImporteCalculo : function(field, value,
							oldValue) {
						var me = this;
						var tipoCalculoField = me
								.lookupReference('tipoCalculoHonorario')
						var importeField = me
								.lookupReference('importeCalculoHonorario')
						var tipoCalculo = me
								.lookupReference('tipoCalculoHonorario').value;
						var record = Ext
								.isDefined(me
										.lookupReference('listadohoronarios').rowEditing.context) ? me
								.lookupReference('listadohoronarios').rowEditing.context.record
								: null;

						if (CONST.TIPOS_CALCULO['FIJO'] == tipoCalculo) {// importe
																			// fijo
							var honorarios = me.lookupReference('honorarios');
							var importeCalculoHonorario = me
									.lookupReference('importeCalculoHonorario').value;
							...
							var importeCalculoHonorario = me
									.lookupReference('importeCalculoHonorario').value;
							var importeParticipacion = Ext.isEmpty(record) ? null
									: record.get("participacionActivo");
							var honorario;
							if (!Ext.isEmpty(record)
									&& Ext.isEmpty(importeParticipacion)) {
								me
										.fireEvent(
												"errorToast",
												HreRem
								...
								importeParticipacion = parseFloat(
										importeParticipacion).toFixed(2);
								honorario = (importeParticipacion * importeCalculoHonorario) / 100;
								honorarios
										.setValue(Math.round(honorario * 100) / 100);
								importeField.setMaxValue(100);
							}
							me.fireEvent("log", "[HONORARIOS: Tipo: "
									+ tipoCalculo + " | Calculo: "
									+ importeCalculoHonorario
									+ " | Participacion: "
									+ importeParticipacion + " | Importe: "
									+ Math.round(honorario * 100) / 100 + "]");
						}

						else if (CONST.TIPOS_CALCULO['FIJO_ALQ'] == tipoCalculo) {// Importe
																					// fijo
																					// Alquiler
							var honorarios = me.lookupReference('honorarios');
							var importeCalculoHonorario = me
						...
						else if (CONST.TIPOS_CALCULO['PORCENTAJE_ALQ'] == tipoCalculo) {// Porcentaje
																						// Alquiler
							var honorarios = me.lookupReference('honorarios');
							var importeCalculoHonorario = me
									.lookupReference('importeCalculoHonorario').value;
						...
						else if (CONST.TIPOS_CALCULO['MENSUALIDAD_ALQ'] == tipoCalculo) {// Mensualidad
																							// Alquiler
							var honorarios = me.lookupReference('honorarios');
							var importeCalculoHonorario = me
									.lookupReference('importeCalculoHonorario').value;
							...
							if (!Ext.isEmpty(record)
								...
								importeParticipacion = parseFloat(
										importeParticipacion).toFixed(2);
								honorario = importeParticipacion
										* importeCalculoHonorario;
								honorarios
										.setValue(Math.round(honorario * 100) / 100);
								importeField.setMaxValue(100);
					...
					onHaCambiadoSolicitaReserva : function(combo, value) {
						var me = this;
						var carteraCodigo = me.getViewModel().get(
								'expediente.entidadPropietariaCodigo');
						var esCarteraGaleonOZeus = ('15' == carteraCodigo || '14' == carteraCodigo);
						if (!esCarteraGaleonOZeus && value == 1) {
							me.lookupReference('tipoCalculo')
									.setDisabled(false);
						} else {
					...
					abrirFormularioCrearComprador : function(grid) {
						...
						var me = this, idExpediente = me.getViewModel().get(
								...
								.getViewModel().get(
										"expediente.tipoExpedienteCodigo"), origen = me
								.getViewModel().get("expediente.origen"), bloqueado = me
								.getViewModel().get("expediente.bloqueado"), tipoOrigenWCOM = CONST.TIPOS_ORIGEN["WCOM"], fechaSancion = me
								.getViewModel().get('expediente.fechaSancion');
							...
							if (CONST.ESTADOS_EXPEDIENTE['VENDIDO'] != codigoEstado) {
								if (CONST.TIPOS_EXPEDIENTE_COMERCIAL['ALQUILER'] == tipoExpedienteCodigo) {
									...
									if (Ext.isEmpty(fechaSancion)) {
										...
										var expediente = me.getViewModel().get(
												"expediente");
										Ext
												.create(
														'HreRem.view.expedientes.WizardAltaComprador',
								...
								me
										.fireEvent(
												"errorToast",
												HreRem
														.i18n("msg.operacion.ko.expediente.vendido"));
							}
						} else {
					...
					consultarComiteSancionador : function(btn) {
						...
						var me = this, comboComitePropuesto = me
								.lookupReference('comboComitePropuesto');

						var url = $AC
								.getRemoteUrl('expedientecomercial/consultarComiteSancionador');
					...
					enviarTitularesUvem : function(btn) {
						var me = this;
						var url = $AC
								.getRemoteUrl('expedientecomercial/enviarTitularesUvem');
						me.getView().mask(HreRem.i18n("msg.mask.espere"));
					...
					enviarHonorariosUvem : function(btn) {
						var me = this;
						var url = $AC
								.getRemoteUrl('expedientecomercial/enviarHonorariosUvem');
						me.getView().mask(HreRem.i18n("msg.mask.espere"));
										...
										.load({
											callback : function(records,
												...
												if (success) {
													...
													Utils
															.defaultRequestFailure(operation
																	.getResponse());
												}
											}
										});
					...
					mostrarDetallesClienteUrsus : function(field, newValue,
							oldValue, eOpts) {
						var me = this;
						var url = $AC
								.getRemoteUrl('expedientecomercial/buscarDatosClienteNumeroUrsus');
						var numeroUrsus = field.up('formBase').down(
								'[reference=seleccionClienteUrsus]').getValue();
						var fichaComprador = field.up('[xtype=formBase]');
						var idExpediente = fichaComprador.getBindRecord().get(
								'idExpedienteComercial');
						var parent = field.up('datoscompradorwindow');
					...
					changeComboTipoProveedor : function(combo, value, c) {
						var me = this;
						if (combo.getValue() == CONST.TIPOS_PROVEEDOR_EXPEDIENTE['CAT']
								|| combo.getValue() == CONST.TIPOS_PROVEEDOR_EXPEDIENTE['MEDIADOR_OFICINA']) {
							me.lookupReference('proveedorRef').setValue(null);
							me.lookupReference('proveedorRef')
									.setDisabled(true);
						} else {
							me.lookupReference('proveedorRef').setDisabled(
									false);
						}
					},
					...
					borrarComprador : function(grid, record) {
						...
						var idExpediente = me.getViewModel().get(
								"expediente.id");
						var codigoEstado = me.getViewModel().get(
								"expediente.codigoEstado");
						var idComprador = record.get('id');
						var tipoExpedienteCodigo = me.getViewModel().get(
								"expediente.tipoExpedienteCodigo");
						var origen = me.getViewModel().get("expediente.origen");
						var bloqueado = me.getViewModel().get(
								"expediente.bloqueado");
						var fechaSancion = me.getViewModel().get(
								'expediente.fechaSancion');
					...
					// Este método establece el atributo 'allowBlank' en los
					// campos peticionario y motivo anulación
					// de la ficha del expediente si existe una fecha de
					// anulación.
					onFechaAnulacionChange : function(dateField) {
						var me = this;

						var peticionario = dateField
								.up('expedientedetallemain').lookupReference(
										'textFieldPeticionario');
						var motivoAnulacion = dateField.up(
								'expedientedetallemain').lookupReference(
						...
						// Hacer saltar inmediatamente la validación de los
						// campos.
					...
					cargaValorVpo : function() {
						var me = this, disabled = me
								.lookupReference('comboVpo').getValue() == 0, procedeDescalificacion = me
								.lookupReference('procedeDescalificacionRef');
					...
					comprobarCamposFechas : function(editor, gridNfo) {
						...
						if (editor.isNew) {
							...
							me.lookupReference('fechaPosicionamientoRef')
									.setValue();
							me.lookupReference('horaAvisoRef').setValue();
							me.lookupReference('horaPosicionamientoRef')
									.setValue();
						}
						me.changeFecha(me.lookupReference('fechaAvisoRef'));
						me.changeFecha(me
								.lookupReference('fechaPosicionamientoRef'));
						me.changeHora(me.lookupReference('horaAvisoRef'));
						me.changeHora(me
								.lookupReference('horaPosicionamientoRef'));

						me.lookupReference('fechaPosicionamientoRef')
								.setDisabled(false);
						me.lookupReference('horaPosicionamientoRef')
								.setDisabled(false);
						me.lookupReference('motivoAplazamientoRef')
								.setDisabled(true);
					},
					...
					onRowClickPosicionamiento : function(gridView, record) {
						...
						if (!Ext.isEmpty(record.get('fechaFinPosicionamiento'))) {
							gridView.grid.down('#removeButton').setDisabled(
									true);
						} else {
							gridView.grid.down('#removeButton').setDisabled(
									false);
						...
						if (!Ext.isEmpty(record.get('idProveedorNotario'))) {
							...
							me.getViewModel().notify();
							me.lookupReference('listadoNotarios').getStore()
									.load();
						}
					},

					onRowClickPosicionamientoAlquiler : function(gridView,
							record) {
						var me = this;
						if (!Ext.isEmpty(record.get('fechaFinPosicionamiento'))) {
							gridView.grid.down('#removeButton').setDisabled(
					...
					changeFecha : function(campoFecha) {
						var me = this, referencia = campoFecha.getReference()
								.replace('fecha', 'hora'), campoHora = me
								.lookupReference(referencia);

						if (campoFecha.getValue() != null) {
							if (campoHora.isVisible()) {
								campoHora.setDisabled(false);
								campoHora.allowBlank = false;
								if (campoHora.getValue() == null) {// De esta
																	// forma se
																	// marca en
																	// rojo como
																	// obligatorio
					...
					changeHora : function(campoHora) {
						...
						if (campoHora.getValue() != null) {
							...
							// Le metemos la hora a la fecha completa que
							// contiene fecha y hora, que es la que se guardará
							// en bd.
							var fechaHora = me.lookupReference(campoHora
									.getReference()
									.replace('hora', 'fechaHora'));
							var fechaRef = campoHora.getReference().replace(
									'hora', 'fecha')

							fechaHora.setValue(me.lookupReference(fechaRef)
									.getValue());
							fechaHora.value.setTime(me
									.lookupReference(fechaRef).value);
							fechaHora.value
									.setHours(campoHora.value.getHours());
							fechaHora.value.setMinutes(campoHora.value
									.getMinutes());
						} else
					...
					numVisitaIsEditable : function() {
						var me = this, campoNumVisita = me
								.lookupReference('numVistaFromOfertaRef');
						// Si el estado de la visita no es REALIZADA, no debe
						// haber numVisita relacionada
					...
					comprobarObligatoriedadCamposNexos : function() {
						try {
							...
							if (me.lookupReference('tipoPersona').getValue() === "1") {
									...
									if (campoEstadoCivil.getValue() === "02") {
										...
										if (me.getViewModel().get(
												...
												|| me
														.getViewModel()
														.get(
																'comprador.entidadPropietariaCodigo') == CONST.CARTERA['LIBERBANK']) {
											...
											if (!Ext.isEmpty(campoRegEconomico)
													&& !Ext
															.isEmpty(campoNumConyuge)) {
												if (campoRegEconomico
														.getValue() === "01"
														|| campoRegEconomico
																.getValue() === "03") {
													campoNumConyuge.allowBlank = false;
													campoNumConyuge.validate();
												} else if (campoRegEconomico
														.getValue() === "02") {
								...
								// Si el tipo de persona es 'Jurídica' entonces
								// desactivar los campos dependientes del otro
								// estado.
								if (!Ext.isEmpty(campoEstadoCivil)) {
							...
							// Validar campos para que se muestre o desaparezca
							// la visual roja.
							if (!Ext.isEmpty(campoEstadoCivil)) {
								campoEstadoCivil.validate();
							}
					...
					enviarCondicionantesEconomicosUvem : function(btn) {
						var me = this;
						var url = $AC
								.getRemoteUrl('expedientecomercial/enviarCondicionantesEconomicosUvem');
						me.getView().mask(HreRem.i18n("msg.mask.espere"));
					...
					onClickGeneraOfertarHojaExcel : function(btn) {
						...
						Ext.Msg.confirm(HreRem.i18n("title.propuesta.oferta"),
								HreRem.i18n("msg.propuesta.oferta"), function(
										btn) {
									if (btn == "yes") {
					...
					onClickEnviarMailAprobacion : function(btn) {
						...
						Ext.Msg
								.confirm(
										HreRem
												.i18n("title.enviar.email.de.aprobacion"),
										HreRem
												.i18n("msg.enviar.mail.aprobacion"),
										function(btn) {
											if (btn == "yes") {

												var url = $AC
														.getRemoteUrl("ofertas/enviarMailAprobacion");
												var parametros = {
													idOferta : me
															.getViewModel()
															.get(
					...
					onClickEnviarEmailAsegurador : function(btn) {
						...
						Ext.Msg
								.confirm(
										HreRem
												.i18n("title.enviar.email.a.la.aseguradora"),
										HreRem
												.i18n("msg.header.enviar.mail.a.la.aseguradora"),
										function(btn) {
											if (btn == "yes") {

												var url = $AC
														.getRemoteUrl("expedientecomercial/enviarCorreoAsegurador");
												var parametros = {
													idExpediente : me
															.getViewModel()
															.get(
					...
					onClickEnviarComercializadora : function(rec) {
						var me = this;
						var url = $AC
								.getRemoteUrl('expedientecomercial/enviarCorreoComercializadora');
						var idExpediente = me.getViewModel().get(
								'expediente.id');
					...
					onChangeRevision : function(checkbox, newVal, oldVal) {
						var me = this;
						var scoringComentarios = me
								.lookupReference('scoringComentarios');
						if (checkbox.checked) {
					...
					onDisableMotivoRechazo : function(checkbox, newVal, oldVal) {
						var me = this;
						var estado = me.getViewModel().get(
								'scoring.comboEstadoScoring');
						var scoringMotivoRechazo = me
								.lookupReference('scoringMotivoRechazo');
						var scoringCheckEnRevision = me
								.lookupReference('scoringCheckEnRevision');
						...
						} else if (estado == "Aprobado") { // estado ==
															// "Aprobado ||
															// estado == En
															// tramite"
					...
					enlaceAbrirTrabajo : function(button) {
						...
						var me = this, idTrabajo = me.getViewModel().get(
								"expediente.idTrabajo");

						if (!Ext.isEmpty(idTrabajo)) {
							me.getView().fireEvent('abrirDetalleTrabajoById',
									idTrabajo, null, button.reflinks);
						}
					},
					...
					onClickEnviarGestionLlaves : function(button) {
						...
						Ext.Msg
								.confirm(
										HreRem
												.i18n("title.enviar.email.de.gestion.llaves"),
										HreRem
												.i18n("msg.enviar.email.gestion.llaves"),
										function(btn) {
											if (btn == "yes") {
												var url = $AC
														.getRemoteUrl("expedientecomercial/enviarCorreoGestionLlaves");
												var parametros = {
													idExpediente : me
															.getViewModel()
															.get(
					...
					onChangeCarencia : function(checkbox, newValue, oldValue,
							eOpts) {
						if (!Ext.isEmpty(oldValue)) {
							var me = this, meses = me
									.lookupReference('mesesCarencia'), importe = me
									.lookupReference('importeCarencia');
					...
					onChangeBonificacion : function(checkbox, newValue,
							oldValue, eOpts) {
						var me = this, meses = me
								.lookupReference('mesesBonificacion'), importe = me
								.lookupReference('importeBonificacion');
					...
					onChangeRepercutibles : function(checkbox, newValue,
							oldValue, eOpts) {
						if (!Ext.isEmpty(oldValue)) {
							var me = this, comentarios = me
									.lookupReference('textareafieldcondicioncomentariosgastos');
