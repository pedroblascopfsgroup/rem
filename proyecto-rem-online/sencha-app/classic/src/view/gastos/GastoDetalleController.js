			...
			onSaveFormularioCompleto : function(btn, form, success) {
				...
				//				
					...
					buscarPropietario : function(field, e) {
						...
						var me = this;
						var url = $AC
								.getRemoteUrl('gastosproveedor/searchPropietarioNif');
					...
					/*
					 * estaExento: function(get){ var me= this;
					 * if(get('detalleeconomico.impuestoIndirectoExento')==true ||
					 * get('detalleeconomico.abonoCuenta')==true){ return true; }
					 * else{ return false; } },
					 */
					...
					onChangeFechaTopePago : function(field, value) {
						/*
						 * var me= this; var fechaPago=
						 * me.lookupReference('fechaPago').getValue();
						 * if(!Ext.isEmpty(me.lookupReference('destinatariosPago'))){
						 * if(fechaPago<value){
						 * me.lookupReference('destinatariosPago').setDisabled(false);
						 * me.lookupReference('destinatariosPago').allowBlank=
						 * false; }else{
					...
					onClickBotonGuardarGastoActivo : function(btn) {
						...
						var checkbox = window.lookupReference('checkboxGastoRecalcular');
						var recalcular = checkbox.getValue();
					...
					refrescarGastoAlIncluirTrabajo : function(view) {
						...
						// mostrando.
						Ext.Array.each(
								view.query('component[funcionRecargar]'),
								function(component) {
									if (component.rendered) {
										component.recargar = true;
									}
					...
					downloadDocumentoAdjunto : function(grid, record) {
						...
						config.url = $AC.getWebPath()
								+ "gastosproveedor/bajarAdjuntoGasto."
								+ $AC.getUrlPattern();
						config.params = {};
					...
					/*
					 * onChangeFechaDevengoEspecial: function(calendario){ var
					 * me = this; var fecha = new Date(calendario.value); var
					 * anyo = fecha.getFullYear(); var combo =
					 * me.lookupReference('comboboxfieldFechaEjercicio');
					 * if(combo.getStore().byText.map){ combo.value=
					 * combo.getStore().byText.map[anyo].id;
					 * combo.bindStore(combo.getStore()); }else{
					...
					botonesEdicionGasto : function(estadoGasto, autorizado, rechazado, agrupado, gestoria, tabSeleccionada) {
						...
						if (tabSeleccionada.xtype == 'datosgeneralesgasto' && $AU.userHasFunction('EDITAR_TAB_DATOS_GENERALES_GASTOS')
								&& (CONST.ESTADOS_GASTO['INCOMPLETO'] == estadoGasto
										...
										|| (CONST.ESTADOS_GASTO['RECHAZADO_PROPIETARIO'] == estadoGasto
												&& !autorizado && !agrupado)
										|| (CONST.ESTADOS_GASTO['SUBSANADO'] == estadoGasto && !autorizado) || CONST.ESTADOS_GASTO['RETENIDO'] == estadoGasto)) {
							return true;
						}
						...
						if (tabSeleccionada.xtype == 'detalleeconomicogasto' && $AU.userHasFunction('EDITAR_TAB_DETALLE_ECONOMICO_GASTOS')
								&& ((CONST.ESTADOS_GASTO['PAGADO'] == estadoGasto)
										...
										|| CONST.ESTADOS_GASTO['PENDIENTE'] == estadoGasto
										|| CONST.ESTADOS_GASTO['RECHAZADO'] == estadoGasto
										|| (CONST.ESTADOS_GASTO['SUBSANADO'] == estadoGasto && !autorizado)
										|| (CONST.ESTADOS_GASTO['RECHAZADO_PROPIETARIO'] == estadoGasto
												&& !autorizado && !agrupado) || CONST.ESTADOS_GASTO['RETENIDO'] == estadoGasto)) {
						...
						if (tabSeleccionada.xtype == 'gestiongasto'
								...
								&& (CONST.ESTADOS_GASTO['RECHAZADO_PROPIETARIO'] != estadoGasto || (CONST.ESTADOS_GASTO['RECHAZADO_PROPIETARIO'] == estadoGasto
										&& !autorizado && !agrupado))
