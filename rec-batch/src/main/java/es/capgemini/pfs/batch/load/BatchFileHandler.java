...
/**
 * Clase con el código genérico para los file handlers. Se fuerza la generación
 * de varios métodos que son necesarios para que Spring asocie las clases
 * correctas en el momento de la ejecución.
 * 
 * @author aesteban
 * 
 */
public abstract class BatchFileHandler extends FileHandler {
	...
	public void handle(Message message) {
		...
		String workingCode = getWorkingCodeFromFileName(file.getName(),
				getAppProperty(getSemaphoreName()));
		// Debería ejecutar el proceso ahora?
		Date now = new Date();
		Date next = shouldProcessOn(now);

		// Si es un evento relanzado o la fecha es nula, ejecuto ahora
		boolean reThrowed = (Long) message.getHeaders().get(
				EventManager.RETHROWED_KEY) != null;
		if (reThrowed || next == null) {

			// Actualizar aquí el evento relanzado para prevenir pérdidas de
			// mensajes y relanzados
			if (reThrowed) {
				Long id = new Long(message.getHeaders()
						.get(EventManager.DEFERED_EVENT_KEY).toString());
				getEventManager().updateProcessDate(id, now);
			}
			setChannel(getChannel());

			PollableChannel messageChannel = getEventManager()
					.createPollableChannel(
							QueueUtils.getQueueNameForEntity(getChainChannel(),
									workingCode));
			...
			getEventManager().fireEvent(
					EventManager.GENERIC_CHANNEL,
					"Nuevo fichero [" + getChannelName() + ":"
							+ file.getAbsolutePath()
							+ "] posterga. Será procesada en  " + next);

			getEventManager()
					.createDeferedEvent(
							QueueUtils.getQueueNameForEntity(getChannel(),
									workingCode), file.getAbsolutePath(),
	...
	public void handle(File file) {
		...
		BatchJobLauncher jobLauncher = (BatchJobLauncher) ApplicationContextUtil
				.getBean(getJobLauncherKey());
