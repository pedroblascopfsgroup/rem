...
/**
 * Clase que implementa el Job Launcher para el proceso de Facturacion correspondiente a las Agencias de Recobro
 * @author Javier
 *
 */
public class ProcesoFacturacionJobLauncher implements BatchPCRConstantes, JobRunner {
	...
	/**
	 * Inicia el proceso de Facturacion
	 * @param workingCode
	 * @param extractTime
	 */
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(ProcesoFacturacion.PROCESO_FACTURACION_JOBNAME, workingCode, this);
	...
	public void run() {
		try{
			logger.info(ProcesoFacturacion.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			...
			BatchExitStatus result = batchManager.run(ProcesoFacturacion.PROCESO_FACTURACION_JOBNAME, parameters);
			if (BatchExitStatus.COMPLETED.equals(result)) {
				logger.info(ProcesoFacturacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
			...
			if (BatchExitStatus.NOOP.equals(result)) {
				logger.info(ControlSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
			}	
			if (BatchExitStatus.FAILED.equals(result)) {
				logger.error(ProcesoFacturacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
			}
	    } catch (Exception e) {
	    	logger.error(ProcesoFacturacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
