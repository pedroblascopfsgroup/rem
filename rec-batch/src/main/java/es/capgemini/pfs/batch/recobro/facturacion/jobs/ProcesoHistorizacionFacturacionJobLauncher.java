...
/**
 * Clase que implementa el Job Launcher para el proceso de Historización de la Facturacion correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoHistorizacionFacturacionJobLauncher implements BatchPCRConstantes, JobRunner {
	...
	/**
	 * Inicia el proceso de Historización de la Facturación
	 * @param workingCode
	 * @param extractTime
	 */
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(HistorizacionFacturacion.PROCESO_HISTORIZACION_FACTURACION_JOBNAME, workingCode, this);
	...
	public void run() {
		try{
			logger.info(HistorizacionFacturacion.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			...
			BatchExitStatus result = batchManager.run(HistorizacionFacturacion.PROCESO_HISTORIZACION_FACTURACION_JOBNAME, parameters);
			if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(HistorizacionFacturacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
			...
			if (BatchExitStatus.FAILED.equals(result)) {
				logger.error(HistorizacionFacturacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
			}
	    } catch (Exception e) {
	    	logger.error(HistorizacionFacturacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
