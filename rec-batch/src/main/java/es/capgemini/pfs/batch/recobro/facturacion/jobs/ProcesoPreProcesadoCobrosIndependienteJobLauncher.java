package es.capgemini.pfs.batch.recobro.facturacion.jobs;
...
/**
 * Clase que implementa el Job Launcher para el proceso INDEPENDIENTE de PreProcesado de los Cobros de Recobro para la posterior facturaci�n a Agencias
 * @author Javier Ruiz
 *
 */
public class ProcesoPreProcesadoCobrosIndependienteJobLauncher implements BatchPCRConstantes, JobRunner {
	...
	/**
	 * Inicia el proceso INDEPENDIENTE de PreProcesado de los Cobros de Recobro para la posterior facturaci�n a Agencias
	 * @param workingCode
	 * @param extractTime
	 */
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(PreProcesadoIndependienteCobros.PROCESO_PREPROCESADO_INDEPENDIENTE_COBROS_RECOBRO_JOBNAME, workingCode, this);
	...
	public void run() {
		try{
			logger.info(PreProcesadoCobros.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			...
			BatchExitStatus result = batchManager.run(PreProcesadoIndependienteCobros.PROCESO_PREPROCESADO_INDEPENDIENTE_COBROS_RECOBRO_JOBNAME, parameters);
			if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(PreProcesadoCobros.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
			}
			if (BatchExitStatus.FAILED.equals(result)) {
				logger.error(PreProcesadoCobros.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
			}
	    } catch (Exception e) {
	    	logger.error(PreProcesadoCobros.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
