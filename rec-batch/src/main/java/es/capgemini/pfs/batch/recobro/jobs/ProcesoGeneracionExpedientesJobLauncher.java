...
/**
 * Clase que implementa el Job Launcher para el proceso de Generación de Expedientes correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoGeneracionExpedientesJobLauncher implements BatchPCRConstantes, JobRunner{
	...
	/**
	 * Inicia el proceso de Generación de Expedientes
	 * @param workingCode
	 * @param extractTime
	 */
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoGeneracionExpedientes.PROCESO_GENERACION_EXPEDIENTES_JOBNAME, workingCode, this);
    ...
    public void run() {      
    	try{
	    	logger.info(ProcesoGeneracionExpedientes.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoGeneracionExpedientes.PROCESO_GENERACION_EXPEDIENTES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoGeneracionExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoGeneracionExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }       
        } catch (Exception e) {
        	logger.error(ProcesoGeneracionExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
