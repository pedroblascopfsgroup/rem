...
/**
 * Clase que implementa el Job Launcher para el proceso de Persistencia Previa Envío correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoPersistenciaPreviaEnvioJobLauncher implements BatchPCRConstantes, JobRunner{
	...
	/**
	 * Inicia el proceso de Persistencia Previa Envío
	 * @param workingCode
	 * @param extractTime
	 */
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoPersistenciaPreviaEnvio.PROCESO_PERSISTENCIA_PREVIA_ENVIO_JOBNAME, workingCode, this);
    ...
    public void run() {  
    	try{
	    	logger.info(ProcesoPersistenciaPreviaEnvio.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoPersistenciaPreviaEnvio.PROCESO_PERSISTENCIA_PREVIA_ENVIO_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoPersistenciaPreviaEnvio.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoPersistenciaPreviaEnvio.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }        
        } catch (Exception e) {
        	logger.error(ProcesoPersistenciaPreviaEnvio.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
