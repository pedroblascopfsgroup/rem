...
/**
 * Clase que implementa el Job Launcher para el proceso de Preparación del Batch a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoPreparacionRecobroJobLauncher implements BatchPCRConstantes, JobRunner {
	...
	/**
	 * Inicia el proceso de Preparación del Batch de REcobro
	 * @param workingCode
	 * @param extractTime
	 */
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(PreparacionRecobro.PROCESO_PREPARACION_RECOBRO_JOBNAME, workingCode, this);
	...
	public void run() {
		try{
			logger.info(PreparacionRecobro.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			...
			BatchExitStatus result = batchManager.run(PreparacionRecobro.PROCESO_PREPARACION_RECOBRO_JOBNAME, parameters);
			if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(PreparacionRecobro.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
			...
			if (BatchExitStatus.FAILED.equals(result)) {
				logger.error(PreparacionRecobro.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
			}
        } catch (Exception e) {
        	logger.error(PreparacionRecobro.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
