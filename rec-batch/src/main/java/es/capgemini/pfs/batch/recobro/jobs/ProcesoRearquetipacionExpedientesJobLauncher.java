...
/**
 * Clase que implementa el Job Launcher para el proceso de Rearquetipación de Expedientes correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoRearquetipacionExpedientesJobLauncher implements BatchPCRConstantes, JobRunner{
	...
	/**
	 * Inicia el proceso de Rearquetipación de Expedientes
	 * @param workingCode
	 * @param extractTime
	 */
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoRearquetipacionExpedientes.PROCESO_REARQUETIPACION_EXPEDIENTES_JOBNAME, workingCode, this);
    ...
    public void run() {       
    	try{
	    	logger.info(ProcesoRearquetipacionExpedientes.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoRearquetipacionExpedientes.PROCESO_REARQUETIPACION_EXPEDIENTES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoRearquetipacionExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoRearquetipacionExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }        
	    } catch (Exception e) {
	    	logger.error(ProcesoRearquetipacionExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
