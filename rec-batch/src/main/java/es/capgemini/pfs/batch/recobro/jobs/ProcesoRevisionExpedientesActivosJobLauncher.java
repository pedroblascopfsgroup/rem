...
/**
 * Clase que implementa el Job Launcher para el proceso de Revisión de Expedientes Activos correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoRevisionExpedientesActivosJobLauncher implements BatchPCRConstantes, JobRunner{
	...
	/**
	 * Inicia el proceso de Revisión de Expedientes Activos
	 * @param workingCode
	 * @param extractTime
	 */
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoRevisionExpedientesActivos.PROCESO_REVISION_EXPEDIENTES_ACTIVOS_JOBNAME, workingCode, this);
    ...
    public void run() {        
    	try{
	    	logger.info(ProcesoRevisionExpedientesActivos.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoRevisionExpedientesActivos.PROCESO_REVISION_EXPEDIENTES_ACTIVOS_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoRevisionExpedientesActivos.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoRevisionExpedientesActivos.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }        
	    } catch (Exception e) {
	    	logger.error(ProcesoRevisionExpedientesActivos.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
