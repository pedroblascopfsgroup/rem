...
/**
 * Clase que implementa el Job Launcher para el proceso de Preparaci�n de la Simulaci�n correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoControlSimulacionJobLauncher implements BatchPCRConstantes, JobRunner {
	...
	/**
	 * Inicia el proceso de Preparaci�n de la simulaci�n
	 * @param workingCode
	 * @param extractTime
	 */
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(ControlSimulacion.PROCESO_CONTROL_RECOBRO_JOBNAME, workingCode, this);
	...
	public void run() {
		try{
			logger.info(ControlSimulacion.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			...
			BatchExitStatus result = batchManager.run(ControlSimulacion.PROCESO_CONTROL_RECOBRO_JOBNAME, parameters);
			...
			if (BatchExitStatus.FAILED.equals(result)) {
				logger.error(ControlSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
			}		
	    } catch (Exception e) {
	    	logger.error(ControlSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
