...
/**
 * Clase que implementa el Job Launcher para el proceso de Preparación de la Simulación correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoPreparacionSimulacionJobLauncher implements BatchPCRConstantes, JobRunner {
	...
	/**
	 * Inicia el proceso de Preparación de la simulación
	 * @param workingCode
	 * @param extractTime
	 */
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(PreparacionSimulacion.PROCESO_PREPARACION_SIMULACION_RECOBRO_JOBNAME, workingCode, this);
	...
	public void run() {
		try{
			logger.info(PreparacionSimulacion.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			...
			BatchExitStatus result = batchManager.run(PreparacionSimulacion.PROCESO_PREPARACION_SIMULACION_RECOBRO_JOBNAME, parameters);
			if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(PreparacionSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
			...
			if (BatchExitStatus.FAILED.equals(result)) {
				logger.error(PreparacionSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
			}
	    } catch (Exception e) {
	    	logger.error(PreparacionSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
