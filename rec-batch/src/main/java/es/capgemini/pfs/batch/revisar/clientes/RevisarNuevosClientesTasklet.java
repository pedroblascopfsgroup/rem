 ...
 *
 */
public class RevisarNuevosClientesTasklet implements Tasklet, StepExecutionListener, ClienteBPMConstants {
    ...
    public ExitStatus execute() {
        logger.debug("Inicio del job RevisarNuevosClientes");

        List<Long> idPersonasFuturosClientes = contratosBatchManager.buscarFuturosClientes();
        Long idArquetipo;
        ...
        for (Long idPersona : idPersonasFuturosClientes) {
            ...
            if (idArquetipo == null || idArquetipo.longValue() == 0) {
                logger.error("Error al intentar crear un cliente. La persona " + idPersona + " no tiene un arquetipo calculado");
            } else {
    ...
    /**
     * @param contratosBatchManager clase auxiliar
     */
    public void setContratosBatchManager(ContratosBatchManager contratosBatchManager) {
        this.contratosBatchManager = contratosBatchManager;
    }
