...
public class ProcesoCreacionClientesRecuperacionesJobLauncher implements JobRunner,BatchPCRConstantes{
	...
	public void run() {
		try{
	    	logger.info(ProcesoCreacionClientesRecuperaciones.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoCreacionClientesRecuperaciones.PROCESO_CREACION_CLIENTES_RECUPERACIONES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoCreacionClientesRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoCreacionClientesRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }
        } catch (Exception e) {
        	logger.error(ProcesoCreacionClientesRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoCreacionClientesRecuperaciones.PROCESO_CREACION_CLIENTES_RECUPERACIONES_JOBNAME, workingCode, this);
