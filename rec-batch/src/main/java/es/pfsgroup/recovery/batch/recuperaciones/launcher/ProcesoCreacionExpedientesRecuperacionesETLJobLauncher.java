...
public class ProcesoCreacionExpedientesRecuperacionesETLJobLauncher implements JobRunner,BatchPCRConstantes{
	...
	public void run() {
		try{
	    	logger.info(ProcesoCreacionExpedientesRecuperacionesETL.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoCreacionExpedientesRecuperacionesETL.PROCESO_CREACION_EXPEDIENTES_RECUPERACIONES_ETL_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoCreacionExpedientesRecuperacionesETL.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoCreacionExpedientesRecuperacionesETL.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoCreacionExpedientesRecuperacionesETL.PROCESO_CREACION_EXPEDIENTES_RECUPERACIONES_ETL_JOBNAME, workingCode, this);
