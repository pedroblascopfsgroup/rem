...
public class ProcesoCreacionExpedientesRecuperacionesJobLauncher implements JobRunner,BatchPCRConstantes{
	...
	public void run() {
		try{
	    	logger.info(ProcesoCreacionExpedientesRecuperaciones.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoCreacionExpedientesRecuperaciones.PROCESO_CREACION_EXPEDIENTES_RECUPERACIONES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoCreacionExpedientesRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoCreacionExpedientesRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }
        } catch (Exception e) {
        	logger.error(ProcesoCreacionExpedientesRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoCreacionExpedientesRecuperaciones.PROCESO_CREACION_EXPEDIENTES_RECUPERACIONES_JOBNAME, workingCode, this);
