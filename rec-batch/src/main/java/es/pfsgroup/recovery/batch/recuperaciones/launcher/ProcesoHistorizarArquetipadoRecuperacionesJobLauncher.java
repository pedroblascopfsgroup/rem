...
public class ProcesoHistorizarArquetipadoRecuperacionesJobLauncher implements JobRunner,BatchPCRConstantes{
...
private final Log logger = LogFactory.getLog(getClass());
	...
	public void run() {
		try{
	    	logger.info(ProcesoHistorizarArquetipadoRecuperaciones.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoHistorizarArquetipadoRecuperaciones.PROCESO_HISTORIZAR_ARQUETIPADO_RECUPERACIONES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoHistorizarArquetipadoRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoHistorizarArquetipadoRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }
        } catch (Exception e) {
        	logger.error(ProcesoHistorizarArquetipadoRecuperaciones.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoHistorizarArquetipadoRecuperaciones.PROCESO_HISTORIZAR_ARQUETIPADO_RECUPERACIONES_JOBNAME, workingCode, this);
