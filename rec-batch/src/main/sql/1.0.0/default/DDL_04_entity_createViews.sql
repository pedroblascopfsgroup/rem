...
begin
  ...
  t_dep(t_dep.LAST + 1).name := 'EXR_EXPEDIENTE_RECOBRO';
  ...
  a_ddls(a_ddls.LAST + 1) := '
    CREATE OR REPLACE VIEW BATCH_DATOS_CNT_INFO AS
      SELECT DISTINCT CNT.CNT_ID,  PRO.DD_PRO_CODIGO CODIGO_PROPIETARIO, TPE.DD_TPE_CODIGO TIPO_PRODUCTO, CNT.CNT_CONTRATO NUMERO_CONTRATO
...
SELECT 
  ...
  FOR contador IN 1..a_tables.LAST LOOP
    ...
    IF ncount > 0 THEN
       v_sql:='DROP TABLE '||a_tables(contador);
