...
select count(1) into table_count from TAB WHERE TNAME='DATA_RULE_ENGINE';
    if table_count > 0 then  
			query_body :='DROP MATERIALIZED VIEW '||schema_name||'.DATA_RULE_ENGINE'; EXECUTE IMMEDIATE query_body;
			DBMS_OUTPUT.PUT_LINE('[INFO] OK BORRADA VISTA MATERIALIZADA (DATA_RULE_ENGINE) EN:> '||schema_name||'.DD_RULE_DEFINITION.');
		end if;
...
WITH PRIMARY KEY 
AS 
SELECT per.per_id, per_riesgo, per_riesgo_autorizado, per_riesgo_ind, 
       per_riesgo_dir_vencido,
       per_sexo, per_empleado, per.DD_COS_ID, per.dd_pnv_id,
       case when titular.per_id is null then 0 else 1 end as per_titular, 
       per_riesgo_dispuesto, per_nacionalidad, per_ecv, dd_sce_id,
       ant.ant_reincidencia_internos, per.dd_tpe_id, per_pais_nacimiento,
       dd_pol_id, per_fecha_nacimiento, 0 AS serv_nomina_pension, dd_rex_id,
       per_fecha_constitucion, per.ofi_id, mov_int_remuneratorios,
       ...
       dd_ct1_id, cnt_domici_ext, CNT.dd_esc_id, IACESTADO.dd_ece_id,
       mov_ltv_fin, cnt_limite_ini, mov_saldo_pasivo, mov_fecha_pos_vencida, mov_limite_desc,
       cnt_fecha_venc, cnt_fecha_creacion, mov_saldo_exce, cnt_limite_fin,
       dd_efc_id_ant, mov_gastos,
       ...
       FROM per_personas per LEFT JOIN cpe_contratos_personas cpe ON per.per_id = cpe.per_id --PER_CPE
       ...
       LEFT JOIN (SELECT IAC1.CNT_ID, ECE.DD_ECE_ID FROM EXT_IAC_INFO_ADD_CONTRATO IAC1
                 JOIN EXT_DD_IFC_INFO_CONTRATO DDIACESTADO ON DDIACESTADO.DD_IFC_ID=IAC1.DD_IFC_ID AND DDIACESTADO.DD_IFC_CODIGO = ''DD_ECE_CODIGO''
                 JOIN DD_ECE_ESTADO_CONTRATO_ENTIDAD ECE ON ECE.DD_ECE_CODIGO=IAC1.IAC_VALUE) IACESTADO ON IACESTADO.CNT_ID=CNT.CNT_ID
       LEFT JOIN (select distinct per_id from cpe_contratos_personas where dd_tin_id 
                 in (select dd_tin_id from DD_TIN_TIPO_INTERVENCION where dd_tin_titular=1)) TITULAR ON TITULAR.PER_ID=PER.PER_ID
...
query_body :='COMMENT ON MATERIALIZED VIEW '||schema_name||'.DATA_RULE_ENGINE IS ''snapshot table for snapshot '||schema_name||'.DATA_RULE_ENGINE''';

EXECUTE IMMEDIATE query_body;

query_body :='CREATE INDEX '||schema_name||'.IDX_DRE_PER_ID ON '||schema_name||'.DATA_RULE_ENGINE (PER_ID)';
EXECUTE IMMEDIATE query_body;

DBMS_OUTPUT.PUT_LINE('[INFO] OK CREADA VISTA MATERIALIZADA (DATA_RULE_ENGINE) EN:> '||schema_name||'.DD_RULE_DEFINITION.');
