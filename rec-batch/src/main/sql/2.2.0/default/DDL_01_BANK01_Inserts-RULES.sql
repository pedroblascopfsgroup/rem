...
select count(1) into table_count from all_indexes WHERE index_name='IDX_PER_PER_FECHA_EXTRACCION';
    if table_count = 0 then      
		query_body :='CREATE INDEX '||schema_name||'.IDX_PER_PER_FECHA_EXTRACCION ON '||schema_name||'.PER_PERSONAS (PER_FECHA_EXTRACCION)'; EXECUTE IMMEDIATE query_body;
	end if;		
