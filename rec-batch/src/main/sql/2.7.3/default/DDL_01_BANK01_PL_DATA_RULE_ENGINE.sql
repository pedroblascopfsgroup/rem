...
-- ALTER TABLE SOBRE DATA RULE ENGINE
DECLARE
  COLUMN_ALREADY_EXISTS EXCEPTION;
  PRAGMA EXCEPTION_INIT(COLUMN_ALREADY_EXISTS,-1430);
BEGIN
  EXECUTE IMMEDIATE 'ALTER TABLE BANK01.DATA_RULE_ENGINE ADD ES_TITULAR NUMBER(1)';
...
DECLARE
  ...
  PRAGMA EXCEPTION_INIT(COLUMN_ALREADY_EXISTS,-1430);
BEGIN
  EXECUTE IMMEDIATE 'ALTER TABLE BANK01.DATA_RULE_ENGINE ADD MAX_DIAS_IRREGULAR NUMBER(16)';
  EXCEPTION WHEN COLUMN_ALREADY_EXISTS THEN NULL;
END;
/

-- Regenerar la tabla TMP_REC_DATA_RULE_ENGINE
DROP TABLE BANK01.TMP_REC_DATA_RULE_ENGINE PURGE;

CREATE TABLE BANK01.TMP_REC_DATA_RULE_ENGINE AS SELECT * FROM BANK01.DATA_RULE_ENGINE WHERE ROWNUM = 0;

CREATE INDEX BANK01.IDX_TMP_REC_DATA_RULE_ENGINE_1 ON BANK01.TMP_REC_DATA_RULE_ENGINE(PER_ID);

-- Regenerar la vista TMP_REC_PER_DATA_RULE_ENGINE
CREATE OR REPLACE VIEW BANK01.TMP_REC_PER_DATA_RULE_ENGINE AS 
SELECT * FROM BANK01.data_rule_engine
