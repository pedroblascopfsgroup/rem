...
import es.capgemini.pfs.batch.recobro.api.test.GenericConstantsTest.ProcesoRearquetipacionExpedientes;
...
/**
 * Test Case para el subproceso de Rearquetipación de Expedientes del proceso de Recobro
 * @author Guillem
 *
 */
@ContextConfiguration(locations = Genericas.FICHERO_CONTEXTO_SPRING)
@RunWith(SpringJUnit4ClassRunner.class)
public class ProcesoRearquetipacionExpedientesJobLauncherTest extends GenericTest{
	...
	public void test() {	
		try{
	        ...
	        testLaunchDevonJob(getJob(ProcesoRearquetipacionExpedientes.PROCESO_REARQUETIPACION_EXPEDIENTES_JOBNAME), parameters);
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try{
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/procesoRearquetipacionExpedientes.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try{
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/procesoRearquetipacionExpedientes.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try{
			...
			configPostDataTest.setSqlFile("postTestData/recobro/procesoRearquetipacionExpedientes.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
