...
import es.capgemini.pfs.batch.recobro.api.test.GenericConstantsTest.ProcesoReparto;
import es.capgemini.pfs.batch.recobro.api.test.GenericTest;

/**
 * Test Case para el subproceso de Reparto de Expedientes del proceso de Recobro
 * @author Guillem
 *
 */
@ContextConfiguration(locations = Genericas.FICHERO_CONTEXTO_SPRING)
@RunWith(SpringJUnit4ClassRunner.class)
public class ProcesoRepartoJobLauncherTest extends GenericTest{
	...
	public void test() {	
		try{
	        ...
	        testLaunchDevonJob(getJob(ProcesoReparto.PROCESO_REPARTO_JOBNAME), parameters);
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try{
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/procesoReparto.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try{
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/procesoReparto.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try{
			...
			configPostDataTest.setSqlFile("postTestData/recobro/procesoReparto.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
