...
/**
 * Test Case para el subproceso de Validación del proceso de Recobro
 * @author Guillem
 *
 */
@ContextConfiguration(locations = Genericas.FICHERO_CONTEXTO_SPRING)
@RunWith(SpringJUnit4ClassRunner.class)
public class ProcesoValidacionRecobroJobLauncherTest extends GenericTest{
	...
	public void test() {	
		try{
	        ...
	        testLaunchDevonJob(getJob(ValidacionRecobro.PROCESO_VALIDACION_RECOBRO_JOBNAME), parameters);
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try{
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/procesoValidacionRecobro.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try{
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/procesoValidacionRecobro.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try{
			...
			configPostDataTest.setSqlFile("postTestData/recobro/procesoValidacionRecobro.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
