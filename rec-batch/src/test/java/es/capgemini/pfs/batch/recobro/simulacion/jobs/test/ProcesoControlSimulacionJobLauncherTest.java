...
public class ProcesoControlSimulacionJobLauncherTest extends GenericTest {
	/**
	...
	public void test() {
		try {
			...
			testLaunchDevonJob(getJob(ControlSimulacion.PROCESO_CONTROL_RECOBRO_JOBNAME), parameters);
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try {
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/simulacion/procesoControlSimulacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try {
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/simulacion/procesoControlSimulacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try {
			...
			configPostDataTest.setSqlFile("postTestData/recobro/simulacion/procesoControlSimulacion.xml");
		} catch(Throwable e){
			logger.error(e);
			fail();
		}
