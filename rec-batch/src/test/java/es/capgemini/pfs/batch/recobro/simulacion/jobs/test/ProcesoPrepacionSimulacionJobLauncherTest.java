...
/**
 * Test Case para el subproceso de Preparacion de la Simulación del proceso de Recobro
 * @author Guillem
 *
 */
@ContextConfiguration(locations= Genericas.FICHERO_CONTEXTO_SPRING)
@RunWith(SpringJUnit4ClassRunner.class)
public class ProcesoPrepacionSimulacionJobLauncherTest extends GenericTest {
	/**
	...
	public void test() {
		try {
			...
			testLaunchDevonJob(getJob(PreparacionSimulacion.PROCESO_PREPARACION_SIMULACION_RECOBRO_JOBNAME), parameters);
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try {
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/simulacion/procesoPreparacionSimulacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try {
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/simulacion/procesoPreparacionSimulacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try {
			...
			configPostDataTest.setSqlFile("postTestData/recobro/simulacion/procesoPreparacionSimulacion.xml");
		} catch(Throwable e){
			logger.error(e);
			fail();
		}
