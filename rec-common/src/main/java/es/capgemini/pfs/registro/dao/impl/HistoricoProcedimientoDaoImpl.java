package es.capgemini.pfs.registro.dao.impl;
...
/**
 * Consulta agregada que recopila información sobre recursos, tareas, comunicaciones, decisiones y acuerdos de un procedimiento.
 * La respuesta la mapea en un objeto {@link HistoricoProcedimiento}
 * Implementa el interface {@link HistoricoProcedimientoDao}
 * @author pajimene
 */
@Repository("HistoricoProcedimientoDao")
public class HistoricoProcedimientoDaoImpl extends AbstractEntityDao<HistoricoProcedimiento, Long> implements HistoricoProcedimientoDao {
    ...
    private String getConsultaAgregada() {
        ...
        return "SELECT TIPO_ENTIDAD, ID_ENTIDAD, RESPUESTA, PRC_ID, NOMBRE, FECHA, FECHA_INI, FECHA_VENC, FECHA_FIN, USUARIO FROM "
        //Tareas externa
                + "( " + "    SELECT "
                ...
                + ", TAR.TAR_FECHA_INI AS FECHA_INI, TAR.TAR_FECHA_VENC AS FECHA_VENC, TAR.TAR_FECHA_FIN AS FECHA_FIN, TAR.USUARIOCREAR AS USUARIO "
                + "FROM TAR_TAREAS_NOTIFICACIONES TAR, TEX_TAREA_EXTERNA TEX WHERE TEX.TAR_ID = TAR.TAR_ID "
                + "AND TAR.PRC_ID IS NOT NULL AND (TEX_CANCELADA = 0 OR TEX_CANCELADA IS NULL) "
                + "    UNION SELECT "
                + HistoricoProcedimiento.TIPO_ENTIDAD_TAREA_CANCELADA
                + " AS TIPO_ENTIDAD, TEX.TAR_ID AS ID_ENTIDAD, 0 AS RESPUESTA, TAR.PRC_ID AS PRC_ID, TAR_TAREA AS NOMBRE, TEX.FECHACREAR AS FECHA"
                + ", TAR.TAR_FECHA_INI AS FECHA_INI, TAR.TAR_FECHA_VENC AS FECHA_VENC, TAR.TAR_FECHA_FIN AS FECHA_FIN, TAR.USUARIOCREAR AS USUARIO "
                + "FROM TAR_TAREAS_NOTIFICACIONES TAR, TEX_TAREA_EXTERNA TEX WHERE TEX.TAR_ID = TAR.TAR_ID AND TAR.PRC_ID IS NOT NULL AND TEX_CANCELADA = 1 "
                + "    UNION SELECT "
                ...
                + ", TAR_ID, 0, PRC.PRC_ID, TAR_TAREA, TAR.FECHACREAR, TAR.TAR_FECHA_INI, TAR.TAR_FECHA_VENC, TAR.TAR_FECHA_FIN, TAR.USUARIOCREAR "
                ...
                + "', REC.FECHACREAR, REC.FECHACREAR, null, null, REC.USUARIOCREAR FROM TAR_TAREAS_NOTIFICACIONES TAR, RCR_RECURSOS_PROCEDIMIENTOS REC "
                + "WHERE REC.TAR_ID = TAR.TAR_ID AND TAR.PRC_ID IS NOT NULL "
                + "    UNION SELECT "
                ...
                + "', REC.FECHAMODIFICAR, REC.FECHACREAR, null, REC.FECHAMODIFICAR, REC.USUARIOMODIFICAR FROM TAR_TAREAS_NOTIFICACIONES TAR, RCR_RECURSOS_PROCEDIMIENTOS REC "
                + "WHERE REC.TAR_ID = TAR.TAR_ID AND TAR.PRC_ID IS NOT NULL AND REC.RCR_FECHA_RESOLUCION IS NOT NULL "
                + "    UNION SELECT "
                ...
                + "', SPR.FECHACREAR, SPR.FECHACREAR, null, null, SPR.USUARIOCREAR FROM TAR_TAREAS_NOTIFICACIONES TAR, SPR_SOLICITUD_PRORROGA SPR "
                + "WHERE SPR.TAR_ID = TAR.TAR_ID AND TAR.PRC_ID IS NOT NULL "
                + "    UNION SELECT "
                ...
                + "', SPR.FECHAMODIFICAR, SPR.FECHACREAR, null, SPR.FECHAMODIFICAR, SPR.USUARIOMODIFICAR FROM TAR_TAREAS_NOTIFICACIONES TAR, SPR_SOLICITUD_PRORROGA SPR "
                + "WHERE SPR.TAR_ID = TAR.TAR_ID AND TAR.PRC_ID IS NOT NULL AND SPR.DD_RPR_ID IS NOT NULL "
                + "    UNION SELECT "
                ...
                + "', ACU.FECHACREAR, ACU.FECHACREAR, null, null, ACU.USUARIOCREAR FROM ACU_ACUERDO_PROCEDIMIENTOS ACU, PRC_PROCEDIMIENTOS PRC"
                ...
                + "' || eac.dd_eac_descripcion_larga, ACU.FECHAMODIFICAR, ACU.FECHAMODIFICAR, null, ACU.FECHAMODIFICAR, ACU.USUARIOMODIFICAR "
                ...
                + "', DPR.FECHACREAR, DPR.FECHACREAR, null, null, DPR.USUARIOCREAR FROM DPR_DECISIONES_PROCEDIMIENTOS DPR, PRC_PROCEDIMIENTOS PRC"
                ...
                + "', DPR.FECHAMODIFICAR, DPR.FECHACREAR, null, DPR.FECHAMODIFICAR, DPR.USUARIOMODIFICAR FROM DPR_DECISIONES_PROCEDIMIENTOS DPR"
    ...
    /**
     * Método que devuelve el listado de tareas y operaciones de un procedimiento.
     * @param idProcedimiento Procedimiento sobre el que se ejecuta la query
     * @return HistoricoProcedimiento Listado {@link HistoricoProcedimiento} con la información de tareas y operaciones
     */
    @SuppressWarnings("unchecked")
    @Override
    public List<HistoricoProcedimiento> getListByProcedimiento(Long idProcedimiento) {
        //List ls = null;
        List<HistoricoProcedimiento> result = new ArrayList<HistoricoProcedimiento>();

        try {
            String query = "SELECT TIPO_ENTIDAD, ID_ENTIDAD, RESPUESTA, NOMBRE, FECHA, FECHA_INI, FECHA_VENC, FECHA_FIN, USUARIO FROM ("
            ...
            while (it.hasNext()) {
                ...
                HistoricoProcedimiento hProcedimiento = new HistoricoProcedimiento();
