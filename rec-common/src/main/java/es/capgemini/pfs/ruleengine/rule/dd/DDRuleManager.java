...
public class DDRuleManager {
    ...
    public List<DDRule> getList() {
        ...
        for (DDRule ddRule : ddRules) {
            if(TypeDiccionario.TYPE_DICCIONARIO.equalsIgnoreCase(ddRule.getType())){
                if(ddRule.getBusinessOperation()==null ||StringUtils.isEmpty(ddRule.getBusinessOperation().trim())){
                    logger.warn("No se ha definifo BO para el dictionary: ID "+ddRule.getId()+" - "+ddRule.getTitle()+" - "+ddRule.getColumn());
                    continue;
                }
                try{
                    ddRule.setValues((List)executor.execute(ComunBusinessOperation.BO_DICTIONARY_GET_LIST, ddRule.getBusinessOperation()));
                }catch (Exception e) {
                    logger.error("Error al intentar obtener los valores para el dictionary: ID "+ddRule.getId()+" - "+ddRule.getTitle()+" - "+ddRule.getColumn(),e);
                }
            }
        }
