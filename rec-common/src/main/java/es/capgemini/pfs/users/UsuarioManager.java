...
public class UsuarioManager {
	...
	public Usuario getUsuarioLogado() {
		...
		if (SecurityUtils.getCurrentUser() == null && defaultUserId != null) {
			...
			loggedUser = get(((UsuarioSecurity) SecurityUtils.getCurrentUser())
					.getId());
		}
	...
	public void cambiarEntidadUsuarioLogado(String codEntidadSeleccionada) {
		...
		loggedUser = (Usuario) RequestContextHolder
				.getRequestAttributes()
				.getAttribute(USER_SESSION_KEY, RequestAttributes.SCOPE_SESSION);
	...
	public Usuario cambiarEntidadBaseDatos(String codEntidadSeleccionada) {
		...
		Usuario loggedUser = null;
		Entidad enti = entidadDao.findByDescripcion(codEntidadSeleccionada);
		EntidadConfig entidadConfig = entidadConfigDao.findByEntidad(enti
				.getId());

		try {
			entityDataSource.getConnectionMultientidad(entidadConfig
					.getDataValue());
	...
	public Set<GrantedAuthority> getAuthorities(Usuario usuario) {
		...
		for (Perfil perfil : usuario.getPerfiles()) {
			for (Funcion funcion : perfil.getFunciones()) {
				authorities.add(new GrantedAuthorityImpl(funcion
						.getDescripcion()));
			}
		}
	...
	public String recuperarPassword(String username) {
		...
		if (usuario == null) {
			return messageService.getMessage("admin.users.noExiste");
		}
		try {
			enviarMail(usuario);
	...
	private void enviarMail(Usuario usuario) throws MessagingException {
		try {
			...
			throw new MessagingException(messageService.getMessage(
					"admin.users.mail.error", null));
		}
	}
	...
	private String crearCuerpoMail(Usuario usuario) {
		...
		return messageService.getMessage("admin.users.mail.cuerpoMensaje",
				param);
	...
	public void guardarUsuarioInterno(DtoUsuario dto) {
		...
		// Este metodo se llama antes de estar logeado y por lo tanto no se
		// conoce la entidad de BD
	...
	/**
	 * Valida que el password ingresado sea el actual, que el nuevo se el mismo
	 * que su verficacion, y que el nuevo sea distinto al actual. Caso contrario
	 * Excepcion.
	 * 
	 * @param usr
	 *            Usuario actual
	...
	private void validarPasswords(Usuario usr, DtoUsuario dto) {
		...
		if (!dto.getPasswordNuevo().trim().isEmpty()) {
			if (!dto.getPasswordNuevo()
					.equals(dto.getPasswordNuevoVerificado())) {
				throw new BusinessOperationException(
						"admin.users.password.new.invalid");
			}
			if (dto.getPasswordNuevo().equals(usr.getPassword())) {
				throw new BusinessOperationException(
						"admin.users.password.eqactual");
			}
		}
	...
	public List<MenuItem> getMenuDefinition(String entity) {
		...
		while (getNivel(list, level).size() > 0) {
			for (DynamicElement item : getNivel(list, level)) {
				MenuItem m = new MenuItem(item.getName(), item.getFileName(),
						new ArrayList<MenuItem>());
				if (item.getName().contains("*"))
					m.setCurlyBraces(false);
	...
	public List<DynamicElement> getDynamicElements(String entity, Object param) {
		...
		for (DynamicElement tab : getElements(entity, param)) {
			...
			if (found == null) {
				els.add(new DynamicElementAdapter("", tab.getName(), tab
						.getFileName(), tab.getOrder(), tab.getPriority()));
			} else {
	...
	private List<DynamicElement> getElements(String entity, Object param) {
		...
		for (DynamicElement el : elements) {
			if (isEnabled(el) && el.getEntity().equals(entity)
					&& el.valid(param)) {
				valids.add(el);
			}
		}
