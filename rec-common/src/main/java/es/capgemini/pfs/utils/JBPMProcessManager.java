...
public class JBPMProcessManager implements BPMContants {
	...
	public long crearNewProcess(String instanceName, Map<String, Object> param) {
		...
		// Esto es necesario cuando alguna de las tareas se ejecuta en una
		// instancia JBPM distinta
		determinarBBDD();
		...
		processManager.execute(new JbpmCallback() {
			...
			public Object doInJbpm(JbpmContext context) {
				...
				if (transition != null) {
					...
					logger.warn("No hay transición de Fin en el nodo " + nodoActual.getName() + " token="
	...
	/**
	 * Devuelve cierto o falso en funciÃ³n de si existe o no la transición en el
	 * token activo.
	 * 
	 * @param idToken
	 *            id del token
	 ...
	 *            id del proceso
	 * @param variables
	 *            variables a insertar en el proceso
	 */
	 ...
	 * 
	 * @param idProcess
	 *            el proceso para el cual se busca la fecha de fin
	 * @param nameTimer
	 *            el nombre del timer
	...
	public Date obtenerFechaFinProceso(final Long idProcess, String nameTimer) {
		...
		if (timer == null) {
			logger.warn("Se ha intentado actualizar un timer que no existe. idProceso: " + idProcess + "\t nombre: "
					+ nameTimer);
			return null;
		}
	 ...
	 * 
	 * @param idProcess
	 *            ID del Proceso BPM que se desea aplazar
	 * @param fechaActivacion
	 *            Fecha en la que se activarán las tareas de nuevo
	 */
	...
	/**
	 * Paraliza los procesos de un BPM. Lo único que hará este método es mandar
	 * una transición "TRANSICION_PARALIZAR_TAREAS" a los tokens activos
	 * 
	 * @param idProcess
	 *            id del proceso
	 */
	...
	/**
	 * Activa los procesos de un BPM paralizados. Lo único que hará este método
	 * es mandar una transición "TRANSICION_ACTIVAR_TAREAS" a los tokens
	 * activos.
	 * 
	 * @param idProcess
	 *            id del proceso
	...
	/**
	 * Finaliza el procedimiento. Detiene el proceso BPM, finaliza las tareas
	 * activas y las decisiones.
	 * 
	...
	public void finalizarProcedimiento(final Long idProcedimiento) throws Exception {
		...
		if (procedimiento == null) {
			throw new UserException("El procedimiento [" + idProcedimiento + "] no existe en la BD");
		}
		...
		if (idProcess != null) {
			...
			// Eliminamos las tareas Externas del procedimiento que aun estÃ¡n
			// activas
	...
	/**
	 * Señaliza una transición sobre un proceso o sobre sus tokens activos. El
	 * propio método buscará todos los tokens activos y lanzará un signal por
	 * cada uno de ellos
	 * 
	 * @param idProcess
	 *            ID del proceso BPM
	 * @param nombreTransicion
	 *            Nombre de la transición que se desea lanzar
	 */
				...
				if (it.hasNext()) {
					while (it.hasNext()) {
						...
						try {
							...
							// Pueden haber nodos (los del usuario) que no
							// acepten este tipo de transiciones, no es un
							// problema en el siguiente nodo tarea se dormirá el
							// BPM
							logger.warn("Error al generar la transición [" + nombreTransicion + "] para el token ["
									+ token.getId() + "]", e);
						}
					}
				} else {
					...
					try {
						...
						// Pueden haber nodos (los del usuario) que no acepten
						// este tipo de transiciones, no es un problema en el
						// siguiente nodo tarea se dormirá el BPM
						logger.warn("Error al generar la transición [" + nombreTransicion + "] para el proceso ["
								+ idProcess + "]", e);
					}
				}
	...
	/**
	 * Crea el contexto de ejecuciÃ³n para un script groovy introduciendo
	 * variables útiles, funciones y managers.
	 * 
	 * @return mapa
	 * @throws ClassNotFoundException
	 *             e
	 */
	private Map<String, Object> creaContextoParaScript(Long idProcedimiento, Long idTareaExterna,
			Long idTareaProcedimiento) throws ClassNotFoundException {
		...
		Procedimiento procedimientoPadre = procedimientoManager.getProcedimiento(idProcedimiento)
				.getProcedimientoPadre();
	...
	/**
	 * Devuelve el contexto crado de ejecución para un script groovy
	 * introduciendo variables, funciones y managers.
	 * 
	 * @param idProcedimiento
	...
	public Map<String, Object> getContextoParaScript(Long idProcedimiento, Long idTareaExterna,
			Long idTareaProcedimiento) throws ClassNotFoundException {
		return this.creaContextoParaScript(idProcedimiento, idTareaExterna, idTareaProcedimiento);
	}

	/**
	 * Introduce los diccionarios definidos en el bean
	 * clasesDiccionariosAnotadas en el contexto de los script groovy para que
	 * tengamos acceso.
	 * 
	 * @param context
	 *            contexto
	...
	/**
	 * Tan sÃ­lo haremos el cálculo de las clases diccionarios la primera vez
	...
	/**
	 * Crea un contexto propio para la evaluación en relación con el
	 * procedimiento, la tarea externa y el contexto original y evalua el script
	 * que se le pasa como parámetro.
	 * 
	 * @param idProcedimiento
	 *            ID del procedimiento para crear el contexto
	 * @param idTareaExterna
	 *            ID de la tarea externa para crear el contexto
	 * @param idTareaProcedimiento
	 *            ID de la tarea del procedimimento
	 * @param contextoOriginal
	 *            Contexto Original para crear el contexto
	 * @param script
	 *            Script que se evaluará
	 * @return Resultado de la evaluación en formato String
	 * @throws Exception
	 *             Exception
	 */
	@BusinessOperation(ComunBusinessOperation.BO_JBPM_MGR_EVALUA_SCRIPT)
	public Object evaluaScript(Long idProcedimiento, Long idTareaExterna, Long idTareaProcedimiento,
			Map<String, Object> contextoOriginal, String script) throws Exception {
	...
	public Object evaluaScriptPorContexto(Long idProcedimiento, Long idTareaExterna, Long idTareaProcedimiento,
			Map<String, Object> contextoOriginal, String script) throws Exception {
		if (!StringUtils.isBlank(script)) {
	...
	/**
	 * Crea un contexto propio para la evaluación en relación con el expediente
	 * y el contexto original y evalua el script que se le pasa como parámetro.
	 * 
	 * @param idExpediente
	 *            ID del expediente para crear el contexto
	 * @param contextoOriginal
	 *            Contexto Original para crear el contexto
	 * @param script
	 *            Script que se evaluará
	...
	public Object evaluaScriptRecobro(Long idExpediente, Map<String, Object> contextoOriginal, String script)
			throws Exception {
		if (!StringUtils.isBlank(script)) {
			Map<String, Object> params = new HashMap<String, Object>();
	...
	/**
	 * Crea un Map de maps con los valores introducidos en las pantallas de las
	 * tareas de los procesos jbpm. AsÃ­ podemos hacer referencia a un valor de
	 * un control en el script con la siguiente nomenclatura.
	 *
	...
	public Map<String, Map<String, String>> creaMapValores(Long idProcedimiento) {
		...
		if (tareas != null) {
			for (TareaExterna tarea : tareas) {
				...
				// Queremos meter todas las tareas aunque sean repetitivas. A
				// las repetitivas le iremos incrementando un contador
				String temp = codigo;
				int contador = 0;

				// Mientras el nombre ya esté en el map, vamos incrementando el
				// contador
				...
				/*
				 * Ya tenemos el nombre de la tarea único en el map, con esto
				 * tendremos:
				 *
				 * tarea[P11_Leer] = xxx <-- Esta es la última vez que se
				 * ejecutó la tarea tarea[P11_Leer_1] = yyy <-- Esta es la
				 * segunda vez que se ejecutó la tarea tarea[P11_Leer_2] = zzz
				 * <-- Esta es la primera vez que se ejecutó esta tarea
				 * tarea[P11_Escribir] = xxx
				 *
				 */
				codigo = temp;

				// Si el código no está en el Map lo introducimos (esto es para
				// evitar que se pase dos veces por la misma tarea)
	...
	private int getNVecesTareaExterna(Long idProcedimiento, Long idTareaProcedimiento) {
		if (idTareaProcedimiento == null || idProcedimiento == null) {
			return 0;
		}
		int resultado = 0;
		try {

			List<TareaExterna> lista = tareaExternaManager.getByIdTareaProcedimientoIdProcedimiento(idProcedimiento,
	 ...
	 *            Tipo de procedimiento que se va a generar
	 * @param procPadre
	 *            Procedimiento padre para copiar sus valores
	...
	public Procedimiento creaProcedimientoHijo(TipoProcedimiento tipoProcedimiento, Procedimiento procPadre) {
		...
		DDEstadoProcedimiento estadoProcedimiento = estadoProcedimientoDao
				.buscarPorCodigo(DDEstadoProcedimiento.ESTADO_PROCEDIMIENTO_DERIVADO);
	 ...
	 * 
	 * @param idProcedimiento
	 *            El id del procedimiento del cual se va a lanzar su BPM
	 * @param idTokenAviso
	 *            Si es llamado desde otro BPM, el id del token al que debe
	 *            avisar. (Este parámetro puede ser null)
	...
	/**
	 * Método de ayuda para obtener una variable tipo Boolean de JBPM. Hemos
	 * comprobado que no convierte bien cuando se guardan objetos boolean. A
	 * veces los devuelve como String
	 * 
	 ...
	 * 
	 * @param idToken
	 *            Id del token del que queremos recuperar
	 * @param variable
	...
	public synchronized void startJobExecutor() {
		...
		if ((value != null) && "true".equals(value)) {
			logger.debug("JobExecutor ignorado por variable de la JVM");
		} else {
			processManager.execute(new JbpmCallback() {
				@Override
				public Object doInJbpm(JbpmContext context) {
