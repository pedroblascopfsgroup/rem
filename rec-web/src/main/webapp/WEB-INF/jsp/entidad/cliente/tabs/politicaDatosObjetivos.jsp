...
var createDatosObjetivosPanel = function() {
    btnNuevo = new Ext.Button({
		...
		,handler: function() {
				...
				var w = app.openWindow({ 
					...
					,params: {idPersona:panel.getPersonaId(),idPolitica:idPolitica}
					,width: 900
    ...
    btnModificar = new Ext.Button({
		...
		,handler: function() {
				...
				var w = app.openWindow({
					...
					,params: {idPersona:panel.getPersonaId(),idObjetivo:idObjetivo,idPolitica:idPolitica}
				});
				w.on(app.event.DONE, function(){
					w.close();
   				    cargarObjetivos(pSeleccionado);
	...
	btnJustificar = new Ext.Button({
		...
		,handler: function() {
				...
				var w = app.openWindow({
					...
					,params: {idPersona:panel.getPersonaId(),idObjetivo:idObjetivo,idEstadoItinerarioPolitica:idEstado,idPolitica:idPolitica}
				});
				w.on(app.event.DONE, function(){
					w.close();
   				    cargarObjetivos(pSeleccionado);
    ...
    objetivosStore = page.getStore({
        event:'listado'
        ,storeId : 'politicaObjetivosStore'
