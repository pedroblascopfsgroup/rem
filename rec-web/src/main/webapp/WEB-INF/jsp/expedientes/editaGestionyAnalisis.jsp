...
<fwk:page>
	...
	var titulogestionesRealizadas = new Ext.form.Label({
   	text:'<s:message code="expedientes.consulta.tabgestion.gestiones" text="**Gestiones Realizadas"/>'
	,style:'font-weight:bolder; font-size:11;margin:10px 10px 10px 10px;'
	}); 
	var gestionesRealizadas=new Ext.form.TextArea({
		fieldLabel:'<s:message code="expedientes.consulta.tabgestion.gestiones" text="**Gestiones Realizadas"/>'
		,width:550
		,hideLabel:true
		,labelStyle:style
		,name:'aaa.gestiones'
		,maxLength: 1024
		,value : '<s:message text="${aaa.gestiones}" javaScriptEscape="true" />'
	...
	var validarForm = function() {
		...
		if(comboTiposPropuestaActuacion.getValue() == null || comboTiposPropuestaActuacion.getValue()== '') {
			return false;
		}
	...
	var comboTiposPropuestaActuacion = app.creaCombo({
		data : <app:dict value="${tiposPropuestaActuacion}"/>
		,name : 'aaa.tipoPropuestaActuacion'
		,fieldLabel : '<s:message code="expedientes.consulta.tabgestion.tipopropuestaactuacion" text="**Tipo Propuesta Actuación"/>'
		,labelStyle:style
		,value : '${aaa.tipoPropuestaActuacion.codigo}'
		,typeAhead: false
        ,mode: 'local'
        ,triggerAction: 'all'
        ,editable: false
	...
	var titulopropuesta = new Ext.form.Label({ 
   	text:'<s:message code="expedientes.consulta.tabgestion.propuesta" text="**Propuesta"/>'
	,style:'font-weight:bolder; font-size:11; margin:10px 10px 10px 10px;'
	});
	var propuesta=new Ext.form.TextArea({
		fieldLabel:'<s:message code="expedientes.consulta.tabgestion.propuesta" text="**Propuesta"/>'
		,width:550
		,hideLabel:true
		,labelStyle:style
		,name:'aaa.propuestaActuacion'
		,maxLength: 1024
		,blankText: 'Campo requerido'
		,value : '<s:message text="${aaa.propuestaActuacion}" javaScriptEscape="true" />'
		<app:test id="campoPropuesta" addComa="true"/> 
	});
