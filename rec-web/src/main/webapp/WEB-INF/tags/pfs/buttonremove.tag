...
<c:choose>
	<%--  	/* BKREC-1349
			* Alternativa al handler de arriba, la diferencia reside en que en el siguiente handler, mostrar un mensaje de  
		 	* 'Guardando...' cuando esta procesando la operaci�n de, oscureciendo la pantalla. 
		 	* De esta forma el user puede ver que al pulsar el bot�n, esta realizando c�lculos, y debe esperar. 
		 	*/ --%> 
	<c:when test="${onSuccessMode == 'tabConMsgGuardando'}">
		var ${name}_handler =  function() {
			if (${datagrid}.getSelectionModel().getCount()>0){
				Ext.Msg.confirm('<s:message code="pfs.tags.buttonremove.borrar" text="**Borrar" />', '<s:message code="pfs.tags.buttonremove.pregunta" text="**�Est� seguro de borrar?" />', function(btn){
    				if (btn == 'yes'){
