...
public class RecoveryAnotacionManager implements RecoveryAnotacionApi,
	...
	public void createAnotacion(DtoCrearAnotacionInfo dto) {
		...
		// FIXME Refactorizar toda esta parte para obtener el nombre y ug. Utilizar una factor�a o algo.
		String nombre = "";
		String ug = "";
		...
		// }
		// } else {
		// //notificacion, se la modificaci�n y se deja una traza
		// Long idTarea = crearTarea(idUg, codUg,dto.getAsuntoMail(),
			...
			try {
				if (user.isIncorporar()) {
					...
					if (user.getFecha() != null) {
						...
						// notificacion, se la modificaci�n y se deja una
						// traza
						Long idTarea = crearTarea(idUg, codUg,
		...
		if (mailsPara.size() > 0) {
			try {
				...
				logger.error("Error enviando los mails de una nueva anotaci�n");
				e.printStackTrace();
				throw new FrameworkException(
						"Error enviando los mails de una nueva anotaci�n");
			}
		}
	...
	/*
	 * Este m�todo es protected para poder hacer un spy durante el testeo
	 */
	...
	private String processCuerpoEmail(String origen, String destino, String destinoCC,
		...
		if (customize != null) {
			...
			//FIXME �C�mo sabemos que el mapa debe tener esa propiedad?, buscar un modo mejor de hacer esto.
	...
	/*
	 * Este m�todo se pone como protected para poder hacer un spy sobre el durante el testeo
	 */
		...
		for (ClaveValor cv : infoRegistro) {
			...
			if (tipo.equals(cv.getClave())) {
				try {
					...
					logger.error("Id de tarea inv�lido en traza de anotaci�n "
							+ cv.getClave());
					return null;
