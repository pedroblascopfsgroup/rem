app.calendario = function() {
	...
	/*
	 * var eventStoreXX = new Ext.ensible.sample.MemoryEventStore({ data : {
	 * evts : [] } });
	 */
	...
	var eventStore = new Ext.data.JsonStore({
		...
		fields : Ext.ensible.cal.EventRecord.prototype.fields.getRange()
				.concat(
						[ 'tipoTarea', 'tipoTareaDescripcion',
								'codigoSubtipoTarea', 'idEntidad',
								'codigoEntidadInformacion',
								'descripcionEntidadInformacion', 'descripcion',
								'descripcionTarea', 'tipoSolicitud',
								'codigoSituacion', 'fcreacionEntidad' ])
	});

	var CalendarStore = Ext
			.extend(
					Ext.data.Store,
					{
							...
							config = Ext
									.applyIf(
											config || {},
											{
												...
												proxy : new Ext.data.MemoryProxy(),
												autoLoad : true,
												fields : Ext.ensible.cal.CalendarRecord.prototype.fields
														.getRange()
											});
							this.reader = new Ext.data.JsonReader(config);
							CalendarStore.superclass.constructor.call(this,
									config);
						}
					});
	...
	var calendars = window.calendars = new CalendarStore({
		data : {
			"calendars" : [ {
				...
				"id" : 600,
				"title" : "Citas",
				// "hidden":true, // optionally init this calendar as hidden by
				// default
				"color" : 5
			}, {
				"id" : 16,
	...
	var abrirSegunTipo = function(rec) {
		...
		if (codigoSubtipoTarea == app.subtipoTarea.CODIGO_TAREA_COMUNICACION_DE_GESTOR
				&& permisosVisibilidadGestorSupervisor(rec.get('gestorId')) == true) {
			codigoSubtipoTarea = app.subtipoTarea.CODIGO_NOTIFICACION_COMUNICACION_RESPONDIDA_DE_SUPERVISOR;
		}
		if (codigoSubtipoTarea == app.subtipoTarea.CODIGO_TAREA_COMUNICACION_DE_SUPERVISOR
				&& permisosVisibilidadGestorSupervisor(rec.get('supervisorId')) == true) {
		...
		case app.subtipoTarea.CODIGO_GESTION_SEGUIMIENTO_SISTEMATICO:
			app.openTab("**Gesti&oacute;n de Seguimiento Sistem&aacute;tico",
					"clientes/listadoClientes", {
						gsis : true
					}, {
						id : 'GSIN',
		...
		case app.subtipoTarea.CODIGO_GESTION_SEGUIMIENTO_SINTOMATICO:
			app.openTab("**Gesti&oacute;n de Seguimiento Sintom&aacute;tico",
					"clientes/listadoClientes", {
						gsin : true
					}, {
						id : 'GSIS',
		...
		case app.subtipoTarea.CODIGO_TAREA_COMUNICACION_DE_SUPERVISOR:
			var w = app
					.openWindow({
						...
						params : {
							...
							codigoTipoEntidad : rec
									.get('codigoEntidadInformacion'),
			...
			w.on(app.event.OPEN_ENTITY, function() {
				...
				if (rec.get('codigoEntidadInformacion') == '1') {
					app.abreCliente(rec.get('idEntidadPersona'), rec
							.get('descripcion'));
				}
				...
				if (rec.get('codigoEntidadInformacion') == '3') {
					app
							.abreAsunto(rec.get('idEntidad'), rec
									.get('descripcion'));
				}
				if (rec.get('codigoEntidadInformacion') == '5') {
					app.abreProcedimiento(rec.get('idEntidad'), rec
							.get('descripcion'));
				}
			});
			break;
			...
			var w = app
					.openWindow({
						...
						params : {
							...
							fecha : rec.get('fcreacionEntidad'),
							situacion : rec.get('codigoSituacion'),
							descripcionTareaAsociada : rec
									.get('descripcionTareaAsociada'),
		...
		case app.subtipoTarea.CODIGO_ASUNTO_PROPUESTO:
			app.abreAsuntoTab(rec.get('idEntidad'), rec.get('descripcion'),
					'aceptacionAsunto');
			break;
		...
		case app.subtipoTarea.CODIGO_RECOPILAR_DOCUMENTACION_PROCEDIMIENTO:
			app.abreProcedimientoTab(rec.get('idEntidad'), rec
					.get('descripcion'), 'docRequerida');
			break;
		...
		default:
			...
			w.on(app.event.OPEN_ENTITY, function() {
				...
				if (rec.get('codigoEntidadInformacion') == '7') {
					app.abreClienteTab(rec.get('idEntidadPersona'), rec
							.get('descripcion'), 'politicaPanel');
				}
			});
			break;
		}
	...
	var parseDate = function(fecha, hora) {
		return Date.parseDate(fecha + " " + hora, 'd/m/yy H:i')
				|| Date.parseDate(fecha + " " + hora, 'd/m/yy')
	};
	...
	var cargarEventos = function() {
		...
		fechaLabel.setText("Visualizando del " + fechas.start.format('d-F-Y')
				+ " al " + fechas.end.format('d-F-Y'));
		Ext.Ajax
				.request({
					...
					params : {
						...
						sort : 'fechaVenc',
						dir : 'ASC',
						fechaVencimientoDesde : fechas.start
								.format('d/m/Y h:m'),
													...
													.push({
														...
														start : parseDate(
																...
																|| parseDate(
																		...
																		ev.horaVenc),
														// end :
														// Date.parseDate(ev.fechaFin,
														// ev.horaFin 'd/m/yy
														// H:i'),
														ad : ev.tipoTarea != TIPO_TAREA_CITA
																|| ev.fechaInicio == '',
	...
	var actualizar = function(cal, rec) {
		...
		id = parseInt(id) ? id : -1;
		Ext.Ajax.request({
			url : '/pfs/calendarioagenda/nuevoCalendarioEvento.htm',
			params : {
	...
	/*
	 * cal.on('eventclick', function(cal,rec){ console.log('cal
	 * eventclick',rec); return false; });
	 */
	...
	cal.on('eventdelete', function(cal, rec) {
		Ext.Ajax.request({
			url : '/pfs/calendarioagenda/borrarCalendarioEvento.htm',
			params : {
				id : rec.get("EventId")
			},
			success : function() {
