...
//		return adjuntosConBorrado;
	 ...
	 * 
	 * @param dto
	 *            los parï¿½metros para la BÃºsqueda.
	...
	/**
	 * Nos dice si el modelo multi-gestor estï¿½ activo o no
	 * 
	 * @return
	 */
...
//			}
	...
	/**
	 * Indica si el usuario puede tomar una acciï¿½n sobre el asunto o si debe
	...
	public Boolean puedeTomarAccionAsunto(Long idAsunto) {
		...
		if (usuUltimaEdicion.getId().longValue() == usuarioLogado.getId().longValue()) {
			// El ï¿½ltimo que hizo una modificaciï¿½n a la ficha fue este
			// usuario, asÃ­ que puede tomar acciï¿½n
			return true;
		}
		return false;
		...
		if (!Checks.estaVacio(gestores)) {
			for (EXTGestorInfo gestor : gestores) {
				...
				if (usuario.getId().equals(gestor.getUsuario().getId())) {
					// FIXME Hay que hacer esto basï¿½ndonos en propiedades del
					// tipo de gestor
    ...
    public String upload(WebFileItem uploadForm) {
		...
		//En caso de que estemos aÃ±adiendo un adjunto desde el propio procedimiento
...
//        asuntoDao.save(asunto);
		...
		//Integracion con IPLUS, si estÃ¡ configurado el flag correspondiente en devon.properties
        if (iplus != null && iplus.instalado()) {
    ...
    /**
     * Recupera el lï¿½mite de tamaï¿½o de un fichero.
     * @return integer
     */
	...
	private Boolean isProcedimientoActivo(Long idProcedimiento) {
		...
		if (!Checks.esNulo(procedimiento)) {
			if (procedimiento.getEstadoProcedimiento().getCodigo().equals(DDEstadoProcedimiento.ESTADO_PROCEDIMIENTO_ACEPTADO)
				...
				//Si el procedimiento no estÃ¡ en un estado factible de tener tareas pendientes, nunca se debe marcar como activo
