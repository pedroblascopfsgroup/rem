...
public class TurnadoProcuradoresManager implements TurnadoProcuradoresApi {
	...
	public boolean checkActivarEsquema(Long id) {
		...
		// No se puede activar un esquema sin configuraci贸n
		if (esquema.getConfiguracion()==null) {
			logger.warn(String.format("No se puede activar el esquema [%d][%s] porque no tiene configuraci贸n", id, esquema.getDescripcion()));
			return false;
		}
	...
	public Boolean comprobarSiProcuradorHaSidoCambiado(Long prcId) {
		...
		//Procurador asignado autom谩ticamente por el turnado de procuradores. Tenemos que consultar en las tablas TUP
	...
	public Boolean comprobarSiLosDatosHanSidoCambiados(Long prcId) {
		...
		for (TareaExterna tarea : tareas) {
			//FIXME No reconoce la cadena entera, falta revisar la comparacion pero as铆 entra
			if ("Redactar demanda y adjuntar documentaci贸n".equals(tarea.getTareaProcedimiento().getDescripcion())) {
		...
		for (TareaExterna tarea : tareasAnterior) {
			if ("Validar asignaci贸n".equals(tarea.getTareaProcedimiento().getDescripcion())) {
				List<EXTTareaExternaValor> valores = subastaProcedimientoApi.obtenerValoresTareaByTexId(tarea.getId());
				for (EXTTareaExternaValor valor : valores) {
	...
	@Override
	@Transactional
	public List<Long> a馻dirNuevoTpoAPlazas(Long idEsquema, String codTPO, String[] arrayPlazas) {
