...
BEGIN    
    ...
    --** Motivo de Marca Refinanciación
    v_num_tablas:=0;
    v_msql:='Select count(1) From '||V_ESQUEMA||'.DD_RULE_DEFINITION Where rd_title like ''Motivo Marca R''';
    execute immediate v_msql into v_num_tablas;
    If V_NUM_TABLAS > 0
     Then 
       DBMS_OUTPUT.put_line('[INFO] Varible ''Motivo Marca R.''');
     Else
       DBMS_OUTPUT.put_line('[INFO] Varible ''Motivo Marca R.''');       
       v_msql:='Insert into '||V_ESQUEMA||'.DD_RULE_DEFINITION (rd_id, rd_title, rd_column, rd_type, rd_value_format, rd_bo_values, rd_tab, usuariocrear, fechacrear, borrado)
                Values ('||V_ESQUEMA||'.S_DD_RULE_DEFINITION.nextval, ''Motivo Marca R'', ''dd_mom_id'', ''dictionary'', ''number'', ''DDMotivoMarcaR'', ''Contrato'', ''PFS'', sysdate, 0)';
       execute immediate v_msql;
       DBMS_OUTPUT.put_line('[INFO] Varible ''Motivo Marca R'' insertada correctamente.');
     End If;
