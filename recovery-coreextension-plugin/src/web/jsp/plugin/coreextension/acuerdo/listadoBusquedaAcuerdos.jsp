...
<%-----------------Campos Pesta�a Datos Generales -----------------------%>
	
	var limit=25;
	
	//Campo N�mero Contrato
	var nroContrato = app.creaText('codigo', '<s:message code="acuerdo.busqueda.filtro.contrato" text="**C�d. Contrato" />'); 

	//Campo Numero Cliente
	<%--var codigoCliente = app.creaText('codigo', '<s:message code="acuerdo.busqueda.filtro.cliente" text="**C�d. Cliente" />'); --%>
	var codigoCliente=app.creaNumber('codigo','<s:message code="acuerdo.busqueda.filtro.cliente" text="**C�d. Cliente" />');
...
<%-----------------Campos Pesta�a Jerarqu�a -----------------------%>
	...
	var comboJerarquia = app.creaCombo({triggerAction: 'all',
	 	...
	 	name : 'jerarquia', 
	 	fieldLabel : '<s:message code="acuerdo.busqueda.filtros.jerarquia" text="**Jerarqu�a" />'});
	...
	var zonasStore = page.getStore({
	 ...
	 var zonasCM = new Ext.grid.ColumnModel([
		{header : '<s:message code="expedientes.listado.centros.codigo" text="**C�digo" />', dataIndex : 'codigoZona' ,sortable:false, hidden:false, width:80}
	...
	var buscarFunc = function()
	{
		if(validarForm()){
			...
			Ext.Msg.alert('<s:message code="errores.criteriosIncompatibles" text="**Los criterios de jerarquía no se pueden usar con el origen Todos" />');
		}
	};
	    	...
	    	var rec = grid.getStore().getAt(rowIndex);
	    		...
	    		if(rec.get('idAsunto')){
	    			...
	    			if(rec.get('idExpediente')){
	    		    	...
	    		    	alert('El acuerdo no tiene asociado ning�n expediente o asunto');
	    		    	}
	    		}
