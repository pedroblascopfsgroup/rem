...
CREATE OR REPLACE FORCE VIEW ugas001.vtar_tarea_vs_tge (dd_tge_id_pendiente,
                                                        dd_tge_id_espera,
                                                        dd_tge_id_alerta,
                                                        tar_id,
                                                        cli_id,
...
-- tipo de gestor para las alertas
          ...
          tar.*
     FROM tar_tareas_notificaciones tar JOIN ugasmaster.dd_sta_subtipo_tarea_base sta
...
BUILD IMMEDIATE
REFRESH COMPLETE
START WITH TO_DATE('24-ago-2012 07:05:00','dd-mon-yyyy hh24:mi:ss')
NEXT trunc(sysdate+1)+7/24              
WITH PRIMARY KEY
AS 
/* Formatted on 2012/09/06 17:06 (Formatter Plus v4.8.8) */
SELECT usd.usu_id, usd.des_id, ges.dd_tge_id, asu.*
...
from vtar_asu_vs_usu asu
    join ugasmaster.usu_usuarios usu on asu.usu_id = usu.usu_id
       ...
       SELECT DISTINCT ptes.usu_id usu_pendientes,
                             ...
                             (SELECT CASE
                                ...
                                FROM tar_tareas_notificaciones tn,
                                     ugasmaster.dd_sta_subtipo_tarea_base stb
                        ...
                        FROM vtar_tarea_vs_tge vtar 
                             ...
                             LEFT JOIN ugasmaster.dd_ein_entidad_informacion tpo_ent ON vtar.dd_ein_id = tpo_ent.dd_ein_id
                             LEFT JOIN ugasmaster.dd_sta_subtipo_tarea_base sbt_tar ON vtar.dd_sta_id = sbt_tar.dd_sta_id
                             LEFT JOIN ugasmaster.dd_est_estados_itinerarios est on ptes.dd_est_id = est.dd_est_id
             ...
             SELECT t.tar_id_dest usu_pendientes, -1 usu_espra, -1 usu_alerta,
               ...
               FROM vtar_tarea_vs_tge t 
                    ...
                    LEFT JOIN dd_tpo_tipo_procedimiento tpo ON prc.dd_tpo_id = tpo.dd_tpo_id
                    LEFT JOIN ugasmaster.dd_ein_entidad_informacion tpo_ent ON t.dd_ein_id = tpo_ent.dd_ein_id
                    LEFT JOIN ugasmaster.dd_sta_subtipo_tarea_base sbt_tar ON t.dd_sta_id = sbt_tar.dd_sta_id
                    LEFT JOIN ugasmaster.dd_est_estados_itinerarios est on asu.dd_est_id = est.dd_est_id
