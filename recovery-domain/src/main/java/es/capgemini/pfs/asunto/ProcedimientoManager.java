...
public class ProcedimientoManager {
    ...
    /**
     * Devuelve las personas asociadas al contrato, indicando si también están afectadas al procedimiento.
     * @param idContrato el id del contrato
     * @param idProcedimiento el id del procedimiento
     * @return la lista de personas asociadas y un booleano que indica si está o no asociada al procedimiento.
     */
    ...
    public Procedimiento getProcedimiento(Long idProcedimiento) {
    	EventFactory.onMethodStart(this.getClass());
        if (idProcedimiento == 0L) { return null; }
    ...
    /**
     * Devuelve una lista de tipos de procedimiento, filtrado por tipo de actuación
    ...
    public Long salvarProcedimiento(ProcedimientoDto dto) {
        ...
        // Al crear una nueva lista borramos la anterior (de contratos relacionados con el proc.)
        if (dto.getContratosAfectados() == null) {
            ...
            for (int i = 0; i < idsExpedienteContratos.length; i++) {
                listado.add((ExpedienteContrato) executor.execute(InternaBusinessOperation.BO_EXP_MGR_GET_EXPEDINTE_CONTRATO, Long
                        .decode(idsExpedienteContratos[i])));
            }
     ...
     * @param contratos String: listado de los ids de contratos, separados por "-"
     * @param idProcedimiento Long: id del procedimiento
    ...
    public List<DtoProcedimientoPersona> getPersonasDeLosContratosProcedimiento(String contratos, Long idProcedimiento) {
        ...
        // DTO con la lista anterior más el check que indica si ya estaba marcado en el procedimiento
     ...
     * @param idProcedimiento el id del procedimiento a borrar.
     */
    ...
    public void borrarProcedimiento(Long idProcedimiento) {
        ...
        DDEstadoProcedimiento estadoProcedimiento = (DDEstadoProcedimiento) executor.execute(ComunBusinessOperation.BO_DICTIONARY_GET_BY_CODE,
                DDEstadoProcedimiento.class, DDEstadoProcedimiento.ESTADO_PROCEDIMIENTO_CANCELADO);

        proc.setEstadoProcedimiento(estadoProcedimiento);

        //procedimientoContratoExpedienteDao.actualizaCEXProcedimiento(proc.getProcedimientosContratosExpedientes(), idProcedimiento);
    ...
    private Procedimiento saveOrUpdateProcedimiento(ProcedimientoDto dto) {
        ...
        if (dto.getIdProcedimiento() != null) {
            ...
            p = new Procedimiento();
            DDEstadoProcedimiento estadoProcedimiento = (DDEstadoProcedimiento) executor.execute(ComunBusinessOperation.BO_DICTIONARY_GET_BY_CODE,
                    DDEstadoProcedimiento.class, DDEstadoProcedimiento.ESTADO_PROCEDIMIENTO_EN_CONFORMACION);
        ...
        if (!dto.getEnConformacion()) {
            ...
            DDTipoActuacion tipoActuacion = (DDTipoActuacion) executor.execute(ComunBusinessOperation.BO_DICTIONARY_GET_BY_CODE,
                    DDTipoActuacion.class, dto.getActuacion());
            ...
            DDTipoReclamacion tipoReclamacion = (DDTipoReclamacion) executor.execute(ComunBusinessOperation.BO_DICTIONARY_GET_BY_CODE,
                    DDTipoReclamacion.class, dto.getTipoReclamacion());
    ...
    public void crearTareaRecopilarDocumentacion(Procedimiento procedimiento) {
        Long idBPM = (Long) executor.execute(ComunBusinessOperation.BO_TAREA_MGR_CREAR_TAREA_CON_BPM_CON_ESPERA, procedimiento.getId(),
                DDTipoEntidad.CODIGO_ENTIDAD_PROCEDIMIENTO, SubtipoTarea.CODIGO_RECOPILAR_DOCUMENTACION_PROCEDIMIENTO,
                PlazoTareasDefault.CODIGO_RECOPILAR_DOCUMENTACION, true);
    ...
    public List<Bien> getBienesDeUnProcedimiento(Long idProcedimiento) {
        ...
        List<Persona> personas = procedimientoDao.get(idProcedimiento).getPersonasAfectadas();
        for (Persona per : personas) {
            for (Bien bienPer : per.getBienes()) {
                if (!bienes.contains(bienPer)) {
                    bienes.add(bienPer);
                }
            }
        }
    ...
    public DtoFormularioDemanda formularioDemanda(Long idProcedimiento) {
        ...
        dto.setNombreAbogado(procedimiento.getAsunto().getGestor().getUsuario().getApellidoNombre() + " ("
                + procedimiento.getAsunto().getGestor().getDespachoExterno().getDespacho() + ")");
        ...
        for (Persona persona : procedimiento.getPersonasAfectadas()) {
            ...
            if (persona.getDirecciones().size() > 0) {
                nombresDomiciliosDemandados += ", cuyo domicilio a efectos de comunicaciones en este procedimiento, radica en "
                        + persona.getDirecciones().get(0);
            }
