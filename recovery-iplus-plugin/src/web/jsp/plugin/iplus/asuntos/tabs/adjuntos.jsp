...
(function(page,entidad){
	...
	var crearBotonBorrar = function(config){
		...
		var cfg = {
			...
			,borrar : function(){
				...
				params.nombre = rec.get("nombre");
				params.descripcion = rec.get("descripcion");
				config.page.webflow({
					flow : config.flow
					,params : params || {}
					,success : function() {
	...
	var borrar = crearBotonBorrar({
		page : page
		,flow : 'iplus/borrarAdjunto'
		,params : {}
		,success : function(){
			recargarAdjuntos();
		}
...
<sec:authorize ifAllGranted="PERSONALIZACION-BCC">		    
	...
	grid.on('rowdblclick', function(grid, rowIndex, e){
		var rec = grid.getStore().getAt(rowIndex);
		if (rec.get('id') == "") {
			//alert("No tiene id de adjunto: " + "," + panel.getAsuntoId() + "," + rec.get('nombre') + "," + rec.get('descripcion'));
			var flow = '/pfs/iplus/bajarDocumento';
			var params = {idAsunto : panel.getAsuntoId(), nombre : rec.get('nombre'), descripcion : rec.get('descripcion')};
