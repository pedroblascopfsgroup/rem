package es.pfsgroup.plugin.recovery.masivo.callbacks.admisionDemanda.api.impl;
...
@Component
public class MSVAdmisionDemandaCallback extends
		AbstractRecoveryBPMfwCallbackBOTemplate implements
		MSVAdmisionDemandaCallbackApi {
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ADMISION_DEMANDA_CALLBACK_ONEND)
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ADMISION_DEMANDA_CALLBACK_ONERROR)
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ADMISION_DEMANDA_CALLBACK_ONSTART)
	public void onStartProcess(Long tokenProceso) {
		...
		try {
			proxyFactory.proxy(MSVProcesoApi.class).modificarProcesoMasivo(
					dtoUpdateEstado);
		} catch (Exception e1) {
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ADMISION_DEMANDA_CALLBACK_ONSUCCESS)
	public void onSuccess(Long tokenProceso, RecoveryBPMfwkInputDto input) {
//		boolean actualizaProc = false;
//		try {
//			Procedimiento procedimiento = proxyFactory.proxy(ProcedimientoApi.class).getProcedimiento(input.getIdProcedimiento());
//			
//			if (Checks.esNulo(procedimiento)) {
//				this.onError(tokenProceso, input, "No se encuentra el procedimiento");
//			} else {
//				BigDecimal principal = MSVUtils.getBigDecimal(String.valueOf(input.getDatos().get(MSVAdmisionDemandaDinamicFields.getFields().get(MSVAdmisionDemandaColumns.PRINCIPAL))));
//				if (!Checks.esNulo(principal)) {
//					procedimiento.setSaldoRecuperacion(principal);
//					actualizaProc = true;
//				}
//				
//				String partidoJudicial = String.valueOf(input.getDatos().get(MSVAdmisionDemandaDinamicFields.getFields().get(MSVAdmisionDemandaColumns.PARTIDO_JUDICIAL)));
//				if (!Checks.esNulo(partidoJudicial) && !"null".equals(partidoJudicial)) {				
//					List<TipoJuzgado> juzgados =  proxyFactory.proxy(JuzgadoDao.class).getJuzgadosByPlaza(partidoJudicial);
//					if (!Checks.estaVacio(juzgados)) {
//						TipoJuzgado juzgado = juzgados.get(0);
...
//				
//				if (actualizaProc) {
//					genericDao.update(Procedimiento.class,procedimiento);
//				}
//			}
//		} catch (Exception e) {
//			this.onError(tokenProceso, input, "Se ha producido un error procesado el input: "+e.getMessage());
//		}
	}
