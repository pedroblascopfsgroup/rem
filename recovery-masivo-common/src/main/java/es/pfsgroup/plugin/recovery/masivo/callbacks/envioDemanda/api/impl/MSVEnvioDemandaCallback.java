package es.pfsgroup.plugin.recovery.masivo.callbacks.envioDemanda.api.impl;
...
@Component
public class MSVEnvioDemandaCallback extends
		AbstractRecoveryBPMfwCallbackBOTemplate implements
		MSVEnvioDemandaCallbackApi {
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ENVIO_DEMANDA_CALLBACK_ONEND)
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ENVIO_DEMANDA_CALLBACK_ONERROR)
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ENVIO_DEMANDA_CALLBACK_ONSTART)
	public void onStartProcess(Long tokenProceso) {
		MSVProcesoMasivo msvProcesoMasivo = apiProxy.proxy(MSVProcesoApi.class).getByToken(tokenProceso);
		if (Checks.esNulo(msvProcesoMasivo)) {
			throw new BusinessOperationException("No se encuentra el proceso masivo para el token: " + tokenProceso);
		}
		MSVHojaExcel hojaExcel = getHojaExcel(msvProcesoMasivo.getId());
	 ...
	 */
	@Override
	@BusinessOperation(MSV_ENVIO_DEMANDA_CALLBACK_ONSUCCESS)
