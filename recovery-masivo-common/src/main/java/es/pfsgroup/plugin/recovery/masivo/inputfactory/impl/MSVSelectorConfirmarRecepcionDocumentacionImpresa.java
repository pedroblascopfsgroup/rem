...
@Component
public class MSVSelectorConfirmarRecepcionDocumentacionImpresa implements MSVSelectorTipoInput{

	@Override
	public String getTipoInput(Map<String, Object> map) {
		return MSVDDOperacionMasiva.CODIGO_INPUT_DOCUMENTACION_RECIBIDA;	
	}

	@Override
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODIGO_CONFIRMAR_RECEPCION_DOCUMENTACION_IMPRESA.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
