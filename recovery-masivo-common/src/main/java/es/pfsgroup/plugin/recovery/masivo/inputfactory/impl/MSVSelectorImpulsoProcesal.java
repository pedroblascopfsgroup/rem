...
@Component
public class MSVSelectorImpulsoProcesal implements MSVSelectorTipoInput{

	@Override
	public String getTipoInput(Map<String, Object> map) {
		return MSVDDOperacionMasiva.CODIGO_INPUT_IMPULSO_PROCESAL_BATCH;	
	}

	@Override
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODIGO_IMPULSO_PROCESAL.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
	...
	public String getTipoResolucion() {
		return MSVDDOperacionMasiva.CODIGO_IMPULSO_PROCESAL;
	}
