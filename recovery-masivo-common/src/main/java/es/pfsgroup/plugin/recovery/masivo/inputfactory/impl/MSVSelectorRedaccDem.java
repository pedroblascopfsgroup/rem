...
@Component
public class MSVSelectorRedaccDem implements MSVSelectorTipoInput{

	@Override
	public String getTipoInput(Map<String, Object> map) {
		return MSVDDOperacionMasiva.CODIGO_INPUT_REDACC_DEM_BATCH;	
	}

	@Override
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODIGO_REDACCION_DEMANDA.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
