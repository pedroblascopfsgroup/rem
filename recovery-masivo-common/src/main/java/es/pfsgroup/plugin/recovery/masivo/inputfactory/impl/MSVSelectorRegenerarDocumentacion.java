...
@Component
public class MSVSelectorRegenerarDocumentacion implements MSVSelectorTipoInput{

	@Override
	public String getTipoInput(Map<String, Object> map) {
		return MSVDDOperacionMasiva.CODIGO_INPUT_REGENERAR_DOCUMENTACION;	
	}

	@Override
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODIGO_REGENERAR_DOCUMENTACION.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
