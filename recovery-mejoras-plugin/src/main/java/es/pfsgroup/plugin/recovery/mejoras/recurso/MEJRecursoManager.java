...
public class MEJRecursoManager implements MEJRecursoAPI {
	...
	private void crearTareaActualizarEstado(Recurso recurso) {
		...
		if (proxyFactory.proxy(AsuntoApi.class).esGestor(procedimiento.getAsunto().getId())) {
			idSubtipoTarea = SubtipoTarea.CODIGO_ACTUALIZAR_ESTADO_RECURSO_GESTOR;
		} else if (proxyFactory.proxy(AsuntoApi.class).esSupervisor(procedimiento.getAsunto().getId())) {
