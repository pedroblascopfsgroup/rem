...
@Service
public class MEJGenericFormManager extends BusinessOperationOverrider<GenericFormManagerApi> implements GenericFormManagerApi {
	...
	public void saveValues(DtoGenericForm dto) throws Exception {
		...
		for (int i = 0; i < valores.length; i++) {
			...
			// Si tiene un combo de tipo juzgado, le seteamos al procedimiento
			...
			//si tiene un campo num�rico de principalDemanda, lo seteamos al procedimiento
			if ("principalDemanda".equals(item.getNombre())) {
				prc.setSaldoRecuperacion(new BigDecimal(valores[i]));
				procedimientoDao.saveOrUpdate(prc);
			}

			// Si el tipo de dato es un n�mero de procedimiento lo seteamos al
			// procedimiento
		...
		// Le insertamos los valores del formulario al BPM en una variable de
