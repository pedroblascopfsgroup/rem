...
public class ActualizarActuacionTest {
	...
	@Before
	public void before() {
		// TODO Pendiente de arreglar. Comentado solo para pasar validación de
		// HUDSON
		/*
		 * manager = new MEJDecisionProcedimientoManager(); executor =
		 * mock(Executor.class); DInjector di = new DInjector(manager);
		 * di.inject("executor", executor);
		 */
	}
	...
	public void testActualizarActuacion() throws Exception {
		...
		// HUDSON
		/*
		 * MEJDtoProcedimientoDerivado dto = creaDTO(); ProcedimientoDerivado pd
		 * = mock(ProcedimientoDerivado.class); Procedimiento p =
		 * mock(Procedimiento.class);
		 * when(p.getId()).thenReturn(ID_PROCEDIMIENTO_DERIVADO);
		 * when(pd.getProcedimiento()).thenReturn(p); DecisionProcedimiento
		 * decision = creaDecisionBuena(pd);
		 * obtenerDecision(ID_DECISION,decision);
		 *
		 * manager.actualizarActuacion(ID_DECISION, dto);
		 *
		 * verify(pd,never()).setId(anyLong()); verify(pd,
		 * never()).setDecisionProcedimiento(any(DecisionProcedimiento.class));
		 * verify(pd, never()).setProcedimiento(any(Procedimiento.class));
		 *
		 * verify(p).setTipoActuacion(obtenerTipoActuacion(dto));
		 * verify(p).setTipoProcedimiento(obtenerTipoProcedimiento(dto));
		 ...
		 * verify(p).setPersonasAfectadas(obtenerPersonasAfectadas());
		 *
		 * verify(executor).execute(ExternaBusinessOperation.
		 * BO_PRC_MGR_SAVE_OR_UPDATE_PROCEDIMIMENTO,p);
		 */
	}

	@Test
	public void testActualizarActuacion_decisionInexistente_Excepcion()
			throws Exception {
		// TODO Pendiente de arreglar. Comentado solo para pasar validación de
		// HUDSON
		/*
		 * MEJDtoProcedimientoDerivado dto = new MEJDtoProcedimientoDerivado();
		 * obtenerDecision(ID_DECISION, null); try{
		 * manager.actualizarActuacion(ID_DECISION, dto);
		 * fail("Deber�a  haberse lanzado una excepci�n"); }catch
		 * (BusinessOperationException e) { assertTrue(true); }
		 */
	}

	@Test
	public void testActualizarActuacion_procedimientoIncorrecto_Excepcion()
			throws Exception {
		// TODO Pendiente de arreglar. Comentado solo para pasar validación de
		// HUDSON
		/*
		 * MEJDtoProcedimientoDerivado dto = creaDTO();
		 * obtenerDecision(ID_DECISION, creaDecisionBuena(null)); try{
		 * manager.actualizarActuacion(ID_DECISION, dto);
		 * fail("Deber�a  haberse lanzado una excepci�n"); }catch
		 * (BusinessOperationException e) { assertTrue(true); }
		 */
	...
	private void obtenerDecision(Long id, DecisionProcedimiento decision) {
		when(executor.execute(ExternaBusinessOperation.BO_DEC_PRC_MGR_GET, id))
				.thenReturn(decision);
	}
	...
	private DDTipoReclamacion obtenerTipoReclamacion(
			MEJDtoProcedimientoDerivado dto) {
		// return (DDTipoReclamacion)
		// executor.execute(PluginMejorasBOConstants.BO_TRE_MGR_GET_BY_CODIGO,dto.getTipoReclamacion());
		return null;
	}
	...
	private TipoProcedimiento obtenerTipoProcedimiento(
		...
		// executor.execute(PluginMejorasBOConstants.BO_TPO_MGR_GET_BY_CODIGO,dto.getTipoProcedimiento());
		return null;
	}

	private DDTipoActuacion obtenerTipoActuacion(MEJDtoProcedimientoDerivado dto) {
		// return (DDTipoActuacion)
		// executor.execute(PluginMejorasBOConstants.BO_TAC_MGR_GET_BY_CODIGO,dto.getTipoActuacion());
		return null;
	}
