...
<%-- 	var idAsunto = '${asunto.id}'; --%>
	...
	var comboTipoAcuerdo = new Ext.form.ComboBox({
		...
		,triggerAction: 'all'
		,fieldLabel: (ambito!='asunto') ? '<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.terminos.agregar.flujo.tipoTermino" text="**Soluci贸n Propuesta" />':'<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.terminos.agregar.flujo.tipoAcuerdo" text="**Tipo termino" />'
	...
	var comboSubTipoAcuerdo = new Ext.form.ComboBox({
		...
		,emptyText:'Seleccionar'
		,triggerAction: 'all'
		,fieldLabel: (ambito!='asunto') ? '<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.terminos.agregar.flujo.subtipoTermino" text="**Subtipo Soluci贸n Propuesta" />' :'<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.terminos.agregar.flujo.subtipoAcuerdo" text="**Subtipo termino" />'
		,labelStyle: 'width:150px'
		,width: 150				
	});
	...
	var flujoFieldSetContenedor = new Ext.form.FieldSet({
		title:'<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.terminos.agregar.flujo.titulo" text="**Flujo por soluci贸n"/>' 
		,border:true
		,items: [flujoFieldSet]
	});	
       			...
       			if (Ext.getCmp('cargasPosterioresAnteriores')!=undefined) {
       				if (Ext.getCmp('cargasPosterioresAnteriores').value==1) {
       					if (valoracionCargas.value=='' || descripcionCargas.value=='') {
       						Ext.Msg.show({
       							title:'Aviso',
       							msg: '<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.camposCargas.obligatorios" text="**Debe rellenar la informaci髇 para la Carga" />',
       							buttons: Ext.Msg.OK
       						});
       						return false;
       					}
				...
				if (Ext.getCmp('otrosBienesSolvencia')!=undefined) {
					if (Ext.getCmp('otrosBienesSolvencia').value==1) {
						if (valoracionBienesSolvencia.value=='' || descripcionBienesSolvencia.value=='') {
      						Ext.Msg.show({
       							title:'Aviso',
       							msg: '<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.camposSolvencia.obligatorios" text="**Debe rellenar la informaci髇 para Otros Bienes/Solvencia" />',
       							buttons: Ext.Msg.OK
       						});
       						return false;						
						}
 ...
 --%>	
	...
	var comboBienes = app.creaDblSelect(null,'<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.terminos.agregar.bienes.combo" text="**Bienes del asunto/Bienes para daci贸n" />',config); 
	
	

	var bienesFieldSet = new Ext.form.FieldSet({
		title:'<s:message code="plugin.mejoras.acuerdos.tabTerminos.terminos.terminos.agregar.bienes.titulo" text="**Bienes de la propuesta y garant铆as seleccionadas"/>'
		,layout:'form'
		,autoWidth: true
		,autoHeight: true
		,border:true
