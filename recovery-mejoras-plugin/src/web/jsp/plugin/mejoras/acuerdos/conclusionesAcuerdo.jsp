...
var crearConclusiones=function(){
	...
	//MOTIVO
	var motivo = new Ext.form.TextArea({
		fieldLabel:'<s:message code="plugin.mejoras.acuerdos.motivo" text="**Motivo"/>'
		,width:200
		,readOnly:true
		,labelStyle:labelStyle
		<app:test id="motivo" addComa="true"/>	
	});
	
	motivo.setValue('<s:message javaScriptEscape="true" text="${acuerdo.motivo}"/>' );
		...
		var btnEditConclusiones = new Ext.Button({
	       ...
	       ,handler:function(){
	      	       var w = app.openWindow({
			          flow : 'plugin/mejoras/acuerdos/plugin.mejoras.acuerdos.editarAcuerdo'
			          ,closable:false
			          ,width : 800
