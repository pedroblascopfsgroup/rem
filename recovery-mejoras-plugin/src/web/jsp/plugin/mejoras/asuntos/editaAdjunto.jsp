...
<fwk:page>
	...
	<pfs:editForm saveOrUpdateFlow="plugin/mejoras/asuntos/plugin.mejoras.asuntos.guardaDescripcionAdjExpediente"
	...
	var btnGuardarComprobacion = new Ext.Button({
		...
		,handler : function(){
			if(!validarDescripcion()){
				Ext.Msg.alert('<s:message code="plugin.mejoras.adjunto.descripcion.demasiadoLarga"/>','<s:message code="plugin.mejoras.adjunto.descripcion.demasiadoLarga.mensaje"/>')
			}
			else{
				page.webflow({
	...
	btnGuardar.hide();
	btnGuardarComprobacion.add();
	topToolbar().setHeight(filtroForm.getTopToolbar().getHeight());--%>
