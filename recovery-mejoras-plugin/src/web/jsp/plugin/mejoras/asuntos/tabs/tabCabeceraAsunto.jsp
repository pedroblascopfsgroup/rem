...
(function(){
	...
	var asunto=app.creaLabel('<s:message code="asunto.tabcabecera.asunto" text="**Asunto"/>','<s:message text="${asunto.nombre}" javaScriptEscape="true" />',{labelStyle:'font-weight:bolder;width:100px'});
	...
	var recargar = function (){
		app.abreAsuntoTab(${asunto.id}
							,'<s:message text="${asunto.nombre}" javaScriptEscape="true" />'
							, asuntoTabPanel.getActiveTab().initialConfig.nombreTab);
	};
	
	<pfs:defineParameters name="paramIdAsunto" paramId="${asunto.id}"/>
	<pfs:buttonedit name="btnEditarNombre" 
		flow="editasunto/open"  
		windowTitleKey="plugin.mejoras.asunto.tabCabecera.editar" 
		parameters="paramIdAsunto" 
		windowTitle="**Editar Nombre del Asunto"
		on_success="recargar"/>
	...
	panelNombreAsunto.setWidth(350);	
		...
		,items : [
				  ...
				  { items:[ panelNombreAsunto,codigoAsunto,fecha,estado,expediente]}
   ...
   procedimientosStore.webflow({id:'${asunto.id}'});
	...
	var procedimientosCm = new Ext.grid.ColumnModel([
		...
		,{header : '<s:message code="asunto.tabcabecera.grid.procedimientoPadre" text="**Procedimiento Padre"/>', dataIndex : 'procedimientoPadre',align:'right' }
		...
		,{header : '<s:message code="asunto.tabcabecera.grid.fechainicio" text="**fecha inicio"/>', dataIndex : 'fechaInicio',hidden:true,width:65}
