...
<%-- AS-144 Se comenta para deshabilitar el mensaje en caso de timeout
	var noData = new Ext.util.DelayedTask(function(){
		if(maskPanel!=null) {		
			if (maskPanelShow) {
				Ext.Msg.alert('<s:message code="fwk.ui.errorList.fieldLabel"/>','<s:message code="plugin.mejoras.clientes.listado.noData" text="**No se han encontrado datos, pruebe a ajustar la bï¿½squeda"/>')
				unmaskAll();
			}
		}	
	});
    ...
    //diccionario de si/no nï¿½mina o pensiï¿½n
	...
	var comboIntervencion= app.creaCombo({
		...
		,name : 'intervencion'
		,hidden: false
		,fieldLabel : '<s:message code="menu.clientes.listado.filtro.intervencion" text="**Tipo de Intervenciï¿½n" />'
		,width : 175
	});
	...
	var comboNominaPension=app.creaCombo({
		...
		,name : 'nominaPension'
		,fieldLabel : '<s:message code="menu.clientes.listado.filtro.nominaPension" text="**Nï¿½mina o pension" />'
		,width : 175
	});
	...
	comboTipoRiesgo.on('select', function(){
		...
		//Si ha seleccionado cualquier otra cosa que no sea la vacï¿½a
		else 
		{
...
<%--   var recargarComboZonas = function(){
	...
	//utilizamos implicitamente el mismo flow que ha cargado el grid, si no, le pasarï¿½amos flow : 'admin/listadoUsuarios'
	...
	var filtroSolvenciaRevisada_dict = {"diccionario":[{"codigo":"T","descripcion":"---"},{"codigo":"SI","descripcion":"Sï¿½"},{"codigo":"NO","descripcion":"No"}]}; 
			...
			if (validaTipoIntervencion()) {
				if (validaZonaJerarquia()) {
					if (validaMinMax()){
						...
						//Cerramos el panel de filtros y esto harï¿½ que se abra el listado de clientes
					}else{
	...
	<%--*************PESTAï¿½A DE RIESGOS DEL CLIENTE***************************************** --%>
	...
	var filtrosTabGestion = new Ext.Panel({
		title:'<s:message code="plugin.mejoras.clientes.busqueda.gestion" text="*Gestiï¿½n" />'
		,autoHeight:true
		,bodyStyle:'padding: 10px'
		,layout:'table'
		,layoutConfig:{columns:2}
	...
	**************************************************************************************** --%>
	...
	Ext.onReady(function(){
		//si entramos por gestiï¿½n de vencidos, el combo se seleccionarï¿½ con el valor de gestiï¿½n de vencidos automï¿½ticamente, 
	//y la carga inicial serï¿½ con este filtro
