...
<fwk:page>
		...
		var buttons = <app:includeArray files="${buttonsProcedimiento}" />;
	...
	//tab historico
	<%@ include file="/WEB-INF/jsp/procedimientos/tabHistoricoProcedimiento.jsp" %>
	...
	var botonRefrezcar = new Ext.Button({
		...
		,handler: function() {
			alert('${tarea}')
			if ('${tarea}!=null'){
				app.openTab('<s:message text="${procedimiento.nombreProcedimiento}" javaScriptEscape="true" />', 'procedimientos/consultaProcedimiento', {id:'${procedimiento.id}',tarea:'${tarea}',fechaVenc:'${fechaVenc}',nombreTab:'decision'} , {id:'procedimiento'+id,iconCls:'icon_procedimiento'});
			}else{
