...
(function(page,entidad){
    ...
    var contratosRDCm= new Ext.grid.ColumnModel([
   	...
   	<sec:authorize ifAllGranted="ROLE_INCLUIR_CONTRATO_PROCEDIMIENTO"> 
    var incluirContrato = new Ext.Button({
		text : '<s:message code="plugin.mejoras.procedimiento.consulta.tabcontratos.incluirContrato" text="**Incluir contratos" />'
		,iconCls : 'icon_mas'
		,cls: 'x-btn-text-icon'
        ,handler:function() {
        		var flowVar = 'plugin/mejoras/procedimientos/plugin.mejoras.procedimientos.incluirContrato';
       			if(data.tipoActuacion == 'CO'){
               		flowVar = 'procedimientos/incluirContratoProcedimiento';
       			}
				var widthPantalla = (app.contenido.getSize(true).width || 900)+20;
				var w = app.openWindow({
					flow: flowVar
					,eventName: 'busquedaContrato'
					,params: {idProcedimiento:idProcedimiento,busquedaOrInclusion:'inclusion'}
					,title: '<s:message code="plugin.mejoras.procedimiento.consulta.tabcontratos.incluirContrato" text="**Incluir contratos" />'
					,width: widthPantalla
				});
    ...
    var excluirContrato = new Ext.Button({
		text : '<s:message code="plugin.mejoras.procedimiento.consulta.tabcontratos.excluirContrato" text="**Excluir contratos" />'
		,iconCls : 'icon_menos'
		,cls: 'x-btn-text-icon'
        ,handler:function() {
				var widthPantalla = (app.contenido.getSize(true).width || 900)+20;
				var w = app.openWindow({
					flow: 'plugin/mejoras/procedimientos/plugin.mejoras.procedimientos.excluirContrato'
					,params: {idProcedimiento:idProcedimiento}
					,title: '<s:message code="plugin.mejoras.procedimiento.consulta.tabcontratos.excluirContrato" text="**Excluir contratos" />'
					,width: widthPantalla
				});
				w.on(app.event.DONE, function() {
					w.close();
    ...
    var contratosGrid=app.crearGrid(contratoStore,contratosRDCm,{
           ...
           <sec:authorize ifAllGranted="ROLE_INCLUIR_CONTRATO_PROCEDIMIENTO"> ,bbar: [incluirContrato,excluirContrato]</sec:authorize>
    });
