...
(function(){
	...
	var recargarAdjuntos = function (){
		store.webflow({id:${procedimiento.asunto.id}});
		storePersonas.webflow({id:${procedimiento.asunto.id}});
		storeExpedientes.webflow({id:${procedimiento.asunto.id}});
		storeContratos.webflow({id:${procedimiento.asunto.id}});
	};
	...
	var borrar = app.crearBotonBorrar({
		...
		,params : {asuntoId : ${procedimiento.asunto.id} }
		,success : function(){
			recargarAdjuntos();
		}
	});
...
<sec:authorize ifAllGranted="PERSONALIZACION-BCC">		    
	...
	grid.on('rowdblclick', function(grid, rowIndex, e){
		...
		window.open("/pfs/bajarAdjuntoAsunto.htm?asuntoId=${procedimiento.asunto.id}&id="+rec.get('id')+"&nombre="+rec.data.nombre+"&extension="+rec.data.contentType);
	});
	...
	var panel = new Ext.Panel({
		...
		,nombreTab : 'adjuntosProcedimiento'			
	});
