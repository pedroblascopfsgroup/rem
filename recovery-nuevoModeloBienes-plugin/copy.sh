...
if [ -f target/recovery-$PLUGIN-plugin-*.jar ]; then
	rm /var/tomcat/$ENTORNO/webapps/pfs/WEB-INF/lib/recovery-$PLUGIN-plugin*
	rm -Rf target/*sources*
	mv target/recovery-$PLUGIN-plugin*.jar /var/tomcat/$ENTORNO/webapps/pfs/WEB-INF/lib
	chmod 777 /var/tomcat/$ENTORNO/webapps/pfs/WEB-INF/lib/*$PLUGIN-plugin*
else
...
if [ -d src/web/reports/plugin/$PLUGIN ];then
        rm -Rf /var/tomcat/$ENTORNO/webapps/pfs/reports/plugin/$PLUGIN
        cp -Rf src/web/reports/plugin/* /var/tomcat/$ENTORNO/webapps/pfs/reports/plugin
        chmod -R 777 /var/tomcat/$ENTORNO/webapps/pfs/reports/plugin/$PLUGIN
        echo "Se han copiado los informes"
else
        echo "No hay informes que copiar"
fi

if [ -f target/classes/optionalConfiguration/ac-plugin-$PLUGIN-view.xml ]; then
	rm -Rf /var/tomcat/$ENTORNO/webapps/pfs/WEB-INF/classes/optionalConfiguration/*$PLUGIN-tabsConfig*
 	rm -Rf /var/tomcat/$ENTORNO/webapps/pfs/WEB-INF/classes/optionalConfiguration/*$PLUGIN-view*
