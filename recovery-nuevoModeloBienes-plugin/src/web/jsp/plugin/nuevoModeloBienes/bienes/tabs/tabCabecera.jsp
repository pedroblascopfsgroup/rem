...
(function(){
	...
	var garantiaBien		= app.creaLabel('<s:message code="plugin.mejoras.bienesNMB.garantiaBien" text="**Garant�a Bien"/>','${NMBbien.sarebId}', {labelStyle:labelStyle});
	...
	var cru					= app.creaLabel('<s:message code="plugin.mejoras.bienesNMB.cru" text="**CRU"/>','${NMBbien.datosRegistralesActivo.cru}',{labelStyle:labelStyle});
	...
	var pestanaDatosRegistrales = new Ext.form.FieldSet({
	    ...
	    ,items : [{items:[tiposInmuebles, supTerreno, supConstruida, tomo, libro, folio, provinciaRegistro, municipioRegistro]},
				  {items:[numRegistro, refCatastral, tipoRegistro, inscripcion, fechaInscripcion, numFinca, cru]}
				 ]
	});
	...
	var btnSolicitarTasacionHCJ = new Ext.Button({
		    text: '<s:message code="plugin.nuevoModeloBienes.fichaBien.btnSolicitarTasacion" text="**Solicitar Tasaci�n" />'
			<app:test id="btnSolicitarTasacion" addComa="true" />
		    ,iconCls : 'icon_refresh'
			,cls: 'x-btn-text-icon'
			,style:'margin-left:2px;padding-top:0px'
	...
	<sec:authorize ifAllGranted="ACC_MAN_SERVICIOS_UVEM">
			<c:choose>
    			...
    			<c:otherwise>
					...
					<sec:authorize ifAllGranted="SOLVENCIA_EDITAR">		
						<%--Se oculta el bot�n "Solicitar tasaci�n" en HAYA-SAREB --%>			
