...
<fwk:page>
   ...
   var gridBienes = new Ext.grid.EditorGridPanel({
				...
				if (comprobarSiHayFilSeleccionada()>=1){
					...
					if (accion=='AGREGAR') {
						...
						Ext.Ajax.request({
							...
							,success:function(result, request){
								...
								if((resultado.okko == 'KO')) {
									Ext.Msg.show({
										title:'<s:message code="nuevoModeloBienes.agregarExcluirBien.excluir.bienAsociadoSubasta.titulo" text="AtenciÛn: OperaciÛn no v·lida"/>',
										msg: '<s:message code="nuevoModeloBienes.agregarExcluirBien.excluir.bienAsociadoSubasta" text="No es posible excluir el bien al encontrarse Èste asociado a un lote de la subasta."/>',
										buttons: Ext.Msg.OK,
	      			...
	      			if (accion=='AGREGAR') {
						...
						Ext.Msg.alert('<s:message code="fwk.ui.errorList.fieldLabel"/>','<s:message code="nuevoModeloBienes.agregarExcluirBien.excluir.faltanDatos" text="**Debe seleccionar alg˙n bien"/>');
					}
	      		}
	     	}
	});
	...
	//Cerramos el panel de filtros y esto har√° que se abra el listado de personas
