	...
	var btnAceptar = new Ext.Button({
	       ...
	       ,handler:function(){
				if (comprobarSiHayFilSeleccionada()>=1){								
	      			...
	      			if (accion=='AGREGAR') {
						Ext.Msg.alert('<s:message code="fwk.ui.errorList.fieldLabel"/>','<s:message code="subastas.agregarExcluirBien.agregar.faltanDatos" text="**Debe seleccionar algï¿½n bien y el lote al que pertence"/>');
					} else {
						Ext.Msg.alert('<s:message code="fwk.ui.errorList.fieldLabel"/>','<s:message code="subastas.agregarExcluirBien.excluir.faltanDatos" text="**Debe seleccionar algï¿½n bien"/>');
					}
	      		}
	     	}
	});
