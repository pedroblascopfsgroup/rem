...
@Component
public class NMBBusquedaSubastasManager {

	@Autowired
	private ApiProxyFactory proxyFactory;

	@BusinessOperation("es.pfsgroup.plugin.recovery.nuevoModeloBienes.subastas.buttons.left")
	List<DynamicElement> getButtonsLeft() {
		List<DynamicElement> l = proxyFactory.proxy(DynamicElementApi.class)
			.getDynamicElements("plugin.nuevoModeloBienes.web.subastas.busqueda.buttons.left",null);
		if (l == null) return new ArrayList<DynamicElement>();
		else return l;
	}
			
	@BusinessOperation("es.pfsgroup.plugin.recovery.nuevoModeloBienes.subastas.buttons.right")
	List<DynamicElement> getButtonsRight() {
		List<DynamicElement> l =  proxyFactory.proxy(DynamicElementApi.class).getDynamicElements(
				"plugin.nuevoModeloBienes.web.subastas.busqueda.buttons.right", null);
		if (l == null) return new ArrayList<DynamicElement>();
		else return l;
	}
