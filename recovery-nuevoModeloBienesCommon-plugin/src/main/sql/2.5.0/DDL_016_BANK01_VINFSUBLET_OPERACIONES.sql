...
FROM SUB_SUBASTA SUB
  LEFT JOIN PRC_PROCEDIMIENTOS PRC ON SUB.PRC_ID = PRC.PRC_ID
    INNER JOIN PRC_PER ON PRC.PRC_ID = PRC_PER.PRC_ID
      INNER JOIN PER_PERSONAS PER ON PRC_PER.PER_ID = PER.PER_ID
        INNER JOIN CPE_CONTRATOS_PERSONAS CPE ON PER.PER_ID = CPE.PER_ID 
          AND NOT CPE.CNT_ID IN (SELECT CNT_ID 
                                                  FROM CONTRATOS_PROC CNTPROC
                                                  WHERE CNTPROC.PRC_ID = PRC.PRC_ID)
          INNER JOIN CNT_CONTRATOS CNT ON CPE.CNT_ID = CNT.CNT_ID
