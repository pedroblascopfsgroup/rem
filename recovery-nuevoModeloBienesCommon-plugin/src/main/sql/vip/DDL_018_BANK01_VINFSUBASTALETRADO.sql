...
FROM (
  SELECT PRC.ASU_ID, DDTPO.DD_TPO_DESCRIPCION, ROW_NUMBER() OVER (PARTITION BY PRC.ASU_ID ORDER BY PRC.PRC_ID DESC) NUM
    FROM PRC_PROCEDIMIENTOS PRC
      INNER JOIN DD_TPO_TIPO_PROCEDIMIENTO DDTPO ON PRC.DD_TPO_ID = DDTPO.DD_TPO_ID
...
SELECT 
  ...
  ,CASE WHEN INTSUBASTA.TEV_VALOR IS NULL THEN 0 ELSE TO_NUMBER(REPLACE(INTSUBASTA.TEV_VALOR,''.'','','')) END INT_MORATORIOS
...
FROM  SUB_SUBASTA SUB
  ...
  LEFT JOIN TIPOS_SUBASTA TPOSUB ON TPOSUB.SUB_ID = SUB.SUB_ID
  LEFT JOIN ASU_ASUNTOS ASU ON SUB.ASU_ID = ASU.ASU_ID
    ...
    LEFT JOIN TIPO_PROCEDIMIENTO TPROCEDI ON ASU.ASU_ID = TPROCEDI.ASU_ID
  LEFT JOIN PRC_PROCEDIMIENTOS PRC ON SUB.PRC_ID = PRC.PRC_ID
    ...
    LEFT JOIN VALORES_DEMANDA VFDEMANDA ON PRC.PRC_ID = VFDEMANDA.PRC_ID  AND UPPER(VFDEMANDA.TEV_NOMBRE) IN (''FECHASOLICITUD'', ''FECHADEMANDA'')
    ...
    LEFT JOIN VALORES_SENYALAMIENTO INTSUBASTA ON PRC.PRC_ID = INTSUBASTA.PRC_ID AND UPPER(INTSUBASTA.TEV_NOMBRE) = UPPER(''INTERESES'')
