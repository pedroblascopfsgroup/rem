                    ...
                    WHERE prc.borrado = 0
                      ...
                      AND asu.dd_eas_id NOT IN (
                                    SELECT eas6.dd_eas_id
                                      FROM bankmaster.dd_eas_estado_asuntos eas6
...
INSERT INTO bank01.cli_clientes
...
--OK
/*************************************************/
/** bank01.bank01.CCL_CONTRATOS_CLIENTES      **/
/*************************************************/

INSERT INTO bank01.ccl_contratos_cliente
...
--OK
/*************************************************/
/** bank01.bank01.EXP_EXPEDIENTES             **/
/*************************************************/

INSERT INTO bank01.exp_expedientes
...
--OK
/**********************************************/
/** bank01.bank01.CEX_CONTRATOS_EXPEDIENTE **/
/**********************************************/

INSERT INTO bank01.cex_contratos_expediente
...
--OK
/*********************************************/
/** bank01.bank01.PEX_PERSONAS_EXPEDIENTE **/
/*********************************************/

INSERT INTO bank01.pex_personas_expediente
...
INSERT INTO bank01.asu_asuntos
   ...
   (SELECT apc.asu_id AS asu_id, 6 AS dd_est_id, SYSDATE AS asu_fecha_est_id,
           ...
           (SELECT n_lote
             ...
             WHERE n_caso = apc.n_caso AND creado = 'N') AS lote, (select dd_tas_id from bankmaster.dd_tas_tipos_asunto where dd_tas_codigo = apc.DD_TAS_CODIGO) AS DD_TAS_ID,
...
--OK
/********************************/
/** bank01.PRC_PROCEDIMIENTOS **/
/********************************/

INSERT INTO bank01.prc_procedimientos
...
FROM dd_tpo_tipo_procedimiento tpo
    JOIN (SELECT name_, MAX(id_) maxid FROM bankmaster.jbpm_processdefinition GROUP BY name_) pdm ON tpo.dd_tpo_xml_jbpm = pdm.name_
    JOIN bankmaster.jbpm_node node ON pdm.maxid = node.processdefinition_ AND node.name_ = 'Inicio'
    JOIN bankmaster.jbpm_transition tr ON node.id_ = tr.from_
    JOIN bankmaster.jbpm_node dest ON tr.to_ = dest.id_
   ...
   (SELECT s_gaa_gestor_adicional_asunto.NEXTVAL, asu_id AS asu_id,
           (SELECT CASE
                      WHEN lin.letrado IS NULL
                         ...
                         THEN (SELECT MAX (usd_id) AS usd_id
                                 FROM lin_asuntos_nuevos lin JOIN usd_usuarios_despachos usd
                                      ...
                                      JOIN bankmaster.usu_usuarios usu
                      ...
                      WHEN lin.letrado IS NOT NULL
                         THEN (SELECT MAX (usd_id) AS usd_id
                                 FROM lin_asuntos_nuevos lin JOIN bankmaster.usu_usuarios usu
