...
PRO.NUMERO_ESPEC, CNT.CNT_ID 
    ...
    ON '000001140300071' || PRO.NUMERO_CONTRATO || '000000000000000' = CNT.CNT_CONTRATO;
...
SELECT S_CLI_CLIENTES.NEXTVAL, TMP.PER_ID, arq.ARQ_ID, iti.DD_EST_ID, PER.PER_FECHA_EXTRACCION, '0', 
...
SELECT S_EXP_EXPEDIENTES.NEXTVAL, iti.DD_EST_ID, CNV_AUX_ALTA_PRC.FECHA_PROCESO, zon.OFI_ID, arq.ARQ_ID, '0', '0', '0', 'CARGA_PCO', SYSDATE, '0', 
...
--     	WITH USU AS (SELECT USU_ID, USU_USERNAME FROM HAYAMASTER.USU_USUARIOS WHERE BORRADO = 0)
...
--         		JOIN HAYAMASTER.DD_TDE_TIPO_DESPACHO TDE ON DES.DD_TDE_ID = TDE.DD_TDE_ID
--       			WHERE USD.BORRADO = 0)
--     		, TGE AS (SELECT DD_TGE_ID, DD_TGE_CODIGO FROM HAYAMASTER.DD_TGE_TIPO_GESTOR WHERE DD_TGE_CODIGO IN ('GEXT', 'PROC', 'GEST', 'PREDOC')
--     	)
...
-- 	WITH USU AS (
--     	SELECT USU_ID, USU_USERNAME FROM HAYAMASTER.USU_USUARIOS WHERE BORRADO = 0
--   	)
--   	, USD AS (
...
INSERT INTO HAYAMASTER.DD_STA_SUBTIPO_TAREA_BASE 
	...
	VALUES(HAYAMASTER.S_DD_STA_SUBTIPO_TAREA_BASE.NEXTVAL, '1', 'OCULTO', 'OCULTO', 'OCULTO', null, '0', 'DD', sysdate, null, null, null, null, '0', '0', 'EXTSubtipoTarea');
...
INSERT INTO TAR_TAREAS_NOTIFICACIONES
	(TAR_ID, EXP_ID, ASU_ID, DD_EST_ID, DD_EIN_ID, DD_STA_ID, TAR_CODIGO, TAR_TAREA, TAR_DESCRIPCION, 
		TAR_FECHA_INI, TAR_EN_ESPERA, TAR_ALERTA, TAR_EMISOR, 
		VERSION, USUARIOCREAR, FECHACREAR, BORRADO, PRC_ID, DTYPE, NFA_TAR_REVISADA)
SELECT S_TAR_TAREAS_NOTIFICACIONES.NEXTVAL, EXP_ID, PRO.ASU_ID, '6', '5', DECODE(PCO.PRETURNADO, 1, DD_STA_ID, (SELECT DD_STA_ID FROM HAYAMASTER.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO = 'OCULTO')), '1', TAP_CODIGO, TAP_DESCRIPCION, 
		SYSDATE, '0', '0', 'AUTOMATICA', '0', 
		'CARGA_PCO', SYSDATE, '0', PRC_ID, 'EXTTareaNotificacion', '0' 
	FROM CNV_AUX_ALTA_PRC PRC 
		LEFT JOIN CNV_AUX_ALTA_PCO PCO ON PRC.CODIGO_PROCEDIMIENTO_NUSE=PCO.CODIGO_PROCEDIMIENTO_NUSE
...
SELECT S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID, 
	...
	FROM ASU_ASUNTOS asu 
		...
		INNER JOIN HAYAMASTER.USU_USUARIOS ON USU_USUARIOS.USU_USERNAME=CNV_AUX_ALTA_PRC.COD_LETRADO 
		INNER JOIN USD_USUARIOS_DESPACHOS ON USU_USUARIOS.USU_ID=USD_USUARIOS_DESPACHOS.USU_ID 
		INNER JOIN HAYAMASTER.dd_tge_tipo_gestor ON dd_tge_codigo = 'LETR';
--	WHERE NOT EXISTS (SELECT 1 FROM GAA_GESTOR_ADICIONAL_ASUNTO gaa 
--						WHERE gaa.asu_id = asu.asu_id and gaa.dd_tge_id = 
--							(SELECT dd_tge_id FROM HAYAMASTER.dd_tge_tipo_gestor WHERE dd_tge_codigo = 'LET_PCO'));
...
SELECT S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID, 
	...
	FROM ASU_ASUNTOS asu 
		...
		INNER JOIN HAYAMASTER.USU_USUARIOS ON USU_USUARIOS.USU_USERNAME=CNV_AUX_ALTA_PRC.COD_LETRADO 
		INNER JOIN USD_USUARIOS_DESPACHOS ON USU_USUARIOS.USU_ID=USD_USUARIOS_DESPACHOS.USU_ID 
		INNER JOIN HAYAMASTER.dd_tge_tipo_gestor ON dd_tge_codigo = 'GEXT'; 
--	WHERE NOT EXISTS (SELECT 1 FROM GAA_GESTOR_ADICIONAL_ASUNTO gaa 
--						WHERE gaa.asu_id = asu.asu_id and gaa.dd_tge_id = 
...
SELECT S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID, 
	...
	FROM ASU_ASUNTOS asu 
		INNER JOIN CNV_AUX_ALTA_PRC ON asu.ASU_ID_EXTERNO=CNV_AUX_ALTA_PRC.CODIGO_PROCEDIMIENTO_NUSE 
		INNER JOIN HAYAMASTER.USU_USUARIOS ON USU_USUARIOS.USU_USERNAME=CNV_AUX_ALTA_PRC.COD_PROCURADOR 
		INNER JOIN USD_USUARIOS_DESPACHOS ON USU_USUARIOS.USU_ID=USD_USUARIOS_DESPACHOS.USU_ID 
		INNER JOIN HAYAMASTER.dd_tge_tipo_gestor ON dd_tge_codigo = 'PROC' 
	WHERE NOT EXISTS (SELECT 1 FROM GAA_GESTOR_ADICIONAL_ASUNTO gaa 
						WHERE gaa.asu_id = asu.asu_id and gaa.dd_tge_id = 
...
SELECT S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID, 
	...
	FROM ASU_ASUNTOS asu 
		...
		INNER JOIN HAYAMASTER.USU_USUARIOS ON USU_USUARIOS.USU_USERNAME=cnv.COD_PREPARADORDOC 
		INNER JOIN USD_USUARIOS_DESPACHOS ON USU_USUARIOS.USU_ID=USD_USUARIOS_DESPACHOS.USU_ID 
		INNER JOIN HAYAMASTER.dd_tge_tipo_gestor ON dd_tge_codigo = 'PREDOC' 
	WHERE NOT EXISTS (SELECT 1 FROM GAA_GESTOR_ADICIONAL_ASUNTO gaa 
						WHERE gaa.asu_id = asu.asu_id and gaa.dd_tge_id = 
							(SELECT dd_tge_id FROM HAYAMASTER.dd_tge_tipo_gestor WHERE dd_tge_codigo = 'PREDOC'));
...
SELECT s_GAH_GESTOR_ADIC_HISTORICO.NEXTVAL ,asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID ,sysdate, HAYAMASTER.dd_tge_tipo_gestor.DD_TGE_ID ,'CARGA_PCO', SYSDATE 
	...
	WHERE NOT EXISTS (SELECT 1 FROM GAH_GESTOR_ADICIONAL_HISTORICO gaa 
						WHERE gaa.gah_asu_id = asu.asu_id and gaa.GAH_TIPO_GESTOR_ID = 
							(SELECT dd_tge_id FROM HAYAMASTER.dd_tge_tipo_gestor WHERE dd_tge_codigo = 'LETR'));
...
SELECT s_GAH_GESTOR_ADIC_HISTORICO.NEXTVAL ,asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID ,sysdate, HAYAMASTER.dd_tge_tipo_gestor.DD_TGE_ID ,'CARGA_PCO', SYSDATE 
	FROM ASU_ASUNTOS asu 
		...
		INNER JOIN HAYAMASTER.USU_USUARIOS ON USU_USUARIOS.USU_USERNAME=CNV.COD_LETRADO 
...
using (select tex.tex_id, pi.roottoken_ 
    ...
    inner join hayamaster.jbpm_processinstance pi on pi.id_=p.prc_process_bpm
