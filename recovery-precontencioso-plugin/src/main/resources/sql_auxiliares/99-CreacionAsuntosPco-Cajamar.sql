DEFINE num_asuntos = 10
DEFINE esquema = CM01
DEFINE master = CMMASTER
...
BEGIN 
  ...
  V_SQL := 'SELECT DD_TPO_ID FROM cm01.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO=''PCO''';
  DBMS_OUTPUT.PUT_LINE(V_SQL);
...
WHERE lin.creado='N') tabla
  ...
  V_SQL := q'[INSERT INTO &esquema .cli_clientes (cli_id, per_id, arq_id, dd_est_id, dd_ecl_id, cli_fecha_est_id,
   ...
   (SELECT apc.cli_id, apc.per_id, 302 AS arq_id,
...
--  DBMS_OUTPUT.PUT_LINE(V_SQL);
  ...
  V_SQL := q'[INSERT INTO &esquema .exp_expedientes (exp_id, dd_est_id, exp_fecha_est_id, ofi_id, arq_id, VERSION,
   ...
   (SELECT apc.exp_id, 5 AS dd_est_id, SYSDATE AS exp_fecha_est_id,
           apc.ofi_id AS ofi_id, 302 AS arq_id,
...
--  DBMS_OUTPUT.PUT_LINE(V_SQL);
  ...
  V_SQL := q'[INSERT INTO &esquema .gaa_gestor_adicional_asunto (gaa_id, asu_id, usd_id, dd_tge_id, VERSION, usuariocrear,
             fechacrear, borrado)
   (SELECT &esquema .s_gaa_gestor_adicional_asunto.NEXTVAL, asu_id AS asu_id,
           (SELECT CASE
                      WHEN lin.letrado IS NULL
                           AND lin.grupo IS NULL
                         THEN (SELECT MAX (usd_id) AS usd_id
                                 from &esquema .lin_asuntos_nuevos lin JOIN &esquema .usd_usuarios_despachos usd
                                      ON lin.despacho = usd.des_id
                                      JOIN &master .usu_usuarios usu
                      ...
                      WHEN lin.letrado IS NOT NULL
                         THEN (SELECT MAX (usd_id) AS usd_id
                                 from &esquema .lin_asuntos_nuevos lin JOIN &master .usu_usuarios usu
                                      ON usu.usu_username = lin.letrado
                                      JOIN &esquema .usd_usuarios_despachos usd
                                      ON usu.usu_id = usd.usu_id
                                WHERE lin.creado = 'N'
                                  AND lin.n_caso = a.n_caso
                                  AND usd.usd_supervisor = 0
      ...
      FROM (SELECT n_caso, asu_id
              from &esquema .lin_asuntos_nuevos lin JOIN &esquema .cnt_contratos cnt
                   ON cnt.cnt_contrato = lin.n_caso
                   LEFT JOIN &esquema .cex_contratos_expediente cex ON cex.cnt_id =
                                                                    cnt.cnt_id
                   JOIN &esquema .asu_asuntos asu ON asu.exp_id = cex.exp_id
             WHERE lin.creado = 'N'
               AND asu_id NOT IN (SELECT asu_id
                                    FROM &esquema .gaa_gestor_adicional_asunto
                                   WHERE dd_tge_id = 2)) a)
  ]';
--  DBMS_OUTPUT.PUT_LINE(V_SQL);
  EXECUTE IMMEDIATE V_SQL; 
...
SELECT &esquema .S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID, 
    cmmaster.dd_tge_tipo_gestor.DD_TGE_ID ,0, 'CARGA_PCO', SYSDATE, 0 
...
SELECT &esquema .s_GAH_GESTOR_ADIC_HISTORICO.NEXTVAL ,asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID ,sysdate, cmmaster.dd_tge_tipo_gestor.DD_TGE_ID ,'CARGA_PCO', SYSDATE 
  FROM &esquema .ASU_ASUNTOS asu 
    INNER JOIN &esquema .lin_asuntos_para_crear lin on lin.asu_id=asu.asu_id 
    INNER JOIN &master .USU_USUARIOS ON USU_USUARIOS.USU_USERNAME='SUP_PCO' 
...
SELECT &esquema .S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, asu.ASU_ID ,USD_USUARIOS_DESPACHOS.USD_ID, 
  ...
  FROM &esquema .ASU_ASUNTOS asu 
    INNER JOIN &esquema .lin_asuntos_para_crear lin on lin.asu_id=asu.asu_id
    INNER JOIN &master .USU_USUARIOS ON USU_USUARIOS.USU_USERNAME='PREDOC'
    INNER JOIN &esquema .usd_usuarios_despachos ON USU_USUARIOS.USU_ID=USD_USUARIOS_DESPACHOS.USU_ID 
...
V_SQL := q'[INSERT INTO &esquema .PCO_PRC_HEP_HISTOR_EST_PREP (PCO_PRC_HEP_ID, PCO_PRC_ID, DD_PCO_PEP_ID, PCO_PRC_HEP_FECHA_INCIO, PCO_PRC_HEP_FECHA_FIN, USUARIOCREAR, FECHACREAR)
SELECT &esquema .S_PCO_PRC_HEP_HIST_EST_PREP.NEXTVAL, PCO.PCO_PRC_ID, PEP.DD_PCO_PEP_ID, SYSDATE, SYSDATE,
    'CARGA_PCO', SYSDATE
  FROM &esquema .PRC_PROCEDIMIENTOS prc 
