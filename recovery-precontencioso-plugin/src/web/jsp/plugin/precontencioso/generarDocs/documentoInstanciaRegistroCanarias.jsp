...
--%>
	...
	<%-- Nombre del notario Modificación--%>
	
	var nomNotario2 = new Ext.form.TextField({
		name : 'nomNotario2'
		,allowBlank : true
		,width : 300
		,fieldLabel : '<s:message code="plugin.precontencioso.docInstancia.nomNotario2" text="**Nombre del Notario Modificación" />'
	});    


	<%-- Número de protocolo Modificación--%>

	var numProtocolo2 = new Ext.form.TextField({
		name : 'numProtocolo2'
		,allowBlank : true
		,width : 200 
		,fieldLabel : '<s:message code="plugin.precontencioso.docInstancia.numProtocolo2" text="**N&uacute;m. Protocolo Modificación" />'
	});    
	

	<%-- Fecha de escritura Modificación--%>
	
	<pfsforms:datefield labelKey="plugin.precontencioso.docInstancia.fechaEscritura2" label="**Fecha de escritura Modificación" name="fechaEscritura2" obligatory="false"/>
   ...
   var gridBienes = new Ext.grid.EditorGridPanel({
	     		...
	     		page.webflow({
		      		...
		      		,success: function(result,request){
		      			if(result.resultadoOK==''){
							var flow='/pfs/expedientejudicial/generarDocumentoBienesCanarias';
							var params={idProcedimiento:data.id, idsBien:strIds,
							  localidad: comboLocalidades.getValue(), 
							  nombreNotario: nomNotario.getValue(),  numProtocolo: numProtocolo.getValue(), fechaEscritura: app.format.dateRenderer(fechaEscritura.getValue()),
							  nombreNotario2: nomNotario2.getValue(),  numProtocolo2: numProtocolo2.getValue(), fechaEscritura2: app.format.dateRenderer(fechaEscritura2.getValue()),
	...
	var panelEdicion = new Ext.Panel({
		...
		items: [
			{
				items: [ comboLocalidades, localidadRegProp, numeroRegProp, nomNotario, numProtocolo, fechaEscritura, nomNotario2, numProtocolo2, fechaEscritura2 ]
			}
		]
	});
