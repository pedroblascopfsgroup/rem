...
BEGIN	
    ...
    V_MSQL := 'CREATE UNIQUE INDEX ' || V_ESQUEMA || '.UK_DD_REI_RESULTADO_INFORME ON ' || V_ESQUEMA || '.' || VAR_TABLENAME || '
					(DD_REI_CODIGO)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.UK_DD_REI_RESULTADO_INFORME... Indice creado');
    V_MSQL := 'CREATE UNIQUE INDEX ' || V_ESQUEMA || '.PK_DD_REI_RESULTADO_INFORME ON ' || V_ESQUEMA || '.' || VAR_TABLENAME || '
					(DD_REI_ID)';
		EXECUTE IMMEDIATE V_MSQL;
    V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.' || VAR_TABLENAME || ' ADD (
                CONSTRAINT PK_DD_REI_RESULTADO_INFORME PRIMARY KEY (DD_REI_ID),
                CONSTRAINT UK_DD_REI_RESULTADO_INFORME UNIQUE (DD_REI_CODIGO)
                )';						
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.PK_DD_REI_RESULTADO_INFORME... Creando PK');	
