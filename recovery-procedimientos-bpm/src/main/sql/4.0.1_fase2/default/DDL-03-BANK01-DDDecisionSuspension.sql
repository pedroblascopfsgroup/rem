...
BEGIN	
    ...
    V_MSQL := 'CREATE TABLE ' || V_ESQUEMA || '.' || VAR_TABLENAME || '
					(
            DD_DSU_ID                 NUMBER(16)          NOT NULL,
            DD_DSU_CODIGO             VARCHAR2(50 CHAR)   NOT NULL,
            DD_DSU_DESCRIPCION        VARCHAR2(50 CHAR),
            DD_DSU_DESCRIPCION_LARGA  VARCHAR2(250 CHAR),
            DD_DSU_PATRON             VARCHAR2(250 CHAR),
    ...
    V_MSQL := 'CREATE UNIQUE INDEX ' || V_ESQUEMA || '.UK_DD_DSU_DECISION_SUSPENSION ON ' || V_ESQUEMA || '.' || VAR_TABLENAME || '
					(DD_DSU_CODIGO)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.UK_DD_DSU_DECISION_SUSPENSION... Indice creado');
    V_MSQL := 'CREATE UNIQUE INDEX ' || V_ESQUEMA || '.PK_DD_DSU_DECISION_SUSPENSION ON ' || V_ESQUEMA || '.' || VAR_TABLENAME || '
					(DD_DSU_ID)';
		EXECUTE IMMEDIATE V_MSQL;
    V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.' || VAR_TABLENAME || ' ADD (
                CONSTRAINT PK_DD_DSU_DECISION_SUSPENSION PRIMARY KEY (DD_DSU_ID),
                CONSTRAINT UK_DD_DSU_DECISION_SUSPENSION UNIQUE (DD_DSU_CODIGO)
                )';						
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.PK_DD_DSU_DECISION_SUSPENSION... Creando PK');	
