...
var comboNotificado = items[1 + offset];
var fecha = items[2 + offset];

comboNotificado.on('select', function(){
	
	if(comboNotificado.getValue() == '01') {//si
		fecha.allowBlank = false;
	}
	else if(comboNotificado.getValue() == '02') {//no
		fecha.allowBlank = true;
	}
});

//mostramos el botón guardar cuando la tarea no está terminada y cuando no hay errores de validación
