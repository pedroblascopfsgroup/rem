...
var comboSolicitud = items[1 + offset];
var fechaSolicitud = items[2 + offset];

fechaSolicitud.setDisabled(true);

comboSolicitud.on('select', function(){
	//Borramos todos los combos dependientes ante cualquier cambio
	fechaSolicitud.setValue('');
	
	if(comboSolicitud.getValue() == '01') {//si
		fechaSolicitud.setDisabled(false);
		fechaSolicitud.allowBlank = false;
	}
	else if(comboSolicitud.getValue() == '02') {//no
		fechaSolicitud.setDisabled(true);
		fechaSolicitud.allowBlank = true;
	}
});
