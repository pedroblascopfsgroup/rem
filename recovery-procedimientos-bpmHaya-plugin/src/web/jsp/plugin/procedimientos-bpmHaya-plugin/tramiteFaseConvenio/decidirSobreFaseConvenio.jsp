...
/*
var cb_Propio = items[1 + muestraBotonGuardar];
var cb_Seguimiento = items[2 + muestraBotonGuardar];

cb_Propio.on('select', function(){
	//Borramos todos los combos dependientes ante cualquier cambio
	cb_Seguimiento.setValue('');
	
	if(cb_Propio.getValue() == '01') {//si
		cb_Seguimiento.setDisabled(true);
	}
	else if(cb_Propio.getValue() == '02') {//no
		cb_Seguimiento.setDisabled(false);
	}
});

cb_Seguimiento.on('select', function(){
	//Borramos todos los combos dependientes ante cualquier cambio
	cb_Propio.setValue('');
	
	if(cb_Seguimiento.getValue() == '01') {//si
		cb_Propio.setDisabled(true);
	}
	else if(cb_Seguimiento.getValue() == '02') {//no
		cb_Propio.setDisabled(false);
	}
});
*/
