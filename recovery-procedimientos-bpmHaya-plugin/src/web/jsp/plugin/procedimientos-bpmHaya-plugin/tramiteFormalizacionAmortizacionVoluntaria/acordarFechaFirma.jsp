...
var cb_comboResolucion = items[2 + muestraBotonGuardar];
var cb_fechaPrevista = items[3 + muestraBotonGuardar];
var cb_notaria = items[4 + muestraBotonGuardar];
var cb_notario = items[5 + muestraBotonGuardar];
var cb_asistenciaFirma = items[6 + muestraBotonGuardar];


cb_fechaPrevista.setDisabled(true);
cb_notaria.setDisabled(true);
cb_asistenciaFirma.setDisabled(true);
cb_notario.setDisabled(true);


cb_comboResolucion.on('select', function(){
	if(cb_comboResolucion.getValue() == 'ACEPTADO') {//favorable
		cb_fechaPrevista.setDisabled(false);
		cb_notaria.setDisabled(false);
		cb_asistenciaFirma.setDisabled(false);
		cb_fechaPrevista.setValue('');
		cb_notaria.setValue('');
		cb_asistenciaFirma.setValue('');
		cb_fechaPrevista.allowBlank = false;
		cb_notaria.allowBlank = false;
		cb_asistenciaFirma.allowBlank = false;
	}
	else if(cb_comboResolucion.getValue() == 'RECHAZADO') {//no_favorable
		cb_fechaPrevista.setDisabled(true);
		cb_notaria.setDisabled(true);
		cb_asistenciaFirma.setDisabled(true);
		cb_notario.setDisabled(true);
		cb_fechaPrevista.allowBlank = true;
		cb_notaria.allowBlank = true;
		cb_asistenciaFirma.allowBlank = true;
		cb_notario.allowBlank = true;
	}
});

cb_notaria.on('select', function(){
	if(cb_notaria.getValue() == '01') {//si
		cb_notario.setDisabled(false);
		cb_notario.setValue('');
		cb_notario.allowBlank = false;
	}
	else if(cb_notaria.getValue() == '02') {//no
		cb_notario.setDisabled(true);
		cb_notario.allowBlank = true;
	}
});
