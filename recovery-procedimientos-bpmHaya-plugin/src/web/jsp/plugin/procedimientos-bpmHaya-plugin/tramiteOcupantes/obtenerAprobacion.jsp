...
var cb_alegaciones = items[3 + muestraBotonGuardar];
var cb_vista = items[4 + muestraBotonGuardar];
var fecha_vista = items[5 + muestraBotonGuardar];


if(fecha_vista.getValue() != '') {
	cb_alegaciones.setReadOnly(true);
	cb_alegaciones.setValue('02');
	cb_vista.setValue('01');
	cb_vista.setReadOnly(true);
	fecha_vista.setReadOnly(true);
}

cb_alegaciones.on('select', function(){	
	if(cb_alegaciones.getValue() == '01') {//si
		cb_vista.setReadOnly(true);
		fecha_vista.setReadOnly(true);
		fecha_vista.setValue('');
	}
	else if(cb_alegaciones.getValue() == '02') {//no
		cb_vista.setReadOnly(false);
		fecha_vista.setReadOnly(false);
	}
});	
...
cb_vista.on('select', function(){	
	if(cb_vista.getValue() == '01') {//si
		...
		cb_alegaciones.setReadOnly(false);
		fecha_vista.setReadOnly(true);
	}
});	
