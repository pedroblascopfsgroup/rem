...
var cb_delegada = items[3 + muestraBotonGuardar];
var cb_atribuciones = items[4 + muestraBotonGuardar];
var cb_suspender = items[1 + muestraBotonGuardar];
var cb_modificacion = items[2 + muestraBotonGuardar];

cb_suspender.on('select', function(){	
	if(cb_suspender.getValue() == '01') {//si
		cb_atribuciones.setDisabled(true);
		cb_modificacion.setDisabled(true);
		cb_delegada.setDisabled(true);
		cb_atribuciones.setValue('');
		cb_modificacion.setValue('');
		cb_delegada.setValue('');
		cb_delegada.allowBlank = true;
	}
	else if(cb_suspender.getValue() == '02') {//no
		cb_modificacion.setDisabled(false);
		cb_delegada.setDisabled(false);
		cb_delegada.allowBlank = false;
	}
});

cb_delegada.on('select', function(){
	if(cb_delegada.getValue() == '01') {//si
		cb_atribuciones.setDisabled(false);
		cb_atribuciones.allowBlank = false;
	}else if(cb_delegada.getValue() == '02'){//no
		cb_atribuciones.setDisabled(true);
		cb_atribuciones.setValue('');
		cb_atribuciones.allowBlank = true;
	}
});
