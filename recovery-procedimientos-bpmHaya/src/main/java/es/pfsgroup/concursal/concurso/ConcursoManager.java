	...
	public List<DtoConcurso> listadoFaseComun(Long idAsunto) {
		...
		for (Procedimiento p : asu.getProcedimientos()) {
			if (p.getTipoProcedimiento() != null
					...
					&& ("H009".equals(p.getTipoProcedimiento().getCodigo()) //fase comun abreviadoV4
							|| "H024".equals(p.getTipoProcedimiento().getCodigo()) //fase comun ordinario
							|| "H017".equals(p.getTipoProcedimiento().getCodigo()) //fase convenio
							|| "H021".equals(p.getTipoProcedimiento().getCodigo()) //P. solicitud de concurso necesario
							|| "H029".equals(p.getTipoProcedimiento().getCodigo()) //T. Registrar resolución de interés
							|| "H041".equals(p.getTipoProcedimiento().getCodigo()) //T. seguimiento cumplimiento de convenio
							|| "H039".equals(p.getTipoProcedimiento().getCodigo()) //T. conclusión de concurso
							|| "P420".equals(p.getTipoProcedimiento().getCodigo()) //T. de aceptación y decisión Concursal
							|| "P404".equals(p.getTipoProcedimiento().getCodigo()) //T. de aceptación y decisión Litigios
							|| "H019".equals(p.getTipoProcedimiento().getCodigo()) //T. de solicitud de actuaciones o pronunciamientos
							|| "H033".equals(p.getTipoProcedimiento().getCodigo()))) { //fase liquidacion
