...
public class SubastaConcursalCjLeaveActionHandler extends
			...
			if (!Checks.esNulo(sub)) {
				...
				cambiaEstadoSubasta(
						...
						obtenerEstadoSiguiente(executionContext,
								"ValidarInformeDeSubasta", "comboModificacion"));
			}
		} else if (executionContext.getNode().getName()
				.contains("EnviarInstruccionesProcurador")) {
	...
	private void decidirRamasDocumentacion() {
		...
		/*
		 * resultado[0] -> IO -> Adjuntar Notas Simples 
		 * resultado[1] -> TA -> Adjuntar tasaciones
		 * resultado[2] -> IF -> Informe Fiscal 
