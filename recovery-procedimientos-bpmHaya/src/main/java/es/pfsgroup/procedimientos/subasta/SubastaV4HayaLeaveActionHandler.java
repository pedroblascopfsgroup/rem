...
public class SubastaV4HayaLeaveActionHandler extends
	...
	private final String TAP_SOLICITUD_SUBASTA_SAREB = "H002_SolicitudSubasta";
	...
	private void avanzamosEstadoSubasta() {
		...
		if (executionContext.getNode().getName().contains("SolicitudSubasta")
			...
			if (!Checks.esNulo(sub) && !Checks.esNulo(sub.getEstadoSubasta()) && DDEstadoSubasta.PIN.compareTo(sub.getEstadoSubasta().getCodigo()) == 0) {
				...
				.contains("PrepararPropuestaSubasta")) {
			...
			if (!Checks.esNulo(sub)) {
				...
				cambiaEstadoSubasta(
						...
						obtenerEstadoSiguiente(executionContext,
								"ValidarPropuesta", "comboResultado"));
				...
				.contains("LecturaAceptacionInstrucciones")) {
	...
	private void estableceContexto() {
		...
		/*
		 * resultado[0] -> A -> SuspendidaTerceros resultado[1] -> B ->
		 * SuspendidaEntidad resultado[2] -> C -> CesionRemateSi resultado[3] ->
		 * D -> AdjudicadoEntidadPosibleRemateNo resultado[4] -> E ->
		 * AdjudicadoEntidadPosibleRemateSi resultado[5] -> F ->
		 * AdjudicadoTercero resultado[6] -> G -> SolicitarServicioIndices
		 */
	...
	public Boolean[] bpmGetValoresRamasCelebracion(Procedimiento prc,
		...
		Boolean[] resultado = { false, false, false, false, false, false, false };
		resultado[0] = false;
		resultado[1] = false;
		resultado[2] = false;
		resultado[3] = false;
		...
		if (celebrada) {
			...
			// Hablado con Jorge por chat: siempre que se celebre se lanzará
			// esta tarea.
			resultado[6] = true;
			...
			if (bienAdjuTerceroFondo) {
				resultado[5] = true;
				// No debo ejecutar la tarea porque se ejecutará después en este
				// caso
				resultado[6] = false;
			}
