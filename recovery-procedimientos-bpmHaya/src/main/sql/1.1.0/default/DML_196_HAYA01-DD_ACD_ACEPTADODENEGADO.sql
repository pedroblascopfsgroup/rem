...
--##########################################
--## Author: Nacho
--## Finalidad: DML Inserción en DD_ACD_ACEPTADODENEGADO
...
DECLARE
  ...
  V_TIPO_ACR1 T_ARRAY_ACR1 := T_ARRAY_ACR1(
    ...
    T_TIPO_ACR1('DENEGADO','Denegado','Denegado',0,'DD',sysdate,0)
  ); 
...
DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'.DD_ACD_ACEPTADODENEGADO... Empezando a insertar datos en el diccionario');
    FOR I IN V_TIPO_ACR1.FIRST .. V_TIPO_ACR1.LAST
      LOOP
        V_MSQL := 'SELECT '|| V_ESQUEMA ||'.S_DD_ACD_ACEPTADODENEGADO.NEXTVAL FROM DUAL';
        EXECUTE IMMEDIATE V_MSQL INTO V_ENTIDAD_ID;
        V_TMP_TIPO_ACR1 := V_TIPO_ACR1(I);
        V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.DD_ACD_ACEPTADODENEGADO (
                    DD_ACD_ID,DD_ACD_CODIGO,DD_ACD_DESCRIPCION,DD_ACD_DESCRIPCION_LARGA,VERSION,USUARIOCREAR,FECHACREAR,BORRADO) ' ||
      ...
      END LOOP;
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('[FIN] '||V_ESQUEMA||'.DD_ACD_ACEPTADODENEGADO... Datos del diccionario insertado');
