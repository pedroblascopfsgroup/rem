...
begin  
	...
	set dd_ptp_plazo_script='damePlazo(valoresBPMPadre[''H002_SenyalamientoSubasta''][''fechaSenyalamiento''])+5*24*60*60*1000L'
	where tap_id=(select tap_id from tap_tarea_procedimiento where tap_codigo='H006_AperturaPlazo');	
		
	update dd_ptp_plazos_tareas_plazas
	set dd_ptp_plazo_script='damePlazo(valores[''H006_AperturaPlazo''][''fecha''])+5*24*60*60*1000L'
	where tap_id=(select tap_id from tap_tarea_procedimiento where tap_codigo='H006_ResenyarFechaComparecencia');
		
	update dd_ptp_plazos_tareas_plazas
	set dd_ptp_plazo_script='damePlazo(valores[''H006_ResenyarFechaComparecencia''][''fecha''])+5*24*60*60*1000L'
