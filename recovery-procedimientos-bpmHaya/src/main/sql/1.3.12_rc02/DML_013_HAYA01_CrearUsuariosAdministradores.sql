...
DECLARE
    ...
    V_TIPO_TFA T_ARRAY_TFA := T_ARRAY_TFA(  
      T_TIPO_TFA('ADMIN_H','HREMAD1234','ADMIN_PRUEBA'),      
      T_TIPO_TFA('ADMIN_V','HREVLC1234','ADMIN_PRUEBA')
    ); 
...
BEGIN
	...
	FOR I IN V_TIPO_TFA.FIRST .. V_TIPO_TFA.LAST
      LOOP              
            ...
            V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME='''||V_TMP_TIPO_TFA(1)||''' ';
			...
			IF V_NUM_TABLAS < 1 THEN                        			
					...
					DBMS_OUTPUT.PUT_LINE('Creado el usuario '||V_TMP_TIPO_TFA(1)||' correctamente.');					
					...
					DBMS_OUTPUT.PUT_LINE('Ya dado el perfil ''FPFSRADMIN'' () al usuario '||V_TMP_TIPO_TFA(1));							
			
			--Si el usuario ya existe
			ELSE
				DBMS_OUTPUT.PUT_LINE('El usuario '||V_TMP_TIPO_TFA(1)||' ya existe, NO se creará de nuevo otra vez.');
