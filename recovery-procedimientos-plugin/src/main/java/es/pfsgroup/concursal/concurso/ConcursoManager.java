	...
	public List<DtoConcurso> listadoFaseComun(Long idAsunto) {
		...
		for (Procedimiento p : asu.getProcedimientos()) {
			if (p.getTipoProcedimiento() != null
					...
					&& ("P412".equals(p.getTipoProcedimiento().getCodigo()) //fase comun abreviadoV4
							|| "P56".equals(p.getTipoProcedimiento().getCodigo()) //fase comun abreviado
							|| "P24".equals(p.getTipoProcedimiento().getCodigo()) //fase comun ordinario
							|| "P29".equals(p.getTipoProcedimiento().getCodigo()) //fase convenio
							|| "P55".equals(p.getTipoProcedimiento().getCodigo()) //P. solicitud de concurso necesario
							|| "P28".equals(p.getTipoProcedimiento().getCodigo()) //T. Registrar resolución de interés
							|| "P64".equals(p.getTipoProcedimiento().getCodigo()) //T. seguimiento cumplimiento de convenio
							|| "P62".equals(p.getTipoProcedimiento().getCodigo()) //T. conclusión de concurso
							|| "P61".equals(p.getTipoProcedimiento().getCodigo()) //T. de aceptación y decisión
							|| "P27".equals(p.getTipoProcedimiento().getCodigo()) //T. de solicitud de actuaciones o pronunciamientos
							|| "P31".equals(p.getTipoProcedimiento().getCodigo()))) { //fase liquidacion
