...
<c:forEach items="${form.items}" var="item">
	...
	<c:if test="${item.nombre=='nPlaza' || item.nombre=='comboPlaza'}">
		<c:if test="${item.value!=null}">
...
// *************************************************************** //
	...
	var codPlaza = "${valores['P01_DemandaCertificacionCargas']['plazaJuzgado']}";
	if (codPlaza == ''){
		// la plaza se ha introducido en Confirmar admisiÛn demanda
		codPlaza = codPlazaEnBD;
	}
	
	var codPlazaParaHistorico = "${valores['P01_AutoDespachandoEjecucion']['nPlaza']}";
