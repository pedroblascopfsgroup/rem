...
<fwk:page>
	...
	<pfs:textfield name="descripcionConvenio" labelKey="asunto.concurso.tabConvenios.descripcionConvenio" label="Descripcion convenio" value="${convenio.descripcion}" />
	...
	<pfs:ddCombo name="estado" labelKey="asunto.concurso.tabFaseComun.estado" label="**Estado" value="${estado_por_defecto}" dd="${estados}" />
	<pfs:ddCombo name="postura" labelKey="asunto.concurso.tabConvenio.Postura" label="**Postura" value="" dd="${posturas}" />
	...
	<pfs:currencyfield name="totalMasaOrd" labelKey="asunto.concurso.tabConvenio.totalMasaPasivaOrd" label="Total masa ordinaria" value="" />
	<pfs:currencyfield name="porcentajeOrd" labelKey="asunto.concurso.tabConvenio.porcentajeSobreMasaOrd" label="Porcentaje S/masa ordinaria" value="" />
	...
	<pfs:ddCombo name="alternativa" labelKey="plugin.ugas.asunto.concurso.tabConvenios.alternativa" label="Alternativa" value="" dd="${tipoAlternativa}" />	
		...
		<pfs:defineParameters name="parametros" paramId="${idProcedimiento}"
		...
		var btnGuardar = new Ext.Button({
			...
			,handler : function() {
							page.webflow({
								flow: 'plugin/concursal/agregarConvenio'
								,params: parametros
								,success : function(){ 
			...
			,items : [
			 	...
			 	,{
					...
					,defaults :  {layout:'table', autoHeight : true, border : true, width:655, cellCls : 'vtop', bodyStyle : 'padding-left:5px' }
					 ...
					 ,bbar : [
						<sec:authorize ifAnyGranted="ROLE_PUEDE_VER_AGREGAR_CONVENIO_GUARDAR">btnGuardar, </sec:authorize> btnCancelar
					 ]
				  } 
			  ]
		});
