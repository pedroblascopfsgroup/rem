...
<c:if test="${form.tareaExterna.tareaPadre.fechaFin==null && form.errorValidacion==null && !readOnly}">
	var btnGuardar = new Ext.Button({
		...
		,handler : function(){
			...
			cb_existeOposicion.clearInvalid();
			tx_fechaOposicion.clearInvalid();
			tx_fechaComparecencia.clearInvalid();
			
			var existeOposicion = cb_existeOposicion.getValue();
			var fechaOposicion = tx_fechaOposicion.getValue();
			var fechaComparecencia = tx_fechaComparecencia.getValue();
			var ok = 0;
			
			if (existeOposicion == '01' && fechaOposicion == '' ){
				tx_fechaOposicion.markInvalid(tx_fechaOposicion.blankText);
				ok = 1;
			}
			if (existeOposicion == '01' && fechaComparecencia == ''){
				tx_fechaComparecencia.markInvalid(tx_fechaComparecencia.blankText);
				ok = 1
			}			
			if (existeOposicion == '02' && fechaOposicion != ''){
				tx_fechaOposicion.markInvalid('Este campo no puede tener valor');
				ok = 1
			} 			
			if (existeOposicion == '02' && fechaComparecencia != ''){
				tx_fechaComparecencia.markInvalid('Este campo no puede tener valor');
				ok = 1
			} 
