...
var cb_posesion = items[1 + offset];
var dt_fechaSolicitud = items[2 + offset];
var cb_ocupado = items[3 + offset];
var cb_moratoria = items[4 + offset];
var cb_viviendaHabitual = items[5 + offset];

cb_posesion.on('select', function(){

	//Borramos todos los combos dependientes ante cualquier cambio
	cb_ocupado.setValue('');
	cb_moratoria.setValue('');
	cb_viviendaHabitual.setValue('');
	dt_fechaSolicitud.setValue('');
	
	if(cb_posesion.getValue() == '01') {//si
		cb_ocupado.setDisabled(false);
		cb_moratoria.setDisabled(false);
		cb_viviendaHabitual.setDisabled(false);
		dt_fechaSolicitud.setDisabled(false);
	}
	else if(cb_posesion.getValue() == '02') {//no
		cb_ocupado.setDisabled(true);
		cb_moratoria.setDisabled(true);
		cb_viviendaHabitual.setDisabled(true);
		dt_fechaSolicitud.setDisabled(true);
	}
});
