...
where tar_id in (  
    ...
    where prc_id in (
        ...
        where padre.dd_epr_id = 8 and prc.dd_epr_id != 9
...
execute immediate 'update '||V_ESQUEMA||'.prc_procedimientos set dd_epr_id = 9, fechamodificar = sysdate, usuariomodificar=''REORGANIZADO'' where prc_id in (
select prc.prc_id
    from '||V_ESQUEMA||'.prc_procedimientos prc
        join '||V_ESQUEMA||'.prc_procedimientos padre on prc.prc_prc_id = padre.prc_id
    where padre.dd_epr_id = 8 
    and prc.dd_epr_id != 9       
)';
...
where tar_id in (   
   select tar_id from '||V_ESQUEMA||'.tar_tareas_notificaciones
    ...
    where padre.dd_epr_id = 9 and hijo1.dd_epr_id = 9
    and hijo2.dd_epr_id != 9
...
execute immediate 'update '||V_ESQUEMA||'.prc_procedimientos set dd_epr_id = 9 where prc_id in (
select hijo2.prc_id
    from '||V_ESQUEMA||'.prc_procedimientos hijo2 
    join '||V_ESQUEMA||'.prc_procedimientos hijo1 on hijo2.prc_prc_id = hijo1.prc_prc_id
        join '||V_ESQUEMA||'.prc_procedimientos padre on hijo1.prc_prc_id = padre.prc_id
...
where rpr.rpr_fecha_revisado is null and PRC.DD_EPR_ID=8';
...
from '||V_ESQUEMA||'.rpr_revision_procedimiento tmp
    ...
    where prc.borrado = 0 and prc.dd_Epr_id not in (4,5) 
    and tmp.rpr_fecha_revisado is null';
...
where tmp.rpr_fecha_revisado is null
and prc.borrado = 0 and prc.dd_Epr_id not in (4,5) ) nuevo
