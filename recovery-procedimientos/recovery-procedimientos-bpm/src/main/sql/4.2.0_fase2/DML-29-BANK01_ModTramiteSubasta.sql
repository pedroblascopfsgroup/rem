...
BEGIN
	...
	execute immediate 'update '||V_ESQUEMA||'.tfi_tareas_form_items set tfi_label = ''Con postores'' where tfi_nombre = ''comboCesion'' and tap_id  in (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo in (''P401_CelebracionSubasta'',''P409_CelebracionSubasta''))';
	execute immediate 'update '||V_ESQUEMA||'.tfi_tareas_form_items set tfi_label = ''Requiere elevar propuesta cesión de remate'' where tfi_nombre = ''comboComite'' and tap_id  in (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo in (''P401_CelebracionSubasta'',''P409_CelebracionSubasta''))';
