...
// Comportamiento personalizado en combos para aceptacion del concurso 
var cb_conflictoInteres = items[1 + muestraBotonGuardar];
var cb_aceptacionAsunto = items[2 + muestraBotonGuardar];
var f_aceptacionAsunto  = items[3 + muestraBotonGuardar];

cb_conflictoInteres.on('select', function(){
	//Se inicializa el combo
	cb_aceptacionAsunto.setValue('');
	f_aceptacionAsunto.setValue('');
	
	if(cb_conflictoInteres.getValue() == '01') {//si
		cb_aceptacionAsunto.setValue('02');	
		cb_aceptacionAsunto.setDisabled(true);
		f_aceptacionAsunto.setDisabled(true);
	}
	else if(cb_conflictoInteres.getValue() == '02') {//no
		cb_aceptacionAsunto.setDisabled(false);
		f_aceptacionAsunto.setDisabled(false);
	}
});

cb_aceptacionAsunto.on('select', function(){
	//Se inicializa el combo
	f_aceptacionAsunto.setValue('');
	
	if(cb_aceptacionAsunto.getValue() == '02') {
		f_aceptacionAsunto.setDisabled(true);
	}
	else if(cb_aceptacionAsunto.getValue() == '01') {
		f_aceptacionAsunto.setDisabled(false);
	}
});

var validar = function(){
	if(!cb_aceptacionAsunto.disabled){
		if(cb_aceptacionAsunto.getValue()==''){
			Ext.MessageBox.show({
	           title: 'Campo obligatorio',
	           msg: '<s:message code="tareaExterna.procedimiento.tramiteAceptacionConcurso.campoObligatorioCombo" text="**Campo aceptación asunto obligatorio" />',
	           width:300,
	           buttons: Ext.MessageBox.OK
	       });
			return false;
		}
		else if(f_aceptacionAsunto.getValue()=='' && cb_aceptacionAsunto.getValue() == '01'){
			Ext.MessageBox.show({
	           title: 'Campo obligatorio',
	           msg: '<s:message code="tareaExterna.procedimiento.tramiteAceptacionConcurso.campoObligatorioFecha" text="**Campo fecha aceptación obligatorio" />',
	           width:300,
	           buttons: Ext.MessageBox.OK
	       });
			return false;
