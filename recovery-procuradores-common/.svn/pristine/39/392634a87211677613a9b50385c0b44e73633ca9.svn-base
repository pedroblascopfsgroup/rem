 ...
 * Resolución: Registrar resultado
 * Tareas: P12_RegistrarResultado 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '293', 'R_DEC_DET_VDF', 'Decreto determinando valoración definitiva','Decreto determinando valoración definitiva',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'VBI')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda decreto determinando valoración definitiva.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_DEC_DET_VDF','Input decreto determinando valoración definitiva',
'Input decreto determinando valoración definitiva.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='P12'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_DEC_DET_VDF'),'P12_RegistrarResultado', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_DEC_DET_VDF') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='P12');
