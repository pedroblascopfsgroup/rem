/**
 * Trámite: Tramite certificación de cargas.
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_SOL_INF_CARG','Input de escrito sellado solicitando cargas previas',
'Input de escrito sellado solicitando cargas previas', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_SOL_INF_CARG'),'P08_SolicitarInformacionCargas', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_SOL_INF_CARG') 
