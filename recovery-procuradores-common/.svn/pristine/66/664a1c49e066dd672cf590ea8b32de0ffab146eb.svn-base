 ...
 * Resolución: Registrar Cambio de cerradura
 * Tareas: P417_RegistrarCambioCerradura 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '238', 'R_REG_CAM_CERR', 'Comunicación cambio cerradura', 'Comunicación cambio cerradura al letrado', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'GLL')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de comunicación cambio cerradura al letrado', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REG_CAM_CERR','Input comunicación cambio cerradura al letrado',
'Input comunicación cambio cerradura al letrado', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='P417'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REG_CAM_CERR'),'P417_RegistrarCambioCerradura', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REG_CAM_CERR') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='P417');
