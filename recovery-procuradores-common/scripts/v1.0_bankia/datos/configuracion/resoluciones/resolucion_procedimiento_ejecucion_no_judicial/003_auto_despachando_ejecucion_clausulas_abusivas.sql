 ...
 * Tareas: P15_AutoDespaEjecMasDecretoEmbargo_new1 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '310', 'R_AUT_DSP_ABU', 'Auto despachando ejecución declarando clausulas abusivas','Auto despachando ejecución declarando clausulas abusivas',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PNJ')
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_AUT_DSP_ABU','Input auto despachando ejecución declarando clausulas abusivas',
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='P15'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_AUT_DSP_ABU'),'P15_AutoDespaEjecMasDecretoEmbargo_new1', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_AUT_DSP_ABU') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='P15');
...
Insert into BPM_IDT_INPUT_DATOS(BPM_IDT_ID, BPM_TPI_ID, BPM_IDT_NOMBRE, BPM_IDT_GRUPO, BPM_IDT_DATO, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
select S_BPM_IDT_INPUT_DATOS.nextVal, bpm_tpi_id , 'd_comboAdmisionDemanda', 'generico', 'comboAdmisionDemanda', 0, 'MOD_PROC', sysdate, 0 
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_AUT_DSP_ABU') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='P15');
