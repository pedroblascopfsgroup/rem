 ...
 * Resolución: Acuerdo de precinto
 * Tareas: P19_AcuerdoPrecinto 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '304', 'R_RES_ACO_PRE', 'Resolución acordando precinto','Resolución acordando precinto',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PRE')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda resolución acordando precinto.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_RES_ACO_PRE','Input resolución acordando precinto',
'Input resolución acordando precinto.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='P19'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_RES_ACO_PRE'),'P19_AcuerdoPrecinto', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_RES_ACO_PRE') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='P19');
