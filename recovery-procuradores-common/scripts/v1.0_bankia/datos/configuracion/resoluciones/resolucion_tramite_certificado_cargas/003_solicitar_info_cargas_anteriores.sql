 ...
 * Resolución: Solicitar información cargas anteriores
 * Tareas: P08_SolicitarInformacionCargas 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '283', 'R_INF_CAR_ANT', 'Escrito sellado solicitando información cargas anteriores','Escrito sellado solicitando información cargas anteriores',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CCR')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito sellado solicitando información cargas anteriores.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_INF_CAR_ANT','Input escrito sellado solicitando información cargas anteriores',
'Input escrito sellado solicitando información cargas anteriores', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_INF_CAR_ANT'),'P08_SolicitarInformacionCargas', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_INF_CAR_ANT') 
