 ...
 * Resolución: Registro decreto de embargo
 * Tareas: P14_RegistroDecretoEmbargo_new1 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '297', 'R_ACR_MEJ_EMB', 'Decreto acordando mejora embargo','Decreto acordando mejora embargo',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'MEM')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda decreto acordando mejora embargo.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_ACR_MEJ_EMB','Input decreto acordando mejora embargo',
'Input decreto acordando mejora embargo.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_ACR_MEJ_EMB'),'P14_RegistroDecretoEmbargo_new1', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ACR_MEJ_EMB') 
