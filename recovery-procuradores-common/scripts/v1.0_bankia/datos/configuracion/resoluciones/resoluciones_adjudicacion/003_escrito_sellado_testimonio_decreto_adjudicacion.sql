 ...
 * Resolución: Solicicitud de testimonio de decreto adjudicacion.
 * Tareas: P413_SolicitudTestimonioDecretoAdjudicacion
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '219', 'R_TR_ADJ_TEST_DEC_ADJ', 'Testimonio de decreto de adjudicacion', 'Testimonio de decreto de adjudicacion', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'ADJ')
          , '0','MOD_PROC', SYSDATE, '0', 'Testimonio de decreto de adjudicacion.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_TR_ADJ_TEST_DEC_ADJ','Input testimonio de decreto de adjudicacion',
'Input testimonio de decreto de adjudicacion', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_TR_ADJ_TEST_DEC_ADJ'),'P413_SolicitudTestimonioDecretoAdjudicacion', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_TR_ADJ_TEST_DEC_ADJ') 
