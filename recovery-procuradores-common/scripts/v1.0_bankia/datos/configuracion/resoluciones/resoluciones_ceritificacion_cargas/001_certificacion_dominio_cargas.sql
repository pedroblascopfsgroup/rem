 ...
 * Resolución: solicitud certificación de dominio y cargas
 * Tareas: P08_SolicitudCertificacion 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '227', 'R_SOL_CER_CARG', 'Escrito sellado solicitando certificación de cargas', 'Escrito sellado solicitando certificación de cargas', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CCG')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de escrito sellado solicitando certificación de cargas.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_SOL_CER_CARG','Input de escrito sellado solicitando certificación de cargas',
'Input de escrito sellado solicitando certificación de cargas', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_SOL_CER_CARG'),'P08_SolicitudCertificacion', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_SOL_CER_CARG') 
