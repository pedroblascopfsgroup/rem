 ...
 * Resolución: Registrar recepción información de cargas extinguidas o minoradas
 * Tareas: P08_RegistrarInformacionCargas 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '230', 'R_REG_INF_CARG', 'Traslado oficio acreedores anteriores', 'Traslado oficio acreedores anteriores', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CCG')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda traslado oficio acreedores anteriores.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REG_INF_CARG','Input de traslado oficio acreedores anteriores',
'Input de traslado oficio acreedores anteriores', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REG_INF_CARG'),'P08_RegistrarInformacionCargas', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REG_INF_CARG') 
