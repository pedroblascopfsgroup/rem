 ...
 * Resolución: Requerir más información
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '231', 'R_REQ_MAS_INFO', 'Escrito sellado solicitando información no aportada', 'Escrito sellado solicitando información no aportada', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CCG')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito sellado solicitando información no aportada.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REQ_MAS_INFO','Input de escrito sellado solicitando información no aportada',
'Input de escrito sellado solicitando información no aportada', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REQ_MAS_INFO'),'P08_RequerirMasInfo', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REQ_MAS_INFO') 
