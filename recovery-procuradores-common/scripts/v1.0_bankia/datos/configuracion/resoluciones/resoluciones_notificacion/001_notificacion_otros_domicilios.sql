 ...
 * Resolución: solicitar notificacion otros domicilios o averiguacion
 * Tareas: P06_NotificacionAveriguacionDomicilio 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '240', 'R_NOT_OTR_DOM', 'Escrito sellado solicitando notificación en otros domicilios', 'Escrito sellado solicitando notificación en otros domicilios', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'NOT')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito sellado solicitando notificación en otros domicilios.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_NOT_OTR_DOM','Input escrito sellado solicitando notificación en otros domicilios',
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='P06'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_NOT_OTR_DOM'),'P06_NotificacionAveriguacionDomicilio', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_NOT_OTR_DOM') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='P06');
