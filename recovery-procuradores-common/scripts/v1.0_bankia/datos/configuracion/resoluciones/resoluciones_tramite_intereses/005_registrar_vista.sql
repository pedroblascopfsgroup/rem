 ...
 * Resolución: Registrar Vista
 * Tareas: P10_RegistrarVista 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '236', 'R_REG_VIS_IMP', 'Decreto estimando / desestimando impugnacion', 'Decreto estimando / desestimando impugnacion', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'INT')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de decreto estimando / desestimando impugnacion', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REG_VIS_IMP','Input decreto estimando / desestimando impugnacion',
'Input decreto estimando / desestimando impugnacion', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REG_VIS_IMP'),'P10_RegistrarVista', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REG_VIS_IMP') 
