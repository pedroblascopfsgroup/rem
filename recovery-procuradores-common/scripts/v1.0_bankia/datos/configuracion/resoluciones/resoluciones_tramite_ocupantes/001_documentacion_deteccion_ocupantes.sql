 ...
 * Tareas: P419_SolicitudRequerimientoDocOcupantes 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '240', 'R_ESC_SOL_OCU', 'Solicitud de requerimiento de documentación a ocupantes', 'Solicitud de requerimiento de documentación a ocupantes', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'OCU')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda solicitud de requerimiento de documentación a ocupantes.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_ESC_SOL_OCU','Input escrito sellado solicitando a ocupantes títulos',
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_ESC_SOL_OCU'),'P419_SolicitudRequerimientoDocOcupantes', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ESC_SOL_OCU') 
