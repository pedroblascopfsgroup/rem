...
AS
   SELECT vtar.usu_pendientes, vtar.tar_id, vtar.asu_id, vtar.tar_tarea,
    ...
    WHERE (   (res.res_epf_id = 2)
      ...
      /*
       * 
       * VISTA PARA AÑADIR LOS RECORDATORIOS A LAS TAREAS PENDIENTES DE VALIDAR
       * 
       *  CREATE OR REPLACE FORCE VIEW haya01.vtar_tarea_vs_procuradores (usu_pendientes,
                                                                tar_id,
                                                                asu_id,
...
AS
   SELECT rec.usu_id, tn.tar_id, NULL, tn.tar_tarea, tn.tar_descripcion, NULL,
          tn.tar_fecha_venc, NULL, 'PRC', NULL, NULL, cat.ctg_id, rec.cat_id,
          rec.usu_id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
          tn.dd_est_id, tn.dd_ein_id, tn.dd_sta_id, tn.tar_codigo,
          tn.tar_fecha_fin, tn.tar_fecha_ini, tn.tar_en_espera, tn.tar_alerta,
          tn.tar_tarea_finalizada, tn.tar_emisor, tn.VERSION, tn.usuariocrear,
          tn.fechacrear, tn.usuariomodificar, tn.fechamodificar,
          tn.usuarioborrar, tn.fechaborrar, tn.borrado, tn.cmb_id, tn.set_id,
          tn.obj_id, tn.tar_fecha_venc_real, tn.dtype, tn.nfa_tar_revisada,
          tn.nfa_tar_fecha_revis_aler, tn.nfa_tar_comentarios_alerta,
          tn.dd_tra_id, tn.cnt_id, tn.tar_destinatario,
          tn.tar_tipo_destinatario, tn.tar_id_dest, tn.per_id,
          tn.rpr_referencia, NULL, tn.dtype, sbtt.dd_sta_codigo,
          sbtt.dd_sta_descripcion, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
          NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
          NULL, NULL, NULL, NULL, NULL, NULL, NULL
     FROM tar_tareas_notificaciones tn INNER JOIN hayamaster.dd_sta_subtipo_tarea_base sbtt
          ON tn.dd_sta_id = sbtt.dd_sta_id
          INNER JOIN rec_recordatorio rec
          ON (   tn.tar_id = rec.tar_id_uno
              OR tn.tar_id = rec.tar_id_dos
              OR tn.tar_id = rec.tar_id_tres
             )
          LEFT JOIN cat_categorias cat ON cat.cat_id = rec.cat_id
    WHERE sbtt.dd_sta_codigo = 'TAREA_RECORDATORIO'
      AND tn.tar_tarea_finalizada != 0
   UNION
   SELECT vtar.usu_pendientes, vtar.tar_id, vtar.asu_id, vtar.tar_tarea,
          vtar.tar_descripcion, vtar.prc_id, vtar.tar_fecha_venc,
          tr.dd_tr_descripcion, estf.dd_epf_codigo, res.res_id,
