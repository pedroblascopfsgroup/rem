 ...
 * Resolución: Resolución acuerda averiguación domiciliaria.
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '360', 'R_TR_NOTI_RES_ACU_AVE', 'Resolución acuerda averiguación domiciliaria.', 'Resolución acuerda averiguación domiciliaria.', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'NOT')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de resolución acuerda averiguación domiciliaria.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'INFO')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_TR_NOTI_RES_ACU_AVE','Input Resolución acuerda averiguación domiciliaria',
'Input Resolución acuerda averiguación domiciliaria', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_TR_NOTI_RES_ACU_AVE'),'P400_GestionarNotificaciones', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_TR_NOTI_RES_ACU_AVE') 
