 ...
 * Resolución: Escrito sellado solicitando entrega de cantidades consignadas.
 * Tareas: H002_SolicitarMandamientoPago
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '214', 'R_TR_SUB_SAR_ESC_SELL_SOL', 'Escrito sellado solicitando entrega SAREB', 'Escrito sellado solicitando entrega SAREB', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'SBS')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda Escrito sellado solicitando entrega SAREB.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_TR_SUB_SAR_ESC_SELL_SOL','Input Escrito sellado solicitando entrega SAREB',
'Input Escrito sellado solicitando entrega SAREB', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_TR_SUB_SAR_ESC_SELL_SOL'),'H002_SolicitarMandamientoPago', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_TR_SUB_SAR_ESC_SELL_SOL') 
