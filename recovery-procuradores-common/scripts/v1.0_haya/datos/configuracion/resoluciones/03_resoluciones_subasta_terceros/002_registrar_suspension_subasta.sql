 ...
 * Resolución: Registrar resultado de suspensión de subasta.
 * Tareas: H004_RegistrarResolucionSolicitudSuspension
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '217', 'R_ACU_SUS_SUB_T', 'Resolución acuerda suspensión de subasta', 'Resolución acuerda suspensión de subasta', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'SBT')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda resolución acuerda suspensión de subasta', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_ACU_SUS_SUB_T','Input resolución acuerda suspensión de subasta',
'Input resolución acuerda suspensión de subasta', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_ACU_SUS_SUB_T'),'H004_RegistrarResolucionSolicitudSuspension', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ACU_SUS_SUB_T') 
...
Insert into BPM_IDT_INPUT_DATOS(BPM_IDT_ID, BPM_TPI_ID, BPM_IDT_NOMBRE, BPM_IDT_GRUPO, BPM_IDT_DATO, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
select S_BPM_IDT_INPUT_DATOS.nextVal, bpm_tpi_id , 'd_comboSolicitudSubastaSuspendida', 'generico', 'comboSolicitudSubastaSuspendida', 0, 'MOD_PROC', sysdate, 0 
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ACU_SUS_SUB_T') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H004');
