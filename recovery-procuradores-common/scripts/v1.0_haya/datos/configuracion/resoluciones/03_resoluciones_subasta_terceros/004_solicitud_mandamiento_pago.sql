 ...
 * Resolución: Solicitud mandamiento de pago.
 * Tareas: H004_SolicitarMandamientoPago
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '219', 'R_ESC_SLL_CCN_T', 'Escrito sellado solicitando entrega de cantidades consignadas', 'Escrito sellado solicitando entrega de cantidades consignadas', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'SBT')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito sellado solicitando entrega de cantidades consignadas', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_ESC_SLL_CCN_T','Input escrito sellado solicitando entrega de cantidades consignadas',
'Input escrito sellado solicitando entrega de cantidades consignadas', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_ESC_SLL_CCN_T'),'H004_SolicitarMandamientoPago', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ESC_SLL_CCN_T') 
