 ...
 * Resolución: Confirmar mandamiento de pago
 * Tareas: H004_ConfirmarRecepcionMandamientoDePago
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '220', 'R_DO_CAN_CNS_T', 'D.O Entrega de cantidad consignada 3º', 'D.O Entrega de cantidad consignada 3º', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'SBT')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda D.O Entrega de cantidad consignada 3º', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_DO_CAN_CNS_T','Input D.O Entrega de cantidad consignada 3º',
'Input D.O Entrega de cantidad consignada 3º', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_DO_CAN_CNS_T'),'H004_ConfirmarRecepcionMandamientoDePago', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_DO_CAN_CNS_T') 
