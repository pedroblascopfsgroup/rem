/**
 * Trámite: Cesión de Remate
 * Resolución: Acordando fecha de cesión
 * Tareas: H006_ResenyarFechaComparecencia
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '221', 'R_TR_CES_FECH_CES', 'Resolucion acordando fecha de cesion', 'Resolucion acordando fecha de cesion', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CES')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de resolucion acordando fecha de cesion.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_TR_CES_FECH_CES','Input fecha cesion',
'Input fecha cesion', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='H006'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_TR_CES_FECH_CES'),'H006_ResenyarFechaComparecencia', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_TR_CES_FECH_CES') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H006');
