 ...
 * Resolución: Acta cesión de remate
 * Tareas: H006_RealizacionCesionRemate
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '222', 'R_TR_CES_ACT_CES', 'Resolucion acta cesión de remate', 'Resolucion acta cesión de remate', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CES')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de resolucion acta cesión de remate.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_TR_CES_ACT_CES','Input acta cesión',
'Input acta cesión', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_TR_CES_ACT_CES'),'H006_RealizacionCesionRemate', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_TR_CES_ACT_CES') 
...
Insert into BPM_IDT_INPUT_DATOS(BPM_IDT_ID, BPM_TPI_ID, BPM_IDT_NOMBRE, BPM_IDT_GRUPO, BPM_IDT_DATO, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
select S_BPM_IDT_INPUT_DATOS.nextVal, bpm_tpi_id , 'd_comboTitularizado', 'generico', 'comboTitularizado', 0, 'MOD_PROC', sysdate, 0 
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_TR_CES_ACT_CES') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H006');
