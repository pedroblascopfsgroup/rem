 ...
 * Tareas: H007_AutoAprobacionCostas 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '231', 'R_AUT_APRO_COST', 'Decreto aprobando tasación de costas', 'Decreto aprobando tasación de costas', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'TCS')
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_AUT_APRO_COST','Input Decreto aprobando tasación de costas',
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_AUT_APRO_COST'),'H007_AutoAprobacionCostas', 'NONE', 0, 'MOD_PROC', 
...
Insert into BPM_IDT_INPUT_DATOS(BPM_IDT_ID, BPM_TPI_ID, BPM_IDT_NOMBRE, BPM_IDT_GRUPO, BPM_IDT_DATO, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
select S_BPM_IDT_INPUT_DATOS.nextVal, bpm_tpi_id , 'd_comboAprobada', 'generico', 'comboAprobada', 0, 'MOD_PROC', sysdate, 0 
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_AUT_APRO_COST') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H007');
