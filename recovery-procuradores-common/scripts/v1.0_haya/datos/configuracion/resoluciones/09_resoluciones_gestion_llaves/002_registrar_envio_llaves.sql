/**
 * Trámite: Tramite de gestión de llaves.
 * Resolución: Registrar Envio de Llaves
 * Tareas: H040_RegistrarEnvioLlaves 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '243', 'R_REG_ENV_LLV', 'Comunicación al letrado del envio llaves a la entidad', 'Comunicación al letrado que se han enviado las llaves a la entidad', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'GLL')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de comunicación al letrado que se han enviado las llaves a la entidad', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REG_ENV_LLV','Input comunicación al letrado que se han enviado las llaves a la entidad',
'Input comunicación al letrado que se han enviado las llaves a la entidad', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='H040'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REG_ENV_LLV'),'H040_RegistrarEnvioLlaves', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REG_ENV_LLV') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H040');
...
Insert into BPM_IDT_INPUT_DATOS(BPM_IDT_ID, BPM_TPI_ID, BPM_IDT_NOMBRE, BPM_IDT_GRUPO, BPM_IDT_DATO, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
select S_BPM_IDT_INPUT_DATOS.nextVal, bpm_tpi_id , 'd_nombre', 'generico', 'nombre', 0, 'MOD_PROC', sysdate, 0 
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REG_ENV_LLV') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H040');
