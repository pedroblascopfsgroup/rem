/**
 * Trámite: Ocupantes.
 * Resolución: Presentar escrito alegaciones
 * Tareas: H048_PresentarEscritoAlegaciones 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '248', 'R_PRS_ESC_ALE', 'Escrito sellado alegaciones frente a ocupantes de la vivienda', 'Escrito sellado alegaciones frente a ocupantes de la vivienda', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'OCU')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda Escrito sellado alegaciones frente a ocupantes de la vivienda.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_PRS_ESC_ALE','Input escrito sellado alegaciones frente a ocupantes de la vivienda',
'Input escrito sellado alegaciones frente a ocupantes de la vivienda', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_PRS_ESC_ALE'),'H048_PresentarEscritoAlegaciones', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_PRS_ESC_ALE') 
