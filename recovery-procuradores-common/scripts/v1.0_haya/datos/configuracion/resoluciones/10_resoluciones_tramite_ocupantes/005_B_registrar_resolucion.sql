/**
 * Trámite: Ocupantes.
 * Resolución: Registrar resolución no tiene derecho
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '251', 'R_REG_RES_NPR', 'Auto declara ocupante no tiene derecho a permanecer en la vivienda', 'Auto declara ocupante no tiene derecho a permanecer en la vivienda', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'OCU')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda Auto declara ocupante no tiene derecho a permanecer en la vivienda.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REG_RES_NPR','Auto declara ocupante no tiene derecho a permanecer en la vivienda',
'Input auto declara ocupante no tiene derecho a permanecer en la vivienda', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='H048'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REG_RES_NPR'),'H048_RegistrarResolucion', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REG_RES_NPR') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H048');
