 ...
 * Tareas: H052_RegistrarPresentacionEscritoSub 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '261', 'R_REG_PRS_ESB', 'Escrito sellado solicitando subsanación decreto adjudicación', 'Escrito sellado solicitando subsanación decreto adjudicación', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'SDA')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito sellado solicitando subsanación decreto adjudicación.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REG_PRS_ESB','Input escrito sellado solicitando subsanación decreto adjudicación',
'Input escrito sellado solicitando subsanación decreto adjudicación', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REG_PRS_ESB'),'H052_RegistrarPresentacionEscritoSub', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REG_PRS_ESB') 
