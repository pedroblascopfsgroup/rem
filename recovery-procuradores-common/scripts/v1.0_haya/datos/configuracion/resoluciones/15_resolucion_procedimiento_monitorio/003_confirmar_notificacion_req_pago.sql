/**
 * Trámite: P.Monitorio
 * Resolución: Confirmar notificación requerimiento de pago
 * Tareas: H022_ConfirmarNotificacionReqPago 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '274', 'R_NOT_REQ_PAG', 'Notificacion requerimiento pago', 'Notificacion requerimiento pago', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'MON')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda notificacion requerimiento pago.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_NOT_REQ_PAG','Input notificacion requerimiento pago',
'Input notificacion requerimiento pago', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_NOT_REQ_PAG'),'H022_ConfirmarNotificacionReqPago', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_NOT_REQ_PAG') 
