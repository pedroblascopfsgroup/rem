 ...
 * Resolución: Solicitar la notificación retención al pagador
 * Tareas: H038_SolicitarNotificacion 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '292', 'R_SOL_NOT_EMB', ' Escrito solicitándose libre oficio',' Escrito solicitándose libre oficio',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'EMS')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito solicitándose libre oficio.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_SOL_NOT_EMB','Input escrito solicitándose libre oficio',
'Input escrito solicitándose libre oficio.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_SOL_NOT_EMB'),'H038_SolicitarNotificacion', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_SOL_NOT_EMB') 
