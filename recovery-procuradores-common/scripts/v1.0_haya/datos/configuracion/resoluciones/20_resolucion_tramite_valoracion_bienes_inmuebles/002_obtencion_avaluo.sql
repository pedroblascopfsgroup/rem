 ...
 * Resolución: Obtención avaluo
 * Tareas: H058_ObtencionAvaluo 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '297', 'R_OBTE_AVAO', 'Informe pericial','Informe pericial',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'VBI')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda informe pericial.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_OBTE_AVAO','Input informe pericial',
'Input informe pericial.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_OBTE_AVAO'),'H058_ObtencionAvaluo', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_OBTE_AVAO') 
