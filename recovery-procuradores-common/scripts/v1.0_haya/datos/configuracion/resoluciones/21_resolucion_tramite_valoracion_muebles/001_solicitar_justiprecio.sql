 ...
 * Resolución: Solicitar justiprecio
 * Tareas: H060_SolicitarJustiprecio 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '300', 'R_SLC_AVL_NOM', 'Solicitud avaluo y nombre perito tasador','Escrito sellado solicitando avaluo y se nombre perito tasador',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'VBM')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito sellado solicitando avaluo y se nombre perito tasador.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_SLC_AVL_NOM','Input escrito sellado solicitando avaluo y se nombre perito tasador',
'Input escrito sellado solicitando avaluo y se nombre perito tasador.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_SLC_AVL_NOM'),'H060_SolicitarJustiprecio', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_SLC_AVL_NOM') 
