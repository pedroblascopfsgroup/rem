 ...
 * Resolución: Nombremiento de depositario
 * Tareas: H034_NombramientoDepositario 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '316', 'R_NOMBR_DEP', 'Resolución nombramiento de depositario','Resolución nombramiento de depositario',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PDD')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda resolución nombramiento de depositario.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_NOMBR_DEP','Input resolución nombramiento de depositario',
'Input resolución nombramiento de depositario.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_NOMBR_DEP'),'H034_NombramientoDepositario', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_NOMBR_DEP') 
