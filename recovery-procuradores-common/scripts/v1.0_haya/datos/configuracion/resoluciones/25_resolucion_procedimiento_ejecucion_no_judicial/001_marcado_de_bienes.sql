 ...
 * Resolución: Interposición de la demanda - marcado de bienes
 * Tareas: H020_InterposicionDemandaMasBienes 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '320', 'R_DEM_SLL_NJD', 'Demanda sellada','Demanda sellada',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PNJ')
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_DEM_SLL_NJD','Input demanda sellada',
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_DEM_SLL_NJD'),'H020_InterposicionDemandaMasBienes', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_DEM_SLL_NJD') 
...
Insert into BPM_IDT_INPUT_DATOS(BPM_IDT_ID, BPM_TPI_ID, BPM_IDT_NOMBRE, BPM_IDT_GRUPO, BPM_IDT_DATO, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
select S_BPM_IDT_INPUT_DATOS.nextVal, bpm_tpi_id , 'd_capitalNoVencido', 'generico', 'capitalNoVencido', 0, 'MOD_PROC', sysdate, 0 
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_DEM_SLL_NJD') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H020');
