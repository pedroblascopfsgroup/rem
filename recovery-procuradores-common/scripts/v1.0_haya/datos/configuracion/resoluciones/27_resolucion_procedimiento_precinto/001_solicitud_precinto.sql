 ...
 * Resolución: Solicitud de precinto
 * Tareas: H050_SolicitarPrecinto 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '344', 'R_ESC_SOL_PRE', 'Escrito sellado solicitando precinto','Escrito sellado solicitando precinto',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PRE')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda Escrito sellado solicitando precinto.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_ESC_SOL_PRE','Input escrito sellado solicitando precinto',
'Input escrito sellado solicitando precinto.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_ESC_SOL_PRE'),'H050_SolicitarPrecinto', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ESC_SOL_PRE') 
