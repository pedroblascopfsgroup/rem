/**
 * Trámite: Trámite de consignación.
 * Resolución: Escrito sellado acreditando consignación.
 * Tareas: H064_presentacionJuzgado
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '350', 'R_PR_CON_ESC_SELL_ACR', 'Escrito sellado acreditando consignacion', 'Escrito sellado acreditando consignacion', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CON')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de Escrito sellado acreditando consignacion.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='H064'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_PR_CON_ESC_SELL_ACR'),'H064_presentacionJuzgado', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_PR_CON_ESC_SELL_ACR') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H064');
