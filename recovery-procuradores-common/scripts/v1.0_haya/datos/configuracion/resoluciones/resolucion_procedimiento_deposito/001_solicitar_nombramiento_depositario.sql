/**
 * Trámite: P. DE DEPOSITO
 * Resolución: Solicitar nombremiento de depositario
 * Tareas: H034_SolicitarNombramiento 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '298', 'R_ESL_SOL_DEP', 'Escrito sellado solicitando depósito','Escrito sellado solicitando depósito',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PDD')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda escrito sellado solicitando depósito.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_ESL_SOL_DEP','Input escrito sellado solicitando depósito',
'Input escrito sellado solicitando depósito.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
Values(S_BPM_TPI_TIPO_PROC_INPUT.NEXTVAL, (SELECT DD_TPO_ID FROM DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO='H034'), 
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_ESL_SOL_DEP'),'H034_SolicitarNombramiento', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ESL_SOL_DEP') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H034');
