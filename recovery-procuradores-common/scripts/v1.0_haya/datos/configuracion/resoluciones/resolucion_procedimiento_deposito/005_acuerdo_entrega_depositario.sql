 ...
 * Resolución: Acuerdo entrega depositario
 * Tareas: H034_AcuerdoEntrega 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '319', 'R_ACU_ENT_DEP', 'Resolución de entrega del bien','Resolución de entrega del bien',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PDD')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda Resolución de entrega del bien.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_ACU_ENT_DEP','Input resolución de entrega del bien',
'Input resolución de entrega del bien.', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_ACU_ENT_DEP'),'H034_AcuerdoEntrega', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_ACU_ENT_DEP') 
