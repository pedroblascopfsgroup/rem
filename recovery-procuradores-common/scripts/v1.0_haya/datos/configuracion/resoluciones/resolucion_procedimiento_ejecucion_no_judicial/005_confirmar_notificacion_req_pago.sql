 ...
 * Tareas: H020_ConfirmarNotifiReqPago 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '312', 'R_REQ_POS_PNJ', 'Requerimiento positivo','Requerimiento positivo',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'PNJ')
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_REQ_POS_PNJ','Input requerimiento positivo',
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_REQ_POS_PNJ'),'H020_ConfirmarNotifiReqPago', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REQ_POS_PNJ') 
...
Insert into BPM_IDT_INPUT_DATOS(BPM_IDT_ID, BPM_TPI_ID, BPM_IDT_NOMBRE, BPM_IDT_GRUPO, BPM_IDT_DATO, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
select S_BPM_IDT_INPUT_DATOS.nextVal, bpm_tpi_id , 'd_comboConfirmacionReqPago', 'generico', 'comboConfirmacionReqPago', 0, 'MOD_PROC', sysdate, 0 
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_REQ_POS_PNJ') 
and dd_tpo_id=(select dd_tpo_id from dd_tpo_tipo_procedimiento where dd_tpo_codigo='H020');
