/**
 * Trámite: P.Monitorio
 * Resolución: Confirmar admision de demanda
 * Tareas: H022_ConfirmarAdmisionDemanda 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '266', 'R_DEC_ADI_DEM', 'Decreto de admision de demanda', 'Decreto de admision de demanda', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'MON')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda decreto de admision de demanda.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_DEC_ADI_DEM','Input decreto de admision de demanda',
'Input decreto de admision de demanda', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_DEC_ADI_DEM'),'H022_ConfirmarAdmisionDemanda', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_DEC_ADI_DEM') 
