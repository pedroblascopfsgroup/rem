 ...
 * Resolución: Solicitar certificación de dominio y cargas
 * Tareas: H030_SolicitudCertificacion 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '281', 'R_CER_DOM_CAR', 'Certificación de dominio de cargas','Escrito solicitando certificación de dominio de cargas',(SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'CCR')
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_CER_DOM_CAR','Input escrito solicitando certificación de dominio de cargas',
'Input escrito solicitando certificación de dominio de cargas', 0, 'MOD_PROC', sysdate, '0',null from dual;
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_CER_DOM_CAR'),'H030_SolicitudCertificacion', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_CER_DOM_CAR') 
