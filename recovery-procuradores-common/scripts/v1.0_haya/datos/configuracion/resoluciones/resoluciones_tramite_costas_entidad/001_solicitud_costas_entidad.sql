 ...
 * Resolución: Solicitud costas entidad
 * Tareas: H032_RegistrarSolCostasContrario 
 **/
...
INSERT INTO dd_tr_tipos_resolucion
   ...
   SELECT '227', 'R_TR_COS_ENT_SOL_COST', 'Translado de contrario instando solicitud de costas', 'Translado de contrario instando solicitud de costas', (SELECT dd_tj_id FROM dd_tj_tipo_juicio WHERE dd_tj_codigo = 'TCE')
          , '0','MOD_PROC', SYSDATE, '0', 'Ayuda de translado de contrario instando solicitud de costas.', (SELECT bpm_dd_tac_id FROM bpm_dd_tac_tipo_accion WHERE bpm_dd_tac_codigo = 'ADVANCE')
     FROM DUAL;
...
SELECT S_BPM_DD_TIN_TIPO_INPUT.nextVal,'I_TR_COS_ENT_SOL_COST','Input escrito sellado solicitando tasacion de costas y liquidacion de intereses',
...
(SELECT BPM_DD_TIN_ID FROM BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='I_TR_COS_ENT_SOL_COST'),'H032_RegistrarSolCostasContrario', 'NONE', 0, 'MOD_PROC', 
...
from bpm_tpi_tipo_proc_input where bpm_dd_tin_id = (select bpm_dd_tin_id from bpm_dd_tin_tipo_input where bpm_dd_tin_codigo='I_TR_COS_ENT_SOL_COST') 
