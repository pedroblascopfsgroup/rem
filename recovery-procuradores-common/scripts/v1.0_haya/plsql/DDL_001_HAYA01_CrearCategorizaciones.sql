...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_CONSTRAINTS WHERE TABLE_NAME = ''CTG_CATEGORIZACIONES'' and owner = '''||V_ESQUEMA||''' and CONSTRAINT_TYPE = ''P''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;

	IF table_count = 1 THEN
		V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.CTG_CATEGORIZACIONES
			DROP PRIMARY KEY CASCADE';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.CTG_CATEGORIZACIONES... Claves primarias eliminadas');
	END IF;
	...
	IF table_count = 1 THEN
		V_MSQL := 'DROP TABLE '||V_ESQUEMA||.'CTG_CATEGORIZACIONES CASCADE CONSTRAINTS';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'CTG_CATEGORIZACIONES... Tabla borrada');
	END IF;
	
	
	-- Si existe la secuencia la borramos
	V_MSQL := 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_NAME = ''S_DD_CTG_CATEGORIZACIONES'' and sequence_owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;
	IF table_count = 1 THEN
		V_MSQL := 'DROP SEQUENCE '||V_ESQUEMA||'S_CTG_CATEGORIZACIONES';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA|| 'S_CTG_CATEGORIZACIONES... Secuencia eliminada');
	END IF;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.CTG_CATEGORIZACIONES... Compobaciones previas FIN');
	...
	EXECUTE IMMEDIATE V_MSQL;
	DMBS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'S_CTG_CATEGORIZACIONES... Secuencia creada');
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.CTG_CATEGORIZACIONES ADD (
					CONSTRAINT PK_CTG_CATEGORIZACIONES PRIMARY KEY (CTG_ID)
					)';
	EXECUTE IMMEDIATE V_MSQL;
