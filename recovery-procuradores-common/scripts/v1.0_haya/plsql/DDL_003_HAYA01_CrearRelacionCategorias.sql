...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_CONSTRAINTS WHERE TABLE_NAME = ''REL_CATEGORIAS'' and owner = '''||V_ESQUEMA||''' and CONSTRAINT_TYPE = ''P''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;

	IF table_count = 1 THEN
		V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.REL_CATEGORIAS
			DROP PRIMARY KEY CASCADE';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.REL_CATEGORIAS... Claves primarias eliminadas');
	END IF;
	...
	IF table_count = 1 THEN
		V_MSQL := 'DROP TABLE '||V_ESQUEMA||.'REL_CATEGORIAS CASCADE CONSTRAINTS';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'REL_CATEGORIAS... Tabla borrada');
	END IF;
	
	
	-- Si existe la secuencia la borramos
	V_MSQL := 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_NAME = ''S_REL_CATEGORIAS'' and sequence_owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;
	IF table_count = 1 THEN
		V_MSQL := 'DROP SEQUENCE '||V_ESQUEMA||'S_REL_CATEGORIAS';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA|| 'S_REL_CATEGORIAS... Secuencia eliminada');
	END IF;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.REL_CATEGORIAS... Compobaciones previas FIN');
	...
	EXECUTE IMMEDIATE V_MSQL;
	DMBS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'S_REL_CATEGORIAS... Secuencia creada');
