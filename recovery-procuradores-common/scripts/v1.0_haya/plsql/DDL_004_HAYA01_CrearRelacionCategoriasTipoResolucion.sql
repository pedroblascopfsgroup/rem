...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_CONSTRAINTS WHERE TABLE_NAME = ''REL_CATEGORIAS_TIPORESOL'' and owner = '''||V_ESQUEMA||''' and CONSTRAINT_TYPE = ''P''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;

	IF table_count = 1 THEN
		V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.REL_CATEGORIAS_TIPORESOL
			DROP PRIMARY KEY CASCADE';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.REL_CATEGORIAS_TIPORESOL... Claves primarias eliminadas');
	END IF;
	...
	IF table_count = 1 THEN
		V_MSQL := 'DROP TABLE '||V_ESQUEMA||.'REL_CATEGORIAS_TIPORESOL CASCADE CONSTRAINTS';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'REL_CATEGORIAS_TIPORESOL... Tabla borrada');
	END IF;
	
	
	-- Si existe la secuencia la borramos
	V_MSQL := 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_NAME = ''S_REL_CATEGORIAS_TIPORESOL'' and sequence_owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;
	IF table_count = 1 THEN
		V_MSQL := 'DROP SEQUENCE '||V_ESQUEMA||'S_REL_CATEGORIAS_TIPORESOL';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA|| 'S_REL_CATEGORIAS_TIPORESOL... Secuencia eliminada');
	END IF;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.REL_CATEGORIAS_TIPORESOL... Compobaciones previas FIN');
	...
	EXECUTE IMMEDIATE V_MSQL;
	DMBS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'S_REL_CATEGORIAS_TIPORESOL... Secuencia creada');
