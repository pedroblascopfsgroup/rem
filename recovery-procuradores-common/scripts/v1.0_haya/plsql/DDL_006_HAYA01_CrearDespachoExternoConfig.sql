...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_CONSTRAINTS WHERE TABLE_NAME = ''DEC_DESPACHO_EXT_CONFIG'' and owner = '''||V_ESQUEMA||''' and CONSTRAINT_TYPE = ''P''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;

	IF table_count = 1 THEN
		V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.DEC_DESPACHO_EXT_CONFIG
			DROP PRIMARY KEY CASCADE';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.DEC_DESPACHO_EXT_CONFIG... Claves primarias eliminadas');
	END IF;
	...
	IF table_count = 1 THEN
		V_MSQL := 'DROP TABLE '||V_ESQUEMA||.'DEC_DESPACHO_EXT_CONFIG CASCADE CONSTRAINTS';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'DEC_DESPACHO_EXT_CONFIG... Tabla borrada');
	END IF;
	
	
	-- Si existe la secuencia la borramos
	V_MSQL := 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_NAME = ''S_DEC_DESPACHO_EXT_CONFIG'' and sequence_owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;
	IF table_count = 1 THEN
		V_MSQL := 'DROP SEQUENCE '||V_ESQUEMA||'S_DEC_DESPACHO_EXT_CONFIG';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA|| 'S_DEC_DESPACHO_EXT_CONFIG... Secuencia eliminada');
	END IF;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.DEC_DESPACHO_EXT_CONFIG... Compobaciones previas FIN');
	...
	EXECUTE IMMEDIATE V_MSQL;
	DMBS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'S_DEC_DESPACHO_EXT_CONFIG... Secuencia creada');
