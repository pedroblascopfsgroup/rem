...
--## PRODUCTO=SI
--##
--## Finalidad: Inserta los valores de prueba de categorias de la Prioridad.
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_CAT_CATEGORIAS T_ARRAY_CAT := T_ARRAY_CAT(
      T_CAT_CATEGORIAS('Prioridad Alta', 'Prioridad Alta', 0, 'MOD_PROC',SYSDATE,0,1),
      T_CAT_CATEGORIAS('Prioridad Media', 'Prioridad Media', 0, 'MOD_PROC',SYSDATE,0,2),
      T_CAT_CATEGORIAS('Prioridad Baja', 'Prioridad Baja', 0, 'MOD_PROC',SYSDATE,0,3)
    );   
...
BEGIN	
    ...
    --Obtenemos categorización de Prioridad
    V_SQL := 'SELECT CTG_ID FROM '||V_ESQUEMA||'.CTG_CATEGORIZACIONES WHERE CTG_CODIGO = ''PRIORIDAD''';
