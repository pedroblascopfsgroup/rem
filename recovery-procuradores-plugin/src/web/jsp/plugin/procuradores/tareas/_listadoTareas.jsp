...
<%--	categorizacionesStore.webflow({});		 --%>
	...
	var getParametrosBusqueda=function(){
		return {
			...
			,categorizacion:comboCtgResol.getValue()
		}
	}
						...
						success : function(data) {
							...
							if(count < limit){
							 app.openBrowserWindow('/pfs/tareanotificacion/exportacionTareasPaginaDescarga',parametros);  
								...
								Ext.MessageBox.hide();
								Ext.Msg.alert('<s:message code="plugin.mejoras.error" text="**Error" />', '<s:message code="plugin.mejoras.tareas.exportarExcel.limiteSuperado1" text="**Se ha establecido un l\EDmite m\E1ximo de " />'+ limit + ' '+
									'<s:message code="plugin.mejoras.tareas.exportarExcel.limiteSuperado2" text="**Tareas a Exportar. Por favor utilice los filtros para limitar el n\FAmero de resultados." />');
							}							    			
						},
						failure: function (result) {
							Ext.MessageBox.hide();
							Ext.Msg.alert('<s:message code="plugin.ugas.ws.error" text="**Error" />', '<s:message code="plugin.ugas.asuntos.exportarExcel.errorExportando" text="**Se ha producido un error durante el proceso de validaci\F3n de la exportaci\F3n a excel." />');
					    }
					});
	...
	var panelFiltros = new Ext.Panel({
		title : '<s:message code="tareas.listado.busqueda.procuradores" text="**Busqueda de Tareas"/>'
		,collapsible : true
		,titleCollapse : true
		,layout:'table'
		...
		,items:[{
				...
				,width:'320px'
				<c:if test="${activoDespachoIntegral==true}">
				,items:[nombreTarea, comboFechaDesdeOp, comboFechaHastaOp, comboEstado]
				</c:if>
				<c:if test="${activoDespachoIntegral==false}">
	...
	var tarea = Ext.data.Record.create([
		...
		<%-- ,{name:'fechaVencOri',type:'date', dateFormat:'d/m/Y'}--%>
		,{name:'fechaVencReal',type:'date', dateFormat:'d/m/Y'}
	...
	var tareasNewCm=new Ext.grid.ColumnModel([
		...
		,{	/*Columna 5*/ header: '<s:message code="plugin.mejoras.tareas.listado.fechavencOri" text="**Fecha Vto. Orig."/>', sortable: true, dataIndex: 'fechaVencReal', renderer:app.format.dateRenderer, width:50}
	...
	if(eval(enEspera)) {
		var tareasNewCm=new Ext.grid.ColumnModel([
			...
			,{	/*Columna 4*/ header: '<s:message code="tareas.listado.fechainicio" text="**Fecha inicio"/>', sortable: true, hidden:true, dataIndex: 'fechaInicio', renderer:app.format.dateRenderer, width:50}
			,{	/*Columna 5*/ header: '<s:message code="tareas.listado.fechavenc" text="**Fecha Vto."/>', sortable: true, dataIndex: 'fechaVenc', renderer:app.format.dateRenderer, width:50}
			,{	/*Columna 5*/ header: '<s:message code="plugin.mejoras.tareas.listado.fechavencOri" text="**Fecha Realizacion."/>', sortable: true, dataIndex: 'fechaVencReal', renderer:app.format.dateRenderer, width:50}
			,{	/*Columna 6*/ header: '<s:message code="tareas.listado.tiposolicitud" text="**Tipo solicitud"/>',sortable: true, dataIndex: 'tipoSolicitudSQL', width:75}
			,{  /*Columna 7*/ header: '<s:message code="tareas.listado.diasvencida" text="**Dias Vencida"/>', sortable: true, dataIndex: 'diasVencidoSQL', width:50}
			,{  /*Columna 8*/ header: '<s:message code="tareas.listado.gestor" text="**Gestor"/>', sortable: false, dataIndex: 'gestor', hidden:true}
			,{  /*Columna 9*/ header: '<s:message code="tareas.listado.supervisor" text="**Supervisor"/>', sortable: false, dataIndex: 'supervisor', hidden:true}
