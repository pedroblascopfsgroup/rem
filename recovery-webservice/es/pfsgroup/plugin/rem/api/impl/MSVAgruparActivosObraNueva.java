...
@Component
public class MSVAgruparActivosObraNueva implements MSVLiberator {
	...
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODE_FILE_BULKUPLOAD_NEW_BUILDING.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
