...
public class AccionesCaixaManager extends BusinessOperationOverrider<AccionesCaixaApi> implements AccionesCaixaApi {
	...
	public boolean accionRechazo(DtoAccionRechazoCaixa dto) throws Exception {
		Oferta ofr = genericDao.get(Oferta.class,
				genericDao.createFilter(FilterType.EQUALS, "numOferta", dto.getNumOferta()));
		if (dto.getIdExpediente() != null) {
			ExpedienteComercial eco = genericDao.get(ExpedienteComercial.class,
					genericDao.createFilter(FilterType.EQUALS, "id", dto.getIdExpediente()));
			...
			if (DDTipoOferta.isTipoAlquiler(tipoOferta) || DDTipoOferta.isTipoAlquilerNoComercial(tipoOferta)) {
				...
				TareaExterna tarea = genericDao.get(TareaExterna.class,
						genericDao.createFilter(FilterType.EQUALS, "tareaPadre.id", dto.getIdTarea()));
				String codigoTarea = tarea.getTareaProcedimiento().getCodigo();
				dto.setEstadoBc(calcularEstadoBcRechazo(codigoTarea));
				dto.setMotivoAnulacion(expedienteComercialApi.getMotivoRechazoAccionRechazo(tipoOferta, codigoTarea,
						dto.getMotivoAnulacion()));

				eco.setMotivoAnulacion(genericDao.get(DDMotivoAnulacionExpediente.class,
						genericDao.createFilter(FilterType.EQUALS, "codigo", dto.getMotivoAnulacion())));
				...
				eco.setEstadoBc(genericDao.get(DDEstadoExpedienteBc.class, genericDao.createFilter(FilterType.EQUALS,
						"codigo", expedienteComercialApi.devolverEstadoCancelacionBCEco(ofr, eco))));
				genericDao.save(ExpedienteComercial.class, eco);
		...
		if (ofr != null) {
			ofr.setEstadoOferta(genericDao.get(DDEstadoOferta.class,
	...
	public void accionResultadoRiesgo(DtoAccionResultadoRiesgoCaixa dto) throws Exception {
		OfertaCaixa ofrCaixa = genericDao.get(OfertaCaixa.class,
				genericDao.createFilter(FilterType.EQUALS, "oferta.numOferta", dto.getNumOferta()));
		DDRiesgoOperacion rop = genericDao.get(DDRiesgoOperacion.class,
				genericDao.createFilter(FilterType.EQUALS, "codigoC4C", dto.getRiesgoOperacion()));
		...
		if (ofrCaixa.getOferta() != null) {
			...
			if (DDTipoOferta.isTipoAlquiler(oferta.getTipoOferta())
					|| DDTipoOferta.isTipoAlquilerNoComercial(oferta.getTipoOferta())) {
				...
				if (eco != null && dto.getIdTarea() != null && dto.getCodTipoTarea() != null
						&& (DDTipoTareaPbc.CODIGO_PBC.equals(dto.getCodTipoTarea())
								|| DDTipoTareaPbc.CODIGO_PBCARRAS.equals(dto.getCodTipoTarea()))
						&& (DDEstadoExpedienteBc.CODIGO_PTE_CALCULO_RIESGO.equals(estadoBC)
								|| DDEstadoExpedienteBc.CODIGO_ARRAS_FIRMADAS.equals(estadoBC))) {
					eco.setEstadoBc(genericDao.get(DDEstadoExpedienteBc.class, genericDao
							.createFilter(FilterType.EQUALS, "codigo", DDEstadoExpedienteBc.PTE_SANCION_PBC_SERVICER)));
					genericDao.save(ExpedienteComercial.class, eco);
				}
			}
		}
	...
	public void accionIngresoFinalAprobado(DtoOnlyExpedienteYOfertaCaixa dto) {
		ExpedienteComercial expediente = expedienteComercialApi.findOne(dto.getIdExpediente());
		DDEstadoExpedienteBc estadoExpedienteBc = genericDao.get(DDEstadoExpedienteBc.class, genericDao
				.createFilter(FilterType.EQUALS, "codigo", DDEstadoExpedienteBc.CODIGO_IMPORTE_FINAL_APROBADO));
		expediente.setEstadoBc(estadoExpedienteBc);
		genericDao.save(ExpedienteComercial.class, expediente);
	...
	public void accionFirmaArrasAprobadas(DtoFirmaArrasCaixa dto) throws Exception {
		ExpedienteComercial expediente = expedienteComercialApi.findOne(dto.getIdExpediente());
		DDEstadoExpedienteBc estadoExpedienteBc = genericDao.get(DDEstadoExpedienteBc.class, genericDao
				.createFilter(FilterType.EQUALS, "codigo", DDEstadoExpedienteBc.CODIGO_FIRMA_DE_ARRAS_AGENDADAS));
		expediente.setEstadoBc(estadoExpedienteBc);
	...
	public Map<String, String[]> createRequestAccionFirmaArras(DtoFirmaArrasCaixa dto) throws ParseException {
		...
		String[] fechaPropuesta = {
				dto.getFechaPropuesta() != null ? sdf.format(sdfEntrada.parse(dto.getFechaPropuesta())) : null };
	...
	@Transactional
	@Override
	public void accionFirmaContratoAprobada(JSONObject dto) throws Exception {
		
		ExpedienteComercial expediente = expedienteComercialApi.findOne(Long.parseLong(dto.get("idExpediente").toString()));
		
		if (DDTipoOferta.isTipoVenta(expediente.getOferta().getTipoOferta())) {
		
			DDEstadoExpedienteBc estadoExpedienteBc = genericDao.get(DDEstadoExpedienteBc.class, genericDao
					.createFilter(FilterType.EQUALS, "codigo", DDEstadoExpedienteBc.CODIGO_FIRMA_DE_CONTRATO_AGENDADO));
			expediente.setEstadoBc(estadoExpedienteBc);
		
			Posicionamiento posicionamiento = genericDao.get(Posicionamiento.class,
					genericDao.createFilter(FilterType.EQUALS, "id", Long.parseLong(dto.get("idPosicionamiento").toString())));
		
			posicionamiento.setValidacionBCPos(genericDao.get(DDMotivosEstadoBC.class,
					genericDao.createFilter(FilterType.EQUALS, "codigo", DDMotivosEstadoBC.CODIGO_APROBADA_BC)));
	...
	public Map<String, String[]> createRequestVentaContabilizada(DtoAccionVentaContabilizada dto)
			throws ParseException {
		Map<String, String[]> map = new HashMap<String, String[]>();

		String[] idTarea = { dto.getIdTarea().toString() };
		String[] fechaIngreso = {
				dto.getFechaReal() != null ? sdf.format(sdfEntrada.parse(dto.getFechaReal())) : null };
	...
	public void accionArrasPteDoc(DtoOnlyExpedienteYOfertaCaixa dto) {
		...
		DDEstadoExpedienteBc estadoExpedienteBc = genericDao.get(DDEstadoExpedienteBc.class, genericDao
				.createFilter(FilterType.EQUALS, "codigo", DDEstadoExpedienteBc.CODIGO_ARRAS_PTE_DOCUMENTACION));
		expediente.setEstadoBc(estadoExpedienteBc);
	...
	public void accionIngresoFinalPdteDoc(DtoOnlyExpedienteYOfertaCaixa dto) {
		...
		DDEstadoExpedienteBc estadoExpedienteBc = genericDao.get(DDEstadoExpedienteBc.class, genericDao
				.createFilter(FilterType.EQUALS, "codigo", DDEstadoExpedienteBc.CODIGO_IMPORTE_FINAL_PTE_DOC));
		expediente.setEstadoBc(estadoExpedienteBc);
	...
	public void accionAprobarModTitulares(DtoOnlyExpedienteYOfertaCaixa dto) {
		...
		DDEstadoInterlocutor eicActivo = genericDao.get(DDEstadoInterlocutor.class,
				genericDao.createFilter(FilterType.EQUALS, "codigo", DDEstadoInterlocutor.CODIGO_ACTIVO));
		DDEstadoInterlocutor eicInActivo = genericDao.get(DDEstadoInterlocutor.class,
				genericDao.createFilter(FilterType.EQUALS, "codigo", DDEstadoInterlocutor.CODIGO_INACTIVO));
		...
		List<CompradorExpediente> cex = genericDao.getList(CompradorExpediente.class,
				genericDao.createFilter(FilterType.EQUALS, "expediente", dto.getIdExpediente()));
		...
		for (CompradorExpediente compradorExpediente : cex) {
			if (DDEstadoInterlocutor.isSolicitudBaja(compradorExpediente.getEstadoInterlocutor())
					|| DDEstadoInterlocutor.isBaja(compradorExpediente.getEstadoInterlocutor())) {
		...
		if (expediente != null) {
			...
			if (expediente.getOferta() != null && expediente.getOferta().getOfertaCaixa() != null) {
				...
				ofrCaixa.setEstadoComunicacionC4C(genericDao.get(DDEstadoComunicacionC4C.class,
						genericDao.createFilter(FilterType.EQUALS, "codigo", DDEstadoComunicacionC4C.C4C_VALIDADO)));
				genericDao.save(OfertaCaixa.class, ofrCaixa);
			}
			genericDao.save(ExpedienteComercial.class, expediente);
		...
		if (expediente != null) {
			DDEstadoExpedienteBc estadoExpedienteBc = genericDao.get(DDEstadoExpedienteBc.class,
					genericDao.createFilter(FilterType.EQUALS, "codigo",
							DDEstadoExpedienteBc.CODIGO_SOLICITAR_DEVOLUCION_DE_RESERVA_Y_O_ARRAS_A_BC));
			expediente.setEstadoBc(estadoExpedienteBc);
			...
			if (reserva != null) {
				...
				DDEstadosReserva estadoReserva = genericDao.get(DDEstadosReserva.class, genericDao
						.createFilter(FilterType.EQUALS, "codigo", DDEstadosReserva.CODIGO_PENDIENTE_DEVOLUCION));
	...
	public void accionIncautarArras(DtoOnlyExpedienteOfertaCaixaYFecha dto) throws ParseException {
		...
		if (expediente != null) {
			...
			if (reserva != null) {
				...
				DDEstadosReserva estadoReserva = genericDao.get(DDEstadosReserva.class, genericDao
						.createFilter(FilterType.EQUALS, "codigo", DDEstadosReserva.CODIGO_RESUELTA_POSIBLE_REINTEGRO));
				DDEstadoDevolucion estadoDevolucion = genericDao.get(DDEstadoDevolucion.class,
						genericDao.createFilter(FilterType.EQUALS, "codigo", DDEstadoDevolucion.ESTADO_PENDIENTE));
	...
	public void accionDevolArrasCont(DtoAccionRechazoCaixa dto) throws ParseException {
		...
		if (eco != null) {
			eco.setEstado(genericDao.get(DDEstadosExpedienteComercial.class,
					genericDao.createFilter(FilterType.EQUALS, "codigo", DDEstadosExpedienteComercial.ANULADO)));

			Reserva reserva = eco.getReserva();
			if (reserva != null) {
				DDEstadosReserva estadoReserva = genericDao.get(DDEstadosReserva.class, genericDao
						.createFilter(FilterType.EQUALS, "codigo", DDEstadosReserva.CODIGO_RESUELTA_DEVUELTA));
				DDEstadoDevolucion estadoDevolucion = genericDao.get(DDEstadoDevolucion.class,
			...
			if (tareasActivas != null) {
				...
				for (TareaExterna tareaExterna : tareasActivas) {
					if (TareaProcedimientoConstants.CODIGO_RESOLUCION_EXPEDIENTE_T017
							.equals(tareaExterna.getTareaProcedimiento().getCodigo())) {
	...
	public void accionIncautacionArrasCont(DtoAccionRechazoCaixa dto) throws ParseException {
		...
		if (eco != null) {
			...
			Reserva reserva = eco.getReserva();
			if (reserva != null) {
				reserva.setEstadoReserva(genericDao.get(DDEstadosReserva.class, genericDao
						.createFilter(FilterType.EQUALS, "codigo", DDEstadosReserva.CODIGO_RESUELTA_REINTEGRADA)));
				genericDao.save(Reserva.class, reserva);
			}
	...
	@Override
	@Transactional
	public void accionFirmaContratoRechazada(JSONObject dto) throws Exception {
		ExpedienteComercial expediente = expedienteComercialApi.findOne(Long.parseLong(dto.get("idExpediente").toString()));
	...
	public void accionArrasContabilizadas(DtoExpedienteFechaYOfertaCaixa dto) throws ParseException {
		...
		expediente.setEstadoBc(estadoExpedienteBc);
		expediente.setEstado(genericDao.get(DDEstadosExpedienteComercial.class,
				genericDao.createFilter(FilterType.EQUALS, "codigo", DDEstadosExpedienteComercial.PTE_PBC_VENTAS)));
	...
	public Boolean avanzarTareaGenerico(net.sf.json.JSONObject dto) throws Exception {
		...
		dataHolder.setPreviousStateExpedienteBcCod(
				expedienteComercialApi.getEstadoExpedienteBcFromIdTarea(dataHolder.getIdTarea()));
	...
	public void sendReplicarOfertaAccion(Long idExpediente) {
		...
		ofertaApi.replicateOfertaFlushDto(eco.getOferta(),
				expedienteComercialApi.buildReplicarOfertaDtoFromExpediente(eco));
	}
	...
	private String calcularEstadoBcRechazo(String codigoTarea) {
		String resultado;
		if (TareaProcedimientoConstants.CODIGO_T018_ANALISIS_BC.equals(codigoTarea)
				|| TareaProcedimientoConstants.CODIGO_T018_SCORING_BC.equals(codigoTarea)) {
	...
	private HistoricoTareaPbc createHistoricoTareaPbc(Oferta oferta, String codTipoTarea) {
		...
		HistoricoTareaPbc historico = genericDao.get(HistoricoTareaPbc.class, filterOferta, filterTipoPbc,
				filterActiva);
	...
	public Map<String, String[]> createRequestAccionCalculoRiesgo(DtoAccionResultadoRiesgoCaixa dto)
			throws ParseException {
		Map<String, String[]> map = new HashMap<String, String[]>();
	...
	public void accionIngresoDeposito(Long numOferta) {
		...
		if (oferta.getIsEnConcurrencia() != null && oferta.getIsEnConcurrencia()) {
			principalCaducada = concurrenciaApi.caducaOfertaConcurrencia(oferta.getActivoPrincipal().getId(),
					oferta.getId());
		}
	...
	public boolean incautaODevuelveDeposito(String codEstado, Long numOferta) {
		...
		if (ofr != null && ofr.getOfertaCaixa() != null) {
			ofr.getOfertaCaixa().setEstadoOfertaBc(genericDao.get(DDEstadoOfertaBC.class,
					genericDao.createFilter(FilterType.EQUALS, "codigo", DDEstadoOfertaBC.CODIGO_CANCELADA)));

			if (ofr.getExpedienteComercial() != null) {
				ofr.getExpedienteComercial()
						.setEstadoBc(genericDao.get(DDEstadoExpedienteBc.class,
								genericDao.createFilter(FilterType.EQUALS, "codigo", expedienteComercialApi
										.devolverEstadoCancelacionBCEco(ofr, ofr.getExpedienteComercial()))));
			}
