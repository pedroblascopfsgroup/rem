...
public class AltaActivoFinanciero implements AltaActivoService  {
	...
	private Activo dtoToEntityActivo(DtoAltaActivoFinanciero dtoAAF) throws Exception{
		...
		if(!Checks.esNulo(activo.getCartera())) {
			// Calcular la subcartera en base a la cartera (siempre tipo asistida) y establecer el 'num activo cartera'.
		...
		beanUtilNotNull.copyProperty(activo, "tipoComercializacion", utilDiccionarioApi.dameValorDiccionarioByCod(DDTipoComercializacion.class, dtoAAF.getDestinoComercialCodigo()));
		...
		beanUtilNotNull.copyProperty(activo, "situacionComercial", utilDiccionarioApi.dameValorDiccionarioByCod(DDSituacionComercial.class, DDSituacionComercial.CODIGO_NO_COMERCIALIZABLE));
	...
	private void dtoToEntitiesOtras(DtoAltaActivoFinanciero dtoAAF, Activo activo) throws Exception{
		...
		beanUtilNotNull.copyProperty(localizacionBien, "unidadPoblacional", utilDiccionarioApi.dameValorDiccionarioByCod(DDUnidadPoblacional.class, dtoAAF.getUnidadMunicipioCodigo()));
		...
		beanUtilNotNull.copyProperty(activoBancario, "estadoExpRiesgo", utilDiccionarioApi.dameValorDiccionarioByCod(DDEstadoExpRiesgoBancario.class,dtoAAF.getEstadoExpedienteRiesgoCodigo()));
		...
		beanUtilNotNull.copyProperty(activoPropietarioActivo, "tipoGradoPropiedad", utilDiccionarioApi.dameValorDiccionarioByCod(DDTipoGradoPropiedad.class, dtoAAF.getGradoPropiedadCodigo()));
		...
		beanUtilNotNull.copyProperty(admisionDocumento, "tipoCalificacionEnergetica", utilDiccionarioApi.dameValorDiccionarioByCod(DDTipoCalificacionEnergetica.class, dtoAAF.getCalificacionCeeCodigo()));
		...
		activoInfoComercial = genericDao.save(ActivoInfoComercial.class, activoInfoComercial);
		...
		if(!Checks.esNulo(dtoAAF.getNumBanyosVivienda()) && dtoAAF.getNumBanyosVivienda() > 0) {
			...
			banyo.setInfoComercial(activoInfoComercial);
		...
		if(!Checks.esNulo(dtoAAF.getNumAseosVivienda()) && dtoAAF.getNumAseosVivienda() > 0) {
			...
			aseos.setInfoComercial(activoInfoComercial);
		...
		if(!Checks.esNulo(dtoAAF.getNumDormitoriosVivienda()) && dtoAAF.getNumDormitoriosVivienda() > 0) {
			...
			dormitorio.setInfoComercial(activoInfoComercial);
		...
		// Garaje Anejo.
		if(dtoAAF.getGarajeAnejo()) {
			ActivoDistribucion garajeAnejo = new ActivoDistribucion();
			garajeAnejo.setInfoComercial(activoInfoComercial);
		...
		// Trastero Anejo.
		if(dtoAAF.getTrasteroAnejo()) {
			ActivoDistribucion trasteroAnejo = new ActivoDistribucion();
			trasteroAnejo.setInfoComercial(activoInfoComercial);
