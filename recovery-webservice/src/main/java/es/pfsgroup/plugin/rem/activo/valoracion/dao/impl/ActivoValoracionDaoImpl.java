...
public class ActivoValoracionDaoImpl extends AbstractEntityDao<ActivoValoraciones, Long> implements ActivoValoracionDao {
	...
	public Double getImporteValoracionVentaWebPorIdActivo(Long idActivo) {
		String sql = "        SELECT   " + 
				...
				"                                     (CASE DD_TPC_ID WHEN  (SELECT DD_TPC_ID FROM DD_TPC_TIPO_PRECIO WHERE DD_TPC_CODIGO = '02')  THEN DD_TPC_ID ELSE NULL END) AS APROBADO_VENTA, " + 
				...
				"                                       AND DD_TPC_ID IN (SELECT DD_TPC_ID FROM DD_TPC_TIPO_PRECIO WHERE DD_TPC_CODIGO IN ('02','04','07','13')))       " + 
				"                                 WHERE APROBADO_VENTA IS NOT NULL AND ACT_ID =     "+idActivo+"     AND VAL_BORRADO = 0 ) AS APROBADO_VENTA ,  " + 
