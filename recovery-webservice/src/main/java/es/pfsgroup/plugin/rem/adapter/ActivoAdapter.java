...
*/
	...
	public boolean saveTabActivoTransactional(WebDto dto, Long id, String tab) {
		...
		// Comprobar cambios de tipo activo
		/*Filter filtroSupervisor, filtroGestor;
		EXTDDTipoGestor tipoGestor, tipoSupervisor,tipoGestorBorrar, tipoSupervisorBorrar;
		GestorEntidadDto dtoGestor,dtoSupervisor,dtoGestorBorrar,dtoSupervisorBorrar,dtoGestorActivo,dtoSupervisorActivo;
		
		if(DDTipoActivo.COD_SUELO.equals(((DtoActivoFichaCabecera)dto).getTipoActivoCodigo())) {
			// Asignar gestor activo de suelos
			filtroGestor = genericDao.createFilter(FilterType.EQUALS, "codigo", GestorActivoApi.CODIGO_GESTOR_SUELOS);
			tipoGestor = genericDao.get(EXTDDTipoGestor.class, filtroGestor);
			
			DtoUsuario gestor = this.getComboUsuarios(tipoGestor.getId()).get(0);
			
			dtoGestor = new GestorEntidadDto();
			dtoGestor.setIdEntidad(activo.getId());
			dtoGestor.setIdUsuario(gestor.getId());
			dtoGestor.setIdTipoGestor(tipoGestor.getId());

			if(!this.insertarGestorAdicional(dtoGestor))
				logger.error("Error en ActivoAdpter: No se ha podido guardar el Gestor de Suelos");
			
			filtroSupervisor = genericDao.createFilter(FilterType.EQUALS, "codigo", GestorActivoApi.CODIGO_SUPERVISOR_SUELOS);
			tipoSupervisor = genericDao.get(EXTDDTipoGestor.class, filtroSupervisor);
			
			DtoUsuario supervisor = this.getComboUsuarios(tipoSupervisor.getId()).get(0);
			
			dtoSupervisor = new GestorEntidadDto();
			dtoSupervisor.setIdEntidad(activo.getId());
			dtoSupervisor.setIdUsuario(supervisor.getId());
			dtoSupervisor.setIdTipoGestor(tipoSupervisor.getId());

			if(!this.insertarGestorAdicional(dtoSupervisor))
				logger.error("Error en ActivoAdpter: No se ha podido guardar el Supervisor de Suelos");

		}else 
		if(gestorActivoApi.existeGestorEnActivo(activo, GestorActivoApi.CODIGO_GESTOR_SUELOS)) {
			// Si trabajos abiertos -> Asignar a Gestor de Activo
			...
			// GET Gestor y Supervisor de Edificaciones
			filtroGestor = genericDao.createFilter(FilterType.EQUALS, "codigo", GestorActivoApi.CODIGO_GESTOR_SUELOS);
			tipoGestorBorrar = genericDao.get(EXTDDTipoGestor.class, filtroGestor);
			
			DtoUsuario gestorBorrar = this.getComboUsuarios(tipoGestorBorrar.getId()).get(0);
			
			dtoGestorBorrar = new GestorEntidadDto();
			dtoGestorBorrar.setIdEntidad(activo.getId());
			dtoGestorBorrar.setIdUsuario(gestorBorrar.getId());
			dtoGestorBorrar.setIdTipoGestor(tipoGestorBorrar.getId());
			dtoGestorBorrar.setTipoEntidad(GestorEntidadDto.TIPO_ENTIDAD_ACTIVO);
			gestorActivoApi.borrarGestorAdicionalEntidad(dtoGestorBorrar); 
			
			filtroSupervisor = genericDao.createFilter(FilterType.EQUALS, "codigo", GestorActivoApi.CODIGO_SUPERVISOR_SUELOS);
			tipoSupervisorBorrar = genericDao.get(EXTDDTipoGestor.class, filtroSupervisor);
			
			DtoUsuario supervisorBorrar = this.getComboUsuarios(tipoSupervisorBorrar.getId()).get(0);
			
			dtoSupervisorBorrar = new GestorEntidadDto();
			dtoSupervisorBorrar.setIdEntidad(activo.getId());
			dtoSupervisorBorrar.setIdUsuario(supervisorBorrar.getId());
			dtoSupervisorBorrar.setIdTipoGestor(tipoGestorBorrar.getId());
			dtoSupervisorBorrar.setTipoEntidad(GestorEntidadDto.TIPO_ENTIDAD_ACTIVO);
			gestorActivoApi.borrarGestorAdicionalEntidad(dtoSupervisorBorrar); 
		}
			
		if(!Checks.esNulo(((DtoActivoFichaCabecera)dto).getTipoActivoCodigo()) && !Checks.esNulo(activo.getEstadoActivo())) {
			// Asignar gestor activo de edificación
			filtroGestor = genericDao.createFilter(FilterType.EQUALS, "codigo", GestorActivoApi.CODIGO_GESTOR_EDIFICACIONES);
			tipoGestor = genericDao.get(EXTDDTipoGestor.class, filtroGestor);
			...
			if(!this.insertarGestorAdicional(dtoGestor))
				logger.error("Error en ActivoAdpter [saveTabActivoTransactional]: No se ha podido guardar el Gestor de Edificaciones");
			
			filtroSupervisor = genericDao.createFilter(FilterType.EQUALS, "codigo", GestorActivoApi.CODIGO_SUPERVISOR_EDIFICACIONES);
			tipoSupervisor = genericDao.get(EXTDDTipoGestor.class, filtroSupervisor);
			...
			if(!this.insertarGestorAdicional(dtoSupervisor))
				logger.error("Error en ActivoAdpter[saveTabActivoTransactional]: No se ha podido guardar el Supervisor de Edificaciones");
		}else if(gestorActivoApi.existeGestorEnActivo(activo, GestorActivoApi.CODIGO_GESTOR_EDIFICACIONES)) { // Quitar gestor activo de edificacion y poner en el Historico (GAH)
				// Si trabajos abiertos -> Asignar a Gestor de Activo
				for(ActivoTrabajo activoTrabajo : activo.getActivoTrabajos()) {
					Usuario  usuGestor = activoTrabajo.getTrabajo().getResponsableTrabajo();
					String estadoTrabajo = activoTrabajo.getTrabajo().getEstado().getCodigo();
				...
				// GET Gestor y Supervisor de Edificaicones
				filtroGestor = genericDao.createFilter(FilterType.EQUALS, "codigo", GestorActivoApi.CODIGO_GESTOR_EDIFICACIONES);
				tipoGestorBorrar = genericDao.get(EXTDDTipoGestor.class, filtroGestor);
