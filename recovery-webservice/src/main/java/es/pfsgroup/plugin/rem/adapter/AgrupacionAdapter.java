...
public class AgrupacionAdapter {
		...
		try {
			...
			if (agrupacion.getTipoAgrupacion() != null) {
				...
				if (DDTipoAgrupacion.AGRUPACION_LOTE_COMERCIAL.equals(agrupacion.getTipoAgrupacion().getCodigo())
					...
					if (agrupacionTemp.getDireccion() != null) {
						BeanUtils.copyProperty(dtoAgrupacion, "direccion", agrupacionTemp.getDireccion());
					} 
				...
				// SI ES TIPO PROYECTO
					...
					if (proyectoTemp.getDireccion() != null) {
						BeanUtils.copyProperty(dtoAgrupacion, "direccion", proyectoTemp.getDireccion());
					} 
					...
					if(!Checks.esNulo(am)) {
						...
						if (am.getDireccion() != null) {
							BeanUtils.copyProperty(dtoAgrupacion, "direccion", am.getDireccion());
						} 
					}
			...
			if(agrupacion.getActivoPrincipal() != null && Checks.esNulo(dtoAgrupacion.getDireccion())) {
				dtoAgrupacion.setDireccion(agrupacion.getActivoPrincipal().getDireccionCompleta());
			}else if(activoCero != null && Checks.esNulo(dtoAgrupacion.getDireccion())) {
	...
	public Oferta createOfertaAgrupacion(DtoOfertasFilter dto) throws Exception {
		...
		try {
			...
			if (DDTiposPersona.CODIGO_TIPO_PERSONA_JURIDICA.equals(dto.getTipoPersona()) || ("02").equals(dto.getTipoPersona())) {
	...
	public String saveAgrupacion(DtoAgrupaciones dto, Long id) throws Exception {
			...
			try {
				if (!permiteCambiarDestinoComercial(pa)) {
					...
					if(!Checks.esNulo(dto.getDireccion())) {
						pa.setDireccion(dto.getDireccion());
					}
		...
		if (agrupacion.getTipoAgrupacion().getCodigo().equals(DDTipoAgrupacion.AGRUPACION_OBRA_NUEVA)) {
			...
			try {
				...
				else
				{
					...
					if(!Checks.esNulo(dto.getDireccion())) {
						obraNueva.setDireccion(dto.getDireccion());
					}
		...
		if (agrupacion.getTipoAgrupacion().getCodigo().equals(DDTipoAgrupacion.AGRUPACION_LOTE_COMERCIAL)) {
			...
			try {
				...
				if(!Checks.esNulo(dto.getDireccion())) {
					loteComercial.setDireccion(dto.getDireccion());
				}
		...
		if (agrupacion.getTipoAgrupacion().getCodigo().equals(DDTipoAgrupacion.AGRUPACION_ASISTIDA)) {
			...
			try {
				...
				if(!Checks.esNulo(dto.getDireccion())) {
					asistida.setDireccion(dto.getDireccion());
				}
		...
		if (agrupacion.getTipoAgrupacion().getCodigo().equals(DDTipoAgrupacion.AGRUPACION_RESTRINGIDA)) {
			...
			try
			{
				...
				else
				{
					...
					if(!Checks.esNulo(dto.getDireccion())) {
						restringida.setDireccion(dto.getDireccion());
					}
		...
		if (agrupacion.getTipoAgrupacion().getCodigo().equals(DDTipoAgrupacion.AGRUPACION_RESTRINGIDA_ALQUILER)) {
			...
			try
			{
				...
				else
				{
					...
					if(!Checks.esNulo(dto.getDireccion())) {
						restringidaAlquiler.setDireccion(dto.getDireccion());
					}
		...
		if (agrupacion.getTipoAgrupacion().getCodigo().equals(DDTipoAgrupacion.AGRUPACION_RESTRINGIDA_OB_REM)) {
			...
			try
			{
				...
				else
				{
					...
					if(!Checks.esNulo(dto.getDireccion())) {
						restringidaObrem.setDireccion(dto.getDireccion());
					}
	...
	private DDTipoComercializar calcularCanalDistribucionBcOfrCaixa(ActivoAgrupacion agrupacion, DDTipoOferta tipoOferta) {
		List<ActivoAgrupacionActivo> agas = agrupacion.getActivos();
		...
		for (ActivoAgrupacionActivo aga : agas) {
			ActivoCaixa activoCaixa = genericDao.get(ActivoCaixa.class, genericDao.createFilter(FilterType.EQUALS, "activo.id", aga.getActivo().getId()));
			if(activoCaixa != null){
