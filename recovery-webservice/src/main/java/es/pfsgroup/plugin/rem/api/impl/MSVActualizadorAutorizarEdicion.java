...
@Component
public class MSVActualizadorAutorizarEdicion implements MSVLiberator {
	...
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODE_FILE_BULKUPLOAD_ACTUALIZAR_AUTORIZAREDICION.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
