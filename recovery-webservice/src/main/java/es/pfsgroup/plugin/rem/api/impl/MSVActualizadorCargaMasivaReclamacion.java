...
public class MSVActualizadorCargaMasivaReclamacion extends AbstractMSVActualizador implements MSVLiberator {
	...
	protected static final Log logger = LogFactory.getLog(MSVActualizadorCargaMasivaComunicaciones.class);
	...
	public ResultadoProcesarFila procesaFila(MSVHojaExcel exc, int fila, Long prmToken)
		...
		try {
			ComunicacionGencat comunicacionGencat = comunicacionGencatDao
					.getComunicacionByActivoIdAndEstadoComunicado(exc.dameCelda(fila, POSICION_COLUMNA_NUMERO_ACTIVO));

			ReclamacionGencat reclamacionGencat = reclamacionGencatDao
					.getReclamacionByComunicacionGencatId(comunicacionGencat.getId());
			...
			reclamacionGencat
					.setFechaReclamacion(formatoFecha.parse(exc.dameCelda(fila, POSICION_COLUMNA_FECHA_RECLAMACION)));
