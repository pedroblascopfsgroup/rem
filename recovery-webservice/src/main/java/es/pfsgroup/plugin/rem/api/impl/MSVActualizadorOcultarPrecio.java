...
@Component
public class MSVActualizadorOcultarPrecio implements MSVLiberator {
	...
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODE_FILE_BULKUPLOAD_ACTUALIZAR_OCULTARPRECIO.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
