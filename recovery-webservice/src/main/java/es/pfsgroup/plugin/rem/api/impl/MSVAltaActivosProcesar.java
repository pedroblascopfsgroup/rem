...
public class MSVAltaActivosProcesar implements MSVLiberator {
	...
	/**
	 * Este método establece cada columna de la hoja excel en la variable correspondiente del DTO.
	 * 
	 * @param exc : Hoja excel.
	 * @param dtoAAF : DTO para settear las columnas de la hoja excel.
	 * @param fila : indica la fila actual para obtener datos de la hoja excel.
	 * @return Devuelve un objeto DtoAltaActivoFinanciero relleno con la información de las columnas
	 * de la excel.
	...
	private DtoAltaActivoFinanciero filaExcelToDtoAltaActivoFinanciero(MSVHojaExcel exc, DtoAltaActivoFinanciero dtoAAF, int fila) throws IllegalArgumentException, IOException, ParseException{
		...
		dtoAAF.setCodigoSociedadAcreedora(this.obtenerLongExcel(exc.dameCelda(fila, COL_NUM.CODIGO_SOCIEDAD_ACREEDORA)));
		...
		dtoAAF.setImporteDeuda(this.obtenerDoubleExcel(exc.dameCelda(fila, COL_NUM.IMPORTE_DEUDA)));
		...
		dtoAAF.setSuperficieConstruidaRegistro(this.obtenerFloatExcel(exc.dameCelda(fila, COL_NUM.SUPERFICIE_CONSTRUIDA_M2)));
		dtoAAF.setSuperficieUtilRegistro(this.obtenerFloatExcel(exc.dameCelda(fila, COL_NUM.SUPERFICIE_UTIL_M2)));
		dtoAAF.setSuperficieRepercusionEECCRegistro(this.obtenerFloatExcel(exc.dameCelda(fila, COL_NUM.SUPERFICIE_REPERCUSION_EE_CC)));
		dtoAAF.setParcelaRegistro(this.obtenerFloatExcel(exc.dameCelda(fila, COL_NUM.PARCELA)));
		dtoAAF.setEsIntegradoDivHorizontalRegistro(this.obtenerBooleanExcel(exc.dameCelda(fila, COL_NUM.ES_INTEGRADO_DIV_HORIZONTAL)));
		...
		dtoAAF.setPercentParticipacionPropiedad(this.obtenerIntegerExcel(exc.dameCelda(fila, COL_NUM.PERCENT_PROPIEDAD)));
		...
		dtoAAF.setNumDormitoriosVivienda(this.obtenerIntegerExcel(exc.dameCelda(fila, COL_NUM.VIVIENDA_NUM_DORMITORIOS)));
		...
		dtoAAF.setPrecioMinimo(this.obtenerDoubleExcel(exc.dameCelda(fila, COL_NUM.PRECIO_MINIMO)));
		dtoAAF.setPrecioVentaWeb(this.obtenerDoubleExcel(exc.dameCelda(fila, COL_NUM.PRECIO_VENTA_WEB)));
		dtoAAF.setValorTasacion(this.obtenerDoubleExcel(exc.dameCelda(fila, COL_NUM.VALOR_TASACION)));
	 ...
	 * 
	 * @param celdaExcel : valor Date de la celda excel a analizar.
	 * @return Devuelve Null si la celda está vacía, o la fecha parseada
	 * al objeto Date si contiene una fecha válida.
	 */
	 ...
	 * @return Devuelve Null si la celda está vacía, True si el String es S/SI
	 ...
	 * 
	 * @param celdaExcel : valor Integer de la celda excel a analizar.
	 * @return Devuelve Null si la celda está vacía o un objeto de tipo
	 * Integer con la conversión de la celda excel.
	 */
	 ...
	 * 
	 * @param celdaExcel : valor Double de la celda excel a analizar.
	 * @return Devuelve Null si la celda está vacía o un objeto de tipo
	 * Double con la conversión de la celda excel.
	 */
	 ...
	 * 
	 * @param celdaExcel : valor Float de la celda excel a analizar.
	 * @return Devuelve Null si la celda está vacía o un objeto de tipo
	 * Double con la conversión de la celda excel.
	 */
	 ...
	 * 
	 * @param celdaExcel : valor Long de la celda excel a analizar.
	 * @return Devuelve Null si la celda está vacía o un objeto de tipo
	 * Long con la conversión de la celda excel.
	 */
