...
public class MSVSReclamacionesPlusvaliasProcesar extends AbstractMSVActualizador implements MSVLiberator {
	...
	private final String DD_ACM_DEL = "02";
	...
	public ResultadoProcesarFila procesaFila(MSVHojaExcel exc, int fila, Long prmToken) throws Exception {
		...
		try {
			...
			Filter filtroIdActivoPlusvalia = genericDao.createFilter(FilterType.EQUALS, "activo.id", activo.getId());				
			ActivoPlusvalia activoPlusvalia = genericDao.get(ActivoPlusvalia.class, filtroIdActivoPlusvalia);
			...
			if(DD_ACM_DEL.equals(celdaAccion)) {				
				if(!Checks.esNulo(activoPlusvalia)) {
					...
					throw new IllegalArgumentException("MSVSReclamacionesPlusvaliasProcesar::ResultadoProcesarFila -> No existe registro del activoPlusvalia para eliminar");
				}
			}else{				
				if(Checks.esNulo(activoPlusvalia)) {
				...
				activoPlusvalia.getAuditoria().setBorrado(false);
				...
				if(!Checks.esNulo(celdaGastoAsociado)) {
					...
					activoPlusvalia.setGastoProveedor(genericDao.get(GastoProveedor.class, filtroGasto));
				}else {
					activoPlusvalia.setGastoProveedor(null);
				}				
				genericDao.update(ActivoPlusvalia.class, activoPlusvalia);
			}
		} catch (Exception e) {
