...
public class ClienteComercialManager extends BusinessOperationOverrider<ClienteComercialApi>
		...
		try {
			...
			if (Checks.esNulo(clienteDto.getIdClienteWebcom())) {
				...
				if (Checks.esNulo(cliente) && !Checks.esNulo(clienteDto.getIdClienteRem())) {
					listaErrores.add("No existe en REM el idClienteWebcom: " + clienteDto.getIdClienteWebcom()
							+ " pero si en Webcom con idClienteRem: " + clienteDto.getIdClienteRem());
				...
				if (alta) {
					...
					if (!Checks.esNulo(error) && !error.isEmpty()) {
						listaErrores
								.add("No se cumple la especificación de parámetros para el alta del idClienteWebcom: "
										+ clienteDto.getIdClienteWebcom() + ".Traza: " + error);
					}
				} else {
					...
					if (Checks.esNulo(cliente)) {
						listaErrores.add("No existe en REM el cliente con idClienteWebcom: "
								+ clienteDto.getIdClienteWebcom() + " idClienteRem: " + clienteDto.getIdClienteRem());
					}
				}

				if (!Checks.esNulo(clienteDto.getIdUsuarioRemAccion())) {
					Filter filtroUser = genericDao.createFilter(FilterType.EQUALS, "id",
							clienteDto.getIdUsuarioRemAccion());
					Usuario user = (Usuario) genericDao.get(Usuario.class, filtroUser);
					if (Checks.esNulo(user)) {
						listaErrores.add("No existe el usuario en REM especificado en el campo idUsuarioRemAccion: "
								+ clienteDto.getIdUsuarioRemAccion());
					}
				}
				if (!Checks.esNulo(clienteDto.getCodTipoDocumento())) {
					Filter filtrotipodoc = genericDao.createFilter(FilterType.EQUALS, "codigo",
							clienteDto.getCodTipoDocumento());
					DDTipoDocumento tipoDoc = (DDTipoDocumento) genericDao.get(DDTipoDocumento.class, filtrotipodoc);
					if (Checks.esNulo(tipoDoc)) {
						listaErrores.add("No existe el tipo de documento especificado en el campo codTipoDocumento: "
								+ clienteDto.getCodTipoDocumento());
					}
				}

				if (!Checks.esNulo(clienteDto.getCodTipoDocumentoRepresentante())) {
					Filter filtrotipodocrep = genericDao.createFilter(FilterType.EQUALS, "codigo",
					...
					if (Checks.esNulo(tipoDocRep)) {
						listaErrores
								.add("No existe el tipo de documento en REM especificado en el campo codTipoDocumentoRepresentante: "
										+ clienteDto.getCodTipoDocumentoRepresentante());
					}
				}
				/*
				 * if(!Checks.esNulo(clienteDto.getCodTipoPrescriptor())){
				 * Filter filtrotipoPres =
				 * genericDao.createFilter(FilterType.EQUALS, "codigo",
				 * clienteDto.getCodTipoPrescriptor()); DDTiposColaborador
				 * tipoCol= (DDTiposColaborador)
				 * genericDao.get(DDTiposColaborador.class, filtrotipoPres);
				 * if(Checks.esNulo(tipoCol)){ listaErrores.add(
				 * "No existe el tipo de prescriptor en REM especificado en el campo codTipoPrescriptor: "
				 * + clienteDto.getCodTipoPrescriptor()); } }
				 */
				if (!Checks.esNulo(clienteDto.getIdProveedorRemPrescriptor())) {
					Filter filtroPres = genericDao.createFilter(FilterType.EQUALS, "id",
							clienteDto.getIdProveedorRemPrescriptor());
					ActivoProveedor prescriptor = (ActivoProveedor) genericDao.get(ActivoProveedor.class, filtroPres);
					if (Checks.esNulo(prescriptor)) {
						listaErrores
								.add("No existe el prescriptor en REM especificado en el campo idProveedorRemPrescriptor: "
										+ clienteDto.getIdProveedorRemPrescriptor());
					}
				}
				if (!Checks.esNulo(clienteDto.getIdProveedorRemResponsable())) {
					Filter filtroApi = genericDao.createFilter(FilterType.EQUALS, "id",
							clienteDto.getIdProveedorRemResponsable());
					ActivoProveedor apiResp = (ActivoProveedor) genericDao.get(ActivoProveedor.class, filtroApi);
					if (Checks.esNulo(apiResp)) {
						listaErrores
								.add("No existe el api responsable en REM especificado en el campo idProveedorRemResponsable: "
										+ clienteDto.getIdProveedorRemResponsable());
					}
				}
				if (!Checks.esNulo(clienteDto.getCodTipoVia())) {
					...
					if (Checks.esNulo(tipovia)) {
						listaErrores.add("No existe el tipo de vía en REM especificado en el campo codTipoVia: "
								+ clienteDto.getCodTipoVia());
					}
				}
				if (!Checks.esNulo(clienteDto.getCodMunicipio())) {
					Filter filtroMun = genericDao.createFilter(FilterType.EQUALS, "codigo",
							clienteDto.getCodMunicipio());
					Localidad localidad = (Localidad) genericDao.get(Localidad.class, filtroMun);
					if (Checks.esNulo(localidad)) {
						listaErrores.add("No existe el municipio en REM especificado en el campo codMunicipio: "
								+ clienteDto.getCodMunicipio());
					}
				}
				if (!Checks.esNulo(clienteDto.getCodProvincia())) {
					Filter filtroProv = genericDao.createFilter(FilterType.EQUALS, "codigo",
							clienteDto.getCodProvincia());
					DDProvincia provincia = (DDProvincia) genericDao.get(DDProvincia.class, filtroProv);
					if (Checks.esNulo(provincia)) {
						listaErrores.add("No existe la provincia en REM especificado en el campo codProvincia: "
								+ clienteDto.getCodProvincia());
					}
				}
				if (!Checks.esNulo(clienteDto.getCodPedania())) {
					...
					if (Checks.esNulo(pedania)) {
						listaErrores.add("No existe la pedania en REM especificada en el campo codPedania: "
								+ clienteDto.getCodPedania());
					}
				}
			}
	...
	public void saveClienteComercial(ClienteDto clienteDto) {
		...
		try {
			...
			/*
			 * if(!Checks.esNulo(clienteDto.getCodTipoPrescriptor())){
			 * DDTiposColaborador tipoCol= (DDTiposColaborador)
			 * genericDao.get(DDTiposColaborador.class,
			 * genericDao.createFilter(FilterType.EQUALS, "codigo",
			 * clienteDto.getCodTipoPrescriptor()));
			 * if(!Checks.esNulo(tipoCol)){ cliente.setTipoColaborador(tipoCol);
			 * } }
			 */
			if (!Checks.esNulo(clienteDto.getIdProveedorRemPrescriptor())) {
	...
	public List<String> updateClienteComercial(ClienteComercial cliente, ClienteDto clienteDto, Object jsonFields) {
		...
		try {
			...
			if (errorsList.isEmpty()) {
				 ...
				 * cliente.setTipoColaborador(tipoCol); } }else{
			...
			e.printStackTrace();
			errorsList.add("Ha ocurrido un error en base de datos al actualizar el cliente webcom: "
					+ clienteDto.getIdClienteWebcom() + ". Traza: " + e.getMessage());
			return errorsList;
		}
