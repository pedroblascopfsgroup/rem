...
public class ExpedienteComercialManager extends BusinessOperationOverrider<ExpedienteComercialApi>
	...
	private static final String PESTANYA_FIANZA = "fianza";
	...
	public Object getTabExpediente(Long id, String tab) {
		...
		} else if (PESTANYA_FIANZA.equals(tab)) {
			dto = getTabFianza(expediente);
		}
	...
	private DtoFichaExpediente expedienteToDtoFichaExpediente(ExpedienteComercial expediente) {
		...
		if (!Checks.esNulo(expediente)) {
			...
			Fianzas fianza = genericDao.get(Fianzas.class, genericDao.createFilter(FilterType.EQUALS,"oferta.id",expediente.getOferta().getId()));
			Boolean ecoConFianza = false;
			if(fianza != null) {
				ecoConFianza = true;
			}
			dto.setEcoConFianza(ecoConFianza);
	...
	public DtoCompradorLLamadaBC saveFichaComprador(VBusquedaDatosCompradorExpediente dto) {
		...
		if (!Checks.esNulo(comprador)) {
			...
			documentoRteModificado = dto.getNumDocumentoRte() != null ? !dto.getNumDocumentoRte().equals(compradorExpediente.getDocumentoRepresentante()) : false;
			...
			if(iapComprador != null) {
				...
				iapComprador.setSociedad(dto.getSociedad());
				iapComprador.setOficinaTrabajo(dto.getOficinaTrabajo());
			}
...
//							}
	...
	public DtoCompradorLLamadaBC createComprador(VBusquedaDatosCompradorExpediente dto, Long idExpediente) {
		...
		if (!Checks.esNulo(compradorBusqueda)) {
			...
			assignIAPCompradorRepresentante(compradorExpediente,expediente.getId(),compradorBusqueda,expediente.getOferta(), false);
			...
			try {
				...
				assignIAPCompradorRepresentante(compradorExpediente,expediente.getId(),comprador,expediente.getOferta(), false);
...
//			}
	...
	@Override
	public HistoricoSancionesBc dtoRespuestaToHistoricoSancionesBc(DtoRespuestaBCGenerica dto, ExpedienteComercial eco) {
		HistoricoSancionesBc historico = new HistoricoSancionesBc();
		
		DDComiteBc comiteBC = genericDao.get(DDComiteBc.class, genericDao.createFilter(FilterType.EQUALS,"codigo", dto.getComiteBc()));
		DDApruebaDeniega resultado = genericDao.get(DDApruebaDeniega.class, genericDao.createFilter(FilterType.EQUALS,"codigo", dto.getRespuestaBC()));
		
		historico.setExpedienteComercial(eco);
		historico.setComiteBc(comiteBC);
		historico.setResultado(resultado);
		historico.setObserbacionesBc(dto.getObservacionesBC());
		...
		try {
			...
			if(!Checks.esNulo(oferta)) {
				...
				if(!Checks.esNulo(ofrCaixa)) {
					...
					if(ofrCaixa.getFinalidadOperacion() != null) {
						dtoOfertaCaixaPbc.setFinalidadOperacion(ofrCaixa.getFinalidadOperacion().getDescripcion());
					}
					if(ofrCaixa.getProcedenciaFondosPropios() != null) {
	...
	private DtoTabFianza getTabFianza(ExpedienteComercial expediente) {
		DtoTabFianza dto = new DtoTabFianza();
		Fianzas fianza = null;
		Oferta oferta = expediente.getOferta();
		
		if(oferta != null) {
			fianza = genericDao.get(Fianzas.class, genericDao.createFilter(FilterType.EQUALS,"oferta.id",expediente.getOferta().getId()));
		}
		
		if(fianza != null) {
			dto.setId(fianza.getId());
			dto.setIdFianza(fianza.getId());
			dto.setAgendacionIngreso(fianza.getFechaAgendacionIngreso());
			dto.setImporteFianza(fianza.getImporte());
	...
	@Override
	@Transactional
	public Boolean saveTabFianza(DtoTabFianza dto, Long id) {
		Fianzas fianza = genericDao.get(Fianzas.class, genericDao.createFilter(FilterType.EQUALS,"id" ,dto.getId()));
		
		if(fianza == null) {
			fianza = new Fianzas();
		}else {
			if(dto.getAgendacionIngreso() != null) {
				fianza.setFechaAgendacionIngreso(dto.getAgendacionIngreso());
			}
			if(dto.getImporteFianza() != null) {
				fianza.setImporte(dto.getImporteFianza());
			}
			if(dto.getFechaIngreso() != null) {
				fianza.setFechaIngreso(dto.getFechaIngreso());
			}
			if(dto.getIbanDevolucion() != null) {
				fianza.setIbanDevolucion(dto.getIbanDevolucion());
			}
		}
	...
	@Override
	public boolean checkEstadoBcClausuladoNoComerciable(TareaExterna tareaExterna) {
		ExpedienteComercial expedienteComercial = tareaExternaToExpedienteComercial(tareaExterna);
