...
public class GastoProveedorManager implements GastoProveedorApi {
	...
	public DtoPage getListGastos(DtoGastosFilter dtoGastosFilter) {
		...
		// Comprobar si el usuario es externo y de tipo proveedor y, en tal caso,
		// seteamos proveedores contacto del
		// usuario logado para filtrar los gastos en los que esté como emisor
		// Ademas si es un tipo de gestoria concreto, se filtrará los gastos que le
		// pertenezcan como gestoria.
		if (gestorActivoDao.isUsuarioGestorExternoProveedor(usuarioLogado.getId())) {
			Boolean isGestoria = genericAdapter.tienePerfil(COD_PEF_GESTORIA_ADMINISTRACION, usuarioLogado)
					...
					|| genericAdapter.tienePerfil(COD_PEF_GESTORIA_POSTVENTA, usuarioLogado)
					|| genericAdapter.tienePerfil(COD_PEF_USUARIO_CERTIFICADOR, usuarioLogado);
			return gastoDao.getListGastosFilteredByProveedorContactoAndGestoria(dtoGastosFilter, usuarioLogado.getId(),
					isGestoria, false);
		}
	...
	private DtoFichaGastoProveedor gastoToDtoFichaGasto(GastoProveedor gasto) {
		...
		String[] subtiposGasto = new String[] { COD_HONORARIOS_GESTION_ACTIVOS, COD_REGISTRO, COD_NOTARIA, COD_ABOGADO,
				COD_PROCURADOR, COD_OTROS_SERVICIOS_JURIDICOS, COD_ADMINISTRADOR_COMUNIDAD_PROPIETARIOS, COD_ASESORIA,
				COD_TECNICO, COD_TASACION, COD_OTROS, COD_GESTION_DE_SUELO, COD_ABOGADO_OCUPACIONAL,
				COD_ABOGADO_ASUNTOS_GENERALES, COD_ABOGADO_ASISTENCIA_JURIDiCA };
		...
		if (!Checks.esNulo(gasto)) {
			...
			if (gasto.getGastoLineaDetalleList() != null && !gasto.getGastoLineaDetalleList().isEmpty()) {
				for (GastoLineaDetalle gastoLinea : gasto.getGastoLineaDetalleList()) {
					Filter filter = genericDao.createFilter(FilterType.EQUALS, "gastoLineaDetalle.id",
							gastoLinea.getId());
					Filter filterAct = genericDao.createFilter(FilterType.EQUALS, "entidadGasto.codigo",
							DDEntidadGasto.CODIGO_ACTIVO);
					List<GastoLineaDetalleEntidad> gastoLineaEntidadList = genericDao
							.getList(GastoLineaDetalleEntidad.class, filter, filterAct);
					if (gastoLineaEntidadList == null || gastoLineaEntidadList.isEmpty()) {
						...
						for (GastoLineaDetalleEntidad gastoLineaDetalleEntidad : gastoLineaEntidadList) {
							...
							ConfiguracionSuplidos config = genericDao.get(ConfiguracionSuplidos.class,
									genericDao.createFilter(FilterType.EQUALS, "cartera.codigo",
											gasto.getCartera().getCodigo()),
									genericDao.createFilter(FilterType.EQUALS, "subCartera.codigo",
											activo.getSubcartera().getCodigo()),
									genericDao.createFilter(FilterType.EQUALS, "tipoGasto.codigo",
											gasto.getTipoGasto().getCodigo()),
									genericDao.createFilter(FilterType.EQUALS, "subtipoGasto.codigo",
											gastoLinea.getSubtipoGasto().getCodigo()));
							if (config == null) {
								dto.setVisibleSuplidos(false);
								break;
							}
			...
			if (!Checks.esNulo(gasto.getProveedor())) {
				...
				dto.setEstadoEmisor(Checks.esNulo(gasto.getProveedor().getEstadoProveedor()) ? null
						: gasto.getProveedor().getEstadoProveedor().getDescripcion());
			}
			...
			if (!Checks.esNulo(gasto.getGastoGestion())
				...
				dto.setAutorizado(DDEstadoAutorizacionHaya.CODIGO_AUTORIZADO
						.equals(gasto.getGastoGestion().getEstadoAutorizacionHaya().getCodigo()));
			}
			if (!Checks.esNulo(gasto.getGastoGestion())
					&& !Checks.esNulo(gasto.getGastoGestion().getEstadoAutorizacionHaya())) {
				dto.setRechazado(DDEstadoAutorizacionHaya.CODIGO_RECHAZADO
						.equals(gasto.getGastoGestion().getEstadoAutorizacionHaya().getCodigo()));
			}
			dto.setAsignadoATrabajos(false);
			if (gasto.getGastoLineaDetalleList() != null && !gasto.getGastoLineaDetalleList().isEmpty()) {
				for (GastoLineaDetalle gastoLinea : gasto.getGastoLineaDetalleList()) {
					if (gastoLinea.getGastoLineaTrabajoList() != null
							&& !gastoLinea.getGastoLineaTrabajoList().isEmpty()) {
			...
			if (!Checks.estaVacio(gasto.getGastoLineaDetalleList())) {
				for (GastoLineaDetalle gastoLinea : gasto.getGastoLineaDetalleList()) {
					if (!Checks.esNulo(gastoLinea.getGastoLineaEntidadList())) {
						for (GastoLineaDetalleEntidad gastoLineaDetalleEntidad : gastoLinea
			...
			if (!Checks.esNulo(gasto.getGastoGestion())
					&& (!Checks.esNulo(gasto.getGastoGestion().getFechaEnvioGestoria())
							|| !Checks.esNulo(gasto.getGastoGestion().getFechaEnvioPropietario()))) {
				dto.setEnviado(true);
			} else {
			...
			/*
			 * Double gastoTotal = 0.0; List<GastoPrinex> listGastoPrinex = new
			 * ArrayList<GastoPrinex>(); Filter filtro3 =
			 * genericDao.createFilter(FilterType.EQUALS, "idGasto",gasto.getId());
			 * listGastoPrinex = genericDao.getList(GastoPrinex.class, filtro3);
			 * 
			 * if(!Checks.estaVacio(listGastoPrinex)) { for (GastoPrinex gastoPrinexList :
			 * listGastoPrinex) {
			 * 
			 * if(!Checks.esNulo(gastoPrinexList.getIdActivo()) &&
			 * !Checks.esNulo(gastoPrinexList.getImporteGasto())) {
			 ...
			 * }else{ dto.setImporteTotal(gastoTotal); } }
			 */
			...
			if (!Checks.esNulo(gasto.getNumeroPrimerGastoSerie())) {
				Filter filtroNumGs = genericDao.createFilter(FilterType.EQUALS, "id",
						gasto.getNumeroPrimerGastoSerie());
			...
			if (gasto.getEstadoGasto() != null && (DDEstadoGasto.INCOMPLETO.equals(gasto.getEstadoGasto().getCodigo())
			...
			if (!gastoLineaDetalleList.isEmpty()) {
				...
				for (GastoLineaDetalle gastoLineaDetalle : gastoLineaDetalleList) {
					if (gastoLineaDetalle.getGastoLineaTrabajoList() != null
							&& !gastoLineaDetalle.getGastoLineaTrabajoList().isEmpty()) {
						tieneTrabajos = true;
						break;
					}
				}
			...
			if (gasto.getGastoLineaDetalleList() != null && !gasto.getGastoLineaDetalleList().isEmpty()) {
				for (GastoLineaDetalle linea : gasto.getGastoLineaDetalleList()) {
					if (DDSubtipoGasto.IBI_RUSTICA.equals(linea.getSubtipoGasto().getCodigo()) 
							|| DDSubtipoGasto.IBI_URBANA.equals(linea.getSubtipoGasto().getCodigo())) {
						esIBI = true;
					}
				}
			}
	...
	public GastoProveedor createGastoProveedor(DtoFichaGastoProveedor dto) {
		...
		Filter filtroEjercicio = genericDao.createFilter(FilterType.EQUALS, "anyo",
				String.valueOf(gastoProveedor.getFechaEmision().getYear() + 1900));
		...
		if (dto.getSuplidosVinculadosCod() != null) {
			DDSinSiNo suplidoVinculado = genericDao.get(DDSinSiNo.class,
					genericDao.createFilter(FilterType.EQUALS, "codigo", dto.getSuplidosVinculadosCod()));
		...
		if (dto.getFacturaPrincipalSuplido() != null) {
			...
			GastoProveedor gastoPrincipal = genericDao.get(GastoProveedor.class,
					genericDao.createFilter(FilterType.EQUALS, "referenciaEmisor", dto.getFacturaPrincipalSuplido()));
		...
		if (gestorDocumentalAdapterApi.modoRestClientActivado()) {
			...
			try {
				...
				logger.debug("GESTOR DOCUMENTAL [ crearGasto para " + gastoProveedor.getNumGastoHaya()
						+ "]: ID EXPEDIENTE RECIBIDO " + idExpediente);
		...
		if (listaNumerosGasto != null && !listaNumerosGasto.isEmpty()) {
			for (String numGasto : listaNumerosGasto) {
				...
				Filter filtraGastos = genericDao.createFilter(FilterType.EQUALS, "numGastoHaya",
						Long.valueOf(numGasto));
				...
				if (gastoRefacturableHaya != null) {
					...
					if (gastoLineaDetalleList != null && !gastoLineaDetalleList.isEmpty()) {
						tipoGastoImpuestoHash = gastoLineaDetalleApi.devolverNumeroLineas(gastoLineaDetalleList,
								tipoGastoImpuestoHash);
					}
			...
			if (!listaNumerosGasto.isEmpty() && !tipoGastoImpuestoList.isEmpty()) {
				gastoLineaDetalleApi.createLineasDetalleGastosRefacturados(tipoGastoImpuestoList, listaNumerosGasto,
						gastoProveedor);
			}
	...
	private GastoProveedor dtoToGastoProveedor(DtoFichaGastoProveedor dto, GastoProveedor gastoProveedor) {
		...
		if (!Checks.esNulo(dto.getDestinatarioGastoCodigo())) {
			DDDestinatarioGasto destinatarioGasto = (DDDestinatarioGasto) utilDiccionarioApi
					.dameValorDiccionarioByCod(DDDestinatarioGasto.class, dto.getDestinatarioGastoCodigo());
			gastoProveedor.setDestinatarioGasto(destinatarioGasto);
		}

		if (!Checks.esNulo(dto.getTipoGastoCodigo())) {
			DDTipoGasto tipoGasto = (DDTipoGasto) utilDiccionarioApi.dameValorDiccionarioByCod(DDTipoGasto.class,
					dto.getTipoGastoCodigo());
...
//		}
	...
	public boolean saveGastosProveedor(DtoFichaGastoProveedor dto, Long id) {
		...
		if (gastoProveedor != null && gastoProveedor.getSuplidosVinculados() != null
				...
				&& (dto.getReferenciaEmisor() != null
						&& !dto.getReferenciaEmisor().equals(gastoProveedor.getReferenciaEmisor()))) {
		...
		if (!Checks.esNulo(dto.getCodigoProveedorRem())) {
			if (!esGastoHijo) {
				Filter filtroCodigoEmisorRem = genericDao.createFilter(FilterType.EQUALS, "codigoProveedorRem",
				...
				throw new JsonViewerException(
						"No se puede cambiar el emisor, este gasto refacturable está incluido en el gasto: "
								+ numGastoPadre);
			}
		}
		...
		if (!Checks.esNulo(dto.getBuscadorNifPropietario())) {
			if (!esGastoHijo) {
				Filter filtroNifPropietario = genericDao.createFilter(FilterType.EQUALS, "docIdentificativo",
						dto.getBuscadorNifPropietario());
				ActivoPropietario propietario = genericDao.get(ActivoPropietario.class, filtroNifPropietario);
				gastoProveedor.setPropietario(propietario);
			} else {
				throw new JsonViewerException(
						"No se puede cambiar el propietario, este gasto refacturable está incluido en el gasto: "
								+ numGastoPadre);
			}
		}
		...
		if (!Checks.esNulo(dto.getDestinatario())) {
			DDDestinatarioGasto destinatario = (DDDestinatarioGasto) utilDiccionarioApi
					.dameValorDiccionarioByCod(DDDestinatarioGasto.class, dto.getDestinatario());
			gastoProveedor.setDestinatarioGasto(destinatario);
		}
		if (!Checks.esNulo(dto.getPeriodicidad())) {
			DDTipoPeriocidad periodicidad = (DDTipoPeriocidad) utilDiccionarioApi
					.dameValorDiccionarioByCod(DDTipoPeriocidad.class, dto.getPeriodicidad());
			gastoProveedor.setTipoPeriocidad(periodicidad);
		}
		if (!Checks.esNulo(dto.getTipoOperacionCodigo())) {
			DDTipoOperacionGasto tipoOperacion = (DDTipoOperacionGasto) utilDiccionarioApi
					.dameValorDiccionarioByCod(DDTipoOperacionGasto.class, dto.getTipoOperacionCodigo());
		...
		if (!Checks.esNulo(dto.getNumGastoAbonado())) {
			...
			Filter filtroGastoAbonado = genericDao.createFilter(FilterType.EQUALS, "numGastoHaya",
					dto.getNumGastoAbonado());
			...
			if (!Checks.estaVacio(listaGastos)) {
				...
				if (!Checks.esNulo(gasto.getProveedor()) && !Checks.esNulo(gastoProveedor.getProveedor())) {
					if (gasto.getProveedor().getCodigoProveedorRem()
							.equals(gastoProveedor.getProveedor().getCodigoProveedorRem())
		...
		if (!Checks.esNulo(dto.getDestinatario()) && !DDDestinatarioGasto.CODIGO_HAYA.equals(dto.getDestinatario())) {
			if (!esGastoHijo) {
				...
				throw new JsonViewerException(
						"No se puede cambiar el destinatario, este gasto refacturable está incluido en el gasto: "
								+ numGastoPadre);
			}
		}
		...
		if (dto.getFacturaPrincipalSuplido() != null && !dto.getFacturaPrincipalSuplido().isEmpty()
			...
			Filter filtroSuplidos = genericDao.createFilter(FilterType.EQUALS, "suplidosVinculados.codigo",
					DDSinSiNo.CODIGO_SI);
			GastoProveedor gastoPrincipal = genericDao.get(GastoProveedor.class,
					genericDao.createFilter(FilterType.EQUALS, "referenciaEmisor", dto.getFacturaPrincipalSuplido()),
					filtroSuplidos);
			...
			if (esGastoAutorizado(gastoPrincipal)) {
				throw new JsonViewerException("No se puede añadir como suplido al gasto "
						+ gastoPrincipal.getNumGastoHaya() + ", ya que este se encuentra autorizado.");
			}
	...
	private void updateEjercicio(GastoProveedor gasto) {
		...
		if (Checks.esNulo(gastoInfoContabilidad.getFechaDevengoEspecial())) {
			if (!Checks.esNulo(gasto.getFechaEmision())) {
				...
				Filter filtroEjercicio = genericDao.createFilter(FilterType.EQUALS, "anyo",
						sdf.format(gasto.getFechaEmision()));
				Ejercicio ejercicio = genericDao.get(Ejercicio.class, filtroEjercicio);
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy");
			Filter filtroEjercicio = genericDao.createFilter(FilterType.EQUALS, "anyo",
					sdf.format(gasto.getGastoInfoContabilidad().getFechaDevengoEspecial()));
			Ejercicio ejercicio = genericDao.get(Ejercicio.class, filtroEjercicio);
			gastoInfoContabilidad.setEjercicio(ejercicio);
		}
	...
	public boolean existeGasto(DtoFichaGastoProveedor dto) {
		...
		Filter filtroReferencia = genericDao.createFilter(FilterType.EQUALS, "referenciaEmisor",
				dto.getReferenciaEmisor());
		...
		Filter filtroDestinatario = genericDao.createFilter(FilterType.EQUALS, "destinatarioGasto.codigo",
				dto.getDestinatarioGastoCodigo());

		List<GastoProveedor> lista = genericDao.getList(GastoProveedor.class, filtroReferencia, filtroTipo,
				filtroEmisor, filtroFechaEmision, filtroDestinatario);
	...
	public ActivoProveedor searchProveedorCodigo(String codigoUnicoProveedor) {
		...
		Filter filtro = genericDao.createFilter(FilterType.EQUALS, "codigoProveedorRem",
				Long.parseLong(codigoUnicoProveedor));
	...
	public boolean searchActivoCarteraAndGastoPrinex(String numGastoHaya) {
		...
		if (!Checks.esNulo(gastoProveedor)) {
			...
			Filter filtro2 = genericDao.createFilter(FilterType.EQUALS, "gastoLineaDetalle.gastoProveedor.id",
					idGastoProveedor);
				...
				for (GastoLineaDetalleEntidad gastoProveedorObject : gastoLineaDetalleEntidad) {
					if (!Checks.esNulo(gastoProveedorObject)) {
						Filter filtroId = genericDao.createFilter(FilterType.EQUALS, "id",
								gastoProveedorObject.getEntidad());
		...
		if (!Checks.esNulo(detalleGasto)) {
			if (!Checks.esNulo(detalleGasto.getGastoProveedor())) {
				...
				if (!Checks.estaVacio(listGastoPrinex)) {
					...
					String result = rawDao.getExecuteSQL(
							"SELECT SUM(GPL_IMPORTE_GASTO) FROM GPL_GASTOS_PRINEX_LBK WHERE GPV_ID =  :idGasto ");
					...
					for (GastoPrinex gastoPrinex : listGastoPrinex) {
						if (!Checks.esNulo(gastoPrinex.getIdActivo())) {
							...
							Filter filtro2 = genericDao.createFilter(FilterType.EQUALS,
									"gastoLineaDetalle.gastoProveedor.id", idGasto);
							Filter filtro4 = genericDao.createFilter(FilterType.EQUALS, "activo.id",
									gastoPrinex.getIdActivo());
						...
						if (!Checks.esNulo(gastoPrinex.getDiario1())) {
							...
							if ("1".equals(gastoPrinex.getDiario1()) || "20".equals(gastoPrinex.getDiario1())
									|| "2".equals(gastoPrinex.getDiario1())) {
				...
				if (!Checks.esNulo(gasto)) {
					...
					if (DDEstadoGasto.RECHAZADO_PROPIETARIO.equals(gasto.getEstadoGasto().getCodigo())) {
						...
						gasto.getGastoGestion()
								.setEstadoAutorizacionPropietario(genericDao.get(DDEstadoAutorizacionPropietario.class,
	...
	private DtoDetalleEconomicoGasto detalleEconomicoToDtoDetalleEconomico(GastoProveedor gasto) {
		...
		if (!Checks.esNulo(detalleGasto)) {
			...
			if (Checks.esNulo(detalleGasto.getGastoRefacturable())
					|| (!Checks.esNulo(detalleGasto.getGastoRefacturable()) && detalleGasto.getGastoRefacturable())) {
		...
		try {
			if (!Checks.esNulo(detalleGasto)) {
				...
				if (!Checks.esNulo(dto.getDestinatariosPagoCodigo())) {
					DDDestinatarioPago destinatarioPago = (DDDestinatarioPago) utilDiccionarioApi
							.dameValorDiccionarioByCod(DDDestinatarioPago.class, dto.getDestinatariosPagoCodigo());
					detalleGasto.setDestinatariosPago(destinatarioPago);
				}
				if (!Checks.esNulo(dto.getTipoPagadorCodigo())) {
					DDTipoPagador tipoPagador = (DDTipoPagador) utilDiccionarioApi
							.dameValorDiccionarioByCod(DDTipoPagador.class, dto.getTipoPagadorCodigo());
				...
				if (dto.getRetencionGarantiaAplica() != null) {
					if (dto.getRetencionGarantiaAplica()) {
						DDTipoRetencion tipoRetencion = (DDTipoRetencion) utilDiccionarioApi
								.dameValorDiccionarioByCod(DDTipoRetencion.class, dto.getTipoRetencionCodigo());
				...
				if ((dto.getRetencionGarantiaAplica() != null || dto.getTipoRetencionCodigo() != null
						|| dto.getIrpfTipoImpositivoRetG() != null)
						&& (gasto != null && gasto.getPropietario() != null
								&& gasto.getPropietario().getCartera() != null && DDCartera.CODIGO_CARTERA_LIBERBANK
										.equalsIgnoreCase(gasto.getPropietario().getCartera().getCodigo()))) {
	...
	public boolean createGastoActivo(Long idGasto, Long numActivo, Long numAgrupacion) {
		...
		if (!Checks.esNulo(numActivo)) {
			...
			if (activoSareb != null && activoSareb.getReoContabilizado() != null
					&& activoSareb.getReoContabilizado().getCodigo().equals(DDSinSiNo.CODIGO_NO)) {
			...
			if (!Checks.esNulo(act)) {
				Filter filtroG = genericDao.createFilter(FilterType.EQUALS, "gastoLineaDetalle.gastoProveedor.id",
						idGasto);
				Filter filtroE = genericDao.createFilter(FilterType.EQUALS, "entidad", act.getId());
				Filter filtroD = genericDao.createFilter(FilterType.EQUALS, "entidadGasto.codigo",
						DDEntidadGasto.CODIGO_ACTIVO);
	...
	/**
	 * Este método recibe el activo o la agrupación de activos que se quiere asociar
	 * con el gasto y valida que no haya ningún activo de la cartera de BANKIA y de
	 * las subcarteras de Solvia, Sareb y SAREB Pre-IBERO que tenga una fecha de
	 * traspaso posterior a la fecha de devengo del gasto.
	 * 
	 ...
	 * @param ActivoAgrupacion: La agrupación de activos que se quiere asociar con
	 *                          el gasto
	 */
	public boolean fechaDevengoPosteriorFechaTraspaso(Long idGasto, Long idActivo, Long idAgrupacion) {
		...
		if (idActivo != -1) {
			...
			ActivoAgrupacion agrupacion = activoAgrupacionApi
					.get(activoAgrupacionApi.getAgrupacionIdByNumAgrupRem(idAgrupacion));
	...
	/**
	 * Este método recibe un listado de asociaciones de activos a un gasto y
	 * recalcula el porcentaje de participación en el gasto para cada activo de
	 * forma equitativa.
	 * 
	 * @param gastosActivosList: listado con las asociaciones de los activos y el
	 *                           gasto.
	 */
	private void calculaPorcentajeEquitativoGastoActivos(List<GastoLineaDetalleEntidad> gastosActivosList) {
		...
		Filter filtro = genericDao.createFilter(FilterType.EQUALS, "idGasto",
				gastosActivosList.get(0).getGastoLineaDetalle().getGastoProveedor().getId());
		...
		if (!Checks.estaVacio(gastoPrinexList)) {
			...
			for (GastoLineaDetalleEntidad gastoProveedorItem : gastosActivosList) {
				...
				Filter filtro3 = genericDao.createFilter(FilterType.EQUALS, "idActivo",
						gastoProveedorItem.getEntidad());
	...
	public boolean deleteGastoProveedor(Long id) {
		...
		// TODO en el caso de utilizarse, se deberá tener en cuenta si se borran todas
		// las
	...
	public DtoInfoContabilidadGasto infoContabilidadToDtoInfoContabilidad(GastoProveedor gasto) {
		...
		if (!Checks.esNulo(gasto)) {
			...
			if (cartera != null && DDCartera.CODIGO_CARTERA_LIBERBANK.equals(cartera.getCodigo())) {
				...
				ErrorDiariosLbk errorDiariosLbk = genericDao.get(ErrorDiariosLbk.class,
						genericDao.createFilter(FilterType.EQUALS, "id", gasto.getId()));
			...
			if (!Checks.esNulo(contabilidadGasto)) {
				if (!Checks.esNulo(contabilidadGasto.getEjercicio())) {
					...
					if (!Checks.esNulo(contabilidadGasto.getFechaDevengoEspecial())) {
						Filter filtroFecha = genericDao.createFilter(FilterType.EQUALS, "anyo",
								String.valueOf(contabilidadGasto.getFechaDevengoEspecial().getYear() + 1900));
				...
				if (!Checks.esNulo(contabilidadGasto.getTipoPeriocidadEspecial())) {
					dto.setPeriodicidadEspecialDescripcion(
							contabilidadGasto.getTipoPeriocidadEspecial().getDescripcion());
				}
		...
		try {
			...
			if (!Checks.esNulo(contabilidadGasto)) {
				...
				if (!Checks.esNulo(dtoContabilidadGasto.getEjercicioImputaGasto())) {
					Filter filtroEjercicio = genericDao.createFilter(FilterType.EQUALS, "id",
							dtoContabilidadGasto.getEjercicioImputaGasto());
					Ejercicio ejercicio = genericDao.get(Ejercicio.class, filtroEjercicio);
				...
				if (!Checks.esNulo(dtoContabilidadGasto.getComboActivable())) {
					Filter filtro = genericDao.createFilter(FilterType.EQUALS, "codigo",
							dtoContabilidadGasto.getComboActivable());
				...
				if (dtoContabilidadGasto.getIdSubpartidaPresupuestaria() != null) {
					Filter filtroSubpartidaPresupuestaria = genericDao.createFilter(FilterType.EQUALS, "id",
							dtoContabilidadGasto.getIdSubpartidaPresupuestaria());
					ConfiguracionSubpartidasPresupuestarias cps = genericDao
							.get(ConfiguracionSubpartidasPresupuestarias.class, filtroSubpartidaPresupuestaria);
				...
				if (dtoContabilidadGasto.getTipoComisionadoHreCodigo() != null) {
					Filter filtro = genericDao.createFilter(FilterType.EQUALS, "codigo",
							dtoContabilidadGasto.getTipoComisionadoHreCodigo());
	...
	private boolean hayCambiosGasto(Object dtoIni, Object dtoFin, GastoProveedor gasto) {
		String[] camposMinimos = new String[] { "numGastoHaya", "numGastoGestoria", "referenciaEmisor",
				"tipoGastoCodigo", "subtipoGastoCodigo", "idEmisor", "destinatario", "propietario", "fechaEmision",
				"periodicidad", "concepto", "tipoOperacionCodigo", "importeTotal", "impuestoIndirectoTipoCodigo",
				"asignadoAActivos", "gestoria", "fechaAltaRem" };
		...
		if (!Checks.esNulo(gasto)) {
			...
			if (!Checks.esNulo(gastoGestion)) {
				...
				if (!Checks.esNulo(gastoGestion.getMotivoAutorizacionPropietario())) {
					dtoGestion.setComboMotivoAutorizacionPropietario(
							gastoGestion.getMotivoAutorizacionPropietario().getCodigo());
				}
				if (!Checks.esNulo(gasto.getGestoria())) {
				...
				if (!Checks.esNulo(gastoGestion.getUsuarioEstadoAutorizacionHaya())) {
					dtoGestion.setGestorAutorizacionHaya(
							gastoGestion.getUsuarioEstadoAutorizacionHaya().getApellidoNombre());
				}
				if (!Checks.esNulo(gastoGestion.getMotivoRechazoAutorizacionHaya()) && gastoGestion
						.getEstadoAutorizacionHaya().getCodigo().equals(DDEstadoAutorizacionHaya.CODIGO_RECHAZADO)) {
				...
				if (!Checks.esNulo(gastoGestion.getEstadoAutorizacionPropietario())) {
					dtoGestion.setComboEstadoAutorizacionPropietario(
							gastoGestion.getEstadoAutorizacionPropietario().getCodigo());
				}
		...
		try {
			...
			if (!Checks.esNulo(gasto)) {
				...
				if (!Checks.esNulo(gestionGasto)) {
					...
					if (!Checks.esNulo(dtoGestionGasto.getComboMotivoAutorizacionPropietario())
							&& !dtoGestionGasto.getComboMotivoAutorizacionPropietario().equals("")) {
						DDMotivoAutorizacionPropietario motivoAutoPro = (DDMotivoAutorizacionPropietario) utilDiccionarioApi
								.dameValorDiccionarioByCod(DDMotivoAutorizacionPropietario.class,
										dtoGestionGasto.getComboMotivoAutorizacionPropietario());
					...
					if (!Checks.esNulo(dtoGestionGasto.getComboEstadoAutorizacionHaya())) {
						DDEstadoAutorizacionHaya estadoAutoHaya = (DDEstadoAutorizacionHaya) utilDiccionarioApi
								.dameValorDiccionarioByCod(DDEstadoAutorizacionHaya.class,
					...
					if (!Checks.esNulo(dtoGestionGasto.getComboMotivoRechazoHaya())) {
						DDMotivoRechazoAutorizacionHaya motivoAutoHaya = (DDMotivoRechazoAutorizacionHaya) utilDiccionarioApi
								.dameValorDiccionarioByCod(DDMotivoRechazoAutorizacionHaya.class,
										dtoGestionGasto.getComboMotivoRechazoHaya());
					...
					if (!Checks.esNulo(dtoGestionGasto.getComboMotivoAnulado())) {
						...
						DDMotivoAnulacionGasto motivoAnulacion = (DDMotivoAnulacionGasto) utilDiccionarioApi
								.dameValorDiccionarioByCod(DDMotivoAnulacionGasto.class,
					...
					if (!Checks.esNulo(dtoGestionGasto.getComboMotivoRetenerPago())) {
						DDMotivoRetencionPago retenerPago = (DDMotivoRetencionPago) utilDiccionarioApi
								.dameValorDiccionarioByCod(DDMotivoRetencionPago.class,
						...
						// Si borro el campo eliminamos los detalles de retencion y ponemos el gasto en
						// estado incompleto o pendiente.
						gestionGasto.setMotivoRetencionPago(null);
						gestionGasto.setFechaRetencionPago(null);
						gestionGasto.setUsuarioRetencionPago(null);
						gasto.setGastoGestion(gestionGasto);
	...
	public boolean updateImpugnacionGasto(DtoImpugnacionGasto dto, Long idGasto) {
		...
		try {
			...
			if (!Checks.esNulo(dto.getResultadoCodigo())) {
				DDResultadoImpugnacionGasto resultado = (DDResultadoImpugnacionGasto) utilDiccionarioApi
						.dameValorDiccionarioByCod(DDResultadoImpugnacionGasto.class, dto.getResultadoCodigo());
	...
	private GastoProveedor asignarActivos(GastoProveedor gasto, Trabajo trabajo) {
		...
		for (ActivoTrabajo activo : trabajo.getActivosTrabajo()) {
			...
			GastoLineaDetalleEntidad gastoActivo = genericDao.get(GastoLineaDetalleEntidad.class, filtroActivo,
					filtroGasto);
			...
			try {
				...
				if (GestorDocumentalException.CODIGO_ERROR_CONTENEDOR_NO_EXISTE.equals(gex.getCodigoError())) {
					...
					try {
						...
						logger.debug("GESTOR DOCUMENTAL [ crearGasto para " + gasto.getNumGastoHaya()
								+ "]: ID EXPEDIENTE RECIBIDO " + idExpediente);
					} catch (GestorDocumentalException gexc) {
	...
	public String upload(WebFileItem fileItem) {
		...
		try {
			...
			if (gestorDocumentalAdapterApi.modoRestClientActivado()) {
				try {
					Long idDocRestClient = gestorDocumentalAdapterApi.uploadDocumentoGasto(gasto, fileItem,
			...
			// TODO Falta definir que tipo de documento provoca marcar el campo
			// existeDocumento.
	...
	@Override
	public AdjuntoGasto createAdjuntoGasto(WebFileItem fileItem, GastoProveedor gasto, Long idDocRestClient)
			throws Exception {
	...
	public boolean deleteAdjunto(DtoAdjunto dtoAdjunto) {
		...
		try {
			// Borramos en el gestor documental si hay, y buscamos el adjunto de BBDD para
			// borrarlo
			if (gestorDocumentalAdapterApi.modoRestClientActivado()) {
				Usuario usuarioLogado = genericAdapter.getUsuarioLogado();
	...
	public boolean desasignarTrabajos(Long idGasto, Long[] ids) {
		...
		for (Long id : ids) {
			...
			Filter filtro2 = genericDao.createFilter(FilterType.EQUALS, "gastoLineaDetalle.gastoProveedor.id",
					gasto.getId());
			GastoLineaDetalleTrabajo gastoTrabajoEliminado = genericDao.get(GastoLineaDetalleTrabajo.class, filtro,
					filtro2);
	...
	public Object searchProveedorCodigoByTipoEntidad(String codigoUnicoProveedor, String codigoTipoProveedor) {
		...
		List<ActivoProveedor> listaProveedores = new ArrayList<ActivoProveedor>();
		Filter filtroCodigo = genericDao.createFilter(FilterType.EQUALS, "codigoProveedorRem",
				Long.parseLong(codigoUnicoProveedor));
		// DDTipoEntidad tipoEntidad = (DDTipoEntidad)
		// utilDiccionarioApi.dameValorDiccionarioByCod(DDTipoEntidad.class,
		// codigoTipoEntidad);
		Filter filtroEntidad = genericDao.createFilter(FilterType.EQUALS, "tipoProveedor.tipoEntidadProveedor.codigo",
				codigoTipoProveedor);
		...
		if (!Checks.estaVacio(listaProveedores)) {
			...
			if (!Checks.esNulo(activoProveedor.getTipoProveedor())
					&& !Checks.esNulo(activoProveedor.getTipoProveedor().getTipoEntidadProveedor())) {
				dto.setSubtipoProveedorDescripcion(
						activoProveedor.getTipoProveedor().getTipoEntidadProveedor().getDescripcion());
			}
	...
	public Object searchGastoNumHaya(String numeroGastoHaya, String proveedorEmisor, String destinatario) {
		...
		DDDestinatarioGasto destinatarioGasto = (DDDestinatarioGasto) utilDiccionarioApi
				.dameValorDiccionarioByCod(DDDestinatarioGasto.class, destinatario);

		if (!Checks.estaVacio(listaGastos)) {
			GastoProveedor gasto = listaGastos.get(0);
			if (!Checks.esNulo(gasto.getProveedor()) && !Checks.esNulo(proveedorEmisor)) {
				if (gasto.getProveedor().getCodigoProveedorRem().equals(Long.parseLong(proveedorEmisor))
						&& gasto.getDestinatarioGasto().equals(destinatarioGasto)) {
					return gasto;
				}
			}
	...
	public boolean autorizarGasto(Long idGasto, boolean validarAutorizacion) {
		...
		if (validarAutorizacion) {
			...
			if (!Checks.esNulo(error)) {
				throw new JsonViewerException(
						"El gasto " + gasto.getNumGastoHaya() + " no se puede autorizar: " + error);
			}
		}
	...
	public boolean autorizarGastoContabilidad(Long idGasto, String fechaConta, String fechaPago, Boolean individual) {
		...
		if (!Checks.esNulo(gasto.getProvision()) && individual) {
			throw new JsonViewerException("El gasto " + gasto.getNumGastoHaya()
					+ " no se puede autorizar individualmente: pertenece a una agrupación.");
		} else {
			DDEstadoAutorizacionPropietario estadoAutorizacionPropietario = (DDEstadoAutorizacionPropietario) utilDiccionarioApi
					.dameValorDiccionarioByCod(DDEstadoAutorizacionPropietario.class,
	...
	@Override
	@Transactional(readOnly = false)
	public boolean autorizarGastosContabilidadAgrupacion(Long[] idsGastos, Long idAgrupacion, String fechaConta,
			String fechaPago, Boolean individual) {

		// Setamos el estado de la agrupación
		ProvisionGastos agrupGasto = provisionGastosDao.get(idAgrupacion);

		DDEstadoProvisionGastos estadoProvisionGastos = (DDEstadoProvisionGastos) utilDiccionarioApi
				.dameValorDiccionarioByCod(DDEstadoProvisionGastos.class, DDEstadoProvisionGastos.CODIGO_AUTORIZADO);
	...
	public boolean rechazarGasto(Long idGasto, String motivoRechazo, String motivoRechazoDescripcion) {
		...
		GastoRefacturable gastoRefacturado = genericDao.get(GastoRefacturable.class, filtroBorrado,
				genericDao.createFilter(FilterType.EQUALS, "idGastoProveedorRefacturado", idGasto));
	...
	public boolean rechazarGastosContabilidad(Long[] idsGastos, String motivoRechazo, Boolean individual) {
		...
		for (Long id : idsGastos) {
			...
			if (!Checks.esNulo(gasto.getProvision()) && individual) {
				throw new JsonViewerException("El gasto " + gasto.getNumGastoHaya()
						+ " no se puede rechazar individualmente: pertenece a una agrupación.");
			} else {
				String error = updaterStateApi.validarCamposMinimos(gasto, false);
				if (!Checks.esNulo(error)) {
					throw new JsonViewerException(
							"El gasto " + gasto.getNumGastoHaya() + " no se puede rechazar: " + error);
				}
	...
	public String checkReglaCambioEstado(String codigoEstado, boolean coniva, String matriculaTipoDoc,
			GastoProveedor gasto) {
		Pattern factPattern = Pattern.compile(".*-FACT-.*");
		Pattern justPattern = Pattern.compile(".*-CERA-.*");

		if (factPattern.matcher(matriculaTipoDoc).matches()
				&& Checks.esNulo(updaterStateApi.validarCamposMinimos(gasto, false))
				&& DDEstadoGasto.INCOMPLETO.equals(codigoEstado)) {
	...
	@Override
	public void actualizarPorcentajeParticipacionGastoProveedorActivo(Long idActivo, Long idGasto,
		...
		Filter filterGastoId = genericDao.createFilter(FilterType.EQUALS, "gastoLineaDetalle.gastoProveedor.id",
				idGasto);
	...
	public GastoLineaDetalleEntidad buscarRelacionPorActivoYGasto(Activo activo, GastoProveedor gasto) {
		...
		return genericDao.get(GastoLineaDetalleEntidad.class,
	...
	public boolean rechazarGastosContabilidadAgrupGastos(Long idAgrupGasto, Long[] idsGasto, String motivoRechazo,
		...
		DDEstadoProvisionGastos estadoProvisionGastos = (DDEstadoProvisionGastos) utilDiccionarioApi
				.dameValorDiccionarioByCod(DDEstadoProvisionGastos.class,
			...
			if (!Checks.estaVacio(listaVistaGastos)) {
				...
				for (VGastosRefacturados vGastosRefacturado : listaVistaGastos) {
					GastoProveedor gastoProveedorRefacturable = genericDao.get(GastoProveedor.class,
							genericDao.createFilter(FilterType.EQUALS, "numGastoHaya",
									Long.parseLong(vGastosRefacturado.getNumGastoHaya())));
					if (gastoProveedorRefacturable.getTipoGasto() != null
							...
							&& nifPropietario != null
							&& this.gastoMismoPropietario(nifPropietario, gastoProveedorRefacturable)
	...
	public Boolean eliminarGastoRefacturado(Long idGasto, Long numGastoRefacturado) {
		...
		Filter gastoRefacturadoId = genericDao.createFilter(FilterType.EQUALS, "idGastoProveedorRefacturado",
				gastoProveedor.getId());
	...
	public boolean esGastoRefacturable(GastoProveedor gasto) {
		...
		if (!Checks.esNulo(gasto.getGastoDetalleEconomico())) {
			if (!Checks.esNulo(gasto.getGastoDetalleEconomico().getGastoRefacturable())) {
				return gasto.getGastoDetalleEconomico().getGastoRefacturable();// Devolvemos el valor del campo, ya que
	...
	private boolean gastoMismoPropietario(String nifPropietario, GastoProveedor gastoProveedorRefacturable) {
		...
		if (!Checks.esNulo(nifPropietario)) {
			if (!Checks.esNulo(gastoProveedorRefacturable)
					&& !Checks.esNulo(gastoProveedorRefacturable.getPropietario())) {
				mismoPropietario = nifPropietario
						.equals(gastoProveedorRefacturable.getPropietario().getDocIdentificativo());
			}
		}
	...
	public void validarGastosARefacturar(String idGasto, String listaGastos) {
		...
		for (String numGastoRefacturable : listaGastosRefacturables) {
			Filter gastoHijoId = genericDao.createFilter(FilterType.EQUALS, "numGastoHaya",
					Long.valueOf(numGastoRefacturable));
			GastoProveedor gastoHijo = genericDao.get(GastoProveedor.class, gastoHijoId);

			Filter gastoLineaDetalle = genericDao.createFilter(FilterType.EQUALS, "gastoProveedor.id",
					gastoHijo.getId());
			List<GastoLineaDetalle> gastoLineaDetalleList = genericDao.getList(GastoLineaDetalle.class,
					gastoLineaDetalle);

			if (gastoLineaDetalleList == null || gastoLineaDetalleList.isEmpty()) {
				throw new JsonViewerException(
						"El gasto " + numGastoRefacturable + " no tiene niguna línea de detalle.");
			}

			Filter detalleEconomicoGastoId = genericDao.createFilter(FilterType.EQUALS, "gastoProveedor.id",
					gastoHijo.getId());
			GastoDetalleEconomico gastoDetalleEconomico = genericDao.get(GastoDetalleEconomico.class,
					detalleEconomicoGastoId);

			Filter facturadosGastoId = genericDao.createFilter(FilterType.EQUALS, "idGastoProveedorRefacturado",
					gastoHijo.getId());
			GastoRefacturable gastoRefacturable = genericDao.get(GastoRefacturable.class, facturadosGastoId);

			if (!Checks.esNulo(gastoDetalleEconomico) && (!Checks.esNulo(gastoDetalleEconomico.getGastoRefacturable())
					&& !gastoDetalleEconomico.getGastoRefacturable())) {
				throw new JsonViewerException("El gasto " + numGastoRefacturable
						+ " no es refacturable, por favor corrija el listado de gastos");
			}

			if (!Checks.esNulo(gastoRefacturable)) {
				Filter gastoPadreActualId = genericDao.createFilter(FilterType.EQUALS, "id",
						gastoRefacturable.getGastoProveedor());
				GastoProveedor gastoPadreActual = genericDao.get(GastoProveedor.class, gastoPadreActualId);

				throw new JsonViewerException(
						"El gasto " + gastoHijo.getNumGastoHaya() + " ya está refacturado en el gasto "
								+ gastoPadreActual.getNumGastoHaya() + ", por favor corrija el listado de gastos");
			}

			if (!Checks.esNulo(gastoPadre) && !Checks.esNulo(gastoHijo) && !Checks.esNulo(gastoPadre.getPropietario())
					&& !Checks.esNulo(gastoHijo.getPropietario())
					&& !gastoPadre.getPropietario().equals(gastoHijo.getPropietario())) {
				throw new JsonViewerException("El gasto " + numGastoRefacturable
						+ " no tiene el mismo propietario que el gasto " + gastoPadre.getNumGastoHaya());
			}
		}
	...
	public boolean isPosibleRefacturable(GastoProveedor gasto) {
		...
		if (!Checks.esNulo(gasto) && !Checks.esNulo(gasto.getEstadoGasto())
			...
			String estadoGasto = gasto.getEstadoGasto().getCodigo();
			List<GastoRefacturable> gastosPadres = genericDao.getList(GastoRefacturable.class,
					genericDao.createFilter(FilterType.EQUALS, "idGastoProveedor", gasto.getId()),
			...
			GastoRefacturable gastoRefacturado = genericDao.get(GastoRefacturable.class,
	...
	public Double recalcularImporteTotalGasto(GastoDetalleEconomico gasto) {
		...
		if (gasto.getGastoProveedor() != null) {
			...
			retencionAntes = gasto.getTipoRetencion() != null
					&& DDTipoRetencion.CODIGO_TRE_ANTES.equals(gasto.getTipoRetencion().getCodigo());

			Filter filter = genericDao.createFilter(FilterType.EQUALS, "gastoProveedor.id",
					gasto.getGastoProveedor().getId());
			...
			if (gastoLineaDetalleList != null && !gastoLineaDetalleList.isEmpty()) {
				for (GastoLineaDetalle gastoLineaDetalle : gastoLineaDetalleList) {
					...
					if (gastoLineaDetalle.getImporteIndirectoTipoImpositivo() != null
							&& gastoLineaDetalle.getPrincipalSujeto() != null &&
							// op exenta no y rellena
							((gastoLineaDetalle.getEsImporteIndirectoExento() != null
									&& !gastoLineaDetalle.getEsImporteIndirectoExento()) ||
							// op exenta si y rellena y renuncia si y rellena
									(gastoLineaDetalle.getEsImporteIndirectoExento() != null
											&& gastoLineaDetalle.getEsImporteIndirectoExento()
											&& gastoLineaDetalle.getEsImporteIndirectoRenunciaExento() != null
											&& gastoLineaDetalle.getEsImporteIndirectoRenunciaExento())
									||
									// op exenta vacia
									(gastoLineaDetalle.getEsImporteIndirectoExento() == null))) {
	...
	@Override
	@Transactional(readOnly = false)
	public void anyadirGastosRefacturablesSiCumplenCondiciones(String idGasto, String gastosRefacturables,
			String nifPropietario) throws IllegalAccessException, InvocationTargetException, Exception {
		...
		if (gastoProveedor.getGastoLineaDetalleList() != null && !gastoProveedor.getGastoLineaDetalleList().isEmpty()
				&& (gastoProveedor.getCartera() != null
						&& gastoProveedor.getCartera().getCodigo().equals(DDCartera.CODIGO_CARTERA_SAREB))
		...
		if (gastosRefacturables != null && gastoProveedor != null && gastoProveedor.getTipoGasto() != null
				&& gastoSinLineas) {
			gastosRefacturablesLista = this.getGastosRefacturados(gastosRefacturables, nifPropietario,
					gastoProveedor.getTipoGasto().getCodigo());
		}
	...
	public List<DtoVImporteGastoLbk> getVImporteGastoLbk(Long idGasto) {
		...
		if (gastosImportesGastoLBK != null && !gastosImportesGastoLBK.isEmpty()) {
			for (GastosImportesLBK gastoImportesGastoLBK : gastosImportesGastoLBK) {
				...
				Filter idTipoEnt = genericDao.createFilter(FilterType.EQUALS, "id",
						gastoImportesGastoLBK.getEntidadGasto());
				...
				if (DDEntidadGasto.CODIGO_ACTIVO.equals(tipoEntidad.getCodigo())) {
					...
					Filter filterId = genericDao.createFilter(FilterType.EQUALS, "id",
							gastoImportesGastoLBK.getIdEntidad());
	...
	public List<DDTipoTrabajo> getTiposTrabajoByIdGasto(Long idGasto) {
		...
		Filter filterTipoGasto = genericDao.createFilter(FilterType.EQUALS, "tipoGasto.id",
				gastoProveedor.getTipoGasto().getId());
		...
		for (DDSubtipoGasto subTipoGasto : subTiposGastos) {
			Filter filterSubTipoGasto = genericDao.createFilter(FilterType.EQUALS, "subtipoGasto.id",
					subTipoGasto.getId());
	...
	private void validacionPreviaSaveUpdateGasto(DtoFichaGastoProveedor dto, Long id) {
		...
		Filter filtroBorrado = genericDao.createFilter(FilterType.EQUALS, "auditoria.borrado", false);
		// Codigos de Tipo de gasto Servicios profesionales independientes y 'Subtipo'
		// honorarios gestión activos.
		String[] subtiposGasto = new String[] { COD_HONORARIOS_GESTION_ACTIVOS, COD_REGISTRO, COD_NOTARIA, COD_ABOGADO,
				COD_PROCURADOR, COD_OTROS_SERVICIOS_JURIDICOS, COD_ADMINISTRADOR_COMUNIDAD_PROPIETARIOS, COD_ASESORIA,
				COD_TECNICO, COD_TASACION, COD_OTROS, COD_GESTION_DE_SUELO, COD_ABOGADO_OCUPACIONAL,
				COD_ABOGADO_ASUNTOS_GENERALES, COD_ABOGADO_ASISTENCIA_JURIDiCA };
		...
		if (dto.getFacturaPrincipalSuplido() != null && !dto.getFacturaPrincipalSuplido().isEmpty()) {
			...
			Filter filtroFactura = genericDao.createFilter(FilterType.EQUALS, "referenciaEmisor",
					dto.getFacturaPrincipalSuplido());
			Filter filtroSuplidos = genericDao.createFilter(FilterType.EQUALS, "suplidosVinculados.codigo",
					DDSinSiNo.CODIGO_SI);
			List<GastoProveedor> gastosProveedor = genericDao.getList(GastoProveedor.class, filtroFactura,
					filtroSuplidos);

			if (gastosProveedor == null || gastosProveedor.isEmpty()) {
				throw new JsonViewerException("No existe ninguna factura " + dto.getFacturaPrincipalSuplido()
						+ " en un gasto con 'Suplido vinculado'");
			} else if (gastosProveedor != null && !gastosProveedor.isEmpty() && gastosProveedor.size() > 1) {
				throw new JsonViewerException(
						"No se puede dar de alta un 'Suplido vinculado' cuyo numero de factura esta en más de un gasto");
			} else {
				gastoProveedor = gastosProveedor.get(0);

				if (gastoProveedor != null && (gastoProveedor.getSuplidosVinculados() == null
						|| DDSinSiNo.CODIGO_NO.equals(gastoProveedor.getSuplidosVinculados().getCodigo()))) {
					throw new JsonViewerException(
							"No se puede dar de alta ya que el gasto de la factura indicada tiene 'Suplido vinculado' a 'No'");
				}
		...
		if (dto.getSuplidosVinculadosCod() != null) {
			...
			if (DDSinSiNo.CODIGO_SI.equals(dto.getSuplidosVinculadosCod())) {
				if (gastoProveedor != null && gastoProveedor.getNumeroFacturaPrincipal() != null) {
					throw new JsonViewerException(
							"No se puede actualizar el campo 'Suplidos vinculados' a 'Si' si tiene factura principal");
				}
			} else {
				if (gastoProveedor != null) {

					filtroProveedorPadre = genericDao.createFilter(FilterType.EQUALS, "gastoProveedorPadre",
							gastoProveedor);
					...
					if (gastosSuplidos != null && !gastosSuplidos.isEmpty()) {
						throw new JsonViewerException(
								"No se puede actualizar el campo 'Suplidos vinculados' a 'No' si tiene suplidos vinculados");
					}
				}
			}
		}
		...
		if (dto.getFacturaPrincipalSuplido() != null && !dto.getFacturaPrincipalSuplido().isEmpty()
				|| dto.getSuplidosVinculadosCod() != null) {
			if (gastoProveedor.getGastoLineaDetalleList() != null
					&& !gastoProveedor.getGastoLineaDetalleList().isEmpty()) {
				for (GastoLineaDetalle gastoLinea : gastoProveedor.getGastoLineaDetalleList()) {
					...
					if (gastoLineaEntidadList == null || gastoLineaEntidadList.isEmpty()) {
						...
						for (GastoLineaDetalleEntidad gastoLineaDetalleEntidad : gastoLineaEntidadList) {
							Activo activo = activoDao.getActivoById(gastoLineaDetalleEntidad.getEntidad());
							if (activo == null || gastoProveedor.getCartera() == null || activo.getSubcartera() == null
									|| gastoProveedor.getTipoGasto() == null) {
								throw new JsonViewerException("No hay datos");
							}
							ConfiguracionSuplidos config = genericDao.get(ConfiguracionSuplidos.class,
									genericDao.createFilter(FilterType.EQUALS, "cartera.codigo",
											gastoProveedor.getCartera().getCodigo()),
									genericDao.createFilter(FilterType.EQUALS, "subCartera.codigo",
											activo.getSubcartera().getCodigo()),
									genericDao.createFilter(FilterType.EQUALS, "tipoGasto.codigo",
											gastoProveedor.getTipoGasto().getCodigo()),
	...
	public ActivoSubtipoGastoProveedorTrabajo getSubtipoGastoBySubtipoTrabajo(Trabajo trabajo) {
		...
		Filter subtipoTrabajo = genericDao.createFilter(FilterType.EQUALS, "subtipoTrabajo.id",
				trabajo.getSubtipoTrabajo().getId());
		ActivoSubtipoGastoProveedorTrabajo subtipoGastoTrabajo = genericDao
				.get(ActivoSubtipoGastoProveedorTrabajo.class, subtipoTrabajo);
	...
	private Boolean esGastoAutorizado(GastoProveedor gasto) {
		...
		if (gasto != null && gasto.getEstadoGasto() != null) {
			return DDEstadoGasto.AUTORIZADO_ADMINISTRACION.equals(gasto.getEstadoGasto().getCodigo())
					|| DDEstadoGasto.AUTORIZADO_PROPIETARIO.equals(gasto.getEstadoGasto().getCodigo())
					|| DDEstadoGasto.CONTABILIZADO.equals(gasto.getEstadoGasto().getCodigo())
	...
	public String validacionNifEmisorFactura(DtoFichaGastoProveedor dto, Long idGasto) {
		...
		if (gastoProveedor != null && gastoProveedor.getSuplidosVinculados() != null
				...
				&& ((dto.getReferenciaEmisor() != null
						&& !dto.getReferenciaEmisor().equals(gastoProveedor.getReferenciaEmisor()))
						|| (dto.getCodigoProveedorRem() != null && !dto.getCodigoProveedorRem()
								.equals(gastoProveedor.getProveedor().getCodigoProveedorRem())))) {

			Filter filtroProveedorPadre = genericDao.createFilter(FilterType.EQUALS, "gastoProveedorPadre",
					gastoProveedor);
			Filter filtroBorrado = genericDao.createFilter(FilterType.EQUALS, "auditoria.borrado", false);
			List<GastoSuplido> gastosSuplidos = genericDao.getList(GastoSuplido.class, filtroProveedorPadre,
					filtroBorrado);
	...
	public void actualizaSuplidosAsync(Long idGasto, Long codProveedorRem, String referenciaEmisor) {
		...
		if (idGasto != null) {
			...
			if (gastoPrincipal != null) {
				...
				List<GastoSuplido> gastosSuplidos = genericDao.getList(GastoSuplido.class,
						genericDao.createFilter(FilterType.EQUALS, "gastoProveedorPadre", gastoPrincipal));
				...
				for (GastoSuplido suplido : gastosSuplidos) {
					...
					if (codProveedorRem != null) {
						...
						GastoDetalleEconomico detalleGasto = genericDao.get(GastoDetalleEconomico.class,
								genericDao.createFilter(FilterType.EQUALS, "gastoProveedor.id", gastoSuplido.getId()));

						if (detalleGasto != null && detalleGasto.getAbonoCuenta() != null
								&& detalleGasto.getAbonoCuenta() == 1 && proveedor != null
								&& proveedor.getDocIdentificativo() != null) {
	...
	public Double recalcularImporteRetencionGarantia(GastoDetalleEconomico gasto) {
		...
		if (gasto.getTipoRetencion() != null
				&& DDTipoRetencion.CODIGO_TRE_DESPUES.equals(gasto.getTipoRetencion().getCodigo())) {
			esDespues = true;
		}
		...
		if (gasto.getGastoProveedor() != null) {
			...
			if (gastoLineaDetalleList != null && !gastoLineaDetalleList.isEmpty()) {
				for (GastoLineaDetalle gastoLineaDetalle : gastoLineaDetalleList) {
					if (gastoLineaDetalle.getPrincipalSujeto() != null) {
						importeRetencionGarantia = importeRetencionGarantia
								.add(new BigDecimal(gastoLineaDetalle.getPrincipalSujeto()));
					}
					if (gastoLineaDetalle.getPrincipalNoSujeto() != null) {
						importeRetencionGarantia = importeRetencionGarantia
								.add(new BigDecimal(gastoLineaDetalle.getPrincipalNoSujeto()));
					}

					if (esDespues && gastoLineaDetalle.getImporteIndirectoCuota() != null && carteraGasto != null
							&& DDCartera.CODIGO_CARTERA_LIBERBANK.equals(carteraGasto.getCodigo())) {
						importeRetencionGarantia = importeRetencionGarantia
								.add(new BigDecimal(gastoLineaDetalle.getImporteIndirectoCuota()));
					}
				}
			}
		}
	...
	public List<VTasacionesGastos> getListTasacionesGasto(Long idGasto) {
		return idGasto != null
				? genericDao.getList(VTasacionesGastos.class,
						genericDao.createFilter(FilterType.EQUALS, "idGasto", idGasto))
				: null;
	}
	...
	public List<VGridMotivosRechazoGastoCaixa> getMotivosRechazoGasto(Long idGasto) throws Exception {
		...
		if (idGasto != null) {
			listaMotivosRec = genericDao.getList(VGridMotivosRechazoGastoCaixa.class,
					genericDao.createFilter(FilterType.EQUALS, "gastoId", idGasto));
		}
				...
				if (!Checks.esNulo(gasto.getProveedor())) {
					if (DDEstadoGasto.INCOMPLETO.equals(gasto.getEstadoGasto().getCodigo())
						...
						if (Checks.esNulo(gasto.getProveedor().getNombreComercial())
								...
								|| Checks.estaVacio(activoProveedorDireccion)) {
							return isProveedorIncompleto = true;
						}
					}
				}

		}
		return isProveedorIncompleto;
	}
	...
	public List<DDTipoOperacionGasto> getComboTipoOperacionByGasto(Long idGasto) {
		...
		try {
			...
			if (gastoProveedor.getGastoLineaDetalleList() != null && !gastoProveedor.getGastoLineaDetalleList().isEmpty()) {
				for (GastoLineaDetalle linea : gastoProveedor.getGastoLineaDetalleList()) {
					if (DDSubtipoGasto.IBI_RUSTICA.equals(linea.getSubtipoGasto().getCodigo()) 
							|| DDSubtipoGasto.IBI_URBANA.equals(linea.getSubtipoGasto().getCodigo())) {
						esIBI = true;
					}
