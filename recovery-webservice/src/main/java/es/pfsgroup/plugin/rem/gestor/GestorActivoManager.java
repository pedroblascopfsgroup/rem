...
public class GestorActivoManager extends GestorEntidadManager implements GestorActivoApi {
	...
	public Boolean insertarGestorAdicionalActivo(GestorEntidadDto dto) {
		...
		if (GestorEntidadDto.TIPO_ENTIDAD_ACTIVO.equals(dto.getTipoEntidad())) {
			if (!Checks.esNulo(dto.getIdEntidad()) && !Checks
					.esNulo(dto.getIdUsuario())/*
												 * && !Checks.esNulo(dto.
												 * getIdTipoDespacho())
												 */) {

				EXTDDTipoGestor tipoGestor = genericDao.get(EXTDDTipoGestor.class,
						genericDao.createFilter(FilterType.EQUALS, "id", dto.getIdTipoGestor()));
				Filter filtro = genericDao.createFilter(FilterType.EQUALS, "activo.id", dto.getIdEntidad());
				Filter filtroTipoGestor = genericDao.createFilter(FilterType.EQUALS, "tipoGestor.id",
						dto.getIdTipoGestor());
				GestorActivo gac = genericDao.get(GestorActivo.class, filtro, filtroTipoGestor);

				Usuario usu = genericDao.get(Usuario.class,
						genericDao.createFilter(FilterType.EQUALS, "id", dto.getIdUsuario()));
				...
				if (Checks.esNulo(gac)) {
					...
					// Comprobamos que el activo cumpla las condiciones para
					// poder cambiar de gestor
					if (gac.getTipoGestor().getCodigo().equals("GPREC")) {
						...
						// if
						// (!dto.getIdUsuario().equals(gac.getUsuario().getId()))
						// {
						this.actualizaFechaHastaHistoricoGestorAdicionalActivo(gac);
						gac.setUsuario(usu);
						gac.setAuditoria(Auditoria.getNewInstance());
	...
	public void actualizarTareas(Long idActivo) {
		...
		for (ActivoTramite tramite : listaTramites) {
			...
			for (TareaExterna tareaExterna : listaTareas) {
				...
				UserAssigantionService userAssigantionService = userAssigantionServiceFactoryApi
						.getService(tareaProcedimiento.getCodigo());
	...
	public List<GestorEntidadHistorico> getListGestoresActivosAdicionalesHistoricoData(
			GestorEntidadDto gestorEntidadDto) {
	...
	public Usuario getGestorByActivoYTipo(Activo activo, Long tipo) {
		List<Usuario> usuariosGestoresList = ((GestorActivoDao) gestorEntidadDao)
				.getListUsuariosGestoresActivoByTipoYActivo(tipo, activo);
	...
	public Boolean isGestorActivo(Activo activo, Usuario usuario) {
		...
		List<Usuario> usuariosActivos = ((GestorActivoDao) gestorEntidadDao)
				.getListUsuariosGestoresActivoByTipoYActivo(tipoGestor.getId(), activo);
	...
	public Boolean isGestorAdmision(Activo activo, Usuario usuario) {
		...
		List<Usuario> usuariosAdmision = ((GestorActivoDao) gestorEntidadDao)
				.getListUsuariosGestoresActivoByTipoYActivo(tipoGestor.getId(), activo);
	...
	public Boolean isGestorActivoYTipo(Usuario usuario, Activo activo, String codigoGestor) {
		if (Checks.esNulo(codigoGestor))
			...
			List<Usuario> usuarios = ((GestorActivoDao) gestorEntidadDao)
					.getListUsuariosGestoresActivoBycodigoTipoYActivo(codigoGestor, activo);
	...
	private void actualizaFechaHastaHistoricoGestorAdicional(GestorActivo gac) {
		...
		List<GestorActivoHistorico> gah = genericDao.getList(GestorActivoHistorico.class, filtroTipoGestor,
				filtroUsuario, filtroActivo);
