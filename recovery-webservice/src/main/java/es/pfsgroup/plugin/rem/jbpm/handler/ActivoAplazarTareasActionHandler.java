...
public class ActivoAplazarTareasActionHandler extends ActivoBaseActionHandler implements ActivoJBPMEnterEventHandler {
	...
	public void onEnter(ExecutionContext executionContext) {
		...
		for (TareaNotificacion t : activoTramite.getTareas()) {
			if (!t.getAuditoria().isBorrado() && t.getTareaFinalizada() == null
					|| (t.getTareaFinalizada() != null && t.getTareaFinalizada()) && t.getTareaExterna() != null) {
		...
		if (plazo == null) {
			logger.error("No se ha creado correctamente el timer de aplazamiento para el nodo "
					+ getNombreNodo(executionContext));
		} else {
		...
		logger.debug("\tAplazamos la tarea, desactivamos su timer y creamos uno de activación ["
				+ getNombreNodo(executionContext) + "]");
