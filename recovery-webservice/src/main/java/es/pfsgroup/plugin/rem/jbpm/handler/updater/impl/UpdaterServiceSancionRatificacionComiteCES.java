...
@Component
public class UpdaterServiceSancionRatificacionComiteCES implements UpdaterService {
	@Autowired
	...
	protected static final Log logger = LogFactory.getLog(UpdaterServiceSancionRatificacionComiteCES.class);
	 
	private static final String COMBO_RATIFICACION = "comboRatificacion";
	private static final String FECHA_RATIFICACION = "fechaRatificacion";
		...
		if (!Checks.esNulo(ofertaAceptada)) {
			...
			if (!Checks.esNulo(expediente)) {
				...
				for (TareaExternaValor valor : valores) {
					if (FECHA_RATIFICACION.equals(valor.getNombre()) && !Checks.esNulo(valor.getValor())) {
						try {
	...
	public String[] getCodigoTarea() {
		return new String[] { CODIGO_T017_RATIFIACION_COMITE_CES };
	}
