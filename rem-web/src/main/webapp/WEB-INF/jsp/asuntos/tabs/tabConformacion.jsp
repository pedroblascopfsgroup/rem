...
(function(){
	...
	var taId = "observacionAsu"+"${asunto.id}";
	var show=false;                                                                                                                                                      
    var sesion =app.creaLabel('<s:message code="decisioncomite.consulta.sesion" text="**Sesion" />','${asunto.comite.ultimaSesion.id}');
    var fecha  =app.creaLabel('<s:message code="decisioncomite.consulta.fecha" text="**Fecha" />','<fwk:date value="${asunto.comite.ultimaSesion.fechaInicio}"/>');
    var plazorestante =app.creaLabel('<s:message code="decisioncomite.consulta.plazo" text="**Plazo Restante" />',app.format.ifNullRenderer('${asunto.cantidadDiasParaVencer}','Vencido'));
	var observaciones = new Ext.form.TextArea({
				...
				,readOnly: false
				,labelStyle: ''
				,value:'<s:message text="${asunto.decisionComite.observaciones}" javaScriptEscape="true" />'
				,name: ''
		   });
...
//LISTADO DE CONTRATOS
	...
	var contratosStore = page.getStore({
        //flow: 'asuntos/procedimientosAsunto'
        //,event:'listadoProcedimientos'                                                                                                                         
	...
	var btnActuacion = new Ext.Button({
		   ...
		   ,evaluateAndSend: function(seguir) {
	       		if(seguir== 'yes') {
					...
					//contratosStore.webflow({id:${asunto.id}});
					contratosStore.webflow({id:${asunto.expediente.id}});
					asuntosStore.webflow({id:'${asunto.id}'});
					//contratosActuacionesStore.webflow({idAsunto:'${asunto.id}'});
					contratosActuacionesStore.webflow({idExpediente:'${asunto.expediente.id}'});
				}
	       }
	});
	...
	function refrescaCheckBox(){
		...
		for(i=0; i < contratosSinActuacion.contratosId.length; i++) {
			for(j=0; j < contratosGrid.getStore().getTotalCount(); j++) {
				if(contratosSinActuacion.contratosId[i].id==contratosGrid.getStore().getAt(j).data.id) {
...
/*
	var contratosActuacionesStore = page.getStore({
		eventName : 'listado'
		,flow:'asuntos/listadoContratosActuacionesAsuntosData'
		,reader: new Ext.data.JsonReader({
...
*/
	...
	,{header : '<s:message code="asuntos.listado.procedimiento" text="**Código Actuación" />', width:100, dataIndex : 'procedimiento', sortable: false}
	,{header : '<s:message code="asuntos.listado.tipoActuacion" text="**Tipo actuación" />', width: 75, dataIndex : 'tipoActuacion', sortable: false}
	,{header : '<s:message code="asuntos.listado.actuacion" text="**Actuación" />', width:150, dataIndex : 'actuacion', sortable: false}
	,{header : '<s:message code="decisioncomite.consulta.gridcontratos.fechaCreacion" text="**Fecha creación" />', width: 100, dataIndex : 'fechaCreacionProcedimiento', sortable: false}
	]);  
	...
	var asuntosCm = new Ext.grid.ColumnModel([
    {
		...
		header: '<s:message code="asuntos.listado.procedimiento" text="**Procedimiento"/>',
		width:100, dataIndex: 'idProcedimiento'
		,sortable:false
	}, {
		header: '<s:message code="asuntos.listado.tipoActuacion" text="**Tipo Actuación"/>',
		width:100, dataIndex: 'tipoActuacion',sortable:false
	}, {
		header: '<s:message code="asuntos.listado.actuacion" text="**Actuación"/>',
			...
			win = app.openWindow(
				{
					...
					params: {idAsuntoOrigen:${asunto.id}},
					/*				
					flow:'asuntos/altaAsuntos', 
					title : '<s:message code="decisionComite.asuntos" text="**Alta Asuntos" />',
					params: {idExpediente:${asunto.expediente.id}},
					*/
					width: 870
				}
			);
		...
		handler: function(){
			if (idAsuntoSeleccionado){
				var win = app.openWindow({
					...
					,closable:true
					,params:{idExpediente:${asunto.expediente.id},idAsunto:idAsuntoSeleccionado}
					,width: 870
				});
				win.on(app.event.CANCEL,function(){win.close();});
				win.on(app.event.DONE,
				...
				if (asuntosGrid.store.data.length && asuntosGrid.store.data.length>0){
					...
					var w = app.openWindow({
						...
						,title : '<s:message code="procedimientos.edicion.titulo" text="**Nuevo registro" />' 
							,params : {idExpediente:${asunto.expediente.id},idAsuntoOrigen:${asunto.id},idAsunto:idAsuntoSeleccionado, idProcedimiento:0}
					});
					...
					if (asuntosGrid.store.data.length && asuntosGrid.store.data.length>0){
						if (idProcSeleccionado){
							var w = app.openWindow({
								...
								,params : {idExpediente:${asunto.expediente.id},idAsuntoOrigen:${asunto.id},idProcedimiento:idProcSeleccionado,idAsunto:idAsuntoSeleccionado}
							});
							w.on(app.event.DONE, function(){
								w.close();
								contratosStore.on('load',cargarArrayInicial);											
	...
	var asuntosGrid = new Ext.grid.GridPanel({
		...
		height: 125,
		bbar: [
			<c:if test="${asunto.estaPropuesto}">                                                                                                                                 
	...
	asuntosGrid.on('rowclick', function(grid, rowIndex, e){
		...
		if(idAsuntoSeleccionado!='') {
			...
			//Si está en estado propuesto no se permite su borrado
			if (asuEstado == asuConformacion) btnBorrar.enable();
			//Si está en estado en conformación si que se permite su borrado
	...
	//----------------------------------------------------------------
	// Boton de cerrar decisionn
	//----------------------------------------------------------------
		var cerrarDecision = function(){
			page.webflow({
            flow: 'asunto/cerrarSesionAsunto',
            params: {idAsunto:${asunto.id}, observaciones:observaciones.getValue()}
            ,success: function(){
	    ...
	    var marcarSinActuacion = function(){
			page.webflow({
	      		...
	      		,params:{idContratos:contratosSinActString,idExpediente:${asunto.expediente.id}}
	      		,success: function(){
					contratosSinActuacion=new Array();
					contratosSinActString = armarString(contratosSinActuacion);
					//contratosStore.webflow({id:${asunto.id}});
	...
	var panel = new Ext.Panel({                                                                                                                                          
		...
		,items:[                                                                                                                                                           
			...
			,{                                                                                                                                                               
				autoHeight:true                                                                                                                                                
				,style:'margin-left:600px;padding-top:20px'
				,border:false                                                                                                                                                  
