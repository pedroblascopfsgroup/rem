...
(function(){
	...
	contratosExpedienteStore.webflow({idExpediente:${expediente.id}});
				...
				page.webflow({
					...
					,success: function() {
						var w = app.openWindow({
							flow: 'expedientes/incluirContrato'
							,eventName: 'busquedaContrato'
							,params: {idExpediente:${expediente.id},busquedaOrInclusion:'inclusion'}
	...
	<c:if test="${expediente.estaDecidido}">incluirContrato.disable();</c:if>
	...
	var excluirContrato = new Ext.Button({
		...
		,cls: 'x-btn-text-icon'
		,handler: function() {
				Ext.Msg.confirm('<s:message code="expedientes.consulta.tabcabecera.otrosCont.excluirContratos" text="**Excluir contrato" />', 
	                    	       '<s:message code="expedientes.consulta.tabcabecera.otrosCont.excluirContratos.confirmar" text="**¿Está seguro que desea excluir el contrato del expediente?" />',
		...
		,evaluateAndSend: function(seguir) {
	         			if(seguir== 'yes') {
	            			page.webflow({
								...
								,params:{idExpediente:${expediente.id},contratos:idContrato}
								,success: function(){
									contratosExpedienteStore.webflow({idExpediente:${expediente.id}});
								}
							});
	...
	<c:if test="${!expediente.estaDecidido}">
