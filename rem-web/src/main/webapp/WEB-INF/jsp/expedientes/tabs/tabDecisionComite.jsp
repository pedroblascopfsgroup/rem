...
(function(){
	...
	var comite =app.creaLabel('<s:message code="decisioncomite.consulta.comite" text="**Comite" />','${expediente.comite.nombre}');                           
	...
	var observaciones = new Ext.form.TextArea({
				...
				,readOnly: true
				,labelStyle: ''
				,value:'<s:message text="${expediente.decisionComite.observaciones}" javaScriptEscape="true" />'
				,name: ''
		   });
	...
	var btnActuacion = new Ext.Button({
		   ...
		   ,evaluateAndSend: function(seguir) {
	       		if(seguir== 'yes') {
					...
					contratosActuacionesStore.webflow({idExpediente:'${expediente.id}'});
				}
	       }
	});
...
//FIN LISTADO DE CONTRATOS
	...
	var asuntosCm = new Ext.grid.ColumnModel([
    {
		...
		header: '<s:message code="asuntos.listado.procedimiento" text="**Código actuación"/>',
				...
				if (asuntosGrid.store.data.length && asuntosGrid.store.data.length>0){
					...
					var w = app.openWindow({
						...
						,title : '<s:message code="procedimientos.edicion.titulo" text="**Nuevo registro" />' 
							,params : {idExpediente:${expediente.id},idAsunto:idAsuntoSeleccionado, idProcedimiento:0}
					});
					...
					if (asuntosGrid.store.data.length && asuntosGrid.store.data.length>0){
						if (idProcSeleccionado){
							var w = app.openWindow({
								...
								,params : {idExpediente:${expediente.id},idProcedimiento:idProcSeleccionado,idAsunto:idAsuntoSeleccionado}
							});
							w.on(app.event.DONE, function(){
								w.close();
								contratosStore.webflow({id:${expediente.id}});
	...
	<c:if test="${expediente.estaCongelado}">      
	    ...
	    var marcarSinActuacion = function(){
			page.webflow({
	      		...
	      		,params:{idContratos:contratosSinActString,idExpediente:${expediente.id}}
	      		,success: function(){
					contratosSinActuacion=new Array();
					contratosSinActString = armarString(contratosSinActuacion);
					contratosStore.webflow({id:${expediente.id}});
