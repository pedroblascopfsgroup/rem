/**
 * Código obtenido de 
 * https://yui-ext.com/forum/showthread.php?t=14658
 * y modificado por aesteban
 */
...
Ext.extend(Ext.ux.MoneyField, Ext.form.TextField,{
	...
	getValue: function(floatPoint, decimalSep) {
    	var field = this.el.dom;
		var value = (field.value.replace(/\D/g, "").replace(/^0+/g, "")).replace(/\D/g, "");
        ...
        length <= floatPoint && (value = new Array(floatPoint - length + 2).join("0") + value);
        for(var i = (length = (value = value.split("")).length) - floatPoint; (i -= 3) > 0;);
        floatPoint && floatPoint < length && (value[length - ++floatPoint] += decimalSep);
		return value.join("");		
    },
