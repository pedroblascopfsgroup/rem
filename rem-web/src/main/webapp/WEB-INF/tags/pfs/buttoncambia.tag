...
var ${name}= new Ext.Button({
		text : '<s:message code="pfs.tags.buttoncambia.cambiar" text="**Cambiar por defecto" />'
		,handler : function(){
			if (${datagrid}.getSelectionModel().getCount()>0){
				Ext.Msg.confirm('<s:message code="pfs.tags.buttoncambia.cambiar" text="**Borrar" />', '<s:message code="pfs.tags.buttoncambia.pregunta" text="**¿Está seguro de borrar?" />', function(btn){
    				if (btn == 'yes'){
				...
				Ext.Msg.alert('<s:message code="pfs.tags.buttoncambia.cambiar" text="**Borrar" />','<s:message code="${novalueMsgKey}" text="${novalueMsg}" />');
			}
		}
});
