...
--## INCIDENCIA_LINK=BCFI-695-BCFI-711
--## PRODUCTO=NO
--##
--## Finalidad: Modificar el procedimiento PROCESA_PROCESO_FACT para que facture el concepto de cobro '%sobre impuestos'
--## 			Tambien arregla el tema de seleccionar correctamente el tramo de facturacion
...
DBMS_OUTPUT.PUT_LINE('[START] Alterar procedimiento PROCESA_PROCESO_FACT');
...
CREATE OR REPLACE PROCEDURE PROCESA_PROCESO_FACT AS
  ...
  WITH PFS AS (
    ...
    FROM '|| v_schema ||'.PFS_PROC_FAC_SUBCARTERA
    WHERE BORRADO = 0
  )
  , DATOS AS (
    SELECT /*+ PARALLEL */ PFS.PFS_ID, CPR.CPA_ID, CPR.CPR_ID, CPA.CNT_ID, CPR.EXP_ID, CPR.RCF_SCA_ID, CPR.RCF_AGE_ID, CPR.CPA_FECHA, COC.RCF_DD_COC_CPA_CONCEP_IMPORT
      ...
      FROM BANK01.CPR_COBROS_PAGOS_RECOBRO CPR
        JOIN '|| v_schema ||'.CPA_COBROS_PAGOS CPA ON CPR.CPA_ID = CPA.CPA_ID
        JOIN PFS ON CPR.RCF_SCA_ID = PFS.RCF_SCA_ID
        JOIN '|| v_schema ||'.PRF_PROCESO_FACTURACION PRF ON PFS.PRF_ID = PRF.PRF_ID AND PRF.BORRADO = 0
        JOIN '|| v_schema ||'.RCF_MFA_MODELOS_FACTURACION MFA ON PFS.RCF_MFA_ID = MFA.RCF_MFA_ID AND MFA.BORRADO = 0
        JOIN '|| v_schema ||'.RCF_TCF_TIPO_COBRO_FACTURA TCF ON MFA.RCF_MFA_ID = TCF.RCF_MFA_ID AND CPA.DD_SCP_ID = TCF.DD_SCP_ID AND TCF.BORRADO = 0
        JOIN '|| v_schema ||'.RCF_TCC_TARIFAS_CONCEP_COBRO TCC ON TCF.RCF_TCF_ID = TCC.RCF_TCF_ID AND TCC.BORRADO = 0
        JOIN '|| v_schema ||'.RCF_DD_COC_CONCEPTO_COBRO COC ON TCC.RCF_DD_COC_ID = COC.RCF_DD_COC_ID
        JOIN '|| v_schema ||'.RCF_TCT_TARIF_COBRO_TRAMO TCT ON TCC.RCF_TCC_ID = TCT.RCF_TCC_ID AND TCC.BORRADO = 0
  ...
  , DATOS_TRAMOS AS (
    ...
    FROM DATOS D
      LEFT JOIN '|| v_schema ||'.RCF_TRF_TRAMO_FACTURACION TRF ON D.RCF_TRF_ID = TRF.RCF_TRF_ID AND TRF.BORRADO = 0
...
BEGIN
  -- Borramos la tabla
  EXECUTE IMMEDIATE ''TRUNCATE TABLE '|| v_schema ||'.TMP_RECOBRO_DETALLE_FACTURA'';

  -- Inicio bucle procesos pendientes
  FOR P IN (SELECT PRF_ID
            FROM '|| v_schema ||'.PRF_PROCESO_FACTURACION PRF
              JOIN '|| v_schema ||'.RCF_DD_EPF_ESTADO_PROC_FAC EFP ON PRF.RCF_DD_EPF_ID = EFP.RCF_DD_EPF_ID
  ...
  LOOP
    ...
    LOOP
      ...
      INSERT INTO '|| v_schema ||'.TMP_RECOBRO_DETALLE_FACTURA(PFS_ID, CPA_ID, CNT_ID, EXP_ID, RCF_SCA_ID, RCF_AGE_ID, RDF_FECHA_COBRO, RDF_PORCENTAJE, RDF_IMPORTE_A_PAGAR, RCF_TCC_ID, RDF_IMP_CONCEP_FACTU, RDF_IMP_REAL_FACTU, CPR_ID)
      VALUES(
      L_COBROS(I).PFS_ID
      , L_COBROS(I).CPA_ID
      , L_COBROS(I).CNT_ID
...
DBMS_OUTPUT.PUT_LINE('REPLACE PROCEDURE '|| v_schema ||'.PROCESA_PROCESO_FACT ... OK');
