...
-- */
execute immediate 'update '||V_ESQUEMA||'.tap_tarea_procedimiento set usuariomodificar=''BKREC-1131'', fechamodificar=sysdate, tap_script_validacion_jbpm = ''valores[''''P401_CelebracionSubasta''''][''''comboCelebrada''''] == ''''02'''' ? null : (comprobarImporteEntidadAdjudicacionBienes() ? (comprobarNumeroActivo() ? null : ''''Antes de dar la subasta por celebrada, deber&aacute; acceder a la ficha del bien y solicitar el n&uacute;mero de activo mediante el bot&oacute;n habilitado para tal efecto.'''') : ''''Debe rellenar en cada bien el importe adjudicaci&oacute;n y la entidad.'''')'' where tap_codigo = ''P401_CelebracionSubasta''';

execute immediate 'update '||V_ESQUEMA||'.tap_tarea_procedimiento set usuariomodificar=''BKREC-1131'', fechamodificar=sysdate, tap_script_validacion_jbpm = ''valores[''''P409_CelebracionSubasta''''][''''comboCelebrada''''] == ''''02'''' ? null : (comprobarImporteEntidadAdjudicacionBienes() ? (comprobarNumeroActivo() ? null : ''''Antes de dar la subasta por celebrada, deber&aacute; acceder a la ficha del bien y solicitar el n&uacute;mero de activo mediante el bot&oacute;n habilitado para tal efecto.'''') : ''''Debe rellenar en cada bien el importe adjudicaci&oacute;n y la entidad.'''')'' where tap_codigo = ''P409_CelebracionSubasta''';
