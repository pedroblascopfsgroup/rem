...
--## INCIDENCIA_LINK=BKREC-1245
--## PRODUCTO=NO
--## Finalidad: DML , Inserción de los tipos de liquidacion
--##           
...
IF V_NUM_TABLAS = 0 THEN
    ...
    EXECUTE IMMEDIATE V_INSERT || ' ('||V_ESQUEMA||'.S_DD_PCO_LIQ_TIPO.nextval, ''HIPOTECARIO'', ''HIPOTECARIO Descripcion'', ''HIPOTECARIO Descripcion'',''PTAMO. HIPOTECARIO.docx'',''INICIAL'',sysdate) ';
END IF;
...
IF V_NUM_TABLAS = 0 THEN
    ...
    EXECUTE IMMEDIATE V_INSERT || ' ('||V_ESQUEMA||'.S_DD_PCO_LIQ_TIPO.nextval, ''PERSONAL'', ''PERSONAL Descripcion'', ''PERSONAL Descripcion'',''PTAMO. PERSONAL.docx'',''INICIAL'' ,sysdate) ';
END IF;
