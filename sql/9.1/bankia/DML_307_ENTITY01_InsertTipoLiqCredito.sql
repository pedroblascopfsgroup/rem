...
--## PRODUCTO=NO
--## Finalidad: DML , Inserción del tipo de liquidación CREDITO
--##           
...
V_SQL := 'SELECT COUNT(*) FROM ' || V_ESQUEMA || '.' || V_DDNAME || ' WHERE DD_PCO_LIQ_CODIGO = ''CREDITO''';
EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

IF V_NUM_TABLAS = 0 THEN
    DBMS_OUTPUT.PUT_LINE('[INFO] Insertar Sí');
    EXECUTE IMMEDIATE V_INSERT || ' ('||V_ESQUEMA||'.S_DD_PCO_LIQ_TIPO.nextval, ''CREDITO'', ''CREDITO Descripcion'', ''CREDITO Descripcion'',''CREDITO.docx'',''INICIAL'',sysdate) ';
END IF;
