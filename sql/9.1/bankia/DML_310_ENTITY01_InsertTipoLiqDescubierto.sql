...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_PCO_LIQ_TIPO';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_PCO_LIQ_TIPO';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_PCO_LIQ_';

   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ' USING (SELECT :id id, :codigo codigo, :des descripcion, :des_l descripcion_larga, :ru ruta_plantilla, ''INICIAL'' usuariocrear, sysdate fechacrear from DUAL) actual ' ||
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('DESCUBIERTO', 'DESCUBIERTO Descripcion', 'DESCUBIERTO Descripcion', 'DESCUBIERTO.docx')
    ); 
    V_TMP_TIPO T_TIPO;
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE IMMEDIATE V_MERGE USING V_ENTIDAD_ID, V_TMP_TIPO(1), V_TMP_TIPO(2), V_TMP_TIPO(3), V_TMP_TIPO(4);
    DBMS_OUTPUT.PUT_LINE('INSERTANDO: ' || V_ENTIDAD_ID || ', '''||V_TMP_TIPO(1)||''','''||TRIM(V_TMP_TIPO(2))||''','''||TRIM(V_TMP_TIPO(3))||''','''||TRIM(V_TMP_TIPO(4))||'''');
  END LOOP;
  COMMIT;
