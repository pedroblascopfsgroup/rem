...
--## INCIDENCIA_LINK=PRODUCTO-775
--## PRODUCTO=NO
--## Finalidad: DML , Inserción del tipo de liquidación DESCUENTO
--##           
...
V_SQL := 'SELECT COUNT(*) FROM ' || V_ESQUEMA || '.' || V_DDNAME || ' WHERE DD_PCO_LIQ_CODIGO = ''DESCUENTO''';
EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

IF V_NUM_TABLAS = 0 THEN
    DBMS_OUTPUT.PUT_LINE('[INFO] Insertar Sí');
    EXECUTE IMMEDIATE V_INSERT || ' ('||V_ESQUEMA||'.S_DD_PCO_LIQ_TIPO.nextval, ''DESCUENTO'', ''DESCUENTO Descripcion'', ''DESCUENTO Descripcion'',''DESCUENTO.docx'',''INICIAL'',sysdate) ';
END IF;
