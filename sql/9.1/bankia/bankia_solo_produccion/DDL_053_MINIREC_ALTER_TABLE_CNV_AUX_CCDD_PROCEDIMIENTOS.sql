...
BEGIN    
    ...
    SELECT COUNT(1) INTO V_NUM_TABLAS FROM user_tab_cols WHERE UPPER(table_name) = 'CNV_AUX_CCDD_PROCEDIMIENTOS' and UPPER(column_name) = 'NUMERO_AUTOS';
	
    IF V_NUM_TABLAS = 1 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE ' || V_ESQUEMA || '.CNV_AUX_CCDD_PROCEDIMIENTOS MODIFY NUMERO_AUTOS VARCHAR2(15 CHAR)';
        DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.CNV_AUX_CCDD_PROCEDIMIENTOS... NUMERO_AUTOS creada');
    END IF;	
