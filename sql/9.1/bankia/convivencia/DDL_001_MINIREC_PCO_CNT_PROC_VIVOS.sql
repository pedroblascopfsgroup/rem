...
BEGIN
    ...
    /*  tabla MINIREC_PCO_CNT_PROC_VIVOS <-- ELIMINAR*/

    DBMS_OUTPUT.PUT_LINE('[START] DROP TABLE tabla MINIREC_PCO_CNT_PROC_VIVOS');

    select count(1) into V_NUM_TABLAS from ALL_TABLES where table_name = 'MINIREC_PCO_CNT_PROC_VIVOS';
    if V_NUM_TABLAS > 0 then
        EXECUTE IMMEDIATE ' DROP TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PROC_VIVOS CASCADE CONSTRAINTS';
            DBMS_OUTPUT.PUT_LINE('DROP TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PROC_VIVOS... Tabla borrada OK');
    end if;
            
    EXECUTE IMMEDIATE '    
    CREATE TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PROC_VIVOS
    ...
    EXECUTE IMMEDIATE 
    'CREATE UNIQUE INDEX '|| V_ESQUEMA_MIN ||'.PK_MPC_PCO_CNT_PROC_VIVOS ON '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PROC_VIVOS
        (MPC_CNT_ID, MPC_CD_PROCEDIMIENTO)  ';

EXECUTE IMMEDIATE 
'ALTER TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PROC_VIVOS ADD (
     CONSTRAINT PK_MPC_PCO_CNT_PROC_VIVOS
     PRIMARY KEY
     (MPC_CNT_ID, MPC_CD_PROCEDIMIENTO) )';
        
DBMS_OUTPUT.PUT_LINE('TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PROC_VIVOS... Tabla CREADA OK');
    
DBMS_OUTPUT.PUT_LINE('[END] Tabla MINIREC_PCO_CNT_PROC_VIVOS');
