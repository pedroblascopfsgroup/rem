...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[START] DROP TABLE tabla MINIREC_PCO_CNT_PRC_BAJA_TOTAL');

    select count(1) into V_NUM_TABLAS from ALL_TABLES where table_name = 'MINIREC_PCO_CNT_PRC_BAJA_TOTAL';
    if V_NUM_TABLAS > 0 then
        EXECUTE IMMEDIATE ' DROP TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PRC_BAJA_TOTAL CASCADE CONSTRAINTS';
            DBMS_OUTPUT.PUT_LINE('DROP TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PRC_BAJA_TOTAL... Tabla borrada OK');
    end if;
            
    EXECUTE IMMEDIATE '    
    CREATE TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PRC_BAJA_TOTAL
...
DBMS_OUTPUT.PUT_LINE('TABLE '|| V_ESQUEMA_MIN ||'.MINIREC_PCO_CNT_PRC_BAJA_TOTAL... Tabla CREADA OK');
    
DBMS_OUTPUT.PUT_LINE('[END] Tabla MMINIREC_PCO_CNT_PRC_BAJA_TOTAL');
