...
CREATE OR REPLACE PROCEDURE GEST_PDM_LITIGIO AS
...
SELECT DISTINCT
   CASE WHEN ULT_MOVIMIENTO_CNT.CD_EXPEDIENTE_NUSE IS NOT NULL
	THEN ULT_MOVIMIENTO_CNT.CD_EXPEDIENTE_NUSE 
	ELSE EXPEDIENTE_ASUNTO.CD_EXPEDIENTE_NUSE 
	END ID_EXPEDIENTE
  ...
  ,CASE WHEN ULT_MOVIMIENTO_CNT.NUMERO_EXP_NUSE IS NOT NULL 
	THEN ULT_MOVIMIENTO_CNT.NUMERO_EXP_NUSE 
	ELSE EXPEDIENTE_ASUNTO.NUMERO_EXP_NUSE 
	END NUMERO_EXP
  ...
  ,CASE WHEN (FECHA_FINALIZACION.FECHAMODIFICAR IS NULL AND DD_EAS.DD_EAS_CODIGO IN ('05', '06')) 
	THEN ULT_TAR.TAR_FECHA_FIN 
	ELSE NULL 
	END FECHA_BAJA
...
MERGE INTO #ESQUEMA_MINIREC#.RCV_GEST_PDM_LITIGIO RCV
	USING 
		(SELECT ID_PROCEDI, FECHA_ALTA, FECHA_BAJA, ESTADO
		FROM #ESQUEMA#.TMP_RCV_GEST_PDM_LITIGIO
		) TMP
	ON (RCV.ID_PROCEDI = TMP.ID_PROCEDI)                       
	WHEN MATCHED 
	THEN UPDATE 
	SET RCV.FECHA_BAJA = TMP.FECHA_BAJA
		...
		FROM 
			...
			FROM #ESQUEMA#.CPE_CONTRATOS_PERSONAS 					CPE  
				...
				INNER JOIN #ESQUEMA#.OFI_OFICINAS 					OFICR 
					ON OFICR.OFI_CODIGO_OFICINA = PERSONAS_OFI_GESTORA.ICC_VALUE
			) ORDEN 
			WHERE RN = 1
		) SRC
ON (RCV.ID_CUENTA_RCV = SRC.CNT_ID)
