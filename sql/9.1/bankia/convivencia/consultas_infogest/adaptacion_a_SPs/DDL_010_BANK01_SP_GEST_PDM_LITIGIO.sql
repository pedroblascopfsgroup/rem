...
ULTIMO_PROC_ASUNTO AS (
	...
	FROM (
	  ...
	  INNER JOIN #ESQUEMA#.PRC_PROCEDIMIENTOS PRC ON PRC.ASU_ID = ASU.ASU_ID ) TMP_AP
	WHERE RN = 1
),
...
MERGE INTO #ESQUEMA_MINIREC#.RCV_GEST_PDM_LITIGIO RCV
      USING (
     select * from (
     SELECT DPR.PRC_ID, DPR.DD_DFI_ID, DFI.DD_DFI_DESCRIPCION, TRUNC(GREATEST(DPR.FECHACREAR, NVL(DPR.FECHAMODIFICAR,'01/01/1000'))) FECHAMODIFICAR, ROW_NUMBER () OVER (PARTITION BY dpr.PRC_ID ORDER BY DPR.FECHAMODIFICAR DESC) RN
      FROM #ESQUEMA#.DPR_DECISIONES_PROCEDIMIENTOS DPR
