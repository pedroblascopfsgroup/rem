...
-- Responsable ultima modificacion: Pedro Sebastián, PFS Group
-- Fecha ultima modificacion: 01/09/2015
...
-- ----------------------------------------------------------------------------------------------
  EXECUTE IMMEDIATE
    'INSERT INTO D_ASU (ASUNTO_ID, NOMBRE_ASUNTO, ESTADO_ASUNTO_ID, EXPEDIENTE_ID, PROPIETARIO_ASUNTO_ID, GESTION_ASUNTO_ID)
    SELECT ASU_ID, NVL(ASU_NOMBRE, ''Desconocido''), DD_EAS_ID, EXP_ID, -1, NVL(DD_GES_ID, -1)
  ...
  update D_ASU A
    set PROPIETARIO_ASUNTO_ID = NVL((select B.DD_PAS_ID from recovery_bankia_datastage.ASU_ASUNTOS B where B.ASU_ID = A.ASUNTO_ID), -1);
  COMMIT;
...
--                                      D_ASU_GESTION_ASUNTO
-- ----------------------------------------------------------------------------------------------
  SELECT COUNT(*) INTO V_NUM_ROW FROM D_ASU_GESTION_ASUNTO WHERE GESTION_ASUNTO_ID = -1;
  IF (V_NUM_ROW = 0) THEN
    INSERT INTO D_ASU_GESTION_ASUNTO (GESTION_ASUNTO_ID, GESTION_ASUNTO_DESC) VALUES (-1 ,'Desconocido');
  END IF;

  EXECUTE IMMEDIATE
    'INSERT INTO D_ASU_GESTION_ASUNTO (GESTION_ASUNTO_ID, GESTION_ASUNTO_DESC)
    SELECT DD_GES_ID, DD_GES_DESCRIPCION from '||V_DATASTAGE||'.DD_GES_GESTION_ASUNTO';

  V_ROWCOUNT := sql%rowcount;     
  COMMIT;
  
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_ASU_GESTION_ASUNTO. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
