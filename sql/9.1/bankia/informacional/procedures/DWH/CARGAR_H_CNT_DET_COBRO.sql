...
-- ----------------------------- Loop fechas a cargar -----------------------------
  ...
  loop --READ_LOOP
    ...
    V_SQL :=  'BEGIN OPERACION_DDL.DDL_INDEX(''DROP'', ''TMP_H_CNT_DET_COB_COBRO_ID_IX'', '''', ''S'', '''', :O_ERROR_STATUS); END;';
    execute immediate V_SQL USING OUT O_ERROR_STATUS;
    ...
    merge into TMP_H_CNT_DET_COBRO dc
    using (select max(NVL(TARIFA, -1)) as MAX_TARIFA, COBRO_ID from TMP_CNT_FACTURACION group by COBRO_ID) cf
    on (cf.COBRO_ID = dc.COBRO_ID)   
    when matched then update set dc.IMPORTE_FACTURA_TARIFA = cf.MAX_TARIFA  where dc.IMPORTE_FACTURA_TARIFA is null;
    commit;
