  ...
  if V_COUNT <> 1 then
    ...
    if V_PARAMETRO_DATE_INI is not null AND V_PARAMETRO_DATE_FIN is not null then    
      ...
      if V_FECHA_CARGA_END IS NULL then
        --SELECT MAX(mov_fecha_extraccion) INTO V_FECHA_CARGA_END FROM recovery_bankia_datastage.MOV_MOVIMIENTOS;
        SELECT max_dia_mov INTO V_FECHA_CARGA_END FROM recovery_bankia_datastage.FECHAS_MOV;
        if V_FECHA_CARGA_END IS NULL then
