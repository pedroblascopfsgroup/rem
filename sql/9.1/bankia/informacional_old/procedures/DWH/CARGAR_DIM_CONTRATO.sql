...
-- Fecha última modificación: 10/11/2015
...
-- ----------------------------------------------------------------------------------------------
  ...
  if (V_NUM_ROW = 0) then
    insert into D_CNT_EST_INSINUACION_CNT (ESTADO_INSINUACION_CNT_ID, ESTADO_INSINUACION_CNT_DESC, ESTADO_INSINUACION_CNT_DESC2) values (-2 ,'Sin Cédito Insinuado Asociado', 'Sin Cédito Insinuado Asociado');
  end if;
...
-- ----------------------------------------------------------------------------------------------
  ...
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS where T_IRREG_DIAS_ID = 1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS (T_IRREG_DIAS_ID, T_IRREG_DIAS_DESC) values (1 ,'<= 30 Días');
  end if;
  ...
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS (T_IRREG_DIAS_ID, T_IRREG_DIAS_DESC) values (4 ,'91-120 Días');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS where T_IRREG_DIAS_ID = 5;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS (T_IRREG_DIAS_ID, T_IRREG_DIAS_DESC) values (5 ,'121 Días-1 Año');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS where T_IRREG_DIAS_ID = 6;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS (T_IRREG_DIAS_ID, T_IRREG_DIAS_DESC) values (6 ,'1-3 Años');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS where T_IRREG_DIAS_ID = 7;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS (T_IRREG_DIAS_ID, T_IRREG_DIAS_DESC) values (7 ,'> 3 Años');
  end if;
...
-- ----------------------------------------------------------------------------------------------
  ...
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS_PERIODO_ANT (T_IRREG_DIAS_PERIODO_ANT_ID, T_IRREG_DIAS_PERIODO_ANT_DESC) values (-2 ,'No Existe En Periodo Anterior'); 
  end if;
  ...
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS_PERIODO_ANT where T_IRREG_DIAS_PERIODO_ANT_ID = 1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS_PERIODO_ANT (T_IRREG_DIAS_PERIODO_ANT_ID, T_IRREG_DIAS_PERIODO_ANT_DESC) values (1 ,'<= 30 Días');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS_PERIODO_ANT where T_IRREG_DIAS_PERIODO_ANT_ID = 2;
  if (V_NUM_ROW = 0) then
  ...
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS_PERIODO_ANT (T_IRREG_DIAS_PERIODO_ANT_ID, T_IRREG_DIAS_PERIODO_ANT_DESC) values (4 ,'91-120 Días');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS_PERIODO_ANT where T_IRREG_DIAS_PERIODO_ANT_ID = 5;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS_PERIODO_ANT (T_IRREG_DIAS_PERIODO_ANT_ID, T_IRREG_DIAS_PERIODO_ANT_DESC) values (5 ,'121 Días-1 Año');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS_PERIODO_ANT where T_IRREG_DIAS_PERIODO_ANT_ID = 6;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS_PERIODO_ANT (T_IRREG_DIAS_PERIODO_ANT_ID, T_IRREG_DIAS_PERIODO_ANT_DESC) values (6 ,'1-3 Años');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_IRREG_DIAS_PERIODO_ANT where T_IRREG_DIAS_PERIODO_ANT_ID = 7;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_IRREG_DIAS_PERIODO_ANT (T_IRREG_DIAS_PERIODO_ANT_ID, T_IRREG_DIAS_PERIODO_ANT_DESC) values (7 ,'> 3 Años');
  end if;
...
-- ----------------------------------------------------------------------------------------------
  ...
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_SALDO_TOTAL (T_SALDO_TOTAL_CNT_ID, T_SALDO_TOTAL_CNT_DESC) values (3 ,'90.000 € - 300.000 €');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_SALDO_TOTAL where T_SALDO_TOTAL_CNT_ID = 4;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_SALDO_TOTAL (T_SALDO_TOTAL_CNT_ID, T_SALDO_TOTAL_CNT_DESC) values (4 ,'> 300.000 €');
  end if;

  commit;
  
  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_T_SALDO_TOTAL. Realizados INSERTS', 3;
...
-- ----------------------------------------------------------------------------------------------
  ...
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_DEUDA_IRREGULAR (T_DEUDA_IRREGULAR_CNT_ID, T_DEUDA_IRREGULAR_CNT_DESC) values (0 ,'0 € - 25.000 €');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_DEUDA_IRREGULAR where T_DEUDA_IRREGULAR_CNT_ID = 1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_DEUDA_IRREGULAR (T_DEUDA_IRREGULAR_CNT_ID, T_DEUDA_IRREGULAR_CNT_DESC) values (1 ,'25.000 € - 50.000 €');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_DEUDA_IRREGULAR where T_DEUDA_IRREGULAR_CNT_ID = 2;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_DEUDA_IRREGULAR (T_DEUDA_IRREGULAR_CNT_ID, T_DEUDA_IRREGULAR_CNT_DESC) values (2 ,'50.000 € - 75.000 €');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_T_DEUDA_IRREGULAR where T_DEUDA_IRREGULAR_CNT_ID = 3;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_T_DEUDA_IRREGULAR (T_DEUDA_IRREGULAR_CNT_ID, T_DEUDA_IRREGULAR_CNT_DESC) values (3 ,'> 75.000 €');
  end if;
...
-- ----------------------------------------------------------------------------------------------
  ...
  V_MSQL := 'SELECT DISTINCT IDFACTURACION, NOMBRE_FACTURACION FROM '||V_DATASTAGE||'.MINIRECOVERY_FACT WHERE IDFACTURACION IS NOT NULL group by IDFACTURACION, NOMBRE_FACTURACION ORDER BY 1';

  OPEN c_facturacion FOR V_MSQL;
  LOOP
    FETCH c_facturacion INTO IDFACTURACION, NOMBRE_FACTURACION;        
    EXIT WHEN c_facturacion%NOTFOUND;  
    select count(*) into V_NUM_ROW from D_CNT_FACTURA_COBRO where FACTURA_COBRO_ID = IDFACTURACION;
    if (V_NUM_ROW = 0) then
      insert into D_CNT_FACTURA_COBRO (FACTURA_COBRO_ID, FACTURA_COBRO_DESC) values (IDFACTURACION, NOMBRE_FACTURACION);
      commit;
    end if;  
  END LOOP;
  CLOSE c_facturacion;
...
--                                      D_CNT_ZONA_NIVEL_0
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_0 where ZONA_NIVEL_0_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_0 (ZONA_NIVEL_0_ID, ZONA_NIVEL_0_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_0 (ZONA_NIVEL_0_ID, ZONA_NIVEL_0_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_0 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_0 niv where niv.ZONA_NIVEL_0_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
     
  V_ROWCOUNT := sql%rowcount;     
  commit;

   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_0. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   

-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_1
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_1 where ZONA_NIVEL_1_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_1 (ZONA_NIVEL_1_ID, ZONA_NIVEL_1_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_1 (ZONA_NIVEL_1_ID, ZONA_NIVEL_1_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_1 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_1 niv where niv.ZONA_NIVEL_1_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_1. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   
-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_2
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_2 where ZONA_NIVEL_2_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_2 (ZONA_NIVEL_2_ID, ZONA_NIVEL_2_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_2 (ZONA_NIVEL_2_ID, ZONA_NIVEL_2_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_2 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_2 niv where niv.ZONA_NIVEL_2_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_2. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   

-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_3
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_3 where ZONA_NIVEL_3_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_3 (ZONA_NIVEL_3_ID, ZONA_NIVEL_3_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_3 (ZONA_NIVEL_3_ID, ZONA_NIVEL_3_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_3 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_3 niv where niv.ZONA_NIVEL_3_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_3. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   

-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_4
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_4 where ZONA_NIVEL_4_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_4 (ZONA_NIVEL_4_ID, ZONA_NIVEL_4_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_4 (ZONA_NIVEL_4_ID, ZONA_NIVEL_4_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_4 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_4 niv where niv.ZONA_NIVEL_4_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_4. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   

-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_5
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_5 where ZONA_NIVEL_5_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_5 (ZONA_NIVEL_5_ID, ZONA_NIVEL_5_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_5 (ZONA_NIVEL_5_ID, ZONA_NIVEL_5_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_5 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_5 niv where niv.ZONA_NIVEL_5_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_5. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   

-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_6
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_6 where ZONA_NIVEL_6_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_6 (ZONA_NIVEL_6_ID, ZONA_NIVEL_6_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_6 (ZONA_NIVEL_6_ID, ZONA_NIVEL_6_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_6 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_6 niv where niv.ZONA_NIVEL_6_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_6. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;


-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_7
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_7 where ZONA_NIVEL_7_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_7 (ZONA_NIVEL_7_ID, ZONA_NIVEL_7_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_7 (ZONA_NIVEL_7_ID, ZONA_NIVEL_7_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_7 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_7 niv where niv.ZONA_NIVEL_7_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_7. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   

-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_8
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_8 where ZONA_NIVEL_8_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_8 (ZONA_NIVEL_8_ID, ZONA_NIVEL_8_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_8 (ZONA_NIVEL_8_ID, ZONA_NIVEL_8_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_8 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_8 niv where niv.ZONA_NIVEL_8_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_8. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
   

-- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ZONA_NIVEL_9
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_CNT_ZONA_NIVEL_9 where ZONA_NIVEL_9_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ZONA_NIVEL_9 (ZONA_NIVEL_9_ID, ZONA_NIVEL_9_DESC) values (-1 ,'Desconocido');
  end if;
  
  execute immediate
    'insert into D_CNT_ZONA_NIVEL_9 (ZONA_NIVEL_9_ID, ZONA_NIVEL_9_DESC) 
     select distinct ofi.OFI_CODIGO, ofi.OFI_NOMBRE 
     from ' || V_DATASTAGE || '.ZONA_JERARQUIA zon
     join ' || V_DATASTAGE || '.OFI_OFICINAS ofi on zon.NIVEL_9 = ofi.OFI_CODIGO 
     where not exists (select 1 from D_CNT_ZONA_NIVEL_9 niv where niv.ZONA_NIVEL_9_ID = ofi.OFI_CODIGO)
     order by ofi.OFI_CODIGO';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ZONA_NIVEL_9. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
...
--  V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''ALTER'', ''D_CNT'', ''ENABLE CONSTRAINT D_CNT_PK'', :O_ERROR_STATUS); END;';
--  execute immediate V_SQL USING OUT O_ERROR_STATUS;
  ...
  V_SQL :=  'BEGIN OPERACION_DDL.DDL_INDEX(''CREATE'', ''D_CNT_CCC_LITIGIO_IX'', ''D_CNT (CONTRATO_COD_CONTRATO)'', ''S'', '''', :O_ERROR_STATUS); END;';
  execute immediate V_SQL USING OUT O_ERROR_STATUS;
...
-- ----------------------------------------------------------------------------------------------
  ...
  execute immediate
    'insert into D_CNT_GESTION_ESPECIAL(GESTION_ESPECIAL_ID, GESTION_ESPECIAL_COD_ID, GESTION_ESPECIAL_DESC)
    select DD_GES_ID, DD_GES_CODIGO, DD_GES_DESCRIPCION from '||V_DATASTAGE||'.DD_GES_GESTION_ESPECIAL';   

  select count(*) into V_NUM_ROW from D_CNT_GESTION_ESPECIAL where GESTION_ESPECIAL_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_GESTION_ESPECIAL (GESTION_ESPECIAL_ID, GESTION_ESPECIAL_COD_ID, GESTION_ESPECIAL_DESC)
    values (-1 ,'Desconocido', 'Desconocido');
  end if;
  ...
  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_GESTION_ESPECIAL. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
  
    -- ----------------------------------------------------------------------------------------------
--                                      D_CNT_ACTUACION_RESTRICTIVA
-- ----------------------------------------------------------------------------------------------
  
   select count(*) into V_NUM_ROW from D_CNT_ACTUACION_RESTRICTIVA where ACTUACION_RESTRICTIVA_ID = 0;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ACTUACION_RESTRICTIVA (ACTUACION_RESTRICTIVA_ID, ACTUACION_RESTRICTIVA_DESC) values (0 ,'Actuacion no restrictiva');
  end if;
  select count(*) into V_NUM_ROW from D_CNT_ACTUACION_RESTRICTIVA where ACTUACION_RESTRICTIVA_ID = 1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_ACTUACION_RESTRICTIVA (ACTUACION_RESTRICTIVA_ID, ACTUACION_RESTRICTIVA_DESC) values (1 ,'Actuación restrictiva');
  end if;
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_CNT_ACTUACION_RESTRICTIVA. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
