...
create or replace PROCEDURE CNV_ALTA_GESTORES_ASUNTO AS
  ...
  --/* Consulta para gestores del Asunto */
     ...
     WITH USU AS (
      SELECT USU_ID, USU_USERNAME FROM BANKMASTER.USU_USUARIOS WHERE BORRADO = 0
    )
    , USD AS (
      ...
      FROM USD_USUARIOS_DESPACHOS USD
        ...
        JOIN BANKMASTER.DD_TDE_TIPO_DESPACHO TDE ON DES.DD_TDE_ID = TDE.DD_TDE_ID
      WHERE USD.BORRADO = 0 AND USD.USD_GESTOR_DEFECTO = 1
    )
    , TGE AS (
      SELECT DD_TGE_ID, DD_TGE_CODIGO FROM BANKMASTER.DD_TGE_TIPO_GESTOR WHERE DD_TGE_CODIGO IN ('GEXT', 'PROC')
    )
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, L_GEST(I).ASU_ID, L_GEST(I).GEXT, L_GEST(I).TGE_GEXT, 0, C_USUCNV, SYSDATE, 0
    );

    FORALL I IN 1..L_GEST.COUNT
    INSERT INTO GAA_GESTOR_ADICIONAL_ASUNTO (GAA_ID, ASU_ID, USD_ID, DD_TGE_ID, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
    VALUES (
      S_GAA_GESTOR_ADICIONAL_ASUNTO.NEXTVAL, L_GEST(I).ASU_ID, L_GEST(I).PROC, L_GEST(I).TGE_PROC, 0, C_USUCNV, SYSDATE, 0
    );
    ...
    VALUES(
      S_GAH_GESTOR_ADIC_HISTORICO.NEXTVAL, L_GEST(I).ASU_ID, L_GEST(I).GEXT, L_GEST(I).TGE_GEXT, TRUNC(SYSDATE), 0, C_USUCNV, SYSDATE, 0
    );

    FORALL I IN 1..L_GEST.COUNT
    INSERT INTO GAH_GESTOR_ADICIONAL_HISTORICO (GAH_ID, GAH_ASU_ID, GAH_GESTOR_ID, GAH_TIPO_GESTOR_ID, GAH_FECHA_DESDE, VERSION, USUARIOCREAR, FECHACREAR,BORRADO)
    VALUES(
      S_GAH_GESTOR_ADIC_HISTORICO.NEXTVAL, L_GEST(I).ASU_ID, L_GEST(I).PROC, L_GEST(I).TGE_PROC, TRUNC(SYSDATE), 0, C_USUCNV, SYSDATE, 0
    );
