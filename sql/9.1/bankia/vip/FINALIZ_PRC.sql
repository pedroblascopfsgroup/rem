...
-- FINALIZAMOS TAREAS (TAR_TAREAS_NOTIFICACIONES) QUE CONTENGAN EL PRC_ID QUE TENEMOS EN PROCEDIMIENTOS_A_CERRAR (FIN_AUX_PRC_CERRAR_FINAL)
  ...
  WHERE TAR.TAR_ID IN (
    ...
    INNER JOIN BANK01.FIN_AUX_PRC_CERRAR_FINAL CER
		ON PRC.ASU_ID = CER.ASU_ID
    INNER JOIN BANK01.TAR_TAREAS_NOTIFICACIONES TAR 
...
-- INDICAMOS LA DECISION FINALIZAR EN DECISIONES PROCEDIMIENTOS QUE CONTENGAN EL PRC_ID QUE TENEMOS EN PROCEDIMIENTOS_A_CERRAR (FIN_AUX_PRC_CERRAR_FINAL)
  ...
  INSERT INTO BANK01.DPR_DECISIONES_PROCEDIMIENTOS (DPR_ID, PRC_ID, DPR_FINALIZA, DPR_PARALIZA, DD_EDE_ID, VERSION, USUARIOCREAR, FECHACREAR, BORRADO, DD_DFI_ID)
  SELECT 
  BANK01.S_DPR_DEC_PROCEDIMIENTOS.NEXTVAL,
  PRC_ID,
  1,
  0,
  DD_EDE_ID, 
  ...
  FROM BANK01.PRC_PROCEDIMIENTOS PRC
	  INNER JOIN BANK01.FIN_AUX_PRC_CERRAR_FINAL CER 
		ON PRC.ASU_ID = CER.ASU_ID
	  INNER JOIN BANK01.DD_DFI_DECISION_FINALIZAR DFI ON DFI.DD_DFI_DESCRIPCION = 'Venta de cartera'
	  INNER JOIN BANKMASTER.DD_EDE_ESTADOS_DECISION EDE ON EDE.DD_EDE_CODIGO = '02'
	;
