...
--## PRODUCTO=NO
--## 
--## Finalidad: ARREGLAR SECUENCIAS EN CMMASTER
--##                   
...
EXECUTE IMMEDIATE 'CREATE SEQUENCE '|| V_ESQUEMA ||'.S_RULE_DEFINITION INCREMENT BY 1 START WITH 1000      MINVALUE 1      MAXVALUE 999999999999999999999999999      NOCACHE       NOCYCLE ;';
...
EXECUTE IMMEDIATE 'select  '|| V_ESQUEMA_M ||'.'||SECUENCIA||'.nextval from dual' INTO V_NEXT1;


IF V_NEXT1 = MIN_VALOR THEN


V_MSQL3 := 'select '|| V_ESQUEMA_M ||'.'||SECUENCIA||'.CURRVAL from dual';
...
EXECUTE IMMEDIATE 'alter sequence '|| V_ESQUEMA_M ||'.'||SECUENCIA||' increment by -1';


EXECUTE IMMEDIATE 'select  '|| V_ESQUEMA_M ||'.'||SECUENCIA||'.nextval from dual' INTO V_NEXT2;
...
V_MSQL3 := 'select '|| V_ESQUEMA_M ||'.'||SECUENCIA||'.CURRVAL from dual';
 ...
 V_MSQL1 := 'SELECT MAX('||CAMPO_ID||') FROM '|| V_ESQUEMA_M ||'.'||TABLA||'';
 ...
 ELSIF SEQ_ACTUAL >= MAX_VALOR THEN
  ...
  EXECUTE IMMEDIATE 'alter sequence '|| V_ESQUEMA_M ||'.'||SECUENCIA||' increment by '||TOTAL||'';
  
    EXECUTE IMMEDIATE 'select '|| V_ESQUEMA_M ||'.'||SECUENCIA||'.nextval from dual' INTO V_NEXT3;
