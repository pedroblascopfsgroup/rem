		...
		CREATE VIEW '||V_ESQUEMA||'.'||V_NOMBRE_VISTA|| ' AS
			...
			,CNT_MAS_VENCIDO AS (
			  SELECT CEX.EXP_ID
			    ...
			    ,ROW_NUMBER() OVER (PARTITION BY CEX.EXP_ID ORDER BY (SYSDATE-MOV.MOV_FECHA_POS_VENCIDA) DESC) NUMERO
			  FROM '||V_ESQUEMA||'.CEX_CONTRATOS_EXPEDIENTE CEX
			    INNER JOIN (SELECT CNT_ID, CNT_FECHA_EXTRACCION FROM '||V_ESQUEMA||'.CNT_CONTRATOS WHERE BORRADO = 0) CNT ON CNT.CNT_ID = CEX.CNT_ID
            INNER JOIN (SELECT CNT_ID,  MOV_FECHA_EXTRACCION, MOV_FECHA_POS_VENCIDA FROM '||V_ESQUEMA||'.MOV_MOVIMIENTOS) MOV ON MOV.CNT_ID = CNT.CNT_ID AND MOV.MOV_FECHA_EXTRACCION = CNT.CNT_FECHA_EXTRACCION
			  WHERE CEX.BORRADO = 0
			)
			...
			,RIESGO_EXP AS (
			  ...
			  FROM '||V_ESQUEMA||'.CEX_CONTRATOS_EXPEDIENTE CEX
			    INNER JOIN (SELECT CNT_ID, CNT_FECHA_EXTRACCION FROM '||V_ESQUEMA||'.CNT_CONTRATOS WHERE BORRADO = 0) CNT ON CNT.CNT_ID = CEX.CNT_ID
            INNER JOIN (SELECT CNT_ID,  MOV_FECHA_EXTRACCION, MOV_RIESGO FROM '||V_ESQUEMA||'.MOV_MOVIMIENTOS) MOV ON MOV.CNT_ID = CNT.CNT_ID AND MOV.MOV_FECHA_EXTRACCION = CNT.CNT_FECHA_EXTRACCION
			  WHERE CEX.BORRADO = 0
			  GROUP BY CEX.EXP_ID
			)
