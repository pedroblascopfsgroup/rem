...
CREATE OR REPLACE PROCEDURE EXP_SALDO_IRREGULAR (RESULT_EXE OUT VARCHAR2 ) AUTHID CURRENT_USER IS
	...
	CURSOR C_CONSULTA IS
		...
		(select max(mov_pos_viva_vencida) 
...
BEGIN 
   ...
   LOOP
		...
		EXECUTE IMMEDIATE V_SQL_INSERT USING V_CONS.EXP_ID, V_DD_EST_ID, V_DD_EIN_ID, V_DD_STA_ID, V_TAR_CODIGO, V_TAR_TAREA, V_TAR_DESCRIPCION, V_TAR_EMISOR, V_USUARIOCREAR;
		DBMS_OUTPUT.PUT_LINE('CREADA NOTIFICACION PARA EXPEDIENTE: ' || V_CONS.EXP_ID);
    END LOOP;
