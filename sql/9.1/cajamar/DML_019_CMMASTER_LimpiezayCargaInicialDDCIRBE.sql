...
--## PRODUCTO=NO
--## 
--## Finalidad: Inserción de datos de Diccionarios CIRBE , esquema CMMASTER.
...
--    )*/
   ...
   V_CRC T_ARRAY_CRC := T_ARRAY_CRC(
                                     T_CRC('0000','SIN INFORMAR CLASE RIESGO CIRBE','SIN INFORMAR CLASE RIESGO CIRBE')
...
--    )*/
   ...
   V_TPC T_ARRAY_TPC := T_ARRAY_TPC(
                                     T_TPC('0000','SIN INFORMAR TIPO PRODUCTO CIRBE','SIN INFORMAR TIPO PRODUCTO CIRBE') 
...
--	)*/
   ...
   V_DIC T_ARRAY_DIC := T_ARRAY_DIC(
                                     T_DIC('0000','SIN INFORMAR DIVISA CIRBE','SIN INFORMAR DIVISA CIRBE')   
...
--	)*/
   ...
   V_VCI T_ARRAY_VCI := T_ARRAY_VCI(
                                     T_VCI('0000','SIN INFORMAR VENCIMIENTO CIRBE','SIN INFORMAR VENCIMIENTO CIRBE') 
...
--	)*/
   ...
   V_GAC T_ARRAY_GAC := T_ARRAY_GAC(
                                     T_GAC('0000',' SIN INFORMAR GARANTIA CIRBE','SIN INFORMAR GARANTIA CIRBE')
...
--	)*/
   ...
   V_SIC T_ARRAY_SIC := T_ARRAY_SIC(
                                   T_SIC('0000','SIN INFORMAR SITUACION CIRBE','SIN INFORMAR SITUACION CIRBE')
...
BEGIN
    if V_ESQUEMA_MASTER not like '%TER' then
        DBMS_OUTPUT.PUT_LINE('ATENCION, debe ejecutarse en CMMASTER');
    end if;
    ...
    EXECUTE IMMEDIATE('DELETE FROM '|| V_ESQUEMA_MASTER ||'.DD_TPC_TIPO_PRODUCTO_CIRBE');
   ...
   DBMS_OUTPUT.PUT_LINE('Creando DD_TPC_TIPO_PRODUCTO_CIRBE......');
   ...
   LOOP
      ...
      V_MSQL := 'INSERT INTO '||V_ESQUEMA_MASTER||'.DD_TPC_TIPO_PRODUCTO_CIRBE (DD_TPC_ID, DD_TPC_CODIGO, DD_TPC_DESCRIPCION,' ||
        'DD_TPC_DESCRIPCION_LARGA, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
