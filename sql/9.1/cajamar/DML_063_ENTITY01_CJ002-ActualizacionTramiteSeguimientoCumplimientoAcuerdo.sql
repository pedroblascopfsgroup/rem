...
BEGIN
	 ...
	 * ---------------------------------------------------------------------------------------------------------
	 */
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.DD_PTP_PLAZOS_TAREAS_PLAZAS SET DD_PTP_PLAZO_SCRIPT = ''damePlazo(dameFechaPago() == null ? valores[''''CJ002_RegistrarAcuerdoAprobado''''][''''fecha''''] : dameFechaPago())'' WHERE TAP_ID = (SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''CJ002_RegistrarCumplimiento'')';
	
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_VALOR_INICIAL = ''dameFechaPago() == null ? valores[''''CJ002_RegistrarAcuerdoAprobado''''][''''fecha''''] : dameFechaPago()'' WHERE TFI_NOMBRE = ''fechaPago'' AND TAP_ID = (SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''CJ002_RegistrarCumplimiento'')';
