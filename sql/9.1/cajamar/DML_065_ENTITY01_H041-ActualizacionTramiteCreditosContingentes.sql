...
BEGIN
	 ...
	 * ---------------------------------------------------------------------------------------------------------
	 */
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO SET TAP_SCRIPT_VALIDACION = ''dameSiguienteFechaVencimiento() == null ? ''''<div align="justify" style="font-size: 8pt; font-family: Arial;"><p style="margin-bottom: 10px">No existen cr&eacute;ditos contingentes pr&oacute;ximos en el tiempo.</p></div>'''' : null'', TAP_SCRIPT_VALIDACION_JBPM=''valores[''''CJ003_RevisarCreditosContingentes''''][''''comboSeguimiento''''] == DDSiNo.SI ? !existeSiguienteFechaVencimiento() ? ''''<div align="justify" style="font-size: 8pt; font-family: Arial;"><p style="margin-bottom: 10px">No existen cr&eacute;ditos contingentes pr&oacute;ximos en el tiempo.</p></div>'''' : null : null'' WHERE TAP_CODIGO = ''CJ003_RevisarCreditosContingentes''';
	
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.DD_PTP_PLAZOS_TAREAS_PLAZAS SET DD_PTP_PLAZO_SCRIPT = ''dameSiguienteFechaVencimiento() == null ? 1*24*60*60*1000L : damePlazo(dameSiguienteFechaVencimiento())'' WHERE TAP_ID = (SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''CJ003_RevisarCreditosContingentes'')';
	
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_VALOR_INICIAL = ''dameSiguienteFechaVencimiento()'' WHERE TFI_NOMBRE = ''fechaPago'' AND TAP_ID = (SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''CJ003_RevisarCreditosContingentes'')';
