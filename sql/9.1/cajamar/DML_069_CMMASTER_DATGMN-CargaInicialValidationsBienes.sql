...
-- Validaciones bienes
             ...
             , T_JBV('bie-01.entityValidator',1,'SELECT CODIGO_ENTIDAD AS ERROR_FIELD, to_char(CODIGO_ENTIDAD) AS ENTITY_CODE  FROM APR_AUX_ABI_BIENES_CONSOL WHERE CODIGO_ENTIDAD <> #TOKEN_ENTITY#',10,2,0)
             , T_JBV('bie-02.entityValidatorRelCnt',1,'SELECT CODIGO_ENTIDAD AS ERROR_FIELD, to_char(CODIGO_ENTIDAD) AS ENTITY_CODE  FROM APR_AUX_ABC_BIECNT_CONSOL WHERE CODIGO_ENTIDAD <> #TOKEN_ENTITY#',11,2,0)
             , T_JBV('bie-03.entityValidatorRelPer',1,'SELECT CODIGO_ENTIDAD AS ERROR_FIELD, to_char(CODIGO_ENTIDAD) AS ENTITY_CODE  FROM APR_AUX_ABC_BIEPER_CONSOL WHERE CODIGO_ENTIDAD <> #TOKEN_ENTITY#',12,2,0)             
             , T_JBV('bie-04.loadDateValidator',1,'SELECT FECHA_EXTRACCION AS ERROR_FIELD, CODIGO_BIEN AS ENTITY_CODE from APR_AUX_ABI_BIENES_CONSOL where FECHA_EXTRACCION <> to_date(#TOKEN_DATE#,''''YYYYMMDD'''') ',10,2,0)
             , T_JBV('bie-05.loadDateValidatorRelCnt',1,'SELECT FECHA_EXTRACCION AS ERROR_FIELD, CODIGO_BIEN AS ENTITY_CODE from APR_AUX_ABC_BIECNT_CONSOL where FECHA_EXTRACCION <> to_date(#TOKEN_DATE#,''''YYYYMMDD'''') ',11,2,0)               
             , T_JBV('bie-06.loadDateValidatorRelPer',1,'SELECT FECHA_EXTRACCION AS ERROR_FIELD, CODIGO_BIEN AS ENTITY_CODE from APR_AUX_ABC_BIEPER_CONSOL where FECHA_EXTRACCION <> to_date(#TOKEN_DATE#,''''YYYYMMDD'''') ',12,2,0)
             ...
             , T_JBV('bie-14.bieCntTipoProductoValidator',1,'SELECT DISTINCT ERROR_FIELD as DD_TPR_CODIGO, ''''Tipo producto pendiente de definir (''''||ERROR_FIELD||'''')'''' as DD_TPR_DESCRIPCION, ''''Tipo producto pendiente de definir (''''||ERROR_FIELD||'''')'''' as DD_TPR_DESCRIPCION_LARGA   from ( SELECT TIPO_PRODUCTO AS ERROR_FIELD, TIPO_PRODUCTO ||''''-''''|| to_char(CODIGO_BIEN) AS ENTITY_CODE from APR_AUX_ABC_BIECNT_CONSOL  where TIPO_PRODUCTO is not null and not exists (select * from DD_TPR_TIPO_PROD dd where dd.DD_TPR_CODIGO = TIPO_PRODUCTO ) )',11,2,0)                                         
             ...
             , T_JBV('bie-15.bieCntTipoRelacionValidator',1,'SELECT DISTINCT ERROR_FIELD as DD_TBC_CODIGO, ''''Tipo rel contrato bien pendiente de definir (''''||ERROR_FIELD||'''')'''' as DD_TBC_DESCRIPCION, ''''Tipo rel contrato bien pendiente de definir (''''||ERROR_FIELD||'''')'''' as DD_TBC_DESCRIPCION_LARGA  from ( SELECT TIPO_RELACION_CONTRATO_BIEN AS ERROR_FIELD, TIPO_RELACION_CONTRATO_BIEN ||''''-''''|| to_char(CODIGO_BIEN) AS ENTITY_CODE  from APR_AUX_ABC_BIECNT_CONSOL  where TIPO_RELACION_CONTRATO_BIEN is not null  and not exists (select * from DD_TBC_TIPO_REL_BIEN_CNT dd where dd.DD_TBC_CODIGO = TIPO_RELACION_CONTRATO_BIEN ) )',11,2,0)                            
             ...
             , T_JBV('bie-19.biePerBienValidator',1,'select abc.CODIGO_BIEN AS ERROR_FIELD, to_char(abc.CODIGO_BIEN) AS ENTITY_CODE from APR_AUX_ABC_BIEPER_CONSOL   abc left join APR_AUX_ABI_BIENES_CONSOL abi  on abc.codigo_bien = abi.codigo_bien where abi.codigo_bien is null',12,2,0)             
           );   
