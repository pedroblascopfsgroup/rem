...
-- Validaciones bienes
             ...
             , T_JBV('bie-05.loadDateValidatorRelCnt',1,'SELECT FECHA_EXTRACCION AS ERROR_FIELD, TO_CHAR(CODIGO_BIEN) AS ENTITY_CODE from APR_AUX_ABC_BIECNT_CONSOL where FECHA_EXTRACCION <> to_date(#TOKEN_DATE#,''''YYYYMMDD'''') ',11,2,0)               
             , T_JBV('bie-06.loadDateValidatorRelPer',1,'SELECT FECHA_EXTRACCION AS ERROR_FIELD, TO_CHAR(CODIGO_BIEN) AS ENTITY_CODE from APR_AUX_ABC_BIEPER_CONSOL where FECHA_EXTRACCION <> to_date(#TOKEN_DATE#,''''YYYYMMDD'''') ',12,2,0)
