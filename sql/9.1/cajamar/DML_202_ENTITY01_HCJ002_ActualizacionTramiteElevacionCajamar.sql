...
BEGIN
	 ...
	 * ---------------------------------------------------------------------------------------------------------
	 */
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO SET TAP_VIEW = ''plugin/cajamar/tramiteElevacionCajamar/obtenerValidacionComite'' WHERE TAP_CODIGO = ''HCJ002_ObtenerValidacionComite''';
	
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_ERROR_VALIDACION=NULL, TFI_VALIDACION=NULL WHERE TFI_NOMBRE=''comboSuspension'' AND TAP_ID=(SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO=''HCJ002_ObtenerValidacionComite'')';
