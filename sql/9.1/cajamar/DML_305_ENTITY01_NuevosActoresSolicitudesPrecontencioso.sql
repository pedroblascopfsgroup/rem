...
-- tipo actor NOTARI CON ACCESO a recovery
...
IF V_NUM_TABLAS = 0 THEN
    ...
    EXECUTE IMMEDIATE 'INSERT INTO ' || V_ESQUEMA || '.' || V_DDNAME || ' (DD_PCO_DSA_ID, DD_PCO_DSA_CODIGO, DD_PCO_DSA_DESCRIPCION, DD_PCO_DSA_DESCRIPCION_LARGA, DD_PCO_DSA_ACCESO_RECOVERY, VERSION, USUARIOCREAR, FECHACREAR) VALUES '
     || ' ('||V_ESQUEMA||'.S_DD_PCO_DOC_SOLICIT_ACTOR.nextval, ''NOTARI'', ''Notaria'', ''Notaria'', 1, 0, ''PCO'', sysdate) ';
ELSE
    ...
    IF V_NUM_TABLAS = 0 THEN
        DBMS_OUTPUT.PUT_LINE('[INFO] se actualiza el actor notaria como que tiene acceso a recovery');
        V_SQL := 'UPDATE ' || V_ESQUEMA || '.' || V_DDNAME || ' SET DD_PCO_DSA_ACCESO_RECOVERY=1 WHERE DD_PCO_DSA_CODIGO = ''NOTARI''';
