...
where f.fun_descripcion='GENERAR_DOC_PRECONTENCIOSO' 
and p.pef_id in (SELECT PEF_ID from ]' || V_TABLENAME2 || q'[ where fun_id in (select FUN_ID from ]' || V_TABLENAME1 || q'[ where fun_descripcion='ACCIONES_PRECONTENCIOSO')) 
and f.borrado=0 and p.borrado=0) nuevo_fp
