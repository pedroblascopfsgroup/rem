...
BEGIN
	...
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.PCO_PRC_HEP_HISTOR_EST_PREP WHERE usuariocrear = ''SINCRO_CM_HAYA_PARALIZADO'' '
			|| ' AND TRUNC(pco_prc_hep_fecha_incio) = ''31/12/2050'' ';
	...
	IF V_NUM_TABLAS > 0 THEN	  
	  ...
	  V_MSQL := 'UPDATE '||V_ESQUEMA||'.PCO_PRC_HEP_HISTOR_EST_PREP set pco_prc_hep_fecha_incio = sysdate, ' ||
			' pco_prc_hep_fecha_fin = ''31/12/2050'' '||
			' where usuariocrear= ''SINCRO_CM_HAYA_PARALIZADO'' ' ||
			' AND TRUNC(pco_prc_hep_fecha_incio) = ''31/12/2050'' ';

	  EXECUTE IMMEDIATE V_MSQL;
        ELSE
	
          DBMS_OUTPUT.PUT_LINE('[INFO] Update ejecutado ');
