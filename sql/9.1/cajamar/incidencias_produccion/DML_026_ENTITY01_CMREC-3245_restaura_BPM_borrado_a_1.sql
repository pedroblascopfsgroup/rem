...
DECLARE
  ...
  FROM CM01.PRC_PROCEDIMIENTOS PRC
     inner join CM01.asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 1
...
BEGIN
  ...
  FROM CM01.TEX_TAREA_EXTERNA TEX
    ...
    inner join CM01.asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 1
  where tex.USUARIOCREAR = 'MIGRACM01PCO'
       AND PRC.PRC_ID in 
                                          (select distinct prc.prc_ID
                                           from CM01.MIG_EXPEDIENTES_CABECERA mig
                                             inner join CM01.asu_asuntos asu  on mig.cd_expediente = asu.asu_id_externo
  ...
  )and tex2.USUARIOCREAR = 'MIGRACM01PCO';  
