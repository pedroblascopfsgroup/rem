...
--##            0.3 Ajustes para HAYA y litigios.
...
DECLARE
  ...
  WHERE PRC.PRC_PROCESS_BPM IS NULL
    AND PRC.PRC_ID IN ( SELECT distinct prc.prc_id
                    FROM CM01.PRC_PROCEDIMIENTOS PRC
                      inner join CM01.asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 2
                      JOIN cm01.TAR_TAREAS_NOTIFICACIONES TAR ON PRC.PRC_ID = TAR.PRC_ID
                      JOIN CM01.TEX_TAREA_EXTERNA TEX ON TAR.TAR_ID = TEX.TAR_ID
                      JOIN CM01.TAP_TAREA_PROCEDIMIENTO TAP ON TEX.TAP_ID = TAP.TAP_ID
                    WHERE PRC.PRC_PROCESS_BPM IS NULL
                      and (tar.TAR_TAREA_FINALIZADA IS NULL OR tar.TAR_TAREA_FINALIZADA = 0) AND tar.BORRADO = 0
                      AND tap.tap_codigo = 'H009_RegistrarPublicacionBOE'
     )
  ORDER BY PRC_ID;
