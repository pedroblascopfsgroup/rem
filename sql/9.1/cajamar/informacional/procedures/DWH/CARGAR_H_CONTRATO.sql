 ...
 */   
    ...
    if((dia_periodo_ant <= max_dia_h) or ((dia_periodo_ant > max_dia_h) and (dia_periodo_ant < penult_dia_mov))) then
       --execute immediate 'update TMP_H_CNT set NUM_DIAS_VENC_PERIODO_ANT = (select NVL((MOV_FECHA_EXTRACCION - MOV_FECHA_POS_VENCIDA),0) from '||V_CM01||'.H_MOV_MOVIMIENTOS where MOV_FECHA_EXTRACCION = '''||dia_periodo_ant||''' and CNT_ID = CONTRATO_ID) where DIA_ID = '''||fecha||'''';
       execute immediate 'merge into TMP_H_CNT hc
              using (select NVL((MOV_FECHA_EXTRACCION - MOV_FECHA_POS_VENCIDA),0) as NUM_DIAS, CNT_ID from '||V_CM01||'.H_MOV_MOVIMIENTOS where MOV_FECHA_EXTRACCION = '''||dia_periodo_ant||''') thc
