...
--## INCIDENCIA_LINK=CMREC-3166
--## PRODUCTO=NO
--## 
--## Finalidad: ambio  FECHA_PUBLICACION_BOE
...
-- =========================================================================================================================================
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
            (select distinct DD_TPO_ID
             ...
             where DD_TPO_DESCRIPCION = ''P. verbal - HCJ'' or DD_TPO_DESCRIPCION = ''P. ordinario - HCJ'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID';
  commit;
 ...
 V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_EJEC_ORD_JERARQUIA'', '''', :O_ERROR_STATUS); END;';
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
                  (select distinct DD_TPO_ID
                   from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
                   join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
                   where DD_TPO_DESCRIPCION = ''T. de investigación judicial - HCJ'' or  DD_TPO_DESCRIPCION = ''P. Ej. de título no judicial - HCJ'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID ';
...
-- Borrar Indices TMP_HIPO_JERARQUIA
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
    (select distinct DD_TPO_ID
       from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
       join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
       where DD_TPO_DESCRIPCION = ''P. hipotecario - HCJ'' or DD_TPO_DESCRIPCION like ''T. de subasta%'' or DD_TPO_DESCRIPCION = ''T. Subsanación decreto adjudicación - HCJ'' or DD_TPO_DESCRIPCION = ''T. de cesión de remate - HCJ'' ) b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID ';
...
-- Borrar Indices TMP_MON_JERARQUIA
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
              (select distinct DD_TPO_ID from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
               join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
               where DD_TPO_DESCRIPCION = ''P. Monitorio - HCJ'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID ';
  V_ROWCOUNT := sql%rowcount;
  commit;
...
-- =========================================================================================================================================
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
                  (select DD_TPO_ID from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
                   join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
                   where DD_TPO_DESCRIPCION = ''T. de ejecución notarial - HCJ'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID';
  V_ROWCOUNT := sql%rowcount;
  commit;
