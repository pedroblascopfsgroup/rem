...
DBMS_OUTPUT.PUT_LINE('[INICIO]');
    ...
    V_TMP_TIPO_TGE := V_TIPO_TFA(I);
		...
		V_MSQL := 'insert into '|| V_ESQUEMA ||'.GAA_GESTOR_ADICIONAL_ASUNTO (GAA_ID, ASU_ID, USD_ID, DD_TGE_ID, USUARIOCREAR, FECHACREAR)'
			...
			|| '       (select usd_id from '|| V_ESQUEMA ||'.usd_usuarios_despachos usd inner join '||V_ESQUEMA_M||'.usu_usuarios usu on usu.usu_id = usd.usu_id where usu.usu_username = '''||V_TMP_TIPO_TGE(2)||''') usd_id,'
