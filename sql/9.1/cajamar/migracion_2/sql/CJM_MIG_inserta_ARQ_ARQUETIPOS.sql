...
--MRA_REL_MODELO_ARQ
  V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.MRA_REL_MODELO_ARQ (MRA_ID,MOA_ID,LIA_ID,MRA_NIVEL,ITI_ID,MRA_PRIORIDAD,USUARIOCREAR, FECHACREAR,MRA_PLAZO_DISPARO) VALUES ('||V_ESQUEMA||'.S_MRA_REL_MODELO_ARQ.nextval,(SELECT MAX(MOA_ID) FROM '||V_ESQUEMA||'.MOA_MODELOS_ARQ WHERE MOA_NOMBRE = ''Modelo Migracion'') ,(SELECT LIA_ID FROM '||V_ESQUEMA||'.LIA_LISTA_ARQUETIPOS WHERE LIA_NOMBRE = ''GR. Migracion''),0,(SELECT ITI_ID FROM '||V_ESQUEMA||'.ITI_ITINERARIOS WHERE ITI_NOMBRE = ''Migracion''), 1,''PFS-CONF'',sysdate,90)';
     EXECUTE IMMEDIATE V_MSQL;
