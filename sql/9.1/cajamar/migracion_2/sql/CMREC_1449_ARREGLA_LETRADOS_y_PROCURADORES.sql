...
DECLARE
        ...
        v_procurador CM01.MIG_PROCEDIMIENTOS_CABECERA.CD_PROCURADOR%TYPE;
        v_usuario_id CMMASTER.USU_USUARIOS.USU_ID%TYPE;
	v_letrado    CM01.MIG_PROCEDIMIENTOS_ACTORES.CD_ACTOR%TYPE;

        V_MSQL  VARCHAR2(2500);
        
        V_ESQUEMA    VARCHAR2(25 CHAR):= 'CM01';
        V_ESQUEMA_MASTER VARCHAR2(25 CHAR):= 'CMMASTER';
        V_USUARIO VARCHAR2(25 CHAR):= 'MIGRA2CM01';        
...
FOR v_letrado IN (SELECT DISTINCT CD_ACTOR FROM MIG_PROCEDIMIENTOS_ACTORES WHERE TIPO_ACTOR = 1) LOOP
  ...
  V_MSQL:= 'insert into '||V_ESQUEMA_MASTER||'.usu_usuarios (
   ...
   values  ( 
             '||V_ESQUEMA_MASTER||'.s_usu_usuarios.nextval
            , (select ID from '||V_ESQUEMA_MASTER||'.ENTIDAD where DESCRIPCION = ''CAJAMAR'')
            , ''val.LETRADO_'||v_letrado.CD_ACTOR||'''
            , ''1234''
            , ''Letrado LETRADO_'||v_letrado.CD_ACTOR||'''
            , ''''
            , ''''
            , '''' 
            , '''||V_USUARIO||'''
	...
	INTO V_EXISTE
	FROM CMMASTER.USU_USUARIOS
	WHERE USU_USERNAME = 'val.LETRADO_'||v_letrado.CD_ACTOR;
...
-- Insertamos la relación de usuarios despachos para procuradores USD_USUARIOS_DESPACHOS , .DES_DESPACHO_EXTERNO      
FOR v_usuario_id IN (SELECT USU_ID FROM CMMASTER.USU_USUARIOS WHERE UPPER(USU_USERNAME) LIKE 'VAL.LETRADO%') LOOP
  ...
  V_MSQL:= 'insert into '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS  
        ...
        values (
                '||V_ESQUEMA||'.s_usd_usuarios_despachos.nextval
              , '||v_usuario_id.USU_ID||'
              , (select des.des_id from '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO des WHERE des.borrado = 0 and des.DES_DESPACHO = ''Despacho Letrado'')
              , 0
              , 0 
              , '''||V_USUARIO||'''
              , sysdate 
  ...
  FROM CM01.USD_USUARIOS_DESPACHOS
  WHERE USU_ID = v_usuario_id.USU_ID
    AND DES_ID = (select des.des_id from CM01.DES_DESPACHO_EXTERNO des WHERE des.borrado = 0 and des.DES_DESPACHO = 'Despacho Letrado');
  
  IF V_EXISTE = 0 THEN
     EXECUTE IMMEDIATE V_MSQL;
     DBMS_OUTPUT.PUT_LINE('Insertando el despacho para el usuario LETRADO '||v_usuario_id.USU_ID||'');
  ELSE
     DBMS_OUTPUT.PUT_LINE('El despacho para el usuario LETRADO '||v_usuario_id.USU_ID||' YA EXISTE!');
  END IF;
     ...
     (
      ...
      from (
          select distinct asu.asu_id, usd.usd_id,
                 rank() over (partition by asu.asu_id order by usd.usu_id) as ranking
          from '||V_ESQUEMA||'.asu_asuntos asu                                                                inner join 
               '||V_ESQUEMA||'.mig_procedimientos_ACTORES  migp on migp.cd_procedimiento = asu.asu_id_externo inner join
               '||V_ESQUEMA||'.des_despacho_externo        des  on des.des_despacho = ''Despacho Letrado''    inner join 
               '||V_ESQUEMA||'.usd_usuarios_despachos      usd  on usd.des_id = des.des_id                    inner join
               '||V_ESQUEMA_MASTER||'.usu_usuarios         usu  on usu.usu_id = usd.usu_id and usu.USU_EXTERNO = 1 AND MIGP.TIPO_aCTOR = 1 
