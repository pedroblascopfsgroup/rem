...
DECLARE
    ...
    *---------------------------------------------------------------------
    */
    PAR_TIT_TRAMITE VARCHAR2(50 CHAR)   := 'Trámite de subasta concursal';   -- [PARAMETRO] Título del trámite
    ...
    */    
    ...
    V_COD_PROCEDIMIENTO VARCHAR(10 CHAR) := 'CJ004'; -- Código de procedimiento para reemplazar
...
BEGIN
	 ...
	 * ---------------------------------------------------------------------------------------------------------
	 */
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO SET DD_TPO_ID_BPM=(SELECT DD_TPO_ID FROM '||PAR_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO=''CJ105'') WHERE TAP_CODIGO=''CJ004_BPMTramiteAdjudicacion''';
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO SET DD_TPO_ID_BPM=(SELECT DD_TPO_ID FROM '||PAR_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO=''CJ106'') WHERE TAP_CODIGO=''CJ004_BPMTramiteCesionRemate''';
