...
BEGIN
	...
	OPEN ASUNTOS_CURSOR FOR 'SELECT a.ASU_ID ' ||
							...
							'usd.USU_ID IN (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''LETRADO'')';
	...
	TOTAL_INSERTS := 0;
 	LOOP
 		...
 		OPEN USUARIOS_CURSOR FOR 'SELECT tg.DD_TGE_ID, ud.USD_ID, u.USU_USERNAME, u.USU_ID ' ||
							...
							' WHERE U.USU_USERNAME IN (''CJ_DRECU'', ''CJ_GAREO'', ''CJ_GAEST'', ''CJ_GAFIS'', ''CJ_GAJUR'', ''CJ_GCON'', ''CJ_GCONGE'', ''CJ_GCONPR'' ' ||
							' , ''CJ_GCTRGE'', ''CJ_GGESDOC'', ''CJ_GESTILL'', ''CJ_LETRADO'', ''CJ_SAREO'', ''CJ_SAEST'', ''CJ_SFIS'', ''CJ_SAJUR'', ''CJ_SCON'', ''CJ_SUCONT'' ' ||
							' , ''CJ_SUCONGE'', ''CJ_SUCONPR'', ''CJ_SCTRGE'', ''CJ_SGESDOC'', ''CJ_SPGL'' )';
 	 	LOOP
