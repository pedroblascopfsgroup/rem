...
--## PRODUCTO=NO
--##
--## Finalidad: Relaciona perfil con funcion
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
BEGIN	
    ...
    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE FUN_ID = (SELECT FUN_ID FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''ROLE_RECOVERY_BI_HAYA'')';
	...
	ELSE
		V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF' ||
					...
					' WHERE FUN.FUN_DESCRIPCION = ''ROLE_RECOVERY_BI_HAYA'' ' || 
					' AND PEF.PEF_CODIGO = ''FPFSRHAYBI'' ';
