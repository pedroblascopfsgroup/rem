...
DECLARE
    ...
    *---------------------------------------------------------------------
    */
    PAR_TIT_TRAMITE VARCHAR2(50 CHAR)   := 'Procedimiento monitorio';   -- [PARAMETRO] Título del trámite
...
BEGIN
	 ...
	 * Actualizaciones 
	 */
	--EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO SET DD_TPO_DESCRIPCION=''T. de ejecución notarial - HCJ'', DD_TPO_XML_JBPM=''hcj_tramiteEjecucionNotarial'' WHERE DD_TPO_CODIGO = '''||V_COD_PROCEDIMIENTO||'''';
	EXECUTE IMMEDIATE 'DELETE FROM '||PAR_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS WHERE TFI_NOMBRE IN (''fechaElevacion'', ''comboResultadoComite'', ''observaciones'') AND TAP_ID  = (SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''H036_ElevarPropuestaAComite'')';
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO SET DD_TPO_ID_BPM = (SELECT DD_TPO_ID FROM '||PAR_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO = ''HCJ002'') WHERE TAP_CODIGO = ''H036_ElevarPropuestaAComite''';
