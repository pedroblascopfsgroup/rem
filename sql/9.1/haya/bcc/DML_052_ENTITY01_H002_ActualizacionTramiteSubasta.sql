...
DECLARE
    ...
    *---------------------------------------------------------------------
    */
    PAR_TIT_TRAMITE VARCHAR2(50 CHAR)   := 'Trámite de subastas';   -- [PARAMETRO] Título del trámite
    ...
    */    
    ...
    V_COD_PROCEDIMIENTO VARCHAR(10 CHAR) := 'H002'; -- Código de procedimiento para reemplazar
...
BEGIN
	 ...
	 * ---------------------------------------------------------------------------------------------------------
	 */
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO SET TAP_SCRIPT_VALIDACION_JBPM = null WHERE TAP_CODIGO = ''H004_SenyalamientoSubasta''';
	
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_NOMBRE = ''comboSuspension'' WHERE TFI_NOMBRE = ''comboMotivo'' AND TAP_ID = (SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''H004_ObtenerValidacionComite'')';
	EXECUTE IMMEDIATE 'UPDATE '||PAR_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_NOMBRE = ''comboSuspension'' WHERE TFI_NOMBRE = ''comboMotivo'' AND TAP_ID = (SELECT TAP_ID FROM '||PAR_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''H002_ObtenerValidacionComite'')';
