...
BEGIN	
	...
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''TAB_MODIFICAR_CONTRATO_OTROS''';
	DBMS_OUTPUT.PUT_LINE(V_SQL);
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
	...
	ELSE
		V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.FUN_FUNCIONES (FUN_ID,FUN_DESCRIPCION_LARGA,FUN_DESCRIPCION,VERSION,USUARIOCREAR,FECHACREAR,USUARIOMODIFICAR,FECHAMODIFICAR,USUARIOBORRAR,FECHABORRAR,BORRADO) VALUES ('||V_ESQUEMA_M||'.S_FUN_FUNCIONES.nextVal, ''Modificar Riesgo Operacional pestaña OTROS de la ficha contrato'',''TAB_MODIFICAR_CONTRATO_OTROS'',0,''PRODUCTO-459'',sysdate,null,null,null,null,0)';  
		DBMS_OUTPUT.PUT_LINE(V_MSQL);
		execute immediate V_MSQL;
	  	DBMS_OUTPUT.PUT_LINE('OK modificado');
	END IF;
