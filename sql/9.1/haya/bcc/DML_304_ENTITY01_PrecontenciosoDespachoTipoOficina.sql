...
-- Insertar despacho tipo oficina

V_SQL := 'SELECT DD_TDE_ID FROM ' || V_ESQUEMA_MASTER || '.DD_TDE_TIPO_DESPACHO WHERE DD_TDE_CODIGO = ''OFICINA''';
EXECUTE IMMEDIATE V_SQL INTO V_DD_TDE_ID_OFI;

DBMS_OUTPUT.PUT_LINE('[INFO] Se consulta si existe un DES_DESPACHO_EXTERNO con tipo oficina');

V_SQL := 'SELECT COUNT(1) FROM ' || V_ESQUEMA || '.DES_DESPACHO_EXTERNO WHERE DD_TDE_ID = ' || V_DD_TDE_ID_OFI;
EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

IF V_NUM_TABLAS = 0 THEN
	DBMS_OUTPUT.PUT_LINE('[INFO] INSERTANDO DES_DESPACHO_EXTERNO con tipo oficina');

	V_MSQL := 'INSERT INTO ' || V_ESQUEMA || '.DES_DESPACHO_EXTERNO (DES_ID, DES_DESPACHO, VERSION, USUARIOCREAR, FECHACREAR, DD_TDE_ID) VALUES (' || V_ESQUEMA || '.S_DES_DESPACHO_EXTERNO.NEXTVAL, ''Despacho Oficina'', 0, ''PCO'', sysdate, ' || V_DD_TDE_ID_OFI || ')'; 
	EXECUTE IMMEDIATE V_MSQL;
END IF;
