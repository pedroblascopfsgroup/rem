...
--## PRODUCTO=NO
--## 
--## Finalidad: Se inserta CAJAMAR en DD_GES_GESTION_ASUNTO si no existe.
--##                   
...
--*/
 ...
 V_TABLADD1 VARCHAR(30) :='DD_GES_GESTION_ASUNTO';
 err_num NUMBER;
...
-- DEFINICION CURSORES.
 ...
 V_GES T_ARRAY_GES := T_ARRAY_GES(
    T_GES('CAJAMAR','CAJAMAR','CAJAMAR.','0','DD','SYSDATE','0')
 );
...
BEGIN 
 ...
 V_EXISTE:=0; 
 V_MSQL1 := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||V_TABLADD1||' WHERE DD_GES_CODIGO = ''CAJAMAR''';
 ...
 LOOP
   ...
   DBMS_OUTPUT.PUT_LINE('Creando '||V_TABLADD1||': '||V_TMP_GES(1));   
        ...
        V_MSQL1 := 'INSERT INTO ' ||V_ESQUEMA|| '.'||V_TABLADD1||' 
                    (DD_GES_ID, DD_GES_CODIGO, DD_GES_DESCRIPCION, DD_GES_DESCRIPCION_LARGA, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
   ...
   ELSE   
      DBMS_OUTPUT.PUT_LINE('YA EXISTE EL DD_GES_CODIGO = CAJAMAR EN LA TABLA '||V_TABLADD1||'');
   END IF;     
