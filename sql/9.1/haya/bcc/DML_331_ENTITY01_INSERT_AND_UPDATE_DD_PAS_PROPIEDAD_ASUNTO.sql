...
BEGIN
	...
	ELSE
		...
		DBMS_OUTPUT.PUT_LINE('[INFO] Registro SAREB borrado en '||V_ESQUEMA||'.DD_PAS_PROPIEDAD_ASUNTO');
		
	END IF;
	
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_PAS_PROPIEDAD_ASUNTO WHERE DD_PAS_CODIGO = ''BANKIA'' AND BORRADO = 1';
    DBMS_OUTPUT.PUT_LINE(V_SQL);
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    IF V_NUM_TABLAS > 0 THEN	  
		DBMS_OUTPUT.PUT_LINE('[INFO] El registro ya ha sido borrado en la tabla '||V_ESQUEMA||'.DD_PAS_PROPIEDAD_ASUNTO.');
	ELSE
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.DD_PAS_PROPIEDAD_ASUNTO SET BORRADO = 1, USUARIOBORRAR = ''CMREC-1096'', FECHABORRAR= SYSDATE WHERE DD_PAS_CODIGO = ''BANKIA''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Registro BANKIA borrado en '||V_ESQUEMA||'.DD_PAS_PROPIEDAD_ASUNTO');
	...
	ELSE
		...
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Registro HAYA insertado en '||V_ESQUEMA||'.DD_PAS_PROPIEDAD_ASUNTO');
	...
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_GES_GESTION_ASUNTO WHERE DD_GES_CODIGO = ''BANKIA'' AND BORRADO = 1';
    DBMS_OUTPUT.PUT_LINE(V_SQL);
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    IF V_NUM_TABLAS > 0 THEN	  
		DBMS_OUTPUT.PUT_LINE('[INFO] El registro ya ha sido borrado en la tabla '||V_ESQUEMA||'.DD_GES_GESTION_ASUNTO.');
	ELSE
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.DD_GES_GESTION_ASUNTO SET BORRADO = 1, USUARIOBORRAR = ''CMREC-1096'', FECHABORRAR= SYSDATE WHERE DD_GES_CODIGO = ''BANKIA''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Registro BANKIA borrado en '||V_ESQUEMA||'.DD_GES_GESTION_ASUNTO');
