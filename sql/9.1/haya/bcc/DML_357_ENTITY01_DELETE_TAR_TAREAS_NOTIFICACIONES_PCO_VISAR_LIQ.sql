...
BEGIN
	...
	V_MSQL := 'DELETE FROM '||V_ESQUEMA||'.TER_TAREA_EXTERNA_RECUPERACION WHERE TEX_ID IN (SELECT TEX_ID FROM '||V_ESQUEMA||'.TEX_TAREA_EXTERNA WHERE TAR_ID IN (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''PCO_VisarLiq''))';
