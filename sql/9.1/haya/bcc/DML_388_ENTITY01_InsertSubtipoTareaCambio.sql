...
BEGIN   
        ...
        END IF;
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO = ''CAMBIO_ACUERDOS'' ';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
        IF V_NUM_TABLAS > 0 THEN                                
          DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE... Ya existe el DD_STA_SUBTIPO_TAREA_BASE PCO_TAR_APP_EXT');
        ELSE
          V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE(DD_STA_ID, DD_TAR_ID, DD_STA_CODIGO, DD_STA_DESCRIPCION, DD_STA_DESCRIPCION_LARGA, DD_STA_GESTOR, VERSION, USUARIOCREAR, FECHACREAR,  USUARIOMODIFICAR, FECHAMODIFICAR, USUARIOBORRAR, FECHABORRAR, BORRADO, DD_TGE_ID, DTYPE) 
                        VALUES('||V_ESQUEMA_M||'.S_DD_STA_SUBTIPO_TAREA_BASE.NEXTVAL, ''1'', ''CAMBIO_ACUERDOS'', ''Notificación automática por alta de propuestas o acuerdo'', ''A través de la presenta notificación se le informa que en estos momentos se está llevando a cabo una propuesta de solución por parte de Cajamar sobre la operación afecta al asunto referencia.'', 1, ''0'', ''DD'', sysdate, null, null, null, null, ''0'', null, ''EXTSubtipoTarea'') ';
              EXECUTE IMMEDIATE V_MSQL;
        END IF;
