...
BEGIN
  ...
  V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_MNC_MTO_CORRECTIVO WHERE DD_MNC_CODIGO = ''PREVIOVAL'' ';
  EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    
  IF V_NUM_TABLAS > 0 THEN    
    DBMS_OUTPUT.PUT_LINE('[INFO] Ya existen el registro en la tabla '||V_ESQUEMA||'.DD_MNC_MTO_CORRECTIVO');
  ELSE    
    V_MSQL := '
	insert into '||V_ESQUEMA||'.DD_MNC_MTO_CORRECTIVO(DD_MNC_CODIGO,DD_MNC_DESCRIPCION)
