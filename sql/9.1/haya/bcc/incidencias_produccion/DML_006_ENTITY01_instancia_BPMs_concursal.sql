...
DECLARE
  ...
  WHERE PRC.PRC_PROCESS_BPM IS NULL
    AND PRC.USUARIOMODIFICAR = 'PARAMIGRAHAYA02PCO'
  ORDER BY PRC_ID;
...
BEGIN
  ...
  where prc.prc_id in (
    ...
    from Prc_Procedimientos prc
      inner join asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 1
    where prc.USUARIOMODIFICAR = 'PARAMIGRAHAYA02PCO'
  ) and prc.USUARIOMODIFICAR = 'PARAMIGRAHAYA02PCO';
  ...
  FROM TEX_TAREA_EXTERNA TEX
    ...
    inner join asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 1 
  where tex.USUARIOMODIFICAR = 'PARAMIGRAHAYA02PCO'
  )and tex2.USUARIOMODIFICAR = 'PARAMIGRAHAYA02PCO';  
