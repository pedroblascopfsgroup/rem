...
BEGIN
  ...
  FROM HAYA02.TEX_TAREA_EXTERNA TEX
    ...
    inner join HAYA02.asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 1
  where tex.USUARIOCREAR = 'MIGRAHAYA02PCO'
       AND PRC.PRC_ID in 
                                          (select distinct prc.prc_ID
                                           from HAYA02.MIG_EXPEDIENTES_CABECERA mig
                                             inner join haya02.asu_asuntos asu  on mig.cd_expediente = asu.asu_id_externo
  ...
  )and tex2.USUARIOCREAR = 'MIGRAHAYA02PCO';  
...
--select count(*) from HAYA02.TMP_ALTA_BPM_INSTANCES;
--select prc_id, count(*) from HAYA02.TMP_ALTA_BPM_INSTANCES group by prc_id having count(*)>1;
-- select * from HAYA02.TMP_ALTA_BPM_INSTANCES where prc_id=1000000000066843;
-- select count(distinct token_padre_id) from HAYA02.TMP_ALTA_BPM_INSTANCES 
...
/*UPDATE HAYA02.tar_tareas_notificaciones
 ...
 WHERE fechacrear > SYSDATE - 1
   ...
   AND USUARIOCREAR = 'MIGRAHAYA02PCO';
