...
--LIA_LISTA_ARQUETIPOS
  V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.LIA_LISTA_ARQUETIPOS (LIA_ID,ITI_ID,LIA_PRIORIDAD,LIA_NOMBRE,USUARIOCREAR,FECHACREAR,LIA_NIVEL,LIA_GESTION,LIA_PLAZO_DISPARO,DD_TSN_ID,RD_ID) VALUES ('||V_ESQUEMA||'.S_LIA_LISTA_ARQUETIPOS.nextval,null,null,''GR. Migracion'',''PFS-CONF'',sysdate,0,1,90,(SELECT DD_TSN_ID  FROM HAYAMASTER.DD_TSN_TIPO_SALTO_NIVEL  WHERE DD_TSN_CODIGO = ''TODOS''),(SELECT RD_ID  FROM '||V_ESQUEMA||'.RULE_DEFINITION  WHERE RD_NAME =''Migracion''))';
     EXECUTE IMMEDIATE V_MSQL;
