...
--##        0.2 Incluimos contros sobre la existencia de Secuencias usadas.
--##########################################
--*/
...
DECLARE
 ...
 flag number;
 TABLA1 VARCHAR(30 char) :='MOA_MODELOS_ARQ'; 
...
--Validamos si la tabla existe antes de crearla
 ...
 --MOA_MODELOS_ARQ
  ...
  WHILE (flag <= 0) LOOP
        V_MSQL1 := 'SELECT '||V_ESQUEMA||'.S_'||TABLA1||'.nextval FROM dual';
      EXECUTE IMMEDIATE V_MSQL1 INTO V_ID;      
      DBMS_OUTPUT.PUT_LINE(''||V_ID||' Pk dada... ');
      
      V_MSQL1 := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||TABLA1||' WHERE MOA_ID = '||V_ID;
      EXECUTE IMMEDIATE V_MSQL1 INTO V_EXISTE;      

      DBMS_OUTPUT.PUT_LINE(''||V_EXISTE||' Existe??... ');
  ...
  V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.MOA_MODELOS_ARQ (MOA_ID,MOA_NOMBRE,MOA_DESCRIPCION,DD_ESM_ID,MOA_OBSERVACIONES,MOA_FECHA_INI_VIGENCIA,MOA_FECHA_FIN_VIGENCIA,USUARIOCREAR,FECHACREAR) VALUES ('||V_ID||',''Modelo Migracion'',''Modelo Migracion'',(SELECT DD_ESM_ID FROM '||V_ESQUEMA||'.DD_ESM_ESTADOS_MODELO  WHERE DD_ESM_DESCRIPCION = ''HISTÓRICO''),null,sysdate,null,''PFS-CONF'', sysdate )';          
     EXECUTE IMMEDIATE V_MSQL;

--ITI_ITINERARIOS
  flag := 0;
  TABLA1 := 'ITI_ITINERARIOS';
  WHILE (flag <= 0) LOOP
        V_MSQL1 := 'SELECT '||V_ESQUEMA||'.S_'||TABLA1||'.nextval FROM dual';
      EXECUTE IMMEDIATE V_MSQL1 INTO V_ID;
      DBMS_OUTPUT.PUT_LINE(''||V_ID||' Pk dada... ');
      
      V_MSQL1 := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||TABLA1||' WHERE ITI_ID = '||V_ID;
      EXECUTE IMMEDIATE V_MSQL1 INTO V_EXISTE;
...
--MRA_REL_MODELO_ARQ
  ...
  V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.MRA_REL_MODELO_ARQ (MRA_ID,MOA_ID,LIA_ID,MRA_NIVEL,ITI_ID,MRA_PRIORIDAD,USUARIOCREAR, FECHACREAR,MRA_PLAZO_DISPARO) VALUES ('||V_ID||',(SELECT MOA_ID FROM '||V_ESQUEMA||'.MOA_MODELOS_ARQ WHERE MOA_NOMBRE = ''Modelo Migracion'') ,(SELECT LIA_ID FROM '||V_ESQUEMA||'.LIA_LISTA_ARQUETIPOS WHERE LIA_NOMBRE = ''GR. Migracion''),0,(SELECT ITI_ID FROM '||V_ESQUEMA||'.ITI_ITINERARIOS WHERE ITI_NOMBRE = ''Migracion''), 1,''PFS-CONF'',sysdate,90)';
     EXECUTE IMMEDIATE V_MSQL;
     
--ARQ_ARQUETIPOS
  flag := 0;
  TABLA1 := 'ARQ_ARQUETIPOS';
  WHILE (flag <= 0) LOOP
        V_MSQL1 := 'SELECT '||V_ESQUEMA||'.S_'||TABLA1||'.nextval FROM dual';
      EXECUTE IMMEDIATE V_MSQL1 INTO V_ID;
      DBMS_OUTPUT.PUT_LINE(''||V_ID||' Pk dada... ');

      V_MSQL1 := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||TABLA1||' WHERE ARQ_ID = '||V_ID;
      EXECUTE IMMEDIATE V_MSQL1 INTO V_EXISTE;
...
--EST_ESTADOS
  ...
  WHILE (flag <= 0) LOOP
      ...
      V_MSQL1 := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||TABLA1||' WHERE EST_ID = '||V_ID;
      EXECUTE IMMEDIATE V_MSQL1 INTO V_EXISTE;
  ...
  V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.EST_ESTADOS (EST_ID,PEF_ID_GESTOR,PEF_ID_SUPERVISOR,TEL_ID,DCA_ID,ITI_ID,DD_EST_ID,EST_TELECOBRO,EST_PLAZO,EST_AUTOMATICO,USUARIOCREAR,FECHACREAR) VALUES ('||V_ID||',(SELECT PEF_ID FROM PEF_PERFILES WHERE PEF_CODIGO = ''SER_CENTRALES''),(SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''DIR_RIESGOS''),null,null,(SELECT ITI_ID FROM '||V_ESQUEMA||'.ITI_ITINERARIOS WHERE ITI_NOMBRE = ''Migracion''), (SELECT DD_EST_ID FROM '||V_ESQUEMA_M||'.DD_EST_ESTADOS_ITINERARIOS WHERE DD_EST_CODIGO = ''CAR''),0,5184000000,null,''PFS-CONF'',sysdate)';
     EXECUTE IMMEDIATE V_MSQL;
