...
--               , (SELECT *
--                    FROM (SELECT BIE_ID, BIE_CODIGO_INTERNO, RANK() OVER (PARTITION BY BIE_CODIGO_INTERNO ORDER BY BIE_ID) AS R1 FROM '||V_ESQUEMA||'.BIE_BIEN  )
--                   WHERE R1 = 1 ) bie
--            WHERE 
...
--
--
--   HAYA02.UTILES.ANALIZA_TABLA('HAYA02','TMP_GUID_PRB_PRC_BIE_CODIGO');   
...
--                           )';       
    ...
    EXECUTE IMMEDIATE('TRUNCATE TABLE '||V_ESQUEMA||'.TMP_GUID_TAR_TAREAS_HRE'); 
     ...
     V_SQL:= 'insert into TMP_GUID_TAR_TAREAS_HRE (SYS_GUID, TAR_ID)
...
--                             , '||V_ESQUEMA||'.TMP_GUID_PRB_PRC_BIE_CODIGO prb
  ...
  --                         AND prchre.PRC_ID     = prb.PRC_ID (+)
                           ...
                           AND tarhre.TAR_ID     = tex.TAR_ID
                           AND tex.TAP_ID        = tap.TAP_ID
                           AND bcc.GUID_ASU_ID_EXTERNO = asuhre.ASU_ID_EXTERNO 
                           AND bcc.GUID_DD_TPO_CODIGO  = dd.DD_TPO_CODIGO
                           AND bcc.GUID_TAP_CODIGO     = tap.TAP_CODIGO
    --                       AND NVL(bcc.GUID_BIE_CODIGO_INTERNO,0) = NVL(prb.BIE_CODIGO_INTERNO,0)                                                      
