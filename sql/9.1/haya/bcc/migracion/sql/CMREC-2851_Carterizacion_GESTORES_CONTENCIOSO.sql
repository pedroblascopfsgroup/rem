...
BEGIN
     ...
     -- GESTORES CONTENCIOSOS EN LA GAA
    ------------------------------
...
select distinct auxi.asu_id, auxi.usd_id, auxi.dd_tge_id
    from (
          select distinct asu.asu_id, usd.usd_id, tge.dd_tge_id,
           ...
           inner join '||V_ESQUEMA||'.MIG_PROCEDIMIENTOS_CABECERA mig on rge.cd_expediente = mig.cd_expediente_nuse
           inner join '||V_ESQUEMA||'.EXP_EXPEDIENTES exp                   on mig.cd_expediente_nuse = exp.cd_expediente_nuse
           inner join '||V_ESQUEMA||'.ASU_ASUNTOS asu                       on exp.exp_id = asu.exp_id
...
--                                                     where dd_tge_codigo=''SUP_PCO''))
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAA_GESTOR_ADICIONAL_ASUNTO cargada. Gestores contenciosos '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
    
    
    -- GESTORES CONTENCIOSOS en GAH
    --------------------------
...
--                            )
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAH_GESTOR_ADICIONAL_HISTORICO cargada. Gestores contenciosos '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
