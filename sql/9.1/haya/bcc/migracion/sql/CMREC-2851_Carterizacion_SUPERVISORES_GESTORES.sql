...
--## FECHA_CREACION=2016048
--## ARTEFACTO=batch
--## VERSION_ARTEFACTO=0.1
--## INCIDENCIA_LINK=CMREC-2851
--## PRODUCTO=NO
--## 
--## Finalidad: Carterizacion de gestores supervisores
--## INSTRUCCIONES:  
--## VERSIONES:
...
BEGIN
     ...
     -- SUPERVISORES CONTENCIOSOS EN LA GAA
    ------------------------------
 ...
 /*   EXECUTE IMMEDIATE('delete
                    ...
                    where gaa1.gaa_id in (
                              select distinct gaa.gaa_id
                               ...
                               where gaa.dd_tge_id = (select dd_tge_id from HAYAMASTER.dd_tge_tipo_gestor where dd_tge_codigo=''SUP'')
                               )');
     */
...
select distinct auxi.asu_id, auxi.usd_id
    from (
          select distinct asu.asu_id, usd.usd_id,
           ...
           inner join '||V_ESQUEMA||'.DD_GHS_GEST_HAYA_SUPERVISOR ghs on ghc.dd_ghc_bcc_codigo = ghs.DD_GHS_HAYA_CODIGO           
...
--           inner join '||V_ESQUEMA_MASTER||'.DD_TGE_TIPO_GESTOR  tge  on tgp.DD_TGE_ID = tge.DD_TGE_ID and tge.borrado = 0 
...
/*    EXECUTE IMMEDIATE('delete from GAH_GESTOR_ADICIONAL_historico gaa1
                      ...
                      select distinct gaa.gah_id
                       ...
                       where gaa.gah_tipo_gestor_id = (select dd_tge_id from '||V_ESQUEMA_MASTER||'.dd_tge_tipo_gestor where dd_tge_codigo=''SUP'')
                     )');   
...
--           inner join '||V_ESQUEMA_MASTER||'.DD_TGE_TIPO_GESTOR  tge  on tgp.DD_TGE_ID = tge.DD_TGE_ID and tge.borrado = 0 
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAA_GESTOR_ADICIONAL_ASUNTO cargada. Supervisores precontenciosos '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
...
--           inner join '||V_ESQUEMA_MASTER||'.DD_TGE_TIPO_GESTOR  tge  on tgp.DD_TGE_ID = tge.DD_TGE_ID and tge.borrado = 0 
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAH_GESTOR_ADICIONAL_HISTORICO cargada. Supervisores precontenciosos '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
