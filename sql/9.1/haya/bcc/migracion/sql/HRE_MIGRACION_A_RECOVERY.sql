...
--***************************************/
 -- La secuencia S_ASU_ASUNTOS se crea en el scrip anterior.
--    SELECT COUNT(1)
--    INTO V_COUNT
--    FROM ALL_SEQUENCES
...
--    	  SELECT CD_CONCURSO CD_PROCEDIMIENTO, NULL CD_EXPEDIENTE_NUSE , NULL NUMERO_EXP_NUSE FROM '||V_ESQUEMA||'.MIG_CONCURSOS_CABECERA
        ...
        , ( select op.CD_PROCEDIMIENTO, op.numero_contrato
             ...
             where op.numero_contrato = cnt.cnt_contrato
               ...
               and r.dd_cre_codigo  in (''EX'',''CN'',''IM'',''AR'',''MA'',''AL'') ) MARCA
           ...
           (
             ...
             from (
                    ...
                    from '||V_ESQUEMA||'.asu_asuntos asu                                                                 inner join 
                         ...
                         '||V_ESQUEMA||'.usd_usuarios_despachos      usd  on usd.des_id = des.des_id                     inner join
                         '||V_ESQUEMA_MASTER||'.usu_usuarios         usu  on usu.usu_id = usd.usu_id and usu.USU_EXTERNO = 1
                    where not exists (select 1 from HAYA02.GAA_GESTOR_ADICIONAL_ASUNTO gaa where gaa.asu_id = asu.asu_id and gaa.dd_tge_id = (select dd_tge_id 
                                                                                                                                              from '||V_ESQUEMA_MASTER||'.dd_tge_tipo_gestor
...
select asu_id
  ...
  where not exists (select 1 from '||V_ESQUEMA||'.GAA_GESTOR_ADICIONAL_ASUNTO gaa where gaa.asu_id = asu.asu_id and gaa.dd_tge_id =
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAA_GESTOR_ADICIONAL_ASUNTO cargada. SUPERVISOR NIVEL 1 según propiedad. '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
...
from 
 (
  ...
  where not exists (select 1 from '||V_ESQUEMA||'.GAA_GESTOR_ADICIONAL_ASUNTO gaa where gaa.asu_id = asu.asu_id and gaa.dd_tge_id =
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAA_GESTOR_ADICIONAL_ASUNTO cargada. SUPERVISOR NIVEL 2 según propiedad. '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
...
and ges.dd_ges_codigo = ''HAYA'')
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAA_GESTOR_ADICIONAL_ASUNTO cargada. SUPERVISOR NIVEL 1 según propiedad SAREB y gestion HAYA. '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
...
V_SQL:= 'gaa SUPERVISOR 2';
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' - GAA_GESTOR_ADICIONAL_ASUNTO cargada. SUPERVISOR NIVEL 2 según propiedad SAREB y gestion HAYA. '||SQL%ROWCOUNT||' Filas.');
    COMMIT;
...
select asu_id
  from '||V_ESQUEMA||'.asu_asuntos asu 
  where not exists (select 1 from HAYA02.GAA_GESTOR_ADICIONAL_ASUNTO gaa where gaa.asu_id = asu.asu_id and gaa.dd_tge_id =
                        (select dd_tge_id from HAYAMASTER.dd_tge_tipo_gestor where dd_tge_codigo = ''GESTPROP''))
...
from 
 (
   select asu_id
     from '||V_ESQUEMA||'.asu_asuntos asu 
     where not exists (select 1 from HAYA02.GAA_GESTOR_ADICIONAL_ASUNTO gaa where gaa.asu_id = asu.asu_id and gaa.dd_tge_id =
                           (select dd_tge_id from HAYAMASTER.dd_tge_tipo_gestor where dd_tge_codigo = ''GESTLLA''))
     and asu.asu_id in (select distinct asuu.asu_id
                       from '||V_ESQUEMA||'.asu_asuntos asuu inner join
                            '||V_ESQUEMA||'.prc_procedimientos prc on prc.asu_id = asuu.asu_id inner join
                            '||V_ESQUEMA||'.dd_tpo_tipo_procedimiento tpo on tpo.dd_tpo_id = prc.dd_tpo_id and tpo.DD_TPO_CODIGO in (''P409'',''P401'',''P01'',''P15'') inner join
