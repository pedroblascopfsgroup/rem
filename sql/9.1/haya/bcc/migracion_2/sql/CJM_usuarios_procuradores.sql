...
DECLARE
        ...
        v_usuario_id HAYAMASTER.USU_USUARIOS%ROWTYPE;        
...
FOR v_usuario_id IN (SELECT USU_ID FROM HAYAMASTER.USU_USUARIOS WHERE UPPER(USU_USERNAME) LIKE 'VAL.PROCU%') LOOP
  ...
  WHERE USU_ID = v_usuario_id.USU_ID
    AND DES_ID = (select des.des_id from HAYA02.DES_DESPACHO_EXTERNO des WHERE des.borrado = 0 and des.DES_DESPACHO = 'Despacho Procuradores');
