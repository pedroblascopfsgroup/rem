...
for c in
    (select contrato, id_cobro, id_expediente, cierre, nominal, intereses, demoras, gto_abogado, gto_procurador, gto_otros,
            ...
            (SELECT dd_ate_id FROM HAYA02.dd_ate_adj_tpo_entrega WHERE dd_ate_id = tipo_entrega) tipo_entrega,
            ...
            (SELECT dd_ace_id FROM HAYA02.dd_ace_adj_concepto_entrega WHERE dd_ace_codigo = concepto_entrega) concepto_entrega,
       ...
       from HAYA02.mig_cco_contabilidad_cobros m)
loop
   v_sta_id := 0; 
   v_tge_id := 0;
      ...
      select Max(a.asu_id) into v_asu_id 
        from cnt_contratos c, cex_contratos_expediente cex, 
            (SELECT asu_id, exp_id, Decode(dd_eas_codigo, '03', 1, '02', 2, '01', 3, 4) estado_asunto
               FROM HAYA02.asu_asuntos aa, HAYAMASTER.dd_eas_estado_asuntos eas 
   ...
   begin
      SELECT dd_sta_id, dd_tge_id into v_sta_id, v_tge_id FROM HAYAMASTER.dd_sta_subtipo_tarea_base WHERE dd_sta_codigo = 'CONTACOBR';
      
      SELECT u.usu_nombre into v_emisor
        FROM HAYAMASTER.usu_usuarios u, HAYA02.usd_usuarios_despachos usd, HAYA02.gaa_gestor_adicional_asunto gaa
   ...
   if (v_asu_id <> 0) then
      ...
      v_fila := 0;
      begin
         select tar_id into v_fila from HAYA02.tar_tareas_notificaciones where asu_id = v_asu_id and usuariocrear = 'PRODU-1806';
      exception
      when others then
         v_fila := 0; 
      end;
      ...
      if (v_fila = 0 ) then
         ...
         -- Nueva tarea
         INSERT INTO HAYA02.tar_tareas_notificaciones
         ...
         VALUES
            (v_tar_id, v_asu_id, 
             (SELECT dd_est_id FROM HAYAMASTER.dd_est_estados_itinerarios WHERE dd_est_codigo = 'AS'), 
             (SELECT dd_ein_id FROM HAYAMASTER.dd_ein_entidad_informacion WHERE dd_ein_codigo = 3), v_sta_id, 1,
      ...
      v_fila := 0;
      begin
         select cco_id into v_fila from HAYA02.cco_contabilidad_cobros 
      ...
      if (v_fila = 0 ) then
         ...
         INSERT INTO HAYA02.cco_contabilidad_cobros
