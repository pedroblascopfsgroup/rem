...
/*
    ...
    execute immediate 'merge into TMP_H_CNT h
                       using (select CNT_ID, exp.EXP_ID, NVL(DD_TPX_ID, -1) DD_TPX_ID
                              from '||V_HAYA01||'.EXP_EXPEDIENTES exp
                              join '||V_HAYA01||'.CEX_CONTRATOS_EXPEDIENTE cex on exp.EXP_ID = cex.EXP_ID
    ...
    */
    ...
    if((dia_periodo_ant <= max_dia_h) or ((dia_periodo_ant > max_dia_h) and (dia_periodo_ant < penult_dia_mov))) then
       --execute immediate 'update TMP_H_CNT set NUM_DIAS_VENC_PERIODO_ANT = (select NVL((MOV_FECHA_EXTRACCION - MOV_FECHA_POS_VENCIDA),0) from '||V_HAYA01||'.H_MOV_MOVIMIENTOS where MOV_FECHA_EXTRACCION = '''||dia_periodo_ant||''' and CNT_ID = CONTRATO_ID) where DIA_ID = '''||fecha||'''';
       execute immediate 'merge into TMP_H_CNT hc
              using (select NVL((MOV_FECHA_EXTRACCION - MOV_FECHA_POS_VENCIDA),0) as NUM_DIAS, CNT_ID from '||V_HAYA01||'.H_MOV_MOVIMIENTOS where MOV_FECHA_EXTRACCION = '''||dia_periodo_ant||''') thc
