...
if ((select count(*) from D_EXP_ESTADO_PALANCA where ESTADO_PALANCA_ID = -1) = 0) then
 ...
 insert into D_EXP_ESTADO_PALANCA (ESTADO_PALANCA_ID, ESTADO_PALANCA_DESC)
    select id_estado_palanca, desc_estado_palanca from recovery_haya_datastage.expedientes_fake where id_estado_palanca is not null group by id_estado_palanca;
...
if ((select count(*) from D_EXP_TIPO_SANCION where TIPO_SANCION_ID = -1) = 0) then
 ...
 insert into D_EXP_TIPO_SANCION (TIPO_SANCION_ID, TIPO_SANCION_DESC)
    select id_tipo_sancion, desc_tipo_sancion from recovery_haya_datastage.expedientes_fake where id_tipo_sancion is not null group by id_tipo_sancion;
...
if ((select count(*) from D_EXP_ESTADO_INCIDENCIA where ESTADO_INCIDENCIA_ID = -1) = 0) then
 ...
 insert into D_EXP_ESTADO_INCIDENCIA (ESTADO_INCIDENCIA_ID, ESTADO_INCIDENCIA_DESC)
    select id_estado_incidencia, desc_estado_incidencia from recovery_haya_datastage.expedientes_fake where id_estado_incidencia is not null group by id_estado_incidencia;
