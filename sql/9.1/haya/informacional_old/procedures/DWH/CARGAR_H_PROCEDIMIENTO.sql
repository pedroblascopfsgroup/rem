...
-- Fecha ultima modificación: 29/01/2016
-- Motivos del cambio:modificación finalizaciones
...
BEGIN
  ...
  select valor into V_DATASTAGE from PARAMETROS_ENTORNO where parametro = 'ESQUEMA_DATASTAGE'; 
	...
	select valor into V_HAYAMASTER from PARAMETROS_ENTORNO where parametro = 'ORIGEN_MASTER';
...
execute immediate V_SQL USING OUT O_ERROR_STATUS;
    ...
    if((fecha <= max_dia_h) or ((fecha > max_dia_h) and (fecha < penult_dia_mov))) then
      execute immediate 'select max(TRUNC(MOV_FECHA_EXTRACCION)) from ' || V_HAYA01 || '.H_MOV_MOVIMIENTOS where TRUNC(MOV_FECHA_EXTRACCION) <= to_date(''' || fecha || ''')' into max_dia_con_contratos;  
      ...
      execute immediate 'insert into TMP_PRC_CONTRATO (ITER, CONTRATO, CEX_ID, SALDO_VENCIDO, SALDO_NO_VENCIDO, INGRESOS_PENDIENTES_APLICAR, FECHA_POS_VENCIDA)
                          select prc.PRC_ID, hmov.CNT_ID, cex.CEX_ID, hmov.MOV_POS_VIVA_VENCIDA, hmov.MOV_POS_VIVA_NO_VENCIDA, hmov.MOV_EXTRA_1, hmov.MOV_FECHA_POS_VENCIDA 
                               ...
                               join '||V_HAYA01||'.H_MOV_MOVIMIENTOS hmov on cex.CNT_ID = hmov.CNT_ID
    ...
    /*  
    execute immediate 'merge into TMP_PRC_CONTRATO t1 using (select max(MOV_ID) MAX_MOV_ID, CNT_ID from '||V_HAYA01||'.H_MOV_MOVIMIENTOS group by CNT_ID) t2 on (t1.CONTRATO = t2.CNT_ID) when matched then update set t1.MAX_MOV_ID = t2.MAX_MOV_ID WHERE CEX_ID is null';
    ...
    execute immediate 'merge into TMP_PRC_CONTRATO t1 using '||V_HAYA01||'.H_MOV_MOVIMIENTOS t2 on (t2.MOV_ID = t1.MAX_MOV_ID and t2.CNT_ID = t1.CONTRATO) when matched then update set t1.SALDO_VENCIDO = t2.MOV_POS_VIVA_VENCIDA, t1.SALDO_NO_VENCIDO = t2.MOV_POS_VIVA_NO_VENCIDA, t1.INGRESOS_PENDIENTES_APLICAR = t2.MOV_EXTRA_1, t1.FECHA_POS_VENCIDA = t2.MOV_FECHA_POS_VENCIDA where CEX_ID is null'; 
    commit;
    */
      --Log_Proceso
