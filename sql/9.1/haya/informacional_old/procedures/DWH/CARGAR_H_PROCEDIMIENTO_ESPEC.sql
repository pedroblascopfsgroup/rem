...
-- =========================================================================================================================================
 ...
 V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_CONCU_TAREA'', '''', :O_ERROR_STATUS); END;';
    ...
    from TMP_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.PRC_PROCEDIMIENTOS prc on tpj.FASE_ACTUAL = prc.PRC_ID and trunc(prc.FECHACREAR) <= :fecha and prc.DD_TPO_ID=2351 and prc.BORRADO = 0
   ...
   V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_CONCU_TAREA'', '''', :O_ERROR_STATUS); END;';
    ...
    from TMP_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= :fecha
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003085)
   ...
   V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_CONCU_TAREA'', '''', :O_ERROR_STATUS); END;';
    ...
    from TMP_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= :fecha
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003083)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
    where  DIA_ID = :fecha  ' using fecha, fecha; --20150421
   ...
   V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_CONCU_TAREA'', '''', :O_ERROR_STATUS); END;';
    ...
    from TMP_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= :fecha
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003144)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fechaPre''
    where  DIA_ID = :fecha   ' using fecha, fecha;
    commit;
   ...
   V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_CONCU_TAREA'', '''', :O_ERROR_STATUS); END;';
    ...
    from TMP_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= :fecha
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000002996)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
    where  DIA_ID = :fecha  ' using fecha, fecha;
    commit;
...
*/
    ...
    -- 0	No Aprobado / 1	Aprobado
    merge into TMP_CONCU_DETALLE a
    using (select distinct ITER, (case when VALOR_FORMULARIO = '02' or VALOR_FORMULARIO = '2' then 0
                              when VALOR_FORMULARIO = '01' or VALOR_FORMULARIO = '1' then 1
                              when VALOR_FORMULARIO is null or trim(VALOR_FORMULARIO) = '' then null
    ...
    from TMP_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= :fecha
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003161)
 ...
 V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_CONCU_CONVENIO'', '''', :O_ERROR_STATUS); END;';
    ...
    using (select distinct ITER,  VALOR_FORMULARIO from TMP_CONCU_CONVENIO) b
    on (b.ITER = a.ITER)
    when matched then update set a.QUITA_CONVENIO = b.VALOR_FORMULARIO;
    commit;
    ...
    from TMP_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and (tar.TAR_FECHA_FIN is not null OR TRUNC(tar.TAR_FECHA_FIN) <= :fecha)
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003163)
  ...
  V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_CONCU_AUX'', '''', :O_ERROR_STATUS); END;';
    ...
    using (select ITER,  MAX( CASE WHEN CODIGO_FASE_ACTUAL = 'P404' THEN 1
		   ...
		   group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.FASE_SUBASTA_CONCURSAL_ID = b.FASE_SUBASTA_CONCURSAL;
    commit;
    ...
    -- P_AUTO_FC_REG_RESOL_APER_LIQ_D -> Procedimiento que NO pasa por T. fase convenio (2384)
    execute immediate 'merge into H_CONCU a
          using (select distinct tpj.ITER from TMP_CONCU_JERARQUIA tpj
                             ...
                             and prc.DD_TPO_ID = 2384 and prc.BORRADO=0) b
...
-- =========================================================================================================================================
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
            (select distinct DD_TPO_ID
             ...
             where DD_TPO_DESCRIPCION = ''P. verbal - HAYA'' or DD_TPO_DESCRIPCION = ''P. ordinario - HAYA'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID';
  commit;
  ...
  for d in c_fecha loop
    ...
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003244, 10000000003444)
    ...
    from TMP_DECL_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= '''||d.DIA_ID||'''
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003240, 10000000003441)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
    where  DIA_ID = '''||d.DIA_ID||'''
    ';
    commit;
...
-- Borrar Indices TMP_EJEC_ORD_JERARQUIA
 ...
 V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''TRUNCATE'', ''TMP_EJEC_ORD_JERARQUIA'', '''', :O_ERROR_STATUS); END;';
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
                  (select distinct DD_TPO_ID
                   from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
                   join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
                   where DD_TPO_DESCRIPCION = ''T. de investigación judicial - HAYA'' or  DD_TPO_DESCRIPCION = ''P. Ej. de título no judicial - HAYA'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID ';
 ...
 V_SQL :=  'BEGIN OPERACION_DDL.DDL_INDEX(''CREATE'', ''TMP_EJEC_ORD_JRQ_FASE_ACT_IX'', ''TMP_EJEC_ORD_JERARQUIA (DIA_ID, FASE_ACTUAL)'', ''S'', '''', :O_ERROR_STATUS); END;';
  ...
  for h in c_fecha loop
    ...
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003219,10000000003208)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE in (''fechaInterposicion'',''fecha'')
    ...
    from TMP_EJEC_ORD_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= '''||h.DIA_ID||'''
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003094)
...
-- Borrar Indices TMP_HIPO_JERARQUIA
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
    (select distinct DD_TPO_ID
       from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
       join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
       where DD_TPO_DESCRIPCION = ''P. hipotecario - HAYA'' or DD_TPO_DESCRIPCION like ''T. de subasta%'' or DD_TPO_DESCRIPCION = ''T. Subsanación decreto adjudicación - HAYA'' or DD_TPO_DESCRIPCION = ''T. de cesión de remate - HAYA'' ) b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID ';
  ...
  for l in c_fecha loop
    ...
    from TMP_HIPO_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= '''||l.DIA_ID||'''
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id = 10000000003027
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
    where  DIA_ID = '''||l.DIA_ID||'''
    and tpj.FASE_FINALIZADA is null
    and tpj.FASE_PARALIZADA is null
    ...
    from TMP_HIPO_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= '''||l.DIA_ID||'''
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID  and tex.tap_id in (10000000003032)
    ...
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003034)
    ...
    from TMP_HIPO_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003050)
    ...
    from TMP_HIPO_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= '''||l.DIA_ID||'''
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003008)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fechaSolicitud''
    where  DIA_ID = '''||l.DIA_ID||'''
    and tpj.FASE_FINALIZADA is null
    and tpj.FASE_PARALIZADA is null
    ...
    using (select ITER, max(TRUNC(FECHA_FORMULARIO)) as FECHA_FORMULARIO from TMP_HIPO_TAREA group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.FECHA_SOL_DECRETO_ADJ = b.FECHA_FORMULARIO;
    commit;
    ...
    using (select ITER, max(TRUNC(FECHA_FORMULARIO)) as FECHA_FORMULARIO from TMP_HIPO_TAREA group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.FECHA_RECEP_TESTIMONIO = b.FECHA_FORMULARIO;
    commit;
    ...
    from TMP_HIPO_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= '''||l.DIA_ID||'''
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003007)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fechaPresentacion''
    where  DIA_ID = '''||l.DIA_ID||'''
    and tpj.FASE_FINALIZADA is null
    and tpj.FASE_PARALIZADA is null
    and tpj.FASE_CON_RECURSO is null';
    ...
    using (select ITER, max(TRUNC(FECHA_FORMULARIO)) as FECHA_FORMULARIO from TMP_HIPO_TAREA group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.FECHA_DECRETO_ADJ = b.FECHA_FORMULARIO;
    commit;
    ...
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003016,
                           10000000003017, 10000000003020, 10000000003018, 10000000003019, 10000000003021, 10000000003022, 10000000003023, 10000000003024,
                           10000000003032, 10000000003040, 10000000003640, 1000000000000001, 1000000000000004,
                           1000000000000002, 10000000003034, 10000000003035, 10000000003041, 10000000003042, 10000000003043, 10000000003049, 10000000003050, 10000000003036, 10000000003037, 10000000003038, 10000000003039, 10000000003380, 10000000003381, 10000000003383, 10000000003382,
                           10000000003039,
                           10000000003027, 10000000003030,
                           10000000003029, 10000000003031,
                           10000000003051, 10000000003052, 10000000003053, 10000000003044, 10000000003045, 10000000003038, 10000000003012, 10000000003010,
                           10000000003009, 10000000003009, 10000000003013, 10000000003004, 10000000003006, 10000000003007, 10000000003005, 10000000003345, 10000000003346,
                           10000000002963, 10000000002966, 10000000002965,
                           10000000002966, 10000000002967, 10000000002968)
    where  DIA_ID = '''||l.DIA_ID||'''
    ';
    commit;
    ...
    select l.DIA_ID,
     ...
     (FECHA_SOL_DECRETO_ADJ - FECHA_CREACION_ASUNTO),
     (FECHA_SUBASTA_SOLICITADA - FECHA_INTERP_DEM_HIP),
     (FECHA_CESION_REMATE - FECHA_INTERP_DEM_HIP),
     (FECHA_SOL_DECRETO_ADJ - FECHA_INTERP_DEM_HIP),
     (FECHA_SOL_DECRETO_ADJ - FECHA_SUBASTA),
     (FECHA_CELEBRACION_SUBASTA - FECHA_SUBASTA_SOLICITADA),
     (FECHA_CESION_REMATE - FECHA_CELEBRACION_SUBASTA),
     (FECHA_SOL_DECRETO_ADJ - FECHA_CELEBRACION_SUBASTA), 
     (FECHA_DECRETO_ADJ - FECHA_RECEP_TESTIMONIO)
  ...
  update H_HIPO set TD_SUB_SOL_SUB_CEL_ID = (case when P_SUB_SOL_SUB_CEL >= 0 and P_SUB_SOL_SUB_CEL <= 30 then 0
                                                         ...
                                                         when P_SUB_SOL_SUB_CEL > 90 and P_SUB_SOL_SUB_CEL <= 120 then 3
                                                         when P_SUB_SOL_SUB_CEL > 120 and P_SUB_SOL_SUB_CEL <= 150 then 4
                                                         when P_SUB_SOL_SUB_CEL > 150 then 5
                                                         else -1
                                                        end);
  commit;
  update H_HIPO set TD_SUB_CEL_CESION_REMATE_ID = (case when P_SUB_CEL_CESION_REMATE >= 0 and P_SUB_CEL_CESION_REMATE <= 30 then 0
                                                               ...
                                                               when P_SUB_CEL_CESION_REMATE > 90 and P_SUB_CEL_CESION_REMATE <= 120 then 3
                                                               when P_SUB_CEL_CESION_REMATE > 120 and P_SUB_CEL_CESION_REMATE <= 150 then 4
                                                               when P_SUB_CEL_CESION_REMATE > 150 then 5
                                                               else -1
                                                            end);
  commit;
  update H_HIPO set TD_CEL_ADJUDICACION_ID = (case when P_CEL_ADJUDICACION >= 0 and P_CEL_ADJUDICACION <= 30 then 0
                                                               when P_CEL_ADJUDICACION > 30 and P_CEL_ADJUDICACION <= 60 then 1
                                                               when P_CEL_ADJUDICACION > 60 and P_CEL_ADJUDICACION <= 90 then 2
                                                               when P_CEL_ADJUDICACION > 90 and P_CEL_ADJUDICACION <= 120 then 3
                                                               when P_CEL_ADJUDICACION > 120 and P_CEL_ADJUDICACION <= 150 then 4
                                                               when P_CEL_ADJUDICACION > 150 then 5
                                                               else -1
                                                            end);
  commit;
  update H_HIPO set TD_RECEP_DECRE_ADJUDICA_ID = (case when P_ENTRA_REG_RECEPCION >= 0 and P_ENTRA_REG_RECEPCION <= 30 then 0
                                                               when P_ENTRA_REG_RECEPCION > 30 and P_ENTRA_REG_RECEPCION <= 60 then 1
                                                               when P_ENTRA_REG_RECEPCION > 60 and P_ENTRA_REG_RECEPCION <= 90 then 2
                                                               when P_ENTRA_REG_RECEPCION > 90 and P_ENTRA_REG_RECEPCION <= 120 then 3
                                                               when P_ENTRA_REG_RECEPCION > 120 and P_ENTRA_REG_RECEPCION <= 150 then 4
                                                               when P_ENTRA_REG_RECEPCION > 150 then 5
                                                               else -1
                                                            end);
  commit;
...
-- Borrar Indices TMP_MON_JERARQUIA
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
              (select distinct DD_TPO_ID from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
               join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
               where DD_TPO_DESCRIPCION = ''P. Monitorio - HAYA'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID ';
  V_ROWCOUNT := sql%rowcount;
  commit;
...
-- Bucle para las taras e inclusiOn en H_MON
  for p in c_fecha loop
      ...
      join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003250)
      join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
...
-- =========================================================================================================================================
  ...
  select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
                  (select DD_TPO_ID from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
                   join '||V_DATASTAGE||'.DD_TAC_TIPO_ACTUACION ta on tp.DD_TAC_ID = ta.DD_TAC_ID
                   where DD_TPO_DESCRIPCION = ''T. de ejecución notarial - HAYA'') b
  where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID';
  V_ROWCOUNT := sql%rowcount;
  commit;
...
-- =========================================================================================================================================
  ...
  execute immediate 'insert into TMP_PRE_CONCU_JERARQUIA
                      select a.* from TMP_PRC_ESPECIFICO_JERARQUIA a,
                                (select distinct DD_TPO_ID
								from '||V_DATASTAGE||'.DD_TPO_TIPO_PROCEDIMIENTO tp
								where DD_TPO_DESCRIPCION = ''T. Homologación de acuerdo - HAYA'' or DD_TPO_DESCRIPCION = ''T. Elevacion Propuesta Sareb Litigios - HAYA'') b
                      where a.TIPO_PROCEDIMIENTO_DET = b.DD_TPO_ID';
  ...
  loop
    ...
    insert into TMP_PRE_CONCU_DETALLE(ITER)
    select distinct ITER from  TMP_PRE_CONCU_JERARQUIA where DIA_ID = fecha;
    commit;
    ...
    -- FECHA_SOL_ART5_BIS
    -- Tap ID: 10000000003188 campo "fecha"
     --Log_Proceso
    execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'FECHA_SOL_ART5_BIS', 5;
    ...
    execute immediate '
    insert into TMP_PRE_CONCU_TAREA (ITER, FASE, TAREA, DESCRIPCION_TAREA, FECHA_INI, FECHA_FIN, TAP_ID, TAR_ID, TEX_ID, DESCRIPCION_FORMULARIO, FECHA_FORMULARIO)
    select  tpj.ITER, tpj.FASE_ACTUAL, tar.TAR_ID, tar.TAR_TAREA, tar.TAR_FECHA_INI, tar.TAR_FECHA_FIN ,TAP_ID,tar.TAR_ID,tex.TEX_ID, TEV_NOMBRE, TO_date(TEV_VALOR,''YYYY-MM-DD'')
    from TMP_PRE_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and trunc(tar.TAR_FECHA_FIN) <= :fecha and tar.TAR_FECHA_FIN is not null
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003188)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
    where  DIA_ID = :fecha' using fecha, fecha; 
    commit;

    merge into TMP_PRE_CONCU_DETALLE a
    using (select ITER,  max(TRUNC(FECHA_FORMULARIO)) as FECHA_FOR from TMP_PRE_CONCU_TAREA group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.FECHA_SOL_ART5_BIS = b.FECHA_FOR;
    commit;

    -- FECHA_PREP_DEC_PROP
    -- Tarea: TAP ID: 10000000003187 campo "fecha"
     --Log_Proceso
    execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'FECHA_PREP_DEC_PROP', 5;
    ...
    from TMP_PRE_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and trunc(tar.TAR_FECHA_FIN) <= :fecha and tar.TAR_FECHA_FIN is not null
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000003187)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
    where  DIA_ID = :fecha' using fecha, fecha; 
    commit;
    ...
    using (select ITER,  max(TRUNC(FECHA_FORMULARIO)) as FECHA_FOR from TMP_PRE_CONCU_TAREA group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.FECHA_PREP_DEC_PROP = b.FECHA_FOR;
    commit;
    ...
    from TMP_PRE_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= :fecha
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000002973)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''fecha''
    where  DIA_ID = :fecha' using fecha, fecha; 
    commit;
    ...
    using (select ITER,  max(TRUNC(FECHA_FORMULARIO)) as FECHA_FOR from TMP_PRE_CONCU_TAREA group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.FECHA_ULT_PROPUESTA = b.FECHA_FOR;
    commit;

    -- PROP_SAREB_ID
	-- Coger Tap Id: 10000000002976 "comboResultado"
     --Log_Proceso
    execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'PROP_SAREB_ID', 5;
    ...
    from TMP_PRE_CONCU_JERARQUIA tpj
    join '||V_DATASTAGE||'.TAR_TAREAS_NOTIFICACIONES tar on tpj.FASE_ACTUAL = tar.PRC_ID and tar.TAR_FECHA_FIN is not null and TRUNC(tar.TAR_FECHA_FIN) <= :fecha
    join '||V_DATASTAGE||'.TEX_TAREA_EXTERNA tex on tar.TAR_ID = tex.TAR_ID and tex.tap_id in (10000000002976)
    join '||V_DATASTAGE||'.TEV_TAREA_EXTERNA_VALOR tev on tex.TEX_ID = tev.TEX_ID and tev.TEV_NOMBRE = ''comboResultado''
    where  DIA_ID = :fecha' using fecha, fecha; 
    commit;
    ...
    using (select ITER,  max(TRUNC(FECHA_FORMULARIO)) as FECHA_FOR from TMP_PRE_CONCU_TAREA group by ITER) b
    on (b.ITER = a.ITER)
    when matched then update set a.PROP_SAREB_ID = CASE WHEN b.FECHA_FOR IS NOT NULL THEN 1 ELSE 0 END;
    commit;
    ...
    -- Borrado de los días a insertar
    delete from H_PRE_CONCU where DIA_ID = fecha;
    commit;

    -- Insertamos en H_PRE_CONCU sólo el registro que tiene la última actuación
    insert into H_PRE_CONCU
    (DIA_ID,
     FECHA_CARGA_DATOS,
     PROCEDIMIENTO_ID,
	 ENT_CEDENTE_ID,
    ...
    select fecha,
     fecha,
     ITER,
	 0, --Entidad cedente: BANKIA
	 NVL(PROP_SAREB_ID, 0), -- 0: No
     FECHA_SOL_ART5_BIS,
     FECHA_PREP_DEC_PROP,
     FECHA_ULT_PROPUESTA,
     1
    from TMP_PRE_CONCU_DETALLE;

    V_ROWCOUNT := sql%rowcount;
    commit;

     --Log_Proceso
    execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 4;
  ...
  commit;
  -- Incluimos el último día cargado
  select max(DIA_ID) into max_dia_carga from H_PRE_CONCU;
  update H_PRE_CONCU set FECHA_CARGA_DATOS = max_dia_carga;

  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU. Termina Carga', 3;
...
-- ----------------------------------------------------------------------------------------------
  ...
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU_SEMANA. Empieza Carga', 3;
    ...
    insert into TMP_FECHA (DIA_H)
    select distinct DIA_ID from H_PRE_CONCU where DIA_ID between date_start and date_end;
    commit;
  ...
  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU_SEMANA. Termina Carga', 3;
...
-- ----------------------------------------------------------------------------------------------
  ...
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU_MES. Empieza Carga', 3;
    ...
    insert into TMP_FECHA (DIA_H) select distinct DIA_ID from H_PRE_CONCU where DIA_ID between date_start and date_end;
    commit;
  ...
  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU_MES. Termina Carga', 3;
...
-- ----------------------------------------------------------------------------------------------
  ...
  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU_TRIMESTRE. Termina Carga', 3;
...
-- ----------------------------------------------------------------------------------------------
  ...
  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'H_PRE_CONCU_ANIO. Termina Carga', 3;
