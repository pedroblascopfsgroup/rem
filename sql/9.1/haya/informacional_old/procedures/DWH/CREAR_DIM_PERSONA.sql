...
-- Fecha ultima modificacion: 26/01/2016
-- Motivos del cambio:D_PER_TIPO_GESTOR, D_PER_ANOTACION_ENT_INF y D_PER_ANOTACION_TIPO
...
DBMS_OUTPUT.PUT_LINE('---- Creacion tabla D_PER_GRUPO_ECONOMICO');
    ...
    execute immediate V_SQL USING OUT error;
      DBMS_OUTPUT.PUT_LINE('---- Creacion tabla D_PER_ANOTACION_TIPO');	
	            ...
	            ------------------------------ D_PER_ANOTACION --------------------------
     V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''CREATE'', ''D_PER_ANOTACION'',
		''ANOTACION_ID NUMBER(16,0) NOT NULL ENABLE
		ANOTACION_TIPO_ID NUMBER(16,0) ,
		ANOTACION_DESC VARCHAR2(255 CHAR) , 
		  PRIMARY KEY (ANOTACION_ID)'', 
                      :error); END;';
    execute immediate V_SQL USING OUT error;
      DBMS_OUTPUT.PUT_LINE('---- Creacion tabla D_PER_ANOTACION');	
