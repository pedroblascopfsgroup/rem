...
V_MSQL := 'CREATE OR REPLACE FORCE VIEW '||V_ESQUEMA||'.VINFSUBASTALETRADO AS 
SELECT distinct sub.sub_id, prc.prc_cod_proc_en_juzgado numero_auto, dd_juz.dd_juz_descripcion numero_juz, dd_pla.dd_pla_descripcion prov_juz,
  ...
  FROM '||V_ESQUEMA||'.sub_subasta sub LEFT JOIN '||V_ESQUEMA||'.dd_tsu_tipo_subasta dd_tsu ON sub.dd_tsu_id = dd_tsu.dd_tsu_id
       ...
       (SELECT DISTINCT gaa.asu_id, des.des_id, des.des_despacho, dd_tge_codigo codigo
                   FROM '||V_ESQUEMA||'.gaa_gestor_adicional_asunto gaa INNER JOIN '||V_ESQUEMA_M||'.dd_tge_tipo_gestor tge ON gaa.dd_tge_id = tge.dd_tge_id AND dd_tge_codigo IN (''GEXT'', ''PROC'')
       ...
       (SELECT /*+ MATERIALIZE */
          ...
          FROM '||V_ESQUEMA||'.tar_tareas_notificaciones tar INNER JOIN tex_tarea_externa tex ON tex.tar_id = tar.tar_id
               ...
               INNER JOIN '||V_ESQUEMA||'.tev_tarea_externa_valor tev ON tev.tex_id = tex.tex_id AND UPPER (tev_nombre) IN (''FECHASOLICITUD'', ''FECHADEMANDA'',''FECHAPRESENTACIONDEMANDA'',''FECHA'',''FECHAINTERPOSICION'',''FECHAPRE'')      --AND UPPER (tfi.tfi_nombre) = UPPER (tev.tev_nombre)
         WHERE NOT tev.tev_valor IS NULL
           AND tap.tap_codigo IN
                  (''H001_DemandaCertificacionCargas'', ''H022_InterposicionDemanda'', ''H024_InterposicionDemanda'', ''H026_InterposicionDemanda'', ''H020_InterposicionDemandaMasBienes'',
                   ''H016_interposicionDemandaMasBienes'', ''H023_interposicionDemanda'')) demanda ON prc.prc_id = demanda.prc_id
       LEFT JOIN
       (SELECT /*+ MATERIALIZE */
       ...
       (SELECT /*+ MATERIALIZE */
         ...
         WHERE NOT tev.tev_valor IS NULL AND tap.tap_codigo IN (''H002_SenyalamientoSubasta'', ''H003_SenyalamientoSubasta'',''H004_SenyalamientoSubasta'',''H036_registrarAnuncioSubasta'')) valores_senyalamiento ON prc.prc_id = valores_senyalamiento.prc_id
         ';
