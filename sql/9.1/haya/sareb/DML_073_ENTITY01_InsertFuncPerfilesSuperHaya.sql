...
BEGIN	
    ...
    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE FUN_ID = (SELECT FUN_ID FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''ASU_MULTIGESTOR_SUPERVISOR'') ' ||
    		 ' AND PEF_ID IN (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO IN (''HAYASUPVISOR''))';
	...
	ELSE
		V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF' ||
					...
					' WHERE FUN.FUN_DESCRIPCION = ''ASU_MULTIGESTOR_SUPERVISOR'' AND PEF.PEF_CODIGO IN '||
					' (''HAYASUPVISOR'')';
    	
		EXECUTE IMMEDIATE V_MSQL_1;
		DBMS_OUTPUT.PUT_LINE('[INFO] Permiso ASU_MULTIGESTOR_SUPERVISOR insertado correctamente.');
