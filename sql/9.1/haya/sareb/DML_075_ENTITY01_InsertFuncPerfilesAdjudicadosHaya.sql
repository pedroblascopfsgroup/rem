...
BEGIN	
    ...
    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE FUN_ID = (SELECT FUN_ID FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''TAB_ADJUDICADOS'') ' ||
    		 ' AND PEF_ID IN (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO IN (''HAYAADMIN'',''HAYASUPVISOR'',''HAYAGEST'',''HAYAGESTEXT''))';
	...
	ELSE
		V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF' ||
					...
					' WHERE FUN.FUN_DESCRIPCION = ''TAB_ADJUDICADOS'' AND PEF.PEF_CODIGO IN '||
					' (''HAYAADMIN'',''HAYASUPVISOR'',''HAYAGEST'',''HAYAGESTEXT'')';
    	
		EXECUTE IMMEDIATE V_MSQL_1;
		DBMS_OUTPUT.PUT_LINE('[INFO] Permiso TAB_ADJUDICADOS insertado correctamente.');
