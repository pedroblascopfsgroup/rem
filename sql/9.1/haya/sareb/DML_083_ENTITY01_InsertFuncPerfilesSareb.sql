...
BEGIN	
    ...
    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE PEF_ID IN (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''HAYASAREB'')';
	...
	ELSE
		V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF' ||
					...
					' WHERE PEF.PEF_CODIGO = ''HAYASAREB'' AND FUN.FUN_ID IN '||
					' (SELECT FUN_ID FROM '||V_ESQUEMA||'.FUN_PEF WHERE PEF_ID = '||
					' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO IN ''HAYACONSULTA''))';
    	
		EXECUTE IMMEDIATE V_MSQL_1;
		DBMS_OUTPUT.PUT_LINE('[INFO] Permiso ACC_MAN_SERVICIOS_UVEM insertado correctamente.');
