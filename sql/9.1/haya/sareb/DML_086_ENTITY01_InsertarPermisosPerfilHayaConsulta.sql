...
--## PRODUCTO=NO
--##
--## Finalidad: Permisos perfil restringido CONSULTA
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_FUNCION T_FUNCION := T_FUNCION(   
		'MENU-LIST-CLI','MENU-LIST-ASU','BUSQUEDA','MENU-LIST-CNT','VER-OBSERVACIONES-ASUNTO','SECCION_BIENES_EN_CONTRATO',
		'TAB-CNT-EXP-ASU','BUSCAR-TAREAS','BUSCAR-BIENES','TAB_CLIENTE_CONTRATOS','TAB_CLIENTE_CABECERA','TAB_CLIENTE_DATOS',
		'TAB_CLIENTE_ANTECEDENTES','TAB_CLIENTE_GRUPO','TAB_CLIENTE_HISTORICOS','OPTIMIZACION_BUZON_TAREAS','TAB_COBROS_PAGOS_EXP',
		'TAB_CONTRATO_RECIBOS','MENU_BUSQUEDAS_GENERAL','TAB_CLIENTE_CIRBE','TAB_CONTRATO_DOCUMENTOS','BUSCAR-SUBASTAS','SOLO_CONSULTA'
    ); 
...
BEGIN	
    ...
    IF V_NUM_TABLAS != 1 THEN
    	DBMS_OUTPUT.PUT_LINE('[INFO] No existe el perfil HAYACONSULTA o hay varios perfiles HAYACONSULTA en la tabla '||V_ESQUEMA||'.PEF_PERFILES... no se modifica nada.');    	
    ELSE    	
       	V_SQL := 'SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''HAYACONSULTA''';
		EXECUTE IMMEDIATE V_SQL INTO PEF_ID;
    
    	DBMS_OUTPUT.PUT_LINE('[INFO] Eliminando permisos de HAYACONSULTA.');
    	V_MSQL_1 := 'DELETE FROM '||V_ESQUEMA||'.FUN_PEF' ||
					' WHERE PEF_ID ='||PEF_ID||'';    	
		EXECUTE IMMEDIATE V_MSQL_1;
		
		DBMS_OUTPUT.PUT_LINE('[INFO] Permisos de HAYACONSULTA eliminados correctamente.');
		DBMS_OUTPUT.PUT_LINE('[INFO] Insertando nuevos permisos para HAYACONSULTA.');
