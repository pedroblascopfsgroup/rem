...
--## PRODUCTO=NO
--##
--## Finalidad: Asigna el Subtipo de tarea PREDOC a PCO_AcuseReciboBurofax
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN	
    ...
    IF V_NUM_TABLAS > 0 THEN				
		...
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO '||
		  ' SET DD_STA_ID = (SELECT DD_STA_ID FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO=''PREDOC''), DD_TSUP_ID = (SELECT DD_STA_ID FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO=''SUP_PCO'')
