...
--## PRODUCTO=NO
--## 
--## Finalidad: Modificacion columna en PROCEDIMIENTO_JUDICIAL_FOTO
...
BEGIN 
  ...
  BEGIN 
	  ...
	  WHERE TABLE_NAME = 'PROCEDIMIENTO_JUDICIAL_FOTO'
		  AND COLUMN_NAME = 'MOTIVO_CIERRE';
  
	  DBMS_OUTPUT.PUT_LINE('LA COLUMNA MOTIVO_CIERRE TIENE '||V_LENGTH_MOTIVO_CIERRE||' CARACTERES');
  
	  -- CAMBIA A CARACTER CORRECTO COD_HITO_ACTUAL
	   IF V_LENGTH_MOTIVO_CIERRE <=2500 then
		   DBMS_OUTPUT.put_line('MOTIVO CIERRE YA TIENE 2500 CARATERES');
	   ELSE
		   V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||TABLA||' MODIFY MOTIVO_CIERRE VARCHAR2(2500 CHAR)';
