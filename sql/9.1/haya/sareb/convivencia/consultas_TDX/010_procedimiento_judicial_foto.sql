...
DELETE FROM MINIREC.H_PROCEDIMIENTO_JUDICIAL_FOTO
...
INSERT INTO MINIREC.H_PROCEDIMIENTO_JUDICIAL_FOTO (
    FECHA_HISTORICO,
    ASU_ID,
    ASU_NOMBRE,
    ID_PROCEDIMIENTO,
    ...
    NIF_PROCURADOR,
    PROCURADOR,
    IMPORTE_DEMANDA_PRINCIPAL,
    IMPORTE_DEMANDA_INTERESES,
  ...
  SELECT 
    ...
    IMPORTE_COMUNICADO,
    IMPORTE_RECONOCIDO
  FROM MINIREC.PROCEDIMIENTO_JUDICIAL_FOTO
;

--Truncado de tablas TDX
TRUNCATE TABLE MINIREC.PROCEDIMIENTO_JUDICIAL_FOTO;

--Generacion de las tablas TDX
INSERT INTO MINIREC.PROCEDIMIENTO_JUDICIAL_FOTO
  (
ASU_ID
,ASU_NOMBRE
,ID_PROCEDIMIENTO
...
WITH FECHA_CIERRE AS (
    ...
    SELECT * FROM(
      SELECT DD_EAS.DD_EAS_CODIGO, GREATEST(DPR.FECHACREAR, NVL(DPR.FECHAMODIFICAR,to_date('01/01/1900','DD/MM/RRRR'))) FECHA_RECIENTE, 
        ROW_NUMBER () OVER (PARTITION BY DPR.PRC_ID ORDER BY GREATEST(DPR.FECHACREAR, NVL(DPR.FECHAMODIFICAR,to_date('01/01/1900','DD/MM/RRRR'))) DESC) RN, PRC.PRC_ID
      FROM HAYA01.DPR_DECISIONES_PROCEDIMIENTOS DPR 
      LEFT JOIN HAYA01.PRC_PROCEDIMIENTOS PRC ON DPR.PRC_ID = PRC.PRC_ID
      LEFT JOIN HAYA01.ASU_ASUNTOS ASU ON PRC.ASU_ID = ASU.ASU_ID
      LEFT JOIN HAYAMASTER.DD_EAS_ESTADO_ASUNTOS DD_EAS ON ASU.DD_EAS_ID = DD_EAS.DD_EAS_ID AND DD_EAS.DD_EAS_CODIGO IN ('05','06')
      LEFT JOIN HAYAMASTER.DD_EDE_ESTADOS_DECISION DD_EDE ON DPR.DD_EDE_ID = DD_EDE.DD_EDE_ID AND DD_EDE.DD_EDE_CODIGO = '02'
...
TAR AS(
    ...
    FROM(
        ...
        FROM HAYA01.ASU_ASUNTOS ASU
          ...
          INNER JOIN ULT_PROC_PADRE PRC ON TAR.PRC_ID = PRC.PRC_ID
...
select 
  ...
  ,PRC.PRC_PRC_ID ID_PROCEDIMIENTO_PADRE
  ,'2038' ENTI
  ,'BANKIA' ENTIDAD
  ,'HAYA' SERVICER
  , CASE UPPER(TAS.DD_TAS_DESCRIPCION) 
    ...
    ,TAR.FECHA_DEMANDA
    ,CASE WHEN (FECHA_CIERRE.FECHA_RECIENTE IS not NULL AND FECHA_CIERRE.DD_EAS_CODIGO IN ('05', '06')) THEN FECHA_CIERRE.FECHA_RECIENTE ELSE NULL END FECHA_CIERRE
...
LEFT JOIN FECHA_CIERRE ON FECHA_CIERRE.PRC_ID = ULTIMO_PROC_ASUNTO.PRC_ID
...
--MERGE 3(SUBASTA)
MERGE INTO MINIREC.PROCEDIMIENTO_JUDICIAL_FOTO PJF
USING 
(
...
MERGE INTO MINIREC.PROCEDIMIENTO_JUDICIAL_FOTO VAL
USING (
...
-- MERGE 11 (MOTIVO CIERRE)

  MERGE INTO MINIREC.PROCEDIMIENTO_JUDICIAL_FOTO FOTO
USING (
    select * from (
        SELECT DPR.PRC_ID, DPR.DD_DFI_ID, DPR_COMENTARIOS, DFI.DD_DFI_DESCRIPCION, TRUNC(GREATEST(DPR.FECHACREAR, NVL(DPR.FECHAMODIFICAR,to_date('01/01/1900','DD/MM/RRRR')))) FECHA_CAMBIO, 
        ...
        FROM HAYA01.DPR_DECISIONES_PROCEDIMIENTOS DPR
        INNER JOIN HAYA01.DD_DFI_DECISION_FINALIZAR DFI ON DFI.DD_DFI_ID=DPR.DD_DFI_ID
        WHERE DPR.DD_EDE_ID = (SELECT DD_EDE_ID FROM HAYAMASTER.DD_EDE_ESTADOS_DECISION WHERE DD_EDE_DESCRIPCION = 'ACEPTADO')
    ) TMP_RN
      WHERE RN = 1) TMP
ON (FOTO.ID_PROCEDIMIENTO=TMP.PRC_ID)
WHEN MATCHED THEN UPDATE SET FOTO.MOTIVO_CIERRE=TMP.DD_DFI_DESCRIPCION;
