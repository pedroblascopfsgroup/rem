...
MERGE INTO "+context.schemaSANDBOX+".VAL_PROC_JUDICIAL_FOTO VAL
USING (
        SELECT TEV_VALOR, PRC.PRC_ID, ASU.ASU_ID FROM "+context.schemaRECOVERY+".TEV_TAREA_EXTERNA_VALOR TEV
        INNER JOIN "+context.schemaRECOVERY+".TEX_TAREA_EXTERNA TEX ON TEX.TEX_ID = TEV.TEX_ID
        INNER JOIN "+context.schemaRECOVERY+".TAR_TAREAS_NOTIFICACIONES TAR ON TEX.TAR_ID = TAR.TAR_ID
        INNER JOIN "+context.schemaRECOVERY+".PRC_PROCEDIMIENTOS PRC ON TAR.ASU_ID = PRC.ASU_ID
        INNER JOIN "+context.schemaRECOVERY+".ASU_ASUNTOS ASU ON PRC.ASU_ID = ASU.ASU_ID
...
MERGE INTO "+context.schemaSANDBOX+".VAL_PROC_JUDICIAL_FOTO VAL
USING (
        SELECT DISTINCT TEV_VALOR, PRC.PRC_ID, ASU.ASU_ID FROM "+context.schemaRECOVERY+".TEV_TAREA_EXTERNA_VALOR TEV
        INNER JOIN "+context.schemaRECOVERY+".TEX_TAREA_EXTERNA TEX ON TEX.TEX_ID = TEV.TEX_ID 
        INNER JOIN "+context.schemaRECOVERY+".TAP_TAREA_PROCEDIMIENTO TAP ON TEX.TAP_ID = TAP.TAP_ID
        INNER JOIN "+context.schemaRECOVERY+".TAR_TAREAS_NOTIFICACIONES TAR ON TEX.TAR_ID = TAR.TAR_ID
        INNER JOIN "+context.schemaRECOVERY+".PRC_PROCEDIMIENTOS PRC ON TAR.ASU_ID = PRC.ASU_ID       
        INNER JOIN "+context.schemaRECOVERY+".ASU_ASUNTOS ASU ON PRC.ASU_ID = ASU.ASU_ID
...
-- MERGE 11 (MOTIVO CIERRE)
  ...
  MERGE INTO MINIREC.VAL_PROC_JUDICIAL_FOTO FOTO
USING (
    select * from (
