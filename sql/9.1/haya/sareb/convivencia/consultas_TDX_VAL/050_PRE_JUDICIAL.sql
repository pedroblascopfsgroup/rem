...
--MERGE DEL CAMPO IN_TITULOS (no)

MERGE INTO MINIREC.VAL_PRE_JUDICIAL PJ
   USING (
      SELECT DISTINCT HAYA01.ASU.ASU_ID 
      FROM HAYA01.ASU_ASUNTOS ASU 
      INNER JOIN HAYA01.PRC_PROCEDIMIENTOS PRC ON PRC.ASU_ID = ASU.ASU_ID
 ...
 ON (TITULO.ASU_ID=PJ.ID_ASUNTO_HRE)
 WHEN MATCHED THEN 
 UPDATE SET PJ.IN_TITULOS='NO';

COMMIT;

--MERGE DEL CAMPO  IN_TITULOS (si)
...
MERGE INTO MINIREC.VAL_PRE_JUDICIAL PJ
   ...
   ON (TITULO.ASU_ID=PJ.ID_ASUNTO_HRE)
   WHEN MATCHED THEN 
   UPDATE SET PJ.IN_TITULOS='SI';

COMMIT;

--MERGE DEL CAMPO PREPARACÍON_DOCUMENTACIÓN

    MERGE INTO MINIREC.VAL_PRE_JUDICIAL PJ
    USING (
        SELECT ASU.ASU_ID, PCO.PRC_ID, MAX(DOC.FECHACREAR) AS FECHA
          FROM HAYA01.ASU_ASUNTOS ASU 
          INNER JOIN HAYA01.PRC_PROCEDIMIENTOS PRC ON ASU.ASU_ID = PRC.ASU_ID
          INNER JOIN HAYA01.PCO_PRC_PROCEDIMIENTOS PCO ON PRC.PRC_ID = PCO.PRC_ID
          INNER JOIN HAYA01.PCO_DOC_DOCUMENTOS DOC ON DOC.PCO_PRC_ID=PCO.PCO_PRC_ID
        GROUP BY ASU.ASU_ID, PCO.PRC_ID      
    ) FECHA
   ON (FECHA.ASU_ID=PJ.ID_ASUNTO_HRE)
   WHEN MATCHED THEN 
   UPDATE SET PJ.FC_PENDIENTE_LIQUIDAR=FECHA.FECHA
   WHERE PJ.IN_TITULOS='SI';
   
COMMIT;

--MERGE DEL CAMPO FECHA_SANCION_SAREB
