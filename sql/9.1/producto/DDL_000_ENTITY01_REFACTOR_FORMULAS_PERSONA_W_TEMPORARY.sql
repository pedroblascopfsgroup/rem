...
BEGIN
    ...
    V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.PER_PERSONAS_FORMULAS
				...
				DESC_FECHA_SIT_CONCURSAL VARCHAR2(50 CHAR),
				DESC_CLIENTE_REESTRUC VARCHAR2(50 CHAR)
   )';
   EXECUTE IMMEDIATE V_MSQL;
...
-- NUM_ASU_ACTIVOS_POR_PRC
 SELECT COUNT(1) INTO TABLE_COUNT FROM ALL_TABLES WHERE OWNER = V_ESQUEMA AND TABLE_NAME = 'TMP_PER_NUM_ASU_ACT_POR_PRC';
  ...
  -- DESC_FECHA_SIT_CONCURSAL
  SELECT COUNT(1) INTO TABLE_COUNT FROM ALL_TABLES WHERE OWNER = V_ESQUEMA AND TABLE_NAME = 'TMP_PER_DESC_F_SIT_CON';
    IF TABLE_COUNT > 0 THEN
      V_MSQL := 'DROP TABLE '||V_ESQUEMA||'.TMP_PER_DESC_F_SIT_CON';
      EXECUTE IMMEDIATE V_MSQL;
  END IF;
  
  V_MSQL := 'CREATE GLOBAL TEMPORARY TABLE '||V_ESQUEMA||'.TMP_PER_DESC_F_SIT_CON (
    PER_ID                  NUMBER(16) NOT NULL
    ,DESC_FECHA_SIT_CONCURSAL   VARCHAR2(50 CHAR) 
  )ON COMMIT DELETE ROWS'; EXECUTE IMMEDIATE V_MSQL;
  
  V_MSQL := 'CREATE INDEX '||V_ESQUEMA||'.IDX_TMP_PER_DESC_F_SIT_CON ON '||V_ESQUEMA||'.TMP_PER_DESC_F_SIT_CON(PER_ID)'; EXECUTE IMMEDIATE V_MSQL;  

    -- DESC_CLIENTE_REESTRUC
  SELECT COUNT(1) INTO TABLE_COUNT FROM ALL_TABLES WHERE OWNER = V_ESQUEMA AND TABLE_NAME = 'TMP_PER_DESC_CLI_REESTRUC';
    IF TABLE_COUNT > 0 THEN
      V_MSQL := 'DROP TABLE '||V_ESQUEMA||'.TMP_PER_DESC_CLI_REESTRUC PURGE';
      EXECUTE IMMEDIATE V_MSQL;
  END IF;
  
  V_MSQL := 'CREATE GLOBAL TEMPORARY TABLE '||V_ESQUEMA||'.TMP_PER_DESC_CLI_REESTRUC (
    PER_ID                  NUMBER(16) NOT NULL
    ,DESC_CLIENTE_REESTRUC   VARCHAR2(50 CHAR) 
  )ON COMMIT DELETE ROWS'; EXECUTE IMMEDIATE V_MSQL;
  
  V_MSQL := 'CREATE INDEX '||V_ESQUEMA||'.IDX_TMP_PER_DESC_CLI_REESTRUC ON '||V_ESQUEMA||'.TMP_PER_DESC_CLI_REESTRUC(PER_ID)'; EXECUTE IMMEDIATE V_MSQL;  
