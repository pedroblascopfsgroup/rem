...
DECLARE
    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.DD_PCO_LIQ_ESTADO
			   ...
			   ,CONSTRAINT PK_DD_PCO_LIQ_ESTADO_ID PRIMARY KEY (DD_PCO_LIQ_ID)
			   ,CONSTRAINT UNIQUE_DD_PCO_LIQ_CODIGO UNIQUE (DD_PCO_LIQ_CODIGO)
               )';
		...
		'  PCTFREE 10 INITRANS 2 MAXTRANS 255 COMPUTE STATISTICS STORAGE(INITIAL 65536 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS 2147483645 ' ||
  	    ...
  	    execute immediate 'CREATE SEQUENCE ' || V_ESQUEMA || '.S_DD_PCO_LIQ_ESTADO  MINVALUE 1 MAXVALUE 999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 100 NOORDER  NOCYCLE';
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.S_DD_PCO_LIQ_ESTADO... Secuencia creada correctamente.');
    END IF;
