...
CREATE VIEW '||V_ESQUEMA||'.'||V_NOMBRE_VISTA||' AS
  SELECT
    ...
    END PEF_ID
	FROM HAYA01.TAR_TAREAS_NOTIFICACIONES TN
	  LEFT JOIN HAYA01.EXP_EXPEDIENTES EXP ON TN.EXP_ID=EXP.EXP_ID
	    INNER JOIN HAYA01.ARQ_ARQUETIPOS ARQ ON EXP.ARQ_ID=ARQ.ARQ_ID
	    INNER JOIN HAYA01.OFI_OFICINAS OFI ON EXP.OFI_ID=OFI.OFI_ID
	      INNER JOIN HAYA01.ZON_ZONIFICACION DDZON ON OFI.OFI_ID=DDZON.OFI_ID
	    LEFT JOIN HAYAMASTER.DD_EST_ESTADOS_ITINERARIOS DDEST ON EXP.DD_EST_ID=DDEST.DD_EST_ID
	      LEFT JOIN HAYA01.EST_ESTADOS EST ON DDEST.DD_EST_ID=EST.DD_EST_ID
	  INNER JOIN HAYAMASTER.DD_EST_ESTADOS_ITINERARIOS DDESTITI ON TN.DD_EST_ID=DDESTITI.DD_EST_ID
	  INNER JOIN HAYAMASTER.DD_EIN_ENTIDAD_INFORMACION DDTPOENTI ON TN.DD_EIN_ID=DDTPOENTI.DD_EIN_ID
	  INNER JOIN HAYAMASTER.DD_STA_SUBTIPO_TAREA_BASE STA ON TN.DD_STA_ID=STA.DD_STA_ID
	  LEFT JOIN HAYA01.TEX_TAREA_EXTERNA TEX ON TN.TAR_ID = TEX.TAR_ID
	     LEFT JOIN HAYA01.TAP_TAREA_PROCEDIMIENTO TAP ON TEX.TAP_ID = TAP.TAP_ID
	WHERE  TN.BORRADO=0
