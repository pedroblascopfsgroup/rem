...
BEGIN
	...
	IF table_count = 1 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.ACU_CONFIG_ACUERDO_ASUNTO... Ya existe');
	ELSE
		...
		(
		  ...
		  DD_TGE_ID_PROPONENTE    NUMBER(16)                   NOT NULL,
		  DD_TGE_ID_VALIDADOR    NUMBER(16)                   NOT NULL,
		  DD_TGE_ID_DECISOR    NUMBER(16)                   NOT NULL,
          VERSION                  INTEGER,
		...
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.ACU_CONFIG_ACUERDO_ASUNTO ADD (
						CONSTRAINT DD_TGE_ID_PROPONENTE_FK FOREIGN KEY (DD_TGE_ID_PROPONENTE) REFERENCES '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR (DD_TGE_ID) ON DELETE SET NULL
						)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.DD_TGE_ID_PROPONENTE_FK... Creando FK');
		
		
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.ACU_CONFIG_ACUERDO_ASUNTO ADD (
						CONSTRAINT DD_TGE_ID_VALIDADOR_FK FOREIGN KEY (DD_TGE_ID_VALIDADOR) REFERENCES '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR (DD_TGE_ID) ON DELETE SET NULL
						)';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.ACU_CONFIG_ACUERDO_ASUNTO ADD (
						CONSTRAINT DD_TGE_ID_DECISOR_FK FOREIGN KEY (DD_TGE_ID_DECISOR) REFERENCES '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR (DD_TGE_ID) ON DELETE SET NULL
						)';
		EXECUTE IMMEDIATE V_MSQL;
