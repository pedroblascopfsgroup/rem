...
DECLARE
    ...
    V_TABLA VARCHAR2(50 CHAR) := 'ETU_ESQUEMA_TURNADO';
    V_SECUENCIA VARCHAR2(30 CHAR) := 'S_ETU_ESQUEMA_TURNADO';
    V_PRIMARY VARCHAR2(30 CHAR) := 'PK_ETU_ESQUEMA_TURNADO';
...
--##CREACION DE TABLA, SECUENCIA Y PK de tabla
  ...
  DBMS_OUTPUT.PUT('[INFO] Tabla '||V_TABLA||': CREADA...');	     	 
	...
	(
		...
		ETU_DESCRIPCION	            VARCHAR2(100)       NOT NULL,
		DD_EET_ID 		    		NUMBER(16)          NOT NULL,
		ETU_CON_STOCK_ANUAL 		NUMBER(5,2)          	,
		ETU_LIT_STOCK_ANUAL 		NUMBER(5,2)          	,
		ETU_FECHA_INI_VIGENCIA 		TIMESTAMP(6)       	,
		ETU_FECHA_FIN_VIGENCIA 		TIMESTAMP(6)       	,
		...
		CONSTRAINT CHK_CON_STOCK CHECK (ETU_CON_STOCK_ANUAL BETWEEN 0 AND 100),
		CONSTRAINT CHK_LIT_STOCK CHECK (ETU_LIT_STOCK_ANUAL BETWEEN 0 AND 100)
	)';
	
	DBMS_OUTPUT.PUT_LINE('OK');

	DBMS_OUTPUT.PUT('[INFO] PK '||V_TABLA||' (ETU_ID): CREADA...');
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (
  			CONSTRAINT '||V_PRIMARY||' PRIMARY KEY(ETU_ID))';

  	DBMS_OUTPUT.PUT_LINE('OK');
  
  	V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.'||V_TABLA||'  ADD (
			CONSTRAINT FK_ETU_EET FOREIGN KEY (DD_EET_ID) 
				 REFERENCES ' || V_ESQUEMA || '.DD_EET_ESTADO_ESQUEMA_TURNADO (DD_EET_ID))';
			
	EXECUTE IMMEDIATE V_MSQL;     
	DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.FK_ETU_EET ... FK Creada');
