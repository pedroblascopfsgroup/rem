...
BEGIN
    ...
    V_SQL := 'SELECT COUNT(1) FROM ALL_TABLES WHERE TABLE_NAME = ''TMP_CICLOS_ESC'' and owner = '''||v_esquema||'''';
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.TMP_CICLOS_ESC CASCADE CONSTRAINTS';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.TMP_CICLOS_ESC... Tabla borrada');  
    END IF;
    DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.TMP_CICLOS_ESC... Comprobaciones previas FIN');
    ...
    V_MSQL := ' CREATE GLOBAL TEMPORARY TABLE '||v_esquema||'.TMP_CICLOS_ESC
		(	
	      ...
	      "RCF_ESQ_ID_DEST"           NUMBER, 
	      "RCF_ESC_ID_DEST"           NUMBER
		) ON COMMIT PRESERVE ROWS';
		
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.TMP_CICLOS_ESC... Tabla creada');   
