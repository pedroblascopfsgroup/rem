...
SET SERVEROUTPUT ON;
create or replace PROCEDURE REFRESH_DATA_RULE_ENGINE AUTHID CURRENT_USER AS
 ...
 SELECT /*+ PARALLEL */
    ...
    FROM  #ESQUEMA#.per_personas per
      ...
      LEFT JOIN (select cac.cnt_id, cac.cac_actuacion_en_curso
                   ...
                   join #ESQUEMA_MASTER#.DD_SIN_SINO sin ON cac.cac_actuacion_en_curso = sin.DD_SIN_ID) cac ON cac.CNT_ID=cnt.CNT_ID
...
BEGIN
  ...
  #ESQUEMA#.OPERACION_DDL.DDL_TABLE('TRUNCATE','DATA_RULE_ENGINE');
  ...
  LOOP
    ...
    INSERT INTO  #ESQUEMA#.DATA_RULE_ENGINE
			(PER_ID
			,PER_RIESGO
			,PER_RIESGO_AUTORIZADO
			,PER_RIESGO_IND
