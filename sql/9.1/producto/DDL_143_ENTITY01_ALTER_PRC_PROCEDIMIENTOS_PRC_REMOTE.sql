...
--## PRODUCTO=SI
--##
--## Finalidad: Crear nuevo campo en la table PRC_PROCEDIMIENTOS
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''PRC_REMOTO'' and TABLE_NAME=''PRC_PROCEDIMIENTOS'' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;
	
	IF table_count = 1 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.PRC_PROCEDIMIENTOS.PRC_REMOTO... Ya existe');
	ELSE
		----Creamos el campo
		EXECUTE IMMEDIATE 'ALTER TABLE ' || V_ESQUEMA || '.PRC_PROCEDIMIENTOS ADD 
			(PRC_REMOTO NUMBER(1))';
		
		DBMS_OUTPUT.PUT_LINE('ALTER TABLE '|| V_ESQUEMA ||'.PRC_PROCEDIMIENTOS ADD PRC_REMOTO ... OK');
