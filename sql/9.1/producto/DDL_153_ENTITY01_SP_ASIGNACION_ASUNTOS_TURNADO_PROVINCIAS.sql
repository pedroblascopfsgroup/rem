...
IS
  ...
  -- PRODUCTO-582 - 4 CURSORES PARA ELEGIR LA PROVINCIA DEL ASUNTO crs_filtro_situacion, nuevas restricciones para elegir la provincia.
   ...
   CURSOR crs_filtro_prov_bien_contrato (p_asu_id BANK01.asu_asuntos.asu_id%TYPE)
   IS
	SELECT DISTINCT bil.dd_prv_id
		FROM BANK01.asu_asuntos asu
			JOIN BANK01.exp_expedientes exp ON asu.exp_id=exp.exp_id
			JOIN BANK01.cex_contratos_expediente cex ON exp.exp_id=cex.EXP_ID
			JOIN BANK01.cnt_contratos cnt ON cex.cnt_id=cnt.cnt_id
			JOIN BANK01.bie_cnt bic ON cnt.cnt_id=bic.cnt_id
			JOIN BANK01.bie_localizacion bil ON bic.bie_id=bil.bie_id AND bil.dd_prv_id IS NOT NULL
   ...
   CURSOR crs_filtro_prov_bien_menor (p_asu_id BANK01.asu_asuntos.asu_id%TYPE)
   IS			
	SELECT DISTINCT FIRST_VALUE (dir.dd_prv_id) OVER (ORDER BY cnt.cnt_dispuesto DESC, cpe.cpe_orden ASC)
		FROM BANK01.asu_asuntos asu
			...
			JOIN BANK01.cex_contratos_expediente cex ON exp.exp_id=cex.EXP_ID
			JOIN BANK01.cnt_contratos cnt ON cex.cnt_id=cnt.cnt_id
			JOIN BANK01.cpe_contratos_personas cpe ON cnt.cnt_id=cpe.cnt_id
			JOIN BANK01.dir_per dip ON cpe.per_id=dip.per_id
			JOIN BANK01.dir_direcciones dir ON dip.dir_id=dir.dir_id AND dir.dd_prv_id IS NOT NULL
		WHERE asu.asu_id = p_asu_id
	...
	CURSOR crs_filtro_prov_oficina (p_asu_id BANK01.asu_asuntos.asu_id%TYPE)
   	IS
   		SELECT DISTINCT FIRST_VALUE (ofi.dd_prv_id) OVER (ORDER BY cnt.cnt_dispuesto DESC)
			FROM BANK01.asu_asuntos asu
				...
				JOIN BANK01.cex_contratos_expediente cex ON exp.exp_id=cex.EXP_ID
				JOIN BANK01.cnt_contratos cnt ON cex.cnt_id=cnt.cnt_id
				JOIN BANK01.ofi_oficinas ofi ON cnt.ofi_id=ofi.ofi_id
			WHERE asu.asu_id = p_asu_id
	...
	CURSOR crs_filtro_prov_bien_tas (p_asu_id BANK01.asu_asuntos.asu_id%TYPE)
   	IS
		SELECT DISTINCT FIRST_VALUE (bil.dd_prv_id) OVER (ORDER BY cnt.cnt_dispuesto DESC, val.BIE_IMPORTE_VALOR_TASACION DESC )
			FROM BANK01.asu_asuntos asu
				...
				JOIN BANK01.bie_cnt bic ON cnt.cnt_id=bic.cnt_id
				JOIN BANK01.bie_localizacion bil ON bic.bie_id=bil.bie_id AND bil.dd_prv_id is not null
				JOIN BANK01.bie_valoraciones val ON val.bie_id=bic.bie_id
      ...
      -- Cursor para elegir despacho segun la provincia elegida  y segun la calidad de la provincia del despacho
	CURSOR crs_filtro_despacho_prov (
      p_asu_id       BANK01.asu_asuntos.asu_id%TYPE,
      p_dd_prv_id	 BANKMASTER.dd_prv_provincia.dd_prv_id%TYPE)
	IS
	    SELECT des_id FROM (
	      SELECT DISTINCT daa.des_id, daa.DAA_PROV_CALIDAD_PORCENTAJE
	           FROM   BANKMASTER.dd_prv_provincia prv
	              JOIN BANK01.daa_despacho_ambito_actuacion daa ON daa.dd_prv_id = prv.dd_prv_id
	              JOIN BANK01.des_despacho_externo des ON daa.des_id = des.des_id
	            WHERE prv.dd_prv_id = p_dd_prv_id
	              AND daa.DAA_PROV_CALIDAD_PORCENTAJE IS NOT NULL
	              AND prv.borrado=0 AND daa.borrado=0 AND des.borrado=0
	              ORDER BY daa.DAA_PROV_CALIDAD_PORCENTAJE DESC);
   ...
   IS
    ...
    CURSOR crs_relacion_existente (p_tge_codigo BANKMASTER.dd_tge_tipo_gestor.dd_tge_codigo%TYPE, p_asu_id BANK01.asu_asuntos.asu_id%TYPE, v_des_id BANK01.des_despacho_externo.des_id%TYPE)
    IS
      SELECT distinct FIRST_VALUE (gah_id) OVER (ORDER BY gah_fecha_desde ASC)
      FROM BANK01.gah_gestor_adicional_historico 
              WHERE gah_fecha_hasta is null AND gah_gestor_id = 
                  (SELECT DISTINCT FIRST_VALUE (usd.usd_id) OVER (ORDER BY usd.usd_gestor_defecto DESC) FROM BANK01.usd_usuarios_despachos usd WHERE usd.des_id = v_des_id)
                AND gah_tipo_gestor_id = (SELECT tge.dd_tge_id  FROM BANKMASTER.dd_tge_tipo_gestor tge  WHERE tge.dd_tge_codigo = p_tge_codigo)
   ...
   v_dd_prv_id				 BANKMASTER.DD_PRV_PROVINCIA.dd_prv_id%TYPE;
   v_gah_existente           BANK01.gah_gestor_adicional_historico.gah_id%TYPE;
BEGIN
...
END IF;
   ...
   THEN
      ...
      INSERT INTO BANK01.gaa_gestor_adicional_asunto
           ...
           VALUES (BANK01.s_gaa_gestor_adicional_asunto.NEXTVAL, p_asu_id, 
        ...
        UPDATE BANK01.gah_gestor_adicional_historico gah
          SET gah.gah_fecha_hasta = SYSDATE
          WHERE gah.gah_asu_id = p_asu_id
            AND gah.gah_tipo_gestor_id = (SELECT tge.dd_tge_id  FROM BANKMASTER.dd_tge_tipo_gestor tge  WHERE tge.dd_tge_codigo = p_tge_codigo)
            AND gah.gah_fecha_hasta is null
            AND gah.gah_gestor_id <> (SELECT DISTINCT FIRST_VALUE (usd.usd_id) OVER (ORDER BY usd.usd_gestor_defecto DESC) FROM BANK01.usd_usuarios_despachos usd WHERE usd.des_id = v_des_id);
        ...
        THEN
          INSERT INTO BANK01.gah_gestor_adicional_historico
              ...
              (SELECT BANK01.s_gah_gestor_adic_historico.NEXTVAL
              , (SELECT DISTINCT FIRST_VALUE (usd.usd_id) OVER (ORDER BY usd.usd_gestor_defecto DESC) FROM BANK01.usd_usuarios_despachos usd WHERE usd.des_id = v_des_id)
