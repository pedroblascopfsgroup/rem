...
BEGIN	
    ...
    V_SQL := 'UPDATE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_VALOR_INICIAL=''dameProcedimientoPropuesto()'', TFI_ERROR_VALIDACION = '''', TFI_VALIDACION = '''' WHERE TAP_CODIGO IN  ' || 
      '(''PCO_RegistrarTomaDec'') AND TFI_NOMBRE = ''proc_propuesto''';
    EXECUTE IMMEDIATE V_SQL;
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('[FIN] '||V_ESQUEMA||'... ACTUALIZACION DEL CAMPO TFI_VALOR_INICIAL DE TFI_TAREAS_FORM_ITEMS');

    DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'... ACTUALIZACION DEL CAMPO TFI_TAREAS_FORM_ITEMS DE TFI_TAREAS_FORM_ITEMS PCO_RegistrarTomaDec');
    V_SQL := 'UPDATE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_TIPO=''combo'', TFI_BUSINESS_OPERATION=''DDTipoProblemaDocPco'' WHERE TAP_CODIGO IN  ' || 
      '(''PCO_RegistrarTomaDec'') AND TFI_NOMBRE = ''tipo_problema''';
    EXECUTE IMMEDIATE V_SQL;
    COMMIT;
