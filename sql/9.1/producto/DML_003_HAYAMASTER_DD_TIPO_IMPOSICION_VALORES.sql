...
BEGIN
	...
	ELSE
		V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.DD_TPI_TIPO_IMPOSICION (DD_TPI_ID,DD_TPI_CODIGO,DD_TPI_DESCRIPCION,DD_TPI_DESCRIPCION_LARGA,VERSION,USUARIOCREAR,FECHACREAR,BORRADO) VALUES(S_TPI_TIPO_IMPOSICION.nextval,''16'',''16%'',''16%'',0,''DML'',sysdate,0)';
    ...
    IF V_NUM_TABLAS > 0 THEN	  
		DBMS_OUTPUT.PUT_LINE('[INFO] Ya existen el registro en la tabla '||V_ESQUEMA_M||'.DD_TPI_TIPO_IMPOSICION.');
	ELSE
		V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.DD_TPI_TIPO_IMPOSICION (DD_TPI_ID,DD_TPI_CODIGO,DD_TPI_DESCRIPCION,DD_TPI_DESCRIPCION_LARGA,VERSION,USUARIOCREAR,FECHACREAR,BORRADO) values(S_TPI_TIPO_IMPOSICION.nextval,''21'',''21%'',''21%'',0,''DML'',sysdate,0)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Registro insertado en '||V_ESQUEMA_M||'.DD_TPI_TIPO_IMPOSICION.');
	END IF;
