...
DECLARE
    ...
    V_DDNAME VARCHAR2(30):= 'DD_EIN_ENTIDAD_INFORMACION';
...
DBMS_OUTPUT.PUT_LINE('[INFO] Bloque scripts para la inclusión de un nuevo tipo de registro del histórico de operaciones - Notificación - (1/4)');
...
-- */
V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA_M||'.'||V_DDNAME||' WHERE DD_EIN_CODIGO= ''10''';

EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

IF V_NUM_TABLAS > 0 THEN    
  DBMS_OUTPUT.PUT_LINE('OK no se modifica nada.');
  DBMS_OUTPUT.PUT_LINE('[INFO] Ya existe el registro "Notificacion" en el diccionario '||V_DDNAME||'.');
