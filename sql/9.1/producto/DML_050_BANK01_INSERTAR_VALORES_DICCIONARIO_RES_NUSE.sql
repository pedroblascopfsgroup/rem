...
BEGIN	
    ...
    FOR I IN V_TIPO_RVC.FIRST .. V_TIPO_RVC.LAST
      LOOP
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.DD_RVN_RES_VALIDACION_NUSE WHERE DD_RVN_CODIGO = '''||TRIM(V_TMP_TIPO_RVC(1))||'''';
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_RVC(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA_M ||'.DD_RVN_RES_VALIDACION_NUSE '||
       ...
       ELSE
          ...
          V_MSQL := 'SELECT '|| V_ESQUEMA_M ||'.S_DD_RVN_RES_VALIDACION_NUSE.NEXTVAL FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL INTO V_ID;	
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.DD_RVN_RES_VALIDACION_NUSE (' ||
                      'DD_RVN_ID, DD_RVN_CODIGO, DD_RVN_DESCRIPCION, DD_RVN_DESCRIPCION_LARGA, DD_RVN_CODIGO_UVEM, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_RVC(1)||''','''||TRIM(V_TMP_TIPO_RVC(2))||''','''||TRIM(V_TMP_TIPO_RVC(3))||''','''||TRIM(V_TMP_TIPO_RVC(4))||''', 0, ''FASE-1464'',SYSDATE,0 FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
