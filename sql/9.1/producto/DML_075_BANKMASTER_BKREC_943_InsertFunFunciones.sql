...
DBMS_OUTPUT.PUT_LINE('[INICIO]');
	...
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION=''MENU-LIST-EXP-RECOBRO-ALL-USERS''';
	...
	IF CUENTA=0 THEN
	  EXECUTE IMMEDIATE 'insert INTO '||V_ESQUEMA_M||'.FUN_FUNCIONES(FUN_ID,FUN_DESCRIPCION_LARGA,FUN_DESCRIPCION,VERSION,USUARIOCREAR,FECHACREAR) 
			values('||V_ESQUEMA_M||'.S_FUN_FUNCIONES.NEXTVAL,''Muestra el menú de busqueda de expedientes de recobro aunque el usuario sea externo'',''MENU-LIST-EXP-RECOBRO-ALL-USERS'',0,''BKREC-943'',sysdate)';
	  COMMIT;
