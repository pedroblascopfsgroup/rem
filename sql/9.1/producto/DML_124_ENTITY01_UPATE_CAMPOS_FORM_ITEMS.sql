...
BEGIN	
    ...
    IF V_NUM_TABLAS > 0 THEN
		...
		V_SQL := 'UPDATE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_LABEL = ''Subsanar'' WHERE  TFI_NOMBRE = ''subsanar'' AND TAP_ID =  ' || 
      		'(SELECT TAP_ID FROM TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''PCO_RevisarSubsanacionCONC'')';
    	EXECUTE IMMEDIATE V_SQL;
    ...
    IF V_NUM_TABLAS > 0 THEN
		...
		V_SQL := 'UPDATE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_ERROR_VALIDACION = '''', TFI_VALIDACION = '''' WHERE  TFI_NOMBRE = ''anterior_letrado'' AND TAP_ID =  ' || 
      		'(SELECT TAP_ID FROM TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''PCO_RevisarNoAceptacion'')';
    	EXECUTE IMMEDIATE V_SQL;
    ...
    IF V_NUM_TABLAS > 0 THEN
    	...
    	V_SQL := 'UPDATE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS SET TFI_ERROR_VALIDACION = '''', TFI_VALIDACION = '''' WHERE  TFI_NOMBRE = ''anterior_letrado'' AND TAP_ID =  ' || 
      		'(SELECT TAP_ID FROM TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''PCO_RevisarNoAceptacionPost'')';
    	EXECUTE IMMEDIATE V_SQL;
