...
ELSE
	...
	V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.FUN_FUNCIONES (FUN_ID, FUN_DESCRIPCION_LARGA, FUN_DESCRIPCION,  VERSION, USUARIOCREAR, FECHACREAR,  BORRADO )  VALUES '||
				'(S_FUN_FUNCIONES.NEXTVAL, ''Solicitar Expediente Manual Recobro'', ''SOLICITAR_EXP_MANUAL_RECOBRO'', 0, ''DML'', SYSDATE, 0)';
	EXECUTE IMMEDIATE V_MSQL;
END IF;
...
ELSE
	...
	V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.FUN_FUNCIONES (FUN_ID, FUN_DESCRIPCION_LARGA, FUN_DESCRIPCION,  VERSION, USUARIOCREAR, FECHACREAR,  BORRADO )  VALUES '||
				'(S_FUN_FUNCIONES.NEXTVAL, ''Solicitar Expediente Manual Seguimiento'', ''SOLICITAR_EXP_MANUAL_SEGUIMIENTO'', 0, ''DML'', SYSDATE, 0)';
	EXECUTE IMMEDIATE V_MSQL;
END IF;
...
IF V_NUM_TABLAS = 0 THEN
	...
	V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.FUN_FUNCIONES (FUN_ID, FUN_DESCRIPCION_LARGA, FUN_DESCRIPCION,  VERSION, USUARIOCREAR, FECHACREAR,  BORRADO )  VALUES '||
				'(S_FUN_FUNCIONES.NEXTVAL, ''Solicitar Expediente Manual Recuperaciones'', ''SOLICITAR_EXP_MANUAL_RECUPERACIONES'', 0, ''DML'', SYSDATE, 0)';
	EXECUTE IMMEDIATE V_MSQL;
END IF;
