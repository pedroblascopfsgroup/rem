...
BEGIN	
    ...
    IF V_NUM_TABLAS > 0 THEN
    	...
    	V_SQL := 'DELETE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS WHERE TFI_NOMBRE = ''anterior_letrado'' AND TAP_ID =  ' || 
    ...
    IF V_NUM_TABLAS > 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'... ACTUALIZACION DEL CAMPO TFI_TAREAS_FORM_ITEMS PCO_RevisarNoAceptacion');
    	V_SQL := 'DELETE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS WHERE TFI_NOMBRE = ''nuevo_letrado'' AND TAP_ID =  ' || 
      		'(SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''PCO_RevisarNoAceptacion'')';
    	EXECUTE IMMEDIATE V_SQL;
    ...
    IF V_NUM_TABLAS > 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'... ACTUALIZACION DEL CAMPO TFI_TAREAS_FORM_ITEMS PCO_RevisarNoAceptacion');
    	V_SQL := 'DELETE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS WHERE TFI_NOMBRE = ''asignacion_correcta'' AND TAP_ID =  ' || 
      		'(SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''PCO_RevisarNoAceptacion'')';
    	EXECUTE IMMEDIATE V_SQL;
    ...
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    IF V_NUM_TABLAS > 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'... ACTUALIZACION DEL CAMPO TFI_TAREAS_FORM_ITEMS PCO_RevisarNoAceptacionPost');
    	V_SQL := 'DELETE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS WHERE TFI_NOMBRE = ''anterior_letrado'' AND TAP_ID =  ' || 
