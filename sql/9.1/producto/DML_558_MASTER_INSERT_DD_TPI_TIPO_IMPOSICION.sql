...
--## PRODUCTO=SI
--##
--## Finalidad: inserción en la tabla DD_TPI_TIPO_IMPOSICION nuevos valores para la ficha del BIEN
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN	
	...
	V_SQL := 'UPDATE '||V_ESQUEMA_M||'.DD_TPI_TIPO_IMPOSICION set BORRADO = 1';
	EXECUTE IMMEDIATE V_SQL; 
	
   	V_TABLENAME := V_ESQUEMA_M || '.DD_TPI_TIPO_IMPOSICION';
    V_SEQUENCENAME := V_ESQUEMA_M || '.S_DD_TPI_TIPO_IMPOSICION';
    ...
    ON (tabla.DD_TPI_CODIGO=actual.codigo)
    WHEN NOT MATCHED THEN
    INSERT (DD_TPI_ID,DD_TPI_CODIGO,DD_TPI_DESCRIPCION,DD_TPI_DESCRIPCION_LARGA,USUARIOCREAR,FECHACREAR,VERSION,BORRADO)
	...
	WHEN MATCHED THEN
	UPDATE SET DD_TPI_DESCRIPCION=actual.descripcion, DD_TPI_DESCRIPCION_LARGA=actual.descripcion_larga, usuariomodificar=actual.usuariocrear, fechamodificar=actual.fechacrear, borrado=actual.borrado where DD_TPI_CODIGO=actual.codigo]';
        EXECUTE IMMEDIATE V_SQL; 
