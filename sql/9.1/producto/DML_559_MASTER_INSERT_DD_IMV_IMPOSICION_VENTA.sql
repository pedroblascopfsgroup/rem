...
--## PRODUCTO=SI
--##
--## Finalidad: inserción en la tabla DD_IMV_IMPOSICION_VENTA nuevos valores para la ficha del BIEN
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_TIPO_TPO T_ARRAY_TPO := T_ARRAY_TPO(
	    T_TIPO_TPO('7','7% (IGIC)'),
		T_TIPO_TPO('4','4% (IVA)'),
		T_TIPO_TPO('10','10% (IVA)'),
		T_TIPO_TPO('21','21% (IVA)'),
		T_TIPO_TPO('CCAA','CCAA (TPO)')
	); 
    V_TMP_TIPO_TPO T_TIPO_TPO;
BEGIN	
	-- Borramos los registros, y si existen, luego los actualizamos con el merge
	V_SQL := 'UPDATE '||V_ESQUEMA_M||'.DD_IMV_IMPOSICION_VENTA set BORRADO = 1';
	EXECUTE IMMEDIATE V_SQL; 
	
   	V_TABLENAME := V_ESQUEMA_M || '.DD_IMV_IMPOSICION_VENTA';
    V_SEQUENCENAME := V_ESQUEMA_M || '.S_DD_IMV_IMPOSICION_VENTA';
    ...
    ON (tabla.DD_IMV_CODIGO=actual.codigo)
    WHEN NOT MATCHED THEN
    INSERT (DD_IMV_ID,DD_IMV_CODIGO,DD_IMV_DESCRIPCION,DD_IMV_DESCRIPCION_LARGA,USUARIOCREAR,FECHACREAR,VERSION,BORRADO)
	...
	WHEN MATCHED THEN
	UPDATE SET DD_IMV_DESCRIPCION=actual.descripcion, DD_IMV_DESCRIPCION_LARGA=actual.descripcion_larga, usuariomodificar=actual.usuariocrear, fechamodificar=actual.fechacrear, borrado=actual.borrado where DD_IMV_CODIGO=actual.codigo]';
        EXECUTE IMMEDIATE V_SQL; 
