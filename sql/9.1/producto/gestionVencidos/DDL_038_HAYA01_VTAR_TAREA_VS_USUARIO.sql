...
FROM (SELECT *
    ...
    JOIN HAYAMASTER.DD_EIN_ENTIDAD_INFORMACION EIN ON TAR.DD_EIN_ID = EIN.DD_EIN_ID
    JOIN HAYAMASTER.DD_STA_SUBTIPO_TAREA_BASE STA ON TAR.DD_STA_ID = STA.DD_STA_ID
    ...
    LEFT JOIN EXP_EXPEDIENTES EXP ON TAR.EXP_ID = EXP.EXP_ID
      LEFT JOIN ARQ_ARQUETIPOS ARQ_EXP ON EXP.ARQ_ID = ARQ_EXP.ARQ_ID
        LEFT JOIN ITI_ITINERARIOS ITI_EXP ON ARQ_EXP.ITI_ID = ITI_EXP.ITI_ID
          LEFT JOIN HAYAMASTER.DD_TIT_TIPO_ITINERARIOS TITI_EXP ON ITI_EXP.DD_TIT_ID = TITI_EXP.DD_TIT_ID
    LEFT JOIN PER_PERSONAS PER ON TAR.PER_ID = PER.PER_ID
      LEFT JOIN ARQ_ARQUETIPOS ARQ_PER ON PER.ARQ_ID = ARQ_PER.ARQ_ID
        LEFT JOIN ITI_ITINERARIOS ITI_PER ON ARQ_PER.ITI_ID = ITI_PER.ITI_ID
          LEFT JOIN HAYAMASTER.DD_TIT_TIPO_ITINERARIOS TITI_PER ON ITI_PER.DD_TIT_ID = TITI_PER.DD_TIT_ID    
    LEFT JOIN HAYAMASTER.DD_EST_ESTADOS_ITINERARIOS EST ON TAR.DD_EST_ID = EST.DD_EST_ID
