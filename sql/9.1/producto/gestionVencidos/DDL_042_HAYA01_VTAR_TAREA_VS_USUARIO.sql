	...
	SELECT V.USU_PENDIENTES
      , CASE WHEN (NVL(TAR.TAR_EN_ESPERA,0) = 1) THEN NVL(ESP.USU_ID,-1) ELSE -1 END USU_ESPERA
         ...
         , CASE EIN.DD_EIN_CODIGO
             ...
             WHEN ''7'' THEN SUP.APELLIDO_NOMBRE
          ...
          , CASE EIN.DD_EIN_CODIGO
             ...
             WHEN ''7'' THEN 
				CASE WHEN PEROBJ.PER_NOM50 IS NOT NULL THEN PEROBJ.PER_NOM50 ELSE TOB.TOB_DESCRIPCION END
             -- TODO poner para el resto de unidades de gestion
          ELSE NULL
...
FROM (SELECT *
           ...
           UNION
           SELECT *
             FROM '||V_ESQUEMA||'.VTAR_TAREA_VS_USUARIO_PART2
           UNION
           SELECT *
              FROM '||V_ESQUEMA||'.VTAR_TAREA_VS_USUARIO_EXPE
		   UNION
		   SELECT *
			  FROM '||V_ESQUEMA||'.VTAR_TAREA_VS_USUARIO_OBJ
             ) V
