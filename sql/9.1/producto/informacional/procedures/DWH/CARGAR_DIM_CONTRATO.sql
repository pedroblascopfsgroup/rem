...
-- ----------------------------------------------------------------------------------------------
  ...
  select count(*) into V_NUM_ROW from D_CNT_CARTERA where CARTERA_CONTRATO_ID = 1;
  if (V_NUM_ROW = 0) then
    insert into D_CNT_CARTERA (CARTERA_CONTRATO_ID, CARTERA_CONTRATO_DESC) values (1 ,'SAREB');
  end if;
...
-- ----------------------------------------------------------------------------------------------
  ...
  V_SQL :=  'BEGIN OPERACION_DDL.DDL_INDEX(''DROP'', ''D_CNT_PK'', '''', ''S'', '''', :O_ERROR_STATUS); END;';
  ...
  'insert /*+ APPend PARALLEL(D_CNT_1, 16) PQ_DISTRIBUTE(D_CNT_1, NONE) */ into D_CNT
    ...
    select distinct cnt.CNT_ID,
      ...
      NVL(DD_CT6_ID, -1),
      -- 0 - Bankia / 1 - SAREB
      decode(to_char(IAC_VALUE), ''2038'', 0, ''05074'', 1, ''5074'', 1, -1)
...
--      left join '||V_DATASTAGE||'.ZONA_JERARQUIA zon on cnt.CNT_COD_OFICINA = zon.NIVEL_0
 ...
 V_SQL :=  'BEGIN OPERACION_DDL.DDL_INDEX(''CREATE'', ''D_CNT_PK'', ''D_CNT (CONTRATO_ID)'', ''S'', '''', :O_ERROR_STATUS); END;';
            execute immediate V_SQL USING OUT O_ERROR_STATUS;
