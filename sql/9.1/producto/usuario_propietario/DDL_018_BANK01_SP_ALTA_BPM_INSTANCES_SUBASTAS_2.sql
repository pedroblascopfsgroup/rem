...
create or replace PROCEDURE ALTA_BPM_INSTANCES_SUBASTAS_2 AUTHID CURRENT_USER AS
  ...
  , TAP AS (
    ...
    WHERE TAP_CODIGO LIKE '%CelebracionSubasta'
      OR TAP_CODIGO LIKE '%LecturaAceptacionInstrucciones'
  )
  , TEX AS (
...
--  #ESQUEMA#.OPERACION_DDL.DDL_TABLE('TRUNCATE','TMP_ALTA_BPM_INSTANCES');	
  ...
  /* ----- Insertamos instancia de BPM ------- */
    ...
    FROM (
      ...
      , CASE WHEN TKNS.TAP_CODIGO LIKE ''%Lectura%'' THEN ''PrepararPropuesta''
    ...
    IF C_DEBUG = 1 THEN
      ...
      SELECT * FROM #ESQUEMA#.PRC_PROCEDIMIENTOS WHERE PRC_ID IN (SELECT PRC_ID FROM TMP_ALTA_BPM_INST_SUBASTAS)';

      EXECUTE IMMEDIATE 'INSERT INTO #ESQUEMA#.TMP_BPM_TEX_TAREA_EXTERNA
      SELECT * FROM #ESQUEMA#.TEX_TAREA_EXTERNA WHERE TEX_ID IN (SELECT TEX_ID FROM TMP_ALTA_BPM_TKNS_SUBASTAS)';
    END IF;
  ...
  --/* ----- Insertamos el moduleinstance para cada instancia ------- */
    ...
    V_SQL := V_SQL||' (ID_, CLASS_, VERSION_, NAME_, TOKEN_, TOKENVARIABLEMAP_, PROCESSINSTANCE_, STRINGVALUE_)
      ...
      , TMP.VMAP_ID  --TOKENVARIABLEMAP_
      , TMP.INST_ID --PROCESSINSTANCE_
      , TPO.DD_TPO_CODIGO||''_PrepararPropuestaSubasta'' --STRINGVALUE_
    FROM #ESQUEMA#.TMP_ALTA_BPM_INST_SUBASTAS TMP
  ...
  END IF;
    ...
    EXCEPTION
     WHEN OTHERS THEN
          ...
          ROLLBACK;
          RAISE;
END ALTA_BPM_INSTANCES_SUBASTAS_2;
/
