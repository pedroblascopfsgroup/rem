...
--## INCIDENCIA_LINK=HREOS-818
--## PRODUCTO=SI
--## Finalidad: Insert nuevos valores diccionario DD_TDN_TIPO_DOCUMENTO
--##           
...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_TDN_TIPO_DOCUMENTO';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ' USING (SELECT :id id, :codigo codigo, :des descripcion, :des_l descripcion_larga, ''HREOS-818'' usuariocrear, sysdate fechacrear from DUAL) actual ' ||
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('FOTO-04', 'Otra fotografía', 'Otra fotografía')
     ,T_TIPO('FOTO-01', 'Fotografía estado / avería', 'Fotografía estado / avería')
     ,T_TIPO('FOTO-02', 'Fotografía reparación', 'Fotografía reparación')
     ,T_TIPO('FOTO-05', 'Postventa: fotografía', 'Postventa: fotografía')
     ,T_TIPO('FOTO-06', 'Tasación: fotografía', 'Tasación: fotografía')
     ,T_TIPO('FOTO-07', 'Infografía', 'Infografía')
     ,T_TIPO('FOTO-08', 'Fotografía testigos', 'Fotografía testigos')
     ,T_TIPO('FOTO-09', 'Fotografía web', 'Fotografía web')
     ,T_TIPO('FOTO-10', 'Fotografía web principal activo', 'Fotografía web principal activo')
     ,T_TIPO('FOTO-11', 'Fotografía web principal promoción', 'Fotografía web principal promoción')
     ,T_TIPO('FOTO-03', 'Obra: fotografía estado / avería', 'Obra: fotografía estado / avería')
