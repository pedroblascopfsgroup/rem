...
--Para permitir la visualización de texto en un bloque PL/SQL utilizando DBMS_OUTPUT.PUT_LINE
	...
	BEGIN	
		...
		DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION Y MODIFICACION EN USD_USUARIOS_DESPACHOS');
		
		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS WHERE USU_ID = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''morandeira'') ' ||
			       		' AND DES_ID = (SELECT DES_ID FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = ''REMSUPACT'')';
       	EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
       ...
       ELSE
			DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS EL NUEVO DESPACHO EN USD_USUARIOS_DESPACHOS');
			
			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS USD'||
					'(USD_ID,USU_ID,DES_ID,USD_GESTOR_DEFECTO,USD_SUPERVISOR,VERSION,USUARIOCREAR,FECHACREAR,BORRADO) ' || 
					' (SELECT S_USD_USUARIOS_DESPACHOS.NEXTVAL FROM DUAL),'||
					' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''morandeira''),' ||
					' (SELECT DES_ID FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO DES WHERE DES.DES_DESPACHO = ''REMSUPACT'')' || 
					' 1,1,0,''HREOS-1506'',SYSDATE,0)';		
			
			EXECUTE IMMEDIATE V_MSQL;
			
		END IF;
		
		DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL DESPACHO REMADM POR REMACT EN USD_USUARIOS_DESPACHOS');
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS ' ||
				'SET DES_ID = (SELECT DES_ID FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = ''REMACT''),' ||
