...
--##            (para igualar en todos los entornos) 
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
	...
	T_TIPO_DATA('1', 'adelaroja' , 'TY!88g@E', 'AGUSTIN'  , 'DE LA ROJA', 'BUENO'     , 'adelaroja@haya.es'             , '0', 'HAYAGESACT', 'REMACT'),
	T_TIPO_DATA('1', 'ibenedito' , '#Huxy^3k', 'IGNACIO'  , 'BENEDITO'  , 'BORONAT'   , 'ibenedito@externos.bankia.com' , '0', 'HAYAGESACT', 'REMACT'),
	T_TIPO_DATA('1', 'jberenguer', 'J7!#VKW8', 'JOSÉ'     , 'BERENGUER' , 'ALEIXANDRE', 'jberenguer@haya.es'            , '0', 'HAYAGESACT', 'REMACT'),
	T_TIPO_DATA('1', 'jlpelaz'   , 'H^ZzF5D@', 'JOSE LUIS', 'PELAZ'     , 'GÓMEZ'     , 'jlpelaz@haya.es'               , '0', 'HAYAGESACT', 'REMACT'),
	T_TIPO_DATA('1', 'rdl'       , 'e>Ctv3?R', 'RAFAEL'   , 'DOMINGUEZ' , 'LEÓN'      , 'rdl@haya.es'                   , '0', 'HAYAGESACT', 'REMACT'),
	T_TIPO_DATA('1', 'rdura'     , 'C5%L^T+Q', 'RAÚL'     , 'DURÁ'      , 'GARCÉS'    , 'rdura@haya.es'                 , '0', 'HAYAGESACT', 'REMACT')
  ); 
...
BEGIN	
 ...
 DBMS_OUTPUT.PUT_LINE('[INICIO] ');
   ...
   FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
     LOOP
       ...
       DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN USD_USUARIOS_DESPACHOS ');
		...
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS' ||
					...
					' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_TIPO_DATA(2))||'''),' ||
					' 1,1,''HREOS-2022'',SYSDATE,0 FROM DUAL';
       ...
       -- Se comprueba que el nuevo usuario sea un Gestor de admisión
	--   IF TRIM(V_TMP_TIPO_DATA(9)) = 'HAYAGESADM' THEN
	...
	--		AND USU_ID_GRUPO = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''GESTADM'')
	--		'
	--		;
	--		
	--		EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
	...
	--   ELSE
	--		
	--		DBMS_OUTPUT.PUT_LINE('[INFO] El usuario no es un Gestor de admisión, no se inserta nada en '||V_ESQUEMA||'.GRU_GRUPOS_USUARIOS.');
	--		
	--   END IF;
