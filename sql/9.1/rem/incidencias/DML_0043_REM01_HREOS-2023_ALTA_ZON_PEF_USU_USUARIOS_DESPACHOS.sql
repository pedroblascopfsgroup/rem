...
--## INCIDENCIA_LINK=HREOS-2023
--## PRODUCTO=NO
--## 
--## Finalidad: Adición de perfil y despacho para usuario bbaviera
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
------------------------------------------------------------------------
--Comprobamos el dato a insertar en zon_pef_usu (el usuario ya existe)--
------------------------------------------------------------------------
	
DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN ZON_PEF_USU '); 
		
V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ZON_PEF_USU 
			WHERE USU_ID = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''bbaviera'')';
EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
		
-- Si existe se inserta el nuevo perfil
IF V_NUM_TABLAS > 0 THEN	  
	
	DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS EL NUEVO PERFIL PARA bbaviera EN ZON_PEF_USU');
	...
	V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU' ||
				...
				' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''bbaviera''),' ||
				' ''HREOS-2023'',
				SYSDATE,
				0 
				FROM DUAL';
	...
	DBMS_OUTPUT.PUT_LINE('[INFO]: REGISTRO DE NUEVO PERFIL PARA bbaviera INSERTADO CORRECTAMENTE EN ZON_PEF_USU');
...
IF V_NUM_TABLAS > 0 THEN	
	...
	DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS EL NUEVO DESPACHO PARA bbaviera EN USD_USUARIOS_DESPACHOS');
	...
	V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS' ||
			...
			' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''bbaviera''),' ||
			' 1,1,''HREOS-2023'',SYSDATE,0 FROM DUAL';
  	
	EXECUTE IMMEDIATE V_MSQL;

	DBMS_OUTPUT.PUT_LINE('[INFO]: REGISTRO DE NUEVO DESPACHO PARA bbaviera INSERTADO CORRECTAMENTE EN USD_USUARIOS_DESPACHOS');			
