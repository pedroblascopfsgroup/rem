...
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Tabla para gestionar la información administrativa de los activos.
--##           
...
DECLARE
    ...
    V_TEXT_TABLA_OLD VARCHAR2(2400 CHAR) := 'ACT_INF_ADMINISTRATIVA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla para gestionar la información administrativa de los activos.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	(
		ADM_ID 								NUMBER(16,0)                NOT NULL,
		ACT_ID           					NUMBER(16,0)                NOT NULL,
		DD_TVP_ID 							NUMBER(16,0),		
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(ADM_ID) TABLESPACE '||V_TABLESPACE_IDX;			
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');
	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (ADM_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_INFOADMIN_ACTIVO FOREIGN KEY (ACT_ID) REFERENCES '||V_ESQUEMA||'.ACT_ACTIVO (ACT_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_INFOADMIN_ACTIVO... Foreign key creada.');
	
	-- Creamos foreign key DD_TVP_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_INFOADMIN_DDTVPO FOREIGN KEY (DD_TVP_ID) REFERENCES '||V_ESQUEMA||'.DD_TVP_TIPO_VPO (DD_TVP_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_INFOADMIN_DDTVPO... Foreign key creada.');
	
	-- Creamos unique key ACT_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT UK_INFOADMIN_ACTIVO UNIQUE (ACT_ID, BORRADO) ENABLE';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.UK_INFOADMIN_ACTIVO... Unique key creada.');
