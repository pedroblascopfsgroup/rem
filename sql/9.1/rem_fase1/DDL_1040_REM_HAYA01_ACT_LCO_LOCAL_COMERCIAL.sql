...
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Tabla para gestionar la información comercial específica de los activos de tipo local comercial.
--##           
...
DECLARE
    ...
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla para gestionar la información comercial de los activos de tipo local comercial.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_LOCALCOM_INFOCOM FOREIGN KEY (ICO_ID) REFERENCES '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL (ICO_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_LOCALCOM_INFOCOM... Foreign key creada.');	
