...
--## PRODUCTO=SI
--##
--## Finalidad: Tabla para almacenar los gestores de activos.
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'GAC_GESTOR_ADD_ACTIVO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_TEXT_TABLA_OLD VARCHAR2(2400 CHAR) := 'ACT_GAC_GESTOR_ADD_ACTIVO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla para almacenar los gestores de activos.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GESTORADDI_GESTORENTIDAD FOREIGN KEY (GEE_ID) REFERENCES '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD (GEE_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GESTORADDI_GESTORENTIDAD... Foreign key creada.');
	
	-- Creamos foreign key ACT_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GESTORADDI_ACTIVO FOREIGN KEY (ACT_ID) REFERENCES '||V_ESQUEMA||'.ACT_ACTIVO (ACT_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GESTORADDI_ACTIVO... Foreign key creada.');
