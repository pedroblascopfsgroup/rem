...
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Tabla para gestionar la información de los trabajos de tasación de los activos.
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'ACT_TTA_TRABAJO_TASACION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_TEXT_TABLA_OLD VARCHAR2(2400 CHAR) := 'ACT_TAS_TASACION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla para gestionar la información de los trabajos de tasación de los activos.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_TASACION_TRABAJO FOREIGN KEY (TBJ_ID) REFERENCES '||V_ESQUEMA||'.ACT_TBJ_TRABAJO (TBJ_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_TASACION_TRABAJO... Foreign key creada.');
