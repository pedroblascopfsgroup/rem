...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_TAREAS_PARALIZADAS_ADMISION 
	AS
		...
		INNER JOIN ' || V_ESQUEMA || '.ACT_AJD_ADJJUDICIAL AJD ON BIE.BIE_ADJ_ID = AJD.BIE_ADJ_ID
		INNER JOIN ' || V_ESQUEMA || '.TAC_TAREAS_ACTIVOS TAC ON TAC.ACT_ID = AJD.ACT_ID
		...
		AND TAR.TAR_FECHA_FIN IS NULL
      	...
      	AND BIE.BIE_ADJ_F_REA_POSESION IS NOT NULL';
