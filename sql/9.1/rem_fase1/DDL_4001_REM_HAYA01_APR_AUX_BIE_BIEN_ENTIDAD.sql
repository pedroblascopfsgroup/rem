...
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Tabla auxiliar para el aprovisionamiento de la información de los bienes de la entidad.
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'APR_AUX_BIE_BIEN_ENTIDAD'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla auxiliar para el aprovisionamiento de la información de los bienes de la entidad.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	-- Creamos foreign key FK_APRBIEENTIDAD_APRBIEN
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_APRBIEENTIDAD_APRBIEN FOREIGN KEY (BIE_ID) REFERENCES '||V_ESQUEMA||'.APR_AUX_BIE_BIEN (BIE_ID) ON DELETE CASCADE)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_APRBIEENTIDAD_APRBIEN... Foreign key creada.');
