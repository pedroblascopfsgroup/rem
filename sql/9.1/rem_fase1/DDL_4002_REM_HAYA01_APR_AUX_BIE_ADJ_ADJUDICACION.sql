...
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Tabla auxiliar para el aprovisionamiento de la información de adjudicación de los bienes de recovery-HAYA.
--##           
...
DECLARE
    ...
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla auxiliar para el aprovisionamiento de la información de adjudicación de los bienes de recovery-HAYA.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	(
		BIE_ADJ_ID 								NUMBER(16,0)			NOT NULL, 
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.APR_AUX_BIE_ADJ_ADJUDICA_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(BIE_ADJ_ID) TABLESPACE '||V_TABLESPACE_IDX;			
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.APR_AUX_BIE_ADJ_ADJUDICA_PK... Indice creado.');
	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT APR_AUX_BIE_ADJ_ADJUDICA_PK PRIMARY KEY (BIE_ADJ_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.APR_AUX_BIE_ADJ_ADJUDICA_PK... PK creada.');
	
	
	-- Creamos foreign key FK_APRBIEADJ_APRBIEN
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_APRBIEADJ_APRBIEN FOREIGN KEY (BIE_ID) REFERENCES '||V_ESQUEMA||'.APR_AUX_BIE_BIEN (BIE_ID) ON DELETE CASCADE)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_APRBIEADJ_APRBIEN... Foreign key creada.');
