...
--##CREACION DE TABLA, PK y FK de tabla
	...
	EXECUTE IMMEDIATE 'CREATE OR REPLACE PROCEDURE '||V_ESQUEMA||'.ALTA_BPM_INSTANCES AS
		...
		SELECT TRA_ID, count(*) AS CUENTA FROM TMP_ALTA_BPM_INSTANCES group by TRA_ID having count(*)>1;
		...
		BEGIN
		  ...
		  LOOP
		    ...
		    INSERT INTO TMP_ALTA_BPM_INSTANCES(TRA_ID, INST_ID,DEF_ID, NODE_ID, TAP_CODIGO, TEX_ID, FORK_NODE) 
		  ...
		  DBMS_OUTPUT.PUT_LINE(''module y vmap actualizadas!!'') ;
		    ...
		    INSERT INTO '||V_ESQUEMA_M||'.JBPM_VARIABLEINSTANCE
		      ...
		      , TMP.INST_ID --PROCESSINSTANCE_
		      , V_DBID --LONGVLAUE_
		    FROM (SELECT DISTINCT INST_ID, TRA_ID, TOKEN_PADRE_ID AS TOKEN_ID,MODULE_PADRE_ID AS MODULE_ID,VMAP_PADRE_ID AS VMAP_ID FROM TMP_ALTA_BPM_INSTANCES) TMP;
		...
		--Insertamos variable sin concatenar TOKEN_PADRE_ID    
		  ...
		  USING (SELECT DISTINCT TRA_ID,INST_ID FROM TMP_ALTA_BPM_INSTANCES) TMP
