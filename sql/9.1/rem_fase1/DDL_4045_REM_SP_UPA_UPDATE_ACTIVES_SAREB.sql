...
--------------------------------------------------------------------------------------
				...
				SELECT APR.FECHA_AUTO_ADJUD, ADJ.BIE_ADJ_F_DECRETO_N_FIRME,
				APR.FECHA_AUTO_ADJUD_FIRME, AJD.AJD_FECHA_ADJUDICACION,
				ACT.ACT.ACT_ID, BIE.BIE_ID,
				CASE 
				...
				WHEN MATCHED THEN UPDATE SET
				AJD.DD_EDJ_ID = TMP.DD_EDJ_ID,
				AJD.USUARIOMODIFICAR = ''APR'',
				AJD.FECHAMODIFICAR = SYSDATE
				WHERE AJD.DD_EDJ_ID != TMP.DD_EDJ_ID
				'
				;
  ...
  FOR I IN V_TIPO_TABLA5.FIRST .. V_TIPO_TABLA5.LAST
    ...
    LOOP
			...
			IF V_NUM_TABLAS != 0 THEN
				...
				DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_ACTIVO. '||V_NUM_TABLAS||' Filas.');
...
--------------------------------------------------------------------
----------- ACTUALIZAMOS ACT_CAT_CATASTRO -------------------------- TENEMOS QUE VER QUE HACEMOS CON ESTO, DE QUE MANERA LO ACTUALIZAMOS
--------------------------------------------------------------------
  ...
  FOR I IN V_TIPO_TABLA5.FIRST .. V_TIPO_TABLA5.LAST
    LOOP
			...
			V_TMP_TIPO_TABLA5 := V_TIPO_TABLA5(I);
				...
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT
					ON ACT.ACT_ID = CAT.ACT_ID
				INNER JOIN '||V_ESQUEMA||'.BIE_BIEN BIE
				  ON BIE.BIE_ID = ACT.BIE_ID
				INNER JOIN PERIMETRO APR
				  ON APR.NUMERO_ACTIVO = ACT.ACT_NUM_ACTIVO_UVEM
				WHERE CAT.'||V_TMP_TIPO_TABLA5(2)||' IS NULL
				AND APR.'||V_TMP_TIPO_TABLA5(1)||' IS NOT NULL
				AND APR.ORDEN = 1
				'
				;
