...
BEGIN
  ...
  SELECT DISTINCT AGR.AGR_ID,
                ...
                (SELECT COUNT(AGRACTIVO.ACT_ID)
                 FROM ACT_AGA_AGRUPACION_ACTIVO AGRACTIVO
                 WHERE AGRACTIVO.AGR_ID = AGR.AGR_ID
                   AND AGR.BORRADO = 0) AS activos,

                (SELECT COUNT(AGRACTIVO.ACT_ID)
                 FROM ACT_AGA_AGRUPACION_ACTIVO AGRACTIVO
                 WHERE AGRACTIVO.AGR_ID = AGR.AGR_ID
                   AND AGR.AGR_PUBLICADO = 1
                   AND AGR.BORRADO = 0) AS publicados,
