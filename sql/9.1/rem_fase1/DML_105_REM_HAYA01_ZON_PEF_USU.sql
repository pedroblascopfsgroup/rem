...
--BEGIN	
    ...
    ---- update necesario para que funcione este script si no tienes ya cambiada esa zonificación
    ---- se decidió cambiar manualmente la zonificación que tenía Bankia por la de REM.
    --V_MSQL := 'update zon_zonificacion set zon_descripcion = ''REM'', zon_descripcion_larga = ''REM'' where zon_descripcion = ''BANKIA'' ';
    ...
    --DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN ZON_PEF_USU] ');
     --FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      --LOOP
			...
			--V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ZON_PEF_USU WHERE PEF_ID = (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(1))||''') AND USU_ID = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(2))||''')';
