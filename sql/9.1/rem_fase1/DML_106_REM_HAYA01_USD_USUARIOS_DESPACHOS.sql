...
----## PRODUCTO=NO
----##
----## Finalidad: Script que añade en USD_USUARIOS_DESPACHOS los datos añadidos en T_ARRAY_DATA
----## INSTRUCCIONES:
----## VERSIONES:
----##        0.1 Versión inicial
----##########################################
...
--DECLARE
    ...
    --V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
      --T_FUNCION('REMADM', 'GESTADM','HAYAGESTADM'),
      --T_FUNCION('REMACT', 'GESTACT','HAYAGESACT'),
       --T_FUNCION('REMPROV', 'USUPROV','HAYAPROV')
    --); 
    ...
    --DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN USD_USUARIOS_DESPACHOS] ');
     --FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      --LOOP
			...
			--V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS WHERE DES_ID = (SELECT DES_ID FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = '''||TRIM(V_TMP_FUNCION(1))||''') AND USU_ID = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(2))||''')';
