	...
	DECLARE
	  ...
	  V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
			--	ENTIDAD	 USER_NAME	PASS   NOMBRE_USU	   APELL1  APELL2         EMAIL              GRP	PEF_COD  
		T_TIPO_DATA('1', 'mzafra', 'gt4e71', 'MARIA PILAR', 'ZAFRA', '', 'mzafra@externos.haya.es' , '0', 'HAYASUPER')
	  ); 
	   ...
	   FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
		 LOOP
		  ...
		  V_SQL := '
			...
			AND USU_ID_GRUPO = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''SUPER'')
			'
			;
			...
			INSERT INTO '||V_ESQUEMA_M||'.GRU_GRUPOS_USUARIOS
				...
				(SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''SUPER''),
				(SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_TIPO_DATA(2))||'''),
				''HREOS-617'',
				SYSDATE,
				0 
