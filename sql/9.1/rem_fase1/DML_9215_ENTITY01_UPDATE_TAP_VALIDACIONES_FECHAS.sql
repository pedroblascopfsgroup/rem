...
BEGIN 
  ...
  V_TAP_VALOR := 'esFechaEntre(fechaSolicitudTrabajo(), valores[''''T006_ValidacionInforme''''][''''fechaValidacion''''], hoyDate() ) == false? ''''Fecha de validaci&oacute;n debe ser posterior o igual a fecha solicitud y no superior a hoy.'''' : (valores[''''T006_ValidacionInforme''''][''''comboCorreccion''''] == DDSiNo.NO ? (valores[''''T006_ValidacionInforme''''][''''motivoIncorreccion''''] == '''''''' ? ''''Si declina la obtenci&oacute;n del informe, debe indicar un motivo de incorrecci&oacute;n'''' : null) : (valores[''''T006_ValidacionInforme''''][''''comboValoracion''''] == '''''''' ? ''''Debe indicar la valoraci&oacute;n del proveedor'''' : existeAdjuntoUGValidacion("01","T")) )';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
             ';
