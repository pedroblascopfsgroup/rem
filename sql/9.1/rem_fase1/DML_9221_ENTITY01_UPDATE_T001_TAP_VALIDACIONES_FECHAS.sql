...
BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZANDO VALIDACIONES - TRAMITE ADMISION');


  --ACTUALIZACION DE VALIDACIONE POST 
  V_TAP_CAMPO := 'TAP_SCRIPT_VALIDACION_JBPM';

  -- ADMISION - CHECKING ADMISION - Sin Validación ----------------------------------
  V_TAP_TAP_CODIGOS := ' ''T001_CheckingDocumentacionAdmision'' ';
  V_TAP_VALOR := '';
  ...
  -- ADMISION - CHECKING GESTION - Sin Validación ----------------------------------
  V_TAP_TAP_CODIGOS := ' ''T001_CheckingDocumentacionGestion'' ';
  V_TAP_VALOR := '';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
  ...
  -- ADMISION - CHECKING INFORMACION - Comprobación de campos obligatorios de pest. Admisión -
  ...
  -- ADMISION - VERIFICAR ESTADO POSESORIO - Sin Validación ----------------------------------
  V_TAP_TAP_CODIGOS := ' ''T001_VerificarEstadoPosesorio'' ';
  V_TAP_VALOR := '';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
