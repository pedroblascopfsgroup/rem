...
T008_SolicitudDocumento   
T008_ObtencionDocumento   valores['T008_ObtencionDocumento']['comboObtencion'] == DDSiNo.NO ? (valores['T008_ObtencionDocumento']['motivoNoObtencion'] == '' ? 'Si declina la obtenci&oacute;n del documento, debe indicar un motivo de no obtenci&oacute;n' : null) : existeAdjuntoUGValidacion("""",""T"")
...
-- CEDULA - SOLICITUD DOCUMENTO - Validación POST fecha mayor a aprobación Trabajo --------
  V_TAP_TAP_CODIGOS := ' ''T008_SolicitudDocumento'' ';
  V_TAP_VALOR := 'esFechaMenor(valores[''''T008_SolicitudDocumento''''][''''fechaSolicitud''''], fechaAprobacionTrabajo()) ? ''''Fecha solicitud debe ser posterior o igual a fecha de aprobaci&oacute;n del trabajo'''' : null';
...
-- CEDULA - SOLICITUD EXTRAORDINARIA - Validación POST fecha mayor a aprobación Trabajo --------
  ...
  V_TAP_VALOR := 'esFechaMenor(valores[''''T008_ObtencionDocumento''''][''''fechaEmision''''], fechaAprobacionTrabajo()) ? ''''Fecha emisi&oacute;n debe ser posterior o igual a fecha de aprobaci&oacute;n del trabajo'''' : null';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
             ';
...
-- CEDULA - CIERRE ECONOMICO - Validación POST fecha mayora a validación Trabajo ----
  ...
  V_TAP_VALOR := 'esFechaMenor(valores[''''T008_CierreEconomico''''][''''fechaCierre''''], fechaValidacionTrabajo()) ? ''''Fecha cierre debe ser posterior o igual a fecha de validaci&oacute;n del trabajo'''' : null';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
             ';
