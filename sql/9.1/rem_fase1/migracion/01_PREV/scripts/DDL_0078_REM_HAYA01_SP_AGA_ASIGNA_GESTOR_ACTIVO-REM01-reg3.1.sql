...
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Procedimiento que asigna Gestores a Activos
--##           
...
CREATE OR REPLACE PROCEDURE SP_AGA_ASIGNA_GESTOR_ACTIVO AS 
...
--------------------------------------------------------------------
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' TMP_GEST_ACT cargada. '||SQL%ROWCOUNT||' Filas.');
  ...
  FROM (
  WITH PROVINCIAS AS (
    SELECT * FROM '||V_ESQUEMA||'.ACT_GES_DISTRIBUCION WHERE COD_PROVINCIA IS NOT NULL AND COD_POSTAL IS NULL AND COD_MUNICIPIO IS NULL
    )
  ...
  LEFT JOIN PROVINCIAS PRV
    ON PRV.COD_PROVINCIA = BIE.BIE_LOC_PROVINCIA
  LEFT JOIN '||V_ESQUEMA||'.TMP_GEST_ACT GEACT
    ON GEACT.ACT_ID = ACT.ACT_ID
  WHERE BIE.BIE_LOC_PROVINCIA IS NOT NULL
  AND BIE.BIE_LOC_PROVINCIA != 0
  AND GEACT.ACT_ID IS NULL
  and NOT EXISTS (
  SELECT 1 FROM GESTOR_ACTIVOS_NO_INSERTAR TMP WHERE TMP.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO
  )
  ...
  ELSE
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] EXISTEN '||V_NUM||' ACTIVOS A LOS QUE NO SE HA ASIGNADO GESTOR. SE LES ASIGNARÁ EL GESTOR DEL ACTIVO POR DEFECTO ''GESTACT''.');
  ...
  SET TMP.USU_ID = (SELECT USU.USU_ID FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''GESTACT''),
  TMP.USUARIO_GESTION_ACTIVOS = (SELECT USU.USU_USERNAME FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''GESTACT''),
  TMP.NOMBRE_GESTION_ACTIVOS = (SELECT USU.USU_NOMBRE FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''GESTACT'')
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' GEE_GESTOR_ENTIDAD cargada. '||SQL%ROWCOUNT||' Filas.');
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' GAC_GESTOR_ADD_ACTIVO cargada. '||SQL%ROWCOUNT||' Filas.');
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' GEH_GESTOR_ENTIDAD_HIST cargada. '||SQL%ROWCOUNT||' Filas.');
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' GAH_GESTOR_ACTIVO_HISTORICO cargada. '||SQL%ROWCOUNT||' Filas.');
...
----------------------------------------------------------------------
  ...
  WITH SUPADM_NO_INSERTAR AS (
    ...
    LEFT JOIN '||V_ESQUEMA_MASTER||'.DD_TGE_TIPO_GESTOR TGE
      ON TGE.DD_TGE_ID = GEE.DD_TGE_ID
    WHERE TGE.DD_TGE_ID IS NOT NULL
    AND TGE.DD_TGE_DESCRIPCION = ''Supervisor admisión REO''
)
  SELECT
  ACT_ID,
  s_gee_gestor_entidad.NEXTVAL gee_id,
  ...
  TMP.gee_id,
  TMP.usu_id,
  (SELECT dd_tge_id FROM '||V_ESQUEMA_MASTER||'.dd_tge_tipo_gestor WHERE dd_tge_codigo = ''SAREO''),
  0,
  ''MIG'',
  SYSDATE,
  0
