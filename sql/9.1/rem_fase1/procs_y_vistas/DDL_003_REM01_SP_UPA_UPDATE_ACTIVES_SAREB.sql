  ...
  FOR I IN V_TIPO_TABLA2.FIRST .. V_TIPO_TABLA2.LAST
    ...
    END LOOP;
        ...
        DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
				...
				ACT.ACT_ID, BIE.BIE_ID, AJD.DD_EDJ_ID,
				CASE
			...
			ELSE
				...
				DBMS_OUTPUT.PUT_LINE('[INFO] NO SE HAN ACTUALIZADO DD_EDJ_ID EN '||V_ESQUEMA||'.ACT_AJD_ADJJUDICIAL.');
...
/*-----------------------------------------------------------------------
----- COPIAMOS A ACT_SPS_SIT_POSESORIA.SPS_FECHA_TOMA_POSESION    -----
----- EL CONTENIDO DE BIE_ADJ_ADJUDICACION.BIE_ADJ_F_REA_POSESION -----
-----------------------------------------------------------------------
-----------------------------------------------------------------------
------ COPIAMOS A ACT_SPS_SIT_POSESORIA.SPS_FECHA_TITULO --------------
------ EL CONTENIDO DE BIE_ADJ_ADJUDICACION.BIE_ADJ_F_CONTRATO_ARREN --
-----------------------------------------------------------------------

    DBMS_OUTPUT.PUT_LINE('[INFO] ACTUALIZANDO FECHA DE TOMA DE POSESIÓN...');
    DBMS_OUTPUT.PUT_LINE('[INFO] ACTUALIZANDO FECHA TITULO...');
  
    EXECUTE IMMEDIATE '
    MERGE INTO '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA SPS USING
    (
    SELECT DISTINCT
    ACT.ACT_ID, BIE.BIE_ID, SPS.SPS_ID, ADJ.BIE_ADJ_F_REA_POSESION, ADJ.BIE_ADJ_F_CONTRATO_ARREN
    FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
    INNER JOIN '||V_ESQUEMA||'.BIE_BIEN BIE
      ON BIE.BIE_ID = ACT.BIE_ID
    LEFT JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION ADJ
    ...
    WHERE ADJ.BIE_ADJ_ID IS NOT NULL
    AND (ADJ.BIE_ADJ_F_REA_POSESION IS NOT NULL
    AND SPS.SPS_FECHA_TOMA_POSESION IS NULL)
    OR (ADJ.BIE_ADJ_F_CONTRATO_ARREN IS NOT NULL
    AND SPS.SPS_FECHA_TITULO IS NULL)
    AND SPS.BORRADO = 0
    ) TMP
    ON (SPS.SPS_ID = TMP.SPS_ID)
    WHEN MATCHED THEN UPDATE SET
    SPS.SPS_FECHA_TOMA_POSESION = TMP.BIE_ADJ_F_REA_POSESION,
    SPS.SPS_FECHA_TITULO = TMP.BIE_ADJ_F_CONTRATO_ARREN,
    SPS.USUARIOMODIFICAR = '''||V_USUARIO||''',
    ...
    V_NUM_TABLAS := SQL%ROWCOUNT;
			...
			IF V_NUM_TABLAS != 0 THEN
				...
				DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA. '||V_NUM_TABLAS||' Filas.');
        PL_OUTPUT := PL_OUTPUT||'[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA. '||V_NUM_TABLAS||' Filas. '||CHR(10)||CHR(10);

				COMMIT;

			END IF;

    DBMS_OUTPUT.PUT_LINE('[INFO] FECHA DE TOMA DE POSESIÓN ACTUALIZADA');
    DBMS_OUTPUT.PUT_LINE('[INFO] FECHA TITULO ACTUALIZADA');

    DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
    
    
*/-----------------------------------------------------------
------ ACTUALIZAMOS DD_TPO_TIPO_TITULO_POSESORIO ----------
------ EN ACT_SPS_SIT_POSESORIA                  ----------
-----------------------------------------------------------
/*
  	DBMS_OUTPUT.PUT_LINE('[INFO] ACTUALIZANDO TITULO POSESORIO...');
    ...
    SELECT
    CASE
      WHEN SPS.SPS_OCUPADO = 1 AND SPS.SPS_CON_TITULO = 1 AND SPS.SPS_FECHA_TITULO IS NOT NULL THEN (SELECT DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_TITULO_POSESORIO WHERE DD_TPO_CODIGO = ''02'')
    END AS DD_TPO_ID,
    SPS.SPS_ID,
    ACT.ACT_ID
    FROM '||V_ESQUEMA||'.APR_AUX_STOCK_BIENES APR
    ...
    LEFT JOIN '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA SPS
      ON SPS.ACT_ID = ACT.ACT_ID
    WHERE SPS.SPS_FECHA_TITULO IS NOT NULL
    AND ACT.BIE_ID IS NOT NULL
    AND DISTINTOS.ORDEN = 1
    AND SPS.BORRADO = 0
    ) TMP
    ON (SPS.ACT_ID = TMP.ACT_ID)
    WHEN MATCHED THEN UPDATE SET
    SPS.DD_TPO_ID = TMP.DD_TPO_ID,
    SPS.USUARIOMODIFICAR = '''||V_USUARIO||''',
    SPS.FECHAMODIFICAR = SYSDATE
    '
    ;
    ...
    V_NUM_TABLAS := SQL%ROWCOUNT;
			...
			IF V_NUM_TABLAS != 0 THEN
				...
				DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA.DD_TPO_ID '||V_NUM_TABLAS||' Filas.');
        PL_OUTPUT := PL_OUTPUT||'[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA.DD_TPO_ID '||V_NUM_TABLAS||' Filas. '||CHR(10)||CHR(10);

				COMMIT;

			END IF;
    
    DBMS_OUTPUT.PUT_LINE('[INFO] TIPO TITULO POSESORIO ACTUALIZADO');
