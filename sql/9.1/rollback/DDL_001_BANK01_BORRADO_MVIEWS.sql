...
BEGIN
    ...
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.TMP_ARQ_RECOBRO';
    ...
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_RCF_ENTRADA';
            EXECUTE IMMEDIATE V_MSQL;
    ...
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''BATCH_DATOS_EXP'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_EXP';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_EXP... Tabla borrada');  
    END IF;
    ...
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_CNT_EXP';
            EXECUTE IMMEDIATE V_MSQL;
    ...
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''BATCH_DATOS_PER_EXP'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_PER_EXP';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_PER_EXP... Tabla borrada');  
    END IF;
    ...
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_EXP_MANUAL';
            EXECUTE IMMEDIATE V_MSQL;
    ...
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_CNT_PER';
            EXECUTE IMMEDIATE V_MSQL;
    ...
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''BATCH_DATOS_CNT'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_CNT';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_CNT... Tabla borrada');  
    END IF;
    ...
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_PER';
            EXECUTE IMMEDIATE V_MSQL;
    ...
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''BATCH_DATOS_CNT_INFO'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_CNT_INFO';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_CNT_INFO... Tabla borrada');  
    END IF;
    DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_CNT_INFO... Comprobaciones previas FIN');          
         
	--BATCH_DATOS_CLI
	
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''BATCH_DATOS_CLI'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_CLI';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_CLI... Tabla borrada');  
    END IF;
    DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_CLI... Comprobaciones previas FIN');          
          
	--BATCH_DATOS_EXCEPTUADOS
	
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''BATCH_DATOS_EXCEPTUADOS'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_EXCEPTUADOS';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_EXCEPTUADOS... Tabla borrada');  
    END IF;
    DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_EXCEPTUADOS... Comprobaciones previas FIN');          
            
	--BATCH_DATOS_GCL
	
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''BATCH_DATOS_GCL'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.BATCH_DATOS_GCL';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_GCL... Tabla borrada');  
    END IF;
    DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.BATCH_DATOS_GCL... Comprobaciones previas FIN');          

	--T_ARP_ARQ_RECOBRO_PERSONA
	
    V_SQL := 'Select COUNT(1) from ALL_TABLES where TABLE_NAME = ''T_ARP_ARQ_RECOBRO_PERSONA'' and owner= '''||v_esquema||'''';	
    EXECUTE IMMEDIATE v_sql INTO v_num_tablas;
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.T_ARP_ARQ_RECOBRO_PERSONA';
            EXECUTE IMMEDIATE V_MSQL;
            DBMS_OUTPUT.PUT_LINE('[INFO] '||v_esquema||'.T_ARP_ARQ_RECOBRO_PERSONA... Tabla borrada');  
    END IF;
    ...
    -- Si existe la borramos
    IF V_NUM_TABLAS = 1 THEN 
            V_MSQL := 'DROP TABLE '||v_esquema||'.T_ARP_ARQ_RECOBRO_PERSONA_SIM';
            EXECUTE IMMEDIATE V_MSQL;
