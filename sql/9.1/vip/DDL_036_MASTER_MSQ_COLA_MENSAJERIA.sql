...
DECLARE
    ...
    V_MSQL := 'CREATE TABLE ' || V_ESQUEMA_M || '.MSQ_COLA_MENSAJERIA
					(
					  ...
					  MSQ_COLA        VARCHAR2(20 CHAR)   NOT NULL,
    ...
    V_MSQL := 'CREATE INDEX ' || V_ESQUEMA_M || '.IDX_MSQ_COLA_1 ON ' || V_ESQUEMA_M || '.MSQ_COLA_MENSAJERIA
					(MSQ_COLA)  TABLESPACE ' || V_TS_INDEX;
		EXECUTE IMMEDIATE V_MSQL;
    ...
    V_MSQL := 'GRANT ALL ON '||V_ESQUEMA_M||'.S_MSQ_COLA_MENSAJERIA TO '||V_ESQUEMA; 			
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Grant ejecutado correctamente.');
