...
--## PRODUCTO=SI
--## 
--## Finalidad: Añadimos motivo de finalización "Finalizacion masiva de asuntos" en el
...
V_SENTENCIA := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_DFI_DECISION_FINALIZAR WHERE DD_DFI_DESCRIPCION = ''Finalizacion masiva de asuntos''';
                
		EXECUTE IMMEDIATE V_SENTENCIA INTO V_NUM;

		IF V_NUM > 0 THEN
		
		DBMS_OUTPUT.PUT_LINE('[INFO] El motivo de finalización "Finalizacion masiva de asuntos" ya existe');
...
V_SENTENCIA := 'INSERT INTO '||V_ESQUEMA||'.DD_DFI_DECISION_FINALIZAR VALUES (
				...
				''FINASU'',
				''Finalizacion masiva de asuntos'',
				''Finalizacion masiva de asuntos'',
				1,
				''DD'',
				SYSDATE,
