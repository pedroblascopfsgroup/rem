...
BEGIN
    ...
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    IF V_NUM_TABLAS = 1 THEN
    	V_MSQL := 'UPDATE '||V_ESQUEMA||'.DD_TAC_TIPO_ACTUACION SET DD_TAS_ID = (SELECT DD_TAS_ID FROM '||V_ESQUEMA_M||'.DD_TAS_TIPOS_ASUNTO WHERE DD_TAS_CODIGO = ''01'') WHERE DD_TAC_CODIGO NOT IN (''CO'') ';
		DBMS_OUTPUT.PUT_LINE(V_MSQL);
		EXECUTE IMMEDIATE V_MSQL;
