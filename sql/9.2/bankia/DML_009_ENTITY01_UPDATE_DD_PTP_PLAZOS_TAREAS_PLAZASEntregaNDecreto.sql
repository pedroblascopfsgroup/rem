...
BEGIN	
	...
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_PTP_PLAZOS_TAREAS_PLAZAS WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''P414_EntregarNuevoDecreto'')';
	
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS > 0 THEN
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.DD_PTP_PLAZOS_TAREAS_PLAZAS SET DD_PTP_PLAZO_SCRIPT = ''valores[''''P414_RegistrarPresentacionEscritoSub''''] == null ? 1*24*60*60*1000L :(valores[''''P414_RegistrarPresentacionEscritoSub''''][''''fechaPresentacion''''] == null ? 1*24*60*60*1000L : damePlazo(valores[''''P414_RegistrarPresentacionEscritoSub''''][''''fechaPresentacion''''])+60*24*60*60*1000L)'' WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''P414_EntregarNuevoDecreto'')';
		DBMS_OUTPUT.PUT_LINE(V_MSQL);
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[FIN] DD_PTP_PLAZOS_TAREAS_PLAZAS  actualizado');
	ELSE
