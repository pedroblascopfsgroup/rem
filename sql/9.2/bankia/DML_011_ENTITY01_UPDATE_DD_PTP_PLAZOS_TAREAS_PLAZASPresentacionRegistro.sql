...
BEGIN	
	...
	IF V_NUM_TABLAS > 0 THEN
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.DD_PTP_PLAZOS_TAREAS_PLAZAS SET DD_PTP_PLAZO_SCRIPT = ''(valores[''''P413_RegistrarPresentacionEnHacienda''''] != null && valores[''''P413_RegistrarPresentacionEnHacienda''''][''''fechaPresentacion''''] != null) ? damePlazo(valores[''''P413_RegistrarPresentacionEnHacienda''''][''''fechaPresentacion''''])+10*24*60*60*1000L : ((valores[''''P414_EntregarNuevoDecreto''''] != null && valores[''''P414_EntregarNuevoDecreto''''][''''fechaEnvio''''] != null) ? damePlazo(valores[''''P414_EntregarNuevoDecreto''''][''''fechaEnvio''''])+1*24*60*60*1000L :  1*24*60*60*1000L)'' WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''P413_RegistrarPresentacionEnRegistro'')';
