  ...
  IF V_NUM_TABLAS = 0 THEN
	...
	IF V_NUM_TABLAS = 0 THEN
	    ...
	    EXECUTE IMMEDIATE 'Insert into '||V_ESQUEMA||'.PEF_PERFILES
		 ...
		 Values
  		 ('||V_ESQUEMA||'.s_pef_perfiles.nextval, ''Supervisor DRT'', ''Supervisor DRT'', 0, ''RECOVERY-2479'', sysdate, null, null, 0, ''FPFSRSUPGEXP'', 0, ''EXTPerfil'')';
		DBMS_OUTPUT.PUT_LINE('[FIN] '||V_ESQUEMA||'.PEF_PERFILES...  nuevo perfil creado');
	ELSE
		EXECUTE IMMEDIATE 'Update '||V_ESQUEMA||'.PEF_PERFILES SET PEF_CODIGO = ''FPFSRSUPGEXP'', USUARIOMODIFICAR = ''RECOVERY-2479'', FECHAMODIFICAR = sysdate WHERE PEF_CODIGO = ''SUPGESTEXP''';
  ...
  IF V_NUM_TABLAS = 0 THEN
	  ...
	  IF V_NUM_TABLAS = 0 THEN
	      ...
	      EXECUTE IMMEDIATE 'Insert into '||V_ESQUEMA||'.PEF_PERFILES
	       (PEF_ID, PEF_DESCRIPCION_LARGA, PEF_DESCRIPCION, VERSION, USUARIOCREAR, FECHACREAR, USUARIOMODIFICAR, FECHAMODIFICAR, BORRADO, PEF_CODIGO, PEF_ES_CARTERIZADO, DTYPE)
	     Values
	       ('||V_ESQUEMA||'.s_pef_perfiles.nextval, ''Gestor DRT'', ''Gestor DRT'', 0, ''RECOVERY-2479'', sysdate, null, null, 0, ''FPFSRGESTEXP'', 0, ''EXTPerfil'')';
	    DBMS_OUTPUT.PUT_LINE('[FIN] '||V_ESQUEMA||'.PEF_PERFILES...  nuevo perfil creado');
	  ELSE
	    EXECUTE IMMEDIATE 'Update '||V_ESQUEMA||'.PEF_PERFILES SET PEF_CODIGO = ''FPFSRGESTEXP'', USUARIOMODIFICAR = ''RECOVERY-2479'', FECHAMODIFICAR = sysdate WHERE PEF_CODIGO = ''GESTEXP''';
		DBMS_OUTPUT.PUT_LINE('[UPDATE] '||V_ESQUEMA||'.PEF_PERFILES...  hacemos update');
	   END IF;
   END IF;
