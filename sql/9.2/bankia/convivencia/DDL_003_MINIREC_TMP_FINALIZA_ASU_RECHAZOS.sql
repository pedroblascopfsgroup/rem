...
DBMS_OUTPUT.PUT_LINE('[START]  tabla TMP_FINALIZA_ASU_RECHAZOS');

EXECUTE IMMEDIATE 'select count(1) from all_tables where table_name = ''TMP_FINALIZA_ASU_RECHAZOS'' and owner = ''' || V_ESQUEMA_MINIREC || '''' into v_table_count;
if v_table_count = 0 then
EXECUTE IMMEDIATE 'CREATE TABLE '|| V_ESQUEMA_MINIREC ||'.TMP_FINALIZA_ASU_RECHAZOS 
       (
   ASU_ID NUMBER(16,0) NOT NULL 
  ,COMENTARIO VARCHAR2(250 CHAR) NOT NULL 
  ,ERRORCODE VARCHAR2(250 CHAR) NOT NULL 
)';


    DBMS_OUTPUT.PUT_LINE('CREATE TABLE '|| V_ESQUEMA_MINIREC ||'.TMP_FINALIZA_ASU_RECHAZOS.. Tabla creada OK');
