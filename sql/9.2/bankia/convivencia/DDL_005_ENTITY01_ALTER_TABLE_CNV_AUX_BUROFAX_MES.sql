...
BEGIN
	...
	BEGIN
		...
		SELECT COUNT(1) INTO V_NUM_TABLAS FROM ALL_TAB_COLUMNS WHERE UPPER(table_name) = 'CNV_AUX_BUROFAX_MES' and UPPER(column_name) = 'PROCESADO' AND NULLABLE = 'N';
		...
		ELSE
		  DBMS_OUTPUT.PUT_LINE('[INFO] Campo ' || V_ESQUEMA || '.CNV_AUX_BUROFAX_MES.PROCESADO NULLABLE no encontrado');			
		END IF;	
	...
	BEGIN
		...
		SELECT COUNT(1) INTO V_NUM_TABLAS FROM ALL_TAB_COLUMNS WHERE UPPER(table_name) = 'CNV_AUX_BUROFAX_MES' and UPPER(column_name) = 'NO_ENVIO' AND NULLABLE = 'N';
		...
		ELSE
		  DBMS_OUTPUT.PUT_LINE('[INFO] Campo ' || V_ESQUEMA || '.CNV_AUX_BUROFAX_MES.NO_ENVIO NULLABLE no encontrado');						
		END IF;	
    ...
    BEGIN
		...
		SELECT COUNT(1) INTO V_NUM_TABLAS FROM ALL_TAB_COLUMNS WHERE UPPER(table_name) = 'CNV_AUX_BUROFAX_MES' and UPPER(column_name) = 'CODIGO_POSTAL_DEST' AND NULLABLE = 'N' ;
		...
		ELSE
		  DBMS_OUTPUT.PUT_LINE('[INFO] Campo ' || V_ESQUEMA || '.CNV_AUX_BUROFAX_MES.CODIGO_POSTAL_DEST NULLABLE no encontrado');									
		END IF;			
