...
--## PRODUCTO=NO
--##
--## Finalidad: Modidicamos la PK de la tabla MINIRECOVERY_CNT_PROC_BAJA.
...
DECLARE
    ...
    TABLA VARCHAR2(30 CHAR) :='MINIRECOVERY_CNT_PROC_BAJA';
    PK_NEW_COLUMNS VARCHAR2(100 CHAR) :='MR_CODIGO_PROPIETARIO,MR_TIPO_PRODUCTO,MR_NUMERO_CONTRATO,MR_NUMERO_ESPEC,MR_CD_PROCEDIMIENTO';
    PK_NEW_NAME VARCHAR2(100 CHAR) :='PK_MINIRE_CNT_PROC_BAJA';
    V_EXISTE NUMBER (1);
    PK_EXISTE NUMBER (1);
...
BEGIN 
  ...
  IF PK_SAME <> 1 THEN
    ...
    DBMS_OUTPUT.PUT_LINE ('[INFO] Creamos la nueva clave primaria... '|| PK_NEW_NAME ||'.');
    V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||TABLA||' ADD CONSTRAINT '|| PK_NEW_NAME ||' PRIMARY KEY ('||PK_NEW_COLUMNS||')';
    Execute Immediate V_MSQL;
    commit;
    DBMS_OUTPUT.PUT_LINE ('[INFO] Nueva clave primaria creada... '|| PK_NEW_NAME ||'.');
  ELSE 
    DBMS_OUTPUT.PUT_LINE ('[INFO] La nueva PK es la misma PK que ya existe, no hacemos mas acciones.');
  END IF;
