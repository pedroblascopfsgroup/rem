...
DECLARE
    ...
    --** Compilamos cabecera
    v_msql :=
    'Create or Replace Package '||v_esquema||'.OPERACION_DDL as
        ------------------------------------------------------------------------------------------------------
        -- OPERACION DDL v.1.0
        -- 23/09/2015
        ...
        -- 4.- Actualización de estadísticas de tablas y vistas materializadas.
        -- 5.- Activación y desactivación de índices.
        -- 6.- Creación y borrado de índices.
        -- 7.- Refresco de vistas materializadas.
        ------------------------------------------------------------------------------------------------------
    ...
    --** Compilamos cuerpo del paquete
    v_msql := 'Create or Replace Package Body '||v_esquema||'.OPERACION_DDL  as
      ...
      EXCEPTION
        ...
        WHEN INSERT_NULL then
          O_ERROR_STATUS := ''Has intentado insertar un valor nulo'';err_num := SQLCODE;
          err_msg := SQLERRM;
          DBMS_OUTPUT.put_line(''Error:''||TO_CHAR(err_num));
          DBMS_OUTPUT.put_line(O_ERROR_STATUS||''. ''||err_msg);
          raise;
        WHEN PARAMETERS_NUMBER then
          O_ERROR_STATUS := ''Parámetros obligatorios ausentes o mal especificados'';
          err_num := SQLCODE;
          err_msg := SQLERRM;
          DBMS_OUTPUT.put_line(''Error:''||TO_CHAR(err_num));
          DBMS_OUTPUT.put_line(O_ERROR_STATUS||''. ''||err_msg);
      ...
      begin
        ...
        if V_OPERACION not in (''REFRESH'',''REFRESCA'',''REFRESCAR'',''ANALYZE'',''STATS'',''STATISTICS'',''ESTADISTICAS'')
         Then Raise PARAMETERS_NUMBER;
        End if;
