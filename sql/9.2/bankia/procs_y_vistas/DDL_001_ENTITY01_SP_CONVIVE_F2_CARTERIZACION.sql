...
from
 (
  select asu.asu_id
  from #ESQUEMA#.asu_asuntos asu, #ESQUEMA#.CNV_AUX_ALTA_PRC cnv where asu.asu_id_externo=cnv.codigo_procedimiento_nuse and not exists (select 1 from #ESQUEMA#.GAA_GESTOR_ADICIONAL_ASUNTO gaa 
                    where gaa.asu_id = asu.asu_id and gaa.dd_tge_id =
                        (select dd_tge_id from #ESQUEMA_MASTER#.dd_tge_tipo_gestor where dd_tge_codigo = 'SUP_ASIG_CONC'))
  and asu_id in (select asuu.asu_id
                    from #ESQUEMA#.asu_asuntos asuu inner join
                         #ESQUEMA#.dd_pas_propiedad_asunto pas on pas.dd_pas_id = asuu.dd_pas_id inner join
                         #ESQUEMA#.dd_ges_gestion_asunto ges on ges.dd_ges_id = asuu.dd_ges_id inner join
...
from
 (select asu_id
  from #ESQUEMA#.asu_asuntos asu, #ESQUEMA#.CNV_AUX_ALTA_PRC cnv where asu.asu_id_externo=cnv.codigo_procedimiento_nuse and not exists (select 1 from #ESQUEMA#.GAH_GESTOR_ADICIONAL_HISTORICO gaa 
...
from
 (
  ...
  from #ESQUEMA#.asu_asuntos asu, #ESQUEMA#.CNV_AUX_ALTA_PRC cnv where asu.asu_id_externo=cnv.codigo_procedimiento_nuse and not exists (select 1 from #ESQUEMA#.GAA_GESTOR_ADICIONAL_ASUNTO gaa 
                    where gaa.asu_id = asu.asu_id and gaa.dd_tge_id =
                        (select dd_tge_id from #ESQUEMA_MASTER#.dd_tge_tipo_gestor where dd_tge_codigo = 'PREDOC'))
  and asu_id in (select asuu.asu_id
                    from #ESQUEMA#.asu_asuntos asuu inner join
                         #ESQUEMA#.dd_pas_propiedad_asunto pas on pas.dd_pas_id = asuu.dd_pas_id inner join
                         #ESQUEMA#.dd_ges_gestion_asunto ges on ges.dd_ges_id = asuu.dd_ges_id inner join
