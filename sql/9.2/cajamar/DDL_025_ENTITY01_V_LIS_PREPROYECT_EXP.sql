...
--## INCIDENCIA_LINK=CMREC-2983
--## PRODUCTO=NO
--## Finalidad: Se quita la condición PEX_PASE = 1
--##           
      ...
      ,CNT_CONTRATOS_M AS (
        ...
        WHERE NUMERO = 1
			...
			,IMPORTES_EXP AS (
			  ...
			  FROM '||V_ESQUEMA||'.CEX_CONTRATOS_EXPEDIENTE CEX
			    INNER JOIN (SELECT CNT_ID, VEN_IMPORTE_INICIAL, VEN_IMPORTE_PTE_DIFER, VEN_CAPITAL_VIVO FROM '||V_ESQUEMA||'.VEN_VENCIDOS WHERE BORRADO = 0) VEN ON VEN.CNT_ID = CEX.CNT_ID
			  WHERE CEX.BORRADO = 0
			  GROUP BY CEX.EXP_ID
			)
