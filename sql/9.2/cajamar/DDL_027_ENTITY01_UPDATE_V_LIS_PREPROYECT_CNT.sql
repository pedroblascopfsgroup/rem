...
--## INCIDENCIA_LINK=CMREC-3010
--## PRODUCTO=NO
--## Finalidad: Se modifica la vista para que solo incluya primeros titulares.
--##           
				                        ...
				                        ,TITULARES_CNT AS (
				                          SELECT PER_DOC_ID, UPPER(PER_NOMBRE || '' '' || PER_APELLIDO1 ||  '' '' || PER_APELLIDO2) PER_NOM_COMPLETO, CNT_ID 
				                            FROM '||V_ESQUEMA||'.CPE_CONTRATOS_PERSONAS CPE
				                              INNER JOIN '||V_ESQUEMA||'.DD_TIN_TIPO_INTERVENCION TIN ON TIN.DD_TIN_ID = CPE.DD_TIN_ID AND TIN.DD_TIN_CODIGO = ''01'' AND TIN.BORRADO = 0
