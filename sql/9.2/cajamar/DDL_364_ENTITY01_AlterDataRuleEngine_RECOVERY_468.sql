...
BEGIN
  ...
  V_SQL := 'select count(1) from all_tab_columns
             where COLUMN_NAME = ''PER_EMP_PROD_GESTION''
               and table_name = ''DATA_RULE_ENGINE''
               and owner = '''||v_esquema||'''
           ';
  ...
  IF V_NUM_TABLAS > 0
   THEN
     ...
     DBMS_OUTPUT.PUT_LINE('[INFO] Atributo PER_EMP_PROD_GESTION ya existe.');
   ELSE
     V_SQL := 'ALTER TABLE '||v_esquema||'.DATA_RULE_ENGINE
                 ADD PER_EMP_PROD_GESTION number(16)';
     EXECUTE IMMEDIATE V_SQL;
     DBMS_OUTPUT.PUT_LINE('[INFO] AÑADIDO EL CAMPO PER_EMP_PROD_GESTION');
  END IF;
