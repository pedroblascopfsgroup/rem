...
BEGIN
	...
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.DD_RULE_DEFINITION WHERE RD_TITLE = ''Actuacion FSR en Centro Gestor, no Agencias'' AND BORRADO = 1';
	DBMS_OUTPUT.PUT_LINE(V_SQL);
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    IF V_NUM_TABLAS > 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] El registro ya ha sido actualizado en la tabla '||V_ESQUEMA||'.DD_RULE_DEFINITION.');    	
	ELSE
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.DD_RULE_DEFINITION SET BORRADO = 1, USUARIOBORRAR= ''CMREC-1996'', FECHABORRAR = SYSDATE WHERE RD_TITLE = ''Actuacion FSR en Centro Gestor, no Agencias''';
