...
BEGIN
	...
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO = ''54''';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_EXISTE;
	
	IF V_NUM_EXISTE > 0 THEN 
		V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE SET DD_STA_GESTOR = 0 WHERE DD_STA_CODIGO = ''54''';
		EXECUTE IMMEDIATE V_MSQL;	
		DBMS_OUTPUT.PUT_LINE('[INFO] DD_STA_SUBTIPO_TAREA_BASE actualizado');
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] DD_STA_SUBTIPO_TAREA_BASE no existe');
	END IF;
