...
V_SQL := 'UPDATE ' || V_ESQUEMA || '.' || V_DDNAME || ' SET DD_PCO_BFT_PLANTILLA = ''EN SU CALIDAD DE ${tipoIntervencion} DE LA PÓLIZA DE CRÉDITO PARA OPERACIONES DE COMERCIO EXTERIOR ' || 
	'Nº ${CODIGO_DE_CONTRATO}  ${FORMALIZADO} EL DÍA ${FECFORMALIZ} POR UN LÍMITE DE ${CAL_RUT_POLIZA} EUROS, LE NOTIFICAMOS QUE POR INCUMPLIMIENTO DE LAS OBLIGACIONES DE PAGO PACTADAS EN LA MISMA, ' || 
...
--Aval
V_SQL := 'UPDATE ' || V_ESQUEMA || '.' || V_DDNAME || ' SET DD_PCO_BFT_PLANTILLA = ''EN SU CALIDAD DE ${tipoIntervencion} DE LA POLIZA DE RELEVACION DE FIANZA Nº ${CODIGO_DE_CONTRATO} ' || 
...
V_SQL := 'UPDATE ' || V_ESQUEMA || '.' || V_DDNAME || ' SET DD_PCO_BFT_PLANTILLA = ''EN SU CALIDAD DE ${tipoIntervencion} DE LA POLIZA DE  AFIANZAMIENTO Y GARANTÍA DE OPERACIONES MERCANTILES Nº ${CODIGO_DE_CONTRATO} ' || 
...
IF V_NUM_TABLAS = 0 THEN
    ...
    ''EN SU CALIDAD DE ${tipoIntervencion} DE LA CUENTA DE AHORRO A LA VISTA / CUENTA CORRIENTE Nº ${CODIGO_DE_CONTRATO},' || 
...
ELSE
	V_SQL := 'UPDATE ' || V_ESQUEMA || '.' || V_DDNAME || ' SET DD_PCO_BFT_PLANTILLA = ''EN SU CALIDAD DE ${tipoIntervencion} DE LA CUENTA DE AHORRO A LA VISTA / CUENTA CORRIENTE Nº ${CODIGO_DE_CONTRATO},' || 
	', POR LA PRESENTE LE COMUNICAMOS QUE LA MISMA AL DÍA ${FECHA_CIERRE_LIQUIDACION}, ARROJA UN SALDO DEUDOR DE ${IMPCER} EUROS A FAVOR DE ${GEN_ENT_N}. <br/><br/>' || 
	'LE SOLICITAMOS QUE EN UN PLAZO MÁXIMO DE CINCO DÍAS HÁBILES REGULARICE ESTA SITUACIÓN YA QUE, EN CASO CONTRARIO, PROCEDEREMOS A LA INMEDIATA RECLAMACIÓN JUDICIAL.<br/><br/>' || 
	'RECUERDE QUE POR EL IMPAGO DE SU DEUDA SUS DATOS PUEDEN SER INCORPORADOS A FICHEROS DE CUMPLIMIENTO O INCUMPLIMIENTO DE OBLIGACIONES DINERARIAS.<br/><br/>' || 
	'LA PRESENTE COMUNICACIÓN ANULA Y DEJA SIN EFECTO CUALQUIER OTRA QUE, RESPECTO DE LA MISMA OPERACIÓN CREDITICIA, Y CON EXPRESIÓN DE CANTIDAD, HUBIESE PODIDO RECIBIR CON ANTERIORIDAD.<br/><br/>' || 
...
'CORRIENTE Nº ${CODIGO_DE_CONTRATO} CONCEDIDA EL DÍA ${FECFORMALIZ} POR UN LÍMITE DE ${CAL_RUT_POLIZA} EUROS, LE NOTIFICAMOS QUE POR INCUMPLIMIENTO DE ' ||
...
'CONCEPTOS EXPRESADOS A LA CANTIDAD DE ${IMPCER} EUROS AL DÍA ${FECHA_CIERRE_LIQUIDACION}, IMPORTE CUYO PAGO LE REQUERIMOS A EFECTOS DE LO DISPUESTO EN LOS ' ||
...
'Nº ${CODIGO_DE_CONTRATO} ${FORMALIZADO} EL DIA ${FECFORMALIZ} MEDIANTE ESCRITURA PUBLICA ANTE EL NOTARIO ${nombreNotario} POR UN PRINCIPAL DE ${CAL_RUT_POLIZA} EUROS, ' ||
...
'Nº ${CODIGO_DE_CONTRATO} ${FORMALIZADO} EL DIA ${FECFORMALIZ} POR UN PRINCIPAL DE ${CAL_RUT_POLIZA} EUROS, ' ||
...
'ARRENDAMIENTO FINANCIERO Nº ${CODIGO_DE_CONTRATO} ${FORMALIZADO} EL DÍA ${FECFORMALIZ}, LE NOTIFICAMOS QUE POR INCUMPLIMIENTO DE LAS OBLIGACIONES ' ||
