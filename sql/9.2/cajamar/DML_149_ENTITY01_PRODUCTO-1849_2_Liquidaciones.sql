...
BEGIN
        ...
        V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.PCO_LIQ_LIQUIDACIONES (PCO_LIQ_ID, PCO_PRC_ID, DD_PCO_LIQ_ID, CNT_ID, usuariocrear, fechacrear, SYS_GUID)
                      SELECT '||V_ESQUEMA||'.s_PCO_LIQ_LIQUIDACIONES.nextval, pco_prc_id, 1, cnt_id, ''SAG'', sysdate, SYS_GUID() AS SYS_GUID
                   FROM (
                    ...
                    where hep.DD_PCO_PEP_ID IN (select DD_PCO_PEP_ID from '||V_ESQUEMA||'.DD_PCO_PRC_ESTADO_PREPARACION where DD_PCO_PEP_CODIGO IN (''PR'',''EN'')) 
