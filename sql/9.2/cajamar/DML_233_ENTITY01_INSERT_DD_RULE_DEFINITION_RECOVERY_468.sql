...
BEGIN
    ...
    V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.DD_RULE_DEFINITION WHERE RD_COLUMN = ''PER_PROD_GESTION'' AND RD_TAB = ''Persona''';
    ...
    ELSE
        V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.DD_RULE_DEFINITION
                       (RD_ID, RD_TITLE, RD_COLUMN, RD_TYPE, RD_VALUE_FORMAT, RD_BO_VALUES, RD_TAB, USUARIOCREAR, FECHACREAR, USUARIOMODIFICAR, FECHAMODIFICAR, USUARIOBORRAR, FECHABORRAR, BORRADO)
                   VALUES ('||V_ESQUEMA||'.S_DD_RULE_DEFINITION.NEXTVAL
                          ,''Empleado con productos gestionables''
