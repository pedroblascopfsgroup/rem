...
DECLARE
    ...
    V_TIPO_TAP T_ARRAY_TAP := T_ARRAY_TAP(
		T_TIPO_TAP('CJ062','CJ062_confirmarAnotacionRegistro' ,null ,'comprobarExisteDocumentoMEMB() ? null : ''<div align="justify" style="font-size: 8pt; font-family: Arial;"><p style="margin-bottom: 10px">Para poder continuar debe adjuntar el documento "Mandamiento de Embargo".</p></div>''' ,'comprobarBienesAsociadoPrc() ? null : ''<div align="justify" style="font-size:8pt; font-family:Arial; margin-bottom:10px;">No se puede avanzar la tarea, la actuaci&oacute;n no tiene bienes asociados.</div>''' ,'valores[''CJ062_confirmarAnotacionRegistro''][''comboAlerta''] == DDSiNo.SI ? ''siAlerta'' : ''noAlerta''' ,null ,'0','Confirmar anotación en el registro' ,'0','RECOVERY-2026','0' ,null ,null ,null,'0' ,'EXTTareaProcedimiento','0',null ,null,null ,null,null),
		T_TIPO_TAP('CJ062','CJ062_revisarRegistroEmbargo' ,null ,'comprobarExisteDocumentoREREVC() ? null : ''<div align="justify" style="font-size: 8pt; font-family: Arial;"><p style="margin-bottom: 10px">Para poder continuar debe adjuntar el documento "Renovaci&oacute;n del registro".</p></div>''' ,null ,'valores[''CJ062_revisarRegistroEmbargo''][''comboAlerta''] == DDSiNo.SI ? ''mantenerAlerta'' : ''terminar''' ,null ,'0','Revisar registro de embargo' ,'0','RECOVERY-2026','0' ,null ,null ,1,'0' ,'EXTTareaProcedimiento','0',null ,null,null ,null,null)
    );
    ...
    V_TIPO_PLAZAS T_ARRAY_PLAZAS := T_ARRAY_PLAZAS(
		T_TIPO_PLAZAS(null ,null ,'CJ062_confirmarAnotacionRegistro','15*24*60*60*1000L' ,'0','0','RECOVERY-2026'),
		T_TIPO_PLAZAS(null ,null ,'CJ062_revisarRegistroEmbargo','damePlazo(valores[''CJ062_confirmarAnotacionRegistro''][''fecha'']) + 42*30*24*60*60*1000L' ,'4','0','RECOVERY-2026')
    ); 
    ...
    V_TIPO_TFI T_ARRAY_TFI := T_ARRAY_TFI(
		T_TIPO_TFI('CJ062_confirmarAnotacionRegistro','0','label' ,'titulo','<div align="justify" style="font-size: 8pt; font-family: Arial; margin-bottom: 30px"><p style="margin-bottom: 10px">En esta pantalla se ha de informar la fecha de anotación de los embargos trabados en el Registro de la Propiedad correspondiente.</p><p style="margin-bottom: 10px">Para el supuesto de la existencia de embargo de varios bienes y que estos se encuentren inscritos en diferentes Registros de la Propiedad, se deberá informar en esta pantalla únicamente el de la anotación del primero de ellos. Se deberá abrir la pestaña de "Bienes" para introducir la fecha de anotación en el Registro de la Propiedad de cada uno de los bienes embargados.</p><p style="margin-bottom: 10px">En el caso de que no se haya admitido el registro de alguno de los embargos solicitados, informelo en observaciones.</p><p style="margin-bottom: 10px">En el campo observaciones informar cualquier aspecto relevante que le interesa quede reflejado en ese punto del procedimiento.</p><p style="margin-bottom: 10px">Para finalizar la tarea, deberá adjuntar a través de la pestaña de Adjuntos, el mandamiento de embargo inscrito.</p></div>' ,null ,null ,null ,null,'0','RECOVERY-2026'),
		T_TIPO_TFI('CJ062_confirmarAnotacionRegistro','1','date' ,'fecha','Fecha' ,'tareaExterna.error.PGENERICO_TareaGenerica.campoObligatorio' ,'valor != null && valor != '' ? true : false' ,null ,null,'0','RECOVERY-2026'),
		T_TIPO_TFI('CJ062_confirmarAnotacionRegistro','2','combo' ,'comboAlerta','Activar alerta periódica' ,'tareaExterna.error.PGENERICO_TareaGenerica.campoObligatorio' ,'valor != null && valor != '' ? true : false' ,null ,'DDSiNo','0','RECOVERY-2026'),
		T_TIPO_TFI('CJ062_confirmarAnotacionRegistro','3','textarea' ,'observaciones','Observaciones' ,null ,null ,null ,null,'0','RECOVERY-2026'),
		T_TIPO_TFI('CJ062_revisarRegistroEmbargo','0','label' ,'titulo','<div align="justify" style="font-size: 8pt; font-family: Arial; margin-bottom: 30px;"><p style="margin-bottom: 10px">Para completar esta tarea deber&aacute; acceder a la pesta&ntilde;a "Bienes" de este procedimiento, donde puede encontrar los datos sobre los distintos embargos registrados y que deben ser revisados en este momento.</p><p style="margin-bottom: 10px">Introduzca la fecha en la que se efect&uacute;a la solicitud de renovaci&oacute;n en el campo "Fecha" y la fecha en la que queda renovado cada bien en el campo correspondiente de la pesta&ntilde;a "Bienes".</p><p style="margin-bottom: 10px">En caso de no quedar ning&uacute;n embargo anotado tras esta tarea, indique "No" en el campo "Mantener alerta de caducidad" o "Si" en caso contrario.</p><p style="margin-bottom: 10px">En el campo observaciones informar cualquier aspecto relevante que le interesa quede reflejado en ese punto del procedimiento.</p></div>' ,null ,null ,null ,null,'0','RECOVERY-2026'),
		T_TIPO_TFI('CJ062_revisarRegistroEmbargo','1','combo' ,'comboAlerta','Mantener alerta de caducidad' ,'tareaExterna.error.PGENERICO_TareaGenerica.campoObligatorio' ,'valor != null && valor != '' ? true : false' ,null ,'DDSiNo','0','RECOVERY-2026'),
		T_TIPO_TFI('CJ062_revisarRegistroEmbargo','2','textarea' ,'observaciones','Observaciones' ,null ,null ,null ,null,'0','RECOVERY-2026')
    );
