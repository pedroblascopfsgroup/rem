...
-- Responsable última modificación: Pedro S., PFS Groupp
...
--                                 D_EXP_GESTOR
-- ----------------------------------------------------------------------------------------------
  SELECT COUNT(*) INTO V_NUM_ROW FROM D_EXP_GESTOR WHERE GESTOR_EXP_ID = -1;
  IF (V_NUM_ROW = 0) THEN
    INSERT INTO D_EXP_GESTOR(
      GESTOR_EXP_ID,
      GESTOR_EXP_NOMBRE_COMP) VALUES (-1 ,'Sin Gestor Asignado');
  END IF;
...
/*  
  ...
  'INSERT INTO D_EXP_GESTOR (
      GESTOR_EXP_ID,
      GESTOR_EXP_NOMBRE_COMP,
      GESTOR_EXP_NOMBRE,
      GESTOR_EXP_APELLIDO1,
      GESTOR_EXP_APELLIDO2
      )
  SELECT USU.USU_ID,
  ...
  FROM '||V_SCHEMA_DS||'.USD_USUARIOS_DESPACHOS USD
    JOIN '||V_SCHEMA_DS||'.USU_USUARIOS USU ON USD.USU_ID = USU.USU_ID
    JOIN '||V_SCHEMA_DS||'.GAA_GESTOR_ADICIONAL_ASUNTO GAA ON GAA.USD_ID = USD.USD_ID
    JOIN '||V_SCHEMA_DS||'.DD_TGE_TIPO_GESTOR TGES ON GAA.DD_TGE_ID = TGES.DD_TGE_ID
  WHERE TGES.DD_TGE_DESCRIPCION = ''Gestor EDP''
  GROUP BY USU.USU_ID, USU.USU_NOMBRE, USU.USU_APELLIDO1, USU.USU_APELLIDO2';
*/
  ...
  SELECT PEF.PEF_ID,
      PEF.PEF_DESCRIPCION
  FROM '||V_SCHEMA_DS||'.PEF_PERFILES PEF
  WHERE PEF.BORRADO = 0';
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_EXP_GESTOR. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;


-- ----------------------------------------------------------------------------------------------
--                                 D_EXP_GESTOR_COMITE
-- ----------------------------------------------------------------------------------------------
 /* SELECT COUNT(*) INTO V_NUM_ROW FROM D_EXP_GESTOR_COMITE WHERE GESTOR_COMITE_EXP_ID = -1;
  IF (V_NUM_ROW = 0) THEN
    INSERT INTO D_EXP_GESTOR_COMITE(
      GESTOR_COMITE_EXP_ID,
      GESTOR_COMITE_EXP_NOMBRE_COMP,
      GESTOR_COMITE_EXP_NOMBRE,
      GESTOR_COMITE_EXP_APELLIDO1,
      GESTOR_COMITE_EXP_APELLIDO2) VALUES (-1 ,'Sin Gestor Asignado','Sin Gestor Asignado', 'Sin Gestor Asignado', 'Sin Gestor Asignado');
  END IF;
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_EXP_GESTOR_COMITE. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;

*/
-- ----------------------------------------------------------------------------------------------
--                                 D_EXP_SUPERVISOR
-- ----------------------------------------------------------------------------------------------
  SELECT COUNT(*) INTO V_NUM_ROW FROM D_EXP_SUPERVISOR WHERE SUPERVISOR_EXP_ID = -1;
  IF (V_NUM_ROW = 0) THEN
    INSERT INTO D_EXP_SUPERVISOR(
      SUPERVISOR_EXP_ID,
      SUPERVISOR_EXP_NOMBRE_COMP) VALUES (-1 ,'Sin Supervisor Asignado');
  END IF;
/*
  EXECUTE IMMEDIATE
  'INSERT INTO D_EXP_SUPERVISOR (
      SUPERVISOR_EXP_ID,
      SUPERVISOR_EXP_NOMBRE_COMP,
      SUPERVISOR_EXP_NOMBRE,
      SUPERVISOR_EXP_APELLIDO1,
      SUPERVISOR_EXP_APELLIDO2
      )
  SELECT USU.USU_ID,
      NVL(TRIM(REPLACE(USU.USU_NOMBRE||'' ''||USU.USU_APELLIDO1||'' ''||USU.USU_APELLIDO2,'' '',''  '')), ''Desconocido''),
      NVL(USU.USU_NOMBRE, ''Desconocido''),
  ...
  FROM '||V_SCHEMA_DS||'.USD_USUARIOS_DESPACHOS USD
    ...
    JOIN '||V_SCHEMA_DS||'.GAA_GESTOR_ADICIONAL_ASUNTO GAA ON GAA.USD_ID = USD.USD_ID
    JOIN '||V_SCHEMA_DS||'.DD_TGE_TIPO_GESTOR TGES ON GAA.DD_TGE_ID = TGES.DD_TGE_ID
  WHERE TGES.DD_TGE_DESCRIPCION = ''Supervisor EDP''
  GROUP BY USU.USU_ID, USU.USU_NOMBRE, USU.USU_APELLIDO1, USU.USU_APELLIDO2';
*/
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_EXP_SUPERVISOR. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;


-- ----------------------------------------------------------------------------------------------
--                                 D_EXP_FASE
-- ----------------------------------------------------------------------------------------------
  SELECT COUNT(1) INTO V_NUM_ROW FROM D_EXP_FASE WHERE FASE_EXPEDIENTE_ID = -1;
  IF (V_NUM_ROW = 0) THEN
  V_SQL := 'insert into D_EXP_FASE (FASE_EXPEDIENTE_ID, FASE_EXPEDIENTE_DESC, FASE_EXPEDIENTE_DESC_2) values (-1 ,''Desconocido'', ''Desconocido'')';
    EXECUTE IMMEDIATE (V_SQL);
  END IF;

  V_SQL :=  'insert into D_EXP_FASE(FASE_EXPEDIENTE_ID, FASE_EXPEDIENTE_DESC, FASE_EXPEDIENTE_DESC_2)
    select DD_EST_ID, DD_EST_DESCRIPCION, DD_EST_DESCRIPCION_LARGA FROM ' || V_SCHEMA_DS || '.DD_EST_ESTADOS_ITINERARIOS EST
  where not exists (select 1 from D_EXP_FASE EXP WHERE EXP.FASE_EXPEDIENTE_ID = EST.DD_EST_ID)';
  EXECUTE IMMEDIATE (V_SQL);
...
if ((select count(*) from D_EXP_ESTADO_PALANCA where ESTADO_PALANCA_ID = -1) = 0) then
 ...
 insert into D_EXP_ESTADO_PALANCA (ESTADO_PALANCA_ID, ESTADO_PALANCA_DESC)
    select id_estado_palanca, desc_estado_palanca from RECOVERY_CM_datastage.expedientes_fake where id_estado_palanca is not null group by id_estado_palanca;
...
if ((select count(*) from D_EXP_TIPO_SANCION where TIPO_SANCION_ID = -1) = 0) then
 ...
 insert into D_EXP_TIPO_SANCION (TIPO_SANCION_ID, TIPO_SANCION_DESC)
    select id_tipo_sancion, desc_tipo_sancion from RECOVERY_CM_datastage.expedientes_fake where id_tipo_sancion is not null group by id_tipo_sancion;
...
if ((select count(*) from D_EXP_ESTADO_INCIDENCIA where ESTADO_INCIDENCIA_ID = -1) = 0) then
 ...
 insert into D_EXP_ESTADO_INCIDENCIA (ESTADO_INCIDENCIA_ID, ESTADO_INCIDENCIA_DESC)
    select id_estado_incidencia, desc_estado_incidencia from RECOVERY_CM_datastage.expedientes_fake where id_estado_incidencia is not null group by id_estado_incidencia;
...
--                                      D_EXP_TIPO_EXPEDIENTE
-- ----------------------------------------------------------------------------------------------
SELECT COUNT(1) INTO V_NUM_ROW FROM D_EXP_TIPO_EXPEDIENTE WHERE TIPO_EXPEDIENTE_ID = -1;
  IF (V_NUM_ROW = 0) THEN
  V_SQL := 'insert into D_EXP_TIPO_EXPEDIENTE (TIPO_EXPEDIENTE_ID, TIPO_EXPEDIENTE_DESC, TIPO_EXPEDIENTE_DESC_2) values (-1 ,''Desconocido'',''Desconocido'')';
    EXECUTE IMMEDIATE (V_SQL);
  END IF;

  V_SQL :=  'insert into D_EXP_TIPO_EXPEDIENTE (TIPO_EXPEDIENTE_ID, TIPO_EXPEDIENTE_DESC, TIPO_EXPEDIENTE_DESC_2)
    select  DD_TPX_ID, DD_TPX_DESCRIPCION, DD_TPX_DESCRIPCION_LARGA from ' || V_SCHEMA_DS || '.dd_tpx_tipo_expediente tge
  where not exists (select 1 from D_EXP_TIPO_EXPEDIENTE exp WHERE exp.TIPO_EXPEDIENTE_ID = tge.DD_TPX_ID)';
  EXECUTE IMMEDIATE (V_SQL);
  ...
  commit;
   ...
   --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_EXP_TIPO_EXPEDIENTE. Registros Insertados: ' || TO_CHAR(V_ROWCOUNT), 3;
...
--                                      D_EXP_DIR_TERRITORIAL
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_EXP_DIR_TERRITORIAL where DIR_TERRITORIAL_ID = -1;

  if (V_NUM_ROW = 0) then
    insert into D_EXP_DIR_TERRITORIAL (DIR_TERRITORIAL_ID, DIR_TERRITORIAL_DESC, DIR_TERRITORIAL_DESC_2) values (-1 ,'Desconocido','Desconocido');

  end if;
  
  V_SQL := 'insert into D_EXP_DIR_TERRITORIAL (DIR_TERRITORIAL_ID, DIR_TERRITORIAL_DESC, DIR_TERRITORIAL_DESC_2)
    select ZON_ID, ZON_DESCRIPCION, OFI.OFI_CODIGO_OFICINA from ' || V_SCHEMA_DS || '.ZON_ZONIFICACION ZON, ' || V_SCHEMA_DS || '.OFI_OFICINAS OFI 
  where ZON.NIV_ID = 3 AND OFI.OFI_ID = ZON.OFI_ID --(DT)
  AND not exists (select 1 from D_EXP_DIR_TERRITORIAL EXP WHERE EXP.DIR_TERRITORIAL_ID = ZON.ZON_ID)';
  EXECUTE IMMEDIATE (V_SQL);
  V_ROWCOUNT := sql%rowcount;
  commit;
  ...
  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_EXP_DIR_TERRITORIAL. Realizados INSERTS', 3;


-- ----------------------------------------------------------------------------------------------
--                                      D_EXP_CNT_ES_PASE
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_EXP_CNT_ES_PASE where CNT_ES_PASE_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_EXP_CNT_ES_PASE (CNT_ES_PASE_ID, CNT_ES_PASE_DESC) values (-1 ,'Desconocido');
  end if;
  
  select count(*) into V_NUM_ROW from D_EXP_CNT_ES_PASE where CNT_ES_PASE_ID = 0;
  if (V_NUM_ROW = 0) then
    insert into D_EXP_CNT_ES_PASE (CNT_ES_PASE_ID, CNT_ES_PASE_DESC) values (0 ,'No');
  end if;
  select count(*) into V_NUM_ROW from D_EXP_CNT_ES_PASE where CNT_ES_PASE_ID = 1;
  if (V_NUM_ROW = 0) then
    insert into D_EXP_CNT_ES_PASE (CNT_ES_PASE_ID, CNT_ES_PASE_DESC) values (1 ,'Si');
  end if; 
  
  commit;

  --Log_Proceso
  execute immediate 'BEGIN INSERTAR_Log_Proceso(:NOMBRE_PROCESO, :DESCRIPCION, :TAB); END;' USING IN V_NOMBRE, 'D_EXP_CNT_ES_PASE. Realizados INSERTS', 3;

  
-- ----------------------------------------------------------------------------------------------
--                                      D_EXP_PER_ES_PASE
-- ----------------------------------------------------------------------------------------------
  select count(*) into V_NUM_ROW from D_EXP_PER_ES_PASE where PER_ES_PASE_ID = -1;
  if (V_NUM_ROW = 0) then
    insert into D_EXP_PER_ES_PASE (PER_ES_PASE_ID, PER_ES_PASE_DESC) values (-1 ,'Desconocido');
  end if;
  
  select count(*) into V_NUM_ROW from D_EXP_PER_ES_PASE where PER_ES_PASE_ID = 0;
  if (V_NUM_ROW = 0) then
    insert into D_EXP_PER_ES_PASE (PER_ES_PASE_ID, PER_ES_PASE_DESC) values (0 ,'No');
  end if;
  select count(*) into V_NUM_ROW from D_EXP_PER_ES_PASE where PER_ES_PASE_ID = 1;
  if (V_NUM_ROW = 0) then
    insert into D_EXP_PER_ES_PASE (PER_ES_PASE_ID, PER_ES_PASE_DESC) values (1 ,'Si');
  end if; 
