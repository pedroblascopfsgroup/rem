...
--## INCIDENCIA_LINK=RECOVERY-2359
--## PRODUCTO=NO
--## 
--## Finalidad: Modificación de la carga de bienes, para que estos se asocien al procedimiento padre, siempre que este no sea precontencioso.
...
-- Fecha ultima modificación: 08/07/2016
-- Motivos del cambio: Modificación de la carga de bienes, para que estos se asocien al procedimiento padre, siempre que este no sea precontencioso
        ...
        'insert into TMP_H_BIE(
            ...
            SELECT  ''' || fecha || ''',
                    ''' || fecha || ''',
                     ...
                     (SELECT MIN(PRC2.PRC_ID) FROM '|| V_DATASTAGE ||'.PRC_PROCEDIMIENTOS PRC2 join '|| V_DATASTAGE ||'.dd_tpo_tipo_procedimiento TPO ON prc2.dd_tpo_id=TPO.dd_tpo_id left join
'|| V_DATASTAGE ||'.pco_prc_procedimientos pre
on pre.prc_id= PRC2.prc_id
where pre.prc_id  is null AND TPO.DD_TPO_DESCRIPCION LIKE ''P.%'' AND PRC2.ASU_ID = SUB.ASU_ID GROUP BY PRC2.ASU_ID) AS PROCEDIMIENTO_ID,
...
'|| V_DATASTAGE ||'.pco_prc_procedimientos pre
on pre.prc_id= PRC2.prc_id
where pre.prc_id  is null AND TPO.DD_TPO_DESCRIPCION LIKE ''P.%'' AND PRC2.ASU_ID = PRC.ASU_ID GROUP BY PRC2.ASU_ID) AS PROCEDIMIENTO_ID,
                     (case when BIE.BIE_VIVIENDA_HABITUAL=1 THEN 1 when BIE.BIE_VIVIENDA_HABITUAL =2 THEN 0 else -1 end)  AS VIVIENDA_HABITUAL_ID
