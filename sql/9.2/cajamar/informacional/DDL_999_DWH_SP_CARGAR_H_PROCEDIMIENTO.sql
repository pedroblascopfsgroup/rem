...
execute immediate V_SQL USING OUT O_ERROR_STATUS;
 ...
 select  DPR1.PRC_ID, DPR1.DPR_PARALIZA,  DPR1.DPR_FINALIZA,  max(trunc(DPR1.DPR_FECHA_PARA))DPR_FECHA_PARA, DPR1.DD_DPA_ID
 
  from  '||V_DATASTAGE||'.DPR_DECISIONES_PROCEDIMIENTOS DPR1
  
  JOIN (SELECT PRC_ID,MAX(FECHAMODIFICAR)FECHAMODIFICAR FROM '||V_DATASTAGE||'.DPR_DECISIONES_PROCEDIMIENTOS WHERE DD_EDE_ID = 2 and DPR_PARALIZA = 1 
  and trunc(FECHACREAR) <='''||fecha||''' AND TRUNC(DPR_FECHA_PARA)>='''||fecha||''' AND BORRADO=0
  GROUP BY PRC_ID )DPR2
  
  ON DPR1.PRC_ID=DPR2.PRC_ID 
  
  where  DPR1.DD_EDE_ID = 2 and  DPR1.DPR_PARALIZA =1
  AND DPR1.FECHAMODIFICAR=DPR2.FECHAMODIFICAR
  
  and trunc( DPR1.FECHACREAR) <='''||fecha||''' 
  AND TRUNC(DPR1.DPR_FECHA_PARA)>='''||fecha||''' 
  AND  DPR1.BORRADO=0 
  group by DPR1.PRC_ID, DPR1.DPR_PARALIZA,  DPR1.DPR_FINALIZA, DPR1.DD_DPA_ID';
