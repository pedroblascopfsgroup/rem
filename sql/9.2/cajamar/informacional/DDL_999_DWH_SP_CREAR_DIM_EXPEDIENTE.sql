 ...
 V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''CREATE'', ''D_EXP_DIR_TERRITORIAL'',
   ...
   V_SQL := 'BEGIN OPERACION_DDL.DDL_INDEX(''CREATE'', ''DIR_TERRITORIAL_PK'', ''D_EXP_DIR_TERRITORIAL (DIR_TERRITORIAL_ID)'', ''S'', ''UNIQUE'', :error); END;';
    ...
    execute immediate V_SQL USING OUT error;
       ...
       ------------------------------ D_EXP_PER_ES_PASE --------------------------
    V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''CREATE'', ''D_EXP_PER_ES_PASE'',
                            ''PER_ES_PASE_ID NUMBER(16,0) NOT NULL,
                              PER_ES_PASE_DESC VARCHAR2(250 CHAR)'', 
                          :error); END;';

    execute immediate V_SQL USING OUT error;

	 V_SQL := 'BEGIN OPERACION_DDL.DDL_INDEX(''CREATE'', ''D_EXP_PER_ES_PASE_PK'', ''D_EXP_PER_ES_PASE (PER_ES_PASE_ID)'', ''S'', ''UNIQUE'', :error); END;';
    execute immediate V_SQL USING OUT error;
	
       ------------------------------ D_EXP_CNT_ES_PASE --------------------------
    V_SQL :=  'BEGIN OPERACION_DDL.DDL_TABLE(''CREATE'', ''D_EXP_CNT_ES_PASE'',
                            ''CNT_ES_PASE_ID NUMBER(16,0) NOT NULL,
                              CNT_ES_PASE_DESC VARCHAR2(250 CHAR)'', 
                          :error); END;';

    execute immediate V_SQL USING OUT error;

	 V_SQL := 'BEGIN OPERACION_DDL.DDL_INDEX(''CREATE'', ''D_EXP_CNT_ES_PASE_PK'', ''D_EXP_CNT_ES_PASE (CNT_ES_PASE_ID)'', ''S'', ''UNIQUE'', :error); END;';
    execute immediate V_SQL USING OUT error;
