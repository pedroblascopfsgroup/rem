...
BEGIN
	...
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_MASTER||'.usu_usuarios WHERE usu_username = ''val.gesriesgo''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;          

    IF V_NUM_TABLAS = 0 THEN

		V_MSQL := 'insert into ' || V_ESQUEMA_MASTER || '.usu_usuarios (usu_id, entidad_id, usu_username, usu_password, usu_nombre,usu_apellido1,usu_apellido2,usu_mail, usuariocrear,fechacrear, usu_externo, usu_grupo)  values  ( ' || V_ESQUEMA_MASTER || '.s_usu_usuarios.nextval, 1,''val.gesriesgo'',''1234'',''Gestor de riesgos'','''','''','''' , ''JSV'', sysdate, 0,0)';
		
		EXECUTE IMMEDIATE V_MSQL;
	END IF;
	
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_MASTER||'.usu_usuarios WHERE usu_username = ''val.dirterriegos''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;          

    IF V_NUM_TABLAS = 0 THEN

		V_MSQL := 'insert into ' || V_ESQUEMA_MASTER || '.usu_usuarios (usu_id, entidad_id, usu_username, usu_password, usu_nombre,usu_apellido1,usu_apellido2,usu_mail, usuariocrear,fechacrear, usu_externo, usu_grupo)  values  ( ' || V_ESQUEMA_MASTER || '.s_usu_usuarios.nextval, 1,''val.dirterriegos'',''1234'',''Director territorial riesgos'','''','''','''' , ''JSV'', sysdate, 0,0)';
	...
	V_SQL := 'SELECT COUNT(1) FROM '|| V_ESQUEMA ||'.PEF_PERFILES WHERE PEF_CODIGO = ''GES_RIESGOS''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;          

    IF V_NUM_TABLAS = 0 THEN

		V_MSQL := 'Insert into  ' || V_ESQUEMA || '.PEF_PERFILES (PEF_ID,PEF_DESCRIPCION_LARGA,PEF_DESCRIPCION,VERSION,USUARIOCREAR,FECHACREAR,USUARIOMODIFICAR,FECHAMODIFICAR,USUARIOBORRAR,FECHABORRAR,BORRADO,PEF_CODIGO,PEF_ES_CARTERIZADO,DTYPE) values (' || V_ESQUEMA || '.S_PEF_PERFILES.nextval,''Acceso a las funcionalidades de Gestor de riesgos'',''Gestor de riesgos'',''0'',''SAG'',sysdate,null,null,null,null,''0'',''GES_RIESGOS'',''0'',''EXTPerfil'')';
	
		EXECUTE IMMEDIATE V_MSQL;
	END IF;
	
	V_SQL := 'SELECT COUNT(1) FROM '|| V_ESQUEMA ||'.PEF_PERFILES WHERE PEF_CODIGO = ''DIR_TERRITORIAL_RIESGOS''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;          

    IF V_NUM_TABLAS = 0 THEN

		V_MSQL := 'Insert into  ' || V_ESQUEMA || '.PEF_PERFILES (PEF_ID,PEF_DESCRIPCION_LARGA,PEF_DESCRIPCION,VERSION,USUARIOCREAR,FECHACREAR,USUARIOMODIFICAR,FECHAMODIFICAR,USUARIOBORRAR,FECHABORRAR,BORRADO,PEF_CODIGO,PEF_ES_CARTERIZADO,DTYPE) values (' || V_ESQUEMA || '.S_PEF_PERFILES.nextval,''Acceso a las funcionalidades propias de la dirección territorial de riesgos'',''Director territorial riesgos'',''0'',''SAG'',sysdate,null,null,null,null,''0'',''DIR_TERRITORIAL_RIESGOS'',''0'',''EXTPerfil'')';
	...
	V_SQL := 'SELECT COUNT(1) FROM '|| V_ESQUEMA ||'.zon_pef_usu zpu WHERE zpu.zon_id = (select max(zon_id) from ' || V_ESQUEMA || '.zon_zonificacion where zon_cod =''01'') AND zpu.pef_id = (SELECT pef_id FROM ' || V_ESQUEMA || '.pef_perfiles WHERE PEF_CODIGO = ''GES_RIESGOS'' and borrado = 0) AND zpu.usu_id = (SELECT usu_id FROM ' || V_ESQUEMA_MASTER || '.usu_usuarios WHERE usu_username = ''val.gesriesgo'')';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;          

    IF V_NUM_TABLAS = 0 THEN
		V_MSQL := 'insert into ' || V_ESQUEMA || '.zon_pef_usu zpu (zpu.zpu_id, zpu.zon_id, zpu.pef_id, zpu.usu_id, zpu.usuariocrear, zpu.fechacrear)values ( ' || V_ESQUEMA || '.s_zon_pef_usu.nextval, (select max(zon_id) from ' || V_ESQUEMA || '.zon_zonificacion where zon_cod =''01''),(SELECT pef_id FROM ' || V_ESQUEMA || '.pef_perfiles WHERE PEF_CODIGO = ''GES_RIESGOS'' and borrado = 0),(SELECT usu_id FROM ' || V_ESQUEMA_MASTER || '.usu_usuarios WHERE usu_username = ''val.gesriesgo'') , ''JSV'', sysdate )';
		
		EXECUTE IMMEDIATE V_MSQL;
	END IF;
	
	V_SQL := 'SELECT COUNT(1) FROM '|| V_ESQUEMA ||'.zon_pef_usu zpu WHERE zpu.zon_id = (select max(zon_id) from ' || V_ESQUEMA || '.zon_zonificacion where zon_cod =''01'') AND zpu.pef_id = (SELECT pef_id FROM ' || V_ESQUEMA || '.pef_perfiles WHERE PEF_CODIGO = ''DIR_TERRITORIAL_RIESGOS'' and borrado = 0) AND zpu.usu_id = (SELECT usu_id FROM ' || V_ESQUEMA_MASTER || '.usu_usuarios WHERE usu_username = ''val.dirterriegos'')';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;          

    IF V_NUM_TABLAS = 0 THEN
		V_MSQL := 'insert into ' || V_ESQUEMA || '.zon_pef_usu zpu (zpu.zpu_id, zpu.zon_id, zpu.pef_id, zpu.usu_id, zpu.usuariocrear, zpu.fechacrear)values ( ' || V_ESQUEMA || '.s_zon_pef_usu.nextval, (select max(zon_id) from ' || V_ESQUEMA || '.zon_zonificacion where zon_cod =''01''),(SELECT pef_id FROM ' || V_ESQUEMA || '.pef_perfiles WHERE PEF_CODIGO = ''DIR_TERRITORIAL_RIESGOS'' and borrado = 0),(SELECT usu_id FROM ' || V_ESQUEMA_MASTER || '.usu_usuarios WHERE usu_username = ''val.dirterriegos'') , ''JSV'', sysdate )';
