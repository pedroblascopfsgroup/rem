...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA_02||'.FUN_PEF... Comprobaciones previas entidad02'); 
   	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_02||'.FUN_PEF WHERE PEF_ID IN (SELECT PEF_ID FROM '||V_ESQUEMA_02||'.PEF_PERFILES WHERE PEF_CODIGO=''PROCUINTEGRAL'' OR PEF_CODIGO=''PROCUCAJAMAR'')';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA_02 || '.FUN_PEF... NO hay registros que borrar ');
	ELSE
		V_MSQL := 'DELETE FROM '||V_ESQUEMA_02||'.FUN_PEF' ||
					' WHERE PEF_ID IN (SELECT PEF_ID FROM '||V_ESQUEMA_02||'.PEF_PERFILES WHERE PEF_CODIGO=''PROCUINTEGRAL'' OR PEF_CODIGO=''PROCUCAJAMAR'') ';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Datos BORRADOS de la tabla '||V_ESQUEMA_02||'.FUN_PEF -- donde los perfiles son PROCUINTEGRAL o PROCUCAJAMAR');
	END IF;
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA_02||'.ZON_PEF_USU... Comprobaciones previas entidad02'); 
   	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_02||'.ZON_PEF_USU WHERE PEF_ID IN (SELECT PEF_ID FROM '||V_ESQUEMA_02||'.PEF_PERFILES WHERE PEF_CODIGO=''PROCUINTEGRAL'' OR PEF_CODIGO=''PROCUCAJAMAR'')';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA_02 || '.ZON_PEF_USU... NO hay registros que borrar ');
	ELSE
		V_MSQL := 'DELETE FROM '||V_ESQUEMA_02||'.ZON_PEF_USU' ||
					' WHERE PEF_ID IN (SELECT PEF_ID FROM '||V_ESQUEMA_02||'.PEF_PERFILES WHERE PEF_CODIGO=''PROCUINTEGRAL'' OR PEF_CODIGO=''PROCUCAJAMAR'') ';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Datos BORRADOS de la tabla '||V_ESQUEMA_02||'.ZON_PEF_USU -- donde los perfiles son PROCUINTEGRAL o PROCUCAJAMAR');
	END IF;
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA_02||'.PEF_PERFILES... BORRADO DE PROCUINTEGRAL y PROCUCAJAMAR en entidad02 '); 
   	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_02||'.PEF_PERFILES WHERE PEF_CODIGO=''PROCUINTEGRAL'' OR PEF_CODIGO=''PROCUCAJAMAR''';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA_02 || '.PEF_PERFILES... NO hay registros que borrar ');
	ELSE
		V_MSQL := 'DELETE FROM '||V_ESQUEMA_02||'.PEF_PERFILES' ||
					' WHERE PEF_CODIGO=''PROCUINTEGRAL'' OR PEF_CODIGO=''PROCUCAJAMAR'' ';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Datos BORRADOS de la tabla '||V_ESQUEMA_02||'.PEF_PERFILES -- donde los perfiles son PROCUINTEGRAL o PROCUCAJAMAR');
	END IF;
