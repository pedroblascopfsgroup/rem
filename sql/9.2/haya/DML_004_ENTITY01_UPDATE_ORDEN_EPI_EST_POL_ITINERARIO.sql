...
BEGIN	
	...
	V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.DD_EPI_EST_POL_ITINERARIO SET DD_EPI_ORDEN = 4   WHERE DD_EPI_CODIGO = ''ENSAN'' ';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[FIN] DD_EPI_EST_POL_ITINERARIO ENSAN actualizado');
	
	V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.DD_EPI_EST_POL_ITINERARIO SET DD_EPI_ORDEN = 5   WHERE DD_EPI_CODIGO = ''DC'' ';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[FIN] DD_EPI_EST_POL_ITINERARIO DC actualizado');
	
	V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.DD_EPI_EST_POL_ITINERARIO SET DD_EPI_ORDEN = 6   WHERE DD_EPI_CODIGO = ''SANC'' ';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[FIN] DD_EPI_EST_POL_ITINERARIO SANC actualizado');
	
	V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.DD_EPI_EST_POL_ITINERARIO SET DD_EPI_ORDEN = 7   WHERE DD_EPI_CODIGO = ''VIG'' ';
	EXECUTE IMMEDIATE V_MSQL;
