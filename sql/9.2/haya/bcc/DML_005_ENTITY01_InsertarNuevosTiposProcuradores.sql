...
V_SQL := 'SELECT COUNT(DD_TGE_ID) FROM '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR WHERE DD_TGE_CODIGO = ''GESPROC''';
EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

	IF V_NUM_TABLAS = 1 THEN
          		
         V_SQL := 'SELECT COUNT(ID) FROM '||V_ESQUEMA_M||'.ENTIDAD WHERE DESCRIPCION = ''CAJAMAR''';
         EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
         ...
         IF V_NUM_TABLAS = 1 THEN
			...
			V_SQL := 'SELECT ID FROM '||V_ESQUEMA_M||'.ENTIDAD WHERE DESCRIPCION = ''CAJAMAR''';
			EXECUTE IMMEDIATE V_SQL INTO V_TMP_2;
			...
			END IF;
		ELSE
			DBMS_OUTPUT.PUT_LINE('[INFO] NO EXISTE ID DE ENTIDAD  CAJAMAR');
		END IF;
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] NO EXISTE PARA CODIGO GESPROC');
	END IF;
