...
BEGIN	
    ...
    IF V_NUM_TABLAS > 0 THEN
    	V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO SET DD_TSUP_ID = (SELECT DD_TGE_ID FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO = ''TGCONGE'') WHERE TAP_CODIGO IN (''H006_AperturaPlazo'',''H006_ResenyarFechaComparecencia'', ''H006_ConfirmarDocCompleta'', ''H006_RealizacionCesionRemate'')';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('OK ACTUALIZADO');
    ELSE
