...
BEGIN	
    ...
    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.MTT_MAP_ADJRECOVERY_ADJCM WHERE TFA_CODIGO_EXTERNO  = ''00240'' AND DD_TFA_ID = (SELECT DD_TFA_ID FROM '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO WHERE DD_TFA_CODIGO = ''INFFISC'')';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    
    IF V_NUM_TABLAS = 0 THEN
	    --Lo relacionamos con el gestor documental
	    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.MTT_MAP_ADJRECOVERY_ADJCM (MTT_ID,DD_TFA_ID,TFA_CODIGO_EXTERNO,VERSION,USUARIOCREAR,FECHACREAR,USUARIOMODIFICAR,FECHAMODIFICAR,USUARIOBORRAR,FECHABORRAR,BORRADO) VALUES ('||V_ESQUEMA||'.S_MTT_METRICAS_TIPO.NEXTVAL,(SELECT DD_TFA_ID FROM '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO WHERE DD_TFA_CODIGO = ''INFFISC''),''00240'',''0'',''CMREC-2886'',SYSDATE ,null,null,null,null,''0'')';
	    DBMS_OUTPUT.PUT_LINE(V_MSQL);
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('OK INSERTADO');
	...
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.MTT_MAP_ADJRECOVERY_ADJCM WHERE TFA_CODIGO_EXTERNO  = ''00370'' AND DD_TFA_ID = (SELECT DD_TFA_ID FROM '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO WHERE DD_TFA_CODIGO = ''DTC'')';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    
    IF V_NUM_TABLAS = 0 THEN
    
		--Lo relacionamos con el gestor documental
	    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.MTT_MAP_ADJRECOVERY_ADJCM (MTT_ID,DD_TFA_ID,TFA_CODIGO_EXTERNO,VERSION,USUARIOCREAR,FECHACREAR,USUARIOMODIFICAR,FECHAMODIFICAR,USUARIOBORRAR,FECHABORRAR,BORRADO) VALUES ('||V_ESQUEMA||'.S_MTT_METRICAS_TIPO.NEXTVAL,(SELECT DD_TFA_ID FROM '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO WHERE DD_TFA_CODIGO = ''DTC''),''00370'',''0'',''CMREC-2886'',SYSDATE ,null,null,null,null,''0'')';
	    DBMS_OUTPUT.PUT_LINE(V_MSQL);
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('OK INSERTADO');
