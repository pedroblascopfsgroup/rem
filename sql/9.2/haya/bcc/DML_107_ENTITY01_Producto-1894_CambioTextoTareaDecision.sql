...
DBMS_OUTPUT.PUT_LINE('[INICIO]');
      ...
      EXECUTE IMMEDIATE V_SQL ;      
	   ...
	   END IF;
        ...
        V_SQL := 'UPDATE '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES SET TAR_TAREA = ''Tarea toma de decisión'', TAR_DESCRIPCION=''Tarea toma de decisión'', DD_STA_ID=(SELECT DD_STA_ID FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO=''CJ-814D''),  USUARIOMODIFICAR=''PRODUCTO-1894'', FECHAMODIFICAR=SYSDATE WHERE DD_STA_ID = (select dd_sta_id from '||V_ESQUEMA_M||'.dd_sta_subtipo_Tarea_base where dd_sta_codigo=''CJ-814'') AND TAR_DESCRIPCION=''Tarea del Letrado''';
        EXECUTE IMMEDIATE V_SQL ; 
