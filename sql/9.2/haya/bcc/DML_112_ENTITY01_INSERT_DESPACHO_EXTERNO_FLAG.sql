...
DBMS_OUTPUT.PUT_LINE('[INFO] Actualizar FLAG despacho Notaria de escrituras, Notaria para Acta de liquidación');
V_NUM_TABLAS := 0;
...
IF V_NUM_TABLAS > 0 THEN
  ...
  EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
  IF V_NUM_TABLAS > 0 THEN
  	V_MSQL := 'UPDATE ' || V_ESQUEMA || '.DES_DESPACHO_EXTERNO SET DES_FLAG_INPUT_DESC_USU = 1 WHERE DES_DESPACHO= ''Notaria para nota simple'''; 
  ...
  EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
  IF V_NUM_TABLAS > 0 THEN
  	V_MSQL := 'UPDATE ' || V_ESQUEMA || '.DES_DESPACHO_EXTERNO SET DES_FLAG_INPUT_DESC_USU = 1 WHERE DES_DESPACHO= ''Notaria de escrituras'''; 
  	EXECUTE IMMEDIATE V_MSQL;
  END IF;
  ...
  EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
  IF V_NUM_TABLAS > 0 THEN
    V_MSQL := 'UPDATE ' || V_ESQUEMA || '.DES_DESPACHO_EXTERNO SET DES_FLAG_INPUT_DESC_USU = 0 WHERE DES_DESPACHO= ''Notaria para Acta de liquidación'''; 
    EXECUTE IMMEDIATE V_MSQL;
  END IF;
