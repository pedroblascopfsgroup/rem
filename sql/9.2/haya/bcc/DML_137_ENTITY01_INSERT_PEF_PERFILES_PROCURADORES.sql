...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'PEF_PERFILES';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_PEF_PERFILES';
   V_PREFIJO VARCHAR2(50 CHAR) := 'PEF_';

   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ' USING (SELECT :id id, :codigo codigo, :des descripcion, :des_l descripcion_larga, ''PRODUCTO-1312'' usuariocrear, sysdate fechacrear from DUAL) actual ' ||
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('CENTROPROCURA', 'Perfil para la figura del Servicer de Procuradores', 'Perfil para la figura del Servicer de Procuradores')
    ); 
    V_TMP_TIPO T_TIPO;
...
BEGIN
  ...
  --DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA_M||'.' || V_DDNAME ||'... Borrando datos previos en el diccionario');
  --EXECUTE IMMEDIATE V_BORRADO_LOGICO;
  
  -- LOOP Insertando valores en la tabla del diccionario
  DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'.' || V_DDNAME ||'... Empezando a insertar datos en PEF_PERFILES');
  ...
  LOOP
    ...
    DBMS_OUTPUT.PUT_LINE('INSERTANDO: ' || V_ENTIDAD_ID || ', '''||V_TMP_TIPO(1)||''','''||TRIM(V_TMP_TIPO(2))||''','''||TRIM(V_TMP_TIPO(3))||'''');
  END LOOP;
  COMMIT;
  DBMS_OUTPUT.PUT_LINE('[FIN] '||V_ESQUEMA||'.' || V_DDNAME ||'... Datos del perfil insertado');
