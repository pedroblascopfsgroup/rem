...
--## INCIDENCIA_LINK=PRODUCT0-1195
--## PRODUCTO=NO
--## Finalidad: DML para insertar el fichero ESCJUZTB para el T.Tributacion de Bienes
--##           
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO... Empezando a insertar datos en el diccionario');
    ...
    V_SQL := 'SELECT COUNT(*) FROM ' || V_ESQUEMA || '.DD_TFA_FICHERO_ADJUNTO WHERE DD_TFA_CODIGO = ''ESCJUZTB''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

    IF V_NUM_TABLAS = 0 THEN
    
        DBMS_OUTPUT.PUT_LINE('[INFO] Insertar nuevo tipo documento: ESCJUZTB=Escrito presentado al juzgado (T.Bienes)');
    EXECUTE IMMEDIATE 'Insert into '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO
     ...
     Values
       ('||V_ESQUEMA||'.s_dd_tfa_fichero_adjunto.nextval, ''ESCJUZTB'', ''Escrito presentado al juzgado (T.Bienes)'', ''Escrito presentado al juzgado (T.Bienes)'', 0, ''PRODUCTO-1195'', sysdate, 0, (select dd_tac_id from '||V_ESQUEMA||'.DD_TAC_TIPO_ACTUACION WHERE DD_TAC_CODIGO = ''AP''))';
