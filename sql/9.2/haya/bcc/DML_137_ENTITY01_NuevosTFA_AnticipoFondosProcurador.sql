...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO... Empezando a insertar datos en el diccionario');
    ...
    V_SQL := 'SELECT COUNT(*) FROM ' || V_ESQUEMA || '.DD_TFA_FICHERO_ADJUNTO WHERE DD_TFA_CODIGO = ''FACJUS''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

    IF V_NUM_TABLAS = 0 THEN
    
        DBMS_OUTPUT.PUT_LINE('[INFO] Insertar nuevo tipo documento: FACJUS=Factura justificativa');
    EXECUTE IMMEDIATE 'Insert into '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO
     ...
     Values
       ('||V_ESQUEMA||'.s_dd_tfa_fichero_adjunto.nextval, ''FACJUS'', ''Factura justificativa'', ''Factura justificativa'', 0, ''PRODUCTO-1315'', sysdate, 0, (select dd_tac_id from '||V_ESQUEMA||'.DD_TAC_TIPO_ACTUACION WHERE DD_TAC_CODIGO = ''TR''))';
    
       DBMS_OUTPUT.PUT_LINE('[INFO] Insertar nuevo tipo documento: DOCSUP=Documento del suplido');
    EXECUTE IMMEDIATE 'Insert into '||V_ESQUEMA||'.DD_TFA_FICHERO_ADJUNTO
       (DD_TFA_ID, DD_TFA_CODIGO, DD_TFA_DESCRIPCION, DD_TFA_DESCRIPCION_LARGA, VERSION, USUARIOCREAR, FECHACREAR, BORRADO, DD_TAC_ID)
     Values
       ('||V_ESQUEMA||'.s_dd_tfa_fichero_adjunto.nextval, ''DOCSUP'', ''Documento del suplido'', ''Documento del suplido'', 0, ''PRODUCTO-1315'', sysdate, 0, (select dd_tac_id from '||V_ESQUEMA||'.DD_TAC_TIPO_ACTUACION WHERE DD_TAC_CODIGO = ''TR''))';
    END IF;
