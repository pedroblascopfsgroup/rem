...
--## PRODUCTO=NO
--##
--## Finalidad: Realiza las inserciones de la resolución Auto despachando ejecución para la tarea H001_AutoDespachandoEjecucion Desfavorable.
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    */
    V_TR_ID VARCHAR2(16 CHAR):= 			'203';
    V_TR_CODIGO VARCHAR2(25 CHAR):= 		'R_PR_HIP_AUTO_DESP_DESF';
    V_TR_DESCRIPCION  VARCHAR2(100 CHAR):=	'Auto Despachando Ejecución Desfavorable';
    V_TJ_CODIGO VARCHAR2(20 CHAR):=			'HIP';
    V_TAC_CODIGO VARCHAR2(20 CHAR):=		'ADVANCE'; -- ADVANCE, INFO, etc.
    
    V_TIN_CODIGO VARCHAR2(50 CHAR):=		'I_PR_HIP_AUTO_DESP_DESF';
    
    V_TPO_CODIGO VARCHAR2(20 CHAR):=		'H001';
    V_NODO VARCHAR2(50 CHAR):=				'H001_AutoDespachandoEjecucion';
    ...
    V_INPUT T_ARRAY_INPUT := T_ARRAY_INPUT(
    	...
    	T_INPUT('d_nPlaza','nPlaza'),
    	T_INPUT('d_numJuzgado','numJuzgado'),
    	T_INPUT('d_numProcedimiento','numProcedimiento'),
	    ...
	    FOR I IN V_INPUT.FIRST .. V_INPUT.LAST
	      LOOP
	          ...
	          V_MSQL := 'DELETE FROM '|| V_ESQUEMA ||'.BPM_IDT_INPUT_DATOS WHERE BPM_TPI_ID = (SELECT BPM_TPI_ID FROM '||V_ESQUEMA||'.BPM_TPI_TIPO_PROC_INPUT WHERE BPM_DD_TIN_ID =('||
				'SELECT BPM_DD_TIN_ID FROM '||V_ESQUEMA||'.BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='''||V_TIN_CODIGO||''')  AND BPM_IDT_ID = '||V_ENTIDAD_ID ||' AND DD_TPO_ID = ('||
				'SELECT DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO ='''||V_TPO_CODIGO||'''))';
			  DBMS_OUTPUT.PUT_LINE('Borrando anteriores registros: '''||V_TMP_T_INPUT(1)||''' de '''||V_TR_CODIGO||'''');
		  	  EXECUTE IMMEDIATE V_MSQL;
