...
--## INCIDENCIA_LINK=RECOVERY-119
--## PRODUCTO=NO
--## Finalidad: DML para insertar nuevos tipos en tipo juicio
--##           
...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_TJ_TIPO_JUICIO';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_TJ_TIPO_JUICIO';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_TJ_';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ' USING (SELECT :id id, :codigo codigo, :des descripcion, :des_l descripcion_larga, ''RECOVERY-119'' usuariocrear, sysdate fechacrear from DUAL) actual ' ||
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('ADT', 'T. Adjudicación terceros - HCJ', 'T. Adjudicación terceros - HCJ')
     ,T_TIPO('SBE', 'Subasta electrónica - HCJ', 'Subasta electrónica - HCJ')
    ); 
    V_TMP_TIPO T_TIPO;
...
BEGIN
  ...
  --Insertamos el tipo de procedimiento para cada juicio
  V_SQL:= 'SELECT COUNT(*) FROM ' || V_ESQUEMA || '.DD_TJ_TIPO_JUICIO WHERE DD_TJ_CODIGO = ''ADT''';
  EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
  
  V_SQL_2:= 'SELECT DD_TPO_ID FROM ' || V_ESQUEMA || '.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO = ''P457''';
  EXECUTE IMMEDIATE V_SQL_2 INTO V_NUM_TABLAS_2;
  
  IF V_NUM_TABLAS > 0 THEN
  	V_SQL := 'UPDATE DD_TJ_TIPO_JUICIO SET DD_TPO_ID = '||V_NUM_TABLAS_2||' WHERE DD_TJ_CODIGO = ''ADT''';
    EXECUTE IMMEDIATE V_SQL;
  END IF;
  
  V_SQL:= 'SELECT COUNT(*) FROM ' || V_ESQUEMA || '.DD_TJ_TIPO_JUICIO WHERE DD_TJ_CODIGO = ''SBE''';
  EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
  
  V_SQL_2:= 'SELECT DD_TPO_ID FROM ' || V_ESQUEMA || '.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO = ''P458''';
  EXECUTE IMMEDIATE V_SQL_2 INTO V_NUM_TABLAS_2;
  IF V_NUM_TABLAS > 0 THEN 
  	V_SQL := 'UPDATE DD_TJ_TIPO_JUICIO SET DD_TPO_ID = '||V_NUM_TABLAS_2||' WHERE DD_TJ_CODIGO = ''SBE''';
    EXECUTE IMMEDIATE V_SQL;
  END IF;
  COMMIT;
