...
--## PRODUCTO=NO
--##
--## Finalidad: Realiza las inserciones de la  para la tarea P458_EntregaRecepcionEdictoImpresoTasa.
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    */
    V_TR_ID VARCHAR2(16 CHAR):=             '468';
    V_TR_CODIGO VARCHAR2(25 CHAR):=         'R_ENT_REC_EDI';
    V_TR_DESCRIPCION  VARCHAR2(100 CHAR):=  'Diligencia de ordenación y edicto de subasta';
    V_TJ_CODIGO VARCHAR2(20 CHAR):=         'SBE';
    V_TAC_CODIGO VARCHAR2(20 CHAR):=        'ADVANCE'; -- ADVANCE, INFO, etc.
    
    V_TIN_CODIGO VARCHAR2(50 CHAR):=        'I_ENT_REC_EDI';
    
    V_TPO_CODIGO VARCHAR2(20 CHAR):=        'P458';
    V_NODO VARCHAR2(50 CHAR):=              'P458_EntregaRecepcionEdictoImpresoTasa';
    ...
    V_INPUT T_ARRAY_INPUT := T_ARRAY_INPUT(
        ...
        T_INPUT('d_fechaEntregaEdicto','fechaEntregaEdicto'),
        T_INPUT('d_comboEdicto','comboEdicto'),
        T_INPUT('d_comboCorrecto','comboCorrecto'),
        T_INPUT('d_fechaEntregaImpresoLiquidacionTasa','fechaEntregaImpresoLiquidacionTasa'),
...
BEGIN   
    ...
    EXECUTE IMMEDIATE V_SQL INTO table_count;
        ...
        --Borrado previo
        V_MSQL := 'DELETE FROM '|| V_ESQUEMA ||'.BPM_IDT_INPUT_DATOS WHERE BPM_TPI_ID = (SELECT BPM_TPI_ID FROM '||V_ESQUEMA||'.BPM_TPI_TIPO_PROC_INPUT WHERE BPM_DD_TIN_ID =('||
                'SELECT BPM_DD_TIN_ID FROM '||V_ESQUEMA||'.BPM_DD_TIN_TIPO_INPUT WHERE BPM_DD_TIN_CODIGO='''||V_TIN_CODIGO||''') AND DD_TPO_ID = ('||
                'SELECT DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO ='''||V_TPO_CODIGO||'''))';
        EXECUTE IMMEDIATE V_MSQL;           
