...
DBMS_OUTPUT.PUT_LINE('[INICIO] LAMINACION DE TAREAS DE LA MIGRACION');
    ...
    v_sql := 'DELETE FROM '||v_esquema||'.TMP_GUID_TAR_TAREAS_BCC ';
    execute immediate v_sql;
    DBMS_OUTPUT.PUT_LINE('Truncate');
    v_sql := 'insert into '||v_esquema||'.TMP_GUID_TAR_TAREAS_BCC (SYS_GUID,TAR_TAREA) 
                  select tar.SYS_GUID, tar.TAR_FECHA_VENC from '||v_esquema||'.TAR_TAREAS_NOTIFICACIONES TAR where tar.usuarioborrar ='''||USUARIO||''' ';
    execute immediate v_sql;
