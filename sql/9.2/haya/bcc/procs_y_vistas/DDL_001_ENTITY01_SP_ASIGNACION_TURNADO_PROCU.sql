...
CREATE OR REPLACE PROCEDURE HAYA02.asignacion_turnado_procu (
   p_prc_id       HAYA02.prc_procedimientos.prc_id%TYPE,
   p_username     HAYAMASTER.usu_usuarios.usu_username%TYPE,
   p_plaza_codigo       HAYA02.dd_pla_plazas.dd_pla_codigo%TYPE,
   p_tpo_codigo       HAYA02.dd_tpo_tipo_procedimiento.dd_tpo_codigo%TYPE,
   p_tge_codigo   HAYAMASTER.dd_tge_tipo_gestor.dd_tge_codigo%TYPE := 'PROC'
)
...
CURSOR crs_usd_id (p_usu_id HAYAMASTER.usu_usuarios.usu_id%TYPE)
   IS
      SELECT usd.usd_id
        FROM HAYA02.usd_usuarios_despachos usd
       WHERE usd.usu_id = p_usu_id and borrado = 0;

CURSOR crs_des_id (p_usd_id HAYA02.usd_usuarios_despachos.usd_id%TYPE)
   IS
      SELECT usd.des_id
        FROM HAYA02.usd_usuarios_despachos usd
       WHERE usd.usd_id = p_usd_id;

CURSOR crs_gestores_activos (p_tge_codigo HAYAMASTER.dd_tge_tipo_gestor.dd_tge_codigo%TYPE, p_asu_id HAYA02.asu_asuntos.asu_id%TYPE)
   IS
      SELECT gaa.gaa_id, gaa.usd_id, gaa.fechacrear
        FROM HAYA02.gaa_gestor_adicional_asunto gaa, HAYAMASTER.dd_tge_tipo_gestor tge
       WHERE tge.dd_tge_codigo = p_tge_codigo AND tge.dd_tge_id = gaa.dd_tge_id AND gaa.asu_id = p_asu_id AND gaa.borrado = 0;

CURSOR crs_relacion_existente (p_tge_codigo HAYAMASTER.dd_tge_tipo_gestor.dd_tge_codigo%TYPE, p_asu_id HAYA02.asu_asuntos.asu_id%TYPE, v_des_id HAYA02.des_despacho_externo.des_id%TYPE)
    IS
      SELECT distinct FIRST_VALUE (gah_id) OVER (ORDER BY gah_fecha_desde ASC)
      FROM HAYA02.gah_gestor_adicional_historico
              WHERE gah_fecha_hasta is null AND gah_gestor_id =
                  (SELECT DISTINCT FIRST_VALUE (usd.usd_id) OVER (ORDER BY usd.usd_gestor_defecto DESC) FROM HAYA02.usd_usuarios_despachos usd WHERE usd.des_id = v_des_id)
                AND gah_tipo_gestor_id = (SELECT tge.dd_tge_id  FROM HAYAMASTER.dd_tge_tipo_gestor tge  WHERE tge.dd_tge_codigo = p_tge_codigo)
...
CURSOR crs_plazas_esquema (p_esquema_vigente HAYA02.tup_etp_esq_turnado_procu.etp_id%TYPE, p_plaza_id HAYA02.dd_pla_plazas.dd_pla_id%TYPE, p_tipo_tpo HAYA02.dd_tpo_tipo_procedimiento.dd_tpo_id%TYPE)
   IS
      SELECT ept.ept_id
...
CURSOR crs_id_usu_ganador(p_importe_tarea NUMBER, p_esquema_vigente HAYA02.tup_etp_esq_turnado_procu.etp_id%TYPE)
   IS
...
CURSOR crs_id_usu_ganador_tmp(p_importe_tarea NUMBER, p_esquema_vigente HAYA02.tup_etp_esq_turnado_procu.etp_id%TYPE)
   IS
      SELECT tpc.usu_id
      FROM tup_tpc_turnado_procu_config tpc
      WHERE tpc.ept_id = p_esquema_vigente and p_importe_tarea >= tpc.tpc_importe_desde and p_importe_tarea < tpc.tpc_importe_hasta and tpc.borrado=0;

CURSOR crs_resguardo_tmp(p_importe_tarea NUMBER, p_plazas_esquema HAYA02.tup_etp_esq_turnado_procu.etp_id%TYPE)
    IS
      select tpc.usu_id, tpc.tpc_porcentaje, null, null, ept.dd_pla_id, ept.dd_tpo_id
      FROM tup_tpc_turnado_procu_config tpc
	  join tup_ept_esquema_plazas_tpo ept on tpc.ept_id = ept.ept_id
...
-- Recolecta el numero de asuntos totales por usuarios procurador
   CURSOR crs_num_asu(p_tge_codigo HAYAMASTER.dd_tge_tipo_gestor.dd_tge_codigo%TYPE)
      IS
        select tmp.usu_id, tmp.porc_real, his.dd_pla_id, his.dd_tpo_id, count(asu.asu_id) num_asu
            from haya02.tup_his_historico his 
...
-- Elige el usuario con mejor puntuacion
  CURSOR crs_usuario_puntos
    IS
      select usu_id from
        (select usu_id, (PORC_REAL - PORC_ASU) res from HAYA02.TUP_TMP_CALCULOS_TURN_PROCU order by res DESC, PORC_REAL desc)
      where rownum=1;
...
--Recogemos el id del tpo a partir del código que llega por parámetro
  CURSOR crs_obtener_id_tpo
    IS
      select dd_tpo_id
      from HAYA02.DD_TPO_TIPO_PROCEDIMIENTO
...
--Recogemos el total de registros de la tabla tmp
  CURSOR crs_obtener_total_tmp
    IS
      select count(*)
      from HAYA02.TUP_TMP_CALCULOS_TURN_PROCU;

--------ESTA CONSULTA ES PARA EL CASO GENÉRICO, DONDE TANTO PLAZA COMO TPO ES NULL
CURSOR crs_plazas_esquema_generico (p_esquema_vigente HAYA02.tup_etp_esq_turnado_procu.etp_id%TYPE)
   IS
      SELECT ept.ept_id
      FROM tup_ept_esquema_plazas_tpo ept
...
p_asu_id       HAYA02.asu_asuntos.asu_id%TYPE;
v_usu_id_pfs                  HAYA02.des_despacho_externo.des_id%TYPE;
v_usu_id_aba                 HAYA02.des_despacho_externo.des_id%TYPE;
v_usu_id_mc                  HAYA02.des_despacho_externo.des_id%TYPE;
p_importe_tarea     NUMBER;
p_tpo_tarea    HAYA02.DD_TPO_TIPO_PROCEDIMIENTO.DD_TPO_CODIGO%TYPE;

v_usd_id_result       HAYA02.usd_usuarios_despachos.usd_id%TYPE;
v_des_id                  HAYA02.des_despacho_externo.des_id%TYPE;
v_gah_existente           HAYA02.gah_gestor_adicional_historico.gah_id%TYPE;

p_tipo_tpo               HAYA02.dd_tpo_tipo_procedimiento.dd_tpo_id%TYPE;

p_plaza_id               HAYA02.dd_pla_plazas.dd_pla_id%TYPE;

p_esquema_vigente         HAYA02.tup_etp_esq_turnado_procu.etp_id%TYPE;
p_plazas_esquema   HAYA02.tup_ept_esquema_plazas_tpo.ept_id%TYPE;

p_id_usu_ganador HAYA02.des_despacho_externo.des_id%TYPE;

p_id_usu_ganador_tmp HAYA02.des_despacho_externo.des_id%TYPE;
v_resguardo_tmp          HAYA02.TUP_TMP_CALCULOS_TURN_PROCU%ROWTYPE;

v_usuid_tabla_tmp          HAYA02.TUP_TMP_CALCULOS_TURN_PROCU.usu_id%TYPE;

v_filtro_numAsuntos       crs_num_asu%ROWTYPE;
v_calculo                 NUMBER;
v_filtro_porcAsuntos      crs_porcentaje_asu%ROWTYPE;

p_plaza_id_parametro       HAYA02.dd_pla_plazas.dd_pla_id%TYPE;
p_tpo_id_parametro       HAYA02.dd_tpo_tipo_procedimiento.dd_tpo_id%TYPE;

v_total_tmp                 NUMBER;
p_plazas_esquema_generico   HAYA02.tup_ept_esquema_plazas_tpo.ept_id%TYPE;
        ...
        OPEN crs_resguardo_tmp(p_importe_tarea, p_plazas_esquema);
            LOOP
              FETCH crs_resguardo_tmp INTO v_resguardo_tmp;
                ...
                insert into HAYA02.TUP_TMP_CALCULOS_TURN_PROCU
...
THEN
    ...
    DBMS_OUTPUT.put_line ('EL EPT QUE OBTENEMOS ES: '||p_plazas_esquema);
     ...
     -- Si existe algun gestor activo del tipo indicado para el asunto, se añade al historico y se borra la relacion
          ...
          LOOP
             DELETE FROM HAYA02.gaa_gestor_adicional_asunto gaa
    ...
    -- Insertamos en temporal el numero de asuntos por usuario
                      ...
                      LOOP
                        FETCH crs_num_asu INTO v_filtro_numAsuntos;
                          ...
                          update HAYA02.TUP_TMP_CALCULOS_TURN_PROCU
            ...
            execute immediate 'select sum(num_asu) from HAYA02.TUP_TMP_CALCULOS_TURN_PROCU' INTO v_calculo;
    ...
    OPEN crs_usd_id (v_usuid_tabla_tmp);
      ...
      -- Se añade la relacion entre el despacho elegido y el asunto
          INSERT INTO HAYA02.gaa_gestor_adicional_asunto
                      (gaa_id, asu_id, usd_id, dd_tge_id, VERSION,
                       usuariocrear, fechacrear, borrado
                      )
               VALUES (HAYA02.s_gaa_gestor_adicional_asunto.NEXTVAL, p_asu_id,
                      v_usd_id_result,(SELECT tge.dd_tge_id
                          FROM HAYAMASTER.dd_tge_tipo_gestor tge
      ...
      -- Si quedan rastros en GAH_GESTOR_ADICIONAL_HISTORICO para el mismo asunto, y mismo tipo de gestor, le asignamos una fecha hasta para evitar conflictos
            UPDATE HAYA02.gah_gestor_adicional_historico gah
              ...
              WHERE gah.gah_asu_id = p_asu_id
                AND gah.gah_tipo_gestor_id = (SELECT tge.dd_tge_id  FROM HAYAMASTER.dd_tge_tipo_gestor tge  WHERE tge.dd_tge_codigo = p_tge_codigo)
                AND gah.gah_fecha_hasta is null
                AND (gah.gah_gestor_id <> (SELECT DISTINCT FIRST_VALUE (usd.usd_id) OVER (ORDER BY usd.usd_gestor_defecto DESC) FROM HAYA02.usd_usuarios_despachos usd WHERE usd.des_id = v_des_id) or gah.gah_gestor_id = (SELECT DISTINCT FIRST_VALUE (usd.usd_id) OVER (ORDER BY usd.usd_gestor_defecto DESC) FROM HAYA02.usd_usuarios_despachos usd WHERE usd.des_id = v_des_id));
    ...
    -- Ahora insertamos la relacion asunto-despacho, con el nuevo PROCURADOR seleccionado, y lo insertamos en el historico, pero sin la fecha HASTA, para que recovery lo recoja como el activo.
            ...
            THEN
              INSERT INTO HAYA02.gah_gestor_adicional_historico
