...
DECLARE
    ...
    V_PEF_GESTOR NUMBER;
    V_PEF_SUPER NUMBER;
    V_FUN_EXP_MANUAL NUMBER;
    V_FUN_PERSO_HY NUMBER := NULL;
...
BEGIN
	...
	-- Obtemos el ID de la función: SOLICITAR_EXP_MANUAL_GESTION_DEUDA
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''SOLICITAR_EXP_MANUAL_GESTION_DEUDA''';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM;
	IF V_NUM = 0 THEN
		RAISE_APPLICATION_ERROR(-20101, 'No existe la función SOLICITAR_EXP_MANUAL_GESTION_DEUDA, ejecute previamente el script: sql/9.2/producto/DML_007_ENTITY01_FUNCIONES_CREACION_MANUAL_EXP.sql.');
	ELSE
		V_SQL := 'SELECT FUN_ID FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''SOLICITAR_EXP_MANUAL_GESTION_DEUDA''';
		EXECUTE IMMEDIATE V_SQL INTO V_FUN_EXP_MANUAL;
	END IF;
		
	-- Agregamos la funcion SOLICITAR_EXP_MANUAL_GESTION_DEUDA al nuevo perfil gestor
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE FUN_ID	= '||V_FUN_EXP_MANUAL||' AND PEF_ID = '||V_PEF_GESTOR;
	EXECUTE IMMEDIATE V_SQL INTO V_NUM;
	IF V_NUM = 0 THEN
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF (FUN_ID, PEF_ID, FP_ID, USUARIOCREAR, FECHACREAR, BORRADO)
					VALUES ('||V_FUN_EXP_MANUAL||','||V_PEF_GESTOR||','||V_ESQUEMA||'.S_FUN_PEF.NEXTVAL,''DML'',SYSDATE,0)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Se ha agregado la función  SOLICITAR_EXP_MANUAL_GESTION_DEUDA al perfil GESTDEUDA.');
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] El perfil GESTDEUDA ya tenía asignada la función: SOLICITAR_EXP_MANUAL_GESTION_DEUDA');
	END IF;
	
	-- Obtemos el ID de la función: PERSONALIZACION-HY
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''PERSONALIZACION-HY''';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM;
	IF V_NUM = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] No existe la función PERSONALIZACION-HY.');
	ELSE
		V_SQL := 'SELECT FUN_ID FROM '||V_ESQUEMA_M||'.FUN_FUNCIONES WHERE FUN_DESCRIPCION = ''PERSONALIZACION-HY''';
		EXECUTE IMMEDIATE V_SQL INTO V_FUN_PERSO_HY;
	END IF;	
	
	-- Agregamos la funcion PERSONALIZACION-HY al nuevo perfil gestor	
	IF V_FUN_PERSO_HY IS NOT NULL THEN
		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE FUN_ID	= '||V_FUN_PERSO_HY||' AND PEF_ID = '||V_PEF_GESTOR;
		EXECUTE IMMEDIATE V_SQL INTO V_NUM;
		IF V_NUM = 0 THEN
			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF (FUN_ID, PEF_ID, FP_ID, USUARIOCREAR, FECHACREAR, BORRADO)
						VALUES ('||V_FUN_PERSO_HY||','||V_PEF_GESTOR||','||V_ESQUEMA||'.S_FUN_PEF.NEXTVAL,''DML'',SYSDATE,0)';
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO] Se ha agregado la función PERSONALIZACION-HY al perfil GESTDEUDA.');
		ELSE
			DBMS_OUTPUT.PUT_LINE('[INFO] El perfil GESTDEUDA ya tenía asignada la función: PERSONALIZACION-HY');
		END IF;		
	END IF;
	...
	-- Agregamos la funcion SOLICITAR_EXP_MANUAL_GESTION_DEUDA al nuevo perfil supervisor
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE FUN_ID	= '||V_FUN_EXP_MANUAL||' AND PEF_ID = '||V_PEF_SUPER;
	EXECUTE IMMEDIATE V_SQL INTO V_NUM;
	IF V_NUM = 0 THEN
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF (FUN_ID, PEF_ID, FP_ID, USUARIOCREAR, FECHACREAR, BORRADO)
					VALUES ('||V_FUN_EXP_MANUAL||','||V_PEF_SUPER||','||V_ESQUEMA||'.S_FUN_PEF.NEXTVAL,''DML'',SYSDATE,0)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Se ha agregado la función  SOLICITAR_EXP_MANUAL_GESTION_DEUDA al perfil SUPGESTDEUDA.');
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] El perfil SUPGESTDEUDA ya tenía asignada la función: SOLICITAR_EXP_MANUAL_GESTION_DEUDA');
	END IF;	
    
	-- Agregamos la funcion PERSONALIZACION-HY al nuevo perfil gestor	
	IF V_FUN_PERSO_HY IS NOT NULL THEN
		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.FUN_PEF WHERE FUN_ID	= '||V_FUN_PERSO_HY||' AND PEF_ID = '||V_PEF_SUPER;
		EXECUTE IMMEDIATE V_SQL INTO V_NUM;
		IF V_NUM = 0 THEN
			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF (FUN_ID, PEF_ID, FP_ID, USUARIOCREAR, FECHACREAR, BORRADO)
						VALUES ('||V_FUN_PERSO_HY||','||V_PEF_SUPER||','||V_ESQUEMA||'.S_FUN_PEF.NEXTVAL,''DML'',SYSDATE,0)';
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO] Se ha agregado la función PERSONALIZACION-HY al perfil SUPGESTDEUDA.');
		ELSE
			DBMS_OUTPUT.PUT_LINE('[INFO] El perfil SUPGESTDEUDA ya tenía asignada la función: PERSONALIZACION-HY');
		END IF;		
	END IF;	
	...
	IF V_NUM = 0 THEN		
		...
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.EST_ESTADOS (EST_ID, PEF_ID_GESTOR, PEF_ID_SUPERVISOR, ITI_ID, DD_EST_ID, EST_TELECOBRO, EST_PLAZO, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) 
					VALUES ('||V_EST_RE||', '||V_PEF_SUPER||', '||V_PEF_SUPER||', '||V_ITI||', '||V_DD_EST||', 0, 864000000, 0, ''DML'', SYSDATE, 0)';
		EXECUTE IMMEDIATE V_MSQL;
	...
	*/
	...
	IF V_NUM = 0 THEN		
		...
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.EST_ESTADOS (EST_ID, PEF_ID_GESTOR, PEF_ID_SUPERVISOR, ITI_ID, DD_EST_ID, EST_TELECOBRO, EST_PLAZO, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) 
					VALUES ('||V_EST_SANC||', '||V_PEF_SUPER||', '||V_PEF_SUPER||', '||V_ITI||', '||V_DD_EST||', 0, 864000000, 0, ''DML'', SYSDATE, 0)';
		EXECUTE IMMEDIATE V_MSQL;
