...
BEGIN
	...
	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.DD_TIT_TIPO_ITINERARIOS WHERE DD_TIT_CODIGO = ''DEU'' AND borrado = 0';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM;
	IF V_NUM > 0 THEN
		V_MSQL := 'SELECT DD_TIT_ID FROM '||V_ESQUEMA_M||'.DD_TIT_TIPO_ITINERARIOS WHERE DD_TIT_CODIGO = ''DEU'' AND borrado = 0';
		EXECUTE IMMEDIATE V_MSQL INTO V_DD_TIT_ID;
		
		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.DD_AEX_AMBITOS_EXPEDIENTE WHERE DD_AEX_CODIGO = ''PPGRA'' AND borrado = 0';
		EXECUTE IMMEDIATE V_SQL INTO V_NUM;
		IF V_NUM > 0 THEN
			V_MSQL := 'SELECT DD_AEX_ID FROM '||V_ESQUEMA_M||'.DD_AEX_AMBITOS_EXPEDIENTE WHERE DD_AEX_CODIGO = ''PPGRA'' AND borrado = 0';
			EXECUTE IMMEDIATE V_MSQL INTO V_DD_AEX_ID;
			
			-- Comprobar si existe el itinerario de tipo Gestión de deuda con nombre Estándar
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ITI_ITINERARIOS WHERE ITI_NOMBRE = ''Estándar'' AND borrado = 0';
			EXECUTE IMMEDIATE V_SQL INTO V_NUM;
			
			IF V_NUM > 0 THEN
				V_SQL := 'SELECT ITI_ID FROM '||V_ESQUEMA||'.ITI_ITINERARIOS WHERE ITI_NOMBRE = ''Estándar'' AND DD_TIT_ID = '||V_DD_TIT_ID||' AND borrado = 0 AND ROWNUM = 1';
				EXECUTE IMMEDIATE V_SQL INTO V_ITI;
		
				-- Comprobar si existe el perfil de gestión para los nuevos estados del itinerario
				V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''GESTDEUDA'' AND borrado = 0';
				EXECUTE IMMEDIATE V_SQL INTO V_NUM;
				IF V_NUM > 0 THEN
					V_SQL := 'SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''GESTDEUDA'' AND borrado = 0';
					...
					IF V_NUM > 0 THEN
						...
						END IF;
					ELSE
						DBMS_OUTPUT.PUT_LINE('[ERROR] NO se ha encontrado el estado itinerario Carencia(CAR). No se ha registrado el estado de carencia en el nuevo itinerario.');
					END IF;
					...
					IF V_NUM > 0 THEN
						...
						END IF;
					ELSE
						DBMS_OUTPUT.PUT_LINE('[ERROR] NO se ha encontrado el estado itinerario Gestion de Vencidos(GV). No se ha registrado el estado gestion de vencidos en el nuevo itinerario.');
					END IF;
					
				ELSE		
					DBMS_OUTPUT.PUT_LINE('[ERROR] NO existe el perfil gestor deuda(GESTDEUDA). No se puede continuar..');
				END IF;
		
			ELSE
				DBMS_OUTPUT.PUT_LINE('[ERROR] NO existe un itinerario con nombre Estándar. No se puede continuar..');
			END IF;
		ELSE
			DBMS_OUTPUT.PUT_LINE('[ERROR] NO se ha encontrado el ambito de expediente necesario. No se puede continuar..');
		END IF;
	ELSE
		DBMS_OUTPUT.PUT_LINE('[ERROR] NO se ha encontrado el tipo de itinerario de deuda. No se puede continuar..');
	END IF;
