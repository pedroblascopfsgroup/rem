...
--## PRODUCTO=NO
--## Finalidad: DML para crear nuevos tipos de despacho de Haya-Sareb
--##           
...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_TDE_TIPO_DESPACHO';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_TDE_TIPO_DESPACHO';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_TDE_';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA_M || '.' || V_DDNAME|| ' tabla  ' || 
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('DGDEU', 'Despacho Gestor deuda', 'Despacho Gestor deuda')
     ,T_TIPO('DGAUD', 'Despacho Gestor auditoría', 'Despacho Gestor auditoría')
     ,T_TIPO('DSAUD', 'Despacho Supervisor auditoría', 'Despacho Supervisor auditoría')
     ,T_TIPO('DAJUR', 'Despacho Asesoría jurídica', 'Despacho Asesoría jurídica')
     ,T_TIPO('DGSDE', 'Despacho Gestor soporte deuda', 'Despacho Gestor soporte deuda')
     ,T_TIPO('DSSDE', 'Despacho Supervisor soporte deuda', 'Despacho Supervisor soporte deuda')
     ,T_TIPO('DEADQ', 'Despacho Equipo de Adquisiciones', 'Despacho Equipo de Adquisiciones')
     ,T_TIPO('DSADQ', 'Despacho Supervisor de Adquisiciones', 'Despacho Supervisor de Adquisiciones')
     ,T_TIPO('DGCN', 'Despacho Gestor de CN', 'Despacho Gestor de CN')
     ,T_TIPO('DSCN', 'Despacho Supervisor de CN', 'Despacho Supervisor de CN')
    ); 
    V_TMP_TIPO T_TIPO;
