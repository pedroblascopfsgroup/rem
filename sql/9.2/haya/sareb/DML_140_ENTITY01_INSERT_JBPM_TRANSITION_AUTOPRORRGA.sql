...
--## INCIDENCIA_LINK=PRODUCTO-1885
--## PRODUCTO=NO
--## Finalidad: DML - Soluciona autoProrrogas para tareas que la requieren
--##
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
...
-- EN ALGUNOS ASUNTOS MIGRADOS, NO SE HA INSERTADO EN JBPM_TRANSITION LAS AUTOPRORROGAS PARA CIERTAS TAREAS
...
DECLARE
  ...
  , TAREAS AS (
    ...
    FROM HAYA01.TAR_TAREAS_NOTIFICACIONES
  ...
  FROM HAYA01.PRC_PROCEDIMIENTOS PRC
     inner join HAYA01.asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 1
    JOIN HAYA01.DD_TPO_TIPO_PROCEDIMIENTO TPO ON PRC.DD_TPO_ID = TPO.DD_TPO_ID
    JOIN BPM_DEFINITIONS DEF ON TPO.DD_TPO_XML_JBPM = DEF.NAME_
    JOIN TAREAS TAR ON PRC.PRC_ID = TAR.PRC_ID
    JOIN HAYA01.TEX_TAREA_EXTERNA TEX ON TAR.TAR_ID = TEX.TAR_ID
    JOIN HAYA01.TAP_TAREA_PROCEDIMIENTO TAP ON TEX.TAP_ID = TAP.TAP_ID
  ...
  WHERE PRC.PRC_PROCESS_BPM IS    NULL
    AND PRC.PRC_ID IN (select tar.PRC_ID 
                                      from HAYA01.TAR_TAREAS_NOTIFICACIONES tar, HAYA01.TEX_TAREA_EXTERNA TEX
                                    WHERE TAR.TAR_ID = TEX.TAR_ID
                                      AND tar.borrado = 0 and (tar.tar_tarea_finalizada is null or tar.tar_tarea_finalizada = 0) 
                                      AND tar.USUARIOCREAR LIKE 'HAYA'
                                    GROUP BY tar.PRC_ID
                                    HAVING COUNT(1) > 1)
    AND PRC.USUARIOCREAR = 'HAYA'
  ORDER BY PRC_ID;
  CURSOR CUR_PROCEDIMIENTOS2 IS
  SELECT PRC_ID, count(*) AS CUENTA FROM HAYA01.TMP_ALTA_BPM_INSTANCES group by PRC_ID having count(*)>1;
...
USUARIO VARCHAR(50) := 'PRODUCTO-1885';
BEGIN
  ...
  DELETE FROM HAYA01.TMP_ALTA_BPM_INSTANCES;
  
  
  /*------------ Cerramos conjunto de litigios a usar ---------------------- */
  update HAYA01.prc_procedimientos prc set  prc.prc_process_bpm = null 
  where prc.prc_id in (
    select distinct prc.prc_id
    from HAYA01.Prc_Procedimientos prc
      inner join HAYA01.asu_asuntos asu on prc.asu_id = asu.asu_id and asu.DD_TAS_ID = 1
    where PRC.USUARIOCREAR = 'HAYA'
        AND PRC.PRC_ID IN (select tar.PRC_ID 
                                      from HAYA01.TAR_TAREAS_NOTIFICACIONES tar, HAYA01.TEX_TAREA_EXTERNA TEX
                                    WHERE TAR.TAR_ID = TEX.TAR_ID
                                      AND tar.borrado = 0 and (tar.tar_tarea_finalizada is null or tar.tar_tarea_finalizada = 0) 
  ...
  update HAYA01.TEX_TAREA_EXTERNA TEX2 set TEX2.TEX_TOKEN_ID_BPM=null
  WHERE TEX2.TEX_ID IN (
  SELECT tex.tex_id
  FROM HAYA01.TEX_TAREA_EXTERNA TEX
  ...
  LOOP
    ...
    INSERT INTO HAYA01.TMP_ALTA_BPM_INSTANCES(PRC_ID, INST_ID,DEF_ID, NODE_ID, TAP_CODIGO, TEX_ID, FORK_NODE) 
  ...
  LOOP
    ...
    UPDATE HAYA01.TMP_ALTA_BPM_INSTANCES
    SET
  ...
  UPDATE HAYA01.TMP_ALTA_BPM_INSTANCES SET 
  ...
  FROM HAYA01.TMP_ALTA_BPM_INSTANCES TMP;
  ...
  INSERT INTO HAYAMASTER.JBPM_TOKEN
          ...
          select DISTINCT 
            ...
            ,0
            ,0
      FROM HAYA01.TMP_ALTA_BPM_INSTANCES TMP WHERE TOKEN_PADRE_ID<>TOKEN_ID;
  DBMS_OUTPUT.PUT_LINE('Insertados los root token!!!') ;
  ...
  USING (
                    SELECT DISTINCT INST_ID, TOKEN_PADRE_ID AS TOKEN 
                       FROM HAYA01.TMP_ALTA_BPM_INSTANCES
         ) TMP
  ...
  DBMS_OUTPUT.PUT_LINE('module y vmap actualizadas!!') ;
    ...
    INSERT INTO HAYAMASTER.JBPM_VARIABLEINSTANCE
      ...
      , TMP.INST_ID --PROCESSINSTANCE_
      , V_DBID --LONGVLAUE_
    FROM (SELECT DISTINCT INST_ID, PRC_ID, TOKEN_PADRE_ID AS TOKEN_ID,MODULE_PADRE_ID AS MODULE_ID,VMAP_PADRE_ID AS VMAP_ID FROM HAYA01.TMP_ALTA_BPM_INSTANCES) TMP;
...
--Insertamos variable sin concatenar TOKEN_PADRE_ID  
  ...
  USING (SELECT DISTINCT PRC_ID,INST_ID FROM HAYA01.TMP_ALTA_BPM_INSTANCES) TMP
  ...
  MERGE INTO HAYA01.TEX_TAREA_EXTERNA TEX
  USING (SELECT * FROM HAYA01.TMP_ALTA_BPM_INSTANCES) TMP
...
insert into HAYAMASTER.jbpm_transition (id_, name_, processdefinition_, from_, to_, fromindex_) 
	...
	from HAYA01.tar_tareas_notificaciones tar 
	   join HAYA01.tex_tarea_externa tex on tar.tar_id = tex.tar_id 
	   join HAYA01.tap_tarea_procedimiento tap on tex.tap_id = tap.tap_id 
	   join HAYA01.prc_procedimientos prc on tar.prc_id = prc.prc_id 
