    ...
    IF V_NUM_TABLAS = 1 THEN 
     ...
     END IF;
		...
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.TUP_EPT_ESQUEMA_PLAZAS_TPO ADD (
 		...
 		EXECUTE IMMEDIATE V_MSQL;
 		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.TUP_EPT_ESQUEMA_PLAZAS_TPO... FKs creadas');
 		...
 		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.TUP_TPC_TURNADO_PROCU_CONFIG ADD (
				 ...
				 REFERENCES '||V_ESQUEMA||'.TUP_EPT_ESQUEMA_PLAZAS_TPO (EPT_ID),
  				CONSTRAINT TPC_USU_FK 
				 FOREIGN KEY (USU_ID) 
				 REFERENCES '||V_ESQUEMA_M||'.USU_USUARIOS (USU_ID))';
				 
 		EXECUTE IMMEDIATE V_MSQL; 		
 		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.TUP_TPC_TURNADO_PROCU_CONFIG... FKs creadas');
 		
 		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.TUP_ETP_ESQ_TURNADO_PROCU ADD (
				  CONSTRAINT ETP_EET_FK 
				 FOREIGN KEY (DD_EET_ID) 
				 REFERENCES '||V_ESQUEMA||'.DD_EET_ESTADO_ESQUEMA_TURNADO (DD_EET_ID))';
				 
 		EXECUTE IMMEDIATE V_MSQL;
 		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.TUP_ETP_ESQ_TURNADO_PROCU... FKs creadas');
