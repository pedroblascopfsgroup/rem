...
--## INCIDENCIA_LINK=PRODUCTO-744
--## PRODUCTO=SI
--## Finalidad: DDL Para incluir el filtrado por expediente en VOLUMENRIESGOSQL
--##           
...
SELECT CEX.EXP_ID EXP_ID, SUM (CASE
                             ...
                             ELSE 0
                          END) RIESGO
    FROM CEX_CONTRATOS_EXPEDIENTE CEX JOIN CNT_CONTRATOS CNT ON CEX.CNT_ID = CNT.CNT_ID
...
GROUP BY cex.exp_id
  ...
  FROM
    ...
    LEFT JOIN '||V_ESQUEMA||'.VTAR_TAR_VRE_VIA_PRC VRE_PRC ON TAR.TAR_ID = VRE_PRC.TAR_ID
 	JOIN tmp_riesgo on exp.exp_id = tmp_riesgo.exp_id';
 
 EXECUTE IMMEDIATE V_MSQL;

    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_NOMBRE_VISTA||' Creada o actualizada');     
