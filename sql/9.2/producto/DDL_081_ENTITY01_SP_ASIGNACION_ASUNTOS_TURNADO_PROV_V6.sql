...
AUTHID CURRENT_USER IS
  ...
  -- PRODUCTO-582 - 4 CURSORES PARA ELEGIR LA PROVINCIA DEL ASUNTO crs_filtro_situacion, nuevas restricciones para elegir la provincia.
   ...
   IS
    ...
    -- **Modificacion por BKREC-2260 
   CURSOR crs_filtro_prov_bien_menor (p_asu_id #ESQUEMA#.asu_asuntos.asu_id%TYPE)
   IS
	SELECT DISTINCT FIRST_VALUE (dir.dd_prv_id) OVER (ORDER BY mov.mov_dispuesto DESC, cpe.cpe_orden ASC)
		FROM #ESQUEMA#.asu_asuntos asu
			JOIN #ESQUEMA#.exp_expedientes exp ON asu.exp_id=exp.exp_id
			JOIN #ESQUEMA#.cex_contratos_expediente cex ON exp.exp_id=cex.EXP_ID
			JOIN #ESQUEMA#.cnt_contratos cnt ON cex.cnt_id=cnt.cnt_id
			...
			JOIN #ESQUEMA#.mov_movimientos mov ON mov.CNT_ID = cnt.cnt_id
		WHERE asu.asu_id = p_asu_id
	...
	CURSOR crs_filtro_prov_oficina (p_asu_id #ESQUEMA#.asu_asuntos.asu_id%TYPE)
   	IS
   		SELECT DISTINCT FIRST_VALUE (ofi.dd_prv_id) OVER (ORDER BY mov.mov_dispuesto DESC)
			FROM #ESQUEMA#.asu_asuntos asu
				JOIN #ESQUEMA#.exp_expedientes exp ON asu.exp_id=exp.exp_id
				JOIN #ESQUEMA#.cex_contratos_expediente cex ON exp.exp_id=cex.EXP_ID
				JOIN #ESQUEMA#.cnt_contratos cnt ON cex.cnt_id=cnt.cnt_id
  ...
  CURSOR crs_despacho_porc_real(p_tas_codigo #ESQUEMA_MASTER#.dd_tas_tipos_asunto.dd_tas_codigo%TYPE,list_despachos SYS.odcinumberlist, p_dd_prv_id	 #ESQUEMA_MASTER#.dd_prv_provincia.dd_prv_id%TYPE, p_calculo NUMBER)
   ...
   CURSOR crs_num_asu_prv(p_tas_codigo #ESQUEMA_MASTER#.dd_tas_tipos_asunto.dd_tas_codigo%TYPE, p_dd_prv_id	#ESQUEMA_MASTER#.dd_prv_provincia.dd_prv_id%TYPE, p_tge_codigo #ESQUEMA_MASTER#.dd_tge_tipo_gestor.dd_tge_codigo%TYPE)
      IS
        select tmp.des_id,tmp.porc_real, count(*) num_asu
            ...
            where gaa.fechacrear > add_months(sysdate, -1) and gaa.borrado=0 and asu.borrado=0 and eas.borrado=0
            and eas.dd_eas_codigo='03'
...
BEGIN
   ...
   ELSE
      ...
      ELSE
         ...
         -- Si algÃºn letrado ha sido asignado anteriormente a un asunto relacionado con alguno de los deudores del asunto actual, se asigna el asunto al mismo letrado
         ...
         IF v_dd_prv_id IS NOT NULL
       		THEN
            --DBMS_OUTPUT.put_line ('Provincia encontrada: '||v_dd_prv_id);
            -- Si se ha encontrado una provincia, veamos que despacho trabaja en esa provincia, y el de 
            -- mayor calidad serÃ¡ el despacho elegido para este asunto.
	            ...
	            THEN
                  ...
                  --DBMS_OUTPUT.put_line ('Suma % despachos: '||v_calculo);
                  ...
                  OPEN crs_num_asu_prv(v_datos_asunto.dd_tas_codigo, v_dd_prv_id, p_tge_codigo);
                  LOOP
                  ...
                  --DBMS_OUTPUT.put_line ('Suma Total ASU de los desp encontrdos: '||v_calculo);
                  ...
                  CLOSE crs_despaco_puntos_prv;
                   --DBMS_OUTPUT.put_line ('Des_id encontrado final: '||v_des_id);
	            ELSE
                	CLOSE crs_filtro_despacho_prov;
            	END IF;
        END IF;          
        ...
        IF v_des_id IS NULL	
        THEN
            -- Filto antiguo para la provincia: Se filtran los letrados por provincia, Comunidad AutÃ³noma o nacionales
            -- Se usarÃ¡ si en los filtros anteriores no encuentra provincia.
            ...
            ELSE
               ...
               ELSE
                  ...
                  -- Se filtran los letrados por calificaciÃ³n
                  -- Se obtienen las calificaciones de los letrados que aÃºn quedan
                  FOR calificacion IN crs_calificaciones_candidatos (p_asu_id)
                  LOOP
                     -- Se obtiene el porcentaje de asuntos asignados a despachos con esta calificaciÃ³n durante el mes actual
                     ...
                     -- Se obtiene el porcentaje para esa calificaciÃ³n segÃºn el esquema actual
                     ...
                     -- Si el porcentaje permitido por el esquema para la calificaciÃ³n es mayor que el porcentaje de asuntos asignados a la calificaciÃ³n actualmente
                     -- nos quedamos con los letrados que tengan esta calificaciÃ³n y finalizamos el bucle
                     ...
                     THEN
                        ...
                        v_enc := 'S';
                        EXIT;
                     -- En otro caso, nos quedamos con la calificaciÃ³n si es la primera y continuamos el bucle
                     ELSE
                  ...
                  -- Si no se ha encontrado ninguna calificaciÃ³n cuyo porcentaje de asuntos sea menor que el definido en el esquema nos quedamos con los letrados del primer tipo de calificaciÃ³n
                  IF v_enc = 'N'
                  THEN
                     DELETE FROM #ESQUEMA#.tmp_aat_asig_turnado_letrados aat
...
END IF;
   ...
   THEN
	...
	-- Actualizamos ASU_ASUNTOS, para aÃ±adir la provincia que tiene asignada
      IF v_dd_prv_id IS NOT NULL     
      THEN
      ...
      -- Si existe algÃºn gestor activo del tipo indicado para el asunto, se aÃ±ade al histÃ³rico y se borra la relaciÃ³n
      ...
      -- Se aÃ±ade la relaciÃ³n entre el despacho elegido y el asunto
