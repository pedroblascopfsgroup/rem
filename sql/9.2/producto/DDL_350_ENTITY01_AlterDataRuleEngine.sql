...
DECLARE
  ...
  V_SQL := 'select count(1) from all_tab_columns ' ||
  	'where COLUMN_NAME=''PER_RIESGO_TOTAL_FALLIDO'' and table_name=''DATA_RULE_ENGINE'' and owner=''' || V_ESQUEMA || '''';
  ...
  ELSE
  	V_SQL := 'ALTER TABLE ' || V_ESQUEMA || '.DATA_RULE_ENGINE ADD PER_RIESGO_TOTAL_FALLIDO NUMBER(15,2)'; 
	  EXECUTE IMMEDIATE V_SQL;	
  END IF;
  
  DBMS_OUTPUT.PUT_LINE('[INFO] AÑADIDO EL CAMPO PER_RIESGO_TOTAL_FALLIDO');
