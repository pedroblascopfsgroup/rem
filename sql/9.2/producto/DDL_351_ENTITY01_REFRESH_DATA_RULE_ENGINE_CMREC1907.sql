...
create or replace PROCEDURE      REFRESH_DATA_RULE_ENGINE AS
 ...
 SELECT /*+ PARALLEL */
    ...
    FROM  per_personas per
      ...
      LEFT JOIN  (select iac.cnt_id, iac.iac_value from ext_iac_info_add_contrato iac where iac.dd_ifc_id = (select ifc.dd_ifc_id from ext_dd_ifc_info_contrato ifc where ifc.dd_ifc_codigo = 'char_extra10')) CNT_CHAR_EXTRA10 ON CNT_CHAR_EXTRA10.CNT_ID=CNT.CNT_ID
      LEFT JOIN  DD_EIC_ESTADO_INTERNO_ENTIDAD EIC ON CNT_CHAR_EXTRA10.IAC_VALUE = EIC.DD_EIC_CODIGO
      ...
      LEFT JOIN  tmp_per_personas tmp_per ON tmp_per.tmp_per_cod_persona = per.per_cod_cliente_entidad
