...
--##########################################
--## AUTOR=MANUEL MEJIAS
--## FECHA_CREACION=20163030
--## ARTEFACTO=web
--## VERSION_ARTEFACTO=9.1.0-cj-rc36.1-CMREC-1907s
--## INCIDENCIA_LINK=CMREC-2907
--## PRODUCTO=SI
--## Finalidad: DDL
--##           
--## INSTRUCCIONES: Añadir una nueva variable de arquetipacion
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
  ...
  V_SQL := 'create or replace PROCEDURE ' || V_ESQUEMA || '.REFRESH_DATA_RULE_ENGINE AS
 ...
 SELECT /*+ PARALLEL */
    ...
    FROM  ' || V_ESQUEMA || '.per_personas per
      ...
      LEFT JOIN (select cac.cnt_id, cac.cac_actuacion_en_curso
                   from ' || V_ESQUEMA || '.cac_contratos_actuacion_curso cac 
                   join ' || V_ESQUEMA_M || '.DD_SIN_SINO sin ON cac.cac_actuacion_en_curso = sin.DD_SIN_ID) cac ON cac.CNT_ID=cnt.CNT_ID
