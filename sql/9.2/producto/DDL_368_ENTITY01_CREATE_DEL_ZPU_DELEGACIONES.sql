...
BEGIN
	...
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] Ya existía la tabla DEL_ZPU_DELEGACIONES');
	END IF;

	--Secuencia para la tabla DEL_ZPU_DELEGACIONES
	V_SQL := 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_OWNER='''||V_ESQUEMA||''' AND SEQUENCE_NAME=''S_DEL_ZPU_DELEGACIONES''';
	EXECUTE IMMEDIATE V_SQL INTO VN_COUNT;
	IF VN_COUNT = 0 THEN
	...
	-- Modificación de la tabla ZON_PEF_USU
 	V_SQL := 'SELECT COUNT(1) FROM USER_TAB_COLS WHERE TABLE_NAME=''ZON_PEF_USU'' AND COLUMN_NAME=''DEL_ZPU_ID''';
	EXECUTE IMMEDIATE V_SQL INTO VN_COUNT;
	IF VN_COUNT = 0 THEN
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.ZON_PEF_USU 
					ADD (DEL_ZPU_ID NUMBER(16, 0) )';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Columna DEL_ZPU_ID agreada a ZON_PEF_USU');

		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.ZON_PEF_USU
					ADD CONSTRAINT DEL_DELEGACION_FK FOREIGN KEY (DEL_ZPU_ID)
					REFERENCES '||V_ESQUEMA||'.DEL_ZPU_DELEGACIONES (DEL_ZPU_ID) ENABLE';
		EXECUTE IMMEDIATE V_MSQL;
