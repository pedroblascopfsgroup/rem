...
BEGIN	
    ...
    V_SQL := 'SELECT COUNT (1) FROM ALL_IND_COLUMNS WHERE TABLE_NAME = UPPER(''CPA_COBROS_PAGOS'') AND COLUMN_NAME = UPPER(''CNT_ID'')';
	...
	ELSE
		...
		V_SQL := 'SELECT COUNT(*) FROM USER_INDEXES WHERE INDEX_NAME = ''IDX_CNT_ID''';
		 DBMS_OUTPUT.PUT_LINE(V_SQL);
		EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
		...
		ELSE
			V_MSQL := 'CREATE INDEX IDX_CNT_ID ON '||V_ESQUEMA||'.CPA_COBROS_PAGOS(CNT_ID)';
