...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''SPR_OBSERVACIONES_RESPUESTA'' and TABLE_NAME=''SPR_SOLICITUD_PRORROGA'' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO table_count;
	
	IF table_count = 0 THEN
			EXECUTE IMMEDIATE 'ALTER TABLE ' || V_ESQUEMA || '.SPR_SOLICITUD_PRORROGA ADD SPR_OBSERVACIONES_RESPUESTA VARCHAR2(4000 CHAR)';
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.SPR_SOLICITUD_PRORROGA.SPR_OBSERVACIONES_RESPUESTA... Ya existe');
	END IF;
