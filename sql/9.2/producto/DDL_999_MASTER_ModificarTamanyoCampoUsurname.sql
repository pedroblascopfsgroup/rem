	 ...
	 FOR I IN V_TIPO_COLUMN.FIRST .. V_TIPO_COLUMN.LAST
      LOOP
        ...
        OPEN TABLES_CURSOR FOR 'SELECT t.TABLE_NAME ' ||
							...
							' AND EXISTS (SELECT a.OBJECT_NAME FROM ALL_OBJECTS a WHERE a.OWNER = '''||V_ESQUEMA_M||''' AND a.OBJECT_NAME = t.TABLE_NAME AND a.OBJECT_TYPE = ''TABLE'')';
 	
 		LOOP
 			FETCH TABLES_CURSOR INTO TABLE_NAME;
 			EXIT WHEN TABLES_CURSOR%NOTFOUND;
 			
        	V_MSQL := 'ALTER TABLE '||V_ESQUEMA_M||'.'|| TABLE_NAME ||' MODIFY '||V_TMP_TIPO_COLUMN(1)||' VARCHAR2(50 CHAR)';
