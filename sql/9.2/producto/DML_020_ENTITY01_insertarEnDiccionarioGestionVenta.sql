...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_GVE_GESTION_VENTA';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_GVE_GESTION_VENTA';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_GVE_';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('PROM', 'Promotor', 'Promotor')
     ,T_TIPO('TERC', 'Tercero', 'Tercero')
     ,T_TIPO('HRE', 'HRE', 'HRE')
    ); 
    V_TMP_TIPO T_TIPO;
