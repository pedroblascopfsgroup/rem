...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_RRE_RESULTADO_REVISION';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_RRE_RESULTADO_REVISION';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_RRE_';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('CANCE', 'Cancelar PDV', 'Cancelar PDV')
     ,T_TIPO('RENOV', 'Renovación', 'Renovación')
     ,T_TIPO('RECON', 'Reconsideración', 'Reconsideración')
    ); 
    V_TMP_TIPO T_TIPO;
