...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_CUA_CUANTIA';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_CUA_CUANTIA';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_CUA_';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('CCV', '50% Ves', '50% Ves')
     ,T_TIPO('SSV', '60% Ves', '60% Ves')
     ,T_TIPO('STV', '70% Ves', '70% Ves')
     ,T_TIPO('CFC', 'Cantidad fija total crédito', 'Cantidad fija total crédito')
    ); 
    V_TMP_TIPO T_TIPO;
