...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_ECR_ENTIDAD_CES_REMATE';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_ECR_ENTIDAD_CES_REMATE';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_ECR_';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('CIM', 'Cimentado2', 'Cimentado2')
     ,T_TIPO('OVE', 'Otras vehiculares', 'Otras vehiculares')
     ,T_TIPO('TER', 'Tercero', 'Tercero')
    ); 
    V_TMP_TIPO T_TIPO;
