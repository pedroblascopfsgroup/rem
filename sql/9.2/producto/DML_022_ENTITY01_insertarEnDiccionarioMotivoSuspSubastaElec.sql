...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_MSE_MOTIVO_SUSP_ELEC';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_MSE_MOTIVO_SUSP_ELEC';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_MSE_';

   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ' USING (SELECT :id id, :codigo codigo, :des descripcion, :des_l descripcion_larga, ''PRODUCTO-1093'' usuariocrear, sysdate fechacrear from DUAL) actual ' ||
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('NEX', 'Negociación extrajudicial (entidad)', 'Negociación extrajudicial (entidad)')
     ,T_TIPO('DJU', 'Deuda judicial inferior al 60% (entidad)', 'Deuda judicial inferior al 60% del valor de tasación a efectos de subasta tipo (entidad)')
     ,T_TIPO('CPR', 'Cargas previas (entidad)', 'Cargas previas con contenido económico (entidad)')
     ,T_TIPO('DCO', 'Declaración del concurso (terceros)', 'Declaración del concurso(terceros)')
     ,T_TIPO('OTR', 'Otros', 'Otros')
    ); 
    V_TMP_TIPO T_TIPO;
