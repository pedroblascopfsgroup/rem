...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_PRI_PRIORIDAD';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_PRI_PRIORIDAD';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_PRI_';
   ...
   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('ALT', 'Alta', 'Alta')
     ,T_TIPO('MED', 'Media', 'Media')
     ,T_TIPO('BAJ', 'Baja', 'Baja')
    ); 
    V_TMP_TIPO T_TIPO;
