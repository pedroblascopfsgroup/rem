...
--## PRODUCTO=SI
--## Finalidad: INSERT diccionario DD_DCC_DESPACHO_CLASI_CONCURSO
--##           
...
DECLARE
   ...
   V_DDNAME VARCHAR2(50 CHAR):= 'DD_DRE_DESPACHO_REL_ENTIDAD';
   V_SEQNAME VARCHAR2(50 CHAR):= 'S_DD_DRE_DES_REL_ENTIDAD';
   V_PREFIJO VARCHAR2(50 CHAR) := 'DD_DRE_';

   V_MERGE  VARCHAR2(4000 CHAR) := 'MERGE INTO ' || V_ESQUEMA || '.' || V_DDNAME|| ' tabla  ' || 
    ' USING (SELECT :id id, :codigo codigo, :des descripcion, :des_l descripcion_larga, ''PRODUCTO-1963'' usuariocrear, sysdate fechacrear from DUAL) actual ' ||
    ...
    V_TIPO T_ARRAY := T_ARRAY(
      T_TIPO('En desvinculación', 'En desvinculación', 'En desvinculación')
     ,T_TIPO('Sin turno', 'Sin turno', 'Sin turno')
     ,T_TIPO('Sin acceso', 'Sin acceso', 'Sin acceso')
     ,T_TIPO('Turno Activo', 'Turno Activo', 'Turno Activo')
     ,T_TIPO('Historico', 'Historico', 'Historico')
    ); 
 V_TMP_TIPO T_TIPO;
