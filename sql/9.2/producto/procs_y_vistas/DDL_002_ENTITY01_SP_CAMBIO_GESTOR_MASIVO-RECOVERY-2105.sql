...
create or replace PROCEDURE haya01.CAMBIO_MASIVOS_GESTOR  (RESULT_EXE OUT VARCHAR2 ) AUTHID CURRENT_USER IS
	...
	V_SQL_GAA VARCHAR2(4000 CHAR)  := 'UPDATE HAYA01.GAA_GESTOR_ADICIONAL_ASUNTO SET USD_ID=:1, USUARIOMODIFICAR=:2, FECHAMODIFICAR=SYSDATE WHERE GAA_ID=:3';
	V_SQL_GAH1 VARCHAR2(4000 CHAR) := 'UPDATE HAYA01.GAH_GESTOR_ADICIONAL_HISTORICO SET GAH_FECHA_HASTA=SYSDATE, USUARIOMODIFICAR=:1, FECHAMODIFICAR=SYSDATE  ' ||
		'WHERE GAH_ASU_ID=:2 AND GAH_TIPO_GESTOR_ID=:3 AND GAH_GESTOR_ID=:4 AND GAH_FECHA_HASTA IS NULL';
	V_SQL_GAH2 VARCHAR2(4000 CHAR) := 'INSERT INTO  HAYA01.GAH_GESTOR_ADICIONAL_HISTORICO ' ||
		'(GAH_ID, GAH_GESTOR_ID, GAH_ASU_ID, GAH_TIPO_GESTOR_ID, GAH_FECHA_DESDE, USUARIOCREAR, FECHACREAR) VALUES ' ||
		'(HAYA01.S_GAH_GESTOR_ADIC_HISTORICO.NEXTVAL, :1, :2, :3, SYSDATE, :4, SYSDATE)';
	V_SQL_CMA VARCHAR2(4000 CHAR)  := 'UPDATE HAYA01.CMA_CAMBIOS_MASIVOS_ASUNTOS SET REASIGNADO=:1, USUARIOMODIFICAR=:2, FECHAMODIFICAR=SYSDATE WHERE CMA_ID=:3';

	V_SQL_TRAZA1 VARCHAR2(4000 CHAR)  := 'SELECT HAYA01.S_MEJ_REG_REGISTRO.NEXTVAL FROM DUAL';
	V_SQL_TRAZA2 VARCHAR2(4000 CHAR)  := 'SELECT DD_TRG_ID FROM HAYA01.MEJ_DD_TRG_TIPO_REGISTRO WHERE DD_TRG_CODIGO=''CAMBIO_MASIVO''';
	V_SQL_TRAZA3 VARCHAR2(4000 CHAR)  := 'INSERT INTO HAYA01.MEJ_REG_REGISTRO (CMA_ID, REG_ID, DD_TRG_ID, TRG_EIN_CODIGO, TRG_EIN_ID, USU_ID, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
		' VALUES (:1, :2, :3, 3, :4, :5, 0, :6, SYSDATE, 0)';
	V_SQL_TRAZA4 VARCHAR2(4000 CHAR)  := 'SELECT USU_ID FROM HAYA01.USD_USUARIOS_DESPACHOS WHERE USD_ID=:1';
	V_SQL_TRAZA5 VARCHAR2(4000 CHAR)  := 'SELECT TRIM(USU_NOMBRE||'' ''||USU_APELLIDO1||'' ''||USU_APELLIDO2) FROM HAYAMASTER.USU_USUARIOS WHERE USU_ID=:1';
	V_SQL_TRAZA6 VARCHAR2(4000 CHAR)  := 'INSERT INTO HAYA01.MEJ_IRG_INFO_REGISTRO (IRG_ID, REG_ID, IRG_CLAVE, IRG_VALOR, VERSION, USUARIOCREAR,FECHACREAR, BORRADO) ' ||
		' VALUES (HAYA01.S_MEJ_IRG_INFO_REGISTRO.NEXTVAL, :1, :2, :3, 0, :4, sysdate, 0)';
	...
	CURSOR C_SUSTITUCIONES IS
		SELECT CMA.CMA_ID CMA_ID, GAA.GAA_ID GAA_ID, GAA.ASU_ID ASU_ID, GAA.DD_TGE_ID DD_TGE_ID, GAA.USD_ID VIEJO ,
			...
			CMA.SOL_ID SOL_ID
		FROM HAYA01.GAA_GESTOR_ADICIONAL_ASUNTO GAA
		JOIN HAYA01.CMA_CAMBIOS_MASIVOS_ASUNTOS CMA
