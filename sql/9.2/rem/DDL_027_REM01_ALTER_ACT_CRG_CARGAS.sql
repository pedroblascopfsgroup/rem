...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('DD_ECG_ID','NUMBER(16, 0)','','Estado de la carga','FK_DD_ECG_ACT_CRG')
    ); 
...
BEGIN
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP      
		...
		ELSE
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD ('||V_TMP_TIPO_DATA(1)||' '||V_TMP_TIPO_DATA(2)||V_TMP_TIPO_DATA(3)||')';
            DBMS_OUTPUT.PUT_LINE(V_MSQL);
			EXECUTE IMMEDIATE V_MSQL;
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT '||V_TMP_TIPO_DATA(5)||' FOREIGN KEY ('||V_TMP_TIPO_DATA(1)||') REFERENCES '||V_ESQUEMA||'.DD_ECG_ESTADO_CARGA('||V_TMP_TIPO_DATA(1)||')';
            DBMS_OUTPUT.PUT_LINE(V_MSQL);
            EXECUTE IMMEDIATE V_MSQL;
