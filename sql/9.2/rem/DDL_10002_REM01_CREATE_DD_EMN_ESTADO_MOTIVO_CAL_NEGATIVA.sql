...
--## PRODUCTO=NO
--##
--## Finalidad: Crear el diccionario para la calificacion negativa.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
 ...
 V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
                DD_EMN_ID                 NUMBER(16,0) NOT NULL
 			 , DD_EMN_CODIGO            VARCHAR2(20 CHAR) NOT NULL
             , DD_EMN_DESCRIPCION       VARCHAR2(100 CHAR)
             , DD_EMN_DESCRIPCION_LARGA VARCHAR2(100 CHAR)
			 ...
			 , CONSTRAINT DD_EMN_ID PRIMARY KEY (DD_EMN_ID)
			)
		  ';
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] Creacion de los comentarios en la tabla '||V_TABLA);
    ...
    V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EMN_ID IS ''Identificador único del estado de la calificación negativa''';    EXECUTE IMMEDIATE V_SQL;
    V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EMN_CODIGO IS ''Identificador único del estado de la calificación negativa''';    EXECUTE IMMEDIATE V_SQL;
    V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EMN_DESCRIPCION IS ''Descripcion de la calificación negativa''';    EXECUTE IMMEDIATE V_SQL;
    V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EMN_DESCRIPCION_LARGA IS ''Descripcion larga de la calificación negativa''';    EXECUTE IMMEDIATE V_SQL;
    ...
    V_SQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla diccionario del estado del motivo de la calificacion negativa''';    EXECUTE IMMEDIATE V_SQL;
