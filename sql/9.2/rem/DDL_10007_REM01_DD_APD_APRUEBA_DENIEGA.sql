...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_APD_APRUEBA_DENIEGA.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(27 CHAR) := 'DD_APD_APRUEBA_DENIEGA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_CDG VARCHAR2(27 CHAR) := 'APD'; -- Vble. auxiliar para almacenar el codigo de la tabla para las columnas.
 BEGIN
	...
	IF V_COUNT = 0 THEN
		...
		V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
				...
				, DD_'||V_CDG||'_CODIGO VARCHAR(50 CHAR) NOT NULL
				, DD_'||V_CDG||'_DESCRIPCION VARCHAR2(50 CHAR) NOT NULL
				, DD_'||V_CDG||'_DESCRIPCION_LARGA VARCHAR2(250 CHAR) NOT NULL
				...
				, CONSTRAINT DD_APD_ID_PK PRIMARY KEY (DD_APD_ID)
				)
			  ';
