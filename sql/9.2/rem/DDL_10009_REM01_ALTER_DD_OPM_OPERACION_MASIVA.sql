...
DECLARE
    ...
    ERR_MSG VARCHAR2(1024 CHAR); -- Vble. auxiliar para registrar errores en el script.
	  V_USUARIO VARCHAR2(32 CHAR):= 'HREOS-8178';
    ...
    V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
	  T_FUNCION('MASCT', 'Control de tributos', 'Control de tributos','MASIVO_CONTROL_TRIBUTOS','n*,f*,f*,f*,s*,s,f*,f*,f*,s*,n,s*,n')
    ); 
     ...
     FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      ...
      DBMS_OUTPUT.PUT_LINE(V_SQL);
			...
			IF V_NUM_TABLAS > 0 THEN	  
				DBMS_OUTPUT.PUT_LINE('[INFO] Ya existen los datos en la tabla '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA...se modifica.');
				
        V_MSQL_1 := 'UPDATE '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA' ||
        ' SET DD_OPM_VALIDACION_FORMATO = '''||V_TMP_FUNCION(5)||'''' ||
        ' WHERE DD_OPM_CODIGO = '''||V_TMP_FUNCION(1)||'''';
        DBMS_OUTPUT.PUT_LINE(V_MSQL_1);
        EXECUTE IMMEDIATE V_MSQL_1;
        DBMS_OUTPUT.PUT_LINE('[INFO] Datos de la tabla '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA mnodificados correctamente.');
