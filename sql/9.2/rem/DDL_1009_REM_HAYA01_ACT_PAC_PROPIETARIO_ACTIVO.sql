...
BEGIN
	...
	(
		PAC_ID           			NUMBER(16)                  NOT NULL,
		ACT_ID						NUMBER(16)                  NOT NULL,
		PRO_ID						NUMBER(16)                  NOT NULL,
		DD_TGP_ID					NUMBER(16),
		PAC_PORC_PROPIEDAD			NUMBER(5,2),			
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_PROPACT_ACTIVO FOREIGN KEY (ACT_ID) REFERENCES '||V_ESQUEMA||'.ACT_ACTIVO (ACT_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_PROPACT_ACTIVO... Foreign key creada.');
	
	-- Creamos foreign key PRO_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_PROPACT_PROPIETARIO FOREIGN KEY (PRO_ID) REFERENCES '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO (PRO_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_PROPACT_PROPIETARIO... Foreign key creada.');
		
	-- Creamos foreign key DD_TGP_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_PROP_DDTGRADOPROP FOREIGN KEY (DD_TGP_ID) REFERENCES '||V_ESQUEMA||'.DD_TGP_TIPO_GRADO_PROPIEDAD (DD_TGP_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_PROP_DDTGRADOPROP... Foreign key creada.');
