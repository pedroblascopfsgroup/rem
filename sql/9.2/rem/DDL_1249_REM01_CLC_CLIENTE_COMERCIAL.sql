...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_CLIENTE_LOCALIDAD... Foreign key creada.');
		...
		-- Creamos foreign key DD_UPO_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_CLIENTE_PEDANIA FOREIGN KEY (DD_UPO_ID) REFERENCES '||V_ESQUEMA_M||'.DD_UPO_UNID_POBLACIONAL (DD_UPO_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_CLIENTE_PEDANIA... Foreign key creada.');
