...
BEGIN
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_OFR_CLC FOREIGN KEY (CLC_ID) REFERENCES '||V_ESQUEMA||'.CLC_CLIENTE_COMERCIAL (CLC_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_OFR_CLC... Foreign key creada.');
	
			-- Creamos foreign key VIS_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_OFR_VIS FOREIGN KEY (VIS_ID) REFERENCES '||V_ESQUEMA||'.VIS_VISITAS (VIS_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_OFR_VIS... Foreign key creada.');
	
			-- Creamos foreign key DD_EVO_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_OFR_DD_EVO FOREIGN KEY (DD_EVO_ID) REFERENCES '||V_ESQUEMA||'.DD_EVO_EST_VISITA_OFERTA (DD_EVO_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_OFR_DD_EVO... Foreign key creada.');
	
				-- Creamos foreign key USU_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_OFR_USU FOREIGN KEY (USU_ID) REFERENCES '||V_ESQUEMA_M||'.USU_USUARIOS (USU_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_OFR_USU... Foreign key creada.');

		-- Creamos foreign key PVE_ID_PRESCRIPTOR
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_OFR_PVE_PRESCRIPTOR FOREIGN KEY (PVE_ID_PRESCRIPTOR) REFERENCES '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR (PVE_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_OFR_PVE_PRESCRIPTOR... Foreign key creada.');
	
		
		-- Creamos foreign key PVE_ID_API_RESPONSABLE
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_OFR_PVE_API_RESPONSABLE FOREIGN KEY (PVE_ID_API_RESPONSABLE) REFERENCES '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR (PVE_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_OFR_PVE_API_RESPONSABLE... Foreign key creada.');
