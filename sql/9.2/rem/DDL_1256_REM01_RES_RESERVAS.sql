...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_RES_ECO_ID... Foreign key creada.');
			...
			-- Creamos foreign key DD_TAR_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_RES_DD_TAR FOREIGN KEY (DD_TAR_ID) REFERENCES '||V_ESQUEMA||'.DD_TAR_TIPOS_ARRAS (DD_TAR_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_RES_DD_TAR... Foreign key creada.');
	
				-- Creamos foreign key DD_ERE_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_RES_DD_ERE FOREIGN KEY (DD_ERE_ID) REFERENCES '||V_ESQUEMA||'.DD_ERE_ESTADOS_RESERVA (DD_ERE_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_RES_DD_ERE... Foreign key creada.');
