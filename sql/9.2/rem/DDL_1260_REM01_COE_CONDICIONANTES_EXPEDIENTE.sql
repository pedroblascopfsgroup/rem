...
BEGIN
	...
	V_SQL := 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_NAME = ''S_COE_CONDICIONANTES_EXP'' and SEQUENCE_OWNER = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS; 
	IF V_NUM_TABLAS = 1 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '|| V_ESQUEMA ||'.S_COE_CONDICIONANTES_EXP... Ya existe. Se borrará.');  
		EXECUTE IMMEDIATE 'DROP SEQUENCE '||V_ESQUEMA||'.S_COE_CONDICIONANTES_EXP';
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.COE_EXP_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(COE_ID) TABLESPACE '||V_TABLESPACE_IDX;		
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.COE_EXP_PK... Indice creado.');	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT COE_EXP_PK PRIMARY KEY (COE_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.COE_EXP_PK ... PK creada.');	
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_COE_ECO_ID... Foreign key creada.');
		...
		-- Creamos foreign key DD_ESF_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_COE_ESF_ID FOREIGN KEY (DD_ESF_ID) REFERENCES '||V_ESQUEMA||'.DD_ESF_ESTADOS_FINANCIACION (DD_ESF_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_COE_ESF_ID... Foreign key creada.');
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_COE_TPC_DESCALIFICA FOREIGN KEY (DD_TPC_ID_DESCALIFICACION) REFERENCES '||V_ESQUEMA||'.DD_TPC_TIPOS_PORCUENTA (DD_TPC_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_COE_TPC_DESCALIFICA... Foreign key creada.');
	
	
	

	-- Creamos sequence
	V_MSQL := 'CREATE SEQUENCE '||V_ESQUEMA||'.S_COE_CONDICIONANTES_EXP';		
	EXECUTE IMMEDIATE V_MSQL;		
