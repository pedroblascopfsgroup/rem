...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_CEX_UAC_ID... Foreign key creada.');
			...
			-- Creamos foreign key DD_TDI_ID_RTE
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_CEX_TDI_ID FOREIGN KEY (DD_TDI_ID_RTE) REFERENCES '||V_ESQUEMA||'.DD_TDI_TIPO_DOCUMENTO_ID (DD_TDI_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
