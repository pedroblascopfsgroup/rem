...
--## FECHA_CREACION=20160825
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.1
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla que contiene los datos de los trabajos
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
		...
		T_ALTER(  'TBJ_WEBCOM_ID',		 			'NUMBER(16,0)',					'Código identificador único del trabajo en WEBCOM.'	)
		);
    V_T_ALTER T_ALTER;
...
BEGIN
	...
	-- Ampliamos campo descripción
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''TBJ_DESCRIPCION'' and DATA_TYPE = ''VARCHAR2'' and CHAR_LENGTH = ''256'' and TABLE_NAME='''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 1 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.TBJ_DESCRIPCION... existe con precisión 256. La cambiamos 1024.');
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' MODIFY (TBJ_DESCRIPCION VARCHAR2(1024 CHAR))';	
	ELSE
		
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.TBJ_DESCRIPCION... modificado con precisión 1024.');
	END IF;
