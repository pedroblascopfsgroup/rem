...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('CLC_NUM_CLIENTE_HAYA', 'NUMBER','(16,0)', 'Código identificador único del Cliente Comercial en HAYA.'),
        T_TIPO_DATA('CLC_NUM_CLIENTE_UVEM', 'NUMBER','(16,0)', 'Código identificador único del Cliente Comercial en UVEM')
    ); 
...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''CLC_FECHA_ACCION'' and DATA_TYPE = ''DATE'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CLC_FECHA_ACCION... No existe');
	ELSE
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' RENAME COLUMN CLC_FECHA_ACCION TO CLC_FECHA_ALTA';	
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CLC_FECHA_ACCION modificada a CLC_FECHA_ALTA');
	END IF;
