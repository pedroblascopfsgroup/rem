...
BEGIN
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_PAC_MNC FOREIGN KEY (DD_MNC_ID) REFERENCES '||V_ESQUEMA||'.DD_MNC_MOT_NOCOMERCIALIZACION (DD_MNC_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_PAC_MNC... Foreign key creada.');
	
	-- Creamos foreign key PAC_MOTIVO_COMERCIALIZAR
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_PAC_MCO FOREIGN KEY (DD_MCO_ID) REFERENCES '||V_ESQUEMA||'.DD_MCO_MOTIVO_COMERCIALIZACION (DD_MCO_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_PAC_MNC... Foreign key creada.');
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MNC_ID IS ''Motivo de exclusión de comercializar.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MCO_ID IS ''Motivo de inclusión de comercializar.''';
