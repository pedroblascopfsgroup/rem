...
BEGIN
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_ASISTIDA_DDESTADO_OBRAN FOREIGN KEY (DD_EON_ID) REFERENCES '||V_ESQUEMA||'.DD_EON_ESTADO_OBRA_NUEVA (DD_EON_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_ASISTIDA_DDESTADO_OBRAN... Foreign key creada.');
