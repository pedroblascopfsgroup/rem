...
BEGIN
	...
	(
		...
		GDE_IRPF_TIPO_IMPOSITIVO		NUMBER(4,2),
		GDE_IRPF_CUOTA					NUMBER(16,2),
		GDE_IMPORTE_TOTAL				NUMBER(16,2),
		GDE_FECHA_TOPE_PAGO				DATE,
		GDE_REPERCUTIBLE_INQUILINO		NUMBER(1),
		GDE_IMPORTE_PAGADO				NUMBER(16,2),
		GDE_FECHA_PAGO					DATE,
		DD_TPA_ID						NUMBER(16,2),
		DD_TPP_ID						NUMBER(16,2),
		DD_DEP_ID						NUMBER(16,2),		
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.GDE_GASTOS_DETALLE_ECO_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(GDE_ID) TABLESPACE '||V_TABLESPACE_IDX;			
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');
	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT GDE_GASTOS_DETALLE_ECO_PK PRIMARY KEY (GDE_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GDE_DD_TPA FOREIGN KEY (DD_TPA_ID) REFERENCES '||V_ESQUEMA||'.DD_TPA_TIPOS_PAGADOR (DD_TPA_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GDE_DD_TPA... Foreign key creada.');
	
	-- Creamos foreign key DD_TPP_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GDE_DD_TPP FOREIGN KEY (DD_TPP_ID) REFERENCES '||V_ESQUEMA||'.DD_TPP_TIPOS_PAGO (DD_TPP_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GDE_DD_TPP... Foreign key creada.');
	
	-- Creamos foreign key DD_DEP_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GDE_DD_DEP FOREIGN KEY (DD_DEP_ID) REFERENCES '||V_ESQUEMA||'.DD_DEP_DESTINATARIOS_PAGO (DD_DEP_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GDE_DD_DEP... Foreign key creada.');
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.GDE_FECHA_TOPE_PAGO IS ''Fecha tope de pago''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.GDE_REPERCUTIBLE_INQUILINO IS ''Indicador de si es repercutible al inquilino''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.GDE_IMPORTE_PAGADO IS ''Importe pagado''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.GDE_FECHA_PAGO IS ''Fecha de pago''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TPA_ID IS ''Pagado por''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TPP_ID IS ''Forma de pago''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_DEP_ID IS ''Responsable de pago fuera de plazo''';
