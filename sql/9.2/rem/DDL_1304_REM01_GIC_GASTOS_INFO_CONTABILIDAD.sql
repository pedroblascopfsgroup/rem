...
BEGIN
	...
	(
		...
		DD_PRE_ID_ESPECIAL				NUMBER(16,2),
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(GIC_ID) TABLESPACE '||V_TABLESPACE_IDX;			
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');
	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (GIC_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	...
	-- Creamos foreign key DD_PPR_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GIP_DD_PRE FOREIGN KEY (DD_PPR_ID) REFERENCES '||V_ESQUEMA||'.DD_PPR_PARTIDAS_PRESUPUESTARIAS (DD_PPR_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GIP_DD_PPR... Foreign key creada.');
	
	-- Creamos foreign key DD_CCO_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GIP_DD_CCO FOREIGN KEY (DD_CCO_ID) REFERENCES '||V_ESQUEMA||'.DD_CCO_CUENTAS_CONTABLES (DD_CCO_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GIP_DD_CCO... Foreign key creada.');
	
		-- Creamos foreign key DD_PPR_ID_ESPECIAL
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GIP_DD_PPR_ESPECIAL FOREIGN KEY (DD_PPR_ID_ESPECIAL) REFERENCES '||V_ESQUEMA||'.DD_PPR_PARTIDAS_PRESUPUESTARIAS (DD_PPR_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GIP_DD_PPR... Foreign key creada.');
	
	-- Creamos foreign key DD_CCO_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_GIP_DD_CCO_ESPECIAL FOREIGN KEY (DD_CCO_ID_ESPECIAL) REFERENCES '||V_ESQUEMA||'.DD_CCO_CUENTAS_CONTABLES (DD_CCO_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_GIP_DD_CCO_ESPECIAL... Foreign key creada.');
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.GIP_ID IS ''Código identificador único de la información del pago''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.EJE_ID IS ''Código identificador del ejercicio''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_PRE_ID IS ''Código identificador de la partida presupuestaria''';
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_PRE_ID_ESPECIAL IS ''Código identificador de partida presupuestaria especial''';
