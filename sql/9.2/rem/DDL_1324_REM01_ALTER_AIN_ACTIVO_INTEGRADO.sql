...
--## INCIDENCIA_LINK=0
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla que contiene la información de un activo integrado
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'AIN_OBSERVACIONES',		 		'VARCHAR2(1024)',					'Observaciones.'),
    	T_ALTER(  'AIN_PAGO_RETENIDO',		 		'NUMBER(1)',						'Código único del comite sancionador propuesto.'),
    	T_ALTER(  'DD_MRE_ID',						'NUMBER(16)',						'Diccionario de motivos de retención de pago '),
    	T_ALTER(  'AIN_FECHA_RETENCION_PAGO',		'DATE',								'Fecha retención de pago')
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_AIN_DD_MRE',						'DD_MRE_ID',				V_ESQUEMA||'.DD_MRE_MOTIVO_RETENCION',				'DD_MRE_ID'			)
    );
    V_T_FK T_FK;
