      ...
      DBMS_OUTPUT.PUT_LINE(''[INFO] Procesando agrupaciones:'');
	    ...
	    WHILE (AGRUPACION%FOUND) LOOP
         DBMS_OUTPUT.PUT_LINE(''[INFO] actualizar agrupación con ID: ''||FILA_AGR.AGR_ID||'' fecha baja a hoy'');
          ...
          IF (NOT ACTIVOS_AGR%ISOPEN) THEN
	        OPEN ACTIVOS_AGR;
		  END IF;
          
          FETCH ACTIVOS_AGR INTO FILA_ACT;
          
          WHILE (ACTIVOS_AGR%FOUND) LOOP
          
			  UPDATE ACT_ACTIVO ACT
			  SET
			  ...
			  WHERE ACT.ACT_ID = FILA_ACT.ACT_ID;
			  
			  UPDATE ACT_HEP_HIST_EST_PUBLICACION HEP
			  SET 
				HEP.HEP_FECHA_HASTA = SYSDATE,
				HEP.USUARIOMODIFICAR = ''REM'',
				HEP.FECHAMODIFICAR = SYSDATE
			  WHERE HEP.ACT_ID = FILA_ACT.ACT_ID
			  AND HEP.HEP_FECHA_HASTA IS NULL
			  AND HEP.BORRADO = 0;
			  
			  INSERT INTO ACT_HEP_HIST_EST_PUBLICACION (HEP_ID, ACT_ID, HEP_FECHA_DESDE, DD_POR_ID, DD_TPU_ID, DD_EPU_ID, HEP_MOTIVO, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
			  VALUES (
			  S_ACT_HEP_HIST_EST_PUBLICACION.NEXTVAL,
			  FILA_ACT.ACT_ID,
			  SYSDATE,
			  NULL,
			  NULL,
			  (SELECT EPU.DD_EPU_ID FROM DD_EPU_ESTADO_PUBLICACION EPU WHERE EPU.DD_EPU_CODIGO = ''06''),
			  ''Fin vigencia asistida'',
			  0,
			  ''REM'',
			  SYSDATE,
			  0
