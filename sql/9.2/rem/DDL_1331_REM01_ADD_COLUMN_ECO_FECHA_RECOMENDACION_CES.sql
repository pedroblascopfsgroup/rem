...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('ECO_FECHA_RECOMENDACION_CES', 'DATE', 'Campo con la fecha recomendacion de la tarea Recomendacion CES')
    ); 
	...
	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL
					   ADD ('||V_TMP_TIPO_DATA(1)||' '||V_TMP_TIPO_DATA(2)||')';
					   
			EXECUTE IMMEDIATE V_MSQL;

			V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL.'||V_TMP_TIPO_DATA(1)||' IS '''||V_TMP_TIPO_DATA(3)||'''';
