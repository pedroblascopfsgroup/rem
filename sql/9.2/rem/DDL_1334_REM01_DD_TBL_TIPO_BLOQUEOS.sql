...
--## PRODUCTO=NO
--## Finalidad: Tabla para gestionar el diccionario de tipo de bloqueos.
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(30 CHAR) := 'DD_TBL_TIPO_BLOQUEOS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_TEXT_CHARS VARCHAR2(3 CHAR) := 'TBL'; -- Vble. auxiliar para almacenar las 3 letras orientativas de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla para gestionar el diccionario de tipo bloqueos'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	ELSE
		...
		-- Crear foreign key.
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_DD_ABL_ID FOREIGN KEY (DD_ABL_ID) REFERENCES '||V_ESQUEMA||'.DD_ABL_AREA_BLOQUEOS (DD_ABL_ID) ON DELETE SET NULL)';
		EXECUTE IMMEDIATE V_MSQL;
