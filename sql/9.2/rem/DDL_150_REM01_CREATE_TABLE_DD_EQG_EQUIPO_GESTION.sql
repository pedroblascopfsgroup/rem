...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_EQG_EQUIPO_GESTION.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'DD_EQG_EQUIPO_GESTION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_CDG VARCHAR2(27 CHAR) := 'EQG'; -- Vble. auxiliar para almacenar el codigo de la tabla para las columnas.
...
BEGIN
	...
	ELSE 
		...
		V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' IS ''Tabla de equipo de gestion.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la tabla creado.');		

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_ID IS ''Código identificador único del equipo de gestion''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CDG||'_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_CODIGO IS ''Código identificador único del equipo de gestion''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CDG||'_CODIGO creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_DESCRIPCION IS ''Descripción del estado de equipo de gestion''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CDG||'_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_DESCRIPCION_LARGA IS ''Descripción larga del equipo de gestion''';
		EXECUTE IMMEDIATE V_MSQL;
