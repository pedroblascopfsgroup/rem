...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_MDT_MOTIVO_DEVOLUCION_TBJ.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'DD_MDT_MOTIVO_DEVOLUCION_TBJ'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_CDG VARCHAR2(27 CHAR) := 'MDT'; -- Vble. auxiliar para almacenar el codigo de la tabla para las columnas.
...
BEGIN
	...
	ELSE 
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT DD_MDT_MOTIVO_DEVOLU_TBJ_PK PRIMARY KEY (DD_'||V_CDG||'_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_ID IS ''Código identificador único de motivo devolución''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CDG||'_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_CODIGO IS ''Código identificador único del motivo devolución''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CDG||'_CODIGO creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_DESCRIPCION IS ''Descripción del estado de motivo devolución''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CDG||'_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_'||V_CDG||'_DESCRIPCION_LARGA IS ''Descripción larga del motivo devolución''';
		EXECUTE IMMEDIATE V_MSQL;
