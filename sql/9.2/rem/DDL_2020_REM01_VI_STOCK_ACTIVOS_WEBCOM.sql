...
BEGIN
	...
	AS
		...
		VALORES_PRECIOS_ANT AS (
			...
			SELECT * FROM (
			SELECT HVAL.ACT_ID, HVAL.DD_TPC_ID, DD.DD_TPC_CODIGO, HVAL.HVA_IMPORTE, HVAL.HVA_FECHA_INICIO, HVAL.HVA_FECHA_FIN,
	    ...
	    ACCION AS (
				SELECT * FROM (
					SELECT ACT.ACT_ID,
			        CASE WHEN (ACT.FECHACREAR IS NOT NULL) 
			            THEN CAST(TO_CHAR(ACT.FECHACREAR,''YYYY-MM-DD'') || 
		...
		CASE WHEN (VAL1.VAL_FECHA_FIN IS NOT NULL) 
			THEN CAST(TO_CHAR(VAL1.VAL_FECHA_FIN ,''YYYY-MM-DD'') 	
			|| ''T'' ||
			TO_CHAR(VAL1.VAL_FECHA_FIN ,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		ELSE NULL
		...
		LEFT JOIN ACCION ACC ON ACC.ACT_ID = ACT.ACT_ID';
