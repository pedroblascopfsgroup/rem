...
BEGIN
   ...
   -- Creamos vista materializada
	...
	WITH ACCION AS (
			SELECT * FROM (
				SELECT OFR.OFR_ID,
		        CASE WHEN (OFR.FECHACREAR IS NOT NULL) 
		            THEN CAST(TO_CHAR(OFR.FECHACREAR,''YYYY-MM-DD'') || 
		            ''T'' ||TO_CHAR(OFR.FECHACREAR,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		            ELSE NULL
		        END FECHA_ACCION,
		        CASE WHEN (OFR.USUARIOMODIFICAR IS NOT NULL) 
		            THEN (SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
		            WHERE USU.USU_USERNAME = OFR.USUARIOMODIFICAR)
		            ELSE (SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
		            WHERE USU.USU_USERNAME = OFR.USUARIOCREAR) 
		...
		LEFT JOIN ACCION ACC ON ACC.OFR_ID = OFR.OFR_ID
