...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
	    ...
	    END VENDIDO, 
	    CASE WHEN (OFR.FECHAMODIFICAR IS NOT NULL)
			THEN CAST(TO_CHAR(OFR.FECHAMODIFICAR,''YYYY-MM-DD'') ||
				''T'' ||TO_CHAR(OFR.FECHAMODIFICAR,''HH24:MI:SS'') AS VARCHAR2(50 CHAR))
			ELSE CAST(TO_CHAR(OFR.FECHACREAR,''YYYY-MM-DD'') ||
				''T'' ||TO_CHAR(OFR.FECHACREAR,''HH24:MI:SS'') AS VARCHAR2(50 CHAR))
		END 																					FECHA_ACCION,
		CASE WHEN (OFR.USUARIOMODIFICAR IS NOT NULL)
			THEN CAST((SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU
				WHERE USU.USU_USERNAME = OFR.USUARIOMODIFICAR) AS NUMBER(16,0))
			ELSE CAST((SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU
				WHERE USU.USU_USERNAME = OFR.USUARIOCREAR) AS NUMBER(16,0))
		...
		WHERE OFR.OFR_WEBCOM_ID IS NOT NULL AND OFR.USUARIOMODIFICAR IS NOT NULL AND OFR.USUARIOMODIFICAR != ''REST-USER''';
		...
		-- Creamos indice	
		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_IDX ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(ID_OFERTA_WEBCOM) TABLESPACE '||V_TABLESPACE_IDX;		
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_IDX... Indice creado.');	
	
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (ID_OFERTA_WEBCOM) USING INDEX)';
		EXECUTE IMMEDIATE V_MSQL;
