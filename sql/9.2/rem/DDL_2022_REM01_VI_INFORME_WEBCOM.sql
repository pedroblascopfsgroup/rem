...
BEGIN
   ...
   -- Creamos vista materializada
	...
	SELECT 
      ...
      CAST(ACT.ACT_NUM_ACTIVO || ''0000'' || ICO.ICO_ID AS NUMBER(32,0))                  		AS ID_INFORME_COMERCIAL,
	  ...
	  CAST((SELECT CASE WHEN (HIC.HIC_ID IS NOT NULL) 
          THEN ''0''  
          ELSE ''1'' 
        END POSIBLE_INFORME
		FROM '||V_ESQUEMA||'.ACT_HIC_EST_INF_COMER_HIST HIC,
		'||V_ESQUEMA||'.DD_AIC_ACCION_INF_COMERCIAL DDAIC
		WHERE HIC.ACT_ID = ACT.ACT_ID AND DDAIC.DD_AIC_ID = HIC.DD_AIC_ID 
		AND DDAIC.DD_AIC_CODIGO = ''04'') AS NUMBER(1,0))  										AS POSIBLE_INFORME,
	  CAST((SELECT MAX(HIC.HIC_MOTIVO)
		FROM '||V_ESQUEMA||'.ACT_HIC_EST_INF_COMER_HIST HIC,
		'||V_ESQUEMA||'.DD_AIC_ACCION_INF_COMERCIAL DDAIC
		WHERE HIC.ACT_ID = ACT.ACT_ID AND DDAIC.DD_AIC_ID = HIC.DD_AIC_ID 
		AND DDAIC.DD_AIC_CODIGO = ''04'') AS VARCHAR2(500 CHAR))  								AS MOTIVO_NO_POSIBLE_INFORME,
      CAST(NULL AS NUMBER(1,0))                               AS recepcionLlavesHaya,
      CAST(NULL AS NUMBER(1,0))                               AS envioLlavesAApi,
      ...
      CAST(BREG.BIE_DREG_NUM_REGISTRO AS NUMBER(16,0))                                          AS NUMERO_REGISTRO,
        CAST((SELECT MAX(CAT.CAT_REF_CATASTRAL) 
      ...
      CAST((SELECT MAX(VAL.VAL_IMPORTE) 
		FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL 
      	LEFT JOIN DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = VAL.DD_TPC_ID 
      	WHERE VAL.ACT_ID = ACT.ACT_ID AND DD.DD_TPC_CODIGO = ''02'') AS NUMBER(16,2))           AS VALOR_APROBADO_VENTA,
     CAST((SELECT CASE 
          ...
          ELSE NULL 
        END FECHA_VALOR_APROBADO_VENTA
      FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL 
      LEFT JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = VAL.DD_TPC_ID 
      WHERE VAL.ACT_ID = ACT.ACT_ID AND DD.DD_TPC_CODIGO = ''02'') AS VARCHAR2(50 CHAR))        AS FECHA_VALOR_APROBADO_VENTA,
      CAST((SELECT MAX(VAL.VAL_IMPORTE) 
		FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL 
      	LEFT JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = VAL.DD_TPC_ID 
      	WHERE VAL.ACT_ID = ACT.ACT_ID AND DD.DD_TPC_CODIGO = ''03'') AS NUMBER(16,2))           AS VALOR_APROBADO_RENTA,
      CAST((SELECT CASE 
          WHEN (VAL.VAL_FECHA_INICIO IS NOT NULL) 
          THEN TO_CHAR(VAL.VAL_FECHA_INICIO,''YYYY-MM-DD'') || 
          ''T'' ||TO_CHAR(VAL.VAL_FECHA_INICIO,''HH24:MM:SS'')
      ...
      FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL 
      LEFT JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = VAL.DD_TPC_ID 
      WHERE VAL.ACT_ID = ACT.ACT_ID AND DD.DD_TPC_CODIGO = ''03'') AS VARCHAR2(50 CHAR))        AS FECHA_VALOR_APROBADO_RENTA,
      ...
      CASE WHEN (SPS.SPS_FECHA_TOMA_POSESION IS NOT NULL) 
          THEN CAST(TO_CHAR(SPS.SPS_FECHA_TOMA_POSESION,''YYYY-MM-DD'') || 
              ''T'' ||TO_CHAR(SPS.SPS_FECHA_TOMA_POSESION,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
          ELSE NULL
      END 																				        FECHA_POSESION,
      ...
      CASE WHEN (SPS.SPS_FECHA_TITULO IS NOT NULL) 
          THEN CAST(TO_CHAR(SPS.SPS_FECHA_TITULO ,''YYYY-MM-DD'') || 
              ''T'' ||TO_CHAR(SPS.SPS_FECHA_TITULO  ,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
          ELSE NULL
      ...
      CASE WHEN (SPS.SPS_FECHA_VENC_TITULO IS NOT NULL) 
          THEN CAST(TO_CHAR(SPS.SPS_FECHA_VENC_TITULO ,''YYYY-MM-DD'') || 
              ''T'' ||TO_CHAR(SPS.SPS_FECHA_VENC_TITULO  ,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
          ELSE NULL
      END                                                                                       PLAZO_CONTRATO_DATOS_OCU,
      CAST(SPS.SPS_RENTA_MENSUAL AS NUMBER(16,2))                                               AS RENTA_MENSUAL_DATOS_OCU,
      
      CAST(NULL AS NUMBER(9,2))                 AS recibidoImporteDatosAdm,
      CAST(NULL AS NUMBER(9,2))                 AS ibiImporteDatosAdm,
      CAST(NULL AS NUMBER(9,2))                 AS derramaImporteDatosAdm,
      CAST(NULL AS VARCHAR2(500 CHAR))          AS detalleDerramaDatosAdm,
      ...
      CAST(ICO.ICO_FECHA_ULTIMA_VISITA  AS VARCHAR2(50 CHAR))                                   AS FECHA_ULTIMA_VISITA,
      ...
      CAST(NULL  AS VARCHAR2(50 CHAR))             AS  fechaRecepcionLlavesApi,
      ...
      CAST(PAC.PAC_PORC_PROPIEDAD AS NUMBER(8,2))                                               AS PORCENTAJE_PROPIEDAD,
      CAST((SELECT MAX(VAL.VAL_IMPORTE) 
		FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL 
      	LEFT JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = VAL.DD_TPC_ID 
      ...
      CAST(ICO.ICO_JUSTIFICACION_VENTA AS VARCHAR2(500 CHAR))                                   AS JUSTIFICACION_VALOR_EST_VENTA,
      CAST((SELECT MAX(VAL.VAL_IMPORTE) 
		FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL 
      	LEFT JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = VAL.DD_TPC_ID 
      	WHERE VAL.ACT_ID = ACT.ACT_ID AND DD.DD_TPC_CODIGO = ''12'') AS NUMBER(9,2))            AS VALOR_ESTIMADO_RENTA,
      CAST((SELECT CASE 
          WHEN (VAL.VAL_FECHA_INICIO IS NOT NULL) 
          THEN TO_CHAR(VAL.VAL_FECHA_INICIO,''YYYY-MM-DD'') || 
          ''T'' ||TO_CHAR(VAL.VAL_FECHA_INICIO,''HH24:MM:SS'')
        ...
        END FECHA_VALOR_ESTIMADO_RENTA
      	FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL 
      	LEFT JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = VAL.DD_TPC_ID 
      	WHERE VAL.ACT_ID = ACT.ACT_ID AND DD.DD_TPC_CODIGO = ''12'') AS VARCHAR2(50 CHAR))	    AS FECHA_VALOR_ESTIMADO_RENTA, 
      CAST(ICO.ICO_JUSTIFICACION_RENTA AS VARCHAR2(500 CHAR))                                   AS JUSTIFICACION_VALOR_EST_RENTA,
      CAST(REG.REG_SUPERFICIE_UTIL AS NUMBER(9,2))                                              AS UTIL_SUPERFICIE,
      CAST((SELECT MAX(BIE.BIE_DREG_SUPERFICIE_CONSTRUIDA)
      	FROM '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES BIE 
      	WHERE BIE.BIE_DREG_ID = REG.BIE_DREG_ID) AS NUMBER(9,2))                                AS CONSTRUIDA_SUPERFICIE,
      CAST((SELECT MAX(BIE.BIE_DREG_SUPERFICIE)
      	FROM '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES BIE 
      WHERE BIE.BIE_DREG_ID = REG.BIE_DREG_ID) AS NUMBER(9,2))                                  AS REGISTRAL_SUPERFICIE,
      CAST(REG.REG_SUPERFICIE_PARCELA AS NUMBER(9,2))                                           AS PARCELA_SUPERFICIE,
      ...
      --CAST(DIS.DIS_NUM_PLANTA AS NUMBER(16,0))                                                AS NUMERO_PLANTA,
      --CAST(TPH.DD_TPH_CODIGO AS VARCHAR2(5 CHAR))                                             AS COD_TIPO_ESTANCIA,
      --CAST(DIS.DIS_CANTIDAD AS NUMBER(16,0))                                                  AS NÚMERO_ESTANCIAS,
      --CAST(DIS.DIS_SUPERFICIE AS NUMBER(9,2))                                                 AS METROS_ESTANCIAS,
      CAST(NULL AS VARCHAR2(150 CHAR))                                                          AS DESCRIPCIÓN_ESTANCIAS,
      CAST((SELECT SUM(DIS_CANTIDAD)
      	FROM '||V_ESQUEMA||'.ACT_DIS_DISTRIBUCION DIS2 
      	INNER JOIN '||V_ESQUEMA||'.DD_TPH_TIPO_HABITACULO TPH2 ON TPH2.DD_TPH_ID = DIS2.DD_TPH_ID
      	WHERE DIS2.ICO_ID = ICO.ICO_ID
      	AND TPH2.DD_TPH_CODIGO = ''16'') AS NUMBER(16,0))                                       AS NUMERO_TERRAZAS_DESCUBIERTAS,
      CAST(NULL AS VARCHAR2(150 CHAR))                                                          AS DESC_TERRAZAS_DESCUBIERTAS,
      CAST((SELECT SUM(DIS_CANTIDAD)
      	FROM '||V_ESQUEMA||'.ACT_DIS_DISTRIBUCION DIS2 
      	INNER JOIN '||V_ESQUEMA||'.DD_TPH_TIPO_HABITACULO TPH2 ON TPH2.DD_TPH_ID = DIS2.DD_TPH_ID
      	WHERE DIS2.ICO_ID = ICO.ICO_ID
      	AND TPH2.DD_TPH_CODIGO = ''15'') AS NUMBER(16,0))                                       AS NUMERO_TERRAZAS_CUBIERTAS,
      CAST(NULL  AS VARCHAR2(150 CHAR))                                                         AS DESC_TERRAZAS_CUBIERTAS,
      --CAST(EXISTS(DIS_CANTIDAD (tipo 13))                                                     AS DESPENSA_OTRAS_DEPENDENCIAS,
      --CAST(EXISTS(DIS_CANTIDAD (tipo 14))                                                     AS OTROS_OTRAS_DEPENDENCIAS,


      CAST(INS.INS_ELECTR AS NUMBER(1,0))                                                       AS INSTALACION_ELECTRICIDAD_INST,
      CAST(INS.INS_ELECTR_CON_CONTADOR AS NUMBER(1,0))                                          AS CONTADOR_ELECTRICIDAD_INST,
      CAST(INS.INS_AGUA AS NUMBER(1,0))                                                         AS INSTALACION_AGUA_INST,
      CAST(INS.INS_AGUA_CON_CONTADOR AS NUMBER(1,0))                                            AS CONTADOR_AGUA_INST,
      CAST(INS.INS_GAS AS NUMBER(1,0))                                                          AS GAS_INST,
      
      CAST(NULL AS NUMBER(1,0))                           AS contadorGasInstalacion,
      ...
      CAST(VIV.VIV_REFORMA_PRESUPUESTO AS NUMBER(9,2))                                          AS OTRAS_REFORMAS_NEC_IMPORTE,
--CAST(ICO_ACTIVOS_VINC AS NUMBER(1,0))                      AS ACTIVOS_VINCULADOS,
      ...
      CAST(LCO.LCO_USO_ANTERIOR AS VARCHAR2(150 CHAR))                                          AS ANTERIOR_USO,
      ...
      CAST(LCO.LCO_MTS_FACHADA_PPAL AS NUMBER(3,2))                      						AS MTS_LINEALES_FACHADA_PPAL,
      CAST(LCO.LCO_MTS_ALTURA_LIBRE AS NUMBER(3,2))                      						AS ALTURA,
      CAST((SELECT CASE 
          WHEN (ANJ.ANJ_ID IS NOT NULL) 
          THEN ''1'' 
          ELSE ''0'' 
        END EXISTE_ANEJO_TRASTERO
		FROM '||V_ESQUEMA||'.ACT_ANJ_ANEJOS ANJ,
		'||V_ESQUEMA||'.DD_TNJ_TIPO_ANEJO DDTNJ
		WHERE ANJ.ICO_ID = ICO.ICO_ID 
		AND DDTNJ.DD_TNJ_CODIGO = ''02'') AS NUMBER(1,0))                     					AS EXISTE_ANEJO_TRASTERO,
 	  CAST((SELECT MAX(ANJ.ANJ_DESCRIPCION)
		FROM '||V_ESQUEMA||'.ACT_ANJ_ANEJOS ANJ,
		'||V_ESQUEMA||'.DD_TNJ_TIPO_ANEJO DDTNJ
		WHERE ANJ.ICO_ID = ICO.ICO_ID 
		AND DDTNJ.DD_TNJ_CODIGO = ''02'') AS VARCHAR2(150 CHAR))                     			AS ANEJO_TRASTERO,
	  CAST((SELECT MAX(ANJ.ANJ_DESCRIPCION)
		FROM '||V_ESQUEMA||'.ACT_ANJ_ANEJOS ANJ,
		'||V_ESQUEMA||'.DD_TNJ_TIPO_ANEJO DDTNJ
		WHERE ANJ.ICO_ID = ICO.ICO_ID 
		AND DDTNJ.DD_TNJ_CODIGO = ''01'') AS VARCHAR2(150 CHAR))                     			AS ANEJO_GARAJE,
      CAST((SELECT COUNT(*)
		FROM '||V_ESQUEMA||'.ACT_ANJ_ANEJOS ANJ,
		'||V_ESQUEMA||'.DD_TNJ_TIPO_ANEJO DDTNJ
		WHERE ANJ.ICO_ID = ICO.ICO_ID 
		AND DDTNJ.DD_TNJ_CODIGO = ''01'') AS NUMBER(16,0))                    					AS NUMERO_PLAZAS_GARAJE,
  	  CAST((SELECT SUM(ANJ.ANJ_SUPERFICIE)
		FROM '||V_ESQUEMA||'.ACT_ANJ_ANEJOS ANJ,
		'||V_ESQUEMA||'.DD_TNJ_TIPO_ANEJO DDTNJ
		WHERE ANJ.ICO_ID = ICO.ICO_ID 
		AND DDTNJ.DD_TNJ_CODIGO = ''01'') AS NUMBER(3,2))                    					AS SUPERFICIE_PLAZAS_GARAJE,
	  CAST((SELECT MAX(DDSPG2.DD_SPG_CODIGO)
		FROM '||V_ESQUEMA||'.ACT_ANJ_ANEJOS ANJ,
		'||V_ESQUEMA||'.DD_SPG_SUBTIPO_PLAZA_GARAJE DDSPG2
		WHERE ANJ.ICO_ID = ICO.ICO_ID) AS VARCHAR2(5 CHAR))                    					AS COD_SUBTIPO_PLAZAS_GARAJE,
      ...
      CAST(APR.APR_ANCHURA AS NUMBER(3,2))                      								AS ANCHO,
      CAST(APR.APR_ALTURA AS NUMBER(3,2))                      									AS ALTO,
      CAST(APR.APR_PROFUNDIDAD AS NUMBER(3,2))                      							AS LARGO,
      ...
      CAST(NULL AS NUMBER(1,0))                      AS EstadoInstElectricidadInst,
      CAST(NULL AS NUMBER(1,0))                      AS EstadoContadorElectricidadInst,
      CAST(NULL AS NUMBER(1,0))                      AS EstadoInstalacionAguaInst,
      CAST(NULL AS NUMBER(1,0))                      AS EstadoContadorAguaInst,
      CAST(NULL AS NUMBER(1,0))                      AS EstadoGasInst,
      CAST(NULL AS NUMBER(1,0))                      AS EstadoContadorGasInst,

      CAST(DDECV.DD_ECV_CODIGO AS VARCHAR2(5 CHAR))                                 				AS ESTADO_CONSERVACION_EDI,
      CAST(ICO.ICO_ANO_REHABILITACION AS NUMBER(16,0))                                    		AS ANYO_REHABILITACION_EDI,
      CAST(EDI.EDI_NUM_PLANTAS AS NUMBER(16,0))                                           		AS NUMERO_PLANTAS_EDI,
      CAST(EDI.EDI_ASCENSOR AS NUMBER(1,0))                                            			AS ASCENSOR_EDI,
      CAST(EDI.EDI_NUM_ASCENSORES AS NUMBER(16,0))                                        		AS NUMERO_ASCENSORES_EDI,
	  CASE WHEN (CPR.CPR_ID IS NOT NULL) 
			THEN CAST(''1'' AS NUMBER(1,0))
			ELSE CAST(''0'' AS NUMBER(1,0))
	  END 																			   			EXISTE_COMUNIDAD_EDI,
      CAST(ICO.ICO_CUOTACP_ORIENTATIVA AS NUMBER(16,2))                                   		AS CUOTA_COMUNIDAD_EDI,
      CAST(CPR.CPR_PRESIDENTE_NOMBRE AS VARCHAR2(100 CHAR))                               		AS NOMBRE_PRESIDENTE_COM_EDI,
      CAST(CPR.CPR_PRESIDENTE_TELF AS VARCHAR2(20 CHAR))                                  		AS TELF_PRESIDENTE_COM_EDI,
      CAST(CPR.CPR_ADMINISTRADOR_NOMBRE AS VARCHAR2(100 CHAR))                            		AS NOMBRE_ADMIN_COM_EDI,
      CAST(CPR.CPR_ADMINISTRADOR_TELF AS VARCHAR2(20 CHAR))                               		AS TELF_ADMIN_COM_EDI,
      CAST(ICO.ICO_DERRAMACP_ORIENTATIVA AS VARCHAR2(150 CHAR))                                 AS DESC_DERRAMA_COM_EDI,
      ...
      CAST(INF.INF_FACIL_ACCESO_DESC AS VARCHAR2(250 CHAR))                               		AS FACIL_ACCESO_CARRETERA,
      ...
      CAST(NULL AS NUMBER(1,0))                                                           		AS ESTADO_PINTURA_TECHO_GOTELE,
      CAST(PRV.PRV_PINTURA_LISA_TECHO_EST AS NUMBER(1,0))                                 		AS ESTADO_PINTURA_TECHO_LISA,
      CAST(NULL AS NUMBER(1,0))                                                           		AS ESTADO_PINTURA_TECHO_PINTADO,
      ...
      CAST(BNY.BNY_BANYO_OTROS AS VARCHAR2(250 CHAR))                                     		AS OTROS_BNY,
	  ...
	  CAST(INS.INS_CALEF_RADIADORES_ALU AS NUMBER(1,0))                                   		AS EXISTE_RADIADORES_DE_ALUMINIO,
      ...
      END 																			      		ID_USUARIO_REM_ACCION
	    ...
	    --LEFT JOIN '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL ON VAL.ACT_ID = ACT.ACT_ID
	    ...
	    WHERE ICO.ICO_WEBCOM_ID IS NOT NULL AND ICO.USUARIOMODIFICAR IS NOT NULL AND ICO.USUARIOMODIFICAR != ''REST_USER''';
