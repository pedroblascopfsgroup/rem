...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		...
		FROM(
			SELECT 
		  	AGA.AGR_ID||AGA.ACT_ID||SUBD.DD_TPA_ID||SUBD.DD_SAC_ID||SUBD.PLANTAS||SUBD.DORMITORIOS||SUBD.BANYO 		ID,
			ACC.FECHA_ACCION,
			ACC.ID_USUARIO_REM_ACCION,
		  	SUBD.ACT_ID,
		  	AGA.AGR_ID,
			TPA.DD_TPA_CODIGO,
			SAC.DD_SAC_CODIGO,
			...
			INNER JOIN ACCION ACC ON ACC.ACT_ID = AGA.ACT_ID
		) ACT_SD
		GROUP BY ACT_SD.ID, ACT_SD.AGR_ID, ACT_SD.DESCRIPCION,
		ACT_SD.DORMITORIOS,ACT_SD.PLANTAS, ACT_SD.BANYO, 
		ACT_SD.DD_TPA_CODIGO, ACT_SD.DD_SAC_CODIGO, ACT_SD.FECHA_ACCION, ACT_SD.ID_USUARIO_REM_ACCION';
