...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		WITH SUBDIVISION AS (
		SELECT * FROM (
		    SELECT SUBD.ACT_ID, AGA.AGR_ID,
		    AGA.AGR_ID||SUBD.DD_TPA_ID||SUBD.DD_SAC_ID||SUBD.PLANTAS||SUBD.DORMITORIOS||SUBD.BANYO ID_SUBDIVISION_REM,
		    ROW_NUMBER() OVER (PARTITION BY AGA.ACT_ID ORDER BY AGA.ACT_ID DESC) ORDEN 
		    FROM (
	    ...
	    ACCION AS (
				SELECT * FROM (
			        ...
			        CASE WHEN (AGA.FECHACREAR IS NOT NULL) 
			            ...
			            ELSE NULL
			        END FECHA_ACCION,
			        CASE WHEN (AGA.USUARIOMODIFICAR IS NOT NULL) 
			            THEN (SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
			            WHERE USU.USU_USERNAME = AGA.USUARIOMODIFICAR)
		...
		CAST(SDV.ID_SUBDIVISION_REM AS NUMBER(16,0)) 											            AS CABECERAS_ID_SUBDIVISION_REM,
		CAST(NVL(ACC.FECHA_ACCION, TO_CHAR((SELECT SYSDATE FROM DUAL),''YYYY-MM-DD'') || 
		...
		LEFT JOIN ACCION ACC ON ACC.AGA_ID = AGA.AGA_ID
