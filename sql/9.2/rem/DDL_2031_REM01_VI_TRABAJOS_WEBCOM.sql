   ...
   -- Creamos vista materializada
	...
	AS
		WITH ACCION AS (
			SELECT * FROM (
				SELECT TBJ.TBJ_ID,
		        CASE WHEN (TBJ.FECHACREAR IS NOT NULL) 
		            THEN CAST(TO_CHAR(TBJ.FECHACREAR,''YYYY-MM-DD'') || 
		            ''T'' ||TO_CHAR(TBJ.FECHACREAR,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		            ELSE NULL
		        END FECHA_ACCION,
		        CASE WHEN (TBJ.USUARIOMODIFICAR IS NOT NULL) 
		            THEN (SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
		            WHERE USU.USU_USERNAME = TBJ.USUARIOMODIFICAR)
		            ELSE (SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
		            WHERE USU.USU_USERNAME = TBJ.USUARIOCREAR) 
	    ...
	    CAST(DDEST.DD_EST_CODIGO AS VARCHAR2(5 CHAR)) 											AS COD_ESTADO_TRABAJO,
		...
		LEFT JOIN ACCION ACC ON ACC.TBJ_ID = TBJ.TBJ_ID
   	 	WHERE TBJ.TBJ_WEBCOM_ID IS NOT NULL';
		...
		-- Creamos indice	
		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_IDX ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(ID_TRABAJO_REM) TABLESPACE '||V_TABLESPACE_IDX;		
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_IDX... Indice creado.');	
	
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (ID_TRABAJO_REM) USING INDEX)';
		EXECUTE IMMEDIATE V_MSQL;
