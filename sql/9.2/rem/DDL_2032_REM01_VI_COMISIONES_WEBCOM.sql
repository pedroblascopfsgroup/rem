...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
	    ...
	    CAST(GEX.GEX_PROVEEDOR AS NUMBER(16,0))  											  	AS ID_PROVEEDOR_REM,
	    CAST((SELECT CASE 
		    WHEN (DDACC.DD_ACC_CODIGO = ''04'') 
				THEN 1 
				ELSE 0 
			END ES_PRESCRIPCION 
    		FROM DD_ACC_ACCION_GASTOS DDACC
    		WHERE DDACC.DD_ACC_ID = GEX.DD_ACC_ID ) AS NUMBER(1,0))               				AS ES_PRESCRIPCION,
	    CAST((SELECT CASE 
			...
			END ES_COLABORACION 
		    FROM DD_ACC_ACCION_GASTOS DDACC
		    WHERE DDACC.DD_ACC_ID = GEX.DD_ACC_ID ) AS NUMBER(1,0))                				AS ES_COLABORACION, 
	    CAST((SELECT CASE 
			...
			END ES_RESPONSABLE 
		    FROM DD_ACC_ACCION_GASTOS DDACC
		    WHERE DDACC.DD_ACC_ID = GEX.DD_ACC_ID ) AS NUMBER(1,0))                				AS ES_RESPONSABLE,
	    CAST((SELECT CASE 
			WHEN (DDACC.DD_ACC_CODIGO = ''05'' AND 
	            (SELECT PVE.PVE_ID 
					FROM ACT_PVE_PROVEEDOR PVE, DD_TPR_TIPO_PROVEEDOR DDTPR
			...
			END ES_FDV 
		    FROM DD_ACC_ACCION_GASTOS DDACC
		    WHERE DDACC.DD_ACC_ID = GEX.DD_ACC_ID ) AS NUMBER(1,0))               AS ES_FDV,
	    CAST((SELECT CASE 
	      	WHEN (DDACC.DD_ACC_CODIGO = ''06'') 
				THEN 1 
				ELSE 0 
			END ES_RESPONSABLE 
		    FROM DD_ACC_ACCION_GASTOS DDACC
		    WHERE DDACC.DD_ACC_ID = GEX.DD_ACC_ID ) AS NUMBER(1,0))                				AS ES_DOBLE_PRESCRIPCION,
	    CAST(GEX.GEX_OBSERVACIONES AS VARCHAR2(250 CHAR))               						AS OBSERVACIONES,
	    CAST(GEX.GEX_IMPORTE_FINAL AS NUMBER(9,2))                      						AS IMPORTE,
	    CAST(GEX.GEX_IMPORTE_CALCULO AS NUMBER(3,2))                    						AS PORCENTAJE,	    
	    CASE WHEN (GEX.FECHAMODIFICAR IS NOT NULL) 
				THEN CAST(TO_CHAR(GEX.FECHAMODIFICAR,''YYYY-MM-DD'') || 
					''T'' ||TO_CHAR(GEX.FECHAMODIFICAR,''HH24:MI:SS'') AS VARCHAR2(50 CHAR))
				ELSE CAST(TO_CHAR(GEX.FECHACREAR,''YYYY-MM-DD'') || 
					''T'' ||TO_CHAR(GEX.FECHACREAR,''HH24:MI:SS'') AS VARCHAR2(50 CHAR))
		END 																				  	FECHA_ACCION,
		CASE WHEN (GEX.USUARIOMODIFICAR IS NOT NULL) 
			THEN CAST((SELECT USU.USU_ID FROM REMMASTER.USU_USUARIOS USU 
				WHERE USU.USU_USERNAME = GEX.USUARIOMODIFICAR) AS NUMBER(16,0))
			ELSE CAST((SELECT USU.USU_ID FROM REMMASTER.USU_USUARIOS USU 
				WHERE USU.USU_USERNAME = GEX.USUARIOCREAR) AS NUMBER(16,0))
		END 			                                                      					ID_USUARIO_REM_ACCION      
		FROM  GEX_GASTOS_EXPEDIENTE GEX
		LEFT JOIN ECO_EXPEDIENTE_COMERCIAL ECO ON ECO.ECO_ID = GEX.ECO_ID
	    LEFT JOIN OFR_OFERTAS OFR ON OFR.OFR_ID = ECO.OFR_ID
	    LEFT JOIN DD_ACC_ACCION_GASTOS DDACC ON DDACC.DD_ACC_ID = GEX.DD_ACC_ID
	   	WHERE OFR.OFR_WEBCOM_ID IS NOT NULL AND (GEX.USUARIOMODIFICAR IS NULL OR GEX.USUARIOMODIFICAR != ''REST-USER'')';  	
