...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
    	...
    	--CAST(TO_CHAR(FRT.FECHA_RESP,''YYYY-MM-DD'')  
       --  || ''T'' ||
        -- TO_CHAR(FRT.FECHA_RESP,''HH24:MM:SS'')  AS VARCHAR2(50 CHAR))          AS FECHA_REALIZACION,

    	CAST(NVL(TO_CHAR(FRT.FECHACREAR,''YYYY-MM-DD'')
             || ''T'' ||
             TO_CHAR(FRT.FECHACREAR,''HH24:MM:SS''),
             TO_CHAR((SELECT SYSDATE FROM DUAL),''YYYY-MM-DD'') 
             || ''T'' ||
             TO_CHAR((SELECT SYSDATE FROM DUAL),''HH24:MM:SS'')
