   ...
   -- Creamos vista materializada
	...
	AS
		WITH TAREA_WEBCOM AS (
			...
			WHERE M2.IRG_CLAVE = ''ID_TAREA_WEBCOM'')
			WHERE ORDEN = 1
		),
		DESC_TAREA AS (
			SELECT * FROM (
			...
			WHERE M3.IRG_CLAVE = ''DESCRIPCION_TAREA'')
			WHERE ORDEN = 1
		)
		SELECT 
		CAST(TAR.TAR_ID AS NUMBER(16,0))														AS ID_NOTIFICACION_REM,
		CAST(REG3.IRG_VALOR AS NUMBER(16,0))													AS ID_NOTIFICACION_WEBCOM,
		CAST(MEJ.TRG_EIN_ID AS NUMBER(16,0))													AS ID_ACTIVO_HAYA,
		CAST(REG4.IRG_VALOR  AS VARCHAR2(250 CHAR))												AS DESCRIPCION,
		CAST(NVL((SELECT TO_CHAR(M2.FECHACREAR,''YYYY-MM-DD'') 
		    || ''T'' ||TO_CHAR(M2.FECHACREAR,''HH24:MI:SS'')  
			FROM '||V_ESQUEMA||'.MEJ_IRG_INFO_REGISTRO M2 
			WHERE M2.REG_ID = REG.REG_ID 
			AND M2.IRG_CLAVE = ''FECHA_RESP_TAREA'')
			,
		...
		CAST(NVL((SELECT USU.USU_ID FROM '||V_ESQUEMA||'.MEJ_IRG_INFO_REGISTRO M2, 
			'||V_ESQUEMA_M||'.USU_USUARIOS USU
			WHERE M2.REG_ID = REG.REG_ID AND M2.IRG_CLAVE = ''FECHA_RESP_TAREA''
			AND USU.USU_USERNAME = M2.USUARIOCREAR), 
		...
		LEFT JOIN TAREA_WEBCOM REG3 ON REG3.REG_ID = REG.REG_ID 
		LEFT JOIN DESC_TAREA REG4 ON REG4.REG_ID = REG.REG_ID 
		WHERE REG3.IRG_VALOR IS NOT NULL AND TAR.USUARIOCREAR = ''REST-USER''';  	
