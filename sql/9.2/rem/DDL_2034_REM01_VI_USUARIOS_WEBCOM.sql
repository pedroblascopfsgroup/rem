...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		...
		END 																		ACTIVO,
		CASE WHEN (PVC.FECHAMODIFICAR IS NOT NULL) 
			THEN CAST(TO_CHAR(PVC.FECHAMODIFICAR,''YYYY-MM-DD'') || 
			''T'' ||TO_CHAR(PVC.FECHAMODIFICAR,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
			ELSE CAST(TO_CHAR(PVC.FECHACREAR,''YYYY-MM-DD'') || 
			''T'' ||TO_CHAR(PVC.FECHACREAR,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		END 																		FECHA_ACCION,
		CASE WHEN (PVC.USUARIOMODIFICAR IS NOT NULL) 
			THEN CAST((SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
			WHERE USU.USU_USERNAME = PVC.USUARIOMODIFICAR) AS NUMBER(16,0))
