   ...
   -- Creamos vista materializada
	...
	AS
		WITH ACCION AS (
			SELECT * FROM (
				SELECT PVE.PVE_ID,
		        CASE WHEN (PVE.FECHACREAR IS NOT NULL) 
		            THEN CAST(TO_CHAR(PVE.FECHACREAR,''YYYY-MM-DD'') || 
		        ...
		        CASE WHEN (PVE.USUARIOMODIFICAR IS NOT NULL) 
		            ...
		            WHERE USU.USU_USERNAME = PVE.USUARIOMODIFICAR)
		            ELSE (SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
		            WHERE USU.USU_USERNAME = PVE.USUARIOCREAR) 
		...
		LEFT JOIN ACCION ACC ON ACC.PVE_ID = PVE.PVE_ID
