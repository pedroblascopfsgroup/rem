...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		SELECT 
		CAST(  NVL(LPAD(PVE.PVE_ID,10,0) , ''00000'')   
			|| NVL(LPAD(PRD.PRD_ID,5,0) , ''00000'')AS NUMBER(32,0))                           	AS ID_PROV_DELEGACION,
		...
		CASE WHEN (PVE.FECHAMODIFICAR IS NOT NULL) 
			THEN CAST(TO_CHAR(PVE.FECHAMODIFICAR,''YYYY-MM-DD'') || 
			''T'' ||TO_CHAR(PVE.FECHAMODIFICAR,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
			ELSE CAST(TO_CHAR(PVE.FECHACREAR,''YYYY-MM-DD'') || 
			''T'' ||TO_CHAR(PVE.FECHACREAR,''HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		END FECHA_ACCION,
		CASE WHEN (PVE.USUARIOMODIFICAR IS NOT NULL) 
			THEN CAST((SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
			WHERE USU.USU_USERNAME = PVE.USUARIOMODIFICAR) AS NUMBER(16,0))
			ELSE CAST((SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
			WHERE USU.USU_USERNAME = ''REST-USER'') AS NUMBER(16,0))
		...
		WHERE PVE.PVE_ID IS NOT NULL AND PVE.PVE_COD_UVEM IS NOT NULL AND DDTPR.DD_TPR_CODIGO IS NOT NULL AND DDTPR.DD_TPR_CODIGO = ''04''';
