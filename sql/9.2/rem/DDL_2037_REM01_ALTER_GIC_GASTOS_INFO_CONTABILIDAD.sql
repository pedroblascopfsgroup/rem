...
BEGIN
	...
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_DES_ID_CONTABILIZA... No existe');
	ELSE
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' RENAME COLUMN DD_DES_ID_CONTABILIZA TO DD_DEG_ID_CONTABILIZA';	
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_DES_ID_CONTABILIZA modificada a DD_DEG_ID_CONTABILIZA');
	END IF;
	
		-- Comprobamos si existe columna GIC_FECHA_DEVENGO_ESPECIAL
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''GIC_FECHA_DEVENGO_ESPECIAL'' and DATA_TYPE = ''NUMBER'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 1 THEN
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' MODIFY GIC_FECHA_DEVENGO_ESPECIAL DATE';	
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.GIC_FECHA_DEVENGO_ESPECIAL... Cambiado');
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.GIC_FECHA_DEVENGO_ESPECIAL... No existe');
	END IF;
