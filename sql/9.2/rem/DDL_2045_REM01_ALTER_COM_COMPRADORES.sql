...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'DD_LOC_ID',		 				'NUMBER(16,0)',						'Código identificador único de la localidad.'	),
    	T_ALTER(  'DD_PRV_ID',		 				'NUMBER(16,0)',						'Código identificador único de la provincia.'	)
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_COM_LOC',							'DD_LOC_ID',		V_ESQUEMA_M||'.DD_LOC_LOCALIDAD',					'DD_LOC_ID'			),
    	T_FK(	'FK_COM_PRV',							'DD_PRV_ID',		V_ESQUEMA_M||'.DD_PRV_PROVINCIA',					'DD_PRV_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	-- Comprobamos si existe columna COM_MUNICIPIO (si es así, la borramos)
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''COM_MUNICIPIO'' and DATA_TYPE = ''VARCHAR2'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.COM_MUNICIPIO... Ya borrada');
	ELSE
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DROP COLUMN COM_MUNICIPIO';	
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.COM_MUNICIPIO... Borrada');
	END IF;
	
		-- Comprobamos si existe columna COM_PROVINCIA (si es así, la borramos)
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''COM_PROVINCIA'' and DATA_TYPE = ''VARCHAR2'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.COM_PROVINCIA... Ya borrada');
	ELSE
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DROP COLUMN COM_PROVINCIA';	
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.COM_PROVINCIA... Borrada');
	END IF;
