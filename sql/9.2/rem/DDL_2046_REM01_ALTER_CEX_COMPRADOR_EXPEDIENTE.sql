...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'DD_LOC_ID_RTE',		 				'NUMBER(16,0)',						'Código identificador único de la localidad del representante.'	),
    	T_ALTER(  'DD_PRV_ID_RTE',		 				'NUMBER(16,0)',						'Código identificador único de la provincia del representante.'	)
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_CEX_LOC_RTE_ID',							'DD_LOC_ID_RTE',		V_ESQUEMA_M||'.DD_LOC_LOCALIDAD',					'DD_LOC_ID'			),
    	T_FK(	'FK_CEX_PRV_RTE_ID',							'DD_PRV_ID_RTE',		V_ESQUEMA_M||'.DD_PRV_PROVINCIA',					'DD_PRV_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''CEX_MUNICIPIO_RTE'' and DATA_TYPE = ''VARCHAR2'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CEX_MUNICIPIO_RTE... Ya borrada');
	ELSE
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DROP COLUMN CEX_MUNICIPIO_RTE';	
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CEX_MUNICIPIO_RTE... Borrada');
	END IF;
	
		-- Comprobamos si existe columna COM_PROVINCIA (si es así, la borramos)
	V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= ''CEX_PROVINCIA_RTE'' and DATA_TYPE = ''VARCHAR2'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
	IF V_NUM_TABLAS = 0 THEN
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CEX_PROVINCIA_RTE... Ya borrada');
	ELSE
		EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DROP COLUMN CEX_PROVINCIA_RTE';	
		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CEX_PROVINCIA_RTE... Borrada');
	END IF;
