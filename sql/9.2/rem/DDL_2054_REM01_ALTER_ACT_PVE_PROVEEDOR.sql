...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'PVE_COD_REM',		 			'NUMBER(16,0)',						'Código proveedor único de REM.'	)
		);
    V_T_ALTER T_ALTER;
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INFO]: Comprobaciones previas sequencia S_PVE_COD_REM.');
	V_MSQL := 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_NAME = ''S_PVE_COD_REM'' AND SEQUENCE_OWNER = '''||V_ESQUEMA||'''';
	EXECUTE IMMEDIATE V_MSQL INTO v_table_count;
	
	IF v_table_count > 0 THEN 
		DBMS_OUTPUT.PUT_LINE('[INFO]: S_PVE_COD_REM ya existe.');  
	ELSE
	   -- Crear sequencia
	   	V_MSQL := 'CREATE SEQUENCE ' || V_ESQUEMA ||'.S_PVE_COD_REM';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO]: Sequencia S_PVE_COD_REM creada correctamente.'); 
	END IF;
