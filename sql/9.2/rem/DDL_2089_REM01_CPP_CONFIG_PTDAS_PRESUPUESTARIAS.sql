...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'CPP_CONFIG_PTDAS_PREP'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla para gestionar la información de configuración de partidas presupuestarias.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	-- Creamos foreign key EJE_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_CPP_EJE FOREIGN KEY (EJE_ID) REFERENCES '||V_ESQUEMA||'.ACT_EJE_EJERCICIO (EJE_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_CPP_EJE... Foreign key creada.');
	
	
	-- Creamos foreign key DD_STG_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_CPP_STG FOREIGN KEY (DD_STG_ID) REFERENCES '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO (DD_STG_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_CPP_STG... Foreign key creada.');
	
	-- Creamos foreign key DD_CRA_ID
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_CPP_CRA FOREIGN KEY (DD_CRA_ID) REFERENCES '||V_ESQUEMA||'.DD_CRA_CARTERA (DD_CRA_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
