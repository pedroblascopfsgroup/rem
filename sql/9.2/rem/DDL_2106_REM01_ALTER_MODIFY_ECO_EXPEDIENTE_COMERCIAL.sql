...
--## PRODUCTO=NO
--## Finalidad: Cambiar la tabla ECO_EXPEDIENTE_COMERCIAL, con campo No. contrato alquiler Varchar
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'ECO_NUMERO_CONTRATO_ALQUILER',			'VARCHAR2(50 CHAR)',						'No. de contrato de alquiler'	)
		);
    V_T_ALTER T_ALTER;
...
BEGIN
	...
	LOOP
		...
		IF V_NUM_TABLAS > 0 THEN
			...
			--Primero se vacia la columna
			V_MSQL := 'UPDATE '||V_TEXT_TABLA|| ' 
					   SET '||V_T_ALTER(1)||' = NULL
			';

			EXECUTE IMMEDIATE V_MSQL;
			--DBMS_OUTPUT.PUT_LINE('[1] '||V_MSQL);
			DBMS_OUTPUT.PUT_LINE('[INFO] ... '||V_T_ALTER(1)||' Campo VACIADO (a null) en tabla, con tipo '||V_T_ALTER(2));

			--Existe la columna y la cambiamos
			...
			EXECUTE IMMEDIATE V_MSQL;
			--DBMS_OUTPUT.PUT_LINE('[1] '||V_MSQL);
			DBMS_OUTPUT.PUT_LINE('[INFO] ... '||V_T_ALTER(1)||' Campo MODIFICADO en tabla, con tipo '||V_T_ALTER(2));
		...
		ELSE
			DBMS_OUTPUT.PUT_LINE('[INFO] El campo ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'['||V_T_ALTER(1)||'] EL CAMPO NO EXISTE. NO SE HACE NADA -----------------');
		END IF;
