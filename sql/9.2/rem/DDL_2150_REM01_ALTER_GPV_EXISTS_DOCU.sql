...
BEGIN
	...
	IF V_NUM_TABLAS = 1 THEN
		...
		-- Comprobamos si existe columna GPV_EXISTE_DOCUMENTO
		V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE UPPER(COLUMN_NAME)= ''GPV_EXISTE_DOCUMENTO'' and TABLE_NAME = '''||V_TABLA||''' and owner = '''||V_ESQUEMA||'''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
	
		IF V_NUM_TABLAS = 1 THEN
			DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'.GPV_EXISTE_DOCUMENTO... Ya existe en la tabla');
		ELSE
			EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (GPV_EXISTE_DOCUMENTO NUMBER(1,0))';	
			DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'.GPV_EXISTE_DOCUMENTO... Creado');
		END IF;
