...
--## PRODUCTO=NO
--##
--## Finalidad: Crear tabla nueva ACT_CRE_CONFIG_RESIDENCIAL
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
  ...
  V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'ACT_CRE_CONFIG_RESIDENCIAL'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
  ...
  V_FK T_ARRAY_FK := T_ARRAY_FK(
      ...
      T_FK(''||V_ESQUEMA||'', ''||V_TEXT_TABLA||''   ,'DD_CRA_ID'            ,''||V_ESQUEMA||''    ,'DD_CRA_CARTERA'            ,'DD_CRA_ID'         ,'FK_ACT_CRE_DD_CRA_ID')
  );      
  V_TMP_FK T_FK;
...
BEGIN
	...
	IF V_NUM_TABLAS = 0 THEN
		...
		V_SQL:= ' CREATE TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ( 
		  CRE_ID                         NUMBER(16,0)   NOT NULL ENABLE,
		  DD_CRA_ID                      NUMBER(16,0)	NOT NULL,
		  CRE_PRECIO_PUBLI_MAX           NUMBER(16,2),
		  CRE_ULT_TASACION_MAX           NUMBER(16,2),
		...
		V_SQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' IS ''Tabla para configuracion de comisionamiento residencial.'' ';      
		EXECUTE IMMEDIATE V_SQL;
				
		-- Creamos comentarios columnas
		V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CRE_ID IS ''Código identificador único de configuración residencial.'' ';      
		EXECUTE IMMEDIATE V_SQL;		

		V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_CRA_ID IS ''Código identificador de diccionario del tipo de cartera.'' ';      
		EXECUTE IMMEDIATE V_SQL;	
		
		V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CRE_PRECIO_PUBLI_MAX IS ''Precio máximo de publicación.'' ';      
		EXECUTE IMMEDIATE V_SQL;

		V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CRE_ULT_TASACION_MAX IS ''Máximo del último valor de la tasación.'' ';      
		EXECUTE IMMEDIATE V_SQL;
    ...
    ELSE
        ...
        EXECUTE IMMEDIATE 'ALTER TABLE '||V_TEXT_TABLA||' 
            ADD (CONSTRAINT PK_'||V_TEXT_TABLA||' PRIMARY KEY (CRE_ID))
        ';               
    END IF;    
