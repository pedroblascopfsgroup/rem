...
--## PRODUCTO=NO
--## Finalidad: DDL Creación de la tabla ACT_HAG_HIST_ADECUACION_GENCAT
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(50 CHAR) := 'ACT_HAG_HIST_ADECUA_GENCAT'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    VN_COUNT NUMBER(16); -- Vble. para consulta que valida la existencia de una secuencia.

 BEGIN

    -- ******** ACT_HAG_HIST_ADECUACION_GENCAT *******
    DBMS_OUTPUT.PUT_LINE('******** ACT_HAG_HIST_ADECUACION_GENCAT ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'... Comprobaciones previas'); 
    

    
    -- Comprobamos si existe la tabla  ACT_HAG_HIST_ADECUACION_GENCAT
    ...
    ELSE  
    	 --Creamos la tabla ACT_HAG_HIST_ADECUACION_GENCAT
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
               (HAG_ID                   NUMBER(16,0) NOT NULL PRIMARY KEY,
			    HCG_ID                   NUMBER(16,0) NOT NULL,
			    HAG_REFORMA              NUMBER(1),
			    HAG_IMPORTE              NUMBER(16,2),
			    HAG_FECHA_REVISION       DATE,
			    ...
			    FECHABORRAR              DATE,
                CONSTRAINT FK_HAG_HIST_HCG_HIST FOREIGN KEY (HCG_ID) REFERENCES '|| V_ESQUEMA ||'.ACT_HCG_HIST_COM_GENCAT (HCG_ID)
			  )';
