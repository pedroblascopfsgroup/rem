...
--## PRODUCTO=NO
--## Finalidad: DDL Creación de la tabla ACT_HNG_HIST_NOTIFICACION_GENCAT
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(50 CHAR) := 'ACT_HNG_HIST_NOTIF_GENCAT'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    VN_COUNT NUMBER(16); -- Vble. para consulta que valida la existencia de una secuencia.
   
    BEGIN

    -- ******** ACT_HNG_HIST_NOTIFICACION_GENCAT *******
    DBMS_OUTPUT.PUT_LINE('******** ACT_HNG_HIST_NOTIF_GENCAT ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'... Comprobaciones previas'); 
    
    -- Creacion Tabla ACT_HNG_HIST_NOTIFICACION_GENCAT
    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
               (HNG_ID                            NUMBER(16,0) NOT NULL PRIMARY KEY,
			    HCG_ID                            NUMBER(16,0) NOT NULL,
			    HNG_CHECK_NOTIFICACION            NUMBER(1,0),
                HNG_FECHA_NOTIFICACION            DATE,
                DD_NOG_ID                         NUMBER(16,0) NOT NULL,
                HNG_FECHA_SANCION_NOTIFICACION    DATE,
                HNG_FECHA_CIERRE_NOTIFICACION     DATE,
			    ...
			    FECHABORRAR                       DATE,
                CONSTRAINT FK_HNG_HIST_HCG_HIST FOREIGN KEY (HCG_ID) REFERENCES '|| V_ESQUEMA ||'.ACT_HCG_HIST_COM_GENCAT (HCG_ID),
                CONSTRAINT FK_HNG_HIST_DD_NOG_NOT FOREIGN KEY (DD_NOG_ID) REFERENCES '|| V_ESQUEMA ||'.DD_NOG_NOTIFICACION_GENCAT (DD_NOG_ID)
			  )';
    ...
    END IF;
    	...
    	--Secuencia para la tabla ACT_HNG_HIST_NOTIFICACION_GENCAT
