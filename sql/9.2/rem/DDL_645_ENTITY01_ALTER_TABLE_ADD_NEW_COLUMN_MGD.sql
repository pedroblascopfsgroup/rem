...
--## PRODUCTO=SI
--## 
--## Finalidad: Incluir campo PRO_ID en tabla MGD_MAPEO_GESTOR_DOC.
--##            
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  V_COL T_ARRAY_COL := T_ARRAY_COL(
      -- Recordatorios
  	  T_COL(''||V_ESQUEMA||'', 'MGD_MAPEO_GESTOR_DOC', 'PRO_ID'                   	  , 'NUMBER(16,2)'       	  , 'Propietario')
  ...
  V_FK T_ARRAY_FK := T_ARRAY_FK(
      ...
      T_FK(''||V_ESQUEMA||'','MGD_MAPEO_GESTOR_DOC'              , 'PRO_ID'              ,''||V_ESQUEMA||''    ,'ACT_PRO_PROPIETARIO'   			,'PRO_ID'      ,'FK_MGD_PRO')
 );      
  V_TMP_FK T_FK;
