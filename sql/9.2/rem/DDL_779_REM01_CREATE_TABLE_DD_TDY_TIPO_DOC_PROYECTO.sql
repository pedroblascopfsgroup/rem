...
--## PRODUCTO=NO
--##
--## Finalidad: DDL Creación de la tabla DD_TDY_TIPO_DOC
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(150 CHAR):= 'DD_TDY_TIPO_DOC_PROYECTO'; -- Vble. con el nombre de la tabla.
...
BEGIN
    ...
    -- Creacion Tabla DD_TDY_TIPO_DOC
    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||'
               (DD_TDY_ID NUMBER (16,0) 
				  ...
				  , DD_TDY_MATRICULA_GD VARCHAR2(20 CHAR)
				  , DD_TDY_VISIBLE NUMBER(1,0)
				  , CONSTRAINT PK_DD_TDY_ID PRIMARY KEY(DD_TDY_ID)
			  )';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.'||V_TABLA||'... Tabla creada');
		
		-- execute immediate 'grant select, insert, delete, update, REFERENCES(DD_TDY_ID) on ' || V_ESQUEMA || '.'||V_TABLA||' to '||V_ESQUEMA||'';
    ...
    -- Creamos comentario	
	V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla para gestionar los tipos de documentos de proyecto.''';		
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TABLA||'... Comentario creado sobre la tabla.');

	-- Comentarios sobre las columnas
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_TDY_ID IS ''Código identificador único de tipo de documento de proyecto.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_TDY_CODIGO IS ''Código único de tipo de documento de proyecto.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_TDY_DESCRIPCION IS ''Descripción breve del tipo de documento de proyecto.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_TDY_DESCRIPCION_LARGA IS ''Descripción larga del tipo de documento de proyecto.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_TDY_MATRICULA_GD IS ''Matrícula del tipo de documento de proyecto.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_TDY_VISIBLE IS ''Visibilidad del tipo de documento de proyecto.''';
