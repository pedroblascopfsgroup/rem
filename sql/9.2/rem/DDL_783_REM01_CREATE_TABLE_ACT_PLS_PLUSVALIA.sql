    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||'
               (ACT_PLS_ID NUMBER (16,0) 
				  ...
				  , CONSTRAINT '||V_FK||'_GPV_ID FOREIGN KEY (GPV_ID) REFERENCES '||V_ESQUEMA_M||'.DD_SIN_SINO (DD_SIN_ID) ON DELETE SET NULL
			  )';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.'||V_TABLA||'... Tabla creada');
		
		-- execute immediate 'grant select, insert, delete, update, REFERENCES(ACT_PLS_ID) on ' || V_ESQUEMA || '.'||V_TABLA||' to '||V_ESQUEMA||'';
