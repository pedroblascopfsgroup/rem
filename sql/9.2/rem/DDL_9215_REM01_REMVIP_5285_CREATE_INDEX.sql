...
DECLARE
    ...
    BEGIN
				...
				V_MSQL := 'SELECT COUNT(1) FROM ALL_INDEXES WHERE INDEX_NAME = ''GPL_GASTOS_PRINEX_LBK_IDX_02'' ';
        
				EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
				IF V_NUM_TABLAS = 0 THEN
						V_MSQL := 'CREATE INDEX '||V_ESQUEMA||'.GPL_GASTOS_PRINEX_LBK_IDX_02 ON '||V_ESQUEMA||'.GPL_GASTOS_PRINEX_LBK ( GPV_ID, GPL_PROYECTO ) TABLESPACE '||V_TABLESPACE_IDX;		
						EXECUTE IMMEDIATE V_MSQL;
						DBMS_OUTPUT.PUT_LINE('[INFO] Indice GPL_GASTOS_PRINEX_LBK_IDX_02 - creado.');
				ELSE
						DBMS_OUTPUT.PUT_LINE('[INFO] Ya existe el índice GPL_GASTOS_PRINEX_LBK_IDX_02 ');
				END IF;
