...
--## INCIDENCIA_LINK=REMVIP-5695
--## PRODUCTO=NO
--## Finalidad: Añadimos a la tabla H_ASPRO_12_INMOVILIZADO + ASPRO_12_INMOVILIZADO el campo NGASTO
--##           
...
BEGIN
	...
	V_COLUMN := 'NGASTO'; 
    	V_NEW_TYPE  := 'VARCHAR2(16 CHAR)'; 

	V_TABLA := 'ASPRO_12_INMOVILIZADO';
	...
	IF V_NUM_TABLAS = 1 THEN	
		...
		IF V_NUM_TABLAS = 1 THEN
			DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'.' || V_COLUMN ||'... Columna ya existe');
		ELSE
			EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD ' || V_COLUMN ||' ' || V_NEW_TYPE ||' ';	

			EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.'|| V_COLUMN || ' IS ''NÚMERO DEL GASTO DE HAYA''';

			DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'.'|| V_COLUMN ||'... Creado');
		END IF;
	...
	V_TABLA := 'H_ASPRO_12_INMOVILIZADO';
