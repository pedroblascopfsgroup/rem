...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'HREOS-5530'; -- USUARIOCREAR/USUARIOMODIFICAR.
    V_ECO_ID NUMBER(16); 
    
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] ALTER TABLA TMP_FUN_PEF');

	-- Crear la columna GTOPOSTV
	DBMS_OUTPUT.PUT_LINE('[INFO] Se va a añadir la columna GTOPOSTV.');
	
	V_MSQL := 'SELECT COUNT(*) 
				FROM USER_TAB_COLS
				WHERE COLUMN_NAME = ''GTOPOSTV''
