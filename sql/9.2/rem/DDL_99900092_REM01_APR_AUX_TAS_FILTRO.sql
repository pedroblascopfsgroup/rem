...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'APR_AUX_TAS_FILTRO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla auxiliar para almacenar la información de la tabla de tasaciones procedente de BD REM.'; -- Vble. para los comentarios de las tablas
...
--	DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.S_'||V_TEXT_TABLA||'... Secuencia creada');
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.FEC_ULTIMA_TASACION IS ''Fecha ultima tasacion ''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.IMPORTE_TASACION IS ''Importe tasacion ''';
