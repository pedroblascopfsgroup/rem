...
--## INCIDENCIA_LINK=HREOS-1845
--## PRODUCTO=NO
--## Finalidad: Poner DEFAULT a dos columnas de la tabla CEX
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    			-- NOMBRE CAMPO						TIPO CAMPO									DESCRIPCION							DEFAULT
    	T_ALTER(  'BORRADO',		 				'NUMBER(1,0)',								'Borrado.',							'0'	),
    	T_ALTER(  'CEX_TITULAR_CONTRATACION',		'NUMBER(1,0)',								'Bit que indica si es titular',		'0')
		);
    V_T_ALTER T_ALTER;
...
BEGIN
	...
	LOOP
		...
		IF V_NUM_TABLAS = 1 THEN
			...
			V_MSQL := 'ALTER TABLE  '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (DUMMY '||V_T_ALTER(2)||')';
			...
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' MODIFY ('||V_T_ALTER(1)||' '||V_T_ALTER(2)||' DEFAULT ' ||V_T_ALTER(4)||') ';
			EXECUTE IMMEDIATE V_MSQL;
			...
			-- Rellenamos los nulls con el valor por defecto
			V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' SET '||V_T_ALTER(1)||' = '||V_T_ALTER(4)||' WHERE '||V_T_ALTER(1)||' IS NULL';
			EXECUTE IMMEDIATE V_MSQL;
