...
DECLARE
    ...
    V_COMMENT_TABLE VARCHAR2( 500 CHAR):= 'Tabla asignación para un activo de UVEM, el número activo HAYA.'; -- Vble. para los comentarios de las tablas
...
--	END IF; 
	...
	(
		...
		ACT_NUM_ACTIVO		NUMBER(16,0) 		NOT NULL ENABLE, 
		FEC_ASIGNACION		DATE,
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.ACT_NUM_ACTIVO_UVEM_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(ACT_NUM_ACTIVO_UVEM) TABLESPACE '||V_TABLESPACE_IDX;		
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.ACT_NUM_ACTIVO_UVEM_PK... Indice creado.');	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT ACT_NUM_ACTIVO_UVEM_PK PRIMARY KEY (ACT_NUM_ACTIVO_UVEM) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.ACT_NUM_ACTIVO_UVEM_PK ... PK creada.');	
