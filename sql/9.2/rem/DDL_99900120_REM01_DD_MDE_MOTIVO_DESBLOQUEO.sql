...
--## PRODUCTO=NO
--## Finalidad: Tabla diccionario para el tipo de MOTIVO DE DESBLOQUEO
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(30 CHAR) := 'DD_MDE_MOTIVO_DESBLOQUEO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	(
		DD_MDE_ID           			NUMBER(16)                  NOT NULL,
		DD_MDE_CODIGO        			VARCHAR2(20 CHAR)          	NOT NULL,
		DD_MDE_DESCRIPCION				VARCHAR2(100 CHAR),
		DD_MDE_DESCRIPCION_LARGA		VARCHAR2(250 CHAR),
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(DD_MDE_ID) TABLESPACE '||V_TABLESPACE_IDX;		
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');


	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (DD_MDE_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MDE_ID IS ''ID único del registro del diccionario''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MDE_CODIGO IS ''Código único del registro del diccionario''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MDE_DESCRIPCION IS ''Descripción del registro del diccionario''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MDE_DESCRIPCION_LARGA IS ''Descripción larga del registro del diccionario''';
