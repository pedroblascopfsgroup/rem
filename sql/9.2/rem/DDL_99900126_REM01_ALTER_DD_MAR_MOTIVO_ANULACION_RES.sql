...
--## PRODUCTO=NO
--## Finalidad: Alterar la tabla diccionario para los motivos de anulación de la reserva.
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(30 CHAR) := 'DD_MAR_MOTIVO_ANULACION_RES'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla diccionario para los motivos de anulación de la reserva.'; -- Vble. para los comentarios de las tablas.
...
BEGIN
	...
	IF V_NUM_TABLAS = 1 THEN
		...
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MAR_ID IS ''ID único del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MAR_CODIGO IS ''Código único del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MAR_DESCRIPCION IS ''Descripción del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_MAR_DESCRIPCION_LARGA IS ''Descripción larga del registro del diccionario''';
