...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'APR_AUX_BIE_DATOS_REGISTRALES'; 
    V_TEXT_COLUMN VARCHAR2(2400 CHAR) := 'BIE_DREG_CRU';
...
BEGIN
    ...
    V_SQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE OWNER =  '''|| V_ESQUEMA ||''' AND TABLE_NAME = '''||V_TEXT_TABLA||''' AND COLUMN_NAME =  '''||V_TEXT_COLUMN||'''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

    IF V_NUM_TABLAS = 0 THEN 
        V_MSQL := 'ALTER TABLE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' ADD '||V_TEXT_COLUMN||' VARCHAR2(20 CHAR)';
