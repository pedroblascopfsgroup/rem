...
BEGIN
	...
	ELSE 
		...
		(
			...
			AHP_CHECK_OCULTAR_PRECIO_V		NUMBER(1,0) DEFAULT 0 NOT NULL ENABLE,
			...
			AHP_CHECK_OCULTAR_PRECIO_A		NUMBER(1,0) DEFAULT 0 NOT NULL ENABLE,
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.AHP_CHECK_OCULTAR_PRECIO_V IS ''Check de publicar precio de la venta.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna AHP_CHECK_OCULTAR_PRECIO_V creado.');
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.AHP_CHECK_OCULTAR_PRECIO_A IS ''Check de publicar precio del alquilera.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna AHP_CHECK_OCULTAR_PRECIO_A creado.');
