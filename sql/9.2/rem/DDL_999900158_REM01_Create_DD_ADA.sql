...
--## PRODUCTO=NO 
--##
--## Finalidad:  Creación tabla diccionario DD_ADA_ADECUACION_ALQUILER         
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'DD_ADA_ADECUACION_ALQUILER'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	ELSE 
		...
		(
			DD_ADA_ID           			NUMBER(16, 0) NOT NULL,
			DD_ADA_CODIGO        			VARCHAR2(20 CHAR) NOT NULL,
			DD_ADA_DESCRIPCION			VARCHAR2(100 CHAR),
			DD_ADA_DESCRIPCION_LARGA		VARCHAR2(250 CHAR),
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (DD_ADA_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' IS ''Tabla de estado de venta.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la tabla creado.');		

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ADA_ID IS ''Código identificador único de la adecuacion alquiler''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EPV_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ADA_CODIGO IS ''Código identificador único de la adecuacion alquiler''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EPV_CODIGO creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ADA_DESCRIPCION IS ''Descripción de la adecuacion alquiler''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EPV_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ADA_DESCRIPCION_LARGA IS ''Descripción larga de la adecuacion alquiler''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EPV_DESCRIPCION_LARGA creado.');	
