...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_H_EST_V_ACT.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
 ...
 BEGIN
	...
	IF V_COUNT = 0 THEN
		...
		V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
				  DD_EVA_ID NUMBER(16,0) NOT NULL
				, DD_EVA_CODIGO VARCHAR2(5 CHAR) NOT NULL
				, DD_EVA_DESCRIPCION VARCHAR2(100 CHAR) NOT NULL
				, DD_EVA_DESCRIPCION_LARGA VARCHAR2(250 CHAR) NOT NULL
				...
				, CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (DD_EVA_ID)
				)
			  ';
