...
--## PRODUCTO=NO
--## Finalidad: Crear tabla SCO_SCORING
--##           
...
DECLARE
    ...
    V_TEXT_TABLA_AUX VARCHAR2(2400 CHAR) := 'DD_ESS_ESTADO_SCORING'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla de SCO_SCORING  con el diccionario DD_ESS_ESTADO_SCORING.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	V_MSQL := 'CREATE TABLE ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'
   	...
   	(		 
		SCO_ID 			 NUMBER(16,0) NOT NULL, 
		SCO_MOTIVO_RECHAZO 	 VARCHAR2(100 CHAR), 
		SCO_ID_SOLICITUD 	 VARCHAR2(40 CHAR), 
		SCO_EN_REVISION 	 NUMBER(1,0), 
		DD_ESS_ID 		 NUMBER(16,0),
		SCO_COMENTARIOS		 CLOB,
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(SCO_ID) TABLESPACE '||V_TABLESPACE_IDX;			
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (SCO_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... PK creada.');
	
	-- Creamos foreing key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_'||V_TEXT_TABLA||' FOREIGN KEY (DD_ESS_ID) REFERENCES '||V_ESQUEMA|| '.' ||V_TEXT_TABLA_AUX|| ' (DD_ESS_ID))';
	EXECUTE IMMEDIATE V_MSQL;
	...
	-- Creamos comentarios columnas
    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SCO_ID IS ''Código identificador único de la publicación.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;
	

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SCO_MOTIVO_RECHAZO IS ''Descripción del motivo Rechazo.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;	

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SCO_ID_SOLICITUD IS ''Código identificador único de la solicitud.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SCO_EN_REVISION IS ''Descripción de la revisión.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ESS_ID IS ''FK de la tabla DD_ESS_ESTADO_SCORING.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SCO_COMENTARIOS IS ''Comentarios.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;
