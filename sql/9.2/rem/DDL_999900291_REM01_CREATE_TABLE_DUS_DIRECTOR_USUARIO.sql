...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla de relación de usuarios y directores de equipo
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(27 CHAR) := 'DUS_DIRECTOR_USUARIO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_USUARIO VARCHAR2(32 CHAR) := 'REMVIP-1768';
 ...
 V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
 			   DUS_ID NUMBER(16,0) NOT NULL
 			 , USU_ID NUMBER(16,0) NOT NULL
		         , USU_DIR_ID NUMBER(16,0) NOT NULL
			 ...
			 , CONSTRAINT DUS_ID_PK PRIMARY KEY (DUS_ID)
			 , CONSTRAINT FK_USU_ID
		       FOREIGN KEY (USU_ID)
	           REFERENCES '||V_ESQUEMA_M||'.USU_USUARIOS(USU_ID)
			 , CONSTRAINT FK_USU_DIR_ID
			 FOREIGN KEY (USU_DIR_ID)
			 REFERENCES '||V_ESQUEMA_M||'.USU_USUARIOS(USU_ID)
			)
		  ';
