...
--## PRODUCTO=NO
--## Finalidad: DDL Creación de la tabla DD_TCO_OKU_TIPO_ACTUACION
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(30 CHAR) := 'DD_TCO_OKU_TIPO_ACTUACION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.

    BEGIN

    -- ******** DD_TCO_OKU_TIPO_ACTUACION *******
    DBMS_OUTPUT.PUT_LINE('******** '||V_TEXT_TABLA||' ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'... Comprobaciones previas'); 
    
    -- Creacion Tabla DD_TCO_OKU_TIPO_ACTUACION
    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
               (DD_TCO_ID NUMBER (16,0) NOT NULL ENABLE
				  , DD_TCO_CODIGO VARCHAR2(10 CHAR) NOT NULL ENABLE
				  , DD_TCO_DESCRIPCION VARCHAR2(50 CHAR) NOT NULL ENABLE
				  , DD_TCO_DESCRIPCION_LARGA VARCHAR2(250 CHAR) NOT NULL ENABLE
		...
		--Comprobamos si existen PK de esa tabla
    		...
    		ELSE
			EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (DD_TCO_ID)';
		...
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TCO_ID IS ''ID único del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TCO_CODIGO IS ''Código único del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TCO_DESCRIPCION IS ''Descripción del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TCO_DESCRIPCION_LARGA IS ''Descripción larga del registro del diccionario''';
