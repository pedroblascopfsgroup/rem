...
--## PRODUCTO=NO
--## Finalidad: Crear tabla  SRE_SEGURO_RENTAS
--##           
...
DECLARE
    ...
    V_TEXT_TABLA_AUX VARCHAR2(2400 CHAR) := 'DD_ESR_ESTADO_SEGURO_RENTAS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla de SRE_SEGURO_RENTAS  con el diccionario DD_ESR_ESTADO_SEGURO_RENTAS.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	V_MSQL := 'CREATE TABLE ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'
   	...
   	(		 
		SRE_ID				 NUMBER(16,0) NOT NULL, 
		SRE_MOTIVO_RECHAZO 		 VARCHAR2(100 CHAR), 
		SRE_ASEGURADORAS 		 VARCHAR2(100 CHAR), 
		SRE_EN_REVISION 		 NUMBER(1,0), 
		DD_ESR_ID 			 NUMBER(16,0),
		SRE_EMAIL_POLIZA_ASEGURADORA	 VARCHAR2(100 CHAR),
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(SRE_ID) TABLESPACE '||V_TABLESPACE_IDX;			
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (SRE_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... PK creada.');
	
	-- Creamos foreing key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_'||V_TEXT_TABLA||' FOREIGN KEY (DD_ESR_ID) REFERENCES '||V_ESQUEMA|| '.' ||V_TEXT_TABLA_AUX|| ' (DD_ESR_ID))';
	EXECUTE IMMEDIATE V_MSQL;
	...
	-- Creamos comentarios columnas
    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SRE_ID IS ''Código identificador único de la publicación.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SRE_MOTIVO_RECHAZO IS ''Descripción del motivo Rechazo.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SRE_ASEGURADORAS IS ''Descripción de las aseguradoras.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SRE_EN_REVISION IS ''Check de revisión.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ESR_ID IS ''FK con la tabla DD_ESR_ESTADO_SEGURO_RENTAS.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;

    	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.SRE_EMAIL_POLIZA_ASEGURADORA IS ''Email de la poliza aseguradora.'' ';      
    	EXECUTE IMMEDIATE V_MSQL;
