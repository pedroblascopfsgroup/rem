...
--## PRODUCTO=NO
--## Finalidad: Columna nueva en DD_PLA_PLAZAS
--##           
--## INSTRUCCIONES: Inserta la columna DD_TPI_ID en la tabla CEX_COMPRADOR_EXPEDIENTE
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_COLUMN_NAME VARCHAR2(2400 CHAR) := 'DD_TPI_ID'; -- Vble. auxiliar para almacenar el nombre de la columna a añadir.
    ...
    T_FK(''||V_ESQUEMA||'', ''||V_TEXT_TABLA||''   ,'DD_TPI_ID'       ,''||V_ESQUEMA||''    ,'DD_TPI_TIPO_INQUILINO'    ,'DD_TPI_ID'         ,'FK_DD_TPI_ID')
  );      
  V_TMP_FK T_FK;
...
BEGIN
    ...
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_COLUMN_NAME||' IS ''Campo que relaciona DD_TPI_ID con DD_TPI_TIPO_INQUILINO.'' ';      
