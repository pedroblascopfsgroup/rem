...
--## INCIDENCIA_LINK=HREOS-4367
--## PRODUCTO=NO
--## Finalidad: Columna nueva en SRE_SEGURO_RENTAS
--##           
--## INSTRUCCIONES: Inserta la columna ECO_ID en la tabla SRE_SEGURO_RENTAS
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_COLUMN_NAME VARCHAR2(2400 CHAR) := 'ECO_ID'; -- Vble. auxiliar para almacenar el nombre de la columna a añadir.
    ...
    T_FK(''||V_ESQUEMA||'', ''||V_TEXT_TABLA||''   ,'ECO_ID'       ,''||V_ESQUEMA||''    ,'ECO_EXPEDIENTE_COMERCIAL'    ,'ECO_ID'         ,'FK_ECO_ID')
  );      
  V_TMP_FK T_FK;
...
BEGIN
    ...
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_COLUMN_NAME||' IS ''Campo que relaciona ECO_ID con ECO_EXPEDIENTE_COMERCIAL.'' ';      
