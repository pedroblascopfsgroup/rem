...
--## FECHA_CREACION=20181207
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.2
--## INCIDENCIA_LINK=HREOS-4961
--## PRODUCTO=NO
--## Finalidad: Columna nueva en ECO_EXPEDIENTE_COMERCIAL
--##           
--## INSTRUCCIONES: Inserta la columna DD_MRE_ID en la tabla ECO_EXPEDIENTE_COMERCIAL
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_COLUMN_NAME VARCHAR2(2400 CHAR) := 'DD_MRE_ID'; -- Vble. auxiliar para almacenar el nombre de la columna a añadir.
    ...
    T_FK(''||V_ESQUEMA||'',''||V_TEXT_TABLA||'',''||V_COLUMN_NAME||'',''||V_ESQUEMA||''  ,'DD_MRE_MOTIVO_RECHAZO_EXP'    ,'DD_MRE_ID'       ,'FK_ECO_DD_MRE_ID')
  );      
  V_TMP_FK T_FK;
...
BEGIN
    ...
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_COLUMN_NAME||' IS ''Motivo de rechazo del expediente.'' ';      
