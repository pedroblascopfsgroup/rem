...
BEGIN	
	...
	-- Crear la columna SUPERUSUARIO
	DBMS_OUTPUT.PUT_LINE('[INFO] Se va a añadir la columna SUPERUSUARIO.');
	
	V_MSQL := 'SELECT COUNT(*) 
				FROM USER_TAB_COLS
				WHERE COLUMN_NAME = ''SUPERUSUARIO''
				AND TABLE_NAME = ''TMP_FUN_PEF''';
	...
	IF V_NUM_FILAS = 0 THEN
		...
		IF V_NUM_FILAS = 0 THEN
			...
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.TMP_FUN_PEF
						ADD SUPERUSUARIO VARCHAR2(1 CHAR)';
