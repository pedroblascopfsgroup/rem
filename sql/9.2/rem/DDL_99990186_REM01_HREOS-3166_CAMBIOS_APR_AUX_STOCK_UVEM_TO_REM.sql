...
/*		COD_MUNICIPIO_REGISTRO	VARCHAR2(9 CHAR) ,*/
		COD_MUNICIPIO_REGISTRO	NUMBER(9) ,
...
/*		COD_CALIF_EFICI_ENERGETICA	NUMBER(8) ,*/
		COD_CALIF_EFICI_ENERGETICA	VARCHAR2(1 CHAR) ,
...
/*		SUBMOTIVO_OCULTACION	VARCHAR2(1 CHAR) ,*/
		SUBMOTIVO_OCULTACION	VARCHAR2(3 CHAR) ,
/*		INDICADOR_ACTIVO_PUBLICADO	VARCHAR2(1 CHAR) ,*/
		INDICADOR_ACTIVO_PUBLICADO	NUMBER(2) ,
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.FECEVT IS ''FEC_PRECIO_EVENTO''';
        ...
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.COCDIN IS ''DIRECCION NEGOCIO ACTIVO''';	   	
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.IDPROV IS ''Identificador de la gestoría de administración''';	
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.CASPRE IS ''Superficie real suelo parcela''';	
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.CASUCS IS ''Superficie real construida sobre rasante''';	
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.CASUCB IS ''Superficie real construida bajo rasante''';	
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.IMVLIQ IS ''Valor liquidativo''';	
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.IMVFSV IS ''Valor liquidatovio FSV''';
        EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.COACHA IS ''Identificador activo Haya''';
