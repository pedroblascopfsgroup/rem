...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('SPS_DISPONIBILIDAD_JURIDICA', 'NUMBER','(16,0)', 'Disponibilidad jurídica, solo aplica a cartera BANKIA. 0 - DESCONOCIDO; 3 - ACTA; 4 - SE DESCONOCE. SIN AUTO-TITULO')
    ); 
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP      
		...
		ELSE
       	...
       	END IF;
      END LOOP;
	--EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_DD_EEC_ID_ANT FOREIGN KEY (DD_EEC_ID_ANT) REFERENCES '||V_ESQUEMA||'.DD_EEC_EST_EXP_COMERCIAL(DD_EEC_ID)';
EXCEPTION
     WHEN OTHERS THEN
          err_num := SQLCODE;
          err_msg := SQLERRM;
