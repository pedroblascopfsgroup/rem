...
--## INCIDENCIA_LINK=HREOS-2233
--## PRODUCTO=No
--## Finalidad: Añadir columna ACT_BEX_SOLUCIONAR a la tabla ACT_BEX_BLOQ_EXP_FORMALIZAR
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'ACT_BEX_SOLUCIONAR',		'VARCHAR2(1 CHAR)',		'Campo que indica si un bloqueo se ha solucionado o no.')
		);
    V_T_ALTER T_ALTER;
...
BEGIN
	...
	LOOP
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] No existe la columna: ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'['||V_T_ALTER(1)||'('||V_T_ALTER(2)||')] . La creamos.');
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA|| ' 
					   ADD ('||V_T_ALTER(1)||' '||V_T_ALTER(2)||' DEFAULT 1 NOT NULL)
			';

			EXECUTE IMMEDIATE V_MSQL;
			--DBMS_OUTPUT.PUT_LINE('[1] '||V_MSQL);
			DBMS_OUTPUT.PUT_LINE('[INFO] ... Columna '||V_T_ALTER(1)||' creada en tabla '||V_ESQUEMA||'.'||V_TEXT_TABLA||', con tipo '||V_T_ALTER(2));
		 ...
		 ELSE  
		  	...
		  	--Existe la columna. Ampliamos el campo.
			DBMS_OUTPUT.PUT_LINE('[INFO] Existe la columna:' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'['||V_T_ALTER(1)||']. Ampliamos campo a: ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'['||V_T_ALTER(1)||'('||V_T_ALTER(2)||')]');
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA|| ' 
			...
			EXECUTE IMMEDIATE V_MSQL;
			--DBMS_OUTPUT.PUT_LINE('[1] '||V_MSQL);
			DBMS_OUTPUT.PUT_LINE('[INFO] ... Columna '||V_T_ALTER(1)||' modificada en tabla '||V_ESQUEMA||'.'||V_TEXT_TABLA||', con tipo '||V_T_ALTER(2));
