...
--## PRODUCTO=NO
--## Finalidad: Anyadir columna DD_SDE_TPD_ID en DD_SDE_SUBTIPO_DOC_EXP
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'DD_SDE_TPD_ID',			'NUMBER(16,0)',		'Campo que relaciona los tipos de documentos vinculables.'	)
...
BEGIN
	...
	LOOP
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			-- Creamos foreign key
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_SDE_TPD_DDTDOC FOREIGN KEY (DD_SDE_TPD_ID) REFERENCES '||V_ESQUEMA||'.DD_TPD_TIPO_DOCUMENTO (DD_TPD_ID) ON DELETE SET NULL)';
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_SDE_TPD_TIPO_DOCUMENTO ... Foreign key creada.');
		END IF;
