...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'DD_ODT_ID',			'NUMBER(16,0)',						'Id Origen Dato de la Carga.'	)
...
BEGIN
	...
	LOOP
		...
		IF V_NUM_TABLAS = 1 THEN
			DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.FK_CRG_DD_ODT_ID... Ya existe. No hacemos nada.');		
		ELSE
			-- Creamos foreign key FK_COE_DD_ETI_ID
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_CRG_DD_ODT_ID FOREIGN KEY (DD_ODT_ID) REFERENCES '||V_ESQUEMA||'.DD_ODT_ORIGEN_DATO (DD_ODT_ID) ON DELETE SET NULL)';
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_CRG_DD_ODT_ID... Foreign key creada.');
		END IF;
	END LOOP;
