...
DECLARE
    ...
    C_CAMPOS T_ARRAY_CAMPOS := T_ARRAY_CAMPOS(
        T_CAMPOS('OFR_OFERTAS','OFR_OFERTA_EXPRESS','DEFAULT (0) NOT NULL ENABLE')
    );
    ...
    FOR I IN C_CAMPOS.FIRST .. C_CAMPOS.LAST
        LOOP
            ...
            IF V_EXISTS = 1 THEN
                ...
                IF V_EXISTS = 1 THEN
                    --MODIFICAMOS COLUMNA
                    V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEMP_CAMPOS(1)||' SET '||V_TEMP_CAMPOS(2)||' = 0 WHERE '||V_TEMP_CAMPOS(2)||' IS NULL';
