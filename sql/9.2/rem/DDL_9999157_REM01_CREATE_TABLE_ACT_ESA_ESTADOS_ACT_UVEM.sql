...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla ACT_ESA_ESTADOS_ACT_UVEM
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(27 CHAR) := 'ACT_ESA_ESTADOS_ACT_UVEM'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
 ...
 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
  ...
  -- Creamos indice.
  V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TABLA||'(ESA_ID) TABLESPACE '||V_TABLESPACE_IDX;		
  EXECUTE IMMEDIATE V_MSQL;
  DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TABLA||'_PK... Indice creado.');
  
  -- Creamos indice.
  V_MSQL := 'CREATE UNIQUE INDEX ESA_ACT_ID_PK ON '||V_ESQUEMA|| '.'||V_TABLA||'(ACT_ID) TABLESPACE '||V_TABLESPACE_IDX;		
  EXECUTE IMMEDIATE V_MSQL;
  DBMS_OUTPUT.PUT_LINE('[INFO] ESA_ACT_ID_PK... Indice creado.');
  
  -- Creamos primary key.
  V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT ESA_ID_PK PRIMARY KEY (ESA_ID) USING INDEX)';
  EXECUTE IMMEDIATE V_MSQL;
  DBMS_OUTPUT.PUT_LINE('[INFO] ESA_ID_PK... PK creada.');
  
  -- Creamos unique key ACT_ID.
  V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT UK_ESA_ACT_ID UNIQUE (ACT_ID))';
  EXECUTE IMMEDIATE V_MSQL;
  DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.UK_ESA_ACT_ID... Unique key creada.');
  
  -- Creamos foreign key ACT_ID.
  V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT FK_ESA_ACT_ID FOREIGN KEY (ACT_ID) REFERENCES '||V_ESQUEMA||'.ACT_ACTIVO (ACT_ID))';
  EXECUTE IMMEDIATE V_MSQL;
  DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.FK_ESA_ACT_ID... Foreign key creada.');
  
  -- Creamos comentario de tabla.
  V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla que contiene los estados de los activos de UVEM''';		
  EXECUTE IMMEDIATE V_MSQL;
  DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TABLA||'... Comentario creado.');

  -- Creamos comentario de columnas.
  EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.ESA_ID IS ''ID del estado del activo UVEM.''';
