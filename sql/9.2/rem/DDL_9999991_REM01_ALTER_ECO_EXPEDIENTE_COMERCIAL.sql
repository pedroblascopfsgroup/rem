...
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla que contiene la información de un expediente comercial
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'DD_COA_ID',		 					'NUMBER(16,0)',						'Código único del comite de alquiler.')
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_ECO_DD_COA',						'DD_COA_ID',				V_ESQUEMA||'.DD_COA_COMITES_ALQUILER',				'DD_COA_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_TEXT_TABLA||'.DD_COS_ID IS ''Código único del comite sancionador de venta.''';
