...
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla SRE_HIS_SEGURO_RENTAS eliminando la columna DD_ESR_ID con su FK y crear la columna DD_REC_ID con su FK.
--##           
...
DECLARE
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_ECO_DD_REC',						'DD_REC_ID',				V_ESQUEMA||'.DD_REC_RESULTADO_CAMPO',				'DD_REC_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	-- Verificar si la FK ya existe. Si ya existe, se elimina
		V_MSQL := 'SELECT COUNT(1) FROM all_constraints WHERE OWNER = '''||V_ESQUEMA||''' and table_name = '''||V_TEXT_TABLA||''' and constraint_name = ''FK_SRE_HIS_RENTAS''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina.
			
			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' DROP CONSTRAINT FK_SRE_HIS_RENTAS ';
