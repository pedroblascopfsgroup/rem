...
DECLARE
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_SCO_HIS_DD_REC',						'DD_REC_ID',				V_ESQUEMA||'.DD_REC_RESULTADO_CAMPO',				'DD_REC_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	-- Verificar si la FK ya existe. Si ya existe, se elimina
		V_MSQL := 'SELECT COUNT(1) FROM all_constraints WHERE OWNER = '''||V_ESQUEMA||''' and table_name = '''||V_TEXT_TABLA||''' and constraint_name = ''FK_SCO_HIS_SCO''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina.
			
			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' DROP CONSTRAINT FK_SCO_HIS_SCO ';
