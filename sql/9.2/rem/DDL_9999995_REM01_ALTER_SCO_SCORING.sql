...
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla SCO_HIS_SCORING eliminando la columna DD_ESS_ID con su FK y crear la columna DD_REC_ID con su FK.
--##           
...
DECLARE
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_SCO_DD_REC',						'DD_REC_ID',				V_ESQUEMA||'.DD_REC_RESULTADO_CAMPO',				'DD_REC_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	-- Verificar si la FK ya existe. Si ya existe, se elimina
		V_MSQL := 'SELECT COUNT(1) FROM all_constraints WHERE OWNER = '''||V_ESQUEMA||''' and table_name = '''||V_TEXT_TABLA||''' and constraint_name = ''FK_SCO_SCORING''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina.
			
			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' DROP CONSTRAINT FK_SCO_SCORING ';
		...
		V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLS WHERE COLUMN_NAME = ''DD_ESS_ID'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina

			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' 
					   DROP COLUMN DD_ESS_ID';
