...
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla SCO_HIS_SCORING eliminando la columna DD_ESR_ID con su FK y crear la columna DD_REC_ID con su FK.
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'SRE_SEGURO_RENTAS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'DD_REC_ID',		 					'NUMBER(16,0)',						'FK a la tabla DD_REC_RESULTADO_CAMPO.')
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_SRE_DD_REC',						'DD_REC_ID',				V_ESQUEMA||'.DD_REC_RESULTADO_CAMPO',				'DD_REC_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	-- Verificar si la FK ya existe. Si ya existe, se elimina
		V_MSQL := 'SELECT COUNT(1) FROM all_constraints WHERE OWNER = '''||V_ESQUEMA||''' and table_name = '''||V_TEXT_TABLA||''' and constraint_name = ''FK_SRE_SEGURO_RENTAS''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina.
			
			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' DROP CONSTRAINT FK_SRE_SEGURO_RENTAS ';
		...
		V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLS WHERE COLUMN_NAME = ''DD_ESR_ID'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina

			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' 
					   DROP COLUMN DD_ESR_ID';
