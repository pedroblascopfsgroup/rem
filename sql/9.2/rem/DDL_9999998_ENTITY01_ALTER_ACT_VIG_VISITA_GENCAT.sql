...
--## INCIDENCIA_LINK=HREOS-5187
--## PRODUCTO=NO
--## Finalidad: añadir las columnas de T_ARRAY_ALTER a ACT_VIG_VISITA_GENCAT
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'ACT_VIG_VISITA_GENCAT'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'ID_LEAD_SF',		 					'VARCHAR2(500 CHAR)',			'ID de la visita en Salesforce'),
    	T_ALTER(  'FECHA_ENVIO_SOLICITUD',		 		'TIMESTAMP(6)',					'Fecha de envío de la visita a Salesforce'),
    	T_ALTER(  'FECHA_RECEPCION_ALTA',		 		'TIMESTAMP(6)',					'Fecha de recepción de la visita desde Webcom')
	);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK( /* EMPTY */ );
    V_T_FK T_FK;
...
BEGIN
	...
	-- El script creara las FK solo si esta activo el indicador de creacion FK.
		...
		LOOP
			...
			IF V_NUM_TABLAS = 0 THEN
				...
				V_MSQL := '
					...
					ADD CONSTRAINT '||V_T_FK(1)||' 
					FOREIGN KEY ( '||V_T_FK(2)||' )
					REFERENCES '||V_T_FK(3)||' ( '||V_T_FK(4)||' )
