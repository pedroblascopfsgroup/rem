...
DECLARE
  ...
  V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
			--	CRA		NIF		   PASS           NOMBRE_USU				    EMAIL          GRP	 PEF_COD	DESPACHO_EXTERNO
	T_TIPO_DATA('01','B54448097','tr1v57','TRISOLIA INVESTMENT, S.L.U.', 'jruiz@trisolia.es' , '0', 'HAYAPROV', 'REMPROV')
  ); 
       ...
       DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN USD_USUARIOS_DESPACHOS ');
		...
		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS 
			WHERE DES_ID = 
				...
				WHERE DES_DESPACHO = '''||TRIM(V_TMP_TIPO_DATA(8))||''') 
				AND USU_ID = 
		...
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS' ||
					...
					' (SELECT DES_ID FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = '''||TRIM(V_TMP_TIPO_DATA(8))||'''),' ||							
					' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_TIPO_DATA(2))||'''),' ||
					' 1,1,''HREOS-468'',SYSDATE,0 FROM DUAL';
