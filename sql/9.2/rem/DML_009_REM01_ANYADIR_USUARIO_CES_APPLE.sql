...
--## PRODUCTO=SI
--##
--## Finalidad: Insertar nuevo usuario para buzon Apple
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN
    ...
    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' WHERE USU_USERNAME = ''buzoncesapple'' AND BORRADO = 0';
    ...
    IF V_NUM_TABLAS = 0 THEN
        ...
        V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' (USU_ID, USU_USERNAME, USU_PASSWORD, USU_NOMBRE, USU_MAIL, USUARIOCREAR, FECHACREAR) 
          VALUES ('||V_ESQUEMA_M||'.S_USU_USUARIOS.NEXTVAL, ''buzoncesapple'', ''GULTicAtENtH'', ''Buzon CES Apple'', ''AppleAdvisoryNotes@cesaiberia.com'', '''||V_USUARIOMODIFICAR||''', SYSDATE)';
      DBMS_OUTPUT.PUT_LINE('[INFO] Insertando usuario ficticio buzoncesapple.......');
