...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('CARGA_MASIVA_ACTUALIZACION_FORMALIZACION',   'plantillas/plugin/masivo/MASIVO_FORMALIZACION.xls',  'FORM')
    );
      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.POP_PLANTILLAS_OPERACION (' ||
                      ...
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''' ,'''||V_TMP_TIPO_DATA(2)||''',(SELECT DD_OPM_ID FROM '|| V_ESQUEMA ||'.DD_OPM_OPERACION_MASIVA WHERE DD_OPM_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(3))||'''), 0, ''HREOS-6182'',SYSDATE,0 FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
