...
DECLARE
    ...
    V_USUARIOCREAR VARCHAR(40 CHAR); -- Usuario
    ...
    V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
      T_FUNCION('Precio: Carga de distribución de precios' , 'CARGA_DISTRIBUCION_PRECIOS')
    ); 
...
BEGIN	
    ...
    V_USUARIOCREAR := 'HREOS-7593';
    DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA_M||'.'||V_TABLA||'... Empezando a insertar datos');
        ...
        EXECUTE IMMEDIATE V_MSQL INTO V_ENTIDAD_ID;
			...
			ELSE		
				V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.'||V_TABLA||' (' ||
						...
						'0, '''||V_USUARIOCREAR||''',SYSDATE,0 FROM DUAL';
				DBMS_OUTPUT.PUT_LINE('INSERTANDO: '''||V_TMP_FUNCION(1)||''','''||TRIM(V_TMP_FUNCION(2))||'''');
				EXECUTE IMMEDIATE V_MSQL;
			END IF;
      END LOOP;
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('[FIN] '||V_ESQUEMA_M||'.'||V_TABLA||'... Datos insertados');
