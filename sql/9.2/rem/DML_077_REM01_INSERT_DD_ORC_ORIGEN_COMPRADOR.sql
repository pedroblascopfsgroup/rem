...
--## PRODUCTO=NO
--##
--## Finalidad: Insercion inicial DD_ORC_ORIGEN_COMPRADOR
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_AUX NUMBER(16);
    V_TABLA VARCHAR2(40 CHAR) := 'DD_ORC_ORIGEN_COMPRADOR'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-6197';
    ...
    (
		T_TIPO_DATA_2('01','HRE'),
		T_TIPO_DATA_2('02','API'),
		T_TIPO_DATA_2('03','Oficina'),
		T_TIPO_DATA_2('04','FDV')
	); 
  ...
  IF V_COUNT > 0 THEN
		...
		FOR I IN V_TIPO_DATA_2.FIRST .. V_TIPO_DATA_2.LAST
		  LOOP
			  ...
			  V_SQL := 'SELECT COUNT(1) FROM '||V_TABLA||' WHERE DD_ORC_CODIGO = '''||TRIM(V_TMP_TIPO_DATA_2(1))||'''';				                
