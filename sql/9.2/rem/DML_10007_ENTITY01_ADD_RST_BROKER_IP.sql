...
DECLARE
  ...
  V_COL_PRO T_ARRAY_COL_PRO := T_ARRAY_COL_PRO(
      -- Recordatorios
	T_COL_PRO('3.120.129.210', 'qdSD3vhRca',1, 1),
	T_COL_PRO('10.10.12.27', 'qdSD3vhRca',1, 1),
	T_COL_PRO('195.55.65.157', 'qdSD3vhRca',1, 1)
...
--Array que contiene los registros que se van a crear en entorno PREVIO
  ...
  V_COL_PRE T_ARRAY_COL_PRE := T_ARRAY_COL_PRE(
      -- Recordatorios
	T_COL_PRE('52.58.34.215', '0123456789',1, 1),
	T_COL_PRE('10.10.25.205', '0123456789',1, 1),
	T_COL_PRE('10.10.44.72', '0123456789',1, 1),
	T_COL_PRE('10.10.44.92', '0123456789',1, 1),
    	T_COL_PRE('10.10.44.132', '0123456789',1, 1)
...
BEGIN
		...
		V_TMP_COL_PRO := V_COL_PRO(I);
			...
			EXECUTE IMMEDIATE V_SQL INTO V_NUM_REGISTROS;
				...
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.RST_BROKER (RST_BROKER_ID, RST_IP, RST_KEY, RST_VALIDAR_FIRMA, RST_VALIDAR_TOKEN, USUARIOCREAR, FECHACREAR) 
				    VALUES ('||V_ESQUEMA||'.S_RST_BROKER.NEXTVAL, '''||TRIM(V_TMP_COL_PRO(1))||''', '''||TRIM(V_TMP_COL_PRO(2))||''', '''||TRIM(V_TMP_COL_PRO(3))||''' , '''||TRIM(V_TMP_COL_PRO(4))||''' , ''HREOS-6785'', SYSDATE)';
		...
		V_TMP_COL_PRE := V_COL_PRE(I);
			...
			EXECUTE IMMEDIATE V_SQL INTO V_NUM_REGISTROS;
				...
				-- Añadimos el registro
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.RST_BROKER (RST_BROKER_ID, RST_IP, RST_KEY, RST_VALIDAR_FIRMA, RST_VALIDAR_TOKEN, USUARIOCREAR, FECHACREAR) 
				    VALUES ('||V_ESQUEMA||'.S_RST_BROKER.NEXTVAL, '''||TRIM(V_TMP_COL_PRE(1))||''', '''||TRIM(V_TMP_COL_PRE(2))||''', '''||TRIM(V_TMP_COL_PRE(3))||''' , '''||TRIM(V_TMP_COL_PRE(4))||''' , ''HREOS-6785'', SYSDATE)';
