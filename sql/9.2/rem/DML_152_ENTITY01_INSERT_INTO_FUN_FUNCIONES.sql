...
--## PRODUCTO=NO
--##
--## Finalidad:	Añade en la tabla FUN_FUCIONES una nueva función
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
  ...
  V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA( 		
    ...
    T_TIPO_DATA('Autorizar tramitación oferta', 'AUTORIZAR_TRAMITACION_OFERTA')
  ); 
...
BEGIN	
  ...
  V_SQL := 'SELECT COUNT(1) FROM ALL_TABLES WHERE TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA_M||'''';
  EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
  
  IF V_NUM_TABLAS = 1 THEN 

    -- LOOP para insertar o modificar los valores en ACT_GES_DIST_GESTORES -----------------------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN '||V_ESQUEMA_M||'.'||V_TEXT_TABLA);
    
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
    LOOP
      V_TMP_TIPO_DATA := V_TIPO_DATA(I);
      --Comprobamos si los datos ya existen
      V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' 
      WHERE FUN_DESCRIPCION_LARGA = '''||TRIM(V_TMP_TIPO_DATA(1))||''' 
      AND FUN_DESCRIPCION = '''||TRIM(V_TMP_TIPO_DATA(2))||''' 
      AND BORRADO = 0';
      ...
      IF V_NUM_LINES = 0 THEN
        ...
        DBMS_OUTPUT.PUT_LINE('[INFO]: INSERT CORRECTO PARA FUN: '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
      ELSE
