...
--##		0.3 Daniel Albert: Añadimos la validación de cuenta contable y partida presupuestaria.
--##########################################
--*/
...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        ...
        T_TIPO_DATA('F02' ,'Llega alta de gasto que ya existe', '1', 'WHERE AUX.TIPO_ENVIO like ''''01'''' AND EXISTS (SELECT * FROM GPV_GASTOS_PROVEEDOR GPV WHERE GPV.GPV_NUM_GASTO_GESTORIA = AUX.COD_GASTO_GESTORIA AND GPV.BORRADO = 0 AND GPV.DD_GRF_ID = #TOKEN_IDGESTORIA#)'),
        ...
        T_TIPO_DATA('F08' ,'Llega anulación con datos diferentes al alta', '1' ,'JOIN GPV_GASTOS_PROVEEDOR GPV ON GPV.GPV_NUM_GASTO_GESTORIA = AUX.COD_GASTO_GESTORIA AND GPV.DD_GRF_ID = #TOKEN_IDGESTORIA# JOIN GDE_GASTOS_DETALLE_ECONOMICO GDE ON GDE.GPV_ID = GPV.GPV_ID JOIN GPV_ACT GA ON GPV.GPV_ID = GA.GPV_ID JOIN ACT_ACTIVO ACT ON ACT.ACT_ID = GA.ACT_ID WHERE AUX.TIPO_ENVIO LIKE ''''04'''' AND  (AUX.COD_ACTIVO       = ACT.ACT_NUM_ACTIVO or AUX.PRINCIPAL = GDE.GDE_PRINCIPAL_NO_SUJETO or AUX.RECARGO = GDE.GDE_RECARGO or AUX.INT_DEMORA = GDE.GDE_INTERES_DEMORA or AUX.COSTAS = GDE.GDE_COSTAS or AUX.OTROS_INCREMENTOS = GDE.GDE_OTROS_INCREMENTOS or AUX.PROVISIONES_Y_SUPL = GDE.GDE_PROV_SUPLIDOS or AUX.IMPORTE_PAGADO  = GDE.GDE_IMPORTE_PAGADO)'),
        ...
        /*T_TIPO_DATA('F10' ,'El propietario no existe' , '1' ,'' ),destinatario del gasto no existe, o ese destinatario no corresponde con el activo.*/
		...
		/*“Tipo de operación” abono y no informa gasto abonadoT_TIPO_DATA('F15' ,'Tipo de operación” abono y no informa gasto abonado', '1' ,'' ),*/
