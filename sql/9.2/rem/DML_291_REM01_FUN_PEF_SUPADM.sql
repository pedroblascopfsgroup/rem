...
DECLARE
    ...
    CURSOR FUNCIONES IS
    	SELECT DISTINCT FUN.FUN_ID AS FUN FROM REMMASTER.FUN_FUNCIONES FUN 
    		JOIN REM01.FUN_PEF FP ON FP.FUN_ID = FUN.FUN_ID 
    		JOIN REM01.PEF_PERFILES PEF ON PEF.PEF_ID = FP.PEF_ID
    		WHERE PEF.PEF_CODIGO = 'HAYAGESTADM' AND FUN.FUN_ID NOT IN (
		        SELECT FUN2.FUN_ID FROM REMMASTER.FUN_FUNCIONES FUN2 
    				...
    				WHERE PEF2.PEF_CODIGO = 'HAYASUPADM'
    			) ORDER BY FUN.FUN_ID;
      ...
      LOOP
		...
		DBMS_OUTPUT.PUT_LINE('INSERTANDO PROCURADOR: ' || FILA.FUN);
				...
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.FUN_PEF' ||
						...
						' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''HAYASUPADM''),' || FILA.FUN ||
