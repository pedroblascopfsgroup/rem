...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('1','CONSUSAREB' , 'CONSULTA', 'SAREB', '', '0', 'Sareb2015'),
    	T_TIPO_DATA('1','CONSUCAJAM' , 'CONSULTA', 'CAJAMAR', '', '0', 'Cajamar2015'),
    	T_TIPO_DATA('1','CONSUALL' , 'CONSULTA', 'ALL', '', '0', 'All2015')
    	); 
      ...
      LOOP
        ...
        ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.USU_USUARIOS (' ||
                      'USU_ID, ENTIDAD_ID, USU_USERNAME, USU_PASSWORD, USU_NOMBRE, USU_APELLIDO1, USU_APELLIDO2, USU_GRUPO, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(7))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', '''||TRIM(V_TMP_TIPO_DATA(4))||''', '''||TRIM(V_TMP_TIPO_DATA(5))||''', '''||TRIM(V_TMP_TIPO_DATA(6))||''',''DML'',SYSDATE,0 FROM DUAL';
