...
DECLARE
    ...
    (
		T_TIPO_DATA_2('6', 'comboboxinicial', 'comboTarifaPlana', 'Tarifa Plana', 'DDSiNo'),
		T_TIPO_DATA_2('6', 'label', 'huecoTP', '', ''),
		T_TIPO_DATA_2('7', 'textarea', 'observaciones', '', ''),
		T_TIPO_DATA_2('8', 'elctrabajo', 'GESECO', '', '')
	); 
    V_TMP_TIPO_DATA_2 T_TIPO_DATA_2;
    
BEGIN	
	
	V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''T004_AnalisisPeticion'')';
		...
		LOOP
			...
			IF V_NUM_TABLAS > 0 THEN
                ...
                V_MSQL := 'UPDATE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS '||
						  ...
						  '    ,USUARIOMODIFICAR = ''HREOS-6143'' ,FECHAMODIFICAR = SYSDATE '||
						  ' WHERE TFI_NOMBRE = '''||V_TMP_TIPO_DATA_2(3)||''' AND TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''T004_AnalisisPeticion'') 
		        ';
