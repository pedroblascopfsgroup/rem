...
DECLARE
    ...
    TYPE T_TIPO_DATA_2 IS TABLE OF VARCHAR2(4000);
    TYPE T_ARRAY_DATA_2 IS TABLE OF T_TIPO_DATA_2;
    V_TIPO_DATA_2 T_ARRAY_DATA_2 := T_ARRAY_DATA_2
    (    
      T_TIPO_DATA_2('2','comboboxinicial', 'comboConflicto', 'Conflicto de intereses', '','false'),
      T_TIPO_DATA_2('3','comboboxinicial', 'comboRiesgo', 'Riesgo reputacional', '',''),
      T_TIPO_DATA_2('0','label','titulo', '', '',''),
      T_TIPO_DATA_2('1','comboboxreadonly', 'comite', 'Comité sancionador', '','false'),
      T_TIPO_DATA_2('4','datefield', 'fechaEnvio', 'Fecha de envío', '',''),
      T_TIPO_DATA_2('4','textfield', 'numExpediente', 'Nº Expediente origen', '',''),
      T_TIPO_DATA_2('6','textarea', 'observaciones', 'Observaciones', '',''),
      T_TIPO_DATA_2('5','comboboxinicial', 'comiteSuperior', 'Aplica comité superior', '',''),
      T_TIPO_DATA_2('1','numberfield', 'importeTotalOfertaAgrupada', 'Importe Total Oferta Agrupada', '',''),
      T_TIPO_DATA_2('7','label', 'huecoVenta', '', '',''),
      T_TIPO_DATA_2('2','comboboxreadonly', 'comitePropuesto', 'Comité Propuesto', 'DDComiteSancion','false'),
      T_TIPO_DATA_2('3','numberfield', 'numOfertaPrincipal', 'Núm. Oferta Agrupada (Principal)', '','')
...
BEGIN	
	...
	V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''T013_DefinicionOferta'')';
		...
		LOOP
			...
			IF V_NUM_TABLAS > 0 THEN
                ...
                V_MSQL := 'UPDATE '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS 
						   SET 
						  TFI_ORDEN = '''||V_TMP_TIPO_DATA_2(1)||''',
						  USUARIOMODIFICAR = ''HREOS-7007'',
						  FECHAMODIFICAR = SYSDATE 
						   WHERE TFI_NOMBRE = '''||V_TMP_TIPO_DATA_2(3)||''' AND TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''T013_DefinicionOferta'') 
		        ';
			...
			ELSE
				...
				V_MSQL :=  'INSERT INTO '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS 
							...
							'''||V_TMP_TIPO_DATA_2(6)||'''		AS TFI_VALIDACION,
							'''||V_TMP_TIPO_DATA_2(5)||''' 		AS TFI_BUSINESS_OPERATION,
							0 									AS VERSION,
							''HREOS-7007'' 						AS USUARIOCREAR,
