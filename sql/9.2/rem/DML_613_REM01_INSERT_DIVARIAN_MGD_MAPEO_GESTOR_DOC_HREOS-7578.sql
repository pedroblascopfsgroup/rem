...
--## PRODUCTO=NO
--##
--## Finalidad: Script que replica los valores de la cartera Apple para la cartera Divarian en la tabla MGD_MAPEO_GESTOR_DOC.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(30 CHAR) := 'MGD_MAPEO_GESTOR_DOC'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN   
    ...
    V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (MGD_ID, DD_CRA_ID, DD_SCR_ID, CLIENTE_GD, 
				VERSION, USUARIOCREAR,	FECHACREAR, BORRADO, PRO_ID, CLIENTE_WS)
				SELECT S_CCC_CONFIG_CTAS_CONTABLES.NEXTVAL, MGD.DD_CRA_ID, '||V_ID_SCR_DIVARIAN||', MGD.CLIENTE_GD, 
				0, ''HREOS-7578'',	SYSDATE, 0, MGD.PRO_ID, MGD.CLIENTE_WS
				FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' MGD WHERE MGD.DD_SCR_ID = '||V_ID_SCR_APPLE||' AND MGD.CLIENTE_WS = ''PANTELARIA'' AND MGD.BORRADO = 0';
