...
--## INCIDENCIA_LINK=HREOS-7020
--## PRODUCTO=NO
--##
--## Finalidad: Script que modifica la carga masiva, ya que debe subir borrada.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
	...
	V_USUARIO VARCHAR2(32 CHAR):= 'HREOS-7020';
     ...
     FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      LOOP
			...
			IF V_NUM_TABLAS > 0 THEN	  
				DBMS_OUTPUT.PUT_LINE('[INFO] Actualizando tabla '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA');
												
			  	  V_MSQL := 'UPDATE '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA '  ||
	          			' SET BORRADO = 1 '||
	          			', USUARIOMODIFICAR = ''HREOS-7020'' '||
	          			', FECHAMODIFICAR = SYSDATE '||
	          			' WHERE DD_OPM_CODIGO = '''||V_TMP_FUNCION(1)||'''';
	          			
		          DBMS_OUTPUT.PUT_LINE('Actualizado Campo '''||V_TMP_FUNCION(1)||''''); 
			...
			ELSE
				V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA' ||
							...
							' 0, '''|| V_USUARIO ||''', SYSDATE, 1, '''||V_TMP_FUNCION(5)||''' FROM DUAL';
