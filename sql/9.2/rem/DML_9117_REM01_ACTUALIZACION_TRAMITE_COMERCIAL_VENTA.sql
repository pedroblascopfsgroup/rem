...
BEGIN
    ...
    EXECUTE IMMEDIATE V_MSQL;
        ...
        V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO' ||
			  ' SET TAP_SCRIPT_VALIDACION_JBPM = ''valores[''''T013_RespuestaOfertante''''][''''comboRespuesta''''] == DDSiNo.SI ? (checkBankia() ? (ratificacionComite() ? null : ''''Ha fallado la ratificación del comité'''') : null) : null'' '||	  
			  ' WHERE TAP_CODIGO = ''T013_RespuestaOfertante'' ';
	DBMS_OUTPUT.PUT_LINE('[INFO] Actualizando el tipo del campo de la tarea.......');
    DBMS_OUTPUT.PUT_LINE(V_MSQL);
    EXECUTE IMMEDIATE V_MSQL;
