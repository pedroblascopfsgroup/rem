...
DECLARE
    ...
    V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
	  T_FUNCION('07', '138', 'Global Pantelaria SA','A88203534'),
	  T_FUNCION('07', '138', 'Global Licata SA','A88203542')
    );          
     ...
     FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      LOOP
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.MGD_MAPEO_GESTOR_DOC WHERE DD_CRA_ID = 
						...
						AND CLIENTE_GD = '''|| (V_TMP_FUNCION(3)) ||'''';
						DBMS_OUTPUT.PUT_LINE(V_SQL);
			EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
			-- Si existe la FUNCION
			IF V_NUM_TABLAS > 0 THEN	  
				V_MSQL := 'UPDATE '||V_ESQUEMA||'.MGD_MAPEO_GESTOR_DOC SET '||
					'USUARIOMODIFICAR = ''HREOS-5914'', FECHAMODIFICAR = SYSDATE, '||
					'PRO_ID = (SELECT PRO_ID FROM '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO WHERE PRO_DOCIDENTIF = '''||V_TMP_FUNCION(4)||''') '||
					'WHERE DD_CRA_ID = (SELECT DD_CRA_ID FROM '||V_ESQUEMA||'.DD_CRA_CARTERA WHERE DD_CRA_CODIGO = '''||V_TMP_FUNCION(1)||''') '||
					'AND DD_SCR_ID = (SELECT DD_SCR_ID FROM '||V_ESQUEMA||'.DD_SCR_SUBCARTERA WHERE DD_SCR_CODIGO = '''||V_TMP_FUNCION(2)||''') '||
					'AND CLIENTE_GD = '''||V_TMP_FUNCION(3)||''' 
					';
				EXECUTE IMMEDIATE V_MSQL;
				DBMS_OUTPUT.PUT_LINE('[INFO] Datos de la tabla '||V_ESQUEMA||'.MGD_MAPEO_GESTOR_DOC actualizados correctamente.');
			ELSE
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.MGD_MAPEO_GESTOR_DOC (' ||
					'MGD_ID, DD_CRA_ID, DD_SCR_ID, CLIENTE_GD, USUARIOCREAR, FECHACREAR, BORRADO,PRO_ID) ' ||
					...
					(V_TMP_FUNCION(3)) ||''',''HREOS-5914'',SYSDATE,0, '||
					'(SELECT PRO_ID FROM '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO WHERE PRO_DOCIDENTIF = '''||V_TMP_FUNCION(4)||''') FROM DUAL';
