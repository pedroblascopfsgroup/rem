...
--## PRODUCTO=NO
--##
--## Finalidad: Insercion de nuevo registro en  DD_MTO_MOTIVOS_OCULTACION
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_AUX NUMBER(16);
    V_TABLA VARCHAR2(50 CHAR) := 'DD_MTO_MOTIVOS_OCULTACION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-5599';
	V_SIGLAS_TABLA VARCHAR2(3 CHAR) := 'MTO';
	...
	T_TIPO_DATA_2('16','Activo matriz alquilado')
	);
			  ...
			  V_SQL := 'SELECT COUNT(1) FROM '||V_TABLA||' WHERE 
				...
				IF V_AUX = 0 THEN 
				  ...
				  V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (		  
                    ...
                    ,0
                    ,17
                    ,(SELECT DD_TCO_ID FROM DD_TCO_TIPO_COMERCIALIZACION WHERE DD_TCO_CODIGO = ''03'')
					)';
