...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('manpuapple',1234,'Grupo gestor mantenimiento y publicaciones Apple', '1')
    	); 
        ...
        ELSE
          ...
          EXECUTE IMMEDIATE V_MSQL INTO V_ID;	
           ...
           V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.USU_USUARIOS (' ||
                      'USU_ID, ENTIDAD_ID, USU_USERNAME, USU_PASSWORD, USU_NOMBRE,USU_GRUPO, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ',1,'''||TRIM(V_TMP_TIPO_DATA(1))||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''','''||TRIM(V_TMP_TIPO_DATA	        (4))||''',0,''REMVIP-3870'',SYSDATE,0  FROM DUAL';
