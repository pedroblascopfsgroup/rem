      ...
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
					...
					', USUARIOMODIFICAR = ''REMVIP-4120'' , FECHAMODIFICAR = SYSDATE '||
					', DD_'||V_TEXT_CHARS||'_VISIBLE = '||TRIM(V_TMP_TIPO_DATA(5)) ||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                      ...
                      'SELECT '|| V_ID || ',  '''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', '''||TRIM(V_TMP_TIPO_DATA(4))||''', 0, ''REMVIP-4120'',SYSDATE,0 '||
					  ',  ' || TRIM(V_TMP_TIPO_DATA(5)) || '  FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
