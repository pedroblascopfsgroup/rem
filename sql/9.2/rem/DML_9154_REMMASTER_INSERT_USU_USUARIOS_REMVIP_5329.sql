...
BEGIN	
	...
	--Comprobamos el dato a insertar EN USU_USUARIOS
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''Gastos cíclicos'' ';
        ...
        ELSE
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS EL USUARIO ''Gastos cíclicos''');   	
          V_MSQL := 'SELECT '|| V_ESQUEMA_M ||'.S_USU_USUARIOS.NEXTVAL FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL INTO V_ID;	
           
           V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.USU_USUARIOS (' ||
                      'USU_ID, USU_NOMBRE,  USU_USERNAME, USU_FECHA_VIGENCIA_PASS,USU_GRUPO, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ',''Gastos cíclicos'', ''GASTOS_CICLICOS'', SYSDATE,0,0,''REMVIP-5329'',SYSDATE,0  FROM DUAL';
