...
--##		    0.4 HREOS-5806 | Mariam Lliso: validaciones nuevas "Fichero de alta y subsanación de gastos 1.G-R"
--##########################################
--*/
...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		T_TIPO_DATA('F40' ,'Obligatorio campo [Referencia emisor]' , '1', 'WHERE AUX.REF_EMISOR IS NULL' ),
		T_TIPO_DATA('F41' ,'Obligatorio campo [Concepto]' , '1', 'WHERE AUX.CONCEPTO IS NULL' ),
		T_TIPO_DATA('F42' ,'Obligatorio campo [NIF destinatario]' , '1', 'WHERE AUX.NIF_DESTINATARIO IS NULL' ),
    T_TIPO_DATA('F43' ,'Obligatorio campo [Fecha inicio emisión], solo si es gasto cíclico indicado en campo [Periodicidad especial]', '1', 'INNER JOIN DD_TPE_TIPOS_PERIOCIDAD TPE ON TPE.DD_TPE_CODIGO = AUX.PERIO_REAL AND TPE.DD_TPE_CODIGO <> ''''01'''' AND AUX.FECHA_INICIO_EMISION IS NULL' ),
		T_TIPO_DATA('F44' ,'Obligatorio campo [Fecha fin emisión],  solo si es gasto cíclico indicado en campo [Periodicidad especial]', '1', 'INNER JOIN DD_TPE_TIPOS_PERIOCIDAD TPE ON TPE.DD_TPE_CODIGO = AUX.PERIO_REAL AND TPE.DD_TPE_CODIGO <> ''''01''''  AND AUX.FECHA_FIN_EMISION IS NULL' ),
		T_TIPO_DATA('F45' ,'Obligatorio campo [ID primer gasto], solo si es gasto cíclico indicado en campo [Periodicidad especial]', '1', 'INNER JOIN DD_TPE_TIPOS_PERIOCIDAD TPE ON TPE.DD_TPE_CODIGO = AUX.PERIO_REAL AND TPE.DD_TPE_CODIGO <> ''''01'''' AND AUX.ID_PRIMER_GASTO_SERIE IS NULL' )
		); 
    	...
    	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      	LOOP
			...
			DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
					...
					', USUARIOMODIFICAR = ''HREOS-5806'' , FECHAMODIFICAR = SYSDATE '||
          	...
          	--Si no existe, lo insertamos   
       		ELSE
				...
				''''||TRIM(V_TMP_TIPO_DATA(3))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''','||
                      		'0, ''HREOS-5806'',SYSDATE,0 FROM DUAL';
			DBMS_OUTPUT.PUT_LINE(V_MSQL);
          		EXECUTE IMMEDIATE V_MSQL;
