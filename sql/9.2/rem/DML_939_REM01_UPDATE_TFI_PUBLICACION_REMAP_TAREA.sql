...
--## PRODUCTO=NO
--##
--## Finalidad: INSERTA filas de TFI_TAREAS_FORM_ITEMS - Campos tarea T011_AnalisisPeticionCorreccion del T. publicacion
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_TFI T_ARRAY_TFI := T_ARRAY_TFI(
		...
		T_TFI(   'T011_AnalisisPeticionCorreccion',   		'combo',   			'comboAceptacion',  'DDSiNo',		'Aceptar nueva tipología',				'Debe indicar si acepta/rechaza',				'false' ),
		T_TFI(   'T011_AnalisisPeticionCorreccion',   		'text',   			'motivoRechazo',  	'',				'Motivo de denegación',							'',												'' ),
		T_TFI(   'T011_AnalisisPeticionCorreccion',   		'textarea',   		'observaciones',	'',				'Observaciones',								'',												'' )
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Insertando datos de TFI_TAREAS_FORM_ITEMS - remapeo de tarea en T. publicacion');

	V_SQL := '
		SELECT COUNT(1) FROM '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo''  
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T011_AnalisisPeticionCorreccion'' )
	';
	...
	IF (V_NUM_ENLACES > 0) THEN
		V_MSQL := 'DELETE '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo''  
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T011_AnalisisPeticionCorreccion'' )
	';
		DBMS_OUTPUT.PUT_LINE('[DELETE] Se han encontrado '||V_NUM_ENLACES||' campos antiguos en TFI que necesitan borrarse para T011_AnalisisPeticionCorreccion. Se eliminan todos antes de insertar los nuevos.'); 
