...
--## PRODUCTO=NO
--##
--## Finalidad: INSERTA filas de TFI_TAREAS_FORM_ITEMS - Campos tarea T009_SancionCargaPropuesta del T. propuesta precios
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_TFI T_ARRAY_TFI := T_ARRAY_TFI(
		...
		T_TFI(   'T009_SancionCargaPropuesta',   		'datemaxtod',		'fechaSancion',  		'',				'Fecha de sanción de la propuesta',				'Debe indicar la fecha de sanción',						'false' ),
		T_TFI(   'T009_SancionCargaPropuesta',   		'textarea',   		'observaciones',		'',				'Observaciones',								'',														'' )
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Insertando datos de TFI_TAREAS_FORM_ITEMS - remapeo de tarea T009_SancionCargaPropuesta  en T. propuesta precios');

	V_SQL := '
		SELECT COUNT(1) FROM '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo'' 
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T009_SancionCargaPropuesta'' )
	';
	...
	IF (V_NUM_ENLACES > 0) THEN
		V_MSQL := 'DELETE '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo'' 
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T009_SancionCargaPropuesta'' )
	';
		DBMS_OUTPUT.PUT_LINE('[DELETE] Se han encontrado '||V_NUM_ENLACES||' campos antiguos en TFI que necesitan borrarse para T009_SancionCargaPropuesta. Se eliminan todos antes de insertar los nuevos.'); 
