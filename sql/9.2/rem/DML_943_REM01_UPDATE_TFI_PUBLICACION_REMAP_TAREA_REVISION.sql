...
--## PRODUCTO=NO
--##
--## Finalidad: INSERTA filas de TFI_TAREAS_FORM_ITEMS - Campos tarea T011_RevisionInformeComercial del T. publicacion
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_TFI T_ARRAY_TFI := T_ARRAY_TFI(
		...
		T_TFI(   'T011_RevisionInformeComercial',   		'combo',   				'comboAceptacion',  	'DDSiNo',				'Aceptar informe comercial',				'Debe indicar si acepta/rechaza',				'false' ),    				
		T_TFI(   'T011_RevisionInformeComercial',   		'text',   				'motivoDenegacion',  	'',						'Motivo de rechazo',						'Debe indicar un motivo de rechazo',			'false' ),
		T_TFI(   'T011_RevisionInformeComercial',   		'combo',   				'comboDatosIguales',  	'DDSiNo',				'Continuar con proceso de publicación',		'Debe indicar si continúa el proceso',			'false' ),    				
		T_TFI(   'T011_RevisionInformeComercial',   		'textarea',   			'observaciones',		'',						'Observaciones',							'',													'' )
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Insertando datos de TFI_TAREAS_FORM_ITEMS - remapeo de tarea T011_RevisionInformeComercial en T. propuesta de precios');

	V_SQL := '
		SELECT COUNT(1) FROM '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo''  
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T011_RevisionInformeComercial'' )
	';
	...
	IF (V_NUM_ENLACES > 0) THEN
		V_MSQL := 'DELETE '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo''  
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T011_RevisionInformeComercial'' )
	';
		DBMS_OUTPUT.PUT_LINE('[DELETE] Se han encontrado '||V_NUM_ENLACES||' campos antiguos en TFI que necesitan borrarse para T011_RevisionInformeComercial. Se eliminan todos antes de insertar los nuevos.'); 
