...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	--			codigo STR	Descripcion									Descripcion Larga							codigo TTR (trabajo)
    	T_TIPO_DATA('42'		,'Actualización de precios'					,'Actualización de precios'					,'04'),
    	T_TIPO_DATA('43'		,'Actualización de precios de descuento'	,'Actualización de precios de descuento'	,'04')
	); 
...
BEGIN	
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_STR_SUBTIPO_TRABAJO WHERE DD_STR_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
        
        -- Recuperamos el id del Tipo Trabajo
        V_SQL := 'SELECT DD_TTR_ID FROM '||V_ESQUEMA||'.DD_TTR_TIPO_TRABAJO WHERE DD_TTR_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(4))||'''';
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||''' ');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.DD_STR_SUBTIPO_TRABAJO '||
                    'SET DD_STR_DESCRIPCION = '''||TRIM(V_TMP_TIPO_DATA(2))||''' '|| 
					', DD_STR_DESCRIPCION_LARGA = '''||TRIM(V_TMP_TIPO_DATA(3))||''' '||
					', DD_TTR_ID = '|| V_TIPO_ID ||' '||
					', USUARIOMODIFICAR = ''HREOS-927_remF2'' , FECHAMODIFICAR = SYSDATE '||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.DD_STR_SUBTIPO_TRABAJO (' ||
                      ' DD_STR_ID, DD_STR_CODIGO, DD_STR_DESCRIPCION, DD_STR_DESCRIPCION_LARGA, DD_TTR_ID, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      ' SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', '||
                      ' '|| V_TIPO_ID ||', 0, ''HREOS-927_remF2'',SYSDATE,0 FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
