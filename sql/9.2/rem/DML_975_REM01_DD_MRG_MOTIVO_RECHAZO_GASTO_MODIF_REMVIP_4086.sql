...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		T_TIPO_DATA('F21' ,'Emisor (proveedor) no existe o está dado de baja' , '1', 'WHERE AUX.NIF_PROVEEDOR is null OR NOT EXISTS (SELECT * FROM ACT_PVE_PROVEEDOR PVE WHERE PVE.PVE_DOCIDENTIF = AUX.NIF_PROVEEDOR AND PVE.BORRADO = 0 AND PVE.PVE_FECHA_BAJA IS NOT NULL)' )
		); 
    	...
    	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      	LOOP
			...
			DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
					...
					', USUARIOMODIFICAR = ''REMVIP-4086'' , FECHAMODIFICAR = SYSDATE '||
          	...
          	--Si no existe, lo insertamos   
       		...
       		ELSE
				...
				''''||TRIM(V_TMP_TIPO_DATA(3))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''','||
                      		'0, ''REMVIP-4086'',SYSDATE,0 FROM DUAL';
			DBMS_OUTPUT.PUT_LINE(V_MSQL);
          		EXECUTE IMMEDIATE V_MSQL;
