...
DECLARE
	...
	T_TIPO_DATA('F39' , 'Gastos tipo plusvalía por gestoras Postventa',
			...
			'INNER JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = AUX.COD_ACTIVO
			 ...
			 INNER JOIN '|| V_ESQUEMA ||'.DD_GRF_GESTORIA_RECEP_FICH GRF ON GRF.USERNAME_GTOPOSTV = USU.USU_USERNAME and GRF.DD_GRF_CODIGO = AUX.COD_GESTORIA
	...
	T_TIPO_DATA('F36' ,'Gastos IBI por gestoras Plusvalía',
			...
			'INNER JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = AUX.COD_ACTIVO
			 ...
			 INNER JOIN '|| V_ESQUEMA_M ||'.USU_USUARIOS USU ON USU.USU_ID = GEE.USU_ID
			 INNER JOIN '|| V_ESQUEMA ||'.DD_GRF_GESTORIA_RECEP_FICH GRF ON GRF.USERNAME_GTOPLUS = USU.USU_USERNAME and GRF.DD_GRF_CODIGO = AUX.COD_GESTORIA
