...
--## PRODUCTO=NO
--##
--## Finalidad: INSERTA filas de TFI_TAREAS_FORM_ITEMS - Campos tarea T014_PosicionamientoFirma del T. propuesta precios
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_TFI T_ARRAY_TFI := T_ARRAY_TFI(
		...
		T_TFI(   'T014_PosicionamientoFirma',   		'combo',			'comboFirma',  		'DDSiNo',			'¿Se ha firmado la escritura?',					'Debe indicar si se firma la escritura',				'false' ),
		T_TFI(   'T014_PosicionamientoFirma',   		'datemaxtod',		'fechaInicio',  	'',					'Fecha inicio',									'Debe indicar la fecha de inicio',						'false' ),
		T_TFI(   'T014_PosicionamientoFirma',   		'datemaxtod',		'fechaFin',  		'',					'Fecha fin',									'Debe indicar la fecha de fin',							'false' ),
		T_TFI(   'T014_PosicionamientoFirma',   		'number',   		'numContrato', 		'',					'No. contrato',									'Debe indicar el no. de contrato',						'false' ),
		T_TFI(   'T014_PosicionamientoFirma',   		'textarea',   		'observaciones',	'',					'Observaciones',								'',														'' )
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Insertando datos de TFI_TAREAS_FORM_ITEMS - remapeo de tarea T014_PosicionamientoFirma  en T. propuesta precios');

	V_SQL := '
		SELECT COUNT(1) FROM '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo'' 
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T014_PosicionamientoFirma'' )
	';
	...
	IF (V_NUM_ENLACES > 0) THEN
		V_MSQL := 'DELETE '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo'' 
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T014_PosicionamientoFirma'' )
	';
		DBMS_OUTPUT.PUT_LINE('[DELETE] Se han encontrado '||V_NUM_ENLACES||' campos antiguos en TFI que necesitan borrarse para T014_PosicionamientoFirma. Se eliminan todos antes de insertar los nuevos.'); 
