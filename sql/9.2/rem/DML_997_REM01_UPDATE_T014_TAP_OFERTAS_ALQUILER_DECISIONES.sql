...
BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZANDO DECISIONES - TRAMITE SANCION OFERTA ALQUILER: DECISIONES');
  ...
  V_TAP_VALOR := 'valores[''''T014_ResolucionComiteExterno''''][''''comboResAprobada''''] == DDSiNo.SI ? ''''OK'''' : ''''KO''''  ';
...
-- OBTENCIÓN DOC - SOLICITUD GESTOR INTERNO - Validación POST fecha mayor a aprobación Trabajo --------
  ...
  V_TAP_VALOR := 'valores[''''T014_PosicionamientoFirma''''][''''comboFirma''''] == DDSiNo.SI ? ''''OK'''' : ''''KO'''' ';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
             ';
