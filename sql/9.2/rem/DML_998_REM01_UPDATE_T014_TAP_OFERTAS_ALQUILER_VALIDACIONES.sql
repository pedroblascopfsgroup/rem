...
BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZANDO VALIDACIONES - TRAMITE SANCION OFERTA ALQUILER: VALIDACIONES');
  ...
  V_TAP_TAP_CODIGOS := ' ''T014_ResolucionComiteExterno'' ';
  V_TAP_VALOR := 'valores[''''T014_ResolucionComiteExterno''''][''''comboResAprobada''''] == DDSiNo.SI ? existeAdjuntoUGValidacion("08","E") : null';
...
-- OBTENCIÓN DOC - SOLICITUD GESTOR INTERNO - Validación POST fecha mayor a aprobación Trabajo --------
  V_TAP_TAP_CODIGOS := ' ''T014_PosicionamientoFirma'' ';
  V_TAP_VALOR := 'valores[''''T014_PosicionamientoFirma''''][''''comboFirma''''] == DDSiNo.SI ? existeAdjuntoUGValidacion("09","E") : null';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
             ';
