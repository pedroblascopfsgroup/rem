...
BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] Cambio de descripciones de subtipos de documentos del expediente');
  ...
  V_SDE_CAMPO := 'DD_SDE_DESCRIPCION_LARGA';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_SDE_TABLA||'
             SET '||V_SDE_CAMPO||'='''||V_SDE_VALOR||''' 
             ,FECHAMODIFICAR = sysdate
             ,USUARIOMODIFICAR = ''DML_99900020''
...
-- Contrato de oferta --> Contrato de alquiler
  V_SDE_CODIGOS := ' ''09'' ';
  V_SDE_VALOR := 'Contrato de alquiler';
  V_SDE_CAMPO := 'DD_SDE_DESCRIPCION';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_SDE_TABLA||'
             SET '||V_SDE_CAMPO||'='''||V_SDE_VALOR||'''
             ,FECHAMODIFICAR = sysdate
