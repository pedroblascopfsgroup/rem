...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      -- TIPO TRABAJO(DD_TTR_CODIGO),  	SUBTIPO(DD_STR_CODIGO),  DESCRIPCION,  					DESCRIPCION LARGA
        T_TIPO_DATA('06'				,'55'					,'Sanción oferta de alquiler'	,'Sanción oferta de alquiler'),
        T_TIPO_DATA('06'				,'56'					,'Sanción oferta de venta'		,'Sanción oferta de venta')
    ); 
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(2)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.DD_STR_SUBTIPO_TRABAJO '||
					...
					'WHERE DD_STR_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(2))||'''';
