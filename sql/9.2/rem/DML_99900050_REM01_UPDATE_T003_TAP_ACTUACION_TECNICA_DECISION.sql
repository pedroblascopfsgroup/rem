...
BEGIN 
  ...
  V_TAP_VALOR := 'valores[''''T004_EleccionPresupuesto''''][''''comboPresupuesto''''] == DDSiNo.NO ? ''''PresupuestoInvalido'''' : (checkBankia() ? (checkSuperaPresupuestoActivo() ? ''''ValidoSuperaLimiteBankia'''' : ''''ValidoDentroLimiteBankia'''') : ''''PresupuestoValido'''') ';
  ...
  V_TAP_VALOR := 'checkBankia() ? (checkSuperaPresupuestoActivo() ? ''''SuperaLimiteBankia'''' : ''''DentroLimiteBankia'''') : ''''avanzaBPM'''' ';
  V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
             SET '||V_TAP_CAMPO||'='''||V_TAP_VALOR||''' 
             WHERE TAP_CODIGO IN ('||V_TAP_TAP_CODIGOS||') 
             ';
