...
BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZANDO DECISIONES - TRAMITE OBTENCION DOCUMENTAL: DECISIONES');
...
-- checkBankia() ? (checkSuperaPresupuestoActivo() ? ''''OKConPagoYSinSaldoBankia'''' : ''''OKConPagoYSaldo'''') :
  -- ACTUACION TECNICA - T002_AnalisisPeticion --------
  V_TAP_TAP_CODIGOS := ' ''T002_AnalisisPeticion'' ';
  V_TAP_VALOR := 'valores[''''T002_AnalisisPeticion''''][''''comboTramitar''''] == DDSiNo.NO ? ''''Fin'''' : (checkBankia() ? (checkSuperaPresupuestoActivo() ? ''''OKConPagoYSinSaldoBankia'''' : (checkSuperaDelegacion() ? ''''OKConPagoYSinSaldoBankia'''' : ''''OKConPagoYSaldo'''')) : (valores[''''T002_AnalisisPeticion''''][''''comboGasto''''] == DDSiNo.SI ? (valores[''''T002_AnalisisPeticion''''][''''comboSaldo''''] == DDSiNo.SI ? ''''OKConPagoYSaldo'''' : ''''OKConPagoYSinSaldo'''') : ''''OKSinPago'''')) ';
