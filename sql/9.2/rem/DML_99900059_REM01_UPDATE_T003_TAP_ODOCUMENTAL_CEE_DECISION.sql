...
BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZANDO DECISIONES - TRAMITE OBTENCION DOCUMENTAL CEE: DECISIONES');
...
-- checkBankia() ? (checkSuperaPresupuestoActivo() ? ''''OKSinSaldoBankia'''' : ''''OKConSaldo'''') :
  -- ACTUACION TECNICA - T003_AnalisisPeticion --------
  V_TAP_TAP_CODIGOS := ' ''T003_AnalisisPeticion'' ';
  V_TAP_VALOR := 'valores[''''T003_AnalisisPeticion''''][''''comboTramitar''''] == DDSiNo.NO ? ''''KO'''' : (checkBankia() ? (checkSuperaPresupuestoActivo() ? ''''OKSinSaldoBankia'''' : (checkSuperaDelegacion() ? ''''OKSinSaldoBankia'''' : ''''OKConSaldo'''')) : (valores[''''T003_AnalisisPeticion''''][''''comboSaldo''''] == DDSiNo.SI ? ''''OKConSaldo'''' : ''''OKSinSaldo'''')) ';
