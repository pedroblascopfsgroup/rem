...
DECLARE
    ...
    V_TFI T_ARRAY_TFI := T_ARRAY_TFI(
		...
		T_TFI(   'T003_AutorizacionBankia',   			'combo',   				'comboAmpliacion',  	'DDSiNo',				'Ampliación del presupuesto',						'Debe indicar si autoriza el trabajo o el incremento de presupuesto',	'false' ),
		T_TFI(   'T003_AutorizacionBankia',   			'textarea',   			'observaciones',  		'',						'Observaciones',									'',																		'' ),
		T_TFI(   'T003_AutorizacionBankia',   			'elctrabajo',   		'GESECO',				'',						'TRABAJO > Gesti&oacuten econ&oacutemica',			'',																		'' )
...
BEGIN
	...
	V_SQL := '
		SELECT COUNT(1) FROM '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo''  
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T003_AutorizacionBankia'' )
	';
