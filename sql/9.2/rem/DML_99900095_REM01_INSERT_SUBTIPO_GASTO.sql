...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      -- TIPO TRABAJO(DD_TGA_CODIGO),   SUBTIPO(DD_STG_CODIGO),  DESCRIPCION,  					DESCRIPCION LARGA
		T_TIPO_DATA('01'				,'92'					,'Recargos e intereses'			,'Recargos e intereses'),
		T_TIPO_DATA('05'				,'93'					,'Certificado deuda comunidad'	,'Certificado deuda comunidad'),
		T_TIPO_DATA('11'				,'94'					,'Gestión de suelo'				,'Gestión de suelo'),
		T_TIPO_DATA('11'				,'95'					,'Abogado (Ocupacional)'		,'Abogado (Ocupacional)'),
		T_TIPO_DATA('11'				,'96'					,'Abogado (Asuntos generales)'	,'Abogado (Asuntos generales)'),
		T_TIPO_DATA('11'				,'97'					,'Abogado (Asistencia jurídica)','Abogado (Asistencia jurídica)')
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      ...
      -- Activamos el borrado lógico para el subitpo '45' ABOGADO 
       --Comprobamos el dato a borrar
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = ''45'' ';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
        --Si existe lo modificamos
        IF V_NUM_TABLAS > 0 THEN				
          
          DBMS_OUTPUT.PUT_LINE('[INFO]: ACTIVAMOS EL BORRADO DEL REGISTGO ''45'' ');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.DD_STG_SUBTIPOS_GASTO '||
                    'SET BORRADO = 1 '|| 
					'WHERE DD_STG_CODIGO = ''45'' ';
          EXECUTE IMMEDIATE V_MSQL;
