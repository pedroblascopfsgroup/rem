...
--##########################################
--## AUTOR=JORGE ROS
--## FECHA_CREACION=20170313
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.2
--## INCIDENCIA_LINK=HREOS-1740
--## PRODUCTO=NO
--##
...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      -- TIPO TRABAJO(DD_TGA_CODIGO),   SUBTIPO(DD_STG_CODIGO),  DESCRIPCION,  											DESCRIPCION LARGA
		T_TIPO_DATA('18'				,'98'					,'Costas judiciales (demanda comunidad propietarios)'	,'Costas judiciales (demanda comunidad propietarios)'),
		T_TIPO_DATA('18'				,'99'					,'Costas judiciales (otras demandas)'					,'Costas judiciales (otras demandas)')
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTGO '''|| TRIM(V_TMP_TIPO_DATA(2)) ||'''');
       ...
       ELSE
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS EL REGISTGO '''|| TRIM(V_TMP_TIPO_DATA(2)) ||'''');   
      ...
      -- Activamos el borrado lógico para el subitpo '45' ABOGADO 
       --Comprobamos el dato a borrar
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = ''90'' ';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
        --Si existe lo modificamos
        IF V_NUM_TABLAS > 0 THEN				
          
          DBMS_OUTPUT.PUT_LINE('[INFO]: ACTIVAMOS EL BORRADO DEL REGISTGO ''90'' ');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.DD_STG_SUBTIPOS_GASTO '||
                    'SET BORRADO = 1 '|| 
					'WHERE DD_STG_CODIGO = ''90'' ';
          EXECUTE IMMEDIATE V_MSQL;
          DBMS_OUTPUT.PUT_LINE('[INFO]: BORRADO ACTIVADO CORRECTAMENTE');
