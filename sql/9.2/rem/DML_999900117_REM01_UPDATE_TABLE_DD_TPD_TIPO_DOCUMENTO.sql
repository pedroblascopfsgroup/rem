...
DECLARE
    ...
    V_USU_MODIFICAR VARCHAR2(30 CHAR) := '''HREOS-2112'''; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.

    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(150);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    --				     CODIGO		   DESCRIPCION							                            DESCRIPCION_LARGA
        T_TIPO_DATA('26',		'Respuesta propietario incremento de presupuesto',	  'Respuesta propietario incremento de presupuesto'),
        T_TIPO_DATA('27',		'Certificado sustitutivo de Cédula de Habitabilidad',	'Certificado sustitutivo de Cédula de Habitabilidad')
    );
	...
	LOOP
      ...
      ELSE
  			...
  			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                        ...
                        'SELECT '|| V_NUM_SEQUENCE || ', '''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', 0, '|| V_USU_MODIFICAR ||',SYSDATE,0 FROM DUAL';
  			EXECUTE IMMEDIATE V_MSQL;
