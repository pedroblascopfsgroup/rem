...
--## PRODUCTO=NO
--## Finalidad: Asignación de las cuentas contables a los gastos en GIC_GASTOS_INFO_CONTABILIDAD
--##           
...
BEGIN
	...
	V_SQL := '
		...
		USING (
			SELECT DISTINCT GIC1.GIC_ID, CCC.CCC_CUENTA_CONTABLE
			...
			INNER JOIN '||V_ESQUEMA||'.CCC_CONFIG_CTAS_CONTABLES CCC ON CCC.EJE_ID = GIC1.EJE_ID AND CCC.DD_STG_ID = STG.DD_STG_ID AND CCC.DD_CRA_ID = CRA.DD_CRA_ID AND CCC.CCC_ARRENDAMIENTO = DECODE(NVL(SPS.SPS_OCUPADO,0)+NVL(SPS.SPS_CON_TITULO,0),2,1,0)
			WHERE GIC1.GIC_CUENTA_CONTABLE IS NULL
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'... SE HAN ASIGNADO CORRECTAMENTE '||SQL%ROWCOUNT||' CUENTAS CONTABLES');

	DBMS_OUTPUT.PUT_LINE('[FIN] El proceso de asignación de las cuentas conbales a los gastos a finalizado correctamente');
	COMMIT;
