    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.TMP_MAPEO_PUB_VENTA '||
                    'SET DD_EPV_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(2))||''''|| 
					          ', APU_CHECK_OCULTAR_V = '''||TRIM(V_TMP_TIPO_DATA(3))||''''||
                    ', APU_CHECK_OCULT_PRECIO_V = '''||TRIM(V_TMP_TIPO_DATA(4))||''''||
                    ', DD_TPU_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(5))||''''||
                    ', DD_MTO_V_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(6))||''''||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.TMP_MAPEO_PUB_VENTA (DD_EPU_CODIGO, DD_EPV_CODIGO, APU_CHECK_OCULTAR_V, APU_CHECK_OCULT_PRECIO_V, DD_TPU_CODIGO, DD_MTO_V_CODIGO)'|| 
