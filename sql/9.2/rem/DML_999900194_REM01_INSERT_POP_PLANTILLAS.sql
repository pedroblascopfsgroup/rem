...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] ');
    	...
    	DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN POP_PLANTILLAS_OPERACION] ');
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.POP_PLANTILLAS_OPERACION WHERE POP_NOMBRE = ''MASIVO_OCULTACION_VENTA''';
			...
			ELSE
                V_MSQL := 'SELECT DD_OPM_ID FROM '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA WHERE DD_OPM_CODIGO = ''CMOV''';
				...
				''MASIVO_OCULTACION_VENTA'',
				''plantillas/plugin/masivo/OcultarActivosVenta.xls'',
				'||V_ID||',
				0,
				''HREOS-3957'',
				SYSDATE,
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.POP_PLANTILLAS_OPERACION WHERE POP_NOMBRE = ''MASIVO_OCULTACION_ALQUILER''';
			EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
			-- Si existe la FUNCION
			IF V_NUM_TABLAS > 0 THEN	  
				DBMS_OUTPUT.PUT_LINE('[INFO] Ya existen los datos en la tabla '||V_ESQUEMA||'.POP_PLANTILLAS_OPERACION...no se modifica nada.');
			...
			ELSE
                V_MSQL := 'SELECT DD_OPM_ID FROM '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA WHERE DD_OPM_CODIGO = ''CMOA''';
                EXECUTE IMMEDIATE V_MSQL INTO V_ID;
                
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.POP_PLANTILLAS_OPERACION (POP_ID, POP_NOMBRE, POP_DIRECTORIO, DD_OPM_ID, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
				VALUES (
				('||V_ESQUEMA||'.S_POP_PLANTILLAS_OPERACION.NEXTVAL),
				''MASIVO_OCULTACION_ALQUILER'',
				''plantillas/plugin/masivo/OcultarActivosAlquiler.xls'',
				'||V_ID||',
				0,
				''HREOS-3957'',
				SYSDATE,
