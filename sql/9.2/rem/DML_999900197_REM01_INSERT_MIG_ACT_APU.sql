        ...
        IF V_NUM_TABLAS > 0 THEN
          ...
          EXECUTE IMMEDIATE 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
                    ...
                    FROM(
                        SELECT  
                          ACT.ACT_ID
                          , (SELECT DDTPU.DD_TPU_ID FROM '||V_ESQUEMA||'.DD_TPU_TIPO_PUBLICACION DDTPU WHERE DDTPU.DD_TPU_CODIGO = (SELECT TMP.DD_TPU_CODIGO FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_VENTA TMP WHERE TMP.DD_EPU_CODIGO = ''06'')) DD_TPU_ID
                          ...
                          , ACT.DD_TCO_ID
                          , (SELECT DDMTO.DD_MTO_ID FROM '||V_ESQUEMA||'.DD_MTO_MOTIVOS_OCULTACION DDMTO WHERE DDMTO.DD_MTO_CODIGO = (SELECT TMP.DD_MTO_V_CODIGO FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_VENTA TMP WHERE TMP.DD_EPU_CODIGO = ''06'')) DD_MTO_V_ID
                          , (SELECT TMP.MTO_OCULTACION FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_VENTA TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_MOT_OCULTACION_MANUAL_V
                          , (SELECT TMP.APU_CHECK_PUBLICAR_V FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_VENTA TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_PUBLICAR_V
                          , (SELECT TMP.APU_CHECK_OCULTAR_V FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_VENTA TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_OCULTAR_V
                          , (SELECT TMP.APU_CHECK_OCULT_PRECIO_V FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_VENTA TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_OCULTAR_PRECIO_V
                          , (SELECT TMP.APU_CHECK_PUB_SIN_PRECIO_V FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_VENTA TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_PUB_SIN_PRECIO_V
                          , NULL DD_MTO_A_ID
          ...
          EXECUTE IMMEDIATE 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' 
                    ...
                    FROM(
                      SELECT  ACT.ACT_ID
                        , (SELECT DDTPU.DD_TPU_ID FROM '||V_ESQUEMA||'.DD_TPU_TIPO_PUBLICACION DDTPU WHERE DDTPU.DD_TPU_CODIGO = (SELECT TMP.DD_TPU_CODIGO FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_ALQUILER TMP WHERE TMP.DD_EPU_CODIGO = ''06'')) DD_TPU_ID
                        , (SELECT DDEPV.DD_EPV_ID FROM '||V_ESQUEMA||'.DD_EPV_ESTADO_PUB_VENTA DDEPV WHERE DDEPV.DD_EPV_CODIGO = ''01'') DD_EPV_ID
                        , (SELECT DDEPA.DD_EPA_ID FROM '||V_ESQUEMA||'.DD_EPA_ESTADO_PUB_ALQUILER DDEPA WHERE DDEPA.DD_EPA_CODIGO = ''01'') DD_EPA_ID
                        , ACT.DD_TCO_ID
                        , NULL DD_MTO_V_ID
                        ...
                        , (SELECT DDMTO.DD_MTO_ID FROM '||V_ESQUEMA||'.DD_MTO_MOTIVOS_OCULTACION DDMTO WHERE DDMTO.DD_MTO_CODIGO = (SELECT TMP.DD_MTO_A_CODIGO FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_ALQUILER TMP WHERE TMP.DD_EPU_CODIGO = ''06'')) DD_MTO_A_ID
                        , (SELECT TMP.MTO_OCULTACION FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_ALQUILER TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_MOT_OCULTACION_MANUAL_A
                        , (SELECT TMP.APU_CHECK_PUBLICAR_A FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_ALQUILER TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_PUBLICAR_A
                        , (SELECT TMP.APU_CHECK_OCULTAR_A FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_ALQUILER TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_OCULTAR_A
                        , (SELECT TMP.APU_CHECK_OCULT_PRECIO_A FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_ALQUILER TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_OCULTAR_PRECIO_A
                        , (SELECT TMP.APU_CHECK_PUB_SIN_PRECIO_A FROM '||V_ESQUEMA||'.TMP_MAPEO_PUB_ALQUILER TMP WHERE TMP.DD_EPU_CODIGO = ''06'') APU_CHECK_PUB_SIN_PRECIO_A 
