...
BEGIN
    ...
    IF V_NUM_TABLAS > 0 THEN        
      V_SQL := 'UPDATE '||V_ESQUEMA||'.'||VAR_TABLENAME||' SET TAP_SCRIPT_VALIDACION_JBPM = ''checkExpedienteFechaChequeLiberbank ? (checkExpedienteBloqueado() ? (valores[''T013_PosicionamientoYFirma''][''comboFirma''] == DDSiNo.SI ? (checkPosicionamiento() ? existeAdjuntoUGValidacion("15,E") : ''El expediente debe tener algún posicionamiento'') : null) : ''El expediente no está bloqueado'') : ''Debe rellenar la fecha de ingreso de cheque mediante el fichero de Reservas-Ventas''';
    END IF;
