...
DECLARE
    ...
    V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
   	  T_FUNCION('grucoinv', 'grucoinv'), 
	    T_FUNCION('grucoinm', 'grucoinm'),
	    T_FUNCION('grucodi',  'grucodi')
    ); 
      ...
      LOOP
        ...
        V_SQL := '
          SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.'||V_TABLA||' 
			 	  WHERE 
            USU_ID_GRUPO = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_TMP_FUNCION(1)||''') 
