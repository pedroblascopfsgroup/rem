...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] ');
    	...
    	DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN POP_PLANTILLAS_OPERACION] ');
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.POP_PLANTILLAS_OPERACION WHERE POP_NOMBRE = ''MASIVO_PRINEX_LBK''';
			...
			ELSE
                V_MSQL := 'SELECT DD_OPM_ID FROM '||V_ESQUEMA||'.DD_OPM_OPERACION_MASIVA WHERE DD_OPM_CODIGO = ''PRINEX''';
				...
				''MASIVO_PRINEX_LBK'',
				''plantillas/plugin/masivo/MASIVO_PRINEX_LBKA.xls'',
				'||V_ID||',
				0,
				''HREOS-4330'',
				SYSDATE,
