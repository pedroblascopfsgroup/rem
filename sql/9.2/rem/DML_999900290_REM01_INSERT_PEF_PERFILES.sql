...
DECLARE
    ...
    --		    Codigo			Descripcion			Descripcion Larga
    	T_TIPO_DATA('GESTALQ',			'Gestor de alquileres',		'Gestor de alquileres'),
    	T_TIPO_DATA('GESTSUE',			'Gestor de suelos',		'Gestor de suelos'),
	T_TIPO_DATA('GESTEDI',			'Gestor de edificaciones',	'Gestor de edificaciones')
    ); 
      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.PEF_PERFILES (' ||
                      ...
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''',''HREOS-4434'',SYSDATE,0 FROM DUAL';
