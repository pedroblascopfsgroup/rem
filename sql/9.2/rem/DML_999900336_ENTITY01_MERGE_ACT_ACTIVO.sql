...
BEGIN	
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] '||V_ESQUEMA_M||'.ACT_ACTIVO... Empezando a modificar datos en la tabla');
	...
	MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT 
	    ...
	    ON (ACT.ACT_ID = BOR.ID)
	  WHEN MATCHED THEN
	    UPDATE 
	       SET ACT.DD_TCO_ID = (SELECT DD_TCO_ID FROM DD_TCO_TIPO_COMERCIALIZACION WHERE DD_TCO_CODIGO = ''03''),
		ACT.DD_TAL_ID = (SELECT DD_TAL_ID FROM DD_TAL_TIPO_ALQUILER WHERE DD_TAL_CODIGO = ''02''),
