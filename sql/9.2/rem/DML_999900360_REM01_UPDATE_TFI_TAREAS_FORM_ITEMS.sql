...
DECLARE
    ...
    V_NUM_TABLAS NUMBER(25); -- Vble. count tablas
...
BEGIN
	...
	V_SQL := 'SELECT COUNT(1) 
		...
		JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP ON TAP.TAP_ID = TFI.TAP_ID AND TAP.TAP_CODIGO = '''||TRIM(V_TMP_TIPO_DATA4(1))||'''
		WHERE TFI.TFI_ORDEN = '||TRIM(V_TMP_TIPO_DATA4(2))||' AND TFI.TFI_TIPO = ''emptyField'' AND TFI.BORRADO = 0';
	...
	IF V_NUM_TABLAS = 0 THEN
	    ...
	    EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('Insertado elemento ''emptyField'' en la posicion '||TRIM(V_TMP_TIPO_DATA4(2)));	
	ELSE
		DBMS_OUTPUT.PUT_LINE('Ya existe un elemento ''emptyField'' en la posicion '||TRIM(V_TMP_TIPO_DATA4(2)));
	END IF;
