...
--## PRODUCTO=SI
--##
--## Finalidad: Insertar nuevo usuario para buzon REM
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN
    ...
    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' WHERE USU_USERNAME = ''buzonrem'' AND BORRADO = 0';
    ...
    IF V_NUM_TABLAS = 0 THEN
        ...
        V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' (USU_ID, USU_USERNAME, USU_PASSWORD, USU_NOMBRE, USU_MAIL, USUARIOCREAR, FECHACREAR) 
          VALUES ('||V_ESQUEMA_M||'.S_USU_USUARIOS.NEXTVAL, ''buzonrem'', ''GQt3AReokW'', ''BUZON REM'', ''notificaciones.rem@haya.es'', '''||V_USUARIOMODIFICAR||''', SYSDATE)';
