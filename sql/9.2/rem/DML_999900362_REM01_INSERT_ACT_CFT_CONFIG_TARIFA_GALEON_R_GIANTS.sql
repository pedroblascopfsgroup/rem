...
BEGIN	
	     DBMS_OUTPUT.PUT_LINE('[INICIO] BORRADO_1   tarifado que actualmente tiene la cartera GALEON que no es de GIANTS ');
	
	V_MSQL :='DELETE  FROM  ACT_TCT_TRABAJO_CFGTARIFA TCT WHERE  TCT.CFT_ID IN (SELECT TA.CFT_ID FROM '|| V_ESQUEMA ||'.ACT_CFT_CONFIG_TARIFA TA WHERE
		 TA.DD_CRA_ID = (SELECT CRA.DD_CRA_ID FROM '|| V_ESQUEMA ||'.DD_CRA_CARTERA CRA WHERE CRA.DD_CRA_CODIGO=''15'')
	AND  NOT EXISTS 
		(SELECT 1 FROM '|| V_ESQUEMA ||'.ACT_CFT_CONFIG_TARIFA TAC  WHERE 
		TAC.DD_CRA_ID = (SELECT DD.DD_CRA_ID FROM '|| V_ESQUEMA ||'.DD_CRA_CARTERA DD WHERE DD.DD_CRA_CODIGO=''12'') AND TA.DD_TTF_ID= TAC.DD_TTF_ID AND TAC.DD_STR_ID=TA.DD_STR_ID ))';
		
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] BORRADOS GALEON '||sql%rowcount||' en la tabla  ACT_TCT_TRABAJO_CFGTARIFA');
	
  	     DBMS_OUTPUT.PUT_LINE('[INICIO] BORRADO_2  tarifado que actualmente tiene la cartera GALEON que no es de GIANTS');

	V_MSQL := 'DELETE FROM '|| V_ESQUEMA ||'.'|| V_TABLA_CFT ||' TA  WHERE
		 TA.DD_CRA_ID = (SELECT CRA.DD_CRA_ID FROM '|| V_ESQUEMA ||'.DD_CRA_CARTERA CRA WHERE CRA.DD_CRA_CODIGO=''15'')
	AND  NOT EXISTS 
		...
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] BORRADOS GALEON '||sql%rowcount||' en la tabla  '||V_TABLA_CFT);

    DBMS_OUTPUT.PUT_LINE('Empieza la insercción de  tarifario que actualmente tiene la cartera GIANTS para GALEON');   
		V_MSQL := '
