...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('CARGA_MASIVA_SANCION',   'plantillas/plugin/masivo/CARGA_MASIVA_SANCION.xls',  'CMSAN')
    );
      ...
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.POP_PLANTILLAS_OPERACION '||
					...
					', USUARIOMODIFICAR = ''HREOS-3945'' , FECHAMODIFICAR = SYSDATE '||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.POP_PLANTILLAS_OPERACION (' ||
                      ...
                      ' SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''' ,'''||V_TMP_TIPO_DATA(2)||''',(SELECT DD_OPM_ID ' ||
                      ' FROM '|| V_ESQUEMA ||'.DD_OPM_OPERACION_MASIVA WHERE DD_OPM_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(3))||'''), ' || 
                      ' 0, ''HREOS-4915'',SYSDATE,0 FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
