...
BEGIN	
	...
	IF V_NUM_TABLAS > 0 THEN	
		...
		IF V_NUM_TABLAS > 0 THEN
			...
			V_MSQL := '
			  ...
			  SET 
				TFI_TIPO = ''textarea'',
				TFI_ORDEN = 10,
				TFI_LABEL = ''Asistencia PBC descripción'',
				TFI_ERROR_VALIDACION = ''Debe indicar las observaciones'',
				TFI_VALIDACION = ''false'',
				VERSION = 1,
				USUARIOMODIFICAR = ''REMVIP-2726'',
				FECHAMODIFICAR = SYSDATE
			  WHERE TFI.TFI_NOMBRE = ''obsAsisPBC''
		...
		ELSE
			...
			V_MSQL := '
			  INSERT INTO '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS (TFI_ID, TAP_ID, TFI_ORDEN, TFI_TIPO, TFI_NOMBRE, TFI_LABEL, TFI_ERROR_VALIDACION, TFI_VALIDACION, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
			  SELECT '||V_ESQUEMA||'.S_TFI_TAREAS_FORM_ITEMS.NEXTVAL AS TFI_ID,
			  TAP.TAP_ID AS TAP_ID,
			  10 AS TFI_ORDEN,
			  ''textarea'' AS TFI_TIPO,
			  ''obsAsisPBC'' AS TFI_NOMBRE,
			  ''Asistencia PBC descripción'' AS TFI_LABEL,
			  ''Debe indicar los motivos'' AS TFI_ERROR_VALIDACION,
			  ...
			  ;
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO] Se ha insertado correctamente el registro ''obsAsisPBC'' en la tabla '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS');
