...
--## INCIDENCIA_LINK=HREOS-5503
--## PRODUCTO=NO
--## Finalidad: Modificación de literales para el email
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN   
	...
	V_SQL:= 'DELETE FROM '||V_ESQUEMA||'.MAIL_COMUNICACION_GENCAT WHERE A =''jpoyatos@haya.es;backup.rem@pfsgroup.es''';
	DBMS_OUTPUT.PUT_LINE(V_SQL);
	  ...
	  DBMS_OUTPUT.PUT_LINE(V_NUM_TABLAS || 'registros borrados');
		...
		V_SQL := 'UPDATE '||V_ESQUEMA||'.MAIL_COMUNICACION_GENCAT SET
						...
						A = ''jpoyatos@haya.es;backup.rem@pfsgroup.es;GFORMADM'',  
