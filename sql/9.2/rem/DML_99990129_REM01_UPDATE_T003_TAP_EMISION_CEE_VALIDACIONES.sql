...
BEGIN 
  ...
  V_SDE_VALOR := 'valores[''''T003_EmisionCertificado''''][''''comboEmision''''] == DDSiNo.SI ? esFechaMenor(valores[''''T003_EmisionCertificado''''][''''fechaEmision''''], fechaAprobacionTrabajo()) ? ''''Fecha emisi&oacute;n debe ser posterior o igual a fecha de aprobaci&oacute;n del trabajo'''' : existeAdjuntoUGValidacion("11","T") : null ';

  V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_SDE_TABLA||'
             SET '||V_SDE_CAMPO||'='''||V_SDE_VALOR||'''
             ,FECHAMODIFICAR = sysdate
             ,USUARIOMODIFICAR = ''DML_99990129''
  ...
  V_SDE_CODIGOS := ' ''T003_SolicitudEtiqueta'' ';
  V_SDE_VALOR := 'esFechaMenor(valores[''''T003_SolicitudEtiqueta''''][''''fechaPresentacion''''], fechaAprobacionTrabajo()) ? ''''Fecha presentaci&oacute;n debe ser posterior o igual a fecha de aprobaci&oacute;n del trabajo'''' : existeAdjuntoUGValidacion("24","T") ';
