...
BEGIN	
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] ');
    ...
    V_MSQL :=   'UPDATE '|| V_ESQUEMA ||'.DD_SDE_SUBTIPO_DOC_EXP '||
                'SET DD_TDE_ID = (SELECT DD_TDE_ID FROM '||V_ESQUEMA||'.DD_TDE_TIPO_DOC_EXP WHERE DD_TDE_CODIGO = ''02'')'||
                ',USUARIOMODIFICAR = ''HREOS-2543'''||
                ',FECHAMODIFICAR = SYSDATE'||
                ' WHERE DD_SDE_CODIGO = ''09''';
    EXECUTE IMMEDIATE V_MSQL;
    
    DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS LOS REGISTROS');
    V_MSQL :=   'UPDATE '|| V_ESQUEMA ||'.DD_SDE_SUBTIPO_DOC_EXP '||
                'SET DD_TDE_ID = (SELECT DD_TDE_ID FROM '||V_ESQUEMA||'.DD_TDE_TIPO_DOC_EXP WHERE DD_TDE_CODIGO = ''03'')'||
                ',USUARIOMODIFICAR = ''HREOS-2543'''||
                ',FECHAMODIFICAR = SYSDATE'||
                ' WHERE DD_SDE_CODIGO = ''21''';
