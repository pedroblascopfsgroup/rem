...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-2733'; -- USUARIO CREAR/MODIFICAR
    V_TABLA VARCHAR2(50 CHAR) := 'DD_ODT_ORIGEN_DATO';
    ...
    V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
	  T_FUNCION('01', 'REM', 'REM'),
	  T_FUNCION('02', 'Recovery', 'Recovery')
    ); 
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN '||V_TABLA||'] ');
     FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      LOOP
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE DD_ODT_CODIGO = '''||V_TMP_FUNCION(1)||'''';
			...
			ELSE
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||'' ||
							' (DD_ODT_ID, DD_ODT_CODIGO, DD_ODT_DESCRIPCION, DD_ODT_DESCRIPCION_LARGA, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)' || 
