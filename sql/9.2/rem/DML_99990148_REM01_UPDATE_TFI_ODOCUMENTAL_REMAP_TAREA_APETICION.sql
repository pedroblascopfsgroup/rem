...
--## PRODUCTO=NO
--##
--## Finalidad: INSERTA filas de TFI_TAREAS_FORM_ITEMS - Campos tarea T002_AnalisisPeticion del T. Obtencion Documental
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_TFI T_ARRAY_TFI := T_ARRAY_TFI(
		...
		T_TFI(	'T002_AnalisisPeticion',	'combo',			'comboTramitar',		'DDSiNo',	'Tramitar petición',						'Debe indicar si se tramita la petici&oacute;n',	'false'	),
		T_TFI(	'T002_AnalisisPeticion',	'textfield',		'motivoDenegacion',		'',			'Motivo de denegación',						'Debe indicar un motivo de denegaci&oacute;n',		'false'	),
		T_TFI(	'T002_AnalisisPeticion',	'combo',			'comboGasto',			'DDSiNo',	'El trabajo se encarga a un proveedor',		'Debe indicar si se encarga a un proveedor',		'false'	),
		T_TFI(	'T002_AnalisisPeticion',	'textinf',			'saldoDisponible',		'',			'Saldo disponible',							'',													''		),
		T_TFI(	'T002_AnalisisPeticion',	'textarea',			'observaciones',		'',			'Observaciones',							'',													''		),
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Insertando datos de TFI_TAREAS_FORM_ITEMS - remapeo de tarea en T. Obtencion Documental');

	V_SQL := '
		SELECT COUNT(1) FROM '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo''  
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T002_AnalisisPeticion'' )
	';
	...
	IF (V_NUM_ENLACES > 0) THEN
		V_MSQL := 'DELETE '||V_ESQUEMA||'.tfi_tareas_form_items WHERE tfi_nombre <> ''titulo''  
			AND tap_id = (select tap_id from '||V_ESQUEMA||'.tap_tarea_procedimiento where tap_codigo = ''T002_AnalisisPeticion'' )
	';
		DBMS_OUTPUT.PUT_LINE('[DELETE] Se han encontrado '||V_NUM_ENLACES||' campos antiguos en TFI que necesitan borrarse para T002_AnalisisPeticion. Se eliminan todos antes de insertar los nuevos.'); 
