...
--## PRODUCTO=NO
--##
--## Finalidad: Script que modifica en GEE_GESTOR_ENTIDAD los datos añadidos en T_ARRAY_DATA
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_ID NUMBER(16);
    V_TEXT_TABLA VARCHAR2(50 CHAR) := 'GEE_GESTOR_ENTIDAD';
...
BEGIN
  ...
  FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
    LOOP
      ...
      V_SQL := 'SELECT COUNT(1) FROM '|| V_ESQUEMA ||'.'|| V_TEXT_TABLA ||' GEE '||
      ...
      V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' GEE '||
      				' SET GEE.DD_TGE_ID = (SELECT TGE.DD_TGE_ID FROM '|| V_ESQUEMA_M ||'.DD_TGE_TIPO_GESTOR TGE WHERE TGE.DD_TGE_CODIGO = '''|| TRIM(V_TMP_TIPO_DATA(2)) ||''' '||
              							' ) '||
      				' WHERE GEE.DD_TGE_ID = (SELECT TGE2.DD_TGE_ID FROM '|| V_ESQUEMA_M ||'.DD_TGE_TIPO_GESTOR TGE2 WHERE TGE2.DD_TGE_CODIGO = '''|| TRIM(V_TMP_TIPO_DATA(1)) ||''' '||
              									' )';
