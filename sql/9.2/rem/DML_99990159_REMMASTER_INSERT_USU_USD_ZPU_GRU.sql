...
--## INCIDENCIA_LINK=HREOS-2832
--## PRODUCTO=NO
--##
--## Finalidad: Script que actualiza usuario 'rsanchez'
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
  ...
  V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-2832';
  ...
  V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
	  ...
	  T_TIPO_DATA('rsanchez', 'Raúl', 'Sánchez', 'Salas', 'rsanchez@haya.es', '1234', '0', 'HAYAGBOINM', '', 'REMGBOINM'),
	  T_TIPO_DATA('rsanchez', 'Raúl', 'Sánchez', 'Salas', 'rsanchez@haya.es', '1234', '0', 'HAYASBOINM', '', 'REMSBOINM')
 ...
 DBMS_OUTPUT.PUT_LINE('[INICIO] ');
   ...
   FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
     LOOP
       ...
       V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_TIPO_DATA(1))||'_old''';
       EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
       
       IF V_NUM_TABLAS > 0 THEN				         
			
		DBMS_OUTPUT.PUT_LINE('[INFO] Ya se ha insertado el usuario!!! NO SE HACE NADA MÁS');
       ...
       ELSE
        ...
        V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.USU_USUARIOS SET USU_USERNAME = '''||TRIM(V_TMP_TIPO_DATA(1))||'_old'', 
					USU_PASSWORD = '''', USUARIOMODIFICAR = '''||V_USUARIO||''', FECHAMODIFICAR = SYSDATE
					WHERE USU_USERNAME = '''||TRIM(V_TMP_TIPO_DATA(1))||'''';
		EXECUTE IMMEDIATE V_MSQL;
