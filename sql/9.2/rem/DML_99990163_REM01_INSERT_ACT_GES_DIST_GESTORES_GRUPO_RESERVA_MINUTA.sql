...
-- GRUPO - GESTOR RESERVA
		T_TIPO_DATA('GESRES'  ,' = ''01'''  ,''  ,''  , ' IS NULL'  ,''  ,''  ,'gesres'),
-- GRUPO - SUPERVISOR RESERVA
		T_TIPO_DATA('SUPRES'  ,' = ''01'''  ,''  ,''  , ' IS NULL'  ,''  ,''  ,'supres'),
-- GRUPO - GESTOR MINUTA		
		T_TIPO_DATA('GESMIN'  ,' = ''01'''  ,''  ,''  , ' IS NULL'  ,''  ,''  ,'gesmin'),
-- GRUPO - SUPERVISOR MINUTA		
		T_TIPO_DATA('SUPMIN'  ,' = ''01'''  ,''  ,''  , ' IS NULL'  ,''  ,''  ,'supmin')
    ); 
      ...
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
       	  ...
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
					...
					' , USUARIOMODIFICAR = ''HREOS-2874'' , FECHAMODIFICAR = SYSDATE '||
					' WHERE TIPO_GESTOR = '''||TRIM(V_TMP_TIPO_DATA(1))||''' '||
					' AND COD_CARTERA '||TRIM(V_TMP_TIPO_DATA(2))||' '||
					' AND COD_ESTADO_ACTIVO IS NULL '||
					' AND COD_TIPO_COMERZIALZACION  IS NULL '||
       ...
       ELSE
         ...
         V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' (' ||
						...
						', 0, ''HREOS-2874'',SYSDATE,0 FROM DUAL';
          --DBMS_OUTPUT.PUT_LINE(V_MSQL);
          EXECUTE IMMEDIATE V_MSQL;
