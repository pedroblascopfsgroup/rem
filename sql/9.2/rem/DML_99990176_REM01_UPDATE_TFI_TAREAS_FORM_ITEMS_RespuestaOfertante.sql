...
DECLARE
    ...
    V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
	  ...
	  T_FUNCION('T013_RespuestaOfertante', '1', 'comboinied', 'comboRespuesta', '¿Acepta contraoferta?', '', '', 'DDRespuestaOfertante' ),
	  T_FUNCION('T013_RespuestaOfertante', '3', 'textinf', 'importeContraoferta', 'Importe contraoferta', '', '', ''),
	  T_FUNCION('T013_RespuestaOfertante', '4', 'number', 'importeOfertante', 'Importe ofertante', '', '', '')
    ); 
...
BEGIN	
	V_MSQL := '
			  ...
			  WHERE TFI.TFI_NOMBRE = ''observaciones''
				AND EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP WHERE TAP_CODIGO = ''T013_RespuestaOfertante''
