...
BEGIN
		...
		IF V_NUM_TABLAS > 0 THEN
		    ...
		    V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' SET TAP_SCRIPT_VALIDACION = '''', TAP_SCRIPT_VALIDACION_JBPM = ''valores[''''T008_ObtencionDocumento''''][''''comboObtencion''''] == DDSiNo.SI ? (existeAdjuntoUG("13","T") ? null : "Es necesario adjuntar sobre el trabajo el documento Cédula de habitabilidad") : (existeAdjuntoUG("27","T") ? (valores[''''T008_ObtencionDocumento''''][''''motivoNoObtencion''''] == DDMotivoNoObtencion.NO_CUMPLE_REQUISITOS ? existeAdjuntoUGValidacion("27","T") : null) : existeAdjuntoUGValidacion("27","T") )'', USUARIOMODIFICAR=''HREOS-3108'', FECHAMODIFICAR=SYSDATE WHERE TAP_CODIGO = ''T008_ObtencionDocumento'' ';
