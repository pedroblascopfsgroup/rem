...
BEGIN
		...
		IF V_NUM_TABLAS > 0 THEN
		    ...
		    V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' SET TAP_SCRIPT_DECISION = ''checkFormalizacion() ? (checkDeDerechoTanteo() == false ? (valores[''''T013_DefinicionOferta''''][''''comiteSuperior''''] != DDSiNo.SI ? (checkAtribuciones() ? (checkReserva() == false ? (checkDerechoTanteo() ? (checkCajamar() ? ''''ConFormalizacionSinTanteoConAtribucionSinReservaConTanteoCajamar'''' : ''''ConFormalizacionSinTanteoConAtribucionSinReservaConTanteo'''') : (checkCajamar() ? ''''ConFormalizacionSinTanteoConAtribucionSinReservaSinTanteoCajamar'''' : ''''ConFormalizacionSinTanteoConAtribucionSinReservaSinTanteo'''')) : (checkCajamar() ? ''''ConFormalizacionSinTanteoConAtribucionConReservaCajamar'''' : ''''ConFormalizacionSinTanteoConAtribucionConReserva'''')) : ''''ConFormalizacionSinTanteoSinAtribucion'''') : ''''ConFormalizacionSinTanteoSinAtribucion'''') : ''''ConFormalizacionConTanteo'''') : ''''SinFormalizacion'''''''|| 
		    ', USUARIOMODIFICAR = ''HREOS-3097'', FECHAMODIFICAR = SYSDATE WHERE TAP_CODIGO = ''T013_DefinicionOferta'' ';
