...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
               T_TIPO_DATA('DD_SITUACION_TITULO','1','NULO','NULO','DD_ETI_ESTADO_TITULO','04','Desconocido','Desconocido'),
               T_TIPO_DATA('DD_SITUACION_TITULO','3','SUBSANAR','SUBSANAR','DD_ETI_ESTADO_TITULO','04','Desconocido','Desconocido'),
               T_TIPO_DATA('DD_SITUACION_TITULO','5','INMATRICULADOS','INMATRICULADOS','DD_ETI_ESTADO_TITULO','04','Desconocido','Desconocido')               
        ...
        IF V_NUM_TABLAS > 0 THEN				
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||''','''||TRIM(V_TMP_TIPO_DATA(2))||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.DD_EQV_BANKIA_REM  '||
		      ...
		      ', USUARIOMODIFICAR         = ''HREOS-3233'' , FECHAMODIFICAR = SYSDATE '||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.DD_EQV_BANKIA_REM (' ||
		      'DD_NOMBRE_BANKIA, DD_CODIGO_BANKIA, DD_DESCRIPCION_BANKIA, DD_DESCRIPCION_LARGA_BANKIA, DD_NOMBRE_REM, DD_CODIGO_REM, DD_DESCRIPCION_REM, DD_DESCRIPCION_LARGA_REM, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '''||TRIM(V_TMP_TIPO_DATA(1))||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', '''||TRIM(V_TMP_TIPO_DATA(4))||''','''||TRIM(V_TMP_TIPO_DATA(5))||''','''||TRIM(V_TMP_TIPO_DATA(6))||''', '''||TRIM(V_TMP_TIPO_DATA(7))||''','''||TRIM(V_TMP_TIPO_DATA(8))||''', 0, ''HREOS-3233'',SYSDATE,0 FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
