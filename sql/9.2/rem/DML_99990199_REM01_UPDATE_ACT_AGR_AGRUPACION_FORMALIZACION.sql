...
BEGIN	
	...
	V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION agr SET agr.AGR_IS_FORMALIZACION = 1, agr.USUARIOMODIFICAR = ''HREOS-3475'', agr.FECHAMODIFICAR = SYSDATE
		where agr.dd_tag_id = (SELECT DD_TAG_ID FROM '|| V_ESQUEMA ||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = ''02'')
		and EXISTS(
					SELECT 1 FROM '|| V_ESQUEMA ||'.ACT_ACTIVO act INNER JOIN '|| V_ESQUEMA ||'.ACT_PAC_PERIMETRO_ACTIVO  pac on act.ACT_ID = pac.ACT_ID 
					WHERE pac.PAC_CHECK_FORMALIZAR = 1 AND act.ACT_ID = agr.AGR_ACT_PRINCIPAL)
		and  NOT EXISTS (SELECT 1 FROM '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION agr2 WHERE agr2.AGR_ID = agr.AGR_ID and agr.AGR_IS_FORMALIZACION = 1)
		AND agr.BORRADO = 0';
