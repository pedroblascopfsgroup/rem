...
--## PRODUCTO=NO
--##
--## Finalidad: Script que añade en DD_TGE; DD_TDE y TGP el proveedor técnico
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('PTEC','Proveedor técnico','Proveedor técnico','REMPTEC')
    ); 
      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.DD_TGE_TIPO_GESTOR (' ||
                      ...
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''' ,'''||V_TMP_TIPO_DATA(2)||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', 0, ''HREOS-3945'',SYSDATE,0 FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
       ...
       END IF;
    	...
    	------------------------------------------------------------------------------------------------------------------------
       DBMS_OUTPUT.PUT_LINE('[INFO]: INSERT/UPDATE EN DES_DESPACHO_EXTERNO');
		...
		ELSE
	          ...
	          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.DES_DESPACHO_EXTERNO (' ||
	                      ...
	                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(4)||''',''HREOS-3945'',SYSDATE,0, ' ||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.TGP_TIPO_GESTOR_PROPIEDAD (' ||
                      ...
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''' ,''DES_VALIDOS'','|| V_TIPO_ID ||', 0, ''HREOS-3945'',SYSDATE,0 FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
          DBMS_OUTPUT.PUT_LINE('[INFO]: REGISTRO INSERTADO CORRECTAMENTE');
