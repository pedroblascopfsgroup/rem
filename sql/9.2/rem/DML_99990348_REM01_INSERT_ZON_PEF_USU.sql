...
DECLARE
    ...
    -- Gestores y Supervisores de suelo
	  T_FUNCION('mgomezp',				'GESTSUE'),
	  T_FUNCION('aalvear',				'GESTSUE'),
	  T_FUNCION('sflores',				'GESTSUE'),
	  T_FUNCION('prodriguezdelema',		'GESTSUE'),
	  T_FUNCION('jcordoba',				'GESTSUE'),
	  -- Gestores y Supervisores de edificación
	  T_FUNCION('rcanales',				'GESTEDI'),
	  T_FUNCION('jfernandez',			'GESTEDI'),
	  T_FUNCION('jsanmartint',			'GESTEDI'),
	  T_FUNCION('bcarrascosa',			'GESTEDI'),
	  T_FUNCION('mgodoy',				'GESTEDI'),
	  T_FUNCION('jpalazon',				'GESTEDI'),
	  T_FUNCION('csalvador',			'GESTEDI'),
	  T_FUNCION('gmoreno',				'GESTEDI'),
	  T_FUNCION('mescribano',			'GESTEDI'),
	  T_FUNCION('jmateo',				'GESTEDI')
    );
      ...
      LOOP
			...
			ELSE
				V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU' ||
							...
							' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||'''),' ||
							' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(1))||'''),' ||
							' ''HREOS-4478'',SYSDATE,0 FROM DUAL';
