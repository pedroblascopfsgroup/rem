...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] Añadimos el perfil SUPERUSUARIO a los usuarios acampos, mblascop, acarabal, dmontero, saragon.');
    ...
    LOOP
		...
		DBMS_OUTPUT.PUT_LINE('[INFO] Comprobamos si existe el usuario ' ||TRIM(V_TMP_TIPO_DATA(1))|| ' con el perfil SUPERUSUARIO.');
		
		V_SQL := 'SELECT COUNT(*) 
				FROM '||V_ESQUEMA||'.ZON_PEF_USU 
				WHERE PEF_ID = (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''SUPERUSUARIO'')
		...
		IF V_NUM_FILAS = 0 THEN
				...
				V_SQL := 	'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU (
								...
								19504,
								(SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''SUPERUSUARIO''),
