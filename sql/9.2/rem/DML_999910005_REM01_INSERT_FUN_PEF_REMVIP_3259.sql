...
BEGIN	
    ...
    FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
    	LOOP
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] Se va a insertar la función '||TRIM(V_TMP_FUNCION(1))||' al perfil '||TRIM(V_TMP_PERFIL(1))||'.');
            		V_SQL := 'SELECT COUNT(1) 
                          FROM '||V_ESQUEMA||'.FUN_PEF 
                          WHERE PEF_ID = (SELECT PEF_ID 
                                          FROM '||V_ESQUEMA||'.PEF_PERFILES 
                                          WHERE PEF_CODIGO = '''||TRIM(V_TMP_PERFIL(1))||''') 
                          AND FUN_ID = (SELECT FUN_ID 
					...
					ELSE
                        ...
                        BORRADO) 
							          SELECT  FUN.FUN_ID, 
                                PEF_ID, '||V_ESQUEMA||'.S_FUN_PEF.NEXTVAL, 
                                0, 
                                ''DML'', 
                                SYSDATE, 0
							          FROM '||V_ESQUEMA||'.PEF_PERFILES, 
                        '||V_ESQUEMA_M||'.FUN_FUNCIONES FUN
						            WHERE FUN_DESCRIPCION = '''||TRIM(V_TMP_FUNCION(1))||''' 
                        AND PEF_CODIGO LIKE ('''||TRIM(V_TMP_PERFIL(1))||''') ';
