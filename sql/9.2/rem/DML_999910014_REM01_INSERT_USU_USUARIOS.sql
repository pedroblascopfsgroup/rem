...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('Grupo Haya Serrano', '1','usugruhs','ETHJSUBV','1'),
    	T_TIPO_DATA('Grupo Cerberus aprobador de la Advisory Note', '1','usugruce','UGXIOAPS','1')
    	); 
      ...
      LOOP
        ...
        ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.USU_USUARIOS (' ||
                      'USU_ID, ENTIDAD_ID, USU_USERNAME, USU_PASSWORD, USU_NOMBRE, USU_GRUPO, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(2)||''','''||TRIM(V_TMP_TIPO_DATA(3))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''','''||TRIM(V_TMP_TIPO_DATA(1))||''', '''||TRIM(V_TMP_TIPO_DATA(5))||''',''HREOS-5457'',SYSDATE,0 FROM DUAL';
