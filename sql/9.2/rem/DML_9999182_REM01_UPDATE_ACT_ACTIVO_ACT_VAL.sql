...
BEGIN	
	...
	WHERE VAL.VAL_FECHA_INICIO IS NOT NULL AND VAL.VAL_FECHA_FIN IS NULL
	    AND VAL.BORRADO = 0 AND VAL.VAL_IMPORTE = 0';
	...
	FOR elemId IN 1 .. T_IDS.COUNT loop
        ...
        SET VAL_IMPORTE = NULL, 
        USUARIOMODIFICAR = ''HREOS-4777'', 
        FECHAMODIFICAR = SYSDATE 
        WHERE ACT_ID = '''||T_IDS(elemId)||''' AND VAL_FECHA_INICIO IS NOT NULL AND VAL_FECHA_FIN IS NULL
        AND BORRADO = 0 AND VAL_IMPORTE = 0 AND DD_TPC_ID IN (SELECT DD_TPC_ID FROM '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO WHERE DD_TPC_CODIGO IN (''02'',''03''))';
