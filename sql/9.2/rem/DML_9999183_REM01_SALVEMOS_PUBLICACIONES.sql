...
BEGIN   
    ...
    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION (APU_ID, ACT_ID, DD_EPV_ID, DD_EPA_ID
        ...
        LEFT JOIN '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU ON APU.ACT_ID = ACT.ACT_ID 
            AND APU.BORRADO = 0
        WHERE ACT.BORRADO = 0 AND APU.APU_ID IS NULL';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      REGISTROS INSERTADOS: '||SQL%ROWCOUNT);
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION T1
        ...
        WHEN MATCHED THEN UPDATE SET
            T1.DD_TPU_V_ID = T2.DD_TPU_V_ID, T1.DD_EPV_ID = T2.DD_EPV_ID, T1.DD_EPA_ID = T2.DD_EPA_ID, T1.DD_TCO_ID = T2.DD_TCO_ID
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION T1
        ...
        ON (T1.APU_ID = T2.APU_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.DD_TPU_A_ID = T2.DD_TPU_A_ID, T1.DD_EPV_ID = T2.DD_EPV_ID, T1.DD_EPA_ID = T2.DD_EPA_ID, T1.DD_TCO_ID = T2.DD_TCO_ID
    ...
    DBMS_OUTPUT.PUT_LINE('  VENDIDOS A OCULTO VENTA');
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION T1
        USING (
            SELECT ACT.ACT_ID
            ...
            WHERE ACT.BORRADO = 0 AND APU.BORRADO = 0
                AND EPV.DD_EPV_CODIGO NOT IN (''01'')) T2
        ON (T1.ACT_ID = T2.ACT_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.DD_TPU_V_ID = NULL
            , T1.DD_EPV_ID = (SELECT DD_EPV_ID FROM '||V_ESQUEMA||'.DD_EPV_ESTADO_PUB_VENTA WHERE DD_EPV_CODIGO = ''04'')
            , T1.APU_CHECK_OCULTAR_V = 1
            , T1.DD_MTO_V_ID = (SELECT DD_MTO_ID FROM '||V_ESQUEMA||'.DD_MTO_MOTIVOS_OCULTACION WHERE DD_MTO_CODIGO = ''13'')
            , T1.APU_FECHA_INI_VENTA = SYSDATE
        '||V_WHERE;
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      REGISTROS ACTUALIZADOS: '||SQL%ROWCOUNT);

    DBMS_OUTPUT.PUT_LINE('  VENDIDOS A OCULTO ALQUILER');
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION T1
        USING (
            SELECT ACT.ACT_ID
            FROM '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU
            ...
            WHERE ACT.BORRADO = 0 AND APU.BORRADO = 0
                AND EPA.DD_EPA_CODIGO NOT IN (''01'')) T2
        ON (T1.ACT_ID = T2.ACT_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.DD_TPU_A_ID = NULL
            , T1.DD_EPA_ID = (SELECT DD_EPA_ID FROM '||V_ESQUEMA||'.DD_EPA_ESTADO_PUB_ALQUILER WHERE DD_EPA_CODIGO = ''04'')
            , T1.APU_CHECK_OCULTAR_A = 1
            , T1.DD_MTO_A_ID = (SELECT DD_MTO_ID FROM '||V_ESQUEMA||'.DD_MTO_MOTIVOS_OCULTACION WHERE DD_MTO_CODIGO = ''13'')
            , T1.APU_FECHA_INI_ALQUILER = SYSDATE
        '||V_WHERE;
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      REGISTROS ACTUALIZADOS: '||SQL%ROWCOUNT);

    DBMS_OUTPUT.PUT_LINE('  ACTIVOS NO PUBLICADOS VENTA A OTROS ESTADOS');
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION T1
        USING (
            '||DATA_SOURCE||'
            SELECT ACT.ACT_ID
            ...
            JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = APU.ACT_ID
            JOIN '||V_ESQUEMA||'.DD_EPV_ESTADO_PUB_VENTA EPV ON EPV.DD_EPV_ID = APU.DD_EPV_ID
            JOIN HEP ON HEP.ACT_ID = ACT.ACT_ID
            JOIN '||V_ESQUEMA||'.DD_EPU_ESTADO_PUBLICACION EPU ON EPU.DD_EPU_ID = HEP.DD_EPU_ID
            WHERE ACT.BORRADO = 0 AND APU.BORRADO = 0
                AND EPV.DD_EPV_CODIGO <> (''01'')
                AND EPU.DD_EPU_ID = ''06'') T2
        ON (T1.ACT_ID = T2.ACT_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.DD_TPU_V_ID = NULL
            , T1.DD_EPV_ID = (SELECT DD_EPV_ID FROM '||V_ESQUEMA||'.DD_EPV_ESTADO_PUB_VENTA WHERE DD_EPV_CODIGO = ''01'')
            , T1.APU_CHECK_OCULTAR_V = 0
            , T1.DD_MTO_V_ID = NULL
            , T1.APU_FECHA_INI_VENTA = NULL
        '||V_WHERE;
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      REGISTROS ACTUALIZADOS: '||SQL%ROWCOUNT);

    DBMS_OUTPUT.PUT_LINE('  ACTIVOS NO PUBLICADOS ALQUILER A OTROS ESTADOS');
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION T1
        USING (
           '||DATA_SOURCE||' 
            SELECT ACT.ACT_ID
        ...
        WHEN MATCHED THEN UPDATE SET
            T1.DD_TPU_A_ID = NULL
            , T1.DD_EPA_ID = (SELECT DD_EPA_ID FROM '||V_ESQUEMA||'.DD_EPA_ESTADO_PUB_ALQUILER WHERE DD_EPA_CODIGO = ''01'')
            , T1.APU_CHECK_OCULTAR_A = 0
            , T1.DD_MTO_A_ID = NULL
            , T1.APU_FECHA_INI_ALQUILER = NULL
        '||V_WHERE;
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      REGISTROS ACTUALIZADOS: '||SQL%ROWCOUNT);
    ...
    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ACT_AHP_HIST_PUBLICACION (AHP_ID, ACT_ID, DD_TPU_V_ID
        ...
        SELECT '||V_ESQUEMA||'.S_ACT_AHP_HIST_PUBLICACION.NEXTVAL, HEP.ACT_ID
            ...
            , ''MIGRACION_PUBLICACIONES'' USUARIO, SYSDATE FECHA
        FROM HEP
        JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = HEP.ACT_ID
    ...
    DBMS_OUTPUT.PUT_LINE('  INSERTAMOS ESTADOS ACTUALES EN TABLA HISTÓRICA');
    DBMS_OUTPUT.PUT_LINE('  TODOS LOS DESTINOS');
    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ACT_AHP_HIST_PUBLICACION (AHP_ID, ACT_ID, DD_TPU_V_ID, DD_TPU_A_ID
            , DD_TCO_ID, DD_EPV_ID, DD_MTO_V_ID, AHP_CHECK_PUBLICAR_V, AHP_CHECK_OCULTAR_V
            , AHP_CHECK_OCULTAR_PRECIO_V, AHP_CHECK_PUB_SIN_PRECIO_V
            , DD_EPA_ID, DD_MTO_A_ID, AHP_CHECK_PUBLICAR_A, AHP_CHECK_OCULTAR_A
            , AHP_CHECK_OCULTAR_PRECIO_A, AHP_CHECK_PUB_SIN_PRECIO_A
            , AHP_FECHA_INI_VENTA, AHP_FECHA_INI_ALQUILER, USUARIOCREAR, FECHACREAR)
        SELECT '||V_ESQUEMA||'.S_ACT_AHP_HIST_PUBLICACION.NEXTVAL AHP_ID, APU.ACT_ID, APU.DD_TPU_V_ID, APU.DD_TPU_A_ID
            , APU.DD_TCO_ID, APU.DD_EPV_ID, APU.DD_MTO_V_ID, APU.APU_CHECK_PUBLICAR_V, APU.APU_CHECK_OCULTAR_V
            , APU.APU_CHECK_OCULTAR_PRECIO_V, APU.APU_CHECK_PUB_SIN_PRECIO_V
            , APU.DD_EPA_ID, APU.DD_MTO_A_ID, APU.APU_CHECK_PUBLICAR_A, APU.APU_CHECK_OCULTAR_A
            , APU.APU_CHECK_OCULTAR_PRECIO_A, APU.APU_CHECK_PUB_SIN_PRECIO_A
            , APU.APU_FECHA_INI_VENTA, APU.APU_FECHA_INI_ALQUILER, ''MIGRACION_ESTADOS'', SYSDATE
        FROM '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU
    ...
    DBMS_OUTPUT.PUT_LINE('  CHECK PUBLICACION A LOS PUBLICADOS');
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO T1
        USING (
            ...
            JOIN '||V_ESQUEMA||'.DD_EPA_ESTADO_PUB_ALQUILER EPA ON EPA.DD_EPA_ID = APU.DD_EPA_ID
                AND EPA.BORRADO = 0
            WHERE EPV.DD_EPV_CODIGO IN (''02'',''03'',''04'') OR EPA.DD_EPA_CODIGO IN (''02'',''03'',''04'')) T2
        ON (T1.ACT_ID = T2.ACT_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.PAC_CHECK_PUBLICAR = 1, T1.PAC_FECHA_PUBLICAR = SYSDATE
        '||V_WHERE;
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      REGISTROS ACTUALIZADOS: '||SQL%ROWCOUNT);
