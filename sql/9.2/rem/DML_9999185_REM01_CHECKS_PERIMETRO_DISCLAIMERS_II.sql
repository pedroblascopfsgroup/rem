...
USING (
        ...
        JOIN ACT_PAC_PERIMETRO_ACTIVO                   PAC
        ON PAC.ACT_ID = ACT.ACT_ID
        LEFT JOIN REM01.ACT_OFR                         AOF
        ON AOF.ACT_ID = ACT.ACT_ID
        LEFT JOIN REM01.OFR_OFERTAS                     OFR
...
EXECUTE IMMEDIATE '
        ...
        AS (
                SELECT DISTINCT ACT.ACT_ID AS ACT_ID, ACT.ACT_NUM_ACTIVO AS ACT_NUM_ACTIVO, SCM.DD_SCM_DESCRIPCION AS SITUACION_COMERCIAL, TCO.DD_TCO_DESCRIPCION AS DESTINO_COMERCIAL, OFR.OFR_ID, 
                                ...
                                ERE.DD_ERE_CODIGO, EEC.DD_EEC_CODIGO, EOF.DD_EOF_CODIGO, SCM.DD_SCM_CODIGO, TCO.DD_TCO_CODIGO
                FROM REM01.ACT_ACTIVO                           ACT
                JOIN ACT_PAC_PERIMETRO_ACTIVO                   PAC
                ON PAC.ACT_ID = ACT.ACT_ID
                LEFT JOIN REM01.ACT_OFR                         AOF
...
';
DBMS_OUTPUT.PUT_LINE('[INFO] Tabla '||V_ESQUEMA_1||'.'||V_TABLA||' creada correctamente. Activos no comercializables con el check comercializar activado.'); 
...
EXECUTE IMMEDIATE 'MERGE INTO REM01.ACT_ACTIVO T1
USING (
            SELECT DISTINCT AUX.ACT_ID AS ACT_ID,
                            AUX.DESTINO_COMERCIAL
            FROM REM01.AUX_MMC_NO_COMERCIALIZABLES                AUX
            JOIN REM01.ACT_ACTIVO                                 ACT
              ON ACT.ACT_ID = AUX.ACT_ID
            WHERE AUX.DD_TCO_CODIGO IN (''03'')
) T2
ON (T1.ACT_ID = T2.ACT_ID)
WHEN MATCHED THEN UPDATE SET
...
USING (
            ...
            JOIN REM01.ACT_ACTIVO                                 ACT
              ON ACT.ACT_ID = AUX.ACT_ID
            WHERE AUX.DD_TCO_CODIGO NOT IN (''03'')
              AND AUX.DD_EOF_CODIGO IN (''01'')
) T2
ON (T1.ACT_ID = T2.ACT_ID)
WHEN MATCHED THEN UPDATE SET
