...
--## INCIDENCIA_LINK=REMNIVDOS-4240
--## PRODUCTO=SI
--##
--## Finalidad: 
--## VERSIONES:
...
--T004_SolicitudExtraordinaria ----------------------------
	V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO 
	SET TAP_SCRIPT_VALIDACION_JBPM = ''esFechaMenor(valores[''''T004_SolicitudExtraordinaria''''][''''fecha''''], fechaAprobacionTrabajo()) ? ''''Fecha solicitud debe ser posterior o igual a fecha de aprobaci&oacute;n del trabajo'''' : ((esCerberus() && (esSubcarteraJaipurInmobiliario() || esSubcarteraAgoraInmobiliario() || esSubcarteraEgeo() || esSubcarteraApple())) || (esEgeo() && (esSubcarteraZeus() || esSubcarteraPromontoria()))) ? mismoNumeroAdjuntosComoTareasTipoUGValidacion("119", "T", 10000000004665) : null'',
	TAP_SCRIPT_DECISION = '''',
	USUARIOMODIFICAR = ''HREOS-5457'', 
	FECHAMODIFICAR = SYSDATE 
...
--T004_SolicitudPresupuestoComplementario ----------------------------
	V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO 
	SET TAP_SCRIPT_DECISION = ''((esCerberus() && (esSubcarteraJaipurInmobiliario() || esSubcarteraAgoraInmobiliario() || esSubcarteraEgeo() || esSubcarteraApple())) || (esEgeo() && (esSubcarteraZeus() || esSubcarteraPromontoria()))) ? ''''ComprobarAdvisoryNote'''' : ''''TramiteNormal'''''',
	USUARIOMODIFICAR = ''HREOS-5457'', 
	FECHAMODIFICAR = SYSDATE 
