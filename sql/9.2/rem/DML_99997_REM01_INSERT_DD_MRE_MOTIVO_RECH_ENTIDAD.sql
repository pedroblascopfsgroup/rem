...
DECLARE
    ...
    --				CODIGO		DESCRIPCION					DESCRIPCION_LARGA		TIPO_TRABAJO_CODIGO
        T_TIPO_DATA('F11',		'validación del número de documento CIF'	,		'validación del número de documento cif para la entidad',		'1', 'WHERE AUX.NIF_ENTIDAD IS NOT NULL AND ((SELECT VALIDACIF(AUX.NIF_ENTIDAD) FROM DUAL) = 0)')
    ); 
