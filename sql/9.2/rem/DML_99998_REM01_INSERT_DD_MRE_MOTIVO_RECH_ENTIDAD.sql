...
DECLARE
    ...
    V_USU_MODIFICAR VARCHAR2(30 CHAR) := '''REMVIP-4337'''; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.

    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(150);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    --				CODIGO		DESCRIPCION					DESCRIPCION_LARGA		TIPO_TRABAJO_CODIGO
        T_TIPO_DATA('F12',		'validación del número de documento cif'	,		'validación del número de documento cif',		'1', 'WHERE AUX.NIF_CONTACTO IS NOT NULL AND AUX.DOCU_NIF_CONTACTO = ''''02'''' AND ((SELECT VALIDACIF(AUX.NIF_CONTACTO) FROM DUAL) = 0)')
    ); 
