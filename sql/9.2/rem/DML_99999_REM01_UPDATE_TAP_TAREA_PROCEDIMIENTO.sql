...
DECLARE
    ...
    V_TIPO_TAP T_ARRAY_TAP := T_ARRAY_TAP(
	 ...
	 T_TIPO_TAP('T017_InformeJuridico'		, '!tieneTramiteGENCATVigenteByIdActivo() ? '	, ' : ''''El activo tiene un tr&aacute;mite GENCAT en curso.'''' ')	
	,T_TIPO_TAP('T017_PBCVenta'			, '!tieneTramiteGENCATVigenteByIdActivo() ? '	, ' : ''''El activo tiene un tr&aacute;mite GENCAT en curso.'''' ')
	,T_TIPO_TAP('T017_PosicionamientoYFirma'	, '!tieneTramiteGENCATVigenteByIdActivo() ? '	, ' : ''''El activo tiene un tr&aacute;mite GENCAT en curso.'''' ')
...
BEGIN	
	...
	IF V_NUM_TABLAS = 0 THEN
		...
		V_SQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO
		...
		V_SQL := 'SELECT NVL(TAP_SCRIPT_VALIDACION, ''null'') 
