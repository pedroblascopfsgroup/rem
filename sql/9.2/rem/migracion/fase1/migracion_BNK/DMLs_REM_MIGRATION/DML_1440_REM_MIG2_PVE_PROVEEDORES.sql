...
BEGIN
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' cargada. '||SQL%ROWCOUNT||' Filas.');
       ...
       --ACTUALIZAMOS EL CODIGO UVEM Y API DE LOS PROVEEDORES QUE NO LLEGAN EN MIGRACION
      EXECUTE IMMEDIATE '
      UPDATE ACT_PVE_PROVEEDOR PVE
	  SET PVE.PVE_COD_UVEM = (
				  SELECT DISTINCT
					  MIG.PVE_COD_UVEM
				FROM MIG2_PVE_PROVEEDORES MIG
				WHERE PVE.USUARIOCREAR != ''MIG2''
				AND PVE.PVE_COD_UVEM = MIG.PVE_COD_API_PROVEEDOR),
		...
		PVE.USUARIOMODIFICAR = ''MIG2'',
		PVE.FECHAMODIFICAR = SYSDATE
	  WHERE PVE_COD_UVEM IN (
				   SELECT DISTINCT
					  MIG.PVE_COD_API_PROVEEDOR
				FROM MIG2_PVE_PROVEEDORES MIG
				INNER JOIN ACT_PVE_PROVEEDOR PVE2
				  ON PVE2.PVE_COD_UVEM = MIG.PVE_COD_API_PROVEEDOR
				WHERE PVE2.USUARIOCREAR != ''MIG2'');
      )      
      '
      ;
