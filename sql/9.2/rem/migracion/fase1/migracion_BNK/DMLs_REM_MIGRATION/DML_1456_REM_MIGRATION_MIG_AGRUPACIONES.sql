...
BEGIN
  ...
  SET AGR.AGR_ACT_PRINCIPAL = NULL
	WHERE AGR.DD_TAG_ID != (SELECT TAG.DD_TAG_ID FROM DD_TAG_TIPO_AGRUPACION TAG WHERE TAG.DD_TAG_CODIGO = ''02'')
	')
	;
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] COMIENZA EL PROCESO DE MIGRACION SOBRE LA TABLA '||V_ESQUEMA||'.'||V_TABLA_3||'.');
	...
	WHERE DD_LOC_CODIGO = LPAD(MIG.LOC_AGRUP_LOTE_RESTRINGIDO,5,0))		DD_LOC_ID,
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] COMIENZA EL PROCESO DE MIGRACION SOBRE LA TABLA '||V_ESQUEMA||'.'||V_TABLA_2||'.');
	...
	(SELECT DD_LOC_ID
	FROM '||V_ESQUEMA_MASTER||'.DD_LOC_LOCALIDAD
	WHERE DD_LOC_CODIGO = LPAD(MIG.LOC_AGRUP_OBRA_NUEVA,5,0))		DD_LOC_ID,
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] COMIENZA EL PROCESO DE MIGRACION SOBRE LA TABLA '||V_ESQUEMA||'.'||V_TABLA_5||'.');
	...
	WITH AGR_UVEM AS (
		...
		WHERE AGR_ID NOT IN (
		  SELECT AGR_ID 
		  FROM '||V_ESQUEMA||'.'||V_TABLA_5||'
			)
		AND DD_TAG_ID = (SELECT DD_TAG_ID FROM '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = ''13'')
	)
	SELECT
	(SELECT AGR_ID
	FROM '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR
  ...
  V_SENTENCIA := '
	SELECT COUNT(1)
	FROM '||V_ESQUEMA||'.'||V_TABLA||' AGR
	WHERE AGR.DD_TAG_ID = (SELECT DD.DD_TAG_ID FROM '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION DD WHERE DD.DD_TAG_CODIGO = ''14'')
	AND AGR.USUARIOCREAR = ''MIGRAREM01BNK''
  '
  ;
  ...
  -- Registros insertados en REM
  V_SENTENCIA := '
	SELECT COUNT(LCO.AGR_ID) FROM '||V_ESQUEMA||'.'||V_TABLA_4||' LCO, '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR
  ...
  -- Registros con DD_TAG_ID = 13, Asistida
  V_SENTENCIA := '
	SELECT COUNT(1)
	FROM '||V_ESQUEMA||'.'||V_TABLA||' AGR
	WHERE AGR.DD_TAG_ID = (SELECT DD.DD_TAG_ID FROM '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION DD WHERE DD.DD_TAG_CODIGO = ''13'')
	AND AGR.USUARIOCREAR = ''MIGRAREM01BNK''
  '
  ;
  ...
  -- Registros insertados en REM
  V_SENTENCIA := '
	SELECT COUNT(LCO.AGR_ID) FROM '||V_ESQUEMA||'.'||V_TABLA_5||' LCO, '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR
	 WHERE LCO.AGR_ID = AGR.AGR_ID
	   AND AGR.USUARIOCREAR = ''MIGRAREM01BNK''
  '
  ;
