...
--v0.3
	...
	V_USUARIO	VARCHAR2(100 char) := 'SP_AGA';
...
--------------------------------------------------------------------
  ...
  LEFT JOIN '||V_ESQUEMA||'.ACT_GES_DISTRIBUCION COD
  ...
  and NOT EXISTS (
  SELECT 1 FROM '||V_ESQUEMA||'.GESTOR_ACTIVOS_NO_INSERTAR TMP WHERE TMP.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO
  )
  )
  '
  ;
...
----------------------------------------------------------------------
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' GAH_GESTOR_ACTIVO_HISTORICO cargada 3. '||v_count_1||' Filas.');
...
----------------------------------------------------------------------
  ...
  CCAA AS (
    ...
    LEFT JOIN '||V_ESQUEMA||'.GESTOR_ACTIVOS_NO_INSERTAR NOIN
...
----------------------------------------------------------------------
  ...
  INSERT INTO '||V_ESQUEMA||'.GESTOR_ACTIVOS_NO_INSERTAR (
    ...
    SELECT DISTINCT  ACT.ACT_ID, ACT.ACT_NUM_ACTIVO, GEH.GEH_ID, GEE.GEE_ID, GEE.DD_TGE_ID, TGE.DD_TGE_DESCRIPCION
    FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
    INNER JOIN '||V_ESQUEMA||'.GAH_GESTOR_ACTIVO_HISTORICO GAH
      ON ACT.ACT_ID = GAH.ACT_ID
    INNER JOIN '||V_ESQUEMA||'.GAC_GESTOR_ADD_ACTIVO GAC
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] CARGANDO LA TABLA GAH_GESTOR_ACTIVO_HISTORICO. 10..');
  ...
  '
  ;
 DBMS_OUTPUT.PUT_LINE('[INFO] CARGANDO LA TABLA GAH_GESTOR_ACTIVO_HISTORICO. 11..');
  v_count_1 := SQL%ROWCOUNT;
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' GAH_GESTOR_ACTIVO_HISTORICO cargada 5. '||v_count_1||' Filas.');
