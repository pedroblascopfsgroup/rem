...
(	
	...
	PVE_COD_UVEM		POSITION(41:60)			CHAR 													"REPLACE(REPLACE(REPLACE(TRIM(:PVE_COD_UVEM),';',' '), '\"',''),'''','')",
	...
	PVC_IND_PRINCIPAL	POSITION(900:901)		INTEGER EXTERNAL NULLIF(PVC_IND_PRINCIPAL=BLANKS)		"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:PVC_IND_PRINCIPAL),';',' '), '\"',''),'''',''),2,1))",
	PVC_COD_CARGO		POSITION(902:921)		CHAR NULLIF(PVC_COD_CARGO=BLANKS)						"REPLACE(REPLACE(REPLACE(TRIM(:PVC_COD_CARGO),';',' '), '\"',''),'''','')",
	...
	PVC_COD_USUARIO		POSITION(1205:1224)		CHAR NULLIF(PVC_COD_USUARIO=BLANKS)						"REPLACE(REPLACE(REPLACE(TRIM(:PVC_COD_USUARIO),';',' '), '\"',''),'''','')"
)
