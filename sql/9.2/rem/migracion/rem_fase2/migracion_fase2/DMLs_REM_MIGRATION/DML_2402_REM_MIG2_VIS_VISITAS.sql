...
BEGIN
      ...
      DUPLICADOS AS (
		...
		WHERE PVE_COD_UVEM = VIS.VIS_COD_PRESCRIPTOR_UVEM and DD_TPR_ID is not null)				PVE_ID_PRESCRIPTOR,
		(SELECT PVE_ID 
		FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR
		WHERE PVE_COD_UVEM = VIS.VIS_COD_API_CUSTODIO_UVEM and DD_TPR_ID is not null)				PVE_ID_API_CUSTODIO,
		(SELECT PVE_ID 
		FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR
		WHERE PVE_COD_UVEM = VIS.VIS_COD_API_RESPONSABLE_UVEM and DD_TPR_ID is not null)			PVE_ID_API_RESPONSABLE,
		(SELECT PVE_ID 
		FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR
		WHERE PVE_COD_UVEM = VIS.VIS_COD_FVD_UVEM and DD_TPR_ID is not null)						PVE_ID_FDV,
