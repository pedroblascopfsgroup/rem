...
BEGIN 
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] COMIENZA EL PROCESO DE MIGRACION SOBRE LA TABLA '||V_ESQUEMA||'.'||V_TABLA||'.');
    ...
    SELECT DISTINCT
     ...
     RES_IMPORTE_DEVUELTO
     RES_COD_TIPO_ARRA,
     RES_COD_ESTADO_RESERVA,
     RES_FECHA_SOLICITUD,
     RES_FECHA_RESOLUCION
     FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIGW
     WHERE NOT EXISTS (
      SELECT 1 FROM '||V_ESQUEMA||'.'||V_TABLA||' RESW WHERE MIG.RES_COD_NUM_RESERVA = RESW.RES_NUM_RESERVA
	)
	SELECT 
	RES.RES_COD_OFERTA									ECO_ID,
	RES.RES_COD_NUM_RESERVA								RES_NUM_RESERVA,
	RES.RES_FECHA_SOLICITUD								null,
	...
	LEFT JOIN '||V_ESQUEMA||'.MIG2_OFR_NOT_EXISTS NOTT
			ON NOTT.OFA_COD_OFERTA = MIG.RES_COD_OFERTA
	WHERE NOTT.OFA_COD_OFERTA IS NULL
	')
	;
