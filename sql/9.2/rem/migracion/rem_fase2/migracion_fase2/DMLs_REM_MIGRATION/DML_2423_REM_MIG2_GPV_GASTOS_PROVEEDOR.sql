...
BEGIN
    ...
    ELSE
     ...
     END IF;   
      ...
      V_SENTENCIA := '
		...
		,NULL 															PRG_ID
		FROM INSERTAR INS
		INNER JOIN '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG2 
			ON INS.GPV_NUM_GASTO_GESTORIA = MIG2.GPV_COD_GASTO_PROVEEDOR
		INNER JOIN '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO TGA 
			ON TGA.DD_TGA_CODIGO = MIG2.GPV_COD_TIPO_GASTO
		INNER JOIN '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO STG 
			ON STG.DD_STG_CODIGO = MIG2.GPV_COD_SUBTIPO_GASTO
		LEFT JOIN  '||V_ESQUEMA||'.DD_TPE_TIPOS_PERIOCIDAD TPE
			ON TPE.DD_TPE_CODIGO = MIG2.GPV_COD_PERIODICIDAD
		LEFT JOIN '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR PVE
			ON PVE.PVE_COD_UVEM = MIG2.GPV_COD_PVE_UVEM_EMISOR
		LEFT JOIN '||V_ESQUEMA||'.DD_DEG_DESTINATARIOS_GASTO DEG
			ON DEG.DD_DEG_CODIGO = MIG2.GPV_COD_DESTINATARIO
		'
		;
    ...
    V_SENTENCIA := 'SELECT MAX(GPV_NUM_GASTO_HAYA) FROM '||V_ESQUEMA||'.'||V_TABLA||'';
    EXECUTE IMMEDIATE V_SENTENCIA INTO MAX_NUM_GASTO_HAYA;
    
    MAX_NUM_GASTO_HAYA := MAX_NUM_GASTO_HAYA +1;
 
	IF MAX_NUM_GASTO_HAYA >= 1 THEN 
    
		EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM ALL_SEQUENCES WHERE SEQUENCE_NAME = ''S_GPV_NUM_GASTO_HAYA'' 
	...
	ELSE
		DBMS_OUTPUT.PUT_LINE('[INFO] La columna GPV_NUM_GASTO_HAYA no tiene valores, no se inicializa la secuencia.');
	END IF;
	  ...
	  IF V_REJECTS != 0 THEN
		...
		IF TABLE_COUNT != 0 THEN
		  ...
		  V_OBSERVACIONES := 'Del total de registros rechazados, '||TABLE_COUNT||' han sido por CODIGOS DD_TGA_TIPOS_GASTO inexistentes. ';
		
		END IF;
		
		IF TABLE_COUNT_2 != 0 THEN
		
		  V_OBSERVACIONES := 'Del total de registros rechazados, '||TABLE_COUNT_2||' han sido por CODIGOS DD_STG_SUBTIPOS_GASTO inexistentes. ';
