...
BEGIN 
    ...
    --COMPROBACIONES PREVIAS - PROVEEDOR DIRECCION
  DBMS_OUTPUT.PUT_LINE('[INFO] ['||V_TABLA||'] COMPROBANDO PROVEEDOR DIRECCION...');
  ...
  IF TABLE_COUNT = 0 THEN
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] TODOS LOS ACTIVOS EXISTEN EN ACT_PVE_PROVEEDOR');
    
  ELSE
  
    DBMS_OUTPUT.PUT_LINE('[INFO] SE HAN INFORMADO '||TABLE_COUNT||' PROVEEDOR DIRECCION INEXISTENTES EN ACT_PVE_PROVEEDOR. SE DERIVARÁN A LA TABLA '||V_ESQUEMA||'.MIG2_PVE_NOT_EXISTS.');
  ...
  --COMPROBACIONES PREVIAS - DD_TDP_TIPO_DIR_PROVEEDOR
    DBMS_OUTPUT.PUT_LINE('[INFO] ['||V_TABLA||'] COMPROBANDO TIPO DIRECCION...');
    ...
    V_SENTENCIA := '
        ...
        WHERE NOT EXISTS (
          ...
          WHERE DD.DD_TDP_CODIGO = MIG2.DD_TDP_TIPO_DIR_PROVEEDOR
    )
    '
    ;
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] COMIENZA EL PROCESO DE MIGRACION SOBRE LA TABLA '||V_ESQUEMA||'.'||V_TABLA||'.');
	...
	INNER JOIN '||V_ESQUEMA||'.DD_TDP_TIPO_DIR_PROVEEDOR TDP
		ON TDP.DD_TDP_CODIGO = MIG.PRD_COD_TIPO_DIRECCION AND TDP.BORRADO = 0
	LEFT JOIN '||V_ESQUEMA_MASTER||'.DD_TVI_TIPO_VIA TVI
		ON TVI.DD_TVI_CODIGO = MIG.PRD_COD_TIPO_VIA AND TVI.BORRADO = 0
	LEFT JOIN '||V_ESQUEMA_MASTER||'.DD_UPO_UNID_POBLACIONAL UPO
		ON UPO.DD_UPO_CODIGO = MIG.PRD_COD_UNIDADPOBLACIONAL AND UPO.BORRADO = 0
	LEFT JOIN '||V_ESQUEMA_MASTER||'.DD_PRV_PROVINCIA PRV
		ON PRV.DD_PRV_CODIGO = MIG.PRD_COD_PROVINCIA AND PRV.BORRADO = 0
	LEFT JOIN '||V_ESQUEMA_MASTER||'.DD_LOC_LOCALIDAD LOC
		ON LOC.DD_LOC_CODIGO = MIG.PRD_COD_LOCALIDAD AND LOC.BORRADO = 0
