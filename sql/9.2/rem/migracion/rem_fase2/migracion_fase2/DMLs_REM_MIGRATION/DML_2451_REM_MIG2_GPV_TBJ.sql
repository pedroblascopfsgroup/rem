...
--## PRODUCTO=NO
--## 
--## Finalidad: Proceso de migración MIG2_GPV_TBJ -> GPV_TBJ
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
V_TABLA VARCHAR2(40 CHAR) := 'GPV_TBJ';
V_TABLA_MIG VARCHAR2(40 CHAR) := 'MIG2_GPV_TBJ';
...
BEGIN
      ...
      WHERE NOT EXISTS (
        ...
        WHERE GPV.GPV_NUM_GASTO_GESTORIA = MIG2.GPT_COD_GASTO_PROVEEDOR
      )
      '
      ;
      ...
      ELSE
          ...
          WITH NOT_EXISTS AS (
            SELECT DISTINCT MIG2.GPT_COD_GASTO_PROVEEDOR 
            FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG2 
            WHERE NOT EXISTS (
              SELECT 1 
              FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
              WHERE MIG2.GPT_COD_GASTO_PROVEEDOR = GPV.GPV_NUM_GASTO_GESTORIA
            )
          )
          SELECT DISTINCT
          '''||V_TABLA_MIG||'''                                                   TABLA_MIG,
          MIG2.GPT_COD_GASTO_PROVEEDOR    						      GPV_NUM_GASTO_GESTORIA,          
          SYSDATE                                                                 FECHA_COMPROBACION
          FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG2  
          INNER JOIN NOT_EXISTS ON NOT_EXISTS.GPT_COD_GASTO_PROVEEDOR = MIG2.GPT_COD_GASTO_PROVEEDOR
          '
          ;
    ...
    TBJ_ID,
		...
		WITH INSERTAR AS (
		SELECT DISTINCT GPV.GPV_ID,  TBJ_ID
    
		FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG
		INNER JOIN '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV ON GPV.GPV_NUM_GASTO_GESTORIA = MIG.GPT_COD_GASTO_PROVEEDOR
    INNER JOIN '||V_ESQUEMA||'.ACT_TBJ_TRABAJO TBJ ON TBJ.TBJ_NUM_TRABAJO = MIG.GPT_TBJ_NUM_TRABAJO
	  )
		SELECT
		'||V_ESQUEMA||'.S_'||V_TABLA||'.NEXTVAL            		 		GPV_TBJ_ID, 
		GIM.GPV_ID  													GPV_ID,
    GIM.TBJ_ID                            TBJ_ID,
		0                                                  				VERSION
		FROM INSERTAR GIM
      ...
      */
	  ...
	  IF V_REJECTS != 0 THEN
		...
		IF TABLE_COUNT != 0 or TABLE_COUNT_2 != 0 THEN
		  ...
		  V_OBSERVACIONES := 'Del total de registros rechazados, '||TABLE_COUNT||' han sido por Codigos de GASTOS_PROVEEDOR inexistentes y '||TABLE_COUNT_2||' ha sido por Códigos de TBJ_NUM_TRABAJOS inexistentes.';
