...
V_TABLA_MIG VARCHAR2(40 CHAR) := 'MIG2_APD_ACTIVO_PLANDINVENTAS';
...
BEGIN
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT WHERE ACT.ACT_NUM_ACTIVO = MIG.APD_ACT_NUMERO_ACTIVO
  )
  '
  ;
  ...
  ELSE
    ...
    WITH ACT_NUM_ACTIVO AS (
		SELECT
		MIG.APD_ACT_NUMERO_ACTIVO 
		FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG 
		WHERE NOT EXISTS (
		  SELECT 1 FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT WHERE MIG.APD_ACT_NUMERO_ACTIVO = ACT.ACT_NUM_ACTIVO
		)
    )
    SELECT DISTINCT
    MIG.APD_ACT_NUMERO_ACTIVO                              						ACT_NUM_ACTIVO,
    '''||V_TABLA_MIG||'''                                                   TABLA_MIG,
    SYSDATE                                                                 FECHA_COMPROBACION
    FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG  
    INNER JOIN ACT_NUM_ACTIVO
    ON ACT_NUM_ACTIVO.ACT_NUMERO_ACTIVO = MIG.APD_ACT_NUMERO_ACTIVO
    '
    ;
  ...
  END IF;
      ...
      V_SENTENCIA := '
		...
		WHEN MATCHED THEN UPDATE SET
		  ...
		  PDV.USUARIOMODIFICAR = ''MIG2''
		  PDV.FECHAMODIFICAR = SYSDATE
      '
      ;
