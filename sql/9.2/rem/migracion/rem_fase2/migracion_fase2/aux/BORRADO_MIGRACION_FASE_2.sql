...
BEGIN
  ...
  COMMIT;   
	...
	WHERE ADE.RES_ID NOT IN (
	  SELECT ECO.RES_ID FROM RES_RESERVAS ECO WHERE ADE.RES_ID = ECO.RES_ID)
	  );
	  
	DELETE FROM CEX_COMPRADOR_EXPEDIENTE ADE2
	WHERE ADE2.COM_ID IN (
	SELECT ADE.COM_ID FROM CEX_COMPRADOR_EXPEDIENTE ADE
	WHERE ADE.COM_ID NOT IN (
	  SELECT ECO.COM_ID FROM COM_COMPRADOR ECO WHERE ADE.COM_ID = ECO.COM_ID)
	  );
