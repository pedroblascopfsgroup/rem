...
(
	...
	ACT_NUMERO_INMOVILIZADO				POSITION(282:298)		INTEGER EXTERNAL NULLIF(ACT_NUMERO_INMOVILIZADO=BLANKS)				"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:ACT_NUMERO_INMOVILIZADO),';',' '),'\"',''),'''',''),2,16))",
	...
	GESTOR_CALIDAD						POSITION(321:360)		CHAR NULLIF(GESTOR_CALIDAD=BLANKS)									"REPLACE(REPLACE(REPLACE(TRIM(:GESTOR_CALIDAD),';',' '), '\"',''),'''','')",	
	FECHA_CALIDAD						POSITION(361:368)		DATE 'YYYYMMDD' NULLIF(FECHA_CALIDAD=BLANKS) 						"REPLACE(:FECHA_CALIDAD, '00000000', '')",
