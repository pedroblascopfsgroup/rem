...
(
   ...
   ,ICO_JUSTIFICANTE_IMP_VENTA		POSITION(10660:13659)	CHAR nullif(ICO_JUSTIFICANTE_IMPORTE_VENTA=BLANKS) 				"replace (replace(replace(TRIM(:ICO_JUSTIFICANTE_IMPORTE_VENTA),';',' '), '\"',''),'''','')"
   ,ICO_JUSTIFICANTE_IMP_ALQUILER	POSITION(13660:16659)	CHAR nullif(ICO_JUSTIFICANTE_IMPORTE_ALQUILER=BLANKS) 				"replace (replace(replace(TRIM(:ICO_JUSTIFICANTE_IMPORTE_ALQUILER),';',' '), '\"',''),'''','')"
   ,ICO_AÑO_REHABILITA_EDIFICIO		POSITION(16660:16668)	INTEGER EXTERNAL  nullif (ICO_AÑO_REHABILITACION_EDIFICIO=BLANKS)	"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:ICO_AÑO_REHABILITACION_EDIFICIO),';',' '), '\"',''),'''',''),2,8))"
