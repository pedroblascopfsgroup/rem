...
(
   ...
   ,ICO_FECHA_RECEPCION_LLAVES			POSITION(10652:10659)	DATE 'YYYYMMDD' NULLIF(ICO_FECHA_RECEPCION_LLAVES=BLANKS)		"REPLACE(:ICO_FECHA_RECEPCION_LLAVES, '00000000', '')"
   ,ICO_JUSTIFICANTE_IMPORTE_VENTA		POSITION(10660:13659)	CHAR nullif(ICO_JUSTIFICANTE_IMPORTE_VENTA=BLANKS) 				"replace (replace(replace(TRIM(:ICO_JUSTIFICANTE_IMPORTE_VENTA),';',' '), '\"',''),'''','')"
   ,ICO_JUSTIFICANTE_IMPORTE_ALQUILER	POSITION(13660:16659)	CHAR nullif(ICO_JUSTIFICANTE_IMPORTE_ALQUILER=BLANKS) 				"replace (replace(replace(TRIM(:ICO_JUSTIFICANTE_IMPORTE_ALQUILER),';',' '), '\"',''),'''','')"
   ,ICO_AÑO_REHABILITACION_EDIFICIO		POSITION(16660:16668)	INTEGER EXTERNAL  nullif (ICO_AÑO_REHABILITACION_EDIFICIO=BLANKS)	"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:ICO_AÑO_REHABILITACION_EDIFICIO),';',' '), '\"',''),'''',''),2,8))"
   ,ICO_DISTRITO						POSITION(16669:16677)	INTEGER EXTERNAL  nullif (ICO_DISTRITO=BLANKS)						"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:ICO_DISTRITO),';',' '), '\"',''),'''',''),2,8))",
