...
(
	...
	OFR_COD_PRESCRIPTOR_UVEM		POSITION(308:324)	INTEGER EXTERNAL NULLIF(OFR_COD_PRESCRIPTOR_UVEM=BLANKS)		"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:OFR_COD_PRESCRIPTOR_UVEM),';',' '), '\"',''),'''',''),2,16))",
	OFR_COD_API_RESPONSABLE_UVEM	POSITION(325:341)	INTEGER EXTERNAL NULLIF(OFR_COD_API_RESPONSABLE_UVEM=BLANKS)	"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:OFR_COD_API_RESPONSABLE_UVEM),';',' '), '\"',''),'''',''),2,16))",
	OFR_COD_CUSTODIO_UVEM			POSITION(342:358)	INTEGER EXTERNAL NULLIF(OFR_COD_CUSTODIO_UVEM=BLANKS)			"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:OFR_COD_CUSTODIO_UVEM),';',' '), '\"',''),'''',''),2,16))",
	OFR_COD_FDV_UVEM				POSITION(359:375)	INTEGER EXTERNAL NULLIF(OFR_COD_FDV_UVEM=BLANKS)				"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:OFR_COD_FDV_UVEM),';',' '), '\"',''),'''',''),2,16))",
	OFR_OFICINA_TRAMITACION			POSITION(376:379)	CHAR NULLIF(OFR_OFICINA_TRAMITACION=BLANKS)						"REPLACE(REPLACE(REPLACE(TRIM(:OFR_OFICINA_TRAMITACION),';',' '), '\"',''),'''','')",
	OFR_FECHA_ANULACION				POSITION(380:387)	DATE 'YYYYMMDD' NULLIF(OFR_FECHA_ANULACION=BLANKS)				"REPLACE(:OFR_FECHA_NOTIFICACION, '00000000', '')",
	OFR_COMITE_SANCION				POSITION(388:407)	CHAR NULLIF(OFR_COMITE_SANCION=BLANKS)							"REPLACE(REPLACE(REPLACE(TRIM(:OFR_COMITE_SANCION),';',' '), '\"',''),'''','')",
	OFR_FECHA_SANCION_COMITE		POSITION(408:415)	DATE 'YYYYMMDD' NULLIF(OFR_FECHA_SANCION_COMITE=BLANKS)			"REPLACE(:OFR_FECHA_SANCION_COMITE, '00000000', '')",
