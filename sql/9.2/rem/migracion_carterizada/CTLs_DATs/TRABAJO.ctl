...
(
	...
	AGR_UVEM							POSITION(18:34)			INTEGER EXTERNAL NULLIF(AGR_UVEM=BLANKS) 							"DECODE( TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:AGR_UVEM),';',' '), '\"',''),'''',''),2,16)), '0',NULL, TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:AGR_UVEM),';',' '), '\"',''),'''',''),2,16)))",
	...
	TBJ_FECHA_DENEGACION				POSITION(1803:1810)		DATE 'YYYYMMDD' NULLIF(TBJ_FECHA_DENEGACION=BLANKS)			 		"REPLACE(:TBJ_FECHA_DENEGACION, '00000000', '')",	
	TBJ_OBSERVACIONES					POSITION(1811:2834)		CHAR NULLIF(TBJ_OBSERVACIONES=BLANKS) 								"REPLACE(REPLACE(REPLACE(TRIM(:TBJ_OBSERVACIONES||'  '),';',' '),'\"',''),'''','')",
	TBJ_CONTINUO_OBSERVACIONES			POSITION(2835:3858)		CHAR NULLIF(TBJ_CONTINUO_OBSERVACIONES=BLANKS) 						"REPLACE(REPLACE(REPLACE(TRIM(:TBJ_CONTINUO_OBSERVACIONES),';',' '),'\"',''),'''','')",
	TBJ_IMPORTE_TOTAL					POSITION(3859:3875)		INTEGER EXTERNAL NULLIF(TBJ_IMPORTE_TOTAL=BLANKS)					"CASE WHEN (:TBJ_IMPORTE_TOTAL) IS NULL THEN NULL ELSE TO_NUMBER(REPLACE(REPLACE(REPLACE(TRIM(SUBSTR(:TBJ_IMPORTE_TOTAL,1,15)||','||SUBSTR(:TBJ_IMPORTE_TOTAL,16,2)),';',' '), '\"',''),'''','')) END",
