...
DECLARE
    ...
    V_VAL T_ARRAY_VAL := T_ARRAY_VAL(
        ...
        T_VAL('ACT_006'   ,'1'           ,'Activos en estado "Disponible para la venta con reserva" creados por la migración que no dispongan de reserva'                                                     ,'MIG_ACA_CABECERA'                  ,'SELECT DISTINCT #SELECT_STATEMENT# FROM REM01.MIG_ACA_CABECERA AUX WHERE AUX.SITUACION_COMERCIAL = ''''04'''' MINUS SELECT DISTINCT #SELECT_STATEMENT# FROM REM01.MIG_ACA_CABECERA AUX INNER JOIN REM01.MIG2_OFA_OFERTAS_ACTIVO OFA ON OFA.OFA_ACT_NUMERO_ACTIVO = AUX.ACT_NUMERO_ACTIVO INNER JOIN REM01.MIG2_RES_RESERVAS RES ON RES.RES_COD_OFERTA = OFA.OFA_COD_OFERTA WHERE AUX.SITUACION_COMERCIAL = ''''04'''''),
        ...
        T_VAL('AGR_002'   ,'1'           ,'Agrupaciones sin activos'                                                                                                                                          ,'MIG_AAG_AGRUPACIONES'              ,'SELECT DISTINCT #SELECT_STATEMENT# FROM REM01.MIG_AAG_AGRUPACIONES AUX LEFT JOIN REM01.MIG_AAA_AGRUPACION_ACTIVO AAA ON AAA.AGR_UVEM = AUX.AGR_UVEM LEFT JOIN REM01.MIG_ACA_CABECERA ACT ON ACT.ACT_NUMERO_ACTIVO = AAA.ACT_NUMERO_ACTIVO WHERE ACT.ACT_NUMERO_ACTIVO IS NULL'),
