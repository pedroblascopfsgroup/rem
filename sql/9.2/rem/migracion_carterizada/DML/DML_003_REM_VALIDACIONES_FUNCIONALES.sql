...
DECLARE
    ...
    V_VAL T_ARRAY_VAL := T_ARRAY_VAL(
        ...
        T_VAL('GPV_006'   ,'1'           ,'Gastos con IVA que no tengan el codigo de gasto agrupado'                                                                                            		      ,'MIG2_GPV_GASTOS_PROVEEDORES'       ,'SELECT DISTINCT #SELECT_STATEMENT# FROM MIG2_GPV_GASTOS_PROVEEDORES AUX JOIN MIG2_GDE_GASTOS_DET_ECONOMICO T1 ON T1.GDE_GPV_ID = AUX.GPV_ID AND T1.GDE_COD_TIPO_IMPUESTO = ''''01'''' AND AUX.GPV_COD_GASTO_AGRUPADO IS NULL'),
        T_VAL('GPV_007'   ,'1'           ,'Gastos cuyo tipo de operacion no sea Pago gasto, Pago gasto inversion o Abono/Factura rectificativa'                                                               ,'MIG2_GPV_GASTOS_PROVEEDORES'       ,'SELECT DISTINCT #SELECT_STATEMENT# FROM MIG2_GPV_GASTOS_PROVEEDORES AUX WHERE NVL(AUX.GPV_COD_TIPO_OPERACION,''''00'''') NOT IN (''''01'''',''''02'''',''''03'''')')
        );
