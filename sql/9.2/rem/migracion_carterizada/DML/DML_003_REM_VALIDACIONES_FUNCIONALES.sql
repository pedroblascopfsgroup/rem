...
DECLARE
    ...
    V_VAL T_ARRAY_VAL := T_ARRAY_VAL(
        ...
        T_VAL('OFR_001'   ,'1'           ,'Ofertas en estado distinto a anulación con motivo de anulación informado'                                                                                          ,'MIG2_OFR_OFERTAS'                  ,'SELECT DISTINCT #SELECT_STATEMENT# FROM REM01.MIG2_OFR_OFERTAS AUX WHERE (AUX.OFR_COD_ESTADO_OFERTA != ''''01-07'''' AND AUX.OFR_COD_MOTIVO_ANULACION != 0) OR (AUX.OFR_COD_MOTIVO_ANULACION != 0 AND AUX.OFR_FECHA_ANULACION IS NULL)'),
        ...
        T_VAL('ETP_001'   ,'1'           ,'Entidad/Proveedor que ya existan en la tabla ACT_ETP_ENTIDAD_PROVEEDOR'                                                                                            ,'MIG_AEP_ENTIDAD_PROVEEDOR'         ,'SELECT DISTINCT #SELECT_STATEMENT# FROM MIG_AEP_ENTIDAD_PROVEEDOR AUX WHERE EXISTS (SELECT 1 FROM REM01.ACT_ETP_ENTIDAD_PROVEEDOR T1 JOIN REM01.ACT_PVE_PROVEEDOR T2 ON T1.PVE_ID = T2.PVE_ID JOIN REM01.DD_CRA_CARTERA T3 ON T3.DD_CRA_ID = T1.DD_CRA_ID WHERE AUX.PVE_DOCIDENTIF = T2.PVE_COD_UVEM AND AUX.ENTIDAD_PROPIETARIA = T3.DD_CRA_CODIGO)')
    );
