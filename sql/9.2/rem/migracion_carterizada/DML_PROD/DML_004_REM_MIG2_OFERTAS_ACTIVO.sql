...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] COMIENZA EL PROCESO DE MIGRACION SOBRE LA TABLA '||V_ESQUEMA||'.'||V_TABLA||'.');
        ...
        WITH INSERTAR AS (
		  ...
		  DUPLICADOS AS(
			SELECT ACT_ID, OFR_ID FROM MIG2_OFA_OFERTAS_ACTIVO MIG
			INNER JOIN ACT_ACTIVO ACT
			  ON MIG.OFA_ACT_NUMERO_ACTIVO = ACT.ACT_NUM_ACTIVO
			INNER JOIN OFR_OFERTAS OFR
			  ON MIG.OFA_COD_OFERTA = OFR.OFR_NUM_OFERTA
			WHERE EXISTS (
			  SELECT 1 FROM ACT_OFR ACT_OFR
			  WHERE ACT_OFR.ACT_ID = ACT.ACT_ID
        ...
        WHERE NOT EXISTS (
			SELECT 1 FROM DUPLICADOS DUP
			WHERE ACT_OFR.ACT_ID = DUP.ACT_ID
			AND ACT_OFR.OFR_ID = DUP.OFR_ID
			)
        ')
        ;
