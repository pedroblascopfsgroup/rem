...
DECLARE
	...
	V_TABLA T_ARRAY_TABLAS := T_ARRAY_TABLAS(
	    ...
	    T_TABLAS('REMMASTER','GRU_GRUPOS_USUARIOS'),
		T_TABLAS('REMMASTER','USU_USUARIOS'),
		T_TABLAS('REM01','USD_USUARIOS_DESPACHOS'),
		T_TABLAS('REM01','ZON_PEF_USU')
	);
...
BEGIN
  ...
  commit;
  delete from rem01.ACT_ETP_ENTIDAD_PROVEEDOR where usuariocrear = '#USUARIO_MIGRACION#';
  ...
  UPDATE REM01.ACT_PRO_PROPIETARIO PRO
	...
	DELETE FROM REMMASTER.GRU_GRUPOS_USUARIOS WHERE USUARIOCREAR = '#USUARIO_MIGRACION#';
	DELETE FROM REMMASTER.USU_USUARIOS WHERE USUARIOCREAR = '#USUARIO_MIGRACION#';
	DELETE FROM REM01.USD_USUARIOS_DESPACHOS WHERE USUARIOCREAR = '#USUARIO_MIGRACION#';
	DELETE FROM REM01.ZON_PEF_USU WHERE USUARIOCREAR = '#USUARIO_MIGRACION#';

	DELETE FROM REM01.ACT_INP_INC_PRESUPUESTO T2 WHERE NOT EXISTS (SELECT 1 FROM REM01.ACT_TBJ_TRABAJO T1 WHERE T2.TBJ_ID = T1.TBJ_ID);
	DELETE FROM REM01.PRP_PROPUESTAS_PRECIOS T2 WHERE NOT EXISTS (SELECT 1 FROM REM01.ACT_TBJ_TRABAJO T1 WHERE T2.TBJ_ID = T1.TBJ_ID);
	DELETE FROM REM01.OEX_OBS_EXPEDIENTE T2 WHERE NOT EXISTS (SELECT 1 FROM REM01.ECO_EXPEDIENTE_COMERCIAL T1 WHERE T2.ECO_ID = T1.ECO_ID);
	DELETE FROM REM01.AIN_ACTIVO_INTEGRADO T2 WHERE NOT EXISTS (SELECT 1 FROM REM01.ACT_PVE_PROVEEDOR T1 WHERE T2.PVE_ID = T1.PVE_ID);
	DELETE FROM REM01.ECO_TAN_TANTEO_ACTIVO T1 WHERE NOT EXISTS (SELECT 1 FROM REM01.ECO_EXPEDIENTE_COMERCIAL T2 WHERE T1.ECO_ID = T2.ECO_ID);
	DELETE FROM REM01.ACT_ECO_INFORME_JURIDICO T1 WHERE NOT EXISTS (SELECT 1 FROM REM01.ECO_EXPEDIENTE_COMERCIAL T2 WHERE T1.ECO_ID = T2.ECO_ID);
	DELETE FROM REM01.ECO_COND_CONDICIONES_ACTIVO T1 WHERE NOT EXISTS (SELECT 1 FROM REM01.ECO_EXPEDIENTE_COMERCIAL T2 WHERE T1.ECO_ID = T2.ECO_ID);
	DELETE FROM REM01.ACT_PDV_PLAN_DIN_VENTAS T1 WHERE NOT EXISTS (SELECT 1 FROM REM01.ACT_ACTIVO T2 WHERE T1.ACT_ID = T2.ACT_ID);
