...
--## INCIDENCIA_LINK=HREOS-2371
--## PRODUCTO=NO
--## 
--## Finalidad: Proceso de borrado lógico de stock no recibido
--##            
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  cartera REM01.DD_CRA_CARTERA.DD_CRA_DESCRIPCION%TYPE := '#CARTERA#';
  ...
  USUARIOBORRAR REM01.ACT_ACTIVO.USUARIOBORRAR%TYPE := 'BORRADO_STOCK_NO_RECIBIDO_';
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Inicio del proceso de borrado de stock no recibido para la cartera '||CARTERA_DESC);
    
    V_MSQL := 'MERGE INTO REM01.ACT_ACTIVO T1
        USING (SELECT T1.ACT_ID
            FROM REM01.ACT_ACTIVO T1
            JOIN REM01.DD_CRA_CARTERA T2 ON T1.DD_CRA_ID = T2.DD_CRA_ID AND T2.DD_CRA_CODIGO = '''||CARTERA_COD||'''
            LEFT JOIN REM01.MIG_ACA_CABECERA T3 ON T3.ACT_NUMERO_ACTIVO = T1.ACT_NUM_ACTIVO AND T3.VALIDACION IN (0,1)
            WHERE T3.ACT_NUMERO_ACTIVO IS NULL AND T1.BORRADO = 0) T2
        ON (T1.ACT_ID = T2.ACT_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.USUARIOBORRAR = '''||USUARIOBORRAR||CARTERA_DESC||'''
            , T1.FECHABORRAR = SYSTIMESTAMP, T1.BORRADO = 1
        WHERE T1.BORRADO = 0';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      [INFO] - '||TO_CHAR(SYSDATE,'HH24:MI:SS')||' - Se han desactivado '||SQL%ROWCOUNT||' filas en REM01.ACT_ACTIVO');
    
    V_MSQL := 'MERGE INTO REM01.ACT_TBJ_TRABAJO T1
        USING (SELECT R.TBJ_ID
            FROM REM01.ACT_ACTIVO A
            JOIN REM01.ACT_TBJ R ON A.ACT_ID = R.ACT_ID
            WHERE A.BORRADO = 1 AND A.USUARIOBORRAR = '''||USUARIOBORRAR||CARTERA_DESC||'''
        ...
        WHERE T1.BORRADO = 0';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      [INFO] - '||TO_CHAR(SYSDATE,'HH24:MI:SS')||' - Se han desactivado '||SQL%ROWCOUNT||' filas en REM01.ACT_TBJ_TRABAJO');

    EXECUTE IMMEDIATE 'MERGE INTO ACT_TBJ_TRABAJO TBJ
        USING (SELECT DISTINCT t3.TBJ_ID
               FROM act_activo t1
               join ACT_TBJ t2 on t1.act_id = t2.act_id
               join ACT_TBJ_TRABAJO t3 on t3.TBJ_ID = t2.TBJ_ID and t3.borrado = 1
               where t1.borrado = 0) AUX
        ON(TBJ.TBJ_ID = AUX.TBJ_ID)
        WHEN MATCHED THEN UPDATE SET
        TBJ.BORRADO = 0,
        TBJ.FECHABORRAR = NULL,
        TBJ.USUARIOBORRAR = NULL';
    DBMS_OUTPUT.PUT_LINE('      [INFO] - '||TO_CHAR(SYSDATE,'HH24:MI:SS')||' - Se han reactivado '||SQL%ROWCOUNT||' filas en REM01.ACT_TBJ_TRABAJO');

    V_MSQL := 'MERGE INTO REM01.ACT_AGR_AGRUPACION T1
        USING (SELECT R.AGR_ID
            FROM REM01.ACT_ACTIVO A
            JOIN REM01.ACT_AGA_AGRUPACION_ACTIVO R ON A.ACT_ID = R.ACT_ID
            WHERE A.BORRADO = 1 AND A.USUARIOBORRAR = '''||USUARIOBORRAR||CARTERA_DESC||'''
        ...
        WHERE T1.BORRADO = 0';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('      [INFO] - '||TO_CHAR(SYSDATE,'HH24:MI:SS')||' - Se han desactivado '||SQL%ROWCOUNT||' filas en REM01.ACT_AGR_AGRUPACION');

    EXECUTE IMMEDIATE 'MERGE INTO REM01.ACT_AGR_AGRUPACION T1
        USING (SELECT DISTINCT T1.AGR_ID FROM ACT_AGR_AGRUPACION T1
            JOIN REM01.ACT_AGA_AGRUPACION_ACTIVO T2 ON T1.AGR_ID = T2.AGR_ID AND T2.BORRADO = 0
            JOIN REM01.ACT_ACTIVO T3 ON T3.ACT_ID = T2.ACT_ID AND T3.BORRADO = 0
            WHERE T1.BORRADO = 1) T2
        ON (T1.AGR_ID = T2.AGR_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.USUARIOBORRAR = NULL, T1.FECHABORRAR = NULL, T1.BORRADO = 0';

    DBMS_OUTPUT.PUT_LINE('      [INFO] - '||TO_CHAR(SYSDATE,'HH24:MI:SS')||' - Se han reactivado '||SQL%ROWCOUNT||' filas en REM01.ACT_AGR_AGRUPACION');
        ...
        BEGIN
            V_MSQL := 'MERGE INTO '||TABLA||' T1
                USING (SELECT '||CLAVE_REF||' CLAVE_REF
                    FROM '||TABLA_REF||' T2
                    WHERE T2.BORRADO = 1 AND T2.USUARIOBORRAR = '''||USUARIOBORRAR||CARTERA_DESC||''') T2
