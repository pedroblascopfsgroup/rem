...
BEGIN
    ...
    USING REM01.MIG_ACA_CABECERA T2
    ON (T1.ACT_NUM_ACTIVO = T2.ACT_NUMERO_ACTIVO AND T2.VALIDACION IN (0,1))
    WHEN MATCHED THEN UPDATE SET
        T1.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS''
    WHERE T1.DD_CRA_ID IN (SELECT DD_CRA_ID FROM REM01.DD_CRA_CARTERA 
      WHERE DD_CRA_CODIGO IN (''01'',''04'',''06'',''07'',''08''))';
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'MERGE INTO REM01.ACT_VAL_VALORACIONES T1
    USING REM01.ACT_ACTIVO T2
    ON (T1.ACT_ID = T2.ACT_ID AND T2.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS'')
    WHEN MATCHED THEN UPDATE SET
        T1.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS''';
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'MERGE INTO REM01.ACT_AGA_AGRUPACION_ACTIVO T1
    USING REM01.ACT_ACTIVO T2
    ON (T1.ACT_ID = T2.ACT_ID AND T2.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS'')
    WHEN MATCHED THEN UPDATE SET
        T1.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS''';
    ...
    USING (SELECT DISTINCT T1.AGR_ID
        FROM REM01.ACT_AGA_AGRUPACION_ACTIVO T1
        WHERE NOT EXISTS 
            (SELECT 1 
            FROM REM01.ACT_AGA_AGRUPACION_ACTIVO T2 
            WHERE T1.AGR_ID = T2.AGR_ID 
                AND NVL(T2.USUARIOBORRAR,'' '') <> ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS'')) T2
    ...
    USING (SELECT T1.TBJ_ID FROM ACT_TBJ_TRABAJO T1
        WHERE NOT EXISTS 
            (SELECT 1 
                FROM REM01.ACT_TBJ T2 
                JOIN REM01.ACT_ACTIVO T3 ON T2.ACT_ID = T3.ACT_ID 
                    AND NVL(T3.USUARIOBORRAR,'' '') <> ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS''
                WHERE T1.TBJ_ID = T2.TBJ_ID)) T2
    ON (T1.TBJ_ID = T2.TBJ_ID)
    WHEN MATCHED THEN UPDATE SET
        T1.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS''';
    EXECUTE IMMEDIATE V_MSQL;
        
    V_MSQL := 'MERGE INTO REM01.ACT_LLV_LLAVE T1
    USING REM01.ACT_ACTIVO T2
    ON (T1.ACT_ID = T2.ACT_ID AND T2.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS'')
    WHEN MATCHED THEN UPDATE SET
        T1.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS''';
    ...
    V_MSQL := 'MERGE INTO REM01.BIE_CAR_CARGAS T1
    USING REM01.ACT_ACTIVO T2
    ON (T1.BIE_ID = T2.BIE_ID AND T2.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS'')
    WHEN MATCHED THEN UPDATE SET
        T1.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS''';
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'DELETE FROM REM01.ACT_SDV_SUBDIVISION_ACTIVO T1 
        WHERE EXISTS (SELECT 1 FROM REM01.ACT_ONV_OBRA_NUEVA T3 
            JOIN REM01.ACT_AGR_AGRUPACION T2 ON T2.AGR_ID = T3.AGR_ID 
            WHERE T2.AGR_ID = T1.AGR_ID AND T2.USUARIOBORRAR = ''BORRADO_FISICO_TABLAS_ACTIVOS_RECIBIDOS'')';
    EXECUTE IMMEDIATE V_MSQL;
    ...
    V_STMT_VAL := 'WITH DEPENDENCIAS AS (
         ...
         JOIN ALL_CONSTRAINTS T2 ON T2.CONSTRAINT_NAME = T1.R_CONSTRAINT_NAME
             ...
             AND T2.TABLE_NAME IN (''ACT_AGR_AGRUPACION'',''ACT_VAL_VALORACIONES'',''ACT_LLV_LLAVE'',''ACT_TBJ_TRABAJO'',''BIE_CAR_CARGAS'')
        ...
        ORDER BY DECODE(D.TABLE_REFERENCED,''BIE_CAR_CARGAS'',0,''ACT_AGR_AGRUPACION'',1,''ACT_VAL_VALORACIONES'',1,''ACT_LLV_LLAVE'',1,''ACT_TBJ_TRABAJO'',2)';
