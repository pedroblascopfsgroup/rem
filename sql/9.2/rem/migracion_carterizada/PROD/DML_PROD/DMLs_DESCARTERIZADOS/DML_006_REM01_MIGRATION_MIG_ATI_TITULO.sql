...
BEGIN
  ...
  EXECUTE IMMEDIATE 'MERGE INTO REM01.ACT_TIT_TITULO T1
  	USING (SELECT T1.ACT_ID, MIG.ESTADO_TITULO, MIG.TIT_FECHA_INSC_REG
  		FROM REM01.MIG_ATI_TITULO MIG
  		JOIN REM01.ACT_ACTIVO T1 ON T1.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO AND T1.BORRADO = 0
  		WHERE MIG.VALIDACION = 0) T2
  	ON (T1.ACT_ID = T2.ACT_ID)
  	WHEN MATCHED THEN UPDATE SET
  		T1.DD_ETI_ID = (SELECT DD_ETI_ID FROM REM01.DD_ETI_ESTADO_TITULO DD WHERE DD.DD_ETI_CODIGO = MIG.ESTADO_TITULO)
  		, T1.TIT_FECHA_INSC_REG = T2.TIT_FECHA_INSC_REG';

  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' actualizada (ESTADO_TITULO/TIT_FECHA_INSC_REG). '||SQL%ROWCOUNT||' Filas.');
