...
BEGIN
  ...
  V_SENTENCIA := '
    ...
    FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG 
      ...
      LEFT JOIN '||V_ESQUEMA||'.CLC_CLIENTE_COMERCIAL CLC ON CLC.CLC_WEBCOM_ID_OLD = MIG.OFR_COD_CLIENTE_WEBCOM
...
DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' actualizada (DD_EOF_ID). '||SQL%ROWCOUNT||' Filas.');
  ...
  V_SENTENCIA := 'BEGIN '||V_ESQUEMA||'.OPERACION_DDL.DDL_TABLE(''ANALYZE'',''ECO_EXPEDIENTE_COMERCIAL'',''1''); END;';
  EXECUTE IMMEDIATE V_SENTENCIA;
