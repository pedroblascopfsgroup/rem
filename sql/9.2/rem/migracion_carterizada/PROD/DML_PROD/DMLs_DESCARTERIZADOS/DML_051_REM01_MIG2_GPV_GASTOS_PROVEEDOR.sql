...
BEGIN
    ...
    V_SENTENCIA := 'TRUNCATE TABLE REM01.MIG_AUX_GASTOS_FILTRADOS';
    EXECUTE IMMEDIATE V_SENTENCIA;
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' Tabla MIG_AUX_GASTOS_FILTRADOS truncada.');

    V_SENTENCIA := 'BEGIN OPERACION_DDL.DDL_TABLE(''ANALYZE'',''MIG_AUX_GASTOS_FILTRADOS'',''1''); END;';
    EXECUTE IMMEDIATE V_SENTENCIA;   

    FOR i IN V_VAL.FIRST..V_VAL.LAST
        LOOP
        V_TMP_VAL := V_VAL(i);
            V_SENTENCIA := 'INSERT INTO MIG_AUX_GASTOS_FILTRADOS
                SELECT GPV_NUM_GASTO_HAYA, DD_TGA_ID, DD_TPR_ID, DD_STG_ID
                FROM (
                    SELECT
                        ...
                        ,ROW_NUMBER() OVER(PARTITION BY MIG2.GPV_ID ORDER BY TGA.DD_TGA_ID, TPR.DD_TPR_ID, STG.DD_STG_ID) RN
                    ...
                    JOIN REM01.MIG_AUX_GASTOS_FILTRO FIL ON NVL(FIL.DD_TGA_CODIGO,TGA.DD_TGA_CODIGO) = TGA.DD_TGA_CODIGO AND NVL(FIL.DD_TPR_CODIGO,''0'') = TPR.DD_TPR_CODIGO AND NVL(FIL.DD_STG_CODIGO,STG.DD_STG_CODIGO) = STG.DD_STG_CODIGO
                    LEFT JOIN REM01.GPV_GASTOS_PROVEEDOR GPV ON GPV.GPV_NUM_GASTO_HAYA = MIG2.GPV_ID
                    LEFT JOIN REM01.MIG_AUX_GASTOS_FILTRADOS AUX ON AUX.GPV_NUM_GASTO_HAYA = MIG2.GPV_ID
                    WHERE MIG2.VALIDACION = 0 AND GPV.GPV_ID IS NULL AND AUX.GPV_NUM_GASTO_HAYA IS NULL '||V_TMP_VAL(1)||')
                WHERE RN = 1';
            EXECUTE IMMEDIATE V_SENTENCIA;
            DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.MIG_AUX_GASTOS_FILTRADOS cargada. '||SQL%ROWCOUNT||' Filas de gastos de proveedores '||V_TMP_VAL(2)||'.');
        END LOOP;
        COMMIT;
    
    V_SENTENCIA := 'BEGIN OPERACION_DDL.DDL_TABLE(''ANALYZE'',''MIG_AUX_GASTOS_FILTRADOS'',''1''); END;';
    EXECUTE IMMEDIATE V_SENTENCIA; 
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' Tabla MIG_AUX_GASTOS_FILTRADOS analizada.');

    V_SENTENCIA := '
        INSERT INTO /*+ APPEND*/ REM01.GPV_GASTOS_PROVEEDOR
        (GPV_ID, GPV_NUM_GASTO_HAYA, GPV_NUM_GASTO_GESTORIA, GPV_REF_EMISOR, DD_TGA_ID
            ,DD_STG_ID, GPV_CONCEPTO, DD_TPE_ID, PVE_ID_EMISOR, PRO_ID, GPV_FECHA_EMISION, GPV_FECHA_NOTIFICACION, DD_DEG_ID, GPV_CUBRE_SEGURO
            ...
            ,TPE.DD_TPE_ID
            ,PVE.PVE_ID
            ,CASE WHEN CRA.DD_CRA_ID IS NOT NULL THEN PRO.PRO_ID ELSE (SELECT PRO_ID FROM REM01.ACT_PRO_PROPIETARIO WHERE PRO_DOCIDENTIF = ''A86602158'') END PRO_ID
        ...
        FROM REM01.MIG_AUX_GASTOS_FILTRADOS AUX
        JOIN REM01.MIG2_GPV_GASTOS_PROVEEDORES MIG2 ON AUX.GPV_NUM_GASTO_HAYA = MIG2.GPV_ID
        JOIN REM01.MIG2_GDE_GASTOS_DET_ECONOMICO GDE ON GDE.GDE_GPV_ID = MIG2.GPV_ID
        JOIN REM01.MIG_AUX_ACT_PVE_PROVEEDOR PVE ON PVE.PVE_COD_UVEM = TO_CHAR(MIG2.GPV_COD_PVE_UVEM_EMISOR) AND AUX.DD_TPR_ID = PVE.DD_TPR_ID
        LEFT JOIN REM01.DD_TPE_TIPOS_PERIOCIDAD TPE ON TPE.DD_TPE_CODIGO = NVL(MIG2.GPV_COD_PERIODICIDAD, ''01'')            
        LEFT JOIN REM01.DD_DEG_DESTINATARIOS_GASTO DEG ON DEG.DD_DEG_CODIGO = ''01''
        LEFT JOIN REM01.DD_TOG_TIPO_OPERACION_GASTO TOG ON TOG.DD_TOG_CODIGO = MIG2.GPV_COD_TIPO_OPERACION
        LEFT JOIN REM01.DD_CRA_CARTERA CRA ON UPPER(CRA.DD_CRA_DESCRIPCION) = SUBSTR('''||V_USUARIO||''',5)
        LEFT JOIN REM01.ACT_PRO_PROPIETARIO PRO ON PRO.PRO_DOCIDENTIF = ''A86602158'' AND CRA.DD_CRA_ID = PRO.DD_CRA_ID';
    EXECUTE IMMEDIATE V_SENTENCIA;
      
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' cargada. '||SQL%ROWCOUNT||' Filas de gastos de proveedores.');
    COMMIT;
      
    V_SENTENCIA := 'BEGIN '||V_ESQUEMA||'.OPERACION_DDL.DDL_TABLE(''ANALYZE'','''||V_TABLA||''',''1''); END;';
    EXECUTE IMMEDIATE V_SENTENCIA;
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' Tabla '||V_TABLA||' analizada.');
