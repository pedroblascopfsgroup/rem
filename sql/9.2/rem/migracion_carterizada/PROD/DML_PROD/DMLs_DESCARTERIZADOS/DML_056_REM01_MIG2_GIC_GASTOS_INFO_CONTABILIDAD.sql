...
BEGIN
      ...
      EXECUTE IMMEDIATE '
            ...
            FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG2
      		...
      		WHERE MIG2.VALIDACION = 0 AND NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.'||V_TABLA||' T1 WHERE T1.GPV_ID = GPV.GPV_ID)'
      ;
      ...
      USING (
            SELECT /*+ NO_INDEX(GPV GPV_GASTOS_PROVEEDOR_PK, GDE GDE_GASTOS_DETALLE_ECO_PK, GIC GIC_GASTOS_INFO_CONTA_PK)  */
                GPV.GPV_ID, CPP.CPP_PARTIDA_PRESUPUESTARIA
      ...
      USING (
            SELECT /*+ NO_INDEX(GPV GPV_GASTOS_PROVEEDOR_PK, GDE GDE_GASTOS_DETALLE_ECO_PK, GIC GIC_GASTOS_INFO_CONTA_PK)  */
                   GPV.GPV_ID, CCC.CCC_CUENTA_CONTABLE
            FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
            INNER JOIN '||V_ESQUEMA||'.GDE_GASTOS_DETALLE_ECONOMICO GDE on GPV.gpv_id = GDE.gpv_id
            INNER JOIN '||V_ESQUEMA||'.GIC_GASTOS_INFO_CONTABILIDAD GIC on GPV.gpv_id = GIC.gpv_id
