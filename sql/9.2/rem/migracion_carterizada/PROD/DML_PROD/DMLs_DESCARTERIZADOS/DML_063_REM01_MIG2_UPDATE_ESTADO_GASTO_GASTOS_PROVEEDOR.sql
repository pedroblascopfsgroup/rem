  ...
  */
    V_SENTENCIA := '
        ...
        USING (SELECT GPV.GPV_ID 
            ...
            WHERE GPV.USUARIOCREAR = '''||V_USUARIO||''' AND (GDE.GDE_FECHA_PAGO IS NOT NULL OR EAP.DD_EAP_CODIGO IN (''05'',''06'',''07'')) AND GPV.DD_EGA_ID IS NULL) T2
        ON (T1.GPV_ID = T2.GPV_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.DD_EGA_ID = (SELECT DD_EGA_ID FROM REM01.DD_EGA_ESTADOS_GASTO WHERE DD_EGA_CODIGO = ''05'')
    '
    ...
    V_SENTENCIA := '
        ...
        USING (SELECT distinct GGE.GGE_ID 
            ...
            WHERE GPV.USUARIOCREAR = '''||V_USUARIO||''' AND GPV.DD_EGA_ID = (SELECT DD_EGA_ID FROM REM01.DD_EGA_ESTADOS_GASTO WHERE DD_EGA_CODIGO = ''05'')) T2
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' ACTUALIZADAS. '||V_REG_ACTUALIZADOS||' Filas. GASTOS EN ESTADO RECHAZADO PROPIETARIO');
        ...
        USING (SELECT GGE.GGE_ID 
            ...
            WHERE GPV.USUARIOCREAR = '''||V_USUARIO||''' AND GPV.DD_EGA_ID = (SELECT DD_EGA_ID FROM REM01.DD_EGA_ESTADOS_GASTO WHERE DD_EGA_CODIGO = ''08'')) T2
        ON (T1.GGE_ID = T2.GGE_ID)
        WHEN MATCHED THEN UPDATE SET
    ...
    EXECUTE IMMEDIATE 'update rem01.GGE_GASTOS_GESTION t1 set t1.dd_eah_id = (select dd_eah_id from rem01.DD_EAH_ESTADOS_AUTORIZ_HAYA where dd_eah_codigo = ''01'')
             where exists (SELECT 
            FROM REM01.GPV_GASTOS_PROVEEDOR GPV
            JOIN REM01.GGE_GASTOS_GESTION GGE ON GGE.GPV_ID = GPV.GPV_ID
            JOIN REM01.GDE_GASTOS_DETALLE_ECONOMICO GDE ON GDE.GPV_ID = GPV.GPV_ID
            JOIN REM01.GIC_GASTOS_INFO_CONTABILIDAD GIC ON GIC.GPV_ID = GPV.GPV_ID
            WHERE GGE.GGE_ID = t1.GGE_ID and GPV.DD_EGA_ID = (SELECT DD_EGA_ID FROM REM01.DD_EGA_ESTADOS_GASTO WHERE DD_EGA_CODIGO = ''08'') and dd_eah_id <> (select dd_eah_id from rem01.DD_EAH_ESTADOS_AUTORIZ_HAYA where dd_eah_codigo = ''01''))';
    
    COMMIT;

    V_SENTENCIA := 'MERGE INTO REM01.GPV_GASTOS_PROVEEDOR T1
