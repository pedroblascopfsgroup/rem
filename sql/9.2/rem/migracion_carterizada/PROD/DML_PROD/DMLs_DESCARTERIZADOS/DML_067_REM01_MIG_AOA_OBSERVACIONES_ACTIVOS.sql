...
BEGIN
	...
	WHERE EXISTS (
		SELECT 1 FROM '||V_ESQUEMA||'.'||V_TABLA_MIG||' MIG, '||V_ESQUEMA||'.ACT_ACTIVO ACT WHERE MIG.ACT_NUMERO_ACTIVO = ACT.ACT_NUM_ACTIVO_UVEM AND ACT.ACT_ID = AOB.ACT_ID
	)';
  ...
  -- EL SEGUNDO WITH OBTIENE LAS USU_ID DE LA TABLA MIG QUE EXISTEN EN LA TABLA DE USU_USUARIOS 
	...
	(SELECT ACT.ACT_ID 
	  ...
	  WHERE ACT.ACT_NUM_ACTIVO_UVEM = MIG.ACT_NUMERO_ACTIVO)    	ACT_ID,
	(SELECT USU_ID 
  FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS
  WHERE USU_USERNAME = MIG.USERNAME)                          	USU_ID,
