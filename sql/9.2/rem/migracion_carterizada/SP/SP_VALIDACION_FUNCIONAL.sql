...
BEGIN
    ...
    LOOP
        ...
        V_SQL := 'SELECT CASE FUNC.SEVERIDAD
                    WHEN 1 THEN ''[CRITICAL] - ''||DIC.MOTIVO_RECHAZO ||'' - ''||FUNC.VALIDACION
                    ELSE ''[WARNING] - ''||DIC.MOTIVO_RECHAZO ||'' - ''||FUNC.VALIDACION
        ...
        vQuery := REPLACE(vQuery, '#SELECT_STATEMENT#', V_ESQUEMA||'.'||V_TABLA_VALIDACION_SEC||'.NEXTVAL , '''||vTabla||''', AUX.'||vClave||', '''||CODIGO_RECHAZO_FUN||''', '''||vMotivoRechazo||'''');
        
        -- Insertamos en VALIDACIONES_MIGRACION
        V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA_VALIDACION||' (VALIDACION_ID, NOMBRE_INTERFAZ, CLAVE_DATO, COD_RECHAZO, MOTIVO_RECHAZO) '||vQuery;
