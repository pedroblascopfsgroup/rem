...
(   
	...
	PVE_COD_ORIGEN           	POSITION(18:34)      INTEGER EXTERNAL                   				"TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:PVE_COD_ORIGEN),';',' '), '\"',''),'''',''),2,16))",
	...
	PRD_COD_LOCALIDAD           POSITION(187:206)    CHAR  NULLIF(PRD_COD_LOCALIDAD=BLANKS)             "LPAD(NVL(REPLACE(REPLACE(REPLACE(TRIM(:PRD_COD_LOCALIDAD),';',' '), '\"',''),'''',''),'00000'),5,'0')",
	...
	PRD_LOCAL_ABIERTO_PUBLICO   POSITION(579:580)    INTEGER EXTERNAL  NULLIF(PRD_LOCAL_ABIERTO_PUBLICO=BLANKS)   "TO_NUMBER(SUBSTR(REPLACE(REPLACE(REPLACE(TRIM(:PRD_LOCAL_ABIERTO_PUBLICO),';',' '), '\"',''),'''',''),2,1))",
	PRD_ESCALERA           		POSITION(581:590)    CHAR  NULLIF(PRD_ESCALERA=BLANKS)                  "REPLACE(REPLACE(REPLACE(TRIM(:PRD_ESCALERA),';',' '), '\"',''),'''','')",
	PRD_PLANTA           		POSITION(591:601)    CHAR  NULLIF(PRD_PLANTA=BLANKS)                    "REPLACE(REPLACE(REPLACE(TRIM(:PRD_PLANTA),';',' '), '\"',''),'''','')",
	PRD_TELEFONO2           	POSITION(602:621)    CHAR  NULLIF(PRD_TELEFONO2=BLANKS)                 "REPLACE(REPLACE(REPLACE(TRIM(:PRD_TELEFONO2),';',' '), '\"',''),'''','')",
