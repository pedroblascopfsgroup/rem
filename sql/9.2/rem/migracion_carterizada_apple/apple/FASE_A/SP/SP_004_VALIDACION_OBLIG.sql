create or replace PROCEDURE SP_004_VALIDACION_OBLIG IS
    ...
    CODIGO_RECHAZO_OBLIG REM01.VALIDACIONES_TIPOS.CODIGO_RECHAZO%TYPE := '4';

    vValidacion     REM01.VALIDACIONES_OBLITORIEDAD.MENSAJE_ERROR%TYPE;
    vTabla          REM01.VALIDACIONES_OBLITORIEDAD.NOMBRE_INTERFAZ%TYPE;
    vCampo          VALIDACIONES_OBLITORIEDAD.NOMBRE_CAMPO%TYPE;
    ...
    CURSOR TABLES_CURSOR IS
		  ...
		  FROM VALIDACIONES_OBLITORIEDAD
		  WHERE BORRADO = 0
    ;
...
BEGIN
    ...
    V_MSQL := 'DELETE FROM '||V_ESQUEMA||'.'||V_TABLA_VALIDACION||' WHERE COD_RECHAZO = '''||CODIGO_RECHAZO_OBLIG||'''';
    ...
    LOOP
        ...
        vValidacion := '[OBLIGATORIEDAD] '||vValidacion;
        ...
        BEGIN
            ...
            vQuery := REPLACE(vQuery, '#SELECT_STATEMENT#', ' '''||vTabla||''', AUX.'||vClave||', '''||CODIGO_RECHAZO_OBLIG||''', '''||vValidacion||''' ');
            ...
            V_SQL := 'MERGE INTO '||V_ESQUEMA||'.'||vTabla||' T1
					  USING (
							 ...
							 WHERE NOMBRE_INTERFAZ = '''||vTabla||'''
							   AND COD_RECHAZO = '''||CODIGO_RECHAZO_OBLIG||'''
							) T2
					  ON (TO_CHAR(T2.CLAVE_DATO) = TO_CHAR(T1.'||vClave2||'))
					  WHEN MATCHED THEN UPDATE SET
						T1.VALIDACION = '''||CODIGO_RECHAZO_OBLIG||'''';
            EXECUTE IMMEDIATE V_SQL;
