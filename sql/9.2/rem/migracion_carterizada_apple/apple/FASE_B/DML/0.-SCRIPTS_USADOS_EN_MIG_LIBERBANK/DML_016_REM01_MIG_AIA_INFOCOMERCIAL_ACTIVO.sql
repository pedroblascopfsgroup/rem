...
--Llenamos la primera tabla ACT_ICO_INFO_COMERCIAL
  ...
  SELECT 
    ...
    (SELECT PVE.PVE_ID
      ...
      WHERE PVE.PVE_COD_PRINEX = MIG.PVE_CODIGO AND PVE.PVE_FECHA_BAJA IS NULL AND ROWNUM=1)                        ICO_MEDIADOR_ID,
...
--Llenamos la tercera tabla ACT_EDI_EDIFICIO
  ...
  WHERE MIGW.VALIDACION IN (0,1) AND NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.'||V_TABLA3||' T1 WHERE T1.ICO_ID = ICOW.ICO_ID AND T1.BORRADO = ICOW.BORRADO)
  )
  SELECT
...
---Validamos que el Subtipo Activo es de Vivienda
  ...
  EXECUTE IMMEDIATE ('
    ...
    FROM ACT_NUM_ACTIVO MIG
    INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO
    INNER JOIN '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL AC ON AC.ACT_ID = ACT.ACT_ID
    ...
    WHERE NOT EXISTS
      ...
      INNER JOIN '||V_ESQUEMA||'.ACT_VIV_VIVIENDA VIV ON VIV.ICO_ID = INFO.ICO_ID
      WHERE VIV.ICO_ID = AC.ICO_ID)
  ')
  ;
...
END IF;
  ...
  SELECT
    ...
    WHERE NOT EXISTS
      (SELECT 1
      FROM '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL INFO
      INNER JOIN '||V_ESQUEMA||'.ACT_LCO_LOCAL_COMERCIAL LCO ON LCO.ICO_ID = INFO.ICO_ID
      WHERE LCO.ICO_ID = AC.ICO_ID)
  ')
  ;
...
INSERT INTO '||V_ESQUEMA||'.'||V_TABLA6||' (
  ...
  SELECT
    ...
    WHERE NOT EXISTS
      (SELECT 1
      FROM '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL INFO
      INNER JOIN '||V_ESQUEMA||'.ACT_APR_PLAZA_APARCAMIENTO APR ON APR.ICO_ID = INFO.ICO_ID
      WHERE APR.ICO_ID = AC.ICO_ID)
  ')
  ;
