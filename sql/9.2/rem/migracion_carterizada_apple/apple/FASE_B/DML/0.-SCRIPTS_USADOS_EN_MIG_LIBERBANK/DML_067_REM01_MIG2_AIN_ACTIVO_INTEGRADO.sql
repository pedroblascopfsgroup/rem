...
BEGIN
      ...
      -------------------------------------------------
       V_SQL := 'INSERT INTO '||V_ESQUEMA||'.AIN_ACTIVO_INTEGRADO 
                 (
				  ...
				  SELECT
				  '||V_ESQUEMA||'.S_AIN_ACTIVO_INTEGRADO.NEXTVAL,
				  AUX.*
				  FROM (
					SELECT DISTINCT
					...
					JOIN '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR                                 PVE 
					  ON PVE.PVE_COD_PRINEX = MIG.CPR_COD_COM_PROP_UVEM
					...
					JOIN '||V_ESQUEMA||'.MIG_ADA_DATOS_ADI                                 ADA
					  ON ADA.CPR_COD_COM_PROP_UVEM = CPR.CPR_COD_COM_PROP_UVEM
      ...
      DBMS_OUTPUT.PUT_LINE('  [INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.AIN_ACTIVO_INTEGRADO creados. '||SQL%ROWCOUNT||' Filas.');
      
      V_SQL := 'BEGIN '||V_ESQUEMA||'.OPERACION_DDL.DDL_TABLE(''STATS'',''AIN_ACTIVO_INTEGRADO'',''2''); END;';
      EXECUTE IMMEDIATE V_SQL;
