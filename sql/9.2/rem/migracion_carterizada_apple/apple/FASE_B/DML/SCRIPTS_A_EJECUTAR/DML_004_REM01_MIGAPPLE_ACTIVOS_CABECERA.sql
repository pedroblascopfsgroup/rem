...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] Comienza la migración de ACTIVOS_CABECERA');

  -------------------------------------------------
  --INSERCION EN ACT_ACTIVO--
  -------------------------------------------------
  DBMS_OUTPUT.PUT_LINE('  [INFO] INSERCION EN ACT_ACTIVO'); 
  
  EXECUTE IMMEDIATE ('alter table '||V_ESQUEMA||'.'||V_TABLA||' disable constraint FK_ACTIVO_BIEN');
  EXECUTE IMMEDIATE ('alter table '||V_ESQUEMA||'.'||V_TABLA||' disable constraint FK_ACTIVO_SUBDIVI');
		
  --INSERTAMOS ACTIVOS QUE NO ESTUVIERAN EN REM
  EXECUTE IMMEDIATE ('
  INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (
	  ...
	  SELECT
		  ...
		  MIG.ACT_NUMERO_EXTERNO                                            ACT_NUM_ACTIVO_UVEM,
		  MIG.ACT_NUMERO_EXTERNO                                            ACT_NUM_ACTIVO_SAREB,
		  MIG.ACT_NUMERO_EXTERNO                                            ACT_NUM_ACTIVO_PRINEX,
		  ...
		  NULL                     			                                SDV_ID,
		  NULL												                CPR_ID,
		  MIG.ACT_COD_PROMOCION_EXTERNO										ACT_COD_PROMOCION_PRINEX,
		  ''0''                                                             VERSION,
  ...
  --ACTUALIZAMOS ACTIVOS QUE YA ESTUVIERAN EN REM.
  EXECUTE IMMEDIATE '
    MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
    USING (
		  SELECT ACT_NUMERO_ACTIVO, TIPO_ACTIVO, SUBTIPO_ACTIVO, ESTADO_ACTIVO, ESTADO_GESTION, SITUACION_COMERCIAL
    ...
    WHEN MATCHED THEN UPDATE SET
      ...
      , T1.ACT_GESTION = T2.ESTADO_GESTION
      , T1.ACT_ADMISION = T2.ESTADO_ADMISION
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' campos actualizados. '||SQL%ROWCOUNT||' Filas.');
  
  
  --ACTUALIZAMOS EL DD_ENO_ORIGEN_ANT_ID DEL ACTIVO (CON EL CODIGO '9999' = 'PENDIENTE DE DEFINIR') CUANDO ESTE CAMPO VENGA NULO.
  EXECUTE IMMEDIATE '
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' DD_ENO_ORIGEN_ANT_ID nulos actualizados a <Pendiente de definir>. '||SQL%ROWCOUNT||' Filas.');
  
  
  
  -------------------------------------------------
  --INSERCION EN BIE_BIEN--
  -------------------------------------------------
  DBMS_OUTPUT.PUT_LINE('  [INFO] INSERCION EN BIE_BIEN'); 
  ...
  WHERE NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.BIE_BIEN BIE WHERE BIE.BIE_ID = ACT.BIE_ID)
  ...
  -------------------------------------------------
  --INSERCION EN BIE_ADJ_ADJUDICACION--
  -------------------------------------------------
  DBMS_OUTPUT.PUT_LINE('  [INFO] INSERCION EN BIE_ADJ_ADJUDICACION'); 
  ...
  SELECT
	  ...
	  0                                                                 BORRADO
  FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
  WHERE NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE WHERE BIE.BIE_ID = ACT.BIE_ID)
  ...
  EXECUTE IMMEDIATE ('ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ENABLE CONSTRAINT FK_ACTIVO_BIEN');
  EXECUTE IMMEDIATE ('ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ENABLE CONSTRAINT FK_ACTIVO_SUBDIVI');
  ...
  DBMS_OUTPUT.PUT_LINE('[FIN] Acaba la migración de ACTIVOS CABECERA');
