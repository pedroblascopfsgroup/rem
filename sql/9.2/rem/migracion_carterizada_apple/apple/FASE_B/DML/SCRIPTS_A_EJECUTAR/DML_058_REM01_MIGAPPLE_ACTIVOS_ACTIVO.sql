...
BEGIN
      ...
      DBMS_OUTPUT.PUT_LINE('[INICIO] Comienza la migración de ACTIVOS ACTIVO');

	  -------------------------------------------------
      --MERGE EN ACT_ACTIVO--
      -------------------------------------------------
      DBMS_OUTPUT.PUT_LINE('	[INFO] MERGE EN ACT_ACTIVO');
      ...
      V_SENTENCIA := '
        ...
        USING ( 
				SELECT  
					...
					DD_TCR_ID,
					DD_TAL_ID,
					ACT_COD_TIPO_ALQUILER,
					ACT_VENTA_EXTERNA_FECHA,
					ACT_VENTA_EXTERNA_OBSERVACION,
					ACT_VENTA_EXTERNA_IMPORTE,
					ACT_FECHA_IND_PUBLICABLE,
					ACT_BLOQUEO_TIPO_COMERCIALIZAR,
					ACT_IBI_EXENTO,   
					OK_TECNICO,
					ACT_PUJA
		...
		WHEN MATCHED THEN UPDATE SET
			  ...
			  ,ACT.DD_TCR_ID = (SELECT DD_TCR_ID FROM '||V_ESQUEMA||'.DD_TCR_TIPO_COMERCIALIZAR WHERE DD_TCR_CODIGO = AUX.DD_TCR_ID)
			  ,ACT.DD_TAL_ID = (SELECT DD_TAL_ID FROM '||V_ESQUEMA||'.DD_TAL_TIPO_ALQUILER WHERE DD_TAL_CODIGO = NVL(AUX.DD_TAL_ID, AUX.ACT_COD_TIPO_ALQUILER))
			  ,ACT.ACT_VENTA_EXTERNA_FECHA = AUX.ACT_VENTA_EXTERNA_FECHA
			  ,ACT.ACT_VENTA_EXTERNA_OBSERVACION = AUX.ACT_VENTA_EXTERNA_OBSERVACION
			  ,ACT.ACT_VENTA_EXTERNA_IMPORTE = AUX.ACT_VENTA_EXTERNA_IMPORTE
			  ,ACT.ACT_FECHA_IND_PUBLICABLE = AUX.ACT_FECHA_IND_PUBLICABLE
			  ,ACT.ACT_BLOQUEO_TIPO_COMERCIALIZAR = AUX.ACT_BLOQUEO_TIPO_COMERCIALIZAR
			  ,ACT.ACT_IBI_EXENTO = AUX.ACT_IBI_EXENTO
			  ,ACT.OK_TECNICO = AUX.OK_TECNICO
			  ,ACT.ACT_PUJA = AUX.ACT_PUJA
      ...
      -- ACTUALIZACIÓN DE LA FECHA IND PUBLICABLE DEL ACTIVO
      /*EXECUTE IMMEDIATE 'MERGE INTO REM01.ACT_ACTIVO T1
        USING (
            SELECT ACT_ID 
            FROM '||V_ESQUEMA||'.ACT_ACTIVO 				ACT
            JOIN '||V_ESQUEMA||'.'||V_TABLA_MIG||' 			MIG ON MIG.ACT_NUMERO_ACTIVO = ACT.ACT_NUM_ACTIVO AND MIG.VALIDACION IN (0,1)
            JOIN '||V_ESQUEMA||'.DD_EPU_ESTADO_PUBLICACION 	DD  ON DD.DD_EPU_ID = ACT.DD_EPU_ID AND DD.DD_EPU_CODIGO NOT IN  (''05'',''06'')
            WHERE ACT.BORRADO = 0
         ) T2
      ...
      DBMS_OUTPUT.PUT_LINE('[FIN] Acaba la migración de ACTIVOS ACTIVO');
