...
if [ "$#" -ne 2 ]; then
   echo "[INFO] Parametros: <entidad> [SAREB, CAJAMAR, COOPER, LIBER, BANKIA, APPLE]"
   echo "[INFO] Parametros: <cadena_conexion> <pass@host:puerto/ORACLE_SID>"
   exit 1
fi

if [[ "$1" != "SAREB" ]] && [[ "$1" != "CAJAMAR" ]]&& [[ "$1" != "COOPER" ]]&& [[ "$1" != "LIBER" ]] && [[ "$1" != "BANKIA" ]] && [[ "$1" != "APPLE" ]]; then
    echo "[INFO] Entidad no válida."
    echo "[INFO] Valores aceptados [SAREB, CAJAMAR, COOPER, LIBER, BANKIA, APPLE]"
    exit 1
fi
...
echo "${ORACLE_HOME}/bin/sqlplus REM01/$2 @${DIR_SP}/${SP} > ${DIR_LOGS}/002_$1_compilar_sp.log"
...
wc -l ./FASE_A/CTLs_DATs/DATs/*.dat | head --lines=-1 | sed 's/CTLs_DATs\/DATs\///g ; s/.dat//g' | sed -e 's/^[ \t]*//' > $fichero
...
if [[ $USUARIO_MIGRACION != *$1* ]]; then
  for palabra in $OUTPUT; do
    if [[ $palabra = *$1* ]]; then
		...
		echo "USUARIO_MIGRACION="$USUARIO_MIGRACION
    fi
  done
fi
if [[ $USUARIO_MIGRACION != *$1* ]]; then
