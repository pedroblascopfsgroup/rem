...
do
	if [ -f $ruta$line.ctl ] ; then
		if [ -s $ruta"DATs/"$line.* ] ; then
			...
			grep -ri -a 'Total de registro\|\Tabla' ./$ruta"logs/"$line.log | grep -a -v 'cargada\|\activa\|\omitidos\|\desechados' | sed 's/\://g' | tr -s ' ' | sed 's/rechazados/[RECHAZADOS] /g' | sed 's/Tabla/[TABLA] /g' | sed 's/dos/[REGISTROS] /g' | sed -e 's/.*\[/		\[/' | awk -F\[ '{print "		["$2}'
