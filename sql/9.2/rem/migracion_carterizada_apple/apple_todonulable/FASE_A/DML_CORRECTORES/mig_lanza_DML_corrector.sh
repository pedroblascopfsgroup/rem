...
fichero=DML_CORRECTORES/DMLs.list
ruta=DML_CORRECTORES/
ls --format=single-column $ruta*.sql > $fichero

##Se ejecutan los DMLs de la carpeta [DML_CORRECTORES/]
echo "[INFO] Ejecutando los DMLs de la carpeta [DML_CORRECTORES/*.sql]"
while read line
do
	if [ -f "$line" ] ; then
		echo "--------------------------------------------------------------------------------------------"
