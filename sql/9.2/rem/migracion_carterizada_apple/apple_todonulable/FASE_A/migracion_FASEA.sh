...
if [ "$#" -ne 2 ]; then
    echo "Parametros:"
    echo "$ ./migracion_FASEA.sh usuario/pass@host:puerto/ORACLE_SID USUARIO_MIGRACION"
    echo "*************************************************************************************************"
    echo "USUARIO_MIGRACION ---> MIG_SAREB, MIG_CAJAMAR, MIG_BANKIA, MIG_COOPER, MIG_LIBERBANK, MIG_APPLE"
    echo "Ejecutar desde: /migracion_carterizada_apple/FASE_A/"
    rm -f sqlnet.log
    exit 1
fi
inicio=`date +%s`
...
log_completo="LOGS/migracion_completo_FASEA_"$2"_"$fecha_log".log"
...
echo " "
echo "#########################################################################"
echo "####### [INICIO] Comienza la migración (FASE_A) de $migracion: $hora"
echo "#########################################################################"
echo " "
...
if [ $numFicherosReubicados != $numFicherosDescomprimidos ] ; then
	echo "	[ERROR] Hay ficheros en el .zip que nose han podido reubicar. Es posible que el nombre de algún fichero sea incorrecto."
	echo "	[ERROR] Mirar el fichero FICHERO/renombrado.list para comprobarlo"
else
...
num_ctls_ejecutados=`grep -ri -c "cargadas correctamente" $log3`
...
if [ $nr_errors_leves != 0 ] ; then 
   ...
   find  CTLs_DATs/reject/ -type f | sed s/"CTLs_DATs\/reject\/empty"/""/g | sed '/^[[:space:]]*$/d' | perl -ne 'print "	[LOG] ======>>> Revise el fichero de rechazos $_"'
   echo -e "	[LOG] ======>>> Log de la carga: $log3"
   echo -e "	[LOG] ======>>> Log completo: $log_completo"
   echo " "
fi
...
echo " "
echo "	-------------------------------------------------------------------------------------"
echo "	------ [INFO] [`date +%H:%M:%S`] Comienza el rellenando en las tablas de validaciones"
echo "	-------------------------------------------------------------------------------------"
fecha=`date +%Y%m%d_%H%M%S`
inicioparte=`date +%s`
...
if [ $total = 0 ] ; then
   let total=($fin-$inicioparte)
   echo "	[INFO] Tablas de validación rellenadas en $total segundos."
else
   echo "	[INFO] Tablas de validación rellenadas en $total minutos."
fi
...
echo " "
echo "	-------------------------------------------------------------------------------------"
echo "	------ [INFO] [`date +%H:%M:%S`] Comienza la ejecucion de los scripts correctores"
echo "	-------------------------------------------------------------------------------------"
fecha=`date +%Y%m%d_%H%M%S`
inicioparte=`date +%s`
