...
--#########################################
--## AUTOR=DAP
--## FECHA_CREACION=20170803
--## ARTEFACTO=migracion
--## VERSION_ARTEFACTO=9.2
--## INCIDENCIA_LINK=HREOS-2513
--## PRODUCTO=NO
--## 
...
DECLARE
      ...
      V_TABLA VARCHAR2(40 CHAR) := 'MIG2_TRA_TRAMITES_TRABAJOS'; -- Vble. Tabla pivote
      ...
      SELECT DISTINCT TBJ_ID  FROM REM01.MIG2_TRA_TRAMITES_TRABAJOS TRA
      ;
...
BEGIN
      ...
      EXECUTE IMMEDIATE '
           INSERT INTO MIG2_TRA_TRAMITES_TRABAJOS (TBJ_ID, ACT_ID, TPO_ID, TAP_ID)
            ...
            --JOIN REM01.MIG_ATR_TRABAJO ATR ON ATR.TBJ_NUM_TRABAJO = TBJ.TBJ_NUM_TRABAJO AND ATR.VALIDACION = 0
            JOIN REM01.DD_TTR_TIPO_TRABAJO TTR ON TTR.DD_TTR_ID = TBJ.DD_TTR_ID AND TTR.DD_TTR_CODIGO IN (''02'',''03'')
            JOIN REM01.DD_EST_ESTADO_TRABAJO EST ON EST.DD_EST_ID = TBJ.DD_EST_ID AND EST.DD_EST_CODIGO IN (''01'', ''04'',''05'', ''06'',''09'',''10'',''11'',''13'')
            JOIN REM01.ACT_TBJ ATB ON ATB.TBJ_ID = TBJ.TBJ_ID
            JOIN REM01.ACT_ACTIVO ACT ON ACT.ACT_ID = ATB.ACT_ID AND ACT.BORRADO = 0
            JOIN REM01.DD_TPO_TIPO_PROCEDIMIENTO TPO_T004 ON TPO_T004.DD_TPO_CODIGO = ''T004''
            JOIN REM01.TAP_TAREA_PROCEDIMIENTO TAP_T004 ON TAP_T004.TAP_CODIGO = ''T004_AnalisisPeticion''
            JOIN REM01.DD_TPO_TIPO_PROCEDIMIENTO TPO_T003 ON TPO_T003.DD_TPO_CODIGO = ''T003''
            JOIN REM01.TAP_TAREA_PROCEDIMIENTO TAP_T003 ON TAP_T003.TAP_CODIGO = ''T003_AnalisisPeticion''
            JOIN REM01.DD_TPO_TIPO_PROCEDIMIENTO TPO_T008 ON TPO_T008.DD_TPO_CODIGO = ''T008''
            JOIN REM01.TAP_TAREA_PROCEDIMIENTO TAP_T008 ON TAP_T008.TAP_CODIGO = ''T008_AnalisisPeticion''
            JOIN REM01.DD_TPO_TIPO_PROCEDIMIENTO TPO_T002 ON TPO_T002.DD_TPO_CODIGO = ''T002''
            JOIN REM01.TAP_TAREA_PROCEDIMIENTO TAP_T002 ON TAP_T002.TAP_CODIGO = ''T002_AnalisisPeticion''
            LEFT JOIN REM01.DD_STR_SUBTIPO_TRABAJO STR ON STR.DD_STR_ID = TBJ.DD_STR_ID
      ...
      ---------------------------------------------------------------------------------------------------------------
      -- UPDATE MIG2_TRA_TRAMITES_TRABAJOS (TBJ_ID, TRA_ID, TAR_ID, TEX_ID) --
      ---------------------------------------------------------------------------------------------------------------
      ...
      LOOP
            ...
            EXIT WHEN CURSOR_TRABAJOS%NOTFOUND;
                  ...
                  EXECUTE IMMEDIATE '
                        UPDATE '||V_ESQUEMA||'.MIG2_TRA_TRAMITES_TRABAJOS TRA
      ...
      ---------------------------------------------------------------------------------------------------------------
      -- UPDATE MIG2_TRA_TRAMITES_TRABAJOS (USU_ID, SUP_ID) --
      ---------------------------------------------------------------------------------------------------------------
      
      DBMS_OUTPUT.PUT_LINE('[INFO] ACTUALIZANDO USU_ID Y SUP_ID...');
      
     EXECUTE IMMEDIATE 'MERGE INTO MIG2_TRA_TRAMITES_TRABAJOS T1
USING (SELECT T1.ACT_ID, T1.TBJ_ID, T1.TPO_ID, T1.TAP_ID, T3.USU_ID
    FROM MIG2_TRA_TRAMITES_TRABAJOS T1
...
WHEN MATCHED THEN UPDATE SET
    T1.USU_ID = T2.USU_ID';
    DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.MIG2_TRA_TRAMITES_TRABAJOS actualizada. '||SQL%ROWCOUNT||' Filas.');
EXECUTE IMMEDIATE 'MERGE INTO MIG2_TRA_TRAMITES_TRABAJOS T1
USING (SELECT T1.ACT_ID, T1.TBJ_ID, T1.TPO_ID, T1.TAP_ID, T3.USU_ID
    FROM MIG2_TRA_TRAMITES_TRABAJOS T1
    JOIN GAC_GESTOR_ADD_ACTIVO T2 ON T1.ACT_ID = T2.ACT_ID
