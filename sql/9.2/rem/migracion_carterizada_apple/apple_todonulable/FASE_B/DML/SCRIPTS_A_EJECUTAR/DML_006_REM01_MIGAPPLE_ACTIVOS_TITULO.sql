...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Comienza la migración de ACTIVOS TITULO');

    -------------------------------------------------
    --INSERCION EN ACT_TIT_TITULO--
    -------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('  [INFO] INSERCION EN ACT_TIT_TITULO'); 
    
    
    --BORRAMOS INICIALMENTE TODOS LOS TITULOS DE ACTIVOS QUE VENGAN POR MIGRACIÓN
    EXECUTE IMMEDIATE ('DELETE FROM '||V_ESQUEMA||'.ACT_TIT_TITULO 
						WHERE ACT_ID IN (SELECT DISTINCT ACT_ID FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT INNER JOIN '||V_ESQUEMA||'.MIG_ACA_CABECERA ACA ON ACA.ACT_NUMERO_ACTIVO = ACT.ACT_NUM_ACTIVO)');
  ...
  --ACTUALIZAMOS LA BIE_DREG_FECHA_INSCRIPCION DE LA BIE_DATOS_REGISTRALES
  EXECUTE IMMEDIATE 
   'MERGE INTO '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES T1
	USING (
	    SELECT T1.BIE_ID, TIT.TIT_FECHA_INSC_REG
	    FROM '||V_ESQUEMA||'.ACT_TIT_TITULO TIT
	    JOIN '||V_ESQUEMA||'.ACT_ACTIVO T1 ON T1.ACT_ID = TIT.ACT_ID AND T1.BORRADO = 0
	    WHERE TIT.BORRADO = 0
	) T2
	...
	WHERE T1.BORRADO = 0 
	  AND (TRUNC(T2.TIT_FECHA_INSC_REG) <> TRUNC(T1.BIE_DREG_FECHA_INSCRIPCION) 
	       OR 
	      (T1.BIE_DREG_FECHA_INSCRIPCION IS NULL AND T2.TIT_FECHA_INSC_REG IS NOT NULL)
  )';
  ...
  DBMS_OUTPUT.PUT_LINE('[FIN] Acaba la migración de ACTIVOS TITULO');
