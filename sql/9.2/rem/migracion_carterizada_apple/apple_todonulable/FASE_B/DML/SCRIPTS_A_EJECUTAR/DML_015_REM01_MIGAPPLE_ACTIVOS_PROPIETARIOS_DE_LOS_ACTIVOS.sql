...
BEGIN
   ...
   --SI EL USUARIO ES MIG_APPLE, EL PROPIETARIO LO COGERA DIRECTAMENTE POR CODIGO UVEM EN LA TABLA ACT_PRO_PROPIETARIO.
   IF V_USUARIO = 'MIG_APPLE' THEN
	   EXECUTE IMMEDIATE ('
	   ...
	   (
		...
		WITH MIG AS (
			SELECT DISTINCT (MIG.ACT_NUMERO_ACTIVO),
				...
				JOIN '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO 	PRO 
						ON PRO.PRO_CODIGO_UVEM = MIG.PRO_CODIGO
		...
		JOIN '||V_ESQUEMA||'.ACT_ACTIVO 					ACT 
		  ON ACT.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO 
		JOIN '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO 			PRO
		  ON PRO.PRO_DOCIDENTIF = MIG.PRO_DOCIDENTIF 
		JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA 				CRA
		  ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
	  ');
  ELSE
