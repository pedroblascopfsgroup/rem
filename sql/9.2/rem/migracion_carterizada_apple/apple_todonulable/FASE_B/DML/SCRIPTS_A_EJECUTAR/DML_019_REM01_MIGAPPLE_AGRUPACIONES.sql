...
BEGIN
  ...
  /*EXECUTE IMMEDIATE 'MERGE INTO REM01.ACT_AGR_AGRUPACION T1
	...
	LEFT JOIN '||V_ESQUEMA||'.ACT_RES_RESTRINGIDA RES 
	ON RES.AGR_ID = AGR.AGR_ID 
	WHERE RES.AGR_ID IS NULL
   ');
   ...
   DBMS_OUTPUT.PUT_LINE('	[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA_3||' cargada. '||SQL%ROWCOUNT||' Filas.');
    ...
    EXECUTE IMMEDIATE ('
	MERGE INTO '||V_ESQUEMA||'.ACT_ASI_ASISTIDA ASI
	USING (
		SELECT  AGR.AGR_ID, 
				(SELECT DD_PRV_ID
			...
			FROM '||V_ESQUEMA||'.MIG_AAG_AGRUPACIONES 	MIG
			INNER JOIN '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR ON MIG.AGR_EXTERNO = AGR.AGR_NUM_AGRUP_UVEM
			JOIN '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR 		PVE ON MIG.ONV_ACRE_PDV_AGRUP_OBRA_NUEVA = PVE.PVE_COD_API_PROVEEDOR
