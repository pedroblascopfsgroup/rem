...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('[INICIO] Comienza la migración de ACTIVOS OBSERVACIONES');

  -------------------------------------------------
  --INSERCION EN ACT_AOB_ACTIVO_OBS--
  -------------------------------------------------
  DBMS_OUTPUT.PUT_LINE('	[INFO] INSERCION EN ACT_AOB_ACTIVO_OBS');
  ...
  INNER JOIN '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU ON USU.USU_ID = (SELECT USU_ID FROM REMMASTER.USU_USUARIOS WHERE USU_USERNAME = ''MIGSAN'')
  ...
  DBMS_OUTPUT.PUT_LINE('[FIN] Acaba la migración de ACTIVOS OBSERVACIONES');
