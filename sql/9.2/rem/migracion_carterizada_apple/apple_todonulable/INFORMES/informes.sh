#!/bin/bash
if [ "$#" -ne 3 ]; then
   echo "[INFO] Parametros: <entidad> [SAREB, CAJAMAR, COOPER, LIBER,BANKIA]"
   echo "[INFO] Parametros: <informe> [CONTRASTE, KPIS, PARES]"
   echo "[INFO] Parametros: <cadena_conexion> <pass@host:puerto/ORACLE_SID>"
   exit 1
fi

if [[ "$1" != "SAREB" ]] && [[ "$1" != "CAJAMAR" ]]&& [[ "$1" != "COOPER" ]]&& [[ "$1" != "LIBER" ]] && [[ "$1" != "BANKIA" ]];  then
    echo "[INFO] Entidad no válida."
    echo "[INFO] Valores aceptados [SAREB, CAJAMAR, COOPER, LIBER,BANKIA]"
    exit 1
fi
