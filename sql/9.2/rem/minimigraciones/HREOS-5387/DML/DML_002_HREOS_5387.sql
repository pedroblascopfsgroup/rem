...
SELECT 
  ...
  , AUX.DD_PRV_CODIGO
  , 'dgonzalez'
  , (SELECT USU.USU_NOMBRE ||' '|| USU.USU_APELLIDO1 ||' '|| USU.USU_APELLIDO2 FROM REMMASTER.USU_USUARIOS USU WHERE USU.USU_USERNAME = 'dgonzalez')
  , 0
  , 'HREOS-5387'
  , SYSDATE
  , 0
FROM (
      SELECT DD_TGE_CODIGO
      , DD_CRA_CODIGO
      , DD_TCR_CODIGO 
      , DD_PRV_CODIGO
      FROM REM01.AUX_CARTERIAZA_HREOS_5387
      WHERE USU_USERNAME IS NULL
      GROUP BY DD_TGE_CODIGO
      , DD_CRA_CODIGO
      , DD_TCR_CODIGO
      , DD_PRV_CODIGO
     ) AUX;
