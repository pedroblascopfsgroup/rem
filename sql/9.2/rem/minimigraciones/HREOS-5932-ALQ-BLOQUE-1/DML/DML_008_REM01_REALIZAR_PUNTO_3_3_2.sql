...
/**Se procede a marcar los activos del punto 3.3.2 **/
		...
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.ACT_ACTIVO SET 
					...
					WHERE ACT_NUM_ACTIVO IN (
						...
						LEFT JOIN '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL SCM ON SCM.DD_SCM_ID = ACT.DD_SCM_ID AND SCM.DD_SCM_CODIGO <> ''05''
						LEFT JOIN '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION TCOAUX ON AUX.DESTINO_COMERCIAL = TCOAUX.DD_TCO_CODIGO
						LEFT JOIN '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION TCOACT ON apu.dd_tco_id = TCOACT.DD_TCO_id
