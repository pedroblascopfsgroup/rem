...
--##        0.2 ODP 05/06/19 
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.AUX_INF_HREOS_5932 T1 
		USING (
			WITH DATOS AS 	
				(
				SELECT 									 					  ACTIVO,DESTINO_COMERCIAL,TIPO_CONTRATO_ALQUILER,
				ESTADO_ADECUACION,SUBROGADO,ALQUILADO,INQUILINO_ANT_PROP,
				RENTA_ANTIGUA,AGR_ID 
				FROM (
					SELECT	AUX2.ID_HAYA AS ACTIVO,							         							AUX2.DESTINO_COMERCIAL,
						AUX2.TIPO_CONTRATO_ALQUILER, 
						AUX2.ESTADO_ADECUACION,
						AUX2.SUBROGADO,
						AUX2.ALQUILADO,
						AUX2.INQUILINO_ANT_PROP,
						AUX2.RENTA_ANTIGUA,
						AGR.AGR_ID,
						ROW_NUMBER() OVER (PARTITION BY AGR.AGR_ID ORDER BY 1 ASC) AS RN

						FROM '||V_ESQUEMA||'.AUX_INF_HREOS_5932 AUX2
						...
						AND AUX2.DESTINO_COMERCIAL  IS NOT NULL
					) WHERE RN = 1
				)
				SELECT AUX.ID_HAYA,
					DAT.ACTIVO,
					DAT.DESTINO_COMERCIAL,
					DAT.TIPO_CONTRATO_ALQUILER,
					DAT.ESTADO_ADECUACION,
					DAT.SUBROGADO,
					DAT.ALQUILADO,
					DAT.INQUILINO_ANT_PROP,
					DAT.RENTA_ANTIGUA
				FROM '||V_ESQUEMA||'.AUX_INF_HREOS_5932 AUX 
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = AUX.ID_HAYA
				INNER JOIN '||V_ESQUEMA||'.ACT_AGA_AGRUPACION_ACTIVO AGA ON ACT.ACT_ID = AGA.ACT_ID AND AGA.BORRADO = 0
				...
				WHERE TAG.DD_TAG_CODIGO = ''02'' AND AGR.AGR_FECHA_BAJA IS NULL  
				AND AUX.DESTINO_COMERCIAL  IS NULL 
				AND AUX.TIPO_CONTRATO_ALQUILER IS NULL 
				AND AUX.ESTADO_ADECUACION IS NULL 
				AND AUX.SUBROGADO IS NULL 
				AND AUX.ALQUILADO IS NULL 
				AND AUX.INQUILINO_ANT_PROP IS NULL 
				AND AUX.RENTA_ANTIGUA IS NULL
			 )T2
...
--##        0.2 ODP 05/06/19 
	...
	EXECUTE IMMEDIATE  'UPDATE '||V_ESQUEMA||'.AUX_INF_HREOS_5932 SET DESTINO_COMERCIAL = ''03'' WHERE DESTINO_COMERCIAL = ''Alquiler''';
	...
	-- SUBROGADO
	EXECUTE IMMEDIATE  'UPDATE '||V_ESQUEMA||'.AUX_INF_HREOS_5932 SET SUBROGADO = ''1'' WHERE SUBROGADO = ''S''';
