			...
			FROM (
				...
				WHERE AGR.AGR_NUM_AGRUP_REM in (SELECT AGR.AGR_NUM_AGRUP_REM
								...
								INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACt 
									on ACT.ACT_NUM_ACTIVO = aux.ID_HAYA							inner join '||V_ESQUEMA||'.ACT_AGA_AGRUPACION_ACTIVO aga 
									on act.act_id = aga.act_id							inner join '||V_ESQUEMA||'.ACT_AGR_AGRUPACION agr on aga.agr_id = agr.agr_id AND AGR.AGR_FECHA_BAJA IS NULL
								inner join '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION tag on tag.dd_tag_id = agr.dd_tag_id 
								WHERE TAG.DD_TAG_CODIGO = ''02''
								)
				)MINUS(
