...
**/
		...
		V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING 
							(SELECT ACT.ACT_NUM_ACTIVO 
								...
								AND PTA.CHECK_HPM = 1) T2
					ON (ACT.ACT_NUM_ACTIVO = T2.ACT_NUM_ACTIVO)
					WHEN MATCHED THEN
					UPDATE SET 
					      ACT.DD_SCM_ID = (SELECT SCM.DD_SCM_ID FROM '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL SCM where SCM.dd_scm_codigo = ''02'')
		...
		V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING 
							(SELECT ACT.ACT_NUM_ACTIVO 
								...
								INNER JOIN '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL SCM ON SCM.DD_SCM_ID = ACT.DD_SCM_ID AND SCM.DD_SCM_CODIGO <> ''05''
								WHERE ACT.ACT_NUM_ACTIVO NOT IN (SELECT ACT.ACT_NUM_ACTIVO FROM '||V_ESQUEMA||'.AUX_HREOS_5932 AUX INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = AUX.ID_HAYA)
								AND tco.dd_tco_codigo in (''02'',''03'')) T2
					ON (ACT.ACT_NUM_ACTIVO = T2.ACT_NUM_ACTIVO)
					WHEN MATCHED THEN
					UPDATE SET 
						  ACT.DD_SCM_ID = (SELECT SCM.DD_SCM_ID FROM '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL SCM where SCM.dd_scm_codigo = ''02'')
...
**/
		...
		DBMS_OUTPUT.put_line('	[INFO] QUITAR CHECK PATRIMONIO');
		
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.ACT_PTA_PATRIMONIO_ACTIVO SET 
							CHECK_HPM = 0 
						,   USUARIOMODIFICAR = '''||V_USUARIOMODIFICAR||'''
						,   FECHAMODIFICAR = SYSDATE
					WHERE ACT_ID IN (SELECT ACT.ACT_ID FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT WHERE ACT.USUARIOMODIFICAR = '''||V_USUARIOMODIFICAR||''')';
...
**/    
		...
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD SET
					...
					WHERE GEE_ID IN (SELECT GEE.GEE_ID 
										...
										INNER JOIN '||V_ESQUEMA_MASTER||'.DD_TGE_TIPO_GESTOR TGE on TGE.DD_TGE_ID = GEE.DD_TGE_ID AND TGE.DD_TGE_CODIGO in (''GESTCOMALQ'',''SUPCOMALQ'')
										WHERE ACT.USUARIOMODIFICAR = ''HREOS-5932-PUNTO1''
									)';
		...
		V_MSQL := 'UPDATE GEH_GESTOR_ENTIDAD_HIST SET
						  GEH_FECHA_HASTA = SYSDATE
						, USUARIOBORRAR = ''HREOS-5932-PUNTO1''
						, FECHABORRAR = SYSDATE
						, BORRADO = 1
					WHERE GEH_ID in (SELECT GEH.GEH_ID FROM ACT_ACTIVO ACT 
										INNER JOIN GAH_GESTOR_ACTIVO_HISTORICO GAH ON GAH.ACT_ID = ACT.ACT_ID
										INNER JOIN GEH_GESTOR_ENTIDAD_HIST GEH ON GAH.GEH_ID = GEH.GEH_ID AND GEH.GEH_FECHA_HASTA IS NULL
										INNER JOIN REMMASTER.DD_TGE_TIPO_GESTOR TGE on TGE.DD_TGE_ID = GEH.DD_TGE_ID AND TGE.DD_TGE_CODIGO in (''GESTCOMALQ'',''SUPCOMALQ'')
										WHERE ACT.USUARIOMODIFICAR = ''HREOS-5932-PUNTO1''
									)';
