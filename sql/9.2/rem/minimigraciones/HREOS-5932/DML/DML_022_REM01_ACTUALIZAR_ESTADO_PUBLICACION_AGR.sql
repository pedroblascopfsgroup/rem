...
--## PRODUCTO=NO
--## Finalidad: RECALCULAR ESTADO PUBLICACION AGRUPACIONES
--##           
...
DECLARE
    ...
    CURSOR ESTADO_PUBLI_RECALCULAR IS SELECT DISTINCT AGR.AGR_ID
										FROM REM01.AUX_HREOS_5932_PERIM AUX
										INNER JOIN REM01.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = AUX.ACT_NUM_ACTIVO
										INNER JOIN REM01.ACT_AGA_AGRUPACION_ACTIVO  AGA ON ACT.ACT_ID = AGA.ACT_ID
										INNER JOIN REM01.ACT_AGR_AGRUPACION AGR ON AGR.AGR_ID = AGA.AGR_ID
										INNER JOIN REM01.DD_TAG_TIPO_AGRUPACION TAG on AGR.DD_TAG_ID = TAG.DD_TAG_ID 
										WHERE AGR.AGR_FECHA_BAJA IS NULL AND TAG.DD_TAG_CODIGO = '02'
										GROUP BY AGR.AGR_ID
										ORDER BY AGR.AGR_ID ASC;
...
BEGIN
	...
	LOOP
  		...
  		REM01.SP_CAMBIO_ESTADO_PUBLICACION (FILA.AGR_ID,1,''||V_USUARIOMODIFICAR||'');
