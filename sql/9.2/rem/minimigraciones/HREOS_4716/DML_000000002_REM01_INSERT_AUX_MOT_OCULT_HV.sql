...
DECLARE
    ...
    V_TABLA VARCHAR2(30 CHAR) := 'AUX_MOT_OCULT_HV';  -- Tabla a modificar  
...
BEGIN	
	...
	INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (OCULTO, DD_MTO_CODIGO, AHP_ID) (
			SELECT OCULTO, DD_MTO_CODIGO, AHP_ID
              FROM (SELECT OCULTO, DD_MTO_CODIGO, AHP_ID, ROW_NUMBER () OVER (PARTITION BY AHP_ID ORDER BY ORDEN ASC) ROWNUMBER
                    FROM(
                          SELECT APU.AHP_ID
                          ...
                          SELECT APU.AHP_ID
                               ...
                               , MTO.DD_MTO_ORDEN ORDEN
                                   ...
                                   WHERE APU.BORRADO = 0
                                     AND APU.AHP_CHECK_PUB_SIN_PRECIO_V = 0
    ...
    COMMIT;
    #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_MOT_OCULT_HV');
