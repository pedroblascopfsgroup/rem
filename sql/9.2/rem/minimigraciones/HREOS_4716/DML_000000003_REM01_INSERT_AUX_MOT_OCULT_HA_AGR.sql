...
DECLARE
    ...
    V_TABLA VARCHAR2(30 CHAR) := 'AUX_MOT_OCULT_HA_AGR';  -- Tabla a modificar  
...
BEGIN	
				...
				FROM
					(
						...
						SELECT
							...
							mto.dd_mto_orden orden
						FROM
							'||V_ESQUEMA||'.ACT_AHP_HIST_PUBLICACION act
							JOIN '||V_ESQUEMA||'.act_aga_agrupacion_activo aga ON aga.act_id = act.act_id
																		AND aga.borrado = 0
							JOIN '||V_ESQUEMA||'.act_agr_agrupacion agr ON agr.agr_id = aga.agr_id
																 AND agr.borrado = 0
						...
						WHERE
							act.borrado = 0
							AND   act.AHP_check_pub_sin_precio_a = 0
							AND   NOT EXISTS (
								SELECT
									1
								FROM
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('[FIN]: TABLA AUX_MOT_OCULT_HA_AGR ACTUALIZADA CORRECTAMENTE ');
   
    #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_MOT_OCULT_HA_AGR');
