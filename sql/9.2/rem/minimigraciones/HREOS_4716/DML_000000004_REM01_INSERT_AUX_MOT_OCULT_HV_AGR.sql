...
DECLARE
    ...
    V_TABLA VARCHAR2(30 CHAR) := 'AUX_MOT_OCULT_HV_AGR';  -- Tabla a modificar  
...
BEGIN	
			...
			(
				...
				FROM
					(
						...
						WHERE
							act.borrado = 0
							AND   act.AHP_check_pub_sin_precio_v = 0
							AND   NOT EXISTS (
								SELECT
									1
								FROM
									...
									JOIN '||V_ESQUEMA||'.dd_tpc_tipo_precio tpc ON tpc.dd_tpc_id = val.dd_tpc_id
																		 AND   tpc.borrado = 0
																		 AND   tpc.dd_tpc_codigo = ''02''/*Aprobado de venta (web)*/
								WHERE
									val.borrado = 0
    ...
    COMMIT;
    #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_MOT_OCULT_HV_AGR');
