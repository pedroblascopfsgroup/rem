...
DECLARE
  ...
  V_USUARIO VARCHAR2(100 CHAR) := 'REMVIP-5999';
    
BEGIN	
	
    V_NUM_TABLAS := 0;
	DBMS_OUTPUT.PUT_LINE('[INICIO]');
	EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA||'.GPL_GASTOS_PRINEX_LBK WHERE GPV_ID IN (SELECT GPV_ID FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR WHERE GPV_NUM_GASTO_HAYA IN (SELECT DISTINCT NUM_GASTO FROM REM01.AUX_REMVIP_5999))';
...
FROM REM01.AUX_REMVIP_5999 AUX 
...
;
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.GDE_GASTOS_DETALLE_ECONOMICO GDE
				USING (
				   ...
				   FROM '||V_ESQUEMA||'.GPL_GASTOS_PRINEX_LBK WHERE GPV_ID in (SELECT GPV_ID FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR WHERE GPV_NUM_GASTO_HAYA in (SELECT DISTINCT NUM_GASTO FROM REM01.AUX_REMVIP_5999)) AND GPL_DIARIO1 IS NOT NULL AND BORRADO = 0) WHERE RN = 1) AUX ON (AUX.GPV_ID = GDE.GPV_ID)
				WHEN MATCHED THEN UPDATE SET GDE.GDE_PRINCIPAL_SUJETO = AUX.PRINCIPAL_SUJETO,
				                       ...
				                       GDE.USUARIOMODIFICAR = ''REMVIP-5999'',
