...
DECLARE
	...
	V_TABLA VARCHAR2(30 CHAR) := 'GPV_GASTOS_PROVEEDOR'; -- Variable para tabla de salida para el borrado
	V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-642';
	...
	ACTIVOS NUMBER(6);
	NUMERO_BORRAR NUMBER(6) := 3000;--Numero de activos a borrar en una pasada
	ORDEN NUMBER(2) := 2;
...
BEGIN
	...
	V_REGISTROS := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
	    USING '||V_ESQUEMA||'.AUX_BORRADO_GASTOS_REMVIP_642 T2
	    ON (T1.GPV_NUM_GASTO_HAYA = T2.GASTO)
	...
	ELSE
	    ...
	    LOOP
	        ...
	        IF V_BORRADO_FISICO = 'SI' THEN
		        ...
		        EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA||'.GPV_ACT
                             WHERE NOT EXISTS (
                                SELECT 1 FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR WHERE GPV_ACT.GPV_ID = GPV_GASTOS_PROVEEDOR.GPV_ID)';

                EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA||'.GPV_TBJ
                             WHERE NOT EXISTS (
                                SELECT 1 FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR WHERE GPV_TBJ.GPV_ID = GPV_GASTOS_PROVEEDOR.GPV_ID)';
