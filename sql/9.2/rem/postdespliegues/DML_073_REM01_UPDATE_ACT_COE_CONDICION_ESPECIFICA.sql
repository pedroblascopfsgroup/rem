...
BEGIN	
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_COE_CONDICION_ESPECIFICA COE USING
        (SELECT 
            ROWNUM AS COE_CODIGO
        ,   RECUENTO 
        ,   COE_TEXTO   
        FROM (SELECT COUNT(*) AS RECUENTO 
                    ,   COE_TEXTO
                FROM '||V_ESQUEMA||'.ACT_COE_CONDICION_ESPECIFICA COE 
                GROUP BY  COE.COE_TEXTO 
                HAVING COUNT(*) > 1
                ORDER BY COE_TEXTO)
        )AUX
ON (COE.COE_TEXTO = AUX.COE_TEXTO)
WHEN MATCHED THEN UPDATE SET
COE.COE_CODIGO = AUX.COE_CODIGO';
