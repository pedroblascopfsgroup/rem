...
BEGIN
  ...
  and act.borrado = 0
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.DD_ETI_ID. '||CHR(10)||CHR(10);
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.TIT_FECHA_ENTREGA_GESTORIA. '||CHR(10)||CHR(10);
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.TIT_FECHA_PRESENT_HACIENDA. '||CHR(10)||CHR(10);
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.TIT_FECHA_ENVIO_AUTO. '||CHR(10)||CHR(10);
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.TIT_FECHA_PRESENT1_REG. '||CHR(10)||CHR(10);
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.TIT_FECHA_PRESENT2_REG. '||CHR(10)||CHR(10);
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.TIT_FECHA_INSC_REG. '||CHR(10)||CHR(10);
	...
	PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO.TIT_FECHA_RETIRADA_REG. '||CHR(10)||CHR(10);
    ...
    PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN INSERTADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_TIT_TITULO. '||CHR(10)||CHR(10);
  ...
  and act.borrado = 0
	...
	--------------------------------------------------
	-----       ACTUALIZAMOS ACT_CON_CARGAS DE ACT_ACTIVO  -----***HREOS-2201***
	--------------------------------------------------
	DBMS_OUTPUT.PUT_LINE('[INFO] ACTUALIZAMOS EL CAMPO ACT_CON_CARGAS...');
	DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');

	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING
    			(SELECT  ACT.ACT_ID, NVL2(AUX.ACT_ID, 1 ,0) AS CON_CARGAS
				FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
				LEFT JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON ACT.DD_CRA_ID = CRA.DD_CRA_ID
				LEFT JOIN (
				 SELECT DISTINCT CRG.ACT_ID
				          FROM '||V_ESQUEMA||'.ACT_CRG_CARGAS CRG
				               INNER JOIN '||V_ESQUEMA||'.BIE_CAR_CARGAS BCG ON BCG.BIE_CAR_ID = CRG.BIE_CAR_ID
				               LEFT JOIN '||V_ESQUEMA||'.DD_SIC_SITUACION_CARGA DDSIC ON DDSIC.DD_SIC_ID = BCG.DD_SIC_ID
				               LEFT JOIN '||V_ESQUEMA||'.DD_SIC_SITUACION_CARGA DDSIC2 ON DDSIC2.DD_SIC_ID = BCG.DD_SIC_ID2
				         WHERE CRG.BORRADO = 0
				           AND BCG.BORRADO = 0
				           AND (DDSIC.DD_SIC_CODIGO IN (''VIG'', ''NCN'', ''SAN'') OR BCG.DD_SIC_ID IS NULL)
				           AND (DDSIC2.DD_SIC_CODIGO IN (''VIG'', ''NCN'', ''SAN'') OR BCG.DD_SIC_ID2 IS NULL)) AUX ON (AUX.ACT_ID =ACT.ACT_ID)
				WHERE CRA.DD_CRA_CODIGO = ''02''
        and act.borrado = 0
				GROUP BY ACT.ACT_ID, AUX.ACT_ID) AUX2 ON (ACT.ACT_ID = AUX2.ACT_ID)
					WHEN MATCHED THEN UPDATE SET
					ACT.ACT_CON_CARGAS = AUX2.CON_CARGAS
					WHERE ACT.BORRADO = 0';

	EXECUTE IMMEDIATE V_SQL;
	V_NUM_TABLAS := SQL%ROWCOUNT;
	DBMS_OUTPUT.PUT_LINE('[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_ACTIVO.');
    PL_OUTPUT := PL_OUTPUT||'[INFO] SE HAN ACTUALIZADO ['||V_NUM_TABLAS||'] REGISTROS EN '||V_ESQUEMA||'.ACT_ACTIVO. '||CHR(10)||CHR(10);
