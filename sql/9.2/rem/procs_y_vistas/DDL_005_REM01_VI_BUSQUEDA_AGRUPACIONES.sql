  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_AGRUPACIONES 
	AS
		...
		SELECT DISTINCT
			...
			AGR.AGR_PUBLICADO,
	        (SELECT COUNT(AGA.ACT_ID) FROM ' || V_ESQUEMA || '.ACT_AGA_AGRUPACION_ACTIVO AGA WHERE AGA.AGR_ID = AGR.AGR_ID AND AGR.BORRADO = 0) AS ACTIVOS,
