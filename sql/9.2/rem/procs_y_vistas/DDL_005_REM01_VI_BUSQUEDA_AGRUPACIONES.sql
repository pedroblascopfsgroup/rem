...
BEGIN
	...
	EXECUTE IMMEDIATE 'CREATE VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_AGRUPACIONES
		AS SELECT
		      ...
		      COALESCE (OBR.DD_PRV_ID, RES.DD_PRV_ID, LCO.DD_PRV_ID, ASI.DD_PRV_ID, PRY.DD_PRV_ID, PAL.DD_PRV_ID) AS PROVINCIA,
			  COALESCE (OBR.DD_LOC_ID, RES.DD_LOC_ID, LCO.DD_LOC_ID, ASI.DD_LOC_ID, PRY.DD_LOC_ID, PAL.DD_LOC_ID) AS LOCALIDAD,
			  COALESCE (OBR.ONV_DIRECCION, RES.RES_DIRECCION, LCO.LCO_DIRECCION, ASI.ASI_DIRECCION, PRY.PRY_DIRECCION, PAL.PAL_DIRECCION) AS DIRECCION,
			...
			LEFT JOIN (
			    SELECT AGA.AGR_ID, LOC.DD_PRV_ID, LOC.DD_LOC_ID, PAL.PAL_DIRECCION
			    FROM '|| V_ESQUEMA ||'.ACT_PAL_PROMOCION_ALQUILER PAL
			    JOIN '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA ON AGA.AGR_ID = PAL.AGR_ID AND AGA.AGA_PRINCIPAL = 1
