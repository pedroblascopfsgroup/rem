...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ACTIVOS_PRECIOS 
	AS
	    ...
	    LEFT JOIN  (SELECT ACT1.ACT_ID ACTIVO_TAS, COUNT(TAS_ID) CONTADOR FROM ACT_ACTIVO ACT1
	    ...
	    LEFT JOIN (SELECT VAL1.ACT_ID, VAL1.VAL_IMPORTE 
                	FROM (SELECT  ROW_NUMBER() OVER(PARTITION BY VAL1.ACT_ID ORDER BY VAL1.FECHACREAR DESC) AS r, VAL1.ACT_ID, VAL1.VAL_IMPORTE
	                    FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL1
	                    INNER JOIN ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC1 ON VAL1.DD_TPC_ID = TPC1.DD_TPC_ID AND TPC1.DD_TPC_CODIGO = ''02'') VAL1
                  	WHERE VAL1.r=1) PRECIO_APROBADO_VENTA ON ACT.ACT_ID = PRECIO_APROBADO_VENTA.ACT_ID
	    LEFT JOIN (SELECT VAL2.ACT_ID, VAL2.VAL_IMPORTE 
	                FROM (SELECT  ROW_NUMBER() OVER(PARTITION BY VAL2.ACT_ID ORDER BY VAL2.FECHACREAR DESC) AS r, VAL2.ACT_ID, VAL2.VAL_IMPORTE
	                    FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL2
	                    INNER JOIN ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC2 ON VAL2.DD_TPC_ID = TPC2.DD_TPC_ID AND TPC2.DD_TPC_CODIGO = ''03'') VAL2
                  	WHERE VAL2.r=1) PRECIO_APROBADO_RENTA ON ACT.ACT_ID = PRECIO_APROBADO_RENTA.ACT_ID
	    LEFT JOIN (SELECT VAL3.ACT_ID, VAL3.VAL_IMPORTE 
	                FROM (SELECT  ROW_NUMBER() OVER(PARTITION BY VAL3.ACT_ID ORDER BY VAL3.FECHACREAR DESC) AS r, VAL3.ACT_ID, VAL3.VAL_IMPORTE
	                    FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL3
	                    INNER JOIN ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC3 ON VAL3.DD_TPC_ID = TPC3.DD_TPC_ID AND TPC3.DD_TPC_CODIGO = ''04'') VAL3
                  	WHERE VAL3.r=1) PRECIO_MINIMO_AUTORIZADO ON ACT.ACT_ID = PRECIO_MINIMO_AUTORIZADO.ACT_ID
	    LEFT JOIN (SELECT VAL4.ACT_ID, VAL4.VAL_IMPORTE 
	                FROM (SELECT  ROW_NUMBER() OVER(PARTITION BY VAL4.ACT_ID ORDER BY VAL4.FECHACREAR DESC) AS r, VAL4.ACT_ID, VAL4.VAL_IMPORTE
	                    FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL4
	                    INNER JOIN ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC4 ON VAL4.DD_TPC_ID = TPC4.DD_TPC_ID AND TPC4.DD_TPC_CODIGO = ''07'') VAL4
                  	WHERE VAL4.r=1) PRECIO_DESCUENTO_APROBADO ON ACT.ACT_ID = PRECIO_DESCUENTO_APROBADO.ACT_ID
	    LEFT JOIN (SELECT VAL5.ACT_ID, VAL5.VAL_IMPORTE
                  	FROM (SELECT  ROW_NUMBER() OVER(PARTITION BY VAL5.ACT_ID ORDER BY VAL5.FECHACREAR DESC) AS r, VAL5.ACT_ID, VAL5.VAL_IMPORTE
	                    FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL5
	                    INNER JOIN ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC5 ON VAL5.DD_TPC_ID = TPC5.DD_TPC_ID AND TPC5.DD_TPC_CODIGO = ''13'') VAL5
                  	WHERE VAL5.r=1) PRECIO_DESCUENTO_PUBLICADO ON ACT.ACT_ID = PRECIO_DESCUENTO_PUBLICADO.ACT_ID
	    LEFT JOIN (SELECT VAL6.ACT_ID, VAL6.VAL_IMPORTE 
	                FROM (SELECT  ROW_NUMBER() OVER(PARTITION BY VAL6.ACT_ID ORDER BY VAL6.FECHACREAR DESC) AS r, VAL6.ACT_ID, VAL6.VAL_IMPORTE
	                    FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL6
	                    INNER JOIN ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC6 ON VAL6.DD_TPC_ID = TPC6.DD_TPC_ID AND TPC6.DD_TPC_CODIGO = ''19'') VAL6
                  	WHERE VAL6.r=1) FSV_VENTA ON ACT.ACT_ID = FSV_VENTA.ACT_ID
	    LEFT JOIN (SELECT VAL7.ACT_ID, VAL7.VAL_IMPORTE 
	                FROM (SELECT  ROW_NUMBER() OVER(PARTITION BY VAL7.ACT_ID ORDER BY VAL7.FECHACREAR DESC) AS r, VAL7.ACT_ID, VAL7.VAL_IMPORTE
	                    FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL7
	                    INNER JOIN ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC7 ON VAL7.DD_TPC_ID = TPC7.DD_TPC_ID AND TPC7.DD_TPC_CODIGO = ''20'') VAL7
                  	WHERE VAL7.r=1) FSV_RENTA ON ACT.ACT_ID = FSV_RENTA.ACT_ID';
