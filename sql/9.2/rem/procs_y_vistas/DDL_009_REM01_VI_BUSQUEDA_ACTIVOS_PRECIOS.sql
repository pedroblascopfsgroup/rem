  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ACTIVOS_PRECIOS 
	AS
	    ...
	    LEFT JOIN (SELECT VAL2.ACT_ID, VAL2.VAL_IMPORTE 
	                FROM ACT_VAL_VALORACIONES VAL2 
	                INNER JOIN DD_TPC_TIPO_PRECIO TPC2 ON VAL2.DD_TPC_ID = TPC2.DD_TPC_ID AND TPC2.DD_TPC_CODIGO = ''03'') PRECIO_APROBADO_RENTA ON ACT.ACT_ID = PRECIO_APROBADO_RENTA.ACT_ID
	    LEFT JOIN (SELECT VAL3.ACT_ID, VAL3.VAL_IMPORTE 
	                FROM ACT_VAL_VALORACIONES VAL3 
	                INNER JOIN DD_TPC_TIPO_PRECIO TPC3 ON VAL3.DD_TPC_ID = TPC3.DD_TPC_ID AND TPC3.DD_TPC_CODIGO = ''04'') PRECIO_MINIMO_AUTORIZADO ON ACT.ACT_ID = PRECIO_MINIMO_AUTORIZADO.ACT_ID
	    LEFT JOIN (SELECT VAL4.ACT_ID, VAL4.VAL_IMPORTE 
	                FROM ACT_VAL_VALORACIONES VAL4
	                INNER JOIN DD_TPC_TIPO_PRECIO TPC4 ON VAL4.DD_TPC_ID = TPC4.DD_TPC_ID AND TPC4.DD_TPC_CODIGO = ''07'') PRECIO_DESCUENTO_APROBADO ON ACT.ACT_ID = PRECIO_DESCUENTO_APROBADO.ACT_ID
	    LEFT JOIN (SELECT VAL5.ACT_ID, VAL5.VAL_IMPORTE 
	                FROM ACT_VAL_VALORACIONES VAL5
	                INNER JOIN DD_TPC_TIPO_PRECIO TPC5 ON VAL5.DD_TPC_ID = TPC5.DD_TPC_ID AND TPC5.DD_TPC_CODIGO = ''13'') PRECIO_DESCUENTO_PUBLICADO ON ACT.ACT_ID = PRECIO_DESCUENTO_PUBLICADO.ACT_ID
	    LEFT JOIN (SELECT VAL6.ACT_ID, VAL6.VAL_IMPORTE 
	                FROM ACT_VAL_VALORACIONES VAL6
	                INNER JOIN DD_TPC_TIPO_PRECIO TPC6 ON VAL6.DD_TPC_ID = TPC6.DD_TPC_ID AND TPC6.DD_TPC_CODIGO = ''19'') FSV_VENTA ON ACT.ACT_ID = FSV_VENTA.ACT_ID
	    LEFT JOIN (SELECT VAL7.ACT_ID, VAL7.VAL_IMPORTE 
	                FROM ACT_VAL_VALORACIONES VAL7 
	                INNER JOIN DD_TPC_TIPO_PRECIO TPC7 ON VAL7.DD_TPC_ID = TPC7.DD_TPC_ID AND TPC7.DD_TPC_CODIGO = ''20'') FSV_RENTA ON ACT.ACT_ID = FSV_RENTA.ACT_ID';
