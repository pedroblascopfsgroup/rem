  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_TRABAJOS 
	...
	AS
		SELECT
			...
			NVL2(SOLIC.USU_NOMBRE, INITCAP(SOLIC.USU_NOMBRE) || NVL2(SOLIC.USU_APELLIDO1, '' '' || INITCAP(SOLIC.USU_APELLIDO1), '''') || NVL2(SOLIC.USU_APELLIDO2, '' '' || INITCAP(SOLIC.USU_APELLIDO2), ''''), INITCAP(PVE2.PVE_NOMBRE)) AS SOLICITANTE,
		...
		JOIN ' || V_ESQUEMA || '.V_FIRST_ACTIVO_TRABAJO VAT ON VAT.TBJ_ID = TBJ.TBJ_ID
		LEFT JOIN ' || V_ESQUEMA || '.ACT_ACTIVO ACT ON ACT.ACT_ID = VAT.ACT_ID 
