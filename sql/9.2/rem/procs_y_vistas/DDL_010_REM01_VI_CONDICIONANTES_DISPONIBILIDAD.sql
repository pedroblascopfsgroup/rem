...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_COND_DISPONIBILIDAD
	AS
		WITH ACTIVOS_ALQUILER AS (
			SELECT  SPS.*
			FROM '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA SPS 
			INNER JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_TITULO_POSESORIO TPO ON SPS.DD_TPO_ID = TPO.DD_TPO_ID 
			WHERE TPO.DD_TPO_CODIGO = ''02''
		)
		SELECT ACT.ACT_ID,
               NVL2(SPS4.SPS_ID,1,0) AS SIN_TOMA_POSESION_INICIAL,
		       NVL2(SPS7.SPS_ID,1,0) AS ALQUILADO,
		...
		LEFT JOIN '||V_ESQUEMA||'.ACTIVOS_ALQUILER SPS7 ON SPS7.ACT_ID = ACT.ACT_ID
