...
--## VERSIONES:
--##        0.1 Versión inicial
--##		0.2 Subdivisión Activo - GUILLEM REY - 20170724
--##		0.2 Poner lefts donde los inners para evitar que no salgan los registros si les falta algo
--##########################################
--*/
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_ACTIVOS_AGRUPACION   
   ...
   SELECT DISTINCT DECODE (AGRU.AGR_ACT_PRINCIPAL, ACT.ACT_ID, 1, 0) AS PRINCIPAL,
		...
		EPU.DD_EPU_DESCRIPCION AS PUBLICADO,
  ...
  LEFT JOIN '||V_ESQUEMA||'.V_ACTIVOS_SUBDIVISION ACT_SD ON ACT.ACT_ID = ACT_SD.ACT_ID
  LEFT JOIN '||V_ESQUEMA||'.V_SUBDIVISIONES_AGRUPACION SDV ON ACT_SD.AGR_ID = SDV.AGR_ID AND SDV.ID = ACT_SD.ID
