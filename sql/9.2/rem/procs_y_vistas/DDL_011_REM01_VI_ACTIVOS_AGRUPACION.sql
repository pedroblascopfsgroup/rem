...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_ACTIVOS_AGRUPACION   
   ...
   SELECT DECODE (AGRU.AGR_ACT_PRINCIPAL, ACT.ACT_ID, 1, 0) AS PRINCIPAL,
		...
		(SELECT MAX (VAL2.VAL_IMPORTE) FROM ACT_VAL_VALORACIONES VAL2 WHERE VAL2.DD_TPC_ID = (SELECT DD_TPC_ID FROM DD_TPC_TIPO_PRECIO WHERE DD_TPC_CODIGO = ''02'') AND VAL2.VAL_FECHA_FIN IS NULL AND VAL2.ACT_ID = ACT.ACT_ID) AS VAL_IMPORTE_APROBADO_VENTA,
        (SELECT MAX (VAL2.VAL_IMPORTE) FROM ACT_VAL_VALORACIONES VAL2 WHERE VAL2.DD_TPC_ID = (SELECT DD_TPC_ID FROM DD_TPC_TIPO_PRECIO WHERE DD_TPC_CODIGO = ''04'') AND VAL2.VAL_FECHA_FIN IS NULL AND VAL2.ACT_ID = ACT.ACT_ID) AS VAL_IMPORTE_MINIMO_AUTORIZADO,
        (SELECT MAX (VAL2.VAL_IMPORTE) FROM ACT_VAL_VALORACIONES VAL2 WHERE VAL2.DD_TPC_ID = (SELECT DD_TPC_ID FROM DD_TPC_TIPO_PRECIO WHERE DD_TPC_CODIGO = ''01'') AND VAL2.VAL_FECHA_FIN IS NULL AND VAL2.ACT_ID = ACT.ACT_ID) AS VAL_NETO_CONTABLE,
        --AGR.AGA_PRINCIPAL,
