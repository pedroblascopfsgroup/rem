	...
	AS
		SELECT
			...
			ACT.ACT_GESTION AS GESTION,
      		DECODE(HIC.DD_AIC_ID, ''02'',1,0) INFORME_COMERCIAL,
		...
		AND (HIC.HIC_FECHA IS NULL OR HIC.HIC_FECHA = (SELECT MAX(HIC.HIC_FECHA) FROM ACT_HIC_EST_INF_COMER_HIST HIST2 WHERE ACT.ACT_ID = HIST2.ACT_ID))
    AND (EPU.DD_EPU_CODIGO IS NULL OR EPU.DD_EPU_CODIGO != ''01'')';
