...
--          WHEN ''01'' THEN
--            (SELECT SUM(VAL.VAL_IMPORTE) AS PRECIO_PUBLICADO FROM '|| V_ESQUEMA ||'.ACT_VAL_VALORACIONES VAL INNER JOIN '|| V_ESQUEMA ||'.ACT_OFR OFR2 ON VAL.ACT_ID = OFR2.ACT_ID INNER JOIN '|| V_ESQUEMA ||'.DD_TPC_TIPO_PRECIO TPC ON TPC.DD_TPC_ID = VAL.DD_TPC_ID WHERE TPC.DD_TPC_CODIGO=''02'' AND OFR2.OFR_ID = OFR.OFR_ID GROUP BY OFR2.OFR_ID, TPC.DD_TPC_CODIGO)
--          WHEN ''02'' THEN
--            (SELECT SUM(VAL.VAL_IMPORTE) AS PRECIO_PUBLICADO FROM '|| V_ESQUEMA ||'.ACT_VAL_VALORACIONES VAL INNER JOIN '|| V_ESQUEMA ||'.ACT_OFR OFR3 ON VAL.ACT_ID = OFR3.ACT_ID INNER JOIN '|| V_ESQUEMA ||'.DD_TPC_TIPO_PRECIO TPC ON TPC.DD_TPC_ID = VAL.DD_TPC_ID WHERE TPC.DD_TPC_CODIGO=''03'' AND OFR3.OFR_ID = OFR.OFR_ID GROUP BY OFR3.OFR_ID, TPC.DD_TPC_CODIGO)
--          END) AS PRECIO_PUBLICADO,
...
--				OFRACT.ACTIVOS,
	...
	--		LEFT JOIN (SELECT ACTOFR.OFR_ID, LISTAGG(''''||ACT.ACT_NUM_ACTIVO||'''',',') WITHIN GROUP (ORDER BY OFR_ID) AS ACTIVOS FROM '|| V_ESQUEMA ||'.ACT_OFR ACTOFR JOIN ACT_ACTIVO ACT ON ACT.ACT_ID = ACTOFR.ACT_ID GROUP BY ACTOFR.OFR_ID) OFRACT ON OFRACT.OFR_ID = OFR.OFR_ID
