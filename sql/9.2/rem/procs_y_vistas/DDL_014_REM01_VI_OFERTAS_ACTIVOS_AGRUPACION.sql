...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_OFERTAS_ACTIVOS_AGRUPACION 
	AS
		SELECT
		        ...
		        CLI.CLC_NOMBRE || '' '' ||CLI.CLC_APELLIDOS AS OFERTANTE,
		        ACTVAL.VAL_IMPORTE,
		        OFR.OFR_IMPORTE,
    	...
    	INNER JOIN ' || V_ESQUEMA || '.CLI_CLIENTE_COMERCIAL CLI on CLI.CLC_ID = OFR.CLC_ID
  ...
  EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA || '.VI_OFERTAS_ACTIVOS_AGRUPACION.FECHACREAR IS ''Fecha de creación de la oferta''';
