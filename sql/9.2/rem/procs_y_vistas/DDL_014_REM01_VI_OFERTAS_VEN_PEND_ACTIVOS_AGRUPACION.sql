...
BEGIN
  ...
  V_MSQL := 'CREATE VIEW ' || V_ESQUEMA || '.VI_OFERTAS_ACTIVOS_AGRUPACION2 
	...
	AS
		...
		INNER JOIN '|| V_ESQUEMA ||'.DD_EOF_ESTADOS_OFERTA EOF on EOF.DD_EOF_ID = OFR.DD_EOF_ID AND OFR.DD_EOF_ID IN (SELECT DD_EOF_ID FROM DD_EOF_ESTADOS_OFERTA EOF WHERE DD_EOF_CODIGO IN (''01'',''04''))
