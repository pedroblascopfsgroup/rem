...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_STOCK_ACTIVOS_WEBCOM 
	AS
		...
		(SELECT VAL_FECHA_INICIO FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL, ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC 
		WHERE VAL.ACT_ID = ACT.ACT_ID AND TPC.DD_TPC_ID = VAL.DD_TPC_ID AND TPC.DD_TPC_CODIGO = ''02'' AND VAL.VAL_FECHA_FIN IS NULL) DESDE_IMPORTE,
		(SELECT VAL_FECHA_FIN FROM ' || V_ESQUEMA || '.ACT_VAL_VALORACIONES VAL, ' || V_ESQUEMA || '.DD_TPC_TIPO_PRECIO TPC 
		WHERE VAL.ACT_ID = ACT.ACT_ID AND TPC.DD_TPC_ID = VAL.DD_TPC_ID AND TPC.DD_TPC_CODIGO = ''02'' AND VAL.VAL_FECHA_FIN IS NULL) HASTA_IMPORTE,
		...
		(SELECT CASE DDEAC.DD_EAC_CODIGO WHEN ''03'' THEN ''1'' WHEN ''04'' THEN ''1'' ELSE ''0'' END AS NUEVO 
		...
		NULL AS COD_DETALLE_PUBLICACION, --PENDIENTE CREAR DICCIONARIO -> NO DEBERIA SER DICCIONARIO SINO CAMPOS BOOL
		...
		USU.USU_NOMBRE || '' '' || USU.USU_APELLIDO1 || '' '' || USU.USU_APELLIDO2 AS NOMBRE_GESTOR_COMERCIAL, -- NO EXISTE ESTE TIPO DE GESTOR TODAVIA
		USU.USU_TELEFONO AS TELEFONO_GESTOR_COMERCIAL, -- NO EXISTE ESTE TIPO DE GESTOR TODAVIA
		USU.USU_MAIL AS EMAIL_GESTOR_COMERCIAL, -- NO EXISTE ESTE TIPO DE GESTOR TODAVIA
