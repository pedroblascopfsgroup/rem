  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_BUSQUEDA_GASTOS_PROVEEDOR 
	AS
		SELECT
				...
				PVEGEST.PVE_DOCIDENTIF AS NIF_GESTORIA,
				NVL2(ADJUNTOS.NUM_DOCUMENTOS,1,0) AS TIENE_DOC_ADJUNTOS
		...
		LEFT JOIN ' || V_ESQUEMA || '.ACT_PVE_PROVEEDOR PVEGEST ON GPV.PVE_ID_GESTORIA = PVEGEST.PVE_ID
		...
		LEFT JOIN (SELECT ADG.GPV_ID, COUNT(ADG.ADG_ID) AS NUM_DOCUMENTOS FROM ADG_ADJUNTOS_GASTO ADG GROUP BY ADG.GPV_ID) ADJUNTOS ON ADJUNTOS.GPV_ID = GPV.GPV_ID
