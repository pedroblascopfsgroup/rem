  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_BUSQUEDA_GASTOS_ACTIVOS 
	AS
		SELECT
				...
				ACT.ACT_NUM_ACTIVO,
				BIELOC.BIE_LOC_NOMBRE_VIA || '' '' || BIELOC.BIE_LOC_NUMERO_DOMICILIO AS DIRECCION || '' - '' || BIELOC.BIE_LOC_COD_POST,
				(GDE.GDE_IMPORTE_TOTAL * GPVACT.GPV_PARTICIPACION_GASTO) /100 AS IMPORTE_PROPORCIONAL_TOTAL, 
				REFERENCIAS,
		...
		LEFT JOIN ' || V_ESQUEMA || '.ACT_ACTIVO ACT ON GPVACT.ACT_ID = ACT.ACT_ID
		...
		LEFT JOIN ' || V_ESQUEMA || '.ACT_ICO_INFO_COMERCIAL ACTICO ON ACT.ACT_ID = ACTICO.ACT_ID 
		LEFT JOIN ' || V_ESQUEMA || '.GPV_GASTOS_PROVEEDOR GPV ON GPV.GPV_ID = GPVACT.GPV_ID
		LEFT JOIN ' || V_ESQUEMA || '.GDE_GASTOS_DETALLE_ECONOMICO GDE ON GPVACT.GPV_ID = GDE.GPV_ID	
		LEFT JOIN ' || V_ESQUEMA || '.GGE_GASTOS_GESTION GGE ON GGE.GPV_ID = GPVACT.GPV_ID
