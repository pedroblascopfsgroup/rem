...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_VISITAS_DETALLE 
	AS
		...
		LEFT JOIN ' || V_ESQUEMA || '.ACT_PVE_PROVEEDOR PVEC on PVEC.PVE_ID = VIS.PVE_ID_API_CUSTODIO and PVEC.BORRADO = 0
		LEFT JOIN ' || V_ESQUEMA || '.ACT_PVE_PROVEEDOR PVEP on PVEP.PVE_ID = VIS.PVE_ID_PRESCRIPTOR and PVEP.BORRADO = 0
		LEFT JOIN ' || V_ESQUEMA || '.DD_TPR_TIPO_PROVEEDOR TPRC on TPRC.DD_TPR_ID = PVEC.DD_TPR_ID and TPRC.BORRADO = 0
		LEFT JOIN ' || V_ESQUEMA || '.DD_TPR_TIPO_PROVEEDOR TPRP on TPRP.DD_TPR_ID = PVEP.DD_TPR_ID and TPRP.BORRADO = 0
		LEFT JOIN ' || V_ESQUEMA || '.DD_CRA_CARTERA CART on ACT.DD_CRA_ID = CART.DD_CRA_ID and CART.BORRADO = 0
    ';
