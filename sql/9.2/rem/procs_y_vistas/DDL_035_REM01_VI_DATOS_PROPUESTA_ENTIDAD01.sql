...
BEGIN
	...
	/*SE DESHABILITA DEBIDO A LA EXISTENCIA DE UN DDL POSTERIOR*/
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_DATOS_PROPUESTA_ENTIDAD01 
	AS
		...
		FROM '||V_ESQUEMA||'.ACT_PRP ACP
			...
			LEFT JOIN '||V_ESQUEMA||'.ACT_AGA_AGRUPACION_ACTIVO AGA_ON ON AGA_ON.ACT_ID = ACT.ACT_ID
			LEFT JOIN (SELECT AGR_ON.AGR_ID, AGR_ON.AGR_NUM_AGRUP_REM, AGR_ON.AGR_NOMBRE FROM '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR_ON WHERE AGR_ON.DD_TAG_ID = (SELECT DD_TAG_ID FROM '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO=''01'')) OBRA_NUEVA ON OBRA_NUEVA.AGR_ID = AGA_ON.AGR_ID 
