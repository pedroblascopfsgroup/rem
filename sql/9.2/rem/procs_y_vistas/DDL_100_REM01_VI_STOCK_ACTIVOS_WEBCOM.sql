	...
	AS
		...
		VALORES_PRECIOS_ANT AS (
			SELECT AUX.* FROM (
				...
				LEFT JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO DD ON DD.DD_TPC_ID = HVAL.DD_TPC_ID AND DD.DD_TPC_CODIGO = ''02'') AUX
			WHERE AUX.ORDEN = 1
		),
		...
		CASE WHEN (VAL2.VAL_FECHA_INICIO IS NOT NULL) 
			THEN CAST(TO_CHAR(VAL2.VAL_FECHA_INICIO ,
					''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		ELSE NULL
		...
		LEFT JOIN '||V_ESQUEMA||'.VI_VALORES_PRECIOS_INFMED VAL1 ON (VAL1.ACT_ID = ACT.ACT_ID AND VAL1.DD_TPC_CODIGO = ''02'')
		LEFT JOIN '||V_ESQUEMA||'.VI_VALORES_PRECIOS_INFMED VAL2 ON (VAL2.ACT_ID = ACT.ACT_ID AND VAL2.DD_TPC_CODIGO = ''03'')
