...
BEGIN/*Versión 0.8*/
	...
	AS
       ...
       v.descripcion_otros																	AS descripcion_otros,
		...
		CASE WHEN (AGA_PA.DD_TAG_ID = (SELECT AGA_PA.DD_TAG_ID FROM DD_TAG_TIPO_AGRUPACION TAG WHERE TAG.DD_TAG_CODIGO = ''16'') AND AGA_PA.AGA_PRINCIPAL = 1)
			THEN CAST(1 AS NUMBER(1,0))
			ELSE CAST(0 AS NUMBER(1,0))
		END 																				AS ES_ACTIVO_MATRIZ_PA,
        AGA_PA.AGR_ACT_PRINCIPAL															AS ID_ACTIVO_HAYA_PA,
        CASE WHEN (AGA_PA.DD_TAG_ID = (SELECT AGA_PA.DD_TAG_ID FROM DD_TAG_TIPO_AGRUPACION TAG WHERE TAG.DD_TAG_CODIGO = ''16''))
		...
		CASE WHEN (AGA_PA.DD_TAG_ID = (SELECT AGA_PA.DD_TAG_ID FROM DD_TAG_TIPO_AGRUPACION TAG WHERE TAG.DD_TAG_CODIGO = ''16'') AND AGA_PA.AGA_PRINCIPAL = 1)
			...
			ELSE CAST(0 AS NUMBER(1,0))
		END 																				AS INDICADOR_AM,
        CASE WHEN (AGA_PA.DD_TAG_ID = (SELECT AGA_PA.DD_TAG_ID FROM DD_TAG_TIPO_AGRUPACION TAG WHERE TAG.DD_TAG_CODIGO = ''16'') AND AGA_PA.AGA_PRINCIPAL = 0)
		...
		PVE.PVE_COD_REM																		AS ID_PROVEEDOR_ESPEJO_REM
		...
		LEFT JOIN (SELECT ACT_ID, ''['' || ARR_PROVEEDOR_LLAVES_REM || '']'' ARR_PROVEEDOR_LLAVES_REM FROM (
			   SELECT DISTINCT LLV.ACT_ID,
			   LISTAGG(CASE WHEN LLV.LLV_COD_TENEDOR_POSEEDOR IS NOT NULL AND LLV.LLV_COD_TENEDOR_POSEEDOR NOT LIKE ''N/A'' AND LLV.LLV_COD_TENEDOR_NO_PVE IS NULL THEN PVE.PVE_NOMBRE
			   WHEN LLV.LLV_COD_TENEDOR_POSEEDOR IS NULL AND LLV.LLV_COD_TENEDOR_NO_PVE IS NOT NULL AND LLV.LLV_COD_TENEDOR_NO_PVE NOT LIKE ''N/A'' THEN LLV.LLV_COD_TENEDOR_NO_PVE END , '','') 
			   ...
			   WHERE LLV.BORRADO = 0) WHERE ARR_PROVEEDOR_LLAVES_REM IS NOT NULL) LLV ON LLV.ACT_ID = ACT.ACT_ID
