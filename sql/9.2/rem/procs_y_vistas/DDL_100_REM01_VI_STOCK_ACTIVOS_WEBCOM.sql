...
BEGIN
	...
	AS
	    ...
	    SUBDIVISIONES_ASISTIDAS AS (
			...
			INNER JOIN '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR ON VSUB.AGR_ID = AGR.AGR_ID 
			INNER JOIN '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION TAG ON TAG.DD_TAG_ID = AGR.DD_TAG_ID 
			WHERE AGR.AGR_NUM_AGRUP_REM IS NOT NULL AND TAG.DD_TAG_CODIGO = ''13'' AND AGR.BORRADO = 0 AND (AGR.AGR_FECHA_BAJA IS NULL OR AGR.AGR_FECHA_BAJA > TO_DATE(SYSDATE, ''DD/MM/YY''))
		)
		SELECT 
