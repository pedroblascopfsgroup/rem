...
BEGIN
	...
	AS
		...
		GESTOR_COMERCIAL_SIN AS (
			SELECT AUX.* FROM (
				...
				ROW_NUMBER() OVER (PARTITION BY GAC.ACT_ID, TGE.DD_TGE_CODIGO ORDER BY GEE.GEE_ID DESC) REF
				FROM '||V_ESQUEMA||'.GAC_GESTOR_ADD_ACTIVO GAC 
				JOIN '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE ON GEE.GEE_ID = GAC.GEE_ID 
				JOIN '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR TGE ON (GEE.DD_TGE_ID = TGE.DD_TGE_ID AND DD_TGE_CODIGO = ''GCOMSIN'')) AUX
			WHERE AUX.REF = 1
		),
		GESTOR_COMERCIAL_RET AS (
			SELECT AUX.* FROM (
				...
				FROM '||V_ESQUEMA||'.GAC_GESTOR_ADD_ACTIVO GAC 
				JOIN '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE ON GEE.GEE_ID = GAC.GEE_ID 
				JOIN '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR TGE ON (GEE.DD_TGE_ID = TGE.DD_TGE_ID AND DD_TGE_CODIGO = ''GCOMRET'')) AUX
			WHERE AUX.REF = 1
		),
	    PUBLICADO_DESDE AS (
	        SELECT AUX.* FROM (
	    ...
	    SUBDIVISIONES_ASISTIDAS AS (
			SELECT AUX.* FROM (
				SELECT VSUB.ACT_ID, VSUB.ID, AGR.AGR_NUM_AGRUP_REM, 
				ROW_NUMBER() OVER (PARTITION BY VSUB.ACT_ID, VSUB.ID ORDER BY VSUB.ID DESC) ORDEN 
				FROM '||V_ESQUEMA||'.V_ACTIVOS_SUBDIVISION VSUB 
				INNER JOIN '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR ON VSUB.AGR_ID = AGR.AGR_ID 
				INNER JOIN '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION TAG ON TAG.DD_TAG_ID = AGR.DD_TAG_ID 
				WHERE AGR.AGR_NUM_AGRUP_REM IS NOT NULL AND TAG.DD_TAG_CODIGO = ''13'' AND AGR.BORRADO = 0 AND (AGR.AGR_FECHA_BAJA IS NULL OR AGR.AGR_FECHA_BAJA > TO_DATE(SYSDATE, ''DD/MM/YY''))) AUX
        	WHERE AUX.ORDEN = 1
		)
		SELECT 
		...
		CAST(ACT_DIS.ESTADO AS VARCHAR2(5 CHAR))                       						AS COD_DETALLE_PUBLICACION, 
		...
		CAST((SELECT MAX(PVE2.PVE_COD_REM) FROM 
		'||V_ESQUEMA||'.ACT_ICM_INF_COMER_HIST_MEDI ICM
		JOIN '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR PVE2 ON PVE2.PVE_ID = ICM.ICO_MEDIADOR_ID
		...
		END AS NOMBRE_GESTOR_COMERCIAL,
		CASE WHEN (TCR.DD_TCR_CODIGO = ''01'')		
		...
		CAST(SAS.AGR_NUM_AGRUP_REM AS NUMBER(16,0))	                                 		AS ID_ASISTIDA,
        CAST(SAS.ID AS NUMBER(16,0))                                 						AS CODIGO_CABECERA_ASISTIDA,
    ...
    FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
		...
		LEFT JOIN SUBDIVISIONES_ONV VSUB ON VSUB.ACT_ID = ACT.ACT_ID 
		LEFT JOIN SUBDIVISIONES_ASISTIDAS SAS ON SAS.ACT_ID = ACT.ACT_ID 
		LEFT JOIN DD_TCR_TIPO_COMERCIALIZAR TCR ON TCR.DD_TCR_ID = ACT.DD_TCR_ID
		INNER JOIN V_ACTIVO_ESTADO_DISPONIBILIDAD ACT_DIS ON ACT_DIS.ACT_ID = ACT.ACT_ID';
