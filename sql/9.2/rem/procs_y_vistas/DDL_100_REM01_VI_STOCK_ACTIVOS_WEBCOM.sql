...
BEGIN/*Versión 0.8*/
	...
	AS
        ...
        CAST(NVL((SELECT USU.USU_ID FROM REMMASTER.USU_USUARIOS USU
                    WHERE USU.USU_USERNAME = NVL(ACT.USUARIOMODIFICAR, ACT.USUARIOCREAR)),
                  (SELECT USU.USU_ID FROM REMMASTER.USU_USUARIOS USU
       ...
       v.descripcion_otros                                                                  AS descripcion_otros,
        ...
        HFP.COD_FASE_PUBLICACION AS COD_FASE_PUBLICACION,
        HFP.COD_SUBFASE_PUBLICACION AS COD_SUBFASE_PUBLICACION,
		...
		CAST(PAC.PAC_EXCLUIR_VALIDACIONES AS NUMBER(16,0))											AS EXCLUIR_VALIDACIONES,
		CAST(PAC.PAC_FECHA_GESTION_COMERCIAL AS DATE)												AS FECHA_GESTION_COMERCIAL,
		CAST(PAC.PAC_MOTIVO_GESTION_COMERCIAL AS NUMBER(16,0))										AS MOTIVO_GESTION_COMERCIAL


        FROM REM01.ACT_ACTIVO ACT
        INNER JOIN REM01.ACT_LOC_LOCALIZACION LOC ON LOC.ACT_ID = ACT.ACT_ID
        INNER JOIN REM01.BIE_LOCALIZACION BLOC ON BLOC.BIE_LOC_ID = LOC.BIE_LOC_ID
        LEFT JOIN REMMASTER.DD_TVI_TIPO_VIA DDTVI ON DDTVI.DD_TVI_ID = BLOC.DD_TVI_ID
        LEFT JOIN REMMASTER.DD_UPO_UNID_POBLACIONAL DDUPO ON DDUPO.DD_UPO_ID = BLOC.DD_UPO_ID
        LEFT JOIN REMMASTER.DD_PRV_PROVINCIA DDPRV ON DDPRV.DD_PRV_ID = BLOC.DD_PRV_ID
        LEFT JOIN REMMASTER.DD_LOC_LOCALIDAD DDLOC ON DDLOC.DD_LOC_ID = BLOC.DD_LOC_ID
        LEFT JOIN REM01.DD_TPA_TIPO_ACTIVO DDTPA ON DDTPA.DD_TPA_ID = ACT.DD_TPA_ID
        LEFT JOIN REM01.DD_SAC_SUBTIPO_ACTIVO DDSAC ON DDSAC.DD_SAC_ID = ACT.DD_SAC_ID

        INNER JOIN REM01.ACT_REG_INFO_REGISTRAL REG ON REG.ACT_ID = ACT.ACT_ID
        INNER JOIN REM01.BIE_DATOS_REGISTRALES BREG ON BREG.BIE_DREG_ID = REG.BIE_DREG_ID
        LEFT JOIN REMMASTER.DD_LOC_LOCALIDAD DDBRLOC ON DDBRLOC.DD_LOC_ID = BREG.DD_LOC_ID

        LEFT JOIN REM01.ACT_ICO_INFO_COMERCIAL ICO ON ICO.ACT_ID = ACT.ACT_ID and ico.borrado = 0
        LEFT JOIN REM01.ACT_EDI_EDIFICIO EDI ON EDI.ICO_ID = ICO.ICO_ID
        LEFT JOIN REM01.VI_PIVOT_DISTRIBUCION VPD ON VPD.ICO_ID = ICO.ICO_ID
        LEFT JOIN REM01.DD_EAC_ESTADO_ACTIVO DDEAC ON DDEAC.DD_EAC_ID = ACT.DD_EAC_ID
        LEFT JOIN REM01.DD_TUD_TIPO_USO_DESTINO TUD ON TUD.DD_TUD_ID = ACT.DD_TUD_ID
        LEFT JOIN REM01.DD_SCM_SITUACION_COMERCIAL DDSCM ON DDSCM.DD_SCM_ID = ACT.DD_SCM_ID

        LEFT JOIN REM01.DD_ECT_ESTADO_CONSTRUCCION DDECT ON DDECT.DD_ECT_ID = ICO.DD_ECT_ID
        LEFT JOIN REM01.DD_STA_SUBTIPO_TITULO_ACTIVO DDSTA ON DDSTA.DD_STA_ID = ACT.DD_STA_ID
        LEFT JOIN REM01.DD_SCR_SUBCARTERA DDSCR ON DDSCR.DD_SCR_ID = ACT.DD_SCR_ID
        LEFT JOIN REM01.DD_EQG_EQUIPO_GESTION EQG ON EQG.DD_EQG_ID = ACT.DD_EQG_ID
        LEFT JOIN REM01.DD_TCR_TIPO_COMERCIALIZAR TCR ON TCR.DD_TCR_ID = ACT.DD_TCR_ID

        LEFT JOIN REM01.ACT_VIV_VIVIENDA VIV ON VIV.ICO_ID = ICO.ICO_ID
        LEFT JOIN REM01.ACT_ADM_INF_ADMINISTRATIVA ADM ON ADM.ACT_ID = ACT.ACT_ID
        LEFT JOIN REM01.DD_TVP_TIPO_VPO VPO ON VPO.DD_TVP_ID = ADM.DD_TVP_ID  
        LEFT JOIN REM01.DD_RTG_RATING_ACTIVO DDRTG ON DDRTG.DD_RTG_ID = ACT.DD_RTG_ID
        LEFT JOIN REM01.DD_CRA_CARTERA DDCRA ON DDCRA.DD_CRA_ID = ACT.DD_CRA_ID

        LEFT JOIN REM01.ACT_SPS_SIT_POSESORIA SPS ON SPS.ACT_ID = ACT.ACT_ID and sps.borrado = 0
        LEFT JOIN REM01.V_FECHA_POSESION_ACTIVO FPA ON FPA.ACT_ID = SPS.ACT_ID
        LEFT JOIN REM01.ACT_ZCO_ZONA_COMUN ZCO ON ZCO.ICO_ID = ICO.ICO_ID
        LEFT JOIN REM01.ACT_PVE_PROVEEDOR PVE ON PVE.PVE_ID = ICO.ICO_MEDIADOR_ID
        LEFT JOIN REM01.ACT_PVE_PROVEEDOR PVE_ESPEJO ON PVE_ESPEJO.PVE_ID = ICO.ICO_MEDIADOR_ESPEJO_ID

        LEFT JOIN REM01.VI_STOCK_PIVOT_AGRUP_ACTIVO PIVOT_AGR ON PIVOT_AGR.ACT_ID = ACT.ACT_ID -- VI_STOCK_PIVOT_AGRUP_ACTIVO
        LEFT JOIN REM01.VI_PIVOT_AGRUPACIONES_UA PIVOT_UA ON PIVOT_UA.ACT_ID = ACT.ACT_ID -- VI_PIVOT_AGRUPACIONES_UA
        LEFT JOIN REM01.VI_STOCK_ACTIVOS_SUBDIVISON SUBD_ACT ON SUBD_ACT.ACT_ID = ACT.ACT_ID --v_activos_lite
        LEFT JOIN REM01.VI_STOCK_PIVOT_PRECIOS VAL1 ON VAL1.ACT_ID = ACT.ACT_ID --v_stock_pivot_precios
        LEFT JOIN REM01.VI_STOCK_PIVOT_PRECIOS_ANT VANT ON VANT.ACT_ID = ACT.ACT_ID --v_new_pivot_precios_ant

        INNER JOIN REM01.VI_STOCK_ACTIVO_CONDICIONANTE VCOND ON VCOND.ACT_ID = ACT.ACT_ID --v_stock_activo_condicionantes
        LEFT JOIN REM01.VI_STOCK_ACTIVO_REFCATASTRAL CAT ON CAT.ACT_ID = ACT.ACT_ID
        LEFT JOIN REM01.VI_STOCK_ACTIVO_PROV_ANT PVE_ANT ON PVE_ANT.ACT_ID = ACT.ACT_ID
        LEFT JOIN REM01.VI_STOCK_ACTIVO_FECHAPUBLICA PUB ON PUB.ACT_ID = ACT.ACT_ID
        left join ( SELECT COE.ACT_ID, COE.COE_TEXTO
                    FROM REM01.ACT_COE_CONDICION_ESPECIFICA COE
                    WHERE COE.COE_FECHA_HASTA IS NULL AND COE.BORRADO = 0 AND COE.COE_FECHA_DESDE <= SYSDATE ) COE on COE.ACT_ID = ACT.ACT_ID

        LEFT JOIN REM01.act_apu_activo_publicacion actpub ON actpub.ACT_ID = ACT.ACT_ID
        LEFT JOIN REM01.DD_TCO_TIPO_COMERCIALIZACION DDTCO ON DDTCO.DD_TCO_ID = actpub.DD_TCO_ID
        LEFT JOIN REM01.dd_epv_estado_pub_venta epv ON epv.dd_epv_id = actpub.dd_epv_id
        LEFT JOIN REM01.dd_epa_estado_pub_alquiler epa ON epa.dd_epa_id = actpub.dd_epa_id
        LEFT JOIN REM01.dd_tpu_tipo_publicacion tpu ON tpu.dd_tpu_id = actpub.DD_TPU_V_ID
        LEFT JOIN REM01.dd_tpu_tipo_publicacion tpu ON tpu.dd_tpu_id = actpub.DD_TPU_A_ID
        ...
        LEFT JOIN(  SELECT DISTINCT pve.PVE_COD_REM, gac.ACT_ID, row_number() over(partition by gac.act_id order by pve.fechacrear desc) rn
            FROM REM01.GAC_GESTOR_ADD_ACTIVO gac
                ...
                inner join REM01.ACT_PVC_PROVEEDOR_CONTACTO pvc ON pvc.USU_ID = gee.USU_ID
                inner join REM01.ACT_PVE_PROVEEDOR pve ON pve.PVE_ID = pvc.PVE_ID
                inner join REM01.DD_EPR_ESTADO_PROVEEDOR epr ON epr.DD_EPR_ID = pve.DD_EPR_ID AND epr.DD_EPR_CODIGO = ''04''
                inner join REM01.DD_TPR_TIPO_PROVEEDOR tpr ON tpr.DD_TPR_ID = pve.DD_TPR_ID AND tpr.DD_TPR_CODIGO =''05''
                INNER join REM01.act_activo act on act.act_id = gac.ACT_ID
                INNER join REMMASTER.dd_tge_tipo_gestor tge on tge.DD_TGE_ID = gee.DD_TGE_ID
                INNER join REM01.ACT_ETP_ENTIDAD_PROVEEDOR etp on etp.DD_CRA_ID = act.DD_CRA_ID and etp.PVE_ID = pve.PVE_ID
            WHERE tge.DD_TGE_CODIGO = ''PTEC''
              and act.act_id not in (SELECT DISTINCT t1.ACT_ID FROM REM01.ACT_ACTIVO t1 JOIN REM01.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = t1.DD_CRA_ID WHERE CRA.DD_CRA_CODIGO IN (''02''))
        ...
        LEFT JOIN (
            ...
            JOIN REM01.DD_TCE_TIPO_CALIF_ENERGETICA DDTCE ON ADO.DD_TCE_ID = DDTCE.DD_TCE_ID
            JOIN REM01.ACT_CFD_CONFIG_DOCUMENTO CFD ON CFD.CFD_ID = ADO.CFD_ID
        ...
        LEFT JOIN (
            ...
            INNER JOIN REM01.ACT_AGR_AGRUPACION AGR ON AGA.AGR_ID = AGR.AGR_ID
            INNER JOIN REM01.DD_TAG_TIPO_AGRUPACION DDTAG ON AGR.DD_TAG_ID = DDTAG.DD_TAG_ID
        ...
        LEFT JOIN (
            ...
            INNER JOIN REM01.ACT_AGA_AGRUPACION_ACTIVO AGA2 ON AGA2.AGR_ID = AGR.AGR_ID AND AGA2.AGA_PRINCIPAL = 1
            INNER JOIN REM01.ACT_ACTIVO ACT ON ACT.ACT_ID = AGA2.ACT_ID
            WHERE AGA.BORRADO = 0 AND AGR.BORRADO = 0 AND DDTAG.BORRADO = 0 AND AGR.AGR_FECHA_BAJA IS NULL
        ) AGA_PA ON ACT.ACT_ID = AGA_PA.ACT_ID AND AGA_PA.RN = 1
        LEFT JOIN REM01.ACT_PTA_PATRIMONIO_ACTIVO ACT_PTA ON ACT_PTA.ACT_ID = ACT.ACT_ID AND ACT_PTA.BORRADO = 0
        LEFT JOIN REM01.DD_TAL_TIPO_ALQUILER DD_TAL ON DD_TAL.DD_TAL_ID = ACT.DD_TAL_ID
        LEFT JOIN (
            SELECT GAC.ACT_ID
                , GEE.USU_ID
                , USU.USU_NOMBRE || '' '' || USU.USU_APELLIDO1 || '' '' || USU.USU_APELLIDO2 NOMBRE
                , USU.USU_TELEFONO, USU.USU_MAIL
                , ROW_NUMBER() OVER(PARTITION BY GAC.ACT_ID ORDER BY GEE.FECHACREAR DESC) RN
            FROM REM01.GAC_GESTOR_ADD_ACTIVO GAC
            ...
            JOIN REMMASTER.USU_USUARIOS USU ON USU.USU_ID = GEE.USU_ID
                AND USU.BORRADO = 0
            WHERE TGE.DD_TGE_CODIGO = ''HAYAGBOINM''
        ...
        ) GCO ON GCO.ACT_ID = ACT.ACT_ID AND GCO.RN = 1
        LEFT JOIN (
            SELECT GAC.ACT_ID
                , GEE.USU_ID
                , USU.USU_NOMBRE || '' '' || USU.USU_APELLIDO1 || '' '' || USU.USU_APELLIDO2 NOMBRE
        ...
        LEFT JOIN (
            SELECT HFP.ACT_ID
				...
				, HFP.HFP_FECHA_FIN
                , ROW_NUMBER() OVER(PARTITION BY HFP.ACT_ID ORDER BY HFP.HFP_FECHA_FIN DESC NULLS FIRST, HFP.HFP_FECHA_INI DESC NULLS LAST) RN
			...
			WHERE (hfp.borrado = 0 or hfp.borrado is null) and hfp.hfp_fecha_fin is null
        ) HFP ON HFP.ACT_ID = ACT.ACT_ID AND HFP.RN = 1
        LEFT JOIN REM01.ACT_LLV_LLAVE LLV ON LLV.ACT_ID = ACT.ACT_ID AND LLV.BORRADO = 0 AND (LLV.LLV_COD_TENEDOR_POSEEDOR IS NOT NULL OR NVL(LLV.LLV_COD_TENEDOR_NO_PVE, '' '') <> ''N/A'')
        LEFT JOIN REM01.ACT_PVE_PROVEEDOR PVE_LLV ON LLV.LLV_COD_TENEDOR_POSEEDOR = PVE_LLV.PVE_ID AND PVE_LLV.BORRADO = 0
        LEFT JOIN REM01.ACT_PAC_PERIMETRO_ACTIVO PAC ON PAC.ACT_ID = ACT.ACT_ID AND PAC.BORRADO = 0
		where act.borrado = 0';
