...
BEGIN
	...
	SELECT COUNT(1) INTO CUENTA FROM ALL_OBJECTS WHERE OBJECT_NAME = V_TEXT_VISTA AND OWNER = V_ESQUEMA AND OBJECT_TYPE = 'VIEW';  
	IF CUENTA>0 THEN
    ...
    CAST(DDEPA.DD_EPA_CODIGO AS VARCHAR2(5 CHAR)) AS COD_ESTADO_PUBLICACION_A,
    CAST(DDEPV.DD_EPV_CODIGO AS VARCHAR2(5 CHAR)) AS COD_ESTADO_PUBLICACION_V,
		CASE WHEN (VIV.VIV_REFORMA_CARP_INT =1 OR VIV.VIV_REFORMA_CARP_EXT = 1 
		    ...
		    ELSE CAST(0 AS NUMBER(1,0)) 
		END 																				AS REFORMAS,
    CAST(TO_CHAR(PUB.APU_FECHA_INI_ALQUILER, ''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR)) AS PUBLICADO_DESDE_A,
    CAST(TO_CHAR(PUB.APU_FECHA_INI_VENTA, ''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR)) AS PUBLICADO_DESDE_V,
		...
		CAST(NVL((SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
					WHERE USU.BORRADO = 0 AND USU.USU_USERNAME = NVL(ACT.USUARIOMODIFICAR, ACT.USUARIOCREAR)),
                  (SELECT USU.USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU 
					WHERE USU.BORRADO = 0 AND USU.USU_USERNAME = ''REM-USER'')) AS NUMBER (16, 0)) 				AS ID_USUARIO_REM_ACCION,
		...
		INNER JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BLOC ON BLOC.BIE_LOC_ID = LOC.BIE_LOC_ID AND BLOC.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA_M||'.DD_TVI_TIPO_VIA DDTVI ON DDTVI.DD_TVI_ID = BLOC.DD_TVI_ID AND DDTVI.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA_M||'.DD_UPO_UNID_POBLACIONAL DDUPO ON DDUPO.DD_UPO_ID = BLOC.DD_UPO_ID AND DDUPO.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA DDPRV ON DDPRV.DD_PRV_ID = BLOC.DD_PRV_ID AND DDPRV.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD DDLOC ON DDLOC.DD_LOC_ID = BLOC.DD_LOC_ID AND DDLOC.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA||'.DD_TPA_TIPO_ACTIVO DDTPA ON DDTPA.DD_TPA_ID = ACT.DD_TPA_ID AND DDTPA.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA||'.DD_SAC_SUBTIPO_ACTIVO DDSAC ON DDSAC.DD_SAC_ID = ACT.DD_SAC_ID AND DDSAC.BORRADO = 0

		INNER JOIN '||V_ESQUEMA||'.ACT_REG_INFO_REGISTRAL REG ON REG.ACT_ID = ACT.ACT_ID AND REG.BORRADO = 0
		INNER JOIN '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES BREG ON BREG.BIE_DREG_ID = REG.BIE_DREG_ID AND BREG.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD DDBRLOC ON DDBRLOC.DD_LOC_ID = BREG.DD_LOC_ID AND DDBRLOC.BORRADO = 0
		...
		LEFT JOIN '||V_ESQUEMA||'.DD_EAC_ESTADO_ACTIVO DDEAC ON DDEAC.DD_EAC_ID = ACT.DD_EAC_ID AND DDEAC.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL DDSCM ON DDSCM.DD_SCM_ID = ACT.DD_SCM_ID AND DDSCM.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION DDTCO ON DDTCO.DD_TCO_ID = ACT.DD_TCO_ID AND DDTCO.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA||'.DD_ECT_ESTADO_CONSTRUCCION DDECT ON DDECT.DD_ECT_ID = ICO.DD_ECT_ID AND DDECT.BORRADO = 0
    
    LEFT JOIN '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU ON APU.ACT_ID = ACT.ACT_ID AND APU.BORRADO = 0
    LEFT JOIN '||V_ESQUEMA||'.DD_EPA_ESTADO_PUB_ALQUILER DDEPA ON DDEPA.DD_EPA_ID = APU.DD_EPA_ID AND DDEPA.BORRADO = 0
    LEFT JOIN '||V_ESQUEMA||'.DD_EPV_ESTADO_PUB_VENTA DDEPV ON DDEPV.DD_EPV_ID = APU.DD_EPV_ID AND DDEPV.BORRADO = 0  
    
		LEFT JOIN '||V_ESQUEMA||'.DD_STA_SUBTIPO_TITULO_ACTIVO DDSTA ON DDSTA.DD_STA_ID = ACT.DD_STA_ID AND DDSTA.BORRADO = 0
		...
		LEFT JOIN '||V_ESQUEMA||'.DD_TVP_TIPO_VPO VPO ON VPO.DD_TVP_ID = ADM.DD_TVP_ID AND VPO.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA||'.DD_RTG_RATING_ACTIVO DDRTG ON DDRTG.DD_RTG_ID = ACT.DD_RTG_ID AND DDRTG.BORRADO = 0
		...
		LEFT JOIN '||V_ESQUEMA||'.ACT_ADO_ADMISION_DOCUMENTO ADO ON (ADO.ACT_ID = ACT.ACT_ID AND ADO.DD_TCE_ID IS NOT NULL) AND ADO.BORRADO = 0
		LEFT JOIN '||V_ESQUEMA||'.DD_TCE_TIPO_CALIF_ENERGETICA DDTCE ON ADO.DD_TCE_ID = DDTCE.DD_TCE_ID AND DDTCE.BORRADO = 0
