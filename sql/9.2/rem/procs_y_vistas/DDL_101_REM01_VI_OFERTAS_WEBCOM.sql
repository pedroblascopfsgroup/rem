...
--##		0.3 REMVIP-8069 - Añadir nuevos campos
--##########################################
--*/
...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		...
		RCM_RC.DD_RCM_CODIGO																	AS COD_RECOMENDACION_RC,
		NVL2(RCM_RC.DD_RCM_CODIGO,
				CAST(TO_CHAR(OFR.OFR_FECHA_RECOMENDACION_RC,''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR)),
				NULL)																			AS FECHA_RECOMENDACION_RC,
		RCM_DC.DD_RCM_CODIGO																	AS COD_RECOMENDACION_DC,
		NVL2(RCM_DC.DD_RCM_CODIGO,
				CAST(TO_CHAR(OFR.OFR_FECHA_RECOMENDACION_DC,''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR)),
				NULL)																			AS FECHA_RECOMENDACION_DC,											
	    CASE WHEN (DD_TOF.DD_TOF_CODIGO = ''02'')
		      THEN CAST(ECO.ECO_NUM_EXPEDIENTE AS NUMBER(16,0))
		      ELSE NULL
	    END 																					AS ID_EXPEDIENTE_REM,
		CASE WHEN (ECO.ECO_FECHA_VENTA IS NOT NULL) 
		...
		LEFT JOIN '||V_ESQUEMA||'.DD_RCM_RECOMENDACION RCM_RC ON RCM_RC.DD_RCM_ID = OFR.OFR_RECOMENDACION_RC
		LEFT JOIN '||V_ESQUEMA||'.DD_RCM_RECOMENDACION RCM_DC ON RCM_DC.DD_RCM_ID = OFR.OFR_RECOMENDACION_DC
		...
		MRO.DD_MRO_DESCRIPCION AS MOTIVO_RECHAZO,
		OFR.OFR_IMPORTE_CONTRAOFERTA 															AS IMPORTE_CONTRAOFERTA, 
		NULL																					AS COD_RECOMENDACION_RC,
		NULL																					AS FECHA_RECOMENDACION_RC,
		NULL																					AS COD_RECOMENDACION_DC,		
		NULL																					AS FECHA_RECOMENDACION_DC,
	    CASE WHEN (DD_TOF.DD_TOF_CODIGO = ''02'')
		      THEN CAST(ECO.ECO_NUM_EXPEDIENTE AS NUMBER(16,0))
		      ELSE NULL
