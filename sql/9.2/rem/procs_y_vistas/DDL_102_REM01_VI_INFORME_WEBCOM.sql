...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		...
		ANEJO_GARAJE AS (
			SELECT AUX.* FROM (
				SELECT ANJ.ANJ_ID, ANJ.ICO_ID, ANJ.ANJ_DESCRIPCION, ANJ.ANJ_SUPERFICIE, ANJ.ANJ_CANTIDAD, DDSPG.DD_SPG_CODIGO,
				...
				WHERE DDTNJ.DD_TNJ_CODIGO = ''01'') AUX
      		WHERE AUX.ORDEN = 1
		),
		ANEJO_TRASTERO AS (
			SELECT AUX.* FROM (
				...
				LEFT JOIN '||V_ESQUEMA||'.DD_TNJ_TIPO_ANEJO DDTNJ ON DDTNJ.DD_TNJ_ID = ANJ.DD_TNJ_ID
				LEFT JOIN '||V_ESQUEMA||'.DD_SPG_SUBTIPO_PLAZA_GARAJE DDSPG ON DDSPG.DD_SPG_ID = ANJ.DD_SPG_ID 
				WHERE DDTNJ.DD_TNJ_CODIGO = ''02'') AUX
      		WHERE AUX.ORDEN = 1
		),
		ACCION AS (
			SELECT * FROM (
		...
		SELECT 
			...
			CASE WHEN (ANET.ANJ_ID IS NOT NULL) 
				THEN CAST(''1'' AS NUMBER(1,0))
				ELSE CAST(''0'' AS NUMBER(1,0))
			END EXISTE_ANEJO_TRASTERO,
			CAST(ANET.ANJ_DESCRIPCION AS VARCHAR2(150 CHAR)) AS ANEJO_TRASTERO,
			CAST(ANEG.ANJ_DESCRIPCION AS VARCHAR2(150 CHAR)) AS ANEJO_GARAGE,
			CAST(ANEG.ANJ_CANTIDAD AS NUMBER(16,0)) AS NUMERO_PLAZAS_GARAJE,
			CAST(ANEG.ANJ_SUPERFICIE AS NUMBER(16,2)) AS SUPERFICIE_PLAZAS_GARAJE,
			CAST(ANEG.DD_SPG_CODIGO AS VARCHAR2(5 CHAR)) AS COD_SUBTIPO_PLAZAS_GARAJE,
			...
			LEFT JOIN ANEJO_GARAJE ANEG ON ANEG.ICO_ID = ICO.ICO_ID
      		LEFT JOIN ANEJO_TRASTERO ANET ON ANET.ICO_ID = ICO.ICO_ID
