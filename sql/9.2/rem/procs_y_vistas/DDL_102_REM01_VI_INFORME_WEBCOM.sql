   ...
   -- Creamos vista materializada
	...
	AS
		...
		SELECT 
			...
			CAST((SELECT MAX(PVE.PVE_COD_REM) FROM '||V_ESQUEMA||'.ACT_ICM_INF_COMER_HIST_MEDI ICM
				JOIN '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR PVE ON PVE.PVE_ID = ICM.ICO_MEDIADOR_ID	
				WHERE ICM.ACT_ID = ACT.ACT_ID AND ICM.ICM_FECHA_HASTA = (
					SELECT MAX(ICM2.ICM_FECHA_HASTA) FROM '||V_ESQUEMA||'.ACT_ICM_INF_COMER_HIST_MEDI ICM2 
				...
				)) AS NUMBER(16,0))  AS ID_PROVEEDOR_REM_ANTERIOR,
