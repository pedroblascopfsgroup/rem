	...
	AS
		WITH CARTERA_AGRUPA AS (
			SELECT AUX.* FROM (
				...
				LEFT JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA DDCRA ON DDCRA.DD_CRA_ID = ACT.DD_CRA_ID) AUX
			WHERE AUX.ORDEN = 1
		)
		SELECT 
		...
		CASE WHEN (AGR.AGR_FIN_VIGENCIA IS NOT NULL) 
		  ...
		  ELSE NULL
		END 																				   AS HASTA,	
		CASE WHEN (AGR.BORRADO = 1 OR (AGR.AGR_FECHA_BAJA IS NOT NULL OR AGR.AGR_FECHA_BAJA < TO_DATE(SYSDATE, ''DD/MM/YY''))) 
