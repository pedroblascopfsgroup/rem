   ...
   -- Creamos vista materializada
	...
	AS
		WITH CARTERA_AGRUPA AS (
			SELECT AUX.* FROM (
				SELECT AGA.AGR_ID, AGA.ACT_ID, DDCRA.DD_CRA_CODIGO, DDSCR.DD_SCR_CODIGO,
				...
				LEFT JOIN '||V_ESQUEMA||'.DD_SCR_SUBCARTERA DDSCR ON DDSCR.DD_SCR_ID = ACT.DD_SCR_ID) AUX
			WHERE AUX.ORDEN = 1
		),
		DIRECCION_AGRUPA AS (
		...
		CAST(CRT.DD_SCR_CODIGO AS VARCHAR2(5 CHAR))                     						AS COD_SUB_CARTERA,
		...
		WHERE AGR.AGR_NUM_AGRUP_REM IS NOT NULL AND DDTAG.DD_TAG_CODIGO IS NOT NULL AND (DDTAG.DD_TAG_CODIGO = ''01'' OR  DDTAG.DD_TAG_CODIGO = ''13'')';
