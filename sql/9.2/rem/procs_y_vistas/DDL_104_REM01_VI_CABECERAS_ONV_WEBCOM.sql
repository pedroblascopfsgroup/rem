	...
	AS
		...
		FROM(
			...
			FROM (
					...
					NVL (SUM (DECODE (DIS.DD_TPH_ID, 1, DIS.DIS_CANTIDAD, NULL)) DORMITORIOS,
					NVL (SUM (DECODE (DIS.DD_TPH_ID, 2, DIS.DIS_CANTIDAD, NULL)) BANYOS,
		    ...
		    WHERE (DDTAG.DD_TAG_CODIGO = ''01'' OR DDTAG.DD_TAG_CODIGO = ''13'') AND (AGA.BORRADO IS NULL OR AGA.BORRADO = 0) AND AGR.BORRADO = 0
		) ACT_SD
