...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		...
		CAST(NULL AS NUMBER(16,0))                       	                    						AS BANYOS,
		...
		FROM(
			...
			FROM (
					SELECT 
					ORA_HASH(ACT.DD_TPA_ID||ACT.DD_SAC_ID||NVL(VIV.VIV_NUM_PLANTAS_INTERIOR,0)||NVL(PVD.DORMITORIOS,0)) ID,
					ACT.DD_TPA_ID, ACT.DD_SAC_ID, NVL(VIV.VIV_NUM_PLANTAS_INTERIOR,0) PLANTAS, 
					...
					GROUP BY ACT.DD_TPA_ID, ACT.DD_SAC_ID, VIV.VIV_NUM_PLANTAS_INTERIOR, PVD.DORMITORIOS, AGR.AGR_NUM_AGRUP_REM, 
		...
		ACT_SD.DESCRIPCION,ACT_SD.PLANTAS, ACT_SD.DORMITORIOS, 
