...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		...
		CAST(ACT.ACT_ID AS NUMBER(16,0)) 												        AS ID_ACTIVO_REM,
		...
		LEFT JOIN ACT_OFR AOF ON OFR.OFR_ID = AOF.OFR_ID
        JOIN ACT_ACTIVO ACT ON AOF.ACT_ID = ACT.ACT_ID
 		...
 		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_VISTA||'_IDX ON '||V_ESQUEMA|| '.'||V_TEXT_VISTA||'(ID_COMISION_REM, ID_ACTIVO_REM) TABLESPACE '||V_TABLESPACE_IDX;		
		...
		-- Creamos indice	
		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_IDX ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(ID_COMISION_REM, ID_ACTIVO_REM) TABLESPACE '||V_TABLESPACE_IDX;		
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_IDX... Indice creado.');	
	
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (ID_COMISION_REM, ID_ACTIVO_REM) USING INDEX)';
		EXECUTE IMMEDIATE V_MSQL;
