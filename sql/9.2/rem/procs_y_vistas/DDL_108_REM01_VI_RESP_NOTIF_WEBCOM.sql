   ...
   -- Creamos vista materializada
	...
	AS
		WITH TAREA_WEBCOM AS (
			...
			ROW_NUMBER() OVER (PARTITION BY M2.IRG_ID ORDER BY M2.IRG_ID DESC) ORDEN
		...
		DESC_TAREA AS (
			...
			ROW_NUMBER() OVER (PARTITION BY M3.IRG_ID ORDER BY M3.IRG_ID DESC) ORDEN
		...
		RESP_TAREA AS (
			...
			ROW_NUMBER() OVER (PARTITION BY M4.IRG_ID ORDER BY M4.IRG_ID DESC) ORDEN
		...
		FECHA_RESP_TAREA AS (
			...
			ROW_NUMBER() OVER (PARTITION BY M5.IRG_ID ORDER BY M5.IRG_ID DESC) ORDEN
