...
BEGIN
   ...
   -- Creamos vista materializada
	...
	AS
		SELECT 
		CAST(  NVL(LPAD(PVE.PVE_COD_REM,6,0) , ''000000'')   
			|| NVL(LPAD(USU.USU_ID,6,0) , ''000000'')
			|| NVL(LPAD(PVC.BORRADO,1,0) , ''0'') AS NUMBER(32,0))               AS ID_USUARIO_REM,
		...
		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_VISTA||'_IDX ON '||V_ESQUEMA|| '.'||V_TEXT_VISTA||'(ID_USUARIO_REM) TABLESPACE '||V_TABLESPACE_IDX;		
