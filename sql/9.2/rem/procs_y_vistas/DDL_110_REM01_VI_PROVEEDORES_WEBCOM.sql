   ...
   -- Creamos vista materializada
	...
	WITH DOMICILIO_SOCIAL AS (
		SELECT AUX.* FROM (
	        ...
	        PRD.FECHACREAR, PRD.USUARIOMODIFICAR, ROW_NUMBER() OVER (PARTITION BY PRD.PVE_ID, DDTDP.DD_TDP_ID ORDER BY PRD.PRD_ID DESC) ORDEN
	        ...
	        LEFT JOIN '||V_ESQUEMA_M||'.DD_UPO_UNID_POBLACIONAL DDUPO ON DDUPO.DD_UPO_ID = PRD.DD_UPO_ID) AUX
	      WHERE AUX.ORDEN = 1    
		),
	    DELEGACION AS (
		...
		CAST(  NVL(LPAD(PVE.PVE_COD_REM,10,0) , ''00000'')   
			|| NVL(LPAD(DEL.PRD_ID,5,0) , ''00000'')AS NUMBER(32,0))                           	AS ID_PROV_DELEGACION,
		...
		CAST(SOC.PRD_EMAIL AS VARCHAR2(40 CHAR)) 												AS EMAIL,
	    ...
	    CAST(DEL.PRD_EMAIL AS VARCHAR2(40 CHAR))                                        		AS DELEGACIONES_EMAIL,
		...
		WHERE PVE.PVE_COD_API_PROVEEDOR IS NOT NULL AND DDTPR.DD_TPR_CODIGO IS NOT NULL AND DDTPR.DD_TPR_CODIGO IN (''04'', ''18'', ''23'', ''28'', ''29'', ''30'', ''31'')';
