...
--## Finalidad: DDL
--##           
--## INSTRUCCIONES: Vista para consultar las notificaciones de webcom
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_TEXT_VISTA VARCHAR2(2400 CHAR) := 'VI_NOTIFICACIONES_WEBCOM'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.'|| V_TEXT_VISTA ||'
	AS
		select IRG_VALOR as IDWEBCOM FROM MEJ_IRG_INFO_REGISTRO WHERE IRG_CLAVE = ''ID_TAREA_APP_EXTERNA'' ';
