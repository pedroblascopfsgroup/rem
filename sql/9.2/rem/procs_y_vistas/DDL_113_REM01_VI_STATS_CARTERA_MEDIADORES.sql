...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_STATS_CARTERA_MEDIADORES 
	...
	AS	
		SELECT PVE.PVE_ID AS ID_MEDIADOR,
		  ...
		  (
		    SELECT COUNT(AOF1.OFR_ID) AS NUM_OFERTAS
		    FROM '|| V_ESQUEMA ||'.ACT_ICO_INFO_COMERCIAL ICO1
		    INNER JOIN '|| V_ESQUEMA ||'.ACT_OFR AOF1 ON ICO1.ACT_ID             = AOF1.ACT_ID
		    WHERE ICO1.ICO_MEDIADOR_ID = PVE.PVE_ID
		    -- OFERTAS
		  ) AS NUM_OFERTAS,
		  (
