...
--## VERSIONES:
--##        0.1 Versión inicial
--##		0.2 Valores de puntuación para bankia
--##########################################
--*/
...
AUTHID CURRENT_USER IS
    ...
    -- Recupera el codigo y el valor de los criterios de puntuación para bankia
   	CURSOR crs_criterios_puntuacion_bankia
   	IS
      	SELECT cpu.cpu_codigo, cpu.cpu_valor_bankia
        	FROM #ESQUEMA#.cpu_criterio_puntuacion_act cpu;
	...
	TYPE criterio_valor_bankia_map IS TABLE OF NUMBER INDEX BY VARCHAR2(30);
	criterio_valor_bankia criterio_valor_bankia_map;
...
BEGIN
   ...
   -- Guardamos en una colección el criterio y su valor de las puntuaciones
    ...
    FOR row IN crs_criterios_puntuacion_bankia
      LOOP
         criterio_valor_bankia(row.cpu_codigo) := row.cpu_valor_bankia;
      END LOOP;
	...
	WHILE (crs_activos%FOUND) LOOP
		...
		V_MSQL := 'SELECT COUNT(1) FROM REM01.ACT_ACTIVO ACT
					JOIN REM01.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
					WHERE CRA.DD_CRA_CODIGO = ''03''
					AND ACT.ACT_ID = '||v_act_id;
		EXECUTE IMMEDIATE V_MSQL INTO v_activo_bankia;
		
		IF v_activo_bankia = 0 THEN
		...
		ELSE
			DBMS_OUTPUT.PUT_LINE('	[INFO] Activo bankia');
			...
			FOR row IN crs_datos_rating_activo(v_act_id)
		      LOOP
		         ...
		         v_total_entorno := v_total_entorno + criterio_valor_bankia(row.DD_TPR_CODIGO);
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('04')*row.INF_HOTELES;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('05')*row.INF_TEATROS;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('06')*row.INF_SALAS_CINE;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('07')*row.INF_INST_DEPORT;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('08')*row.INF_CENTROS_COMERC;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('09')*row.INF_CENTROS_EDU;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('10')*row.INF_CENTROS_SANIT;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('11')*row.INF_PARKING_SUP_SUF;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('12')*row.INF_FACIL_ACCESO;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('13')*row.INF_LINEAS_BUS;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('14')*row.INF_METRO;
		         v_total_entorno := v_total_entorno + criterio_valor_bankia('15')*row.INF_EST_TREN;
	
		         IF criterio_valor_bankia('ENT') IS NOT NULL AND v_total_entorno > criterio_valor_bankia('ENT') THEN
		         	v_total_entorno := criterio_valor_bankia('ENT');
		         END IF;
		         ...
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('16')*row.EDI_PREVISIBLE_REHAB;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('17')*row.EDI_BUEN_ESTADO;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('18')*row.EDI_BAJO;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('19')*row.EDI_ASCENSOR;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('20')*row.EDI_VIVIENDA_UNIFAMILIAR;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('21')*row.ZCO_JARDINES;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('22')*row.ZCO_PISCINA;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('23')*row.ZCO_INST_DEP;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('24')*row.ZCO_ZONA_INFANTIL;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('25')*row.ZCO_CONSERJE_VIGILANCIA;
		         v_total_edificio := v_total_edificio + criterio_valor_bankia('26')*row.ZCO_GIMNASIO;
	
		         IF criterio_valor_bankia('EDI') IS NOT NULL AND v_total_edificio > criterio_valor_bankia('EDI') THEN
		         	v_total_edificio := criterio_valor_bankia('EDI');
		         END IF;
		         ...
		         v_total_interior := v_total_interior + criterio_valor_bankia('27')*row.CRI_PTA_ENT_BLINDADA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('28')*row.CRI_PTA_ENT_ACORAZADA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('29')*row.CRI_PTA_PASO_MACIZAS;
		         v_total_interior := v_total_interior + criterio_valor_bankia('30')*row.CRI_PTA_PASO_HUECAS;
		         v_total_interior := v_total_interior + criterio_valor_bankia('31')*row.CRI_ARMARIOS_ACABADO_ALTO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('32')*row.CRI_ARMARIOS_ACABADO_BAJO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('33')*row.CRE_VTNAS_ALU_ANODIZADO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('34')*row.CRE_VTNAS_ALU_LACADO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('35')*row.CRE_PERS_ALU;
		         v_total_interior := v_total_interior + criterio_valor_bankia('36')*row.CRE_VTNAS_CORREDERAS;
		         v_total_interior := v_total_interior + criterio_valor_bankia('37')*row.CRE_VTNAS_ABATIBLES;
		         v_total_interior := v_total_interior + criterio_valor_bankia('38')*row.CRE_VTNAS_OSCILOBAT;
		         v_total_interior := v_total_interior + criterio_valor_bankia('39')*row.CRE_DOBLE_CRISTAL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('40')*row.PRV_HUMEDAD_PARED;
		         v_total_interior := v_total_interior + criterio_valor_bankia('41')*row.PRV_HUMEDAD_TECHO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('42')*row.PRV_GOTELE;
		         v_total_interior := v_total_interior + criterio_valor_bankia('43')*row.PRV_PLASTICA_LISA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('44')*row.PRV_PAPEL_PINTADO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('45')*row.PRV_PINTURA_LISA_TECHO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('46')*row.PRV_MOLDURA_ESCAYOLA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('47')*row.SOL_TARIMA_FLOTANTE;
		         v_total_interior := v_total_interior + criterio_valor_bankia('48')*row.SOL_PARQUE;
		         v_total_interior := v_total_interior + criterio_valor_bankia('49')*row.SOL_MARMOL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('50')*row.SOL_PLAQUETA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('51')*row.COC_AMUEBLADA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('52')*row.COC_ENCI_GRANITO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('53')*row.COC_ENCI_MARMOL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('54')*row.COC_ENCI_OTRO_MATERIAL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('55')*row.COC_VITRO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('56')*row.COC_LAVADORA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('57')*row.COC_FRIGORIFICO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('58')*row.COC_LAVAVAJILLAS;
		         v_total_interior := v_total_interior + criterio_valor_bankia('59')*row.COC_MICROONDAS;
		         v_total_interior := v_total_interior + criterio_valor_bankia('60')*row.COC_HORNO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('61')*row.COC_AZULEJOS_EST;
		         v_total_interior := v_total_interior + criterio_valor_bankia('62')*row.COC_SUELOS;
		         v_total_interior := v_total_interior + criterio_valor_bankia('63')*row.COC_GRIFOS_MONOMANDO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('64')*row.BNY_BANYERA_HIDROMASAJE;
		         v_total_interior := v_total_interior + criterio_valor_bankia('65')*row.BNY_COLUMNA_HIDROMASAJE;
		         v_total_interior := v_total_interior + criterio_valor_bankia('66')*row.BNY_ALICATADO_MARMOL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('67')*row.BNY_ALICATADO_GRANITO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('68')*row.BNY_ALICATADO_AZULEJO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('69')*row.BNY_GRANITO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('70')*row.BNY_MARMOL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('71')*row.BNY_OTRO_MATERIAL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('72')*row.BNY_SANITARIOS_EST;
		         v_total_interior := v_total_interior + criterio_valor_bankia('73')*row.BNY_SUELOS;
		         v_total_interior := v_total_interior + criterio_valor_bankia('74')*row.BNY_GRIFO_MONOMANDO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('75')*row.INS_ELECTR_BUEN_ESTADO;
		         v_total_interior := v_total_interior + criterio_valor_bankia('76')*row.INS_ELECTR_DEFECTUOSA_ANTIGUA;
		         v_total_interior := v_total_interior + criterio_valor_bankia('77')*row.INS_CALEF_CENTRAL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('78')*row.INS_CALEF_GAS_NATURAL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('79')*row.INS_CALEF_RADIADORES_ALU;
		         v_total_interior := v_total_interior + criterio_valor_bankia('80')*row.INS_AGUA_CALIENTE_CENTRAL;
		         v_total_interior := v_total_interior + criterio_valor_bankia('81')*row.INS_AIRE_PREINSTALACION;
		         v_total_interior := v_total_interior + criterio_valor_bankia('82')*row.INS_AIRE_INSTALACION;
	
		         IF criterio_valor_bankia('INT') IS NOT NULL AND v_total_interior > criterio_valor_bankia('INT') THEN
		         	v_total_interior := criterio_valor_bankia('INT');
		         END IF;
