...
BEGIN
  ...
  SELECT COUNT(*) INTO CUENTA FROM ALL_OBJECTS WHERE OBJECT_NAME = 'V_BUSQUEDA_ACT_TBJ_PRESUPUESTO' AND OWNER=V_ESQUEMA AND OBJECT_TYPE='MATERIALIZED VIEW';  
  IF CUENTA>0 THEN
    DBMS_OUTPUT.PUT_LINE('DROP MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO...');
    EXECUTE IMMEDIATE 'DROP MATERIALIZED VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO';  
    DBMS_OUTPUT.PUT_LINE('DROP MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO... borrada OK');
  END IF;

  SELECT COUNT(*) INTO CUENTA FROM ALL_OBJECTS WHERE OBJECT_NAME = 'V_BUSQUEDA_ACT_TBJ_PRESUPUESTO' AND OWNER=V_ESQUEMA AND OBJECT_TYPE='VIEW';  
  IF CUENTA>0 THEN
    DBMS_OUTPUT.PUT_LINE('DROP VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO...');
    EXECUTE IMMEDIATE 'DROP VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO';  
    DBMS_OUTPUT.PUT_LINE('DROP VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO... borrada OK');
  END IF;

  DBMS_OUTPUT.PUT_LINE('CREATE VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO...');
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ACT_TBJ_PRESUPUESTO 
  ...
  SELECT /*+ PARALELL*/
      ...
      FROM REM01.ACT_TBJ ACTTBJ
        ...
        LEFT JOIN REM01.ACT_PTO_PRESUPUESTO PTO ON PTO.ACT_ID =  ACT.ACT_ID
        LEFT JOIN REM01.ACT_EJE_EJERCICIO EJE ON EJE.EJE_ID = PTO.EJE_ID AND EJE.BORRADO = 0
        LEFT JOIN (
          SELECT PTO.ACT_ID ACT1, SUM(NVL(INC.INP_IMPORTE_INCREMENTO,0)) INCREMENTOS, PTO.PTO_IMPORTE_INICIAL INICIAL, EJE_ANYO ANYO
          FROM REM01.ACT_PTO_PRESUPUESTO PTO
          LEFT JOIN REM01.ACT_INP_INC_PRESUPUESTO INC ON PTO.PTO_ID = INC.PTO_ID AND INC.BORRADO = 0
          INNER JOIN REM01.ACT_EJE_EJERCICIO EJE ON EJE.EJE_ID = PTO.EJE_ID AND EJE.BORRADO = 0
          WHERE PTO.BORRADO = 0
        ...
        LEFT JOIN (
          ...
          FROM ACT_TBJ ACTTBJ
          INNER JOIN ACT_TBJ_TRABAJO TBJ ON ACTTBJ.TBJ_ID = TBJ.TBJ_ID AND TBJ.BORRADO = 0
        ...
        LEFT JOIN (
           SELECT AUX_ACTTBJ.ACT_ID, AUX_ACTTBJ.TBJ_ID, AUX_TBJ.TBJ_NUM_TRABAJO, ROW_NUMBER() OVER (PARTITION BY AUX_ACTTBJ.ACT_ID ORDER BY 			  AUX_ACTTBJ.TBJ_ID DESC) RN
           FROM REM01.ACT_TBJ AUX_ACTTBJ
           INNER JOIN REM01.ACT_TBJ_TRABAJO AUX_TBJ ON AUX_ACTTBJ.TBJ_ID = AUX_TBJ.TBJ_ID AND AUX_TBJ.BORRADO = 0
       ) ULTIMO_TRABAJO ON ULTIMO_TRABAJO.RN = 1 AND ACT.ACT_ID = ULTIMO_TRABAJO.ACT_ID 
