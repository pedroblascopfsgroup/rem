...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_PROVEEDORES 
	AS
		...
		INNER JOIN ' || V_ESQUEMA || '.ACT_PTE_PROVEED_TERRITORIAL PTE ON PTE.PVE_ID = PVE.PVE_ID AND PVE.BORRADO = 0
		INNER JOIN ' || V_ESQUEMA_M || '.DD_PRV_PROVINCIA PRV ON PTE.DD_PRV_ID = PRV.DD_PRV_ID AND PRV.BORRADO = 0
		WHERE PVE.BORRADO = 0';
