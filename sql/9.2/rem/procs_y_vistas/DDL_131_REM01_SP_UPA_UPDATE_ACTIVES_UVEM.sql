...
--[21.2] COREAE --> ACB_COREAE 
  ...
  END IF;
      ...
      --[25]TABLA ACT_PAC_PERIMETRO_ACTIVO
      --[25.1] ACT_PAC_PERIMETRO_ACTIVO -> ACTUALIZAR CHECKS QUE PASAN A FORMALIZADO
       
		V_SQL := ' 
			MERGE INTO '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC
			USING (
				SELECT DISTINCT ACT.ACT_ID, 
                   DECODE(T1.PAC_CHECK_COMERCIALIZAR, 0, SYSDATE, 1, T1.PAC_FECHA_COMERCIALIZAR) AS PAC_FECHA_COMERCIALIZAR,
                   1 AS PAC_CHECK_COMERCIALIZAR,
                   DECODE(T1.PAC_CHECK_FORMALIZAR, 0, SYSDATE, 1, T1.PAC_FECHA_FORMALIZAR) AS PAC_FECHA_FORMALIZAR,
                   1 AS PAC_CHECK_FORMALIZAR,
                   DECODE(T1.PAC_CHECK_PUBLICAR, 0, SYSDATE, 1, T1.PAC_FECHA_PUBLICAR) AS PAC_FECHA_PUBLICAR,
            ...
            DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO '||V_NUM_TABLAS||' Registros.');
            PL_OUTPUT := PL_OUTPUT||'[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO '||V_NUM_TABLAS||' Registros. '||CHR(10)||CHR(10);
            DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
