...
--[3.1]--BIE_ADJ_IMPORTE_ADJUDICACION
	...
	EXECUTE IMMEDIATE 'MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1
	USING (
		SELECT ADJ.BIE_ID, APR.FEC_REALIZADA_POSESION
		FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
		INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL
		ON APR_FIL.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
...
--[19.1] CASUTR --> SUP_TOTAL_REAL_UTIL (REG_SUPERFICIE_UTIL)
	...
	EXECUTE IMMEDIATE '
		...
		SELECT ACT.ACT_ID, REG.REG_SUPERFICIE_UTIL AS REG_UTIL, APR.SUP_TOTAL_REAL_UTIL AS APR_UTIL FROM APR_AUX_STOCK_UVEM_TO_REM APR
		    ...
		    WHERE REG.REG_SUPERFICIE_UTIL IS NULL
...
--------------------------------------------
  ...
  --[20.1] CASUTG --> BIE_DREG_SUPERFICIE_CONSTRUIDA (SUP_TOTAL_REGISTRAL_UTIL)

	EXECUTE IMMEDIATE '
		MERGE INTO '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES T1 USING
		(
		SELECT APR.SUP_TOTAL_REGISTRAL_UTIL, BIE.BIE_ID, ACT.ACT_ID, BIE.BIE_DREG_SUPERFICIE_CONSTRUIDA FROM APR_AUX_STOCK_UVEM_TO_REM APR
		    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
		    JOIN '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES BIE ON ACT.BIE_ID = BIE.BIE_ID
		    WHERE APR.SUP_TOTAL_REGISTRAL_UTIL IS NOT NULL
		    AND BIE.BIE_DREG_SUPERFICIE_CONSTRUIDA IS NULL
		) T2 ON (T1.BIE_ID = T2.BIE_ID)
		    WHEN MATCHED THEN UPDATE SET
		    T1.BIE_DREG_SUPERFICIE_CONSTRUIDA = T2.SUP_TOTAL_REGISTRAL_UTIL,
		    T1.USUARIOMODIFICAR = '''||V_USUARIO||''',
		    T1.FECHAMODIFICAR = SYSDATE
		'
	; 
  ...
  --[20.2] CASUTC --> BIE_DREG_SUPERFICIE (SUP_TOTAL_REAL_CONSTRUIDA)


	EXECUTE IMMEDIATE '
		MERGE INTO '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES T1 USING
		(
		SELECT APR.SUP_TOTAL_REAL_CONSTRUIDA, BIE.BIE_ID, ACT.ACT_ID, BIE.BIE_DREG_SUPERFICIE FROM APR_AUX_STOCK_UVEM_TO_REM APR
		    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
		    JOIN '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES BIE ON ACT.BIE_ID = BIE.BIE_ID
		    WHERE APR.SUP_TOTAL_REAL_CONSTRUIDA IS NOT NULL
		    AND BIE.BIE_DREG_SUPERFICIE IS NULL
		) T2 ON (T1.BIE_ID = T2.BIE_ID)
		    WHEN MATCHED THEN UPDATE SET
		    T1.BIE_DREG_SUPERFICIE = T2.SUP_TOTAL_REAL_CONSTRUIDA,
		    T1.USUARIOMODIFICAR = '''||V_USUARIO||''',
		    T1.FECHAMODIFICAR = SYSDATE
	    	'
	; 
