...
--[4.9] ACT_ACTIVO -> ACTUALIZAR ACT_ADMISION/COESEN
		V_SQL := '
		  MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT
			USING (
				SELECT DISTINCT
				  DECODE(COESEN,''02'', 0) AS COESEN, APR_ID, ACT_NUMERO_UVEM, REM
			...
			ACT.USUARIOMODIFICAR = '''||V_USUARIO||''',
			ACT.FECHAMODIFICAR = SYSDATE
			WHERE TEMP.COESEN = 0 AND ACT.ACT_EN_TRAMITE= 1
			'
			;
...
--[9.4]-- ACTUALIZAR ETI EN ACT_TIT_TITULO CON ACT_AHT_HIST_TRAM_TITULO
          ...
          IF V_NUM_TABLAS != 0 THEN
            ...
            DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_TIT_TITULO POR HISTÓRICO '||V_NUM_TABLAS||' Registros.');
            PL_OUTPUT := PL_OUTPUT||'[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_TIT_TITULO  POR HISTÓRICO '||V_NUM_TABLAS||' Registros. '||CHR(10)||CHR(10);
            DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
