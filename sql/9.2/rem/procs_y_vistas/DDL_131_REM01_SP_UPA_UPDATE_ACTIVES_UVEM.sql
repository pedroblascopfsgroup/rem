...
--[3.1]--BIE_ADJ_IMPORTE_ADJUDICACION
          ...
          WHEN MATCHED THEN UPDATE SET
            ...
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          WHERE T1.BIE_ADJ_F_SEN_LANZAMIENTO <> DECODE(T2.FESELA,''00000000'',NULL,TO_DATE(T2.FESELA,''YYYYMMDD'')) OR T1.BIE_ADJ_F_SEN_LANZAMIENTO IS NULL';
          V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
          ...
          WHEN MATCHED THEN UPDATE SET
            ...
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          WHERE T1.BIE_ADJ_F_REA_LANZAMIENTO <> DECODE(T2.FERELA,''00000000'',NULL,TO_DATE(T2.FERELA,''YYYYMMDD'')) OR T1.BIE_ADJ_F_REA_LANZAMIENTO IS NULL';
          V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
          ...
          (
           WITH TEMP AS (
            ...
            WHERE
              TEMP.REM = 1
               AND
             (ACT.SPS_FECHA_TOMA_POSESION <> TEMP.FEC_REALIZADA_POSESION OR ACT.SPS_FECHA_TOMA_POSESION IS NULL) 
               and link.borrado = 0
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
          WHEN MATCHED THEN UPDATE SET
