...
--[3.1]--BIE_ADJ_IMPORTE_ADJUDICACION
          ...
          EXECUTE IMMEDIATE 'MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1
            USING (SELECT BIE.BIE_ID, APR.INDICADOR_NECES_LANZAMIENTO
            FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            INNER JOIN '||V_ESQUEMA||'.APR_AUX_BIE_IDS_FILTRO APR_FIL
                    ON APR_FIL.NUMERO_ACTIVO = APR.ACT_NUMERO_UVEM
            JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE
                    ON BIE.BIE_ID = APR_FIL.BIE_ID_REM) T2
          ON (T1.BIE_ID = T2.BIE_ID)
          WHEN MATCHED THEN UPDATE SET
            T1.BIE_ADJ_LANZAMIENTO_NECES = T2.INDICADOR_NECES_LANZAMIENTO,
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          WHERE T1.BIE_ADJ_LANZAMIENTO_NECES IS NULL';
          ...
          ON (T1.BIE_ID = T2.BIE_ID)
          WHEN MATCHED THEN UPDATE SET
            T1.BIE_ADJ_F_SEN_POSESION = T2.FESEPO,
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          WHERE T1.BIE_ADJ_F_SEN_POSESION IS NULL';
          V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
          ...
          ON (T1.BIE_ID = T2.BIE_ID)
          WHEN MATCHED THEN UPDATE SET
            T1.BIE_ADJ_F_SEN_LANZAMIENTO = T2.FESELA,
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          WHERE T1.BIE_ADJ_F_SEN_LANZAMIENTO IS NULL';
          V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
          ...
          ON (T1.BIE_ID = T2.BIE_ID)
          WHEN MATCHED THEN UPDATE SET
            T1.BIE_ADJ_F_REA_LANZAMIENTO = T2.FERELA,
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          ...
          ON (T1.BIE_ID = T2.BIE_ID)
          WHEN MATCHED THEN UPDATE SET
            T1.BIE_ADJ_FSOLMORATORIA = T2.FEC_SOLICITUD_MORATORIA, T1.BIE_ADJ_F_RES_MORATORIA = T2.FEC_RESOL_MORATORIA,
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE';
          V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
...
--[5.2]--AJD_PROCURADOR
          ...
          EXECUTE IMMEDIATE 'MERGE INTO '||V_ESQUEMA||'.ACT_AJD_ADJJUDICIAL T1
            USING (SELECT ACT.ACT_ID, PLZ.DD_PLA_ID
            FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            INNER JOIN '||V_ESQUEMA||'.APR_AUX_BIE_IDS_FILTRO APR_FIL
              ON APR.ACT_NUMERO_UVEM = APR_FIL.NUMERO_ACTIVO
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT
              ON ACT.ACT_ID = APR_FIL.ACT_ID_REM
            INNER JOIN '||V_ESQUEMA||'.BIE_BIEN BIE
              ON BIE.BIE_ID = ACT.BIE_ID
            INNER JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION ADJ
              ON ADJ.BIE_ID = BIE.BIE_ID
            LEFT JOIN '||V_ESQUEMA||'.DD_PLA_PLAZAS PLZ
              ON PLZ.DD_PLA_CODIGO = APR.NOM_LOCALIDAD_JUZGADO
...
--[6.1]--ADN_FECHA_FIRMA_TITULO
          ...
          EXECUTE IMMEDIATE 'MERGE INTO '||V_ESQUEMA||'.ACT_ADN_ADJNOJUDICIAL T1
            USING (SELECT ACT.ACT_ID, APR.NOM_NOTARIO_ESCRITURA
            FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            INNER JOIN '||V_ESQUEMA||'.APR_AUX_BIE_IDS_FILTRO APR_FIL
              ON APR.ACT_NUMERO_UVEM = APR_FIL.NUMERO_ACTIVO
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT
          ...
          WHEN MATCHED THEN UPDATE SET
            ...
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          WHERE T1.ADN_TRAMITADOR_TITULO IS NULL';
          V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
          
          EXECUTE IMMEDIATE 'MERGE INTO '||V_ESQUEMA||'.ACT_ADN_ADJNOJUDICIAL T1
            USING (SELECT ACT.ACT_ID, APR.NUM_PROTOCOLO
            FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            INNER JOIN '||V_ESQUEMA||'.APR_AUX_BIE_IDS_FILTRO APR_FIL
              ON APR.ACT_NUMERO_UVEM = APR_FIL.NUMERO_ACTIVO
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT
          ...
          WHEN MATCHED THEN UPDATE SET
            ...
            T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE
          WHERE T1.ADN_NUM_REFERENCIA IS NULL';
          V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
