...
CREATE OR REPLACE VIEW #ESQUEMA#.V_GESTORES_ACTIVO
...
SELECT act.act_id, NULL dd_cra_codigo, NULL dd_scr_codigo, NULL dd_eac_codigo, NULL dd_tcr_codigo, NULL dd_prv_codigo, NULL dd_loc_codigo, NULL cod_postal, dist1.tipo_gestor, dist1.username username,
             ...
             FROM #ESQUEMA#.act_activo act 
				  JOIN #ESQUEMA#.act_ges_dist_gestores dist1 ON dist1.tipo_gestor IN ('GADM', 'GMARK', 'GPREC', 'GTOPDV', 'GTOPLUS', 'GESTLLA', 'GADMT', 'GFSV', 'GCAL', 'GESMIN', 'SUPMIN', 'SUPADM')
           where act.borrado = 0
           UNION ALL
...
SELECT  act.act_id , 
            ...
            JOIN #ESQUEMA#.DD_SCR_SUBCARTERA scr on act.dd_scr_id = scr.dd_scr_id AND cra.dd_cra_id = scr.dd_cra_id
            JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'GBACKOFFICE'
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
                ON (dist0.tipo_gestor = TGE.DD_TGE_CODIGO
                    AND dist0.cod_cartera IS NULL
                    AND dist0.cod_subcartera IS NULL)
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist1 
                ON (dist1.tipo_gestor = TGE.DD_TGE_CODIGO
                    AND dist1.cod_cartera = cra.dd_cra_codigo
                    AND dist1.cod_subcartera IS NULL)
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist2
...
SELECT  act.act_id, 
                ...
                FROM #ESQUEMA#.ACT_ACTIVO act
                LEFT JOIN #ESQUEMA#.DD_CRA_CARTERA cra on act.dd_cra_id = cra.dd_cra_id
                LEFT JOIN #ESQUEMA#.DD_SCR_SUBCARTERA scr on act.dd_scr_id = scr.dd_scr_id AND scr.dd_scr_id = act.dd_scr_id
                     JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'SBACKOFFICEINMLIBER'
                LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
                    ON (dist0.tipo_gestor = TGE.DD_TGE_CODIGO
...
/*Gestor capa de control*/
           SELECT act.act_id, 
                  ...
                  COALESCE(dist1.nombre_usuario,dist0.nombre_usuario) nombre
             FROM #ESQUEMA#.act_activo act 
				  JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
				  JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
                  JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR tge on TGE.DD_TGE_CODIGO = 'GCCBANKIA'
                  LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
...
/*Gestor Comercial BackOffice Inmobiliario*/
           SELECT act.act_id, 
             ...
             FROM #ESQUEMA#.act_activo act 
				  JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
				  JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
                  JOIN #ESQUEMA#.BIE_LOCALIZACION BL ON BL.BIE_ID = ACT.BIE_ID
                  LEFT JOIN #ESQUEMA#.dd_tcr_tipo_comercializar dd_tcr ON dd_tcr.dd_tcr_id = act.dd_tcr_id
                  JOIN #ESQUEMA_MASTER#.DD_PRV_PROVINCIA PRV ON PRV.DD_PRV_ID = BL.DD_PRV_ID 
                  LEFT JOIN #ESQUEMA_MASTER#.DD_LOC_LOCALIDAD LOC ON LOC.DD_LOC_ID = BL.DD_LOC_ID 
                  LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
                    ON (dist0.cod_cartera = dd_cra.dd_cra_codigo 
                        AND dist0.cod_subcartera IS NULL
...
/*Supervisor Comercial BackOffice Inmobiliario*/
           SELECT act.act_id, 
             ...
             FROM #ESQUEMA#.act_activo act 
				  ...
				  JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
                  JOIN #ESQUEMA#.BIE_LOCALIZACION BL ON BL.BIE_ID = ACT.BIE_ID
				  JOIN #ESQUEMA#.dd_tcr_tipo_comercializar dd_tcr ON dd_tcr.dd_tcr_id = act.dd_tcr_id
                  JOIN #ESQUEMA_MASTER#.DD_PRV_PROVINCIA PRV ON PRV.DD_PRV_ID = BL.DD_PRV_ID
                  LEFT JOIN #ESQUEMA_MASTER#.DD_LOC_LOCALIDAD LOC ON LOC.DD_LOC_ID = BL.DD_LOC_ID
                  LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0
                        ON (dist0.cod_cartera = dd_cra.dd_cra_codigo  
...
SELECT act.act_id,
  ...
  FROM #ESQUEMA#.act_activo act 
	   JOIN #ESQUEMA#.act_loc_localizacion aloc ON act.act_id = aloc.act_id
       JOIN #ESQUEMA#.bie_localizacion loc ON loc.bie_loc_id = aloc.bie_loc_id
       JOIN #ESQUEMA_MASTER#.dd_loc_localidad dd_loc ON loc.dd_loc_id = dd_loc.dd_loc_id
       JOIN #ESQUEMA_MASTER#.dd_prv_provincia dd_prov ON dd_prov.dd_prv_id = loc.dd_prv_id
       LEFT JOIN #ESQUEMA#.dd_eac_estado_activo dd_eac ON dd_eac.dd_eac_id = act.dd_eac_id
       JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
       JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0
           ...
           left JOIN #ESQUEMA#.act_ges_dist_gestores dist3
           ...
           left JOIN #ESQUEMA#.act_ges_dist_gestores dist4
           ...
           left JOIN #ESQUEMA#.act_ges_dist_gestores dist5
           ...
           left JOIN #ESQUEMA#.act_ges_dist_gestores dist6
           ON (dist6.cod_estado_activo = dd_eac.dd_eac_codigo 
              ...
              left JOIN #ESQUEMA#.act_ges_dist_gestores dist7
           ON (dist7.cod_estado_activo = dd_eac.dd_eac_codigo 
            ...
            left JOIN #ESQUEMA#.act_ges_dist_gestores dist8
           ...
           left JOIN #ESQUEMA#.act_ges_dist_gestores dist9
...
SELECT act.act_id, 
       ...
       COALESCE (dist7.username, dist6.username, dist5.username, dist4.username, dist3.username, dist2.username, dist1.username, dist0.username ) username,
       COALESCE (dist7.nombre_usuario, dist6.nombre_usuario, dist5.nombre_usuario, dist4.nombre_usuario, dist3.nombre_usuario, dist2.nombre_usuario, dist1.nombre_usuario, dist0.nombre_usuario ) nombre
  FROM #ESQUEMA#.act_activo act JOIN act_loc_localizacion aloc ON act.act_id = aloc.act_id
       JOIN #ESQUEMA#.bie_localizacion loc ON loc.bie_loc_id = aloc.bie_loc_id
       JOIN #ESQUEMA_MASTER#.dd_loc_localidad dd_loc ON loc.dd_loc_id = dd_loc.dd_loc_id
       JOIN #ESQUEMA_MASTER#.dd_prv_provincia dd_prov ON dd_prov.dd_prv_id = loc.dd_prv_id
       JOIN #ESQUEMA#.dd_eac_estado_activo dd_eac ON dd_eac.dd_eac_id = act.dd_eac_id
       JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
       JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
       LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
...
/*Gestoría de administración*/
           SELECT DISTINCT act.act_id, 
            ...
            JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
            LEFT JOIN #ESQUEMA#.dd_eac_estado_activo dd_eac ON dd_eac.dd_eac_id = act.dd_eac_id
            LEFT JOIN #ESQUEMA_MASTER#.dd_loc_localidad dd_loc ON loc.dd_loc_id = dd_loc.dd_loc_id
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
                ON (dist0.cod_estado_activo = dd_eac.dd_eac_codigo 
                    AND dist0.cod_cartera = dd_cra.dd_cra_codigo 
                    AND dist0.cod_subcartera IS NULL
...
SELECT act.act_id, 
  ...
  FROM #ESQUEMA#.act_activo act 
       ...
       ON (dist7.cod_tipo_comerzialzacion = dd_tcr.dd_tcr_codigo 
          ...
          where act.borrado = 0
    and act.dd_cra_id in (select dd_cra_id from #ESQUEMA#.dd_cra_cartera where dd_cra_codigo = '01')
    and act.dd_scm_id in (select dd_scm_id from #ESQUEMA#.dd_scm_situacion_comercial  where dd_scm_codigo <> '05')
    UNION ALL
...
SELECT act.act_id, 
  ...
  FROM #ESQUEMA#.act_activo act 
       ...
       JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
       JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
       JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'PTEC'
       left JOIN #ESQUEMA#.act_ges_dist_gestores dist0
       ON (dist0.cod_cartera = dd_cra.dd_cra_codigo
		   AND dist0.cod_subcartera IS NULL
           AND dist0.cod_provincia = dd_prov.dd_prv_codigo 
...
SELECT act.act_id, 
    ...
    JOIN #ESQUEMA#.act_loc_localizacion aloc ON act.act_id = aloc.act_id
    join #ESQUEMA#.BIE_LOCALIZACION loc on loc.bie_loc_id = aloc.bie_loc_id
    left join #ESQUEMA_MASTER#.DD_PRV_PROVINCIA prv on prv.dd_prv_id = loc.dd_prv_id
    left join #ESQUEMA#.DD_TCR_TIPO_COMERCIALIZAR tcr on tcr.dd_tcr_id = act.dd_tcr_id
    left join #ESQUEMA#.DD_EAC_ESTADO_ACTIVO eac on eac.dd_eac_id = act.dd_eac_id
    join #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'GTREE'
    JOIN #ESQUEMA#.dd_cra_cartera cra on cra.dd_cra_id = act.dd_cra_id
    JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
    LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
  ...
  /*Gestor Liberbank Residencial (Liberbank)*/
           SELECT act.act_id, 
             ...
             FROM #ESQUEMA#.act_activo act 
				  JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
				  JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
                  JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
                    ON (dist0.cod_cartera = dd_cra.dd_cra_codigo 
                        AND dist0.cod_subcartera IS NULL 
                        AND dist0.tipo_gestor = 'GLIBRES')
                  JOIN #ESQUEMA#.act_ges_dist_gestores dist1 
                    ON (dist1.cod_cartera = dd_cra.dd_cra_codigo 
                        AND dist1.cod_subcartera = dd_scr.dd_scr_codigo 
  ...
  UNION ALL
    /*Gestor de suelos*/
        ...
        FROM #ESQUEMA#.act_activo act 
            ...
            JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
            JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
            JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'GSUE'
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 ON (
            ...
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist1 ON (
            ...
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist2 ON (
            ...
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist3 ON (
            ...
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist4 ON (
            ...
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist5 ON (
            ...
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist6 ON (
            ...
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist7 ON (
        ...
        WHERE 
            act.borrado = 0 
            AND act.DD_TPA_ID = (SELECT DD_TPA_ID FROM #ESQUEMA#.DD_TPA_TIPO_ACTIVO WHERE DD_TPA_CODIGO = '01')
            AND act.DD_EAC_ID = (SELECT DD_EAC_ID FROM #ESQUEMA#.DD_EAC_ESTADO_ACTIVO WHERE DD_EAC_CODIGO = '01')
    UNION ALL
    /*Supervisor de suelos*/
        ...
        FROM #ESQUEMA#.act_activo act 
            ...
            JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
            JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
            JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'SUPSUE'
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 ON (
                dist0.cod_cartera = dd_cra.dd_cra_codigo
                AND dist0.cod_subcartera IS NULL
                AND dist0.cod_provincia IS NULL
    ...
    /*Gestor de edificación*/
        ...
        FROM #ESQUEMA#.act_activo act 
            ...
            JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
            JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
            JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'GEDI'
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 ON (
                dist0.cod_cartera = dd_cra.dd_cra_codigo
                AND dist0.cod_subcartera IS NULL
                AND dist0.cod_provincia IS NULL
        ...
        WHERE 
            act.borrado = 0 
            AND act.DD_TPA_ID != (SELECT DD_TPA_ID FROM #ESQUEMA#.DD_TPA_TIPO_ACTIVO WHERE DD_TPA_CODIGO = '01')
            AND act.DD_EAC_ID IN (SELECT DD_EAC_ID FROM #ESQUEMA#.DD_EAC_ESTADO_ACTIVO WHERE DD_EAC_CODIGO IN ('09', '02', '06', '11', '10', '05', '08', '07'))
    UNION ALL
    /*Supervisor de edificación*/
        ...
        FROM #ESQUEMA#.act_activo act 
            ...
            JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
            JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
            JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'SUPEDI'
            LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 ON (
                dist0.cod_cartera = dd_cra.dd_cra_codigo
                AND dist0.cod_subcartera IS NULL
                AND dist0.cod_provincia IS NULL
  ...
  SELECT act.act_id, NULL dd_cra_codigo, NULL dd_scr_codigo, NULL dd_eac_codigo, NULL dd_tcr_codigo, NULL dd_prv_codigo, NULL dd_loc_codigo, NULL cod_postal, dist1.tipo_gestor, dist1.username username,
                    dist1.nombre_usuario nombre
               FROM #ESQUEMA#.act_activo act 
					JOIN #ESQUEMA#.act_ges_dist_gestores dist1 ON dist1.tipo_gestor IN ('GALQ', 'SUALQ')
					JOIN #ESQUEMA#.ACT_PTA_PATRIMONIO_ACTIVO ACT_PTA ON ACT_PTA.ACT_ID = ACT.ACT_ID 
  ...
  /*Gestor comercial alquiler - Obligatorio cartera, no mira por más campos*/
    SELECT act.act_id, 
        ...
        COALESCE(dist1.nombre_usuario, dist0.nombre_usuario) nombre  
    FROM #ESQUEMA#.act_activo act 
              INNER JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
                    JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
                    JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'GESTCOMALQ'
                    LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
                        ON (dist0.tipo_gestor = TGE.DD_TGE_CODIGO
                            AND dist0.cod_cartera = dd_cra.dd_cra_codigo
                            AND dist0.cod_subcartera IS NULL)
  ...
  /*Supervisor comercial alquiler - Admite cartera nula*/
    ...
    FROM #ESQUEMA#.act_activo act 
              INNER JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
                    JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
                    JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'SUPCOMALQ'
                    LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0 
                        ON (dist0.tipo_gestor = TGE.DD_TGE_CODIGO
                            AND dist0.cod_cartera = dd_cra.dd_cra_codigo
                            AND dist0.cod_subcartera IS NULL)
...
/*Gestor de publicaciones*/
    ...
    JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
    JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
    JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'GPUBL'
    LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0
        ON (dist0.cod_cartera = dd_cra.dd_cra_codigo
            AND dist0.cod_subcartera IS NULL
            AND dist0.cod_provincia = dd_prov.dd_prv_codigo 
...
/*Supervisor de publicaciones*/
    ...
    JOIN #ESQUEMA#.dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
    JOIN #ESQUEMA#.dd_scr_subcartera dd_scr ON dd_scr.dd_cra_id = dd_cra.dd_cra_id AND dd_scr.dd_scr_id = act.dd_scr_id
    JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_CODIGO = 'SPUBL'
    LEFT JOIN #ESQUEMA#.act_ges_dist_gestores dist0
        ON (dist0.cod_cartera = dd_cra.dd_cra_codigo
            AND dist0.cod_subcartera IS NULL
            AND dist0.cod_provincia = dd_prov.dd_prv_codigo 
