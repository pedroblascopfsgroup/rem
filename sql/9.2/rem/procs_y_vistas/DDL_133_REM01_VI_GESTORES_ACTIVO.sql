...
/*Supervisor Comercial BackOffice Inmobiliario*/
           SELECT act.act_id, dist1.cod_cartera, NULL dd_eac_codigo, NULL dd_tcr_codigo, NULL dd_prv_codigo, NULL dd_loc_codigo, NULL cod_postal, dist1.tipo_gestor tipo_gestor,
             ...
             FROM act_activo act JOIN dd_cra_cartera dd_cra ON dd_cra.dd_cra_id = act.dd_cra_id
                  ...
                  JOIN '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA PRV ON PRV.DD_PRV_ID = BL.DD_PRV_ID
                  LEFT JOIN '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD LOC ON LOC.DD_LOC_ID = BL.DD_LOC_ID
                  LEFT JOIN '||V_ESQUEMA||'.act_ges_dist_gestores dist1 ON dist1.cod_cartera = dd_cra.dd_cra_codigo AND dist1.tipo_gestor = ''HAYASBOINM''
                  AND PRV.DD_PRV_CODIGO = dist1.COD_PROVINCIA AND LOC.DD_LOC_CODIGO = NVL(dist1.COD_MUNICIPIO,LOC.DD_LOC_CODIGO)
           where act.borrado = 0
           UNION ALL
/*Gestor del activo*/
SELECT act.act_id, TO_NUMBER (dd_cra.dd_cra_codigo) dd_cra_codigo, TO_NUMBER (COALESCE (dist4.cod_estado_activo, dist3.cod_estado_activo, dist2.cod_estado_activo, dist1.cod_estado_activo, dist0.cod_estado_activo)) dd_eac_codigo,
    NULL dd_tcr_codigo, dd_prov.dd_prv_codigo, 
       COALESCE (dist4.cod_municipio, dist3.cod_municipio,dist2.cod_municipio,dist1.cod_municipio,dist0.cod_municipio) cod_municipio, 
       COALESCE (dist4.cod_postal, dist3.cod_postal, dist2.cod_postal, dist1.cod_postal, dist0.cod_postal) cod_postal,
       COALESCE (dist4.tipo_gestor, dist3.tipo_gestor, dist2.tipo_gestor, dist1.tipo_gestor, dist0.tipo_gestor) AS tipo_gestor, 
       COALESCE (dist4.username, dist3.username, dist2.username, dist1.username, dist0.username) username,
       COALESCE (dist4.nombre_usuario, dist3.nombre_usuario, dist2.nombre_usuario, dist1.nombre_usuario, dist0.nombre_usuario) nombre
  FROM '||V_ESQUEMA||'.act_activo act JOIN '||V_ESQUEMA||'.act_loc_localizacion aloc ON act.act_id = aloc.act_id
    ...
    LEFT JOIN '||V_ESQUEMA||'.act_ges_dist_gestores dist0
       ...
       ON (dd_eac.dd_eac_codigo = dist4.cod_estado_activo
           AND dist4.cod_cartera = dd_cra.dd_cra_codigo
           AND dd_prov.dd_prv_codigo = dist4.cod_provincia
...
SELECT act.act_id, TO_NUMBER (dd_cra.dd_cra_codigo) dd_cra_codigo, null dd_eac_codigo, dd_tcr.dd_tcr_codigo, dd_prov.dd_prv_codigo, 
       COALESCE (dist3.cod_municipio,dist2.cod_municipio,dist1.cod_municipio) cod_municipio, 
       COALESCE (dist3.cod_postal, dist2.cod_postal, dist1.cod_postal) cod_postal,
       COALESCE (dist3.tipo_gestor, dist2.tipo_gestor, dist1.tipo_gestor) AS tipo_gestor, 
       COALESCE (dist3.username, dist2.username, dist1.username) username,
       COALESCE (dist3.nombre_usuario, dist2.nombre_usuario, dist1.nombre_usuario) nombre
  FROM act_activo act JOIN act_loc_localizacion aloc ON act.act_id = aloc.act_id
       JOIN bie_localizacion loc ON loc.bie_loc_id = aloc.bie_loc_id
       JOIN '||V_ESQUEMA_M||'.dd_loc_localidad dd_loc ON loc.dd_loc_id = dd_loc.dd_loc_id
       JOIN '||V_ESQUEMA_M||'.dd_prv_provincia dd_prov ON dd_prov.dd_prv_id = loc.dd_prv_id
 ...
 SELECT act.act_id, TO_NUMBER (dd_cra.dd_cra_codigo) dd_cra_codigo, null dd_eac_codigo, dd_tcr.dd_tcr_codigo, dd_prov.dd_prv_codigo,
  ...
  FROM act_activo act JOIN act_loc_localizacion aloc ON act.act_id = aloc.act_id
       ...
       ON (dd_tcr.dd_tcr_codigo = dist3.cod_tipo_comerzialzacion
          ...
          where act.borrado = 0
    and act.dd_cra_id in (select dd_cra_id from dd_cra_cartera where dd_cra_codigo = ''01'')
    and act.dd_scm_id in (select dd_scm_id from dd_scm_situacion_comercial  where dd_scm_codigo <> ''05'')
    UNION ALL
...
';
  ...
  EXECUTE IMMEDIATE V_MSQL||V_MSQL2||V_MSQL3;
