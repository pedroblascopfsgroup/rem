	...
	AS
		WITH OBRA_NUEVA AS (
		  SELECT AGA_ON.ACT_ID, AGR_ON.AGR_ID, AGR_ON.AGR_NUM_AGRUP_REM, AGR_ON.AGR_NOMBRE 
		                ...
		                JOIN '||V_ESQUEMA||'.ACT_AGA_AGRUPACION_ACTIVO AGA_ON 
		                  ON AGA_ON.AGR_ID = AGR_ON.AGR_ID AND AGA_ON.BORRADO = 0
		                WHERE AGR_ON.DD_TAG_ID = (SELECT DD_TAG_ID FROM '||V_ESQUEMA||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO=''01'' AND BORRADO = 0)
		                  AND AGR_ON.BORRADO = 0
		)
		SELECT
		  	ACT.ACT_ID,
			ACP.PRP_ID,
			...
			(SELECT SUM(DIS.DIS_CANTIDAD) 
			   FROM '||V_ESQUEMA||'.ACT_DIS_DISTRIBUCION DIS, '||V_ESQUEMA||'.DD_TPH_TIPO_HABITACULO TPH
			  WHERE  DIS.DD_TPH_ID = TPH.DD_TPH_ID 
			    AND ICO.ICO_ID = DIS.ICO_ID 
			    AND TPH.DD_TPH_CODIGO = ''01''
			    AND TPH.BORRADO = 0
			    AND DIS.BORRADO = 0) AS NUM_DORMITORIOS,
			...
			(SELECT SUM(DIS.DIS_SUPERFICIE) 
			   FROM '||V_ESQUEMA||'.ACT_DIS_DISTRIBUCION DIS, '||V_ESQUEMA||'.DD_TPH_TIPO_HABITACULO TPH
			  WHERE DIS.DD_TPH_ID = TPH.DD_TPH_ID 
				AND ICO.ICO_ID = DIS.ICO_ID 
				AND DIS.BORRADO = 0
				AND TPH.BORRADO = 0
				AND (TPH.DD_TPH_CODIGO = ''16'' OR TPH.DD_TPH_CODIGO = ''15'')) AS SUPERFICIE_TERRAZA,
		...
		FROM '||V_ESQUEMA||'.ACT_PRP ACP
			...
			LEFT JOIN '||V_ESQUEMA||'.ACT_EDI_EDIFICIO EDI ON EDI.ICO_ID = ICO.ICO_ID
      		LEFT JOIN (SELECT CAT.ACT_ID, LISTAGG(CAT.CAT_REF_CATASTRAL,'','') WITHIN GROUP (ORDER BY ACT_ID) AS CAT_REF_CATASTRAL 
      		...
      		LEFT JOIN '||V_ESQUEMA||'.ACT_REG_INFO_REGISTRAL REG_INFO ON REG_INFO.ACT_ID = ACT.ACT_ID AND REG_INFO.BORRADO = 0
      		...
      		LEFT JOIN (select act_id, BIE_FECHA_VALOR_TASACION, TAS_IMPORTE_TAS_FIN FROM (
							...
							from (select tas.act_id, bie_val.BIE_FECHA_VALOR_TASACION, TAS.TAS_IMPORTE_TAS_FIN 
