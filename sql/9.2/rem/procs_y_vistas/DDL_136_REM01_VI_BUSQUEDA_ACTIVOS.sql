  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ACTIVOS 
	AS
		SELECT
			...
			TCO.DD_TCO_CODIGO,
		    (SELECT ACT_TAS.TAS_IMPORTE_TAS_FIN FROM ' || V_ESQUEMA || '.ACT_TAS_TASACION ACT_TAS WHERE ACT_TAS.BORRADO=0 AND ACT_TAS.ACT_ID = ACT.ACT_ID ORDER BY ACT_TAS.TAS_FECHA_RECEPCION_TASACION DESC, ACT_TAS.FECHACREAR DESC FETCH FIRST 1 ROW ONLY) TAS_IMPORTE_TAS_FIN,
			EPU.DD_EPU_CODIGO
		...
		LEFT JOIN ' || V_ESQUEMA || '.DD_EPU_ESTADO_PUBLICACION EPU ON EPU.DD_EPU_ID = ACT.DD_EPU_ID AND EPU.BORRADO = 0
