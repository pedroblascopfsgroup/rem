...
--[1.1] ACT_ACTIVO -> ACTUALIZAR ACT_ADMISION/COESEN
		V_SQL := '
		  MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT 
			USING (
				  SELECT DISTINCT
				  DECODE(COESEN,''01'', 1, ''02'', 2) AS COESEN, APR_ID, ACT_NUMERO_UVEM, REM
...
--[1.2]--ACT_VPO
          ...
          (
           WITH TEMP AS (
            ...
            AND (ACT.ACT_VPO IS NULL
				AND TEMP.REGIMEN_PROTECCION IS NOT NULL)
            AND ACT.BORRADO = 0
            AND ACT.ACT_EN_TRAMITE = 1
          ) TMP
...
--[1.4]--ACT_LLAVES_FECHA_RECEP
          ...
          (
           WITH TEMP AS (
            ...
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.ACT_LLAVES_FECHA_RECEP IS NULL
				AND TEMP.FEC_RECEP_LLAVES IS NOT NULL)
            AND ACT.BORRADO = 0
            AND ACT.ACT_EN_TRAMITE = 1
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
...
--[1.6]--DD_ENA_ID
          ...
          (
           WITH TEMP AS (
            ...
            SELECT TEMP.*, ACT.DD_ENA_ID, ACT.ACT_ID, ACT.DD_TPA_ID
            FROM TEMP 
...
--[1.7] COTSIN -> DD_SAC_ID (SUBTIPO ACTIVO)
...
--[1.10] ACT_PAC_PERIMETRO_ACTIVO -> ACTUALIZAR CHECKS QUE PASAN A EN_TRAMITE
       
		V_SQL := ' 
			MERGE INTO '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC
				USING (
					SELECT DISTINCT ACT.ACT_ID, 
                    DECODE(T1.PAC_CHECK_COMERCIALIZAR, 1, SYSDATE, 0, T1.PAC_FECHA_COMERCIALIZAR) AS PAC_FECHA_COMERCIALIZAR,
                    0 AS PAC_CHECK_COMERCIALIZAR,
                    DECODE(T1.PAC_CHECK_FORMALIZAR, 1, SYSDATE, 0, T1.PAC_FECHA_FORMALIZAR) AS PAC_FECHA_FORMALIZAR,
                    0 AS PAC_CHECK_FORMALIZAR,
                    DECODE(T1.PAC_CHECK_PUBLICAR, 1, SYSDATE, 0, T1.PAC_FECHA_PUBLICAR) AS PAC_FECHA_PUBLICAR,
                    0 AS PAC_CHECK_PUBLICAR
					FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
					INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
                    INNER JOIN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO T1 ON T1.ACT_ID = ACT.ACT_ID
					WHERE ACT.ACT_EN_TRAMITE = 1
					AND ACT.BORRADO = 0
					AND CRA.DD_CRA_CODIGO = ''03''
				) TEMP
				ON (PAC.ACT_ID = TEMP.ACT_ID)
				WHEN MATCHED THEN UPDATE SET
				PAC.PAC_CHECK_COMERCIALIZAR= TEMP.PAC_CHECK_COMERCIALIZAR,
				PAC.PAC_FECHA_COMERCIALIZAR = TEMP.PAC_FECHA_COMERCIALIZAR,
				PAC.PAC_CHECK_FORMALIZAR= TEMP.PAC_CHECK_FORMALIZAR,
				PAC.PAC_FECHA_FORMALIZAR = TEMP.PAC_FECHA_FORMALIZAR,
				PAC.PAC_CHECK_PUBLICAR=TEMP.PAC_CHECK_PUBLICAR,
				PAC.PAC_FECHA_PUBLICAR = TEMP.PAC_FECHA_PUBLICAR,
				PAC.PAC_CHECK_GESTIONAR= 1,
				PAC.PAC_MOTIVO_GESTIONAR= NULL,
...
--[1.11] COTSIN -> DD_TPA_ID (TIPO ACTIVO)
         
         V_SQL := '
          MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING
          (  WITH TEMP AS (
                SELECT DISTINCT TPA.DD_TPA_ID, APR.ACT_NUMERO_UVEM, APR.REM
                FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
                JOIN '||V_ESQUEMA||'.DD_EQV_BANKIA_REM EQV ON EQV.DD_CODIGO_BANKIA = APR.COTSIN
                JOIN '||V_ESQUEMA||'.DD_SAC_SUBTIPO_ACTIVO SAC ON SAC.DD_SAC_CODIGO = EQV.DD_CODIGO_REM
...
--[1.12] IDCOEQ -> DD_ENO_ID (COD_SOC_PROPIETARIA_ANT)
         
         V_SQL := '
          MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING
          (  WITH TEMP AS (
                SELECT DISTINCT ENO.DD_ENO_ID, APR.ACT_NUMERO_UVEM, APR.REM
                FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
...
--[7.1]--SPS_FECHA_TITULO,SPS_FECHA_TOMA_POSESION,SPS_FECHA_VENC_TITULO
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.FEC_INICIO_CONTRATO,APR.FEC_REALIZADA_POSESION,APR.FEC_ALTA_FASE_DEFINITIVA, APR_ID, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
...
--[21.2] COREAE --> ACB_COREAE 
  ...
  END IF;
      ...
      --[22.2] NUPOAC --> BIE_LOC_NUMERO_DOMICILIO
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, APR.PORTAL_PUNTO_KM AS BIE_LOC_NUMERO_DOMICILIO
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
