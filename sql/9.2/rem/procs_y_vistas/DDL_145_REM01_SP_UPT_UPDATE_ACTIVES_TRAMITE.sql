...
SET DEFINE OFF;
--0.9
create or replace PROCEDURE SP_UPT_UPDATE_ACTIVES_TRAMITE (
      V_USUARIO VARCHAR2 DEFAULT 'SP_UPT_TRAMITE',
...
V_NOT_UPDATE VARCHAR2(2000 CHAR) := '';
  ...
  --[24]TABLA ACT_ICO_INFO_COMERCIAL
      ...
      --[24.3] OBRA NUEVA --> DD_EAC_ID
  --[25]TABLA ACT_CAT_CATASTRO
	  --[25.1] REFERENCIA_CATASTRAL --> CAT_REF_CATASTRAL
...
--[1.1] ACT_ACTIVO -> ACTUALIZAR ACT_ADMISION/COESEN
		V_SQL := '
		  MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT 
			USING (
                  ...
                  LEFT JOIN '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM AUX ON AUX.ACT_NUMERO_UVEM = ACT.ACT_NUM_ACTIVO_UVEM
				  WHERE REM = 1 
                  OR AUX.ACT_NUMERO_UVEM IS NULL
			) TEMP
			ON (ACT.ACT_ID = TEMP.ACT_ID)
...
--[1.2]--ACT_VPO
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              DECODE(APR.REGIMEN_PROTECCION,''01'',1,''02'',1,''03'',1,''04'',1,NULL) AS REGIMEN_PROTECCION, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
            ...
            AND (ACT.ACT_VPO IS NULL AND TEMP.REGIMEN_PROTECCION IS NOT NULL)
            AND ACT.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
...
--[1.3]--ACT_LLAVES_NECESARIAS
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              DECODE(APR.LLAVES_NECESARIAS,''N'',0,''S'',1,NULL) AS LLAVES_NECESARIAS, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
...
--[1.4]--ACT_LLAVES_FECHA_RECEP
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.FEC_RECEP_LLAVES, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            ...
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.ACT_LLAVES_FECHA_RECEP IS NULL AND TEMP.FEC_RECEP_LLAVES IS NOT NULL)
            AND ACT.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
...
--[1.5]--ACT_NUM_INMOVILIZADO_BNK
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.NUINMU, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
...
--[1.6]--DD_ENA_ID
          ...
          (
           WITH TEMP AS (
             SELECT DISTINCT 
			 ENA.DD_ENA_ID, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
			 FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
			 INNER JOIN '||V_ESQUEMA||'.DD_ENA_ENTRADA_ACTIVO_BNK ENA ON ENA.DD_ENA_CODIGO = APR.COD_ENTRADA_ACTIVO
			)
            ...
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.DD_ENA_ID <> TEMP.DD_ENA_ID OR ACT.DD_ENA_ID IS NULL OR TEMP.DD_ENA_ID IS NULL)
            AND ACT.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
          WHEN MATCHED THEN UPDATE SET
          ACT.DD_ENA_ID = TMP.DD_ENA_ID,
          ACT.USUARIOMODIFICAR = '''||V_USUARIO||''',
          ACT.FECHAMODIFICAR = SYSDATE
          '
          ;
...
--[1.7] COTSIN -> DD_TPA_ID Y DD_SAC_ID (TIPO Y SUBTIPO ACTIVO)
         ...
         V_SQL := '
          ...
          (  
          WITH TEMP AS (
                SELECT DISTINCT SAC.DD_SAC_ID, SAC.DD_TPA_ID, APR.ACT_NUMERO_UVEM, APR.REM, COESEN, FEC_BAJA_ACTIVO
                FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
...
--[1.8] COEXCA -> ACT_CON_CARGAS (EXISTEN_CARGAS_RECOVERY)
 	...
 	V_SQL := '
          MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO T1 USING(
    		SELECT DECODE(APR.EXISTEN_CARGAS_RECOVERY, 1, 0, 2, 0, 3, 1, null, 0) AS CARGAS_UVEM, ACT.ACT_CON_CARGAS, ACT.ACT_ID 
    		FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
...
--[1.9] DD_SCR_ID -> ACTUALIZAR SUBCARTERA
 	...
 	V_SQL := '
		  ...
		  USING(
			SELECT ACT.ACT_ID, SCR.DD_SCR_CODIGO AS SUBCARTERA_ACTIVO, EQV.DD_CODIGO_REM AS SUBCARTERA_NUEVA, CRA.DD_CRA_ID 
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
			INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
			INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
			...
			AND EQV.DD_NOMBRE_BANKIA = ''DD_SCR_CODIGO'' 
			AND EQV.DD_NOMBRE_REM = ''DD_SCR_SUBCARTERA'' 
			AND EQV.DD_CODIGO_REM <> SCR.DD_SCR_CODIGO
			AND SCR.DD_SCR_CODIGO <> ''05'' 
			AND EQV.DD_CODIGO_REM <> ''05'' 
			AND ACT.BORRADO = 0 
			AND EQV.BORRADO = 0 
			AND APR.REM = 1    		
			AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL))
...
--[1.10] COTSIN -> DD_TPA_ID (TIPO ACTIVO)
         
         V_SQL := '
          MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING
          (  
			WITH TEMP AS (
                SELECT DISTINCT TPA.DD_TPA_ID, APR.ACT_NUMERO_UVEM, APR.REM, APR.COESEN, APR.FEC_BAJA_ACTIVO 
                FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
                JOIN '||V_ESQUEMA||'.DD_EQV_BANKIA_REM EQV ON EQV.DD_CODIGO_BANKIA = APR.COTSIN
                JOIN '||V_ESQUEMA||'.DD_SAC_SUBTIPO_ACTIVO SAC ON SAC.DD_SAC_CODIGO = EQV.DD_CODIGO_REM
                JOIN '||V_ESQUEMA||'.DD_TPA_TIPO_ACTIVO TPA ON TPA.DD_TPA_ID=SAC.DD_TPA_ID
          ) 
            SELECT TEMP.*, ACT.DD_TPA_ID AS ACT_TPA_ID, ACT.ACT_ID
            FROM TEMP
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.DD_TPA_ID <> TEMP.DD_TPA_ID OR ACT.DD_TPA_ID IS NULL OR TEMP.DD_TPA_ID IS NULL)
            AND ACT.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
...
--[1.11] IDCOEQ -> DD_ENO_ID (COD_SOC_PROPIETARIA_ANT)
         
         V_SQL := '
          MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING
          (  WITH TEMP AS (
                SELECT DISTINCT ENO.DD_ENO_ID, APR.ACT_NUMERO_UVEM, APR.REM, APR.COESEN, APR.FEC_BAJA_ACTIVO
                FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
                JOIN '||V_ESQUEMA||'.DD_ENO_ENTIDAD_ORIGEN ENO ON lpad(ENO.DD_ENO_CODIGO,5,0) = APR.IDCOEQ
          ) 
            SELECT TEMP.*, ACT.DD_ENO_ID AS ACT_ENO_ID, ACT.ACT_ID
            FROM TEMP
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT  ON ACT.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
...
--[2.1]--BIE_VIVIENDA_HABITUAL
        ...
        (
          WITH TEMP AS (
			SELECT DISTINCT
			DECODE(APR.RESIDENCIA_HABITUAL,''0'',0,''1'',1,''2'',0,''3'',0,''S'',1,''N'',0,NULL) AS RESIDENCIA_HABITUAL, APR_ID, ACT_NUMERO_UVEM, REM, COESEN, FEC_BAJA_ACTIVO
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
          )
          ...
          WHERE TEMP.REM = 1
          AND CRA.DD_CRA_CODIGO = ''03''
          AND BIE.BIE_VIVIENDA_HABITUAL IS NULL
          AND TEMP.RESIDENCIA_HABITUAL IS NOT NULL
          AND ACT.BORRADO = 0
          AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
        ) TMP
...
--[2.2]--BIE_PORCT_IMP_COMPRA
        ...
        (
          WITH TEMP AS (
			SELECT DISTINCT
			APR.PORCENTAJE_IMPUESTO_COMPRA, APR_ID, ACT_NUMERO_UVEM, REM, COESEN, FEC_BAJA_ACTIVO
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
          ...
          WHERE TEMP.REM = 1
          AND CRA.DD_CRA_CODIGO = ''03''
          AND BIE.BIE_PORCT_IMP_COMPRA IS NULL
          AND TEMP.PORCENTAJE_IMPUESTO_COMPRA IS NOT NULL
          AND ACT.BORRADO = 0
          AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
        ) TMP
        ON (TMP.BIE_ID = BIE.BIE_ID)
...
--[3.1]--BIE_DREG_LIBRO, BIE_DREG_TOMO Y BIE_DREG_FOLIO
          ...
          (
            SELECT TEMP.NUM_LIBRO_ESCRITURA, TEMP.NUM_TOMO_ESCRITURA, TEMP.NUM_FOLIO_ESCRITURA, BIE.BIE_ID
...
--[4.1]--BIE_ADJ_IMPORTE_ADJUDICACION
          V_SQL := '
          MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE 
          USING
          (
            WITH TEMP AS (
              SELECT DISTINCT
              APR.IMPORTE_ADJUDICACION, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
              WHERE APR.IMPORTE_ADJUDICACION != 0
            )
            SELECT TEMP.*, BIE.BIE_ID, ROW_NUMBER() OVER(PARTITION BY BIE.BIE_ID ORDER BY BIE.BIE_ADJ_ID DESC) RN
            FROM TEMP
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT           ON ACT.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1
            USING (
				SELECT BIE.BIE_ID, DECODE(APR.INDICADOR_NECES_LANZAMIENTO,''S'',1,''N'', 0, NULL) BIE_ADJ_LANZAMIENTO_NECES, ROW_NUMBER() OVER(PARTITION BY BIE.BIE_ID ORDER BY BIE.BIE_ADJ_ID DESC) RN
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR_FIL.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID
				JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = APR_FIL.ACT_ID
				WHERE APR.REM = 1
				AND CRA.DD_CRA_CODIGO = ''03''
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1
            USING (
				SELECT BIE.BIE_ID, APR.FESEPO, ROW_NUMBER() OVER(PARTITION BY BIE.BIE_ID ORDER BY BIE.BIE_ADJ_ID DESC) RN
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR_FIL.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1
            USING (
				SELECT BIE.BIE_ID, APR.FESELA, ROW_NUMBER() OVER(PARTITION BY BIE.BIE_ID ORDER BY BIE.BIE_ADJ_ID DESC) RN
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR_FIL.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID
	    ...
	    ';
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1
            USING (
				SELECT BIE.BIE_ID, APR.FERELA, ROW_NUMBER() OVER(PARTITION BY BIE.BIE_ID ORDER BY BIE.BIE_ADJ_ID DESC) RN
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR_FIL.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1
            USING (
				SELECT BIE.BIE_ID, APR.FEC_SOLICITUD_MORATORIA, APR.FEC_RESOL_MORATORIA, ROW_NUMBER() OVER(PARTITION BY BIE.BIE_ID ORDER BY BIE.BIE_ADJ_ID DESC) RN
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR_FIL.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID 
				JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = APR_FIL.ACT_ID    
				WHERE CRA.DD_CRA_CODIGO = ''03''
	...
	USING (
	  WITH T1 AS (
	      ...
	      JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
	      WHERE APR.REM = 1
	      AND CRA.DD_CRA_CODIGO = ''03''
	      AND ACT.BORRADO = 0
	      AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL)) 
	...
	USING (
			...
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
			INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR_FIL.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
			JOIN '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION ADJ ON ADJ.BIE_ID = APR_FIL.BIE_ID
			JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID
			JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = APR_FIL.ACT_ID
			WHERE APR.REM = 1
			AND APR_FIL.BORRADO = 0
...
--[5.1]--AJD_NUM_AUTO
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.NUM_AUTOS_JUZGADO, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            ...
            INNER JOIN '||V_ESQUEMA||'.ACT_AJD_ADJJUDICIAL ACT ON ACT.ACT_ID = LINK.ACT_ID
...
--[5.2]--AJD_PROCURADOR
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.NOMBRE_PROCURADOR, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
         ...
         /* V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_AJD_ADJJUDICIAL T1
            ...
            FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR.ACT_NUMERO_UVEM = APR_FIL.ACT_NUM_ACTIVO_UVEM
            INNER JOIN '||V_ESQUEMA||'.BIE_BIEN BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
            ...
            LEFT JOIN '||V_ESQUEMA||'.DD_PLA_PLAZAS PLZ ON PLZ.DD_PLA_DESCRIPCION = APR.NOM_LOCALIDAD_JUZGADO
            WHERE APR.COD_ENTRADA_ACTIVO IN (''01'',''02'')
            AND CRA.DD_CRA_CODIGO = ''03''
            ) T2
...
--[5.3]--AJD_FECHA_ADJUDICACION
	...
	ON (AJD_OLD.ACT_ID = AJD_NEW.ACT_ID)
	    WHEN MATCHED THEN UPDATE SET
...
--[6.1]--ADN_FECHA_FIRMA_TITULO
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.FEC_RESOLUCION, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            SELECT TEMP.*, ACT.ADN_FECHA_FIRMA_TITULO, LINK.ACT_ID
            FROM TEMP
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO LINK ON LINK.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
            INNER JOIN '||V_ESQUEMA||'.ACT_ADN_ADJNOJUDICIAL ACT ON ACT.ACT_ID = LINK.ACT_ID
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = LINK.DD_CRA_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = LINK.ACT_ID
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.ADN_FECHA_FIRMA_TITULO IS NULL
				AND TEMP.FEC_RESOLUCION IS NOT NULL)
			AND LINK.BORRADO = 0
			AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ADN_ADJNOJUDICIAL T1
            USING (
				SELECT APR_FIL.ACT_ID, APR.NOM_NOTARIO_ESCRITURA
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR.ACT_NUMERO_UVEM = APR_FIL.ACT_NUM_ACTIVO_UVEM
				INNER JOIN '||V_ESQUEMA||'.BIE_BIEN BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
				INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID
				INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = APR_FIL.ACT_ID
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ADN_ADJNOJUDICIAL T1
            USING (
				SELECT APR_FIL.ACT_ID, APR.NUM_PROTOCOLO
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO APR_FIL ON APR.ACT_NUMERO_UVEM = APR_FIL.ACT_NUM_ACTIVO_UVEM
				INNER JOIN '||V_ESQUEMA||'.BIE_BIEN BIE ON BIE.BIE_ID = APR_FIL.BIE_ID
				INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = APR_FIL.DD_CRA_ID
          ...
          V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ADN_ADJNOJUDICIAL T1 
          USING (
			  SELECT DECODE(APR.FETIFI,''00000000'',NULL,TO_DATE(APR.FETIFI,''YYYYMMDD'')) AS FECHA, ADN.ADN_FECHA_FIRMA_TITULO, ACT.ACT_ID 
			  FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
			  JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
			  JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
			  JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
...
-----------------------------------
--[7.1]--SPS_FECHA_TITULO,SPS_FECHA_TOMA_POSESION,SPS_FECHA_VENC_TITULO
          V_SQL := '
          MERGE INTO '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA ACT USING
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.FEC_INICIO_CONTRATO,APR.FEC_REALIZADA_POSESION,APR.FEC_ALTA_FASE_DEFINITIVA, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            ...
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.SPS_FECHA_TITULO IS NULL
				AND TEMP.FEC_INICIO_CONTRATO IS NOT NULL)
            AND LINK.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
...
--[7.2]--SPS_OCUPADO
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              DECODE(APR.OCUPADO,''S'',1,''N'',0,NULL) AS OCUPADO, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            ...
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.SPS_OCUPADO IS NULL
				AND TEMP.OCUPADO IS NOT NULL)
            AND LINK.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
          ...
          (
           WITH TEMP AS (
              ...
              CASE
                ...
                END AS TPO, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            ...
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.DD_TPO_ID IS NULL
				AND TEMP.TPO IS NOT NULL)
            AND LINK.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
...
--[7.4]--DD_SIJ_ID
          ...
          (
           	...
           	JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT1.ACT_ID
           	WHERE APR.REM = 1
           	AND CRA.DD_CRA_CODIGO = ''03''
           	AND ACT1.BORRADO = 0
           	AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL))
...
--[8.1]--DD_TVP_ID
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              TVP.DD_TVP_ID AS REGIMEN_PROTECCION, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
              LEFT JOIN '||V_ESQUEMA||'.DD_EQV_BANKIA_REM EQV ON EQV.DD_NOMBRE_BANKIA = ''DD_REGIMEN_PROTECCION'' AND EQV.DD_CODIGO_BANKIA = APR.REGIMEN_PROTECCION
              LEFT JOIN '||V_ESQUEMA||'.DD_TVP_TIPO_VPO TVP ON TVP.DD_TVP_CODIGO = EQV.DD_CODIGO_REM
            )
            SELECT TEMP.*, ACT.DD_TVP_ID, LINK.ACT_ID
            FROM TEMP
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO LINK ON LINK.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
            INNER JOIN '||V_ESQUEMA||'.ACT_ADM_INF_ADMINISTRATIVA ACT ON ACT.ACT_ID = LINK.ACT_ID
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = LINK.DD_CRA_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = LINK.ACT_ID
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.DD_TVP_ID IS NULL
				AND TEMP.REGIMEN_PROTECCION IS NOT NULL)
            AND LINK.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ON (TMP.ACT_ID = ACT.ACT_ID)
...
--[9.1]--ACT_AHT_HIST_TRAM_TITULO
          V_SQL := '
                    MERGE INTO ACT_AHT_HIST_TRAM_TITULO AHT USING (
                      ...
                      WHEN AHT.DD_ESP_CODIGO = ''01'' AND (TEMP.SITUACION_TITULO IN (''06'')
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NOT NULL AND APR.FEC_PRESENTACION_REGISTRO IS NULL
                      AND COALESCE(APR.FEC_SEGUNDA_PRESEN_REG, APR.FEC_PRESENTACION_REGISTRO) + 60 < SYSDATE
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NULL AND APR.FEC_PRESENTACION_REGISTRO IS NOT NULL
                      ...
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NULL AND APR.FEC_PRESENTACION_REGISTRO IS NOT NULL
                      AND COALESCE(APR.FEC_SEGUNDA_PRESEN_REG, APR.FEC_PRESENTACION_REGISTRO) + 60 < SYSDATE
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NOT NULL AND APR.FEC_PRESENTACION_REGISTRO IS NOT NULL
                      THEN COALESCE(APR.FEC_ENVIO_AUTO_ADICCION, TO_DATE(''01/01/1900'',''DD/MM/YYYY''))
                      ...
                      WHEN TEMP.SITUACION_TITULO IN (''06'')
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NOT NULL AND APR.FEC_PRESENTACION_REGISTRO IS NULL
                      AND APR.FEC_SEGUNDA_PRESEN_REG + 60 < SYSDATE
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NULL AND APR.FEC_PRESENTACION_REGISTRO IS NOT NULL
                      AND APR.FEC_PRESENTACION_REGISTRO + 60 < SYSDATE
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NOT NULL AND APR.FEC_PRESENTACION_REGISTRO IS NOT NULL
                      THEN (
                      SELECT DD_ESP_ID
                      FROM   '||V_ESQUEMA||'.DD_ESP_ESTADO_PRESENTACION
                      ...
                      OR TEMP.SITUACION_TITULO IN (''01'') AND APR.FEC_SEGUNDA_PRESEN_REG IS NULL AND APR.FEC_PRESENTACION_REGISTRO IS NULL
                      AND COALESCE(APR.FEC_SEGUNDA_PRESEN_REG, APR.FEC_PRESENTACION_REGISTRO) + 60 > SYSDATE
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NOT NULL AND APR.FEC_PRESENTACION_REGISTRO IS NULL
                      AND COALESCE(APR.FEC_SEGUNDA_PRESEN_REG, APR.FEC_PRESENTACION_REGISTRO) + 60 > SYSDATE
                      OR TEMP.SITUACION_TITULO IS NULL AND APR.FEC_SEGUNDA_PRESEN_REG IS NULL AND APR.FEC_PRESENTACION_REGISTRO IS NOT NULL
                      ...
                      (
                              ...
                              (TIT1.TIT_FECHA_INSC_REG <> APR.FEC_INSCRIPCION_TITULO OR APR.FEC_INSCRIPCION_TITULO IS NOT NULL AND TIT1.TIT_FECHA_INSC_REG IS NULL) OR
                              (ETI.DD_ETI_ID IS NULL AND TEMP.SITUACION_TITULO IS NOT NULL)
          ...
          IF V_NUM_TABLAS != 0 THEN
            ...
            DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS INSERTADOS EN '||V_ESQUEMA||'.ACT_AHT_HIST_TRAM_TITULO '||V_NUM_TABLAS||' Registros.');
            PL_OUTPUT := PL_OUTPUT||'[INFO] REGISTROS INSERTADOS EN '||V_ESQUEMA||'.ACT_AHT_HIST_TRAM_TITULO '||V_NUM_TABLAS||' Registros. '||CHR(10)||CHR(10);
            DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
...
FOR I IN V_TIPO_TABLA10.FIRST .. V_TIPO_TABLA10.LAST
    LOOP
      ...
      V_TMP_TIPO_TABLA10 := V_TIPO_TABLA10(I);
        ...
        (
		    WITH DISTINTOS AS (
		      SELECT APR_ID, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, ROW_NUMBER () OVER (PARTITION BY ACT_NUMERO_UVEM ORDER BY APR_ID DESC) ORDEN
		      FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
		    )
...
--[9.3]--DD_ETI_ID
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              ETI.DD_ETI_ID AS SITUACION_TITULO, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
              LEFT JOIN '||V_ESQUEMA||'.DD_EQV_BANKIA_REM EQV ON EQV.DD_NOMBRE_BANKIA = ''DD_SITUACION_TITULO'' AND EQV.DD_CODIGO_BANKIA = APR.SITUACION_TITULO
              LEFT JOIN '||V_ESQUEMA||'.DD_ETI_ESTADO_TITULO ETI ON ETI.DD_ETI_CODIGO = EQV.DD_CODIGO_REM
            )
            SELECT TEMP.*, ACT.DD_ETI_ID, LINK.ACT_ID
            FROM TEMP
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO LINK ON LINK.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = LINK.DD_CRA_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_TIT_TITULO ACT ON ACT.ACT_ID = LINK.ACT_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = LINK.ACT_ID
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
...
--[9.4]-- ACTUALIZAR ETI EN ACT_TIT_TITULO CON ACT_AHT_HIST_TRAM_TITULO
          ...
          V_SQL := '
				      ...
				      (
				        ...
				        SELECT 
				        TIT.TIT_ID 
				        , CASE WHEN ESP.DD_ESP_CODIGO = ''01'' THEN (SELECT ETI.DD_ETI_ID FROM '||V_ESQUEMA||'.DD_ETI_ESTADO_TITULO ETI WHERE ETI.DD_ETI_CODIGO = ''01'')
				        WHEN ESP.DD_ESP_CODIGO = ''02'' THEN (SELECT ETI.DD_ETI_ID FROM '||V_ESQUEMA||'.DD_ETI_ESTADO_TITULO ETI WHERE ETI.DD_ETI_CODIGO = ''06'')
				        WHEN ESP.DD_ESP_CODIGO = ''03'' THEN (SELECT ETI.DD_ETI_ID FROM '||V_ESQUEMA||'.DD_ETI_ESTADO_TITULO ETI WHERE ETI.DD_ETI_CODIGO = ''02'')
				        END DD_ETI_ID
				        FROM '||V_ESQUEMA||'.ACT_TIT_TITULO TIT
				        JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON TIT.ACT_ID = ACT.ACT_ID
				        JOIN ULT_REG AHT ON TIT.TIT_ID = AHT.TIT_ID AND AHT.RN = 1
				        LEFT JOIN '||V_ESQUEMA||'.DD_ESP_ESTADO_PRESENTACION ESP ON AHT.DD_ESP_ID = ESP.DD_ESP_ID
				        LEFT JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON ACT.DD_CRA_ID = CRA.DD_CRA_ID AND CRA.BORRADO = 0
				        WHERE CRA.DD_CRA_CODIGO = ''03'' AND TIT.DD_ETI_ID <>
						CASE WHEN ESP.DD_ESP_CODIGO = ''01'' THEN (SELECT ETI.DD_ETI_ID FROM '||V_ESQUEMA||'.DD_ETI_ESTADO_TITULO ETI WHERE ETI.DD_ETI_CODIGO = ''01'')
						WHEN ESP.DD_ESP_CODIGO = ''02'' THEN (SELECT ETI.DD_ETI_ID FROM '||V_ESQUEMA||'.DD_ETI_ESTADO_TITULO ETI WHERE ETI.DD_ETI_CODIGO = ''06'')
						WHEN ESP.DD_ESP_CODIGO = ''03'' THEN (SELECT ETI.DD_ETI_ID FROM '||V_ESQUEMA||'.DD_ETI_ESTADO_TITULO ETI WHERE ETI.DD_ETI_CODIGO = ''02'')
						END OR TIT.DD_ETI_ID IS NULL
				      ) TMP
				      ON (TIT.TIT_ID = TMP.TIT_ID)
				      WHEN MATCHED THEN UPDATE SET
				      TIT.DD_ETI_ID = TMP.DD_ETI_ID,
				      TIT.USUARIOMODIFICAR = '''||V_USUARIO||''',
				      TIT.FECHAMODIFICAR = SYSDATE
          '
          ;
          ...
          IF V_NUM_TABLAS != 0 THEN
            ...
            DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_TIT_TITULO '||V_NUM_TABLAS||' Registros.');
            PL_OUTPUT := PL_OUTPUT||'[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_TIT_TITULO '||V_NUM_TABLAS||' Registros. '||CHR(10)||CHR(10);
            DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
...
--[10.1]--OLE_NOMBRE
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.NOMBRE_ARRENDATARIO, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            SELECT TEMP.*, ACT.OLE_NOMBRE, LINK.ACT_ID, SPS.SPS_ID, SYSDATE AS FECHACREAR
            FROM TEMP
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO LINK ON LINK.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
            INNER JOIN '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA SPS ON SPS.ACT_ID = LINK.ACT_ID
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = LINK.DD_CRA_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = LINK.ACT_ID
            LEFT JOIN '||V_ESQUEMA||'.ACT_OLE_OCUPANTE_LEGAL ACT ON ACT.SPS_ID = SPS.SPS_ID
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.OLE_NOMBRE IS NULL
				AND TEMP.NOMBRE_ARRENDATARIO IS NOT NULL)
            AND LINK.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ...
          ACT.FECHAMODIFICAR = SYSDATE
          WHEN NOT MATCHED THEN INSERT
          (OLE_ID, SPS_ID, OLE_NOMBRE, USUARIOCREAR, FECHACREAR)
          VALUES
          (
...
--[11.1]--ACT_ADO_ADMISION_DOCUMENTO
          ...
          (
            ...
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID 
            INNER JOIN '||V_ESQUEMA||'.BIE_BIEN BIE ON BIE.BIE_ID = ACT.BIE_ID
            INNER JOIN '||V_ESQUEMA||'.DD_TCE_TIPO_CALIF_ENERGETICA TCE ON TCE.DD_TCE_DESCRIPCION = APR.COD_CALIF_EFICI_ENERGETICA
            INNER JOIN '||V_ESQUEMA||'.ACT_CFD_CONFIG_DOCUMENTO CFD ON CFD.DD_TPA_ID = ACT.DD_TPA_ID AND CFD.DD_TPD_ID = (SELECT DD_TPD_ID FROM '||V_ESQUEMA||'.DD_TPD_TIPO_DOCUMENTO WHERE DD_TPD_CODIGO = ''11'')
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
          ...
          DD_TCE_ID = TMP.DD_TCE_ID,
          USUARIOMODIFICAR = '''||V_USUARIO||''',
          FECHAMODIFICAR = SYSDATE
          WHERE (ADO_FECHA_EMISION IS NULL OR ADO_FECHA_CADUCIDAD IS NULL)
          WHEN NOT MATCHED THEN INSERT
          (
			ADO_ID, ACT_ID, CFD_ID, ADO_APLICA, ADO_FECHA_EMISION, ADO_FECHA_CADUCIDAD, DD_TCE_ID, USUARIOCREAR, FECHACREAR
          )
          VALUES
          (
...
--[12.1]--MLV_FECHA_ENTREGA
          ...
          (
           WITH TEMP AS (
              SELECT DISTINCT
              APR.FEC_ENVIO_LLAVES_GESTOR, COESEN, FEC_BAJA_ACTIVO, ACT_NUMERO_UVEM, REM
              FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            )
            ...
            INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO LINK ON LINK.ACT_NUM_ACTIVO_UVEM = TEMP.ACT_NUMERO_UVEM
            INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = LINK.DD_CRA_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_LLV_LLAVE LLV ON LLV.ACT_ID = LINK.ACT_ID
            INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = LINK.ACT_ID
            LEFT JOIN '||V_ESQUEMA||'.ACT_MLV_MOVIMIENTO_LLAVE ACT ON ACT.LLV_ID = LLV.LLV_ID
            WHERE TEMP.REM = 1
            AND CRA.DD_CRA_CODIGO = ''03''
            AND (ACT.MLV_FECHA_ENTREGA IS NULL
				AND TEMP.FEC_ENVIO_LLAVES_GESTOR IS NOT NULL)
            AND LINK.BORRADO = 0
            AND (TEMP.COESEN = ''01'' OR (TEMP.COESEN = ''02'' AND ESA.COESEN = 0 AND TEMP.FEC_BAJA_ACTIVO IS NULL))
          ) TMP
          ...
          ACT.FECHAMODIFICAR = SYSDATE
          WHEN NOT MATCHED THEN INSERT
          (MLV_ID, LLV_ID, MLV_FECHA_ENTREGA, USUARIOCREAR, FECHACREAR)
          VALUES
...
--[13.2]--POPROP -> PORCENTAJE_PROPIEDAD
		...
		USING (
				SELECT ACT.ACT_ID, ROUND(APR.PORCENTAJE_PROPIEDAD,2) AS REDONDEO, APR.PORCENTAJE_PROPIEDAD, PAC.PAC_PORC_PROPIEDAD 
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
...
--[13.3]--COSPAT -> Código de Sociedad Patrimonial
		...
		USING (
			...
			SELECT ACT.ACT_ID, PRO.PRO_CODIGO_UVEM AS PROPIETARIO_REM, APR.COD_SOC_PATRIMONIAL AS PROPIETARIO_UVEM, CRA.DD_CRA_ID 
			FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
			...
			AND EXISTS (SELECT PRO2.PRO_CODIGO_UVEM FROM '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO PRO2 WHERE PRO2.PRO_CODIGO_UVEM = APR.COD_SOC_PATRIMONIAL)
			AND ACT.BORRADO = 0 
			AND EQV.BORRADO = 0 
			AND APR.REM = 1
			AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL))
...
--[14.1]--ASCENSORES
  ...
  WHEN MATCHED THEN UPDATE SET
  EDI.EDI_ASCENSOR = TMP.EDI_ASCENSOR,
  EDI.EDI_NUM_ASCENSORES = TMP.EDI_NUM_ASCENSORES
  WHEN NOT MATCHED THEN INSERT
  (
  EDI_ID,
  ICO_ID,
...
--[16.1]--DORMITORIOS
  ...
  ON (TMP.ICO_ID = DIS.ICO_ID AND TMP.DD_TPH_ID = DIS.DD_TPH_ID)
  WHEN MATCHED THEN UPDATE SET
  DIS.DIS_CANTIDAD = TMP.DIS_CANTIDAD,
  DIS.DIS_NUM_PLANTA = TMP.DIS_NUM_PLANTA
  WHEN NOT MATCHED THEN INSERT
  (
  DIS_ID,
  ICO_ID,
...
--[16.2]--BAÑOS
  ...
  (
    ...
    INNER JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
    WHERE APR.REM = 1
    AND APR.BANIOS_ACTIVO IS NOT NULL
    AND CRA.DD_CRA_CODIGO = ''03''
    AND ACT.BORRADO = 0
    AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL))
  ) TMP
...
--[16.5]--GARAJES
  ...
  MERGE INTO '||V_ESQUEMA||'.ACT_DIS_DISTRIBUCION DIS USING
    ...
    ON (TMP.ICO_ID = DIS.ICO_ID AND TMP.DD_TPH_ID = DIS.DD_TPH_ID)
    WHEN MATCHED THEN UPDATE SET
	DIS.DIS_CANTIDAD = COALESCE(DIS_CANTIDAD_2,DIS_CANTIDAD_1),
	DIS.DIS_NUM_PLANTA = TMP.DIS_NUM_PLANTA
    WHEN NOT MATCHED THEN INSERT
    (
    DIS_ID,
...
--[17.1]-- COTIRG --> TIPO RIESGO, (ABA_TIPO_PRODUCTO)
  V_SQL := '
  MERGE INTO '||V_ESQUEMA||'.ACT_ABA_ACTIVO_BANCARIO T1 
  USING (
    SELECT TO_CHAR(APR.TIPO_RIESGO) AS TIPO_RIESGO, ACT.ACT_ID, ABA.ABA_ID 
    FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
    JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
...
--[17.2]-- NUIDOP --> NUM_EXP_RIESGO, (ABA_NEXPRIESGO)
  ...
  MERGE INTO '||V_ESQUEMA||'.ACT_ABA_ACTIVO_BANCARIO T1 
  USING (
      SELECT APR.NUM_EXP_RIESGO, ACT.ACT_ID, ABA.ABA_ID 
	  FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
      JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
      JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
      JOIN '||V_ESQUEMA||'.ACT_ABA_ACTIVO_BANCARIO ABA ON ACT.ACT_ID = ABA.ACT_ID
...
---------------------------------
  ...
  V_SQL := '
		...
		USING (
		    WITH VAL_REF AS (
		        SELECT VAL.VAL_ID, VAL.ACT_ID, VAL.VAL_IMPORTE,VAL.VAL_FECHA_INICIO, ROW_NUMBER() OVER(PARTITION BY VAL.ACT_ID ORDER BY VAL.VAL_FECHA_INICIO DESC) RN
		    ...
		    , PRC_REF AS (
		        SELECT ACT_NUMERO_UVEM, PRECIO_REFERENCIA, COESEN, FEC_BAJA_ACTIVO
		        FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM
		        WHERE NVL(PRECIO_REFERENCIA,0) <> 0
		        )
		    SELECT VAL_REF.VAL_ID, ACT.ACT_ID, PRC_REF.PRECIO_REFERENCIA, VAL_REF.VAL_IMPORTE, VAL_REF.VAL_FECHA_INICIO
		    FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
		    JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
		    JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
		    ...
		    WHERE CRA.DD_CRA_CODIGO = ''03'' 
		    AND ACT.BORRADO = 0
		    AND (PRC_REF.COESEN = ''01'' OR (PRC_REF.COESEN = ''02'' AND ESA.COESEN = 0 AND PRC_REF.FEC_BAJA_ACTIVO IS NULL))
		    ) T2
		ON (T1.VAL_ID = T2.VAL_ID)
		WHEN MATCHED THEN UPDATE SET
		    T1.VAL_IMPORTE = T2.PRECIO_REFERENCIA, T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE,
			T1.VAL_FECHA_INICIO=T2.VAL_FECHA_INICIO
  ...
  --[18.2]-- IMVLIQ --> PRECIO_REFERENCIA, (VAL_IMPORTE, DD_TPC_CODIGO = '16')
   ...
   V_SQL := '
   	MERGE INTO '||V_ESQUEMA||'.ACT_VAL_VALORACIONES T1 
   	USING (
    	  SELECT APR.IMVLIQ/100 AS PRECIO, VAL.DD_TPC_ID, ACT.ACT_ID, VAL.VAL_ID
    	  FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR 
...
--[19.1] CASUTR --> SUP_TOTAL_REAL_UTIL (REG_SUPERFICIE_UTIL)
	...
	V_SQL := '
		...
		(
			SELECT ACT.ACT_ID, REG.REG_SUPERFICIE_UTIL AS REG_UTIL, APR.SUP_TOTAL_REAL_UTIL AS APR_UTIL 
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
		    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
		    JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
...
--[19.2] CASPRE --> Superficie real suelo parcela (REG_SUPERFICIE_PARCELA)


  V_SQL := '
    MERGE INTO '||V_ESQUEMA||'.ACT_REG_INFO_REGISTRAL T1 
    USING (
		SELECT ACT.ACT_ID, REG.REG_SUPERFICIE_PARCELA AS REG_PARCELA, APR.CASPRE AS APR_PARCELA 
		FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
        JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
        JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
        JOIN '||V_ESQUEMA||'.ACT_REG_INFO_REGISTRAL REG ON REG.ACT_ID = ACT.ACT_ID
        JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
        WHERE NVL(REG.REG_SUPERFICIE_PARCELA,0) = 0
        AND CRA.DD_CRA_CODIGO = ''03''
        AND APR.CASPRE IS NOT NULL
        AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL))
        AND ACT.BORRADO = 0
        AND APR.REM = 1
    ) T2 
    ON (T1.ACT_ID = T2.ACT_ID)
    WHEN MATCHED THEN UPDATE SET
    T1.REG_SUPERFICIE_PARCELA = T2.APR_PARCELA,
    T1.USUARIOMODIFICAR = '''||V_USUARIO||''',
    T1.FECHAMODIFICAR = SYSDATE
    '
  ; 
...
--[19.3] LONGITUD,latitud --> LOC_LONGITUD,LOC_LATITUD

 V_SQL := '
    MERGE INTO '||V_ESQUEMA||'.ACT_LOC_LOCALIZACION T1 USING 
    (
		SELECT ACT.ACT_ID, 
		CASE
		...
		CASE
			...
			WHEN APR.SIGNO_LONGITUD = ''+'' THEN cast(to_number(apr.LONGITUD)/1000000000000000 as number(21,15))
		END AS LOC_LONGITUD
		FROM APR_AUX_STOCK_UVEM_TO_REM APR
		JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
		JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
		JOIN '||V_ESQUEMA||'.ACT_REG_INFO_REGISTRAL REG ON REG.ACT_ID = ACT.ACT_ID
		JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
		...
		AND ACT.BORRADO = 0
		AND APR.REM = 1
		AND APR.latitud IS NOT NULL
		AND APR.LONGITUD IS NOT NULL
    ) T2 ON (T1.ACT_ID = T2.ACT_ID)
    WHEN MATCHED THEN UPDATE SET
...
--------------------------------------------
  ...
  --[20.1] CASUTG --> BIE_DREG_SUPERFICIE  (SUP_TOTAL_REGISTRAL_UTIL)
	...
	V_SQL := '
		...
		(
			SELECT APR.SUP_TOTAL_REGISTRAL_UTIL, BIE.BIE_ID, ACT.ACT_ID 
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
		    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
		    JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
  ...
  --[20.2] CASUTC --> BIE_DREG_SUPERFICIE_CONSTRUIDA (SUP_TOTAL_REAL_CONSTRUIDA)
	...
	V_SQL := '
		MERGE INTO '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES T1 USING
		(
			SELECT APR.SUP_TOTAL_REAL_CONSTRUIDA, BIE.BIE_ID, ACT.ACT_ID 
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
		    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
		    JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
		    JOIN '||V_ESQUEMA||'.BIE_DATOS_REGISTRALES BIE ON ACT.BIE_ID = BIE.BIE_ID
...
--------------------------------------------
  ...
  --[21.1] COTSIN --> ACB_COTSIN 
	...
	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO_BNK T1 
		USING (
			SELECT ACT.ACT_ID, APR.COTSIN 
			FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
			JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
			JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
			JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
		...
		WHEN NOT MATCHED THEN INSERT (T1.ACT_ID, T1.ACB_COTSIN, T1.USUARIOCREAR, T1.FECHACREAR) 
	         VALUES (
		     T2.ACT_ID,
		     T2.COTSIN,
		     '''||V_USUARIO||''',
...
--[21.2] COREAE --> ACB_COREAE 
  ...
  END IF;
      ...
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
				USING (
					SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, APR.NOMBRE_VIA AS BIE_LOC_DIRECCION
					FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
					JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
      ...
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (
				SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, APR.PORTAL_PUNTO_KM AS BIE_LOC_NUMERO_DOMICILIO
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
      ...
      --[22.3] NUESAC --> BIE_LOC_ESCALERA
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, APR.ESCALERA AS BIE_LOC_ESCALERA
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
      ...
      --[22.4] NUPIAC --> BIE_LOC_PISO
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, APR.PISO AS BIE_LOC_PISO
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
      ...
      --[22.5] COTIVI --> DD_TVI_ID
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, DD_TVI.DD_TVI_ID AS DD_TVI_ID
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
				...
				LEFT JOIN REMMASTER.DD_TVI_TIPO_VIA DD_TVI ON EQV.DD_CODIGO_REM = DD_TVI.DD_TVI_CODIGO
				WHERE CRA.DD_CRA_CODIGO = ''03'' 
				AND APR.REM = 1
				AND ACT.BORRADO = 0
				AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL))
      ...
      --[22.6] COPRAE --> DD_PRV_ID
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, PRV.DD_PRV_ID AS DD_PRV_ID, PRV.DD_PRV_CODIGO AS BIE_LOC_PROVINCIA, BIE.DD_PRV_ID AS DD_PRV_ID2, BIE.BIE_LOC_PROVINCIA AS BIE_LOC_PROVINCIA2
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
      ...
      --[22.7] COMUIX --> UPO_ID
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, UPO.DD_UPO_CODIGO AS BIE_LOC_MUNICIPIO, UPO.DD_UPO_ID AS DD_UPO_ID, UPO.DD_LOC_ID AS DD_LOC_ID
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
				JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
				LEFT JOIN REMMASTER.DD_UPO_UNID_POBLACIONAL UPO ON UPO.DD_UPO_CODIGO = APR.COD_MUNICIPIO
				WHERE CRA.DD_CRA_CODIGO = ''03'' 
				AND APR.REM = 1
				AND ACT.BORRADO = 0
				AND (APR.COESEN = ''01'' OR (APR.COESEN = ''02'' AND ESA.COESEN = 0 AND APR.FEC_BAJA_ACTIVO IS NULL))
      ...
      --[22.8] NUPUAC --> BIE_LOC_PUERTA
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, APR.NUM_PUERTA AS BIE_LOC_PUERTA
				FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
				JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
				JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
				JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
      ...
      --[22.9] COPOIN --> BIE_LOC_COD_POST
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.BIE_LOCALIZACION T1
        USING (SELECT BIE.BIE_LOC_ID AS BIE_LOC_ID, APR.COD_POSTAL AS BIE_LOC_COD_POST
            FROM '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM APR
            JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = APR.ACT_NUMERO_UVEM
            JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE ON BIE.BIE_ID = ACT.BIE_ID
            JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
      ...
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL T1
        USING (SELECT ICO.ICO_ID, TO_NUMBER(UR.FEC_ANIO_CONSTRUCCION) ICO_ANO_CONSTRUCCION
            ...
            JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
            JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
            WHERE NVL(ICO.ICO_ANO_CONSTRUCCION,0) = 0 AND TO_NUMBER(UR.FEC_ANIO_CONSTRUCCION) > 0
            AND CRA.DD_CRA_CODIGO = ''03'' 
			AND UR.REM = 1
			AND ACT.BORRADO = 0
			AND (UR.COESEN = ''01'' OR (UR.COESEN = ''02'' AND ESA.COESEN = 0 AND UR.FEC_BAJA_ACTIVO IS NULL))
            ) T2
      ...
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL T1
        USING (SELECT ICO.ICO_ID, TO_NUMBER(UR.FEC_ANIO_REHABILITACION) ICO_ANO_REHABILITACION
			   ...
			   AND TO_NUMBER(UR.FEC_ANIO_REHABILITACION) > NVL(ICO.ICO_ANO_CONSTRUCCION,0)
			   AND CRA.DD_CRA_CODIGO = ''03'' 
			   AND UR.REM = 1
			   AND ACT.BORRADO = 0
			   AND (UR.COESEN = ''01'' OR (UR.COESEN = ''02'' AND ESA.COESEN = 0 AND UR.FEC_BAJA_ACTIVO IS NULL))
      ...
      --[24.3] OBRA NUEVA --> DD_EAC_ID
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL T1
        USING (SELECT ICO.ICO_ID, EAC.DD_EAC_ID
			   FROM '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL ICO
			   JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = ICO.ACT_ID
			   JOIN '||V_ESQUEMA||'.APR_AUX_STOCK_UVEM_TO_REM UR ON UR.ACT_NUMERO_UVEM = ACT.ACT_NUM_ACTIVO_UVEM
			   JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
			   JOIN '||V_ESQUEMA||'.ACT_ESA_ESTADOS_ACT_UVEM ESA ON ESA.ACT_ID = ACT.ACT_ID
			   LEFT JOIN '||V_ESQUEMA||'.DD_EQV_BANKIA_REM EQV ON EQV.DD_NOMBRE_BANKIA = ''DD_OBRA_NUEVA'' AND EQV.DD_CODIGO_BANKIA = UR.OBRA_NUEVA
			   LEFT JOIN '||V_ESQUEMA||'.DD_EAC_ESTADO_ACTIVO EAC ON EAC.DD_EAC_CODIGO = EQV.DD_CODIGO_REM
			   WHERE CRA.DD_CRA_CODIGO = ''03'' 
			   AND UR.REM = 1
			   AND ACT.BORRADO = 0
			   AND (UR.COESEN = ''01'' OR (UR.COESEN = ''02'' AND ESA.COESEN = 0 AND UR.FEC_BAJA_ACTIVO IS NULL))
        ...
        ON (T1.ICO_ID = T2.ICO_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.DD_EAC_ID = T2.DD_EAC_ID, T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE';
      EXECUTE IMMEDIATE V_SQL;
      V_NUM_TABLAS := V_NUM_TABLAS + SQL%ROWCOUNT;
      IF V_NUM_TABLAS != 0 THEN
      ...
      --[25]TABLA ACT_CAT_CATASTRO
      --[25.1] REFERENCIA_CATASTRAL --> CAT_REF_CATASTRAL
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_CAT_CATASTRO T1
        USING (SELECT CAT.CAT_ID, UR.REFERENCIA_CATASTRAL
        ...
        WHEN MATCHED THEN UPDATE SET
            T1.CAT_REF_CATASTRAL = T2.REFERENCIA_CATASTRAL, T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE';
      EXECUTE IMMEDIATE V_SQL;
      V_NUM_TABLAS := SQL%ROWCOUNT;
      IF V_NUM_TABLAS != 0 THEN

            DBMS_OUTPUT.PUT_LINE('[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_CAT_CATASTRO '||V_NUM_TABLAS||' Registros.');
            PL_OUTPUT := PL_OUTPUT||'[INFO] REGISTROS ACTUALIZADOS EN '||V_ESQUEMA||'.ACT_CAT_CATASTRO '||V_NUM_TABLAS||' Registros. '||CHR(10)||CHR(10);
            DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
...
END SP_UPT_UPDATE_ACTIVES_TRAMITE;
/
EXIT;
