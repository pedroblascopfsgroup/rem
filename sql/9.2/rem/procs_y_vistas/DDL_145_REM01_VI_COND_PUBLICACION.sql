...
--v0.2
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_COND_PUBLICACION 
    ...
    FROM ACT_HIC_EST_INF_COMER_HIST HIC
    INNER JOIN ACT_ICO_INFO_COMERCIAL ICO ON ICO.ACT_ID = HIC.ACT_ID AND ICO.BORRADO = 0
    INNER JOIN DD_AIC_ACCION_INF_COMERCIAL DDAIC ON DDAIC.DD_AIC_ID = HIC.DD_AIC_ID
...
, PRECIO_APROBADO_VENTA AS (
    ...
    INNER JOIN DD_TPC_TIPO_PRECIO TPC ON VAL.DD_TPC_ID = TPC.DD_TPC_ID AND TPC.DD_TPC_CODIGO = ''02''
...
, PRECIO_APROBADO_RENTA AS (
    SELECT DISTINCT VAL.ACT_ID
    FROM ACT_VAL_VALORACIONES VAL
    INNER JOIN DD_TPC_TIPO_PRECIO TPC ON VAL.DD_TPC_ID = TPC.DD_TPC_ID AND TPC.DD_TPC_CODIGO = ''03''
    WHERE VAL.BORRADO = 0 AND NVL(VAL.VAL_IMPORTE, 0) > 0 
        AND NVL(VAL.VAL_FECHA_INICIO,TO_DATE(''01/01/1900'',''DD/MM/YYYY'')) <= SYSDATE
        AND NVL(VAL.VAL_FECHA_FIN,SYSDATE) >= SYSDATE
    )
, CERTIFICADO_ENERGETICO AS (
    ...
    FROM ACT_ADO_ADMISION_DOCUMENTO ADO
    INNER JOIN ACT_CFD_CONFIG_DOCUMENTO CFD ON CFD.CFD_ID = ADO.CFD_ID AND CFD.BORRADO = 0
    INNER JOIN DD_TPD_TIPO_DOCUMENTO TPD ON TPD.DD_TPD_ID = CFD.DD_TPD_ID AND TPD.BORRADO = 0
...
FROM ACT_ACTIVO ACT
INNER JOIN ACT_APU_ACTIVO_PUBLICACION   APU     ON APU.ACT_ID = ACT.ACT_ID         AND APU.BORRADO = 0
INNER JOIN DD_TCO_TIPO_COMERCIALIZACION   TCO   ON TCO.DD_TCO_ID = APU.DD_TCO_ID     AND TCO.BORRADO = 0       
INNER JOIN DD_TPA_TIPO_ACTIVO             TPA     ON TPA.DD_TPA_ID = ACT.DD_TPA_ID     AND TPA.BORRADO = 0
LEFT JOIN ACT_PTA_PATRIMONIO_ACTIVO       PTA     ON PTA.ACT_ID = ACT.ACT_ID           AND PTA.BORRADO = 0
LEFT JOIN DD_ADA_ADECUACION_ALQUILER      ADA     ON PTA.DD_ADA_ID = ADA.DD_ADA_ID     AND ADA.BORRADO = 0
LEFT JOIN PRECIO_APROBADO_VENTA                 PAV     ON PAV.ACT_ID = ACT.ACT_ID
LEFT JOIN PRECIO_APROBADO_RENTA                 PAR     ON PAR.ACT_ID = ACT.ACT_ID
LEFT JOIN INFORME_APROBADO                      INF     ON INF.ACT_ID = ACT.ACT_ID
LEFT JOIN CERTIFICADO_ENERGETICO                CEE     ON CEE.ACT_ID = ACT.ACT_ID';
