...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_PRESUPUESTO_ACTIVO_MATRIZ_EJERCICIO 
	...
	INNER JOIN REM01.ACT_AGR_AGRUPACION AGR ON AGA.AGR_ID = AGR.AGR_ID AND AGR.BORRADO = 0
	    AND AGR.DD_TAG_ID = (SELECT TAG.DD_TAG_ID FROM REM01.DD_TAG_TIPO_AGRUPACION TAG WHERE TAG.DD_TAG_CODIGO = ''16'')
	INNER JOIN REM01.ACT_AGA_AGRUPACION_ACTIVO AGA2 ON AGA2.AGR_ID = AGR.AGR_ID AND AGA2.BORRADO = 0
	LEFT JOIN REM01.V_PRESUPUESTO_ACTIVO_EJERCICIO PTO_AM ON AGA.ACT_ID = PTO_AM.ACT_ID
	LEFT JOIN REM01.V_PRESUPUESTO_ACTIVO_EJERCICIO PTO ON AGA2.ACT_ID = PTO.ACT_ID AND PTO_AM.EJE_ANYO = PTO.EJE_ANYO
