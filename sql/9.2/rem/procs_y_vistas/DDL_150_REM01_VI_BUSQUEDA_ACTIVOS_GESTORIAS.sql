...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ACTIVOS_GESTORIAS 
    ...
    SELECT
        ACT.ACT_ID,
        CAG.CAG_USU_USERNAME_ADMISION AS GESTORIA_ADMISION,
        CAG.CAG_USU_USERNAME_ADMINISTRACION AS GESTORIA_ADMINISTRACION,
        CAG.CAG_USU_USERNAME_FORMALIZACION AS GESTORIA_FORMALIZACION
    FROM
        ...
        INNER JOIN '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU ON GEE.USU_ID = USU.USU_ID
        INNER JOIN '||V_ESQUEMA_MASTER||'.GRU_GRUPOS_USUARIOS GRU ON GRU.USU_ID_USUARIO = USU.USU_ID
        INNER JOIN '||V_ESQUEMA||'.CAG_CONFIG_ACCESO_GESTORIAS CAG ON GRU.USU_ID_GRUPO IN (
            CAG.CAG_USU_GRUPO_ADMISION,
            CAG.CAG_USU_GRUPO_ADMINISTRACION,
            CAG.CAG_USU_GRUPO_FORMALIZACION
        ) 
    WHERE ACT.BORRADO = 0 AND GEE.BORRADO = 0 AND USU.BORRADO = 0  AND GRU.BORRADO =  0 AND CAG.BORRADO = 0
    GROUP BY (
    ACT.ACT_ID,
