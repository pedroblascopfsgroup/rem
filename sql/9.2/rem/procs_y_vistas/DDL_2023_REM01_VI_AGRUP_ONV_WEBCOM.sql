   ...
   -- Creamos vista materializada
	...
	AS
		...
		CASE WHEN (AGR.AGR_FECHA_ALTA IS NOT NULL) 
		  THEN CAST(TO_CHAR(AGR.AGR_FECHA_ALTA ,''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		  ELSE NULL
		END 																				    AS FECHA_ALTA,	
		CASE WHEN (AGR.AGR_INI_VIGENCIA IS NOT NULL) 
		  THEN CAST(TO_CHAR(AGR.AGR_INI_VIGENCIA ,''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		  ELSE NULL
		END 																				    AS DESDE,	
		CASE WHEN (AGR.AGR_FIN_VIGENCIA IS NOT NULL) 
		  THEN CAST(TO_CHAR(AGR.AGR_FIN_VIGENCIA ,''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR))
		  ELSE NULL
		END 																				   AS HASTA,	
