...
create or replace PROCEDURE AVISO_VENCIMIENTO_RESERVA IS
  ...
  V_ESQUEMA_MASTER VARCHAR2(25 CHAR):= 'REMMASTER'; -- Configuracion Esquema Master.
...
BEGIN
    ...
    EXECUTE IMMEDIATE '
    INSERT INTO '||V_ESQUEMA||'.MNO_MAESTRO_NOTIFICACIONES (MNO_ID,DD_EIN_ID,DD_TNO_ID,EIN_ID,DD_STA_ID,TAR_TAREA,TAR_DESCRIPCION,TAR_ID_DEST,FECHA_NOTIFICACION)
    SELECT '||V_ESQUEMA||'.S_MNO_MAESTRO_NOTIFICACIONES.NEXTVAL, EIN.DD_EIN_ID, TNO.DD_TNO_ID, ACT.ACT_ID, STA.DD_STA_ID, ''Aviso''
    ...
    JOIN '||V_ESQUEMA_MASTER||'.DD_EIN_ENTIDAD_INFORMACION EIN ON EIN.DD_EIN_CODIGO = ''61''
    JOIN '||V_ESQUEMA_MASTER||'.DD_TNO_TIPO_NOTIFICACION TNO ON TNO.DD_TNO_CODIGO = ''AVI_VENC''
    JOIN '||V_ESQUEMA_MASTER||'.DD_STA_SUBTIPO_TAREA_BASE STA ON STA.DD_STA_CODIGO = ''701''
    WHERE ERE.DD_ERE_CODIGO = ''02'' AND EEC.DD_EEC_CODIGO = ''06'' AND TGE.DD_TGE_CODIGO = ''GCOM'' AND OFR.AGR_ID IS NULL
        AND TRUNC(RES_FECHA_VENCIMIENTO)-TRUNC(SYSDATE) < 10 AND TRUNC(RES_FECHA_VENCIMIENTO) > TRUNC(SYSDATE)
        AND NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.MNO_MAESTRO_NOTIFICACIONES AUX WHERE AUX.EIN_ID = ACT.ACT_ID AND AUX.TAR_ID_DEST = USU.USU_ID
    ...
    JOIN '||V_ESQUEMA_MASTER||'.DD_TNO_TIPO_NOTIFICACION TNO ON TNO.DD_TNO_CODIGO = ''AVI_VENC''
    JOIN '||V_ESQUEMA_MASTER||'.DD_STA_SUBTIPO_TAREA_BASE STA ON STA.DD_STA_CODIGO = ''701''
    WHERE ERE.DD_ERE_CODIGO = ''02'' AND EEC.DD_EEC_CODIGO = ''06'' AND TRUNC(RES_FECHA_VENCIMIENTO)-TRUNC(SYSDATE) < 10
        AND TRUNC(RES_FECHA_VENCIMIENTO) > TRUNC(SYSDATE) AND LCO.LCO_GESTOR_COMERCIAL IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.MNO_MAESTRO_NOTIFICACIONES AUX WHERE AUX.EIN_ID = ACT.ACT_ID AND AUX.TAR_ID_DEST = LCO.LCO_GESTOR_COMERCIAL
            AND AUX.TAR_DESCRIPCION = CONCAT(''Vencimiento reserva próximo. Verifique la situación de la firma de la reserva y, en su caso, solicite ampliación de plazo, reflejando la nueva fecha de vencimiento en el expediente comercial número: '', ECO.ECO_NUM_EXPEDIENTE))
    ';
