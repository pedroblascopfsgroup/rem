...
--##		0.4 Cuarta version - HREOS-6164 - Carles Molins
--##########################################
--*/
...
create or replace PROCEDURE SP_ASC_ACT_SIT_COM_VACIOS (
...
BEGIN
  ...
  IF ID_ACTIVO > 0 THEN --IF_1
    ...
    IF V_COUNT > 0 THEN --IF_2
	  ...
	  V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' (ACT_ID,DD_SCM_ID_OLD,DD_SCM_ID_NEW,FECHA_CALCULO)
				SELECT ACT_ID, DD_SCM_ID, (SELECT DD_SCM_ID FROM '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL WHERE DD_SCM_CODIGO = '''||SITCOM_VENTA||''')
				, SYSDATE FROM '||V_ESQUEMA||'.'||TABLA_ACTIVO||' WHERE ACT_ID = '||ID_ACTIVO||' AND BORRADO = 0 AND DD_SCM_ID IS NULL';
  ...
  ELSE --IF_1
    ...
    V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' (ACT_ID,DD_SCM_ID_OLD,DD_SCM_ID_NEW,FECHA_CALCULO)
				SELECT ACT_ID, DD_SCM_ID, (SELECT DD_SCM_ID FROM '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL WHERE DD_SCM_CODIGO = '''||SITCOM_VENTA||''')
				, SYSDATE FROM '||V_ESQUEMA||'.'||TABLA_ACTIVO||' WHERE BORRADO = 0 AND DD_SCM_ID IS NULL';
  ...
  END IF; --IF_1
	...
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||TABLA_APOYO||'';
  ...
  IF V_COUNT > 0 THEN --IF_3
    ...
    V_SQL := '
      MERGE INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' ACT USING (
        ...
        INNER JOIN '||V_ESQUEMA||'.'||TABLA_APOYO||' TMP ON TMP.ACT_ID = ACT.ACT_ID
	...
	/*------------------------------*/
    /*SCM_DISPONIBLE_ALQUILER_OFERTA*/
    /*------------------------------*/
    V_SQL := '
      MERGE INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' ACT USING (
        SELECT DISTINCT ACT.ACT_ID, ACT.DD_SCM_ID AS SIT_ACTUAL, (SELECT DD_SCM_ID FROM '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL WHERE DD_SCM_CODIGO = '''||SITCOM_ALQUILER_OFERTA||''') AS SIT_NUEVA, OFR.DD_EOF_ID
        FROM '||V_ESQUEMA||'.'||TABLA_ACTIVO||' ACT
        INNER JOIN '||V_ESQUEMA||'.'||TABLA_APOYO||' TMP
        ON TMP.ACT_ID = ACT.ACT_ID
		INNER JOIN '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU ON ACT.ACT_ID = APU.ACT_ID
        AND APU.DD_TCO_ID = (SELECT DD_TCO_ID FROM '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION WHERE DD_TCO_CODIGO = '''||TIPOCOM_ALQUILER||''')
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] Informado SITUACION COMERCIAL -> DISPONIBLE PARA ALQUILER OFERTA.');

	/*------------------------------------*/
    /*SCM_DISPONIBLE_VENTA_ALQUILER_OFERTA*/
    /*------------------------------------*/
    V_SQL := '
      MERGE INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' ACT USING (
        SELECT DISTINCT ACT.ACT_ID, ACT.DD_SCM_ID AS SIT_ACTUAL, (SELECT DD_SCM_ID FROM '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL WHERE DD_SCM_CODIGO = '''||SITCOM_VENTA_ALQUILER_OFERTA||''') AS SIT_NUEVA, OFR.DD_EOF_ID
        FROM '||V_ESQUEMA||'.'||TABLA_ACTIVO||' ACT
        INNER JOIN '||V_ESQUEMA||'.'||TABLA_APOYO||' TMP
        ON TMP.ACT_ID = ACT.ACT_ID
		INNER JOIN '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU ON ACT.ACT_ID = APU.ACT_ID
        AND APU.DD_TCO_ID = (SELECT DD_TCO_ID FROM '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION WHERE DD_TCO_CODIGO = '''||TIPOCOM_ALQUILER_VENTA||''')
        INNER JOIN '||V_ESQUEMA||'.ACT_OFR ACTOFR
        ON ACTOFR.ACT_ID = ACT.ACT_ID
        INNER JOIN '||V_ESQUEMA||'.OFR_OFERTAS OFR
        ON OFR.OFR_ID = ACTOFR.OFR_ID
    ...
    V_SQL := '
      MERGE INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' ACT USING (
        ...
        ON TMP.ACT_ID = ACT.ACT_ID
		INNER JOIN '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU ON ACT.ACT_ID = APU.ACT_ID
        AND APU.DD_TCO_ID = (SELECT DD_TCO_ID FROM '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION WHERE DD_TCO_CODIGO = '''||TIPOCOM_VENTA||''')
        INNER JOIN '||V_ESQUEMA||'.ACT_OFR ACTOFR
        ON ACTOFR.ACT_ID = ACT.ACT_ID
        INNER JOIN '||V_ESQUEMA||'.OFR_OFERTAS OFR
        ON OFR.OFR_ID = ACTOFR.OFR_ID
    ...
    V_SQL :=  '
      MERGE INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' ACT USING (
        SELECT DISTINCT ACT.ACT_ID, ACT.DD_SCM_ID AS SIT_ACTUAL, (SELECT DD_SCM_ID FROM '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL WHERE DD_SCM_CODIGO = '''||SITCOM_ALQUILADO||''') AS SIT_NUEVA, ECO.ECO_FECHA_INICIO_ALQUILER
        FROM '||V_ESQUEMA||'.'||TABLA_ACTIVO||' ACT
        INNER JOIN '||V_ESQUEMA||'.'||TABLA_APOYO||' TMP
        ON TMP.ACT_ID = ACT.ACT_ID
        INNER JOIN '||V_ESQUEMA||'.ACT_OFR ACTOFR
        ...
        ON OFR.OFR_ID = ACTOFR.OFR_ID
		AND OFR.BORRADO=0
		INNER JOIN '||V_ESQUEMA||'.DD_TOF_TIPOS_OFERTA TOF
		ON TOF.DD_TOF_ID = OFR.DD_TOF_ID
        AND TOF.DD_TOF_CODIGO = '''||TIPOOFERTA_ALQUILER||'''
        INNER JOIN '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO
        ON ECO.OFR_ID = ACTOFR.OFR_ID
        AND ECO.BORRADO=0
        WHERE ECO.ECO_FECHA_INICIO_ALQUILER IS NOT NULL
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Informado SITUACION COMERCIAL -> ALQUILADO.');
    ...
    V_SQL :=  '
      MERGE INTO '||V_ESQUEMA||'.'||TABLA_APOYO||' ACT USING (
		...
		INNER JOIN '||V_ESQUEMA||'.DD_TOF_TIPOS_OFERTA TOF
		ON TOF.DD_TOF_ID = OFR.DD_TOF_ID
        AND TOF.DD_TOF_CODIGO = '''||TIPOOFERTA_VENTA||'''
        INNER JOIN '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO
        ON ECO.OFR_ID = ACTOFR.OFR_ID
        AND ECO.BORRADO=0
