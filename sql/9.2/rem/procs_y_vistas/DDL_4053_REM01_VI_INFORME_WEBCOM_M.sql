...
DECLARE
    ...
    V_TEXT_VISTA VARCHAR2(2400 CHAR) := 'VI_INFORME_WEBCOM_M'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	...
	AS
		...
		SELECT 
			CAST(  NVL(LPAD(ACT.ACT_ID,10,0) , ''00000'')   
				...
				|| NVL(LPAD(DIS.DIS_ID,8,0) , ''00000000'') 
			...
			CAST(VAL.VALOR_ESTIMADO_VENTA AS NUMBER(16,2)) AS VALOR_ESTIMADO_VENTA,
			CASE WHEN (VAL.VALOR_ESTIMADO_VENTA_F_INI IS NOT NULL) 
				THEN CAST(TO_CHAR(VAL.VALOR_ESTIMADO_VENTA_F_INI ,''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR))
				ELSE NULL
			END AS FECHA_VALOR_ESTIMADO_VENTA,
			CAST(ICO.ICO_JUSTIFICACION_VENTA AS VARCHAR2(1000 CHAR)) AS JUSTIFICACION_VALOR_EST_VENTA,
			CAST(VAL.VALOR_ESTIMADO_RENTA AS NUMBER(16,2)) AS VALOR_ESTIMADO_RENTA,
			CASE WHEN (VAL.VALOR_ESTIMADO_RENTA_F_INI IS NOT NULL) 
				THEN CAST(TO_CHAR(VAL.VALOR_ESTIMADO_RENTA_F_INI ,''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2(50 CHAR))
				ELSE NULL
			...
			CAST((SELECT DIS.DIS_DESCRIPCION
				...
				WHERE DIS.ICO_ID = ICO.ICO_ID AND DIS.DIS_NUM_PLANTA = 0 AND TPH.DD_TPH_CODIGO = ''11'') AS VARCHAR2(150 CHAR)) AS ANEJO_TRASTERO,
			CAST((SELECT DIS.DIS_DESCRIPCION
				FROM '||V_ESQUEMA||'.ACT_DIS_DISTRIBUCION DIS
				INNER JOIN '||V_ESQUEMA||'.DD_TPH_TIPO_HABITACULO TPH ON TPH.DD_TPH_ID = DIS.DD_TPH_ID
				WHERE DIS.ICO_ID = ICO.ICO_ID AND DIS.DIS_NUM_PLANTA = 0 AND TPH.DD_TPH_CODIGO = ''12'') AS VARCHAR2(150 CHAR)) AS ANEJO_GARAGE,
			...
			FROM '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL ICO
			INNER JOIN '||V_ESQUEMA||'.ACT_IMO_INFO_MOD IMO ON IMO.ICO_ID = ICO.ICO_ID
