...
<<<<<<< Updated upstream
    V_GESTOR_INMOBILIAR T_GESTOR := T_GESTOR('GADM','SUPADM','GACT','SUPACT','GPREC','SPREC','GPUBL','SPUBL','GCOM','SCOM','FVDNEG','FVDBACKOFR','FVDBACKVNT','SUPFVD','SFORM','GGADM','GIAFORM','GTOCED','CERT','GIAADMT');
=======
    V_GESTOR_INMOBILIAR T_GESTOR := T_GESTOR('GADM','SUPADM','GACT','SUPACT','GPREC','SPREC','GPUBL','SPUBL','GCOM','SCOM','FVDNEG','FVDBACKOFR','FVDBACKVNT','SUPFVD','SFORM','GGADM','GIAFORM','GTOCED','CERT','GIAADMT','PTEC', 'GTREE');
>>>>>>> Stashed changes
    V_GESTOR T_GESTOR;
...
BEGIN
    ...
    LOOP
        V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.TMP_GEST_ACT (
            ...
            '||V_CLASE_ACTIVO||'
                ...
                '||V_ACT_ID;
        EXECUTE IMMEDIATE V_MSQL;
        PL_OUTPUT := PL_OUTPUT || '   [INFO] - '||TO_CHAR(SYSDATE,'HH24:MI:SS')||' TMP_GEST_ACT cargada con '||SQL%ROWCOUNT||' gestores de tipo '||V_GESTOR(I)||'.'||CHR(10);           
    ...
    PL_OUTPUT := PL_OUTPUT || '[FIN] - PROCESO ASIGNACIÓN GESTORES DE ACTIVO FINALIZADO.'||CHR(10);
