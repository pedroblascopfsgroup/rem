...
--v0.4
    ...
    V_GESTOR_INMOBILIARIO T_GESTOR := T_GESTOR('GADM','SUPADM','GACT','SUPACT','GPREC','SPREC','GPUBL','SPUBL','GCOM','SCOM','FVDNEG','FVDBACKOFR','FVDBACKVNT','SUPFVD','SFORM','GGADM','GIAFORM','GTOCED','CERT','GIAADMT');
    V_GESTOR_FINANCIERO T_GESTOR := T_GESTOR('GPUBL','SPUBL','GCOM','SCOM','FVDNEG','FVDBACKOFR','FVDBACKVNT','SUPFVD','SFORM');
    V_GESTOR T_GESTOR;

BEGIN

    PL_OUTPUT := '[INICIO] - SE VA A PROCEDER A CREAR LA RELACION DE ACTIVOS CON SU GESTOR '||CHR(10);
    EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM ALL_TABLES WHERE TABLE_NAME LIKE ''TMP_GEST_ACT'' AND OWNER LIKE '''||V_ESQUEMA||'''' INTO V_NUM;
    ...
    EXECUTE IMMEDIATE 'CREATE TABLE '||V_ESQUEMA||'.TMP_GEST_ACT (
        ACT_ID NUMBER(16,0)
        , USU_ID NUMBER(16,0)
        , GEE_ID NUMBER(16,0)
        , GEH_ID NUMBER(16,0)
    ...
    ELSE-- ACTIVOS INMOBILIAROS
        V_GESTOR := V_GESTOR_INMOBILIARIO;
        V_CLASE_ACTIVO := 'JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID
            JOIN '||V_ESQUEMA||'.DD_SCR_SUBCARTERA SCR ON SCR.DD_CRA_ID = CRA.DD_CRA_ID AND SCR.DD_SCR_ID = ACT.DD_SCR_ID
            WHERE SCR.DD_SCR_CODIGO <> DECODE (CRA.DD_CRA_CODIGO, ''01'',''01'',''02'',''03'',''03'',''05'',''04'',''10'',''05'',''12'',''09'',''21'')';
    END IF;
            ...
            '||V_CLASE_ACTIVO||'
                ...
                AND NOT EXISTS (
                    ...
                    FROM (
                        ...
                        FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT 
                        JOIN '||V_ESQUEMA||'.GAH_GESTOR_ACTIVO_HISTORICO GAH  ON ACT.ACT_ID = GAH.ACT_ID
    ...
    END LOOP;
    PL_OUTPUT := PL_OUTPUT || '   [INFO] - '||TO_CHAR(SYSDATE,'HH24:MI:SS')||' TMP_GEST_ACT cargada. '||V_COUNT_1||' Filas.'||CHR(10);
    
    REM01.OPERACION_DDL.DDL_TABLE('ANALYZE','TMP_GEST_ACT','2');

    EXECUTE IMMEDIATE 'INSERT INTO '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD (
            GEE_ID,
            USU_ID,
            DD_TGE_ID,
            VERSION,
    ...
    EXECUTE IMMEDIATE 'INSERT INTO '||V_ESQUEMA||'.GAC_GESTOR_ADD_ACTIVO (
            GEE_ID,
            ACT_ID)
        SELECT
            TMP.GEE_ID,
    ...
    EXECUTE IMMEDIATE 'INSERT INTO '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST (
            GEH_ID,
            USU_ID,
            DD_TGE_ID,
            GEH_FECHA_DESDE,
    ...
    EXECUTE IMMEDIATE 'INSERT INTO '||V_ESQUEMA||'.GAH_GESTOR_ACTIVO_HISTORICO (
            GEH_ID,
            ACT_ID)
        SELECT
            TMP.GEH_ID,
    ...
    REM01.OPERACION_DDL.DDL_TABLE('ANALYZE','GEE_GESTOR_ENTIDAD','2');
    
    REM01.OPERACION_DDL.DDL_TABLE('ANALYZE','GAC_GESTOR_ADD_ACTIVO','2');
    
    REM01.OPERACION_DDL.DDL_TABLE('ANALYZE','GEH_GESTOR_ENTIDAD_HIST','2');                
    
    REM01.OPERACION_DDL.DDL_TABLE('ANALYZE','GAH_GESTOR_ACTIVO_HISTORICO','2');

    PL_OUTPUT := PL_OUTPUT || '[FIN] - PROCESO ASIGNACIÓN GESTORES DE ACTIVO FINALIZADO.';
