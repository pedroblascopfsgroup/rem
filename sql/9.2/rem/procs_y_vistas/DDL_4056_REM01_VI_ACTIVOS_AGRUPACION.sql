...
BEGIN
  ...
  V_MSQL := 'CREATE VIEW ' || V_ESQUEMA || '.V_ACTIVOS_AGRUPACION 
	AS
		...
		SPS.SPS_CON_TITULO,
		TPA.DD_TPA_DESCRIPCION||'' - ''
        ||(SELECT DD_SAC_DESCRIPCION FROM REM01.DD_SAC_SUBTIPO_ACTIVO WHERE DD_SAC_ID = ACT.DD_SAC_ID)
        ...
        LEFT JOIN (
            ...
            FROM '|| V_ESQUEMA ||'.ACT_ACTIVO T1 JOIN '|| V_ESQUEMA ||'.ACT_ICO_INFO_COMERCIAL T2 ON T2.ACT_ID = T1.ACT_ID LEFT JOIN '|| V_ESQUEMA ||'.ACT_VIV_VIVIENDA T3 ON T3.ICO_ID = T2.ICO_ID
            LEFT JOIN '|| V_ESQUEMA ||'.ACT_DIS_DISTRIBUCION T4 ON T4.ICO_ID = T2.ICO_ID LEFT JOIN '|| V_ESQUEMA ||'.DD_TPH_TIPO_HABITACULO T5 ON T5.DD_TPH_ID = T4.DD_TPH_ID
            WHERE T5.DD_TPH_CODIGO IN (''01'')
        ) DORMITORIOS ON (DORMITORIOS.ACT_ID = ACT.ACT_ID)
        LEFT JOIN (
            SELECT DISTINCT T1.ACT_ID, SUM(T4.DIS_CANTIDAD) OVER (PARTITION BY T1.ACT_ID,T5.DD_TPH_DESCRIPCION) AS SUMA 
            FROM '|| V_ESQUEMA ||'.ACT_ACTIVO T1 JOIN '|| V_ESQUEMA ||'.ACT_ICO_INFO_COMERCIAL T2 ON T2.ACT_ID = T1.ACT_ID LEFT JOIN '|| V_ESQUEMA ||'.ACT_VIV_VIVIENDA T3 ON T3.ICO_ID = T2.ICO_ID
            LEFT JOIN '|| V_ESQUEMA ||'.ACT_DIS_DISTRIBUCION T4 ON T4.ICO_ID = T2.ICO_ID LEFT JOIN '|| V_ESQUEMA ||'.DD_TPH_TIPO_HABITACULO T5 ON T5.DD_TPH_ID = T4.DD_TPH_ID
            WHERE T5.DD_TPH_CODIGO IN (''02'')
