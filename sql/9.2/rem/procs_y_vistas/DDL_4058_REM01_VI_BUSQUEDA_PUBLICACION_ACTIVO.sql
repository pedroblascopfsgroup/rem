...
BEGIN
  ...
  IF CUENTA>0 THEN
    DBMS_OUTPUT.PUT_LINE('DROP MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_PUBLICACION_ACTIVO...');
    EXECUTE IMMEDIATE 'DROP MATERIALIZED VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_PUBLICACION_ACTIVO';  
    DBMS_OUTPUT.PUT_LINE('DROP MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_PUBLICACION_ACTIVO... borrada OK');
  END IF;
  ...
  IF CUENTA>0 THEN
    DBMS_OUTPUT.PUT_LINE('DROP VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_PUBLICACION_ACTIVO...');
    EXECUTE IMMEDIATE 'DROP VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_PUBLICACION_ACTIVO';  
    DBMS_OUTPUT.PUT_LINE('DROP VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_PUBLICACION_ACTIVO... borrada OK');
  END IF;

  DBMS_OUTPUT.PUT_LINE('CREATING VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_PUBLICACION_ACTIVO...');
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_PUBLICACION_ACTIVO 
	AS
		WITH VISTA_ACT_PUB AS (
          ...
          , EPA.DD_EPA_CODIGO AS EPA_CODIGO_A
          , EPA.DD_EPA_DESCRIPCION AS EPA_DESCRIPCION_A
          , EPV.DD_EPV_CODIGO AS EPV_CODIGO_V
          , EPV.DD_EPV_DESCRIPCION AS EPV_DESCRIPCION_V
        ...
        JOIN '|| V_ESQUEMA ||'.DD_SCM_SITUACION_COMERCIAL SCM ON ACT.DD_SCM_ID = SCM.DD_SCM_ID AND SCM.DD_SCM_CODIGO IN (''02'',''03'',''04'',''06'',''07'',''08'',''09'') AND SCM.BORRADO = 0
        LEFT JOIN '|| V_ESQUEMA ||'.ACT_HIC_EST_INF_COMER_HIST HIC ON ACT.ACT_ID = HIC.ACT_ID AND HIC.BORRADO = 0
        LEFT JOIN '|| V_ESQUEMA ||'.DD_AIC_ACCION_INF_COMERCIAL AIC ON HIC.DD_AIC_ID = AIC.DD_AIC_ID AND AIC.DD_AIC_CODIGO IN (''01'',''02'',''03'',''04'') AND AIC.BORRADO = 0
        ...
        LEFT JOIN '|| V_ESQUEMA ||'.DD_TCO_TIPO_COMERCIALIZACION TCO ON TCO.DD_TCO_ID = ACT.DD_TCO_ID AND TCO.DD_TCO_CODIGO IN (''01'',''02'',''03'',''04'') AND TCO.BORRADO = 0
        ...
        SELECT ACT_ID, ACT_NUM_ACTIVO, TIPO_ACTIVO_CODIGO, TIPO_ACTIVO_DESCRIPCION, SUBTIPO_ACTIVO_CODIGO, SUBTIPO_ACTIVO_DESCRIPCION
          , DIRECCION, CARTERA_CODIGO, EPA_CODIGO_A, EPA_DESCRIPCION_A, EPV_CODIGO_V, EPV_DESCRIPCION_V, ADMISION, GESTION, INFORME_COMERCIAL
          , PUBLICACION, PRECIO
        FROM VISTA_ACT_PUB
        WHERE RN = 1';  

  DBMS_OUTPUT.PUT_LINE('CREATE VIEW '|| V_ESQUEMA ||'.V_BUSQUEDA_PUBLICACION_ACTIVO...Creada OK');
