        ...
        IF pISAGRUPACION = 0 THEN
            ...
            V_MSQL := '
                SELECT USU.USU_ID FROM '|| V_ESQUEMA ||'.ACT_ACTIVO ACT
                    ...
                    JOIN '|| V_ESQUEMA_MASTER ||'.DD_TGE_TIPO_GESTOR TGE ON GEE.DD_TGE_ID = TGE.DD_TGE_ID AND TGE.DD_TGE_CODIGO = '''|| pCODIGODEST ||'''
                    JOIN '|| V_ESQUEMA_MASTER ||'.USU_USUARIOS USU ON USU.USU_ID = GEE.USU_ID
