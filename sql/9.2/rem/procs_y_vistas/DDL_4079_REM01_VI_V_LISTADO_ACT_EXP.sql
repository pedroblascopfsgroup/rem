...
-- Modificación de la vista V_LISTADO_ACT_EXP de Fase1.
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW V_LISTADO_ACT_EXP AS 
		...
		JOIN '|| V_ESQUEMA ||'.ECO_EXPEDIENTE_COMERCIAL ECO ON BEX.ACT_BEX_ECO_ID = ECO.ECO_ID AND BEX.BORRADO = 0
		JOIN '|| V_ESQUEMA ||'.ACT_OFR OFR ON ECO.OFR_ID = OFR.OFR_ID AND BEX.ACT_ID = OFR.ACT_ID AND BEX.BORRADO = 0
		)
		SELECT ROWNUM AS ID_VISTA, ROW_NUMBER () OVER (PARTITION BY ACT.ACT_ID, ECO.ECO_ID 
		ORDER BY CASE WHEN TAN.DD_DRT_ID IS NULL THEN 0
		WHEN TAN.DD_DRT_ID IS NOT NULL AND DRT.DD_DRT_CODIGO = ''02'' THEN 1
		WHEN TAN.DD_DRT_ID IS NOT NULL AND DRT.DD_DRT_CODIGO = ''01'' THEN 2
		ELSE 3
		END ASC) ORDEN, ECO.ECO_ID, OFR.ACT_ID, OFR.OFR_ACT_PORCEN_PARTICIPACION AS PORCENTAJE_PARTICIPACION, OFR.ACT_OFR_IMPORTE AS IMP_ACT_OFERTA,
		...
		CASE WHEN (SIP.DD_SIP_CODIGO IS NOT NULL AND CAL.SITUACION_POS_COD_I IS NOT NULL AND SIP.DD_SIP_CODIGO = CAL.SITUACION_POS_COD_I)
		     ...
		     ELSE 0
		     END AS CONDICIONES,
		CASE WHEN TAN.DD_DRT_ID IS NULL THEN 0
		     WHEN TAN.DD_DRT_ID IS NOT NULL AND DRT.DD_DRT_CODIGO = ''02'' THEN 1
		     WHEN TAN.DD_DRT_ID IS NOT NULL AND DRT.DD_DRT_CODIGO = ''01'' THEN 2
		     ELSE 3
		     END AS CONDICIONANTE,
		...
		JOIN '|| V_ESQUEMA ||'.ACT_TIT_TITULO TIT ON OFR.ACT_ID = TIT.ACT_ID
		JOIN '|| V_ESQUEMA ||'.DD_ETI_ESTADO_TITULO ETI2 ON TIT.DD_ETI_ID = ETI2.DD_ETI_ID
		JOIN '|| V_ESQUEMA ||'.COE_CONDICIONANTES_EXPEDIENTE COE ON ECO.ECO_ID = COE.ECO_ID
		LEFT JOIN '|| V_ESQUEMA ||'.ECO_TAN_TANTEO_ACTIVO TAN ON ECO.ECO_ID = TAN.ECO_ID AND OFR.ACT_ID = TAN.ACT_ID AND TAN.BORRADO = 0
		LEFT JOIN '|| V_ESQUEMA ||'.DD_DRT_RESULTADO_TANTEO DRT ON TAN.DD_DRT_ID = DRT.DD_DRT_ID
