...
BEGIN
      ...
      V_MSQL := 'TRUNCATE TABLE '||V_TABLA||'';
      EXECUTE IMMEDIATE V_MSQL;
      V_MSQL := '
            INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (TBJ_ID, ACT_ID, TPO_ID, TAP_ID ,TRA_ID)
            ...
            LEFT JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T004 ON TPO_T004.DD_TPO_CODIGO = '''||TAREA_TRAMITE||'''
            LEFT JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T004 ON TAP_T004.TAP_CODIGO = '''||TAREA_TRAMITE||'''
            LEFT JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T003 ON TPO_T003.DD_TPO_CODIGO = ''T003''
            LEFT JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T003 ON TAP_T003.TAP_CODIGO = '''||TAREA_TRAMITE||'''
            LEFT JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T008 ON TPO_T008.DD_TPO_CODIGO = ''T008''
            LEFT JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T008 ON TAP_T008.TAP_CODIGO = '''||TAREA_TRAMITE||'''
            LEFT JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T002 ON TPO_T002.DD_TPO_CODIGO = ''T002''
            LEFT JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T002 ON TAP_T002.TAP_CODIGO = '''||TAREA_TRAMITE||'''
            LEFT JOIN '||V_ESQUEMA||'.DD_STR_SUBTIPO_TRABAJO STR ON STR.DD_STR_ID = TBJ.DD_STR_ID
            WHERE TBJ.BORRADO = 0
					AND TBJ.TBJ_NUM_TRABAJO = '||TRABAJO||' AND TRA.TRA_ID = '||ID_TRAMITE||'
      ';
      EXECUTE IMMEDIATE V_MSQL;
     ...
     EXECUTE IMMEDIATE 'MERGE INTO '||V_ESQUEMA||'.TRABAJOS_REPOSICIONAR T1
...
EXECUTE IMMEDIATE 'MERGE INTO TRABAJOS_REPOSICIONAR T1
						USING (SELECT T1.ACT_ID, T1.TBJ_ID, T1.TPO_ID, T1.TAP_ID, T3.USU_ID
							...
							JOIN '||V_ESQUEMA||'.GAC_GESTOR_ADD_ACTIVO T2 ON T1.ACT_ID = T2.ACT_ID
							JOIN '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD T3 ON T3.GEE_ID = T2.GEE_ID
							JOIN #ESQUEMA_MASTER#.DD_TGE_TIPO_GESTOR T4 ON T4.DD_TGE_ID = T3.DD_TGE_ID
							JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO T5 ON T5.DD_TPO_ID = T1.TPO_ID
							JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO T6 ON T6.TAP_ID = T1.TAP_ID
