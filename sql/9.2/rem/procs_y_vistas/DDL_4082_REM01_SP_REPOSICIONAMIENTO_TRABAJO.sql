...
create or replace PROCEDURE #ESQUEMA#.REPOSICIONAMIENTO_TRABAJO (USUARIO VARCHAR2
		, LIST_TRABAJOS VARCHAR2
		, TRAM_DESTINO IN #ESQUEMA#.TAP_TAREA_PROCEDIMIENTO.TAP_CODIGO%TYPE
		, LISTA_TRABAJOS VARCHAR2 DEFAULT NULL
		, EEC_DESTINO IN #ESQUEMA#.DD_EEC_EST_EXP_COMERCIAL.DD_EEC_CODIGO%TYPE DEFAULT NULL
		, PL_OUTPUT OUT VARCHAR2) AUTHID CURRENT_USER AS


      V_ESQUEMA VARCHAR2(10 CHAR) := '#ESQUEMA#';
      V_ESQUEMA_MASTER VARCHAR2(15 CHAR) := '#ESQUEMA_MASTER#';
      V_USUARIO VARCHAR2(50 CHAR) := 'REPOSICIONAMIENTO_TRABAJO';
...
BEGIN
      ...
      EXECUTE IMMEDIATE '
           INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (TBJ_ID, ACT_ID, TPO_ID, TAP_ID)
            ...
            JOIN '||V_ESQUEMA||'.DD_TTR_TIPO_TRABAJO TTR ON TTR.DD_TTR_ID = TBJ.DD_TTR_ID AND TTR.DD_TTR_CODIGO IN (''02'',''03'')
            JOIN '||V_ESQUEMA||'.DD_EST_ESTADO_TRABAJO EST ON EST.DD_EST_ID = TBJ.DD_EST_ID AND EST.DD_EST_CODIGO IN (''01'',''04'',''09'',''10'',''11'',''13'')
            ...
            JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T004 ON TPO_T004.DD_TPO_CODIGO = ''T004''
            JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T004 ON TAP_T004.TAP_CODIGO = ''T004_AnalisisPeticion''
            JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T003 ON TPO_T003.DD_TPO_CODIGO = ''T003''
            JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T003 ON TAP_T003.TAP_CODIGO = ''T003_AnalisisPeticion''
            JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T008 ON TPO_T008.DD_TPO_CODIGO = ''T008''
            JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T008 ON TAP_T008.TAP_CODIGO = ''T008_AnalisisPeticion''
            JOIN '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO_T002 ON TPO_T002.DD_TPO_CODIGO = ''T002''
            JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP_T002 ON TAP_T002.TAP_CODIGO = ''T002_AnalisisPeticion''
            LEFT JOIN '||V_ESQUEMA||'.DD_STR_SUBTIPO_TRABAJO STR ON STR.DD_STR_ID = TBJ.DD_STR_ID
            WHERE TBJ.BORRADO = 0 AND TBJ.USUARIOCREAR = '''||V_USUARIO||'''
