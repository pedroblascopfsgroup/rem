...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.'|| V_TEXT_VISTA ||'
	AS SELECT ROWNUM ID_VISTA, A.* FROM (
...
FROM 
	...
	LEFT JOIN (SELECT GPV_NUM_GASTO_HAYA, sum (a.gpl_importe_gasto) AS SUMA 
		...
		WHERE 1=1
		AND GIC.GIC_FECHA_CONTABILIZACION IS NULL
   ...
   SELECT     
	...
	FROM REM01.GPV_GASTOS_PROVEEDOR GPV
		...
		WHERE 1=1   
		AND GIC_FECHA_CONTABILIZACION IS NULL
      
      ) A WHERE TIPO_GASTO IN (''09'',''10'',''11'',''12'',''13'',''14'',''15'',''16'',''17'',''18'')';
