...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.'|| V_TEXT_VISTA ||'
	AS SELECT PVE_ID||GPV_ID ID_VISTA,SOCIEDAD,VISTA,RANK() OVER (ORDER BY ORDEN ASC) AS ORDEN,CIF,CODIGO,NUM_FRA,FECHA_FRA,FECHA_CONTABLE,DIARIO_CONTB,IMP_BRUTO,TOTAL
