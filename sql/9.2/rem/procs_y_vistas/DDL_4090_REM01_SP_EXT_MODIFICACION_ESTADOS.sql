...
--##        0.5-Daniel Algaba-HREOS-10668-Se añaden modificaciones según el ítem.
--#########################################
--*/
...
create or replace PROCEDURE       SP_EXT_MODIFICACION_ESTADOS (
	 ...
	 DD_STG_CODIGO VARCHAR2 DEFAULT NULL,
     
     DD_TIT_CODIGO VARCHAR2 DEFAULT NULL,
     
     GLD_IMP_IND_TIPO_IMPOSITIVO_ACTUAL INTEGER DEFAULT NULL,

	 GLD_IMP_IND_TIPO_IMPOSITIVO_NUEVO INTEGER DEFAULT NULL,
...
V_GLD_ID NUMBER(25);
V_GLD_IMP_IND_TIPO_IMPOSITIVO NUMBER;
...
V_GPV_ID NUMBER( 25 );
  ...
  FUNCTION ACT_GLD_GASTOS_LINEA_DETALLE RETURN NUMBER  IS
  BEGIN
 ...
 PROCEDURE PLP$ACTUALIZAR_HLD_GLD_GASTOS_LINEA_DETALLE IS

  BEGIN

     -- Busca las provisiones que actualizará y crea registro en HLD:
    V_SQL :=
    ' SELECT DISTINCT GLD_ID, GLD_IMP_IND_TIPO_IMPOSITIVO, GPV_NUM_GASTO_HAYA
      FROM REM01.GPV_GASTOS_PROVEEDOR GPV
      JOIN ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE GLD ON GLD.GPV_ID = GPV.GPV_ID
      WHERE GPV.GPV_NUM_GASTO_HAYA IN (  ' || GPV_NUM_GASTO_HAYA || '  ) 
	  		AND GLD.DD_STG_ID = (SELECT DD_STG_ID FROM ' || V_ESQUEMA || '.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = '''|| DD_STG_CODIGO ||''')
			AND GLD.DD_TIT_ID = (SELECT DD_TIT_ID FROM ' || V_ESQUEMA || '.DD_TIT_TIPOS_IMPUESTO WHERE DD_TIT_CODIGO = '''|| DD_TIT_CODIGO ||''')
			AND GLD.GLD_IMP_IND_TIPO_IMPOSITIVO = '|| GLD_IMP_IND_TIPO_IMPOSITIVO_ACTUAL ||'' ;

      OPEN v_cursor FOR V_SQL;
   
      LOOP
            FETCH v_cursor INTO V_GLD_ID, V_GLD_IMP_IND_TIPO_IMPOSITIVO, V_GPV_NUM_GASTO_HAYA ;
            EXIT WHEN v_cursor%NOTFOUND;

      		 PLP$CREAR_REGISTRO_HLD(
			     	 	 V_GPV_NUM_GASTO_HAYA,
			      		 'GLD_GASTOS_LINEA_DETALLE',
			      		 'GLD_ID',
			      		 V_GLD_ID,
			      		 'GLD_IMP_IND_TIPO_IMPOSITIVO',
			      		 V_GLD_IMP_IND_TIPO_IMPOSITIVO,
			      		 GLD_IMP_IND_TIPO_IMPOSITIVO_NUEVO
			    	      );
...
END IF;
    ...
    --Actualiza GLD_GASTOS_LINEA_DETALLE ? :

 IF ( ACT_GLD_GASTOS_LINEA_DETALLE = 1 ) THEN

        PLP$ACTUALIZAR_HLD_GLD_GASTOS_LINEA_DETALLE;

	V_SQL := 'MERGE INTO ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE T1 USING
    (
        SELECT GPV.GPV_ID
        FROM ' || V_ESQUEMA || '.GPV_GASTOS_PROVEEDOR GPV
        JOIN ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE GLD ON GLD.GPV_ID = GPV.GPV_ID
    ...
    WHEN MATCHED THEN
    UPDATE
    SET ' || CASE WHEN ( ( GLD_IMP_IND_TIPO_IMPOSITIVO_NUEVO IS NOT NULL ) AND ( UPDATE_IMP_IND_TIPO_IMPOSITIVO = 1 ) ) THEN ' T1.GLD_IMP_IND_TIPO_IMPOSITIVO = ' || GLD_IMP_IND_TIPO_IMPOSITIVO_NUEVO || ', ' END 
	  || CASE WHEN ( ( GLD_IMP_IND_TIPO_IMPOSITIVO_NUEVO IS NULL ) AND ( UPDATE_IMP_IND_TIPO_IMPOSITIVO = 1 ) ) THEN ' T1.GLD_IMP_IND_TIPO_IMPOSITIVO = NULL , ' END 
