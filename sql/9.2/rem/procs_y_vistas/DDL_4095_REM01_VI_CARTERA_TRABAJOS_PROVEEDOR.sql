...
BEGIN
	...
	EXECUTE IMMEDIATE 'CREATE OR REPLACE FORCE EDITIONABLE VIEW "'||V_ESQUEMA||'"."'||V_TEXT_VISTA||'" ("DD_CRA_ID", "PVE_ID", "ID_VISTA") AS 
							SELECT DISTINCT CRA.DD_CRA_ID, PVE.PVE_ID, CRA.DD_CRA_ID||PVE.PVE_ID AS ID_VISTA FROM ACT_PVE_PROVEEDOR PVE 
								JOIN ACT_PVC_PROVEEDOR_CONTACTO PVC ON PVE.PVE_ID = PVC.PVE_ID
								JOIN ACT_TBJ_TRABAJO TBJ ON PVC.PVC_ID = TBJ.PVC_ID
								JOIN ACT_TBJ ATBJ ON TBJ.TBJ_ID = ATBJ.TBJ_ID
								JOIN ACT_ACTIVO ACT ON ATBJ.ACT_ID = ACT.ACT_ID
