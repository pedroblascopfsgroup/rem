...
--## INCIDENCIA_LINK=HREOS-7105
--## PRODUCTO=NO
--## Finalidad: Creación de vista para sacar las subcarteras de los trabajos de un proveedor por la cartera
--##           
...
DECLARE
    ...
    V_TEXT_VISTA VARCHAR2(2400 CHAR) := 'V_SUBC_CARTERA_TRABAJOS_PROV'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_MSQL VARCHAR2(4000 CHAR);     
BEGIN
	DBMS_OUTPUT.PUT_LINE('CREATING VIEW '|| V_ESQUEMA ||'.'|| V_TEXT_VISTA ||'...');
	EXECUTE IMMEDIATE 'CREATE OR REPLACE FORCE EDITIONABLE VIEW "'||V_ESQUEMA||'"."'||V_TEXT_VISTA||'" ("DD_CRA_ID", "DD_SCR_ID", "PVE_ID", "ID_VISTA") AS 
							SELECT DISTINCT CRA.DD_CRA_ID, SCR.DD_SCR_ID, PVE.PVE_ID, CRA.DD_CRA_ID||SCR.DD_SCR_ID||PVE.PVE_ID AS ID_VISTA FROM '|| V_ESQUEMA ||'.ACT_PVE_PROVEEDOR PVE 
