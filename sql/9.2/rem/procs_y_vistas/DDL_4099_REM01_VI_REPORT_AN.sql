...
BEGIN
  ...
  V_MSQL := 'CREATE OR REPLACE FORCE VIEW ' || V_ESQUEMA || '.V_REPORT_AN
	AS
        ...
        SELECT ACT.ACT_ID||OFR.OFR_ID AS ID_VISTA, SYSDATE AS FECHA_EMISION, OFR.OFR_NUM_OFERTA, ACT.ACT_NUM_ACTIVO, TPA.DD_TPA_DESCRIPCION, LOC.BIE_LOC_DIRECCION, DDLOC.DD_LOC_DESCRIPCION
        , PRV.DD_PRV_DESCRIPCION, REG.BIE_DREG_SUPERFICIE_CONSTRUIDA, VPV.VAL_IMPORTE,
        ...
        CASE WHEN REG.DD_EON_ID IS NOT NULL THEN ''Yes''
             ELSE ''No''
        END AS SEGUNDA_MANO
        , ECV.DD_ECV_DESCRIPCION, AUX1.NUM_OFERTAS_ACT, NVL(AUX2.NUM_VISITAS_ACT, 0) AS NUM_VISITAS_ACT
        ...
        LEFT JOIN '||V_ESQUEMA||'.V_PRECIOS_VIGENTES VPV ON ACT.ACT_ID = VPV.ACT_ID AND VPV.DD_TPC_CODIGO = ''02''
