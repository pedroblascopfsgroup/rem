...
--## PRODUCTO=NO
--## 
--## Finalidad: Creación de tabla 'AUX_GEE_REMVIP_5110' , 'AUX_GAC_REMVIP_5110' , 'AUX_GEH_REMVIP_5110' , 'AUX_GAH_REMVIP_5110'
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
EXECUTE IMMEDIATE 'CREATE TABLE '||V_ESQUEMA_1||'.'||V_TABLA1||' AS 
  ( SELECT GEE.* FROM '||V_ESQUEMA_1||'.ACT_ACTIVO ACT
    ...
    WHERE DD_TGE_ID IN (399, 442, 401, 441, 393) AND ACT.DD_SCR_ID = 323 
  ) ';
...
END IF;
	...
	V_MSQL := 'CREATE INDEX IDX_AUX_GEE_GEE_ID ON '||V_ESQUEMA_1||'.AUX_GEE_REMVIP_5110(GEE_ID)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] CREADO INDICE PARA AUX_GEE_REMVIP_5110'); 

	V_MSQL := 'CREATE INDEX IDX_AUX_GAC_GAC_ID ON '||V_ESQUEMA_1||'.AUX_GAC_REMVIP_5110(GEE_ID)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] CREADO INDICE PARA AUX_GAC_REMVIP_5110'); 

	V_MSQL := 'CREATE INDEX IDX_AUX_GEH_GEH_ID ON '||V_ESQUEMA_1||'.AUX_GEH_REMVIP_5110(GEH_ID)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] CREADO INDICE PARA AUX_GEH_REMVIP_5110'); 

	V_MSQL := 'CREATE INDEX IDX_AUX_GAH_GAH_ID ON '||V_ESQUEMA_1||'.AUX_GAH_REMVIP_5110(GEH_ID)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] CREADO INDICE PARA AUX_GAH_REMVIP_5110'); 
