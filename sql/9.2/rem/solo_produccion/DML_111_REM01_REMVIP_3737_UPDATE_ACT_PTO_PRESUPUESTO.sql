...
--## FECHA_CREACION=20130327
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.2
--## INCIDENCIA_LINK=REMVIP-3737
--## PRODUCTO=NO
	...
	LOOP
		...
		EXECUTE IMMEDIATE V_MSQL INTO V_MAX_PTO_ID;
  		...
  		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ACT_PTO_PRESUPUESTO(
					...
					,'||V_EJE_ID||'
					,CASE  
						WHEN (SELECT PTO_IMPORTE_INICIAL FROM '||V_ESQUEMA||'.ACT_PTO_PRESUPUESTO WHERE EJE_ID = (SELECT EJE_ID FROM '||V_ESQUEMA||'.ACT_EJE_EJERCICIO WHERE EJE_ANYO = ''2018'') AND ACT_ID = '||FILA.ACT_ID||') IS NOT NULL
							THEN (SELECT PTO_IMPORTE_INICIAL FROM '||V_ESQUEMA||'.ACT_PTO_PRESUPUESTO WHERE EJE_ID = (SELECT EJE_ID FROM '||V_ESQUEMA||'.ACT_EJE_EJERCICIO WHERE EJE_ANYO = ''2018'') AND ACT_ID = '||FILA.ACT_ID||')
						ELSE 
							1000000
						END 
					, SYSDATE
