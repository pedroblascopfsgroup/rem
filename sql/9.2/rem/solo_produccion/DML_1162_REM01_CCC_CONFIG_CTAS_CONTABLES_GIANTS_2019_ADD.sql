...
--## PRODUCTO=NO
--##
--## Finalidad: Script que añade en CCC_CONFIG_CTAS_CONTABLES en la cartera GIANTS con lo existente en APPLE
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
BEGIN   
    ...
    V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.CCC_CONFIG_CTAS_CONTABLES 
	       SET BORRADO = 1,	
		   USUARIOBORRAR = ''REMVIP-4098'',
		   FECHABORRAR  = SYSDATE
 	       WHERE 1 = 1
	       AND DD_CRA_ID = (SELECT DD_CRA_ID FROM '||V_ESQUEMA ||'.DD_CRA_CARTERA WHERE DD_CRA_CODIGO = ''12'')
    ...
    V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.CCC_CONFIG_CTAS_CONTABLES 
		(' ||  'CCC_ID, DD_STG_ID, DD_CRA_ID, PRO_ID, EJE_ID, CCC_CUENTA_CONTABLE, CCC_CUENTA_ACTIVABLE, CCC_ARRENDAMIENTO, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) 
		 ...
		 AND NOT EXISTS ( 
				  ...
				  AND AUX.DD_CRA_ID = (SELECT DD_CRA_ID FROM '||V_ESQUEMA ||'.DD_CRA_CARTERA WHERE DD_CRA_CODIGO = ''12'')
