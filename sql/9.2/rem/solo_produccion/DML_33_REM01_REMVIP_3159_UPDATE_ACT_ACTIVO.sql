...
--## INCIDENCIA_LINK=REMVIP-3159
--## PRODUCTO=NO
--##
--## Finalidad: Borrado logico y update de activos
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-3159'; -- USUARIOCREAR/USUARIOMODIFICAR.
    V_ECO_ID NUMBER(16); 
...
BEGIN	
	...
	V_MSQL := 'UPDATE '||V_ESQUEMA||'.ACT_ACTIVO 
				   SET ACT_RECOVERY_ID = 1000000000194433,
				   USUARIOMODIFICAR = '''||V_USR||''',
				   FECHAMODIFICAR = SYSDATE 
				   WHERE ACT_NUM_ACTIVO = 203065';
	
	EXECUTE IMMEDIATE V_MSQL;
    
	DBMS_OUTPUT.PUT_LINE('[FIN] REGISTRO CON ACT_NUM_ACTIVO: 203065 ACTUALIZADO ACT_RECOVERY_ID');

	V_MSQL := 'UPDATE '||V_ESQUEMA||'.ACT_ACTIVO 
				   SET ACT_NUM_ACTIVO_SAREB = 721718,
				   USUARIOMODIFICAR = '''||V_USR||''',
				   FECHAMODIFICAR = SYSDATE 
				   WHERE ACT_NUM_ACTIVO = 167484';
	
	EXECUTE IMMEDIATE V_MSQL;
    
	DBMS_OUTPUT.PUT_LINE('[FIN] REGISTRO CON ACT_NUM_ACTIVO: 167484 ACTUALIZADO ACT_NUM_ACTIVO_SAREB');
