...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		t_tipo_data('8','Alicante'),
		t_tipo_data('8','Valencia'),
		t_tipo_data('8','Castellón'),
		t_tipo_data('8','Toledo'), 
      ...
      LOOP
        ...
        ELSE
            ...
            V_MSQL := 'DELETE FROM '|| V_ESQUEMA ||'.ACT_GES_DIST_GESTORES ' ||
					  'WHERE TIPO_GESTOR = ''GFORM'' AND COD_CARTERA = '''||V_TMP_TIPO_DATA(1)||''' AND COD_PROVINCIA = '''||V_DD_PRV_CODIGO||'''';            
            EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO]: SE HAN BORRADO '|| SQL%ROWCOUNT);            
            
            --3 INSERT ACT_GES_DIST_GESTORES
			...
			V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_GES_DIST_GESTORES (' || 
					  ...
					  'SELECT '|| V_ESQUEMA ||'.S_ACT_GES_DIST_GESTORES.NEXTVAL, ''GFORM'', '''||V_TMP_TIPO_DATA(1)||''', '''||V_DD_PRV_CODIGO||''', ''mramosg''
					  ,(SELECT USU_NOMBRE FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE BORRADO = 0 AND USU_USERNAME = ''mramosg'') , '''||V_USR||''', SYSDATE, 0 FROM DUAL'; 
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO]: SE HAN INSERTADO '|| SQL%ROWCOUNT);
			--2 INSERT
			V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_GES_DIST_GESTORES (' || 
					  'ID, TIPO_GESTOR, COD_CARTERA, COD_PROVINCIA, USERNAME, NOMBRE_USUARIO, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
					  'SELECT '|| V_ESQUEMA ||'.S_ACT_GES_DIST_GESTORES.NEXTVAL, ''GFORM'', '''||V_TMP_TIPO_DATA(1)||''', '''||V_DD_PRV_CODIGO||''', ''gsantos''
					  ,(SELECT USU_NOMBRE FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE BORRADO = 0 AND USU_USERNAME = ''gsantos'') , '''||V_USR||''', SYSDATE, 0 FROM DUAL';  
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO]: SE HAN INSERTADO '|| SQL%ROWCOUNT);
			--3 INSERT
			V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_GES_DIST_GESTORES (' || 
					  'ID, TIPO_GESTOR, COD_CARTERA, COD_PROVINCIA, USERNAME, NOMBRE_USUARIO, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
					  'SELECT '|| V_ESQUEMA ||'.S_ACT_GES_DIST_GESTORES.NEXTVAL, ''GFORM'', '''||V_TMP_TIPO_DATA(1)||''', '''||V_DD_PRV_CODIGO||''', ''nrivilla''
					  ,(SELECT USU_NOMBRE FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE BORRADO = 0 AND USU_USERNAME = ''nrivilla'') , '''||V_USR||''', SYSDATE, 0 FROM DUAL';  
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO]: SE HAN INSERTADO '|| SQL%ROWCOUNT);
