...
BEGIN	
    ...
    ELSE       
        ...
        V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_GES_DIST_GESTORES (' || 
				  ...
				  'SELECT '|| V_ESQUEMA ||'.S_ACT_GES_DIST_GESTORES.NEXTVAL, ''GIAFORM'', ''1'', '''||V_DD_PRV_CODIGO||''', ''hiposervi03''
				  ,(SELECT USU_NOMBRE FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE BORRADO = 0 AND USU_USERNAME = ''hiposervi03'') , '''||V_USR||''', SYSDATE, 0 FROM DUAL'; 
		EXECUTE IMMEDIATE V_MSQL;
