...
--*******************************************************************************************************************************
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: Actualizar estado de la provisión a <Contabilizada> si todos los gastos están contabilizados' );
			...
			USING (
				SELECT
				DISTINCT PRG.PRG_ID
				FROM '||V_ESQUEMA||'.H_ASPRO_10_CABECERA HCAB,  '||V_ESQUEMA||'.PRG_PROVISION_GASTOS PRG
				WHERE 1 = 1
				AND PRG.PRG_NUM_PROVISION=TO_NUMBER(HCAB.NUFREM)
				AND HCAB.COTIFA = ''A''
				AND PRG.BORRADO = 0
				AND PRG.DD_EPR_ID <> (SELECT DD_EPR_ID FROM '||V_ESQUEMA||'.DD_EPR_ESTADOS_PROVISION_GASTO WHERE DD_EPR_CODIGO = ''06'')
				AND  ( SELECT COUNT(1) 
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: Actualizar estado de la provisión a <Pagado> si todos los gastos están pagados y son de comunidades de propietarios y subcartera apple' );
			...
			USING (
				...
				AND EXISTS ( SELECT 1 
				       	     FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV, '||V_ESQUEMA||'.GPV_ACT, '||V_ESQUEMA||'.ACT_ACTIVO ACT
				       	     WHERE GPV.PRG_ID = PRG.PRG_ID
				       	     ...
				       	     AND DD_TGA_ID = ( SELECT DD_TGA_ID FROM '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO WHERE DD_TGA_CODIGO = ''05'' ) 	
				       	     AND GPV.BORRADO = 0	
				           ) 
