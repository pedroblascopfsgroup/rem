...
DECLARE
  ...
  CARTERA VARCHAR(20 CHAR) := '01'; --CODIGO CARTERA A BORRAR
  ORDEN NUMBER(2) := 2;
...
BEGIN
    ...
    LOOP
        V_MSQL := 'MERGE INTO REM01.BIE_BIEN T1
            USING (SELECT T1.BIE_ID
                FROM REM01.ACT_ACTIVO T1
                JOIN REM01.DD_CRA_CARTERA T2 ON T1.DD_CRA_ID = T2.DD_CRA_ID
                    AND T2.DD_CRA_CODIGO = '''||CARTERA||'''
                WHERE ROWNUM <= '||NUMERO_BORRAR||') T2
        ...
        V_MSQL := 'SELECT COUNT(1) FROM REM01.BIE_BIEN WHERE USUARIOBORRAR = ''HREOS-2719'' AND BORRADO = 1';
        EXECUTE IMMEDIATE V_MSQL INTO ACTIVOS;
        DBMS_OUTPUT.PUT_LINE('[CON_AUDITORIA {A   BORRAR}]: TABLA REM01.BIE_BIEN - '||ACTIVOS||' registros marcados para posterior borrado.');
    
        OPEN V_VAL_CURSOR FOR SELECT TABLA, CLAVE_TABLA, TABLA_REF, CLAVE_REF FROM ACTIVOS_A_BORRAR ORDER BY ID ASC;
            LOOP
