...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO] Inicio del proceso de borrado físico de trabajos duplicados.');
    ...
    LOOP
        V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
            USING (SELECT T3.TBJ_ID, ROW_NUMBER() OVER(PARTITION BY T1.ACT_ID ORDER BY T3.FECHACREAR ASC) RN
              ...
              WHERE T1.AGR_ID = 2861 AND T3.USUARIOCREAR <> ''MIG_SAREB'' AND ROWNUM <= '||NUMERO_BORRAR||') T2
            ON (T1.TBJ_ID = T2.TBJ_ID AND T2.RN > 1)
            WHEN MATCHED THEN UPDATE SET
                T1.USUARIOBORRAR = ''HREOS-2878'', T1.BORRADO = 1';
        ...
        FOR I IN (SELECT TABLA FROM ACTIVOS_A_BORRAR_2 ORDER BY ORDEN_TABLA DESC)
            ...
            DECLARE
                ...
                PRAGMA EXCEPTION_INIT(BORRADO_FK, -1407);
            BEGIN
                  V_MSQL := 'DELETE FROM '||I.TABLA||' WHERE USUARIOBORRAR = ''HREOS-2878'' AND BORRADO = 1';
                  TABLA := I.TABLA;
                  EXECUTE IMMEDIATE V_MSQL;
    ...
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('');
    DBMS_OUTPUT.PUT_LINE('[FIN] Borrado físico de trabajos');
