...
--## PRODUCTO=SI
--##
--## Finalidad: Actualizar el ACT_ADMISION.
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN
		...
		IF V_NUM_TABLAS > 0 THEN
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] Actualizando ACT_ADMISION');

		    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ACT
						USING (
						  SELECT ACT1.ACT_ID, SPS.SPS_FECHA_TOMA_POSESION, TIT.TIT_FECHA_INSC_REG 
						  FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ACT1
						  INNER JOIN '||V_ESQUEMA||'.ACT_TIT_TITULO TIT ON TIT.ACT_ID = ACT1.ACT_ID
						  INNER JOIN '||V_ESQUEMA||'.ACT_SPS_SIT_POSESORIA SPS ON SPS.ACT_ID = ACT1.ACT_ID
						  WHERE TIT.TIT_FECHA_INSC_REG IS NOT NULL 
		    ...
		    DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado '||sql%rowcount||' ACT_ADMISION en '||V_ESQUEMA||'.'||V_TEXT_TABLA||'');
