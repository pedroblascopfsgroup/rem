...
--## PRODUCTO=SI
--##
--## Finalidad: Borrar la tarea de Verificar estado posesorio.
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DECLARE
    ...
    V_TAP_CODIGO VARCHAR2(1024 CHAR) := 'T001_VerificarEstadoPosesorio';--Vble. codigo TAP
...
BEGIN
		...
		IF V_NUM_TABLAS > 0 THEN
			...
			V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ACT
						USING (SELECT TAR.TAR_ID 
							FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA2||' TAR 
							WHERE TAR.TAR_TAREA = (SELECT TAP.TAP_DESCRIPCION
										FROM TAP_TAREA_PROCEDIMIENTO TAP
										WHERE TAP.TAP_CODIGO = '''||V_TAP_CODIGO||''')) AUX
		...
		IF V_NUM_TABLAS2 > 0 THEN
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] Actualizando TAR_TAREAS_NOTIFICACIONES');
			
			V_MSQL2 := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA2||' TAR
						USING (SELECT TAR2.TAR_ID 
							FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA2||' TAR2 
							WHERE TAR2.TAR_TAREA = (SELECT TAP.TAP_DESCRIPCION
										FROM TAP_TAREA_PROCEDIMIENTO TAP
										WHERE TAP.TAP_CODIGO = '''||V_TAP_CODIGO||''')) AUX
