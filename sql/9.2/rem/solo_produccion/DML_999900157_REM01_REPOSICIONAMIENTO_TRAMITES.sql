...
BEGIN
    ...
    V_MSQL := 'INSERT INTO REM01.OFERTAS_REPOSICIONAR
        ...
        WHERE TAR.TAR_FECHA_FIN IS NULL AND TAP.TAP_CODIGO = ''T013_InstruccionesReserva''
            AND OFR.USUARIOCREAR = '''||V_USUARIO||''' AND RES.RES_FECHA_FIRMA IS NOT NULL';
    EXECUTE IMMEDIATE V_MSQL;
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] APROVISIONANDO LA TABLA AUXILIAR '||V_TABLA||'...');
      ...
      EXECUTE IMMEDIATE 'MERGE INTO REM01.ECO_EXPEDIENTE_COMERCIAL T1
        USING (SELECT DISTINCT T1.ECO_ID
            FROM ECO_EXPEDIENTE_COMERCIAL T1
            JOIN MIG2_TRA_TRAMITES_OFERTAS T2 ON T1.OFR_ID = T2.OFR_ID) T2
        ON (T1.ECO_ID = T2.ECO_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.DD_EEC_ID = (SELECT DD_EEC_ID FROM REM01.DD_EEC_EST_EXP_COMERCIAL WHERE DD_EEC_CODIGO = ''06'')';

      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL actualizada (DD_EEC_ID: Reservado). '||SQL%ROWCOUNT||' Filas.');
