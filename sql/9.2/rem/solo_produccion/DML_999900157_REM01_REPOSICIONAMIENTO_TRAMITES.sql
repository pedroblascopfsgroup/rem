...
DECLARE
    ...
    COUNTER NUMBER(2);
    COD_ITEM VARCHAR2(50 CHAR) := 'HREOS-3136';
...
BEGIN
    ...
    IF COUNTER = 1 THEN
        V_MSQL := 'DROP TABLE '||V_ESQUEMA||'.'||V_TABLA_REP||' PURGE';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('Tabla '||V_TABLA_REP||' borrada.');
        V_MSQL := ' CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA_REP||'
           ("OFR_ID" NUMBER(16,0) NOT NULL ENABLE, 
            ...
            "TEX_ID" NUMBER(16,0),
            "TRA_ID" NUMBER(16,0),
            "DD_TOF_CODIGO" VARCHAR2(50 CHAR),
            "ACT_ID" NUMBER(16,0)
           )';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('Tabla '||V_TABLA_REP||' creada.');
    ELSE
        V_MSQL := ' CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA_REP||'
           ("OFR_ID" NUMBER(16,0) NOT NULL ENABLE, 
            "ECO_ID" NUMBER(16,0),
    ...
    V_MSQL := 'INSERT INTO REM01.OFERTAS_REPOSICIONAR
        ...
        JOIN REM01.RES_RESERVAS RES ON RES.ECO_ID = ECO.ECO_ID
        ...
        JOIN REM01.ACT_TBJ_TRABAJO TBJ ON TBJ.TBJ_ID = ECO.TBJ_ID
        ...
        WHERE TAR.TAR_FECHA_FIN IS NULL AND TAP.TAP_CODIGO = ''T013_InstruccionesReserva''
            AND OFR.USUARIOCREAR = ''MIG_CAJAMAR'' AND RES.RES_FECHA_FIRMA IS NOT NULL';
    EXECUTE IMMEDIATE V_MSQL;
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] APROVISIONANDO LA TABLA AUXILIAR '||V_TABLA||'...');
      ...
      EXECUTE IMMEDIATE '
            INSERT INTO REM01.MIG2_TRA_TRAMITES_OFERTAS (OFR_ID, ACT_ID, TPO_ID, TAP_ID, USU_ID, SUP_ID)
            ...
            JOIN REM01.TAP_TAREA_PROCEDIMIENTO TAP ON TAP.TAP_CODIGO = ''T013_ResolucionComite'' AND BORRADO = 0';
      ...
      EXECUTE IMMEDIATE 'MERGE INTO REM01.MIG2_TRA_TRAMITES_OFERTAS T1
        ...
        WHEN MATCHED THEN UPDATE SET
          T1.USU_ID = T2.USU_ID
        WHERE T1.TAP_ID IN (SELECT TAP_ID FROM REM01.TAP_TAREA_PROCEDIMIENTO TAP WHERE TAP.TAP_CODIGO IN (''T013_DefinicionOferta'',''T013_InstruccionesReserva'',''T013_RespuestaOfertante'',''T013_ResolucionComite'',''T014_DefinicionOferta''))';

      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' actualizada (Gestor comercial). '||SQL%ROWCOUNT||' Filas.');
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' actualizada (Supervisor comercial). '||SQL%ROWCOUNT||' Filas.');
      ...
      EXECUTE IMMEDIATE 'MERGE INTO REM01.MIG2_TRA_TRAMITES_OFERTAS T1
        ...
        WHEN MATCHED THEN UPDATE SET
          T1.USU_ID = T2.USU_ID
        WHERE T1.TAP_ID = (SELECT TAP_ID FROM REM01.TAP_TAREA_PROCEDIMIENTO TAP WHERE TAP.TAP_CODIGO = (''T013_ResultadoPBC''))';

      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' actualizada (Gestor formalización). '||SQL%ROWCOUNT||' Filas.');
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA||' actualizada (Supervisor formalización). '||SQL%ROWCOUNT||' Filas.');
