...
BEGIN
    ...
    LOOP
        V_MSQL := 'MERGE INTO '||V_TABLA||' T1
            USING (SELECT BIE_ID FROM REM01.ACT_ACTIVO WHERE ACT_NUM_ACTIVO IN(121863,192715)) T2
            ON (T1.BIE_ID = T2.BIE_ID)
            WHEN MATCHED THEN UPDATE SET
                T1.USUARIOBORRAR = ''RECOVERY-7113'', T1.BORRADO = 1, T1.FECHABORRAR = SYSDATE';
        EXECUTE IMMEDIATE V_MSQL;
        EXIT WHEN SQL%ROWCOUNT = 0;

        V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE USUARIOBORRAR = ''RECOVERY-7113'' AND BORRADO = 1';
        ...
        OPEN V_VAL_CURSOR FOR V_MSQL;
            ...
            BEGIN
                V_MSQL := 'MERGE INTO '||TABLA||' T1 
                    USING (SELECT '||CLAVE_REF||' 
                        FROM '||TABLA_REF||' T2 
                        WHERE T2.USUARIOBORRAR = ''RECOVERY-7113'' AND T2.BORRADO = 1) T2 
            ...
            EXCEPTION
                ...
                DECLARE
                    ...
                    PRAGMA EXCEPTION_INIT(SIN_AUDITORIA, -904);
                BEGIN
                    V_MSQL := 'DELETE FROM '||TABLA||' T1 WHERE EXISTS (SELECT 1 FROM '||TABLA_REF||' T2 WHERE '||CLAVE_REF||' = '||CLAVE_TABLA||' AND T2.USUARIOBORRAR = ''RECOVERY-7113'' AND T2.BORRADO = 1)';
                    EXECUTE IMMEDIATE V_MSQL;
        ...
        OPEN V_VAL_CURSOR FOR V_MSQL;
            ...
            EXIT WHEN V_VAL_CURSOR%NOTFOUND;
            BEGIN
                  V_MSQL := 'DELETE FROM '||TABLA_EXCEPTION||' WHERE USUARIOBORRAR = ''RECOVERY-7113'' AND BORRADO = 1';
                  EXECUTE IMMEDIATE V_MSQL;
            ...
            EXCEPTION
                WHEN BORRADO_FK THEN
                    BEGIN
                        ...
                        V_MSQL := 'UPDATE '||TABLA_EXCEPTION||' SET '||CLAVE_EXCEPTION||' = NULL WHERE USUARIOBORRAR = ''RECOVERY-7113'' AND BORRADO = 1';
                        EXECUTE IMMEDIATE V_MSQL;
                    END;
                WHEN SIN_AUDITORIA THEN
                    NULL;
