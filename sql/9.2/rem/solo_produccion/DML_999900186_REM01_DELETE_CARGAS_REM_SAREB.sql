...
BEGIN
    ...
    IF V_NUM_TABLAS > 0 THEN
		...
		V_SQL := 'DELETE FROM '||V_ESQUEMA||'.'||V_TABLA_1||' CRG1 
              WHERE CRG1.USUARIOCREAR = ''HREOS-3410''';
    ...
    V_SQL := 'select count(1) from '||V_ESQUEMA||'.'||V_TABLA_2||' car
              WHERE USUARIOCREAR = ''HREOS-3410''';
    EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
    ...
    IF V_NUM_TABLAS > 0 THEN
		...
		V_SQL := 'DELETE FROM '||V_ESQUEMA||'.'||V_TABLA_2||' CAR1 
              WHERE CAR1.USUARIOCREAR = ''HREOS-3410''';
        EXECUTE IMMEDIATE V_SQL; 
