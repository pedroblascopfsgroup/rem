...
BEGIN	
	...
	EXECUTE IMMEDIATE 'UPDATE '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL SET DD_EEC_ID = (SELECT DD_EEC_ID FROM DD_EEC_EST_EXP_COMERCIAL WHERE DD_EEC_CODIGO = ''06'') 
					  WHERE ECO_ID = (SELECT ECO.ECO_ID FROM ECO_EXPEDIENTE_COMERCIAL ECO WHERE ECO.OFR_ID = (SELECT OFR_ID FROM OFR_OFERTAS OFR WHERE OFR.OFR_NUM_OFERTA = 70446733))';
