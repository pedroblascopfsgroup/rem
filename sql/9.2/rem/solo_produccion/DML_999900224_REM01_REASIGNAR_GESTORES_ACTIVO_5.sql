...
--## PRODUCTO=NO
--## 
--## Finalidad: Re-asignar activos de gmoreno a acabello
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] COMIENZA EL PROCESO PARA LA RE-ASIGNACIÓN DE ACTIVOS DE gmoreno A acabello');
	...
	V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''acabello''';
...
IF V_USU_NUEVO > 0 THEN
	...
	V_MSQL := 'UPDATE '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE
			  SET GEE.USU_ID = (SELECT USU.USU_ID
					     ...
					     WHERE USU.USU_USERNAME = ''acabello''),
	...
	SELECT
	GEH_ID,
	(SELECT USU_ID FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS WHERE USU_USERNAME = ''acabello'' ) AS USU_ID, 
	DD_TGE_ID,
