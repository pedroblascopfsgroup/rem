...
--## PRODUCTO=NO
--## 
--## Finalidad: Re-asignar activos de gmoreno a amonge
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] COMIENZA EL PROCESO PARA LA RE-ASIGNACIÓN DE ACTIVOS DE gmoreno A amonge');
...
V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''amonge''';
...
IF V_USU_NUEVO > 0 THEN
	...
	FROM (
		...
		AND (BIE.BIE_LOC_PROVINCIA = ''26''
   			OR (BIE.BIE_LOC_PROVINCIA = ''28''
       			AND BIE.BIE_LOC_MUNICIPIO = ''28079''))
	...
	SELECT
	GEH_ID,
	(SELECT USU_ID FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS WHERE USU_USERNAME = ''amonge'' ) AS USU_ID, 
	DD_TGE_ID,
