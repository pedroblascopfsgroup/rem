...
--## PRODUCTO=NO
--## 
--## Finalidad: Re-asignar activos de droncero a jalmansa
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] COMIENZA EL PROCESO PARA LA RE-ASIGNACIÓN DE ACTIVOS DE droncero A jalmansa');
...
V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''jalmansa''';
...
IF V_USU_NUEVO > 0 THEN
	...
	FROM (
		...
		AND BIE.BIE_LOC_PROVINCIA = ''3''
		AND USU.USU_USERNAME = ''droncero''))';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'UPDATE '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE
			  SET GEE.USU_ID = (SELECT USU.USU_ID
					     ...
					     WHERE USU.USU_USERNAME = ''jalmansa''),
	...
	SELECT
	GEH_ID,
	(SELECT USU_ID FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS WHERE USU_USERNAME = ''jalmansa'' ) AS USU_ID, 
	DD_TGE_ID,
	...
	WHERE GEE.DD_TGE_ID = (select dd_tge_id from remmaster.dd_tge_tipo_gestor where dd_tge_codigo = ''GCOM'')
	AND TAP.DD_TGE_ID = (select dd_tge_id from remmaster.dd_tge_tipo_gestor where dd_tge_codigo = ''GCOM'') 
