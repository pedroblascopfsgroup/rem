...
--## PRODUCTO=NO
--## 
--## Finalidad: Re-asignar activos de palcocer a mdiez
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] COMIENZA EL PROCESO PARA LA RE-ASIGNACIÓN DE ACTIVOS DE palcocer A mdiez');
...
V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS USU WHERE USU.USU_USERNAME = ''mdiez''';
...
IF V_USU_NUEVO > 0 THEN
	...
	SELECT
	GEH_ID,
	(SELECT USU_ID FROM '||V_ESQUEMA_MASTER||'.USU_USUARIOS WHERE USU_USERNAME = ''mdiez'' ) AS USU_ID, 
	DD_TGE_ID,
