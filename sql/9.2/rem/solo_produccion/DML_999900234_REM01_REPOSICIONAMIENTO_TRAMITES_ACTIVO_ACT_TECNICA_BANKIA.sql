...
BEGIN
    ...
    FOR I IN V_TRAMITE_ACT_TEC.FIRST .. V_TRAMITE_ACT_TEC.LAST
		...
		V_MSQL := 'select count(1) from GPV_TBJ gpv inner join (
			select tbj.tbj_id from act_activo act
				inner join ACT_TBJ atb on atb.ACT_ID=act.act_id
				inner join ACT_TBJ_TRABAJO tbj on atb.tbj_id=tbj.tbj_id
				inner join ACT_TRA_TRAMITE tra on tra.tbj_id=tbj.tbj_id
				where act.act_num_activo = '||V_TMP_TRAMITE_ACT_TEC(1)||' and tra.tra_id='||V_TMP_TRAMITE_ACT_TEC(2)||') t2 on gpv.tbj_id=t2.tbj_id';
		...
		IF(V_EXISTE_GASTO = 0) THEN
			...
			EXECUTE IMMEDIATE V_MSQL INTO V_PVC_ID;
	    		...
	    		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.OFERTAS_REPOSICIONAR (OFR_ID, ACT_ID, TBJ_ID, TAR_ID, TEX_ID, TRA_ID, PVC_ID)
			    SELECT --OFR.OFR_ID, ECO.ECO_ID, TBJ.TBJ_ID, TAR.TAR_ID, TEX.TEX_ID, TRA.TRA_ID, TOF.DD_TOF_CODIGO, ACT.ACT_ID
				0, ACT.ACT_ID, ATB.TBJ_ID, TAR.TAR_ID, TEX.TEX_ID, TRA.TRA_ID,'||V_PVC_ID||'
    ...
    V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.OFERTAS_REPOSICIONAR';
	...
	V_MSQL := '
            ...
            SELECT 0, ORE.ACT_ID
                ...
                TPO_ID, TAP.TAP_ID, NULL AS USU_ID, NULL AS SUP_ID,ORE.PVC_ID, TRA_ID
            FROM '||V_ESQUEMA||'.OFERTAS_REPOSICIONAR ORE
            JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP ON TAP.TAP_CODIGO = ''T004_ResultadoTarificada'' AND BORRADO = 0';
            EXECUTE IMMEDIATE V_MSQL;
