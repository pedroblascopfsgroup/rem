...
DECLARE
	...
	V_USUARIO VARCHAR2(32 CHAR) := 'REMVIP-99';

 BEGIN

  EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE TIPO_GESTOR = ''GIAFORM'' 
   AND USERNAME = ''tecnotra03'' AND COD_CARTERA = ''10'' ' INTO V_COUNT;
  ...
  IF V_COUNT = 0 THEN
  	  ...
  	  V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (
			...
			, FECHACREAR
 			...
 			, 10
 			, ''tecnotra03''
 			, ''TECNOTRAMIT GESTION SL. Formalización Usuario  genérico''
 			, '''||V_USUARIO||'''
 			, SYSDATE
 			)
		  ';
