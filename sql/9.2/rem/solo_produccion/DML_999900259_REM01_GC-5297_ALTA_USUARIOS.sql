...
--## INCIDENCIA_LINK=REMNIVUNO-332, REMNIVUNO-336
--## PRODUCTO=NO
--## 
...
DECLARE
	...
	V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		...
		T_TIPO_DATA( 'B15704364' ,'ESC SERVICIOS GENERALES S.L.' ,''  ,''  ,'Jose-antonio.costana@prosegur.com'  , 'AGHRRHOAOM'  ,'HAYAPROV'  ,'B15704364'  ,1 ,'REMPROV' ,'REMNIVUNO-336'),
		T_TIPO_DATA( '41499591R' ,'Carmen' ,'Florez'  ,''  ,'carmen.florez@prosegur.com'  , 'ODXWGZUTAH'  ,'HAYAPROV'  ,'B15704364'  ,0 ,'REMPROV' ,'REMNIVUNO-336'),
		T_TIPO_DATA( 'B60546611' ,'DORSERAN SERVEIS DE LA CONSTRUCCIÓ, SL' ,''  ,''  ,'info@dorseran.net'  , 'BMZMIQMAYM'  ,'HAYAPROV'  ,'B60546611'  ,1 ,'REMPROV' ,'REMNIVUNO-332'),
		T_TIPO_DATA( '33944952B' ,'Angel' ,'del Moral'  ,'Rabat'  ,'angel@dorseran.net'  , 'DEHGSRCYNP'  ,'HAYAPROV'  ,'B60546611'  ,0 ,'REMPROV' ,'REMNIVUNO-332'),
		T_TIPO_DATA( '40559874H' ,'Elisabet' ,'Ester'  ,'Irla'  ,'lis@dorseran.net'  , 'MMFPRQDCFJ'  ,'HAYAPROV'  ,'B60546611'  ,0 ,'REMPROV' ,'REMNIVUNO-332')
	...
	SEL_USU_DES_CONT VARCHAR (32000) := q'[SELECT COUNT (1) FROM #ESQUEMA#.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = :1]';
...
BEGIN	
    ...
    LOOP
		...
		IF V_CONT = 0 THEN
			EXECUTE IMMEDIATE IN_USU_USUARIOS USING VV_USERNAME, VV_PASS, VV_NOMBRE, VV_APELLIDO1, VV_APELLIDO2, VV_EMAIL, VV_ES_GRU, VV_TICKET;
			DBMS_OUTPUT.PUT_LINE('[OK_USU_USUARIOS]:  USUARIO '||VV_USERNAME||'');
			ELSE DBMS_OUTPUT.PUT_LINE('[NO_USU_USUARIOS]:  EL USUARIO '||VV_USERNAME||' YA EXISTE');
		END IF;
		...
		IF V_CONT = 0 THEN
			...
			DBMS_OUTPUT.PUT_LINE('[OK]: INSERCION EN ZON_PEF_USU DE '||VV_USERNAME||' CON EL PERFIL '||VV_PEF);
			ELSE DBMS_OUTPUT.PUT_LINE('[NO_PEF_PERFILES]:  EL USUARIO '||VV_USERNAME||' YA TIENE EL PERFIL '||VV_PEF);
		END IF;
		
		--Comprobamos si el usuario ya esta en ese despacho
		DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN USD_USUARIOS_DESPACHOS DE '||VV_USERNAME||' CON EL DESPACHO '||VV_DES);
		EXECUTE IMMEDIATE SEL_USU_DES_CONT INTO V_CONT USING VV_DES;
		IF V_CONT = 0 THEN
			...
			ELSE DBMS_OUTPUT.PUT_LINE('[NO_USD_USUARIOS_DESPACHOS]:  EL USUARIO '||VV_USERNAME||' YA TIENE EL DESPACHO '||VV_DES);
		END IF;
		
		--GRUPOS 
		
		IF VV_ES_GRU IS NULL THEN --NO INSERTA EN GRUPO
