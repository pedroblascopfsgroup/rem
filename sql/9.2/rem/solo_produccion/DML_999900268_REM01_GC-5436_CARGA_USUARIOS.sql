...
DECLARE
	...
	V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		...
		T_TIPO_DATA( '50127192A' ,'María' ,'Mangas'  ,''  ,'maria.mangas@nfq.es'  , 'PVKZGEBDBY'  ,'HAYACONSU'  ,''  ,0 ,'' ,'REMNIVUNO-530'),
		T_TIPO_DATA( '08873278Q' ,'José Manuel' ,'Contador'  ,''  ,'josemanuel.contador@nfq.es'  , 'OSBLSSZWUT'  ,'HAYACONSU'  ,''  ,0 ,'' ,'REMNIVUNO-530'),
		T_TIPO_DATA( 'Y1397026C' ,'Angelica María' ,'Brito'  ,'Challa'  ,'angelica.brito@nfq.es'  , 'OJVFICVHSN'  ,'HAYACONSU'  ,''  ,0 ,'' ,'REMNIVUNO-530')
	); 
	V_TMP_TIPO_DATA T_TIPO_DATA;
  
  
	SEL_ZON_ID VARCHAR2 (32000) := q'[SELECT ZON_ID FROM REM01.ZON_ZONIFICACION WHERE ZON_DESCRIPCION = 'REM']';
	
	SEL_PEF_ID VARCHAR2 (32000) := q'[SELECT PEF_ID FROM REM01.PEF_PERFILES WHERE PEF_CODIGO = :1]';
	
	SEL_USU_ID VARCHAR2 (32000) := q'[SELECT USU_ID FROM REMMASTER.USU_USUARIOS WHERE USU_USERNAME = :1]';
	
	SEL_USU_PASS VARCHAR2 (32000) := q'[SELECT USU_PASSWORD FROM REMMASTER.USU_USUARIOS WHERE USU_ID = :1]';
	
	SEL_USU_EMAIL VARCHAR2 (32000) := q'[SELECT USU_MAIL FROM REMMASTER.USU_USUARIOS WHERE USU_ID = :1]';
		
	SEL_DES_ID VARCHAR2(32000) := q'[SELECT DES_ID FROM REM01.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = :1]';
	
	SEL_GRU_ID VARCHAR2 (32000) := q'[SELECT USU_ID FROM REMMASTER.USU_USUARIOS WHERE USU_GRUPO = 1 AND USU_USERNAME = :1]';
	
	SEL_USU_GRUPO VARCHAR2(32000) := q'[SELECT USU_GRUPO FROM REMMASTER.USU_USUARIOS WHERE USU_ID = :1]';
	
	SEL_GRU_ID_CONT VARCHAR2 (32000) := q'[SELECT COUNT(1) FROM REMMASTER.USU_USUARIOS WHERE USU_USERNAME = :1 AND USU_GRUPO = 1]';
	
	SEL_PEF_USU_CONT VARCHAR2 (32000) := q'[SELECT COUNT(1) FROM REM01.ZON_PEF_USU WHERE PEF_ID = :1 AND USU_ID = :2]';
	
	SEL_DES_CONT VARCHAR (32000) := q'[SELECT COUNT(1) FROM REM01.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = :1]';
	
	SEL_USU_ID_CONT VARCHAR2 (32000) := q'[SELECT COUNT(1) FROM REMMASTER.USU_USUARIOS WHERE USU_USERNAME = :1]';
    
    SEL_DES_USU_CONT VARCHAR2 (32000) := q'[SELECT COUNT(1) FROM REM01.USD_USUARIOS_DESPACHOS WHERE USU_ID = :1 AND DES_ID = :2]';
	
	IN_USU_USUARIOS VARCHAR2 (32000):= q'[INSERT INTO REMMASTER.USU_USUARIOS (USU_ID, ENTIDAD_ID, USU_USERNAME,USU_NOMBRE,USU_PASSWORD,USU_APELLIDO1,USU_APELLIDO2,USU_MAIL,USU_GRUPO, USU_FECHA_VIGENCIA_PASS, USUARIOCREAR,FECHACREAR,BORRADO) VALUES
														(REMMASTER.S_USU_USUARIOS.nextval, 1, :1,:2,:3,:4,:5,:6,:7,SYSDATE+730,:8,SYSDATE,0)]';

	IN_ZON_PEF_USU VARCHAR2 (32000) := q'[INSERT INTO REM01.ZON_PEF_USU (ZPU_ID, ZON_ID, PEF_ID, USU_ID, USUARIOCREAR, FECHACREAR, BORRADO) VALUES
										(REM01.S_ZON_PEF_USU.NEXTVAL, :1,:2,:3,:4,SYSDATE,0)]';
																				
	IN_USU_DES VARCHAR2 (32000) := q'[INSERT INTO REM01.USD_USUARIOS_DESPACHOS (USD_ID, DES_ID, USU_ID, USD_GESTOR_DEFECTO, USD_SUPERVISOR, USUARIOCREAR, FECHACREAR, BORRADO) VALUES
										(REM01.S_USD_USUARIOS_DESPACHOS.NEXTVAL, :1, :2, :3, :4,:5, SYSDATE,0)]';
										
	IN_GRU_USU VARCHAR2 (32000) := q'[INSERT INTO REMMASTER.GRU_GRUPOS_USUARIOS (GRU_ID, USU_ID_GRUPO, USU_ID_USUARIO, USUARIOCREAR, FECHACREAR, BORRADO) VALUES
										(REMMASTER.S_GRU_GRUPOS_USUARIOS.NEXTVAL,:1,:2, :3,SYSDATE,0)]';
										
	UPD_GRU_USU VARCHAR2 (32000) := q'[UPDATE REMMASTER.USU_USUARIOS SET USU_GRUPO = 1 WHERE USU_ID = :1]';
	
	UPD_USU_PASS VARCHAR2 (32000) := q'[UPDATE REMMASTER.USU_USUARIOS SET USU_PASSWORD = :1 WHERE USU_ID = :2]';
	
	UPD_USU_EMAIL VARCHAR2 (32000) := q'[UPDATE REMMASTER.USU_USUARIOS SET USU_MAIL = :1 WHERE USU_ID = :2]';
...
BEGIN	
    ...
    LOOP
		...
		ELSE
			...
			ELSE DBMS_OUTPUT.PUT_LINE('[NO_USD_USUARIOS_DESPACHOS]: NO EXISTE EL DESPACHO '||VV_DES);
			END IF;
		END IF;
