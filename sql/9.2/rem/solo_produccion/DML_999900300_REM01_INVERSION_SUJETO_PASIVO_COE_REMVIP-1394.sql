...
--## PRODUCTO=NO
--## 
--## Finalidad: Cambio de COE_INVERSION_SUJETO_PASIVO en COE
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
BEGIN	
 ...
 DBMS_OUTPUT.PUT_LINE('[INICIO] ');
	...
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||V_TABLA||' COE 
				...
				WHERE COE.COE_INVERSION_SUJETO_PASIVO = 1
				';
	...
	ELSE
		...
		V_SQL := 'UPDATE '||V_ESQUEMA||'.'||V_TABLA||' COE SET COE.USUARIOMODIFICAR = '''||V_USUARIO||''', COE.FECHAMODIFICAR = SYSDATE, COE.COE_INVERSION_SUJETO_PASIVO = 1
			WHERE COE.ECO_ID = (
				SELECT ECO.ECO_ID
