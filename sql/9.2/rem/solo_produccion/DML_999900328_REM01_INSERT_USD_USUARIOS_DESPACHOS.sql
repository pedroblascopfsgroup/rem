...
BEGIN	
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN USD_USUARIOS_DESPACHOS] ');
			...
			WHERE DES_ID = 
				...
				WHERE DES_DESPACHO = ''SBACKOFFICEINMLIBER'') 
				AND USU_ID = 
					(SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS 
					WHERE USU_USERNAME = ''SBACKOFFICEINMLIBER'')';
			...
			ELSE
				V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS' ||
							...
							' (SELECT DES_ID FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO = ''SBACKOFFICEINMLIBER''),' ||							
							' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''SBACKOFFICEINMLIBER''),' ||
							' 1,1,''REMVIP-1942'',SYSDATE,0 FROM DUAL';
