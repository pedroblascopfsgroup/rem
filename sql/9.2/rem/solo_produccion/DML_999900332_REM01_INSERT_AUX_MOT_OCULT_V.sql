...
DECLARE
    ...
    V_TABLA VARCHAR2(30 CHAR) := 'AUX_MOT_OCULT_V';  -- Tabla a modificar  
...
BEGIN	
	...
	INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (OCULTO, DD_MTO_CODIGO, ACT_ID) (
			SELECT OCULTO, DD_MTO_CODIGO, ACT_ID
              FROM (SELECT OCULTO, DD_MTO_CODIGO, ACT_ID, ROW_NUMBER () OVER (PARTITION BY ACT_ID ORDER BY ORDEN ASC) ROWNUMBER
                    FROM(
                          SELECT ACT.ACT_ID
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('[FIN]: TABLA AUX_MOT_OCULT_V ACTUALIZADA CORRECTAMENTE ');
