...
----------------------------------------------------------------------------------------------------------------------------------    
--UPDATEAMOS A FSV 211 ACTIVOS DE BANKIA    
----------------------------------------------------------------------------------------------------------------------------------
UPDATE REM01.AUX_MMC_PERIMETRO_ALQUILABLE AUX
SET AUX.TIPO_CONTRATO_ALQUILER = 'FSV'
WHERE AUX.ID_HAYA IN 
(
SELECT AUX2.ID_HAYA
FROM REM01.AUX_MMC_PERIMETRO_ALQUILABLE   AUX2
JOIN REM01.ACT_ACTIVO                     ACT
  ON ACT.ACT_NUM_ACTIVO||'' = AUX2.ID_HAYA  
...
---------------------------------------------------------------------------------------------------------------
 --MERGE FINAL PARA ACTIVOS CON TITULO Y OCUPADOS QUE NO ESTÉN VENDIDOS Y NO ESTÉN EN LA EXCEL Y NO SEAN DE SAREB.
 ---------------------------------------------------------------------------------------------------------------
MERGE INTO REM01.ACT_SPS_SIT_POSESORIA SPS
USING 
(
    ...
    WHERE   SPS.SPS_CON_TITULO = 1       
        AND SPS.SPS_OCUPADO = 1   
        AND ACT.BORRADO = 0     
        AND SCM.DD_SCM_CODIGO <> '05'
        AND CRA.DD_CRA_CODIGO NOT IN ('02')
