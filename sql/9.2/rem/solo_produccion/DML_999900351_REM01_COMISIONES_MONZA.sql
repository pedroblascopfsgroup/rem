...
BEGIN
	...
	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
					...
					join '||V_ESQUEMA||'.act_ofr ao on ao.act_id = gex.act_id and ao.OFR_ID = ofr.ofr_id
					...
					join '||V_ESQUEMA||'.DD_TPH_TIPO_PROV_HONORARIO tph on tph.dd_tph_id = gex.dd_tph_id) T2 ON (T1.GEX_ID = T2.GEX_ID)
					WHEN MATCHED THEN UPDATE SET 
					T1.GEX_IMPORTE_FINAL = T2.ACT_OFR_IMPORTE * 1/100 * (CASE WHEN T1.DD_ACC_ID = (SELECT ACC.DD_ACC_ID FROM '||V_ESQUEMA||'.DD_ACC_ACCION_GASTOS ACC WHERE ACC.DD_ACC_CODIGO = ''04'')
																		THEN 0.75
