...
--## INCIDENCIA_LINK=REMVIP-2276
--## PRODUCTO=SI
--##
--## Finalidad: Intercambiar datos de activos 
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN
	...
	USING (SELECT BIE_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA2||' ACT WHERE ACT.ACT_NUM_ACTIVO = 6135785) A
	ON (DAT.BIE_ID = A.BIE_ID)
	WHEN MATCHED THEN
	UPDATE SET DAT.BIE_DREG_NUM_FINCA = 98441
	, USUARIOMODIFICAR = ''REMVIP_2276''
	, FECHAMODIFICAR = SYSDATE';

	V_MSQL2 := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DAT
	USING (SELECT BIE_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA2||' ACT WHERE ACT.ACT_NUM_ACTIVO = 6135289) A
	ON (DAT.BIE_ID = A.BIE_ID)
	WHEN MATCHED THEN
	UPDATE SET DAT.BIE_DREG_NUM_FINCA = 98585
	, USUARIOMODIFICAR = ''REMVIP_2276''
	, FECHAMODIFICAR = SYSDATE';
