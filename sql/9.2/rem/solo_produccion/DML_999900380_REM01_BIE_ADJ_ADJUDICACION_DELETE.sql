...
BEGIN
	...
	IF V_KOUNT = 1 THEN 
		...
		V_SQL := 'DELETE FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE BIE_ID = '||BIE_ID;
		EXECUTE IMMEDIATE V_SQL;
		
		DBMS_OUTPUT.PUT_LINE('ELIMINADO EL REGISTRO CON BIE_ID '||BIE_ID||' EN '||V_TABLA);
		
		V_COUNT_UPDATE := V_COUNT_UPDATE + 1;
	
	ELSE
		
		DBMS_OUTPUT.PUT_LINE('[INFO] El registro con BIE_ID '||BIE_ID||' no existe en '||V_TABLA);
