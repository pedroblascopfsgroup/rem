...
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZACION DE LA COLUMNA ACT_NUM_ACTIVO_SAREB');
	
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO T1
			   USING (SELECT *
					  FROM AUX_REMVIP_2432_2) T2
			   ON (T1.ACT_NUM_ACTIVO = T2.ID_ACTIVO_HAYA)
			   WHEN MATCHED THEN
				   UPDATE SET T1.ACT_NUM_ACTIVO_SAREB = T2.ID_SAREB_CORRECTO,
