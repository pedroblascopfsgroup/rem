...
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZACION DE LA COLUMNA ACT_RECOVERY_ID');
	
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO T1
			   USING (SELECT *
					  FROM AUX_REMVIP_2432_3) T2
			   ON (T1.ACT_NUM_ACTIVO = T2.ACT_NUM_ACTIVO)
			   WHEN MATCHED THEN
				   UPDATE SET T1.ACT_RECOVERY_ID = T2.ACT_RECOVERY_ID_CORRECTO,
