...
DECLARE
  ...
  V_MSQL VARCHAR2(10000 CHAR);
  V_TABLA VARCHAR2(100 CHAR) := ''; -- Variable para tabla de salida para el borrado
  V_USUARIO VARCHAR2(100 CHAR) := 'REMVIP-2463';
...
BEGIN
    ...
    V_MSQL := 'UPDATE '||V_ESQUEMA||'.CEX_COMPRADOR_EXPEDIENTE SET
                ...
                WHERE
                ECO_ID = (SELECT ECO_ID FROM '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL WHERE ECO_NUM_EXPEDIENTE = 127217) 
                AND
                COM_ID = (SELECT COM_ID FROM '||V_ESQUEMA||'.COM_COMPRADOR WHERE COM_DOCUMENTO = ''23231535V'')';
