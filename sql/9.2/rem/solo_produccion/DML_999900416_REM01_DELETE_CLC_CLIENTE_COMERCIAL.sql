...
DECLARE
    ...
    V_USUARIO VARCHAR2(32 CHAR):= 'REMVIP-2611';
...
--BORRAR OFR, VIS Y CLC
		...
		V_SQL := 'UPDATE '||V_ESQUEMA||'.OFR_OFERTAS SET 
			  BORRADO = 1,
			  USUARIOBORRAR = ''REMVIP-2611'',
			  FECHABORRAR = SYSDATE
			  WHERE CLC_ID = (SELECT CLC_ID FROM CLC_CLIENTE_COMERCIAL WHERE CLC_EMAIL = ''manuelsan2002@hotmail.com'')';
		...
		V_SQL := 'UPDATE '||V_ESQUEMA||'.CLC_CLIENTE_COMERCIAL SET
			  ...
			  USUARIOBORRAR = ''REMVIP-2611'',
			  FECHABORRAR = SYSDATE
			  WHERE CLC_EMAIL = ''manuelsan2002@hotmail.com''';
