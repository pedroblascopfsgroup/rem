...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-2645'; -- USUARIOCREAR/USUARIOMODIFICAR.
    V_ECO_ID NUMBER(16); 
    
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZACION COLUMNA ACT_NUM_ACTIVO');
	
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ACT_ACTIVO WHERE ACT_NUM_ACTIVO = 113287';
	...
	IF V_NUM_FILAS = 1 THEN
		...
		V_MSQL := 'UPDATE '||V_ESQUEMA||'.ACT_ACTIVO 
				   SET ACT_NUM_ACTIVO = 999113287, 
				   FECHAMODIFICAR = SYSDATE, 
				   USUARIOMODIFICAR = '''||V_USR||''' 
				   WHERE ACT_NUM_ACTIVO = 113287';
