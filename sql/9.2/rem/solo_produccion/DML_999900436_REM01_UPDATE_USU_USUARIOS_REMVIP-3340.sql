...
--## PRODUCTO=NO
--##
--## Finalidad: Borrado logico de usuarios en la USU_USUARIOS
...
BEGIN	
    ...
    LOOP
		...
		DBMS_OUTPUT.PUT_LINE('[INFO] Comprobamos que existe el usuario '||TRIM(V_TMP_USUARIO(1))||'.');

		V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_USUARIO(1))||''' AND BORRADO = 0';
		EXECUTE IMMEDIATE V_SQL INTO V_EXISTE_USUARIO;

		IF V_EXISTE_USUARIO = 1 THEN

            DBMS_OUTPUT.PUT_LINE('[INFO] Se va a aplicar el borrado lógico sobre el usuario '||TRIM(V_TMP_USUARIO(1))||'.');
            ...
            DBMS_OUTPUT.PUT_LINE('[INFO] Se ha borrado el usuario '||TRIM(V_TMP_USUARIO(1))||'.');
