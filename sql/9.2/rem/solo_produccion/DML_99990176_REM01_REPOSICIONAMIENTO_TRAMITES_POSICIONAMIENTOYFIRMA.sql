...
SELECT OFR.OFR_ID, ECO.ECO_ID, TBJ.TBJ_ID, TAR.TAR_ID, TEX.TEX_ID, TRA.TRA_ID, TOF.DD_TOF_CODIGO, ACT.ACT_ID
        ...
        JOIN REM01.MIG2_OFR_OFERTAS MIG ON MIG.OFR_COD_OFERTA = OFR.OFR_NUM_OFERTA
        ...
        WHERE MIG.ofr_cod_estado_oferta = ''01-06'' and MIG.ofr_fecha_anulacion is null';
    EXECUTE IMMEDIATE V_MSQL;
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] APROVISIONANDO LA TABLA AUXILIAR '||V_TABLA||'...');
      ...
      EXECUTE IMMEDIATE '
            ...
            JOIN REM01.TAP_TAREA_PROCEDIMIENTO TAP ON TAP.TAP_CODIGO = ''T013_PosicionamientoYFirma'' AND BORRADO = 0';
      ...
      EXECUTE IMMEDIATE 'MERGE INTO REM01.ECO_EXPEDIENTE_COMERCIAL T1
        USING (SELECT DISTINCT T1.ECO_ID
            FROM rem01.ECO_EXPEDIENTE_COMERCIAL T1
            JOIN REM01.OFR_OFERTAS OFR ON OFR.OFR_ID = T1.OFR_ID
            JOIN REM01.MIG2_TRAMITES_OFERTAS_REP T2 ON T1.OFR_ID = T2.OFR_ID
            JOIN REM01.MIG2_OFR_OFERTAS T3 ON T3.OFR_COD_OFERTA = OFR.OFR_NUM_OFERTA
            where t3.ofr_cod_oferta not in (''67131051'',''68888725'',''69950253'',''70057658'',''70116131'',''71146874''
            ,''70628147'',''71966930'',''72006242'',''72135113'',''72495053'',''72491756'',''72334616'',''71456758'',''72925302'')) T2
        ON (T1.ECO_ID = T2.ECO_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.ECO_ESTADO_PBC = 1';

      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL actualizada (ECO_ESTADO_PBC = 1). '||SQL%ROWCOUNT||' Filas.');
    ...
    EXECUTE IMMEDIATE 'MERGE INTO REM01.ECO_EXPEDIENTE_COMERCIAL T1
        USING (SELECT DISTINCT T1.ECO_ID
            ...
            JOIN REM01.MIG2_TRAMITES_OFERTAS_REP T2 ON T1.OFR_ID = T2.OFR_ID
            JOIN REM01.MIG2_OFR_OFERTAS T3 ON T3.OFR_COD_OFERTA = OFR.OFR_NUM_OFERTA
            where t3.ofr_cod_oferta in (''67131051'',''68888725'',''69950253'',''70057658'',''70116131'',''71146874''
            ,''70628147'',''71966930'',''72006242'',''72135113'',''72495053'',''72491756'',''72334616'',''71456758'',''72925302'')) T2
        ON (T1.ECO_ID = T2.ECO_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.ECO_ESTADO_PBC = NULL';

      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL actualizada (ECO_ESTADO_PBC = NULL). '||SQL%ROWCOUNT||' Filas.');
      
    REM01.ALTA_BPM_INSTANCES('MIG_BANKIA',PL_OUTPUT);
