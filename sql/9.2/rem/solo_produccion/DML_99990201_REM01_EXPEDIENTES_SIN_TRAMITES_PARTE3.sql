...
DECLARE
      ...
      V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-3454';
...
BEGIN
      ...
      ---------------------------------------------------------------------------------------------------------------
      -- INSERT MIG2_TRAMITES_OFERTAS_REP --
      ---------------------------------------------------------------------------------------------------------------
      ...
      EXECUTE IMMEDIATE '
            ...
            WITH OFERTAS_VALIDAS AS (
                  ...
                  WHERE ECO.TBJ_ID IS NULL AND EEC.DD_EEC_CODIGO IN (''10'',''06'',''11'',''04'')
                  AND ECO.ECO_NUM_EXPEDIENTE IN (
                   ...
                   -- Pte. Sanción || Reservado || Aprobado || Contraofertado
            )
            SELECT
                  OV.OFR_ID
                  , OV.ACT_ID
      ...
      ---------------------------------------------------------------------------------------------------------------
      -- UPDATE MIG2_TRAMITES_OFERTAS_REP (USU_ID, SUP_ID) --
      ---------------------------------------------------------------------------------------------------------------
      ...
      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.'||V_TABLA_ECO||' actualizada. '||SQL%ROWCOUNT||' Filas.');
