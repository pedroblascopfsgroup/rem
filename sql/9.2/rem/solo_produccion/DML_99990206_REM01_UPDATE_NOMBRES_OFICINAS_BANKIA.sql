...
BEGIN	
	...
	EXECUTE IMMEDIATE '
	MERGE INTO ACT_PVE_PROVEEDOR T1 
	USING (
	    WITH T1 AS(
		...
		JOIN '||V_ESQUEMA||'.DD_TPR_TIPO_PROVEEDOR DD ON DD.DD_TPR_ID = PVE.DD_TPR_ID AND DD.DD_TPR_CODIGO = ''28''
	      ) SELECT CONCAT(CONCAT(T1.PVE_NOMBRE, '' - '') , T1.PVE_NOMBRE_COMERCIAL) AS NOMBRE_FINAL, T1.PVE_NOMBRE, T1.PVE_NOMBRE_COMERCIAL, T1.PVE_ID FROM T1
