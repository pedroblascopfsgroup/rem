...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-115';
    ERR_NUM NUMBER;-- Numero de errores
    ERR_MSG VARCHAR2(2048);-- Mensaje de error
    V_MSQL VARCHAR2(4000 CHAR);
    COUNTER NUMBER(2);
    COD_ITEM VARCHAR2(50 CHAR) := 'REMVIP-115';
...
BEGIN
    ...
    V_MSQL := 'INSERT INTO REM01.OFERTAS_REPOSICIONAR (OFR_ID,ECO_ID,TBJ_ID,TAR_ID,TEX_ID,TRA_ID,DD_TOF_CODIGO,ACT_ID)
		SELECT OFR.OFR_ID, ECO.ECO_ID, TBJ.TBJ_ID, TAR.TAR_ID, TEX.TEX_ID, TRA.TRA_ID, TOF.DD_TOF_CODIGO, ACT.ACT_ID
		        ...
		        WHERE ACT.ACT_NUM_ACTIVO IN (5956249,5929523)';
    ...
    EXECUTE IMMEDIATE 'MERGE INTO REM01.ECO_EXPEDIENTE_COMERCIAL T1
        USING REM01.DD_EEC_EST_EXP_COMERCIAL T2
        ON (T2.DD_EEC_CODIGO = ''06'')
        WHEN MATCHED THEN UPDATE SET
            T1.DD_EEC_ID = T2.DD_EEC_ID, T1.USUARIOMODIFICAR = ''REMVIP-115'', T1.FECHAMODIFICAR = SYSDATE
        WHERE T1.ECO_NUM_EXPEDIENTE = ''86155'' AND T1.DD_EEC_ID <> T2.DD_EEC_ID';

      DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL actualizado estado expediente comercial a Reservado para expediente 86155. '||SQL%ROWCOUNT||' Fila.');
      
    REM01.ALTA_BPM_INSTANCES('REMVIP-115',PL_OUTPUT);
