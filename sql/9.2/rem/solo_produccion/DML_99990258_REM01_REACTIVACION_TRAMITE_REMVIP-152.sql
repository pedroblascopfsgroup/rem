...
DECLARE
  ...
  ORDEN NUMBER(2) := 2;
  V_TABLA VARCHAR2(30 CHAR) := 'TAR_TAREAS_NOTIFICACIONES'; -- Variable para tabla de salida para el borrado
  V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-152';
...
BEGIN
    ...
    LOOP
        V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
            USING (SELECT TAR.TAR_ID
                ...
                JOIN REM01.ACT_OFR AO ON AO.ACT_ID = ACT.ACT_ID
                JOIN REM01.OFR_OFERTAS OFR ON OFR.OFR_ID = AO.OFR_ID
                ...
                WHERE ACT.ACT_NUM_ACTIVO_UVEM = ''9982459'' AND OFR.OFR_NUM_OFERTA = ''70198636''
                    AND TAR.TAR_TAREA = ''Resolución expediente'' AND TAR.BORRADO = 1 
                    AND TAR.USUARIOBORRAR = ''sbertomeu'') T2
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.TEV_TAREA_EXTERNA_VALOR T1
        USING (SELECT TEV.TEV_ID
            ...
            WHERE ACT.ACT_NUM_ACTIVO_UVEM = ''9982459'' AND OFR.OFR_NUM_OFERTA = ''70198636''
                AND TAR.TAR_TAREA = ''Posicionamiento y firma'' AND TAR.BORRADO = 1 
                AND TAR.USUARIOBORRAR = ''sbertomeu'') T2
        ON (T1.TEV_ID = T2.TEV_ID)
        WHEN MATCHED THEN UPDATE SET
            T1.BORRADO = 0
        DELETE WHERE T1.TEV_ID = T2.TEV_ID';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('  [INFO] Borrado tarea externa valor de posicionamiento y firma reabierta. ('||SQL%ROWCOUNT||')');
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.TEX_TAREA_EXTERNA T1
        ...
        WHEN MATCHED THEN UPDATE SET 
            T1.TEX_TOKEN_ID_BPM = NULL, T1.BORRADO = 0, T1.USUARIOBORRAR = NULL, T1.FECHABORRAR = NULL';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('  [INFO] Tarea externa de posicionamiento y firma reabierta. ('||SQL%ROWCOUNT||')');
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
        ...
        ON (T1.TAR_ID = T2.TAR_ID)
        WHEN MATCHED THEN UPDATE SET 
            T1.TAR_FECHA_FIN = NULL, T1.TAR_TAREA_FINALIZADA = 0, T1.BORRADO = 0, T1.USUARIOBORRAR = NULL, T1.FECHABORRAR = NULL';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('  [INFO] Tarea de posicionamiento y firma reabierta. ('||SQL%ROWCOUNT||')');
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_TRA_TRAMITE T1
        USING (SELECT TRA.TRA_ID
            ...
            WHERE ACT.ACT_NUM_ACTIVO_UVEM = ''9982459'' AND OFR.OFR_NUM_OFERTA = ''70198636'') T2
        ON (T1.TRA_ID = T2.TRA_ID)
        WHEN MATCHED THEN UPDATE SET 
            T1.DD_EPR_ID = (SELECT DD_EPR_ID FROM '||V_ESQUEMA_M||'.DD_EPR_ESTADO_PROCEDIMIENTO WHERE DD_EPR_CODIGO = ''10'')
            , T1.TRA_FECHA_FIN = NULL, T1.TRA_PROCESS_BPM = NULL, T1.USUARIOMODIFICAR = '''||V_USUARIO||'''
            , T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('  [INFO] Trámite reabierto. ('||SQL%ROWCOUNT||')');
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL T1
        USING (SELECT ECO.ECO_ID
            ...
            JOIN '||V_ESQUEMA||'.DD_EEC_EST_EXP_COMERCIAL EEC ON EEC.DD_EEC_ID = ECO.DD_EEC_ID
                AND EEC.DD_EEC_CODIGO <> ''01''
            WHERE ACT.ACT_NUM_ACTIVO_UVEM = ''9982459'' AND OFR.OFR_NUM_OFERTA = ''70198636'') T2
        ON (T1.ECO_ID = T2.ECO_ID)
        WHEN MATCHED THEN UPDATE SET 
            T1.DD_EEC_ID = (SELECT DD_EEC_ID FROM '||V_ESQUEMA||'.DD_EEC_EST_EXP_COMERCIAL WHERE DD_EEC_CODIGO = ''01'')
            , T1.ECO_FECHA_ANULACION = NULL, T1.ECO_PETICIONARIO_ANULACION = NULL, T1.ECO_FECHA_DEV_ENTREGAS = NULL
            , T1.ECO_IMP_DEV_ENTREGAS = NULL, T1.ECO_BLOQUEADO = 1
            , T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('  [INFO] Expediente reabierto. ('||SQL%ROWCOUNT||')');
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.OFR_OFERTAS T1
        ...
        WHEN MATCHED THEN UPDATE SET 
            T1.DD_EOF_ID = (SELECT DD_EOF_ID FROM '||V_ESQUEMA||'.DD_EOF_ESTADOS_OFERTA WHERE DD_EOF_CODIGO = ''01'')
            , T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('  [INFO] Oferta reabierta. ('||SQL%ROWCOUNT||')');
    
    DBMS_OUTPUT.PUT_LINE('[FIN] Borrado de tarea de Resolución de expediente y reapertura de expediente RADOUANE NAZILI.');
