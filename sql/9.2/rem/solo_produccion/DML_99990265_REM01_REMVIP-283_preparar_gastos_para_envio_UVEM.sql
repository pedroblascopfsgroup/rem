...
DECLARE
	...
	V_USUARIO VARCHAR2(32 CHAR):= 'REMVIP-285';
   ...
   EXECUTE IMMEDIATE   'MERGE INTO REM01.GPV_GASTOS_PROVEEDOR T1
         USING (SELECT GPV.GPV_ID
             ...
             WHERE GPV.GPV_ID IN (select gpv.gpv_id 
                                  from REM01.GPV_GASTOS_PROVEEDOR gpv 
                                  join REM01.GGE_GASTOS_GESTION gge on gpv.gpv_id = gge.GPV_ID
                                  left join rem01.act_pve_proveedor pve on gpv.PVE_ID_EMISOR = pve.PVE_ID
                                  left JOIN rem01.DD_TDI_TIPO_DOCUMENTO_ID TDI ON TDI.DD_TDI_ID = PVE.DD_TDI_ID 
                                  where gpv.prg_id is null
                                   and gge.gge_motivo_rechazo_prop =''No existe el proveedor''
                                   and pve.pve_cod_uvem is not null)
             ) T2
         ON (T1.GPV_ID = T2.GPV_ID)
         WHEN MATCHED THEN UPDATE SET
             T1.USUARIOMODIFICAR = ''REMVIP-285'', T1.FECHAMODIFICAR = SYSDATE
             , T1.DD_EGA_ID = (SELECT DD_EGA_ID FROM REM01.DD_EGA_ESTADOS_GASTO WHERE DD_EGA_CODIGO = ''03'')
         WHERE T1.DD_EGA_ID <> (SELECT DD_EGA_ID FROM REM01.DD_EGA_ESTADOS_GASTO WHERE DD_EGA_CODIGO = ''03'')';
