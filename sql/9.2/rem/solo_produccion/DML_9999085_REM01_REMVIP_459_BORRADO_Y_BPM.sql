...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-459';
...
BEGIN
	...
	V_REGISTROS := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
	    USING (SELECT TAR.TAR_ID
            ...
            JOIN REM01.TAC_TAREAS_ACTIVOS TAC ON TAC.ACT_ID = ACT.ACT_ID AND TAC.TRA_ID = TRA.TRA_ID
            ...
            WHERE ACT.ACT_NUM_ACTIVO = ''5932576'' AND ECO.ECO_NUM_EXPEDIENTE = ''85755''
                AND TAP.TAP_CODIGO = ''T013_ResolucionExpediente'') T2
	    ON (T1.TAR_ID = T2.TAR_ID)
	    WHEN MATCHED THEN UPDATE SET
	        T1.USUARIOBORRAR = '''||V_USUARIO||''', T1.BORRADO = 1, T1.FECHABORRAR = SYSDATE
        WHERE NVL(T1.USUARIOBORRAR,'' '') <> '''||V_USUARIO||'''';
	...
	V_MSQL := 'MERGE INTO REM01.TAR_TAREAS_NOTIFICACIONES T1
    USING (
        SELECT TAR.TAR_ID
        FROM REM01.ACT_ACTIVO ACT
        JOIN REM01.ACT_OFR AO ON AO.ACT_ID = ACT.ACT_ID
        ...
        JOIN REM01.TAP_TAREA_PROCEDIMIENTO TAP ON TAP.TAP_ID = TEX.TAP_ID
        WHERE ACT.ACT_NUM_ACTIVO = ''5932576'' AND ECO.ECO_NUM_EXPEDIENTE = ''85755''
            AND TAP.TAP_CODIGO = ''T013_ResultadoPBC'') T2
    ON (T1.TAR_ID = T2.TAR_ID)
    WHEN MATCHED THEN UPDATE SET
        T1.TAR_FECHA_FIN = NULL, T1.TAR_TAREA_FINALIZADA = 0, T1.USUARIOBORRAR = NULL
        , T1.FECHABORRAR = NULL, T1.BORRADO = 0, T1.USUARIOMODIFICAR = ''REMVIP-459''
        , T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
    
    DBMS_OUTPUT.PUT_LINE(SQL%ROWCOUNT||' filas fusionadas en TAR_TAREAS_NOTIFICACIONES');
    
    V_MSQL := 'MERGE INTO REM01.TEX_TAREA_EXTERNA T1
    USING (
        SELECT TEX.TEX_ID
        FROM REM01.ACT_ACTIVO ACT
        JOIN REM01.ACT_OFR AO ON AO.ACT_ID = ACT.ACT_ID
    ...
    WHEN MATCHED THEN UPDATE SET
        T1.TEX_TOKEN_ID_BPM = NULL, T1.USUARIOBORRAR = NULL
        , T1.FECHABORRAR = NULL, T1.BORRADO = 0, T1.USUARIOMODIFICAR = ''REMVIP-459''
        , T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
        
    DBMS_OUTPUT.PUT_LINE(SQL%ROWCOUNT||' filas fusionadas en TEX_TAREA_EXTERNA');
    ...
    WHEN MATCHED THEN UPDATE SET
        T1.TRA_PROCESS_BPM = NULL, T1.USUARIOBORRAR = NULL
        , T1.FECHABORRAR = NULL, T1.BORRADO = 0, T1.USUARIOMODIFICAR = ''REMVIP-459''
        , T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
        
    DBMS_OUTPUT.PUT_LINE(SQL%ROWCOUNT||' filas fusionadas en ACT_TRA_TRAMITE');
        
    REM01.ALTA_BPM_INSTANCES('REMVIP-459',PL_OUTPUT);
