...
BEGIN	
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD T1
			USING(
				...
				JOIN '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_ID = GEE.DD_TGE_ID AND TGE.BORRADO = 0 
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST T1
			USING(
				...
				JOIN '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_ID = GEH.DD_TGE_ID AND TGE.BORRADO = 0 
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Se lanza el SP de reasignacion de gestores de publicación.');

	'||V_ESQUEMA||'.SP_AGA_ASIGNA_GESTOR_PUBLI('SP_ASIGNA_GPUBL_SPUBL',PL_OUTPUT,NULL);
	DBMS_OUTPUT.PUT_LINE(PL_OUTPUT);

	DBMS_OUTPUT.PUT_LINE('[INFO] Se han ejecutado correctamente el SP de reasignacion de gestores de publicación.');
