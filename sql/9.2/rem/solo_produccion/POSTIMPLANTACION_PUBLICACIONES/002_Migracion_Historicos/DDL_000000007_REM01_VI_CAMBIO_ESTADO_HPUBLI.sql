...
BEGIN
      ...
      V_MSQL := 'CREATE OR REPLACE VIEW ' || V_ESQUEMA || '.V_CAMBIO_ESTADO_HPUBLI 
	    AS
        SELECT DISTINCT APU.AHP_ID
    ...
    DBMS_OUTPUT.PUT_LINE('CREATE VIEW '|| V_ESQUEMA ||'.V_CAMBIO_ESTADO_HPUBLI...Creada OK');
  
    -- Creamos comentarios  
    V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI IS ''Vista que se usa en el cálculo del estado de la publicación (SP_CAMBIO_ESTADO_PUBLICACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;  
    ...
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.CODIGO_ESTADO_A IS ''Código del estado de la publicación del alquiler (DD_EPA_ESTADO_PUB_ALQUILER)'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DESC_ESTADO_A IS ''Descripción del estado de la publicación del alquiler (DD_EPA_ESTADO_PUB_ALQUILER)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.CHECK_PUBLICAR_A IS ''Check de publicar el alquiler'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.CHECK_OCULTAR_A IS ''Check ocultado para alquiler'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DD_MTO_CODIGO_A IS ''Código del motivo ocultación del alquiler (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DD_MTO_MANUAL_A IS ''Si es manual el motivo ocultación del alquiler (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.CODIGO_ESTADO_V IS ''Código del estado de la publicación de la venta (DD_EPV_ESTADO_PUB_VENTA)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DESC_ESTADO_V IS ''Descripción del estado de la publicación de la venta (DD_EPV_ESTADO_PUB_VENTA)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.CHECK_PUBLICAR_V IS ''Check de publicar la venta'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.CHECK_OCULTAR_V IS ''Check ocultado para venta'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DD_MTO_CODIGO_V IS ''Código del motivo ocultación de la venta (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DD_MTO_MANUAL_V IS ''Si es manual el motivo ocultación de la venta (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DD_TPU_CODIGO_A IS ''Código del tipo de publicación alquiler (DD_TPU_TIPO_PUBLICACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DD_TPU_CODIGO_V IS ''Código del tipo de publicación venta (DD_TPU_TIPO_PUBLICACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.DD_TAL_CODIGO IS ''Código del tipo de de alquiler (DD_TAL_TIPO_ALQUILER)'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.ADMISION IS ''Admisión 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.GESTION IS ''Gestión 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.INFORME_COMERCIAL IS ''Informe comercial 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.PRECIO_A IS ''Precio alquiler 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.PRECIO_V IS ''Precio venta 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.CEE_VIGENTE IS ''CEE (Certificado de eficiencia energética) 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;  
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.ADECUADO IS ''Adecuado 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;                         

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI.ES_CONDICONADO IS ''Campo calculado en la vista V_COND_DISPONIBILIDAD'' ';      
    EXECUTE IMMEDIATE V_MSQL;         
END;
/
