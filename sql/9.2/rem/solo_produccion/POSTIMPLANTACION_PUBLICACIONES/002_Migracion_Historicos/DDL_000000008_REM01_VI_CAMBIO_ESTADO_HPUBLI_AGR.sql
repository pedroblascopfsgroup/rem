...
BEGIN
      ...
      V_MSQL := 'CREATE OR REPLACE VIEW ' || V_ESQUEMA || '.V_CAMBIO_ESTADO_HPUBLI_AGR 
	    AS
        SELECT AGR_ID
             ...
             , MIN(ES_CONDICONADO) ES_CONDICONADO
 FROM(
 SELECT DISTINCT AGR.AGR_ID
             ...
             , APU.AHP_CHECK_PUBLICAR_A CHECK_PUBLICAR_A
             , APU.AHP_CHECK_OCULTAR_A CHECK_OCULTAR_A 
             ...
             , APU.AHP_CHECK_PUBLICAR_V CHECK_PUBLICAR_V
             , APU.AHP_CHECK_OCULTAR_V CHECK_OCULTAR_V 
                       ...
                       FROM (SELECT ACT2.act_id,
                                   CASE
                                      ...
                                      WHEN NVL (tpc.dd_tpc_codigo, ''00'') = ''03'' /*Aprobado de renta (web)*/ AND NVL (tco.dd_tco_codigo, ''00'') IN (''02'', ''03'', ''04'')
                                         THEN 1
                                      WHEN APU.AHP_CHECK_PUB_SIN_PRECIO_A = 1
                                         THEN 1   
                                      ELSE 0
                                   END AS precio_A,
                              ...
                              JOIN '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION APU ON APU.ACT_ID = ACT2.ACT_ID AND APU.BORRADO = 0
                              JOIN '|| V_ESQUEMA ||'.act_val_valoraciones val ON val.act_id = ACT2.act_id AND val.borrado = 0
                              JOIN '|| V_ESQUEMA ||'.DD_TPC_TIPO_PRECIO tpc ON tpc.dd_tpc_id = val.dd_tpc_id AND tpc.dd_tpc_codigo IN (''02'', ''03'') AND TPC.BORRADO = 0 
          ...
          LEFT JOIN (SELECT act_id, precio_V
                       FROM (SELECT ACT2.act_id,
                                   CASE
                                      ...
                                      WHEN NVL (tpc.dd_tpc_codigo, ''00'') = ''03'' /*Aprobado de renta (web)*/ AND NVL (tco.dd_tco_codigo, ''00'') IN (''02'', ''03'', ''04'')
                                         THEN 1
                                      WHEN APU.AHP_CHECK_PUB_SIN_PRECIO_V = 1
                                         THEN 1   
                                      ELSE 0
                                   END AS precio_V,
                                   tpc.dd_tpc_codigo, tco.dd_tco_codigo,
    ...
    DBMS_OUTPUT.PUT_LINE('CREATE VIEW '|| V_ESQUEMA ||'.V_CAMBIO_ESTADO_HPUBLI_AGR...Creada OK');
  
    -- Creamos comentarios  
    V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR IS ''Vista que se usa en el cálculo del estado de la publicación para agrupaciones restringuidas (SP_CAMBIO_ESTADO_PUBLICACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;  
      
    -- Creamos comentarios     
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.AGR_ID IS ''Identificador único de la agrupación'' ';      
    EXECUTE IMMEDIATE V_MSQL;
 
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TCO_CODIGO IS ''Código del tipo de comercializacion (DD_TCO_TIPO_COMERCIALIZACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;
        
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.CODIGO_ESTADO_A IS ''Código del estado de la publicación del alquiler (DD_EPA_ESTADO_PUB_ALQUILER)'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DESC_ESTADO_A IS ''Descripción del estado de la publicación del alquiler (DD_EPA_ESTADO_PUB_ALQUILER)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_PUBLICAR_A IS ''Check de publicar el alquiler'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_OCULTAR_A IS ''Check ocultado para alquiler'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_CODIGO_A IS ''Código del motivo ocultación del alquiler (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_MANUAL_A IS ''Si es manual el motivo ocultación del alquiler (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.CODIGO_ESTADO_V IS ''Código del estado de la publicación de la venta (DD_EPV_ESTADO_PUB_VENTA)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DESC_ESTADO_V IS ''Descripción del estado de la publicación de la venta (DD_EPV_ESTADO_PUB_VENTA)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_PUBLICAR_V IS ''Check de publicar la venta'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_OCULTAR_V IS ''Check ocultado para venta'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_CODIGO_V IS ''Código del motivo ocultación de la venta (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_MANUAL_V IS ''Si es manual el motivo ocultación de la venta (DD_MTO_MOTIVOS_OCULTACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TPU_CODIGO_A IS ''Código del tipo de publicación alquiler (DD_TPU_TIPO_PUBLICACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TPU_CODIGO_V IS ''Código del tipo de publicación venta (DD_TPU_TIPO_PUBLICACION)'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TAL_CODIGO IS ''Código del tipo de de alquiler (DD_TAL_TIPO_ALQUILER)'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.ADMISION IS ''Admisión 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.GESTION IS ''Gestión 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.INFORME_COMERCIAL IS ''Informe comercial 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.PRECIO_A IS ''Precio alquiler 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;

    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.PRECIO_V IS ''Precio venta 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.CEE_VIGENTE IS ''CEE (Certificado de eficiencia energética) 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;  
    
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.ADECUADO IS ''Adecuado 0/1'' ';      
    EXECUTE IMMEDIATE V_MSQL;                         
 
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.V_CAMBIO_ESTADO_HPUBLI_AGR.ES_CONDICONADO IS ''Campo calculado en la vista V_COND_DISPONIBILIDAD'' ';      
    EXECUTE IMMEDIATE V_MSQL;        
END;
/
