...
--##		0.3 Modificado el DML con los cambios del SP
--##########################################
--*/
...
declare
    pAGR_ID NUMBER(16):= NULL;
    pCondAlquiler VARCHAR2(50 CHAR):= 1;
    pUSUARIOMODIFICAR VARCHAR2(50 CHAR):= 'SP_CAMBIO_EST_HPUB_AGR';
    ...
    nAGR_ID           #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.AGR_ID%TYPE;
    vDD_TCO_CODIGO    #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TCO_CODIGO%TYPE;
    vCODIGO_ESTADO_A  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.CODIGO_ESTADO_A%TYPE;
    vDESC_ESTADO_A    #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DESC_ESTADO_A%TYPE;
    vCHECK_PUBLICAR_A #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_PUBLICAR_A%TYPE;
    vCHECK_OCULTAR_A  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_OCULTAR_A%TYPE;
    vDD_MTO_CODIGO_A  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_CODIGO_A%TYPE;    
    vDD_MTO_MANUAL_A  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_MANUAL_A%TYPE; 
    vCODIGO_ESTADO_V  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.CODIGO_ESTADO_V%TYPE;
    vDESC_ESTADO_V    #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DESC_ESTADO_V%TYPE;
    vCHECK_PUBLICAR_V #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_PUBLICAR_V%TYPE;
    vCHECK_OCULTAR_V  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.CHECK_OCULTAR_V%TYPE;
    vDD_MTO_CODIGO_V  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_CODIGO_V%TYPE;
    vDD_MTO_MANUAL_V  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_MTO_MANUAL_V%TYPE; 
    vDD_TPU_CODIGO_A  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TPU_CODIGO_A%TYPE;
    vDD_TPU_CODIGO_V  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TPU_CODIGO_V%TYPE;
    vDD_TAL_CODIGO	  #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.DD_TAL_CODIGO%TYPE;	
    nADMISION         #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.ADMISION%TYPE;
    nGESTION          #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.GESTION%TYPE;
    nINFORME_COMERCIAL #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.INFORME_COMERCIAL%TYPE;
    nPRECIO_A         #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.PRECIO_A%TYPE;
    nPRECIO_V         #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.PRECIO_V%TYPE;
    nCEE_VIGENTE      #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.CEE_VIGENTE%TYPE;
    nADECUADO         #ESQUEMA#.V_CAMBIO_ESTADO_HPUBLI_AGR.ADECUADO%TYPE;
    nES_CONDICONADO   #ESQUEMA#.V_CAMBIO_ESTADO_PUBLI.ES_CONDICONADO%TYPE;
    
    OutOCULTAR        #ESQUEMA#.ACT_AHP_HIST_PUBLICACION.AHP_CHECK_OCULTAR_A%TYPE;
  ...
  BEGIN
	--V0.2
    V_MSQL := '
      MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
          USING '||vQUERY_SINACT||'
          ON (ACT.ACT_ID = AUX.ACT_ID)
        WHEN MATCHED THEN
          UPDATE 
             SET AHP_MOT_OCULTACION_MANUAL_A = NULL
  ...
  BEGIN
    V_MSQL := '
      MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
        ...
        WHEN MATCHED THEN
          UPDATE 
             SET AHP_MOT_OCULTACION_MANUAL_V = NULL
               , USUARIOMODIFICAR = '''||pUSUARIOMODIFICAR||'''
               , FECHAMODIFICAR = SYSDATE
          WHERE BORRADO = 0
              ';  
      ...
      MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
        ...
        WHEN MATCHED THEN
          UPDATE 
						SET DD_MTO_A_ID = (SELECT DD_MTO_ID
						  ...
						  , AHP_CHECK_OCULTAR_A = '||pOCULTAR||'
						  , USUARIOMODIFICAR = '''||pUSUARIOMODIFICAR||'''
						  , FECHAMODIFICAR = SYSDATE
						  , AHP_MOT_OCULTACION_MANUAL_A = NULL
          WHERE BORRADO = 0
						AND (AHP_CHECK_OCULTAR_A <> '||pOCULTAR||'
        ...
        V_MSQL := '
          MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
            ...
            WHEN MATCHED THEN
              UPDATE 
                SET AHP_MOT_OCULTACION_MANUAL_A = (SELECT substr(PAC.PAC_MOTIVO_PUBLICAR,1,250)
                                   FROM '|| V_ESQUEMA ||'.ACT_PAC_PERIMETRO_ACTIVO PAC
                                  WHERE PAC.ACT_ID = ACT.ACT_ID
                                    AND PAC.BORRADO = 0)
                  , USUARIOMODIFICAR = '''||pUSUARIOMODIFICAR||'''
                  , FECHAMODIFICAR = SYSDATE
                  , AHP_MOT_OCULTACION_MANUAL_V = NULL
              WHERE BORRADO = 0
                  ';     
        ...
        V_MSQL := '      
          MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
            ...
            WHEN MATCHED THEN
              UPDATE 
                SET AHP_MOT_OCULTACION_MANUAL_A = (SELECT SUBSTR(PAC.PAC_MOT_EXCL_COMERCIALIZAR,1,250)
                                   FROM '|| V_ESQUEMA ||'.ACT_PAC_PERIMETRO_ACTIVO PAC
                                  WHERE PAC.ACT_ID = ACT.ACT_ID
                                    AND PAC.BORRADO = 0)
                  , USUARIOMODIFICAR = '''||pUSUARIOMODIFICAR||'''
	...
	IF pTIPO = 'V' THEN
            ...
            WHEN MATCHED THEN
              UPDATE 
                SET DD_MTO_V_ID = (SELECT DD_MTO_ID
                          ...
                          WHERE BORRADO = 0
                            AND DD_MTO_CODIGO = '''||pDD_MTO_CODIGO||''')
                  , AHP_CHECK_OCULTAR_V = '||pOCULTAR||'
                  , USUARIOMODIFICAR = '''||pUSUARIOMODIFICAR||'''
                  , FECHAMODIFICAR = SYSDATE
              WHERE BORRADO = 0
						AND (AHP_CHECK_OCULTAR_V <> '||pOCULTAR||'
        ...
        V_MSQL := '      
          MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
            ...
            WHEN MATCHED THEN
              UPDATE 
                SET AHP_MOT_OCULTACION_MANUAL_V = (SELECT substr(PAC.PAC_MOTIVO_PUBLICAR,1,250)
                                   FROM '|| V_ESQUEMA ||'.ACT_PAC_PERIMETRO_ACTIVO PAC
                                  WHERE PAC.ACT_ID = ACT.ACT_ID
                                    AND PAC.BORRADO = 0)
                  , USUARIOMODIFICAR = '''||pUSUARIOMODIFICAR||'''
        ...
        V_MSQL := '      
          MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
            ...
            WHEN MATCHED THEN
              UPDATE 
                SET AHP_MOT_OCULTACION_MANUAL_V = (SELECT SUBSTR(PAC.PAC_MOT_EXCL_COMERCIALIZAR,1,250)
                                   FROM '|| V_ESQUEMA ||'.ACT_PAC_PERIMETRO_ACTIVO PAC
                                  WHERE PAC.ACT_ID = ACT.ACT_ID
                                    AND PAC.BORRADO = 0)
                  , USUARIOMODIFICAR = '''||pUSUARIOMODIFICAR||'''
		...
		IF pDD_MTO_CODIGO = '06' THEN /*Revisión Publicación*/
		  vACTUALIZAR_COND := 'N';
		  #ESQUEMA#.SP_CREAR_AVISO (nAGR_ID, 'GPUBL', pUSUARIOMODIFICAR, 'Se ha situado en Oculto Venta con motivo Revisión Publicación la agrupación: ', 1);
		END IF; 		
      ...
      IF pPRECIO = 1 THEN
        IF pADMISION = 1 AND pGESTION = 1 THEN
          ...
          END IF;
			...
			/*PUBLICADO FORZADO*/
			V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION ACT
						USING '||vQUERY_SINACT||'
						ON (ACT.ACT_ID = AUX.ACT_ID)
						WHEN MATCHED THEN
							UPDATE
  ...
  BEGIN
      ...
      IF pUSUARIOMODIFICAR IS NULL THEN
        vUSUARIOMODIFICAR := 'SP_CAMBIO_EST_HPUB_AGR';
      ELSE
      ...
      V_MSQL := '
        SELECT DISTINCT
          ...
          FROM '|| V_ESQUEMA ||'.AUX_CAMBIO_ESTADO_HPUBLI_AGR V'
          ||vWHERE
       ;
      ...
      LOOP
        ...
        IF vDD_TCO_CODIGO IN ('02','03','04') THEN
          IF (vCODIGO_ESTADO_A = '03' AND vCHECK_PUBLICAR_A = 1) THEN
            #ESQUEMA#.SP_MOTIVO_OCULTACION_HA_AGR (nAGR_ID, 'A', OutOCULTAR, OutMOTIVO);
        ...
        IF vDD_TCO_CODIGO IN ('01','02') THEN
          IF (vCODIGO_ESTADO_V = '03' AND vCHECK_PUBLICAR_V = 1) THEN
            #ESQUEMA#.SP_MOTIVO_OCULTACION_HV_AGR (nAGR_ID, 'V', OutOCULTAR, OutMOTIVO);
				...
				WHEN MATCHED THEN
				  UPDATE 
					 SET ACT.ES_CONDICONADO_ANTERIOR = '||nES_CONDICONADO||'
					   ...
					   , FECHAMODIFICAR = SYSDATE
				  WHERE BORRADO = 0
				    AND ACT.ES_CONDICONADO_ANTERIOR <> '||nES_CONDICONADO||'
					  '; 		    
		    EXECUTE IMMEDIATE V_MSQL; 
