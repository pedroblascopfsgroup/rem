...
DECLARE
    ...
    TABLA_AUXILIAR VARCHAR2(50 CHAR) := 'AUX_ACTIVOS_TRASP_PRINEX';
...
FROM '||V_ESQUEMA||'.MIG_ACA_CABECERA_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO_PRINEX = MIG.ACT_NUMERO_PRINEX'; 
...
FROM '||V_ESQUEMA||'.MIG_ADA_DATOS_ADI_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ATI_TITULO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_APL_PLANDINVENTAS_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ADJ_JUDICIAL_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ADJ_NO_JUDICIAL_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_APA_PROP_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ACA_CARGAS_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ALA_LLAVES_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ATA_TASACIONES_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_AIA_INFCOMERCIAL_ACT_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ACA_CALIDADES_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_APC_PRECIO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_ACA_CATASTRO_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_AID_INFCOMERCIAL_DISTR_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_AML_MOVIMIENTOS_LLAVE_CJ MIG
...
FROM '||V_ESQUEMA||'.MIG_APC_PROP_CABECERA_CJ MIG
...
FROM '||V_ESQUEMA||'.MIG_ADD_ADMISION_DOC_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG2_ACT_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG2_ACT_PRP_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_PRP_ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG2_ACT_HEP_HIST_EST_PUBLI_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.HEP_ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG2_PAC_PERIMETRO_ACTIVO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.PAC_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG2_PRP_PROPUESTAS_PRECIOS_CJ MIG
...
FROM '||V_ESQUEMA||'.MIG2_ACT_HVA_HIST_VALORACIO_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.HVA_ACT_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG2_PRH_PROP_HIST_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.PRH_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG2_GEA_GESTORES_ACTIVOS_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.GEA_NUMERO_ACTIVO'; 
...
FROM '||V_ESQUEMA||'.MIG_AOA_OBSERVACIONES_ACTIV_CJ MIG
JOIN '||V_ESQUEMA||'.'||TABLA_AUXILIAR||' AUX ON AUX.ACT_NUM_ACTIVO = MIG.ACT_NUMERO_ACTIVO'; 
