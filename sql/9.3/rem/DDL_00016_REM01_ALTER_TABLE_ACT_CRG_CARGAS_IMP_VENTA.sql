...
--## INCIDENCIA_LINK=HREOS-7783
--## PRODUCTO=NO
--## Finalidad: Añadir nuevo campo en la tabla ACT_CRG_CARGA
--##           
...
DECLARE
	...
	V_TABLA_REF_SRA VARCHAR2(50 CHAR):= 'DD_SIN_SINO';
	V_TABLA_REF_COL VARCHAR2(50 CHAR):= 'DD_SIN_ID';
	V_KEY_NAME_SRA VARCHAR2(50 CHAR):= 'FK_CRG_IMP_VEN_DD_SIN';
	
    --Nombre de las columnas
        V_COL_SCG VARCHAR2(50 CHAR):= 'CRG_IMPIDE_VENTA'; 
  ...
  IF V_NUM_TABLAS > 0 THEN  
            ...
            IF V_NUM_TABLAS = 0 THEN
                ...
                V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD CONSTRAINT '||V_KEY_NAME_SRA||' FOREIGN KEY ('||V_COL_SCG||')
	  								REFERENCES '||V_ESQUEMA_M||'.'||V_TABLA_REF_SRA||' ('||V_TABLA_REF_COL||') ON DELETE SET NULL ENABLE';
                EXECUTE IMMEDIATE V_MSQL;
	  			-- Añadimos el comentario al campo
	  			V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.'||V_COL_SCG||' IS ''Id que marca si se impide o no la venta''';
                EXECUTE IMMEDIATE V_MSQL; 					
            ELSE
