...
--## INCIDENCIA_LINK=HREOS-9083
--## PRODUCTO=NO
--## Finalidad: DDL Creación de la tabla CPS_CONFIG_SUBPTDAS_PRE
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(150 CHAR):= 'CPS_CONFIG_SUBPTDAS_PRE'; -- Vble. con el nombre de la tabla.
    ...
    END IF; 
      ...
      DBMS_OUTPUT.PUT_LINE('[CREAMOS '||V_TABLA||']');
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
        		CPS_ID                     		NUMBER(16,0)        GENERATED BY DEFAULT AS IDENTITY
				 ,CCC_ID                 		NUMBER(16,0)		
				 ,CPS_DESCRIPCION               VARCHAR2(250 CHAR)  NOT NULL
				 ,CPS_PARTIDA_PRESUPUESTARIA	VARCHAR2(50 CHAR) 
				 ...
				 ,CONSTRAINT PK_CPS_ID PRIMARY KEY(CPS_ID)
			  )';        	

		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.'||V_TABLA||'... Tabla creada');
		
		-- Creamos foreign key CCC_ID
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT FK_CCC_CONFIG_CTAS_CONTABLES FOREIGN KEY (CCC_ID) REFERENCES '||V_ESQUEMA||'.CCC_CONFIG_CTAS_CONTABLES (CCC_ID) ON DELETE SET NULL)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_CCC_CONFIG_CTAS_CONTABLES... Foreign key creada.');
		
    EXECUTE IMMEDIATE 'COMMENT ON TABLE  ' || V_ESQUEMA || '.'||V_TABLA||' IS ''Tabla para cuentas contables y partidas presupuestarias.''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA || '.'||V_TABLA||'.CPS_ID IS ''ID único del registro de la tabla''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA || '.'||V_TABLA||'.CCC_ID IS ''ID relacional a la tabla CCC_CONFIG_CTAS_CONTABLES''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA || '.'||V_TABLA||'.CPS_PARTIDA_PRESUPUESTARIA IS ''Partida presupuestaria''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA || '.'||V_TABLA||'.CPS_DESCRIPCION IS ''Descripción del registro de la tabla''';
