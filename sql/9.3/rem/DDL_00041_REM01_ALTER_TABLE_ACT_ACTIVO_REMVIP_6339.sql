...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('DD_OAN_ID', 'NUMBER(16,0)', 'Código identificador único del diccionario DD_OAN_ORIGEN_ANTERIOR.'),
        T_TIPO_DATA('ACT_FECHA_TITULO_ANTERIOR', 'DATE', 'Fecha del título anterior del activo.')
		); 
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO CAMPOS]');
  	...
  	IF V_NUM_TABLAS > 0 THEN
		...
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT FK_DD_OAN_ID FOREIGN KEY (DD_OAN_ID) REFERENCES '||V_ESQUEMA||'.DD_OAN_ORIGEN_ANTERIOR (DD_OAN_ID) ON DELETE SET NULL)';
		EXECUTE IMMEDIATE V_MSQL;
