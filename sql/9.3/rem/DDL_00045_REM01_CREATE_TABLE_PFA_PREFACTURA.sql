...
--## INCIDENCIA_LINK=HREOS-10524
--## PRODUCTO=NO
--## 
--## Finalidad: Creación de tabla PFA_PREFACTURA
...
DECLARE
	...
	V_TABLA VARCHAR2(2400 CHAR) := 'PFA_PREFACTURA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_PK_COLUMN VARCHAR2(500 CHAR):= 'PFA_ID'; -- Vble. para la columna PK
	V_UK_COLUMN VARCHAR2(500 CHAR):= 'PFA_NUM_PREFACTURA'; -- Vble. para la columna UK
	
	TYPE T_FK IS TABLE OF VARCHAR2(150);
	TYPE T_ARRAY_FK IS TABLE OF T_FK;
	V_FK T_ARRAY_FK := T_ARRAY_FK(
	-- 	COLUMN 				COLUMN 				TABLE 						FK_NAME
		T_FK('PVE_ID', 		'PVE_ID', 			'ACT_PVE_PROVEEDOR', 		'FK_PFA_PVE_ID'), 
		T_FK('PRO_ID', 		'PRO_ID', 			'ACT_PRO_PROPIETARIO', 		'FK_PFA_PRO_ID'),
		T_FK('DD_TTR_ID', 	'DD_TTR_ID', 		'DD_TTR_TIPO_TRABAJO', 		'FK_PFA_DD_TTR_ID'),
		T_FK('DD_STR_ID', 	'DD_STR_ID', 		'DD_STR_SUBTIPO_TRABAJO', 	'FK_PFA_DD_STR_ID'),
		T_FK('DD_EPF_ID', 	'DD_EPF_ID', 		'DD_EPF_ESTADO_PREFACTURA', 'FK_PFA_DD_EPF_ID'),
		T_FK('GPV_ID', 		'GPV_ID', 			'GPV_GASTOS_PROVEEDOR', 	'FK_PFA_GPV_ID')
	); 
	V_TMP_FK T_FK;
	...
	-- 	COLUMN 							COMMENT
		T_COMMENT('PFA_ID', 				'Id de la prefactura.'), 
		T_COMMENT('PFA_NUM_PREFACTURA', 	'Número de la prefactura.'),
		T_COMMENT('PFA_FECHA_PREFACTURA', 	'Fecha de alta de la prefactura.'),
		T_COMMENT('PVE_ID', 				'Proveedor.'),
		T_COMMENT('PRO_ID', 				'Propietario.'),
		T_COMMENT('DD_TTR_ID', 				'Tipo de los trabajos de la prefactura.'),
		T_COMMENT('DD_STR_ID', 				'Subtipo de los trabajos de la prefactura.'),
		T_COMMENT('DD_EPF_ID', 				'Estado de la prefactura.'),
		T_COMMENT('GPV_ID', 				'Gasto asociado a la prefactura.'),
...
BEGIN
	...
	V_SQL := 'CREATE TABLE ' ||V_ESQUEMA||'.'||V_TABLA||' (
		...
		PFA_NUM_PREFACTURA 		NUMBER(16,0) 			NOT NULL ENABLE, 
		PFA_FECHA_PREFACTURA 	DATE 					NOT NULL ENABLE,
		PVE_ID					NUMBER(16,0) 			NOT NULL ENABLE,
		PRO_ID					NUMBER(16,0) 			NOT NULL ENABLE,
		DD_TTR_ID				NUMBER(16,0) 			NOT NULL ENABLE,
		DD_STR_ID				NUMBER(16,0) 			NOT NULL ENABLE,
		DD_EPF_ID				NUMBER(16,0) 			NOT NULL ENABLE,
