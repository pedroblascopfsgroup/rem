...
DECLARE
	...
	-- COLUMN 								TYPE 					DEFAULT
		T_VAR('GPV_SUPLIDOS_VINCULADOS', 	'NUMBER(16,0)', 		'DEFAULT 2', 'Marca si el gasto suplido está vinculado mediante el DD_SIN_SINO'), 
		T_VAR('GPV_NUMERO_FACTURA_PPAL', 	'VARCHAR2(128 CHAR)', '' ,	'Número de factura principal')
	); 
	V_TMP_VAR T_VAR;
...
BEGIN
	...
	FOR I IN V_VAR.FIRST .. V_VAR.LAST LOOP
		...
		V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||TRIM(V_TMP_VAR(1))||' IS '''||TRIM(V_TMP_VAR(4))||'''';
		EXECUTE IMMEDIATE V_SQL;
