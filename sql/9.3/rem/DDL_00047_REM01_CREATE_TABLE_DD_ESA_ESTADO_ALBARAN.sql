...
--## PRODUCTO=NO
--## 
--## Finalidad: Creación de tabla DD_ESA_ESTADO_ALBARAN
...
DECLARE
	...
	V_TABLA VARCHAR2(2400 CHAR) := 'DD_ESA_ESTADO_ALBARAN'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_PK_COLUMN VARCHAR2(500 CHAR):= 'DD_ESA_ID'; -- Vble. para la columna PK
	V_UK_COLUMN VARCHAR2(500 CHAR):= 'DD_ESA_CODIGO'; -- Vble. para la columna UK
	
	--TYPE T_FK IS TABLE OF VARCHAR2(150);
	--TYPE T_ARRAY_FK IS TABLE OF T_FK;
	--V_FK T_ARRAY_FK := T_ARRAY_FK(
	---- 	COLUMN 					COLUMN 			TABLE 						FK_NAME
	--	T_FK('DD_ESA_ID', 		'DD_ESA_ID', 	'DD_ESA_ESTADO_ALBARAN', 	'FK_ALB_DD_ESA_ID')
	--); 
	--V_TMP_FK T_FK;
	...
	-- 	COLUMN 										COMMENT
		T_COMMENT('DD_ESA_ID', 						'Id del diccionario.'), 
		T_COMMENT('DD_ESA_CODIGO', 					'Código del diccionario (UK).'),
		T_COMMENT('DD_ESA_DESCRIPCION', 			'Descripción.'),
		T_COMMENT('DD_ESA_DESCRIPCION_LARGA', 		'Descripción larga.'),
...
BEGIN
	...
	V_SQL := 'CREATE TABLE ' ||V_ESQUEMA||'.'||V_TABLA||' (
		DD_ESA_ID 						NUMBER(16,0) 			NOT NULL, 
		DD_ESA_CODIGO 					VARCHAR2(20CHAR) 		NOT NULL ENABLE, 
		DD_ESA_DESCRIPCION 				VARCHAR2(100CHAR) 		NOT NULL ENABLE,
		DD_ESA_DESCRIPCION_LARGA 		VARCHAR2(250CHAR) 		NOT NULL ENABLE,
