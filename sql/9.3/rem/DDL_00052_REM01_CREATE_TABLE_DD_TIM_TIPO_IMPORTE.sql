...
--## PRODUCTO=NO
--## Finalidad: Creación diccionario DD_TIM_TIPO_IMPORTE
--##           
...
DECLARE
    ...
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla de tipos de importe'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	ELSE
		...
		(
			DD_TIM_ID                  	NUMBER(16) NOT NULL,
			DD_TIM_CODIGO              	VARCHAR2(20 CHAR) NOT NULL,
			DD_TIM_DESCRIPCION         	VARCHAR2(100 CHAR),
			DD_TIM_DESCRIPCION_LARGA   	VARCHAR2(250 CHAR),
		...
		-- Creamos indice	
		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.DD_TIM_TIPO_IMPORTE_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(DD_TIM_ID ASC) TABLESPACE '||V_TABLESPACE_IDX;		
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.DD_TIM_TIPO_IMPORTE_PK... Indice creado.');
		
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT DD_TIM_TIPO_IMPORTE_PK PRIMARY KEY (DD_TIM_ID) USING INDEX)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.DD_TIM_TIPO_IMPORTE_PK... PK creada.');

		-- Creamos UK constraint
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT UK_DD_TIM_CODIGO UNIQUE (DD_TIM_CODIGO)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] UK_DD_TIM_CODIGO... UK creada.');
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TIM_ID IS ''identificador único del tipos de importe''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_TIM_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TIM_CODIGO IS ''Código identificador único del tipos de importe''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_TIM_CODIGO creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TIM_DESCRIPCION IS ''Descripción del tipos de importe''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_TIM_DESCRIPCION creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TIM_DESCRIPCION_LARGA IS ''Descripción larga del tipos de importe''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_TIM_DESCRIPCION_LARGA creado.');
