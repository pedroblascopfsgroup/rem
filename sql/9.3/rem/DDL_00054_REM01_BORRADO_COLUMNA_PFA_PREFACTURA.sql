...
DECLARE
	...
	V_BORRAR_FK VARCHAR2(2 CHAR) := 'SI'; -- [SI, NO] Vble. para indicar al script si debe o no crear tambien las relaciones Foreign Keys.
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'GPV_ID',			'NUMBER(16,0)',				'Id de la tabla PREFACTURA'	)
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_PFA_GPV_ID',						'GPV_ID',					V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR',		'GPV_ID'			)
    );
    V_T_FK T_FK;
		...
		LOOP
			...
			IF V_NUM_TABLAS = 0 THEN
				...
				V_MSQL := '
					...
					DROP CONSTRAINT '||V_T_FK(1)||' 
				';

				EXECUTE IMMEDIATE V_MSQL;
				--DBMS_OUTPUT.PUT_LINE('[3] '||V_MSQL);
				DBMS_OUTPUT.PUT_LINE('[INFO] ... '||V_T_FK(1)||' BORRADA la FK en columna '||V_T_FK(2)||' ... OK');
