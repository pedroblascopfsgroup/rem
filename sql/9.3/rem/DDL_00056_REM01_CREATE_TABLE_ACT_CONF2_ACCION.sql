...
BEGIN
	...
	ELSE
		...
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT AC2_DD_COS_FK FOREIGN KEY (DD_COS_ID) REFERENCES '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB(DD_COS_ID)';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT UK_DD_COS_ACS_BORRADO UNIQUE (DD_COS_ID, DD_ACS_ID, BORRADO)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] UK_DD_COS_ACS_BORRADO... UK creada.');
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_COS_ID IS ''Relación con DD_COS_CAMPOS_ORIGEN_CONV_SAREB''';
		EXECUTE IMMEDIATE V_MSQL;
