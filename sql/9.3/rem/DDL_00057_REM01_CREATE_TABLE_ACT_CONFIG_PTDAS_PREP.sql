...
DECLARE
    ...
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla en la que se almacenaran las configuraciones de las partidas que corresponden a los importes de tasas, recargas e intereses de un gasto'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	(
	    ...
	    , "EJE_ID"                      NUMBER(16, 0)
	        NOT NULL ENABLE
	    , "CPP_ARRENDAMIENTO"           NUMBER(1, 0) DEFAULT 0
	    , "CPP_REFACTURABLE"            NUMBER(1, 0) DEFAULT 0
	        NOT NULL ENABLE
	    ...
	    , "BORRADO"                     NUMBER(1, 0) DEFAULT 0
	        NOT NULL ENABLE
	    , "CPP_PRINCIPAL"               NUMBER(1, 0) DEFAULT 0
	        NOT NULL ENABLE
	    , "DD_TBE_ID"                   NUMBER(16, 0)
	    , "CPP_APARTADO"                VARCHAR2(50 CHAR)
	    , "CPP_CAPITULO"                VARCHAR2(50 CHAR)
	    , "CPP_ACTIVABLE"               NUMBER(1, 0) DEFAULT 0
	        NOT NULL ENABLE
	    , "CPP_PLAN_VISITAS"            NUMBER(1, 0) DEFAULT 0
	        NOT NULL ENABLE
	    , "DD_TCH_ID"                   NUMBER(16, 0)
	    , "DD_TRT_ID"                   NUMBER(16, 0)
	    , "CPP_VENDIDO"                 NUMBER(1, 0) DEFAULT 0
	    , CONSTRAINT "PK_ACT_CONFIG_PTDAS_PREP" PRIMARY KEY ( "CPP_PTDAS_ID" )
	    ...
	    ENABLE
	    , CONSTRAINT "CK_CPP_CAMPOS_LBK" CHECK ( ( DD_CRA_ID = 43
	       AND CPP_PLAN_VISITAS IN ( 0, 1 ) )
	        OR ( DD_CRA_ID <> 43
	       AND DD_TBE_ID IS NULL
	       AND DD_TCH_ID IS NULL
	       AND CPP_PLAN_VISITAS = 0 ) ) ENABLE
	    , CONSTRAINT "CK_CPP_ACTIVABLE" CHECK ( ( DD_CRA_ID IN ( 43, 162 )
	       AND CPP_ACTIVABLE IN ( 0, 1 ) )
	        OR ( DD_CRA_ID NOT IN ( 43, 162 )
	       AND CPP_ACTIVABLE = 0 ) ) ENABLE
	    , CONSTRAINT "CK_CPP_CON_ACTIVOS" CHECK ( ( DD_SCR_ID IS NOT NULL
	       AND CPP_ARRENDAMIENTO IS NOT NULL
	       AND CPP_VENDIDO IS NOT NULL )
	        OR ( DD_SCR_ID IS NULL
	       AND CPP_ARRENDAMIENTO IS NULL
	       AND CPP_VENDIDO IS NULL ) ) ENABLE
	    , CONSTRAINT "UK_CPP_PTDAS_PREP" UNIQUE ( "DD_TGA_ID"
	    , "DD_STG_ID"
	    , "DD_TIM_ID"
	    , "DD_CRA_ID"
	    , "DD_SCR_ID"
	    ...
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_STG_ID" FOREIGN KEY ( "DD_STG_ID" )
	        REFERENCES '||V_ESQUEMA||'."DD_STG_SUBTIPOS_GASTO" ( "DD_STG_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_TGA_ID" FOREIGN KEY ( "DD_TGA_ID" )
	        REFERENCES '||V_ESQUEMA||'."DD_TGA_TIPOS_GASTO" ( "DD_TGA_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_TIM_ID" FOREIGN KEY ( "DD_TIM_ID" )
	        REFERENCES '||V_ESQUEMA||'."DD_TIM_TIPO_IMPORTE" ( "DD_TIM_ID" )
	            ON DELETE SET NULL
	    ENABLE
	    , CONSTRAINT "FK_CPP_PRO_ID" FOREIGN KEY ( "PRO_ID" )
	        REFERENCES '||V_ESQUEMA||'."ACT_PRO_PROPIETARIO" ( "PRO_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_CRA_ID" FOREIGN KEY ( "DD_CRA_ID" )
	        REFERENCES '||V_ESQUEMA||'."DD_CRA_CARTERA" ( "DD_CRA_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_SCR_ID" FOREIGN KEY ( "DD_SCR_ID" )
	        REFERENCES '||V_ESQUEMA||'."DD_SCR_SUBCARTERA" ( "DD_SCR_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_EJE_ID" FOREIGN KEY ( "EJE_ID" )
	        REFERENCES '||V_ESQUEMA||'."ACT_EJE_EJERCICIO" ( "EJE_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_TBE" FOREIGN KEY ( "DD_TBE_ID" )
	        REFERENCES '||V_ESQUEMA||'."DD_TBE_TIPO_ACTIVO_BDE" ( "DD_TBE_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_TCH" FOREIGN KEY ( "DD_TCH_ID" )
	        REFERENCES '||V_ESQUEMA||'."DD_TCH_TIPO_COMISIONADO_HRE" ( "DD_TCH_ID" )
	    ENABLE
	    , CONSTRAINT "FK_CPP_DD_TRT_ID" FOREIGN KEY ( "DD_TRT_ID" )
