...
BEGIN
	...
	ELSE
		...
		-- Creamos FK constraint
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT CCN_DD_CCS_FK FOREIGN KEY (DD_COS_ID) REFERENCES '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB(DD_COS_ID)';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_COS_ID IS ''Campo destino sobre el que se produce el cambio de valor''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_COS_ID creado.');


		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.VALOR IS ''Nuevo valor''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna VALOR creado.');
