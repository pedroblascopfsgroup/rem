...
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla CFG_PLAZOS_TAREAS
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'CFG_PLAZOS_TAREAS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'DD_CRA_ID',			'NUMBER(16,0) NOT NULL',			'Identificador único de la cartera'	),
		T_ALTER(  'DD_SCR_ID',			'NUMBER(16,0) NOT NULL',			'Identificador único de la subcartera'	)
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_CFT_CRA',			'DD_CRA_ID',		V_ESQUEMA||'.DD_CRA_CARTERA',				'DD_CRA_ID'),
		T_FK(	'FK_CFT_SCR',			'DD_SCR_ID',		V_ESQUEMA||'.DD_SCR_SUBCARTERA',				'DD_SCR_ID')
    );
    V_T_FK T_FK;

	/* -- ARRAY CON BORRADOS DE UKs */
    TYPE T_BUK IS TABLE OF VARCHAR2(4000);
    TYPE T_ARRAY_BUK IS TABLE OF T_BUK;
    V_BUK T_ARRAY_BUK := T_ARRAY_BUK(
    			--NOMBRE UK 
    	T_BUK(	'UK_CPT_TTR_STR_BORRADO')
    );
    V_T_BUK T_BUK;
    ...
    V_UK T_ARRAY_UK := T_ARRAY_UK(
    			--NOMBRE FK 						CAMPOS UK
    	T_UK(	'UK_CPT_TTR_STR_CRA_SCR_BORRADO',	'DD_TTR_ID, DD_STR_ID, DD_CRA_ID, DD_SCR_ID, BORRADO')
    );
    V_T_UK T_UK;
