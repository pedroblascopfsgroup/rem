...
DECLARE
  ...
  V_COL T_ARRAY_COL := T_ARRAY_COL(
    ...
    T_COL('CREATE_GPV_ACT_BACKUP', 'GPV_GDE_GIC_BACKUP'),
    T_COL('CREATE_GPV_TBJ_BACKUP', 'GPV_TBJ_BACKUP'),
    ...
    LOOP
        ...
        IF 'CREATE_GPV_GDE_GIC_BACKUP' = ''||V_TMP_COL(1)||'' THEN
            ...
            IF V_NUM_TABLAS = 0 THEN
                V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.GPV_GDE_GIC_BACKUP AS
        ...
        IF 'CREATE_GPV_ACT_BACKUP' = ''||V_TMP_COL(1)||'' THEN
            DBMS_OUTPUT.PUT_LINE('  [CREATE_GPV_ACT_BACKUP]');
            --Comprobacion de la tabla
            V_SQL := 'SELECT COUNT(*) FROM ALL_TABLES WHERE OWNER = '''||V_ESQUEMA||''' AND TABLE_NAME = '''||V_TMP_COL(2)||'''';
            EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

            IF V_NUM_TABLAS = 0 THEN
                V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.GPV_ACT_BACKUP AS
                            SELECT CURRENT_TIMESTAMP(6) FECHA_BACKUP, GPV_ACT.*
                            FROM '||V_ESQUEMA||'.GPV_ACT';
                EXECUTE IMMEDIATE V_MSQL;
            END IF;
        END IF;

        IF 'CREATE_GPV_TBJ_BACKUP' = ''||V_TMP_COL(1)||'' THEN
            DBMS_OUTPUT.PUT_LINE('  [CREATE_GPV_TBJ_BACKUP]');
            --Comprobacion de la tabla
            V_SQL := 'SELECT COUNT(*) FROM ALL_TABLES WHERE OWNER = '''||V_ESQUEMA||''' AND TABLE_NAME = '''||V_TMP_COL(2)||'''';
            EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

            IF V_NUM_TABLAS = 0 THEN
                V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.GPV_TBJ_BACKUP AS
                            SELECT CURRENT_TIMESTAMP(6) FECHA_BACKUP, GPV_TBJ.*
                            FROM '||V_ESQUEMA||'.GPV_TBJ';
                EXECUTE IMMEDIATE V_MSQL;
            END IF;
        END IF;
