...
--## PRODUCTO=NO
--## Finalidad: Creación tabla que nos rellenarán para casa activos con diarios ACT_ATD_ACT_TDI
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'ACT_ATD_ACT_TDI'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= ' tabla que nos rellenarán para casa activos con diarios'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	ELSE
        ...
        -- Creamos indice	
		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.IDX_ACT_ATD_ACT_TDI ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(ATD_ID,ACT_ID,TDI_ID,BORRADO) TABLESPACE '||V_TABLESPACE_IDX;		
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.IDX_ACT_ATD_ACT_TDI... Indice creado.');

        -- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT PK_ACT_ATD_ACT_TDI PRIMARY KEY (ATD_ID) USING INDEX)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.PK_ACT_ATD_ACT_TDI... PK creada.');

		-- Creamos FK constraint
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_ATD_ACT_ID FOREIGN KEY (ACT_ID) REFERENCES '||V_ESQUEMA||'.ACT_ACTIVO(ACT_ID)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] FK_ATD_ACT_ID... FK creada.');

		-- Creamos FK constraint
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_ATD_TDI_ID FOREIGN KEY (TDI_ID) REFERENCES '||V_ESQUEMA||'.ACT_TDI_TIPO_DIARIO(TDI_ID)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] FK_ATD_TDI_ID... FK creada.');
        ...
        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.ATD_ID IS ''Clave única''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna ATD_ID creado.');		

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.ACT_ID IS ''Campo que relaciona con ACT_ACTIVO''';
		EXECUTE IMMEDIATE V_MSQL;
