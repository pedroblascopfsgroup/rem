...
--## PRODUCTO=NO
--## 
--## Finalidad: Añadir columnas y FK a tablas de tipo comisionado
--##            
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  V_COL T_ARRAY_COL := T_ARRAY_COL(
    T_COL('CREATE_COLUMN', 'GIC_GASTOS_INFO_CONTABILIDAD', 'GIC_PLAN_VISITAS', 'NUMBER(16)', 'Plan Visitas'),
    T_COL('ADD_CONSTRAINT', 'GIC_GASTOS_INFO_CONTABILIDAD', 'FK_GIC_PLAN_VISITAS', 'GIC_PLAN_VISITAS','DD_SIN_SINO','DD_SIN_ID')
  );  
  V_TMP_COL T_COL;
        ...
        IF 'ADD_CONSTRAINT' = ''||V_TMP_COL(1)||'' THEN
            ...
            IF V_NUM_TABLAS = 1 THEN              
                ...
                IF V_NUM_TABLAS = 0 THEN
                    ...
                    EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TMP_COL(2)||' ADD CONSTRAINT '||V_TMP_COL(3)||' FOREIGN KEY('||V_TMP_COL(4)||') REFERENCES '||V_ESQUEMA_M||'.'||V_TMP_COL(5)||'('||V_TMP_COL(6)||')';
                ELSE
