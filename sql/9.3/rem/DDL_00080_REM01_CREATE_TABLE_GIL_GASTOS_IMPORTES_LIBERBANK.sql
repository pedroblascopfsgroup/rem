...
V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT CK_GIL_GASTOS_IMPORTES_LIBERBANK CHECK ((BORRADO = 0 AND FECHABORRAR IS NULL) OR BORRADO = 1) ENABLE)';
EXECUTE IMMEDIATE V_MSQL;
...
V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT UK_GIL_GASTOS_IMPORTES_LIBERBANK unique(GPV_ID, ENT_ID, DD_ENT_ID, BORRADO, FECHABORRAR) using index)';
EXECUTE IMMEDIATE V_MSQL;
