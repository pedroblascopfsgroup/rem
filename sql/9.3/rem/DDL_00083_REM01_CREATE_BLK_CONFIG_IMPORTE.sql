...
BEGIN
	...
	ELSE
		...
		(
			BLK_CONFIG_IMPORTE_ID           NUMBER(16, 0) 			GENERATED BY DEFAULT AS IDENTITY,
			BLK_IMPORTE_MAXIMO 				NUMBER(16, 0) 			NOT NULL,
		...
		EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM ALL_CONSTRAINTS WHERE TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||''' and CONSTRAINT_TYPE = ''P'''
			...
			ELSE
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT PK_'||V_TEXT_TABLA||' PRIMARY KEY (BLK_CONFIG_IMPORTE_ID))';
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.PK_'||V_TEXT_TABLA||'... PK creada.');
		END IF;
		-- Creamos foreign key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_BLK_CONFIG_DD_TBK 
		...
		V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' IS ''Tabla para la configuracion del importe de las ofertas Bulk.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la tabla creado.');		

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.BLK_CONFIG_IMPORTE_ID IS ''Código identificador único.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna BLK_CONFIG_IMPORTE_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.BLK_IMPORTE_MAXIMO IS ''Importe maximo del Bulk''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna BLK_IMPORTE_MAXIMO creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TBK_ID IS ''Identificador único del tipo de Bulk''';
		EXECUTE IMMEDIATE V_MSQL;
