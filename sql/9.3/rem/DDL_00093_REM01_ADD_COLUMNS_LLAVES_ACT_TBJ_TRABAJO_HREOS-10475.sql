...
--## INCIDENCIA_LINK=HREOS-10475
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla ACT_TBJ_TRABAJO para el bloque de LLAVES.
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER(  'PVC_ID_LLAVES',			'NUMBER(16,0)',			'Código identificador único de proveedor contacto de llaves.'),
    	T_ALTER(  'TBJ_FECHA_ENTREGA_LLAVES',		'DATE',				'Fecha entrega de llaves.'),
	T_ALTER(  'TBJ_NO_APLICA_LLAVES',		'NUMBER(1,0)',			'Flag que indica si aplica la posesion llaves.'),
    	T_ALTER(  'TBJ_MOTIVO_LLAVES',			'VARCHAR2(100 CHAR)',		'Motivo llaves.')
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_TBJ_PVC_LLAVES',			'PVC_ID_LLAVES',		V_ESQUEMA||'.ACT_PVC_PROVEEDOR_CONTACTO',				'PVC_ID')
    );
    V_T_FK T_FK;
