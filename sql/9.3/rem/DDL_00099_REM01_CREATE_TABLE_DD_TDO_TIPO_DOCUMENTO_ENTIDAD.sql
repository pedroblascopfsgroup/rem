...
--## PRODUCTO=NO
--##
--## Finalidad: DDL Creación de la tabla DD_TDO_TIPO_DOC_ENTIDAD
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(150 CHAR):= 'DD_TDO_TIPO_DOC_ENTIDAD'; -- Vble. con el nombre de la tabla.
    V_CODIGO_TABLA VARCHAR2(3 CHAR):= 'TDO';
...
BEGIN
    ...
    END IF;
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||'
               ...
               ,DD_TED_ID NUMBER (16,0) NOT NULL 
                  ...
                  , CONSTRAINT FK_DD_TED_ID        FOREIGN KEY (DD_TED_ID) REFERENCES DD_TED_TIP_ENTIDAD_DOC(DD_TED_ID)
			  )';
    ...
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CODIGO_TABLA||'_MATRICULA IS ''Indica la matricula.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_TED_ID IS ''FK a DD_TED_TIP_DOC''';
