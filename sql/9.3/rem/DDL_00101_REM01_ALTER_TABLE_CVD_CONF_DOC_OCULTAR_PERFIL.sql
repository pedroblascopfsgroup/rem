...
--## PRODUCTO=NO
--## 
--## Finalidad: Añadircolumnas y UK a tablas de cvd_conf_doc_ocultar_perfil
--##            
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  --Array que contiene los registros que se van a crear cvd_conf_doc_ocultar_perfil
  TYPE T_COL IS TABLE OF VARCHAR2(250);
  TYPE T_ARRAY_COL IS TABLE OF T_COL;
  V_COL T_ARRAY_COL := T_ARRAY_COL(
    T_COL('ADD_UK', 'CVD_CONF_DOC_OCULTAR_PERFIL', 'UK_CVD_CONF_DOC_OCULTAR_PERFIL','PEF_ID, DD_TDO_ID, FECHABORRAR, BORRADO'),
    T_COL('ADD_CHECK_CONSTRAINT', 'CVD_CONF_DOC_OCULTAR_PERFIL', 'CK_CVD_CONF_DOC_OCULTAR_PERFIL','(BORRADO = 0 AND FECHABORRAR IS NULL) OR BORRADO = 1')
  );  
  V_TMP_COL T_COL;
