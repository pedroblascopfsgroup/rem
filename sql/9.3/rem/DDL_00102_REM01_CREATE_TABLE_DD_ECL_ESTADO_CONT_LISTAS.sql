...
--## PRODUCTO=NO
--## Finalidad: Creación diccionario DD_ECL_ESTADO_CONT_LISTAS
--##           
...
DECLARE
    ...
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'DICCIONARIO DE ESTADO CONTRASTE LISTAS'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	ELSE
		...
		(
			...
			DD_ECL_CODIGO              	VARCHAR2(20 CHAR) NOT NULL,
			DD_ECL_DESCRIPCION         	VARCHAR2(100 CHAR),
		...
		-- Creamos indice	
		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.DD_ECL_ESTADO_CONT_LISTAS_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(DD_ECL_ID ASC) TABLESPACE '||V_TABLESPACE_IDX;		
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.DD_ECL_ESTADO_CONT_LISTAS_PK... Indice creado.');
		
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT DD_ECL_ESTADO_CONT_LISTAS_PK PRIMARY KEY (DD_ECL_ID) USING INDEX)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.DD_ECL_ESTADO_CONT_LISTAS_PK... PK creada.');

		-- Creamos UK constraint
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT UK_DD_ECL_CODIGO UNIQUE (DD_ECL_CODIGO)';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] UK_DD_ECL_CODIGO... UK creada.');
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ECL_ID IS ''Identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_ECL_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ECL_CODIGO IS ''Código identificador único ''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_ECL_CODIGO creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ECL_DESCRIPCION IS ''Descripción ''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_ECL_DESCRIPCION creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_ECL_DESCRIPCION_LARGA IS ''Descripción Larga''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] comentario de la columna DD_ECL_DESCRIPCION_LARGA creado.');
