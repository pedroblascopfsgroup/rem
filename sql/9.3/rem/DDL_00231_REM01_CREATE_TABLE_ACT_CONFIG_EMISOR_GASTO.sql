...
--## PRODUCTO=NO
--## Finalidad: Creación diccionario ACT_CONFIG_EMISOR_GASTO
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'ACT_CONFIG_EMISOR_GASTO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla en la que se almacenarán las configuraciones de los emisores de los gastos.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	(
	    "EGA_ID"                		NUMBER(16, 0)
	        NOT NULL ENABLE
	    , "EGA_FECHA_INICIO"			TIMESTAMP(6)
	    	NOT NULL ENABLE
	    , "EGA_FECHA_FIN"				TIMESTAMP(6)
	    	NOT NULL ENABLE
	    ...
	    , "PRO_ID"                      NUMBER(16, 0)	
	    , "PVE_ID"                      NUMBER(16, 0)
	    , "EGA_ARRENDAMIENTO_SOCIAL"    NUMBER(1, 0)
	    ...
	    , "BORRADO"                     NUMBER(1, 0) DEFAULT 0
	        NOT NULL ENABLE
	    , CONSTRAINT "PK_'||V_TEXT_TABLA||'" PRIMARY KEY ("EGA_ID")
	    	ENABLE
	    ...
	    , CONSTRAINT "CK_'||V_TEXT_TABLA||'" CHECK ((BORRADO = 0 AND FECHABORRAR IS NULL) OR BORRADO = 1)
	    	ENABLE
	    , CONSTRAINT "FK_EGA_DD_STR_ID" FOREIGN KEY (DD_STR_ID) REFERENCES '||V_ESQUEMA||'.DD_STR_SUBTIPO_TRABAJO (DD_STR_ID)
	    	ENABLE
	    , CONSTRAINT "FK_EGA_DD_CRA_ID" FOREIGN KEY (DD_CRA_ID) REFERENCES '||V_ESQUEMA||'.DD_CRA_CARTERA (DD_CRA_ID)
	    	ENABLE
	    , CONSTRAINT "FK_EGA_DD_SCR_ID" FOREIGN KEY (DD_SCR_ID) REFERENCES '||V_ESQUEMA||'.DD_SCR_SUBCARTERA (DD_SCR_ID)
	    	ENABLE
	    , CONSTRAINT "FK_EGA_DD_IRE_ID" FOREIGN KEY (DD_IRE_ID) REFERENCES '||V_ESQUEMA||'.DD_IRE_IDENTIFICADOR_REAM (DD_IRE_ID)
	    	ENABLE
	    , CONSTRAINT "FK_EGA_PRO_ID" FOREIGN KEY (PRO_ID) REFERENCES '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO (PRO_ID)
	    	ENABLE
	    , CONSTRAINT "FK_EGA_PVE_ID" FOREIGN KEY (PVE_ID) REFERENCES '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR (PVE_ID)
	    	ENABLE
	    , CONSTRAINT "FK_EGA_DD_TEG_ID" FOREIGN KEY (DD_TEG_ID) REFERENCES '||V_ESQUEMA||'.DD_TEG_TIPO_EMISOR_GLD (DD_TEG_ID)
	    	ENABLE
	)
	';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.EGA_ID IS ''Identificador único''';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.EGA_ARRENDAMIENTO_SOCIAL IS ''Marca si el emisor del gasto es por arrendamiento social''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna EGA_ARRENDAMIENTO_SOCIAL creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TEG_ID IS ''Identificador único del emisor del gasto''';
	EXECUTE IMMEDIATE V_MSQL;
