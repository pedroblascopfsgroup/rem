...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_EEB_ESTADO_EXPEDIENTE_BC'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	IF V_NUM_TABLAS = 0 THEN
		...
		(
			DD_EEB_ID           			NUMBER(16, 0) NOT NULL,
            DD_EEB_CODIGO               	VARCHAR2(20 CHAR),
            DD_EEB_DESCRIPCION          	VARCHAR2(100 CHAR),
            DD_EEB_DESCRIPCION_LARGA        VARCHAR2(100 CHAR),
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (DD_EEB_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EEB_ID IS ''Código identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EEB_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EEB_CODIGO IS ''Código tipo estado expediente bc''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EEB_CODIGO creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EEB_DESCRIPCION IS ''Descripción tipo estado expediente bc''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EEB_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_EEB_DESCRIPCION_LARGA IS ''Descripción larga tipo estado expediente bc''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_EEB_DESCRIPCION_LARGA creado.');
