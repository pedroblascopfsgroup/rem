...
DECLARE
    ...
    V_TABLA_FK VARCHAR2(2400 CHAR) := 'DD_PRP_PROCEDENCIA_PRODUCTO';  
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(256);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
			T_TIPO_DATA('DD_PRP_ID', 'NUMBER(16,0)', 'ID del tipo de procedencia del producto')
    ); 
      ...
      IF V_NUM_TABLAS > 0 THEN
        ...
        -- Si existe la columna cambiamos/establecemos solo la FK
          IF V_NUM_TABLAS = 1 THEN
              ...
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DROP CONSTRAINT FK_ACT_DD_PRP_ID';
              EXECUTE IMMEDIATE V_MSQL;
              DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_TMP_TIPO_DATA(1)||'... FK Dropeada');
              
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_ACT_DD_PRP_ID FOREIGN KEY (DD_PRP_ID) REFERENCES '||V_ESQUEMA||'.DD_PRP_PROCEDENCIA_PRODUCTO (DD_TS_ID) ON DELETE SET NULL)';
              EXECUTE IMMEDIATE V_MSQL;
          ...
          ELSE
              ...
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_ACT_DD_PRP_ID FOREIGN KEY (DD_PRP_ID) REFERENCES '||V_ESQUEMA||'.DD_PRP_PROCEDENCIA_PRODUCTO (DD_PRP_ID) ON DELETE SET NULL)';
              EXECUTE IMMEDIATE V_MSQL;
