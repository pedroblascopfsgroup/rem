...
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] ACTUALIZACION H_APR_AUX_I_RU_FACT_PROV');
	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        V_TMP_TIPO_DATA := V_TIPO_DATA(I);
	
		V_SQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME = '''||V_TMP_TIPO_DATA(1)||''' 
														 and TABLE_NAME = ''H_APR_AUX_I_RU_FACT_PROV''';
		
		EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
		
		IF V_NUM_TABLAS = 1 THEN
		
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.H_APR_AUX_I_RU_FACT_PROV
		...
		ELSE 
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] LA COLUMNA '||V_TMP_TIPO_DATA(1)||' NUMBER(16,2) NO EXISTE');
		
		END IF;
		
	END LOOP;
	
	DBMS_OUTPUT.PUT_LINE('[FIN] ACTUALIZADA H_APR_AUX_I_RU_FACT_PROV');
