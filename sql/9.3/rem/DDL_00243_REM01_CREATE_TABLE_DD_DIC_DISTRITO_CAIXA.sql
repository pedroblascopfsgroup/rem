...
--## PRODUCTO=NO
--##
--## Finalidad: Crear diccionario DD_DIC_DISTRITO_CAIXA
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_DIC_DISTRITO_CAIXA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	IF V_NUM_TABLAS = 0 THEN
		...
		(
            ...
            DD_DIC_CODIGO               	VARCHAR2(20 CHAR),
            DD_DIC_DESCRIPCION          	VARCHAR2(100 CHAR),
            DD_DIC_DESCRIPCION_LARGA        VARCHAR2(100 CHAR),
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (DD_DIC_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_DIC_ID IS ''Código identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_DIC_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_DIC_CODIGO IS ''Código distrito''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_DIC_CODIGO creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_DIC_DESCRIPCION IS ''Descripción distrito''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_DIC_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_DIC_DESCRIPCION_LARGA IS ''Descripción larga distrito''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_DIC_DESCRIPCION_LARGA creado.');
