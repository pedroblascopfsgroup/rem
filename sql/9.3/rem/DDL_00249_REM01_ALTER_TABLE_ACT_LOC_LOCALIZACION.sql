...
DECLARE
    ...
    V_TABLA_FK VARCHAR2(2400 CHAR) := 'DD_DIC_DISTRITO_CAIXA';  
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(256);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
			T_TIPO_DATA('DD_DIC_ID', 'NUMBER(16,0)', 'ID del distrito')
    ); 
      ...
      IF V_NUM_TABLAS > 0 THEN
        ...
        -- Si existe la columna cambiamos/establecemos solo la FK
          IF V_NUM_TABLAS = 1 THEN
              ...
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DROP CONSTRAINT FK_LOC_DD_DIC_ID';
              EXECUTE IMMEDIATE V_MSQL;
              DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_TMP_TIPO_DATA(1)||'... FK Dropeada');
              
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_LOC_DD_DIC_ID FOREIGN KEY (DD_DIC_ID) REFERENCES '||V_ESQUEMA||'.DD_DIC_DISTRITO_CAIXA (DD_DIC_ID) ON DELETE SET NULL)';
              EXECUTE IMMEDIATE V_MSQL;
