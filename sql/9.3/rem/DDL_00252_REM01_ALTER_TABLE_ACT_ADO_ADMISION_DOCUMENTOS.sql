...
--## PRODUCTO=NO
--##
--## Finalidad:        Anyadir columna a la ACT_ADO_ADMISION_DOCUMENTO
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA_FK VARCHAR2(2400 CHAR) := 'DD_LEM_LISTA_EMISIONES';  
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(256);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
			T_TIPO_DATA('DD_LEM_ID','NUMBER(16,0)','FK a Lista de emisiones')
    ); 
      ...
      IF V_NUM_TABLAS > 0 THEN
        ...
        -- Si existe la columna cambiamos/establecemos solo la FK
          IF V_NUM_TABLAS = 1 THEN
              ...
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' DROP CONSTRAINT FK_ADO_DD_LEM_ID';
              EXECUTE IMMEDIATE V_MSQL;
              DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_TMP_TIPO_DATA(1)||'... FK Dropeada');
              
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_ADO_DD_LEM_ID FOREIGN KEY (DD_LEM_ID) REFERENCES '||V_ESQUEMA||'.DD_LEM_LISTA_EMISIONES (DD_LEM_ID) ON DELETE SET NULL)';
              EXECUTE IMMEDIATE V_MSQL;
