...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_MEB_MOTIVOS_ESTADO_BC
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial - [HREOS-14438] - Sergio Gomez
--##########################################
--*/
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_MEB_MOTIVOS_ESTADO_BC'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_CHARS VARCHAR2(6 CHAR) := 'MEB';
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-14438';
	V_MSQL VARCHAR2(32000 CHAR); -- Sentencia a ejecutar 
	V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Motivos Estado BC'; -- Vble. para los comentarios de las tablas
 ...
 BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Creamos la tabla '||V_TABLA);
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_ID IS ''Código identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CHARS||'_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_CODIGO IS ''Código de tipo Motivos Estado BC''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CHARS||'_CODIGO creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_DESCRIPCION IS ''Descripción de tipo Motivos Estado BC''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_CHARS||'_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_DESCRIPCION_LARGA IS ''Descripción de tipo Motivos Estado BC''';
		EXECUTE IMMEDIATE V_MSQL;
