...
--## PRODUCTO=NO
--## Finalidad: Creación diccionario DD_BQS_BLOQUEO_SAREB
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'DD_BQS_BLOQUEO_SAREB'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Criterios para Okupaciones Ilegales'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	(
		DD_BQS_ID          			NUMBER(16)                  NOT NULL,
		DD_BQS_CODIGO        		VARCHAR2(20 CHAR)           NOT NULL,
		DD_BQS_DESCRIPCION			VARCHAR2(150 CHAR),
		DD_BQS_DESCRIPCION_LARGA	VARCHAR2(250 CHAR),
	...
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (DD_BQS_ID))';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' IS ''Tabla de criterios para Okupaciones Ilegales.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la tabla creado.');		

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_BQS_ID IS ''Identificador único ''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_BQS_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_BQS_CODIGO IS ''Código identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_BQS_CODIGO creado.');	

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_BQS_DESCRIPCION IS ''Descripción''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_BQS_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_BQS_DESCRIPCION_LARGA IS ''Descripción larga''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_BQS_DESCRIPCION_LARGA creado.');	
