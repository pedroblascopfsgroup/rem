...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        ...
        T_TIPO_DATA('ACT_PAC_PERIMETRO_ACTIVO_PK2',               'ACT_PAC_PERIMETRO_ACTIVO',               'ACT_ID, BORRADO')
    ); 
    ...
    BEGIN
        ...
        LOOP
            ...
            IF V_NUM_TABLAS = 0 THEN
                ...
                V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TMP_TIPO_DATA(1)||' ON '||V_ESQUEMA||'.'||V_TMP_TIPO_DATA(2)||' ('||V_TMP_TIPO_DATA(3)||') TABLESPACE '||V_TABLESPACE_IDX;		
            ...
            ELSE
                ...
                V_MSQL := 'DROP INDEX '||V_ESQUEMA||'.'||V_TMP_TIPO_DATA(1)||'';		
                EXECUTE IMMEDIATE V_MSQL;

                V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TMP_TIPO_DATA(1)||' ON '||V_ESQUEMA||'.'||V_TMP_TIPO_DATA(2)||' ('||V_TMP_TIPO_DATA(3)||') TABLESPACE '||V_TABLESPACE_IDX;		
                EXECUTE IMMEDIATE V_MSQL;
                DBMS_OUTPUT.PUT_LINE('[INFO] Indice '||V_TMP_TIPO_DATA(1)||' Borrado y vuelto a crear.');
