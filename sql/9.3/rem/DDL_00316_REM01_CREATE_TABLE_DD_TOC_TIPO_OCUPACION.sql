...
--## PRODUCTO=NO
--## Finalidad: Creación diccionario DD_TOC_TIPO_OCUPACION
--##           
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'DD_TOC_TIPO_OCUPACION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Diccionario que contiene los tipos de ocupación'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	(
		DD_TOC_ID           			NUMBER(16,0)           NOT NULL,
		DD_TOC_CODIGO        			VARCHAR2(20 CHAR)      NOT NULL,
		DD_TOC_DESCRIPCION			VARCHAR2(150 CHAR)     NOT NULL,
        	DD_TOC_DESCRIPCION_LARGA    		VARCHAR2(150 CHAR)     NOT NULL,
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(DD_TOC_ID) TABLESPACE '||V_TABLESPACE_IDX;		
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');
	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (DD_TOC_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TOC_ID IS ''Identificador único ''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_TOC_ID creado.');
	
	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TOC_CODIGO IS ''Código de identificador único''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_TOC_CODIGO creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TOC_DESCRIPCION IS ''Descripción''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_TOC_DESCRIPCION creado.');	

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TOC_DESCRIPCION_LARGA IS ''Descripción Larga''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_TOC_DESCRIPCION_LARGA creado.');	
