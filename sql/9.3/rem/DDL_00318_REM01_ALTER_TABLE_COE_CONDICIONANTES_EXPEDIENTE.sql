...
--## PRODUCTO=NO
--##
--## Finalidad: Actualiza la tabla COE_CONDICIONANTES_EXPEDIENTE.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(50 CHAR) := 'COE_CONDICIONANTES_EXPEDIENTE'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COL_OLD VARCHAR2(50 CHAR) := 'COE_SOLICITA_FINANCIACION';
    V_COL_NEW VARCHAR2(50 CHAR) := 'DD_TFN_ID';
	...
	IF V_COUNT = 1 THEN 
        ...
        IF V_COUNT = 1 THEN 
            ...
            IF V_COUNT = 0 THEN
                ...
                --DML_00853_REM01_UPDATE_COE_CONDICIONANTES_EXPEDIENTE.sql
        ...
        V_SQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE OWNER = '''||V_ESQUEMA||''' AND TABLE_NAME = '''||V_TABLA||''' AND COLUMN_NAME = '''||V_COL_NEW||'''';
        EXECUTE IMMEDIATE V_SQL INTO V_COUNT; 
	
        IF V_COUNT = 0 THEN 
            V_SQL := 'ALTER TABLE ' || V_ESQUEMA || '.'||V_TABLA||' ADD '||V_COL_NEW||' NUMBER (16,0)';
            EXECUTE IMMEDIATE V_SQL;

            DBMS_OUTPUT.PUT_LINE('[INFO] Añadido el campo '||V_COL_NEW||'');

            V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.'||V_COL_NEW||' IS ''FK del diccionario DD_TFN_TIPO_FINANCIACION''';
			EXECUTE IMMEDIATE V_SQL;

            DBMS_OUTPUT.PUT_LINE('[INFO] Añadido comentario en '||V_COL_NEW||'');

            V_SQL := 'ALTER TABLE ' || V_ESQUEMA || '.'||V_TABLA||' ADD 
                            CONSTRAINT FK_DD_TFN_TIPO_FINANCIACION FOREIGN KEY ('||V_COL_NEW||') 
                            REFERENCES '||V_ESQUEMA||'.DD_TFN_TIPO_FINANCIACION ('||V_COL_NEW||') ON DELETE SET NULL';
            EXECUTE IMMEDIATE V_SQL;

            DBMS_OUTPUT.PUT_LINE('[INFO] Añadida FK '||V_COL_NEW||' sobre DD_TFN_TIPO_FINANCIACION');
            
            DBMS_OUTPUT.PUT_LINE('[INFO] Actualizada la tabla '||V_TABLA);
        ELSE
            DBMS_OUTPUT.PUT_LINE('[INFO] Ya existe el campo '||V_COL_NEW||' en la tabla '||V_TABLA);
        END IF;
	ELSE
