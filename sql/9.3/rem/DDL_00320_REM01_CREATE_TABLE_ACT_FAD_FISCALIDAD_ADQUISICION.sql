...
BEGIN
	...
	(
		...
		FAD_PORCENTAJE_IMPUESTO_COMPRA				NUMBER(16,2),
		FAD_COD_TP_IVA_COMPRA						VARCHAR2(2 CHAR),
		FAD_RENUNCIA_EXENCION						NUMBER(1,0),
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_DD_SIC_ID_FAD FOREIGN KEY (DD_SIC_ID) REFERENCES '||V_ESQUEMA||'.DD_SIC_SUBTIPO_IMPUESTO_COMPRA (DD_SIC_ID))';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_DD_SIC_ID_FAD... Foreign key creada.');

	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_FAD_RENUNCIA_EXENCION FOREIGN KEY (FAD_RENUNCIA_EXENCION) REFERENCES '||V_ESQUEMA_M||'.DD_SIN_SINO (DD_SIN_ID))';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_FAD_RENUNCIA_EXENCION... Foreign key creada.');
	...
	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TIC_ID IS ''Identificador único del tipo de impuesto de la compra del activo''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_TIC_ID creado.');	

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_SIC_ID IS ''Identificador único del subtipo de impuesto de la compra del activo''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_SIC_ID creado.');	

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.FAD_PORCENTAJE_IMPUESTO_COMPRA IS ''Porcentaje del impuesto de la compra del activo''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna FAD_PORCENTAJE_IMPUESTO_COMPRA creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.FAD_COD_TP_IVA_COMPRA IS ''Código TP IVA del impuesto de la compra del activo''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna FAD_COD_TP_IVA_COMPRA creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.FAD_RENUNCIA_EXENCION IS ''Renúncia al impuesto de la compra del activo''';
	EXECUTE IMMEDIATE V_MSQL;
