...
BEGIN
	...
	ELSE
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (COR_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... PK creada.');
		
		-- Creamos unique key Para que las combinaciones de los diccionarios sea unica.
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT UK_COR_CONFIG_RECOMENDACION_RCDC UNIQUE (DD_CRA_ID, DD_SCR_ID, DD_TCO_ID, DD_EQG_ID) ENABLE';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.UK_COR_CONFIG_RECOMENDACION_RCDC... Unique key creada.');
		
		--Creamos foreign keys
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_DD_CRA_ID_COR FOREIGN KEY (DD_CRA_ID) '||
	    	'  REFERENCES  '|| V_ESQUEMA ||'.DD_CRA_CARTERA (DD_CRA_ID)';
	    	EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] FK_DD_CRA_ID_COR creada.');
		
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_DD_SCR_ID_COR FOREIGN KEY (DD_SCR_ID) '||
	    	'  REFERENCES  '|| V_ESQUEMA ||'.DD_SCR_SUBCARTERA (DD_SCR_ID)';
	    	EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] FK_DD_SCR_ID_COR creada.');

		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_DD_TCO_ID_COR FOREIGN KEY (DD_TCO_ID) '||
	    	'  REFERENCES  '|| V_ESQUEMA ||'.DD_TCO_TIPO_COMERCIALIZACION (DD_TCO_ID)';
	    	EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] FK_DD_TCO_ID_COR creada.');

		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_DD_EQG_ID_COR FOREIGN KEY (DD_EQG_ID) '||
	    	'  REFERENCES  '|| V_ESQUEMA||'.DD_EQG_EQUIPO_GESTION (DD_EQG_ID)';
	    	EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] FK_DD_EQG_ID_COR creada.');

		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_DD_REC_ID_COR FOREIGN KEY (DD_REC_ID) '||
	    	'  REFERENCES  '|| V_ESQUEMA||'.DD_REC_RECOMENDACION_RCDC (DD_REC_ID)';
	    	EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] FK_DD_REC_ID_COR creada.');
		...
		-- Creamos comentarios
		V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' IS ''Tabla para paremetrizar la obligatoriedad de configuracion de recomendación.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la tabla creado.');		
		
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.COR_ID IS ''Código identificador único de la paremetrización de la obligatoriedad de configuracion de recomendación.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna COR_ID creado.');
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.COR_PORCENTAJE_DESC IS ''Porcentaje de descuento de configuracion de recomendación.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna COR_PORCENTAJE_DESC creado.');
		
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.COR_IMPORTE_MIN IS ''Importe mínimo de configuracion de recomendación.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna COR_IMPORTE_MIN creado.');
		
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_REC_ID IS ''Código identificador único de la recomendacion rc/dc.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_REC_ID creado.');
