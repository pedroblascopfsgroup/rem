...
--## INCIDENCIA_LINK=HREOS-14113
--## PRODUCTO=NO
--## Finalidad: Creacion diccionario FAE_FECHA_ARRAS_EXPEDIENTE
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'FAE_FECHA_ARRAS_EXPEDIENTE'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla con los datos del grid de fecha de arras'; -- Vble. para los comentarios de las tablas
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_FAE_ECO_ID',                        'ECO_ID',             V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL',                'ECO_ID'),
        T_FK(   'FK_FAE_DD_APD',                   'FAE_VALIDACION_BC',       V_ESQUEMA||'.DD_APD_APRUEBA_DENIEGA',                'DD_APD_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            (
                FAE_ID          		    NUMBER(16)                  NOT NULL,
                ECO_ID        		        NUMBER(16),
                FAE_FECHA_ENVIO             DATE,
                FAE_FECHA_PROPUESTA         DATE,
                FAE_FECHA_RESPUESTA_BC      DATE,
                FAE_VALIDACION_BC           NUMBER(16),
                FAE_FECHA_AVISO             DATE,
                FAE_COMENTARIOS_BC          VARCHAR2(512 CHAR),
                FAE_OBSERVACIONES	        VARCHAR2(512 CHAR),
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (FAE_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
		    ...
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.ECO_ID IS ''Id del expediente comercial asociado''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_FECHA_ENVIO IS ''Fecha de envío de la fecha de arras''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_FECHA_PROPUESTA IS ''Fecha de propuesta de arras''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_FECHA_RESPUESTA_BC IS ''Fecha de respuesta de servicio''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_VALIDACION_BC IS ''Resultado de la validación de la fecha enviada''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_FECHA_AVISO IS ''Fecha de aviso de la fecha de arras''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_COMENTARIOS_BC IS ''Comentarios del servicio web''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FAE_OBSERVACIONES IS ''Observaciones del gestor de la fecha de arras''';
            EXECUTE IMMEDIATE V_SQL;
