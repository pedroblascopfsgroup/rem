...
--##########################################
--## AUTOR= Lara Pablo
--## FECHA_CREACION=20210516
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.3
--## INCIDENCIA_LINK=HREOS-13984
--## PRODUCTO=NO
...
DECLARE
    ...
    V_TABLA_FK VARCHAR2(2400 CHAR) := 'DD_TCR_TIPO_COMERCIALIZAR';  
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(256);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(																		--Nombre en la tabla fk
			T_TIPO_DATA('CBX_CANAL_DIST_VENTA', 		'NUMBER(16,0)', 		'Canal de distribución venta', 			'DD_TCR_ID'),
			T_TIPO_DATA('CBX_CANAL_DIST_ALQUILER', 		'NUMBER(16,0)', 		'Canal de distribución alquiler', 		'DD_TCR_ID')
      ...
      IF V_NUM_TABLAS > 0 THEN
        ...
        -- Si existe la columna cambiamos/establecemos solo la FK
          IF V_NUM_TABLAS = 1 THEN
              ...
              IF V_NUM_TABLAS = 0 THEN
 				 V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_'||TRIM(V_TMP_TIPO_DATA(1))||' FOREIGN KEY ('||TRIM(V_TMP_TIPO_DATA(1))||') REFERENCES '||V_ESQUEMA||'.'||V_TABLA_FK||' ('||TRIM(V_TMP_TIPO_DATA(4))||') ON DELETE SET NULL)';
