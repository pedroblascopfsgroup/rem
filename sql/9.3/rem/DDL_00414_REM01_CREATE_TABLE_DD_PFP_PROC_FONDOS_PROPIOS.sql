...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_PFP_PROC_FONDOS_PROPIOS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	IF V_NUM_TABLAS = 0 THEN
		...
		(
			DD_PFP_ID           			NUMBER(16, 0) NOT NULL,
            DD_PFP_CODIGO_BC               	VARCHAR2(20 CHAR),
            DD_PFP_DESCRIPCION          	VARCHAR2(100 CHAR),
            DD_PFP_DESCRIPCION_LARGA        VARCHAR2(100 CHAR),
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (DD_PFP_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_PFP_ID IS ''Código identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_PFP_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_PFP_CODIGO_BC IS ''Código BC''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_PFP_CODIGO_BC creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_PFP_DESCRIPCION IS ''Descripción''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_PFP_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_PFP_DESCRIPCION_LARGA IS ''Descripción larga''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_PFP_DESCRIPCION_LARGA creado.');
