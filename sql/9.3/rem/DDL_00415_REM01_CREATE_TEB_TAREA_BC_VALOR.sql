...
--## PRODUCTO=NO
--## 
--## Finalidad: Creación de tabla TEB_TAREA_BC_VALOR
--## VERSIONES:
--## 		0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'TEB_TAREA_BC_VALOR'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
    ...
    IF V_NUM_TABLAS = 0 THEN
        ...
        (
            TEB_ID                          NUMBER(16,0) 			NOT NULL ,
			TEX_ID           				NUMBER(16)				NOT NULL , 
            TEB_NOMBRE                      VARCHAR2(250 CHAR)		NOT NULL ,  
            TEB_VALOR       	            VARCHAR2(1000 CHAR),       
        ...
        -- Creamos primary key
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (TEB_ID))';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TABLA||'_PK... PK creada.');

        -- Creamos foreign key
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD CONSTRAINT TEB_TEX_ID_FK 
        FOREIGN KEY (TEX_ID)
        REFERENCES '||V_ESQUEMA||'.TEX_TAREA_EXTERNA (TEX_ID)';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] TEB_TEX_ID_FK ... FK creada.');
        ...
        V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla donde se guardarán los valores de las tareas que vienen por WS de BC''';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la tabla creado.');      

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.TEB_ID IS ''Código identificador único.''';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna TEB_ID creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.TEX_ID IS ''FK A la tarea''';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna TEX_ID creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.TEB_NOMBRE IS ''Nombre del campo de la tarea''';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna TEB_NOMBRE creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.TEB_VALOR IS ''Valor del campo de la tarea''';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna TEB_VALOR creado.');
