...
DECLARE
	...
	V_TABLA VARCHAR2(50 CHAR):= 'GGE_GASTOS_GESTION'; -- Nombre de la tabla a crear
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		T_TIPO_DATA('GGE_REPERCUTIDO',			'NUMBER(16,0)',			'Gestión del gasto repercutido',				'FK_GGE_REPERCUTIDO',	V_ESQUEMA_M||'.DD_SIN_SINO', 'DD_SIN_ID'),
		T_TIPO_DATA('GGE_FECHA_REPERCUSION',	'DATE',					'Fecha repercusión de la gestión del gasto',	'',	'', ''),
	  	T_TIPO_DATA('GGE_MOTIVO_RECHAZO',		'VARCHAR2(255 CHAR)',	'Motivo rechazo de la gestión del gasto',		'',	'', '')
); 
	  ...
	  FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
		LOOP
	        ...
	        IF V_NUM_TABLAS = 0 THEN
				...
				DBMS_OUTPUT.PUT_LINE('  [INFO] Campo '||V_TMP_TIPO_DATA(1)||' insertado'); 
	            ...
	            IF V_TMP_TIPO_DATA(4) != 'NO' THEN
	                ...
	                EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD CONSTRAINT '||V_TMP_TIPO_DATA(4)||' FOREIGN KEY ('||V_TMP_TIPO_DATA(1)||')
		  								REFERENCES '||V_TMP_TIPO_DATA(5)||' ('||V_TMP_TIPO_DATA(6)||') ON DELETE SET NULL ENABLE';
