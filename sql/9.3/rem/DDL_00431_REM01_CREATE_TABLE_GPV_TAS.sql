...
--## PRODUCTO=NO
--## Finalidad: Creacion tabla GPV_TAS
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'GPV_TAS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_GPV_TAS_GPV',                   'GPV_ID',               V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR',                         'GPV_ID'),
        T_FK(   'FK_GPV_TAS_TAS',                   'TAS_ID',               V_ESQUEMA||'.ACT_TAS_TASACION',                             'TAS_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (GPV_TAS_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
            ...
            -- Creamos comentario
            V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla de relación de gastos con tasaciones''';
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.GPV_TAS_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.GPV_ID IS ''Id del gasto asociado''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.TAS_ID IS ''Id de la tasación asociada''';
            EXECUTE IMMEDIATE V_SQL;
