...
--## PRODUCTO=NO
--## 
--## Finalidad: Creación de tabla ACT_ROB_RESTRINGIDA_OBREM
--## VERSIONES:
--## 		0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'ACT_ROB_RESTRINGIDA_OBREM'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	-- Creamos la tabla
		...
		(
			...
			ROB_DIRECCION				VARCHAR2(255 CHAR),
			ROB_ACREEDOR_PDV			VARCHAR2(255 CHAR),
			ROB_CP						VARCHAR2(255 CHAR)
		)
		';
	...
	-- Creamos comentario	
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.ROB_DIRECCION IS ''Dirección de la agrupación de lote restringido de alquiler''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna ROB_DIRECCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.ROB_ACREEDOR_PDV IS ''Acreedor de la agrupación de lote restringido de alquiler''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna ROB_ACREEDOR_PDV creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.ROB_CP IS ''Código postal de la agrupación de lote restringido de alquiler''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna ROB_CP creado.');	
