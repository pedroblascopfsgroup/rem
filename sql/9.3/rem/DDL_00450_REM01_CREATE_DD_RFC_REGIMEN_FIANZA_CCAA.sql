...
--## PRODUCTO=NO
--## Finalidad: Creación diccionario DD_RFC_REGIMEN_FIANZA_CCAA
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_RFC_REGIMEN_FIANZA_CCAA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	IF V_NUM_TABLAS = 0 THEN
		...
		(
			DD_RFC_ID           			NUMBER(16, 0) NOT NULL,
            DD_RFC_CODIGO               	VARCHAR2(20 CHAR),
            DD_RFC_DESCRIPCION          	VARCHAR2(100 CHAR),
            DD_RFC_DESCRIPCION_LARGA        VARCHAR2(100 CHAR),
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (DD_RFC_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_RFC_ID IS ''Código identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_RFC_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_RFC_CODIGO IS ''Código''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_RFC_CODIGO creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_RFC_DESCRIPCION IS ''Descripción ''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_RFC_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_RFC_DESCRIPCION_LARGA IS ''Descripción larga''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_RFC_DESCRIPCION_LARGA creado.');
