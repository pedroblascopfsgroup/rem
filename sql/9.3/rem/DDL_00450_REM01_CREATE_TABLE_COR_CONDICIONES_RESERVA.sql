...
--## INCIDENCIA_LINK=HREOS-14797
--## PRODUCTO=NO
--## Finalidad: Creacion tabla COR_CONDICIONES_RESERVA
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'COR_CONDICIONES_RESERVA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_COR_RES_ID',                    'RES_ID',             V_ESQUEMA||'.RES_RESERVAS',                           'RES_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            (
                COR_ID          		    NUMBER(16)                  NOT NULL,
                RES_ID        		        NUMBER(16),
                COR_INS_TITULO              NUMBER(1,0),
                COR_TANTEO_DL               NUMBER(1,0),
                COR_CARGAS                  NUMBER(1,0),
                COR_TEMAS_TECNICOS          NUMBER(1,0),
                COR_POSESION                NUMBER(1,0),
                COR_TEMAS_CATASTRALES       NUMBER(1,0),
                COR_AUTOR_VPO               NUMBER(1,0),
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (COR_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
		    ...
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.RES_ID IS ''FK a RES_RESERVAS''';
            EXECUTE IMMEDIATE V_SQL;                        
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_INS_TITULO IS ''Flag inscripción de título''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_TANTEO_DL IS ''Flag tanteo DL''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_CARGAS IS ''Flag cargas''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_TEMAS_TECNICOS IS ''Flag temas técnicos''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_POSESION IS ''Flag posesión''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_TEMAS_CATASTRALES IS ''Flag temas catastrales''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COR_AUTOR_VPO IS ''Flag autorización VPO''';
            EXECUTE IMMEDIATE V_SQL;
