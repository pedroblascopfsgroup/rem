...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
	    	...
	    	T_ALTER('COE_ADECUACIONES','NUMBER(1,0)','Adecuaciones '),
			T_ALTER('COE_CERTIFICACIONES','NUMBER(1,0)','Certificaciones'),
			T_ALTER('COE_DER_CESION_ARREND','NUMBER(1,0)','Derecho cesión y subarriendo'),
			T_ALTER('COE_FECHA_PREAVISO_VENC_CONT','DATE','Fecha preaviso vencimiento contrato'),
			T_ALTER('COE_FECHA_INGR_ARREND','DATE','Fecha ingreso fianza por parte de arrendatario'),
			T_ALTER('COE_ANT_DEUDOR_LOCALIZABLE','NUMBER(1,0)','Antiguo deudor localizable'),
			T_ALTER('COE_CNT_SUSCR_POST_ADJ','NUMBER(1,0)','Contrato suscrito con posterioridad a la adjudicación'),
			T_ALTER('COE_OFR_NUEVAS_CONDICIONES','NUMBER(1,0)','Oferta con nuevas condiciones'),
			T_ALTER('COE_ENTREGAS_CUENTA','NUMBER(1,0)','Entregas a cuenta'),
			T_ALTER('COE_IMPORTE_ENTREGAS_CUENTA','NUMBER(16,2)','Importe entregas a cuenta (€)'),
			T_ALTER('COE_RENTAS_CUENTA','NUMBER(1,0)','Rentas a cuenta'),
			T_ALTER('COE_FIANZAS_CNT_SUBROGADOS','NUMBER(1,0)','Fianza de contratos subrogados'),
			T_ALTER('COE_VULNERABILIDAD_DETECTADA','NUMBER(1,0)','Vulnerabilidad detectada')
