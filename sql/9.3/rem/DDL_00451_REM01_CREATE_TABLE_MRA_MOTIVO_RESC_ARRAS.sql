...
--## INCIDENCIA_LINK=HREOS-14798
--## PRODUCTO=NO
--## Finalidad: Creacion tabla MRA_MOTIVO_RESC_ARRAS
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'MRA_MOTIVO_RESC_ARRAS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla con los datos del bloque Motivo en la Reserva'; -- Vble. para los comentarios de las tablas
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_MRA_RES_ID',                    'RES_ID',             V_ESQUEMA||'.RES_RESERVAS',                           'RES_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            (
                ...
                RES_ID        		        NUMBER(16),
                MRA_INS_TITULO              NUMBER(1,0),
                MRA_TANTEO_DL               NUMBER(1,0),
                MRA_CARGAS                  NUMBER(1,0),
                MRA_TEMAS_TECNICOS          NUMBER(1,0),
                MRA_POSESION                NUMBER(1,0),
                MRA_TEMAS_CATASTRALES       NUMBER(1,0),
                MRA_AUTOR_VPO               NUMBER(1,0),
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (MRA_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
		    ...
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.RES_ID IS ''FK a RES_RESERVAS''';
            EXECUTE IMMEDIATE V_SQL;                        
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_INS_TITULO IS ''Flag inscripción de título''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_TANTEO_DL IS ''Flag tanteo DL''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_CARGAS IS ''Flag cargas''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_TEMAS_TECNICOS IS ''Flag temas técnicos''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_POSESION IS ''Flag posesión''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_TEMAS_CATASTRALES IS ''Flag temas catastrales''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.MRA_AUTOR_VPO IS ''Flag autorización VPO''';
            EXECUTE IMMEDIATE V_SQL;
