...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
        ...
        T_ALTER('DD_PRV_NAC_ID'     ,'NUMBER(16,0)' ,'FK a DD_PRV_PROVINCIA' ,'FK_COM_DD_PRV_NAC_ID'     ,'DD_PRV_PROVINCIA', 'DD_PRV_ID','COM_COMPRADOR'),
        T_ALTER('DD_PRV_NAC_ID'     ,'NUMBER(16,0)' ,'FK a DD_PRV_PROVINCIA' ,'FK_CLC_DD_PRV_NAC_ID'     ,'DD_PRV_PROVINCIA', 'DD_PRV_ID','CLC_CLIENTE_COMERCIAL'),
        T_ALTER('DD_PRV_NAC_ID'     ,'NUMBER(16,0)' ,'FK a DD_PRV_PROVINCIA' ,'FK_TIA_DD_PRV_NAC_ID'     ,'DD_PRV_PROVINCIA', 'DD_PRV_ID','OFR_TIA_TITULARES_ADICIONALES')
    );
    V_T_ALTER T_ALTER;
	...
	LOOP
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_T_ALTER(7)||' ADD CONSTRAINT '||V_T_ALTER(4)||' FOREIGN KEY 
						('||V_T_ALTER(1)||') REFERENCES '||V_ESQUEMA_M||'.'||V_T_ALTER(5)||'('||V_T_ALTER(6)||')';
			EXECUTE IMMEDIATE V_MSQL;
