...
--## INCIDENCIA_LINK=HREOS-15060
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_GRI_GRUPO_IMPUESTO
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_GRI_GRUPO_IMPUESTO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_CHARS VARCHAR2(6 CHAR) := 'GRI';
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-15060';
	V_MSQL VARCHAR2(32000 CHAR); -- Sentencia a ejecutar 
	V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Diccionario para Grupo de impuesto'; -- Vble. para los comentarios de las tablas
 ...
 BEGIN
	...
	IF V_COUNT = 0 THEN
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_GRI_ID IS ''Código identificador único''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_GRI_ID creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_GRI_CODIGO IS ''Código de tipo de Grupo de impuesto''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_GRI_CODIGO creado.');

        V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_GRI_DESCRIPCION IS ''Descripción de tipo de Grupo de impuesto''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_GRI_DESCRIPCION creado.');

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_GRI_DESCRIPCION_LARGA IS ''Descripción larga de tipo de Grupo de impuesto''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_GRI_DESCRIPCION_LARGA creado.');
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (DD_GRI_ID))';
	EXECUTE IMMEDIATE V_MSQL;
