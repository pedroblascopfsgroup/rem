...
--## PRODUCTO=NO
--## 
--## Finalidad: Sustituir valores booleanos por valores de diccionario y añadir FK al campo ICO_OCUPADO de la tabla ACT_ICO_INFO_COMERCIAL
--##            
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  V_COL T_ARRAY_COL := T_ARRAY_COL(
    T_COL('ADD_CONSTRAINT', 'FK_ICO_OCUPADO', 'ICO_OCUPADO','DD_SIN_SINO','DD_SIN_ID','1')
    );  
  V_TMP_COL T_COL;
 
BEGIN

    DBMS_OUTPUT.PUT_LINE('[INICIO] Actualiza '||V_TEXT_TABLA||'');			

    V_SQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' SET ICO_OCUPADO = (SELECT DD_SIN_ID FROM '||V_ESQUEMA_M||'.DD_SIN_SINO WHERE DD_SIN_CODIGO = ''02''),
    			USUARIOMODIFICAR = ''HREOS-17102'',
