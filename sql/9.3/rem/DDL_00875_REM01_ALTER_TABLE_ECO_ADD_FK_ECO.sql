...
--## PRODUCTO=NO
--## Finalidad: Alter ECO_EXPEDIENTE_COMERCIAL
--##           
...
DECLARE
    ...
    V_TABLA_FK VARCHAR2(2400 CHAR) := 'ECO_EXPEDIENTE_COMERCIAL';  
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(256);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(											--Nombre en la tabla fk
			T_TIPO_DATA('ECO_ID_ANTERIOR', 'NUMBER(16,0)', 'Tipo de régimen de fianza con CCAA', 'ECO_ID', 'ECO')
    ); 
      ...
      IF V_NUM_TABLAS > 0 THEN
        ...
        -- Si existe la columna cambiamos/establecemos solo la FK
          ...
          ELSE
              ...
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_'||V_TMP_TIPO_DATA(5)||'_ANT_'||TRIM(V_TMP_TIPO_DATA(1))||' FOREIGN KEY ('||TRIM(V_TMP_TIPO_DATA(1))||') REFERENCES '||V_ESQUEMA||'.'||V_TABLA_FK||' ('||TRIM(V_TMP_TIPO_DATA(4))||') ON DELETE SET NULL)';
              EXECUTE IMMEDIATE V_MSQL;
