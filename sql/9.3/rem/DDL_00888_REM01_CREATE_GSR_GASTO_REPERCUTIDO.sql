...
--## PRODUCTO=NO
--## Finalidad: CREATE gsr_gasto_repercutido
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'GSR_GASTO_REPERCUTIDO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla de gastos repercutidos'; -- Vble. para los comentarios de las tablas
    V_LETRAS_TABLA VARCHAR2(2400 CHAR) := 'GSR';
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
 	--         CAMPO FK                TABLA DESTINO FK                                 CAMPO DESTINO FK
        ...
        T_FK(   'DD_TGR_ID',             V_ESQUEMA||'.DD_TGR_TIPO_GASTO_REPERCUTIDO',       'DD_TGR_ID')
...
BEGIN
    ...
    ELSE
            ...
            (
                ...
                DD_TGR_ID              						NUMBER(16,0)				NOT NULL,
		 		'||V_LETRAS_TABLA||'_IMPORTE           		NUMBER(16,2)				NOT NULL,
				'||V_LETRAS_TABLA||'_MESES           		NUMBER(16,0),
            ...
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_TGR_ID IS ''Tipo de gasto repercutido ''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||V_LETRAS_TABLA||'_FECHA IS ''Fecha alta''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||V_LETRAS_TABLA||'_IMPORTE IS ''Importe''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||V_LETRAS_TABLA||'_MESES IS 	''Meses repercutidos''';
            EXECUTE IMMEDIATE V_SQL;
