...
DECLARE
    ...
    V_TABLA VARCHAR2(150 CHAR):= 'ACT_ACTIVO_TITULIZADA'; -- Vble. con el nombre de la tabla.
    ERR_NUM NUMBER(25);  -- Vble. auxiliar para registrar errores en el script.
    ERR_MSG VARCHAR2(1024 CHAR); -- Vble. auxiliar para registrar errores en el script.

BEGIN

    -- ******** DD_FIO_FUN_INTERLOCUTOR_OFERTA *******º
    DBMS_OUTPUT.PUT_LINE('******** '||V_TABLA||' ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'... Comprobaciones previas'); 
    
    -- Creacion Tabla DD_FIO_FUN_INTERLOCUTOR_OFERTA
    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||'
               (	AAT_ID NUMBER(16,0) 
				  ...
				  , FEC_EST_POSESORIO_ACTUAL DATE 
				  , DD_ETP_ID NUMBER(16,0)
				  ...
				  , CONSTRAINT PK_AAT_ID PRIMARY KEY(AAT_ID)
				  , CONSTRAINT FK_ATT_ACT_ACTIVO FOREIGN KEY(ACT_ID) REFERENCES '||V_ESQUEMA||'.ACT_ACTIVO(ACT_ID)
				  , CONSTRAINT FK_ATT_DD_ETP FOREIGN KEY(DD_ETP_ID) REFERENCES '||V_ESQUEMA||'.DD_ETP_ESTADO_POSESORIO(DD_ETP_ID)
			  )';
    ...
    -- Creamos comentario	
	V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla para gestionar los subtipos de ofertas de alquiler.''';		
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TABLA||'... Comentario creado sobre la tabla.');

	-- Comentarios sobre las columnas
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.AAT_ID IS ''Código identificador único.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.ACT_ID IS ''Código único del activo.''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.FEC_EST_POSESORIO_ACTUAL IS ''Fecha del último cambio de estado posesorio..''';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_ETP_ID IS ''Diccionario de Estado Posesorio.''';
