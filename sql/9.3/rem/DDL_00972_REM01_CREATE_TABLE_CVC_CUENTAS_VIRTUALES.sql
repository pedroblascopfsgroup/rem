...
--## PRODUCTO=NO
--## Finalidad: Creacion tabla CVC_CUENTAS_VIRTUALES
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'CVC_CUENTAS_VIRTUALES'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla con los datos de cuentas virtuales'; -- Vble. para los comentarios de las tablas
...
BEGIN
    ...
    ELSE
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (CVC_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
		    ...
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CVC_ID IS ''Id de la tabla''';
            ...
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CVC_FECHA_FIN IS ''Fecha fin cuenta virtual''';
            EXECUTE IMMEDIATE V_SQL;
