...
DECLARE
    ...
    V_TABLA VARCHAR2(27 CHAR) := 'OGR_OFERTAS_AGRUPADAS_LBK'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-6924';
 ...
 BEGIN
	...
	IF V_COUNT = 0 THEN 
		V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
				  OGR_ID NUMBER(16,0) NOT NULL
				, ID_OFERTA_PRINCIPAL NUMBER(16,0) NOT NULL
				, ID_OFERTA_DEPENDIENTE NUMBER(16,0) NOT NULL				
				...
				, CONSTRAINT OGR_ID_PK PRIMARY KEY (OGR_ID)
				)
			  ';
              
		EXECUTE IMMEDIATE V_SQL; 
        V_SQL := 'ALTER TABLE ' || V_ESQUEMA || '.'||V_TABLA||' ADD CONSTRAINT FK_ID_OFERTA_PRINCIPAL FOREIGN KEY (ID_OFERTA_PRINCIPAL) ' ||
        '  REFERENCES  ' || V_ESQUEMA || '.OFR_OFERTAS (OFR_ID)';
		EXECUTE IMMEDIATE V_SQL; 
        
        V_SQL := 'ALTER TABLE ' || V_ESQUEMA || '.'||V_TABLA||' ADD CONSTRAINT FK_ID_OFERTA_DEPENDIENTEL FOREIGN KEY (ID_OFERTA_DEPENDIENTE) ' ||
        '  REFERENCES  ' || V_ESQUEMA || '.OFR_OFERTAS (OFR_ID)';
		EXECUTE IMMEDIATE V_SQL; 
