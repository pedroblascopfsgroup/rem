...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DEP_DEPOSITO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_LETRAS_TABLA VARCHAR2(2400 CHAR) := 'DEP';
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
 	--         CAMPO FK                TABLA DESTINO FK                                 CAMPO DESTINO FK
        T_FK(   'OFR_CAIXA_ID',            	 V_ESQUEMA||'.OFR_OFERTAS_CAIXA',      			'OFR_CAIXA_ID'),
        T_FK(   'DD_EDP_ID',             V_ESQUEMA||'.DD_EDP_EST_DEPOSITO',   				'DD_EDP_ID')
...
BEGIN
    ...
    ELSE
            ...
            (
                ...
                OFR_CAIXA_ID        		        		NUMBER(16,0)				NOT NULL,
				DD_EDP_ID									NUMBER(16,0)				NOT NULL,
                '||V_LETRAS_TABLA||'_IMPORTE				NUMBER(16,2)				NOT NULL,
		 		'||V_LETRAS_TABLA||'_FECHA_INGRESO          DATE						NOT NULL,
				'||V_LETRAS_TABLA||'_FECHA_DEVOLUCION       DATE,
				'||V_LETRAS_TABLA||'_IBAN_DEVOLUCION 		VARCHAR2(50 CHAR),
            ...
            -- Comentarios de las columnas propias de la tabla
             ...
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.OFR_CAIXA_ID IS ''OFR_OFERTA_CAIXA a la que que pertenece''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_EDP_ID IS ''DD_EDP_EST_DEPOSITO ''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||V_LETRAS_TABLA||'_IMPORTE IS ''Importe del deposito''';
            EXECUTE IMMEDIATE V_SQL;
             V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||V_LETRAS_TABLA||'_FECHA_INGRESO IS ''Fecha de ingreso del deposito''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||V_LETRAS_TABLA||'_FECHA_DEVOLUCION IS ''Fecha de devolución del deposito''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.'||V_LETRAS_TABLA||'_IBAN_DEVOLUCION IS ''IBAN de devololución de deposito''';
            EXECUTE IMMEDIATE V_SQL;
