...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(												
			T_TIPO_DATA('PVE_COD_API_PROVEEDOR', 'VARCHAR2(10 CHAR)')
    ); 
...
BEGIN
    ...
    LOOP      
      ...
      IF V_NUM_TABLAS > 0 THEN        
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' MODIFY '||V_TMP_TIPO_DATA(1)||' '||V_TMP_TIPO_DATA(2)||'';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] Columna '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_TMP_TIPO_DATA(1)||'... Modificada');  
      ELSE
        DBMS_OUTPUT.PUT_LINE('NO EXISTA LA COLUMNA');
      END IF;
