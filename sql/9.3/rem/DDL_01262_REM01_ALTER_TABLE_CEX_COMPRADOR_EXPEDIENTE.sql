...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER('DD_PAI_ID_ISO',		 			'NUMBER(16,0)',						'FK a la tabla DD_PAI_PAISES ISO.')
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_DD_PAI_ID_ISO_CEX',					'DD_PAI_ID_ISO',			V_ESQUEMA||'.DD_PAI_PAISES',				'DD_PAI_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	-- Verificar si la FK ya existe. Si ya existe, se elimina
		V_MSQL := 'SELECT COUNT(1) FROM all_constraints WHERE OWNER = '''||V_ESQUEMA||''' and table_name = '''||V_TEXT_TABLA||''' and constraint_name = ''FK_DD_PAI_ID_ISO_CEX''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina.
			
			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' DROP CONSTRAINT FK_DD_PAI_ID_ISO_CEX ';
		...
		V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLS WHERE COLUMN_NAME = ''DD_PAI_ID_ISO'' and TABLE_NAME = '''||V_TEXT_TABLA||''' and owner = '''||V_ESQUEMA||'''';
		EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
		IF V_NUM_TABLAS > 0 THEN
			--Si existe se elimina

			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' 
					   DROP COLUMN DD_PAI_ID_ISO';
	...
	LOOP
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] ... Columna INSERTADA en tabla.');
