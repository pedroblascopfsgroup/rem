...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(											
			T_TIPO_DATA('NIF_TITULAR_CARTA', 		'VARCHAR2(20 CHAR)', 		'Titular carta que llega desde gestorías, relacionado con el propietario')
    ); 
    ...
    LOOP      
      ...
      EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
        ...
        ELSE
            ...
            V_MSQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME= '''||TRIM(V_TMP_TIPO_DATA(1))||''' and TABLE_NAME = UPPER('''||V_TEXT_TABLA||'_REJ'') and owner = '''||V_ESQUEMA||'''';
            EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
            IF V_NUM_TABLAS = 1 THEN   
              DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||' '||TRIM(V_TMP_TIPO_DATA(1))||'''... Ya existe');
            ELSE
              V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_REJ ADD ('||V_TMP_TIPO_DATA(1)||' '||V_TMP_TIPO_DATA(2)||')';
              EXECUTE IMMEDIATE V_MSQL;
              DBMS_OUTPUT.PUT_LINE('[INFO] Columna '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_REJ.'||V_TMP_TIPO_DATA(1)||'... Creada');
              EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_REJ.'||V_TMP_TIPO_DATA(1)||' IS '''||V_TMP_TIPO_DATA(3)||'''';  
            END IF;
