...
--## PRODUCTO=NO
--## Finalidad: MODIFICACION DE COLUMNAS DE GLD_GASTOS_LINEA_DETALLE
--##           
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
		...
		T_ALTER('PRIM_TOMA_POSESION',		 		'NUMBER(1,0)',						'Indicador de primera posesión.')
		);
    V_T_ALTER T_ALTER;
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	...
    	T_FK(	'FK_GLD_DD_SED_ID',				'DD_SED_ID',				V_ESQUEMA||'.DD_SED_SUBPARTIDA_EDIFICACION',				'DD_SED_ID'			)
    );
    V_T_FK T_FK;
...
BEGIN
	...
	-- Verificar si la FK ya existe. Si ya existe, se elimina
		V_MSQL := 'SELECT COUNT(1) FROM all_constraints WHERE OWNER = '''||V_ESQUEMA||''' and table_name = '''||V_TEXT_TABLA||''' and constraint_name = ''FK_GEN_DD_SED_ID''';
	...
	IF V_CREAR_FK = 'SI' THEN
		...
		LOOP
			...
			IF ''||V_T_FK(1)||'' = 'FK_GLD_DD_SED_ID' THEN
