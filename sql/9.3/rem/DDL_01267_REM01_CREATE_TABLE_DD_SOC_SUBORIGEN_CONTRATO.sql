...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_SOC_SUBORIGEN_CONTRATO
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_SOC_SUBORIGEN_CONTRATO'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_CHARS VARCHAR2(6 CHAR) := 'SOC';
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-16897';
	V_MSQL VARCHAR2(32000 CHAR); -- Sentencia a ejecutar 
	V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Suborigen del contrato'; -- Vble. para los comentarios de las tablas
 ...
 BEGIN
	...
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_SOC_ID creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_CODIGO IS ''Código identificador único''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_SOC_CODIGO creado.');	

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_DESCRIPCION IS ''Descripción''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_SOC_DESCRIPCION creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_DESCRIPCION_LARGA IS ''Descripción larga''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_SOC_DESCRIPCION_LARGA creado.');	
