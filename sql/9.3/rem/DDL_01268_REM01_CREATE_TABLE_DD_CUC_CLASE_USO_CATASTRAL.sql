...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla DD_CUC_CLASE_USO_CATASTRAL
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_CUC_CLASE_USO_CATASTRAL'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_CHARS VARCHAR2(6 CHAR) := 'CUC';
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-17169';
	V_MSQL VARCHAR2(32000 CHAR); -- Sentencia a ejecutar 
	V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Historico de sanciones BC'; -- Vble. para los comentarios de las tablas
 ...
 BEGIN
	...
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_CUC_ID creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_CODIGO IS ''Código identificador único''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_CUC_CODIGO creado.');	

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_DESCRIPCION IS ''Descripción''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_CUC_DESCRIPCION creado.');

	V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CHARS||'_DESCRIPCION_LARGA IS ''Descripción larga''';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_CUC_DESCRIPCION_LARGA creado.');	
