...
--## PRODUCTO=NO
--## Finalidad: Anyadir columnas COMPRADOR_ACOJE_AYUDA, IMPORTE_AYUDA_FINANCIACION, FECHA_VENCIMIENTO_AVAL_SEGURO, FECHA_DEVOLUCION_AYUDA  en ACT_ADM_INF_ADMINISTRATIVA
--##           
...
DECLARE
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	  ...
    	  T_ALTER('COMPRADOR_ACOJE_AYUDA', 'NUMBER(1,0)', 'DEFAULT 0', 'Comprador se acoje a ayuda'),
		  T_ALTER('IMPORTE_AYUDA_FINANCIACION', 'NUMBER(16,2)', '', 'Importe ayuda financiación'),
		  T_ALTER('FECHA_VENCIMIENTO_AVAL_SEGURO', 'DATE', '', 'Fecha vencimiento aval/seguro'),
		  T_ALTER('FECHA_DEVOLUCION_AYUDA', 'DATE', '', 'Fecha devolución ayuda')
	);
	V_T_ALTER T_ALTER;
	...
	LOOP
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' 
					   ADD ('||V_T_ALTER(1)||' '||V_T_ALTER(2)||' '||V_T_ALTER(3)||')
			';
			...
			-- Creamos comentario	
			V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_T_ALTER(1)||' IS '''||V_T_ALTER(4)||'''';		
			EXECUTE IMMEDIATE V_MSQL;
