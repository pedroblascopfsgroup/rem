...
--## PRODUCTO=NO
--## Finalidad: Creacion tabla CON_CONCURRENCIA
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'CON_CONCURRENCIA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_CON_ACT_ID',                   'ACT_ID',             V_ESQUEMA||'.ACT_ACTIVO',                'ACT_ID'),
        T_FK(   'FK_CON_AGR_ID',                   'AGR_ID',             V_ESQUEMA||'.ACT_AGR_AGRUPACION',         'AGR_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            (
                ...
                ACT_ID        		        NUMBER(16),
                AGR_ID			            NUMBER(16),	
                CON_IMPORTE_MIN_OFR         NUMBER(16,2),
                CON_IMPORTE_DEPOSITO        NUMBER(16,2),
                CON_FECHA_INI               DATE,
                CON_FECHA_FIN               DATE,
            ...
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.ACT_ID IS ''Id del activo''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.AGR_ID IS ''Id de la agrupación''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CON_IMPORTE_MIN_OFR IS ''Importe mínimo de la oferta''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CON_IMPORTE_DEPOSITO IS ''Importe depósito''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CON_FECHA_INI IS ''Fecha inicio concurrencia''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CON_FECHA_FIN IS ''Fecha fin concurrencia''';
            EXECUTE IMMEDIATE V_SQL;
