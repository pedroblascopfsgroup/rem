...
BEGIN
    ...
    ELSE
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (PDE_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
            ...
            -- Creamos comentario
            V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla de parametrizacion deposito''';
		    EXECUTE IMMEDIATE V_MSQL;	
		...
		V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_TCR_ID IS ''Id de tipo comercializar''';
            ...
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.PDE_IMPORTE IS ''Importe deposito''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.PDE_PRECIO IS ''Precio venta activo''';
            EXECUTE IMMEDIATE V_SQL;
