...
--## PRODUCTO=NO
--## Finalidad: Tabla para listar los valores de procedencia
--##           
...
DECLARE
    ...
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Diccionario de estados oferta BC	.'; -- Vble. para los comentarios de las tablas
...
BEGIN
	...
	(
			DD_EOB_ID								NUMBER(16,0)                NOT NULL,
			DD_EOB_CODIGO        					VARCHAR2(20 CHAR),
			DD_EOB_DESCRIPCION       				VARCHAR2(100 CHAR),
			DD_EOB_DESCRIPCION_LARGA  				VARCHAR2(250 CHAR),
	...
	-- Creamos indice	
	V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK ON '||V_ESQUEMA|| '.'||V_TEXT_TABLA||'(DD_EOB_ID) TABLESPACE '||V_TABLESPACE_IDX;		
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... Indice creado.');	
	
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (DD_EOB_ID) USING INDEX)';
	EXECUTE IMMEDIATE V_MSQL;
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_EOB_ID IS ''Código identificador único del tipo de rol.'' ';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_EOB_CODIGO IS ''Código identificador del tipo de rol.'' ';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_EOB_DESCRIPCION IS ''Descripción del tipo de rol'' ';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_EOB_DESCRIPCION_LARGA IS ''Descripción larga del tipo de rol'' ';
