...
--## PRODUCTO=NO
--## Finalidad: Creacion diccionario DD_NCI_NIVEL_CONDUC_INAPROP
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_NCI_NIVEL_CONDUC_INAPROP'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla de nivel de conductas inapropiadas'; -- Vble. para los comentarios de las tablas
...
BEGIN
    ...
    ELSE
            ...
            (
                ...
                DD_NCI_CODIGO               VARCHAR2(20 CHAR),
                DD_NCI_DESCRIPCION          VARCHAR2(40 CHAR),
                DD_NCI_DESCRIPCION_LARGA    VARCHAR2(100 CHAR),
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (DD_NCI_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
		    ...
		    EXECUTE IMMEDIATE V_MSQL;	
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_NCI_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_MSQL;
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_NCI_CODIGO IS ''Código del diccionario''';
            EXECUTE IMMEDIATE V_MSQL;
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_NCI_DESCRIPCION IS ''Descripción del diccionario''';
            EXECUTE IMMEDIATE V_MSQL;
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_NCI_DESCRIPCION_LARGA IS ''Descripción larga del diccionario''';
            EXECUTE IMMEDIATE V_MSQL;
