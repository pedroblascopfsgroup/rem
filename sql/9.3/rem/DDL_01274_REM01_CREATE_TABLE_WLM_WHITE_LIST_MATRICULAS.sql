...
--## PRODUCTO=NO
--## Finalidad: Creacion tabla WLM_WHITE_LIST_MATRICULAS
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'WLM_WHITE_LIST_MATRICULAS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla con los datos de la whitelist de matrículas'; -- Vble. para los comentarios de las tablas
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_WLM_FUN_FUNCIONES',            'FUN_ID',             V_ESQUEMA_M||'.FUN_FUNCIONES',                		'FUN_ID'),
        T_FK(   'FK_WLM_DD_TDO',                   'DD_TDO_ID',             V_ESQUEMA||'.DD_TDO_TIPO_DOC_ENTIDAD', 	        'DD_TDO_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (WLM_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
            ...
            -- Creamos comentario
            V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla con los datos de la whitelist de matrículas''';
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.WLM_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.FUN_ID IS ''FK Id de la función''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_TDO_ID IS ''FK Id del tipo de documento entidad''';
            EXECUTE IMMEDIATE V_SQL;
