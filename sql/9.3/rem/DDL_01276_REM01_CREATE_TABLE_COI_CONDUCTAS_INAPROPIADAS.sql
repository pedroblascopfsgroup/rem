...
--## PRODUCTO=NO
--## Finalidad: Creacion tabla COI_CONDUCTAS_INAPROPIADAS
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'PVE_COI_CONDUCTAS_INAPROPIADAS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla para registrar las conductas inapropiadas'; -- Vble. para los comentarios de las tablas
...
BEGIN
    ...
    ELSE
            ...
            (
                COI_ID          		    NUMBER(16,0)                  NOT NULL,
                COI_PVE          		    NUMBER(16,0)                  NOT NULL,
		    ...
		    EXECUTE IMMEDIATE V_MSQL;	
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COI_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_MSQL;	
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COI_PVE IS ''FK de la tabla ACT_PVE_PROVEEDOR''';
            EXECUTE IMMEDIATE V_MSQL;	
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_CCI_ID IS ''FK de la tabla DD_CCI_CAT_CONDUC_INAPROP''';
            EXECUTE IMMEDIATE V_MSQL;
            V_MSQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.COI_DELEGACION IS ''FK de la tabla ACT_PRD_PROVEEDOR_DIRECCION''';
            EXECUTE IMMEDIATE V_MSQL;	
                ...
                LOOP
                    ...
                    IF V_NUM_TABLAS = 0 THEN
                        ...
                        V_MSQL := '
                            ...
                            ('||V_T_FK(4)||' ) ON DELETE SET NULL ENABLE';
