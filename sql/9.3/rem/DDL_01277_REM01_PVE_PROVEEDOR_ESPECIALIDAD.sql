...
BEGIN
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_PVE_PRD_ID FOREIGN KEY  (PRD_ID) REFERENCES '||V_ESQUEMA||'.ACT_PRD_PROVEEDOR_DIRECCION (PRD_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_PVE_PRD_ID... Foreign key creada.');
