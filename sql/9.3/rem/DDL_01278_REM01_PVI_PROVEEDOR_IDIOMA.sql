...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'PVI_PROVEEDOR_IDIOMA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	(
		PVE_IDM_ID 						NUMBER(16,0) NOT NULL
	...
	-- Creamos primary key
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (PVE_IDM_ID))';
	EXECUTE IMMEDIATE V_MSQL;
	...
	V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_PVE_PVI_PRD_ID FOREIGN KEY  (PRD_ID) REFERENCES '||V_ESQUEMA||'.ACT_PRD_PROVEEDOR_DIRECCION (PRD_ID) ON DELETE SET NULL)';
	EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.FK_PVE_PVI_PRD_ID... Foreign key creada.');
