...
--## PRODUCTO=NO
--## Finalidad: INSERCIÓN diccionario BAL_BLOQUEO_APIS_LNEG
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'BAL_BLOQUEO_APIS_LNEG'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla de bloqueo de apis - Linea negocio'; -- Vble. para los comentarios de las tablas
    V_LETRAS_TABLA VARCHAR2(2400 CHAR) := 'BAL';
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
    	 T_FK(   'DD_TCO_ID',            	 V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION',    'DD_TCO_ID'),
    	 T_FK(   'BAP_ID',            	 	 V_ESQUEMA||'.BAP_BLOQUEO_APIS',   'BAP_ID')
   	);
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
        	...
        	V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.BAP_ID IS ''Bloqueo relacionado ''';
            EXECUTE IMMEDIATE V_SQL;
             
        	V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_TCO_ID IS ''Tipo de comercialización ''';
            EXECUTE IMMEDIATE V_SQL;
