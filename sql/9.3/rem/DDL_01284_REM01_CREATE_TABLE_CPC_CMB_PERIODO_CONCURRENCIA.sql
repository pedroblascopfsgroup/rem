...
--## INCIDENCIA_LINK=HREOS-18256
--## PRODUCTO=NO
--## Finalidad: Creacion diccionario CPC_CMB_PERIODO_CONCURRENCIA
--##           
...
DECLARE
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_CPC_CON_ID',                   'CON_ID',             V_ESQUEMA||'.CON_CONCURRENCIA',                'CON_ID'),
        T_FK(   'FK_CPC_DD_ACO_ID',                'DD_ACO_ID',             V_ESQUEMA||'.DD_ACO_ACCIONES_CONCURRENCIA',   'DD_ACO_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (CPC_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
            ...
            -- Creamos comentario
            V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla con los datos de cambio de periodo de concurrencia''';
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CPC_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CON_ID IS ''Id de la concurrencia asociada''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_ACO_ID IS ''Id de acciones concurrencia asociado''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CPC_FECHA_FIN IS ''Fecha de fin''';
            EXECUTE IMMEDIATE V_SQL;
