...
--## INCIDENCIA_LINK=HREOS-18222
--## PRODUCTO=NO
--## Finalidad: Creacion tabla CVA_CUENTAS_VIRTUALES_ALQUILER
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'CVA_CUENTAS_VIRTUALES_ALQUILER'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla con los datos de cuentas virtuales de alquiler'; -- Vble. para los comentarios de las tablas
    ...
    V_FK T_ARRAY_FK := T_ARRAY_FK(
        ...
        T_FK(   'FK_CVA_DD_SCR',                   'DD_SCR_ID',             V_ESQUEMA||'.DD_SCR_SUBCARTERA',                'DD_SCR_ID')
    );
    V_T_FK T_FK;
...
BEGIN
    ...
    ELSE
            ...
            (
                CVA_ID          		    NUMBER(16)                  NOT NULL,
                CVA_CUENTA_VIRTUAL        	VARCHAR2(150 CHAR),
                DD_SCR_ID          			NUMBER(16),
                CVA_FECHA_INICIO			DATE,
                CVA_FECHA_FIN				DATE,
            ...
            -- Creamos primary key
            V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD (CONSTRAINT '||V_TABLA||'_PK PRIMARY KEY (CVA_ID) USING INDEX)';
            EXECUTE IMMEDIATE V_MSQL;
		    ...
		    EXECUTE IMMEDIATE V_MSQL;	
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CVA_ID IS ''Id de la tabla''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CVA_CUENTA_VIRTUAL IS ''Cuenta virtual''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.DD_SCR_ID IS ''Id de la subcartera''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CVA_FECHA_INICIO IS ''Fecha inicio cuenta virtual''';
            EXECUTE IMMEDIATE V_SQL;
            V_SQL := 'COMMENT ON COLUMN ' ||V_ESQUEMA||'.'||V_TABLA||'.CVA_FECHA_FIN IS ''Fecha fin cuenta virtual''';
            EXECUTE IMMEDIATE V_SQL;
