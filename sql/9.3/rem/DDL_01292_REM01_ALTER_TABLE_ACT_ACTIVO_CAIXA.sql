...
--## PRODUCTO=NO
--##
--## Finalidad: Crear la tabla ACT_ACTIVO_CAIXA
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-12391'; -- USUARIOCREAR/USUARIOMODIFICAR.
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		T_TIPO_DATA('DD_ECV_STOCK_BC', 'VARCHAR2(20 CHAR)', 'Campo que almacena estado conservacion informado por STOCK BC')
    ); 
	...
	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			V_MSQL := 'ALTER TABLE '||V_TABLA|| ' ADD ('||V_TMP_TIPO_DATA(1)||' '||V_TMP_TIPO_DATA(2)||')';		   
			EXECUTE IMMEDIATE V_MSQL;
			DBMS_OUTPUT.PUT_LINE('[INFO] Columna '||V_ESQUEMA||'.'||V_TABLA||'.'||V_TMP_TIPO_DATA(1)||'... Creada');

      V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.'||V_TMP_TIPO_DATA(1)||' IS '''||V_TMP_TIPO_DATA(3)||'''';
			EXECUTE IMMEDIATE V_MSQL;
