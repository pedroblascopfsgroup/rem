...
--## PRODUCTO=NO
--## Finalidad: Creación diccionario DD_TAR_TIPO_ACT_RESCISION
--##           
...
DECLARE
    ...
    V_TABLA VARCHAR2(2400 CHAR) := 'DD_TAR_TIPO_ACT_RESCISION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_LETRAS_TABLA VARCHAR2(2400 CHAR) := 'TAR';
...
BEGIN
	...
	IF V_NUM_TABLAS = 0 THEN
		...
		(
			...
			DD_'||V_LETRAS_TABLA||'_CODIGO_C4C            	VARCHAR2(20 CHAR),
		...
		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_LETRAS_TABLA||'_CODIGO_C4C IS ''Código C$C''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la columna DD_'||V_LETRAS_TABLA||'_CODIGO_C4C creado.');
