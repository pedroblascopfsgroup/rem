...
DECLARE
    ...
    V_TABLA VARCHAR2(27 CHAR) := 'ACT_CNP_CONFIG_PORTAL'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    IF V_NUM_TABLAS = 1 THEN 
  	...
  	ELSE
		...
		V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
			...
			CNP_PENDIENTE_INSCRIPCION		NUMBER(1,0) DEFAULT 0 NOT NULL ENABLE,
			CNP_CON_CARGAS 					NUMBER(1,0) DEFAULT 0 NOT NULL ENABLE,
			CNP_OCUPADO_SIN_TITULO 			NUMBER(1,0) DEFAULT 0 NOT NULL ENABLE,
			CNP_OCUPADO_CON_TITULO 			NUMBER(1,0) DEFAULT 0 NOT NULL ENABLE,
			...
			CONSTRAINT PK_CNP_ID PRIMARY KEY (CNP_ID),
			CONSTRAINT UNIQUE_CRA_SCR_COMP UNIQUE (DD_CRA_ID, DD_SCR_ID),
			CONSTRAINT FK_CNP_CARTERA FOREIGN KEY (DD_CRA_ID) REFERENCES DD_CRA_CARTERA(DD_CRA_ID),
			CONSTRAINT FK_CNP_SUBCARTERA FOREIGN KEY (DD_SCR_ID) REFERENCES DD_SCR_SUBCARTERA(DD_SCR_ID)
			)';
