...
DECLARE
    ...
    V_USUARIO VARCHAR2(200 CHAR) := 'HREOS-17992';

    V_TEXT1 VARCHAR2(2400 CHAR); -- Vble. auxiliar
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'CBA_CONFIG_BLOQUEO_ASUNTOS_MINERVA'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
	...
	ELSE 
		...
		-- Creamos primary key
		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (CBA_ID))';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||'_PK... PK creada.');

		-- Creamos foreign key FK_DD_SCR_ID_CSP
   		V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_DD_SCR_ID_CBA FOREIGN KEY (DD_SCR_ID) REFERENCES '||V_ESQUEMA||'.DD_SCR_SUBCARTERA (DD_SCR_ID) ON DELETE SET NULL)';
   		EXECUTE IMMEDIATE V_MSQL;
   		DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.FK_DD_SCR_ID_CBA... Foreign key creada.');
		...
		V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' IS ''Tabla de CBA_CONFIG_BLOQUEO_ASUNTOS_MINERVA.''';
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] Comentario de la tabla creado.');		

		V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.CBA_ID IS ''Código identificador único del CBA_CONFIG_BLOQUEO_ASUNTOS_MINERVA''';
		EXECUTE IMMEDIATE V_MSQL;
