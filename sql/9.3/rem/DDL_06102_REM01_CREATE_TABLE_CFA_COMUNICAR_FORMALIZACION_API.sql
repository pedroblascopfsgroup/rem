...
DECLARE
    ...
    V_TABLA VARCHAR2(27 CHAR) := 'CFA_COMUNICAR_FORMALIZACION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    IF V_NUM_TABLAS = 1 THEN 
  	...
  	ELSE
		...
		V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
			...
			CFA_LLAMADA_REALIZADA				NUMBER(1,0) 		DEFAULT 0 	NOT NULL ENABLE,
			CFA_FECHA_LLAMADA					DATE,
			CFA_BUROFAX_ENVIADO 				NUMBER(1,0) 		DEFAULT 0 	NOT NULL ENABLE,
			CFA_FECHA_BUROFAX 					DATE,
			...
			CONSTRAINT PK_CFA_ID PRIMARY KEY (CFA_ID),
			CONSTRAINT FK_CFA_OFR_ID FOREIGN KEY (OFR_ID) REFERENCES OFR_OFERTAS(OFR_ID)
			)';
