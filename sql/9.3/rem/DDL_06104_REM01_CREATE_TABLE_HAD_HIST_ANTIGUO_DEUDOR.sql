    ...
    IF V_NUM_TABLAS = 1 THEN 
  	...
  	ELSE
		...
		V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
			...
			HAD_LOCALIZABLE				NUMBER(1,0) DEFAULT 0 NOT NULL ENABLE,
			HAD_FECHA_ILOCALIZABLE				DATE,
			HAD_MOTIVO 					VARCHAR2(200 CHAR),
			DD_EEB_ID					NUMBER(16,0) NOT NULL ENABLE,
		...
		V_SQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.HAD_LOCALIZABLE IS ''Indicador de localización''';
		EXECUTE IMMEDIATE V_SQL;
