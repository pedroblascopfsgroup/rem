...
DECLARE
    ...
    V_TABLA VARCHAR2(150 CHAR):= 'DD_OCN_ORIGEN_CONTRATO_ECC'; -- Vble. con el nombre de la tabla.
    V_CODIGO_TABLA VARCHAR2(3 CHAR):= 'OCN';
    ERR_NUM NUMBER(25);  -- Vble. auxiliar para registrar errores en el script.
    ERR_MSG VARCHAR2(1024 CHAR); -- Vble. auxiliar para registrar errores en el script.

BEGIN

    -- ******** DD_OCN_ORIGEN_CONTRATO_ECC *******º
    DBMS_OUTPUT.PUT_LINE('******** '||V_TABLA||' ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'... Comprobaciones previas'); 
    
    -- Creacion Tabla DD_OCN_ORIGEN_CONTRATO_ECC
    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||'
               (DD_'||V_CODIGO_TABLA||'_ID NUMBER (16,0) 
				  ...
				  , DD_'||V_CODIGO_TABLA||'_CODIGO_C4C VARCHAR2(20 CHAR)
		...
		-- execute immediate 'grant select, insert, delete, update, REFERENCES(DD_OCN_ID) on ' || V_ESQUEMA || '.'||V_TABLA||' to '||V_ESQUEMA||'';
    ...
    -- Creamos comentario	
	V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla para gestionar los tipos de origen de contrato de ECC.''';		
	EXECUTE IMMEDIATE V_MSQL;
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_'||V_CODIGO_TABLA||'_CODIGO_C4C IS ''Código único C4C.''';
