...
DECLARE
    ...
    V_TABLA VARCHAR2(150 CHAR):= 'DD_SCN_SUBORIGEN_CNT_ECC'; -- Vble. con el nombre de la tabla.
    V_CODIGO_TABLA VARCHAR2(3 CHAR):= 'SCN';
    ERR_NUM NUMBER(25);  -- Vble. auxiliar para registrar errores en el script.
    ERR_MSG VARCHAR2(1024 CHAR); -- Vble. auxiliar para registrar errores en el script.

BEGIN

    -- ******** DD_SCN_SUBORIGEN_CNT_ECC *******º
    DBMS_OUTPUT.PUT_LINE('******** '||V_TABLA||' ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TABLA||'... Comprobaciones previas'); 
    
    -- Creacion Tabla DD_SCN_SUBORIGEN_CNT_ECC
    ...
    ELSE  
		...
		-- execute immediate 'grant select, insert, delete, update, REFERENCES(DD_SCN_ID) on ' || V_ESQUEMA || '.'||V_TABLA||' to '||V_ESQUEMA||'';
    ...
    -- Creamos comentario	
	V_MSQL := 'COMMENT ON TABLE '||V_ESQUEMA||'.'||V_TABLA||' IS ''Tabla para gestionar los tipos de suborigen de contrato de ECC.''';		
	EXECUTE IMMEDIATE V_MSQL;
