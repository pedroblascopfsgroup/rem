...
BEGIN
    ...
    LOOP      
      ...
      V_MSQL := 'SELECT COUNT(1) FROM ALL_TABLES WHERE OWNER = '''||V_ESQUEMA||''' AND TABLE_NAME = UPPER('''||TRIM(V_TMP_TIPO_DATA(6))||''')';
      ...
      IF V_NUM_TABLAS > 0 THEN
        ...
        -- Si existe la columna cambiamos/establecemos solo la FK
          IF V_NUM_TABLAS = 1 THEN
              ...
              IF V_NUM_TABLAS = 0 THEN
 				 V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (CONSTRAINT FK_'||V_TMP_TIPO_DATA(5)||'_'||TRIM(V_TMP_TIPO_DATA(1))||' FOREIGN KEY ('||TRIM(V_TMP_TIPO_DATA(1))||') REFERENCES '||V_ESQUEMA||'.'||TRIM(V_TMP_TIPO_DATA(6))||' ('||TRIM(V_TMP_TIPO_DATA(4))||') ON DELETE SET NULL)';
