...
DECLARE
    ...
    V_TABLA VARCHAR2(27 CHAR) := 'DD_ACO_APROBACION_COMITE'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
	V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Tabla Diccionario de estados de aprobación del comité de un trabajo.'; -- Vble. para los comentarios de las tablas	
    
	TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(150);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('SOL','Solicitado','Solicitado','HREOS-10436','SYSDATE','0'),
    	T_TIPO_DATA('APR','Aprobado','Aprobado','HREOS-10436','SYSDATE','0'),
    	T_TIPO_DATA('REC','Rechazado','Rechazado','HREOS-10436','SYSDATE','0')
    ); 
 ...
 BEGIN 
    ...
    IF V_NUM_TABLAS = 1 THEN 
  	...
  	ELSE
		...
		V_SQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' ( 
			DD_ACO_ID						NUMBER(16,0) NOT NULL, 
			DD_ACO_CODIGO					VARCHAR2(20CHAR) NOT NULL ENABLE,
			DD_ACO_DESCRIPCION				VARCHAR2(100CHAR),
			DD_ACO_DESCRIPCION_LARGA		VARCHAR2(250CHAR),
			...
			CONSTRAINT PK_DD_ACO_ID PRIMARY KEY (DD_ACO_ID),
			CONSTRAINT UNIQUE_DD_ACO_CODIGO UNIQUE (DD_ACO_CODIGO)
			)';
	...
	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
		LOOP		      
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE DD_ACO_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''';
			EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;			      
			IF V_NUM_TABLAS <= 0 THEN				
			    -- Si no existe se crea.
			    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
			    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' SELECT 
				S_'||V_TABLA||'.NEXTVAL,
				'''||V_TMP_TIPO_DATA(1)||''',
				'''||V_TMP_TIPO_DATA(2)||''',
				'''||V_TMP_TIPO_DATA(3)||''',
