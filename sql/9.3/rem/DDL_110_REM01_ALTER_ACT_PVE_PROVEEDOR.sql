...
--## INCIDENCIA_LINK=HREOS-9742
--## PRODUCTO=NO
--## Finalidad: Inserción del campo PVE_COD_ORIGEN en la tabla ACT_PVE_PROVEEDOR
--##           
...
BEGIN
    ...
    V_SQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE TABLE_NAME = '''||V_TABLA||''' AND OWNER = '''||V_ESQUEMA||''' 
	      AND COLUMN_NAME IN (''PVE_COD_ORIGEN'')';
    ...
    ELSE
        V_SQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' 
				  ADD (PVE_COD_ORIGEN VARCHAR2(50 CHAR)
				  )';
