...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('DD_ECV_DESCRIPCION_TRADUCIDA', 'VARCHAR2(250 CHAR)', 'Descripción traducida al Inglés')
    ); 
	...
	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
		...
		V_SQL := 'SELECT COUNT(1) FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME = '''||V_TMP_TIPO_DATA(1)||''' 
														 and TABLE_NAME = ''DD_ECV_ESTADO_CONSERVACION'' 
														 and OWNER = '''||V_ESQUEMA||'''';
		...
		IF V_NUM_TABLAS = 0 THEN
			...
			V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.DD_ECV_ESTADO_CONSERVACION
					   ADD ('||V_TMP_TIPO_DATA(1)||' '||V_TMP_TIPO_DATA(2)||')';
					   
			EXECUTE IMMEDIATE V_MSQL;

			V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.DD_ECV_ESTADO_CONSERVACION.'||V_TMP_TIPO_DATA(1)||' IS '''||V_TMP_TIPO_DATA(3)||'''';
