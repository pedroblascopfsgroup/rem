...
BEGIN
    ...
    ELSE  
      ...
      V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD CONSTRAINT FK_DD_MOE_ID FOREIGN KEY (DD_MOE_ID)
              REFERENCES '||V_ESQUEMA||'.DD_MOE_MOTIVO_EXCEPTUACION(DD_MOE_ID) ON DELETE SET NULL ENABLE';
      EXECUTE IMMEDIATE V_MSQL;
      ...
      V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.DD_MOE_ID IS ''FK refenciada ID motivo exceptuacion''';
      EXECUTE IMMEDIATE V_MSQL;
