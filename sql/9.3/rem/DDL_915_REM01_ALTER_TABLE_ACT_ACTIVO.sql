...
--## INCIDENCIA_LINK=HREOS-8208
--## PRODUCTO=NO
--## Finalidad: Ampliar la tabla que contiene la información venta sobre plano
--##           
...
DECLARE
    ...
    T_ALTER(  'ACT_VENTA_PLANO',	'NUMBER(1,0)',	'Identificador de si es venta sobre plano'	)
    );
    V_T_ALTER T_ALTER;
...
V_MSQL := 'ALTER TABLE '||V_TEXT_TABLA|| ' ADD CONSTRAINT FK_'||V_T_ALTER(1)||' FOREIGN KEY ('||V_T_ALTER(1)||') REFERENCES '||V_ESQUEMA_M||'.'||V_TABLA_FK||' ('||V_COLUMNA_FK||') ';
EXECUTE IMMEDIATE V_MSQL;
