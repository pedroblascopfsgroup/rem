...
--## PRODUCTO=NO
--## Finalidad: DDL Creación de la tabla DD_TDC_TERRITORIOS_DIR_COM
--##           
...
DECLARE
    ...
    V_TABLA_SEQ VARCHAR2(30 CHAR) := 'S_DD_TDC_TERRITORIOS_DIR_COM';  -- Tabla a modificar   
    V_TEXT_TABLA VARCHAR2(30 CHAR) := 'DD_TDC_TERRITORIOS_DIR_COM'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    V_COMMENT_TABLE VARCHAR2(500 CHAR):= 'Diccionario de territorios'; -- Vble. para los comentarios de las tablas

    BEGIN

    -- ******** DD_TDC_TERRITORIOS_DIR_COM *******
    DBMS_OUTPUT.PUT_LINE('******** '||V_TEXT_TABLA||' ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.'||V_TEXT_TABLA||'... Comprobaciones previas'); 
    
    -- Creacion Tabla DD_TDC_TERRITORIOS_DIR_COM
    ...
    ELSE  
    	 ...
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
					(DD_TDC_ID NUMBER (16,0) NOT NULL ENABLE
				  , DD_TDC_CODIGO VARCHAR2(20 CHAR) NOT NULL ENABLE
				  , DD_TDC_DESCRIPCION VARCHAR2(100 CHAR) NOT NULL ENABLE
				  , DD_TDC_DESCRIPCION_LARGA VARCHAR2(250 CHAR) NOT NULL ENABLE
    	...
    	IF V_NUM_TABLAS = 1 THEN 
    		...
    		ELSE
			EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT '||V_TEXT_TABLA||'_PK PRIMARY KEY (DD_TDC_ID)';
		...
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TDC_ID IS ''ID único del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TDC_CODIGO IS ''Código único del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TDC_DESCRIPCION IS ''Descripción del registro del diccionario''';
		EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_TDC_DESCRIPCION_LARGA IS ''Descripción larga del registro del diccionario''';
