...
--## PRODUCTO=NO
--## Finalidad: Columna nueva en ACT_CAN_CALIFICACION_NEG
--##           
--## INSTRUCCIONES: Inserta la columna AHT_ID en la tabla ACT_CAN_CALIFICACION_NEG
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_COLUMN_NAME VARCHAR2(2400 CHAR) := 'AHT_ID'; -- Vble. auxiliar para almacenar el nombre de la columna a añadir.
    ...
    ------ ESQUEMA_ORIGEN ---- 		TABLA_ORIGEN --------	 CAMPO_ORIGEN -----			ESQUEMA_DESTINO ------- TABLA_DESTINO -------------CAMPO_DESTINO --------NOMBRE_F -------------------------
   		 T_FK(''||V_ESQUEMA||'', ''||V_TEXT_TABLA||''   ,''||V_COLUMN_NAME||''       ,''||V_ESQUEMA||''    	,'ACT_AHT_HIST_TRAM_TITULO'    ,'AHT_ID'         ,'FK_CAN_AHT_ID')
  );      
  V_TMP_FK T_FK;
...
BEGIN
    ...
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_COLUMN_NAME||' IS ''Campo que relaciona la calificación negativa con histórico al que pertenece.'' ';      
