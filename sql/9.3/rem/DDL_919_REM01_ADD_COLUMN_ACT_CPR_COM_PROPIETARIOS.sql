...
--## INCIDENCIA_LINK=HREOS-8523
--## PRODUCTO=NO
--## Finalidad: Columna nueva en ACT_CPR_COM_PROPIETARIOS
--##           
--## INSTRUCCIONES: Inserta la columna CPR_ASISTENCIA_JUNTA_OBLIGA en la tabla ACT_CPR_COM_PROPIETARIOS
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_COLUMN_NAME VARCHAR2(2400 CHAR) := 'CPR_ASISTENCIA_JUNTA_OBLIGA'; -- Vble. auxiliar para almacenar el nombre de la columna a añadir.
    ...
    ------ ESQUEMA_ORIGEN ---- TABLA_ORIGEN -------- CAMPO_ORIGEN -----ESQUEMA_DESTINO ------
    T_FK(''||V_ESQUEMA||'', ''||V_TEXT_TABLA||''   ,'DD_TAL_ID'       ,''||V_ESQUEMA||'')
  );      
  V_TMP_FK T_FK;
...
BEGIN
    ...
    V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.'||V_COLUMN_NAME||' IS ''Campo que verifica en los datos comunidad de propietarios si la asistencia a junta es obligatoria o no'' ';      
