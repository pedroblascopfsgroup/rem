...
DECLARE
	...
	V_COL VARCHAR2(50 CHAR):= 'TBJ_NOMBRE_UG'; --Nombre de la columna
	V_COL1 VARCHAR2(50 CHAR):= 'TBJ_NOMBRE_EXPEDIENTE_TRABAJO'; --Nombre de la columna
	V_COL2 VARCHAR2(50 CHAR):= 'TBJ_NOMBRE_PROYECTO'; --Nombre de la columna
...
--Comprobacion de la tabla
  ...
  IF V_NUM_TABLAS > 0 THEN  
            ...
            IF V_NUM_TABLAS = 0 THEN
                ...
                V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.'||V_COL||' IS ''DND: Id Ug.''';
...
--Comprobacion de la tabla
  ...
  IF V_NUM_TABLAS > 0 THEN  
            ...
            IF V_NUM_TABLAS = 0 THEN
                ...
                EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD '||V_COL1||' '||V_TIPO1||'';
                 V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.'||V_COL1||' IS ''DND: Id Expediente trabajo''';
				EXECUTE IMMEDIATE V_MSQL;
            ELSE
  ...
  IF V_NUM_TABLAS > 0 THEN  
            ...
            IF V_NUM_TABLAS = 0 THEN
                ...
                -- Añadimos el campo
                EXECUTE IMMEDIATE 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD '||V_COL2||' '||V_TIPO1||'';
                 V_MSQL := 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TABLA||'.'||V_COL2||' IS ''DND: Id proyecto''';
				EXECUTE IMMEDIATE V_MSQL;
            ELSE
                DBMS_OUTPUT.PUT_LINE('  [INFO] El campo '||V_TABLA||'.'||V_COL1||'... YA existe.');
            END IF;    
