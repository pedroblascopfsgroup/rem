...
DECLARE
	...
	-- 	COLUMN 					COLUMN 			TABLE 						FK_NAME
		T_FK('GPV_ID_PADRE', 	'GPV_ID', 		'GPV_GASTOS_PROVEEDOR', 	'FK_GPV_ID_PADRE'), 
		T_FK('GPV_ID_SUPLIDO', 	'GPV_ID', 		'GPV_GASTOS_PROVEEDOR', 	'FK_GPV_ID_SUPLIDO')
	); 
	V_TMP_FK T_FK;
...
BEGIN
	...
	-- Creamos FK's
 	FOR I IN V_FK.FIRST .. V_FK.LAST LOOP
		V_TMP_FK := V_FK(I);
		--V_SQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TABLA||' ADD FOREIGN KEY ('||TRIM(V_TMP_FK(1))||') REFERENCES '||V_ESQUEMA||'.'||TRIM(V_TMP_FK(3))||'('||TRIM(V_TMP_FK(2))||')';
		--EXECUTE IMMEDIATE V_SQL;
		--DBMS_OUTPUT.PUT_LINE('	[INFO] '||V_ESQUEMA||'.S_'||V_TABLA||'... Secuencia creada');
