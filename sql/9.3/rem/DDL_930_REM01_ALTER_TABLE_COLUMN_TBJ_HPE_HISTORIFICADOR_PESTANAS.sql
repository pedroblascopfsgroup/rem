...
--## INCIDENCIA_LINK=HREOS-10907
--## PRODUCTO=NO
--## 
--## Finalidad: Modificar campos TBJ_HPE_HISTORIFICADOR_PESTANAS
--##            
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  V_COL T_ARRAY_COL := T_ARRAY_COL(
      -- Recordatorios
    T_COL(''||V_ESQUEMA||'', 'TBJ_HPE_HISTORIFICADOR_PESTANAS', 'VALOR_ANTERIOR', 'VARCHAR2(500 CHAR)'),
    T_COL(''||V_ESQUEMA||'', 'TBJ_HPE_HISTORIFICADOR_PESTANAS', 'VALOR_NUEVO', 'VARCHAR2(500 CHAR)')
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO MODIFICACION CAMPOS]');
    ...
    LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN              
            ...
            IF V_NUM_TABLAS > 0 THEN
                DBMS_OUTPUT.PUT_LINE('  [INFO] Modificar columna '||V_TMP_COL(3)||'.');  
                
                -- Añadimos el campo
                EXECUTE IMMEDIATE 'ALTER TABLE '||V_TMP_COL(1)||'.'||V_TMP_COL(2)||' MODIFY '||V_TMP_COL(3)||' '||V_TMP_COL(4)||' '; 
                DBMS_OUTPUT.PUT_LINE('  [INFO] El campo '||V_TMP_COL(2)||'.'||V_TMP_COL(3)||'... ha sido modificado'); 
            ELSE
               DBMS_OUTPUT.PUT_LINE('  [INFO] ' ||V_TMP_COL(1)|| '.'||V_TMP_COL(2)||'.'||V_TMP_COL(3)||'... No existe.');   
            END IF;    
    ...
    DBMS_OUTPUT.PUT_LINE('[FIN MODIFICACION CAMPOS]');
