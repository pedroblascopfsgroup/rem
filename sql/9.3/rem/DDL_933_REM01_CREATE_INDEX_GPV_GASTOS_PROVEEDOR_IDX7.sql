...
DECLARE
    ...
    BEGIN
				...
				V_MSQL := 'SELECT COUNT(1) FROM ALL_INDEXES WHERE INDEX_NAME = ''GPV_GASTOS_PROVEEDOR_IDX7'' ';
        
				EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
				IF V_NUM_TABLAS = 0 THEN
						V_MSQL := 'CREATE INDEX '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR_IDX7 ON '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR (PFA_ID) TABLESPACE '||V_TABLESPACE_IDX;		
						EXECUTE IMMEDIATE V_MSQL;
						DBMS_OUTPUT.PUT_LINE('[INFO] Indice GPV_GASTOS_PROVEEDOR_IDX7 - creado.');
				ELSE
						DBMS_OUTPUT.PUT_LINE('[INFO] Ya existe el índice GPV_GASTOS_PROVEEDOR_IDX7 ');
				END IF;
