...
--## PRODUCTO=NO
--## Finalidad: DDL Creación de tabla DD_IRE_IDENTIFICADOR_REAM
--##           
...
BEGIN
	...
	(
			DD_IRE_ID								NUMBER(16,0)                NOT NULL,
			DD_IRE_CODIGO        					VARCHAR2(20 CHAR)          	NOT NULL,
			DD_IRE_DESCRIPCION       				VARCHAR2(100 CHAR),
			DD_IRE_DESCRIPCION_LARGA  				VARCHAR2(250 CHAR),
	...
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_IRE_ID IS ''Código identificador único.'' ';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_IRE_CODIGO IS ''Código identificador.'' ';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_IRE_DESCRIPCION IS ''Descripción de los identificadores.'' ';
	EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||V_ESQUEMA||'.'||V_TEXT_TABLA||'.DD_IRE_DESCRIPCION_LARGA IS ''Descripción larga de los identificadores.'' ';
