...
--## INCIDENCIA_LINK=HREOS-11781
--## PRODUCTO=SI
--## 
--## Finalidad: Incluir campo DD_TTR_ID en tabla ACT_ACTIVO.
--##            
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  ERR_MSG VARCHAR2(1024 CHAR); -- Vble. auxiliar para registrar errores en el script.
    ...
    V_TABLA_FK VARCHAR2(2400 CHAR) := 'DD_TTR_TIPO_TRANSMISION';
    V_COLUMNA_FK VARCHAR2(2400 CHAR) := 'DD_TTR_ID';
   /* -- ARRAY CON NUEVAS COLUMNAS */
    TYPE T_ALTER IS TABLE OF VARCHAR2(4000);
    TYPE T_ARRAY_ALTER IS TABLE OF T_ALTER;
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    -- NOMBRE CAMPO	TIPO CAMPO	DESCRIPCION
    T_ALTER(  'DD_TTR_ID',	'NUMBER(16,0)',	'Tipo de Transmision FK a DD_TTR_TIPO_TRANSMISION'	)
    );
    V_T_ALTER T_ALTER;
...
V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD CONSTRAINT FK_'||V_T_ALTER(1)||' FOREIGN KEY ('||V_T_ALTER(1)||') REFERENCES '||V_ESQUEMA||'.'||V_TABLA_FK||' ('||V_COLUMNA_FK||') ';
