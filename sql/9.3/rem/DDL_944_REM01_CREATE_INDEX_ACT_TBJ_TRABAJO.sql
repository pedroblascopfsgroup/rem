...
DECLARE
    ...
    BEGIN
				...
				V_MSQL := 'SELECT COUNT(1) FROM ALL_INDEXES WHERE INDEX_NAME = ''ACT_TBJ_TRABAJO_IDX_BORRADO'' ';
        
				EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;	
				IF V_NUM_TABLAS = 0 THEN
						V_MSQL := 'CREATE INDEX '||V_ESQUEMA||'.ACT_TBJ_TRABAJO_IDX_BORRADO ON '||V_ESQUEMA||'.ACT_TBJ_TRABAJO (TBJ_ID, BORRADO) TABLESPACE '||V_TABLESPACE_IDX;		
						EXECUTE IMMEDIATE V_MSQL;
						DBMS_OUTPUT.PUT_LINE('[INFO] Indice ACT_TBJ_TRABAJO_IDX_BORRADO - creado.');
				ELSE
						DBMS_OUTPUT.PUT_LINE('[INFO] Ya existe el índice ACT_TBJ_TRABAJO_IDX_BORRADO ');
				END IF;
