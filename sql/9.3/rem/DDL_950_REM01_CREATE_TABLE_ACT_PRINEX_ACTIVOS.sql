...
BEGIN
    ...
    IF V_NUM_TABLAS = 1 THEN 
      ...
      DBMS_OUTPUT.PUT_LINE('[CREAMOS '||V_TABLA||']');
    	 V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.'||V_TABLA||' (
				  PRIMEX_ID                     	NUMBER(16,0)        NOT NULL
				 ...
				 ,CONSTRAINT PK_PRIMEX_ID PRIMARY KEY(PRIMEX_ID)
				 ,CONSTRAINT FK_PRIMEX_ACT_ID FOREIGN KEY (ACT_ID) REFERENCES '||V_ESQUEMA||'.ACT_ACTIVO (ACT_ID)	
         ,CONSTRAINT FK_PRIMEX_DD_DIA_ID FOREIGN KEY (DD_DIA_ID) REFERENCES '||V_ESQUEMA||'.DD_DIA_DISP_ADMINISTRATIVO (DD_DIA_ID)
         ,CONSTRAINT FK_PRIMEX_DD_DIT_ID FOREIGN KEY (DD_DIT_ID) REFERENCES '||V_ESQUEMA||'.DD_DIT_DISP_TECNICO (DD_DIT_ID)
         ,CONSTRAINT FK_PRIMEX_DD_MTC_ID FOREIGN KEY (DD_MTC_ID) REFERENCES '||V_ESQUEMA||'.DD_MTC_MOTIVO_TECNICO (DD_MTC_ID)			
			  )';         	
    ...
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA || '.'||V_TABLA||'.PRIMEX_ID IS ''Identificador de tabla''';
