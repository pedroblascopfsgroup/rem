...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-5383';
    V_ARRAY T_ARRAY := T_ARRAY (
        T_LISTA ('grucclbk', NULL, 'grucclbk', 'GCCLBK', 'GCCLBK', 'Gestor Capa Control Liberbank', 'PERFGCCLIBERBANK' , 'Gestor Capa Control Liberbank', 'GCCLBK'  , 'Gestor Capa Control Liberbank', NULL, 0)
    );
    V_TMP_LISTA T_LISTA;
...
BEGIN
	...
	V_SQL := 'SELECT count(1) FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''grucclbk'' ';
	...
	IF V_NUM_TABLAS > 0 THEN
	    ...
	    V_MSQL := ' 
			UPDATE '||V_ESQUEMA_M||'.USU_USUARIOS SET USU_NOMBRE = ''Grupo Capa Control Liberbank'', 
			USUARIOMODIFICAR = ''REMVIP-5383'', 
			FECHAMODIFICAR = SYSDATE 
			WHERE USU_USERNAME = '''||V_USUARIO||''' ';
	...
	V_SQL := 'SELECT count(1) FROM '||V_ESQUEMA||'.TGP_TIPO_GESTOR_PROPIEDAD WHERE TGP_VALOR = ''GCCLBK'' ';
	...
	IF V_NUM_TABLAS_2 = 0 THEN
	    ...
	    MSQL := 'INSERT INTO '||V_ESQUEMA||'.TGP_TIPO_GESTOR_PROPIEDAD' ||
							...
							',(SELECT DD_TGE_ID FROM '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR WHERE DD_TGE_CODIGO = ''GCCLBK'')' ||
							',''DES_VALIDOS'' ' ||
