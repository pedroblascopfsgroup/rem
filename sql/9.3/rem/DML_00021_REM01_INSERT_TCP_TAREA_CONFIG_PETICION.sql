  ...
  IF V_NUM_TABLAS = 1 THEN 
	...
	-- Si existe el TAP_CODIGO en la tabla hacemos el INSERT	
			IF V_NUM_T017 = 1 THEN  
				...
				V_SQL_TAP_ID_T017_CONF:= 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA_REL||' WHERE  TAP_CODIGO = ''T017_PBCReserva'')';
				...
				ELSE 
					EXECUTE IMMEDIATE 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' SET TCP_ACTIVADA = 1, USUARIOMODIFICAR = ''REMVIP-5080'', FECHAMODIFICAR = SYSDATE, BORRADO = 0, TCP_PERMITIDA = 1 WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA_REL||' WHERE  TAP_CODIGO = ''T017_PBCReserva'')';
				END IF;
	...
	-- Si existe el TAP_CODIGO en la tabla hacemos el INSERT	
			IF V_NUM_T017_2 = 1 THEN  
				...
				V_SQL_TAP_ID_T017_CONF_2:= 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA_REL||' WHERE  TAP_CODIGO = ''T017_PBCVenta'')';
				EXECUTE IMMEDIATE V_SQL_TAP_ID_T017_CONF_2 INTO V_NUM_T017_CONF_2;
				-- Si no existe el TAP_ID en la tabla hacemos el INSERT	
				IF V_NUM_T017_CONF_2 = 0 THEN 
					EXECUTE IMMEDIATE 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
					       'TCP_ID, 
						TAP_ID,
						TCP_ACTIVADA, 
					...
					DBMS_OUTPUT.PUT_LINE('SE HA INSERTADO CORRECTAMENTE EL REGISTRO EN LA TABLA TCP_TAREA_CONFIG_PETICION');
				ELSE 
					EXECUTE IMMEDIATE 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' SET TCP_ACTIVADA = 1, USUARIOMODIFICAR = ''REMVIP-5080'', FECHAMODIFICAR = SYSDATE, BORRADO = 0, TCP_PERMITIDA = 1 WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA_REL||' WHERE  TAP_CODIGO = ''T017_PBCVenta'')';
				END IF;
