...
V_USUARIOMODIFICAR VARCHAR2(50 CHAR) := 'REMVIP-5885';
        ...
        IF V_NUM_TABLAS > 0 THEN              
            ...
            IF V_ENTORNO = 1 THEN
        		...
        		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' 
							WHERE USU_USERNAME = ''buzonboarding''';
        		EXECUTE IMMEDIATE V_SQL INTO V_NUM_REGISTROS;

		        IF V_NUM_REGISTROS > 0 THEN
		          	V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||'  
		          				SET  USU_NOMBRE = ''BUZON BOARDING'', USU_MAIL = ''boardingofertas@haya.es'', USUARIOMODIFICAR = '''||V_USUARIOMODIFICAR||''', FECHAMODIFICAR = SYSDATE
								WHERE USU_USERNAME = ''buzonboarding''';		     		
		        	EXECUTE IMMEDIATE V_MSQL;
		        	DBMS_OUTPUT.PUT_LINE('[INFO] Modifiado usuario buzonboarding.......');
		        ELSE
		      		V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' (USU_ID, USU_USERNAME, USU_PASSWORD, USU_NOMBRE, USU_MAIL, USUARIOCREAR, FECHACREAR) 
		          				VALUES ('||V_ESQUEMA_M||'.S_USU_USUARIOS.NEXTVAL, ''buzonboarding'', ''QTerWb'', ''BUZON BOARDING'', ''boardingofertas@haya.es'', '''||V_USUARIOMODIFICAR||''', SYSDATE)';		     		
		        	EXECUTE IMMEDIATE V_MSQL;
		        	DBMS_OUTPUT.PUT_LINE('[INFO] Creado usuario buzonboarding.......');
		        END IF;
		    ...
		    ELSE
        		...
        		IF V_NUM_REGISTROS > 0 THEN
		          	V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||'  
		          				SET  USU_NOMBRE = ''BUZON BOARDING'', USU_MAIL = ''pruebashrem@gmail.com'', USUARIOMODIFICAR = '''||V_USUARIOMODIFICAR||''', FECHAMODIFICAR = SYSDATE
								WHERE USU_USERNAME = ''buzonboarding''';		     		
		        	EXECUTE IMMEDIATE V_MSQL;
		        	DBMS_OUTPUT.PUT_LINE('[INFO] Modifiado usuario buzonboarding.......');
		        ELSE		    
			    	V_MSQL := 'INSERT INTO '||V_ESQUEMA_M||'.'||V_TEXT_TABLA||' (USU_ID, USU_USERNAME, USU_PASSWORD, USU_NOMBRE, USU_MAIL, USUARIOCREAR, FECHACREAR) 
			          VALUES ('||V_ESQUEMA_M||'.S_USU_USUARIOS.NEXTVAL, ''buzonboarding'', ''1234'', ''BUZON BOARDING'', ''pruebashrem@gmail.com'', '''||V_USUARIOMODIFICAR||''', SYSDATE)';
			      	DBMS_OUTPUT.PUT_LINE('[INFO] Insertando usuario ficticio buzonboarding.......');
					EXECUTE IMMEDIATE V_MSQL;
				END IF;
			END IF;
