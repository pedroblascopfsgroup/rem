...
DECLARE    
    ...
    V_NUM_TABLAS NUMBER(16); -- Vble. para validar la existencia de una tabla.
		...
		V_USU_CREAR VARCHAR2(30 CHAR) := '''HREOS-8706'''; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.
    ...
    TYPE T_ARRAY_TAREA IS TABLE OF T_TIPO_TAREA;
		...
		V_TAREA_T017 VARCHAR2(4000 CHAR);
    ...
    (
		...
		T_TIPO_DATA('T013_PBCReserva',					'1'	,				'comboRespuesta',					 'SELECT DD_APD_CODIGO FROM '||V_ESQUEMA||'.DD_APD_APRUEBA_DENIEGA', 		'IN'),
		T_TIPO_DATA('T013_PBCReserva',					'1'	,				'comboRespuesta',				 	'', 																		'IS NOT NULL')
...
BEGIN	
	...
	FOR I IN V_TIPO_TAREA.FIRST .. V_TIPO_TAREA.LAST
		LOOP
			...
			-- Recuperar TAP_ID de T013_PBCReserva 
			V_TAREA_T017 := 'SELECT TAP_ID FROM TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = '''||TRIM(V_TMP_TIPO_TAREA(1))||''' AND BORRADO = 0';
			EXECUTE IMMEDIATE V_TAREA_T017 INTO V_NUM_TAREA_T013;
	...
	FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
		LOOP
			...
			V_CAMPO := 'SELECT TFI_ID FROM TFI_TAREAS_FORM_ITEMS WHERE TFI_NOMBRE = '''||TRIM(V_TMP_TIPO_DATA(3))||''' AND TAP_ID = '''||V_TAP_ID||''' AND BORRADO = 0';
