...
--## PRODUCTO=NO
--##
--## Finalidad: Activado Carga Masiva Cambio API.
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
BEGIN
	...
	V_SQL := 'SELECT COUNT(1) FROM ALL_TABlES WHERE TABLE_NAME = '''||V_TEXT_TABLA||''' AND OWNER = '''||V_ESQUEMA||'''';
		...
		IF V_NUM_TABLAS > 0 THEN
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] Inicio activado Carga Masiva Cambio API de DD_OPM_OPERACION_MASIVA');

			V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' OPM
				SET OPM.BORRADO = 0, OPM.FECHAMODIFICAR = SYSDATE, OPM.USUARIOMODIFICAR = ''HREOS-8750''
