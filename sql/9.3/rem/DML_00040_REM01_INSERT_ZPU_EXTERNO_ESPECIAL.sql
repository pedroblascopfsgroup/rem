...
DECLARE
    ...
    --			USU_USERNAME			CODIGO PERFIL
		T_FUNCION('ogf.lnogales', 'EXTERNOESPECIAL'),
		T_FUNCION('ogf.lromero', 'EXTERNOESPECIAL'),
		T_FUNCION('ogf.oruiz', 'EXTERNOESPECIAL'),
		T_FUNCION('ogf.mamich', 'EXTERNOESPECIAL'),
		T_FUNCION('ogf.sdiaz', 'EXTERNOESPECIAL'),
		T_FUNCION('ogf.gherrero', 'EXTERNOESPECIAL'),
		T_FUNCION('malcalde', 'EXTERNOESPECIAL')
    );
  			...
  			ELSE
				...
				ELSE
					V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU' ||
								...
								' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||'''),' ||
								' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(1))||'''),' ||
								' ''REMVIP-6056'',SYSDATE,0 FROM DUAL';
