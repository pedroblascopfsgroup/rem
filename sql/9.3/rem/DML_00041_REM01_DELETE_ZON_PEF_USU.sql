...
DECLARE
    ...
    --			USU_USERNAME			CODIGO PERFIL
	  T_FUNCION('ejust',	'HAYABACKOFFICE'),
	  T_FUNCION('ejust',	'HAYAGESTFORM'),
	  T_FUNCION('ejust',	'HAYASUPCOM'),
	  T_FUNCION('ejust',	'SUPFVD')
...
BEGIN	        
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: BORRADO LOGICO EN ZON_PEF_USU] ');
     FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      LOOP
			...
			IF V_NUM_TABLAS = 0 THEN	  
				DBMS_OUTPUT.PUT_LINE('[INFO] No existen los datos en la tabla '||V_ESQUEMA||'.ZON_PEF_USU...no se modifica nada.');
			...
			ELSE
				...
				ELSE
					   ...
					   V_MSQL_1 := 	'UPDATE '||V_ESQUEMA||'.ZON_PEF_USU
							SET BORRADO = 1,
							...
							AND PEF_ID = (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||''')';
								
					EXECUTE IMMEDIATE V_MSQL_1;
					DBMS_OUTPUT.PUT_LINE('[INFO] Datos de la tabla '||V_ESQUEMA||'.ZON_PEF_USU modificados correctamente.');
