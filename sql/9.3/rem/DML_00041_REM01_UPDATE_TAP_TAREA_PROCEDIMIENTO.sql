...
DECLARE
    ...
    V_USUARIO VARCHAR2(20 CHAR) := 'HREOS-8889'; 
  	...
  	IF V_NUM_TABLAS = 1 THEN
    	...
    	IF V_NUM = 1 THEN
            ...
            V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
                    SET TAP_DESCRIPCION = ''Resolución comité''
    	...
    	V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' 
		WHERE DD_TPO_ID = (SELECT DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO= ''T017'') AND TAP_CODIGO = ''T017_RespuestaOfertanteCES'' ';
    	...
    	IF V_NUM = 1 THEN
            ...
            V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
                    SET TAP_DESCRIPCION = ''Respuesta ofertante comité''
					, USUARIOMODIFICAR = '''||V_USUARIO||'''
					, FECHAMODIFICAR = SYSDATE
            		WHERE DD_TPO_ID = (SELECT DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO= ''T017'') AND  TAP_CODIGO = ''T017_RespuestaOfertanteCES''';
            EXECUTE IMMEDIATE V_MSQL;
			    DBMS_OUTPUT.PUT_LINE('Actualizado descripción de tarea T017_RespuestaOfertanteCES de '||V_TEXT_TABLA);
    	END IF;
    	
    	
    	
    	V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' 
		WHERE DD_TPO_ID = (SELECT DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO= ''T017'') AND TAP_CODIGO = ''T017_RatificacionComiteCES'' ';
    	...
    	IF V_NUM = 1 THEN
            ...
            V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
                    SET TAP_DESCRIPCION = ''Ratificación comité''
					, USUARIOMODIFICAR = '''||V_USUARIO||'''
					, FECHAMODIFICAR = SYSDATE
            		WHERE DD_TPO_ID = (SELECT DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO= ''T017'') AND  TAP_CODIGO = ''T017_RatificacionComiteCES''';
            EXECUTE IMMEDIATE V_MSQL;
			    DBMS_OUTPUT.PUT_LINE('Actualizado descripción de tarea T017_RatificacionComiteCES de '||V_TEXT_TABLA);
    	END IF;    	
