...
DECLARE
    ...
    --			USU_USERNAME			CODIGO PERFIL
	  T_FUNCION('rcf',	'HAYAGESTFORM'),
	  T_FUNCION('sch',	'HAYAGESTFORM'),
	  T_FUNCION('pmoliner',	'HAYAGESTFORM'),
	  T_FUNCION('vlouzan',	'HAYAGESTFORM'),
	  T_FUNCION('jcano',	'HAYAGESTFORM'),
	  T_FUNCION('rescuredo',	'HAYAGESTFORM'),
	  T_FUNCION('mperezg',	'HAYAGESTFORM'),
	  T_FUNCION('agonzalezr',	'HAYAGESTFORM'),
	  T_FUNCION('lmorillom',	'HAYAGESTFORM')
			...
			ELSE
				...
				ELSE
					V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU' ||
								...
								' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||'''),' ||
								' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(1))||'''),' ||
								' ''HREOS-8363'',SYSDATE,0 FROM DUAL';
