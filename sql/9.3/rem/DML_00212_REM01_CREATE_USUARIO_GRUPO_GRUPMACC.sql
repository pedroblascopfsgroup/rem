...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-7786';
    V_ARRAY T_ARRAY := T_ARRAY (
        T_LISTA ('grupgmacc', NULL, 'grupgmacc', 'GESTCOMALQ', 'GESTCOMALQ', 'Gestor comercial alquiler', 'GESTALQ' , 'Gestor de alquileres', 'GESTCOMALQ'  , 'Gestor comercial alquiler', NULL, 0)
    );
    V_TMP_LISTA T_LISTA;
...
BEGIN
	...
	V_SQL := 'SELECT count(1) FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''grupgmacc'' ';
	...
	IF V_NUM_TABLAS > 0 THEN
	     ...
	     -- Verificar el entorno en el que estamos
			...
			DBMS_OUTPUT.PUT_LINE('[INFO]: ACTUALIZAR USU_USUARIOS EN PRO ' );    
			...
			DBMS_OUTPUT.PUT_LINE('[INFO]: ACTUALIZAR USU_USUARIOS EN VAL/DESA ' );    

			V_MSQL := ' 
				UPDATE '||V_ESQUEMA_M||'.USU_USUARIOS SET USU_NOMBRE = ''Grupo de gestores MACC'',
				USU_MAIL = ''pruebashrems@gmail.com'',
				USUARIOMODIFICAR = ''REMVIP-7786'', 
				FECHAMODIFICAR = SYSDATE 
				WHERE USU_USERNAME = ''grupgmacc'' ';
...
/*
	V_SQL := 'SELECT count(1) FROM '||V_ESQUEMA||'.TGP_TIPO_GESTOR_PROPIEDAD WHERE TGP_VALOR = ''GACT'' ';
	...
	IF V_NUM_TABLAS_2 = 0 THEN
	    ...
	    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.TGP_TIPO_GESTOR_PROPIEDAD' ||
							...
							',''DES_VALIDOS'' ' ||
							', ''GACT'',0, '''||V_USUARIO||''', SYSDATE, 0 FROM DUAL';
