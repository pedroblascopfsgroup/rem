...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-7786'; -- USUARIO CREAR/MODIFICAR
    V_TABLA VARCHAR2(50 CHAR) := 'GRU_GRUPOS_USUARIOS';
    V_USUARIO_GRUPO VARCHAR2(50 CHAR) := 'grupgmacc';
	...
	T_FUNCION( 'grupgmacc' , 'nserrano'),
	T_FUNCION( 'grupgmacc' , 'mrodriguezmo'),
	T_FUNCION( 'grupgmacc' , 'fpereza'),
	T_FUNCION( 'grupgmacc' , 'ext.fborreguero')
...
BEGIN	
	...
	V_SQL := 'SELECT count(1) FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_USUARIO_GRUPO||''' ';
		...
		ELSE
			DBMS_OUTPUT.PUT_LINE('[ INFO ]: El grupo no existe.');
		END IF;	
