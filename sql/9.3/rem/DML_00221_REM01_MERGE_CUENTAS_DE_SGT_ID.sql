...
--## PRODUCTO=NO
--##
--## Finalidad: Script que fusiona los datos de SGT_ID en ACT_CONFIG_CTAS_CONTABLES
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
BEGIN	
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: MERGE EN  '||V_TEXT_TABLA||' ');  
          V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' CTAS USING
                        (SELECT CTAS.CCC_CTAS_ID, SGT.SGT_ID
                        FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' CTAS
                        JOIN '||V_ESQUEMA||'.ACT_SGT_SUBTIPO_GPV_TBJ SGT ON CTAS.DD_STG_ID = SGT.DD_STG_ID
                        ...
                        ON (CTAS.CCC_CTAS_ID = AUX.CCC_CTAS_ID)
                        WHEN MATCHED THEN UPDATE SET CTAS.SGT_ID = AUX.SGT_ID';
