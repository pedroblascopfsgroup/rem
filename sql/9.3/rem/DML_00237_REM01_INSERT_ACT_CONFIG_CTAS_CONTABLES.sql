			...
			V_TMP_TIPO_DATA := V_TIPO_DATA(I);
				...
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (
							...
							SELECT 
								...
								, '''||TRIM(V_TMP_TIPO_DATA(10))||''' CCC_CUENTA_CONTABLE
								, TGA.DD_TGA_ID
								, STG.DD_STG_ID
								, CRA.DD_CRA_ID
								, PRO.PRO_ID
								...
								, '||TRIM(V_TMP_TIPO_DATA(9))||' CCC_PLAN_VISITAS
								, TCH.DD_TCH_ID
								, '||TRIM(V_TMP_TIPO_DATA(7))||' CCC_PRINCIPAL
				...
				DBMS_OUTPUT.PUT_LINE('[INFO]: Se ha insertado el valor '''||TRIM(V_TMP_TIPO_DATA(2))||''', '''||TRIM(V_TMP_TIPO_DATA(5))||''' y '''||TRIM(V_TMP_TIPO_DATA(10))||'''');
		END LOOP;
	END IF;
	COMMIT;
