...
DBMS_OUTPUT.PUT_LINE('[INICIO] EXISTE LA TABLA, PROCEDEMOS A INSERTAR...');
	V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ACT_SAREB_ACTIVOS (
			ASA_ID,
			ACT_ID,
			DD_TPA_ID,
			DD_SAC_ID,
			...
			,SYSDATE -- FECHACREAR
			,ACT.USUARIOBORRAR
			,ACT.FECHABORRAR -- FECHABORRAR
			,ACT.BORRADO -- BORRADO
			FROM
			'||V_ESQUEMA||'.ACT_ACTIVO ACT
			LEFT JOIN '||V_ESQUEMA||'.ACT_LOC_LOCALIZACION ACTLOC ON ACTLOC.ACT_ID = ACT.ACT_ID
			LEFT JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION LOC ON LOC.BIE_LOC_ID = ACTLOC.BIE_LOC_ID
