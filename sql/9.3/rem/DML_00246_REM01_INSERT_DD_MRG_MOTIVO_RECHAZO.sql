...
DECLARE
    ...
    T_TIPO_DATA('F56' ,'Gasto cíclico de Bankia sin partida informada', '1', 'WHERE EXISTS(SELECT 1 FROM #ESQUEMA#.ACT_ACTIVO ACT JOIN #ESQUEMA#.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID AND CRA.BORRADO = 0 WHERE ACT.ACT_NUM_ACTIVO = AUX.COD_ACTIVO AND CRA.DD_CRA_CODIGO = ''''03'''') AND AUX.ID_PRIMER_GASTO_SERIE IS NOT NULL AND AUX.TIPO_ENVIO = ''''01'''' AND AUX.PARTIDA_PRESU_ESPECIAL IS NULL')
		); 
