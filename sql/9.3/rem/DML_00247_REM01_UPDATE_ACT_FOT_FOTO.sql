...
BEGIN
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1 
                    USING (SELECT DISTINCT FOT.FOT_ID, DD.DD_DFA_ID, DD.DD_DFA_DESCRIPCION
                    FROM '||V_ESQUEMA||'.'||V_TABLA||' FOT
                    INNER JOIN '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_ID = FOT.AGR_ID 
                    INNER JOIN '||V_ESQUEMA||'.DD_DFA_DESCRIPCION_FOTO_ACTIVO DD ON DD.DD_SAC_ID = 23
                    WHERE FOT.BORRADO = 0 AND AGR.BORRADO = 0 AND FOT.ACT_ID IS NULL AND FOT.SDV_ID IS NULL
                    AND LOWER(FOT.FOT_DESCRIPCION) LIKE ''%''||LOWER(DD.DD_DFA_DESCRIPCION)||''%'') T2
                    ON (T1.FOT_ID = T2.FOT_ID)
                    WHEN MATCHED THEN UPDATE SET 
                    T1.DD_DFA_ID = T2.DD_DFA_ID,
                    T1.FOT_DESCRIPCION = T2.DD_DFA_DESCRIPCION,
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1 
                    ...
                    WHERE FOT.BORRADO = 0 AND ACT.BORRADO = 0 AND FOT.AGR_ID IS NULL AND FOT.SDV_ID IS NULL) T2
                    ON (T1.FOT_ID = T2.FOT_ID)
                    WHEN MATCHED THEN UPDATE SET 
                    T1.DD_DFA_ID = T2.DD_DFA_ID,
                    T1.FOT_DESCRIPCION = T2.DD_DFA_DESCRIPCION, 
