...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('014','Vigilancia','ACT_SPS_SIT_POSESORIA','SPS_VIGILANCIA','ACT_ID','LEFT JOIN (SELECT CASE WHEN AUX.VALOR_NUEVO = 1 THEN 1 WHEN AUX.VALOR_NUEVO = 0 THEN 0 END SPS_VIGILANCIA, AUX.ACT_NUM_ACTIVO FROM '||V_ESQUEMA||'.ESPARTA_EXCEL1 AUX WHERE AUX.CAMPO = ''''014'''') CRUCE ON CRUCE.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO'),
      T_TIPO_DATA('015','Alarma','ACT_SPS_SIT_POSESORIA','SPS_ALARMA','ACT_ID','LEFT JOIN (SELECT CASE WHEN AUX.VALOR_NUEVO = 1 THEN 1 WHEN AUX.VALOR_NUEVO = 0 THEN 0 END SPS_ALARMA, AUX.ACT_NUM_ACTIVO FROM '||V_ESQUEMA||'.ESPARTA_EXCEL1 AUX WHERE AUX.CAMPO = ''''015'''') CRUCE ON CRUCE.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO')
    ); 
