...
DECLARE
	...
	-- 			TABLA 					DESCRIPCION 		DESCRIPCION_LARGA 		CAMPOS AFECTADOS
		T_VAR('DD_CRA_CARTERA', 		'BBVA', 			'BBVA', 				'DD_CRA_ID', 'DD_CRA_CODIGO', 'DD_CRA_DESCRIPCION', 'DD_CRA_DESCRIPCION_LARGA')
	); 
	V_TMP_VAR T_VAR;
	...
	FOR I IN V_VAR.FIRST .. V_VAR.LAST LOOP
		...
		IF V_NUM = 0 THEN
			...
			SELECT 
				...
				(
					SELECT MAX(DD_CRA_CODIGO)+1 FROM '||V_ESQUEMA||'.'||V_TMP_VAR(1)||'
				),
				'''||V_TMP_VAR(2)||''',
				'''||V_TMP_VAR(3)||''',
				0,
