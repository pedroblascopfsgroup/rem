...
DECLARE
	...
	V_USU VARCHAR2(50 CHAR) := 'HREOS-10578';
	...
	--				USU_USERNAME			CODIGO PERFIL
		T_FUNCION( 'saragon', 				'HAYAGESTPREC'),
		T_FUNCION( 'arueda', 				'HAYAGESTPREC'),
		T_FUNCION( 'rblanco', 				'HAYAGESTPREC'),
		T_FUNCION( 'blecea', 				'HAYAGESTPREC'),
		T_FUNCION( 'avilarino', 			'HAYAGESTPREC')
	);
...
BEGIN
	...
	FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST LOOP
		...
		ELSE
			...
			ELSE
				...
				V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ZON_PEF_USU WHERE PEF_ID = ( SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||''') 
					AND USU_ID = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(1))||''')';
