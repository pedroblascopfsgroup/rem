...
DECLARE
    ...
    --				Codigo						Descripcion								Descripcion Larga
    	T_TIPO_DATA('PERFSEGURIDAD',			'Perfil de Seguridad',			'Perfil de Seguridad')
    ); 
      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.PEF_PERFILES (' ||
                      ...
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''',''HREOS-11911'',SYSDATE,0 FROM DUAL';
