...
--## INCIDENCIA_LINK=HREOS-11911
--## PRODUCTO=NO
--##
--## Finalidad: Script que añade Usuarios al Perfil Seguridad
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    --			USU_USERNAME			CODIGO PERFIL
 	  T_FUNCION('SUPER',	'PERFSEGURIDAD'),
 	  T_FUNCION('dortiz',	'PERFSEGURIDAD'),
	  T_FUNCION('lfabe',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.lmena',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.jsanchez',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.dsanchez',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.mpedraza',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.rrodelgo',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.jreyez',	'PERFSEGURIDAD'),
 	  T_FUNCION('ext.mbarbon',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.jgonzalez',	'PERFSEGURIDAD'),
	  T_FUNCION('ext.fescribano',	'PERFSEGURIDAD')
			...
			ELSE
				...
				ELSE
					V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU' ||
								...
								' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||'''),' ||
								' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(1))||'''),' ||
								' ''HREOS-11911'',SYSDATE,0 FROM DUAL';
