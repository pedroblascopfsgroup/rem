...
DECLARE
	...
	V_USUARIOCREAR VARCHAR2(50 CHAR) := 'REMVIP-8611';

	TYPE T_PWD IS TABLE OF VARCHAR2(50 CHAR);
	V_PWD T_PWD := T_PWD('NFBIBHCSHA');
	...
	V_USER T_USUARIOS := T_USUARIOS(
		T_USU('buzongda', ''||V_PWD(1)||'', 'Buzón de tareas','','','1')
	...
	LOOP
		...
		IF V_COUNT > 0 THEN
			...
			V_SQL := 'UPDATE '||V_ESQUEMA_M||'.'||V_TABLA||' SET 
				...
				BORRADO = 0, 
				USU_FECHA_VIGENCIA_PASS = TO_DATE(''06/04/2022'',''DD/MM/YYYY'')
