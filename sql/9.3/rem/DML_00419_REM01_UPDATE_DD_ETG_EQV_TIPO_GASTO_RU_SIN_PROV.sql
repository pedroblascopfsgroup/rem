...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-16167';
    ...
    M_TABLA T_ARRAY_TABLA := T_ARRAY_TABLA(
      --DD_STG_ID|COGRUG_POS|COTACA_POS|COSBAC_POS|COGRUG_NEG|COTACA_NEG|COSBAC_NEG|Descrip Pos| Descrip Neg | SUBROGADO-----
...
BEGIN
  ...
  IF V_NUM_REGISTROS > 0 THEN
	...
	V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||'(DD_ETG_ID, DD_TGA_ID, DD_STG_ID
		  ...
		  ,BORRADO,SUBROGADO) VALUES ('||V_ESQUEMA||'.S_DD_ETG_EQV_TIPO_GASTO_RU.NEXTVAL, (SELECT DD_TGA_ID FROM '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO WHERE DD_TGA_CODIGO = '''||V_TMP_TABLA(1)||'''), (SELECT DD_STG_ID FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = '''||V_TMP_TABLA(2)||''')
		  ,'''||V_TMP_TABLA(3)||''', '''||V_TMP_TABLA(10)||''', '''||V_TMP_TABLA(10)||'''
		  ,'''||V_TMP_TABLA(4)||''', '''||V_TMP_TABLA(5)||''', '''||V_TMP_TABLA(6)||''', '''||V_TMP_TABLA(7)||''' , '''||V_TMP_TABLA(8)||''', '''||V_TMP_TABLA(9)||''', 0, ''HREOS-15516'',SYSDATE,0, '''||V_TMP_TABLA(11)||''')';
		EXECUTE IMMEDIATE V_SQL;	
