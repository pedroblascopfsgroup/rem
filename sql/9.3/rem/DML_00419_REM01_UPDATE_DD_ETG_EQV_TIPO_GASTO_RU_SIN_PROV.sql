...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-14421';
...
BEGIN
  ...
  IF V_NUM_REGISTROS > 0 THEN
	...
	V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||'(DD_ETG_ID, DD_TGA_ID, DD_STG_ID
		  ...
		  ,'''||V_TMP_TABLA(4)||''', '''||V_TMP_TABLA(5)||''', '''||V_TMP_TABLA(6)||''', '''||V_TMP_TABLA(7)||''' , '''||V_TMP_TABLA(8)||''', '''||V_TMP_TABLA(9)||''', 0, ''REMVIP-6858'',SYSDATE,0)';
		EXECUTE IMMEDIATE V_SQL;	
