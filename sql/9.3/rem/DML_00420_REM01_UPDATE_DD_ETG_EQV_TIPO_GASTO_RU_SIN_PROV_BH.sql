...
DECLARE
    ...
    M_TABLA T_ARRAY_TABLA := T_ARRAY_TABLA(
      --DD_STG_ID|COGRUG_POS|COTACA_POS|COSBAC_POS|COGRUG_NEG|COTACA_NEG|COSBAC_NEG|Descrip Pos| Descrip Neg | SUBROGADO-----
		T_TABLA('09','36','100','2','3','2','2','3','52',''),
		...
		T_TABLA('19','100','183','3','31','11','3','31','12','1'),
		T_TABLA('01','05','184','2','1','10','','','','')
...
BEGIN
  ...
  IF V_NUM_REGISTROS > 0 THEN
    ...
    IF '||V_TMP_TABLA(7)||' = '' AND '||V_TMP_TABLA(8)||' = '' AND '||V_TMP_TABLA(9)||' = '' THEN
	    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA_AUX||' WHERE 
						DD_TGA_ID = (SELECT DD_TGA_ID FROM '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO WHERE DD_TGA_CODIGO = '''||V_TMP_TABLA(1)||''') AND
						DD_STG_ID = (SELECT DD_STG_ID FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = '''||V_TMP_TABLA(2)||''') AND
						DD_ETG_CODIGO = '''||V_TMP_TABLA(3)||''' AND
						COGRUG_POS = '||V_TMP_TABLA(4)||' AND
						COTACA_POS = '||V_TMP_TABLA(5)||' AND
						COSBAC_POS = '||V_TMP_TABLA(6)||' AND
						COGRUG_NEG = '||V_TMP_TABLA(7)||' AND
						COTACA_NEG = '||V_TMP_TABLA(8)||' AND
						COSBAC_NEG = '||V_TMP_TABLA(9)||' AND
						PRO_ID = (SELECT PRO_ID FROM '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO WHERE PRO_CODIGO_UVEM = ''3148'')
	    ';
	...
	V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||'(DD_ETG_ID, DD_TGA_ID, DD_STG_ID
		  ...
		  ,BORRADO, PRO_ID, SUBROGADO) VALUES ('||V_ESQUEMA||'.S_DD_ETG_EQV_TIPO_GASTO_RU.NEXTVAL, (SELECT DD_TGA_ID FROM '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO WHERE DD_TGA_CODIGO = '''||V_TMP_TABLA(1)||'''), (SELECT DD_STG_ID FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = '''||V_TMP_TABLA(2)||''')
		  ,'''||V_TMP_TABLA(3)||''', null, null
		  ,'''||V_TMP_TABLA(4)||''', '''||V_TMP_TABLA(5)||''', '''||V_TMP_TABLA(6)||''', '''||V_TMP_TABLA(7)||''' , '''||V_TMP_TABLA(8)||''', '''||V_TMP_TABLA(9)||''', 0, ''HREOS-15516'',SYSDATE,0,(SELECT PRO_ID FROM '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO WHERE PRO_CODIGO_UVEM = ''3148''), '''||V_TMP_TABLA(10)||''')';
