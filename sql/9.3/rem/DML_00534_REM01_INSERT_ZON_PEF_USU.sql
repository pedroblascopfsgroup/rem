...
DECLARE
	...
	T_FUNCION('agonzaleza','HAYA_SUPER_CONTABILIDAD'),
	T_FUNCION('mgonzalezf','HAYA_SUPER_CONTABILIDAD'),
	T_FUNCION('jantolin','HAYA_SUPER_CONTABILIDAD'),
	T_FUNCION('vhernandezi','HAYA_SUPER_CONTABILIDAD'),
	T_FUNCION('bcarrasco','HAYA_SUPER_CONTABILIDAD'),
	T_FUNCION('jgutierrezc','HAYA_SUPER_CONTABILIDAD'),
	T_FUNCION('iplaza','HAYA_SUPER_CONTABILIDAD'),
	T_FUNCION('smoreno','HAYA_SUPER_CONTABILIDAD')
    );
			...
			ELSE
				...
				ELSE
					V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU' ||
								...
								' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||'''),' ||
								' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(1))||'''),' ||
								' ''HREOS-12499'',SYSDATE,0 FROM DUAL';
