...
--## INCIDENCIA_LINK=HREOS-1237
--## PRODUCTO=NO
--##
--## Finalidad: delete from DD_TTR_TIPO_TRABAJO
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Version inicial
--##########################################
...
DBMS_OUTPUT.PUT_LINE('[INICIO]');
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: Borrar en EN '||V_TEXT_TABLA);
    ...
    IF V_NUM_TABLAS = 0 THEN	
    	 DBMS_OUTPUT.PUT_LINE('No existe la tabla:'||V_TEXT_TABLA);
    ELSE
	      ...
	      LOOP
	        ...
	        IF V_NUM_TABLAS = 0 THEN				
	          -- Si existe se modifica.
	        	 DBMS_OUTPUT.PUT_LINE('[FIN]: El registro no existe en la tabla  '||V_TEXT_TABLA||'  ');
	
	       ELSE
	       	-- Si no existe se inserta.
	          DBMS_OUTPUT.PUT_LINE('[INFO]: Borrar EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
	            
	          	V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
	            'SET BORRADO = 1 , USUARIOBORRAR = '''||V_INCIDENCIA||''' , FECHABORRAR = SYSDATE '||
