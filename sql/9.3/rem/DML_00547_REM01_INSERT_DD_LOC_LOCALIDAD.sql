...
DECLARE
    ...
    V_DESCRIPCION VARCHAR2(30 CHAR) := 'Toral de los Vados';
    V_CODIGO VARCHAR2(30 CHAR) := '24206';
    V_PRV_CODIGO VARCHAR2(30 CHAR) := '24'; 
...
BEGIN	
    ...
    IF V_NUM_TABLAS > 0 THEN				
        ...
        DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''||V_CODIGO||'''');
    	  ...
    	  V_MSQL := 'UPDATE '|| V_ESQUEMA_M ||'.'||V_TABLA||' SET 
                  DD_LOC_DESCRIPCION = UPPER('''||V_DESCRIPCION||'''), DD_LOC_DESCRIPCION_LARGA = '''||V_DESCRIPCION||''',
                  DD_PRV_ID = (SELECT DD_PRV_ID FROM '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA WHERE DD_PRV_CODIGO='''||V_PRV_CODIGO||'''),
    ...
    ELSE
        ...
        V_MSQL := 'INSERT INTO '|| V_ESQUEMA_M ||'.'||V_TABLA||' (DD_LOC_ID, DD_PRV_ID, DD_LOC_CODIGO, DD_LOC_DESCRIPCION, DD_LOC_DESCRIPCION_LARGA, USUARIOCREAR, FECHACREAR)
                  SELECT '|| V_ESQUEMA_M ||'.S_'||V_TABLA||'.NEXTVAL,(SELECT DD_PRV_ID FROM '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA WHERE DD_PRV_CODIGO='''||V_PRV_CODIGO||'''),
                  '''||V_CODIGO||''',UPPER('''||V_DESCRIPCION||'''),'''||V_DESCRIPCION||''','''||V_USUARIO||''',SYSDATE FROM DUAL';
        EXECUTE IMMEDIATE V_MSQL;
