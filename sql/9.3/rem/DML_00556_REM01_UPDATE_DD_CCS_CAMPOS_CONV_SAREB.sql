...
DECLARE
  ...
  V_USUARIO VARCHAR2(32 CHAR):= 'HREOS-13106';
...
BEGIN
  ...
  V_SQL := 'UPDATE '||V_ESQUEMA||'.'||V_TABLA||' SET 
        ...
        , DD_CTD_ID = (SELECT CTD.DD_CTD_ID FROM '||V_ESQUEMA||'.DD_CTD_CAMPO_TIPO_DATO CTD WHERE CTD.DD_CTD_CODIGO = ''01'' AND CTD.BORRADO = 0)
        WHERE DD_COS_ID = (SELECT COS.DD_COS_ID FROM '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS
        WHERE DD_COS_CODIGO = ''042'' AND COS.BORRADO = 0) AND DD_CCS_CAMPO = ''BIE_LOC_NOMBRE_VIA''
        ';
