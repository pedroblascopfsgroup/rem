...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-9313'; -- USUARIOCREAR/USUARIOMODIFICAR.
    V_ECO_ID NUMBER(16); 
    
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] INSERCION TABLA PEF_PERFILES');
	
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''DATOSBASICOSACT''';
	...
	IF V_NUM_FILAS = 0 THEN
		...
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.PEF_PERFILES 
					...
					''DATOSBASICOSACT'',
					''Modificar checks y datos basicos'',
					''Modificar checks perimetros y datos basicos del activoo'',
					0,
					''REMVIP-9313'',
					SYSDATE,
					0
