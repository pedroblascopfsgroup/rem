...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('136','Seguros del activo (Siniestro)','ACT_SAREB_ACTIVOS','SINIESTRO','ACT_ID','LEFT JOIN '||V_ESQUEMA_M||'.DD_SIN_SINO CRUCE ON CRUCE.DD_SIN_CODIGO = ''''01'''' AND TMP.VALOR_NUEVO IS NOT NULL AND CRUCE.BORRADO = 0'),
      T_TIPO_DATA('143','Tipo correctivo SAREB','ACT_SAREB_ACTIVOS','DD_TCS_ID','ACT_ID','LEFT JOIN '||V_ESQUEMA||'.DD_TCS_TIPO_CORRECTIVO_SAREB CRUCE ON CRUCE.DD_TCS_CODIGO = TMP.VALOR_NUEVO AND CRUCE.BORRADO = 0'),
      T_TIPO_DATA('144','Fecha fin correctivo SAREB','ACT_SAREB_ACTIVOS','FEC_FIN_CORRECTIVO','ACT_ID','FECHA')
    ); 
