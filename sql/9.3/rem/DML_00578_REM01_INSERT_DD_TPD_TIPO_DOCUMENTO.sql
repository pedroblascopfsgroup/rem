...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('169'	,'Informe de estado Liberbank (KPI 4)'		,'Informe de estado Liberbank (KPI 4)'		,'AI-03-ESIN-DX',	1),
    	T_TIPO_DATA('170'	,'Informe de estado Liberbank (KPI 4)'		,'Informe de estado Liberbank (KPI 4)'		,'OP-13-ESIN-DX',	0)
    ); 
      ...
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
					...
					', USUARIOMODIFICAR = ''REMVIP-9278'' , FECHAMODIFICAR = SYSDATE, DD_'||V_TEXT_CHARS||'_VISIBLE = '||TRIM(V_TMP_TIPO_DATA(5))||''||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                      ...
                      'SELECT '|| V_ID || ',  '''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', '''||TRIM(V_TMP_TIPO_DATA(4))||''', '||TRIM(V_TMP_TIPO_DATA(5))||', 0, ''REMVIP-9278'',SYSDATE,0 '||
					  ' FROM DUAL';
