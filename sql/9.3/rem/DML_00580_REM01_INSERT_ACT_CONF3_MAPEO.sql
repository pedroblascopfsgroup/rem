...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('116','Valor unitario de la tasación relacionado con la superficie','ACT_TAS_TASACION','TAS_VALOR_REPER_SUELO_CONST','ACT_ID',NULL),
      T_TIPO_DATA('166','Nº expdte. Riesgo asociado','ACT_ABA_ACTIVO_BANCARIO','ABA_NEXPRIESGO','ACT_ID',NULL),
      T_TIPO_DATA('167','Importe adjudicación','BIE_ADJ_ADJUDICACION','BIE_ADJ_IMPORTE_ADJUDICACION','BIE_ID',NULL),
      T_TIPO_DATA('168','Tipo de juzgado','ACT_AJD_ADJJUDICIAL','DD_JUZ_ID','ACT_ID','LEFT JOIN '||V_ESQUEMA||'.DD_JUZ_JUZGADOS_PLAZA CRUCE ON CRUCE.DD_JUZ_CODIGO = TMP.VALOR_NUEVO AND CRUCE.BORRADO = 0'),
      T_TIPO_DATA('169','Población juzgado','ACT_AJD_ADJJUDICIAL','DD_PLA_ID','ACT_ID','LEFT JOIN '||V_ESQUEMA||'.DD_PLA_PLAZAS CRUCE ON CRUCE.DD_PLA_CODIGO = TMP.VALOR_NUEVO AND CRUCE.BORRADO = 0')
    ); 
