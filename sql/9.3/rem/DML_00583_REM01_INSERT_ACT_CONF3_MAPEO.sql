...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('162','Fecha subsanación','ACT_CAN_CALIFICACION_NEG','ACT_FECHA_SUBSANACION','ACT_ID','JOIN (SELECT ACT.ACT_ID, CAN.ACT_CAN_ID 
        FROM '||V_ESQUEMA||'.ACT_CAN_CALIFICACION_NEG CAN 
        JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON CAN.ACT_ID = ACT.ACT_ID AND ACT.BORRADO = 0
        JOIN '||V_ESQUEMA||'.ESPARTA_EXCEL1 EXC1 ON EXC1.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO
        JOIN '||V_ESQUEMA||'.DD_MCN_MOTIVO_CALIFIC_NEG MCN ON MCN.DD_MCN_CODIGO = EXC1.VALOR_NUEVO AND CAN.DD_MCN_ID = MCN.DD_MCN_ID AND MCN.BORRADO = 0
        WHERE CAN.BORRADO = 0 AND EXC1.CAMPO = ''''140'''') CRUCE ON CRUCE.ACT_ID = ACT.ACT_ID'),
        T_TIPO_DATA('164','Fecha calificación negativa','ACT_AHT_HIST_TRAM_TITULO','AHT_FECHA_CALIFICACION','AHT_ID','JOIN (SELECT TIT.ACT_ID, AHT.AHT_ID, ROW_NUMBER() OVER (PARTITION BY TIT.ACT_ID ORDER BY AHT.AHT_ID DESC) RN
	FROM '||V_ESQUEMA||'.ACT_TIT_TITULO TIT 
    JOIN '||V_ESQUEMA||'.ACT_AHT_HIST_TRAM_TITULO AHT ON TIT.TIT_ID = AHT.TIT_ID AND AHT.BORRADO = 0
    LEFT JOIN '||V_ESQUEMA||'.DD_ESP_ESTADO_PRESENTACION ESP ON ESP.DD_ESP_ID = AHT.DD_ESP_ID AND ESP.BORRADO = 0
    WHERE TIT.BORRADO = 0  AND ESP.DD_ESP_CODIGO = ''''02'''') CRUCE ON CRUCE.RN = 1 AND ACT.ACT_ID = CRUCE.ACT_ID')
    ); 
