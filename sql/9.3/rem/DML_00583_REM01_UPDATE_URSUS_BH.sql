...
--## INCIDENCIA_LINK=HREOS-13705
--## PRODUCTO=NO
--## Finalidad: actualizar datos a null de URSUS
--##           
--## INSTRUCCIONES: Eliminar todos los números de ursus de todas las ofertas de BH antes de estar aprobado
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
	...
	V_USUARIO VARCHAR2(32 CHAR) := 'HREOS-13705';
BEGIN
    DBMS_OUTPUT.PUT_LINE('[INFO]: UPDATEAMOS VALORES EN '||V_TEXT_TABLA);
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL REGISTRO');
       	  ...
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' COMPRA 
                    ...
                    WHERE cex.eco_id = eco.eco_id)) AND COMPRA.id_comprador_ursus_bh IS NOT NULL';
        DBMS_OUTPUT.put_line(V_MSQL);
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: REGISTRO MODIFICADO CORRECTAMENTE');
       	  ...
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA_SEC||' CEX 
                    SET CEX_NUM_URSUS_CONYUGE_BH_REM = NULL 
					, USUARIOMODIFICAR = '''||V_USUARIO||''' , FECHAMODIFICAR = SYSDATE 
				    WHERE exists 
                        (SELECT ECO.ECO_ID FROM '|| V_ESQUEMA ||'.eco_expediente_comercial ECO
                        JOIN '|| V_ESQUEMA ||'.dd_eec_est_exp_comercial EXPCOM ON EXPCOM.DD_EEC_ID = ECO.DD_EEC_ID AND EXPCOM.DD_EEC_CODIGO IN (''01'',''27'',''04'',''34'',''14'',''43'')
                        ...
                        AND  NOT EXISTS (
                        SELECT COMPRADOR.COM_ID FROM '|| V_ESQUEMA ||'.COM_COMPRADOR COMPRADOR WHERE cex.com_id=COMPRADOR.COM_ID AND comprador.id_comprador_ursus_bh IS NOT NULL)';
DBMS_OUTPUT.put_line(V_MSQL);
