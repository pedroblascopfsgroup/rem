...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('097','Estado adecuación','ACT_ACTIVO','DD_EAC_ID','ACT_ID','	JOIN (SELECT ACT.ACT_ID, CASE WHEN VAN.ACT_NUM_ACTIVO IS NOT NULL THEN EAS.DD_EAC_ID_VANDALIZADO ELSE EAS.DD_EAC_ID END DD_EAC_ID
        FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
        JOIN '||V_ESQUEMA||'.ESPARTA_EXCEL1 EXC1 ON EXC1.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO
        LEFT JOIN '||V_ESQUEMA||'.ESPARTA_EXCEL1 VAN ON VAN.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO AND VAN.CAMPO = ''''138''''
        LEFT JOIN '||V_ESQUEMA||'.DD_EAS_ESTADO_ADECUACION_SAREB EAS ON EAS.DD_EAS_CODIGO = EXC1.VALOR_NUEVO AND EAS.BORRADO = 0
        WHERE EAS.BORRADO = 0 AND EXC1.CAMPO = ''''097'''') CRUCE ON CRUCE.ACT_ID = ACT.ACT_ID')
    ); 
