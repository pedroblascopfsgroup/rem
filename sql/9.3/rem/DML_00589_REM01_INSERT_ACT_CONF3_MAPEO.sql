...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('032','Subtipo de Carga','ACT_CRG_CARGAS','CRG_IMPIDE_VENTA','ACT_ID','    JOIN (SELECT EXC1.ACT_NUM_ACTIVO, EXC1.ID_REGISTRO, CASE WHEN STC.DD_STC_CODIGO IN (''''51'''',''''28'''',''''08'''',''''013'''',''''05'''',''''33'''') THEN (SELECT DD_SIN_ID FROM '||V_ESQUEMA_M||'.DD_SIN_SINO SIN WHERE SIN.DD_SIN_CODIGO = ''''01'''') 
            ELSE(SELECT DD_SIN_ID FROM '||V_ESQUEMA_M||'.DD_SIN_SINO SIN WHERE SIN.DD_SIN_CODIGO = ''''02'''') END CRG_IMPIDE_VENTA
    FROM '||V_ESQUEMA||'.ESPARTA_EXCEL1 EXC1
    LEFT JOIN '||V_ESQUEMA||'.DD_STC_SUBTIPO_CARGA STC ON STC.DD_STC_CODIGO = EXC1.VALOR_NUEVO AND STC.BORRADO = 0
    WHERE EXC1.CAMPO = ''''032'''' AND EXC1.NUEVO = 0) CRUCE ON CRUCE.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO AND CRUCE.ID_REGISTRO = TMP.ID_REGISTRO')
    ); 
