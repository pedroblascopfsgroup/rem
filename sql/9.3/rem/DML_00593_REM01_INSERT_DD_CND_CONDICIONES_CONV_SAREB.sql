...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      ...
      , T_TIPO_DATA('MOD_REM_FEC_INSCRIP', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''018'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_TIT_TITULO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND AUX_ACT.TIT_FECHA_INSC_REGIS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND AUX_ACT.TIT_FECHA_INSC_REG IS NULL OR TO_DATE(ULT_VALOR.VALOR, ''''YYYY/MM/DD'''') != AUX_ACT.TIT_FECHA_INSC_REG) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''018'''''
            ,''
            ,'0') 
