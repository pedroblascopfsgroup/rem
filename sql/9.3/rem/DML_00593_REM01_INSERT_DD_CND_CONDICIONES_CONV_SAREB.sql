...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      ...
      , T_TIPO_DATA('MOD_REM_SAC', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''004'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.DD_SAC_SUBTIPO_ACTIVO AUX_SAC ON AUX_ACT.DD_SAC_ID = AUX_SAC.DD_SAC_ID AND AUX_SAC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND AUX_SAC.DD_SAC_CODIGO IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND AUX_SAC.DD_SAC_CODIGO IS NULL OR ULT_VALOR.VALOR != AUX_SAC.DD_SAC_CODIGO) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''004'''''
            ,''
            ,'0')        
      , T_TIPO_DATA('MOD_REM_FEC_INSCRIP', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''018'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_TIT_TITULO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND AUX_ACT.TIT_FECHA_INSC_REG IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND AUX_ACT.TIT_FECHA_INSC_REG IS NULL OR TO_DATE(ULT_VALOR.VALOR, ''''YYYY/MM/DD'''') != AUX_ACT.TIT_FECHA_INSC_REG) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''018'''''
            ,''
            ,'0') 
      ...
      , T_TIPO_DATA('MOD_REM_NOMBRE_VIA', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''042'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND BIE_LOC.BIE_LOC_NOMBRE_VIA IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND BIE_LOC.BIE_LOC_NOMBRE_VIA IS NULL OR ULT_VALOR.VALOR != BIE_LOC.BIE_LOC_NOMBRE_VIA) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''042'''''
            ,''
            ,'0')    
      , T_TIPO_DATA('MOD_REM_NUM_DOMIC', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''044'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND BIE_LOC.BIE_LOC_NUMERO_DOMICILIO IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND BIE_LOC.BIE_LOC_NUMERO_DOMICILIO IS NULL OR ULT_VALOR.VALOR != BIE_LOC.BIE_LOC_NUMERO_DOMICILIO) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''044'''''
            ,''
            ,'0')     
      , T_TIPO_DATA('MOD_REM_PISO', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''048'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND BIE_LOC.BIE_LOC_PISO IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND BIE_LOC.BIE_LOC_PISO IS NULL OR ULT_VALOR.VALOR != BIE_LOC.BIE_LOC_PISO) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''048'''''
            ,''
            ,'0') 
      , T_TIPO_DATA('MOD_REM_PUERTA', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''050'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND BIE_LOC.BIE_LOC_PUERTA IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND BIE_LOC.BIE_LOC_PUERTA IS NULL OR ULT_VALOR.VALOR != BIE_LOC.BIE_LOC_PUERTA) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''050'''''
            ,''
            ,'0') 
      , T_TIPO_DATA('MOD_REM_MUNICIPIO', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''054'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 JOIN '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD AUX_LOC ON BIE_LOC.DD_LOC_ID = AUX_LOC.DD_LOC_ID AND AUX_LOC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND AUX_LOC.DD_LOC_CODIGO IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND AUX_LOC.DD_LOC_CODIGO IS NULL OR ULT_VALOR.VALOR != AUX_LOC.DD_LOC_CODIGO) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''054'''''
            ,''
            ,'0') 
      , T_TIPO_DATA('MOD_REM_COD_POSTAL', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''058'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND BIE_LOC.BIE_LOC_COD_POST IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND BIE_LOC.BIE_LOC_COD_POST IS NULL OR ULT_VALOR.VALOR != BIE_LOC.BIE_LOC_COD_POST) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''058'''''
            ,''
            ,'0') 
      , T_TIPO_DATA('MOD_REM_LATITUD', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''060'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_LOC_LOCALIZACION AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND AUX_ACT.LOC_LATITUD IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND AUX_ACT.LOC_LATITUD IS NULL OR TO_NUMBER(REPLACE(ULT_VALOR.VALOR, ''''.'''','''','''')) != AUX_ACT.LOC_LATITUD) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''060'''''
            ,''
            ,'0') 
      , T_TIPO_DATA('MOD_REM_LONGITUD', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''062'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_LOC_LOCALIZACION AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND AUX_ACT.LOC_LONGITUD IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND AUX_ACT.LOC_LONGITUD IS NULL OR TO_NUMBER(REPLACE(ULT_VALOR.VALOR, ''''.'''','''','''')) != AUX_ACT.LOC_LONGITUD) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''062'''''
            ,''
            ,'0') 
      , T_TIPO_DATA('MOD_REM_PROVINCIA', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''052'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 JOIN '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA AUX_PRV ON BIE_LOC.DD_PRV_ID = AUX_PRV.DD_PRV_ID AND AUX_PRV.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND AUX_PRV.DD_PRV_CODIGO IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND AUX_PRV.DD_PRV_CODIGO IS NULL OR ULT_VALOR.VALOR != AUX_PRV.DD_PRV_CODIGO) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''052'''''
            ,''
            ,'0') 
      , T_TIPO_DATA('MOD_REM_ESCALERA', 'Primera vez modificado en REM, deja de actualizarse', 'Si se han modificado los datos en REM, deja de actualizarse este campo con la información del Data Tape y permanece la de REM'
            ,'AND EXISTS (SELECT 1 FROM (SELECT HCCN.ACT_ID, HCCN.VALOR, ROW_NUMBER() OVER (PARTITION BY HCCN.ACT_ID ORDER BY HCCN.FECHACREAR DESC) RN FROM '||V_ESQUEMA||'.H_CCN_CAMBIOS_CONV_SAREB HCCN JOIN '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON HCCN.DD_COS_ID = COS.DD_COS_ID AND COS.BORRADO = 0 WHERE COS.DD_COS_CODIGO = ''''046'''') ULT_VALOR JOIN '||V_ESQUEMA||'.ACT_ACTIVO AUX_ACT ON ULT_VALOR.ACT_ID = AUX_ACT.ACT_ID AND AUX_ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.BIE_LOCALIZACION BIE_LOC ON AUX_ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0 WHERE ULT_VALOR.RN = 1 AND (ULT_VALOR.VALOR IS NULL AND BIE_LOC.BIE_LOC_ESCALERA IS NOT NULL OR ULT_VALOR.VALOR IS NOT NULL AND BIE_LOC.BIE_LOC_ESCALERA IS NULL OR ULT_VALOR.VALOR != BIE_LOC.BIE_LOC_ESCALERA) AND ULT_VALOR.ACT_ID = ACT.ACT_ID) AND TMP.ORIGEN = ''''046'''''
            ,''
            ,'0') 
    ); 
