...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('170','Estado','ACT_SPS_SIT_POSESORIA','SPS_OCUPADO','ACT_ID','LEFT JOIN (SELECT CASE WHEN AUX.VALOR_NUEVO = 1 THEN 1 WHEN AUX.VALOR_NUEVO = 0 THEN 0 END SPS_OCUPADO, AUX.ACT_NUM_ACTIVO FROM '||V_ESQUEMA||'.ESPARTA_EXCEL1 AUX WHERE AUX.CAMPO = ''''009'''') CRUCE ON CRUCE.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO'),
      T_TIPO_DATA('170','Estado','ACT_SPS_SIT_POSESORIA','DD_TPA_ID','ACT_ID','LEFT JOIN '||V_ESQUEMA||'.DD_TPA_TIPO_TITULO_ACT CRUCE ON CRUCE.BORRADO = 0 AND CRUCE.DD_TPA_CODIGO = ''''02''''')
    ); 
