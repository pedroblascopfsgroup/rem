...
BEGIN
  ...
  V_SQL := 'UPDATE '||V_ESQUEMA||'.'||V_TABLA||' SET 
        ...
        , FECHABORRAR = SYSDATE
        , DD_CCS_CAMPO = ''DD_SCG_ID''
        , DD_CCS_DESCRIPCION = ''Subestado de la carga''
        WHERE DD_COS_ID = (SELECT COS.DD_COS_ID FROM '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS
        WHERE DD_COS_CODIGO = ''036'' AND COS.BORRADO = 0) AND DD_CCS_CAMPO = ''DD_ECG_ID''
        ';
  EXECUTE IMMEDIATE V_SQL;
    ...
    V_SQL := 'UPDATE '||V_ESQUEMA||'.'||V_TABLA||' SET 
          USUARIOBORRAR = '''||V_USUARIO||'''
        , FECHABORRAR = SYSDATE
        , DD_CCS_DESCRIPCION = ''Estado de la carga''
        WHERE DD_COS_ID = (SELECT COS.DD_COS_ID FROM '||V_ESQUEMA||'.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS
        WHERE DD_COS_CODIGO = ''035'' AND COS.BORRADO = 0) AND DD_CCS_CAMPO = ''DD_ECG_ID''
        ';
  EXECUTE IMMEDIATE V_SQL;
