...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-2941';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(150);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T004_ResultadoTarificada'		,'datefield', 	'2'		,'fechaAtPrimaria'		,'Fecha atención primaria'),
    	T_TIPO_DATA('T004_ResultadoTarificada'		,'textarea', 	'3'		,'observaciones'		,'Observaciones'),
    	T_TIPO_DATA('T004_ResultadoTarificada'		,'elctrabajo', 	'4'		,'GESECO'				,'TRABAJO > Gesti&oacuten econ&oacutemica'),
    	T_TIPO_DATA('T004_ResultadoNoTarificada'	,'datefield', 	'3'		,'fechaAtPrimaria'		,'Fecha atención primaria'),
    	T_TIPO_DATA('T004_ResultadoNoTarificada'	,'textarea', 	'4'		,'observaciones'		,'Observaciones'),
    	T_TIPO_DATA('T004_ResultadoNoTarificada'	,'elctrabajo', 	'5'		,'FOTOS'				,'TRABAJO > Fotos')
    ); 
      ...
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL CAMPO '''|| TRIM(V_TMP_TIPO_DATA(3)) ||''' DE '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
					...
					', '||V_TEXT_CHARS||'_LABEL = '''||TRIM(V_TMP_TIPO_DATA(5))||''''||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                      ''||V_TEXT_CHARS||'_ID, TAP_ID, '||V_TEXT_CHARS||'_ORDEN, '||V_TEXT_CHARS||'_TIPO, '||V_TEXT_CHARS||'_NOMBRE, '||V_TEXT_CHARS||'_LABEL, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ', '||V_TAP_ID||', '''||V_TMP_TIPO_DATA(3)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''', '''||TRIM(V_TMP_TIPO_DATA(5))||''', 0, '''||V_USUARIO||''',SYSDATE,0 '||
					  ' FROM DUAL';
