...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('175','No mostrar precio Venta','ACT_APU_ACTIVO_PUBLICACION','APU_CHECK_OCULTAR_PRECIO_V','ACT_ID','LEFT JOIN (SELECT CASE WHEN AUX.VALOR_NUEVO = 1 THEN 1 WHEN AUX.VALOR_NUEVO = 0 THEN 0 END APU_CHECK_OCULTAR_PRECIO_V, AUX.ACT_NUM_ACTIVO FROM ESPARTA_EXCEL1 AUX WHERE AUX.CAMPO = ''''175'''') CRUCE ON CRUCE.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO')
    ); 
