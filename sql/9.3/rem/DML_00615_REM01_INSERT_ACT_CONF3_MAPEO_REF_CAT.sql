...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('074','Referencia catastral activo','ACT_CAT_CATASTRO','CAT_REF_CATASTRAL','CAT_ID','JOIN (SELECT AUX_CAT.ACT_ID, AUX_CAT.CAT_ID, ROW_NUMBER() OVER (PARTITION BY AUX_CAT.ACT_ID ORDER BY AUX_CAT.CAT_ID) AS RN FROM '||V_ESQUEMA||'.ACT_CAT_CATASTRO AUX_CAT) CRUCE ON CRUCE.RN = 1 AND CRUCE.ACT_ID = ACT.ACT_ID')
    ); 
