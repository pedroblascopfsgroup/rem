...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      T_TIPO_DATA('072','Subfinca','BIE_DATOS_REGISTRALES','BIE_DREG_NUM_FINCA','BIE_ID','LEFT JOIN (SELECT COALESCE(AUX2.VALOR_NUEVO,BREG.BIE_DREG_NUM_FINCA) || ''''/'''' || AUX.VALOR_NUEVO AS BIE_DREG_NUM_FINCA, AUX.ACT_NUM_ACTIVO FROM REM01.ESPARTA_EXCEL1 AUX LEFT JOIN REM01.ESPARTA_EXCEL1 AUX2 ON AUX2.ACT_NUM_ACTIVO = AUX.ACT_NUM_ACTIVO AND AUX2.CAMPO = ''''071'''' JOIN REM01.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = AUX.ACT_NUM_ACTIVO JOIN REM01.ACT_REG_INFO_REGISTRAL REG ON REG.ACT_ID = ACT.ACT_ID JOIN REM01.BIE_DATOS_REGISTRALES BREG ON BREG.BIE_DREG_ID = REG.BIE_DREG_ID WHERE AUX.CAMPO = ''''072'''' AND ACT.BORRADO = 0 AND REG.BORRADO = 0 AND BREG.BORRADO = 0) CRUCE ON CRUCE.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO')
    ); 
