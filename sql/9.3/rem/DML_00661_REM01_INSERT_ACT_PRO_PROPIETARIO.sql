...
BEGIN	
    ...
    IF V_COUNT = 0 THEN     
        ...
        (SELECT DD_LOC_ID FROM '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD WHERE DD_LOC_CODIGO = '''||V_CODIGO_LOC||'''),
        (SELECT DD_PRV_ID FROM '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD WHERE DD_LOC_CODIGO = '''||V_CODIGO_LOC||'''),
        ''UNICAJA BANCO S.A.'',
