...
--## PRODUCTO=NO
--##
--## Finalidad: Merge para insertar la configuración de las tarifas de la subcartera Jaguar
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
BEGIN
 ...
 SELECT  DISTINCT
                    ...
                    JOIN '||V_ESQUEMA||'.DD_SCR_SUBCARTERA scr ON scr.DD_SCR_ID = cft.DD_SCR_ID AND scr.DD_SCR_CODIGO = ''138''
                    WHERE  NOT EXISTS (
                        SELECT 1 
                        FROM '||V_ESQUEMA||'.ACT_CFT_CONFIG_TARIFA cft2 
                        WHERE cft2.DD_SCR_ID= (SELECT DD_SCR_ID FROM DD_SCR_SUBCARTERA WHERE DD_SCR_CODIGO = ''70''))                
