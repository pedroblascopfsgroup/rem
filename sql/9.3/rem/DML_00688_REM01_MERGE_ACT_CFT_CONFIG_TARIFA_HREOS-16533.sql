...
--## INCIDENCIA_LINK=HREOS-16533
--## PRODUCTO=NO
--##
--## Finalidad: Merge para insertar la configuración de las tarifas de la subcartera Jaguar con la configuración de BBVA.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_USUARIO VARCHAR2(30 CHAR) := 'HREOS-16533'; -- USUARIOCREAR/USUARIOMODIFICAR
...
BEGIN
    ...
    V_MSQL := 'UPDATE     '||V_ESQUEMA||'.ACT_CFT_CONFIG_TARIFA  
                            ...
                            SET 	USUARIOBORRAR ='''||V_USUARIO||''',
                                    FECHABORRAR = SYSDATE,
                                    BORRADO = 1
 									WHERE DD_SCR_ID= (SELECT DD_SCR_ID FROM '||V_ESQUEMA||'.DD_SCR_SUBCARTERA WHERE DD_SCR_CODIGO = ''70'')';
    EXECUTE IMMEDIATE V_MSQL;

    DBMS_OUTPUT.PUT_LINE('[INFO] '||SQL%ROWCOUNT||' REGISTROS BORRADOS EN ACT_CFT_CONFIG_TARIFA');  
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_CFT_CONFIG_TARIFA act
				...
				SELECT  DISTINCT
                    ...
                    JOIN '||V_ESQUEMA||'.DD_SCR_SUBCARTERA scr ON scr.DD_SCR_ID = cft.DD_SCR_ID AND scr.DD_SCR_CODIGO = ''153''
                    WHERE  NOT EXISTS (
                        SELECT 1 
