...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(2400 CHAR) := 'APR_AUX_TAS_TASACIONES'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
    ...
    V_ALTER T_ARRAY_ALTER := T_ARRAY_ALTER(
    	...
    	T_ALTER( 'NUM_IDENTIF_TASACION', 'NUM_IDENTIF_TASACION NUMBER(10,0)')
		);
    V_T_ALTER T_ALTER;
...
BEGIN
	...
	LOOP
		...
		IF V_NUM_TABLAS = 1 THEN
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] ... '||V_T_ALTER(1)||' Columna MODIFICADA en tabla, con modificacion '||V_T_ALTER(2));

        ELSE
            
            DBMS_OUTPUT.PUT_LINE('[INFO] No existe la columna '||V_T_ALTER(1)||' NO SE REALIZA MODIFICACION');
		END IF;

	END LOOP;
	
	DBMS_OUTPUT.PUT_LINE('[INFO] ' ||V_ESQUEMA||'.'||V_TEXT_TABLA||' MODIFICADA CORRECTAMENTE.... OK *************************************************');
