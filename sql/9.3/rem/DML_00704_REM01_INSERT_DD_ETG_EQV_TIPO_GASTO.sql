    ...
    FOR I IN M_TABLA.FIRST .. M_TABLA.LAST
	  ...
	  V_TMP_TABLA := M_TABLA(I);
	    ...
	    V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA_AUX||' WHERE 
						DD_TGA_ID = (SELECT DD_TGA_ID FROM '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO WHERE DD_TGA_CODIGO = '''||V_TMP_TABLA(6)||''') AND
						DD_STG_ID = (SELECT DD_STG_ID FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = '''||V_TMP_TABLA(7)||''') AND
						DD_ETG_CODIGO = '''||V_TMP_TABLA(1)||''' AND
						COGRUG_POS = '''||V_TMP_TABLA(3)||''' AND
						COTACA_POS = '''||V_TMP_TABLA(4)||''' AND
						COSBAC_POS = '''||V_TMP_TABLA(5)||''' AND
						COGRUG_NEG = '''||V_TMP_TABLA(3)||''' AND
						COTACA_NEG = '''||V_TMP_TABLA(4)||''' AND
						COSBAC_NEG = '''||V_TMP_TABLA(5)||''' AND
						DD_TCO_ID = (SELECT DD_TCO_ID FROM '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION WHERE DD_TCO_CODIGO = '''||V_TMP_TABLA(8)||''') AND 
						DD_EAL_ID = (SELECT DD_EAL_ID FROM '||V_ESQUEMA||'.DD_EAL_ESTADO_ALQUILER WHERE DD_EAL_CODIGO = '''||V_TMP_TABLA(9)||''') AND
						DD_TTR_ID = (SELECT DD_TTR_ID FROM '||V_ESQUEMA||'.DD_TTR_TIPO_TRANSMISION WHERE DD_TTR_CODIGO = '''||V_TMP_TABLA(10)||''') AND 
						PRIM_TOMA_POSESION = '||V_TMP_TABLA(11)||' AND
						EJE_ID = (SELECT EJE2.EJE_ID FROM '|| V_ESQUEMA ||'.ACT_EJE_EJERCICIO EJE2 WHERE EJE2.EJE_ANYO = ''2022'') AND
						ELEMENTO_PEP = '''||V_TMP_TABLA(2)||'''
	    ';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM;
    IF V_NUM > 0  THEN
        
        DBMS_OUTPUT.PUT_LINE('[INFO] YA ESTÁ INSERTADO EL REGISTRO');
        
    ELSE
		
	DBMS_OUTPUT.PUT_LINE('[INFO]: ACTUALIZANDO '|| V_TABLA);

	--Gastos BH (con PRO_ID relleno)
