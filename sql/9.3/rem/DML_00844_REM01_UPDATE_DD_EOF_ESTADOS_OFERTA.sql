...
--## PRODUCTO=NO
--##
--## Finalidad: Actualiza o inserta en DD_EOF_ESTADOS_OFERTA
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_USUARIO VARCHAR(40 CHAR) := 'HREOS-14526'; -- Usuario (Artefacto)
...
BEGIN
    ...
    FOR I IN V_DATA.FIRST .. V_DATA.LAST
      LOOP
      V_TMP_DATA := V_DATA(I);
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'|| TRIM(V_TABLA)||' WHERE DD_EOF_CODIGO = '''||TRIM(V_TMP_DATA(1))||'''';
        ...
        V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'|| TRIM(V_TABLA)||
						' SET DD_EOF_DESCRIPCION = '''|| TRIM(V_TMP_DATA(2))||''', DD_EOF_DESCRIPCION_LARGA = '''|| TRIM(V_TMP_DATA(3))||''', VERSION = VERSION + 1, USUARIOMODIFICAR = '''|| TRIM(V_USUARIO)||''''||
            ', FECHAMODIFICAR = SYSDATE, BORRADO = 0 WHERE DD_EOF_CODIGO = ''' || TRIM(V_TMP_DATA(1))||'''';
