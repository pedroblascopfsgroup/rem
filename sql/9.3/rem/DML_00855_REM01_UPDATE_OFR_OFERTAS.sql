...
BEGIN
    ...
    V_MSQL := 'SELECT DD_SNS_ID FROM '||V_ESQUEMA||'.DD_SNS_SINONOSABE WHERE DD_SNS_CODIGO = ''02''';
    EXECUTE IMMEDIATE V_MSQL INTO V_DDSNS_ID;

    IF V_DDSNS_ID != 0 THEN
        V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.'||V_TABLA||' MODIFY '||V_COL_OLD||' DEFAULT '||V_DDSNS_ID||'';
        EXECUTE IMMEDIATE V_MSQL;
