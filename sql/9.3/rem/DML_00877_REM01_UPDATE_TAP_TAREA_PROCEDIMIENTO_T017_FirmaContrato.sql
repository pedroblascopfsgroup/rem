...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('T017_FirmaContrato',
                    '(checkBankia() &&  valores[''''T017_FirmaContrato''''][''''comboResultado''''] == ''''01'''') ? ''''KO'''' : ''''OK''''',
                    'checkBankia() ? (valores[''''T017_FirmaContrato''''][''''comboFirma''''] == DDSiNo.SI ? (checkPosicionamiento() ? null : ''''El expediente debe tener alg&uacute;n posicionamiento'''') : null)	: checkExpedienteBloqueado() ? (valores[''''T017_FirmaContrato''''][''''comboFirma''''] == DDSiNo.SI ? (checkPosicionamiento() ? null : ''''El expediente debe tener alg&uacute;n posicionamiento'''') : null) : ''''El expediente no est&aacute; bloqueado''''')
    ); 
