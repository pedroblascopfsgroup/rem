...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('T017_ObtencionContratoReserva',
                    'checkBankia() && valores[''''T017_ObtencionContratoReserva''''][''''comboQuitar''''] == ''''01'''' ? ''''KO'''' : checkBankia() &&  valores[''''T017_ObtencionContratoReserva''''][''''comboResultado''''] == ''''01'''' ? ''''Confirmado'''' : ''''noConfirmado''''',
                    'esBBVA() ? (checkImporteParticipacion() ? (checkCompradores() ? ( checkVendido() ? ''''El activo est&aacute; vendido'''' : (checkComercializable() ? (checkPoliticaCorporativa() ? existeAdjuntoUGValidacion("06,E;12,E") : ''''El estado de la poliacute;tica corporativa no es el correcto para poder avanzar.'''') : ''''El activo debe ser comercializable'''') ) : ''''Los compradores deben sumar el 100%'''') : ''''El sumatorio de importes de participaci&oacute;n de los activos ha de ser el mismo que el importe total del expediente'''') : (checkBankia() &&  valores[''''T017_ObtencionContratoReserva''''][''''comboQuitar''''] == ''''01'''') ? null :(checkBankia() &&  valores[''''T017_ObtencionContratoReserva''''][''''comboResultado''''] != ''''01'''') ? existeAdjuntoUGValidacion("06,E;12,E") : null')
    ); 
