...
DECLARE
    ...
    V_ID NUMBER(16);
    V_TABLA VARCHAR2(50 CHAR):= 'DD_MAN_MOTIVO_ANULACION';
    V_CHARS VARCHAR2(3 CHAR):= 'MAN';
...
BEGIN	
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      ...
      ELSE
        ...
        , DD_'||V_CHARS||'_VISIBLE_WEB)
          	SELECT 
	            '|| V_ID || ',
	            CONCAT('''||V_TMP_TIPO_DATA(1)||''', ''00''),
	            '''||V_TMP_TIPO_DATA(2)||''',
	            '''||TRIM(V_TMP_TIPO_DATA(2))||''',
	            0, '''||V_USUARIO||''', SYSDATE, '||V_TMP_TIPO_DATA(1)||', '''||TRIM(V_TMP_TIPO_DATA(3))||''', 0, 0, 1 FROM DUAL';
        EXECUTE IMMEDIATE V_MSQL;
