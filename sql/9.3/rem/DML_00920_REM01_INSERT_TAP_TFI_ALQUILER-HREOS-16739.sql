...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('T018_DatosPBC', 'Datos PBC',  '', '', '', 'true')
    ); 
		    ...
		    IF V_COUNT = 0 THEN
	        	...
	        	V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS
	                VALUES (
	                    '||V_ESQUEMA||'.S_TFI_TAREAS_FORM_ITEMS.NEXTVAL
	                    ,(SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = '''||V_TMP_TIPO_DATA(1)||''')
	                    ,  1, ''combobox'', ''comboResultado'' , ''Aprobado'' , ''Debe indicar si el resultado es Negativo o No'' , ''false'' 
	                    , null , ''DDSiNo''  , 0, ''HREOS-16739'', SYSDATE , null, null, null , null  , 0
	                )';
				EXECUTE IMMEDIATE V_MSQL;
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS
	                VALUES (
