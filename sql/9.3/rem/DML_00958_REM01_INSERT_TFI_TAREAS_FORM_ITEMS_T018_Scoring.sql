...
DECLARE
    ...
    V_NUM NUMBER(16) := 0; -- Vble. para indicar el siguiente orden en TFI para una tarea.
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-15723';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(800);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T018_Scoring'		,'combobox', 	'5'		,'ratingHaya'		,'false' 	,'DDRatingScoringServicer'	,'Rating haya')
    ); 
      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                      ''||V_TEXT_CHARS||'_ID, TAP_ID, '||V_TEXT_CHARS||'_ORDEN, '||V_TEXT_CHARS||'_TIPO, '||V_TEXT_CHARS||'_NOMBRE, '||V_TEXT_CHARS||'_LABEL, '||V_TEXT_CHARS||'_VALIDACION, '||V_TEXT_CHARS||'_BUSINESS_OPERATION, USUARIOCREAR, FECHACREAR, USUARIOBORRAR) ' ||
                      'SELECT '|| V_ID || ', '||V_TAP_ID||', '''||V_TMP_TIPO_DATA(3)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''', '''||TRIM(V_TMP_TIPO_DATA(7))||''', '''||TRIM(V_TMP_TIPO_DATA(5))||''', '''||TRIM(V_TMP_TIPO_DATA(6))||''', '''||V_USUARIO||''',SYSDATE,''ORDENOK'' '||
					  ' FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
          DBMS_OUTPUT.PUT_LINE('[INFO]: REGISTRO INSERTADO CORRECTAMENTE');

        FOR I IN V_TMP_TIPO_DATA(3)..V_MAXIMO_ORDEN
        LOOP

          V_NUM := I+1;

          V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' SET 
                    '||V_TEXT_CHARS||'_ORDEN = '''||V_NUM||''', USUARIOBORRAR = ''ORDENOK'',
                    USUARIOMODIFICAR = '''||V_USUARIO||''' , FECHAMODIFICAR = SYSDATE
                    WHERE '||V_TEXT_CHARS||'_ORDEN = '''||I||''' AND TAP_ID = '||V_TAP_ID||' AND USUARIOBORRAR IS NULL';
          EXECUTE IMMEDIATE V_MSQL;
         
          DBMS_OUTPUT.PUT_LINE('[INFO]: REGISTRO ORDEN '''||I||''' MODIFICADO CORRECTAMENTE');

        END LOOP;

       END IF;
       V_MAXIMO_ORDEN := V_MAXIMO_ORDEN+1;

        --BORRAMOS EL USUARIOBORRAR, USADO PARA MARCAR LOS QUE TIENEN BIEN EL ORDEN
       V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' SET USUARIOBORRAR = NULL
                    WHERE TAP_ID = '||V_TAP_ID||' AND USUARIOBORRAR = ''ORDENOK''';
        EXECUTE IMMEDIATE V_MSQL;
