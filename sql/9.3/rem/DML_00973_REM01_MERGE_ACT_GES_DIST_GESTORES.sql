...
--## PRODUCTO=NO
--##
--## Finalidad: Merge para insertar los gestores de Divarian Remaining la subcartera MACC MARINA
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_USUARIO VARCHAR2(30 CHAR) := 'HREOS-17286'; -- USUARIOCREAR/USUARIOMODIFICAR
...
BEGIN
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_GES_DIST_GESTORES ges
				using (
                    ...
                    JOIN '||V_ESQUEMA||'.DD_SCR_SUBCARTERA scr ON scr.DD_SCR_CODIGO = act.COD_SUBCARTERA AND scr.DD_SCR_CODIGO = ''152''
                    WHERE  NOT EXISTS (
                        SELECT 1 
                        FROM '||V_ESQUEMA||'.ACT_GES_DIST_GESTORES ges2 
                        WHERE ges2.COD_SUBCARTERA=71)


                    ) us ON (us.ID = ges.ID AND us.COD_SUBCARTERA = 71)
