...
DECLARE
    ...
    --			USU_USERNAME			CODIGO PERFIL
	  T_FUNCION('gestformcajamar',	'GESTORIAFORM')
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN ZON_PEF_USU] ');
     FOR I IN V_FUNCION.FIRST .. V_FUNCION.LAST
      LOOP
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO LIKE '''||TRIM(V_TMP_FUNCION(2))||''' ';
			EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

			IF V_NUM_TABLAS = 0 THEN	  
				DBMS_OUTPUT.PUT_LINE('[INFO]NO EXISTE EL DESPACHO '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO...no se modifica nada.');
			...
			ELSE
				V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS' ||
							...
							' (SELECT DES_ID FROM '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO WHERE DES_DESPACHO LIKE '''||TRIM(V_TMP_FUNCION(2))||'''),' ||
							' 0,1, ''HREOS-17711'',SYSDATE,0 FROM DUAL';
