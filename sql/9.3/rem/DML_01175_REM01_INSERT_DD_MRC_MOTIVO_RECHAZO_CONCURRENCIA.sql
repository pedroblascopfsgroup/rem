...
DECLARE
	...
	, T_TIPO_DATA('F05' ,'El activo es no comercializable o está fuera de perímetro haya' , '1', 'JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC ON PAC.ACT_ID = ACT.ACT_ID AND PAC.BORRADO = 0 WHERE (PAC.PAC_INCLUIDO = 0 OR PAC.PAC_CHECK_COMERCIALIZAR = 0) AND AUX.FEC_INICIO_CONCURENCIA IS NOT NULL AND AUX.FEC_FIN_CONCURENCIA IS NOT NULL ' )
        ...
        , T_TIPO_DATA('F08' ,'El activo está en un periodo de concurrencia vivo, por lo que no se puede cambiar el precio de venta web' , '1', 'JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.CON_CONCURRENCIA CON ON CON.ACT_ID = ACT.ACT_ID AND CON.BORRADO = 0 WHERE TRUNC(SYSDATE) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN) AND AUX.IMP_PRECIO_VENTA IS NOT NULL AND TO_NUMBER(AUX.IMP_PRECIO_VENTA)/100 <> CON.CON_IMPORTE_MIN_OFR AND AUX.FEC_INICIO_CONCURENCIA IS NOT NULL AND AUX.FEC_FIN_CONCURENCIA IS NOT NULL' )
        , T_TIPO_DATA('F09' ,'El activo está en un periodo de concurrencia vivo, el cual se modifica, tiene ofertas vivas, y el id de concurrencia de la oferta no coincide con el del activo' , '1', 'JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_OFR ACTOFR ON ACTOFR.ACT_ID = ACT.ACT_ID JOIN '||V_ESQUEMA||'.OFR_OFERTAS OFR ON OFR.OFR_ID = ACTOFR.OFR_ID AND OFR.BORRADO = 0 JOIN '||V_ESQUEMA||'.DD_EOF_ESTADOS_OFERTA EOF ON EOF.DD_EOF_ID = OFR.DD_EOF_ID AND EOF.BORRADO = 0 JOIN '||V_ESQUEMA||'.CON_CONCURRENCIA CON ON CON.ACT_ID = ACT.ACT_ID AND CON.BORRADO = 0 WHERE EOF.DD_EOF_CODIGO NOT IN (''''02'''',''''06'''') AND TRUNC(SYSDATE) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN) AND TRUNC(TO_DATE(AUX.FEC_INICIO_CONCURENCIA, ''''yyyymmdd'''')) = TRUNC(CON.CON_FECHA_INI) AND TRUNC(TO_DATE(AUX.FEC_FIN_CONCURENCIA, ''''yyyymmdd'''')) <> TRUNC(CON.CON_FECHA_FIN) AND (OFR.CON_ID IS NULL OR NVL(OFR.CON_ID, 0) <> CON.CON_ID ) AND AUX.FEC_INICIO_CONCURENCIA IS NOT NULL AND AUX.FEC_FIN_CONCURENCIA IS NOT NULL' )
	, T_TIPO_DATA('F10' ,'El activo está en un periodo de concurrencia vivo que se solapa con el periodo de concurrencia nuevo' , '1', 'JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.CON_CONCURRENCIA CON ON CON.ACT_ID = ACT.ACT_ID AND CON.BORRADO = 0 WHERE TRUNC(TO_DATE(AUX.FEC_INICIO_CONCURENCIA,''''yyyymmdd'''')) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN) AND TRUNC(TO_DATE(AUX.FEC_FIN_CONCURENCIA,''''yyyymmdd'''')) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN) AND TRUNC(TO_DATE(AUX.FEC_FIN_CONCURENCIA,''''yyyymmdd'''')) <> TRUNC(SYSDATE) AND TRUNC(TO_DATE(AUX.FEC_FIN_CONCURENCIA,''''yyyymmdd'''')) <> TRUNC(CON.CON_FECHA_FIN) AND TRUNC(TO_DATE(AUX.FEC_INICIO_CONCURENCIA,''''yyyymmdd'''')) <> TRUNC(CON.CON_FECHA_INI) AND AUX.FEC_INICIO_CONCURENCIA IS NOT NULL AND AUX.FEC_FIN_CONCURENCIA IS NOT NULL' )
		);
