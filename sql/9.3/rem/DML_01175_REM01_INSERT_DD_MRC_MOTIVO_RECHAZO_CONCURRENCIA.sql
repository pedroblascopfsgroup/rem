...
DECLARE
	...
	, T_TIPO_DATA('F10' ,'El activo está en un periodo de concurrencia vivo que se solapa con el periodo de concurrencia nuevo' , '1', 'JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.CON_CONCURRENCIA CON ON CON.ACT_ID = ACT.ACT_ID AND CON.BORRADO = 0 WHERE TRUNC(TO_DATE(AUX.FEC_INICIO_CONCURENCIA,''''yyyymmdd'''')) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN) AND TRUNC(TO_DATE(AUX.FEC_FIN_CONCURENCIA,''''yyyymmdd'''')) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN) AND TRUNC(TO_DATE(AUX.FEC_FIN_CONCURENCIA,''''yyyymmdd'''')) <> TRUNC(SYSDATE) AND AUX.FEC_INICIO_CONCURENCIA IS NOT NULL AND AUX.FEC_FIN_CONCURENCIA IS NOT NULL' )
		);
