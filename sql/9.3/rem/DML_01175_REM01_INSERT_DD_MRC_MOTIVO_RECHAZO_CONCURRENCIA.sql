    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        ...
        , T_TIPO_DATA('F03' ,'El activo está en un periodo de concurrencia vivo y tiene la misma fecha inicio que la recibida en el stock, pero la fecha fin recibida es menor a la que tiene y es diferente a la de hoy' , '1', 'JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.CON_CONCURRENCIA CON ON CON.ACT_ID = ACT.ACT_ID AND CON.BORRADO = 0 WHERE SYSDATE BETWEEN CON.CON_FECHA_INI AND CON.CON_FECHA_FIN AND CON.CON_FECHA_INI = TO_DATE(AUX.FEC_INICIO_CONCURENCIA, ''''yyyymmdd'''') AND TO_DATE(AUX.FEC_FIN_CONCURENCIA, ''''yyyymmdd'''') < CON.CON_FECHA_FIN AND TO_DATE(AUX.FEC_FIN_CONCURENCIA, ''''yyyymmdd'''') <> TRUNC(SYSDATE)' )
      	, T_TIPO_DATA('F04' ,'El activo es de alquiler' , '1', 'JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL SCM ON SCM.DD_SCM_ID = ACT.DD_SCM_ID AND SCM.BORRADO = 0 WHERE SCM.DD_SCM_CODIGO = ''''10''''' )
		    ...
		    , T_TIPO_DATA('F06' ,'El activo no tiene un periodo vivo y recibe una fecha de inicio concurrencia anterior a hoy' , '1', 'JOIN REM01.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO AND ACT.BORRADO = 0 LEFT JOIN REM01.CON_CONCURRENCIA CON ON CON.ACT_ID = ACT.ACT_ID AND CON.BORRADO = 0 WHERE (( TRUNC(SYSDATE) < TRUNC(TO_DATE(AUX.FEC_INICIO_CONCURENCIA,''''yyyymmdd'''')) OR TRUNC(SYSDATE) > TRUNC(TO_DATE(AUX.FEC_FIN_CONCURENCIA,''''yyyymmdd''''))) OR CON.CON_ID IS NULL) AND TRUNC(TO_DATE(AUX.FEC_INICIO_CONCURENCIA, ''''yyyymmdd'''')) < TRUNC(SYSDATE)' )
		); 
