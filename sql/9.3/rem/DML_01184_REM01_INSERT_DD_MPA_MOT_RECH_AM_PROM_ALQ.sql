...
?????   ,*/ T_TIPO_DATA('F02' ,'El activo no está dentro del perímetro HAYA' , '1', 'JOIN '||V_ESQUEMA||'.AUX_NUM_UNIDAD_AM AM ON AM.NUM_UNIDAD = AUX.NUM_UNIDAD JOIN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC ON PAC.ACT_ID = AM.ACT_ID AND PAC.BORRADO = 0 WHERE PAC.PAC_INCLUIDO = 1  ' )
        , T_TIPO_DATA('F03' ,'El activo tiene una situación comercial igual a vendido' , '1', 'JOIN '||V_ESQUEMA||'.AUX_NUM_UNIDAD_AM AM ON AM.NUM_UNIDAD = AUX.NUM_UNIDAD JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = AM.ACT_ID AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.DD_SCM_SITUACION_COMERCIAL SCM ON SCM.DD_SCM_ID = ACT.DD_SCM_ID AND SCM.BORRADO = 0 WHERE SCM.DD_SCM_CODIGO = ''''05''''  ' )
        , T_TIPO_DATA('F04' ,'El activo tiene el tipo de título igual a unidad alquilable' , '1', 'JOIN '||V_ESQUEMA||'.AUX_NUM_UNIDAD_AM AM ON AM.NUM_UNIDAD = AUX.NUM_UNIDAD JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = AM.ACT_ID AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.DD_TTA_TIPO_TITULO_ACTIVO TTA ON TTA.DD_TTA_ID = ACT.DD_TTA_ID AND TTA.BORRADO = 0 WHERE TTA.DD_TTA_CODIGO = ''''05'''' ' )
        , T_TIPO_DATA('F05' ,'El activo no tiene patrimonio' , '1', 'JOIN '||V_ESQUEMA||'.AUX_NUM_UNIDAD_AM AM ON AM.NUM_UNIDAD = AUX.NUM_UNIDAD WHERE NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.ACT_PTA_PATRIMONIO_ACTIVO PTA WHERE PTA.ACT_ID = AM.ACT_ID AND PTA.BORRADO = 0 )  ' )
