...
--##########################################
--## AUTOR=IVAN REPISO
--## FECHA_CREACION=20230125
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.3
--## INCIDENCIA_LINK=REMVIP-13169
--## PRODUCTO=NO
--##
...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('106','Notificación al cliente - Fecha de liquidez','04', 'OP-07-COMU-BU')
    ); 
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' WHERE (DD_'||V_TEXT_CHARS||'_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''
                    OR DD_'||V_TEXT_CHARS||'_MATRICULA_GD = '''||TRIM(V_TMP_TIPO_DATA(4))||''') AND BORRADO = 0';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

        IF V_NUM_TABLAS > 0 THEN
          DBMS_OUTPUT.PUT_LINE('[INFO]: REGISTRO CON CODIGO '||TRIM(V_TMP_TIPO_DATA(1))||' O MATRICULA '||TRIM(V_TMP_TIPO_DATA(4))||' YA EXISTE. TERMINAMOS');
       ...
       ELSE
            ...
            V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (
                        ...
                        SELECT '|| V_ID || '
                        , (SELECT DD_TDE_ID FROM '||V_ESQUEMA||'.DD_TDE_TIPO_DOC_EXP WHERE DD_TDE_CODIGO = '''||V_TMP_TIPO_DATA(3)||''')
                        , '''||V_TMP_TIPO_DATA(1)||'''
                        ,'''||TRIM(V_TMP_TIPO_DATA(2))||'''
                        ,'''||TRIM(V_TMP_TIPO_DATA(2))||'''
                        , '''||TRIM(V_TMP_TIPO_DATA(4))||'''
                        , ''REMVIP-13169'',SYSDATE FROM DUAL';
