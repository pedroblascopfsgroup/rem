...
BEGIN
    ...
    LOOP
        ...
        ELSE 
            DBMS_OUTPUT.PUT_LINE('[INFO] Ya existe el subgasto '||TRIM(V_TMP_CUENTA(1))||' con la cuenta contable '||TRIM(V_TMP_CUENTA(3))||' para el año '||TRIM(V_TMP_CUENTA(5))||'. Le asignamos 1 al campo CCC_REFACTURABLE.');

            V_SQL :=   '
                    MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' TAB USING (
                        SELECT CCC_ID
                        FROM '||V_ESQUEMA||'.'||V_TABLA||' 
                        WHERE DD_STG_ID = (SELECT DD_STG_ID 
                                            FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO 
                    ...
                    ON (TMP.CCC_ID = TAB.CCC_ID)
                    WHEN MATCHED THEN UPDATE SET 
                    TAB.CCC_REFACTURABLE = 1,
                    TAB.USUARIOMODIFICAR = '''||V_USUARIO||''',
                    TAB.FECHAMODIFICAR = SYSDATE
                    ';

        EXECUTE IMMEDIATE V_SQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] El subtipo de gasto '||TRIM(V_TMP_CUENTA(1))||' ha sido actualizado satisfactoriamente.');
        END IF;
