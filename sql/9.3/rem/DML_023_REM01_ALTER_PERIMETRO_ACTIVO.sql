...
BEGIN	
    ...
    V_MSQL := 'select count(1) from ALL_TAB_COLUMNS where COLUMN_NAME = ''PAC_OFERTAS_VIVAS''';
    EXECUTE IMMEDIATE V_MSQL into V_NUM;
    IF V_NUM = 0 THEN
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (PAC_OFERTAS_VIVAS NUMBER(1,0))';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO]: '||sql%rowcount||' REGISTROS ACTUALIZADOS.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('[INFO]: EL REGISTRO PAC_OFERTAS_VIVAS YA EXISTE');
    END IF;

    V_MSQL := 'select count(1) from ALL_TAB_COLUMNS where COLUMN_NAME = ''PAC_TRABAJOS_VIVOS''';
    EXECUTE IMMEDIATE V_MSQL into V_NUM;
    IF V_NUM = 0 THEN
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' ADD (PAC_TRABAJOS_VIVOS NUMBER(1,0))';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO]: '||sql%rowcount||' REGISTROS ACTUALIZADOS.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('[INFO]: EL REGISTRO PAC_TRABAJOS_VIVOS YA EXISTE');
    END IF;
