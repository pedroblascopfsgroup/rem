...
BEGIN
  ...
  EXECUTE IMMEDIATE 'select count(1) from ALL_VIEWS where view_name = ''V_ACT_ESTADO_DISP'' and owner = '''||V_ESQUEMA||'''' INTO table_count;

  IF TABLE_COUNT < 1 THEN 

    EXECUTE IMMEDIATE 'CREATE VIEW '||V_ESQUEMA||'.V_ACT_ESTADO_DISP as (
    SELECT act.act_id,
