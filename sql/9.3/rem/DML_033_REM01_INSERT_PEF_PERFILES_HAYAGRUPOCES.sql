...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'DML'; -- USUARIOCREAR/USUARIOMODIFICAR.
    V_ECO_ID NUMBER(16); 
    
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] INSERCION TABLA PEF_PERFILES');
	
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = ''HAYAGRUPOCES''';
	...
	IF V_NUM_FILAS = 0 THEN
		...
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.PEF_PERFILES 
					...
					''HAYAGRUPOCES'',
					''Perfil para usuarios del grupo CES'',
					''Perfil para usuarios del grupo CES'',
					0,
					''DML'',
					SYSDATE,
