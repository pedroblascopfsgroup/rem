	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.GDE_GASTOS_DETALLE_ECONOMICO GDE
				USING (
				   ...
				   CASE WHEN GPL_DIARIO1 = ''1'' OR GPL_DIARIO1 = ''20'' AND GPL_DIARIO2 = ''60'' THEN NVL(GPL_DIARIO2_BASE, 0)
				       WHEN GPL_DIARIO1 = ''2'' OR GPL_DIARIO1 = ''60'' AND NVL(GPL_DIARIO2, 0) <> ''60'' THEN NVL(GPL_DIARIO1_BASE, 0)
				   ...
				   NVL(CASE WHEN GPL_DIARIO1_TIPO > 100 THEN 21 ELSE GPL_DIARIO1_TIPO END, 0) AS IMP_IND_TIPO_IMPOSITIVO,
				   NVL(GPL_DIARIO1_CUOTA, 0) AS IMP_IND_CUOTA,
				   NVL(CASE WHEN GPL_PROCENTAJE_IRPF > 100 THEN 21 ELSE GPL_PROCENTAJE_IRPF END, 0) AS IRPF_TIPO_IMPOSITIVO,
				   NVL(GPL_IMPORTE_IRPF, 0) AS IRPF_CUOTA,
				   ROW_NUMBER() OVER (PARTITION BY gpv_id ORDER BY act_id ASC) AS RN
				   FROM '||V_ESQUEMA||'.GPL_GASTOS_PRINEX_LBK WHERE GPL_DIARIO1 IS NOT NULL AND BORRADO = 0) WHERE RN = 1) AUX ON (AUX.GPV_ID = GDE.GPV_ID)
