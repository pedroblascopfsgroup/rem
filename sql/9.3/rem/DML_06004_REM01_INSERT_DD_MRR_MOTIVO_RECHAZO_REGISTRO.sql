...
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial - [HREOS-14649] - Daniel Algaba
--##        0.2 Añadir validación F67: controlar que todos los activos de una OB-REM pasen todas las validaciones - [HREOS-15059] - Alejandra García
--##########################################
--*/
...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        ...
        -- Validación de agrupaciones OB_REM
        T_TIPO_DATA('F67','Restringidas obrem - Diferente perímetro que el principal','Restringidas obrem - Diferente perímetro que el principal',1,'JOIN(SELECT  AUX.NUM_IDENTIFICATIVO , PAC.PAC_INCLUIDO , PAC.PAC_CHECK_ADMISION , PAC.PAC_CHECK_COMERCIALIZAR , PAC.PAC_CHECK_FORMALIZAR , PAC.PAC_CHECK_GESTIONAR FROM '||V_ESQUEMA||'.AUX_BC_AGRUPACIONES AUX JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON AUX.NUM_IDENTIFICATIVO = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC ON PAC.ACT_ID = ACT.ACT_ID AND PAC.BORRADO = 0 JOIN (SELECT  AUX.NUM_IDENTIFICATIVO , PAC.PAC_INCLUIDO , PAC.PAC_CHECK_ADMISION , PAC.PAC_CHECK_COMERCIALIZAR , PAC.PAC_CHECK_FORMALIZAR , PAC.PAC_CHECK_GESTIONAR FROM '||V_ESQUEMA||'.AUX_BC_AGRUPACIONES AUX JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON AUX.PROMO_CONJUNTA_OB_REM = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC ON PAC.ACT_ID = ACT.ACT_ID AND PAC.BORRADO = 0 WHERE AUX.PROMO_CONJUNTA_OB_REM IS NOT NULL ) PRI ON PRI.NUM_IDENTIFICATIVO = AUX.NUM_IDENTIFICATIVO WHERE AUX.PROMO_CONJUNTA_OB_REM IS NOT NULL AND (PAC.PAC_INCLUIDO <> PRI.PAC_INCLUIDO OR PAC.PAC_CHECK_ADMISION <> PRI.PAC_CHECK_ADMISION OR PAC.PAC_CHECK_COMERCIALIZAR <> PRI.PAC_CHECK_COMERCIALIZAR OR PAC.PAC_CHECK_FORMALIZAR <> PRI.PAC_CHECK_FORMALIZAR OR PAC.PAC_CHECK_GESTIONAR <> PRI.PAC_CHECK_GESTIONAR) )ACTIVOS ON AUX.NUM_IDENTIFICATIVO=ACTIVOS.NUM_IDENTIFICATIVO'),
        T_TIPO_DATA('F68','Restringidas obrem - Diferente propietario que el principal','Restringidas obrem - Diferente propietario que el principal',1,'JOIN(SELECT AUX.NUM_IDENTIFICATIVO , PRO.PRO_ID FROM '||V_ESQUEMA||'.AUX_BC_AGRUPACIONES AUX JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON AUX.NUM_IDENTIFICATIVO = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_PAC_PROPIETARIO_ACTIVO PRO ON PRO.ACT_ID = ACT.ACT_ID AND PRO.BORRADO = 0 JOIN (SELECT  AUX.NUM_IDENTIFICATIVO , PRO.PRO_ID FROM '||V_ESQUEMA||'.AUX_BC_AGRUPACIONES AUX JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON AUX.PROMO_CONJUNTA_OB_REM = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_PAC_PROPIETARIO_ACTIVO PRO ON PRO.ACT_ID = ACT.ACT_ID AND PRO.BORRADO = 0 WHERE AUX.PROMO_CONJUNTA_OB_REM IS NOT NULL ) PRI ON PRI.NUM_IDENTIFICATIVO = AUX.NUM_IDENTIFICATIVO WHERE AUX.PROMO_CONJUNTA_OB_REM IS NOT NULL AND PRO.PRO_ID <> PRI.PRO_ID )ACTIVOS ON AUX.NUM_IDENTIFICATIVO=ACTIVOS.NUM_IDENTIFICATIVO'),
        T_TIPO_DATA('F69','Restringidas obrem - destino comercial diferente al principal','Restringidas obrem - destino comercial diferente al principal',1,'JOIN(SELECT  AUX.NUM_IDENTIFICATIVO ,TCO.DD_TCO_CODIGO FROM '||V_ESQUEMA||'.AUX_BC_AGRUPACIONES AUX JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON AUX.NUM_IDENTIFICATIVO = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU ON APU.ACT_ID = ACT.ACT_ID AND APU.BORRADO = 0 LEFT JOIN '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION TCO ON TCO.DD_TCO_ID = APU.DD_TCO_ID AND TCO.BORRADO = 0 JOIN (SELECT DISTINCT AUX.NUM_IDENTIFICATIVO , TCO.DD_TCO_CODIGO FROM '||V_ESQUEMA||'.AUX_BC_AGRUPACIONES AUX JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON AUX.PROMO_CONJUNTA_OB_REM = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0 JOIN '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU ON APU.ACT_ID = ACT.ACT_ID AND APU.BORRADO = 0 LEFT JOIN '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION TCO ON TCO.DD_TCO_ID = APU.DD_TCO_ID AND TCO.BORRADO = 0 WHERE AUX.PROMO_CONJUNTA_OB_REM IS NOT NULL  ) PRI ON PRI.NUM_IDENTIFICATIVO = AUX.NUM_IDENTIFICATIVO WHERE AUX.PROMO_CONJUNTA_OB_REM IS NOT NULL AND TCO.DD_TCO_CODIGO <> PRI.DD_TCO_CODIGO ) ACTIVOS ON AUX.NUM_IDENTIFICATIVO=ACTIVOS.NUM_IDENTIFICATIVO'),
        
        --La F99 tiene que ser siempre la última
        T_TIPO_DATA('F99','Activo de una OB-REM no ha pasado todas las validaciones','Activo de una OB-REM no ha pasado todas las validaciones',1,'JOIN (SELECT DISTINCT REJ.NUM_IDENTIFICATIVO ,TOT.PROMO_CONJUNTA_OB_REM FROM '||V_ESQUEMA||'.AUX_APR_BCR_STOCK_REJ REJ JOIN (SELECT AUX.PROMO_CONJUNTA_OB_REM , AUX.NUM_IDENTIFICATIVO FROM '||V_ESQUEMA||'.AUX_APR_BCR_STOCK AUX JOIN (SELECT  AUX.PROMO_CONJUNTA_OB_REM FROM '||V_ESQUEMA||'.AUX_APR_BCR_STOCK AUX WHERE AUX.PROMO_CONJUNTA_OB_REM IS NOT NULL GROUP BY AUX.PROMO_CONJUNTA_OB_REM )OBREM_V ON AUX.PROMO_CONJUNTA_OB_REM = OBREM_V.PROMO_CONJUNTA_OB_REM )TOT ON REJ.NUM_IDENTIFICATIVO=TOT.NUM_IDENTIFICATIVO  ) RECH ON RECH.PROMO_CONJUNTA_OB_REM=AUX.PROMO_CONJUNTA_OB_REM')
    ); 
