...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-15639';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(800);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T017_FirmaContrato','comboFirma','combobox','Se ha cancelado la firma','Debe indicar si la firma se ha cancelado o no','DDSiNo'),
    	T_TIPO_DATA('T017_FirmaContrato','motivoAplazamiento','combobox','Motivo aplazamiento',null,'DDSiNo')    	
    ); 
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL CAMPO '''|| TRIM(V_TMP_TIPO_DATA(2)) ||''' DE '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
                    'SET '||V_TEXT_CHARS||'_LABEL = '''||TRIM(V_TMP_TIPO_DATA(4))||''', '||
