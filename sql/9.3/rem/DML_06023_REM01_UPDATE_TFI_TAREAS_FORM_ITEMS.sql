...
--##########################################
--## AUTOR=IVAN REPISO
--## FECHA_CREACION=20211017
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.3
--## INCIDENCIA_LINK=HREOS-15759
--## PRODUCTO=NO
--##
--## Finalidad:
--## INSTRUCCIONES: 
...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-15759';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(800);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T017_FirmaContrato','motivoAplazamiento','textfield')    	
    ); 
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL CAMPO '''|| TRIM(V_TMP_TIPO_DATA(2)) ||''' DE '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||'
                    SET '||V_TEXT_CHARS||'_TIPO = '''||TRIM(V_TMP_TIPO_DATA(3))||''', 
                    '||V_TEXT_CHARS||'_BUSINESS_OPERATION = NULL, USUARIOMODIFICAR = '''||V_USUARIO||''' 
                    , FECHAMODIFICAR = SYSDATE WHERE '||V_TEXT_CHARS||'_NOMBRE = '''||TRIM(V_TMP_TIPO_DATA(2))||''' AND TAP_ID = '||V_TAP_ID||'';
