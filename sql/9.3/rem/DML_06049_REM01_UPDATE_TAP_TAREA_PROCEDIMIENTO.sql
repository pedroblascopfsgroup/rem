...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-10863';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(800);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T017_PBCVenta', 'valores[''''T017_PBCVenta''''][''''comboRespuesta''''] == DDSiNo.SI ? checkBankia() ? checkEstadoBC() ? checkFechaContabilizacionArras() ? actualizarOfertaBoarding() : ''''El expediente tiene arras y no tiene fecha de contabilizaci&oacute;n de arras.'''' : ''''No se puede avanzar porque est&aacute; pendiente de BC'''' : actualizarOfertaBoarding() : valores[''''T017_PBCVenta''''][''''comboRespuesta''''] == DDSiNo.NO ? checkBankia() ? tieneRellenosCamposAnulacion() ? null : ''''Debe estar informado el motivo de anulaci&oacute;n, la fecha y el detalle.'''' : null : null'),
        T_TIPO_DATA('T017_PBCReserva', 'checkBankia() ? valores[''''T017_PBCReserva''''][''''comboRespuesta''''] == DDSiNo.NO ? tieneRellenosCamposAnulacion() ? null : ''''Debe estar informado el motivo de anulaci&oacute;n, la fecha y el detalle.'''' : checkEstadoBC() ? null : ''''No se puede avanzar porque est&aacute; pendiente de BC'''' : null')
    ); 
