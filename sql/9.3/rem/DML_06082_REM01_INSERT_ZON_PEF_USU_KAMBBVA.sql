...
--## INCIDENCIA_LINK=HREOS-17439
--## PRODUCTO=NO
--##
--## Finalidad: Script que añade Usuarios al Perfil Oficina KAM BBVA
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    --			USU_USERNAME			CODIGO PERFIL
 	  T_FUNCION('SUPER',		'KAMBBVA'),
 	  T_FUNCION('ealvarez',		'KAMBBVA'),
	  T_FUNCION('aalcazar',		'KAMBBVA'),
	  T_FUNCION('egonzalezc', 	'KAMBBVA'),
	  T_FUNCION('avalerog',		'KAMBBVA')
    );
			...
			ELSE
				...
				ELSE
					V_MSQL_1 := 'INSERT INTO '||V_ESQUEMA||'.ZON_PEF_USU' ||
								...
								' (SELECT PEF_ID FROM '||V_ESQUEMA||'.PEF_PERFILES WHERE PEF_CODIGO = '''||TRIM(V_TMP_FUNCION(2))||'''),' ||
								' (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||TRIM(V_TMP_FUNCION(1))||'''),' ||
								' ''HREOS-17439'',SYSDATE,0 FROM DUAL';
