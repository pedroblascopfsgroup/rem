...
DECLARE
    ...
    V_USUARIO VARCHAR2(32 CHAR):= 'REMVIP-10974';
	...
	TIPO_TPD_VISIBLE VARCHAR2(256 CHAR);

    TYPE T_JBV IS TABLE OF VARCHAR2(256);
    TYPE T_ARRAY_JBV IS TABLE OF T_JBV; 
	
	V_JBV T_ARRAY_JBV := T_ARRAY_JBV(
		  T_JBV('Informe desocupación CAJAMAR','AI-02-ESIN-EB','191', '1'),
          T_JBV('Informe desocupación CAJAMAR','OP-13-ESIN-EB','192', '0')
	); 
	V_TMP_JBV T_JBV;
...
BEGIN
    ...
    FOR I IN V_JBV.FIRST .. V_JBV.LAST
 	LOOP
        ...
        TIPO_TPD_VISIBLE := TRIM(V_TMP_JBV(4));
        
        V_SQL:='SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE DD_TPD_CODIGO = '''||TIPO_CODIGO||''' AND BORRADO = 0';
        ...
        IF V_COUNT = 0 THEN 	
            ...
            V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (DD_TPD_ID, DD_TPD_CODIGO, DD_TPD_DESCRIPCION, DD_TPD_DESCRIPCION_LARGA,DD_TPD_MATRICULA_GD, USUARIOCREAR, FECHACREAR,DD_TPD_VISIBLE) 
                    VALUES 
                    ('''||V_ID||''','''||TIPO_CODIGO||''','''||TIPO_DESCRIPCION||''','''||TIPO_DESCRIPCION||''','''||TIPO_MATRICULA||''','''||V_USUARIO||''', SYSDATE, '''|| TIPO_TPD_VISIBLE ||''')';
