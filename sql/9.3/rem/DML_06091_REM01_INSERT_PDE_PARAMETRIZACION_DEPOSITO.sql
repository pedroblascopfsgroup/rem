...
DECLARE
    ...
    DD_EQG_ID NUMBER(16); -- Vble. auxiliar para almacenar temporalmente el numero de la sequencia.
    ...
    V_USUARIO VARCHAR2(200 CHAR) := 'HREOS-17635';
...
BEGIN	
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE DD_EQG_ID = '''||TRIM(V_TMP_TIPO_DATA(1))||'''
                AND DD_SCR_ID = '''||TRIM(V_TMP_TIPO_DATA(2))||''' AND PDE_PRECIO = '''||TRIM(V_TMP_TIPO_DATA(4))||'''';
       ...
       ELSE
          ...
          V_MSQL := 'SELECT DD_EQG_ID FROM '||V_ESQUEMA||'.DD_EQG_EQUIPO_GESTION WHERE DD_EQG_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''';
          EXECUTE IMMEDIATE V_MSQL INTO DD_EQG_ID;
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' (' ||
                      'PDE_ID ,DD_EQG_ID, DD_SCR_ID, PDE_IMPORTE, PDE_PRECIO, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ','''||DD_EQG_ID||''','||DD_SCR_ID|| ','''||TRIM(V_TMP_TIPO_DATA(3))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''' ,0, '''||V_USUARIO||''',SYSDATE,0 FROM DUAL';
