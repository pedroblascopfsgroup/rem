      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (DD_'||V_TEXT_CHARS||'_ID, DD_TDE_ID, 
                      ...
                      DD_'||V_TEXT_CHARS||'_MATRICULA_GD, USUARIOCREAR, FECHACREAR) 
                      SELECT '|| V_ID || ', (SELECT DD_TDE_ID FROM DD_TDE_TIPO_DOC_EXP WHERE DD_TDE_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(4))||''')
                      , '''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(2))||'''
                      , '''||TRIM(V_TMP_TIPO_DATA(3))||''', ''HREOS-17751'',SYSDATE 
                  		  FROM DUAL';
