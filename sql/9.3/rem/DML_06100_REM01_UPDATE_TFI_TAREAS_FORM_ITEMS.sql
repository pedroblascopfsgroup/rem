...
DECLARE
    ...
    V_TEXT_CHARS VARCHAR2(2400 CHAR) := 'TFI'; -- Vble. auxiliar para almacenar las 3 letras orientativas de la tabla de ref.
	  V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-11577';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(800);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(

        T_TIPO_DATA('T018_DefinicionOferta'		,'textfield', 	'3'		,'expedienteAnterior','numContratoAnterior' ,null 	,null ,'Numero contrato anterior:'),
        T_TIPO_DATA('T018_DefinicionOferta'		,'datefield', 	'4'		,'fechaContratoAnt','fechaContratoAnt' ,null 	,null ,'Fecha preaviso contrato anterior:'),
    	T_TIPO_DATA('T018_DefinicionOferta'		,'combobox', 	'5'		,'riesgoReputacional' ,'riesgoReputacional' ,'false' 	,'DDSiNo' ,'Riesgo reputacional:'),
    	T_TIPO_DATA('T018_DefinicionOferta'		,'combobox', 	'6'		,'conflictoIntereses','conflictoIntereses' ,'false' 	,'DDSiNo'	,'Conflicto de intereses:'),
        T_TIPO_DATA('T018_DefinicionOferta'		,'textarea', 	'7'		,'observaciones','observaciones'		,null 		,null			,'Observaciones')
    ); 
      ...
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL CAMPO '''|| TRIM(V_TMP_TIPO_DATA(3)) ||''' DE '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
					...
					', '||V_TEXT_CHARS||'_NOMBRE = '''||TRIM(V_TMP_TIPO_DATA(5))||''''||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                      ...
                      'SELECT '|| V_ID || ', '||V_TAP_ID||', '''||V_TMP_TIPO_DATA(3)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''', '''||TRIM(V_TMP_TIPO_DATA(8))||''', '''||TRIM(V_TMP_TIPO_DATA(6))||''', '''||TRIM(V_TMP_TIPO_DATA(7))||''', 0, '''||V_USUARIO||''',SYSDATE,0 '||
					  ' FROM DUAL';
