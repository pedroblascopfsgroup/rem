...
DECLARE
	...
	V_USUARIOCREAR VARCHAR2(50 CHAR) := 'HREOS-18013';

	TYPE T_PWD IS TABLE OF VARCHAR2(50 CHAR);
	V_PWD T_PWD := T_PWD('TAQEMLKL');
	
	-- Arrays de usuarios:
	TYPE T_USU IS TABLE OF VARCHAR2(50 CHAR);
	TYPE T_USUARIOS IS TABLE OF T_USU;
	-- USU_USERNAME, USU_PASSWORD, BUENO, USU_APELLIDO1, USU_APELLIDO2, USU_GRUPO
	V_USER T_USUARIOS := T_USUARIOS(
		T_USU('LREO_USER', ''||V_PWD(1)||'', 'usuario legal reo','','','1')
	...
	LOOP
		...
		IF V_COUNT > 0 THEN
			...
			V_SQL := 'UPDATE '||V_ESQUEMA_M||'.'||V_TABLA||' SET 
				...
				BORRADO = 0, 
				USU_FECHA_VIGENCIA_PASS = TO_DATE(''06/04/2099'',''DD/MM/YYYY'')
