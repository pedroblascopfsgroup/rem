      ...
      LOOP
	    ...
	    IF V_COUNT = 0 THEN
		    ...
		    VALUES(
		            ...
		            , (SELECT DD_STA_ID FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO = ''TGCA'') 
		            ,null
		            ,null
		            ,null
		        )';
	    ...
	    IF V_COUNT = 0 THEN
			V_MSQL := ' INSERT INTO '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS
		            VALUES (
		                    ...
		                    , null , null  , null , null  , 0, ''HREOS-18264'', SYSDATE , null, null, null , null  , 0)';
		    EXECUTE IMMEDIATE V_MSQL;
			...
			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.TFI_TAREAS_FORM_ITEMS
	            VALUES (
	                '||V_ESQUEMA||'.S_TFI_TAREAS_FORM_ITEMS.NEXTVAL
	                ,(SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = '''||V_TMP_TIPO_DATA(1)||''')
	                ,  1, ''textarea'', ''observaciones'' , ''Observaciones'' , '''' , '''' 
	                , null , ''''  , 0, ''HREOS-18264'', SYSDATE , null, null, null , null  , 0
	            )';
			EXECUTE IMMEDIATE V_MSQL;
