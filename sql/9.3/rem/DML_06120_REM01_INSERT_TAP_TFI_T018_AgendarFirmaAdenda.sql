...
DECLARE
	...
	V_TAP_ID NUMBER(16);
	V_NOMBRE_FILA VARCHAR2(2400 CHAR) := 'comboResultado';
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		...
		T_TIPO_DATA('T018_AgendarFirmaAdenda', 'Agendar firma adenda', '', '', '')
    ); 
    ...
    V_TIPO_DATA2 T_ARRAY_DATA2 := T_ARRAY_DATA2(
		...
		T_TIPO_DATA2('T018_AgendarFirmaAdenda'	,'datefield', 	'1'		,'fechaFirma' 				,'' 		,'false'   ,null          	,'Fecha'),
		T_TIPO_DATA2('T018_AgendarFirmaAdenda'	,'textfield', 	'2'		,'lugarFirma'	        	,''			,'false'   ,null  			,'Lugar'),
		T_TIPO_DATA2('T018_AgendarFirmaAdenda'	,'textarea', 	'3'		,'justificacion'	        ,''         ,null      ,null		    ,'Justificación'),
    	T_TIPO_DATA2('T018_AgendarFirmaAdenda'	,'textarea', 	'4'		,'observaciones'	        ,''         ,null      ,null		    ,'Observaciones')
    ); 
...
--INSERT TAP_TAREA_PROCEDIMIENTO ----------------------------
 FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
	...
	--DELETE TFI_TAREA_FORMS_ITEMS ----------------------------

		DBMS_OUTPUT.PUT_LINE('BÚSQUEDA [TFI_TAREAS_FORM_ITEMS]');
		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' 
				WHERE TAP_ID = (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = '''||V_TMP_TIPO_DATA(1)||''')
				AND TFI_NOMBRE = '''||V_NOMBRE_FILA||'''';
       	EXECUTE IMMEDIATE V_SQL INTO V_COUNT;

		   DBMS_OUTPUT.PUT_LINE('BÚSQUEDA CAMPO [TFI_TAREAS_FORM_ITEMS] CORRECTA');
		...
		IF V_COUNT > 0 THEN
			V_MSQL := 'DELETE FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||'
					...
					AND TFI_NOMBRE = '''||V_NOMBRE_FILA||'''';
			EXECUTE IMMEDIATE V_MSQL;
			 DBMS_OUTPUT.PUT_LINE('CAMPO BORRADO [TFI_TAREAS_FORM_ITEMS] CORRECTAMENTE');
		ELSE 
		 	DBMS_OUTPUT.PUT_LINE(' LA FILA NO EXISTE EN [TFI_TAREAS_FORM_ITEMS]');
		END IF;
