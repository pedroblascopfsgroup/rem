...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-18720';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(150);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T018_DecisionContinuidadOferta'    ,'label'		,'0'	,'titulo'					,'<p style="margin-bottom:10px">Instrucciones por defecto de la tarea ''''Decision coninuidad de la oferta'''' </p>'		    			,null		,null),
    	T_TIPO_DATA('T018_DecisionContinuidadOferta'	,'combobox'		,'1'	,'seguirOferta'	        	,'Debe indicar si se sigue con la oferta o No'    																							,'false'   	,'DDSiNo'),
    	T_TIPO_DATA('T018_DecisionContinuidadOferta'	,'textarea'     ,'2'	,'justificacion'	        ,'Justificacion'                                       																						,'false'   	,null)
    ); 
      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                      ...
                      'SELECT '|| V_ID || ', '||V_TAP_ID||', '''||V_TMP_TIPO_DATA(3)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''', '''||TRIM(V_TMP_TIPO_DATA(5))||''', '''||TRIM(V_TMP_TIPO_DATA(6))||''', '''||TRIM(V_TMP_TIPO_DATA(7))||''', 0, '''||V_USUARIO||''',SYSDATE,0 '||
					  ' FROM DUAL';
