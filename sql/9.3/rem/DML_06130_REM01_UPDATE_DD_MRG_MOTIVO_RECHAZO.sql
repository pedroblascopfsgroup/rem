...
--######################################### 
--## AUTOR=Daniel Algaba
--## FECHA_CREACION=20221014
--## ARTEFACTO=batch
--## VERSION_ARTEFACTO=9.3
--## INCIDENCIA_LINK=HREOS-18796
--## PRODUCTO=NO
--## 
...
DECLARE
    ...
    T_TIPO_DATA('F49' , 'Gasto cíclico. Comprobar si tiene el mismo tipo y subtipo que el inicio de la serie','F12')
    ); 
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.DD_MRG_MOTIVO_RECHAZO_GASTO '||
                    'SET DD_MRG_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(3))||''''||
                    ', DD_MRG_DESCRIPCION = '''||TRIM(V_TMP_TIPO_DATA(2))||''''||
