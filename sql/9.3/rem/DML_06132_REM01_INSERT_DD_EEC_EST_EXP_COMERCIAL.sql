...
DECLARE
    ...
    V_ID NUMBER(16);
    V_USU VARCHAR2(50 CHAR):= 'HREOS-18499'; -- Configuracion Esquema Master
    
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(150);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('60', 'Pendiente comunicar subrogación', 'Pendiente comunicar subrogación')
		); 
      ...
      LOOP
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.DD_EEC_EST_EXP_COMERCIAL '||
					...
					', USUARIOMODIFICAR = '''|| V_USU ||''' , FECHAMODIFICAR = SYSDATE,  DD_EEC_VENTA = 0, DD_EEC_ALQUILER = 1'||
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.DD_EEC_EST_EXP_COMERCIAL (' ||
                      ...
                      'SELECT '|| V_ID || ','''||V_TMP_TIPO_DATA(1)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(3))||''', 0, '''|| V_USU ||''', SYSDATE,0, 0, 1  FROM DUAL';
          EXECUTE IMMEDIATE V_MSQL;
