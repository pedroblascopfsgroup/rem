...
DECLARE
	...
	V_USUARIO VARCHAR2(50 CHAR) := 'HREOS-18717';
    TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(800);
    TYPE T_ARRAY_DATA IS TABLE OF T_TIPO_DATA;
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T018_AprobacionOferta'		,'combobox', 	'4'			,'tituloObtenido'			,'false' 	,'Título obtenido'	, 'DDSiNo'),
    	T_TIPO_DATA('T018_AprobacionOferta'		,'combobox', 	'3'			,'contraoferta'				,'false'	,'Contraoferta'		, 'DDSiNo'),
    	T_TIPO_DATA('T018_AprobacionOferta'		,'datefield', 	'5'			,'fechaTitulo'				,'false'	,'Fecha título'		, null),
    	T_TIPO_DATA('T018_AprobacionOferta'		,'textarea', 	'2'			,'motivo'					,null		,'Motivo'			, null),
    	T_TIPO_DATA('T018_AprobacionOferta'		,'combobox', 	'11'		,'tipoAdenda'				,null		,'Tipo adenda'		, 'DDTipoAdenda')
    ); 
      ...
      LOOP
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (' ||
                      ''||V_TEXT_CHARS||'_ID, TAP_ID, '||V_TEXT_CHARS||'_ORDEN, '||V_TEXT_CHARS||'_TIPO, '||V_TEXT_CHARS||'_NOMBRE, '||V_TEXT_CHARS||'_LABEL, '||V_TEXT_CHARS||'_BUSINESS_OPERATION, '||V_TEXT_CHARS||'_VALIDACION, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
                      'SELECT '|| V_ID || ', '||V_TAP_ID||', '''||V_TMP_TIPO_DATA(3)||''','''||TRIM(V_TMP_TIPO_DATA(2))||''','''||TRIM(V_TMP_TIPO_DATA(4))||''', '''||TRIM(V_TMP_TIPO_DATA(6))||''', '''||TRIM(V_TMP_TIPO_DATA(5))||''', '''||TRIM(V_TMP_TIPO_DATA(7))||''', 0, '''||V_USUARIO||''',SYSDATE,0 '||
					  ' FROM DUAL';
