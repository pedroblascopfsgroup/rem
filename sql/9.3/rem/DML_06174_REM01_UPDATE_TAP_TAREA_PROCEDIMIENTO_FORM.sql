...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
    	T_TIPO_DATA('T018_AprobacionOferta', '(checkBankia() == true && checkExpedienteBloqueadoPorFuncion() == true) ? ''''El expediente est&aacute; bloqueado'''' : estanCamposRellenosParaFormalizacion() ? estaPermitidoClaseCondicion() ? null : ''''Clase condicion solo puede tener el valor no aplica para este tipo de alquiler'''' : ''''Alguno o varios de los siguientes campos no estan rellenos: Clase contrato, Grupo CBK, Tipo impuesto, Grupo impuesto, Tipo aplicable''''')
    ); 
