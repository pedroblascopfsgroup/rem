...
DECLARE
    ...
    V_TIPO_DATA2 T_ARRAY_DATA2 := T_ARRAY_DATA2(
		T_TIPO_DATA2('T018_AgendarFirma'		,'fechaAgendacionIngreso' ,'Agendación de firma'),
        T_TIPO_DATA2('T018_AgendarFirma'		,'fechaReagendarIngreso'  ,'Reagendación de firma'),
  		T_TIPO_DATA2('T015_AgendarFechaFirma'	,'fechaAgendacionIngreso' ,'Agendación de firma'),
        T_TIPO_DATA2('T015_AgendarFechaFirma'	,'fechaReagendarIngreso'  ,'Reagendación de firma')
    ...
    FOR I IN V_TIPO_DATA2.FIRST .. V_TIPO_DATA2.LAST
      LOOP
		...
		V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' WHERE '||
				'TAP_ID = '||V_TAP_ID||' '||
				'AND '||V_TEXT_CHARS||'_NOMBRE = '''||TRIM(V_TMP_TIPO_DATA2(2))||'''';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
        IF V_NUM_TABLAS > 0 THEN				
         -- Si existe se modifica.
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAR EL CAMPO '''|| TRIM(V_TMP_TIPO_DATA2(2)) ||''' DE '''|| TRIM(V_TMP_TIPO_DATA2(1)) ||'''');
          
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' '||
                    'SET  '||V_TEXT_CHARS||'_LABEL = '''||TRIM(V_TMP_TIPO_DATA2(3))||''''||
					', USUARIOMODIFICAR = '''||V_USUARIO||''' , FECHAMODIFICAR = SYSDATE, BORRADO = 0 '||
					'WHERE '||V_TEXT_CHARS||'_NOMBRE = '''||TRIM(V_TMP_TIPO_DATA2(2))||''' AND TAP_ID = '||V_TAP_ID||'';
