...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      	T_TIPO_DATA('02','OP-14-DOCI-07','ECO', 'CIF', '0878')
    ); 
...
DBMS_OUTPUT.PUT_LINE('[INICIO]');
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        ELSE 
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: El valor '''||TRIM(V_TMP_TIPO_DATA(1))||''' no existe');
            ...
            V_MSQL := 'UPDATE '||V_ESQUEMA||'.'||V_TEXT_TABLA||' SET
              DD_'||V_LETRAS_TABLA||'_MATRICULA_GD = '''||TRIM(V_TMP_TIPO_DATA(2))||'''
			  , USUARIOMODIFICAR ='''||V_TIQUET ||''' , FECHAMODIFICAR = SYSDATE
			 WHERE  DD_'||V_LETRAS_TABLA||'_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''';
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA2||' 
					WHERE DD_'||V_LETRAS_TABLA2||'_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||''' AND BORRADO = 0';
        EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;
        IF V_NUM_TABLAS = 1 THEN
          DBMS_OUTPUT.PUT_LINE('[INFO]: El valor '''||TRIM(V_TMP_TIPO_DATA(1))||''' ya existe');
        ELSE 
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: El valor '''||TRIM(V_TMP_TIPO_DATA(1))||''' no existe');
            ...
            V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA2||' (
              DD_'||V_LETRAS_TABLA2||'_ID,
			  DD_TED_ID,
              DD_'||V_LETRAS_TABLA2||'_CODIGO,
              DD_'||V_LETRAS_TABLA2||'_DESCRIPCION,
              DD_'||V_LETRAS_TABLA2||'_DESCRIPCION_LARGA,
 			  DD_'||V_LETRAS_TABLA2||'_MATRICULA,
              VERSION,
              USUARIOCREAR,
              FECHACREAR,
              BORRADO
              ) VALUES (
               '||V_ESQUEMA||'.S_'||V_TEXT_TABLA2||'.NEXTVAL,
			  (SELECT DD_TED_ID FROM '||V_ESQUEMA||'.DD_TED_TIP_ENTIDAD_DOC WHERE DD_TED_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(3))||'''),
