...
V_SQL := q'[UPDATE ]' || V_ESQUEMA || q'[.MJR_MAPEO_JUPITER_REM SET MJR_CODIGO_REM=REPLACE(MJR_CODIGO_REM, 'Liberbank', 'Unicaja') WHERE MJR_CODIGO_REM LIKE '%Liberbank%' AND MJR_TIPO_PERFIL like 'cartera' ]';
...
V_SQL := q'[UPDATE ]' || V_ESQUEMA || q'[.mjr_mapeo_jupiter_rem SET USUARIOBORRAR=]' || V_ITEM || q'[, BORRADO=1, FECHABORRAR=SYSDATE WHERE MJR_CODIGO_REM='CaixaBank / BFA' ]';
EXECUTE IMMEDIATE V_SQL;
DBMS_OUTPUT.PUT_LINE(' ... ' || RPAD(substr(V_SQL, 1, 60), 60, ' ') || ' ... registros afectados ...' || sql%rowcount);
