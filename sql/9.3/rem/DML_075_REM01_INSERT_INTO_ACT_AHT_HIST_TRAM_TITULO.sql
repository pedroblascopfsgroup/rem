...
BEGIN	
	...
	V_MSQL:= '	MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' T1
				...
				VALUES (S_'||V_TEXT_TABLA||'.NEXTVAL,
					...
					(select DD_esp_id from DD_ESP_ESTADO_PRESENTACION where dd_esp_codigo like ''01''),
					NULL,
					0, 
					'''||V_USU||''', 
					SYSDATE, 
	   ...
	   DBMS_OUTPUT.PUT_LINE('[INFO]: '||SQL%ROWCOUNT||' REGISTROS ACTUALIZADOS TRAMITACIÓN CASO 2 ');
	    	...
	    	V_MSQL:= '	MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' T1
				...
				VALUES (S_'||V_TEXT_TABLA||'.NEXTVAL,
					...
					T2.TIT_FECHA_PRESENT1_REG,
					T2.TIT_FECHA_INSC_REG, 
					(select DD_esp_id from DD_ESP_ESTADO_PRESENTACION where dd_esp_codigo like ''02''),
					NULL,
					0, 
					'''||V_USU||''', 
					SYSDATE, 
	   ...
	   -- No correcto pero se puede tratar usando solo la segunda fecha de presentación
	     ...
	     V_MSQL:= '	MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' T1
				...
				VALUES (S_'||V_TEXT_TABLA||'.NEXTVAL,
					...
					T2.TIT_FECHA_PRESENT1_REG,
					T2.TIT_FECHA_INSC_REG, 
					(select DD_esp_id from DD_ESP_ESTADO_PRESENTACION where dd_esp_codigo like ''03''),
					NULL,
					0, 
					'''||V_USU||''', 
					SYSDATE, 
