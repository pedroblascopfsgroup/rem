...
DECLARE
	...
	T_TIPO_DATA('DEC','Detalle económico','Detalle económico'),
	T_TIPO_DATA('FIC','Ficha','Ficha')
    ); 
...
BEGIN	
    ...
    -- LOOP para insertar los valores en DD_PES_PESTANAS -----------------------------------------------------------------
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE DD_PES_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''';
        ...
        IF V_NUM_TABLAS > 0 THEN				
          ...
          DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS EL REGISTRO '''|| TRIM(V_TMP_TIPO_DATA(1)) ||'''');
       	  V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'||V_TABLA||' '||
                    'SET DD_PES_DESCRIPCION = '''||TRIM(V_TMP_TIPO_DATA(2))||''''|| 
					', DD_PES_DESCRIPCION_LARGA = '''||TRIM(V_TMP_TIPO_DATA(3))||''''||
					', USUARIOMODIFICAR = ''DML'' , FECHAMODIFICAR = SYSDATE '||
					'WHERE DD_PES_CODIGO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''';
       ...
       ELSE
          ...
          V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.'||V_TABLA||' (' ||
                      'DD_PES_ID, DD_PES_CODIGO, DD_PES_DESCRIPCION, DD_PES_DESCRIPCION_LARGA, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
