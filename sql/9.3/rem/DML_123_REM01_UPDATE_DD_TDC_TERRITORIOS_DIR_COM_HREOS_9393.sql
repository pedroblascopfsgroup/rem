...
--## INCIDENCIA_LINK=HREOS-9393
--## PRODUCTO=NO
--##
--## Finalidad: Actualiza o inserta en DD_TDC_TERRITORIOS_DIR_COM
--## INSTRUCCIONES: 
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_TABLA VARCHAR2(40 CHAR) := 'DD_TDC_TERRITORIOS_DIR_COM'; -- Vble. Nombre de la tabla
    V_USUARIO VARCHAR(40 CHAR) := 'HREOS-9393'; -- Usuario (Artefacto)
    table_count number(3); -- Vble. para validar la existencia de las Tablas.
    err_num NUMBER; -- Numero de errores
    err_msg VARCHAR2(2048); -- Mensaje de error
  	
    V_ENTIDAD_ID NUMBER(16);
    --Valores a insertar en DD_TDC_TERRITORIOS_DIR_COM
    TYPE T_DATA IS TABLE OF VARCHAR2(150);
    TYPE T_ARRAY_DATA IS TABLE OF T_DATA;

    V_DATA T_ARRAY_DATA := T_ARRAY_DATA(
      --     DD_TDC_CODIGO  DD_TDC_DESCRIPCION  DD_TDC_DESCRIPCION_LARGA
      T_DATA('NNO',         'Noroeste',         'Noroeste')
    );
    V_TMP_DATA T_DATA;
...
BEGIN
    ...
    -- LOOP Insertando valores en DD_TDC_TERRITORIOS_DIR_COM
    FOR I IN V_DATA.FIRST .. V_DATA.LAST
      LOOP
      V_TMP_DATA := V_DATA(I);
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'|| TRIM(V_TABLA)||' WHERE DD_TDC_CODIGO = '''||TRIM(V_TMP_DATA(1))||'''';
        ...
        V_MSQL := 'UPDATE '|| V_ESQUEMA ||'.'|| TRIM(V_TABLA)||
						' SET DD_TDC_DESCRIPCION = '''|| TRIM(V_TMP_DATA(2))||''', DD_TDC_DESCRIPCION_LARGA = '''|| TRIM(V_TMP_DATA(3))||''', VERSION = VERSION + 1, USUARIOMODIFICAR = '''|| TRIM(V_USUARIO)||''''||
            ', FECHAMODIFICAR = SYSDATE, BORRADO = 0 WHERE DD_TDC_CODIGO = ''' || TRIM(V_TMP_DATA(1))||'''';
        ...
        EXECUTE IMMEDIATE V_MSQL INTO V_ENTIDAD_ID;
				...
				V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.'|| TRIM(V_TABLA)||' (' ||
						'DD_TDC_ID, DD_TDC_CODIGO, DD_TDC_DESCRIPCION, DD_TDC_DESCRIPCION_LARGA, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)' ||
