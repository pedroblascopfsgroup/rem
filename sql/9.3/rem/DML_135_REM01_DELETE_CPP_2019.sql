...
--## PRODUCTO=NO
--##
--## Finalidad: Script que borrar CPP de Apple, IBI
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Ver1ón inicial
--##########################################
...
BEGIN	
    ...
    V_SQL := 'SELECT COUNT(1) 
              ...
              WHERE CPP.BORRADO = 1 AND CPP.USUARIOBORRAR = ''HREOS-9747'' AND CPP_PARTIDA_PRESUPUESTARIA IN (''G011311'')
    ';
    ...
    ELSE
      ...
      V_MSQL := 'UPDATE '||V_ESQUEMA||'.CPP_CONFIG_PTDAS_PREP CPP SET 
                  ...
                  WHERE CPP.CPP_ID IN (
                    ...
                    AND EXISTS 
                    (
                      SELECT 1 FROM REM01.CPP_CONFIG_PTDAS_PREP CPP_TMP
                      LEFT JOIN REM01.DD_STG_SUBTIPOS_GASTO STG ON CPP_TMP.DD_STG_ID = STG.DD_STG_ID AND STG.BORRADO = 0
                      WHERE CPP_TMP.BORRADO = 0 AND STG.DD_STG_CODIGO IN (''01'',''02'') AND CPP_TMP.CPP_ID = CPP.CPP_ID AND CPP_PARTIDA_PRESUPUESTARIA IN (''G011311'')
                    )
                  )';
