...
V_TABLA VARCHAR2(40 CHAR) := 'MIG2_CAIXA_VOLUMETRIA_TABLAS';
...
BEGIN
        ...
        (
            MIG_NOMBRE_INTERFAZ                 VARCHAR2(50 CHAR),
            MIG_CANTIDAD_REGISTROS              NUMBER(16),
            FECHA                               DATE
	)';

	DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA_1||'.'||V_TABLA||' CREADA');
            EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA_1 || '.'||V_TABLA||'.MIG_NOMBRE_INTERFAZ IS ''Nombre de la interfaz''';
            EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA_1 || '.'||V_TABLA||'.MIG_CANTIDAD_REGISTROS IS ''Total de registros''';
            EXECUTE IMMEDIATE 'COMMENT ON COLUMN ' || V_ESQUEMA_1 || '.'||V_TABLA||'.FECHA IS ''Fecha''';
