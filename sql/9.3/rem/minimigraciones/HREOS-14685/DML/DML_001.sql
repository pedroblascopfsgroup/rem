...
BEGIN
	...
	V_SQL := 'DELETE FROM '||V_ESQUEMA||'.'||V_TABLA_TMP||' WHERE ID_SAP IS NULL';
	...
	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' PVE
				USING(
					...
					, PVE.PVE_COD_UVEM
					, PVE.PVE_DOCIDENTIF
					, ROW_NUMBER() OVER (PARTITION BY ID_UVEM, ID_PROVEEDOR_FISCAL, ID_PROVEEDOR_NIF, PVE.PVE_ID ORDER BY ID_SAP ASC) RN
					FROM (SELECT ID_UVEM, ID_PROVEEDOR_FISCAL, ID_PROVEEDOR_NIF, ID_SAP FROM '||V_ESQUEMA||'.'||V_TABLA_TMP||' GROUP BY ID_UVEM, ID_PROVEEDOR_FISCAL, ID_PROVEEDOR_NIF, ID_SAP) AUX
					JOIN '||V_ESQUEMA||'.'||V_TABLA||' PVE ON AUX.ID_UVEM = PVE.PVE_COD_UVEM AND PVE.BORRADO = 0
				) AUX
				ON (PVE.PVE_ID = AUX.PVE_ID AND AUX.RN = 1)
				WHEN MATCHED THEN
				UPDATE SET
					PVE.PVE_COD_UVEM = AUX.ID_SAP
					, PVE.PVE_DOCIDENTIF = AUX.ID_PROVEEDOR_NIF
