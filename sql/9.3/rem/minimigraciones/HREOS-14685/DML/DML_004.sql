...
BEGIN
	...
	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' PVE
				USING(
					...
					, PVE.PVE_DOCIDENTIF
					FROM (
						SELECT ID_NIF, ID_PROVEEDOR_FISCAL, ID_PROVEEDOR_NIF, ID_SAP 
						FROM '||V_ESQUEMA||'.'||V_TABLA_TMP||' GROUP BY ID_NIF, ID_PROVEEDOR_FISCAL, ID_PROVEEDOR_NIF, ID_SAP
					) AUX
					JOIN '||V_ESQUEMA||'.'||V_TABLA||' PVE ON AUX.ID_NIF = PVE.PVE_DOCIDENTIF 
						AND PVE.BORRADO = 0
				) AUX
