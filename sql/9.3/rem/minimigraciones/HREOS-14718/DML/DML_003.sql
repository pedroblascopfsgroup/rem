...
BEGIN
	...
	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
		USING (
			SELECT TMP.ID_UVEM ACT_NUM_ACTIVO_UVEM, REPLACE(TMP.ID_CAIXA,CHR(13),'''') ID_CAIXA
	...
	V_SQL := 'DELETE FROM '||V_ESQUEMA||'.'||V_TABLA||' T1 WHERE NOT EXISTS (SELECT 1 FROM '||V_ESQUEMA||'.'||V_TABLA_TMP||' TMP WHERE REPLACE(TMP.ID_CAIXA,CHR(13),'''') = T1.ACT_NUM_ACTIVO_CAIXA)';
	EXECUTE IMMEDIATE V_SQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] Se han fusionado un total de '||SQL%ROWCOUNT||' filas de la tabla '||V_TABLA);
