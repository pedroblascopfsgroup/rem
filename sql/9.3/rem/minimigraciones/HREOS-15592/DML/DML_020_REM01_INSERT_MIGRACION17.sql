	...
	WITH NUM_ACT_TOT
           AS( SELECT	
                ...
                COUNT(ACT.ACT_ID) AS TOT
                    ...
                    LEFT JOIN '||V_ESQUEMA||'.AUX_ACT_TRASPASO_ACTIVO AUX ON ACT.ACT_NUM_ACTIVO = AUX.ACT_NUM_ACTIVO_ANT AND aga.BORRADO = 0 group by AGA.AGR_ID
                    HAVING AGR_ID IN (SELECT
                                    AGA.AGR_ID
                                    FROM '||V_ESQUEMA||'.ACT_AGA_AGRUPACION_ACTIVO AGA
                                        JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON AGA.ACT_ID = ACT.ACT_ID
