...
BEGIN
	...
	V_SQL := 'INSERT INTO '||V_ESQUEMA||'.AUX_OFR_ID (
					...
					,'||V_ESQUEMA||'.S_OFR_NUM_OFERTA.NEXTVAL    OFR_NUM_OFERTA 
					FROM(			
					SELECT DISTINCT 
					OFR.OFR_ID
					...
					LEFT JOIN '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO ON ECO.OFR_ID = OFR.OFR_ID
						...
						AND ECO.DD_EEC_ID IN (SELECT DD_EEC_ID FROM '||V_ESQUEMA||'.DD_EEC_EST_EXP_COMERCIAL WHERE DD_EEC_CODIGO IN (''01'',''04'',''10'',''43'',''14'',''38'')))           
						OR OFR.DD_EOF_ID IN (SELECT DD_EOF_ID FROM '||V_ESQUEMA||'.DD_EOF_ESTADOS_OFERTA WHERE DD_EOF_CODIGO IN (''04''))))';
	...
	V_SQL := 'INSERT INTO '||V_ESQUEMA||'.OFR_OFERTAS  (
				...
				AUX_OFR_ID.OFR_NUM_OFERTA               OFR_NUM_OFERTA,
				NULL 									OFR_WEBCOM_ID,
