...
BEGIN
	...
	V_SQL := ('INSERT INTO '||V_ESQUEMA||'.'||V_TABLA_TIPO||' (DD_TTF_ID, DD_TTF_CODIGO, DD_TTF_DESCRIPCION, DD_TTF_DESCRIPCION_LARGA, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
        SELECT '||V_ESQUEMA||'.S_'||V_TABLA_TIPO||'.NEXTVAL, TACCT.CODIGO, TACCT.DESCRIPCION, TACCT.DESCRIPCION, 0, '''||V_USUARIO||''', SYSDATE, 0
        FROM '||V_ESQUEMA||'.'||V_TABLA_TMP||' TACCT
		WHERE  TACCT.CODIGO NOT IN (SELECT DD_TTF_CODIGO FROM '||V_ESQUEMA||'.'||V_TABLA_TIPO||')'
        );
