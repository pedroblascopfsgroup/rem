...
BEGIN
	...
	V_SQL:= ('INSERT INTO '||V_ESQUEMA||'.'||V_TABLA_TARIFA||' (
		...
		CFT_PRECIO_UNITARIO, 
		CFT_UNIDAD_MEDIDA, 
		VERSION, USUARIOCREAR, FECHACREAR, BORRADO,PVE_ID,DD_SCR_ID)        
		...
		(SELECT PVE.PVE_ID FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR PVE WHERE PVE.PVE_DOCIDENTIF = ''B23657521'' AND PVE.PVE_FECHA_ALTA IS NOT NULL AND PVE.PVE_FECHA_BAJA IS NULL),
	...
	V_SQL:= ('INSERT INTO '||V_ESQUEMA||'.'||V_TABLA_TARIFA||' (
		...
		CFT_PRECIO_UNITARIO, 
		CFT_UNIDAD_MEDIDA, 
		VERSION, USUARIOCREAR, FECHACREAR, BORRADO,PVE_ID)        
		SELECT '||V_ESQUEMA||'.S_'||V_TABLA_TARIFA||'.NEXTVAL,
		TARIF.DD_TTF_ID,		
		TMP.TIPO_TRABAJO,
		TMP.SUBTIPO_TRABAJO,
	...
	V_SQL:= ('INSERT INTO '||V_ESQUEMA||'.'||V_TABLA_TARIFA||' (
		...
		INNER JOIN '||V_ESQUEMA||'.DD_SCR_SUBCARTERA SUB ON SUB.DD_CRA_ID=CAR.DD_CRA_ID
		WHERE SUB.DD_SCR_CODIGO=''133''), 
		PRECIO_CIA_HOMESERVE/100,
		TMP.UNIDAD_MEDIDA,
		0, '''||V_USUARIO||''', SYSDATE,0,
		(SELECT PVE.PVE_ID FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR PVE WHERE PVE.PVE_DOCIDENTIF = ''A82451410'' AND PVE.PVE_FECHA_ALTA IS NOT NULL AND PVE.PVE_FECHA_BAJA IS NULL),
		(SELECT SUB.DD_SCR_ID FROM '||V_ESQUEMA||'.DD_CRA_CARTERA CAR
		INNER JOIN '||V_ESQUEMA||'.DD_SCR_SUBCARTERA SUB ON SUB.DD_CRA_ID=CAR.DD_CRA_ID
		WHERE SUB.DD_SCR_CODIGO=''133'')
        FROM '||V_ESQUEMA||'.'||V_TABLA_TMP||' TMP
