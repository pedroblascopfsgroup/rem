...
BEGIN
	...
	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA_TMP||' TMP
		...
		(   
			select 
			LLV_NUM_LLAVE
			, CASE WHEN is_number(LLV_COD_TENEDOR) = 1 THEN (SELECT PVE_ID FROM ACT_PVE_PROVEEDOR WHERE PVE_COD_REM = LLV_COD_TENEDOR)
			END LLV_COD_TENEDOR	
			, CASE WHEN is_number(LLV_COD_TENEDOR) = 0 THEN LLV_COD_TENEDOR
