...
BEGIN
	...
	V_SQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA_TMP||' TMP
		...
		(   
			...
			ID
			, LLV_NUM_LLAVE
			, CASE WHEN DD_TTE_CODIGO_POSEEDOR not like ''08'' THEN (SELECT PVE_ID FROM ACT_PVE_PROVEEDOR WHERE PVE_COD_REM = MLV_COD_TENEDOR_POSEEDOR)
			...
			, CASE WHEN DD_TTE_CODIGO_PEDIDOR not like ''08'' THEN (SELECT PVE_ID FROM ACT_PVE_PROVEEDOR WHERE PVE_COD_REM = MLV_COD_TENEDOR_PEDIDOR)
