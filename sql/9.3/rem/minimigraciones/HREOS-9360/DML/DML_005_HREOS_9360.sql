...
BEGIN
	...
	V_SQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||' 
			...
			, EST.DD_TTE_ID
			, TO_TIMESTAMP(AUX.MLV_FECHA_ENVIO,''DD/MM/RRRR HH24:MI:SS.FF'') MLV_FECHA_ENVIO
			, TO_TIMESTAMP(AUX.MLV_FECHA_RECEPCION,''DD/MM/RRRR HH24:MI:SS.FF'') MLV_FECHA_RECEPCION
			from REM01.AUX_INSERTA_MOVIMIENTOS_LLAVES_ACTIVO aux
			JOIN REM01.ACT_LLV_LLAVE LLV ON aux.LLV_NUM_LLAVE = LLV.LLV_NUM_LLAVE AND LLV.BORRADO = 0
			LEFT JOIN REM01.DD_TTE_TIPO_TENEDOR TTEP ON TTEP.DD_TTE_CODIGO = AUX.DD_TTE_CODIGO_POSEEDOR AND TTEP.BORRADO = 0
			LEFT JOIN REM01.DD_TTE_TIPO_TENEDOR TTED ON TTED.DD_TTE_CODIGO = AUX.DD_TTE_CODIGO_PEDIDOR AND TTED.BORRADO = 0
			LEFT JOIN REM01.ACT_PVE_PROVEEDOR PVEP ON PVEP.PVE_COD_REM = AUX.MLV_COD_TENEDOR_POSEEDOR AND PVEP.BORRADO = 0
			LEFT JOIN REM01.ACT_PVE_PROVEEDOR PVED ON PVED.PVE_COD_REM = AUX.MLV_COD_TENEDOR_PEDIDOR AND PVED.BORRADO = 0
			LEFT JOIN REM01.DD_TTE_TIPO_ESTADO EST ON EST.DD_TTE_CODIGO = AUX.MLV_ESTADO AND EST.BORRADO = 0';				
