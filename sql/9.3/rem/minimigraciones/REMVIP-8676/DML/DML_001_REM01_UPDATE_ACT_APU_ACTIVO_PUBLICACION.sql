...
DECLARE
    ...
    V_USUARIOMODIFICAR VARCHAR(100 CHAR):= 'REMVIP-8676';
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] Actualiza ACT_APU_ACTIVO_PUBLICACION - Fecha revisión publicación venta');
	 ...
	 V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION T1
		        USING (
					...
					JOIN '||V_ESQUEMA||'.AUX_REMVIP_8676 AUX ON AUX.ACT_NUM_ACTIVO = ACT.ACT_NUM_ACTIVO
					WHERE ACT.BORRADO = 0	
        		) T2 
        	  ON (T1.ACT_ID = T2.ACT_ID )
				WHEN MATCHED THEN UPDATE SET 
					T1.APU_FECHA_REVISION_PUB_VENTA = TO_DATE(T2.FECHA_REVISION_PUBLICACION, ''DD/MM/YYYY''),	  
