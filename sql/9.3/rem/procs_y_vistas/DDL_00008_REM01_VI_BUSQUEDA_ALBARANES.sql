  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_ALBARANES 
	...
	AS
		SELECT
		  ...
		  (SELECT COUNT(PFA_ID) FROM PFA_PREFACTURA PFAP INNER JOIN ALB_ALBARAN ALBA ON ALBA.ALB_ID = PFAP.ALB_ID  WHERE ALBA.ALB_ID = ALB.ALB_ID) NUMPREFACTURA,
		...
		JOIN ' || V_ESQUEMA || '.DD_ESA_ESTADO_ALBARAN ESTALB ON ALB.DD_ESA_ID = ESTALB.DD_ESA_ID
		JOIN ' || V_ESQUEMA || '.PFA_PREFACTURA PFA ON ALB.ALB_ID = PFA.ALB_ID
		JOIN ' || V_ESQUEMA || '.DD_EPF_ESTADO_PREFACTURA EPF ON EPF.DD_EPF_ID = PFA.DD_EPF_ID
