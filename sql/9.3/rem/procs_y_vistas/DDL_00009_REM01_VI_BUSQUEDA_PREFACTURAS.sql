  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_PREFACTURAS 
	...
	AS
		SELECT DISTINCT PFA_ID, ALB_ID, PFA_NUM_PREFACTURA, DD_EPF_DESCRIPCION, PVE_NOMBRE, PRO_NOMBRE, ANYO, NVL(IMPORTE_TOTAL,0.0) IMPORTE_TOTAL
		   ...
		   , LISTAGG (GPV_NUM_GASTO_HAYA, '', '') WITHIN GROUP (ORDER BY GPV_NUM_GASTO_HAYA) OVER (PARTITION BY PFA_NUM_PREFACTURA) NUM_GASTOS
		FROM (
		   SELECT
		       PFA.PFA_ID,
			   PFA.ALB_ID,
		       ...
		       COUNT(TBJ.TBJ_ID) AS NUMTRABAJOS,
		   ...
		   JOIN ' || V_ESQUEMA ||'.ACT_TBJ_TRABAJO TBJ ON PFA.PFA_ID = TBJ.PFA_ID
		   LEFT JOIN ' || V_ESQUEMA ||'.GPV_GASTOS_PROVEEDOR GPV ON GPV.PFA_ID = PFA.PFA_ID
		   GROUP BY PFA.PFA_ID, PFA.ALB_ID, PFA.PFA_NUM_PREFACTURA, PVE.PVE_NOMBRE, PRO.PRO_NOMBRE, EPF.DD_EPF_DESCRIPCION, TO_CHAR(TBJ.TBJ_FECHA_VALIDACION,''YYYY''), GPV.GPV_NUM_GASTO_HAYA
		   )
		';
