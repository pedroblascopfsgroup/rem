...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_PREFACTURAS 
	...
	AS
		SELECT DISTINCT PFA_ID, ALB_ID, DD_IRE_DESCRIPCION, DD_IRE_CODIGO, PFA_NUM_PREFACTURA, DD_EPF_DESCRIPCION, PVE_NOMBRE, PRO_NOMBRE, ANYO, NVL(IMPORTE_TOTAL,0.0) IMPORTE_TOTAL
		...
		FROM (
		   SELECT
		       ...
		       (SELECT COUNT(PTGR.TBJ_ID) FROM REM01.PTG_PREFACTURAS PTGR WHERE PTGR.PFA_ID = PFA.PFA_ID) AS NUMTRABAJOS,
		   ...
		   JOIN ' || V_ESQUEMA ||'.DD_EPF_ESTADO_PREFACTURA EPF ON PFA.DD_EPF_ID = EPF.DD_EPF_ID
		   	AND EPF.BORRADO = 0
		   LEFT JOIN ' || V_ESQUEMA ||'.ACT_TBJ_TRABAJO TBJ ON PTG.TBJ_ID = TBJ.TBJ_ID
		   	AND TBJ.BORRADO = 0
		   ...
		   LEFT JOIN '||V_ESQUEMA||'.DD_EGA_ESTADOS_GASTO EGA ON EGA.DD_EGA_ID = GPV.DD_EGA_ID
		   	AND EGA.BORRADO = 0
		   WHERE PFA.BORRADO = 0
		   GROUP BY PFA.PFA_ID, PFA.ALB_ID, PFA.PFA_NUM_PREFACTURA, PVE.PVE_NOMBRE, PRO.PRO_NOMBRE, EPF.DD_EPF_DESCRIPCION, TO_CHAR(TBJ.TBJ_FECHA_VALIDACION,''YYYY''), GPV.GPV_NUM_GASTO_HAYA, PFA.PFA_FECHA_PREFACTURA, EGA.DD_EGA_DESCRIPCION, IRE.DD_IRE_DESCRIPCION, IRE.DD_IRE_CODIGO
		   ) ORDER BY 1
		';
