    ...
    WITH CARTERA_LIBERBANK AS (
        ...
        SELECT GPV.GPV_ID, GLD.GLD_ID
            , CASE WHEN NVL(GLD.GLD_IMP_IND_EXENTO, 0) = 1 AND NVL(GLD.GLD_IMP_IND_RENUNCIA_EXENCION, 0) = 0
                    THEN 0
                    ELSE GLD.GLD_IMP_IND_TIPO_IMPOSITIVO
        ...
        WHERE TEF.TIPO_IMPOSITIVO_EFECTIVO IS NOT NULL
        ...
        FROM '||V_ESQUEMA||'.GLD_GASTOS_LINEA_DETALLE GLD
        JOIN TIPOS_EFECTIVOS TEF ON TEF.GLD_ID = GLD.GLD_ID
        WHERE TEF.TIPO_IMPOSITIVO_EFECTIVO IS NULL
        GROUP BY GLD.GPV_ID
    )
    SELECT TED.GPV_ID
        , CASE WHEN SUJ.SUJETO > 0 THEN TED.TIPO_DIARIO WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN ''60'' ELSE NULL END DIARIO1
        , CASE WHEN SUJ.SUJETO > 0 THEN SUJ.SUJETO WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN NSU.NO_SUJETO + SUE.SUJETO_EXENTO ELSE NULL END DIARIO1_BASE
        , CASE WHEN SUJ.SUJETO > 0 THEN SUJ.TIPO_IMPOSITIVO_EFECTIVO WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN 0 ELSE NULL END DIARIO1_TIPO
        , CASE WHEN SUJ.SUJETO > 0 THEN SUJ.CUOTA WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN 0 ELSE NULL END DIARIO1_CUOTA
        , CASE WHEN SUJ.SUJETO = 0 THEN NULL WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN ''60'' ELSE NULL END DIARIO2
        , CASE WHEN SUJ.SUJETO = 0 THEN NULL WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN NSU.NO_SUJETO + SUE.SUJETO_EXENTO ELSE NULL END DIARIO2_BASE
        , CASE WHEN SUJ.SUJETO = 0 THEN NULL WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN 0 ELSE NULL END DIARIO2_TIPO
        , CASE WHEN SUJ.SUJETO = 0 THEN NULL WHEN (NSU.NO_SUJETO + SUE.SUJETO_EXENTO) > 0 THEN 0 ELSE NULL END DIARIO2_CUOTA
    FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
    JOIN IMPORTES_SUJETOS SUJ ON SUJ.GPV_ID = GPV.GPV_ID
    JOIN IMPORTES_NO_SUJETOS NSU ON NSU.GPV_ID = GPV.GPV_ID
    JOIN IMPORTES_SUJETOS_EXENTOS SUE ON SUE.GPV_ID = GPV.GPV_ID
