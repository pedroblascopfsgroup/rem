    ...
    SELECT IAG.GPV_ID, IAG.ENT_ID, IAG.DD_ENT_ID
        , CASE 
            WHEN DCL.DIARIO1 = ''20'' THEN 
                ROUND((IAG.IMPORTE_ACTIVO_GASTO / IMG.IMPORTE_GASTO) * ((NVL(P20.P20_GASTO, 100) / 100 * DCL.DIARIO1_TIPO / 100 + 1) * (BRU.IMPORTE_BRUTO) + NVL(DCL.DIARIO2_BASE, 0)), 2)
            WHEN DCL.DIARIO1 = ''1'' THEN
                ROUND((IAG.IMPORTE_ACTIVO_GASTO / IMG.IMPORTE_GASTO) * ((BRU.IMPORTE_BRUTO) + NVL(DCL.DIARIO2_BASE, 0)), 2)
            WHEN DCL.DIARIO1 = ''2'' THEN
                ROUND((IAG.IMPORTE_ACTIVO_GASTO / IMG.IMPORTE_GASTO) * ((DCL.DIARIO1_TIPO / 100 + 1) * (BRU.IMPORTE_BRUTO) + NVL(DCL.DIARIO2_BASE, 0)), 2)
            WHEN DCL.DIARIO1 = ''60'' THEN
                ROUND((IAG.IMPORTE_ACTIVO_GASTO / IMG.IMPORTE_GASTO) * (BRU.IMPORTE_BRUTO), 2)
            END IMPORTE_ACTIVO
    FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
