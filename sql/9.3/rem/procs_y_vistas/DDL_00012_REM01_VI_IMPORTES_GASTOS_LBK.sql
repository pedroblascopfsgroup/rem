...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_IMPORTES_GASTOS_LBK 
    WITH IMPORTE_ACTIVO_GASTO AS (
        SELECT GPV.GPV_ID, GEN.ENT_ID, ENT.DD_ENT_DESCRIPCION
            ,  SUM(GEN.GLD_PARTICIPACION_GASTO / 100 * GLD.GLD_IMPORTE_TOTAL) 
        ...
        WHERE GPV.BORRADO = 0
        GROUP BY GPV.GPV_ID, GEN.ENT_ID, ENT.DD_ENT_DESCRIPCION
    )
    , IMPORTE_GASTO AS (
        SELECT GPV.GPV_ID, SUM(GLD.GLD_IMPORTE_TOTAL) IMPORTE_GASTO
        FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
        JOIN '||V_ESQUEMA||'.GLD_GASTOS_LINEA_DETALLE GLD ON GLD.GPV_ID = GPV.GPV_ID
            AND GLD.BORRADO = 0
        WHERE GPV.BORRADO = 0
    ...
    SELECT IAG.GPV_ID, IAG.ENT_ID ID_ELEMENTO, IAG.DD_ENT_DESCRIPCION TIPO_ELEMENTO
        /*, IAG.IMPORTE_ACTIVO_GASTO / IMG.IMPORTE_GASTO PARTICIPACION_ACTIVO_GASTO
        , DCL.DIARIO1, DCL.DIARIO1_BASE, DCL.DIARIO1_TIPO
        , DCL.DIARIO2, DCL.DIARIO2_BASE
        , P20.P20_GASTO*/
        , CASE 
            WHEN DCL.DIARIO1 = ''20'' THEN 
                ROUND((IAG.IMPORTE_ACTIVO_GASTO / IMG.IMPORTE_GASTO) * ((P20.P20_GASTO / 100 * DCL.DIARIO1_TIPO / 100 + 1) * (DCL.DIARIO1_BASE) + DCL.DIARIO2_BASE), 2)
            ELSE 
                ROUND((IAG.IMPORTE_ACTIVO_GASTO / IMG.IMPORTE_GASTO) * ((DCL.DIARIO1_TIPO / 100 + 1) * (DCL.DIARIO1_BASE) + DCL.DIARIO2_BASE), 2)
            END IMPORTE_GASTO
    FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
