...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || V_ESQUEMA || '.V_REPARTO_IMP_TBJ AS
	...
	SELECT PP.GPV_ID, PP.GLD_ID, PP.GLD_ENT_ID
	    , CASE WHEN PP.RN_PVE = 1 THEN PP.AJUSTE_PVE ELSE PP.PARTICIPACION_PVE END PARTICIPACION_PVE
	    , CASE WHEN PP.RN_CLI = 1 THEN PP.AJUSTE_CLI ELSE PP.PARTICIPACION_CLI END PARTICIPACION_CLI
	FROM PRINCIPAL PP';
