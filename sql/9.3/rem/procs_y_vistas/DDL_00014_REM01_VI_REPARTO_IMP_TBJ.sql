  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || V_ESQUEMA || '.V_REPARTO_IMP_TBJ AS
	...
	, PRINCIPAL AS (
	    SELECT PTL.GPV_ID, PTL.GLD_ID, GEN.GLD_ENT_ID
	        , SUM(CASE WHEN ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_PVE, 4) < 0.01 THEN 0.01 ELSE ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_PVE, 4) END) OVER(PARTITION BY GEN.ENT_ID, GEN.DD_ENT_ID) PARTICIPACION_PVE
	        , SUM(CASE WHEN ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_CLI, 4) < 0.01 THEN 0.01 ELSE ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_CLI, 4) END) OVER(PARTITION BY GEN.ENT_ID, GEN.DD_ENT_ID) PARTICIPACION_CLI
	        , 100 - SUM(CASE WHEN ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_PVE, 4) < 0.01 THEN 0.01 ELSE ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_PVE, 4) END) OVER(PARTITION BY PTL.GLD_ID) AJUSTE_PVE
	        , 100 - SUM(CASE WHEN ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_CLI, 4) < 0.01 THEN 0.01 ELSE ROUND(ATB.ACT_TBJ_PARTICIPACION * PTL.PART_TBJ_LIN_CLI, 4) END) OVER(PARTITION BY PTL.GLD_ID) AJUSTE_CLI
	        , ROW_NUMBER() OVER(PARTITION BY PTL.GLD_ID ORDER BY PTL.PART_TBJ_LIN_PVE DESC) RN_PVE
	...
	SELECT DISTINCT PP.GPV_ID, PP.GLD_ID, PP.GLD_ENT_ID
	    , CASE WHEN PP.RN_PVE = 1 THEN PP.PARTICIPACION_PVE - PP.AJUSTE_PVE ELSE PP.PARTICIPACION_PVE END PARTICIPACION_PVE
	    , CASE WHEN PP.RN_CLI = 1 THEN PP.PARTICIPACION_CLI - PP.AJUSTE_CLI ELSE PP.PARTICIPACION_CLI END PARTICIPACION_CLI
	FROM PRINCIPAL PP';
