...
--##        0.2 HREOS-17771 - Javier Esbri - Añadir nuevo aprovisionamiento campo activoEpa de la tabla ACT_BBVA_UIC
--##        0.3 REMVIP-12447 - Juan Bautista Alfonso - modificar aprovisionamiento campo activoEpa de tabla ACT_BBVA_UIC
--##########################################
--*/
...
-- Modificación de la vista V_LISTADO_ACT_EXP_BBVA_GRID de Fase1.

  SELECT COUNT(*) INTO CUENTA FROM ALL_OBJECTS WHERE OBJECT_NAME = 'V_LISTADO_ACT_EXP_BBVA_GRID' AND OWNER=V_ESQUEMA AND OBJECT_TYPE='MATERIALIZED VIEW';
  IF CUENTA>0 THEN
    DBMS_OUTPUT.PUT_LINE('DROP MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_LISTADO_ACT_EXP_BBVA_GRID...');
    EXECUTE IMMEDIATE 'DROP MATERIALIZED VIEW ' || V_ESQUEMA || '.V_LISTADO_ACT_EXP_BBVA_GRID';
    DBMS_OUTPUT.PUT_LINE('DROP MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_LISTADO_ACT_EXP_BBVA_GRID... borrada OK');
  END IF;

  SELECT COUNT(*) INTO CUENTA FROM ALL_OBJECTS WHERE OBJECT_NAME = 'V_LISTADO_ACT_EXP_BBVA_GRID' AND OWNER=V_ESQUEMA AND OBJECT_TYPE='VIEW';
  IF CUENTA>0 THEN
    DBMS_OUTPUT.PUT_LINE('DROP VIEW '|| V_ESQUEMA ||'.V_LISTADO_ACT_EXP_BBVA_GRID...');
    EXECUTE IMMEDIATE 'DROP VIEW ' || V_ESQUEMA || '.V_LISTADO_ACT_EXP_BBVA_GRID';
    DBMS_OUTPUT.PUT_LINE('DROP VIEW '|| V_ESQUEMA ||'.V_LISTADO_ACT_EXP_BBVA_GRID... borrada OK');
  END IF;

  DBMS_OUTPUT.PUT_LINE('CREATE VIEW '|| V_ESQUEMA ||'.V_LISTADO_ACT_EXP_BBVA_GRID...');
  EXECUTE IMMEDIATE 'CREATE VIEW V_LISTADO_ACT_EXP_BBVA_GRID AS 
       ...
       MINI AS (
        ...
        IUC AS (
            SELECT ACTB.ACT_ID,SIN_BBVA.DD_SIN_DESCRIPCION, 
            ROW_NUMBER() OVER(PARTITION BY ACTB.ACT_ID ORDER BY SIN_BBVA.DD_SIN_CODIGO ASC NULLS LAST) RN,
        	SIN_BBVA.DD_SIN_CODIGO 
        	FROM '|| V_ESQUEMA ||'.ACT_BBVA_ACTIVOS ACTB
        	LEFT JOIN '|| V_ESQUEMA ||'.ACT_BBVA_UIC ACTBUIC ON ACTB.ACT_ID = ACTBUIC.ACT_ID AND ACTBUIC.BORRADO = 0
		    LEFT JOIN '|| V_ESQUEMA_M ||'.DD_SIN_SINO SIN_BBVA ON ACTBUIC.BBVA_ACTIVO_EPA = SIN_BBVA.DD_SIN_ID AND SIN_BBVA.BORRADO = 0
		    WHERE ACTB.BORRADO = 0
        )
        SELECT 
          ID_VISTA
          , ECO_ID
        ...
        , IUC.DD_SIN_DESCRIPCION BBVA_ACTIVO_EPA
        , IUC.RN
        FROM '|| V_ESQUEMA ||'.ECO_EXPEDIENTE_COMERCIAL ECO
        JOIN '|| V_ESQUEMA ||'.ACT_OFR OFR ON ECO.OFR_ID = OFR.OFR_ID
        ...
        LEFT JOIN '|| V_ESQUEMA ||'.DD_TPA_TIPO_ACTIVO TPA ON SAC.DD_TPA_ID = TPA.DD_TPA_ID
        ...
        LEFT JOIN '|| V_ESQUEMA_M ||'.DD_LOC_LOCALIDAD LOC ON BIE.DD_LOC_ID = LOC.DD_LOC_ID
		    LEFT JOIN IUC IUC ON IUC.ACT_ID=ACT.ACt_ID AND IUC.RN = 1
        )
  ';

  DBMS_OUTPUT.PUT_LINE('CREATE VIEW '|| V_ESQUEMA ||'.V_LISTADO_ACT_EXP_BBVA_GRID...Creada OK');
