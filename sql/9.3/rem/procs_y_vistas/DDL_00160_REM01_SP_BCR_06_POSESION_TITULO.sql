   ...
   DBMS_OUTPUT.PUT_LINE('[INFO] 1 MERGE A LA TABLA ACT_TIT_TITULO.');
	...
	USING (				
           SELECT 
            ...
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM eqv1 ON eqv1.DD_NOMBRE_CAIXA = ''DD_ETI_CODIGO''  AND eqv1.DD_CODIGO_CAIXA = aux.ESTADO_TITULARIDAD
   ...
   V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_SPS_SIT_POSESORIA ACT
      USING (				
            SELECT 
               ...
               FROM '|| V_ESQUEMA ||'.AUX_APR_BCR_STOCK AUX
               JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT2 ON ACT2.ACT_NUM_ACTIVO_CAIXA=AUX.NUM_IDENTIFICATIVO  AND ACT2.BORRADO=0
               LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM eqv1 ON eqv1.DD_NOMBRE_CAIXA = ''DD_TPO_CODIGO''  AND eqv1.DD_CODIGO_CAIXA = aux.ESTADO_POSESORIO
...
V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_PAC_PERIMETRO_ACTIVO ACT
	USING (				
           SELECT 
            ...
            FROM '|| V_ESQUEMA ||'.AUX_APR_BCR_STOCK AUX
            JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT2 ON ACT2.ACT_NUM_ACTIVO_CAIXA=AUX.NUM_IDENTIFICATIVO AND ACT2.BORRADO=0
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM eqv1 ON eqv1.DD_NOMBRE_CAIXA = ''DD_MEC_CODIGO''  AND eqv1.DD_CODIGO_CAIXA = aux.MOTIVO_NO_COMERCIAL
