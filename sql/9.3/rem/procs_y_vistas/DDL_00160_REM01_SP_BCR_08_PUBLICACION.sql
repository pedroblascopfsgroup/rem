    ...
    V_MSQL :=  'MERGE INTO '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC
                USING (
                        ...
                        WHEN MATCHED THEN UPDATE SET
                            ...
                            ,PAC.PAC_CHECK_COMERCIALIZAR = US.PAC_CHECK_PUBLICAR
                            ,PAC.PAC_FECHA_COMERCIALIZAR = CASE WHEN PAC.PAC_CHECK_COMERCIALIZAR <> US.PAC_CHECK_PUBLICAR THEN SYSDATE END
                            ,PAC.DD_MCO_ID = CASE WHEN PAC.PAC_CHECK_COMERCIALIZAR <> US.PAC_CHECK_PUBLICAR THEN NULL END
                            ,PAC.PAC_MOT_EXCL_COMERCIALIZAR = CASE WHEN PAC.PAC_CHECK_COMERCIALIZAR <> US.PAC_CHECK_PUBLICAR THEN NULL END
                            ,PAC.PAC_CHECK_FORMALIZAR = US.PAC_CHECK_PUBLICAR
                            ,PAC.PAC_FECHA_FORMALIZAR = CASE WHEN PAC.PAC_CHECK_COMERCIALIZAR <> US.PAC_CHECK_PUBLICAR THEN SYSDATE END
                            ,PAC.PAC_MOTIVO_FORMALIZAR = CASE WHEN PAC.PAC_CHECK_COMERCIALIZAR <> US.PAC_CHECK_PUBLICAR THEN NULL END
                            ,PAC.USUARIOMODIFICAR = ''STOCK_BC''
                            ,PAC.FECHAMODIFICAR = SYSDATE
                            WHERE PAC.BORRADO=0
                            AND (NVL(PAC.PAC_CHECK_COMERCIALIZAR,0)!=NVL(US.PAC_CHECK_PUBLICAR,0)
                            OR NVL(PAC.PAC_CHECK_PUBLICAR,0)!=NVL(US.PAC_CHECK_PUBLICAR,0)
                            OR NVL(PAC.PAC_CHECK_FORMALIZAR,0)!=NVL(US.PAC_CHECK_PUBLICAR,0))
...
--12º Revisamos el campo Visible gestión comercial tras recalcular el estado de publicación de los activos, si es Publicado tiene que estar a Si
	...
	USING (
            ...
            WHERE (TCO.DD_TCO_CODIGO = ''01'' AND EPV.DD_EPV_CODIGO = ''03'' AND CAI.CBX_PUBL_PORT_API_VENTA = 1
            OR TCO.DD_TCO_CODIGO = ''02'' AND (EPV.DD_EPV_CODIGO = ''03'' AND CAI.CBX_PUBL_PORT_API_VENTA = 1 OR EPA.DD_EPA_CODIGO = ''03'' AND CAI.CBX_PUBL_PORT_API_ALQUILER = 1)
            OR TCO.DD_TCO_CODIGO = ''03'' AND EPA.DD_EPA_CODIGO = ''03'') AND CAI.CBX_PUBL_PORT_API_ALQUILER = 1
            ...
            WHEN MATCHED THEN UPDATE SET
                ...
                ,PAC.PAC_MOTIVO_GESTION_COMERCIAL = NULL
                ,PAC.USUARIOMODIFICAR = ''STOCK_BC''
                ,PAC.FECHAMODIFICAR = SYSDATE
   ';
   EXECUTE IMMEDIATE V_MSQL;
