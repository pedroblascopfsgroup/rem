...
BEGIN
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGRUPACIONES_REJECTS
                  ...
                  WHERE (AGR.AGR_FIN_VIGENCIA IS NULL OR TRUNC(AGR.AGR_FIN_VIGENCIA) >= TRUNC(SYSDATE))
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGR_SACAR_ACT AUX
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_ID = AGA.AGR_ID AND AGR.BORRADO = 0 AND (AGR.AGR_FIN_VIGENCIA IS NULL OR TRUNC(AGR.AGR_FIN_VIGENCIA) >= TRUNC(SYSDATE)) AND AGR.AGR_UVEM_COAGIW IS NOT NULL
     ...
     V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGR_SACAR_ACT AUX
      ...
      V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_UVEM_COAGIW = AUX.COD_AGRUPACION AND AGR.BORRADO = 0 AND (AGR.AGR_FIN_VIGENCIA IS NULL OR TRUNC(AGR.AGR_FIN_VIGENCIA) >= TRUNC(SYSDATE))
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT ON AUX.ACTIVO = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_UVEM_COAGIW = AUX.COD_AGRUPACION AND (AGR.AGR_FIN_VIGENCIA IS NULL OR TRUNC(AGR.AGR_FIN_VIGENCIA) >= TRUNC(SYSDATE)) AND AGR.DD_TAG_ID = (SELECT DD_TAG_ID FROM '|| V_ESQUEMA ||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = AUX.TIPO) AND AGR.BORRADO = 0 
                  WHERE (AGR.AGR_FIN_VIGENCIA IS NULL OR TRUNC(AGR.AGR_FIN_VIGENCIA) >= TRUNC(SYSDATE))
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_RES_RESTRINGIDA
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.ACTIVO AND ACT.BORRADO = 0
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_ACT_PRINCIPAL = ACT.ACT_ID AND AGR.AGR_UVEM_COAGIW = AUX.COD_AGRUPACION AND (AGR.AGR_FIN_VIGENCIA IS NULL OR TRUNC(AGR.AGR_FIN_VIGENCIA) >= TRUNC(SYSDATE)) AND AGR.DD_TAG_ID = (SELECT DD_TAG_ID FROM '|| V_ESQUEMA ||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = AUX.TIPO) AND AGR.BORRADO = 0
                  LEFT JOIN '|| V_ESQUEMA ||'.BIE_LOCALIZACION BIE_LOC ON ACT.BIE_ID = BIE_LOC.BIE_ID AND BIE_LOC.BORRADO = 0';
