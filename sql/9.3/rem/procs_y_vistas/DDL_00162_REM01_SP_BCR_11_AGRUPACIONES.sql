...
BEGIN
      V_MSQL := 'SELECT TO_CHAR(SYSDATE, ''DD-MM-YYYY HH24:MI:SS'') FROM DUAL';
      EXECUTE IMMEDIATE V_MSQL INTO HORA;
      SALIDA := '[INICIO] ' || HORA ||CHR(10);
   ...
   */
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGR_SACAR_ACT AUX
                  SELECT 
                  AGR.AGR_NUM_AGRUP_BC COD_AGRUPACION
                  , ''02'' TIPO
                  , APR.NUM_IDENTIFICATIVO ACTIVO
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_ID = AGA.AGR_ID AND AGR.BORRADO = 0 AND AGR.AGR_FECHA_BAJA IS NULL AND AGR.AGR_NUM_AGRUP_BC IS NOT NULL
                  JOIN '|| V_ESQUEMA ||'.DD_TAG_TIPO_AGRUPACION TAG ON AGR.DD_TAG_ID = TAG.DD_TAG_ID AND TAG.BORRADO = 0 AND TAG.DD_TAG_CODIGO = ''02''
                  WHERE NVL(APR.PROMO_CONJUNTA_VENTA,-1) <> AGR.AGR_NUM_AGRUP_BC
                  ...
                  WHERE OFR.BORRADO = 0 AND AUX_ACT.ACT_NUM_ACTIVO_CAIXA = AGR.AGR_NUM_AGRUP_BC)';
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGR_SACAR_ACT AUX
                  ...
                  WHERE NVL(APR.PROMO_CONJUNTA_ALQUILER,-1) <> AGR.AGR_NUM_AGRUP_BC
                  AND NOT EXISTS (SELECT 1
                  FROM '|| V_ESQUEMA ||'.OFR_OFERTAS OFR
                  JOIN '|| V_ESQUEMA ||'.ACT_OFR ACO ON OFR.OFR_ID = ACO.OFR_ID
                  JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO AUX_ACT ON AUX_ACT.ACT_ID = ACO.ACT_ID AND AUX_ACT.BORRADO = 0
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGR_SACAR_ACT AUX
                  ...
                  WHERE NVL(APR.PROMO_CONJUNTA_OB_REM,-1) <> AGR.AGR_NUM_AGRUP_BC
                  AND NOT EXISTS (SELECT 1
                  FROM '|| V_ESQUEMA ||'.OFR_OFERTAS OFR
                  JOIN '|| V_ESQUEMA ||'.ACT_OFR ACO ON OFR.OFR_ID = ACO.OFR_ID
                  JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO AUX_ACT ON AUX_ACT.ACT_ID = ACO.ACT_ID AND AUX_ACT.BORRADO = 0
     ...
     V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGR_SACAR_ACT AUX
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGRUPACIONES_ANYADIR AUX
                  ...
                  AND AGR.AGR_NUM_AGRUP_BC = APR.PROMO_CONJUNTA_VENTA)
                  AND NOT EXISTS (SELECT 1 
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGRUPACIONES_ANYADIR AUX
                  ...
                  WHERE AGR.BORRADO = 0 
                  AND AGR.AGR_FECHA_BAJA IS NULL
                  AND AGR.AGR_NUM_AGRUP_BC = APR.PROMO_CONJUNTA_ALQUILER)
                  AND NOT EXISTS (SELECT 1 
                  FROM '|| V_ESQUEMA ||'.AUX_BC_AGRUPACIONES_REJECTS REJ
   ...
   /*
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.AUX_BC_AGRUPACIONES_ANYADIR AUX
                  ...
                  WHERE AGR.BORRADO = 0 
                  AND AGR.AGR_FECHA_BAJA IS NULL
                  AND AGR.AGR_NUM_AGRUP_BC = APR.PROMO_CONJUNTA_OB_REM)
                  AND NOT EXISTS (SELECT 1 
                  FROM '|| V_ESQUEMA ||'.AUX_BC_AGRUPACIONES_REJECTS REJ
   ...
   */
      ...
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_BC_AGR_SACAR_ACT');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_BC_AGRUPACIONES_NUEVAS');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_BC_AGRUPACIONES');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_BC_AGRUPACIONES_REJECTS');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_BC_AGRUPACIONES_ANYADIR');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','AUX_BC_AGR_OBREM_PARCIALES');

      SALIDA := SALIDA || '   [INFO] 5 - PASO A PRODUCCIÓN'||CHR(10);
      ...
      V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_NUM_AGRUP_BC = AUX.COD_AGRUPACION AND AGR.BORRADO = 0 AND AGR.AGR_FECHA_BAJA IS NULL
      ...
      SALIDA := SALIDA || '   [INFO]      SACADOS DE AGRUPACIONES '|| SQL%ROWCOUNT|| CHR(10);
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT ON AUX.ACTIVO = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_NUM_AGRUP_BC = AUX.COD_AGRUPACION AND AGR.AGR_FECHA_BAJA IS NULL AND AGR.DD_TAG_ID = (SELECT DD_TAG_ID FROM '|| V_ESQUEMA ||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = AUX.TIPO) AND AGR.BORRADO = 0 
                  WHERE NOT EXISTS (SELECT 1 FROM '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA WHERE AGA.BORRADO = 0 AND AGR.AGR_ID = AGA.AGR_ID AND ACT.ACT_ID = AGA.ACT_ID)';
   
      EXECUTE IMMEDIATE V_MSQL;

      SALIDA := SALIDA || '   [INFO]      AÑADIDOS A AGRUPACIONES '|| SQL%ROWCOUNT|| CHR(10);
      ...
      V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_RES_RESTRINGIDA
                  ...
                  JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_ACT_PRINCIPAL = ACT.ACT_ID AND AGR.AGR_NUM_AGRUP_BC = AUX.COD_AGRUPACION AND AGR.AGR_FECHA_BAJA IS NULL AND AGR.DD_TAG_ID = (SELECT DD_TAG_ID FROM '|| V_ESQUEMA ||'.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = AUX.TIPO) AND AGR.BORRADO = 0
      ...
      SALIDA := SALIDA || '   [INFO]      AÑADIDOS A AGRUPACIONES DE NUEVA CREACIÓN '|| SQL%ROWCOUNT|| CHR(10);

      SALIDA := SALIDA || '   [INFO] 6 - AGRUPACIONES SIN ACTIVOS'||CHR(10);
      ...
      V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR
                  ...
                  WHERE AGR.BORRADO = 0 
                  AND AGR.AGR_FECHA_BAJA IS NULL
                  AND AGR.AGR_NUM_AGRUP_BC IS NOT NULL
      ...
      V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR
                  ...
                  AND NOT EXISTS (SELECT 1 FROM '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AUX_AGR
                     JOIN '|| V_ESQUEMA ||'.ACT_ACTIVO ACT ON AUX_AGR.AGR_NUM_AGRUP_BC = ACT.ACT_NUM_ACTIVO_CAIXA AND ACT.BORRADO = 0
                     JOIN '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA ON AUX_AGR.AGR_ID = AGA.AGR_ID AND AGA.ACT_ID = ACT.ACT_ID AND AGA.BORRADO = 0
                     WHERE (AUX_AGR.AGR_FIN_VIGENCIA IS NULL OR TRUNC(AUX_AGR.AGR_FIN_VIGENCIA) >= TRUNC(SYSDATE)) AND AGA.AGA_PRINCIPAL = 1 AND AGR.AGR_ID = AUX_AGR.AGR_ID AND AGR.DD_TAG_ID = AUX_AGR.DD_TAG_ID)
                  ) AUX
                  ON (AGR.AGR_ID = AUX.AGR_ID)
                  WHEN MATCHED THEN
                     UPDATE SET AGR.AGR_FECHA_BAJA = SYSDATE
      ...
      SALIDA := SALIDA || '   [INFO] 7 - INFORMAR UNIDAD ECÓNOMICA'||CHR(10);
      ...
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','ACT_RES_RESTRINGIDA');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','ACT_RAL_RESTRINGIDA_ALQUILER');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','ACT_ROB_RESTRINGIDA_OBREM');
      #ESQUEMA#.OPERACION_DDL.DDL_TABLE('ANALYZE','ACT_ONV_OBRA_NUEVA');

      V_MSQL := 'SELECT TO_CHAR(SYSDATE, ''DD-MM-YYYY HH24:MI:SS'') FROM DUAL';
      EXECUTE IMMEDIATE V_MSQL INTO HORA;
      SALIDA := SALIDA || '[FIN] ' || HORA ||CHR(10);
