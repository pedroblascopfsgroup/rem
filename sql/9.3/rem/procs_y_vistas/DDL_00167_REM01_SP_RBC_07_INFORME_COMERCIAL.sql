...
--1º Merge tabla AUX_APR_RBC_STOCK
	...
	USING (			
           SELECT
                ...
                , EQV2.DD_CODIGO_CAIXA CALEFACCION
                , EQV3.DD_CODIGO_CAIXA COCINA_EQUIPADA
                , EQV1.DD_CODIGO_CAIXA as EST_CONSERVACION
                ,CASE
                    WHEN EQV5.DD_CODIGO_CAIXA IS NOT NULL THEN ''002501''
                    WHEN EQV4.DD_CODIGO_CAIXA IS NOT NULL THEN EQV4.DD_CODIGO_CAIXA
                END AS JARDIN
                ,CASE
                    WHEN EQV5.DD_CODIGO_CAIXA IS NOT NULL THEN EQV5.DD_CODIGO_CAIXA
                    ELSE NULL
                END AS USO_JARDIN
            ...
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM EQV1 ON EQV1.DD_NOMBRE_CAIXA = ''EST_CONSERVACION'' AND EQV1.DD_CODIGO_CAIXA = CBX.DD_ECV_STOCK_BC AND EQV1.BORRADO=0
            LEFT JOIN '|| V_ESQUEMA ||'.DD_TCL_TIPO_CLIMATIZACION TCL ON TCL.DD_TCL_ID = ICO.ICO_CALEFACCION  
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM EQV2 ON EQV2.DD_NOMBRE_CAIXA = ''CALEFACCION'' AND EQV2.DD_CODIGO_REM = TCL.DD_TCL_CODIGO AND EQV2.BORRADO=0
            LEFT JOIN '|| V_ESQUEMA_M ||'.DD_SIN_SINO SIN_COC ON SIN_COC.DD_SIN_ID = ICO.ICO_COCINA_AMUEBLADA  
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM EQV3 ON EQV3.DD_NOMBRE_CAIXA = ''COCINA_EQUIPADA'' AND EQV3.DD_CODIGO_REM = SIN_COC.DD_SIN_CODIGO AND EQV3.BORRADO=0
            LEFT JOIN '|| V_ESQUEMA ||'.DD_DIS_DISPONIBILIDAD DIS_JAR ON DIS_JAR.DD_DIS_ID = ICO.ICO_JARDIN  
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM EQV4 ON EQV4.DD_NOMBRE_CAIXA = ''JARDIN'' AND EQV4.DD_CODIGO_REM = DIS_JAR.DD_DIS_CODIGO AND EQV4.BORRADO=0
            LEFT JOIN '|| V_ESQUEMA ||'.DD_DIS_DISPONIBILIDAD DIS_USO_JAR ON DIS_USO_JAR.DD_DIS_ID = ICO.ICO_JARDIN  
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM EQV5 ON EQV5.DD_NOMBRE_CAIXA = ''USO_JARDIN'' AND EQV5.DD_CODIGO_REM = DIS_USO_JAR.DD_DIS_CODIGO AND EQV5.BORRADO=0
            LEFT JOIN '|| V_ESQUEMA ||'.DD_DIS_DISPONIBILIDAD DIS_PISC ON DIS_PISC.DD_DIS_ID = ICO.ICO_PISCINA  
            LEFT JOIN '|| V_ESQUEMA ||'.DD_EQV_CAIXA_REM EQV6 ON EQV6.DD_NOMBRE_CAIXA = ''PISCINA'' AND EQV6.DD_CODIGO_REM = DIS_PISC.DD_DIS_CODIGO AND EQV6.BORRADO=0
            LEFT JOIN '|| V_ESQUEMA_M ||'.DD_SIN_SINO SIN_HUM ON SIN_HUM.DD_SIN_ID = ICO.ICO_SALIDA_HUMOS  
   ...
   SALIDA := SALIDA || '   [INFO] ACTUALIZADOS '|| SQL%ROWCOUNT|| CHR(10);
	...
	USING (WITH FEC_INF AS (
                ...
                (TCO.DD_TCO_CODIGO = ''01'' AND EPV.DD_EPV_CODIGO IN (''01'',''04'')
                OR TCO.DD_TCO_CODIGO = ''02'' AND (EPV.DD_EPV_CODIGO IN (''01'',''04'') OR EPA.DD_EPA_CODIGO IN (''01'',''04''))
                OR TCO.DD_TCO_CODIGO = ''03'' AND EPA.DD_EPA_CODIGO IN (''01'',''04''))
            ), ACT_PUBL AS (
                SELECT HFP.ACT_ID
                , HFP.HFP_FECHA_INI
                , ROW_NUMBER() OVER (PARTITION BY HFP.ACT_ID ORDER BY HFP.HFP_FECHA_INI DESC) RN
                FROM '|| V_ESQUEMA ||'.ACT_HFP_HIST_FASES_PUB HFP
                JOIN '|| V_ESQUEMA ||'.ACT_APU_ACTIVO_PUBLICACION APU ON APU.ACT_ID = HFP.ACT_ID AND APU.BORRADO = 0
                JOIN '|| V_ESQUEMA ||'.DD_FSP_FASE_PUBLICACION FSP ON FSP.DD_FSP_ID = HFP.DD_FSP_ID AND FSP.BORRADO = 0
                LEFT JOIN '|| V_ESQUEMA ||'.DD_TCO_TIPO_COMERCIALIZACION TCO ON TCO.DD_TCO_ID = APU.DD_TCO_ID AND TCO.BORRADO = 0
                LEFT JOIN '|| V_ESQUEMA ||'.DD_EPV_ESTADO_PUB_VENTA EPV ON EPV.DD_EPV_ID = APU.DD_EPV_ID AND EPV.BORRADO = 0
                LEFT JOIN '|| V_ESQUEMA ||'.DD_EPA_ESTADO_PUB_ALQUILER EPA ON EPA.DD_EPA_ID = APU.DD_EPA_ID AND EPA.BORRADO = 0
                WHERE HFP.BORRADO = 0 AND
                (TCO.DD_TCO_CODIGO = ''01'' AND EPV.DD_EPV_CODIGO = ''03''
                OR TCO.DD_TCO_CODIGO = ''02'' AND (EPV.DD_EPV_CODIGO = ''03'' OR EPA.DD_EPA_CODIGO = ''03'')
                OR TCO.DD_TCO_CODIGO = ''03'' AND EPA.DD_EPA_CODIGO = ''03'')
                AND FSP.DD_FSP_CODIGO IN (''08'',''10'')
            ), OFR_APROB AS (
                SELECT HFP.ACT_ID
                , HFP.HFP_FECHA_INI
                , ROW_NUMBER() OVER (PARTITION BY HFP.ACT_ID ORDER BY HFP.HFP_FECHA_INI DESC) RN
                ...
                LEFT JOIN '|| V_ESQUEMA ||'.DD_EPA_ESTADO_PUB_ALQUILER EPA ON EPA.DD_EPA_ID = APU.DD_EPA_ID AND EPA.BORRADO = 0
                WHERE HFP.BORRADO = 0 AND
                (TCO.DD_TCO_CODIGO = ''01'' AND EPV.DD_EPV_CODIGO = ''04''
                OR TCO.DD_TCO_CODIGO = ''02'' AND (EPV.DD_EPV_CODIGO = ''04'' OR EPA.DD_EPA_CODIGO = ''04'')
                OR TCO.DD_TCO_CODIGO = ''03'' AND EPA.DD_EPA_CODIGO = ''04'')
                AND FSP.DD_FSP_CODIGO IN (''08'',''10'')
                AND EXISTS (SELECT 1
                    FROM '|| V_ESQUEMA ||'.ACT_ACTIVO ACT
                    ...
                    AND ACT.ACT_ID = HFP.ACT_ID)
            )		
            SELECT
            ACT.ACT_NUM_ACTIVO_CAIXA AS NUM_IDENTIFICATIVO
            , ACT.ACT_NUM_ACTIVO AS NUM_INMUEBLE  
            , CASE WHEN FEC_FIN.HFP_FECHA_INI IS NOT NULL OR PUBL.HFP_FECHA_INI IS NOT NULL OR OFR.HFP_FECHA_INI IS NOT NULL THEN ''IF2''
                WHEN FEC_INI.HFP_FECHA_INI IS NOT NULL THEN ''IF1''
            END EST_COMERCIAL_SERVICER
            , TO_CHAR(COALESCE(FEC_FIN.HFP_FECHA_INI,FEC_INI.HFP_FECHA_INI, PUBL.HFP_FECHA_INI, OFR.HFP_FECHA_INI),''YYYYMMDD'') FEC_COMERCIAL_SERVICER
            , CASE WHEN FEC_FIN.HFP_FECHA_INI IS NOT NULL OR PUBL.HFP_FECHA_INI IS NOT NULL OR OFR.HFP_FECHA_INI IS NOT NULL THEN NULL
                WHEN FEC_INI.HFP_FECHA_INI IS NOT NULL THEN TO_CHAR(FEC_INI.HFP_FECHA_INI,''YYYYMMDD'')
            END FEC_INICIO_INFORME
            , TO_CHAR(COALESCE(FEC_FIN.HFP_FECHA_INI, PUBL.HFP_FECHA_INI, OFR.HFP_FECHA_INI),''YYYYMMDD'') FEC_FIN_INFORME
            ...
            LEFT JOIN FEC_INF FEC_FIN ON FEC_FIN.ACT_ID = ACT.ACT_ID AND FEC_FIN.RN = 1 AND FEC_FIN.DD_FSP_CODIGO IN (''08'',''10'')
            LEFT JOIN ACT_PUBL PUBL ON PUBL.ACT_ID = ACT.ACT_ID AND PUBL.RN = 1
            LEFT JOIN OFR_APROB OFR ON OFR.ACT_ID = ACT.ACT_ID AND OFR.RN = 1
            WHERE ACT.BORRADO = 0
            AND ACT.ACT_EN_TRAMITE = 0
            AND ACT.ACT_NUM_ACTIVO_CAIXA IS NOT NULL
            AND PRO.PRO_DOCIDENTIF NOT IN (''V84966126'',''V85164648'',''V85587434'',''V84322205'',''V84593961'',''V84669332'',''V85082675'',''V85623668'',''V84856319'',''V85500866'',''V85143659'',''V85594927'',''V85981231'',''V84889229'',''V84916956'',''V85160935'',''V85295087'',''V84175744'',''V84925569'',''A80352750'', ''A80514466'')
