...
--1º Merge tabla AUX_APR_RBC_STOCK
   ...
   SALIDA := SALIDA || '   [INFO] ACTUALIZADOS '|| SQL%ROWCOUNT|| CHR(10);
    ...
    V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.AUX_APR_RBC_STOCK AUX
	USING (			
           WITH EST_INF_COMERCIAL AS (
                SELECT
                HIC.ACT_ID
                , AIC.DD_AIC_CODIGO EST_COMERCIAL_SERVICER
                , HIC.HIC_FECHA FEC_FIN_INFORME
                , ROW_NUMBER() OVER (PARTITION BY HIC.ACT_ID ORDER BY HIC.HIC_FECHA DESC) RN
                FROM '|| V_ESQUEMA ||'.ACT_HIC_EST_INF_COMER_HIST HIC
                JOIN '|| V_ESQUEMA ||'.DD_AIC_ACCION_INF_COMERCIAL AIC ON HIC.DD_AIC_ID = AIC.DD_AIC_ID AND AIC.BORRADO = 0
                WHERE HIC.BORRADO = 0
            )
            SELECT
            ACT.ACT_NUM_ACTIVO_CAIXA AS NUM_IDENTIFICATIVO
            , ACT.ACT_NUM_ACTIVO AS NUM_INMUEBLE  
            , EIC.EST_COMERCIAL_SERVICER
            , NULL FEC_INICIO_INFORME
            , TO_CHAR(EIC.FEC_FIN_INFORME,''YYYYMMDD'') FEC_FIN_INFORME
            ...
            FROM '|| V_ESQUEMA ||'.ACT_ACTIVO ACT
            LEFT JOIN EST_INF_COMERCIAL EIC ON ACT.ACT_ID = EIC.ACT_ID AND EIC.RN = 1
