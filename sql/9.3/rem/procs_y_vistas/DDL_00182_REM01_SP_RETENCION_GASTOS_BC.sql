...
--##        0.4 Retener Tasa e IBIS temporalmente - [REMVIP-12109] - Remus Ovidiu
--##########################################
--*/
...
V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.GASTOS_RETENIDOS_BC (GPV_ID)
		...
		WHERE GGE.GGE_FECHA_ENVIO_PRPTRIO IS NULL 
			...
			AND ( 
				  EXISTS (SELECT 
							1
						  FROM PFSREM.GASTOS_COMPENSABLES_20220214_AGRUPADOS AUX
...
WITH PROPIETARIOS_ACTIVOS AS(
    ...
    JOIN rem01.GLD_ENT GLDENT ON GLDENT.GLD_ID = GLD.GLD_ID
    JOIN rem01.DD_ENT_ENTIDAD_GASTO ENT ON ENT.DD_ENT_ID = GLDENT.DD_ENT_ID AND ENT.DD_ENT_CODIGO = ''ACT''
    JOIN rem01.ACT_ACTIVO ACTIVO ON ACTIVO.ACT_ID = GLDENT.ENT_ID
    JOIN rem01.ACT_PAC_PROPIETARIO_ACTIVO PAC ON PAC.ACT_ID = ACTIVO.ACT_ID AND PAC.BORRADO = 0
    JOIN rem01.ACT_PRO_PROPIETARIO PRO ON PRO.PRO_ID = PAC.PRO_ID AND PRO.BORRADO = 0
...
JOIN rem01.DD_EGA_ESTADOS_GASTO EGA ON EGA.DD_EGA_ID = GPV.DD_EGA_ID
    AND EGA.BORRADO = 0
JOIN rem01.DD_EAH_ESTADOS_AUTORIZ_HAYA DD_EAH ON DD_EAH.DD_EAH_ID = GGE.DD_EAH_ID
    AND DD_EAH.BORRADO = 0
JOIN rem01.ACT_PVE_PROVEEDOR PVE ON PVE.PVE_ID = GPV.PVE_ID_EMISOR 
    AND PVE.BORRADO = 0
JOIN rem01.DD_STG_SUBTIPOS_GASTO STG ON GLD.DD_STG_ID = STG.DD_STG_ID
    AND STG.BORRADO = 0
...
JOIN rem01.ACT_EJE_EJERCICIO EJE ON EJE.EJE_ID = GIC.EJE_ID 
    AND EJE.BORRADO = 0 
LEFT JOIN rem01.GLD_ENT GEN ON GEN.GLD_ID = GLD.GLD_ID 
    AND GEN.BORRADO = 0
LEFT JOIN rem01.DD_ENT_ENTIDAD_GASTO ENT ON ENT.DD_ENT_ID = GEN.DD_ENT_ID 
    AND ENT.BORRADO = 0 
    AND ENT.DD_ENT_CODIGO = ''ACT''
LEFT JOIN rem01.DD_TIT_TIPOS_IMPUESTO DD_TIT ON DD_TIT.DD_TIT_ID = GLD.DD_TIT_ID 
    AND DD_TIT.BORRADO = 0
LEFT JOIN rem01.DD_TDI_TIPO_DOCUMENTO_ID TDI ON TDI.DD_TDI_ID = PVE.DD_TDI_ID
    AND TDI.BORRADO = 0
LEFT JOIN rem01.DD_TPR_TIPO_PROVEEDOR DD_TPR ON DD_TPR.DD_TPR_ID = PVE.DD_TPR_ID
    AND DD_TPR.BORRADO = 0
LEFT JOIN rem01.DD_TOG_TIPO_OPERACION_GASTO TOG ON TOG.DD_TOG_ID = GPV.DD_TOG_ID
    AND TOG.BORRADO = 0
LEFT JOIN rem01.GPV_GASTOS_PROVEEDOR GPV2 ON GPV.NUM_GASTO_ABONADO = GPV2.GPV_ID 
    AND GPV2.BORRADO = 0
JOIN rem01.DD_TGA_TIPOS_GASTO TGA ON TGA.DD_TGA_ID = GPV.DD_TGA_ID
    AND TGA.BORRADO = 0
WHERE GPV.BORRADO = 0
	...
	AND STG.DD_STG_CODIGO NOT IN (''89'')
      AND ( tga.dd_tga_codigo IN (
        ''02''
    )
          OR stg.dd_stg_codigo IN (
        ''01'',
       ''02''
    ) )
	AND NOT EXISTS (
			SELECT 1
            FROM rem01.GPV_GASTOS_PROVEEDOR GPV3
            JOIN rem01.GLD_GASTOS_LINEA_DETALLE GLD ON GLD.GPV_ID = GPV3.GPV_ID
                AND GLD.BORRADO = 0
			JOIN rem01.DD_TGA_TIPOS_GASTO TGA ON TGA.DD_TGA_ID = GPV3.DD_TGA_ID
                AND TGA.BORRADO = 0
            JOIN rem01.DD_STG_SUBTIPOS_GASTO STG ON STG.DD_STG_ID = GLD.DD_STG_ID
                AND STG.BORRADO = 0
            WHERE TGA.DD_TGA_CODIGO IN (''01'')
                AND STG.DD_STG_CODIGO NOT IN (''237'', ''238'', ''239'',''01'',''02'')
                AND GPV3.PVE_ID_GESTORIA IS NOT NULL
                AND GPV3.BORRADO = 0
                AND GPV3.GPV_ID = GPV.GPV_ID
	)
    AND NOT EXISTS (
            SELECT 1
            FROM rem01.GPV_GASTOS_PROVEEDOR GPV2
            JOIN rem01.ACT_PRO_PROPIETARIO PRO2 ON GPV2.PRO_ID = PRO2.PRO_ID
                AND PRO2.BORRADO = 0
            JOIN rem01.DD_CRA_CARTERA CRA2 ON PRO2.DD_CRA_ID = CRA2.DD_CRA_ID 
                AND CRA2.BORRADO = 0
            JOIN rem01.DD_DEG_DESTINATARIOS_GASTO DEG ON DEG.DD_DEG_ID = GPV2.DD_DEG_ID 
                AND DEG.BORRADO = 0
            JOIN rem01.GDE_GASTOS_DETALLE_ECONOMICO GDE2 ON GPV2.GPV_ID = GDE2.GPV_ID 
                AND GDE2.BORRADO = 0
            WHERE GPV2.GPV_ID = GPV.GPV_ID
                AND CRA2.DD_CRA_CODIGO =''03''
                AND DEG.DD_DEG_CODIGO = ''02''
    ...
    AND (
            EXISTS (
                ...
                SELECT 1 
                FROM rem01.GLD_GASTOS_LINEA_DETALLE GLD2
                LEFT JOIN rem01.GLD_ENT GLDENT2 ON GLDENT2.GLD_ID = GLD2.GLD_ID
                    AND GLDENT2.BORRADO = 0
                LEFT JOIN rem01.DD_ENT_ENTIDAD_GASTO ENT2 ON ENT2.DD_ENT_ID = GLDENT2.DD_ENT_ID 
                    AND ENT2.DD_ENT_CODIGO = ''ACT''
                    AND ENT2.BORRADO = 0
                LEFT JOIN rem01.ACT_ACTIVO ACTIVO ON ACTIVO.ACT_ID = GLDENT2.ENT_ID
                    AND ACTIVO.BORRADO = 0
                WHERE GLD2.GPV_ID = GPV.GPV_ID 
                    AND GLD2.BORRADO = 0 
                    AND ACTIVO.ACT_ID IS NULL
	...
	AND (
		(GGE.GGE_CLIENTE_PAGADOR = (SELECT PRO_ID FROM REM01.ACT_PRO_PROPIETARIO WHERE PRO_SOCIEDAD_PAGADORA = ''0015'') AND GGE.GGE_FECHA_ENVIO_PRPTRIO IS NULL)
		 OR 
		 (GGE.GGE_CLIENTE_INFORMADOR = (SELECT PRO_ID FROM REM01.ACT_PRO_PROPIETARIO WHERE PRO_SOCIEDAD_PAGADORA = ''0015'') AND GGE.GGE_FECHA_ENVIO_INFORMATIVA IS NULL)
		)
)
				';
EXECUTE IMMEDIATE V_MSQL;
...
ON (T1.GPV_ID = T2.GPV_ID)
WHEN MATCHED THEN
    UPDATE SET T1.DD_EGA_ID = 7,
	T1.USUARIOMODIFICAR = ''REMVIP_12908'',
	T1.FECHAMODIFICAR = SYSDATE';
EXECUTE IMMEDIATE V_MSQL;
