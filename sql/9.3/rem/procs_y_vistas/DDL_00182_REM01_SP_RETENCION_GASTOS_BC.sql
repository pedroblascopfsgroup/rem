...
V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.GASTOS_RETENIDOS_BC (GPV_ID)
		...
		WHERE GGE.GGE_FECHA_ENVIO_PRPTRIO IS NULL 
			...
			AND ( 
				  EXISTS (SELECT 
							1
						  FROM GASTOS_COMPENSABLES_20220301_AGRUPADOS AUX
						  WHERE AUX.NIF_EMISOR = PVE.PVE_DOCIDENTIF
						 )
				OR 
				  EXISTS (SELECT 
				  			1
						  FROM GASTOS_PAGADOS_CXB_NO_ENVIADOS_BC AUX2
						  JOIN REM01.GPV_GASTOS_PROVEEDOR GPV2 ON GPV2.GPV_NUM_GASTO_HAYA = AUX2.GASTO
						  	AND GPV2.BORRADO = 0
						  JOIN REM01.ACT_PVE_PROVEEDOR PVE2 ON PVE2.PVE_ID = GPV2.PVE_ID_EMISOR
						  	AND PVE2.BORRADO = 0
...
EXECUTE IMMEDIATE V_MSQL;
OUTPUT := 'Se han insertado ' || GASTOS_PROVISION || ' gastos provisionados y ' || GASTOS_MANUALES || ' gastos manuales en la tabla GASTOS_RETENIDOS_BC. ' || GASTOS_RETENIDOS || ' gastos retenidos actualmente de ' || ANYO || '. Adicionalmente, se han borrado ' || SQL%ROWCOUNT || ' que estaban duplicados en la tabla de gastos retenidos de ' || ANYO;
