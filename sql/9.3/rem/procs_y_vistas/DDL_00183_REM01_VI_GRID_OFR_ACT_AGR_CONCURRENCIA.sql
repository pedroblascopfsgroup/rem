		...
		WITH ORDENACIONCONCURRENCIA AS (
			SELECT ID as IDOFERTA, ROWNUM as ORDENFINAL FROM(
				SELECT
					...
					JOIN ' || V_ESQUEMA || '.DEP_DEPOSITO DEP ON DEP.OFR_ID = OFR.OFR_ID AND DEP.DEP_FECHA_INGRESO IS NOT NULL AND OFR.OFR_FECHA_OFERTA_PENDIENTE IS NOT NULL AND DEP.BORRADO = 0
				...
				UNION 
					...
					FROM ' || V_ESQUEMA || '.OFR_OFERTAS OFR
					JOIN ' || V_ESQUEMA || '.DEP_DEPOSITO DEP ON DEP.OFR_ID = OFR.OFR_ID AND DEP.DEP_FECHA_INGRESO IS NULL AND OFR.OFR_FECHA_OFERTA_PENDIENTE IS NOT NULL AND DEP.BORRADO = 0
					WHERE OFR.BORRADO  = 0  AND OFR.OFR_CONCURRENCIA = 1
				...
				UNION
					...
					FROM ' || V_ESQUEMA || '.OFR_OFERTAS OFR
					JOIN ' || V_ESQUEMA || '.DEP_DEPOSITO DEP ON DEP.OFR_ID = OFR.OFR_ID AND DEP.DEP_FECHA_INGRESO IS NULL AND OFR.OFR_FECHA_OFERTA_PENDIENTE IS NULL AND DEP.BORRADO = 0
					WHERE OFR.BORRADO  = 0  AND OFR.OFR_CONCURRENCIA = 1
