...
USING (
    ...
    SELECT 
        ...
        ,AAM.DD_TPU_A_ID
        ,AAM.DD_POR_ID
    FROM '||V_ESQUEMA||'.AHP_ACTIVO_MATRIZ AAM 
    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AAM.NUM_IDENTIFICATIVO  
        AND ACT.BORRADO = 0
...
FROM '||V_ESQUEMA||'.CRG_ACTIVO_MATRIZ CAM
JOIN '||V_ESQUEMA||'.AUX_APR_BCR_STOCK AUX ON AUX.NUM_UNIDAD = CAM.NUM_IDENTIFICATIVO   
JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO  
    AND ACT.BORRADO = 0 
...
WITH ACTIVOS_MATRIZ AS (
    ...
    JOIN REMMASTER.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_ID = GEE.DD_TGE_ID
        AND TGE.BORRADO = 0
    WHERE TGE.DD_TGE_CODIGO IN (''SADM'',''SUPADM'',''SADM'',''SCOM'',''GCOM'',''GTOADM'',''GGADM'',''GADM'',''GADMT'',''GIAADMT'')
...
WHERE NOT EXISTS (SELECT
                  ...
                  AND TGE.DD_TGE_CODIGO IN (''SADM'',''SUPADM'',''SADM'',''SCOM'',''GCOM'',''GTOADM'',''GGADM'',''GADM'',''GADMT'',''GIAADMT'')
                 )';
   EXECUTE IMMEDIATE V_MSQL;
...
WITH ACTIVOS_MATRIZ AS (
    ...
    JOIN REMMASTER.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_ID = GEH.DD_TGE_ID
        AND TGE.BORRADO = 0
    WHERE TGE.DD_TGE_CODIGO IN (''SADM'',''SUPADM'',''SADM'',''SCOM'',''GCOM'',''GTOADM'',''GGADM'',''GADM'',''GADMT'',''GIAADMT'')
) SELECT S_GEH_GESTOR_ENTIDAD_HIST.NEXTVAL, GEH.USU_ID, GEH.DD_TGE_ID, GEH.ACT_ID, GEH.GEH_FECHA_DESDE, GEH.GEH_FECHA_HASTA, ACT.ACT_ID
...
USING (
    ...
    SELECT 
        ...
        ,IAM.ICM_FECHA_HASTA
        ,IAM.DD_TRL_ID
    FROM '||V_ESQUEMA||'.ICM_ACTIVO_MATRIZ IAM 
    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = IAM.NUM_IDENTIFICATIVO  
        AND ACT.BORRADO = 0
) T2 ON (T1.ACT_ID = T2.ACT_ID AND T1.BORRADO = 0)
WHEN NOT MATCHED THEN 
	INSERT(
