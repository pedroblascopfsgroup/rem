...
SALIDA := SALIDA || '[INFO] 3º ACT_PTA_PATRIMONIO_ACTIVO'||CHR(10);
   ...
   V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_PTA_PATRIMONIO_ACTIVO T1
               ...
               WHEN MATCHED THEN UPDATE SET
                  ...
                  ,T1.DD_EAL_ID = NVL(T2.DD_EAL_ID, (SELECT DD_EAL_ID FROM '||V_ESQUEMA||'.DD_EAL_ESTADO_ALQUILER WHERE DD_EAL_CODIGO = ''01''))
               ...
               WHEN NOT MATCHED THEN 
                  INSERT(
                     ...
                     ,NVL(T2.DD_EAL_ID, (SELECT DD_EAL_ID FROM '||V_ESQUEMA||'.DD_EAL_ESTADO_ALQUILER WHERE DD_EAL_CODIGO = ''01''))
                     ,T2.DD_TPI_ID
                     ,T2.CHECK_HPM
                     ,0
   ...
   V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_AHP_HIST_PUBLICACION T1
				USING (
				    SELECT 
						...
						,SYSDATE AHP_FECHA_INI_ALQUILER
				    FROM REM01.AUX_APR_BCR_STOCK AUX
				    JOIN REM01.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_CAIXA = AUX.NUM_IDENTIFICATIVO  
				        AND ACT.BORRADO = 0
				        AND AUX.NUM_UNIDAD IS NOT NULL
