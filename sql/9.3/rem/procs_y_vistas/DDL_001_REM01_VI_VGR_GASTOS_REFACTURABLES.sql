...
V_MSQL := ('CREATE OR REPLACE FORCE VIEW '||V_ESQUEMA||'.'||V_VIEWNAME||'(NUM_GASTO_HAYA) AS
	...
	LEFT JOIN '||V_ESQUEMA||'.GRG_REFACTURACION_GASTOS GRG ON GRG.GRG_GPV_ID_REFACTURADO= GDE.GDE_ID AND GRG.BORRADO=0 AND GRG.GRG_ID IS NULL
	INNER JOIN '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV ON GDE.GPV_ID=GPV.GPV_ID AND GPV.BORRADO=0
	INNER JOIN '||V_ESQUEMA||'.DD_DEP_DESTINATARIOS_PAGO DEP ON GDE.DD_DEP_ID=DEP.DD_DEP_ID AND DD_DEP_CODIGO=''02'' AND DEP.BORRADO=0
	INNER JOIN '||V_ESQUEMA||'.ACT_PRO_PROPIETARIO PRO ON PRO.PRO_ID=GPV.PRO_ID AND PRO.BORRADO=0
	INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.dd_cra_id=pro.DD_CRA_ID AND DD_CRA_CODIGO in (''02'',''03'') AND CRA.BORRADO=0
