...
--## VERSIONES:
--##        0.1 Versión inicial
--##        0.2 REMVIP-11281 - Juan Alfonso - Añadir a Titulizadas, BFA y JAGUAR
--##########################################
--*/
...
V_MSQL := ('CREATE OR REPLACE FORCE VIEW '||V_ESQUEMA||'.'||V_VIEWNAME||'(NUM_GASTO_HAYA) AS
	...
	INNER JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.dd_cra_id=pro.DD_CRA_ID AND CRA.BORRADO=0
	WHERE GDE.GDE_GASTO_REFACTURABLE=1 AND GRG.GRG_ID IS NULL AND
    ( DD_CRA_CODIGO in (''02'',''03'',''18'',''17'') OR (CRA.DD_CRA_CODIGO=''07'' AND PRO.PRO_DOCIDENTIF=''B16896615''))');
