...
V_MSQL := ('CREATE OR REPLACE FORCE VIEW '||V_ESQUEMA||'.'||V_VIEWNAME||' AS
				SELECT
				    ...
				    (SELECT ACT_VAL.VAL_IMPORTE FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES ACT_VAL JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO TPC ON TPC.DD_TPC_ID = ACT_VAL.DD_TPC_ID AND TPC.DD_TPC_CODIGO = ''01'' WHERE ACT_VAL.ACT_ID = ACT.ACT_ID AND TPC.BORRADO = 0 AND ACT_VAL.BORRADO=0) VAL_IMPORTE_VNC,
