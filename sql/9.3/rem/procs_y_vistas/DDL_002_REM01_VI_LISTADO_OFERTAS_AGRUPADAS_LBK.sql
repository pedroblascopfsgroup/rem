...
V_MSQL := ('CREATE OR REPLACE FORCE VIEW '||V_ESQUEMA||'.'||V_VIEWNAME||' AS
				SELECT
				    ...
				    ACT_TAS.TAS_IMPORTE_TAS_FIN TAS_IMPORTE_TAS_FIN,
				    (SELECT ACT_VAL.VAL_IMPORTE FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES ACT_VAL JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO TPC ON TPC.DD_TPC_ID = ACT_VAL.DD_TPC_ID AND TPC.DD_TPC_CODIGO = ''01'' WHERE ACT_VAL.ACT_ID = ACT.ACT_ID) VAL_IMPORTE_VNC,
				    (SELECT ACT_VAL.VAL_IMPORTE FROM '||V_ESQUEMA||'.ACT_VAL_VALORACIONES ACT_VAL JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO TPC ON TPC.DD_TPC_ID = ACT_VAL.DD_TPC_ID AND TPC.DD_TPC_CODIGO = ''23'' WHERE ACT_VAL.ACT_ID = ACT.ACT_ID) VAL_IMPORTE_VR
				FROM '||V_ESQUEMA||'.OGR_OFERTAS_AGRUPADAS_LBK OGR 
				    JOIN '||V_ESQUEMA||'.OFR_OFERTAS PRINCIPAL ON OGR.ID_OFERTA_PRINCIPAL = PRINCIPAL.OFR_ID 
				    JOIN '||V_ESQUEMA||'.OFR_OFERTAS DEPENDIENTE ON OGR.ID_OFERTA_DEPENDIENTE = DEPENDIENTE.OFR_ID
				    JOIN '||V_ESQUEMA||'.ACT_OFR ON ACT_OFR.OFR_ID = DEPENDIENTE.OFR_ID
				    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = ACT_OFR.ACT_ID
				    JOIN '||V_ESQUEMA||'.ACT_TAS_TASACION ACT_TAS ON ACT_TAS.ACT_ID = ACT.ACT_ID');
