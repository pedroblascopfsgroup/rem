...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW '|| V_ESQUEMA ||'.V_ALERTAS_PUBLICACION 
	...
	ACT.ACT_ID AS ID_ACTIVO,
	 CASE WHEN (ACT.DD_CRA_ID = 21 AND STA.DD_STA_ID = 4 AND TRUNC(ADN.ADN_FECHA_TITULO) > TRUNC(TO_DATE(''01/01/2019'',''DD/MM/YYYY''))) 
            THEN ''BANKIA DACION''
	...
	FROM '|| V_ESQUEMA ||'.ACT_ACTIVO ACT 
        LEFT JOIN REM01.ACT_ADN_ADJNOJUDICIAL ADN ON ADN.ACT_ID = ACT.ACT_ID 
        LEFT JOIN REM01.BIE_ADJ_ADJUDICACION ADJ ON ACT.BIE_ID = ADJ.BIE_ID 
        INNER JOIN '|| V_ESQUEMA ||'.DD_TCO_TIPO_COMERCIALIZACION TCO ON ACT.DD_TCO_ID = TCO.DD_TCO_ID AND TCO.BORRADO = 0
	...
	INNER JOIN REM01.DD_STA_SUBTIPO_TITULO_ACTIVO STA ON ACT.DD_STA_ID = STA.DD_STA_ID AND STA.BORRADO = 0
