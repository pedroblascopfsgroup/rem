   ...
   PL_OUTPUT := PL_OUTPUT || '[INICIO]' || CHR(10);
	...
	IF P_ACT_ID IS NULL AND P_AGR_ID IS NULL AND UPPER(V_USUARIO) = 'PROC_CALCULO_PORTAL' THEN 
		V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION AHP
		   USING (
		            SELECT
		                AHP.AHP_ID
		            FROM (SELECT DISTINCT AGR.AGR_ACT_PRINCIPAL FROM '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA
					JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGA.AGR_ID = AGR.AGR_ID AND AGR.BORRADO = 0
					WHERE AGA.BORRADO = 0 AND AGR.AGR_ACT_PRINCIPAL IS NOT NULL) ACT_PRINCIPAL
				    JOIN '|| V_ESQUEMA ||'.V_PORTALES_ACTIVO VPA ON ACT_PRINCIPAL.AGR_ACT_PRINCIPAL = VPA.ACT_ID
				    JOIN '|| V_ESQUEMA ||'.ACT_APU_ACTIVO_PUBLICACION APU ON VPA.ACT_ID = APU.ACT_ID
				    JOIN '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA ON AGA.AGR_ID = VPA.AGR_ID 
				    JOIN '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION AHP ON AGA.ACT_ID = AHP.ACT_ID AND AHP.BORRADO = 0 AND AHP.AHP_FECHA_FIN_VENTA IS NULL
				    LEFT JOIN DD_EPV_ESTADO_PUB_VENTA EPV ON EPV.DD_EPV_ID = AHP.DD_EPV_ID AND EPV.BORRADO = 0
				WHERE (APU.DD_POR_ID IS NULL OR APU.DD_POR_ID <> VPA.DD_POR_ID) AND APU.BORRADO = 0 AND EPV.DD_EPV_CODIGO IN (''03'',''04'')) AUX
		...
		V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION (
		        ...
		        SELECT  '|| V_ESQUEMA ||'.S_ACT_AHP_HIST_PUBLICACION.NEXTVAL
		                                      ...
		                                      , APU2.DD_TCO_ID
		                                      , APU2.DD_MTO_V_ID
		                                      , APU2.APU_MOT_OCULTACION_MANUAL_V
		                                      , APU2.APU_CHECK_PUBLICAR_V
		                                      , APU2.APU_CHECK_OCULTAR_V
		                                      , APU2.APU_CHECK_OCULTAR_PRECIO_V
		                                      , APU2.APU_CHECK_PUB_SIN_PRECIO_V
		                                      , APU2.DD_MTO_A_ID
		                                      , APU2.APU_MOT_OCULTACION_MANUAL_A
		                                      , APU2.APU_CHECK_PUBLICAR_A
		                                      , APU2.APU_CHECK_OCULTAR_A
		                                      , APU2.APU_CHECK_OCULTAR_PRECIO_A
		                                      , APU2.APU_CHECK_PUB_SIN_PRECIO_A
		                                      , SYSDATE
		                                      , APU2.VERSION
		                                      , '''||V_USUARIO||''' USUARIOCREAR
		                                      , SYSDATE FECHACREAR
		                                      , 0 BORRADO
		                                      , APU2.ES_CONDICONADO_ANTERIOR
		                                      , VPA.DD_POR_ID
		            FROM (SELECT DISTINCT AGR.AGR_ACT_PRINCIPAL FROM '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA
					JOIN '|| V_ESQUEMA ||'.ACT_AGR_AGRUPACION AGR ON AGA.AGR_ID = AGR.AGR_ID AND AGR.BORRADO = 0
					WHERE AGA.BORRADO = 0 AND AGR.AGR_ACT_PRINCIPAL IS NOT NULL) ACT_PRINCIPAL
				    ...
				    JOIN '|| V_ESQUEMA ||'.ACT_APU_ACTIVO_PUBLICACION APU ON VPA.ACT_ID = APU.ACT_ID
				    JOIN '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA ON AGA.AGR_ID = VPA.AGR_ID 
				    JOIN ACT_APU_ACTIVO_PUBLICACION APU2 ON AGA.ACT_ID = APU2.ACT_ID AND APU2.BORRADO = 0
				    LEFT JOIN DD_EPV_ESTADO_PUB_VENTA EPV ON EPV.DD_EPV_ID = APU2.DD_EPV_ID AND EPV.BORRADO = 0
				WHERE (APU.DD_POR_ID IS NULL OR APU.DD_POR_ID <> VPA.DD_POR_ID) AND APU.BORRADO = 0 AND EPV.DD_EPV_CODIGO IN (''03'',''04'')';
    ...
    ELSIF P_ACT_ID IS NULL AND P_AGR_ID IS NOT NULL THEN-- AGRUPACIONES
        EXECUTE IMMEDIATE 'SELECT APU.DD_POR_ID FROM '||V_ESQUEMA||'.ACT_APU_ACTIVO_PUBLICACION APU
			   JOIN (SELECT DISTINCT AGR.AGR_ACT_PRINCIPAL FROM '||V_ESQUEMA||'.ACT_AGA_AGRUPACION_ACTIVO AGA
			   JOIN '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR ON AGA.AGR_ID = AGR.AGR_ID AND AGR.BORRADO = 0
		           WHERE AGA.BORRADO = 0 AND AGR.AGR_ID = '||P_AGR_ID||') ACT_PRINCIPAL ON APU.ACT_ID = ACT_PRINCIPAL.AGR_ACT_PRINCIPAL' INTO V_POR_A;
        EXECUTE IMMEDIATE 'SELECT VPA.DD_POR_ID FROM '||V_ESQUEMA||'.V_PORTALES_ACTIVO VPA
			   JOIN (SELECT DISTINCT AGR.AGR_ACT_PRINCIPAL FROM '||V_ESQUEMA||'.ACT_AGA_AGRUPACION_ACTIVO AGA
			   JOIN '||V_ESQUEMA||'.ACT_AGR_AGRUPACION AGR ON AGA.AGR_ID = AGR.AGR_ID AND AGR.BORRADO = 0
			   WHERE AGA.BORRADO = 0 AND AGR.AGR_ID = '||P_AGR_ID||') ACT_PRINCIPAL ON VPA.ACT_ID = ACT_PRINCIPAL.AGR_ACT_PRINCIPAL' INTO V_POR_V;
        IF V_POR_V IS NULL OR V_POR_V = V_POR_A THEN
            PL_OUTPUT := PL_OUTPUT || 'OK. Sin cambios que realizar' || CHR(10);
        ELSE 
             V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_APU_ACTIVO_PUBLICACION APU
