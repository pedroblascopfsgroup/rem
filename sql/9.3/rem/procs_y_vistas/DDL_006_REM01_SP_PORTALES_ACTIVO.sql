...
BEGIN
   PL_OUTPUT := PL_OUTPUT || '[INICIO]' || CHR(10);
	...
	IF P_ACT_ID IS NULL AND P_AGR_ID IS NULL AND UPPER(V_USUARIO) = 'PROC_CALCULO_PORTAL' THEN 
		...
		V_MSQL := 'CREATE MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_COND_DISPONIBILIDAD_M AS SELECT ACT_ID, SIN_TOMA_POSESION_INICIAL, OCUPADO_CONTITULO, PENDIENTE_INSCRIPCION, PROINDIVISO
		, TAPIADO, OBRANUEVA_SINDECLARAR, OBRANUEVA_ENCONSTRUCCION, DIVHORIZONTAL_NOINSCRITA, RUINA, VANDALIZADO, COMBO_OTRO, SIN_INFORME_APROBADO, SIN_INFORME_APROBADO_REM, REVISION
		, PROCEDIMIENTO_JUDICIAL, CON_CARGAS, SIN_ACCESO, OCUPADO_SINTITULO, ESTADO_PORTAL_EXTERNO, ES_CONDICIONADO FROM '|| V_ESQUEMA ||'.V_COND_DISPONIBILIDAD WHERE BORRADO = 0';
		...
		V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.ACT_AHP_HIST_PUBLICACION AHP
		   USING (
				...
				WHERE AHP.BORRADO = 0 AND AHP.AHP_FECHA_FIN_VENTA IS NULL AND (APU.DD_POR_ID IS NULL OR APU.DD_POR_ID <> VPA.DD_POR_ID)
				 ) AUX
    ...
    ELSIF P_ACT_ID IS NOT NULL AND P_AGR_ID IS NULL THEN --ACTIVOS 
		...
		EXECUTE IMMEDIATE 'SELECT DISTINCT VPA.DD_POR_ID FROM '||V_ESQUEMA||'.V_PORTALES_ACTIVO VPA WHERE VPA.ACT_ID = '||P_ACT_ID INTO V_POR_V;
