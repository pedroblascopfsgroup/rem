...
BEGIN
   PL_OUTPUT := PL_OUTPUT || '[INICIO]' || CHR(10);
	...
	IF P_ACT_ID IS NULL AND P_AGR_ID IS NULL AND UPPER(V_USUARIO) = 'PROC_CALCULO_PORTAL' THEN 
		...
		V_MSQL := 'CREATE MATERIALIZED VIEW '|| V_ESQUEMA ||'.V_COND_DISPONIBILIDAD_M AS SELECT COND.ACT_ID, COND.SIN_TOMA_POSESION_INICIAL, COND.OCUPADO_CONTITULO, COND.PENDIENTE_INSCRIPCION, COND.PROINDIVISO
		, COND.TAPIADO, COND.OBRANUEVA_SINDECLARAR, COND.OBRANUEVA_ENCONSTRUCCION, COND.DIVHORIZONTAL_NOINSCRITA, COND.RUINA, COND.VANDALIZADO, COND.COMBO_OTRO, COND.SIN_INFORME_APROBADO, SININF.SIN_INFORME_APROBADO_REM, 
		COND.REVISION, COND.PROCEDIMIENTO_JUDICIAL, COND.CON_CARGAS, COND.SIN_ACCESO, COND.OCUPADO_SINTITULO, COND.ESTADO_PORTAL_EXTERNO, ESCON.ES_CONDICIONADO FROM '|| V_ESQUEMA ||'.V_COND_DISPONIBILIDAD COND
		JOIN '||V_ESQUEMA||'.V_ES_CONDICIONADO ESCON ON ESCON.ACT_ID = COND.ACT_ID JOIN '||V_ESQUEMA||'.V_SIN_INFORME_APROBADO_REM SININF ON SININF.ACT_ID=COND.ACT_ID WHERE COND.BORRADO = 0';
