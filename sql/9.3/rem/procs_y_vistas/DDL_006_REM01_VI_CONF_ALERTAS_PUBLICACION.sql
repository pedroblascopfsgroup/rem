...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW '|| V_ESQUEMA ||'.V_CONF_ALERTAS_PUBLICACION 
	...
	CASE WHEN T1.ALERTA_ALQUILER = 1 OR T1.ALERTA_TIPO_ALQUILER = 1 OR T1.ALERTA_OKUPADO = 1 THEN 1 ELSE 0 END AS INDICADOR_1,
	...
	INNER JOIN '|| V_ESQUEMA ||'.CONF_ALERTAS_PUBLICACION T2 ON  

	(T1.CARTERA = T2.CARTERA AND T1.CARTERA = ''BANKIA'' AND T1.SUBCARTERA IS NOT NULL AND T2.SUBCARTERA IS NULL AND T1.SUBTIPO_TITULO = T2.SUBTIPO_TITULO AND T1.SUBTIPO_TITULO = ''Notarial (dación)'')    
	OR
	(T1.CARTERA = T2.CARTERA AND T1.CARTERA = ''BANKIA'' AND T1.SUBCARTERA IS NOT NULL AND T2.SUBCARTERA IS NULL AND T1.SUBTIPO_TITULO = T2.SUBTIPO_TITULO AND T1.SUBTIPO_TITULO <> ''Notarial (dación)'')   
	OR
	(T1.CARTERA = T2.CARTERA and T1.CARTERA <> ''BANKIA'' AND T1.SUBCARTERA IS NOT NULL AND T2.SUBCARTERA IS NULL)
	OR
	(T1.CARTERA = T2.CARTERA AND T1.CARTERA = ''CERBERUS'' AND T1.SUBCARTERA = T2.SUBCARTERA) 
	OR 
	(T1.CARTERA = T2.CARTERA AND T1.CARTERA = ''CERBERUS'' AND T1.SUBCARTERA <> T2.SUBCARTERA) 
	OR 
	(T1.CARTERA <> T2.CARTERA AND T2.CARTERA = ''OTRA CARTERA'' AND T1.CARTERA NOT IN (''BANKIA'', ''SAREB'',''CERBERUS'',''CAJAMAR'',''LIBERBANK'',''DIVARIAN'',''APPLE'')) 
	AND T1.ALERTA_ALQUILER = T2.ALQUILADO 
	AND T1.ALERTA_CARGAS = T2.CARGAS 
	AND T1.ALERTA_INSCRIPCION = T2.INSCRIPCION
	AND T1.ALERTA_OKUPADO = T2.OCUPADO 
	AND T1.ALERTA_POSESION = T2.POSESION 
	AND T1.ALERTA_TIPO_ALQUILER = T2.TIPO_ALQUILER 
	ORDER BY T1.NUM_ACTIVO';
