...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW '|| V_ESQUEMA ||'.V_CONF_ALERTAS_PUBLICACION 
	...
	ON (T1.CARTERA = T2.CARTERA AND T1.SUBCARTERA = T2.SUBCARTERA AND T2.SUBCARTERA IS NOT NULL AND T2.SUBTIPO_TITULO = T2.SUBTIPO_TITULO AND T2.SUBTIPO_TITULO IS NOT NULL) 
	...
	OR (T1.CARTERA NOT IN (''BANKIA'', ''SAREB'',''CAJAMAR'',''LIBERBANK'') AND ((T1.CARTERA <> T2.CARTERA AND T1.CARTERA <> ''CERBERUS'') OR (T1.CARTERA =''CERBERUS'' AND T1.SUBCARTERA  NOT IN (''APPLE - INMOBILIARIO'',''DIVARIAN'')))  AND T2.CARTERA = ''OTRA CARTERA'') 
	';
