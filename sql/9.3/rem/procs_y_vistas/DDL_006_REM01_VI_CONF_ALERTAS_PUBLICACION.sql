...
--##	    0.6 REMVIP-11041 - Añadida subcartera Jaguar
--##########################################
--*/
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW '|| V_ESQUEMA ||'.V_CONF_ALERTAS_PUBLICACION 
	...
	OR (T1.CARTERA NOT IN (''CAIXABANK'',''BANKIA DACION'',''SAREB'',''CAJAMAR'',''UNICAJA'',''BBVA'') AND ((T1.CARTERA <> T2.CARTERA AND T1.CARTERA <> ''CERBERUS'' AND T1.CARTERA <> ''MACC'') OR (T1.CARTERA =''CERBERUS'' AND T1.SUBCARTERA  NOT IN (''APPLE - INMOBILIARIO'',''DIVARIAN INDUSTRIAL INMB'',''DIVARIAN REMAINING INMB'',''JAIPUR - FINANCIERO'',''JAIPUR - INMOBILIARIO'',''JAGUAR'')))  AND T2.CARTERA = ''OTRA CARTERA'')';
