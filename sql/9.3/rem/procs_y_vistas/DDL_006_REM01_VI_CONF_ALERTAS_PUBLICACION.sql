...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW '|| V_ESQUEMA ||'.V_CONF_ALERTAS_PUBLICACION 
	...
	T1.FASE_PUBLICACION,
	CASE WHEN
	((T1.ALERTA_INSCRIPCION * T2.INSCRIPCION  ) +
	...
	CASE WHEN (T1.ALERTA_ALQUILER * T2.ALQUILADO) = 1 OR (T1.ALERTA_TIPO_ALQUILER * T2.TIPO_ALQUILER) = 1 OR (T1.ALERTA_OKUPADO * T2.OCUPADO) = 1 THEN 1 ELSE 0 END AS INDICADOR_1,
	CASE WHEN (T1.ALERTA_POSESION * T2.POSESION) = 1 OR (T1.ALERTA_INSCRIPCION * T2.INSCRIPCION  ) = 1 OR (T1.ALERTA_CARGAS * T2.CARGAS) = 1  THEN 1 ELSE 0 END AS INDICADOR_2
	FROM '|| V_ESQUEMA ||'.V_ALERTAS_PUBLICACION  T1 
	INNER JOIN '|| V_ESQUEMA ||'.CONF_ALERTAS_PUBLICACION T2 
	ON (T1.CARTERA = T2.CARTERA AND T1.CARTERA <> ''CERBERUS'' AND T1.CARTERA = ''BANKIA''  AND T2.SUBTIPO_TITULO IS NULL AND T1.SUBTIPO_TITULO <> ''Notarial (dación)'')
   	OR (T1.CARTERA = T2.CARTERA AND T1.CARTERA <> ''CERBERUS'' AND T1.CARTERA = ''BANKIA'' AND T1.SUBTIPO_TITULO = T2.SUBTIPO_TITULO AND T2.SUBTIPO_TITULO IS NOT NULL AND T1.SUBTIPO_TITULO = ''Notarial (dación)'')  
    	OR (T1.CARTERA = T2.CARTERA AND T1.CARTERA <> ''CERBERUS'' AND T1.CARTERA <> ''BANKIA'' AND T2.SUBTIPO_TITULO IS NULL ) 
	OR (T1.SUBCARTERA = T2.SUBCARTERA AND T2.SUBCARTERA IS NOT NULL) 
	OR (T1.CARTERA NOT IN (''BANKIA'', ''SAREB'',''CAJAMAR'',''LIBERBANK'') AND ((T1.CARTERA <> T2.CARTERA AND T1.CARTERA <> ''CERBERUS'') OR (T1.CARTERA =''CERBERUS'' AND T1.SUBCARTERA  NOT IN (''APPLE - INMOBILIARIO'',''DIVARIAN REMAINING INMB'',''DIVARIAN ARROW INMB'',''JAIPUR - FINANCIERO'',''JAIPUR - INMOBILIARIO'')))  AND T2.CARTERA = ''OTRA CARTERA'') 
	';
