...
BEGIN
  ...
  AS
    SELECT GPV.GPV_ID, SUM(NVL(GLD_PRINCIPAL_SUJETO,0) + NVL(GLD_PRINCIPAL_NO_SUJETO,0) + NVL(GLD_RECARGO,0) + NVL(GLD_INTERES_DEMORA,0) + NVL(GLD_COSTAS,0) + NVL(GLD_OTROS_INCREMENTOS,0) + NVL(GLD_PROV_SUPLIDOS,0)) AS IMPORTE_BRUTO
