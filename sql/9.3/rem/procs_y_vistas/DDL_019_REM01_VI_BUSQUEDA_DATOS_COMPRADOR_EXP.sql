...
--##		0.17 Versión - REMVIP-10984  - Jesus Jativa - Correccion obtencion de los campos creados en HREOS-16425.
--##########################################
--*/
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.V_BUSQUEDA_DATOS_COMPRADOR_EXP 
	AS
		SELECT
			...
			COM.COM_PRP,
			VIC.DD_VIC_CODIGO,
			PAINCOM.DD_PAI_CODIGO AS DD_PAI_CODIGO_NAC_COM,
			PAINCOM.DD_PAI_DESCRIPCION AS DD_PAI_DESCRIPCION_NAC_COM,
			CEX.CEX_PRP,
			PRVRPR.DD_PRV_CODIGO AS DD_PRV_CODIGO_NAC_REP,
			PRVRPR.DD_PRV_DESCRIPCION AS DD_PRV_DESCRIPCION_NAC_REP,			
			...
			NACION.DD_PAI_CODIGO AS CEX_NACIONALIDAD_PAI,
			NACIONREP.DD_PAI_CODIGO AS CEX_NACIONALIDAD_PAI_RPR,
		...
		FROM REM01.COM_COMPRADOR COM
			...
			LEFT JOIN '|| V_ESQUEMA_MASTER ||'.DD_PRV_PROVINCIA PRVRPR ON CEX.DD_PRV_NAC_ID_REP = PRVRPR.DD_PRV_ID
			LEFT JOIN '|| V_ESQUEMA_MASTER ||'.DD_PRV_PROVINCIA PRVCOM ON COM.DD_PRV_NAC_ID = PRVCOM.DD_PRV_ID
			...
			LEFT JOIN '|| V_ESQUEMA ||'.DD_PAI_PAISES PAINCOM ON COM.DD_PAI_NAC_ID = PAINCOM.DD_PAI_ID
			LEFT JOIN '|| V_ESQUEMA ||'.DD_PAI_PAISES NACION ON CEX.DD_PAI_ID_ISO = NACION.DD_PAI_ID
			LEFT JOIN '|| V_ESQUEMA ||'.DD_PAI_PAISES NACIONREP ON CEX.DD_PAI_ID_ISO_RPR = NACIONREP.DD_PAI_ID';
