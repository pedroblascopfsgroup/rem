  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_BUSQUEDA_GASTOS_PROVEEDOR 
	AS
		SELECT
	        ...
	        TBJ_GLD.TBJ_NUM_TRABAJO AS TBJ_NUM_TRABAJO
            
		FROM ' || V_ESQUEMA || '.GPV_GASTOS_PROVEEDOR GPV
        INNER JOIN (SELECT GLD.GPV_ID, NVL2(GLD.DD_TIT_ID, 1, 0) DD_TIT_ID, ROW_NUMBER() OVER(PARTITION BY GPV.GPV_ID ORDER BY NVL2(GLD.DD_TIT_ID, 1, 0) DESC) RN
          FROM ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE GLD
          JOIN ' || V_ESQUEMA || '.GPV_GASTOS_PROVEEDOR GPV ON GPV.GPV_ID = GLD.GPV_ID
            AND GPV.BORRADO = 0
          WHERE GLD.BORRADO = 0
          ) GLD_TIT ON GPV.GPV_ID = GLD_TIT.GPV_ID
              AND GLD_TIT.RN = 1
    INNER JOIN ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE GLD ON GLD.GPV_ID = GPV.GPV_ID
      ...
      AND PRO.BORRADO = 0
		...
		LEFT JOIN (SELECT TBJ.TBJ_NUM_TRABAJO, GLD.GPV_ID 
					FROM ' || V_ESQUEMA || '.GLD_TBJ GLDTBJ
                    INNER JOIN ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE GLD ON GLD.GLD_ID = GLDTBJ.GLD_ID
                      AND GLD.BORRADO = 0
                    JOIN ' || V_ESQUEMA || '.ACT_TBJ_TRABAJO TBJ ON GLDTBJ.TBJ_ID = TBJ.TBJ_ID 
                      AND TBJ.BORRADO = 0
                    WHERE GLDTBJ.BORRADO = 0) TBJ_GLD ON TBJ_GLD.GPV_ID = GPV.GPV_ID
		WHERE GPV.BORRADO = 0 ';
