...
DECLARE
    ...
    V_TEXT_VISTA VARCHAR2(2400 CHAR) := 'VI_PIVOT_SUP_DISTRIBUCION'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.'|| V_TEXT_VISTA ||'
	AS
		SELECT ICO_ID, MTS_DORMITORIOS, MTS_BANYOS, MTS_ASEOS, MTS_GARAJES, MTS_TERRAZAS_CUBIERTAS, MTS_TERRAZAS_DESCUBIERTAS, MTS_TRASTEROS
		FROM (SELECT  DIS.ICO_ID, TPH.DD_TPH_CODIGO, DIS_SUPERFICIE
				FROM '|| V_ESQUEMA ||'.ACT_DIS_DISTRIBUCION DIS 
				INNER JOIN '|| V_ESQUEMA ||'.DD_TPH_TIPO_HABITACULO TPH ON DIS.DD_TPH_ID = TPH.DD_TPH_ID)
		PIVOT(SUM(DIS_SUPERFICIE) FOR DD_TPH_CODIGO IN (''01'' MTS_DORMITORIOS, ''02'' MTS_BANYOS, ''04'' MTS_ASEOS, ''12'' MTS_GARAJES, ''15'' MTS_TERRAZAS_CUBIERTAS,''16'' MTS_TERRAZAS_DESCUBIERTAS, ''11'' MTS_TRASTEROS))';
