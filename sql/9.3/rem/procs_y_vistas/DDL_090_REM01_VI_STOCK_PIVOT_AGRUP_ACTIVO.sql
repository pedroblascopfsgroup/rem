  ...
  EXECUTE IMMEDIATE 'CREATE MATERIALIZED VIEW ' || V_ESQUEMA || '.'|| V_TEXT_VISTA ||' 
	AS
		...
		FROM (
		    ...
		    JOIN '|| V_ESQUEMA ||'.DD_TAG_TIPO_AGRUPACION TAG ON (TAG.DD_TAG_ID = AGR.DD_TAG_ID AND TAG.DD_TAG_CODIGO IN (''01'',''02'',''13'',''17'',''18''))
		    WHERE AGR.BORRADO = 0 AND AGR.AGR_FECHA_BAJA IS NULL
		) PIVOT (MAX(AGR_NUM_AGRUP_REM) AS NUM_REM, MAX(PRINCIPAL) AS PRINCIPAL, MAX(PISO_PILOTO) AS ES_PISO_PILOTO, MAX(AGA_FECHA_ESCRITURACION) AS FECHA_ESCRITURACION
