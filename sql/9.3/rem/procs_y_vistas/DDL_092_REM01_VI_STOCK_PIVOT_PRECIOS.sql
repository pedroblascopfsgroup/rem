...
--## VERSIONES:
--##        0.1 Versión inicial
--##        0.2 [HREOS-14530] - Añadimos precio: MINIMO AUTORIZADO
--##########################################
--*/
	...
	AS
  		WITH PIVOT_DATA AS (
			SELECT * FROM (
		        ...
		        JOIN '||V_ESQUEMA||'.DD_TPC_TIPO_PRECIO TPC ON (VAL.DD_TPC_ID = TPC.DD_TPC_ID AND TPC.DD_TPC_CODIGO IN (''02'',''03'', ''04'', ''13''))
		        WHERE VAL.BORRADO = 0
			) 
		)
		SELECT ACT_ID, 
