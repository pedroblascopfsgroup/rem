       ...
       v.descripcion_otros                                                                  AS descripcion_otros,
        ...
        LEFT JOIN
           (SELECT vi.act_id,
              ...
              FROM '||V_ESQUEMA||'.v_cond_disponibilidad vi
              LEFT JOIN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO pac on pac.act_id = vi.act_id) v ON v.act_id = actpub.act_id
        ...
        LEFT JOIN '||V_ESQUEMA||'.ACT_LLV_LLAVE LLV ON LLV.ACT_ID = ACT.ACT_ID AND LLV.BORRADO = 0 AND (LLV.LLV_COD_TENEDOR_POSEEDOR IS NOT NULL OR NVL(LLV.LLV_COD_TENEDOR_NO_PVE, '' '') <> ''N/A'')
