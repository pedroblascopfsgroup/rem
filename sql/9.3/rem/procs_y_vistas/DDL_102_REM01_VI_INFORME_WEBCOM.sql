   ...
   -- Creamos vista materializada
	...
	AS
		...
		FECHA_RECEP_INFO AS (
            ...
            FROM '||V_ESQUEMA_M||'.ACT_ICO_INFO_COMERCIAL ICO
            JOIN '||V_ESQUEMA_M||'.ACT_ACTIVO ACT ON ACT.ACT_ID = ICO.ACT_ID AND ACT.BORRADO = 0
            JOIN '||V_ESQUEMA_M||'.ACT_HFP_HIST_FASES_PUB HFP ON HFP.ACT_ID = ACT.ACT_ID
            JOIN '||V_ESQUEMA_M||'.DD_FSP_FASE_PUBLICACION FSP ON HFP.DD_FSP_ID = FSP.DD_FSP_ID AND FSP.DD_FSP_CODIGO = ''05''
            JOIN '||V_ESQUEMA_M||'.DD_SFP_SUBFASE_PUBLICACION SFP ON SFP.DD_SFP_ID = HFP.DD_SFP_ID AND SFP.DD_SFP_CODIGO = ''09''
        )
		SELECT 
			...
			CASE WHEN (FRI.FECHA_RECEPCION_INFORME IS NOT NULL) 
