...
--##		1.1: 20220610 Remus Ovidiu	REMVIP-11720 -- Corregir campo Admite Mascotas
--##########################################
--*/
   ...
   -- Creamos vista materializada
	...
	AS
		...
		SELECT 
			...
			CASE WHEN (ESI.DD_AIC_CODIGO IS NOT NULL)
				THEN CAST(ESI.DD_AIC_CODIGO AS VARCHAR2(20 CHAR))
				ELSE CAST(''03'' AS VARCHAR2(20 CHAR)) 
           ...
           CAST(PIS.DD_DIS_CODIGO AS VARCHAR2(5 CHAR)) AS PISCINA,
            ...
            CAST(ICO.ICO_NUM_PLANTAS_EDI AS NUMBER(11))         AS NUMERO_PLANTAS_EDIFICIO,
            ...
            CASE MASC.DD_SIN_CODIGO
				WHEN ''01'' THEN CAST(''01'' AS VARCHAR2(5 CHAR))
				WHEN ''02'' THEN CAST(''02'' AS VARCHAR2(5 CHAR))
				ELSE CAST(''03'' AS VARCHAR2(5 CHAR))
            ...
            CAST(AAC.DD_AAC_CODIGO AS VARCHAR2(5 CHAR))     AS COD_ACCESIBILIDAD,  
			...
			FROM '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL ICO
			INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = ICO.ACT_ID AND ACT.BORRADO = 0
			...
			LEFT JOIN '||V_ESQUEMA||'.DD_TVP_TIPO_VPO DDTVP ON DDTVP.DD_TVP_ID = ADM.DD_TVP_ID AND DDTVP.BORRADO = 0
