   ...
   -- Creamos vista materializada
	...
	AS
		WITH INFO_ACTIVO_AGRUPACION AS (
			SELECT AUX.* FROM (
				SELECT AGA.AGR_ID, 
						...
						PVE.PVE_COD_REM, 
						AGA.PISO_PILOTO,
				ROW_NUMBER() OVER (PARTITION BY AGA.AGR_ID ORDER BY AGA.PISO_PILOTO DESC NULLS LAST, ACT.ACT_ID) ORDEN
		...
		DIRECCION_AGRUPA AS (
			...
			UNION
			SELECT RES.AGR_ID, RES.RES_CP AS CP, DDPRV.DD_PRV_CODIGO, DDLOC.DD_LOC_CODIGO
			FROM '||V_ESQUEMA||'.ACT_RES_RESTRINGIDA RES
			LEFT JOIN '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA DDPRV ON DDPRV.DD_PRV_ID = RES.DD_PRV_ID
			LEFT JOIN '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD DDLOC ON DDLOC.DD_LOC_ID = RES.DD_LOC_ID
			UNION
			SELECT RAL.AGR_ID, RAL.RAL_CP AS CP, DDPRV.DD_PRV_CODIGO, DDLOC.DD_LOC_CODIGO
			FROM '||V_ESQUEMA||'.ACT_RAL_RESTRINGIDA_ALQUILER RAL
			LEFT JOIN '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA DDPRV ON DDPRV.DD_PRV_ID = RAL.DD_PRV_ID
			LEFT JOIN '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD DDLOC ON DDLOC.DD_LOC_ID = RAL.DD_LOC_ID
			UNION
			SELECT ROB.AGR_ID, ROB.ROB_CP AS CP, DDPRV.DD_PRV_CODIGO, DDLOC.DD_LOC_CODIGO
			FROM '||V_ESQUEMA||'.ACT_ROB_RESTRINGIDA_OBREM ROB
			LEFT JOIN '||V_ESQUEMA_M||'.DD_PRV_PROVINCIA DDPRV ON DDPRV.DD_PRV_ID = ROB.DD_PRV_ID
			LEFT JOIN '||V_ESQUEMA_M||'.DD_LOC_LOCALIDAD DDLOC ON DDLOC.DD_LOC_ID = ROB.DD_LOC_ID
		)
		SELECT 
		...
		CAST(TO_CHAR(NVL(AGR.FECHAMODIFICAR, AGR.AGR_FECHA_PREV_FINALIZACION), 
					''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2 (50 CHAR)) 						 AS AGR_FECHA_PREV_FINALIZACION,
		CAST(TO_CHAR(NVL(AGR.FECHAMODIFICAR, AGR.AGR_INI_VIGENCIA), 
					''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2 (50 CHAR)) 						 AS AGR_FECHA_ESCRITURA_DESDE,
		CAST(TO_CHAR(NVL(AGR.FECHAMODIFICAR, AGR.AGR_FECHA_ESCRITURA_HASTA), 
					''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2 (50 CHAR)) 						 AS AGR_FECHA_ESCRITURA_HASTA
