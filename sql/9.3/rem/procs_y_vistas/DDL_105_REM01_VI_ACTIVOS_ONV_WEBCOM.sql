   ...
   -- Creamos vista materializada
	...
	AS
		WITH SUBDIVISION AS (
			...
			WHERE AGR.AGR_NUM_AGRUP_REM IS NOT NULL AND TAG.DD_TAG_CODIGO IN (''01'', ''16'')
		...
		and aga.borrado = 0';		   
 		...
 		V_MSQL := 'CREATE UNIQUE INDEX '||V_ESQUEMA||'.'||V_TEXT_VISTA||'_IDX ON '||V_ESQUEMA|| '.'||V_TEXT_VISTA||'(ID_AGRUP_ACTIVO) TABLESPACE '||V_TABLESPACE_IDX;		
