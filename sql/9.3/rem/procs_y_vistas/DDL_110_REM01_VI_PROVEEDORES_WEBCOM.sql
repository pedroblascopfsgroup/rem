	...
	AS
	    ...
	    PROVINCIA_ACTUA_DELEG AS ( 
	        SELECT PROV.* FROM (
                ...
                WHERE PRD.BORRADO = 0 GROUP BY PRD.PRD_ID, PRD.PVE_ID) PROV
		),
	    ...
	    CONDUCTAS_INAPROPIADAS AS ( 
	        SELECT COI.* FROM (
                SELECT COI.COI_ID, COI.COI_PVE, COI.FECHACREAR AS FECHA, COI.USUARIOCREAR AS USUARIO,
	    ...
	    CONTACTO AS ( 
	        SELECT PVE_CONTACTO.* FROM (
                SELECT PVE_ID, PVC_NOMBRE,PVC_APELLID01,PVC_APELLID02,
                ROW_NUMBER() OVER (PARTITION BY PVE_ID ORDER BY CASE WHEN PVC_PRINCIPAL = 1 THEN 0 ELSE 1 END, PVC_ID) ORDEN
                FROM '||V_ESQUEMA||'.ACT_PVC_PROVEEDOR_CONTACTO WHERE BORRADO = 0) PVE_CONTACTO
           WHERE PVE_CONTACTO.ORDEN = 1
		)
		SELECT
		...
		CAST(TO_CHAR(PVE.PVE_FECHA_BAJA, 
                    ''YYYY-MM-DD"T"HH24:MM:SS'') AS VARCHAR2 (50 CHAR)) 		                AS FECHA_BAJA,
		CAST(EPR.DD_EPR_CODIGO AS VARCHAR2(20 CHAR)) 			                                AS COD_ESTADO,
		CAST(TPE.DD_TPE_CODIGO AS VARCHAR2(20 CHAR)) 			                                AS COD_TIPO_PERSONA,
		CAST(PVE.PVE_PAGINA_WEB AS VARCHAR2(50 CHAR)) 			                            	AS URL_WEB,
		CAST(CTO.PVC_NOMBRE AS VARCHAR2(256 CHAR)) 			                            		AS NOMBRE_CONTACTO,
		CAST(CTO.PVC_APELLID01 AS VARCHAR2(50 CHAR)) 			                            	AS APELLIDO1_CONTACTO,
		CAST(CTO.PVC_APELLID02 AS VARCHAR2(50 CHAR)) 			                            	AS APELLIDO2_CONTACTO
		...
		LEFT JOIN CONTACTO CTO ON CTO.PVE_ID = PVE.PVE_ID
