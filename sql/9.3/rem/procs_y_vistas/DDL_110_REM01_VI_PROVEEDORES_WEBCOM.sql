   ...
   -- Creamos vista materializada
	...
	WITH DOMICILIO_SOCIAL AS (
		SELECT AUX.* FROM (
			...
			ROW_NUMBER() OVER (PARTITION BY PRD.PVE_ID, DDTDP.DD_TDP_ID ORDER BY PRD.PRD_ID DESC) ORDEN
		...
		CAST((SELECT count(*) FROM '||V_ESQUEMA||'.ACT_PRD_PROVEEDOR_DIRECCION 
			WHERE DD_TDP_ID = (SELECT DD_TDP_ID FROM '||V_ESQUEMA||'.DD_TDP_TIPO_DIR_PROVEEDOR 
			WHERE DD_TDP_CODIGO = ''02'' AND PVE_ID = DEL.PVE_ID)) AS NUMBER (16, 0)) 			AS NUMERO_DELEGACIONES,
