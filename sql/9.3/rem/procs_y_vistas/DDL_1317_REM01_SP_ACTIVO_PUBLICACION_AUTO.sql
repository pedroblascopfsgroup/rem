...
BEGIN
  DBMS_OUTPUT.PUT_LINE('[INFO] Procedure PROCEDURE ACTIVO_PUBLICACION_AUTO: INICIANDO...');	     	 
    ...
    -- que indicar el tipo de dato (resistente a cambios de tipo en las vistas)
        ...
        OPEN V_PUBLICAR FOR
          SELECT ACT_ID, ESTADO_PUBLICACION_CODIGO, CONDICIONADO from REM01.V_ACTIVO_PUBLI_MAN WHERE ACT_ID = ACT_ID_PARAM AND (ESTADO_PUBLICACION_CODIGO IS NULL OR ESTADO_PUBLICACION_CODIGO <> ''05'' AND ACT_FECHA_IND_PUBLICABLE IS NOT NULL)
          UNION
          SELECT ACT_ID, ESTADO_PUBLICACION_CODIGO, CONDICIONADO from REM01.V_ACTIVO_SIN_PRECIO_PUBLI_MAN WHERE ACT_ID = ACT_ID_PARAM AND (ESTADO_PUBLICACION_CODIGO IS NULL OR ESTADO_PUBLICACION_CODIGO <> ''05'' AND ACT_FECHA_IND_PUBLICABLE IS NOT NULL);
      ELSE
	    	OPEN V_PUBLICAR FOR
          SELECT ACT_ID, ESTADO_PUBLICACION_CODIGO, CONDICIONADO from REM01.V_ACTIVO_PUBLI_MAN WHERE (ESTADO_PUBLICACION_CODIGO IS NULL OR ESTADO_PUBLICACION_CODIGO <> ''05'' AND ACT_FECHA_IND_PUBLICABLE IS NOT NULL)
          UNION
          SELECT ACT_ID, ESTADO_PUBLICACION_CODIGO, CONDICIONADO from REM01.V_ACTIVO_SIN_PRECIO_PUBLI_MAN WHERE (ESTADO_PUBLICACION_CODIGO IS NULL OR ESTADO_PUBLICACION_CODIGO <> ''05'' AND ACT_FECHA_IND_PUBLICABLE IS NOT NULL);
	    END IF;
