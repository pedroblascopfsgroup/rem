...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_BUSQUEDA_GASTOS_PROVEEDOR_E
	AS
      ...
      GLD2.DD_TIT_ID AS SUJETO_IMPUESTO_INDIRECTO,
      EAH.DD_EAH_CODIGO,
      ...
      GLD2.GLD_CPP_BASE AS PTDA_PRESUPUESTARIA,
      GLD2.GLD_CCC_BASE AS CONCEPTO_CONTABLE,
      PRG.PRG_NUM_PROVISION	AS PROVISION_FONDOS
		FROM ' || V_ESQUEMA || '.GPV_GASTOS_PROVEEDOR GPV
        INNER JOIN (SELECT GLD.GPV_ID
            , NVL2(GLD.DD_TIT_ID, 1, 0) DD_TIT_ID
            , NVL(GLD.GLD_CPP_BASE,0) GLD_CPP_BASE
            , NVL(GLD.GLD_CCC_BASE,0) GLD_CCC_BASE
            , ROW_NUMBER() OVER(PARTITION BY GPV.GPV_ID ORDER BY NVL2(GLD.DD_TIT_ID, 1, 0) DESC) RN
          FROM ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE GLD
          JOIN ' || V_ESQUEMA || '.GPV_GASTOS_PROVEEDOR GPV ON GPV.GPV_ID = GLD.GPV_ID
            AND GPV.BORRADO = 0
          WHERE GLD.BORRADO = 0
          ) GLD2 ON GPV.GPV_ID = GLD2.GPV_ID
            AND GLD2.RN = 1
    INNER JOIN ' || V_ESQUEMA || '.GLD_GASTOS_LINEA_DETALLE GLD ON GLD.GPV_ID = GPV.GPV_ID
      AND GLD.BORRADO = 0
    ...
    LEFT JOIN (
      SELECT GLDACT_AUX.GLD_ENT_ID
          , GLDACT_AUX.GLD_ID
          , GLDACT_AUX.ENT_ID
          , GLDACT_AUX.GLD_PARTICIPACION_GASTO
          , GLDACT_AUX.GLD_REFERENCIA_CATASTRAL  
        FROM ' || V_ESQUEMA || '.GLD_ENT GLDACT_AUX 
        JOIN ' || V_ESQUEMA || '.DD_ENT_ENTIDAD_GASTO ENT ON GLDACT_AUX.DD_ENT_ID = ENT.DD_ENT_ID
        WHERE GLDACT_AUX.BORRADO = 0 AND ENT.DD_ENT_CODIGO = ''ACT'') GLDACT ON GLD.GLD_ID = GLDACT.GLD_ID
