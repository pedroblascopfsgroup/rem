...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.VI_NOR_BBVA_VT1 
	...
	,CASE 
	    ...
	    WHEN TGP.DD_TGP_CODIGO IN (''04'',''06'')  AND SAC.DD_SAC_CODIGO IN (''24'',''25'') THEN ''73'' /*GARAJE-TRASTERO TERCIARIO*/
	    WHEN TUD.DD_TUD_CODIGO = ''02'' AND SAC.DD_SAC_CODIGO = ''24'' THEN ''73'' /*GARAJE-TRASTERO TERCIARIO*/
	    WHEN SAC.DD_SAC_CODIGO = ''24'' AND  EAC.DD_EAC_CODIGO NOT IN (''06'')  THEN ''72'' /*GARAJE*/
	    WHEN DD_TIB.DD_TIB_CODIGO IS NOT NULL THEN DD_TIB.DD_TIB_CODIGO
	    ELSE DD_TIB2.DD_TIB_CODIGO 
	END AS TIPO_INMUEBLE
        ...
        LEFT JOIN '||V_ESQUEMA||'.DD_TUD_TIPO_USO_DESTINO TUD ON TUD.DD_TUD_ID = ACT.DD_TUD_ID AND TUD.BORRADO = 0
        LEFT JOIN '||V_ESQUEMA||'.DD_TIB_TIPOLOGIA_INMUEBLE DD_TIB
        on DD_TIB.DD_EAC_ID = ACT.DD_EAC_ID AND DD_TIB.DD_EAC_ID IS NOT NULL AND DD_TIB.DD_TPA_ID = ACT.DD_TPA_ID AND DD_TIB.DD_SAC_ID = ACT.DD_SAC_ID 
        LEFT JOIN '||V_ESQUEMA||'.DD_TIB_TIPOLOGIA_INMUEBLE DD_TIB2
        on  DD_TIB2.DD_EAC_ID IS NULL AND DD_TIB2.DD_TPA_ID = ACT.DD_TPA_ID AND DD_TIB2.DD_SAC_ID = ACT.DD_SAC_ID
