...
BEGIN
  ...
  V_MSQL := 'CREATE OR REPLACE FORCE EDITIONABLE VIEW ' || V_ESQUEMA || '."V_ACTIVOS_AGRUPACION" 
	...
	"SDV_NOMBRE", "PUBLICADO", "VAL_IMPORTE_DESCUENTO_PUBLICO", "GENCAT", "COND_PUBL_VENTA", "COND_PUBL_ALQUILER", "ES_PISO_PILOTO", "AGA_FECHA_ESCRITURACION") AS 
	WITH VAL_VALORACIONES (ACT_ID, VAL_IMPORTE_APROBADO_VENTA, VAL_IMPORTE_APROBADO_RENTA, VAL_IMPORTE_MINIMO_AUTORIZADO, VAL_IMPORTE_DESCUENTO_PUBLICO)
        ...
        ||CASE WHEN TPA.DD_TPA_CODIGO = ''02'' AND NVL(ICO.ICO_NUM_PLANTAS,0) = 1 THEN '' - 1 PLANTA''
               WHEN TPA.DD_TPA_CODIGO = ''02'' AND NVL(ICO.ICO_NUM_PLANTAS,0) <> 0 THEN '' - ''||NVL(ICO.ICO_NUM_PLANTAS,0)||'' PLANTAS''
          END
        ||CASE WHEN TPA.DD_TPA_CODIGO = ''02'' AND NVL(ICO.ICO_NUM_DORMITORIOS,0) = 1 THEN '' - 1 DORMITORIO''
               WHEN TPA.DD_TPA_CODIGO = ''02'' AND NVL(ICO.ICO_NUM_DORMITORIOS,0) <> 1 THEN '' - ''||NVL(ICO.ICO_NUM_DORMITORIOS,0)||'' DORMITORIOS''
          END
        ||CASE WHEN TPA.DD_TPA_CODIGO = ''02'' AND NVL(ICO.ICO_NUM_BANYOS,0) = 1 THEN '' - 1 BAÑO''
               WHEN TPA.DD_TPA_CODIGO = ''02'' AND NVL(ICO.ICO_NUM_BANYOS,0) <> 1 THEN '' - ''||NVL(ICO.ICO_NUM_BANYOS,0)||'' BAÑOS''
          END
        AS SDV_NOMBRE,
		...
		TO_DATE(AGR.AGA_FECHA_ESCRITURACION,''DD/MM/YY'') AGA_FECHA_ESCRITURACION
