...
BEGIN	
	...
	ELSIF INTERFAZ IS NOT NULL AND (INTERFAZ IN ('CXB', 'BC', 'RESTO'))  THEN
		...
		V_SQL := 'INSERT INTO '||V_ESQUEMA||'.H_REENVIO_GASTOS (GPV_ID, USUARIO, FECHA, DD_EGA_ID, DD_EAH_ID, DD_EAP_ID
				, GGE_FECHA_EAH, GGE_FECHA_EAP, GGE_CLIENTE_PAGADOR, GGE_FECHA_ENVIO_PRPTRIO, GGE_CLIENTE_INFORMADOR, GGE_FECHA_ENVIO_INFORMATIVA)
			SELECT GPV.GPV_ID, '''||V_USUARIO_MODIFICAR||''' USUARIO, SYSDATE FECHA, GPV.DD_EGA_ID, GGE.DD_EAH_ID, GGE.DD_EAP_ID
				, GGE.GGE_FECHA_EAH, GGE.GGE_FECHA_EAP, GGE.GGE_CLIENTE_PAGADOR, GGE.GGE_FECHA_ENVIO_PRPTRIO, GGE.GGE_CLIENTE_INFORMADOR, GGE.GGE_FECHA_ENVIO_INFORMATIVA
			FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
			...
			IF INTERFAZ = 'BC' THEN
				...
				V_SQL := 'SELECT COUNT(1)
					...
					JOIN '||V_ESQUEMA||'.GGE_GASTOS_GESTION GGE ON GGE.GPV_ID = TMP.GPV_ID
						AND GGE.BORRADO = 0
					WHERE GGE.GGE_CLIENTE_PAGADOR IS NOT NULL AND GGE.GGE_CLIENTE_PAGADOR <> '||V_CLIENTE||'';
				EXECUTE IMMEDIATE INTO V_NUM_TABLAS;
				PL_OUTPUT := PL_OUTPUT || '	[WARNING]: REVISAR ' || V_NUM_TABLAS || ' GASTOS A PRIMERA HORA DE MAÑANA QUE SE INTENTARÁN REENVIAR A BC Y SON DE CLIENTE PAGADOR CAIXA. (SACARLOS DE H_REENVIO_GASTOS DE ESTA EJECUCIÓN).'|| CHR(10);
