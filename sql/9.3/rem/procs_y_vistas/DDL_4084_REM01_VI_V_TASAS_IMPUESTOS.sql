...
DECLARE
    ...
    V_TEXT_VISTA VARCHAR2(2400 CHAR) := 'V_TASAS_IMPUESTOS'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW ' || V_ESQUEMA || '.'|| V_TEXT_VISTA ||'
	AS SELECT PVE_ID||GPV_ID ID_VISTA,SOCIEDAD,VISTA,RANK() OVER (ORDER BY ORDEN ASC) AS ORDEN,CIF,CODIGO,NUM_FRA,FECHA_FRA,FECHA_CONTABLE,DIARIO_CONTB,IMP_BRUTO,TOTAL,OP_ALQ,D347,TIPO_FRA,SUJ_RECC,DELEGACION,BASE_RETENCION,PORCENTAJE_RETENCION,IMPORTE_RETENCION,APLICAR_RETENCION,BASE_IRPF,PORCENTAJE_IRPF,IMPORTE_IRPF,CLAVE_IRPF,SUBCLAVE_IRPF,CEUTA,CONCEPTO,CTA_ACREEDORA,SCTA_ACREEDORA,CTA_GARANTIA,SCTA_GARANTIA
,CTA_IRPF,SCTA_IRPF,CTA_IVAD,SCTA_IVAD,CONDICIONES,PAGADA,CTA_BANCO,SCTA_BANCO,CTA_EFECTOS,SCTA_EFECTOS,APUNTE,CENTRODESTINO,AUTOREPE_INVE_SUJE_PASI,SERIE_AUTOREPE,DIARIO_AUTOREPE,TIPO_FRA_SII,CLAVE_RE,CLAVE_RE_AD1,CLAVE_RE_AD2,TIPO_OP_INTRA,DESC_BIENES,DESCRIPCION_OP,SIMPLIFICADA
,FRA_SIMPLI_IDEN,DIARIO1,BASE1,IVA1,CUOTA1,DIARIO2,BASE2,IVA2,CUOTA2,DIARIO3,BASE3,IVA3,CUOTA3,DIARIO4,BASE4,IVA4,CUOTA4,DIARIO5,BASE5,IVA5,CUOTA5,PROYECTO,TIPO_INMUEBLE,CLAVE1,CLAVE2,CLAVE3,CLAVE4
,ID_ACTIVO,CONCEPTO_GTO_CTE,IMPORTE_GASTO,TIPO_PARTIDA,APARTADO,CAPITULO,PARTIDA,CTA_GASTO,SCTA_GASTO,REPERCUTIR,CONCEPTO_FAC,FECHA_FAC,COD_COEF,NOMBRE,CARACTERISTICA,CODI_DIAR_IVA_V,PCTJE_IVA_V
...
SELECT
    ...
    , CASE
			WHEN PVE.PVE_IVA_CAJA = 1 AND GLD.DD_TIT_ID IS NOT NULL THEN ''D'' --
			WHEN GLD.DD_TIT_ID IS NOT NULL AND COALESCE(PVE.PVE_IVA_CAJA,0) <> 1  THEN ''F''
			ELSE ''C''
	  END											AS  TIPO_FRA
    ...
    , COALESCE(GPL.GPL_BASE_IRPF,
		CASE	WHEN GDE.GDE_IRPF_TIPO_IMPOSITIVO  IS NOT NULL THEN GLD.GLD_PRINCIPAL_SUJETO
	...
	, NULL											AS	AUTOREPE_INVE_SUJE_PASI
	, NULL											AS	SERIE_AUTOREPE
	, NULL											AS	DIARIO_AUTOREPE
...
FROM
    ...
    LEFT JOIN '|| V_ESQUEMA ||'.GDE_GASTOS_DETALLE_ECONOMICO GDE
        ON GPV.GPV_ID = GDE.GPV_ID
            AND COALESCE(GDE.BORRADO,0)= 0

    LEFT JOIN '|| V_ESQUEMA ||'.GIC_GASTOS_INFO_CONTABILIDAD GIC
	...
	LEFT JOIN (SELECT GPV_NUM_GASTO_HAYA, sum (a.gpl_importe_gasto) AS SUMA
		...
		WHERE 1=1
		AND GIC.GIC_FECHA_CONTABILIZACION IS NULL AND TGA.DD_TGA_CODIGO IN (''01'',''02'',''03'',''04'')
   ...
   SELECT
	...
	FROM '|| V_ESQUEMA ||'.GPV_GASTOS_PROVEEDOR GPV
		...
		LEFT JOIN '|| V_ESQUEMA ||'.GDE_GASTOS_DETALLE_ECONOMICO GDE
		 ...
		 LEFT JOIN '|| V_ESQUEMA ||'.GIC_GASTOS_INFO_CONTABILIDAD GCONT
