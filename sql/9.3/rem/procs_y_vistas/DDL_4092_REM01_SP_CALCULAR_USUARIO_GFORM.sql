...
CREATE OR REPLACE PROCEDURE #ESQUEMA#.SP_CALCULAR_USUARIO_GFORM (P_ACT_ID IN NUMBER, P_OFR_ID IN NUMBER) AUTHID CURRENT_USER AS		
    ...
    V_MSQL := 'SELECT CRA.DD_CRA_CODIGO FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT JOIN '||V_ESQUEMA||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID WHERE ACT_ID = '|| P_ACT_ID ||'';
    
    EXECUTE IMMEDIATE V_MSQL INTO V_CRA_ID;
    
	    IF P_OFR_ID IS NOT NULL THEN
	    
	     	V_MSQL := 'SELECT OFR.OFR_IMPORTE FROM '||V_ESQUEMA||'.OFR_OFERTAS OFR WHERE OFR_ID = '|| P_OFR_ID ||'';  
	     
	        EXECUTE IMMEDIATE V_MSQL INTO V_OFR_ID;
	        
	        V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT JOIN '||V_ESQUEMA||'.ACT_OFR ACO ON ACT.ACT_ID = ACO.ACT_ID JOIN '||V_ESQUEMA||'.OFR_OFERTAS OFR ON OFR.OFR_ID = ACO.OFR_ID WHERE OFR.OFR_ID = '|| P_OFR_ID ||' AND ACT.ACT_ID = '|| P_ACT_ID ||'';  
	     	EXECUTE IMMEDIATE V_MSQL INTO V_ACO_ID;
    	    
    	    END IF;
    	    
    IF (V_CRA_ID = '16' AND V_OFR_ID >= 200000 AND V_ACO_ID > 0) THEN
    
    	 SELECT USU_ID
          INTO USUARIO
          FROM REMMASTER.USU_USUARIOS
         WHERE USU_USERNAME = 'ebenitezt'
           AND BORRADO = 0;    
