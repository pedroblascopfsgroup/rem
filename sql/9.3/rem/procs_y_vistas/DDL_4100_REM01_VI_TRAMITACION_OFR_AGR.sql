...
BEGIN
  ...
  V_MSQL:= 'CREATE VIEW '||V_ESQUEMA||'.'||V_TEXT_VISTA||' AS 
    ...
    LEFT JOIN '|| V_ESQUEMA ||'.CON_CONCURRENCIA CON ON CON.ACT_ID = AGA.ACT_ID
    WHERE AGA.BORRADO = 0
    AND EOF.DD_EOF_CODIGO IN (''03'', ''04'') AND (CON.CON_ID IS NULL OR TRUNC(SYSDATE) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN))
    AND AGA.AGR_ID NOT IN (
      ...
      WHERE EOF.DD_EOF_CODIGO = ''01''
    	...
    	JOIN '|| V_ESQUEMA ||'.ACT_AGA_AGRUPACION_ACTIVO AGA2 ON AGA2.ACT_ID = CON2.ACT_ID
    WHERE TRUNC(SYSDATE) BETWEEN TRUNC(CON.CON_FECHA_INI) AND TRUNC(CON.CON_FECHA_FIN) AND CON2.CON_ID = CON.CON_ID AND AGA2.ACT_ID = AGA.ACT_ID)))
