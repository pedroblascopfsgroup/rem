   ...
   IF COD_RETORNO = 0 AND V_NOTABLES = 0 THEN
	   ...
	   EXECUTE IMMEDIATE V_MSQL INTO V_COUNT;
		...
		V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_PRINEX_ACTIVOS PRI USING (
		  	SELECT ACT.ACT_ID, ECO.ECO_FECHA_VENTA, COM.ID_PERSONA_HAYA FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT 
		  	...
		  	WHERE ACT.ACT_NUM_ACTIVO = '||ID_ACTIVO_HAYA||') AUX ON (AUX.ACT_ID = PRI.ACT_ID)
