...
-- Modificación de la vista VI_STOCK_OFERTAS_CAIXA.
  ...
  EXECUTE IMMEDIATE 'CREATE VIEW '|| V_ESQUEMA ||'.VI_STOCK_OFERTAS_CAIXA
      AS
		...
		INNER JOIN '|| V_ESQUEMA ||'.ECO_EXPEDIENTE_COMERCIAL ECO ON OFR.OFR_ID = ECO.OFR_ID AND OFR.BORRADO = 0
		INNER JOIN '|| V_ESQUEMA ||'.OFR_OFERTAS_CAIXA OCA ON OCA.OFR_ID = OFR.OFR_ID AND OCA.BORRADO = 0
		WHERE OFR.BORRADO = 0';  
