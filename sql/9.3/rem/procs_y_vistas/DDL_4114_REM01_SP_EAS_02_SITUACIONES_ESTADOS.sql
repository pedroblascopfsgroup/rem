   	...
   	EXECUTE IMMEDIATE V_SQL;
        ...
        V_SQL := 'MERGE INTO ' || V_ESQUEMA || '.AUX_APR_SITUACIONES_ESTADOS APR USING(
		    ...
		    NULL AS NOTAS
             ...
             JOIN ' || V_ESQUEMA || '.DD_ETP_ESTADO_POSESORIO ETP ON ETP.DD_ETP_ID = CAI.DD_ETP_ID AND ETP.DD_ETP_CODIGO NOT IN (''P05'')
		     JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV ON EQV.DD_NOMBRE_CAIXA = ''ESTADO_POSESORIO'' AND EQV.DD_CODIGO_REM = ETP.DD_ETP_CODIGO  AND EQV.BORRADO=0  
				WHERE AGR.BORRADO = 0 
		    ...
		    NULL AS NOTAS
             ...
             JOIN '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO PAC ON PAC.ACT_ID = ACT.ACT_ID AND PAC.BORRADO = 0  
	            JOIN ACTIVO_MATRIZ MAT ON MAT.AGR_ID = AGR.AGR_ID
                    JOIN ' || V_ESQUEMA || '.ACT_ACTIVO_CAIXA CAI ON CAI.ACT_ID = MAT.ACT_ID
             JOIN ' || V_ESQUEMA || '.DD_EAT_EST_TECNICO EAT ON EAT.DD_EAT_ID = CAI.DD_EAT_ID
		     JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV ON EQV.DD_NOMBRE_CAIXA = ''ESTADO_TECNICO'' AND EQV.DD_CODIGO_REM = EAT.DD_EAT_CODIGO  AND EQV.BORRADO=0  
				WHERE AGR.BORRADO = 0 
				AND AGR.AGR_FECHA_BAJA IS NULL 
				AND AGR.DD_TAG_ID = (SELECT DD_TAG_ID FROM ' || V_ESQUEMA || '.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = ''16'') 
				AND AGA.AGA_PRINCIPAL = 0
		    ...
		    JOIN ACTIVO_MATRIZ MAT ON MAT.AGR_ID = AGR.AGR_ID
		    JOIN ' || V_ESQUEMA || '.ACT_ACTIVO_CAIXA CAI ON CAI.ACT_ID = MAT.ACT_ID
		    JOIN ' || V_ESQUEMA || '.DD_ECA_EST_COM_ALQUILER ECA ON ECA.DD_ECA_ID = CAI.DD_ECA_ID AND ECA.DD_ECA_CODIGO NOT IN (''AA4'')
		    JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV ON EQV.DD_NOMBRE_CAIXA = ''ESTADO_COMERCIAL_ALQUILER'' AND EQV.DD_CODIGO_REM = ECA.DD_ECA_CODIGO  AND EQV.BORRADO=0  
				WHERE AGR.BORRADO = 0 
				AND AGR.AGR_FECHA_BAJA IS NULL 
				AND AGR.DD_TAG_ID = (SELECT DD_TAG_ID FROM ' || V_ESQUEMA || '.DD_TAG_TIPO_AGRUPACION WHERE DD_TAG_CODIGO = ''16'') 
				AND AGA.AGA_PRINCIPAL = 0
