    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO]');
   	...
   	EXECUTE IMMEDIATE V_SQL;
        ...
        V_SQL := 'MERGE INTO ' || V_ESQUEMA || '.AUX_APR_EQUIPAMIENTO APR USING(
			...
			TO_CHAR(EQV21.DD_CODIGO_CAIXA) AS ICO_PATIO,
			...
			TO_CHAR(EQV22.DD_CODIGO_CAIXA) AS ICO_LIC_OBRA,
			...
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV16 ON EQV16.DD_NOMBRE_CAIXA = ''INST_DEPORTIVAS'' AND EQV16.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			LEFT JOIN ' || V_ESQUEMA_MASTER || '.DD_SIN_SINO SIN6 ON SIN6.DD_SIN_ID = ICO.ICO_ARM_EMPOTRADOS
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV17 ON EQV17.DD_NOMBRE_CAIXA = ''ARM_EMPOTRADOS'' AND EQV17.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			LEFT JOIN ' || V_ESQUEMA_MASTER || '.DD_SIN_SINO SIN7 ON SIN7.DD_SIN_ID = ICO.ICO_ANEJO_TRASTERO
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV18 ON EQV18.DD_NOMBRE_CAIXA = ''TRASTERO'' AND EQV18.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			LEFT JOIN ' || V_ESQUEMA_MASTER || '.DD_SIN_SINO SIN8 ON SIN8.DD_SIN_ID = ICO.ICO_ANEJO_GARAJE
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV19 ON EQV19.DD_NOMBRE_CAIXA = ''GARAJE'' AND EQV19.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			LEFT JOIN ' || V_ESQUEMA_MASTER || '.DD_SIN_SINO SIN9 ON SIN9.DD_SIN_ID = ICO.ICO_LIC_APERTURA
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV20 ON EQV20.DD_NOMBRE_CAIXA = ''APERTURA'' AND EQV20.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			LEFT JOIN ' || V_ESQUEMA_MASTER || '.DD_SIN_SINO SIN10 ON SIN10.DD_SIN_ID = ICO.ICO_ASCENSOR
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV21 ON EQV21.DD_NOMBRE_CAIXA = ''ASCENSOR'' AND EQV21.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			LEFT JOIN ' || V_ESQUEMA_MASTER || '.DD_SIN_SINO SIN10 ON SIN10.DD_SIN_ID = ICO.ICO_PATIO
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV21 ON EQV21.DD_NOMBRE_CAIXA = ''PATIO'' AND EQV21.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			LEFT JOIN ' || V_ESQUEMA_MASTER || '.DD_SIN_SINO SIN11 ON SIN11.DD_SIN_ID = ICO.ICO_LIC_OBRA
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV22 ON EQV22.DD_NOMBRE_CAIXA = ''LIC_OBRA'' AND EQV22.DD_CODIGO_REM = SIN.DD_SIN_CODIGO
			...
			SELECT ACT_NUM_ACTIVO AS ZZEXTERNALID, FIXFITCHARACT, NULL AS VALIDFROM, NULL AS VALIDTO,QUALITY AS ADDITIONALINFO 
			FROM UA
			UNPIVOT(
			QUALITY
