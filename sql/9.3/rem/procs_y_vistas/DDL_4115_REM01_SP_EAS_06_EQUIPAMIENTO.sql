...
--##        0.4 Modificaciones [HREOS-18646] Alejandra García
--##########################################
--*/
    ...
    DBMS_OUTPUT.PUT_LINE('[INICIO]');
   	...
   	EXECUTE IMMEDIATE V_SQL;
        ...
        V_SQL := 'MERGE INTO ' || V_ESQUEMA || '.AUX_APR_EQUIPAMIENTO APR USING(
			...
			CASE
				WHEN TCL.DD_TCL_CODIGO = ''03'' THEN ''001002''
				ELSE ''001001''
			END ICO_CALEFACCION_1,
			TO_CHAR(EQV.DD_CODIGO_CAIXA) AS DD_ECV_CODIGO,
			CASE
				WHEN TCA.DD_TCA_CODIGO = ''01'' THEN ''002001''
				ELSE ''002002''
			END DD_TCA_CODIGO,
			CASE
				WHEN DIS1.DD_DIS_CODIGO = ''01'' THEN ''002502'' 
				ELSE ''002501''
			END AS ICO_JARDIN,
			TO_CHAR(EQV22.DD_CODIGO_CAIXA) AS ICO_PATIO,
			CASE
				WHEN DIS2.DD_DIS_CODIGO = ''01'' THEN ''004202'' 
				ELSE ''004201''
			END AS ICO_PISCINA,
			...
			CASE
				WHEN DIS4.DD_DIS_CODIGO = ''01'' THEN NULL
				ELSE EQV8.DD_CODIGO_CAIXA
			END AS ICO_JARDIN_2,
			CASE
				WHEN DIS3.DD_DIS_CODIGO = ''01'' THEN NULL
				ELSE EQV26.DD_CODIGO_CAIXA
			END AS ICO_PISCINA_2,
			...
			CASE
				WHEN TCL.DD_TCL_CODIGO = ''01'' AND TCA.DD_TCA_CODIGO = ''01'' THEN ''073301''
				WHEN TCL.DD_TCL_CODIGO = ''01'' AND TCA.DD_TCA_CODIGO = ''03'' THEN ''073302''
				WHEN TCL.DD_TCL_CODIGO = ''01'' AND TCA.DD_TCA_CODIGO IN (''02'', ''04'') THEN ''073303''
				WHEN TCL.DD_TCL_CODIGO = ''02''  THEN ''073304''
				WHEN TCL.DD_TCL_CODIGO = ''03''  THEN ''073305''
			END ICO_CALEFACCION_2,
			...
			TO_CHAR(EQV28.DD_CODIGO_CAIXA) AS DD_VUB_CODIGO_2,
			TO_CHAR(EQV25.DD_CODIGO_CAIXA) AS ICO_INST_DEPORTIVAS_2,
			TO_CHAR(EQV27.DD_CODIGO_CAIXA) AS DD_ECV_CODIGO_2,
			TO_CHAR(EQV23.DD_CODIGO_CAIXA) AS ICO_LIC_OBRA,
			TO_CHAR(EQV11.DD_CODIGO_CAIXA) AS ICO_SALIDA_HUMOS,
			TO_CHAR(EQV24.DD_CODIGO_CAIXA) AS DD_ESC_CODIGO_2
			...
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV27 ON EQV27.DD_NOMBRE_CAIXA = ''EST_CONSERVACION_2'' AND EQV27.DD_CODIGO_REM = ECV.DD_ECV_CODIGO
			...
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV24 ON EQV24.DD_NOMBRE_CAIXA = ''CONSERV_EDIF_2'' AND EQV24.DD_CODIGO_REM = EST.DD_ESC_CODIGO AND EQV24.BORRADO = 0
			LEFT JOIN ' || V_ESQUEMA || '.DD_DIS_DISPONIBILIDAD DIS1 ON DIS1.DD_DIS_ID = ICO.ICO_JARDIN
			LEFT JOIN ' || V_ESQUEMA || '.DD_DIS_DISPONIBILIDAD DIS2 ON DIS2.DD_DIS_ID = ICO.ICO_PISCINA
			LEFT JOIN ' || V_ESQUEMA || '.DD_DIS_DISPONIBILIDAD DIS3 ON DIS3.DD_DIS_ID = ICO.ICO_PISCINA AND DIS3.BORRADO = 0
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV26 ON EQV26.DD_NOMBRE_CAIXA = ''PISCINA_2'' AND EQV26.DD_CODIGO_REM = DIS3.DD_DIS_CODIGO
			LEFT JOIN ' || V_ESQUEMA || '.DD_DIS_DISPONIBILIDAD DIS4 ON DIS4.DD_DIS_ID = ICO.ICO_JARDIN
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV8 ON EQV8.DD_NOMBRE_CAIXA = ''USO_JARDIN'' AND EQV8.DD_CODIGO_REM = DIS4.DD_DIS_CODIGO
			...
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV28 ON EQV28.DD_NOMBRE_CAIXA = ''UBICACION_2'' AND EQV28.DD_CODIGO_REM = VUB.DD_VUB_CODIGO AND EQV28.BORRADO = 0
			...
			LEFT JOIN ' || V_ESQUEMA || '.DD_EQV_CAIXA_REM EQV25 ON EQV25.DD_NOMBRE_CAIXA = ''INST_DEPORTIVAS_2'' AND EQV25.DD_CODIGO_REM = SIN5.DD_SIN_CODIGO AND EQV25.BORRADO = 0
