        ...
        V_SQL := 'MERGE INTO ' || V_ESQUEMA || '.AUX_APR_TEXTOS_ADICIONALES APR USING(
			SELECT 
		     ACT.ACT_NUM_ACTIVO AS ZZEXTERNALID, 
                    ''RE01'' AS TEXTCATE,
                    REPLACE(ACT.ACT_DESCRIPCION, '';'','','') AS XTEXTCATE
		     ...
		     ACT.ACT_NUM_ACTIVO AS ZZEXTERNALID, 
                    ''RE04'' AS TEXTCATE,
                    REPLACE(APU.APU_MOT_OCULTACION_MANUAL_A, '';'','','') AS XTEXTCATE
		     FROM ' || V_ESQUEMA || '.ACT_AGR_AGRUPACION AGR 
		     JOIN ' || V_ESQUEMA || '.ACT_AGA_AGRUPACION_ACTIVO AGA ON AGR.AGR_ID = AGA.AGR_ID AND AGA.BORRADO = 0
		     JOIN ' || V_ESQUEMA || '.ACT_ACTIVO ACT ON ACT.ACT_ID = AGA.ACT_ID AND ACT.BORRADO = 0 
		     ...
		     ACT.ACT_NUM_ACTIVO AS ZZEXTERNALID, 
                    ''RE07'' AS TEXTCATE,
                    REPLACE(REPLACE(REPLACE(ICO.ICO_INFO_DISTRIBUCION_INTERIOR, CHR(10), '' ''), CHR(13), '' '')||REPLACE(REPLACE(EDIF.EDI_DESCRIPCION, CHR(10), '' ''), CHR(13), '' ''), '';'','','') AS XTEXTCATE
		     FROM ' || V_ESQUEMA || '.ACT_AGR_AGRUPACION AGR 
		     JOIN ' || V_ESQUEMA || '.ACT_AGA_AGRUPACION_ACTIVO AGA ON AGR.AGR_ID = AGA.AGR_ID AND AGA.BORRADO = 0
		     JOIN ' || V_ESQUEMA || '.ACT_ACTIVO ACT ON ACT.ACT_ID = AGA.ACT_ID AND ACT.BORRADO = 0 
		     ...
		     ACT.ACT_NUM_ACTIVO AS ZZEXTERNALID, 
                    ''RE02'' AS TEXTCATE,
                    REPLACE(PAC.PAC_MOT_EXCL_COMERCIALIZAR, '';'','','') AS XTEXTCATE
		     FROM ' || V_ESQUEMA || '.ACT_AGR_AGRUPACION AGR 
		     JOIN ' || V_ESQUEMA || '.ACT_AGA_AGRUPACION_ACTIVO AGA ON AGR.AGR_ID = AGA.AGR_ID AND AGA.BORRADO = 0
		     JOIN ' || V_ESQUEMA || '.ACT_ACTIVO ACT ON ACT.ACT_ID = AGA.ACT_ID AND ACT.BORRADO = 0 
		     JOIN ' || V_ESQUEMA || '.ACT_APU_ACTIVO_PUBLICACION APU ON ACT.ACT_ID = APU.ACT_ID  
