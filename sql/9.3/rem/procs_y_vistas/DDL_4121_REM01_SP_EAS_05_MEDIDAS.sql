...
BEGIN
      ...
      SALIDA := SALIDA || '[INFO] SE VA A PROCEDER A ACTUALIZAR/INSERTAR CAMPOS EN AUX_APR_MEDIDAS.'|| CHR(10);
       ...
       V_MSQL := ' MERGE INTO '|| V_ESQUEMA ||'.AUX_APR_MEDIDAS MED USING(
            ...
            MAT.REG_SUPERFICIE_UTIL,
            MAT.REG_SUPERFICIE_CONSTRUIDA,
            MAT.REG_SUPERFICIE_PARCELA,
               
            MAT.BIE_DREG_SUPERFICIE_CONSTRUIDA,
            ...
            LEFT JOIN '|| V_ESQUEMA ||'.ACT_REG_INFO_REGISTRAL ACT_REG ON ACT_REG.ACT_ID = ACT.ACT_ID 
            ...
            SELECT ACT_NUM_ACTIVO AS ZZEXTERNALID, MEAS, VALOR AS MEASVALUE,NULL AS MEASUNIT, TO_CHAR(FECHACREAR, ''yyyymmdd'') AS VALIDFROM,NULL AS VALIDTO
                 FROM UA
                UNPIVOT(
                VALOR
                FOR MEAS IN 
