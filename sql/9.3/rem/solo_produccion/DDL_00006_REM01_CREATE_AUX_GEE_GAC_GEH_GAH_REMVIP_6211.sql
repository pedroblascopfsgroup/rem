...
EXECUTE IMMEDIATE 'CREATE TABLE '||V_ESQUEMA_1||'.'||V_TABLA1||' AS 
  ( 
	...
	WHERE NOT ( ( TRUNC(FECHACREAR) = TO_DATE( ''11/02/2020'', ''DD/MM/YYYY'' ) ) AND ( USUARIOCREAR = ''SP_AGA_V4'' ) )

  )

';

	-- 	WHERE NOT ( ( TRUNC(FECHACREAR) = TO_DATE( ''07/02/2020'', ''DD/MM/YYYY'' ) ) AND ( USUARIOCREAR = ''ALT_SAREB'' ) )
