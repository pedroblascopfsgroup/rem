...
SET SERVEROUTPUT ON; 
SET DEFINE OFF;
create or replace PROCEDURE INFORME_RATING_ACTIVO (
...
AUTHID CURRENT_USER IS
	...
	V_DESC_RATING                   			VARCHAR2(10 CHAR);
...
BEGIN
	...
	IF(p_username IS NULL) THEN
		v_username := 'INFORME_RATING_ACTIVO';
	END IF;
   ...
   -- Guardamos en una colección el criterio y su valor de las puntuaciones
          ...
          LOOP
        	IF v_total >= tramo.tpr_valor_desde THEN
        		v_dd_rtg_id := tramo.dd_rtg_id;
        		 V_MSQL :=   'SELECT DD_RTG_DESCRIPCION FROM REM01.DD_RTG_RATING_ACTIVO WHERE DD_RTG_ID = '||v_dd_rtg_id;
                	EXECUTE IMMEDIATE V_MSQL INTO V_DESC_RATING;
			DBMS_OUTPUT.PUT_LINE('	[INFO] Rating asignado: ' || V_DESC_RATING);
			DBMS_OUTPUT.PUT_LINE('	[INFO] Rating total: ' || v_total);
        		EXIT;
        	END IF;
          END LOOP;
	...
	EXECUTE IMMEDIATE 'GRANT ALL ON REM01.INFORME_RATING_ACTIVO TO REM_QUERY WITH GRANT OPTION';
	DBMS_OUTPUT.PUT_LINE('[INFO] PERMISOS  OTORGADOS'); 
