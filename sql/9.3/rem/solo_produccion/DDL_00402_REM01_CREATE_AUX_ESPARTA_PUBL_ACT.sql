...
--## PRODUCTO=NO
--## 
--## Finalidad: Creación de tabla 'AUX_ESPARTA_PUBL_ACT'
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
V_TABLA VARCHAR2(40 CHAR) := 'AUX_ESPARTA_PUBL_ACT';
...
EXECUTE IMMEDIATE '
		...
		AS (
			...
			JOIN '||V_ESQUEMA_1||'.DD_CRA_CARTERA CRA ON CRA.DD_CRA_ID = ACT.DD_CRA_ID AND CRA.BORRADO = 0
			JOIN '||V_ESQUEMA_1||'.ACT_APU_ACTIVO_PUBLICACION APU ON APU.ACT_ID = ACT.ACT_ID AND APU.BORRADO = 0
			LEFT JOIN '||V_ESQUEMA_1||'.DD_EPV_ESTADO_PUB_VENTA EPV ON APU.DD_EPV_ID = EPV.DD_EPV_ID AND EPV.BORRADO = 0
			LEFT JOIN '||V_ESQUEMA_1||'.DD_EPA_ESTADO_PUB_ALQUILER EPA ON APU.DD_EPA_ID = EPA.DD_EPA_ID AND EPA.BORRADO = 0
			JOIN '||V_ESQUEMA_1||'.ACT_PAC_PERIMETRO_ACTIVO PAC ON PAC.ACT_ID = ACT.ACT_ID
			WHERE CRA.DD_CRA_CODIGO = ''02''
			AND (EPV.DD_EPV_CODIGO = ''03'' OR EPA.DD_EPA_CODIGO = ''03'')
			AND PAC.PAC_CHECK_PUBLICAR = 0
			AND NOT EXISTS (SELECT 1
			FROM '||V_ESQUEMA_1||'.ACT_AGA_AGRUPACION_ACTIVO AGA 
			JOIN '||V_ESQUEMA_1||'.ACT_AGR_AGRUPACION AGR ON AGR.AGR_ID = AGA.AGR_ID AND AGR.BORRADO = 0
			JOIN '||V_ESQUEMA_1||'.DD_TAG_TIPO_AGRUPACION TAG ON TAG.DD_TAG_ID = AGR.DD_TAG_ID AND TAG.BORRADO = 0 AND TAG.DD_TAG_CODIGO = ''02''
