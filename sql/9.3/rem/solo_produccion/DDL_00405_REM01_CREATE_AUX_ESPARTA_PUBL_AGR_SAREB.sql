...
--## PRODUCTO=NO
--## 
--## Finalidad: Creación de tabla 'AUX_ESPARTA_PUBL_AGR'
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
V_TABLA VARCHAR2(40 CHAR) := 'AUX_ESPARTA_PUBL_AGR_SAREB';
...
EXECUTE IMMEDIATE '
		...
		AS (
			SELECT DISTINCT AGA.AGA_ID
			, 0 AS PROCESADO
			FROM REM01.H_CCN_CAMBIOS_CONV_SAREB CCN
			JOIN REM01.ACT_ACTIVO ACT ON CCN.ACT_ID = ACT.ACT_ID
			...
			JOIN REM01.DD_TAG_TIPO_AGRUPACION TAG ON TAG.DD_TAG_ID = AGR.DD_TAG_ID AND TAG.BORRADO = 0 AND TAG.DD_TAG_CODIGO = ''02''
			JOIN REM01.DD_COS_CAMPOS_ORIGEN_CONV_SAREB COS ON COS.DD_COS_ID = CCN.DD_COS_ID
			WHERE COS.DD_COS_CODIGO IN (''009'',''011'',''013'',''029'',''086'',''097'',''173'',''177'',''175'',''178'',''179'',''181'',''183'',''185'')
