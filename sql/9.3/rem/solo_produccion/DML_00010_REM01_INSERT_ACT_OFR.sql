...
DECLARE
	...
	V_JBV T_ARRAY_JBV := T_ARRAY_JBV(
			T_JBV('90224213','7072620','17833.30','33.30'),
			T_JBV('90224213','7072623','17833.30','33.30'),
			T_JBV('90226896','6867326','15000','50'),
			T_JBV('90226755','6859046','20000','50'),
			T_JBV('90226621','6859046','10100','25'),
			T_JBV('90226621','6858954','10100','25'),
			T_JBV('90226621','6858953','10100','25'),
			T_JBV('90226621','6858614','10100','25')
	); 
	V_TMP_JBV T_JBV;
    
BEGIN	
	DBMS_OUTPUT.PUT_LINE('[INICIO] INSERT ACT_OFR');
	...
	IF V_NUM_FILAS = 1 AND V_NUM_FILAS_2 = 1 THEN
			...
			V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ACT_OFR
			...
			EXECUTE IMMEDIATE V_SQL INTO V_COUNT_ACT_OFR;
				
				IF V_COUNT_ACT_OFR = 0 THEN
	
				V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ACT_OFR (ACT_ID, OFR_ID, ACT_OFR_IMPORTE, OFR_ACT_PORCEN_PARTICIPACION)
					   VALUES (
						   (SELECT ACT_ID FROM '||V_ESQUEMA||'.ACT_ACTIVO WHERE ACT_NUM_ACTIVO = '||ACT_NUM_ACTIVO||'),
						   (SELECT OFR_ID FROM '||V_ESQUEMA||'.OFR_OFERTAS WHERE OFR_NUM_OFERTA = '||OFR_NUM_OFERTA||'),
						   '||V_TMP_JBV(3)||',
						   '||V_TMP_JBV(4)||')';
