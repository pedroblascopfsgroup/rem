...
DECLARE
  ...
  V_USR VARCHAR2(30 CHAR) := 'HREOS-8752'; -- USUARIOCREAR/USUARIOMODIFICAR
    ...
    V_CDC T_ARRAY_CDC := T_ARRAY_CDC(           
        T_CDC('83', 'Consentimiento protección de datos (GDPR)')       
    );
    V_TMP_CDC T_CDC;
...
BEGIN
  ...
  IF V_NUM_TABLAS = 1 THEN
    ...
    LOOP
      ...
      V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE DD_TPD_CODIGO = '''||V_TMP_CDC(1)||''' AND BORRADO = 1';
      ...
      IF V_NUM_TABLAS = 1 THEN
        ...
        EXECUTE IMMEDIATE 'UPDATE '||V_ESQUEMA||'.'||V_TABLA||'
				...
				BORRADO = 0,
	 	    	...
	 	    	USUARIOBORRAR = NULL,
	 	    	FECHABORRAR = NULL
				WHERE DD_TPD_CODIGO IN ('''||V_TMP_CDC(1)||''') 
             	';
      ELSE
        DBMS_OUTPUT.PUT_LINE('      [INFO] El codigo '||V_TMP_CDC(1)||'... No existe y ya está recuperado en ' ||V_ESQUEMA|| '.'||V_TABLA);
      END IF;
