...
BEGIN	
	...
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR WHERE PVE_ID = '||MEDIADOR_ID;
	
	EXECUTE IMMEDIATE V_SQL INTO V_NUM_FILAS_3;	
	
	IF V_NUM_FILAS > 0 AND V_NUM_FILAS_2 > 0 AND V_NUM_FILAS_3 > 0 THEN
	...
	ELSIF V_NUM_FILAS = 0 AND V_NUM_FILAS_2 > 0 AND V_NUM_FILAS_3 > 0 THEN
		...
		DBMS_OUTPUT.PUT_LINE('[INFO] REGISTRO CON ACT_ID: '||ACT_ID||' INSERTADO');
	...
	DBMS_OUTPUT.PUT_LINE('[FIN] Se han INSERTADO en total '||V_COUNT_INSERT||' registros');
