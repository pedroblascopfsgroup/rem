...
BEGIN	
	...
	V_SQL := 'UPDATE '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL 
				SET ICO_MEDIADOR_ID = (SELECT PVE_ID FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR WHERE PVE_COD_REM = 10007348),
					USUARIOMODIFICAR = ''REMVIP-5857'',
	...
	V_SQL := 'UPDATE '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR 
				SET PVE_ID_EMISOR = (SELECT PVE_ID FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR WHERE PVE_COD_REM = 10007348),
					USUARIOMODIFICAR = ''REMVIP-5857'',
					FECHAMODIFICAR = SYSDATE
