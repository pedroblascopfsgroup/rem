...
DECLARE
  ...
  V_USUARIOMODIFICAR VARCHAR2(100 CHAR) := 'REMVIP-5892';
...
BEGIN	
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: ACTUALIZAR ACT_REG_INFO_REGISTRAL.REG_IDUFIR ' );
			...
			MERGE INTO '||V_ESQUEMA||'.ACT_REG_INFO_REGISTRAL T1
				...
				SELECT ACT.ACT_ID, AUX.IDUFIR  
				FROM '||V_ESQUEMA||'.AUX_REMVIP_5892 AUX 
               			JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO = AUX.ACT_NUM_ACTIVO

       				) T2 
		        ON (T1.ACT_ID = T2.ACT_ID )
			WHEN MATCHED THEN UPDATE
			SET T1.REG_IDUFIR = T2.IDUFIR ,
