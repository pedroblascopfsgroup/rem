...
--## PRODUCTO=NO
--## 
--## Finalidad: REMVIP-6289, paso 3, update BIE_ADJ_F_REA_CORRECTA a NULL
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
    ...
    V_USUARIOMODIFICAR VARCHAR(100 CHAR):= 'REMVIP-6289_2_2';
...
BEGIN	
    ...
    V_SQL :='MERGE INTO '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION T1 USING (
		SELECT AUX.ACT_ID, AUX.BIE_ADJ_F_REA_CORRECTA, ADJ.BIE_ADJ_ID
		FROM '||V_ESQUEMA||'.BIE_ADJ_ADJUDICACION ADJ 
		INNER JOIN '||V_ESQUEMA||'.AUX_REMVIP_6289_3 AUX ON AUX.BIE_ADJ_ID = ADJ.BIE_ADJ_ID
		)T2
		ON (T1.BIE_ADJ_ID=T2.BIE_ADJ_ID)
		WHEN MATCHED THEN UPDATE SET 
		T1.BIE_ADJ_F_REA_POSESION = NULL, 
