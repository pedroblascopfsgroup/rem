...
--## INCIDENCIA_LINK=REMVIP-6274
--## PRODUCTO=NO
--## 
--## Finalidad: CREAR PRESUPUESTOS 2020 EN ACT_PTO_PRESUPUESTO PARA CARTERA HYT Y TANGO
--##                    
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  V_USER VARCHAR2(50 CHAR):= 'REMVIP-6274';
		      ...
		      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_PTO_PRESUPUESTO T1
						USING (
							...
							LEFT JOIN '||V_ESQUEMA||'.ACT_PTO_PRESUPUESTO PTO ON PTO.ACT_ID = ACT.ACT_ID AND PTO.EJE_ID = EJE.EJE_ID AND PTO.BORRADO = 0 
                            				WHERE ACT.BORRADO = 0 AND PTO.PTO_ID IS NULL AND ACT.DD_CRA_ID IN (41,81)
						) T2
