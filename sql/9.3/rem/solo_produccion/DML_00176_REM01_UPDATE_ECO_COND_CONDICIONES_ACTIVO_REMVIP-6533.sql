...
--##    comunicada al comprador con los datos de situación real del activo.
...
DECLARE
	...
	V_USUARIO VARCHAR(50):='REMVIP-6533';
    V_NUM_OFERTA NUMBER(25):= 90245369;
	...
	V_MSQL:= ' MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
			...
			(	SELECT
				...
				(CASE SPS.SPS_OCUPADO WHEN 0 THEN 
    				(SELECT DD_SIP_ID FROM '||V_ESQUEMA||'.DD_SIP_SITUACION_POSESORIA WHERE DD_SIP_CODIGO = ''01'') 
				ELSE -- OCUPADO
    				CASE SPS.SPS_CON_TITULO WHEN 1 THEN 
    					(SELECT DD_SIP_ID FROM '||V_ESQUEMA||'.DD_SIP_SITUACION_POSESORIA WHERE DD_SIP_CODIGO = ''02'') 
    				WHEN 0 THEN 
    					(SELECT DD_SIP_ID FROM '||V_ESQUEMA||'.DD_SIP_SITUACION_POSESORIA WHERE DD_SIP_CODIGO = ''03'') 
    				END
				END) DD_SIP_ID
