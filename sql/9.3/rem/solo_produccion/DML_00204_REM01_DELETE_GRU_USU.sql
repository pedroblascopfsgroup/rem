...
DECLARE
	...
	V_FUNCION T_ARRAY_FUNCION := T_ARRAY_FUNCION(
		...
		T_FUNCION( 'grusbackoffman' , 'imartin')
	); 
...
BEGIN	
	...
	LOOP
		...
		IF V_NUM_TABLAS > 0 THEN	  
			V_MSQL := '
			UPDATE '||V_ESQUEMA_M||'.'||V_TABLA||' GRU SET 
				GRU.USUARIOBORRAR = ''REMVIP-6930''
				, GRU.FECHABORRAR = SYSDATE
				, GRU.BORRADO = 1
			WHERE 
				GRU.USU_ID_GRUPO = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_TMP_FUNCION(1)||''') 
				AND GRU.USU_ID_USUARIO = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_TMP_FUNCION(2)||''')
				AND GRU.BORRADO = 0
			';
