...
BEGIN	
    ...
    V_MSQL := '
      INSERT INTO '|| V_ESQUEMA ||'.TMP_'||V_TEXT_TABLA||' (' ||
        ...
        '  , CCC_ACTIVABLE, CCC_ARRENDAMIENTO, CCC_REFACTURABLE, FECHACREAR, BORRADO)'||
        ' SELECT ROWNUM, CCC.CCC_CUENTA_CONTABLE, STG.DD_TGA_ID, STG.DD_STG_ID, TIM.DD_TIM_ID, CCC.DD_CRA_ID, CCC.DD_SCR_ID'||
        '  , CCC.CCC_CUENTA_ACTIVABLE, CCC.CCC_ARRENDAMIENTO, CCC.CCC_REFACTURABLE, SYSDATE, 0'||
          ...
          SELECT CCC_CTAS_ID
              , ROW_NUMBER() OVER(
                  PARTITION BY DD_TGA_ID, DD_STG_ID, DD_TIM_ID, DD_CRA_ID, DD_SCR_ID
                      , PRO_ID, CCC_ARRENDAMIENTO, CCC_REFACTURABLE, DD_TBE_ID
    ...
    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (
      ...
      FROM (
          SELECT TMP.CCC_CUENTA_CONTABLE
              ...
              , 0 CCC_VENDIDO
              , RANK() OVER(
                  PARTITION BY TMP.DD_TGA_ID, TMP.DD_STG_ID, TMP.DD_TIM_ID, TMP.DD_CRA_ID, EJE.EJE_ID, PRO.PRO_ID, TMP.CCC_ARRENDAMIENTO, TMP.CCC_REFACTURABLE
                      , TMP.CCC_PRINCIPAL
                  ORDER BY 
                      CASE 
                          WHEN TMP.PRO_ID IS NOT NULL AND NVL(PRO.PRO_ID, 0) = NVL(TMP.PRO_ID, 0) THEN 0
