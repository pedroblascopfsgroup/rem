...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-6688_TAR4'; -- USUARIOCREAR/USUARIOMODIFICAR.
...
BEGIN
		    ...
		    USING (SELECT
				...
				GEE.USU_ID AS USU_ID2,
                            ...
                            FROM
                                ...
                                INNER JOIN '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE ON GEE.GEE_ID = GAC.GEE_ID AND GEE.DD_TGE_ID = 416 AND GEE.BORRADO = 0
                                INNER JOIN REMMASTER.USU_USUARIOS USU ON USU.USU_ID = TAC.USU_ID 
				INNER JOIN REMMASTER.USU_USUARIOS USU2 ON USU2.USU_ID = GEE.USU_ID
                            WHERE TAC.USU_ID = 87960
                                AND   ACT.DD_CRA_ID = 2
                                AND   TRA.DD_EPR_ID = 30
                                AND   TRA.BORRADO = 0
                                AND   TBJ.BORRADO = 0
                                AND   TAP.TAP_CODIGO IN (''T008_SolicitudDocumento'', ''T008_ObtencionDocumento'')
                                ) C
                    WHERE
                        RN = 1
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado en total '||SQL%ROWCOUNT||' usuarios de tareas T008.');
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado en total '||SQL%ROWCOUNT||' usuarios de tareas T008 Docu.');
		    ...
		    USING (SELECT
				...
				GEE.USU_ID AS USU_ID2,
                            ...
                            FROM
                                ...
                                INNER JOIN '||V_ESQUEMA||'.ACT_PVC_PROVEEDOR_CONTACTO PVC ON TBJ.PVC_ID = PVC.PVC_ID
                                INNER JOIN '||V_ESQUEMA||'.GAC_GESTOR_ADD_ACTIVO GAC ON GAC.ACT_ID = ACT.ACT_ID
                                INNER JOIN '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE ON GEE.GEE_ID = GAC.GEE_ID AND GEE.DD_TGE_ID = 361 AND GEE.BORRADO = 0
                                INNER JOIN REMMASTER.USU_USUARIOS USU ON USU.USU_ID = TAC.USU_ID 
				INNER JOIN REMMASTER.USU_USUARIOS USU2 ON USU2.USU_ID = GEE.USU_ID
                            WHERE TAC.USU_ID = 87960
                                AND   ACT.DD_CRA_ID = 2
                                ...
                                AND   TRA.BORRADO = 0
                                AND   TBJ.BORRADO = 0
                                AND   TAP.TAP_CODIGO IN (''T008_CierreEconomico'')
                                ) C
                    WHERE
                        RN = 1
                        AND   C.TAR_TAREA_FINALIZADA = 0
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado en total '||SQL%ROWCOUNT||' usuarios de tareas T008 Cierre.');
		    ...
		    USING (SELECT
				...
				INNER JOIN REMMASTER.USU_USUARIOS USU2 ON USU2.USU_ID = GEE.USU_ID
                            WHERE TAC.USU_ID = 87960
                                ...
                                AND   TRA.BORRADO = 0
                                AND   TBJ.BORRADO = 0
                                AND   TAP.TAP_CODIGO IN (''T002_CierreEconomico'',''T003_CierreEconomico'')
                                ) C
                    WHERE
                        RN = 1
                        AND   C.TAR_TAREA_FINALIZADA = 0
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado en total '||SQL%ROWCOUNT||' usuarios de tareas T002 Cierre y T003 Cierre.');
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado en total '||SQL%ROWCOUNT||' usuarios de tareas T002 Docu.');
