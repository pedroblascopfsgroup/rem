...
--## 
--## Finalidad: 
--## INSTRUCCIONES: Insertar en la tabla AUX_REMVIP_6954 para guardar las cuentas contables y partidas presupuestaria de Divarian.
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
--*/
...
DECLARE
    ...
    V_TABLA VARCHAR2(30 CHAR) := 'AUX_REMVIP_6954';  -- Tabla objetivo
...
BEGIN
	...
	IF V_NUM_TABLAS > 0 THEN
		...
		V_SQL := 'INSERT INTO '||V_ESQUEMA||'.AUX_REMVIP_6954(
			NUMERO_GASTO,
			CUENTA_CONTABLE,
			PARTIDA_PRESUPUESTARIA)
			SELECT GPV_NUM_GASTO_HAYA AS NUMERO_GASTO,
			GIC_CUENTA_CONTABLE AS CUENTA_CONTABLE,
			GIC_PTDA_PRESUPUESTARIA AS PARTIDA_PRESUPUESTARIA
			FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV 
			JOIN '||V_ESQUEMA||'.GIC_GASTOS_INFO_CONTABILIDAD GIC ON GIC.GPV_ID = GPV.GPV_ID
			WHERE GPV.USUARIOCREAR = ''MIG_DIVARIAN''';
