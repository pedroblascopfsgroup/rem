...
DECLARE
    ...
    V_TABLA VARCHAR2(30 CHAR) := 'AUX_REMVIP_6969';  -- Tabla objetivo
...
BEGIN
	...
	IF V_NUM_TABLAS > 0 THEN
		...
		V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_VAL_VALORACIONES VAL USING
			(SELECT VAL.VAL_ID, TO_DATE(AUX.VAL_FECHA_APROBACION, ''yyyymmdd'') AS FECHA_APROBACION
			FROM '||V_ESQUEMA||'.AUX_REMVIP_6969 AUX 
			...
			WHERE VAL.DD_TPC_ID = (SELECT DD_TPC_ID FROM DD_TPC_TIPO_PRECIO WHERE DD_TPC_CODIGO = ''02'')
			) AUX ON (VAL.VAL_ID = AUX.VAL_ID)
			WHEN MATCHED THEN UPDATE SET
			VAL.VAL_FECHA_APROBACION = AUX.FECHA_APROBACION,
			VAL.FECHAMODIFICAR = SYSDATE,
			VAL.USUARIOMODIFICAR = ''REMVIP-6969''';
