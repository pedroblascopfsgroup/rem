...
BEGIN
	...
	execute immediate 'MERGE INTO '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL T1 USING (
						    SELECT
						    DISTINCT ICO.ICO_ID, AUX.DESCRIPCION_COMERCIAL
						    FROM '||V_ESQUEMA||'.act_activo ACT
						    INNER JOIN '||V_ESQUEMA||'.AUX_DESCRIPCION_COMERCIAL_REMVIP_7064 AUX ON AUX.ACT_NUMERO_ACTIVO = ACT.ACT_NUM_ACTIVO
						    INNER JOIN '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL ICO ON ICO.ACT_ID = ACT.ACT_ID
						) T2
						ON (T1.ICO_ID = T2.ICO_ID)
						WHEN MATCHED THEN UPDATE SET
						T1.ICO_DESCRIPCION = DESCRIPCION_COMERCIAL,
						T1.USUARIOMODIFICAR = ''REMVIP-7064'',
