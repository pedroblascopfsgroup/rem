...
BEGIN	
	...
	LOOP
		...
		V_MSQL := 'DELETE FROM '||V_ESQUEMA||'.CEX_COMPRADOR_EXPEDIENTE WHERE ECO_ID = (SELECT ECO.ECO_ID 
					FROM '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO
					INNER JOIN '||V_ESQUEMA||'.OFR_OFERTAS OFR ON OFR.OFR_ID = ECO.OFR_ID
					WHERE OFR.OFR_NUM_OFERTA = '||V_TMP_FUNCION(1)||') AND
					COM_ID IN (SELECT COM_ID FROM (SELECT CEX.COM_ID, 
