...
DECLARE
    ...
    V_USR VARCHAR2(50 CHAR) := 'REMVIP-7219-4';
...
BEGIN
        ...
        IF V_NUM > 0 THEN
	        ...
	        IF V_NUM > 0 THEN
                ...
                EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC INNER JOIN '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES TAR ON TAR.TAR_ID = TAC.TAR_ID 
                            INNER JOIN '||V_ESQUEMA||'.TEX_TAREA_EXTERNA TEX ON TEX.TAR_ID = TAR.TAR_ID WHERE TAC.TRA_ID = '||V_TRA_ID||'
							AND TEX.TAP_ID IN (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''T017_RecomendCES'')' INTO V_NUM;
				...
				IF V_NUM > 0 THEN
			        EXECUTE IMMEDIATE 'SELECT TAC.TAR_ID FROM '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC INNER JOIN '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES TAR ON TAR.TAR_ID = TAC.TAR_ID 
	                            INNER JOIN '||V_ESQUEMA||'.TEX_TAREA_EXTERNA TEX ON TEX.TAR_ID = TAR.TAR_ID WHERE TAC.TRA_ID = '||V_TRA_ID||'
								AND TEX.TAP_ID IN (SELECT TAP_ID FROM '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''T017_RecomendCES'')' INTO V_TAR_ID;
			    
					EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS WHERE TAR_ID = '||V_TAR_ID;
					EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA||'.TEX_TAREA_EXTERNA WHERE TAR_ID = '||V_TAR_ID;
					EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES WHERE TAR_ID = '||V_TAR_ID;
                    EXECUTE IMMEDIATE 'DELETE FROM '||V_ESQUEMA||'.ETN_EXTAREAS_NOTIFICACIONES WHERE TAR_ID = '||V_TAR_ID;
				END IF;
			END IF;
		END IF;
	...
	#ESQUEMA#.AVANCE_TRAMITE_MIG_DIVARIAN(V_USR, EXPEDIENTES, 'T017_AdvisoryNote', NULL, NULL, PL_OUTPUT);
	...
	FOR CURSOR IN (SELECT 
		...
		WHERE TRA.USUARIOMODIFICAR = 'REMVIP-7219-4'
		AND TAP_CODIGO IN (
		'T017_AdvisoryNote',
