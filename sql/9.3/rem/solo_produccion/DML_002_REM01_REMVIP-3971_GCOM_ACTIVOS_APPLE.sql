...
DECLARE
  ...
  USUARIOCREAR VARCHAR2(50 CHAR):= 'REMVIP-3971';

BEGIN
      
      DBMS_OUTPUT.PUT_LINE('[INICIO] Comienza la asignacion de GCOM a rjimeno, jguarch y pgarciafraile.');
      ...
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD T1
				USING (
						...
						(
							SELECT ACT.ACT_ID,TGE.DD_TGE_ID,TGE.DD_TGE_CODIGO, GEE.GEE_ID
							...
							WHERE ACT.USUARIOCREAR = ''MIG_APPLE'' 
							  AND TGE.DD_TGE_CODIGO IN (''GCOM'',''GESTCOMALQ'')
						)
						SELECT ACT.ACT_NUM_ACTIVO, TGE.DD_TGE_CODIGO, TGE.DD_TGE_DESCRIPCION, AUX.GEE_ID, TGE.DD_TGE_ID, ACT.ACT_ID, PRV.DD_PRV_CODIGO, PRV.DD_PRV_DESCRIPCION,
							   (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME IN CASE WHEN PRV.DD_PRV_CODIGO IN (''4'',''11'',''14'',''18'',''21'',''23'',''29'',''41'',''51'',''52'') THEN ''rjimeno''
																									 WHEN PRV.DD_PRV_CODIGO IN (''3'',''7'',''8'',''12'',''17'',''25'',''30'',''38'',''43'',''46'')   THEN ''pgarciafraile''
																																																	  ELSE ''jguarch''
								END) AS USU_ID,
							   (SELECT USU_USERNAME FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME IN CASE WHEN PRV.DD_PRV_CODIGO IN (''4'',''11'',''14'',''18'',''21'',''23'',''29'',''41'',''51'',''52'') THEN ''rjimeno''
																										   WHEN PRV.DD_PRV_CODIGO IN (''3'',''7'',''8'',''12'',''17'',''25'',''30'',''38'',''43'',''46'')   THEN ''pgarciafraile''
																																																			ELSE ''jguarch''
								END) AS USU_USERNAME
						FROM '||V_ESQUEMA||'.ACT_ACTIVO                   ACT
						...
						JOIN '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR       TGE ON TGE.DD_TGE_CODIGO IN (''GCOM'',''GESTCOMALQ'')
      ...
      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST T1
				USING (
					...
					WHERE ACT.USUARIOCREAR = ''MIG_APPLE''
					  AND GEH.DD_TGE_ID IN (SELECT DD_TGE_ID FROM '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR WHERE DD_TGE_CODIGO IN (''GCOM'',''GESTCOMALQ''))
      ...
      V_SQL := 'INSERT INTO '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST (GEH_ID, USU_ID, DD_TGE_ID, GEH_FECHA_DESDE, GEH_FECHA_HASTA, USUARIOCREAR, FECHACREAR, BORRADO, USUARIOMODIFICAR)
				WITH GESTORES_APPLE AS 
				(
					SELECT ACT.ACT_ID,TGE.DD_TGE_ID,TGE.DD_TGE_CODIGO, GEE.GEE_ID, USU.USU_ID
					FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT
					JOIN '||V_ESQUEMA||'.GAC_GESTOR_ADD_ACTIVO GAC ON GAC.ACT_ID = ACT.ACT_ID
					JOIN '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE ON GEE.GEE_ID = GAC.GEE_ID
					JOIN '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR TGE ON TGE.DD_TGE_ID = GEE.DD_TGE_ID
