...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] ASIGNAR TAREAS');
	...
	V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS SET USU_ID = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''grucontroller''),
				...
				WHERE TAR_ID IN (
					SELECT TAR.TAR_ID FROM REM01.TAR_TAREAS_NOTIFICACIONES TAR
					INNER JOIN REM01.TAC_TAREAS_ACTIVOS TAC ON TAC.TAR_ID = TAR.TAR_ID AND TAC.BORRADO = 0
					INNER JOIN REM01.TEX_TAREA_EXTERNA TEX ON TEX.TAR_ID = TAR.TAR_ID AND TEX.BORRADO = 0
