...
--## INCIDENCIA_LINK=REMVIP-7026
--## PRODUCTO=NO
--## 
--## Finalidad: CREAR PRESUPUESTOS 2020 EN ACT_PTO_PRESUPUESTO PARA CARTERA HYT
--##                    
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
  ...
  V_USER VARCHAR2(50 CHAR):= 'REMVIP-7026';

BEGIN

	--Comprobamos SI ESTA EL EJERCICIO 2020

        	V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ACT_EJE_EJERCICIO WHERE EJE_ANYO = ''2020''';
		      ...
		      V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_PTO_PRESUPUESTO T1
						USING (
							...
							FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT 
                            				LEFT JOIN '||V_ESQUEMA||'.ACT_EJE_EJERCICIO EJE ON EJE.EJE_ANYO = 2020
							LEFT JOIN '||V_ESQUEMA||'.ACT_PTO_PRESUPUESTO PTO ON PTO.ACT_ID = ACT.ACT_ID AND PTO.EJE_ID = EJE.EJE_ID AND PTO.BORRADO = 0 
                            				WHERE ACT.BORRADO = 0 AND PTO.PTO_ID IS NULL AND ACT.DD_CRA_ID IN (41)
						) T2
		...
		ELSE
          		DBMS_OUTPUT.PUT_LINE('[INFO]: NO EXISTE EL EJERCICIO 2020');
