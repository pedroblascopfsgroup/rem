...
BEGIN	
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN '||V_TABLA);
      ...
      BEGIN
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_ETG_EQV_TIPO_GASTO_RU WHERE DD_ETG_CODIGO = ''111''';
	...
	ELSE
		...
		V_MSQL := 'SELECT DD_TGA_ID FROM '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO WHERE DD_TGA_DESCRIPCION = ''Gestoría''';
		EXECUTE IMMEDIATE V_MSQL INTO V_DD_TGA_ID;
		
		V_MSQL := 'SELECT DD_STG_ID FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_TGA_ID = '||V_DD_TGA_ID||' AND DD_STG_DESCRIPCION = ''Honorarios gestión ventas''';
		EXECUTE IMMEDIATE V_MSQL INTO V_DD_STG_ID;
		
		DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS EL REGISTRO ''111''');
		...
		V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.'||V_TABLA||'(DD_ETG_ID, DD_TGA_ID, DD_STG_ID
		  ...
		  ,''111'', ''HONORARIOS GESTORIA'', ''HONORARIOS GESTORIA''
		  ,3, 49, 4
		  ,0, ''REMVIP-6858'',SYSDATE,0)';
