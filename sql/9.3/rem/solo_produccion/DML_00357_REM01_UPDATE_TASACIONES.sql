...
BEGIN
    ...
    V_SQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_TAS_TASACION TAS USING(
		...
		WHERE TAS.USUARIOCREAR = ''REMVIP-7664'' AND PVE.PVE_FECHA_BAJA IS NULL AND PVE.DD_TPR_ID = (SELECT DD_TPR_ID FROM '||V_ESQUEMA||'.DD_TPR_TIPO_PROVEEDOR WHERE DD_TPR_CODIGO = ''02'')
	) AUX ON (TAS.TAS_ID = AUX.TAS_ID)
		WHEN MATCHED THEN UPDATE SET
		TAS.TAS_NOMBRE_TASADOR = AUX.PVE_NOMBRE_COMERCIAL,
		TAS.USUARIOMODIFICAR = ''REMVIP-7664'',
