...
--## PRODUCTO=NO
--##
--## Finalidad: Script que crea tramites.
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
BEGIN	
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        V_SQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ACT_TRA_TRAMITE WHERE TBJ_ID = (SELECT TBJ_ID FROM '||V_ESQUEMA||'.ACT_TBJ_TRABAJO  WHERE TBJ_NUM_TRABAJO = '''||TRIM(V_TMP_TIPO_DATA(1))||''')';
    ...
    V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.ACT_TRA_TRAMITE (TRA_ID, TBJ_ID, DD_TPO_ID, DD_EPR_ID
        , TRA_FECHA_INICIO, USUARIOCREAR, FECHACREAR, DD_TAC_ID)
    SELECT '|| V_ID ||', (SELECT TBJ_ID FROM '||V_ESQUEMA||'.ACT_TBJ_TRABAJO  WHERE TBJ_NUM_TRABAJO = '''||TRIM(V_TMP_TIPO_DATA(1))||'''),
        (SELECT TPO.DD_TPO_ID FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO TPO WHERE TPO.DD_TPO_CODIGO = ''T004''),
