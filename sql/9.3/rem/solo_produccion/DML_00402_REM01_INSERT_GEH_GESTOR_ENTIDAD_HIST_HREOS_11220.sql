...
BEGIN	
  ...
  EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
    ...
    -- LOOP para insertar los valores en TMP_GB_GESTOR_BOARDING -----------------------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERCION EN TMP_GB_GESTOR_BOARDING ');
    FOR I IN 1..V_NUM_TABLAS
      LOOP
          ...
          V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST (GEH_ID, USU_ID, DD_TGE_ID, GEH_FECHA_DESDE, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
                      SELECT '|| V_ID || '
                      , (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = ''gruboarding'')
                      , (SELECT DD_TGE_ID FROM '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR WHERE DD_TGE_CODIGO = ''GBOAR'')
                      , SYSDATE
                      , 0
                      , '''||V_INCIDENCIA||'''
                      , SYSDATE
    ...
    DBMS_OUTPUT.PUT_LINE('[FIN]: TMP_GB_GESTOR_BOARDING MODIFICADO CORRECTAMENTE ');
