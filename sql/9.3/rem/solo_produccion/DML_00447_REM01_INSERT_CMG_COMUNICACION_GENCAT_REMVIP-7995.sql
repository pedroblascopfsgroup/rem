...
DECLARE
    ...
    V_EXISTS NUMBER(1);
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-7995'; -- USUARIOCREAR/USUARIOMODIFICAR
    V_ACTIVO VARCHAR2(4000 CHAR) := '5938080,5938250,5928675,5962837,
							5963695,6815768,7296910,
							5968066,5967504,7296238,6973283,
							5931062,5933953,5950140,5967801,
							6989097,5958706,5953583'; --num_activo separados por coma
...
BEGIN
    ...
    V_MSQL := 'CREATE TABLE '||V_ESQUEMA||'.AUX_MIGRA_GENCAT AS
        ...
        FROM (
            ...
            WHERE CMG.CMG_ID IS NULL AND ACT.ACT_NUM_ACTIVO in ('||TRIM(V_ACTIVO)||'))
        WHERE RN = 1';
    EXECUTE IMMEDIATE V_MSQL;
