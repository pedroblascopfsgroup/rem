...
BEGIN	
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC
                USING (SELECT UNIQUE TAC.TAR_ID, (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_USERNAME||''') USU_ID
                    ...
                    AND TAP.TAP_CODIGO IN (''T013_InstruccionesReserva'',''T013_ObtencionContratoReserva'')
                    AND CRA.DD_CRA_CODIGO = ''01'' AND EEC.DD_EEC_CODIGO != ''02'' AND EOF.DD_EOF_CODIGO != ''02''
