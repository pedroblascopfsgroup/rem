...
DECLARE
	...
	V_CARTERA VARCHAR2(30 CHAR):= 'Sareb';
	
BEGIN
	DBMS_OUTPUT.PUT_LINE('[INICIO] ');

	V_MSQL := 'TRUNCATE TABLE AUX_REMVIP_8177';
	...
	IF V_COUNT = 1 THEN
			...
			-- ACTIVOS
			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.AUX_REMVIP_8177 (ACT_ID,ACT_NUM_ACTIVO,GEH_ID,DD_TGE_BY_GEH_ID,GEE_ID,DD_TGE_BY_GEE_ID,USU_ID_BY_GEE_ID)
			...
			DBMS_OUTPUT.PUT_LINE('[INFO] INSERTADOS  '|| SQL%ROWCOUNT ||' REGISTROS EN AUX_REMVIP_8177');
			...
			V_MSQL := 'UPDATE '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST GEH SET 
					...
					WHERE GEH_ID IN (SELECT GEH_ID FROM '||V_ESQUEMA||'.AUX_REMVIP_8177)';
			EXECUTE IMMEDIATE V_MSQL;
			...
			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST (GEH_ID,USU_ID,DD_TGE_ID, GEH_FECHA_DESDE, USUARIOCREAR, FECHACREAR, USUARIOMODIFICAR)
						...
						SYSDATE, '''||V_USUARIO||''',SYSDATE, TO_CHAR(AUX.ACT_ID) FROM '||V_ESQUEMA||'.AUX_REMVIP_8177 AUX';
			EXECUTE IMMEDIATE V_MSQL;  
			...
			V_MSQL := 'UPDATE '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE SET 
					...
					USUARIOMODIFICAR = '''||V_USUARIO||''',
					FECHAMODIFICAR = SYSDATE
					WHERE GEE_ID IN (SELECT GEE_ID FROM '||V_ESQUEMA||'.AUX_REMVIP_8177)';
			EXECUTE IMMEDIATE V_MSQL;
