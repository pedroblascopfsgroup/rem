...
BEGIN
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_HFP_HIST_FASES_PUB T1 
    	           USING(SELECT AUX.HFP_ID FROM REM01.AUX_REMVIP_7885_1 AUX
			INNER JOIN REM01.ACT_HFP_HIST_FASES_PUB HFP ON AUX.HFP_ID = HFP.HFP_ID
			WHERE AUX.HFP_TIPO = 1 AND ACT_ID NOT IN (SELECT ACT_ID FROM REM01.AUX_REMVIP_7885_1 AUX
			INNER JOIN REM01.ACT_HFP_HIST_FASES_PUB HFP ON AUX.HFP_ID = HFP.HFP_ID
			WHERE AUX.HFP_TIPO = 1 AND HFP.HFP_FECHA_FIN IS NOT NULL)) T2
    ...
    DBMS_OUTPUT.PUT_LINE('[BORRADO LOGICO REGISTORS AÑADIDOS ERRONEAMENTE EL DIA 20/06]');
       ...
       V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_HFP_HIST_FASES_PUB T1 
    	       USING(SELECT AUX.HFP_ID FROM REM01.AUX_REMVIP_7885_1 AUX
			INNER JOIN REM01.ACT_HFP_HIST_FASES_PUB HFP ON AUX.HFP_ID = HFP.HFP_ID
			WHERE AUX.HFP_TIPO = 2 AND ACT_ID NOT IN (SELECT ACT_ID FROM REM01.AUX_REMVIP_7885_1 AUX
			INNER JOIN REM01.ACT_HFP_HIST_FASES_PUB HFP ON AUX.HFP_ID = HFP.HFP_ID
			WHERE AUX.HFP_TIPO = 1 AND HFP.HFP_FECHA_FIN IS NOT NULL)) T2
                ON (T1.HFP_ID = T2.HFP_ID)
            WHEN MATCHED THEN UPDATE SET
    ...
    DBMS_OUTPUT.PUT_LINE('[ACTUALIZAMOS FECHA FIN A NULO EN LOS REGISTROS CORRECTOS]');
          ...
          V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_HFP_HIST_FASES_PUB T1 
    	       USING(SELECT AUX.HFP_ID FROM REM01.AUX_REMVIP_7885_1 AUX
		INNER JOIN REM01.ACT_HFP_HIST_FASES_PUB HFP ON AUX.HFP_ID = HFP.HFP_ID
		WHERE AUX.HFP_TIPO = 3 AND ACT_ID NOT IN (SELECT ACT_ID FROM REM01.AUX_REMVIP_7885_1 AUX
		INNER JOIN REM01.ACT_HFP_HIST_FASES_PUB HFP ON AUX.HFP_ID = HFP.HFP_ID
		WHERE AUX.HFP_TIPO = 1 AND HFP.HFP_FECHA_FIN IS NOT NULL)) T2
                ON (T1.HFP_ID = T2.HFP_ID)
            WHEN MATCHED THEN UPDATE SET		 		   		     
