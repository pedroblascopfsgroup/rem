...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-8310'; -- USUARIO CREAR/MODIFICAR
...
BEGIN	
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC
                USING (SELECT UNIQUE TAC.TAR_ID, (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_USERNAME||''') SUP_ID
                    ...
                    AND TAP.TAP_CODIGO=''T016_ProcesoAdecuacion'' AND EEC.DD_EEC_CODIGO != ''02'' AND EEC.DD_EEC_CODIGO != ''08'' AND EOF.DD_EOF_CODIGO != ''02'' AND USU.USU_USERNAME!='||V_USERNAME||'
                ) AUX ON (TAC.TAR_ID = AUX.TAR_ID)
                WHEN MATCHED THEN UPDATE SET 
                TAC.USU_ID = AUX.SUP_ID, TAC.USUARIOMODIFICAR = '''||V_USUARIO||''', TAC.FECHAMODIFICAR = SYSDATE';
