...
DECLARE
    ...
    V_USUARIO VARCHAR2(100 CHAR):='REMVIP-8318'; --Vble. auxiliar para almacenar el usuario
...
BEGIN	
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE ACT_NUM_ACTIVO = SUBSTR('''||V_TMP_TIPO_DATA(1)||''', 2, LENGTH('''||V_TMP_TIPO_DATA(1)||'''))';
        ...
        IF V_NUM_TABLAS = 0 THEN
            ...
            V_MSQL := 'SELECT SUBSTR(ACT_NUM_ACTIVO, 2, LENGTH(ACT_NUM_ACTIVO)) FROM '||V_ESQUEMA||'.'||V_TABLA||' WHERE ACT_NUM_ACTIVO = '''||V_TMP_TIPO_DATA(1)||'''';
		...
		ELSE
			...
			DBMS_OUTPUT.PUT_LINE('[INFO]: El activo '''||V_TMP_TIPO_DATA(1)||''' sin el prefijo ''-'' ya existe');
