...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-8330'; -- USUARIOCREAR/USUARIOMODIFICAR.
    V_TABLA_AUX VARCHAR( 100 CHAR ) := 'AUX_REMVIP_8330';
    V_COUNT NUMBER(16);	
    V_ID NUMBER(16);	
...
BEGIN		
	...
	OPEN v_cursor FOR V_SQL;
   	...
   	LOOP
       		...
       		REM01.SP_CAMBIO_ESTADO_PUBLICACION( V_ID, 1, 'REMVIP_8330' );
	...
	CLOSE v_cursor;    
	DBMS_OUTPUT.PUT_LINE(' [INFO] Se ha cambiado el estado de publicación de '||V_COUNT||' activos ');
	...
	OPEN v_cursor FOR V_SQL;
   	...
   	LOOP
       		...
       		REM01.SP_CAMBIO_ESTADO_PUBLI_AGR( V_ID, 1, 'REMVIP_8330' );
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] Asignando RATING de la carga masiva a los activos');
     
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO T1
				USING (
				    SELECT AUX.ACT_NUM_ACTIVO_UVEM, RTG.DD_RTG_ID
				    FROM '||V_ESQUEMA||'.AUX_REMVIP_8330 AUX 
				    JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_NUM_ACTIVO_UVEM = AUX.ACT_NUM_ACTIVO_UVEM 
				    JOIN '||V_ESQUEMA||'.DD_RTG_RATING_ACTIVO RTG ON AUX.DD_RTG_DESCRIPCION = RTG.DD_RTG_DESCRIPCION 
				) T2
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado los RATING de '||SQL%ROWCOUNT||' activos');
	...
	V_MSQL := ' MERGE INTO '|| V_ESQUEMA ||'.TAR_TAREAS_NOTIFICACIONES TAR
		    USING( 
				...
				FROM '||V_ESQUEMA||'.ACT_TRA_TRAMITE TRA, '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC, '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES TAR, '||V_ESQUEMA||'.ACT_ACTIVO ACT, '||V_ESQUEMA||'.AUX_REMVIP_8330 AUX
