...
--## PRODUCTO=NO
--## 
--## Finalidad: Carga masiva de aprobación del informe comercial y cargar rating a activos
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
...
DECLARE
    ...
    V_USU VARCHAR2(30 CHAR) := 'REMVIP-8420'; -- USUARIOCREAR/USUARIOMODIFICAR.
    V_TABLA_AUX VARCHAR( 100 CHAR ) := 'AUX_REMVIP_8420';
    V_COUNT NUMBER(16);	
    V_ID NUMBER(16);	
...
BEGIN		
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO] Ejecutándose SP recálculo de la situación comercial ');
    	...
    	V_MSQL := ' MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO ACT USING (
		   ...
		   FROM '||V_ESQUEMA||'.ACT_ACTIVO ACT 
		   INNER JOIN '||V_ESQUEMA||'.'||V_TABLA_AUX||' AUX ON ACT.ACT_NUM_ACTIVO_UVEM = AUX.ACT_NUM_ACTIVO) AUX
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_ACTIVO T1 USING (
			   SELECT ACT.ACT_ID, RTG.DD_RTG_ID
				...
				INNER JOIN '||V_ESQUEMA||'.DD_RTG_RATING_ACTIVO RTG ON RTG.DD_RTG_DESCRIPCION = AUX.RATING AND RTG.BORRADO = 0
	...
	DBMS_OUTPUT.PUT_LINE('[INFO]: '||SQL%ROWCOUNT||' ACTIVOS CARGADOS CON RATING INFORMADO');
