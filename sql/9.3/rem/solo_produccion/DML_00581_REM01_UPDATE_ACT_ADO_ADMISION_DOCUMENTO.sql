...
BEGIN	
    ...
    V_MSQL := 'MERGE INTO '|| V_ESQUEMA ||'.'|| V_TABLA ||' T1 USING 
			(SELECT ADO.ADO_ID, ACT.DD_TPA_ID, ACT.DD_SAC_ID FROM ACT_ACTIVO ACT 
            INNER JOIN ACT_ADO_ADMISION_DOCUMENTO ADO ON ADO.ACT_ID = ACT.ACT_ID
            INNER JOIN ACT_CFD_CONFIG_DOCUMENTO CFD ON CFD.CFD_ID = ADO.CFD_ID
