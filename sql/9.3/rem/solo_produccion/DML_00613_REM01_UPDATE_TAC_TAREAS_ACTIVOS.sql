...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-8628'; -- USUARIO CREAR/MODIFICAR
...
BEGIN	
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS T1
                USING (SELECT DISTINCT TAC.TAR_ID FROM '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO
                    ...
                    INNER JOIN '||V_ESQUEMA_M||'.USU_USUARIOS USU ON TAC.USU_ID = USU.USU_ID AND USU.BORRADO=0
                    INNER JOIN '||V_ESQUEMA_M||'.USU_USUARIOS SUP ON TAC.SUP_ID = SUP.USU_ID AND SUP.BORRADO=0
                    ...
                    AND TAC.SUP_ID != 91906 
                    AND CRA.DD_CRA_CODIGO = ''02'' 
                    AND EEC.DD_EEC_CODIGO != ''02'' 
                    AND EEC.DD_EEC_CODIGO != ''08''
                    AND EOF.DD_EOF_CODIGO != ''02''
                    AND TAP.TAP_CODIGO IN (''T013_InstruccionesReserva'',''T013_DefinicionOferta'',''T013_FirmaPropietario'',''T013_ResolucionComite'',''T013_ResolucionExpediente'',''T013_RespuestaOfertante'',''T013_ObtencionContratoReserva'')
                ) T2 
                ON (T1.TAR_ID = T2.TAR_ID)
                WHEN MATCHED THEN UPDATE SET 
                T1.SUP_ID = (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_USERNAME||'''),
