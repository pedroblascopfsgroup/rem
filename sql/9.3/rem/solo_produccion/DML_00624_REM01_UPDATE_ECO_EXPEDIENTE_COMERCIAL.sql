...
BEGIN	
		...
		DBMS_OUTPUT.PUT_LINE('[INFO] Se va a modificar el comité sancionador.');
        ...
        IF V_NUM_TABLAS=1 THEN
            ...
            V_MSQL:='SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_COS_COMITE_SANCION WHERE DD_COS_CODIGO='||V_COMITE_CODIGO||' AND BORRADO=0';
            EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;

            IF V_NUM_TABLAS=1 THEN

                V_MSQL :=   'UPDATE '||V_ESQUEMA||'.'||V_TABLA||' 
                            SET DD_COS_ID = (SELECT DD_COS_ID FROM '||V_ESQUEMA||'.DD_COS_COMITE_SANCION WHERE DD_COS_CODIGO='||V_COMITE_CODIGO||'),
                            SET DD_COS_ID_PROPUESTO = (SELECT DD_COS_ID FROM '||V_ESQUEMA||'.DD_COS_COMITE_SANCION WHERE DD_COS_CODIGO='||V_COMITE_CODIGO||'),
