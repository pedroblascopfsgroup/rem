...
BEGIN
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.GDE_GASTOS_DETALLE_ECONOMICO T1 USING (
					SELECT DISTINCT GPV.GPV_ID, AUX.FECHA_PAGO FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
                    INNER JOIN '||V_ESQUEMA||'.AUX_REMVIP_8813 AUX ON GPV.GPV_NUM_GASTO_HAYA = AUX.GPV_NUM_GASTO_HAYA
                    WHERE GPV.BORRADO = 0 AND GPV.DD_EGA_ID = '||V_ID_PAGADO||') T2
				ON (T1.GPV_ID = T2.GPV_ID)
				WHEN MATCHED THEN UPDATE SET
				GDE_FECHA_PAGO = T2.FECHA_PAGO,
				USUARIOMODIFICAR = '''||V_USUARIO||''',
				FECHAMODIFICAR = SYSDATE';
	EXECUTE IMMEDIATE V_MSQL;

	DBMS_OUTPUT.PUT_LINE('[INFO] MODIFICADOS  '|| SQL%ROWCOUNT ||' REGISTROS EN GDE_GASTOS_DETALLE_ECONOMICO');  
