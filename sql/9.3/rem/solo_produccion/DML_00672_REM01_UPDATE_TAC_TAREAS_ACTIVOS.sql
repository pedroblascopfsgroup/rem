...
BEGIN	
    ...
    IF V_COUNT = 1 THEN
        ...
        DBMS_OUTPUT.PUT_LINE('[INFO]: REASIGNACIÓN DE GESTORES DE USUARIO '''||V_USUARIO_ORIGEN||''' A USUARIO '''||V_USUARIO_DESTINO||'''');  

        V_MSQL := 'UPDATE '||V_ESQUEMA_M||'.GRU_GRUPOS_USUARIOS SET
                    USUARIOMODIFICAR = '''||V_USUARIO||''',
                    FECHAMODIFICAR = SYSDATE,
                    USU_ID_USUARIO = '||V_ID_USU_DESTINO||'
                    WHERE USU_ID_USUARIO = '||V_ID_USU_ORIGEN||'';

        EXECUTE IMMEDIATE V_MSQL;

        DBMS_OUTPUT.PUT_LINE('[INFO]: '||sql%rowcount||' GESTORES REASIGNADOS A '''||V_USUARIO_DESTINO||'''');

        DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------------------------');  

        DBMS_OUTPUT.PUT_LINE('[INFO]: REASIGNACIÓN DE DESPACHOS DE USUARIO '''||V_USUARIO_ORIGEN||''' A USUARIO '''||V_USUARIO_DESTINO||'''');  

        V_MSQL := 'UPDATE '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS SET
                    USUARIOMODIFICAR = '''||V_USUARIO||''',
                    FECHAMODIFICAR = SYSDATE,
                    USU_ID = '||V_ID_USU_DESTINO||'
                    WHERE USU_ID = '||V_ID_USU_ORIGEN||'';

        EXECUTE IMMEDIATE V_MSQL;

        DBMS_OUTPUT.PUT_LINE('[INFO]: '||sql%rowcount||' DESPACHOS REASIGNADOS A '''||V_USUARIO_DESTINO||'''');

        DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------------------------');  

        DBMS_OUTPUT.PUT_LINE('[INFO]: REASIGNACIÓN DE PERFILES DE USUARIO '''||V_USUARIO_ORIGEN||''' A USUARIO '''||V_USUARIO_DESTINO||'''');  
        ...
        DBMS_OUTPUT.PUT_LINE('[INFO]: '||sql%rowcount||' PERFILES REASIGNADOS A '''||V_USUARIO_DESTINO||'''');
