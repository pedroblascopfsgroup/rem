...
BEGIN
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_PVC_PROVEEDOR_CONTACTO T1 USING (
					...
					FROM '||V_ESQUEMA||'.ACT_PVE_PROVEEDOR PVE WHERE NOT EXISTS (SELECT PVC_ID FROM '||V_ESQUEMA||'.ACT_PVC_PROVEEDOR_CONTACTO PVC WHERE PVE.PVE_ID = PVC.PVE_ID AND BORRADO = 0) AND BORRADO = 0
				) T2
