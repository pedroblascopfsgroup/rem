...
DECLARE
    ...
    V_USR VARCHAR2(50 CHAR) := 'REMVIP-9011';
	...
	V_OFERTA NUMBER(16):= 90286248;
    
BEGIN
	
	DBMS_OUTPUT.PUT_LINE('[INICIO]: REPOSICIONAR TAREAS');

	EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.OFR_OFERTAS WHERE OFR_NUM_OFERTA = '||V_OFERTA||' AND DD_EOF_ID = 1' INTO V_NUM;
        
    IF V_NUM > 0 THEN
		
		EXECUTE IMMEDIATE 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL WHERE OFR_ID = (SELECT OFR_ID FROM '||V_ESQUEMA||'.OFR_OFERTAS WHERE OFR_NUM_OFERTA = '||V_OFERTA||') AND DD_EEC_ID != 2' INTO V_NUM;	

		IF V_NUM > 0 THEN

			EXECUTE IMMEDIATE 'SELECT ECO_ID FROM '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL WHERE OFR_ID = (SELECT OFR_ID FROM '||V_ESQUEMA||'.OFR_OFERTAS WHERE OFR_NUM_OFERTA = '||V_OFERTA||')' INTO V_ECO_ID;
			...
			FOR CURSOR IN (SELECT 
				...
				WHERE TRA.USUARIOMODIFICAR = 'REMVIP-9011'
				AND TAP_CODIGO IN (
				'T017_AdvisoryNote',
