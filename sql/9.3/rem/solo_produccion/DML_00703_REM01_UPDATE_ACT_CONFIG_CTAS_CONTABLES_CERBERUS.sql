...
DECLARE
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        T_TIPO_DATA('6230600090', '14', '114', '138'),
        T_TIPO_DATA('6233011', '14', '114', '151'),
        T_TIPO_DATA('6233011', '14', '114', '152')
		); 
...
BEGIN	
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS LA CUENTA ''6230600020'' POR LA CUENTA ''6230600021'' EN TIPO GASTO ''Servicios profesionales independientes''
     CON SUBTIPO ''CFK'' EN LA SUBCARTERA APPLE');

    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||'  T1 USING (
                SELECT DISTINCT CCC_CTAS_ID,CCC_CUENTA_CONTABLE
                          FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' 
                          WHERE DD_TGA_ID = 11
                AND DD_STG_ID = 95
                AND DD_CRA_ID = 42
                AND DD_SCR_ID = 323
                AND CCC_CUENTA_CONTABLE = ''6230600020''
                AND BORRADO = 0) T2
                ON (T1.CCC_CTAS_ID = T2.CCC_CTAS_ID)
                WHEN MATCHED THEN UPDATE SET 
                T1.CCC_CUENTA_CONTABLE = ''6230600021'',
                T1.USUARIOMODIFICAR = '''||V_ITEM||''',
                T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('[INFO]: '||SQL%ROWCOUNT||' REGISTRO/S MODIFICADO/S CORRECTAMENTE.');

    DBMS_OUTPUT.PUT_LINE('[INFO]: MODIFICAMOS LA CUENTA ''6230000'' POR LA CUENTA ''6230010'' EN TIPO GASTO ''Servicios profesionales independientes''
     CON SUBTIPO ''CFK'' EN LA SUBCARTERA DIVARIAN');
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||'  T1 USING (
                ...
                AND DD_STG_ID = 95
                AND DD_CRA_ID = 42
                AND DD_SCR_ID IN (443,444)
                AND CCC_CUENTA_CONTABLE = ''6230000''
                AND BORRADO = 0) T2
                ON (T1.CCC_CTAS_ID = T2.CCC_CTAS_ID)
                WHEN MATCHED THEN UPDATE SET 
                T1.CCC_CUENTA_CONTABLE = ''6230010'',
                T1.USUARIOMODIFICAR = '''||V_ITEM||''',
                T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('[INFO]: '||SQL%ROWCOUNT||' REGISTRO/S MODIFICADOS/S CORRECTAMENTE.');
