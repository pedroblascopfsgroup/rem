...
BEGIN	
	...
	LOOP
		...
		IF V_COUNT = 1 THEN
			...
			V_MSQL:='SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' WHERE BBVA_ID = '||V_TMP_TIPO_DATA(1)||' AND BBVA_COD_PROMOCION IS NULL';
			EXECUTE IMMEDIATE V_MSQL INTO V_COUNT;
			...
			DBMS_OUTPUT.PUT_LINE('[INFO]: EL ACTIVO CON BBVA_ID '''||V_TMP_TIPO_DATA(1)||''' YA TIENE INFORMADO EL CODIGO PROMOCION BBVA');
