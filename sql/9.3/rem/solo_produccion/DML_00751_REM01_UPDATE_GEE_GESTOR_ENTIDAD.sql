...
DECLARE
    ...
    V_TEXT_TABLA VARCHAR2(27 CHAR) := 'GEE_GESTOR_ENTIDAD'; -- Vble. auxiliar para almacenar el nombre de la tabla de ref.
...
BEGIN	
	...
	IF V_COUNT = 1 THEN
		...
		V_MSQL:= 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' T1 USING(
					SELECT GEE.GEE_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS USU
					INNER JOIN '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD GEE ON USU.USU_ID = GEE.USU_ID
					WHERE USU.USU_USERNAME = ''lsanchis'' AND GEE.DD_TGE_ID = '||V_TGE_ID||'
					) T2
