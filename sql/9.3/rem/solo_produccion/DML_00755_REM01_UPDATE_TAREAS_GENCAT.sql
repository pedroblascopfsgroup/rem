...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-9120'; -- USUARIO CREAR/MODIFICAR
...
BEGIN	
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS T1
                USING (SELECT DISTINCT TAC.TAR_ID, (SELECT USU_ID FROM '||V_ESQUEMA_M||'.USU_USUARIOS WHERE USU_USERNAME = '''||V_USERNAME||''') ID_GRUPO
                    ...
                    WHERE ECO.BORRADO = 0 AND CRA.DD_CRA_CODIGO = ''16'' AND TAR.TAR_TAREA_FINALIZADA = 0
                    AND TAP.TAP_CODIGO IN (''T016_ProcesoAdecuacion'', ''T016_ComunicarGENCAT'') AND EEC.DD_EEC_CODIGO != ''02'' AND EEC.DD_EEC_CODIGO != ''08'' AND EOF.DD_EOF_CODIGO != ''02''
                    AND USU.USU_USERNAME != '''||V_USERNAME||''') T2 
                    ON (T1.TAR_ID = T2.TAR_ID)
                WHEN MATCHED THEN UPDATE SET 
                T1.USU_ID = T2.ID_GRUPO, T1.USUARIOMODIFICAR = '''||V_USUARIO||''', T1.FECHAMODIFICAR = SYSDATE';
