...
BEGIN
    ...
    IF V_COUNT = 1 THEN
        ...
        V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA_LINEA||' T1
                    USING (SELECT DISTINCT GLD.GLD_ID FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
                            JOIN '||V_ESQUEMA||'.GLD_GASTOS_LINEA_DETALLE GLD ON GPV.GPV_ID=GLD.GPV_ID                            
                            WHERE GPV.GPV_NUM_GASTO_HAYA='||V_GASTO||' AND GLD.GLD_ID='||V_GLD_ID||' AND GPV.BORRADO=0 AND GLD.BORRADO=1
                            ) T2 
        ...
        DBMS_OUTPUT.PUT_LINE('[INFO] MODIFICADOS  '|| SQL%ROWCOUNT ||' REGISTROS EN '||V_TABLA_LINEA||' ');
            ...
            V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA_LINEA_TBJ||' T1
                    USING (SELECT DISTINCT GLTJ.GLD_TBJ_ID FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR GPV
                            JOIN '||V_ESQUEMA||'.GLD_GASTOS_LINEA_DETALLE GLD ON GPV.GPV_ID=GLD.GPV_ID
                            JOIN '||V_ESQUEMA||'.GLD_TBJ GLTJ ON GLTJ.GLD_ID=GLD.GLD_ID
                            WHERE GPV.GPV_NUM_GASTO_HAYA='||V_GASTO||' 
                            AND GPV.BORRADO=0 AND GLTJ.BORRADO=1
                            ) T2 
