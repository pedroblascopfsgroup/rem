...
DECLARE
    ...
    V_USU VARCHAR2(30 CHAR) := 'REMVIP-9157'; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.
	V_COUNT NUMBER(16);
	V_TBJ_ID NUMBER(16);
...
BEGIN		
	...
	V_MSQL:= 'UPDATE '||V_ESQUEMA||'.ACT_ICO_INFO_COMERCIAL SET
				ICO_ADMITE_MASCOTAS = 1,
				USUARIOMODIFICAR = '''||V_USU||''',
				FECHAMODIFICAR = SYSDATE
				WHERE ACT_ID IN (SELECT ACT_ID FROM '||V_ESQUEMA||'.ACT_ACTIVO WHERE ACT_PERIMETRO_MACC = 1 AND BORRADO = 0)';
	EXECUTE IMMEDIATE V_MSQL;

	DBMS_OUTPUT.PUT_LINE('[INFO] Se han actualizado correctamente '||SQL%ROWCOUNT||' registros en ACT_ICO_INFO_COMERCIAL');   
