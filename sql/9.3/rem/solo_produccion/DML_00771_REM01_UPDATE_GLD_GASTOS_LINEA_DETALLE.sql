...
DECLARE
    ...
    V_USU VARCHAR2(30 CHAR) := 'REMVIP-9254'; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.
...
BEGIN		
	...
	V_MSQL:= 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' T1 USING (
					...
					JOIN '||V_ESQUEMA||'.AUX_REMVIP_9254 AUX ON AUX.GPV_NUM_GASTO_HAYA = GPV.GPV_NUM_GASTO_HAYA
					WHERE GPV.BORRADO = 0) T2
				ON (T1.GLD_ID = T2.GLD_ID)
				WHEN MATCHED THEN UPDATE SET
				T1.GLD_CCC_BASE = ''6310000001'',
				T1.GLD_CPP_BASE = ''PP051'',
				T1.USUARIOMODIFICAR = '''||V_USU||''',
				T1.FECHAMODIFICAR = SYSDATE';
	EXECUTE IMMEDIATE V_MSQL;
	
	DBMS_OUTPUT.PUT_LINE('[INFO] MODIFICADOS GLD_CCC_BASE = 6310000001 Y GLD_CPP_BASE = PP051 EN '|| SQL%ROWCOUNT ||' GASTOS DE '||V_TEXT_TABLA);
