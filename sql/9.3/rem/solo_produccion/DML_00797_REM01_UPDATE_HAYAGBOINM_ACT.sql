...
--## PRODUCTO=NO
--## 
--## Finalidad: Actualización GESTOR backoffice inmobiliario
--##			
--## INSTRUCCIONES:  
--## VERSIONES:
--##        0.1 Versión inicial
	...
	IF V_COUNT = 1 THEN
			...
			V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.AUX_REMVIP_9367 (ACT_ID,ACT_NUM_ACTIVO,GEH_ID,DD_TGE_BY_GEH_ID,GEE_ID,DD_TGE_BY_GEE_ID,USU_ID_BY_GEE_ID)
							...
							INNER JOIN '||V_ESQUEMA||'.GEH_GESTOR_ENTIDAD_HIST GEH ON GEH.GEH_ID = GAH.GEH_ID
								...
								AND GEH.BORRADO = 0
							AND ACT.DD_CRA_ID = (SELECT DD_CRA_ID FROM '||V_ESQUEMA||'.DD_CRA_CARTERA WHERE DD_CRA_CODIGO = '''||V_CARTERA||''')
							AND ACT.BORRADO = 0';
			EXECUTE IMMEDIATE V_MSQL;
