...
DECLARE
    ...
    V_USU VARCHAR2(30 CHAR) := 'REMVIP-9446'; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.
	V_COUNT NUMBER(16);
...
BEGIN		
	...
	V_MSQL:= 'MERGE INTO '||V_ESQUEMA||'.BLK_OFR T1 USING (
					SELECT DISTINCT OFR_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' OFR
					JOIN '||V_ESQUEMA||'.BLK_OFR BLK ON BLK.OFR_ID = OFR.OFR_ID AND BLK.BORRADO = 0
					WHERE OFR.OFR_NUM_OFERTA = 90289192 AND OFR.BORRADO = 0) T2
