	...
	V_SQL := '
		MERGE INTO '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS T1
			USING (
				...
				INNER JOIN '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC ON ACT.ACT_ID = TAC.ACT_ID AND TAC.BORRADO = 0
				INNER JOIN '||V_ESQUEMA||'.ACT_TRA_TRAMITE TRA ON TAC.TRA_ID = TRA.TRA_ID AND TRA.DD_TPO_ID = 3449 AND TRA.BORRADO = 0
				INNER JOIN '||V_ESQUEMA_M||'.USU_USUARIOS USU ON AUX.USU_USERNAME = USU.USU_USERNAME AND USU.BORRADO = 0
				) T2
