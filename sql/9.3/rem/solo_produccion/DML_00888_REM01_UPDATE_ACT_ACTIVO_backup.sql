...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('[INICIO]');	
  	 ...
  	 V_MSQL :='MERGE INTO '||V_ESQUEMA||'.'||V_TABLA_ACTIVO ||' T1 
                USING (
                    SELECT DISTINCT ACT.ACT_ID,AUX.DD_TPA_ID,AUX.DD_SAC_ID FROM '||V_ESQUEMA||'.'||V_TABLA_ACTIVO ||' ACT                       
                        JOIN '||V_ESQUEMA||'.'||V_TABLA_AUX||' AUX ON AUX.ACT_ID=ACT.ACT_ID
                        WHERE ACT.BORRADO=0
                    ) T2
