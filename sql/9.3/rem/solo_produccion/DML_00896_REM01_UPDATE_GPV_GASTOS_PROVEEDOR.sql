...
DECLARE
    ...
    V_USUARIO VARCHAR2(30 CHAR) := 'REMVIP-9900'; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.
	V_COUNT NUMBER(16);
	V_ID NUMBER(16);
	...
	TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(32000 CHAR);
    	...
    	V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
                T_TIPO_DATA('13937626'),
                T_TIPO_DATA('13957337')
    ...
    V_TMP_TIPO_DATA := V_TIPO_DATA(I);
        ...
        IF V_COUNT = 1 THEN
            ...
            V_MSQL:= 'UPDATE '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR SET
               DD_TGA_ID = (SELECT DD_TGA_ID FROM '||V_ESQUEMA||'.DD_TGA_TIPOS_GASTO WHERE DD_TGA_CODIGO = ''02''),  
            ...
            V_MSQL:= 'UPDATE '||V_ESQUEMA||'.GLD_GASTOS_LINEA_DETALLE SET
               DD_STG_ID = (SELECT DD_STG_ID FROM '||V_ESQUEMA||'.DD_STG_SUBTIPOS_GASTO WHERE DD_STG_CODIGO = ''17''),  
               USUARIOMODIFICAR = '''||V_USUARIO||''',
               GLD_CCC_BASE = ''6312016'',
               GLD_CPP_BASE = ''PP082'',
               FECHAMODIFICAR = SYSDATE               
               WHERE GPV_ID = '||V_ID||' AND BORRADO = 0';
            EXECUTE IMMEDIATE V_MSQL;
