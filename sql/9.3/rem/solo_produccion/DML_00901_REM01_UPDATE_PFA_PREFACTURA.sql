...
DECLARE
    ...
    V_TABLA VARCHAR2(30 CHAR) := 'PFA_PREFACTURA';  -- Tabla a modificar
...
BEGIN
        ...
        V_MSQL :=   'UPDATE '||V_ESQUEMA||'.'||V_TABLA||'
                    SET PVE_ID = 18904
                    WHERE PFA_ID IN (SELECT PFA_ID FROM '||V_ESQUEMA||'.ACT_TBJ_TRABAJO WHERE PVC_ID = 1895 AND DD_EST_ID IN (SELECT DD_EST_ID FROM '||V_ESQUEMA||'.DD_EST_ESTADO_TRABAJO WHERE DD_EST_CODIGO IN (''13'', ''CUR'', ''PCI'')))';
