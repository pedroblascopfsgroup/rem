...
BEGIN			
	...
	HORA_INI := SYSTIMESTAMP;
	    ...
	    DBMS_OUTPUT.put_line('[INICIO] Ejecutando actualizacion estados publicacion agrupaciones ...........'||HORA_INI||' ');
	  
	       	 
		OPEN ESTADO_PUBLI_RECALCULAR_AGR;
	
		V_COUNT := 0;
		V_COUNT2 := 0;
		
		LOOP
	  		FETCH ESTADO_PUBLI_RECALCULAR_AGR INTO FILA_AGR;
	  		EXIT WHEN ESTADO_PUBLI_RECALCULAR_AGR%NOTFOUND;
	  				
	  		REM01.SP_CAMBIO_ESTADO_PUBLI_AGR_VM (FILA_AGR.AGR_ID,1,''||V_USUARIOMODIFICAR||'');

			-- Actualiza el campo procesado = 1		
			V_MSQL := 'UPDATE '||V_ESQUEMA||'.AUX_ESPARTA_PUBL_AGR
		   		   SET PROCESADO = 1
			           WHERE AGR_ID = ' || FILA_AGR.AGR_ID ;
	    ...
	    CLOSE ESTADO_PUBLI_RECALCULAR_AGR;
