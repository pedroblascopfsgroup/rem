...
--## INCIDENCIA_LINK=REMVIP-10479
--## PRODUCTO=NO
--##
--## Finalidad: Script excluir bulk
--## INSTRUCCIONES:
--## VERSIONES:
--##        0.1 Versión inicial
--##########################################
...
DECLARE
    ...
    V_USU VARCHAR2(30 CHAR) := 'REMVIP-10479'; -- Vble. auxiliar para almacenar el nombre de usuario que modifica los registros.
	V_COUNT NUMBER(16);
...
BEGIN		
	...
	V_MSQL:= 'MERGE INTO '||V_ESQUEMA||'.BLK_BULK_ADVISORY_NOTE T1 USING (
					SELECT DISTINCT BLK.BLK_ID FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' OFR
					JOIN '||V_ESQUEMA||'.BLK_OFR BLK ON BLK.OFR_ID = OFR.OFR_ID AND BLK.BORRADO = 0
					WHERE OFR.OFR_NUM_OFERTA = 90340222 AND OFR.BORRADO = 0) T2
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] MODIFICADOS '||SQL%ROWCOUNT||' ID ADVISORY NOTE'); 
