...
BEGIN 
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] ACTUALIZACIÓN NUMERO DE EXPEDIENTES.');
         ...
         V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO USING(
			SELECT DISTINCT ECO.ECO_ID, ECO2.ECO_NUM_EXPEDIENTE FROM '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO 
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||' Actualizadas '||SQL%ROWCOUNT||' Filas.');
           ...
           V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO USING(
		SELECT DISTINCT ECO.ECO_ID, LPAD(ECO_NUM_EXPEDIENTE,7,9) AS NUEVO_EXPEDIENTE FROM '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO
		...
		ECO.USUARIOMODIFICAR = ''REMVIP-10757'',
		ECO.FECHAMODIFICAR = SYSDATE';https://meet.google.com/jdu-zgue-eyi
