...
DECLARE
    ...
    V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-10784'; -- USUARIO CREAR/MODIFICAR
    V_CRA_CODIGO VARCHAR2(50 CHAR) := '17';
		...
		USING (
		   ...
		   FROM '||V_ESQUEMA||'.ACT_ACTIVO aa 
                ...
                WHERE 
                    ...
                    AND dttg.DD_TGE_CODIGO IN (''GCOM'',''HAYAGBOINM'') ) T2
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.GEE_GESTOR_ENTIDAD T1
		USING (
		   ...
		   FROM REM01.ACT_ACTIVO aa 
                    ...
                    WHERE 
                        aa.DD_CRA_ID = (SELECT DD_CRA_ID FROM '||V_ESQUEMA||'.DD_CRA_CARTERA WHERE DD_CRA_CODIGO = '''||V_CRA_CODIGO||''')
                        AND dttg.DD_TGE_CODIGO IN (''GCOM'',''HAYAGBOINM'') 
