...
DECLARE
    ...
    V_ESQUEMA_M VARCHAR2(25 CHAR):= 'REMMASTER'; -- #ESQUEMA_MASTER# Configuracion Esquema
    ERR_NUM NUMBER(25);  -- Vble. auxiliar para registrar errores en el script.
    ERR_MSG VARCHAR2(1024 CHAR); -- Vble. auxiliar para registrar errores en el script.
    V_USR VARCHAR2(50 CHAR) := 'REMVIP-9999';
...
BEGIN
	...
	FOR CURSOR IN (SELECT 
		...
		WHERE TRA.USUARIOMODIFICAR = 'MIG_CAIXA'
		AND TAP_CODIGO IN (
		'T017_AdvisoryNote',
		...
		IF CURSOR.TAR_TAREA_FINALIZADA = 0 THEN
			IF CURSOR.PARENT_ IS NULL THEN
				...
				'||TOKEN_ID||', 0, (SELECT NAME_ FROM '||V_ESQUEMA_M||'.JBPM_NODE WHERE ID_ = '||CURSOR.NODE_PARENT||'), SYSDATE, NULL, SYSDATE, 99, 1, 0, 0, NULL, '||CURSOR.NODE_PARENT||', '||CURSOR.TRA_PROCESS_BPM||', NULL, NULL, NULL, NULL)';
			...
			ELSE
				...
				V_MSQL := 'DELETE FROM '||V_ESQUEMA_M||'.JBPM_MODULEINSTANCE WHERE ID_ = (SELECT CONTEXTINSTANCE_ FROM '||V_ESQUEMA_M||'.JBPM_TOKENVARIABLEMAP WHERE TOKEN_ = '||CURSOR.TEX_TOKEN_ID_BPM;
				EXECUTE IMMEDIATE V_MSQL;
				V_MSQL := 'DELETE FROM '||V_ESQUEMA_M||'.JBPM_VARIABLEINSTANCE WHERE TOKEN_ = '||CURSOR.TEX_TOKEN_ID_BPM;
				EXECUTE IMMEDIATE V_MSQL;
				V_MSQL := 'DELETE FROM '||V_ESQUEMA_M||'.JBPM_TOKENVARIABLEMAP WHERE TOKEN_ = '||CURSOR.TEX_TOKEN_ID_BPM;
				EXECUTE IMMEDIATE V_MSQL;
