...
V_USUARIO VARCHAR2(20 CHAR) := 'REMVIP-11159';
...
BEGIN
    ...
    V_MSQL :='SELECT TO_CHAR('''||V_USUARIO||''' || ''_'' || '''||V_TMP_TIPO_DATA(1)||''')  FROM REM01.eco_expediente_comercial ECO
WHERE ECO.ECO_NUM_EXPEDIENTE='||V_TMP_TIPO_DATA(1)||'';
EXECUTE IMMEDIATE V_MSQL INTO V_USUARIO2;
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_TRA_TRAMITE TRA USING(
					SELECT DISTINCT TRA.TRA_ID
                        ...
                        ''T017_PBCVenta'') AND TAP.BORRADO = 0
						INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON TAC.ACT_ID = ACT.ACT_ID AND ACT.BORRADO = 0
						AND TAC.BORRADO = 0 AND ACT.DD_CRA_ID = 162 AND tar.tar_fecha_ini < to_date(''03-02-2022'',''DD-MM-YYYY'')) AUX
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.TEX_TAREA_EXTERNA TEX USING(
					SELECT DISTINCT TEX.TEX_ID
                        ...
                        ''T017_PBCVenta'') AND TAP.BORRADO = 0
						INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON TAC.ACT_ID = ACT.ACT_ID AND ACT.BORRADO = 0
						AND TAC.BORRADO = 0 AND ACT.DD_CRA_ID = 162 AND eco.eco_num_expediente in ('||V_TMP_TIPO_DATA(1)||') ) AUX
	...
	(SELECT ID_ FROM REMMASTER.JBPM_NODE WHERE NAME_ = 'T017_PBCVenta' AND processdefinition_ = AUX.processdefinition_)
   		ELSE (SELECT ID_ FROM REMMASTER.JBPM_NODE WHERE NAME_ = 'T017_PBCVenta' AND processdefinition_ = AUX.processdefinition_) END AS NODE_PARENT,
		...
		AND TAP_CODIGO IN ('T017_PBCVenta')
