...
DBMS_OUTPUT.PUT_LINE('[INICIO]');
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
      LOOP
        ...
        V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.DD_REC_RECOMENDACION_RCDC WHERE DD_REC_CODIGO = '''||V_TMP_TIPO_DATA(6)||''' AND BORRADO = 0';
        EXECUTE IMMEDIATE V_MSQL INTO V_NUM_REC;

        IF V_NUM_CRA = 1 AND V_NUM_SCR = 1 AND V_NUM_TCO = 1 AND V_NUM_EQG = 1 AND V_NUM_REC = 1 THEN	

            V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' COR
                            ...
                            WHERE COR.BORRADO = 0 AND DD_CRA_CODIGO = '''||V_TMP_TIPO_DATA(1)||''' AND DD_SCR_CODIGO = '''||V_TMP_TIPO_DATA(2)||'''
                            AND DD_TCO_CODIGO = '''||V_TMP_TIPO_DATA(3)||''' AND DD_EQG_CODIGO = '''||V_TMP_TIPO_DATA(4)||''' AND DD_REC_CODIGO = '''||V_TMP_TIPO_DATA(6)||'''';
            EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
            -- Si existe se modifica.
            IF V_NUM_TABLAS = 0 THEN				
            
                    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS REGISTRO');
                    V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' (COR_ID,DD_CRA_ID,DD_SCR_ID,DD_TCO_ID,DD_EQG_ID,COR_PORCENTAJE_DESC,
                                        ...
                                        '''||V_TMP_TIPO_DATA(5)||''', 
                                        (SELECT DD_REC_ID FROM '||V_ESQUEMA||'.DD_REC_RECOMENDACION_RCDC WHERE DD_REC_CODIGO = '''||V_TMP_TIPO_DATA(6)||'''),
                                         '''||V_USUARIO||''', SYSDATE)';
            ...
            ELSE
                ...
                DBMS_OUTPUT.PUT_LINE('[INFO]: YA EXISTE UNA CONFIGURACION:  CARTERA = '''||V_TMP_TIPO_DATA(1)||''',
                                    ...
                                    EQUIPO_GESTION = '''||V_TMP_TIPO_DATA(4)||''', RECOMENDACION = '''||V_TMP_TIPO_DATA(6)||'''');

            END IF;

        ELSIF V_NUM_CRA = 1 AND V_NUM_SCR = 0 AND V_NUM_TCO = 1 AND V_NUM_EQG = 1 AND V_NUM_REC = 1 THEN	
            ...
            V_MSQL := 'SELECT COUNT(1) FROM '||V_ESQUEMA||'.'||V_TEXT_TABLA||' COR
                            ...
                            JOIN '||V_ESQUEMA||'.DD_EQG_EQUIPO_GESTION EQG ON EQG.DD_EQG_ID = COR.DD_EQG_ID AND EQG.BORRADO = 0
                            JOIN '||V_ESQUEMA||'.DD_REC_RECOMENDACION_RCDC REC ON REC.DD_REC_ID = COR.DD_REC_ID AND REC.BORRADO = 0
                            WHERE COR.BORRADO = 0 AND DD_CRA_CODIGO = '''||V_TMP_TIPO_DATA(1)||''' AND COR.DD_SCR_ID IS NULL
                            AND DD_TCO_CODIGO = '''||V_TMP_TIPO_DATA(3)||''' AND DD_EQG_CODIGO = '''||V_TMP_TIPO_DATA(4)||''' AND DD_REC_CODIGO = '''||V_TMP_TIPO_DATA(6)||'''';
            EXECUTE IMMEDIATE V_MSQL INTO V_NUM_TABLAS;
            -- Si existe se modifica.
            IF V_NUM_TABLAS = 0 THEN				
                    ...
                    DBMS_OUTPUT.PUT_LINE('[INFO]: INSERTAMOS REGISTRO');
                    V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.'||V_TEXT_TABLA||' (COR_ID,DD_CRA_ID,DD_TCO_ID,DD_EQG_ID,COR_PORCENTAJE_DESC,
                                        DD_REC_ID,USUARIOCREAR,FECHACREAR) VALUES (
                                        '|| V_ESQUEMA ||'.S_'||V_TEXT_TABLA||'.NEXTVAL,
                                        (SELECT DD_CRA_ID FROM '||V_ESQUEMA||'.DD_CRA_CARTERA WHERE DD_CRA_CODIGO = '''||V_TMP_TIPO_DATA(1)||'''),
                                        (SELECT DD_TCO_ID FROM '||V_ESQUEMA||'.DD_TCO_TIPO_COMERCIALIZACION WHERE DD_TCO_CODIGO = '''||V_TMP_TIPO_DATA(3)||'''),
            ...
            ELSE
                ...
                DBMS_OUTPUT.PUT_LINE('[INFO]: YA EXISTE UNA CONFIGURACION:  CARTERA = '''||V_TMP_TIPO_DATA(1)||''',
                                    ...
                                    RECOMENDACION = '''||V_TMP_TIPO_DATA(6)||'''');
