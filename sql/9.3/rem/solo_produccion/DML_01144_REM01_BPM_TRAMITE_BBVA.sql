...
V_USUARIO VARCHAR2(50 CHAR) := 'REMVIP-11159_20220302';
...
TYPE T_TIPO_DATA IS TABLE OF VARCHAR2(150);
    ...
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
        ...
        T_TIPO_DATA('T017_PBCVenta'),
        T_TIPO_DATA('T017_RatificacionComiteCES'),
        T_TIPO_DATA('T017_ResolucionCES'),
...
DBMS_OUTPUT.PUT_LINE('[INFO] TAREA '''||V_TMP_TIPO_DATA(1)||'''');
...
INNER JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP ON TEX.TAP_ID = TAP.TAP_ID AND TAP.TAP_CODIGO IN ('''||V_TMP_TIPO_DATA(1)||''') AND TAP.BORRADO = 0
INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON TAC.ACT_ID = ACT.ACT_ID AND ACT.BORRADO = 0
AND TAC.BORRADO = 0 AND ACT.DD_CRA_ID = 162) AUX
ON (AUX.TRA_ID = TRA.TRA_ID)
WHEN MATCHED THEN UPDATE SET
TRA.TRA_PROCESS_BPM = NULL,
TRA.USUARIOMODIFICAR ='''||V_USUARIO||''' ';
...
TEX.USUARIOMODIFICAR ='''||V_USUARIO||''' ';
EXECUTE IMMEDIATE V_MSQL;
...
(SELECT ID_ FROM REMMASTER.JBPM_NODE WHERE NAME_ = ''||V_TMP_TIPO_DATA(1)||'' AND processdefinition_ = AUX.processdefinition_)
    ELSE (SELECT ID_ FROM REMMASTER.JBPM_NODE WHERE NAME_ = ''||V_TMP_TIPO_DATA(1)||'' AND processdefinition_ = AUX.processdefinition_) END AS NODE_PARENT,
...
AND TAP_CODIGO IN (''||V_TMP_TIPO_DATA(1)||'')
