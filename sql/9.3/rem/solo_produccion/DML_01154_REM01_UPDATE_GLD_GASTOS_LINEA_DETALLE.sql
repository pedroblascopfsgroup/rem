...
BEGIN		
    ...
    V_MSQL:= 'MERGE INTO '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR T1 USING (
        ...
        WHEN MATCHED THEN UPDATE SET
        T1.DD_EGA_ID = (SELECT DD_EGA_ID FROM '||V_ESQUEMA||'.DD_EGA_ESTADOS_GASTO WHERE DD_EGA_CODIGO = ''01''),
        T1.USUARIOMODIFICAR = '''||V_USU||''',
        T1.FECHAMODIFICAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;

    DBMS_OUTPUT.PUT_LINE('[INFO] MODIFICADO ESTADO PDTE AUTORIZAR EN '|| SQL%ROWCOUNT ||' REGISTROS');
