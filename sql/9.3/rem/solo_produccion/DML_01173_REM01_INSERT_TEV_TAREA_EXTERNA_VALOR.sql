...
DECLARE
    ...
    V_USR VARCHAR2(30 CHAR) := 'REMVIP-11607'; -- USUARIOCREAR/USUARIOMODIFICAR.    
    TEX_ID NUMBER(16);
...
BEGIN	
	...
	V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.ACT_TRA_TRAMITE WHERE TRA_ID = 811664';
	...
	IF V_NUM_FILAS > 0 THEN 
		...
		V_SQL := 'SELECT TEX.TEX_ID FROM '||V_ESQUEMA||'.ACT_TRA_TRAMITE TRA
					...
					WHERE TRA.TRA_ID = 811664 AND TAR.TAR_TAREA_FINALIZADA = 0 AND TAP.TAP_CODIGO = ''T017_ResolucionPROManzana''';
		...
		IF V_NUM_FILAS = 0 THEN 
			...
			V_SQL := 'Insert into '||V_ESQUEMA||'.TEV_TAREA_EXTERNA_VALOR 
						(TEV_ID,TEX_ID,TEV_NOMBRE,TEV_VALOR,USUARIOCREAR,FECHACREAR,DTYPE)
						VALUES 
						(S_TEV_TAREA_EXTERNA_VALOR.NEXTVAL,'||TEX_ID||',''comboRespuesta'',''01'','''||V_USR||''',SYSDATE,''TareaExternaValor'')';
			EXECUTE IMMEDIATE V_SQL;
			V_SQL := 'Insert into '||V_ESQUEMA||'.TEV_TAREA_EXTERNA_VALOR 
						(TEV_ID,TEX_ID,TEV_NOMBRE,TEV_VALOR,USUARIOCREAR,FECHACREAR,DTYPE)
						VALUES 
						(S_TEV_TAREA_EXTERNA_VALOR.NEXTVAL,'||TEX_ID||',''fechaRespuesta'',''2022-04-11'','''||V_USR||''',SYSDATE,''TareaExternaValor'')';
			EXECUTE IMMEDIATE V_SQL;
			V_SQL := 'Insert into '||V_ESQUEMA||'.TEV_TAREA_EXTERNA_VALOR 
						(TEV_ID,TEX_ID,TEV_NOMBRE,TEV_VALOR,USUARIOCREAR,FECHACREAR,DTYPE)
						VALUES 
