...
V_USUARIO VARCHAR2(20 CHAR) := 'REMVIP-12533';
...
BEGIN 
    ...
    WITH PERIMETRO AS (SELECT DISTINCT tra.tra_id
						FROM '||V_ESQUEMA||'.ACT_TRA_TRAMITE TRA 
						JOIN '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC ON TAC.TRA_ID = TRA.TRA_ID 
                        
						INNER JOIN '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES TAR ON TAR.TAR_ID = TAC.TAR_ID --AND TAR.TAR_TAREA_FINALIZADA = 0 AND TAR.BORRADO = 0 AND tar.TAR_FECHA_FIN IS NULL 
						
						WHERE tra.USUARIOmodificar = ''HREOS-17749'' AND tra.TRA_FECHA_FIN IS NULL AND tra.DD_EPR_ID = 30 and tra.borrado = 0)
					SELECT DISTINCT tac.tra_id
		...
		''T017_PBCVenta'',
		''T017_RecomendCES'',
		''T017_ResolucionPROManzana'') AND TAP.BORRADO = 0
        JOIN PERIMETRO PER ON PER.TRA_ID = TAC.TRA_ID
						where TAC.borrado = 0 ) AUX 
	...
	(SELECT ID_ FROM REMMASTER.JBPM_NODE WHERE NAME_ = 'forkReservaSi' AND processdefinition_ = AUX.processdefinition_)
		...
		'T017_PBCReserva',
		'T017_PBCVenta',
		'T017_RecomendCES',		'T017_ResolucionPROManzana')
    ...
    WITH PERIMETRO AS (SELECT DISTINCT tra.tra_id
						...
						WHERE tra.USUARIOmodificar = ''REMVIP-12241'' AND tra.TRA_FECHA_FIN IS NULL AND tra.DD_EPR_ID = 30 and tra.borrado = 0)
					SELECT DISTINCT tac.tra_id
						FROM '||V_ESQUEMA||'.TAC_TAREAS_ACTIVOS TAC 
						INNER JOIN '||V_ESQUEMA||'.TAR_TAREAS_NOTIFICACIONES TAR ON TAR.TAR_ID = TAC.TAR_ID --AND TAR.TAR_TAREA_FINALIZADA = 0 AND TAR.BORRADO = 0 AND tar.TAR_FECHA_FIN IS NULL 
						INNER JOIN '||V_ESQUEMA||'.TEX_TAREA_EXTERNA TEX ON TEX.TAR_ID = TAC.TAR_ID AND TEX.BORRADO = 0
