...
DECLARE
	...
	V_TABLA_COPIA VARCHAR2(30 CHAR):= 'AHP_COPIA_REMVIP_4407_2'; --Vble. para la tabla backup.
	V_USUARIO VARCHAR2(40 CHAR) := 'REMVIP-4407-2'; --Vble. para el usuario REMVIP.
...
/***** CONTENIDOS MISMA FECHA FIN VENTA *****/
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
				USING (
					SELECT ID_MAYOR FROM '||V_ESQUEMA||'.AHP_CONT_PUNTO2 WHERE BORRAR_MAYOR = 1
				) T2
				ON (T1.AHP_ID = T2.ID_MAYOR)
				WHEN MATCHED THEN UPDATE SET
					T1.BORRADO = 1
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
				USING (
					SELECT DISTINCT ID_MAYOR, FECHA_FIN FROM '||V_ESQUEMA||'.AHP_CONT_PUNTO2 WHERE BORRAR_MAYOR = 0
				) T2
				ON (T1.AHP_ID = T2.ID_MAYOR)
				WHEN MATCHED THEN UPDATE SET
...
/***** MISMO INI DISTINTO FIN *****/
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TABLA||' T1
				USING (
					...
					FROM '||V_ESQUEMA||'.'||V_TABLA||' H1
					JOIN '||V_ESQUEMA||'.'||V_TABLA||' H2 ON H1.ACT_ID = H2
					.ACT_ID
					JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = H1.ACT_ID AND DD_CRA_ID = 21 AND ACT.BORRADO = 0
					WHERE H1.AHP_ID <> H2.AHP_ID AND H1.BORRADO = 0 AND H2.BORRADO = 0
					AND TO_DATE(H1.AHP_FECHA_INI_VENTA,''DD/MM/YY'') = TO_DATE(H2.AHP_FECHA_INI_VENTA,''DD/MM/YY'')
