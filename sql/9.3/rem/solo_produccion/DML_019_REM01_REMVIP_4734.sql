...
/************************* PASO 2 *************************/
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_AHP_HIST_PUBLICACION T1
				USING (
					SELECT DISTINCT AHP_RESERVA, FECHA_RESERVA, FECHA_VENTA FROM '||V_ESQUEMA||'.'||V_TABLA_AUX||' WHERE AHP_RESERVA IS NOT NULL
				) T2
				ON (T1.AHP_ID = T2.AHP_RESERVA)
				WHEN MATCHED THEN UPDATE SET
					T1.AHP_FECHA_INI_VENTA = T2.FECHA_RESERVA
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.ACT_AHP_HIST_PUBLICACION T1
				USING (
					SELECT DISTINCT AHP_VENTA, FECHA_VENTA FROM '||V_ESQUEMA||'.'||V_TABLA_AUX||'
				) T2
