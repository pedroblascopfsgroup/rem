...
BEGIN	
	...
	V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.GDE_GASTOS_DETALLE_ECONOMICO GDE
				USING (
				   ...
				   GPL_DIARIO1,
				   GPL_DIARIO2,
				   CASE WHEN GPL_DIARIO1 = ''1'' OR GPL_DIARIO1 = ''20'' THEN NVL(GPL_DIARIO1_BASE,0) ELSE 0 END AS PRINCIPAL_SUJETO,
				   CASE WHEN GPL_DIARIO1 = ''1'' OR GPL_DIARIO1 = ''20'' AND GPL_DIARIO2 = ''60'' THEN NVL(GPL_DIARIO2_BASE, 0)
				       WHEN GPL_DIARIO1 = ''2'' OR GPL_DIARIO1 = ''60'' AND GPL_DIARIO2 <> ''60'' THEN NVL(GPL_DIARIO1_BASE,0)
				       WHEN GPL_DIARIO1 = ''2'' AND GPL_DIARIO2 = ''60'' THEN NVL(GPL_DIARIO1_BASE,0) + NVL(GPL_DIARIO2_BASE, 0)
				   ...
				   FROM '||V_ESQUEMA||'.GPL_GASTOS_PRINEX_LBK WHERE GPV_ID in (SELECT GPV_ID FROM '||V_ESQUEMA||'.GPV_GASTOS_PROVEEDOR WHERE GPV_NUM_GASTO_HAYA in (10667177, 10895986, 10895991)) AND GPL_DIARIO1 IS NOT NULL AND BORRADO = 0) WHERE RN = 1) AUX ON (AUX.GPV_ID = GDE.GPV_ID)
