...
--## 		0.2 Añadir que la oferta debe tener número oferta Caixa - [HREOS-18833] - Alejandra García
--#########################################
--*/
...
V_SENTENCIA := 'TRUNCATE TABLE '||V_ESQUEMA||'.APR_AUX_HREOS_18797';
   ...
   FROM (SELECT DISTINCT OFR.OFR_NUM_OFERTA, ATR.TBJ_ID, ECO.ECO_ID, TAP.TAP_CODIGO, TAC.USU_ID, TAC.SUP_ID, TAR.TAR_TAREA, CASE 
		    ...
		    INNER JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP ON TAP.TAP_ID = TEX.TAP_ID AND TAP.TAP_CODIGO IN(''T015_SancionPatrimonio'', ''T015_AgendarFechaFirma'', ''T015_CierreContrato'',''T015_EnvioContrato'') 
       		...
       		V_SENTENCIA := 'MERGE INTO '||V_ESQUEMA||'.APR_AUX_HREOS_18797 MIG2 USING(
				SELECT NUM_OFERTA, ACT_ID FROM (SELECT MIG.NUM_OFERTA, ROW_NUMBER() OVER ( PARTITION BY MIG.NUM_OFERTA ORDER BY ACT.ACT_ID) row_num, ACT.ACT_ID 
                		...
                		INNER JOIN '||V_ESQUEMA||'.ACT_OFR ACO ON ACO.OFR_ID = OFR.OFR_ID
                   		INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = ACO.ACT_ID
							AND ACT.BORRADO = 0
                    	INNER JOIN '||V_ESQUEMA||'.APR_AUX_HREOS_18797 MIG ON MIG.NUM_OFERTA = OFR.OFR_NUM_OFERTA
        	...
        	EXECUTE IMMEDIATE V_SENTENCIA;
        	       ...
        	       V_SENTENCIA := 'MERGE INTO '||V_ESQUEMA||'.APR_AUX_HREOS_18797 MIG2 USING(
										...
										FROM  '||V_ESQUEMA||'.APR_AUX_HREOS_18797 MIG 
