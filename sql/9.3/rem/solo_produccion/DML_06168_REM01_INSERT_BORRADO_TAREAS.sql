...
BEGIN
	...
	INNER JOIN '||V_ESQUEMA||'.APR_AUX_HREOS_18797 AUX ON AUX.NUM_OFERTA = OFR.OFR_NUM_OFERTA
	) AUX ON (AUX.TAR_ID_ANTERIOR = TAR.TAR_ID)
	WHEN MATCHED THEN UPDATE SET
	TAR.TAR_FECHA_FIN = SYSDATE,
	TAR.TAR_TAREA_FINALIZADA = 1,
	TAR.USUARIOMODIFICAR = ''HREOS-18797'',
	...
	INNER JOIN '||V_ESQUEMA||'.TEX_TAREA_EXTERNA TXT ON TXT.TAR_ID = TAR.TAR_ID
	INNER JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP ON TXT.TAP_ID = TAP.TAP_ID
	INNER JOIN '||V_ESQUEMA||'.APR_AUX_HREOS_18797 AUX ON AUX.NUM_OFERTA = OFR.OFR_NUM_OFERTA AND AUX.TAREA_ANTIGUA_TRAMITE = ''T015_CierreContrato''
	...
	TRA.USUARIOMODIFICAR = ''HREOS-18797'',
	...
	INNER JOIN '||V_ESQUEMA||'.TEX_TAREA_EXTERNA TXT ON TXT.TAR_ID = TAR.TAR_ID
	INNER JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP ON TXT.TAP_ID = TAP.TAP_ID
	INNER JOIN '||V_ESQUEMA||'.APR_AUX_HREOS_18797 AUX ON AUX.NUM_OFERTA = OFR.OFR_NUM_OFERTA AND AUX.TAREA_NUEVA_TRAMITE IS NOT NULL
	) AUX ON (AUX.TRA_ID = TRA.TRA_ID)
	WHEN MATCHED THEN UPDATE SET
    ...
    V_MSQL := '
    MERGE INTO REM01.ECO_EXPEDIENTE_COMERCIAL ECO USING(
	SELECT DISTINCT ECO.ECO_ID, CASE WHEN AUX.TAREA_ANTIGUA_TRAMITE = ''T015_AgendarFechaFirma'' THEN ''019''
    WHEN AUX.TAREA_ANTIGUA_TRAMITE = ''T015_CierreContrato'' THEN ''020''
    WHEN AUX.TAREA_ANTIGUA_TRAMITE = ''T015_SancionPatrimonio'' THEN ''053''
    WHEN AUX.TAREA_ANTIGUA_TRAMITE = ''T015_EnvioContrato'' THEN ''063''
    END AS DD_EEB_ID FROM REM01.OFR_OFERTAS OFR
	JOIN '||V_ESQUEMA||'.ECO_EXPEDIENTE_COMERCIAL ECO ON ECO.OFR_ID = OFR.OFR_ID
	JOIN '||V_ESQUEMA||'.APR_AUX_HREOS_18797 AUX ON AUX.NUM_OFERTA = OFR.OFR_NUM_OFERTA)
	AUX ON (AUX.ECO_ID = ECO.ECO_ID)
	WHEN MATCHED THEN UPDATE SET
	ECO.DD_EEB_ID = (SELECT DD_EEB_ID FROM '||V_ESQUEMA||'.DD_EEB_ESTADO_EXPEDIENTE_BC WHERE DD_EEB_CODIGO = AUX.DD_EEB_ID),
	ECO.USUARIOMODIFICAR = ''HREOS-18797'',
