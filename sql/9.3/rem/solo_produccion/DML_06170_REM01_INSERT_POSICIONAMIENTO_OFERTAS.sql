...
--## 		0.3 Añadir que la oferta debe tener número oferta Caixa - [HREOS-18833] - Alejandra García
--## 		0.4 Corrección DD_TPO_CODIGO = T018 - [HREOS-18941] - Alejandra García
--#########################################
--*/
...
V_SENTENCIA := 'TRUNCATE TABLE '||V_ESQUEMA||'.APR_AUX_HREOS_18797_2';

	EXECUTE IMMEDIATE V_SENTENCIA;

	DBMS_OUTPUT.PUT_LINE('[INFO] COMIENZA EL PROCESO DE MIGRACION SOBRE LA TABLA '||V_ESQUEMA||'.APR_AUX_HREOS_18797_2');
        	
        	V_SENTENCIA := 'INSERT INTO '||V_ESQUEMA||'.APR_AUX_HREOS_18797_2(NUM_OFERTA, TBJ_ID, ECO_ID, TPO_ID, TAREA_ANTIGUA_TRAMITE, TAREA_NUEVA_TRAMITE, USU_ID, SUP_ID, TRA_ID, TAR_ID_ANTERIOR, TEX_ID_ANTERIOR)(SELECT OFR_NUM_OFERTA,TBJ_ID, ECO_ID, (SELECT DD_TPO_CODIGO FROM '||V_ESQUEMA||'.DD_TPO_TIPO_PROCEDIMIENTO WHERE DD_TPO_CODIGO = ''T018''),TAP_CODIGO , NUEVA_TAREA, USU_ID, SUP_ID, TRA_ID, TAR_ID, TEX_ID
   FROM (SELECT DISTINCT OFR.OFR_NUM_OFERTA, ATR.TBJ_ID, ECO.ECO_ID, TAP.TAP_CODIGO, TAC.USU_ID, TAC.SUP_ID, TAR.TAR_TAREA, CASE 
			...
			END AS NUEVA_TAREA, ATR.TRA_ID, TAR.TAR_ID, TEX.TEX_ID 
		    ...
		    INNER JOIN '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO TAP ON TAP.TAP_ID = TEX.TAP_ID AND TAP.TAP_CODIGO IN(''T018_TrasladarOfertaCliente'', ''T018_RevisionBcYCondiciones'', ''T018_AgendarYFirmar'', ''T018_CierreContrato'') 
			WHERE CRA.DD_CRA_CODIGO = ''03'' AND EOF.DD_EOF_CODIGO NOT IN (''02'', ''06'')
			AND OFR.BORRADO = 0))';
                   
        	EXECUTE IMMEDIATE V_SENTENCIA;
        	
       		V_SENTENCIA := 'MERGE INTO '||V_ESQUEMA||'.APR_AUX_HREOS_18797_2 MIG2 USING(
								SELECT NUM_OFERTA, ACT_ID FROM (SELECT MIG.NUM_OFERTA, ROW_NUMBER() OVER ( PARTITION BY MIG.NUM_OFERTA ORDER BY ACT.ACT_ID) row_num, ACT.ACT_ID 
								FROM '||V_ESQUEMA||'.OFR_OFERTAS OFR
								INNER JOIN '||V_ESQUEMA||'.ACT_OFR ACO ON ACO.OFR_ID = OFR.OFR_ID
								INNER JOIN '||V_ESQUEMA||'.ACT_ACTIVO ACT ON ACT.ACT_ID = ACO.ACT_ID
									AND ACT.BORRADO = 0
								INNER JOIN '||V_ESQUEMA||'.APR_AUX_HREOS_18797_2 MIG ON MIG.NUM_OFERTA = OFR.OFR_NUM_OFERTA
								WHERE OFR.BORRADO = 0) WHERE ROW_NUM = 1
							)
        	...
        	EXECUTE IMMEDIATE V_SENTENCIA;
        	       ...
        	       V_SENTENCIA := 'MERGE INTO '||V_ESQUEMA||'.APR_AUX_HREOS_18797_2 MIG2 USING(
										SELECT MIG.NUM_OFERTA, TAP.TAP_ID 
										FROM '||V_ESQUEMA||'.APR_AUX_HREOS_18797_2 MIG 
	...
	DBMS_OUTPUT.PUT_LINE('[INFO] - '||to_char(sysdate,'HH24:MI:SS')||'  '||V_ESQUEMA||'..APR_AUX_HREOS_18797_2 cargada. '||SQL%ROWCOUNT||' Filas.');
