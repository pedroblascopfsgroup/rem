...
DECLARE
    ...
    V_USUARIO VARCHAR2(32 CHAR):= 'REMVIP-5355';
...
BEGIN
	...
	V_SQL := 'UPDATE '||V_ESQUEMA||'.ACT_PAC_PERIMETRO_ACTIVO 
				SET PAC_CHECK_FORMALIZAR = 1
				    ...
				    ,USUARIOMODIFICAR = '''||V_USUARIO||'''
		        WHERE ACT_ID IN (
					SELECT ACT_ID FROM '||V_ESQUEMA||'.ACT_ACTIVO WHERE ACT_NUM_ACTIVO IN (7015114)
				)';
	EXECUTE IMMEDIATE V_SQL;
	...
	V_SQL := 'UPDATE '||V_ESQUEMA||'.ACT_ACTIVO SET DD_SCM_ID = NULL WHERE ACT_NUM_ACTIVO IN (7015114)';
	EXECUTE IMMEDIATE V_SQL;
