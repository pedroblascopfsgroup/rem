...
DECLARE
    ...
    V_USUARIOMODIFICAR VARCHAR(100 CHAR):= 'REMVIP-6211';

    V_TABLA_AUX VARCHAR(100 CHAR):= 'AUX_REMVIP_6211'; --  <<<---- Tabla auxiliar
    V_GESTOR_MODIF VARCHAR(100 CHAR):= 'GPUBL';--  	   <<<---- Gestor a modificar
    V_USERNAME_NUEVO VARCHAR(100 CHAR):= 'usugrupub';--    <<<---- Nuevo usuario a informar en el tipo de gestor anterior
...
--Paso 2: Crea la tabla temporal
	...
	V_SQL := ' CREATE TABLE '||V_ESQUEMA||'.' || V_TABLA_AUX || ' AS  
			...
			FROM 
			GAC_GESTOR_ADD_ACTIVO GAC,
			GEH_GESTOR_ENTIDAD_HIST GEH,
