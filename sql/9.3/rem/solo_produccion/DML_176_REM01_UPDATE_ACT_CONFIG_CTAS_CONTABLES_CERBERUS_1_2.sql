...
BEGIN	
    ...
    V_MSQL := 'INSERT /*+APPEND*/ INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' (
      ...
      FROM (
          SELECT TMP.CCC_CUENTA_CONTABLE
              ...
              , RANK() OVER(
                  PARTITION BY TMP.DD_TGA_ID, TMP.DD_STG_ID, TMP.DD_TIM_ID, TMP.DD_CRA_ID, tmp.DD_SCR_ID, EJE.EJE_ID, PRO.PRO_ID, TMP.CCC_PRINCIPAL
                  ORDER BY 
                      CASE 
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] Por ultimo, borramos fisicamente las CC borradas de manera logica en el DML anterior y que sustituimos por estas nuevas');
