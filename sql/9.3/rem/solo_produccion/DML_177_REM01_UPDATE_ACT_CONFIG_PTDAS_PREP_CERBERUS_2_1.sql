...
DECLARE
    ...
    -- PARTIDA   DD_TGA_CODIGO  DD_STG_CODIGO, TIPO_IMPORTE
    V_TIPO_DATA T_ARRAY_DATA := T_ARRAY_DATA(
		T_TIPO_DATA('PP049', '15', '82', 'BAS')
...
BEGIN	
    ...
    FOR I IN V_TIPO_DATA.FIRST .. V_TIPO_DATA.LAST
    	LOOP
	    	...
	    	DBMS_OUTPUT.PUT_LINE('[INFO] Insertamos los registros para todos los ejercicios.'); 
	       	...
	       	V_MSQL := 'INSERT INTO '||V_ESQUEMA||'.TMP_'||V_TEXT_TABLA||' (CPP_PTDAS_ID, CPP_PARTIDA_PRESUPUESTARIA, DD_TGA_ID, DD_STG_ID, DD_TIM_ID, DD_CRA_ID, DD_SCR_ID, FECHACREAR, BORRADO) 
				WITH MAX AS (
	        		SELECT NVL(MAX(CPP_PTDAS_ID), 0) MAXIMUM
	        		FROM '||V_ESQUEMA||'.TMP_'||V_TEXT_TABLA||'
	        	)
				SELECT MAXIMUM + ROWNUM CPP_PTDAS_ID,
					'''||V_TMP_TIPO_DATA(1)||''' CPP_CUENTA_CONTABLE,
					(SELECT DD_TGA_ID 
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] Partidas BBVA preparadas para tabla de negocio. '||V_DURACION);
    ...
    DBMS_OUTPUT.PUT_LINE('[INFO] Abrimos un nuevo proceso para borrar las PP que puediera haber para esta CARTERA + SUBCARTERA + TIPO GASTO + SUBTIPO GASTO + EJERCICIO para despues insertar las buenas...');
    ...
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('[INFO] Borradas '||SQL%ROWCOUNT||' PP para el Ejercicio 2020.');
    ...
    V_MSQL := 'MERGE INTO '||V_ESQUEMA||'.'||V_TEXT_TABLA||' T1
      ...
      WHEN MATCHED THEN
          ...
          T1.FECHABORRAR = SYSDATE';
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('[INFO] Borradas '||SQL%ROWCOUNT||' PP para el Ejercicio 2021.');
