...
RED='\033[0;31m'
NC='\033[0m' # No Color
TITULO="Busqueda de ficheros sql comiteados por ${RED}$AUTOR (el último mes) ${NC}para los que no se ha modificado el campo FECHA_CREACION"
SEPARADOR="####################################################################################################################"
...
ficheros=`git log --name-status --diff-filter=M  --since="one-month-ago" --committer="$AUTOR" --no-merges | grep "^M.*sql.*sql$" | cut -f2`
ok=0
