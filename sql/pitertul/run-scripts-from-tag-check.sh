...
if [[ "$#" -ge 4 ]] && [[ "$INPUT_PARAM_GO" == "go!" ]]; then
    ...
    do
        if [[ "$INPUT_PARAM_VERBOSE" == "-v" ]]; then
            ...
            echo "--------------------------------------------------------------------------------"
            echo "$BASEDIR/run-single-script-check.sh $line $CUSTOMER_IN_UPPERCASE $OPTION"
            $BASEDIR/run-single-script-check.sh $line $CUSTOMER_IN_UPPERCASE $OPTION
    ...
    do
        ...
        $BASEDIR/run-single-script-check.sh $NEW_LINE $CUSTOMER_IN_UPPERCASE $OPTION
        SALIDA_RUN_SINGLE_SCRIPT=$?
        if [[ $SALIDA_RUN_SINGLE_SCRIPT != 0 ]]; then
            echo "ERROR"
            exit 1
