...
if [[ $VERBOSE == 1 ]]; then
    echo "$ORACLE_HOME/bin/sqlplus -s -l $ESQUEMA_REGISTRO/******* @$BASEDIR/${FECHA_CREACION}-${PASO1}  >> $BASEDIR/$nombreLog"
fi
if [[ $PACKAGE == 0 ]]; then
...
if [[ "$COMPILE" == "9" ]] ; then
	if [[ "$PROC_VISTA" == "1" ]] ; then
	    ...
	    if [[ $VERBOSE == 1 ]]; then
	        echo $'\t'"$ORACLE_HOME/bin/sqlplus -s -l $ESQUEMA_REGISTRO/******* @$BASEDIR/${PASO3} "$NOMBRE_SCRIPT" "$ESQUEMA_EJECUCION" "$AUTOR" "$ARTEFACTO" "$VERSION_ARTEFACTO" "$FECHA_CREACION" "$INCIDENCIA_LINK" "$PRODUCTO""
	    fi
	    if [[ $PACKAGE == 0 ]]; then
	    ...
	    if [[ $VERBOSE == 1 ]]; then
	        echo "$ORACLE_HOME/bin/sqlplus -s -l $ESQUEMA_EJECUCION/******* @$BASEDIR/${nombreSinExt}-$ESQUEMA_EJECUCION-reg3.1.sql >> $BASEDIR/$nombreLog"
	    fi
	    if [[ $PACKAGE == 0 ]]; then
        ...
        if [[ $PACKAGE == 0 ]]; then
            ...
            if [ $RESULTADO != 0 ] ; then
                ...
                if [[ $VERBOSE == 1 ]]; then
                    echo "$ORACLE_HOME/bin/sqlplus -s -l $ESQUEMA_REGISTRO/******* @$BASEDIR/${PASO4} "$NOMBRE_SCRIPT" "$FECHA_CREACION" "$ESQUEMA_EJECUCION" "KO" "$RESULTADO"  >> $BASEDIR/$nombreLog"
                fi
            ...
            if [ $RESULTADO == 0 ] ; then
                ...
                echo "#####    Ejecución la actualización de resultados en la tabla de registro"  >> $BASEDIR/$nombreLog
                if [[ $VERBOSE == 1 ]]; then
                    echo "$ORACLE_HOME/bin/sqlplus -s -l $ESQUEMA_REGISTRO/******* @$BASEDIR/${PASO4} "$NOMBRE_SCRIPT" "$FECHA_CREACION" "$ESQUEMA_EJECUCION" "OK" "$RESULTADO"  >> $BASEDIR/$nombreLog"
                fi
...
PASO2=${regFile}${PASO2}
if [[ $VERBOSE == 1 ]]; then
    echo "$ORACLE_HOME/bin/sqlplus -s -l $ESQUEMA_REGISTRO/******* @$BASEDIR/${PASO2} $NOMBRE_SCRIPT $FECHA_CREACION $ESQUEMA_EJECUCION > /dev/null"
fi
if [[ $PACKAGE == 0 ]]; then
...
if [[ $PACKAGE == 0 ]]; then
    if [[ $RESULTADO == 33 ]] ; then 
		...
		# Si es un script normal y ya ha sido ejecutado
	        if [[ $VERBOSE == 1 ]]; then
	...
	elif [[ $RESULTADO == 34 ]] ; then 
	# Es un script de procs y vistas y se ha ejecutado hace menos de dos horas: no se vuelve a ejecutar
		if [[ "$PROC_VISTA" != "0" ]] ; then
	        if [[ $VERBOSE == 1 ]]; then
	            echo -e "\n\n======>>> Script $BASEDIR/${NOMBRE_SCRIPT} ejecutado recientemente, lo volvemos a ejecutar" > /dev/tty
	        fi
	        echo -e "\n\n======>>> Script $BASEDIR/${NOMBRE_SCRIPT} ejecutado recientemente, lo volvemos a ejecutar" >> $BASEDIR/$nombreLog
	        echo "  --  :  $BASEDIR/$nombreLog"
		fi
    ...
    echo $'\t''    fi' | tee -a ${executionFile}.sh ${executionFile}-one-user.sh > /dev/null
    echo $'\t''elif [[ $RESULTADO == 34 ]] ; then' | tee -a ${executionFile}.sh ${executionFile}-one-user.sh > /dev/null
    echo $'\t'"    echo \" ER : Fichero ejecutado recientemente $NOMBRE_SCRIPT (no se vuelve a ejecutar)\"" | tee -a ${executionFile}.sh ${executionFile}-one-user.sh > /dev/null
