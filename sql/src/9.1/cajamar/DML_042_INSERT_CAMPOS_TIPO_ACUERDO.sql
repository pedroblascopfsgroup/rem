...
--/**
-- * Nuevo valor en el diccionario DD_EIN_ENTIDAD_INFORMACION Para el tipo de acuerdo Refinanciación
-- */
V_SQL := 'SELECT COUNT(*) FROM '||V_ESQUEMA||'.'||V_DDNAME||' WHERE CMP_NOMBRE_CAMPO= ''numExpediente'' AND DD_TPA_ID =(SELECT DD_TPA_ID FROM DD_TPA_TIPO_ACUERDO WHERE DD_TPA_CODIGO = ''04'') ';

EXECUTE IMMEDIATE V_SQL INTO V_NUM_TABLAS;

IF V_NUM_TABLAS > 0 THEN    
  DBMS_OUTPUT.PUT_LINE('OK no se modifica nada.');
  DBMS_OUTPUT.PUT_LINE('[INFO] Ya existe el registro "Refinanciación - numExpediente" '||V_DDNAME||'.');
...
ELSE
  ...
  '('||V_ESQUEMA||'.S_ACU_CAMPOS_TIPO_ACUERDO.nextval,(SELECT DD_TPA_ID FROM DD_TPA_TIPO_ACUERDO WHERE DD_TPA_CODIGO = ''04''),''numExpediente'',''0'',''PRODUCTO'',sysdate,''0'') ';
