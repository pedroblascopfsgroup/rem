...
BEGIN
	...
	V_SQL := 'SELECT USD.USD_ID FROM '||V_ESQUEMA||'.USD_USUARIOS_DESPACHOS USD' || 
			'INNER JOIN '||V_ESQUEMA||'.DES_DESPACHO_EXTERNO DES ON DES.DES_ID = USD.DES_ID AND DES.DES_CODIGO = ''DSUPASU'' ' ||
			'INNER JOIN '||V_ESQUEMA_M||'.USU_USUARIOS USU ON USU.USU_ID = USD.USD_ID AND USU_USERNAME = ''SUPER_UCO'' ';
	EXECUTE IMMEDIATE V_SQL INTO ID_USD;
	
	V_SQL := 'SELECT DD_TGE_ID FROM '||V_ESQUEMA_M||'.DD_TGE_TIPO_GESTOR WHERE DD_TGE_CODIGO = ''SUP'' ';
