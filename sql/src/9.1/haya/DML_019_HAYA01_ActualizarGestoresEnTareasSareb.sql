...
--##########################################
--## AUTOR=Nacho A
--## FECHA_CREACION=20150613
--## ARTEFACTO=online
--## VERSION_ARTEFACTO=9.1.1-hy-rc01
--## INCIDENCIA_LINK=HR-955
--## PRODUCTO=NO
--##
--## Finalidad: Actualizar gestores de las tareas de decisión de elevación a Sareb
...
BEGIN
	...
	V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO' ||
			  ' SET BORRADO = 1 ' ||
	          ' WHERE TAP_CODIGO = ''H036_BPMTramiteElevacionPropSarebLitigios''';
    DBMS_OUTPUT.PUT_LINE('[INFO] Actualizando borrado tarea.......');
    DBMS_OUTPUT.PUT_LINE(V_MSQL);
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('[INFO] Borrado tarea actualizado.');
    
    V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO' ||
			  ' SET DD_STA_ID = (SELECT DD_STA_ID FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO = ''824'')' ||
	          ' WHERE TAP_CODIGO = ''H017_ElevarDecision''';
    ...
    V_MSQL := 'UPDATE '||V_ESQUEMA||'.TAP_TAREA_PROCEDIMIENTO' ||
			  ' SET DD_STA_ID = (SELECT DD_STA_ID FROM '||V_ESQUEMA_M||'.DD_STA_SUBTIPO_TAREA_BASE WHERE DD_STA_CODIGO = ''824'')' ||
	          ' WHERE TAP_CODIGO = ''H009_ElevacionDecision''';
    DBMS_OUTPUT.PUT_LINE('[INFO] Actualizando gestor tarea.......');
    DBMS_OUTPUT.PUT_LINE(V_MSQL);
    EXECUTE IMMEDIATE V_MSQL;
    DBMS_OUTPUT.PUT_LINE('[INFO] Gestor tarea actualizado.');
