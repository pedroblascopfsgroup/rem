...
function print_banner_description() {
    echo ""
    echo "      EJECUTO LOS SCRIPTS DE BD DESDE UN TAG HASTA OTRO"
    echo ""
    echo "******************************************************************************************"
}
...
if [ "$#" -lt 4 ]; then
    ...
    echo ""
    if [ "$ORACLE_SID" == "" ] ; then
        echo "   Uso: $0 <from-tag> <to-tag> CLIENTE password_esquemas@sid"
    else
        echo "   Uso: $0 <from-tag> <to-tag> CLIENTE password_esquemas"
    fi
    echo ""
    echo "Para ejecutarlo:"
    echo ""
    if [ "$ORACLE_SID" == "" ] ; then
        echo "   Uso: $0 <from-tag> <to-tag> CLIENTE password_esquemas@sid go!"
        echo "   Uso: $0 <from-tag> <to-tag> CLIENTE password_esquemas@sid go! -v"
    else
        echo "   Uso: $0 <from-tag> <to-tag> CLIENTE password_esquemas go!"
        echo "   Uso: $0 <from-tag> <to-tag> CLIENTE password_esquemas go! -v"
    fi
    echo ""
    echo "       -v: verbose"
    echo ""
...
CUSTOMER_IN_UPPERCASE=`echo $3 | tr '[:lower:]' '[:upper:]'`
CUSTOMER_IN_LOWERCASE=`echo $3 | tr '[:upper:]' '[:lower:]'`
...
DIRECTORIO=""
if [[ "$#" -ge 5 ]] && [[ "$5" == "package!" ]] && [[ "$4" != "null" ]]; then
    DIRECTORIO="$4/"
fi

#PRODUCTO
for file in `git diff $1 $2 --name-only sql/**/producto/$DIRECTORIO*.sql`
do
    if [ "$MULTIENTIDAD" != "" ] ; then
        IFS=',' read -a entidades <<< "$MULTIENTIDAD"
        for entidad in "${entidades[@]}"
        do
            connectionParam=`getConnectionParam $file ${!entidad} $4`
            registerSQLScript $file $BASEDIR/tmp/product-list-from-tag.txt $connectionParam
        done
    else
        registerSQLScript $file $BASEDIR/tmp/product-list-from-tag.txt $4
    fi
done

#CLIENTE
for file in `git diff $1 $2 --name-only sql/**/$CUSTOMER_IN_LOWERCASE/$DIRECTORIO*.sql`
do
    if [ "$MULTIENTIDAD" != "" ] ; then
        IFS=',' read -a entidades <<< "$MULTIENTIDAD"
        for entidad in "${entidades[@]}"
        ...
        registerSQLScript $file $BASEDIR/tmp/customer-list-from-tag.txt $4
    fi
done
...
if [ "$MULTIENTIDAD" != "" ] ; then
    ...
    do
        ...
        for file in `git diff $1 $2 --name-only sql/**/$CUSTOMER_IN_LOWERCASE/$SUBENTITY/*.sql`
        do
            connectionParam=`getConnectionParam $file ${!entidad} $4`
            registerSQLScript $file $BASEDIR/tmp/customer-list-from-tag.txt $connectionParam
        done
