...
public class ProcesarAnalisisExternaScheduler implements ApplicationContextAware, Initializable {
    ...
    private static final String CRON_EXPRESSION_KEY = "cronExpressionAnalisisExterna";
    ...
    public void initialize() {
        for (Entidad entidad : entidadManager.getListaEntidades()) {
            ...
            JobDetail jobDetail = new JobDetail("procesaranalisisexterna_entidad" + entidad.getId(), DEFAULT_GROUP, ProcesarAnalisisExternaJob.class);
            jobDetail.getJobDataMap().put("applicationContext", applicationContext);
            jobDetail.getJobDataMap().put("entidad", entidad);
            logger.info("-- CREO EL TIMER DE ANALISIS DE EXTERNA --");
            Trigger trigger;
            try {
                trigger = new CronTrigger("triggerAnalisisExterna_entidad" + entidad.getId(), DEFAULT_GROUP, cronExpression);
                try {
