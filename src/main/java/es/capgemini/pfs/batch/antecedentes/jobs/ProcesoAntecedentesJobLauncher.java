package es.capgemini.pfs.batch.antecedentes.jobs;
...
/**
 * Clase que implementa el Job Launcher para el proceso de cálculo de Antecedentes
 * @author Guillem
 *
 */
public class ProcesoAntecedentesJobLauncher implements BatchPCRConstantes, JobRunner{
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoAntecedentes.PROCESO_ANTECEDENTES_JOBNAME, workingCode, this);
    ...
    public void run() {      
    	try{
	    	logger.info(ProcesoAntecedentes.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoAntecedentes.PROCESO_ANTECEDENTES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoAntecedentes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoAntecedentes.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }
        } catch (Exception e) {
        	logger.error(ProcesoAntecedentes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
