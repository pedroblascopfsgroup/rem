 ...
 * 
 */
public class BatchValidatorExtendedTasklet implements ValidationTasklet {
	...
	public ExitStatus execute() {
		...
		List<Map> result;
		if (argumento == null) {
			if(!insercion.toUpperCase().equals("TRUE")){
			...
			if(!insercion.toUpperCase().equals("TRUE")){
				result = jdbcTemplate.queryForList(query, new Object[] { argumento });
			}else{
				jdbcTemplate.update(query, new Object[] { argumento });
				result = new ArrayList<Map>();
			}
		}
		...
		int maximaCantidadDeErrores = new Integer(appProperties.get(
				"batch.validation.maxQuantityError").toString());
		for (Map record : result) {
			...
			Object[] params = new Object[] { codigoValor, codigoEntidad,
					argumento };
			EventBatchUtil.getInstance().throwEventErrorChannel(getMessage(),
					getSeveridad(), false, params);
			cantidadDeErrores++;
		}
		if (severidad != null && "error".equalsIgnoreCase(severidad)) {
			ExitStatus exit = new ExitStatus(false,
					ExitStatus.FAILED.getExitCode(), getMessage());
			return exit;
		}
		return ExitStatus.FINISHED;
