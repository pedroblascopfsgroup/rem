...
public class ProcesarPrepoliticasScheduler implements ApplicationContextAware, Initializable {
    ...
    private static final String CRON_EXPRESSION_KEY = "cronExpressionProcPrepoliticas";
    ...
    public void initialize() {
        for (Entidad entidad : entidadManager.getListaEntidades()) {
            ...
            JobDetail jobDetail = new JobDetail("procesarprepoliticas_entidad" + entidad.getId(), DEFAULT_GROUP, ProcesarPrepoliticasJob.class);
            jobDetail.getJobDataMap().put("applicationContext", applicationContext);
            jobDetail.getJobDataMap().put("entidad", entidad);
            logger.info("-- CREO EL TIMER DE PREPOLITICAS --");
            Trigger trigger;
            try {
                trigger = new CronTrigger("triggerPrepoliticas_entidad" + entidad.getId(), DEFAULT_GROUP, cronExpression);
                try {
