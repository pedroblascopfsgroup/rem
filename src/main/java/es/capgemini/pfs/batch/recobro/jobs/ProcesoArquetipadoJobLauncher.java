...
/**
 * Clase que implementa el Job Launcher para el proceso de Arquetipado correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoArquetipadoJobLauncher implements BatchPCRConstantes, JobRunner{
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoArquetipado.PROCESO_ARQUETIPADO_JOBNAME, workingCode, this);
    ...
    public void run() {     
    	try{
	    	logger.info(ProcesoArquetipado.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoArquetipado.PROCESO_ARQUETIPADO_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoArquetipado.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoArquetipado.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        } 
        } catch (Exception e) {
        	logger.error(ProcesoArquetipado.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
