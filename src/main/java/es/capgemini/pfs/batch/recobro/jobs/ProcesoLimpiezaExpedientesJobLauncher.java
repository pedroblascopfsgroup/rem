...
/**
 * Clase que implementa el Job Launcher para el proceso de Limpieza de Expedientes correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoLimpiezaExpedientesJobLauncher implements BatchPCRConstantes, JobRunner{
	...
	/**
	 * Inicia el proceso de Limpieza de Expedientes
	 * @param workingCode
	 * @param extractTime
	 */
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoLimpiezaExpedientes.PROCESO_LIMPIEZA_EXPEDIENTES_JOBNAME, workingCode, this);
    ...
    public void run() {        
    	try{
	    	logger.info(ProcesoLimpiezaExpedientes.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoLimpiezaExpedientes.PROCESO_LIMPIEZA_EXPEDIENTES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoLimpiezaExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoLimpiezaExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }        
        } catch (Exception e) {
        	logger.error(ProcesoLimpiezaExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
