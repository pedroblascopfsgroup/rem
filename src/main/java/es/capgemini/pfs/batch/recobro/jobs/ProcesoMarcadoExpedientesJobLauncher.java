package es.capgemini.pfs.batch.recobro.jobs;
...
/**
 * Clase que implementa el Job Launcher para el proceso de Marcado de Expedientes correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoMarcadoExpedientesJobLauncher implements BatchPCRConstantes, JobRunner{
	...
	/**
	 * Inicia el proceso de Marcado de Expedientes
	 * @param workingCode
	 * @param extractTime
	 */
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoMarcadoExpedientes.PROCESO_MARCADO_EXPEDIENTES_JOBNAME, workingCode, this);
    ...
    public void run() {      
    	try{
	    	logger.info(ProcesoMarcadoExpedientes.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoMarcadoExpedientes.PROCESO_MARCADO_EXPEDIENTES_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoMarcadoExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoMarcadoExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }
        } catch (Exception e) {
        	logger.error(ProcesoMarcadoExpedientes.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
