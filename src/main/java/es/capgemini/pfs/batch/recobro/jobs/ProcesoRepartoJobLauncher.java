...
/**
 * Clase que implementa el Job Launcher para el proceso de Reparto correspondiente a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoRepartoJobLauncher implements BatchPCRConstantes, JobRunner{
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoReparto.PROCESO_REPARTO_JOBNAME, workingCode, this);
    ...
    public void run() {       
    	try{
	    	logger.info(ProcesoReparto.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoReparto.PROCESO_REPARTO_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoReparto.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoReparto.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        }        
	    } catch (Exception e) {
	    	logger.error(ProcesoReparto.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
