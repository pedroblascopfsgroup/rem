...
/**
 * Clase que implementa el Batch Validation Handler para implementar las validaciones del Batch a las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoValidacionRecobroJobLauncher implements BatchPCRConstantes, JobRunner {
    ...
    /**
     * Inicia el proceso de validaci�n general.
	...
	/**
	 * Inicia el proceso de Validaci�n del Batch de Recobro
	 * @param workingCode
	 * @param extractTime
	 */
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(ValidacionRecobro.PROCESO_VALIDACION_RECOBRO_JOBNAME, workingCode, this);
    ...
    public void run() {
        try {
			logger.info(ValidacionRecobro.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			Entidad entidad = entidadDao.findByWorkingCode(workingCode);
			DbIdContextHolder.setDbId(entidad.getId());
			HashMap<String, Object> parameters = new HashMap<String, Object>();
			parameters.put(Genericas.EXTRACT_TIME, extractTime);
			parameters.put(Genericas.ENTIDAD, workingCode);
			BatchExitStatus result = batchManager.run(ValidacionRecobro.PROCESO_VALIDACION_RECOBRO_JOBNAME, parameters);
            if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
            	logger.info(ValidacionRecobro.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
                Event event = new GenericEvent();
                event.setProperty(Genericas.SOURCE_CHANNEL_KEY, QueueUtils.getQueueNameForEntity(getChannel(), workingCode));
                event.setProperty(Genericas.ENTIDAD, workingCode);
                event.setProperty(Genericas.EXTRACT_TIME, (Date) parameters.get(Genericas.EXTRACT_TIME));
            	...
            	logger.error(ValidacionRecobro.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
