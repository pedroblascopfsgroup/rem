package es.capgemini.pfs.batch.recobro.ranking.jobs;
...
/**
 * Implementación del proceso de cálculo de rankings de las subcarteras
 * @author Guillem
 *
 */
public class ProcesoRankingJobLauncher implements BatchPCRConstantes, JobRunner {
	...
	public void handle(String workingCode, Date extractTime) {
		...
		JobInfo jobInfo = new JobInfo(ProcesoRanking.PROCESO_RANKING_JOBNAME, workingCode, this);
	...
	public void run() {
		try{
			logger.info(ProcesoRanking.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);
			...
			BatchExitStatus result = batchManager.run(ProcesoRanking.PROCESO_RANKING_JOBNAME, parameters);
			if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoRanking.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
			...
			if (BatchExitStatus.FAILED.equals(result)) {
				logger.error(ProcesoRanking.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
			}
	    } catch (Exception e) {
	    	logger.error(ProcesoRanking.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
