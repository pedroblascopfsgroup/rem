...
/**
 * Clase que implementa el Job Launcher para el proceso de Arquetipado correspondiente a la simulación de las Agencias de Recobro
 * @author Guillem
 *
 */
public class ProcesoArquetipadoSimulacionJobLauncher implements BatchPCRConstantes, JobRunner{
    ...
    public void handle(String workingCode, Date extractTime) {
        ...
        JobInfo jobInfo = new JobInfo(ProcesoArquetipadoSimulacion.PROCESO_ARQUETIPADO_SIMULACION_JOBNAME, workingCode, this);
    ...
    public void run() {     
    	try{
	    	logger.info(ProcesoArquetipadoSimulacion.INICIO_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER);    	
	        ...
	        BatchExitStatus result = batchManager.run(ProcesoArquetipadoSimulacion.PROCESO_ARQUETIPADO_SIMULACION_JOBNAME, parameters);        
	        if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
				logger.info(ProcesoArquetipadoSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
	        ...
	        if (BatchExitStatus.FAILED.equals(result)) {
	            logger.error(ProcesoArquetipadoSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ  + workingCode + 
	            		Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	        } 
        } catch (Exception e) {
        	logger.error(ProcesoArquetipadoSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ + workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
