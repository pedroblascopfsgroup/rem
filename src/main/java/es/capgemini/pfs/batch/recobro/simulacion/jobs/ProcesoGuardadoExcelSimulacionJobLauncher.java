...
/**
 * Clase que implementa el Job Launcher para el proceso de generaci�n y guardado
 * en disco de la Excel de Simulaci�n de Recobro
 * 
 * @author Bruno Angl�s
 * 
 *         T
 */
// TODO Implementer este prodceso si realmente nos hace falta
public class ProcesoGuardadoExcelSimulacionJobLauncher implements
		BatchPCRConstantes, JobRunner {
	...
	/**
	 * Inicia el proceso encadenado desde otro job.
	 * 
	 * @param message
	 *            Message
	 */
	public void handle(Message<GenericEvent> message) {
		handle(message.getPayload().getPropertyAsString(ENTIDAD), null,
				new Date());
	}

	/**
	 * Inicia el proceso de generaci�n y guardado de la excel
	 * 
	 * @param workingCode
	 * @param filePath
	 *            Ruta en la que queremos guardar el fichero Excel generado.
	 * @param extractTime
	 */
	public void handle(String workingCode, String filePath, Date extractTime) {
		// this.workingCode = workingCode;
		// this.extractTime = extractTime;
		// this.filePath = filePath;
		// JobInfo jobInfo = new
		// JobInfo(ControlSimulacion.PROCESO_CONTROL_RECOBRO_JOBNAME,
		// workingCode, this);
	...
	public void run() {
		// try{
		// logger.info(ControlSimulacion.INICIO_MSG + Genericas.CORCHETE_IZQ +
		// workingCode + Genericas.CORCHETE_DER);
		...
		// BatchExitStatus result =
		// batchManager.run(ControlSimulacion.PROCESO_CONTROL_RECOBRO_JOBNAME,
		// parameters);
		// if (BatchExitStatus.COMPLETED.equals(result) || BatchExitStatus.NOOP.equals(result)) {
		// logger.info(ControlSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ +
		// workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
		// Event evento = new GenericEvent();
		// evento.setProperty(Genericas.SOURCE_CHANNEL_KEY,
		// QueueUtils.getQueueNameForEntity(getChannel(), workingCode));
		// evento.setProperty(Genericas.ENTIDAD, workingCode);
		// evento.setProperty(Genericas.EXTRACT_TIME, (Date)
		// parameters.get(Genericas.EXTRACT_TIME));
		// eventManager.fireEvent(EventManager.GENERIC_CHANNEL,
		// "Lanzamos un evento de routing [" + workingCode + "]");
		// getEventManager().fireEvent(GenericRouter.ROUTING_CHANNEL, evento);
		// }
		// if (BatchExitStatus.NOOP.equals(result)) {
		// logger.info(ControlSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ +
		// workingCode + Genericas.CORCHETE_DER + Genericas.FINALIZADO_MSG);
		// }
		// if (BatchExitStatus.FAILED.equals(result)) {
		// logger.error(ControlSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ +
		// workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
		// }
		// } catch (Exception e) {
		// logger.error(ControlSimulacion.FIN_MSG + Genericas.CORCHETE_IZQ +
		// workingCode + Genericas.CORCHETE_DER + Genericas.FALLO_MSG);
	...
	private void sendEndChainEvent(String workingCode) {
		getEventManager().fireEvent(
				QueueUtils.getQueueNameForEntity(Genericas.PCR_CHAIN_CHANNEL,
						workingCode), PcrFileHandler.CHAIN_END);
	}
	...
	public Date shouldProcessOn(Date date) {
		return TimeUtils
				.shouldProcessOn(date, getWindow(), getCronExpression());
	}
