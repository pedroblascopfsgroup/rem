...
public class ProcesarAlertasScheduler implements ApplicationContextAware, Initializable {
    ...
    private static final String CRON_EXPRESSION_KEY = "cronExpressionProcAlertas";
    ...
    public void initialize() {
        for (Entidad entidad : entidadManager.getListaEntidades()) {
            ...
            JobDetail jobDetail = new JobDetail("procesarAlertas_entidad" + entidad.getId(), DEFAULT_GROUP, ProcesarAlertasJob.class);
            jobDetail.getJobDataMap().put("applicationContext", applicationContext);
            jobDetail.getJobDataMap().put("entidad", entidad);
            logger.info("-- CREO EL TIMER DE SCORING ["+ entidad.getConfiguracion().get(Entidad.WORKING_CODE_KEY) +"] --");
            Trigger trigger;
            try {
                trigger = new CronTrigger("triggerAlertas_entidad" + entidad.getId(), DEFAULT_GROUP, cronExpression);
                try {
