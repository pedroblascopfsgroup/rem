...
@Component
public class EventoPersonaNoClienteMostrarAutoTareas implements EventoClienteBuilder {
	...
	public List<Evento> getEventos(long idCliente) {
		MEJTrazaDto dto = creaDTOObtenerTrazasCliente(idCliente);
	...
	private List<Evento> convierteRegistroAEvento(List<? extends MEJRegistroInfo> lista){
			...
			for (MEJRegistroInfo evento : lista) {
				...
				if (!Checks.esNulo(idTareaStr)) {
					try {
						...
						if (tarea != null) {
							listaEventos.add(new MEJEvento(tarea, Evento.TIPO_EVENTO_PERSONA,evento.getId()));
						}
	...
	private MEJTrazaDto creaDTOObtenerTrazasCliente(final long idCliente) {
