package es.pfsgroup.plugin.recovery.masivo.callbacks.bpmBO.api.impl;
...
@Component
public class MSVProcedimientoBackOfficeBOCallback extends
		AbstractRecoveryBPMfwCallbackBOTemplate implements
		MSVProcedimientoBackOfficeCallbackApi {
	...
	@Autowired
	private TipoProcedimientoDao tpoProcDao;
	 ...
	 */
	@Override
	@BusinessOperation(MSV_PROCEDIMIENTO_BACK_OFFICE_CALLBACK_ONEND)
	 ...
	 */
	@Override
	@BusinessOperation(MSV_PROCEDIMIENTO_BACK_OFFICE_CALLBACK_ONERROR)
	 ...
	 */
	@Override
	@BusinessOperation(MSV_PROCEDIMIENTO_BACK_OFFICE_CALLBACK_ONSTART)
	 ...
	 */
	@Override
	@BusinessOperation(MSV_PROCEDIMIENTO_BACK_OFFICE_CALLBACK_ONSUCCESS)
	public void onSuccess(Long tokenProceso, RecoveryBPMfwkInputDto input) {
		//Si el input es enviar a impresión debe generar los archivos
		if (MSVDDOperacionMasiva.CODIGO_INPUT_IMPRESO.equals(input.getCodigoTipoInput())) {
			
			Long idTipoProcedimientoMonitorio = tpoProcDao.getByCodigo(MSVProcesoManager.TIPO_PROCEDIMIENTO_FIN_MONITORIO).getId();
			Long idTipoProcedimientoETJ = tpoProcDao.getByCodigo(MSVProcesoManager.TIPO_PROCEDIMIENTO_ETJ).getId();
			
			String numNova = String.valueOf(input.getDatos().get(MSVEnviarImprimirColumns.NUM_NOVA));
			String fecha = String.valueOf(input.getDatos().get(MSVEnviarImprimirColumns.FECHA_SOLICITUD));
			...
			if (!Checks.esNulo(fecha)) {
				try {
					...
					e.printStackTrace();
					onError(tokenProceso, input, "La fecha no es correcta: " + e.getMessage());
					error=true;
				}
			} else {
				onError(tokenProceso, input, "El caso nova no es correcto");
				error=true;
			}
			if (!error) {
				String tipoDocumentacion = String.valueOf(input.getDatos().get(MSVEnviarImprimirColumns.TIPO_DOCUMENTACION));
				...
				Procedimiento procedimiento=apiProxy.proxy(MSVProcedimientoApi.class).buscaProcedimientoDelContrato(numNova, idTipoProcedimiento);
			
				if (!Checks.esNulo(procedimiento)) {
					try {
						apiProxy.proxy(MSVEnvioImpresionApi.class).envioImpresion(procedimiento, fechaImpresion, tipoDocumentacion);
					} catch (RecoveryBPMfwkError e) {
						// No hacemos nada: simplemente no se envía a imprimir
						e.printStackTrace();
						onError(tokenProceso, input, "Ha habido un error al enviar a impresión: " + e.getMessage());
					}
				} else {
					onError(tokenProceso, input, "No se encuentra el procedimiento");
				}
			}
		}
	}
