...
@Component
public class MSVSelectorEnvioJuzgado implements MSVSelectorTipoInput{

	@Override
	public String getTipoInput(Map<String, Object> map) {
		return MSVDDOperacionMasiva.CODIGO_INPUT_ENVIADO_JUZGADO;	
	}

	@Override
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODIGO_ENVIO_JUZGADO.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
	...
	public String getTipoResolucion() {
		return MSVDDOperacionMasiva.CODIGO_ENVIO_JUZGADO;
	}
