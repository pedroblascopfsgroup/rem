...
@Component
public class MSVSelectorImpresionDocumentacion implements MSVSelectorTipoInput{

	@Override
	public String getTipoInput(Map<String, Object> map) {
		return MSVDDOperacionMasiva.CODIGO_INPUT_IMPRESO;	
	}

	@Override
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODIGO_IMPRESION_DOCUMENTACION.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
