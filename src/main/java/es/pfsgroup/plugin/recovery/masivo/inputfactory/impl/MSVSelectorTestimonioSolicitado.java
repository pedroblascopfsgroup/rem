...
@Component
public class MSVSelectorTestimonioSolicitado implements MSVSelectorTipoInput{

	@Override
	public String getTipoInput(Map<String, Object> map) {
		return MSVDDOperacionMasiva.CODIGO_INPUT_TESTIMONIO_SOLICITADO;	
	}

	@Override
	public Boolean isValidFor(MSVDDOperacionMasiva tipoOperacion) {
		if (!Checks.esNulo(tipoOperacion)){
			if (MSVDDOperacionMasiva.CODIGO_SOLICITAR_TESTIMONIO.equals(tipoOperacion.getCodigo())){
				return true;
			}else {
				return false;
			}
