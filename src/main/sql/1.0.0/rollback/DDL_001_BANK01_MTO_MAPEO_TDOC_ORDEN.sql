...
BEGIN	
    ...
    DBMS_OUTPUT.PUT_LINE('******** MTO_MAPEO_TDOC_ORDEN (ROLLBACK) ********'); 
    DBMS_OUTPUT.PUT_LINE('[INFO] ROLLBACK '||V_ESQUEMA||'.MTO_MAPEO_TDOC_ORDEN... Comprobaciones previas'); 
...
--	END IF; 
    DBMS_OUTPUT.PUT_LINE('[INFO] ROLLBACK ' || V_ESQUEMA || '.MTO_MAPEO_TDOC_ORDEN... OK');
EXCEPTION
  WHEN OTHERS THEN
    ERR_NUM := SQLCODE;
    ERR_MSG := SQLERRM;
