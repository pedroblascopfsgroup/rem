...
BEGIN	
	 ...
	 V_MSQL := 'Insert into TFI_TAREAS_FORM_ITEMS (TFI_ID,TAP_ID,TFI_ORDEN,TFI_TIPO,TFI_NOMBRE,TFI_LABEL,TFI_ERROR_VALIDACION,'
    			...
    			|| ' (SELECT TAP_ID FROM HAYA01.TAP_TAREA_PROCEDIMIENTO WHERE TAP_CODIGO = ''H003_ValidarInformeDeSubastaYPrepararCuadroDePujas''),'
	...
	set TFI_ORDEN=6
	where tap_id=(select tap_id from tap_tarea_procedimiento where tap_codigo='H003_ValidarInformeDeSubastaYPrepararCuadroDePujas')
		and tfi_nombre='observaciones';		
	
	update tap_tarea_procedimiento
	set tap_script_validacion_jbpm='(valores[''H003_ValidarInformeDeSubastaYPrepararCuadroDePujas''][''comboNotaSimple''] == DDSiNo.SI ? (comprobarExisteDocumentoNOSI() ? null : ''Es necesario adjuntar el documento Nota Simple'' ) : null)'	
	where tap_codigo='H003_ValidarInformeDeSubastaYPrepararCuadroDePujas';	
	
    --HR-488
    V_MSQL := 'UPDATE TAP_TAREA_PROCEDIMIENTO '
				|| ' SET TAP_SCRIPT_VALIDACION_JBPM = ''(valores[''''H003_CelebracionSubasta''''][''''comboCelebrada''''] == DDSiNo.NO ? ((valores[''''H003_CelebracionSubasta''''][''''comboDecisionSuspension''''] == null || valores[''''H003_CelebracionSubasta''''][''''comboMotivoSuspension''''] == null) ? ''''El campo Decisi&oacute;n suspensi&oacute;n Entidad/terceros es obligatorio'''' : null) : (valores[''''H003_CelebracionSubasta''''][''''comboCesionRemate''''] == null ? ''''El campo Cesi&oacute;n de remate es obligatorio'''' : (valores[''''H003_CelebracionSubasta''''][''''comboAdjudicadoEntidad''''] == null ? ''''El campo Adjudicado a Entidad con posibilidad de Cesi&oacute;n de remate es obligatorio'''' : null ) ) )'' '
