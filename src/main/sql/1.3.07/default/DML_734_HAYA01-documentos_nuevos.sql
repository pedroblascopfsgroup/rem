...
--##########################################
--## Author: Roberto
--## Finalidad: http://link.pfsgroup.es/jira/browse/HR-558 - Documentos nuevos
...
DECLARE
    ...
    V_TIPO_TFA T_ARRAY_TFA := T_ARRAY_TFA(
      T_TIPO_TFA('JUZIVA', 'Escrito al Juzgado para comunicar la operacion sujeta IVA (Tributación de Bienes Sareb)', 'Escrito al Juzgado para comunicar la operacion sujeta IVA (Tributación de Bienes Sareb)', 'AP')      
    ); 
...
BEGIN	
  ...
  DBMS_OUTPUT.PUT_LINE('[INFO] '||V_ESQUEMA||'.dd_tfa_fichero_adjunto... Empezando a insertar datos en el diccionario');
    ...
    update dd_tfa_fichero_adjunto set dd_tac_id=8 where dd_tfa_codigo='INFFIS';
    
    
    delete from dd_tfa_fichero_adjunto where dd_tfa_codigo='OT';
