...
--## Finalidad: DDL modificación tipo de datos de BIE_CODIGO_INTERNO sin perder los datos de la tabla BIE_BIEN
...
BEGIN
        ...
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.BIE_BIEN ADD BIE_CODIGO_INTERNO_AUX VARCHAR(50 CHAR)';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] AÑADIDO BIE_CODIGO_INTERNO_AUX');      
        
        V_MSQL := 'UPDATE '||V_ESQUEMA||'.BIE_BIEN SET BIE_CODIGO_INTERNO_AUX = TO_CHAR(BIE_CODIGO_INTERNO)';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] ACTUALIZADA BIE_CODIGO_INTERNO_AUX');  
        
        COMMIT; 
        
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.BIE_BIEN DROP COLUMN BIE_CODIGO_INTERNO';
        EXECUTE IMMEDIATE V_MSQL;
        DBMS_OUTPUT.PUT_LINE('[INFO] BORRADO BIE_CODIGO_INTERNO ANTIGUO');
        
        V_MSQL := 'ALTER TABLE '||V_ESQUEMA||'.BIE_BIEN RENAME COLUMN BIE_CODIGO_INTERNO_AUX TO BIE_CODIGO_INTERNO';
        EXECUTE IMMEDIATE V_MSQL;
