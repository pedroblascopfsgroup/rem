...
BEGIN	
    ...
    VAR_TABLENAME := 'PRO_PROVISIONES_ASUNTO';
    VAR_SEQUENCENAME := 'S_PRO_PROVISIONES_ASUNTO';
    ...
    V_MSQL := 'CREATE TABLE ' || V_ESQUEMA || '.' || VAR_TABLENAME || '
					(
					  ...
					  PRO_FECHA_ALTA  TIMESTAMP(6)                          NOT NULL,
					  PRO_FECHA_BAJA  TIMESTAMP(6),
    ...
    V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.PRO_PROVISIONES_ASUNTO ADD (CONSTRAINT PRO_PROVISIONES_ASUNTO_PK PRIMARY KEY (PRO_ID))';
		EXECUTE IMMEDIATE V_MSQL;
	DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.PRO_PROVISIONES_ASUNTO_PK... PK creado');
    
	
	
    V_MSQL := 'ALTER TABLE ' || V_ESQUEMA || '.' || VAR_TABLENAME || ' ADD (
  				CONSTRAINT PRO_ASU_FK 
				FOREIGN KEY (ASU_ID) 
 				REFERENCES ' || V_ESQUEMA || '.ASU_ASUNTOS (ASU_ID))';			
		EXECUTE IMMEDIATE V_MSQL;
		DBMS_OUTPUT.PUT_LINE('[INFO] ' || V_ESQUEMA || '.PRO_ASU_FK... Creando FK');	
