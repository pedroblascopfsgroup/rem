      ...
      LOOP
        ...
        EXECUTE IMMEDIATE V_MSQL INTO V_ENTIDAD_ID;
			...
			ELSE		
				...
				V_MSQL := 'INSERT INTO '|| V_ESQUEMA ||'.FUN_PEF (FP_ID, PEF_ID, FUN_ID, VERSION, USUARIOCREAR, FECHACREAR, BORRADO) ' ||
				' SELECT '|| V_ESQUEMA ||'.S_FUN_PEF.NEXTVAL, PEF_ID, FUN_ID,0,''DML-001'',SYSDATE,0 ' ||
				' FROM '|| V_ESQUEMA ||'.PEF_PERFILES, '|| V_ESQUEMA_M ||'.FUN_FUNCIONES WHERE (FUN_DESCRIPCION = '''||TRIM(V_TMP_FUNCION(2))||''')';
				DBMS_OUTPUT.PUT_LINE('INSERTANDO PERMISOS: '''||TRIM(V_TMP_FUNCION(2))||'''');
				EXECUTE IMMEDIATE V_MSQL;
			END IF;
      END LOOP;
    COMMIT;
