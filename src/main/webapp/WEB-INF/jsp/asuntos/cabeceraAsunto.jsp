...
var createCabeceraAsuntoTab=function(){

	var procurador='${asunto.procurador}';

	var labelStyle='font-weight:bolder;width:150px';
	var asunto=app.creaLabel('<s:message code="asunto.tabcabecera.asunto" text="**Asunto"/>','${asunto.nombre}',{labelStyle:labelStyle});
	...
	var expediente = app.creaLabel('<s:message code="expedientes.consulta.titulo" text="**Expediente"/>','${asunto.expediente.id}',{labelStyle:labelStyle});
   ...
   procedimientosStore.webflow({id:'${asunto.id}'});
	...
	procedimientosGrid.on('rowdblclick', function(grid, rowIndex, e) {
    	...
    	//var nombre_procedimiento='${asunto.nombre}'+'-'+rec.get('tipoProcedimiento');
    	var nombre_procedimiento=rec.get('descripcionProcedimiento');
    	var id=rec.get('id');
    	if (id != null && id != ''){
