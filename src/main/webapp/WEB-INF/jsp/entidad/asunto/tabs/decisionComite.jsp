...
(function(page,entidad){
	...
	var asuConformacion = '<fwk:const value="es.capgemini.pfs.asunto.model.DDEstadoAsunto.ESTADO_ASUNTO_EN_CONFORMACION" />';
	...
	var sesion = label('sesion', '<s:message code="decisioncomite.consulta.sesion" text="**Sesion" />');
	var comiteFecha = label('comiteFecha', '<s:message code="decisioncomite.consulta.fecha" text="**Fecha" />');
	var comiteNombre = label('comiteNombre', '<s:message code="decisioncomite.consulta.comite" text="**comite" />');
	var comitePlazoRestante = label('comitePlazoRestante', '<s:message code="decisioncomite.consulta.plazo" text="**Plazo Restante" />');
	...
	var fieldSetCabecera=new Ext.form.FieldSet({                                                                                                                    
		...
		,width:760		                                                                                                                                    
		,items:[
			{items:[sesion,  comiteNombre]}
			,{items:[comiteFecha, comitePlazoRestante]}                                                                                                                                                 
		]                                                                                                                                                                
	}); 
	    ...
	    for(var i=0; i < records; i++) {
	        data = clientesGrid.getStore().getAt(i).data;
		 	if(data.incluido==true)
		       	str+=data.id;
			if(i!=records-1)	// Si no es el elemento
				str+=',';
	    }
	    return str;
	};
	...
	var contratosCm = new Ext.grid.ColumnModel([                                                                                                                         
		...
		,{header : '<s:message code="decisioncomite.consulta.gridcontratos.nActuaciones" text="**Nº Actuaciones" />', width:100, dataIndex : 'nProcedimientos',align:'right'}
	...
	var btnActuacion = new Ext.Button({
		text: '<s:message code="decisionComite.boton.sinActuacion" text="**Sin Actuaci&oacute;"/>'                                                                                              
	   ...
	   ,evaluateAndSend: function(seguir) {
     		if(seguir== 'yes') {
				...
				contratosSinActString = armarString(contratosSinActuacion);
				contratosStore.webflow({id:getExpedienteId()});
				asuntosStore.webflow({id:getAsuntoId(), idSesion:entidad.getData("comite.sesion")});
				contratosActuacionesStore.webflow({idExpediente:getExpedienteId()});
			}
		}
	});
   
	contratosGrid = new Ext.grid.GridPanel({
		store:contratosStore
		,cm:contratosCm 
		,sm: new Ext.grid.RowSelectionModel({singleSelect:true})
        ,title : '<s:message code="asunto.tabDecisionComite.gridcontratos.titulo" text="**Contratos del asunto" />'
		,cls:'cursor_pointer'
	...
	function refrescaCheckBox(){
		...
		// Si ya existla solicitud de esclusidejamos tildados
	...
	var contratosActuacionesCm = new Ext.grid.ColumnModel([                                                                                                                         
		...
		,{header : '<s:message code="asuntos.listado.procedimiento" text="**Actuaci&oacute;n"/>', width:100, dataIndex : 'procedimiento', sortable: false}
		,{header : '<s:message code="asuntos.listado.tipoActuacion" text="**Tipo actuaci&oacute;n"/>', width: 75, dataIndex : 'tipoActuacion', sortable: false}
		,{header : '<s:message code="asuntos.listado.actuacion" text="**Actuaci&oacute;n"/>', width:150, dataIndex : 'actuacion', sortable: false}
		,{header : '<s:message code="decisioncomite.consulta.gridcontratos.fechaCreacion" text="**Fecha creaci&oacute;n"/>', width: 100, dataIndex : 'fechaCreacionProcedimiento', sortable: false}
	]);  
	...
	var asuntosStore = page.getStore({
		eventName : 'listado'
		,flow:'asuntos/listadoAsuntosAsuntoData' 
	...
	var asuntosCm = new Ext.grid.ColumnModel([
		{header: '<s:message code="asuntos.listado.asunto" text="**Asunto"/>', width:150, dataIndex: 'nombre', sortable:false}
		, {header: '<s:message code="asuntos.listado.estado" text="**Estado"/>',	width:100, dataIndex: 'estado',sortable:false}
		, {header: '<s:message code="asuntos.listado.procedimiento" text="**Procedimiento"/>',width:100, dataIndex: 'idProcedimiento',sortable:false}
		, {header: '<s:message code="asuntos.listado.tipoActuacion" text="**Tipo Actuaci&oacute;n"/>', width:100, dataIndex: 'tipoActuacion',sortable:false}
		, {header: '<s:message code="asuntos.listado.actuacion" text="**Actuaci&oacute;n"/>', width:175, dataIndex: 'actuacion',sortable:false}
		, {header: '<s:message code="asuntos.listado.fcreacion" text="**Fecha Creaci&oacute;n"/>', hidden:true, dataIndex: 'fcreacion', renderer:app.format.dateRenderer,sortable:false}
		, {header: '<s:message code="asuntos.listado.gestor" text="**Gestor"/>', hidden:true, dataIndex: 'gestor',sortable:false}
		, {dataIndex: 'despacho',sortable:false, hidden:true}
		, {header: '<s:message code="asuntos.listado.supervisor" text="**Supervisor"/>', hidden:true, dataIndex: 'supervisor',sortable:false}
		, {header: '<s:message code="asuntos.listado.principal" text="**Principal"/>', width:100, dataIndex: 'principal',sortable:false, renderer: app.format.moneyRendererNull,align:'right'}
	...
	var btnNuevo = new Ext.Button({
		...
		,handler : function(){
			...
			win = app.openWindow(
				{
					flow:'asuntos/altaPreasuntos', 
					title : '<s:message code="decisionComite.asuntos" text="**Alta Asuntos" />',
					params: {idAsuntoOrigen:getAsuntoId(), idExpediente:getExpedienteId()},
					width: 870
				}
			);
		...
		handler: function(){
			if (idAsuntoSeleccionado){
				var win = app.openWindow({
					...
					,closable:true
					,params:{idExpediente:getExpedienteId(),idAsunto:idAsuntoSeleccionado}
					,width: 870
				});
				win.on(app.event.CANCEL,function(){win.close();});
				win.on(app.event.DONE,
			...
			if (asuntosGrid.store.data.length && asuntosGrid.store.data.length>0){
				if (idAsuntoSeleccionado){
					var w = app.openWindow({
						flow : 'asuntos/editaProcedimientoAsunto'
						,closable:false
						,width : 840
						,title : '<s:message code="procedimientos.edicion.titulo" text="**Nuevo registro" />' 
						,params : {idExpediente:getExpedienteId(), idAsuntoOrigen:getAsuntoId(), idAsunto:idAsuntoSeleccionado, idProcedimiento:0}
					});
					w.on(app.event.DONE, function(){
						w.close();
						asuntosStore.webflow({id:getAsuntoId()});
						cantidadAsuntos = asuntosStore.getCount();
						contratosStore.webflow({id:getExpedienteId()});   
						contratosActuacionesStore.webflow({idExpediente:getExpedienteId()});
						contratosStore.on('load',cargarArrayInicial);
			...
			if(seguir== 'yes') {            
				page.webflow({
					...
					,success: function(){
						...
						contratosActuacionesStore.webflow({idExpediente:getExpedienteId()});
						contratosStore.on('load',cargarArrayInicial);											
						contratosStore.webflow({id:getAsuntoId()});
					}
				});
	...
	var btnEditProcedimiento = new Ext.Button({
		...
		,handler:function(){
			if (asuntosGrid.store.data.length && asuntosGrid.store.data.length>0){
				if (idProcSeleccionado){
					var w = app.openWindow({
						flow : 'expedientes/editaProcedimientoAsunto'
						,closable:false
						,width : 840
						,title : '<s:message code="app.editar" text="**Nuevo registro" />' 
						,params : {idExpediente:getExpedienteId(),idProcedimiento:idProcSeleccionado,idAsunto:idAsuntoSeleccionado}
					});
					w.on(app.event.DONE, function(){
						w.close();
						contratosStore.webflow({id:getAsuntoId()});
	...
	var asuntosGrid = app.crearGrid(asuntosStore,asuntosCm,{
		...
		bbar: [btnNuevo,btnEditar,btnBorrar
				<sec:authorize ifAllGranted="EDITAR_PROCEDIMIENTO">
					,btnAltaProcedimiento,btnEditProcedimiento,btnBorraProcedimiento
				</sec:authorize>
		]
   }); 
	...
	asuntosGrid.on('rowclick', function(grid, rowIndex, e){
		...
		asuEstado = rec.get('asuEstado');
		prcEstado = rec.get('prcEstado');
		if(idAsuntoSeleccionado!='') {
			//Si está estado propuesto no se permite su borrado
			if (asuEstado == asuConformacion) btnBorrar.enable();
			//Si está estado en conformaciói que se permite su borrado
			else btnBorrar.disable();
			btnEditar.enable();
			btnAltaProcedimiento.enable();
			btnEditProcedimiento.disable();
			btnBorraProcedimiento.disable();
		} else {
			//Si están estado propuesto no se permite su borrado
			if (prcEstado == prcConformacion) btnBorraProcedimiento.enable();
			//Si están estado en conformaciói que se permite su borrado
			else btnBorraProcedimiento.disable();
		...
		var btnEditarObs= new Ext.Button({                                                                                                                                 
			...
			,handler:function(){                                                                                                                                   
				var w = app.openWindow({                                                                                                                                   
					...
					,params : {id:getTaId(), content:(observaciones.getValue()!=''?observaciones.getValue():'<s:message code="dc.obs.editar" text="***Editar Observaciones" />')}                                                                                                                                        
				});                                                                                                                                                        
				w.on(app.event.DONE, function(){                                                                                                                           
					w.close();                                                                                                                                               
				});                                                                                                                                                        
	...
	var cerrarDecision = function(){
		page.webflow({
			flow: 'asunto/cerrarSesionAsunto',
			params: {idAsunto:getAsuntoId(), observaciones:observaciones.getValue()}
			,success: function(){
				//app.contenido.remove(app.contenido.getActiveTab());
				//entidad.cierraTab();
				//app.abreAsunto(getAsuntoId(),'<s:message text="${asunto.nombre}" javaScriptEscape="true" />');
				entidad.refrescar()
			}
		});
	}
	...
	var marcarSinActuacion = function(){
		page.webflow({
			...
			,params:{idContratos:contratosSinActString,idExpediente:getExpedienteId()}
			,success: function(){
				contratosSinActuacion=new Array();
				contratosSinActString = armarString(contratosSinActuacion);
				contratosStore.webflow({id: getExpedienteId()});
	...
	actuacionesTabPanel = new Ext.TabPanel({
		items:[
			asuntosGrid
	...
	function getAsuntoId (){
		return entidad.get("data").id;
	}  
	
	function getExpedienteId (){
		return entidad.get("data").cabecera.expedienteId;
	}
    
	function getTaId () {
		return "observacionAsu" + entidad.get("data").id;
	}
	
	var panel = new Ext.Panel({                                                                                                                                          
		title:'<s:message code="decisionComite.titulo" text="**Decisi&oacute;n comit&eacute;"/>'                                                                           
		,style:'padding: 5px;'                                                                                                                                             
		,nombreTab : 'decisionComiteAsunto'
	...
	panel.estaPropuesto = function(){ return entidad.get("data").toolbar.estaPropuesto; }
	...
	panel.setValue = function(){
		...
		comiteNombre.setValue(data.comite.nombre);
		sesion.setValue(data.comite.sesion);
		comiteFecha.setValue(data.comite.fecha);
		if (data.cabecera.cantidadDiasParaVencer != '') {
			comitePlazoRestante.setValue(data.cabecera.cantidadDiasParaVencer);
		} else {
			comitePlazoRestante.setValue('Vencido');
		}
		observaciones.setValue(data.cabecera.decisionComite);
  
		entidad.cacheOrLoad(data, contratosActuacionesStore, {idExpediente: data.cabecera.expedienteId});
		//contratosActuacionesStore.webflow({idExpediente: data.cabecera.expedienteId});
		
		entidad.cacheOrLoad(data, asuntosStore, {id: entidad.getData("id"), idSesion:entidad.getData("comite.sesion")});
		//asuntosStore.webflow({id: data.id});
		
		entidad.cacheOrLoad(data, contratosStore, {id: data.cabecera.expedienteId});
		//contratosStore.webflow({id: data.cabecera.expedienteId});
		
		var estaPropuesto = panel.estaPropuesto();
		var esVisible = [
				[btnEditarObs, estaPropuesto]
			, 	[btnCerrarDecision, estaPropuesto]
			, 	[btnNuevo, estaPropuesto]
			, 	[btnEditar, estaPropuesto]
			, 	[btnBorrar, estaPropuesto]
			, 	[btnAltaProcedimiento, estaPropuesto]
			, 	[btnEditProcedimiento, estaPropuesto]
			, 	[btnBorraProcedimiento, estaPropuesto]
		];
	...
	panel.setVisibleTab = function(data){
		return data.toolbar.estaPropuesto || data.toolbar.puedeVerDecisionComite;
	}	
