...
fwk.constant = {
	...
	,fwkGenericoError : '<s:message code="fwk.constant.fwkGenericoError" text="**Excepcion no controlada. Contacte con soporte." />'
};
 ...
 * PBO: Si aparece la cadena "Exception" o la cadena "java." consideramos que es un mensaje "no entendible por el usuario"
 *      y ponemos un mensaje generico
 */
...
//	fwkExceptions= "<ul><li>"+fwkExceptions.join("</li><li>")+"</li></ul>";
  ...
  if (fwkExceptions.toString().indexOf("Exception")==-1 && fwkExceptions.toString().indexOf("java.")==-1) {
    fwk.showErrors(fwk.constant.fwkError,fwkExceptions);
  } else {  
    fwk.showErrors(fwk.constant.fwkError,fwk.constant.fwkGenericoError);
  }
};
...
fwk.Page.prototype.executeCallbacks = function(config, data){
    ...
    if (config && config.userConfig && data){
...
fwk.Page.prototype.getStore = function(config){
	...
	store.webflow = function(params, callback){
		...
		//reemplazarán a los params que usa la paginación
