...
/**
 * Test Case para el subproceso de Facturación del proceso de Recobro
 * @author Javier
 *
 */
@ContextConfiguration(locations= Genericas.FICHERO_CONTEXTO_SPRING)
@RunWith(SpringJUnit4ClassRunner.class)
public class ProcesoFacturacionJobLauncherTest extends GenericTest {
	/**
	...
	public void test() {
		try {
			...
			testLaunchDevonJob(getJob(ProcesoFacturacion.PROCESO_FACTURACION_JOBNAME), parameters);
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try {
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/facturacion/procesoFacturacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try {
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/facturacion/procesoFacturacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try {
			...
			configPostDataTest.setSqlFile("postTestData/recobro/facturacion/procesoFacturacion.xml");
		} catch(Throwable e){
			logger.error(e);
			fail();
		}
