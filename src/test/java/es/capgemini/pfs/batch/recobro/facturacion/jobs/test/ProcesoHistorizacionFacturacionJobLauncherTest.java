...
public class ProcesoHistorizacionFacturacionJobLauncherTest extends GenericTest {
	/**
	...
	public void test() {
		try {
			...
			testLaunchDevonJob(getJob(HistorizacionFacturacion.PROCESO_HISTORIZACION_FACTURACION_JOBNAME), parameters);
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try {
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/facturacion/procesoHistorizacionFacturacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try {
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/facturacion/procesoHistorizacionFacturacion.xml");
		} catch(Throwable e) {
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try {
			...
			configPostDataTest.setSqlFile("postTestData/recobro/facturacion/procesoHistorizacionFacturacion.xml");
		} catch(Throwable e){
			logger.error(e);
			fail();
		}
