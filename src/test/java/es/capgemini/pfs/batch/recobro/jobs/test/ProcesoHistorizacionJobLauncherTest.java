...
/**
 * Test Case para el subproceso de Historización del proceso de Recobro
 * @author Guillem
 *
 */
@ContextConfiguration(locations = Genericas.FICHERO_CONTEXTO_SPRING)
@RunWith(SpringJUnit4ClassRunner.class)
public class ProcesoHistorizacionJobLauncherTest extends GenericTest{
	...
	public void test() {	
		try{
	        ...
	        testLaunchDevonJob(getJob(HistorizacionRecobro.PROCESO_HISTORIZACION_RECOBRO_JOBNAME), parameters);
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try{
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/procesoHistorizacion.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try{
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/procesoHistorizacion.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try{
			...
			configPostDataTest.setSqlFile("postTestData/recobro/procesoHistorizacion.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
