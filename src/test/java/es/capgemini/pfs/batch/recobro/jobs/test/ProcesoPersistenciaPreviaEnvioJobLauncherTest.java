...
import es.capgemini.pfs.batch.recobro.api.test.GenericConstantsTest.ProcesoPersistenciaPreviaEnvio;
import es.capgemini.pfs.batch.recobro.api.test.GenericTest;


/**
 * Test Case para el subproceso de Persistencia previa al Env�o del proceso de Recobro
 * @author Guillem
 *
 */
@ContextConfiguration(locations = Genericas.FICHERO_CONTEXTO_SPRING)
@RunWith(SpringJUnit4ClassRunner.class)
public class ProcesoPersistenciaPreviaEnvioJobLauncherTest extends GenericTest{
	...
	public void test() {	
		try{
	        ...
	        testLaunchDevonJob(getJob(ProcesoPersistenciaPreviaEnvio.PROCESO_PERSISTENCIA_PREVIA_ENVIO_JOBNAME), parameters);
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigLoadData loadConfigLoadDataTest() {
		...
		try{
			...
			configLoadDataTest.setSqlFile("loadTestData/recobro/procesoPersistenciaPreviaEnvio.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigValidationData loadConfigValidationDataTest() {
		...
		try{
			...
			configValidationDataTest.setSqlFile("validationTestData/recobro/procesoPersistenciaPreviaEnvio.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
	...
	protected ConfigPostData loadConfigPostDataTest() {
		...
		try{
			...
			configPostDataTest.setSqlFile("postTestData/recobro/procesoPersistenciaPreviaEnvio.xml");
		}catch(Throwable e){
			logger.error(e);
			fail();
		}
