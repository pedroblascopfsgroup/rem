...
(function(){
    ...
    var usuarioDestinoTarea = new Ext.form.ComboBox({
        ...
        ,width:300
        ,fieldLabel: '<s:message code="asuntos.busqueda.filtro.usuarioDestinoTarea" text="**Usuario destino tarea"/>'
        ,tpl: usuarioTemplate  
        ,forceSelection:true
    ...
    var usuarioOrigenTarea = new Ext.form.ComboBox({
        ...
        ,store:usuariosComboStore
        ,width:300
        ,fieldLabel: '<s:message code="asuntos.busqueda.filtro.usuarioOrigenTarea" text="**Usuario origen tarea"/>'
        ,tpl: usuarioTemplate  
        ,forceSelection:true
        //,style:'padding:0px;margin:0px;'
        ,allowBlank:true
   ...
   <%--  var optionsTipoTarea = new Ext.data.JsonStore({
        fields: ['id', 'descripcion']
        ,root: 'data'
        ,data : {data: [{id: 'A', descripcion: 'Alerta' }, {id: 'R', descripcion: 'Recordatorio' }]}
     });
     --%>
   ...
   <%--  
     ...
     var comboTipoTarea = new Ext.form.ComboBox({
                 ...
                 ,editable: false
                 ,triggerAction: 'all'
                 ,fieldLabel : '<s:message code="asuntos.busqueda.filtro.tipoTarea" text="**Tipo de tarea"/>'
                 ,width:300
     });
      --%>
	...
	var comboTipoTarea = new Ext.form.ComboBox({
     ...
     var checkEnvioCorreo = new Ext.form.Checkbox({
         fieldLabel: '<s:message code="asuntos.busqueda.filtro.envioCorreo" text="**Solo asuntos con envios de correos"/>'
         ,inputValue: '1'
     });
     
     checkEnvioCorreo.on('check', function(checkbox, checked) {
         if (checked==true) {
             destinatarioMail.enable();
         } else {
             destinatarioMail.disable();
             destinatarioMail.setValue('');
         }
     });
     
     var destinatarioMail = new Ext.form.TextField({
         fieldLabel:'<s:message code="asuntos.busqueda.filtro.destinatarioMail" text="**Con mails enviados a" />'
         ,width:300
         ,disabled:true
         ,listeners:{
			specialkey: function(f,e){  
	...
	var validarEmptyForm = function(){
		...
		if (destinatarioMail.getValue() != '' && destinatarioMail.getValue() ){
            return true;
        }
...
<%-- 
	var getParametros = function() {
		return {
		    usuarioDestinoTarea: usuarioDestinoTarea.getValue()
			,usuarioOrigenTarea:usuarioOrigenTarea.getValue()
			,tipoAnotacion:comboTipoTarea.getValue()
			,soloAsuntosEnvioCorreo:checkEnvioCorreo.getValue()
			,destinatarioEmail:destinatarioMail.getValue()
...
--%>
	...
	var getParametros = function() {
		...
		if(destinatarioMail.getValue()=='undefined' || !destinatarioMail.getValue()){
			detinatarioMail='';
		}
		
		return {
			params:'origen:agendaMultifuncion;'+'usuarioDestinoTarea'+':'+usuarioDestinoTarea.getValue()+';'+
			'usuarioOrigenTarea'+':'+usuarioOrigenTarea.getValue()+';'+
			'tipoAnotacion'+':'+comboTipoTarea.getValue()+';'+
			'soloAsuntosEnvioCorreo'+':'+checkEnvioCorreo.getValue()+';'+
			'destinatarioEmail'+':'+destinatarioMail.getValue()
	...
	var panel=new Ext.Panel({
		...
		,items: [usuarioOrigenTarea, usuarioDestinoTarea,comboTipoTarea, checkEnvioCorreo, destinatarioMail]
		,listeners:{	
