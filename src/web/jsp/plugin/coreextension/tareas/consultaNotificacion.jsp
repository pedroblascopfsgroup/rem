...
<fwk:page>
	...
	var labeltareaOriginal=new Ext.form.TextArea({
        ...
        ,width:250
        ,readOnly:true              
        ,value:'<s:message text="${valores.textoComunicacion}" javaScriptEscape="true" />'
        ,style: "color: gray;"
    });
	var tipoEntidad='${codigoTipoEntidad}';
	var descEstado = '${situacion}';
	var fechaCrear = '${fecha}';
	var requiereRespuesta = '${valores.requiereRespuesta}';
	var emisorTarea = '${valores.emisor}';
	var finalizada = '${valores.finalizada}';
	var usuLogeado  =app.usuarioLogado.username;
	var fuerzaChkLeida = '${valores.fuerzaChkLeida}';
	
	//Variable para hacer la notificacion solo de consulta
	
	var readOnly=true;
	if(requiereRespuesta == 'false' && emisorTarea != usuLogeado && finalizada == 'false'){
		readOnly = false;
	}
	if(requiereRespuesta == 'true' && emisorTarea == usuLogeado && finalizada == 'false'){
		readOnly = false;
	}
	if(fuerzaChkLeida == 'true' && finalizada == 'false') {
		readOnly = false;
	}
	...
	if('${idTareaAsociada }' != ''){
		...
		var descripcion = new Ext.form.TextArea({
			...
			,labelStyle:"font-weight:bolder"
			,name : 'descripcion'
			,value:'<s:message text="${valores.respuestaComunicacion}" javaScriptEscape="true" />'
