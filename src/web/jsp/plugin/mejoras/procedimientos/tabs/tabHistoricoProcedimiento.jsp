...
constantes.TIPO_ENTIDAD_RECURSO_REVISADO = '15';
	...
	var funcionConsultar = function(){
		...
		{	
			page.webflow({
				...
				{ 
					...
					titulo = '${procedimiento.tipoProcedimiento.descripcion}' +' - ' + rec.get('tarea');
					webflow = 'generico/genericForm';
					width = null;		

					parametros = {idTareaExterna:recDatos['idTareaExterna'], readOnly:'true', idProcedimiento:${procedimiento.id}};
		...
		else if(tipoEntidad == constantes.TIPO_ENTIDAD_PETICION_RECURSO || tipoEntidad == constantes.TIPO_ENTIDAD_RESPUESTA_RECURSO)
		{
			page.webflow({
				...
				{ 
					...
					parametros = {idProcedimiento:'${procedimiento.id}',id: recDatos['idRecurso']};
		...
		<%-- 
		else if(tipoEntidad == constantes.TIPO_ENTIDAD_RECURSO_REVISADO )
		{
			var allowClose= false;
			var w = app.openWindow({
				flow: 'consultahistorico/consultarHistoricoRecursoRevisado'
				,closable: allowClose
				,width : 700
				,title :  '<s:message code="plugin.mejoras.procedimientos.tabTareas.recursoRevisado" text="**Revisión de recurso" />'
				,params: {id:idEntidad}
			});
			w.on(app.event.DONE, function(){
				w.close();
		...
		else if(tipoEntidad == constantes.TIPO_ENTIDAD_PETICION_DECISION || tipoEntidad == constantes.TIPO_ENTIDAD_RESPUESTA_DECISION)
		{
			...
			parametros = {idProcedimiento:${procedimiento.id}, id:idEntidad, readOnly:'true'};
