...
(function(page,entidad){
	...
	entidad.cacheStore(contratoBienesStore);
 	...
 	var contratoBienesCM  = new Ext.grid.ColumnModel([
        ...
        {header: '<s:message code="plugin.nuevoModeloBienes.contrato.columnaDescripcionBien" text="**Descripcón"/>', sortable: true, dataIndex: 'detalle'},
        ...
        {header: '<s:message code="plugin.nuevoModeloBienes.contrato.columnaSuperficieBien" text="**Superficie"/>', renderer: function (pnumber) {if (pnumber=="---"){return "";} if(pnumber!=null && pnumber!="") {var result = app.format.formatNumber(pnumber,2); return String.format("{0} m<sup>2</sup>",result);}return null;}, sortable: true, dataIndex: 'superficie', align:'right', width:60},
		...
		{header: '<s:message code="plugin.nuevoModeloBienes.contrato.columnaParticipacionBien" text="**Participacion"/>', dataIndex: 'participacion', renderer: function (val){if (val=="---"){return "";} var result = app.format.formatNumber(val,2);return String.format("{0} %",result);}, align:'right',width:60},
	...
	entidad.cacheStore(contratoSolvenciasStore);    
 	...
 	var contratoSolvenciasCM  = new Ext.grid.ColumnModel([
        ...
        ,{header: '<s:message code="plugin.nuevoModeloBienes.contrato.solvenciaColumnaPropiedad" text="**% Propiedad"/>', dataIndex: 'propiedad', renderer: function (val){if (val=="---"){return "";} var result = app.format.formatNumber(val,2);return String.format("{0} %",result);}, align:'right', width:60}
        ...
        ,{header: '<s:message code="plugin.nuevoModeloBienes.contrato.solvenciaColumnaSuperficie" text="**Superficie"/>', hidden:true, renderer: function (pnumber) {if (pnumber=="---"){return "";} if(pnumber!=null && pnumber!="") {var result = app.format.formatNumber(pnumber,2); return String.format("{0} m<sup>2</sup>",result);}return null;}, sortable: true, dataIndex: 'superficie', align:'right', width:60}
