...
--delete from TFI_TAREAS_FORM_ITEMS where TAP_ID IN (select TAP_ID from TAP_TAREA_PROCEDIMIENTO where DD_TPO_ID = (select DD_TPO_ID from DD_TPO_TIPO_PROCEDIMIENTO where DD_TPO_CODIGO='P08')); 
--delete from DD_PTP_PLAZOS_TAREAS_PLAZAS where TAP_ID IN (select TAP_ID from TAP_TAREA_PROCEDIMIENTO where DD_TPO_ID = (select DD_TPO_ID from DD_TPO_TIPO_PROCEDIMIENTO where DD_TPO_CODIGO='P08'));
--delete from TAP_TAREA_PROCEDIMIENTO where DD_TPO_ID = (select DD_TPO_ID from DD_TPO_TIPO_PROCEDIMIENTO where DD_TPO_CODIGO='P08');
--delete from DD_TPO_TIPO_PROCEDIMIENTO where DD_TPO_CODIGO = 'P08';
...
--Values(S_DD_TPO_TIPO_PROCEDIMIENTO.nextval, 'P08', 'Trámite de certificación de cargas y revisión', 'Trámite de certificación de cargas y revisión', '<h1>FIXME</h1>Texto descripcion tarea... ', 'tramiteCertificacionCargasRevision', 0, 'DD', SYSDATE, 0);
...
'((valores[''P08_RegistrarCertificacion''][''comboResultado''] == DDSiNo.SI) &&'||' (valores[''P08_RegistrarCertificacion''][''fecha''] == ''''))?''tareaExterna.error.P08_RegistrarCertificacion.fechaOblgatoria'':null', 0, 'DD', SYSDATE, 0);
...
-- Pantalla 7
Insert into TAP_TAREA_PROCEDIMIENTO(TAP_ID, DD_TPO_ID, TAP_CODIGO, TAP_SUPERVISOR, TAP_DESCRIPCION, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
Values(S_TAP_TAREA_PROCEDIMIENTO.nextval, (select DD_TPO_ID from DD_TPO_TIPO_PROCEDIMIENTO where DD_TPO_CODIGO='P08'), 'P08_RequerirMasInfo', 0, 'Requerir información que falta', 0, 'DD', SYSDATE, 0);

Insert into DD_PTP_PLAZOS_TAREAS_PLAZAS(DD_PTP_ID, TAP_ID, DD_PTP_PLAZO_SCRIPT, VERSION, USUARIOCREAR, FECHACREAR, BORRADO)
Values(S_DD_PTP_PLAZOS_TAREAS_PLAZAS.nextval, (select TAP_ID from TAP_TAREA_PROCEDIMIENTO where TAP_CODIGO='P08_RequerirMasInfo'), '5*24*60*60*1000L', 0, 'DD', SYSDATE, 0);
